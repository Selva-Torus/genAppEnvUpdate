...
export default function PageViewperformancecycleV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function addperformancecyclemodify_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewperformancecycle_v1Props.length > 0){
          for(let i=0;i< viewperformancecycle_v1Props.length;i++){
            if(viewperformancecycle_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1"){
              // delete viewperformancecycle_v1Props[i].DFDkey;
              let temp=structuredClone(viewperformancecycle_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function reviewfrequencycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewperformancecycle_v1Props.length > 0){
          for(let i=0;i< viewperformancecycle_v1Props.length;i++){
            if(viewperformancecycle_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:reviewFrequencyCombo:AFVK:v1"){
              // delete viewperformancecycle_v1Props[i].DFDkey;
              let temp=structuredClone(viewperformancecycle_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function cycletypecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewperformancecycle_v1Props.length > 0){
          for(let i=0;i< viewperformancecycle_v1Props.length;i++){
            if(viewperformancecycle_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:cycleTypeCombo:AFVK:v1"){
              // delete viewperformancecycle_v1Props[i].DFDkey;
              let temp=structuredClone(viewperformancecycle_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setviewperformancecycle_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setviewperformancecycle_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(viewperformancecycle_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(viewperformancecycle_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setviewperformancecycle_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
