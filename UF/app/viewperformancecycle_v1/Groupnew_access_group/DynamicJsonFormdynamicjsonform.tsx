...
const DynamicJsonFormdynamicjsonform = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1|027910746c6343f7b637e85490c0c120|properties.notification_schedule_json"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewPerformanceCycle:AFVK:v1|8db8e944155ef151489bacd8e1dc1763|2b78bc903d75fe2c9b1dd7bafb581ed5"
    }
  ],
  "schemaData": {
    "type": "object",
    "properties": {
      "review_start_notification": {
        "type": "string"
      },
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'notification_schedule_json',type:"text"})
        ...
        "items": [
          {
            ...
            "schema": {
              "name": "schema",
              "_label": "Schema",
              "value": "CK:CT006:FNGK:AF:FNK:DF-DST:CATK:ECP:AFGK:HRM:AFK:notificationSchedulejson:AFVK:v1:NDP",
  ...
  const fetchSchema = async (goruleData:any={},groupData:any={}) => {
    try {
      ...
      if (data) {
        ...
        if (!new_access_groupc1763?.notification_schedule_json) {
  ...
  const handleChange = async(values: any) => {
    ...
    //setValidate((pre:any)=>({...pre,notification_schedule_json:{}}))
      setnew_access_groupc1763((prev: any) => ({ ...prev, notification_schedule_json: processedValues }))
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(
      ...
      "2b78bc903d75fe2c9b1dd7bafb581ed5"
    );
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'notification_schedule_json',type:'text'}
        type={
          name:'notification_schedule_json',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.notification_schedule_json.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.notification_schedule_json.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.notification_schedule_json.type
        }
        ...
        type={
          name:'notification_schedule_json',
          type: orchestrationData?.data?.schemaData[0].schema.properties.notification_schedule_json.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.notification_schedule_json.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.notification_schedule_json.type
        }
        setDynamicStateandType(type)
      }
      }
...
const [dynamicDFDData,setDynamicDFDData]=useState<any>({})
  ...
  if (dynamicjsonform81ed5?.isHidden) {
    return <></>
  }
   if (isLoading) {
    return  <div style={{gridColumn: `1 / 13`,gridRow: `37 / 104`, gap:``, height: `100%`, overflow: 'auto'}} >
      Loading schema...</div>
  }
  ...
  return (   
    <div  
      style={{gridColumn: `1 / 13`,gridRow: `37 / 104`, gap:``, height: `100%`, overflow: 'auto'}} >
        ...
        onValidationChange={(isValid) => {
          setValidate((prev: any) => ({
            ...
            viewPerformanceCycle_v1:{
              ...
              notification_schedule_json: isValid ? {} : 'invalid'
            }
          }));
        }}
        values={new_access_groupc1763?.notification_schedule_json}
...
export default DynamicJsonFormdynamicjsonform
