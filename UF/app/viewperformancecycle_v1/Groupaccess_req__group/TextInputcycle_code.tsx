...
const TextInputcycle_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewPerformanceCycle:AFVK:v1|e0ac73943166d4455f166c0a8a870ea9|b410aefd870b4d018996a42f0fb7e30e"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewPerformanceCycle_v1:{...pre?.viewPerformanceCycle_v1,cycle_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group70ea9((prev: any) => ({ ...prev, cycle_code: +e.target.value }));
    }
    else{
    setaccess_req__group70ea9((prev: any) => ({ ...prev, cycle_code: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b410aefd870b4d018996a42f0fb7e30e"
      );
      ...
      //     controlId: "b410aefd870b4d018996a42f0fb7e30e",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b410aefd870b4d018996a42f0fb7e30e") {
        handleChange({target:{value:access_req__group70ea9Ref?.current?.cycle_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b410aefd870b4d018996a42f0fb7e30e") {
        handleBlur({target:{value:access_req__group70ea9Ref?.current?.cycle_code||""}});
      }
    };
  ...
  if(dfd_addperformancecyclemodify_v1Props?.setSearchFilters && dfd_addperformancecyclemodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addperformancecyclemodify_v1Props.data) && dfd_addperformancecyclemodify_v1Props.data.length > 0){
      setaccess_req__group70ea9((pre:any)=>({...pre,cycle_code:dfd_addperformancecyclemodify_v1Props.data[0]?.cycle_code}));
    }
  }
  },[dfd_addperformancecyclemodify_v1Props?.setSearchFilters])
  if (cycle_code7e30e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group70ea9?.cycle_code||""}
         disabled= {cycle_code7e30e?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.viewPerformanceCycle_v1?.cycle_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
