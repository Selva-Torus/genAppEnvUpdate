...
const TextInputcycle_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewPerformanceCycle:AFVK:v1|e0ac73943166d4455f166c0a8a870ea9|324b0c29f8208e4d41156f7c6659f4c1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewPerformanceCycle_v1:{...pre?.viewPerformanceCycle_v1,cycle_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group70ea9((prev: any) => ({ ...prev, cycle_name: +e.target.value }));
    }
    else{
    setaccess_req__group70ea9((prev: any) => ({ ...prev, cycle_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "324b0c29f8208e4d41156f7c6659f4c1"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewPerformanceCycle:AFVK:v1",
      //     componentId: "e0ac73943166d4455f166c0a8a870ea9",
      //     controlId: "324b0c29f8208e4d41156f7c6659f4c1",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "324b0c29f8208e4d41156f7c6659f4c1") {
        handleChange({target:{value:access_req__group70ea9Ref?.current?.cycle_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "324b0c29f8208e4d41156f7c6659f4c1") {
        handleBlur({target:{value:access_req__group70ea9Ref?.current?.cycle_name||""}});
      }
    };
  ...
  if(dfd_addperformancecyclemodify_v1Props?.setSearchFilters && dfd_addperformancecyclemodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addperformancecyclemodify_v1Props.data) && dfd_addperformancecyclemodify_v1Props.data.length > 0){
      setaccess_req__group70ea9((pre:any)=>({...pre,cycle_name:dfd_addperformancecyclemodify_v1Props.data[0]?.cycle_name}));
    }
  }
  },[dfd_addperformancecyclemodify_v1Props?.setSearchFilters])
  if (cycle_name9f4c1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group70ea9?.cycle_name||""}
         disabled= {cycle_name9f4c1?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.viewPerformanceCycle_v1?.cycle_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
