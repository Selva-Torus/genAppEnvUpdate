...
const Checkboxmanager_review_required_checkbox = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, string>>({name:'manager_review_required',type:"text"});
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "c518760316de4e51587d6d7c60e1f25f"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[manager_review_required_checkbox1f25f?.refresh])

  useEffect(()=>{
    if (business_just__group2db99?.manager_review_required === undefined) {
      setbusiness_just__group2db99((prev: any) => ({ ...prev, manager_review_required: true }));
    }
  },[])
  ...
  const handleChange=async(checked:boolean)=>{
    ...
    setbusiness_just__group2db99((prev: any) => ({ ...prev, manager_review_required: checked}));
    }catch (err: any) {
  ...
  if (manager_review_required_checkbox1f25f?.isHidden) {
    return <></>;
  }
  return (
    <div 
       style={{gridColumn: `12 / 22`,gridRow: `10 / 16`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Checkbox 
      className=""
      value={business_just__group2db99?.manager_review_required||false}
      checked={business_just__group2db99?.manager_review_required||true}
      disabled= {manager_review_required_checkbox1f25f?.isDisabled ? true : false}
      content = {'Manager Review Required'}
...
export default Checkboxmanager_review_required_checkbox;
