...
const DatePickerstart_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,viewPerformanceCycle_v1:{...pre?.viewPerformanceCycle_v1,start_date: "invalid"}}))
    setvalid_group35ad5((prev: any) => ({ ...prev, start_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,viewPerformanceCycle_v1:{...pre?.viewPerformanceCycle_v1,start_date:undefined}}));
  if (!date) {
    setvalid_group35ad5((prev: any) => ({ ...prev, start_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_group35ad5((prev: any) => ({ ...prev, start_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_group35ad5?.start_date == "" || valid_group35ad5?.start_date == undefined){
        const result = v.safeParse(schema, valid_group35ad5?.start_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "269631fba82078d6fc7ac92779cd9832"
      );
...
useEffect(()=>{
 ...
 },[start_dated9832?.refresh])

useEffect(()=>{
  if(!valid_group35ad5?.start_date){ 
...
if (start_dated9832?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_group35ad5?.start_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {start_dated9832?.isDisabled ? true : false}
      disabled= {start_dated9832?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Start Date"
      dateValidation=""
      validationState={validate?.viewPerformanceCycle_v1?.start_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerstart_date
