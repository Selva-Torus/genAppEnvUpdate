...
import Textemergency_contact  from "./Textemergency_contact";
import TextInputemergency_contact_name  from "./TextInputemergency_contact_name";
import TextInputemergency_contact_phone  from "./TextInputemergency_contact_phone";
...
const Groupemergency_contact_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "384e62265f854f7192ef19ef6e773988");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemergency_contact_group73988Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("emergency_contact")){
        setemergency_contact67cf5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact67cf5?.isDisabled==null)
      {
        setemergency_contact67cf5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emergency_contact_name")){
        setemergency_contact_name69e51((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact_name69e51?.isDisabled==null)
      {
        setemergency_contact_name69e51((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emergency_contact_phone")){
        setemergency_contact_phonea60bb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact_phonea60bb?.isDisabled==null)
      {
        setemergency_contact_phonea60bb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("relationship")){
        setrelationship2e465((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(relationship2e465?.isDisabled==null)
      {
        setrelationship2e465((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const emergency_contact_group73988Ref = useRef<any>(null);
  const handleClearSearch = () => {
    emergency_contact_group73988Ref.current?.setSearchParams();
    emergency_contact_group73988Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(emergency_contact_group73988) && Object.keys(emergency_contact_group73988)?.length>0)
      {
        setemergency_contact_group73988({})
      }
    }else 
      prevRefreshRef.current= true
  }, [emergency_contact_group73988Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"emergency_contact_group"}))
        }}
    >
          {allowedControls.includes("emergency_contact") ?<Textemergency_contact   /* 67cf5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("emergency_contact_name") ?<TextInputemergency_contact_name   /* 69e51 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("emergency_contact_phone") ?<TextInputemergency_contact_phone   /* a60bb */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("relationship") ?<Dropdownrelationship   /* 2e465 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}

export default Groupemergency_contact_group
