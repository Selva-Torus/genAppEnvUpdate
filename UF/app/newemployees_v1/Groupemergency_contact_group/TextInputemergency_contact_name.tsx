...
const TextInputemergency_contact_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.emergency_contact_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|384e62265f854f7192ef19ef6e773988|42373fe932634b7684e7582aa0f69e51"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'emergency_contact_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emergency_contact_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setemergency_contact_group73988((prev: any) => ({ ...prev, emergency_contact_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(emergency_contact_group73988?.emergency_contact_name == "" || emergency_contact_group73988?.emergency_contact_name == undefined){
      emergency_contact_group73988.emergency_contact_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, emergency_contact_group73988?.emergency_contact_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emergency_contact_name:"invalid"}}));
        }
    }else if(emergency_contact_group73988?.emergency_contact_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +emergency_contact_group73988?.emergency_contact_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "42373fe932634b7684e7582aa0f69e51"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1",
      //     componentId: "384e62265f854f7192ef19ef6e773988",
      //     controlId: "42373fe932634b7684e7582aa0f69e51",
      //     isTable: false,
      //     from:"TextInputemergency_contact_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'emergency_contact_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'emergency_contact_name',type:'text'};
      //   type={
      //     name:'emergency_contact_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emergency_contact_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emergency_contact_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emergency_contact_name.type
      //   }
      ...
      //   type={
      //     name:'emergency_contact_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!emergency_contact_group73988?.emergency_contact_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "42373fe932634b7684e7582aa0f69e51") {
        handleChange({target:{value:emergency_contact_group73988Ref?.current?.emergency_contact_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "42373fe932634b7684e7582aa0f69e51") {
        handleBlur({target:{value:emergency_contact_group73988Ref?.current?.emergency_contact_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setemergency_contact_group73988((pre:any)=>({...pre,emergency_contact_name:dfd_employees_v1Props.data[0]?.emergency_contact_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (emergency_contact_name69e51?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={emergency_contact_group73988?.emergency_contact_name||""}
         disabled= {emergency_contact_name69e51?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.emergency_contact_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputemergency_contact_name
