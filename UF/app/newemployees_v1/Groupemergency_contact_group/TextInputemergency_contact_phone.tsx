...
const TextInputemergency_contact_phone = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.emergency_contact_phone"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|384e62265f854f7192ef19ef6e773988|224d4d295a55474e85f9df60eb3a60bb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'emergency_contact_phone',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emergency_contact_phone:undefined}}));
    if(dynamicStateandType.type=="number"){
    setemergency_contact_group73988((prev: any) => ({ ...prev, emergency_contact_phone: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(emergency_contact_group73988?.emergency_contact_phone == "" || emergency_contact_group73988?.emergency_contact_phone == undefined){
      emergency_contact_group73988.emergency_contact_phone = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, emergency_contact_group73988?.emergency_contact_phone);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emergency_contact_phone:"invalid"}}));
        }
    }else if(emergency_contact_group73988?.emergency_contact_phone !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +emergency_contact_group73988?.emergency_contact_phone);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "224d4d295a55474e85f9df60eb3a60bb"
      );
      ...
      //     controlId: "224d4d295a55474e85f9df60eb3a60bb",
      //     isTable: false,
      //     from:"TextInputemergency_contact_phone",
      ...
      //   type={
      //     name:'emergency_contact_phone',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_phone.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_phone.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emergency_contact_phone.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!emergency_contact_group73988?.emergency_contact_phone)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "224d4d295a55474e85f9df60eb3a60bb") {
        handleChange({target:{value:emergency_contact_group73988Ref?.current?.emergency_contact_phone||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "224d4d295a55474e85f9df60eb3a60bb") {
        handleBlur({target:{value:emergency_contact_group73988Ref?.current?.emergency_contact_phone||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setemergency_contact_group73988((pre:any)=>({...pre,emergency_contact_phone:dfd_employees_v1Props.data[0]?.emergency_contact_phone}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (emergency_contact_phonea60bb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={emergency_contact_group73988?.emergency_contact_phone||""}
         disabled= {emergency_contact_phonea60bb?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. 6374745263'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Emergency Contact Phone"
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.emergency_contact_phone ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputemergency_contact_phone
