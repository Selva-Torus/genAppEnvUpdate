...
import Groupskill  from "../Groupskill/Groupskill";
...
import Iconskills_icon  from "./Iconskills_icon";
import Textskill_text  from "./Textskill_text";
import Textskill_des  from "./Textskill_des";
...
const Groupskills_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {skills_icond2e0a, setskills_icond2e0a}= useContext(TotalContext) as TotalContextProps;
  const {skill_text7b287, setskill_text7b287}= useContext(TotalContext) as TotalContextProps;
  const {skill_des79cc6, setskill_des79cc6}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "10b70b37a5d949af87cb7081fa436679");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setskills_group36679Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("skills_icon")){
        setskills_icond2e0a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_icond2e0a?.isDisabled==null)
      {
        setskills_icond2e0a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill_text")){
        setskill_text7b287((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skill_text7b287?.isDisabled==null)
      {
        setskill_text7b287((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill_des")){
        setskill_des79cc6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skill_des79cc6?.isDisabled==null)
      {
        setskill_des79cc6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill")){
        setskill9f89a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skill9f89a?.isDisabled==null)
      {
        setskill9f89a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['skills_icon'] = skills_icond2e0a,
        codeStates['setskills_icon'] = setskills_icond2e0a,
        codeStates['skill_text'] = skill_text7b287,
        codeStates['setskill_text'] = setskill_text7b287,
        codeStates['skill_des'] = skill_des79cc6,
        codeStates['setskill_des'] = setskill_des79cc6,
  ...
  const skills_group36679Ref = useRef<any>(null);
  const handleClearSearch = () => {
    skills_group36679Ref.current?.setSearchParams();
    skills_group36679Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(skills_group36679) && Object.keys(skills_group36679)?.length>0)
      {
        setskills_group36679({})
      }
    }else 
      prevRefreshRef.current= true
  }, [skills_group36679Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"skills_group"}))
        }}
    >
        {allowedComponent.includes("skill")  &&<Groupskill  
        ...
        {allowedControls.includes("skills_icon")?<Iconskills_icon /* d2e0a */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("skill_text") ?<Textskill_text   /* 7b287 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("skill_des") ?<Textskill_des   /* 79cc6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupskills_group
