...
const TextInputinstitution_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|5f0c193ec40a41e8af544856fd52393a|bdcce6cba9a64e399fe47102eecd7544"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,institution_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    seteducation2393a_11((prev: any) => ({ ...prev, institution_name: +e.target.value }));
    }
    else{
    seteducation2393a_11((prev: any) => ({ ...prev, institution_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "bdcce6cba9a64e399fe47102eecd7544"
      );
      ...
      //     controlId: "bdcce6cba9a64e399fe47102eecd7544",
      //     isTable: false,
  ...
  const education2393a_11Ref = useRef<any>(education2393a_11);
  useEffect(() => { education2393a_11Ref.current = education2393a_11; }, [education2393a_11]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "bdcce6cba9a64e399fe47102eecd7544") {
        handleChange({target:{value:education2393a_11Ref?.current?.institution_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "bdcce6cba9a64e399fe47102eecd7544") {
        handleBlur({target:{value:education2393a_11Ref?.current?.institution_name||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation2393a_11((pre:any)=>({...pre,institution_name:dfd_educationjson_v1Props.data[0]?.institution_name}));
    }
  }
  },[dfd_educationjson_v1Props?.setSearchFilters])
  if (institution_named7544?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={education2393a_11?.institution_name||""}
         disabled= {institution_named7544?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.institution_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
