...
const TextInputoccupation = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|6611a8f7add7494c88bbd137d53b06ea|248efa5bf9c945b7a0772dddadf36ff7"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,occupation:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfamly_detailsb06ea_10((prev: any) => ({ ...prev, occupation: +e.target.value }));
    }
    else{
    setfamly_detailsb06ea_10((prev: any) => ({ ...prev, occupation: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "248efa5bf9c945b7a0772dddadf36ff7"
      );
      ...
      //     controlId: "248efa5bf9c945b7a0772dddadf36ff7",
      //     isTable: false,
  ...
  const famly_detailsb06ea_10Ref = useRef<any>(famly_detailsb06ea_10);
  useEffect(() => { famly_detailsb06ea_10Ref.current = famly_detailsb06ea_10; }, [famly_detailsb06ea_10]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "248efa5bf9c945b7a0772dddadf36ff7") {
        handleChange({target:{value:famly_detailsb06ea_10Ref?.current?.occupation||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "248efa5bf9c945b7a0772dddadf36ff7") {
        handleBlur({target:{value:famly_detailsb06ea_10Ref?.current?.occupation||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb06ea_10((pre:any)=>({...pre,occupation:dfd_familyjson_v1Props.data[0]?.occupation}));
    }
  }
  },[dfd_familyjson_v1Props?.setSearchFilters])
  if (occupation36ff7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb06ea_10?.occupation||""}
         disabled= {occupation36ff7?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.occupation ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
