...
const TextInputproficiency_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setskill9f89a_10((prev: any) => ({ ...prev, proficiency_level: +e.target.value }));
    }
    else{
    setskill9f89a_10((prev: any) => ({ ...prev, proficiency_level: e.target.value }));
    }
  ...
  const skill9f89a_10Ref = useRef<any>(skill9f89a_10);
  useEffect(() => { skill9f89a_10Ref.current = skill9f89a_10; }, [skill9f89a_10]);
  useEffect(()=>{
      handleMapperValue();
    ...
    const handlerChange = (id:any) => {
      if (id === "ba1e68e9a10f4eeba589207ba0dfea56") {
        handleChange({target:{value:skill9f89a_10Ref?.current?.proficiency_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ba1e68e9a10f4eeba589207ba0dfea56") {
        handleBlur({target:{value:skill9f89a_10Ref?.current?.proficiency_level||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskill9f89a_10((pre:any)=>({...pre,proficiency_level:dfd_skilljson_v1Props.data[0]?.proficiency_level}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skill9f89a_10?.proficiency_level||""}
