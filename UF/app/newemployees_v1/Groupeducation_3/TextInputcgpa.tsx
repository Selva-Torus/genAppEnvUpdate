...
const TextInputcgpa = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    seteducation2393a_3((prev: any) => ({ ...prev, grade_cgpa_percentage: +e.target.value }));
    }
    else{
    seteducation2393a_3((prev: any) => ({ ...prev, grade_cgpa_percentage: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "96c7bb0b850a440ca0184c8f4a9db57b") {
        handleChange({target:{value:education2393a_3Ref?.current?.grade_cgpa_percentage||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "96c7bb0b850a440ca0184c8f4a9db57b") {
        handleBlur({target:{value:education2393a_3Ref?.current?.grade_cgpa_percentage||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation2393a_3((pre:any)=>({...pre,grade_cgpa_percentage:dfd_educationjson_v1Props.data[0]?.grade_cgpa_percentage}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={education2393a_3?.grade_cgpa_percentage||""}
