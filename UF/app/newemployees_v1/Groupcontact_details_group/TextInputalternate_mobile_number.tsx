...
const TextInputalternate_mobile_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.alternate_mobile"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|c0fc253df5b343d9b59a2e761d6a1911|12878213b41945378752e037d73aaeb3"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'alternate_mobile',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,alternate_mobile:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_groupa1911((prev: any) => ({ ...prev, alternate_mobile: +e.target.value }));
    }
    else{
    setcontact_details_groupa1911((prev: any) => ({ ...prev, alternate_mobile: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "12878213b41945378752e037d73aaeb3"
      );
      ...
      //     controlId: "12878213b41945378752e037d73aaeb3",
      //     isTable: false,
      //     from:"TextInputalternate_mobile_number",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'alternate_mobile', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'alternate_mobile',type:'text'};
      //   type={
      //     name:'alternate_mobile',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.alternate_mobile.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.alternate_mobile.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.alternate_mobile.type
      //   }
      ...
      //   type={
      //     name:'alternate_mobile',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.alternate_mobile.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.alternate_mobile.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.alternate_mobile.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "12878213b41945378752e037d73aaeb3") {
        handleChange({target:{value:contact_details_groupa1911Ref?.current?.alternate_mobile||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "12878213b41945378752e037d73aaeb3") {
        handleBlur({target:{value:contact_details_groupa1911Ref?.current?.alternate_mobile||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_groupa1911((pre:any)=>({...pre,alternate_mobile:dfd_employees_v1Props.data[0]?.alternate_mobile}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (alternate_mobile_numberaaeb3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 13`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={contact_details_groupa1911?.alternate_mobile||""}
         disabled= {alternate_mobile_numberaaeb3?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. 6374743452'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Alternate Mobile Number"
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.alternate_mobile ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputalternate_mobile_number
