...
const TextInputlinkedin_profile = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|c0fc253df5b343d9b59a2e761d6a1911|a9a2b12222fe4f3eb203980b255507ec"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,linkedin_profile:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_groupa1911((prev: any) => ({ ...prev, linkedin_profile: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_groupa1911?.linkedin_profile == "" || contact_details_groupa1911?.linkedin_profile == undefined){
      contact_details_groupa1911.linkedin_profile = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_groupa1911?.linkedin_profile);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,linkedin_profile:"invalid"}}));
        }
    }else if(contact_details_groupa1911?.linkedin_profile !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_groupa1911?.linkedin_profile);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a9a2b12222fe4f3eb203980b255507ec"
      );
      ...
      //     controlId: "a9a2b12222fe4f3eb203980b255507ec",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_groupa1911?.linkedin_profile)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "a9a2b12222fe4f3eb203980b255507ec") {
        handleChange({target:{value:contact_details_groupa1911Ref?.current?.linkedin_profile||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a9a2b12222fe4f3eb203980b255507ec") {
        handleBlur({target:{value:contact_details_groupa1911Ref?.current?.linkedin_profile||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_groupa1911((pre:any)=>({...pre,linkedin_profile:dfd_employees_v1Props.data[0]?.linkedin_profile}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (linkedin_profile507ec?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_groupa1911?.linkedin_profile||""}
         disabled= {linkedin_profile507ec?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.linkedin_profile ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
