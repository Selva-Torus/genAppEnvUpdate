...
const TextInputmobile_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.mobile"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|c0fc253df5b343d9b59a2e761d6a1911|e9d15e34499e4e3c9a065d41f491ed75"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'mobile',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,mobile:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_groupa1911((prev: any) => ({ ...prev, mobile: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_groupa1911?.mobile == "" || contact_details_groupa1911?.mobile == undefined){
      contact_details_groupa1911.mobile = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_groupa1911?.mobile);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,mobile:"invalid"}}));
        }
    }else if(contact_details_groupa1911?.mobile !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_groupa1911?.mobile);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e9d15e34499e4e3c9a065d41f491ed75"
      );
      ...
      //     controlId: "e9d15e34499e4e3c9a065d41f491ed75",
      //     isTable: false,
      //     from:"TextInputmobile_number",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'mobile', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'mobile',type:'text'};
      //   type={
      //     name:'mobile',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.mobile.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.mobile.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.mobile.type
      //   }
      ...
      //   type={
      //     name:'mobile',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.mobile.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.mobile.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.mobile.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_groupa1911?.mobile)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "e9d15e34499e4e3c9a065d41f491ed75") {
        handleChange({target:{value:contact_details_groupa1911Ref?.current?.mobile||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e9d15e34499e4e3c9a065d41f491ed75") {
        handleBlur({target:{value:contact_details_groupa1911Ref?.current?.mobile||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_groupa1911((pre:any)=>({...pre,mobile:dfd_employees_v1Props.data[0]?.mobile}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (mobile_number1ed75?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={contact_details_groupa1911?.mobile||""}
         disabled= {mobile_number1ed75?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. 9486365542'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Mobile Number"
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.mobile ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmobile_number
