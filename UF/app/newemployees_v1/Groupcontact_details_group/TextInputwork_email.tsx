...
const TextInputwork_email = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|c0fc253df5b343d9b59a2e761d6a1911|d11ce5169b0b4a90b754e6c1672841fe"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_email:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_groupa1911((prev: any) => ({ ...prev, work_email: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_groupa1911?.work_email == "" || contact_details_groupa1911?.work_email == undefined){
      contact_details_groupa1911.work_email = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_groupa1911?.work_email);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_email:"invalid"}}));
        }
    }else if(contact_details_groupa1911?.work_email !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_groupa1911?.work_email);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d11ce5169b0b4a90b754e6c1672841fe"
      );
      ...
      //     controlId: "d11ce5169b0b4a90b754e6c1672841fe",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_groupa1911?.work_email)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "d11ce5169b0b4a90b754e6c1672841fe") {
        handleChange({target:{value:contact_details_groupa1911Ref?.current?.work_email||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d11ce5169b0b4a90b754e6c1672841fe") {
        handleBlur({target:{value:contact_details_groupa1911Ref?.current?.work_email||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_groupa1911((pre:any)=>({...pre,work_email:dfd_employees_v1Props.data[0]?.work_email}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (work_email841fe?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_groupa1911?.work_email||""}
         disabled= {work_email841fe?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.work_email ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
