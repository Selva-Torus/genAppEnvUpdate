...
import Textcontact_details  from "./Textcontact_details";
import TextInputpersonal_email  from "./TextInputpersonal_email";
import TextInputwork_email  from "./TextInputwork_email";
import TextInputalternate_mobile_number  from "./TextInputalternate_mobile_number";
import TextInputmobile_number  from "./TextInputmobile_number";
import TextInputlinkedin_profile  from "./TextInputlinkedin_profile";
...
const Groupcontact_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c0fc253df5b343d9b59a2e761d6a1911");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcontact_details_groupa1911Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("contact_details")){
        setcontact_details450d3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(contact_details450d3?.isDisabled==null)
      {
        setcontact_details450d3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("personal_email")){
        setpersonal_email497eb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(personal_email497eb?.isDisabled==null)
      {
        setpersonal_email497eb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("work_email")){
        setwork_email841fe((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(work_email841fe?.isDisabled==null)
      {
        setwork_email841fe((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("alternate_mobile_number")){
        setalternate_mobile_numberaaeb3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(alternate_mobile_numberaaeb3?.isDisabled==null)
      {
        setalternate_mobile_numberaaeb3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("mobile_number")){
        setmobile_number1ed75((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(mobile_number1ed75?.isDisabled==null)
      {
        setmobile_number1ed75((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("linkedin_profile")){
        setlinkedin_profile507ec((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(linkedin_profile507ec?.isDisabled==null)
      {
        setlinkedin_profile507ec((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const contact_details_groupa1911Ref = useRef<any>(null);
  const handleClearSearch = () => {
    contact_details_groupa1911Ref.current?.setSearchParams();
    contact_details_groupa1911Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(contact_details_groupa1911) && Object.keys(contact_details_groupa1911)?.length>0)
      {
        setcontact_details_groupa1911({})
      }
    }else 
      prevRefreshRef.current= true
  }, [contact_details_groupa1911Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"contact_details_group"}))
        }}
    >
          {allowedControls.includes("contact_details") ?<Textcontact_details   /* 450d3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("personal_email") ?<TextInputpersonal_email   /* 497eb */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("work_email") ?<TextInputwork_email   /* 841fe */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("alternate_mobile_number") ?<TextInputalternate_mobile_number   /* aaeb3 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("mobile_number") ?<TextInputmobile_number   /* 1ed75 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("linkedin_profile") ?<TextInputlinkedin_profile   /* 507ec */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupcontact_details_group
