...
const TextInputpersonal_email = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.personal_email"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|c0fc253df5b343d9b59a2e761d6a1911|b3dcd6d30d3d40318f4616972bb497eb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'personal_email',type:"text"})
  ...
  const {contact_details450d3, setcontact_details450d3}= useContext(TotalContext) as TotalContextProps;
  const {personal_email497eb, setpersonal_email497eb}= useContext(TotalContext) as TotalContextProps;
  const {work_email841fe, setwork_email841fe}= useContext(TotalContext) as TotalContextProps;
  const {alternate_mobile_numberaaeb3, setalternate_mobile_numberaaeb3}= useContext(TotalContext) as TotalContextProps;
  const {mobile_number1ed75, setmobile_number1ed75}= useContext(TotalContext) as TotalContextProps;
  const {linkedin_profile507ec, setlinkedin_profile507ec}= useContext(TotalContext) as TotalContextProps;
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,personal_email:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_groupa1911((prev: any) => ({ ...prev, personal_email: +e.target.value }));
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['contact_details'] = contact_details450d3,
        codeStates['setcontact_details'] = setcontact_details450d3,
        codeStates['personal_email'] = personal_email497eb,
        codeStates['setpersonal_email'] = setpersonal_email497eb,
        codeStates['work_email'] = work_email841fe,
        codeStates['setwork_email'] = setwork_email841fe,
        codeStates['alternate_mobile_number'] = alternate_mobile_numberaaeb3,
        codeStates['setalternate_mobile_number'] = setalternate_mobile_numberaaeb3,
        codeStates['mobile_number'] = mobile_number1ed75,
        codeStates['setmobile_number'] = setmobile_number1ed75,
        codeStates['linkedin_profile'] = linkedin_profile507ec,
        codeStates['setlinkedin_profile'] = setlinkedin_profile507ec,
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_groupa1911?.personal_email == "" || contact_details_groupa1911?.personal_email == undefined){
      contact_details_groupa1911.personal_email = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_groupa1911?.personal_email);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,personal_email:"invalid"}}));
        }
    }else if(contact_details_groupa1911?.personal_email !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_groupa1911?.personal_email);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "c0fc253df5b343d9b59a2e761d6a1911",
        "b3dcd6d30d3d40318f4616972bb497eb"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1",
      //     componentId: "c0fc253df5b343d9b59a2e761d6a1911",
      //     controlId: "b3dcd6d30d3d40318f4616972bb497eb",
      //     isTable: false,
      //     from:"TextInputpersonal_email",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'personal_email', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'personal_email',type:'text'};
      //   type={
      //     name:'personal_email',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.personal_email.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.personal_email.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.personal_email.type
      //   }
      ...
      //   type={
      //     name:'personal_email',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.personal_email.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.personal_email.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.personal_email.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const contact_details_groupa1911Ref = useRef<any>(contact_details_groupa1911);
  useEffect(() => { contact_details_groupa1911Ref.current = contact_details_groupa1911; }, [contact_details_groupa1911]);
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_groupa1911?.personal_email)
      {
        setcontact_details_groupa1911Props((pre:any)=>({...pre,required:true}));
        setIsRequredData(true);
      }
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "b3dcd6d30d3d40318f4616972bb497eb") {
        handleChange({target:{value:contact_details_groupa1911Ref?.current?.personal_email||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b3dcd6d30d3d40318f4616972bb497eb") {
        handleBlur({target:{value:contact_details_groupa1911Ref?.current?.personal_email||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_groupa1911((pre:any)=>({...pre,personal_email:dfd_employees_v1Props.data[0]?.personal_email}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (personal_email497eb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_groupa1911?.personal_email||""}
         disabled= {personal_email497eb?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.personal_email ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpersonal_email
