 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "afff7ad784994fe2b99de869d5833d39"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in dynamicactionsa8358Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsa8358Props?.dynamicActionRule?.cancel,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in newemployees_v1?.dynamicactions && newemployees_v1?.dynamicactions["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(newemployees_v1?.dynamicactions["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newemployees_v1?.dynamicactions?.cancel?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "afff7ad784994fe2b99de869d5833d39") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "cancel33d39");
    return () => {
  ...
  useEffect(()=>{
    if (!cancel33d39?.trigger) return;
      if(cancel33d39?.trigger){
      setcancel33d39((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsa8358((prev: any) => ({ ...prev, cancel: true }));
        //onClick

    // showArtifact
    let filterProps2: any =  [];
      let filterData2 = await getFilterProps(filterProps2,{...new_employee_group42d78,...basic_details_groupd39fd,...contact_details_groupa1911,...address_details_groupb72f4,...identity_details_group6a6fe,...employment_details_group89cd6,...compensation_details_groupf9ef1,...bank_details820cd,...emergency_contact_group73988,...skills_education_groupcb53a,...skills_group36679,...skill9f89a,...education_group70757,...education2393a,...cert_groupedb63,...certificationc7d06,...family_detail_group800b7,...famly_detailsb06ea,...dynamicactionsa8358});
    ...
    routes.push(getRouteScreenDetails('CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1', 'employees_v1'));
      ...
      setIsProcessing(false);
        setdynamicactionsa8358((prev: any) => ({ ...prev, cancel: false }));
  ...
  }, [newemployees_v1?.dynamicactions?.cancel,dynamicactionsa8358Props?.dynamicActionRule?.cancel,])

 if (cancel33d39?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='normal'
          disabled= {cancel33d39?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
