...
const Buttonbutton_update = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {cancel33d39, setcancel33d39}= useContext(TotalContext) as TotalContextProps;
  ...
  const dynamicactionsa8358Ref = useRef(dynamicactionsa8358);
  useEffect(() => {
    dynamicactionsa8358Ref.current = dynamicactionsa8358;
  ...
  const dynamicactionsa8358PropsRef = useRef(dynamicactionsa8358Props);
  useEffect(() => {
    dynamicactionsa8358PropsRef.current = dynamicactionsa8358Props;
  }, [dynamicactionsa8358Props]);
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['cancel'] = cancel33d39,
        codeStates['setcancel'] = setcancel33d39,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...dynamicactionsa8358Ref.current};
      let parentRowSpan = 8;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "33f6e2420861444f9f9472b55f6a8358",
        "8bc69d6bf9f64385abdd380859ed6240"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    }else if(dynamicactionsa8358Props?.isHaveRule==true){
      if("button_update" in dynamicactionsa8358Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsa8358Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in newemployees_v1?.dynamicactions && newemployees_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(newemployees_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newemployees_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_updated6240") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "8bc69d6bf9f64385abdd380859ed6240") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_updated6240");
    return () => {
  ...
  useEffect(()=>{
    if (!button_updated6240?.trigger) return;
      if(button_updated6240?.trigger){
      setbutton_updated6240((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_updated6240?.trigger])

  useEffect(()=>{
    if(button_updated6240?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_updated6240?.refresh])
  ...
  async function handleSave6240_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const dynamicactionsa8358Props = dynamicactionsa8358PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.newEmployees_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(dynamicactionsa8358Ref.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addEmployeeModify:AFVK:v1|21f3396f59cf4f5e92de3623422fdb6a"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addEmployeeModify:AFVK:v1|21f3396f59cf4f5e92de3623422fdb6a"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactionsa8358?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "8bc69d6bf9f64385abdd380859ed6240",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "8bc69d6bf9f64385abdd380859ed6240.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "8bc69d6bf9f64385abdd380859ed6240.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "8bc69d6bf9f64385abdd380859ed6240.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "8bc69d6bf9f64385abdd380859ed6240.1.1.1.1.1",
                  ...
                  "children": [
                    {
                      "id": "267e1421a9f4452a8ebcf3c4183e694e.1.1.1.1.1.1",
                      "value": "",
                      "type": "screen",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addEmployeeModify:AFVK:v1|21f3396f59cf4f5e92de3623422fdb6a"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1";
      sourceId+= "|"+"33f6e2420861444f9f9472b55f6a8358";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1"+"|"+"33f6e2420861444f9f9472b55f6a8358"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactionsa8358?.upId? [dynamicactionsa8358?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactionsa8358?.upId? [dynamicactionsa8358?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactionsa8358?.upId? [dynamicactionsa8358?.upId ] : dynamicactionsa8358?.trs_process_id? [dynamicactionsa8358?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactionsa8358Ref.current, ...nullFilter(new_employee_group42d78), ...nullFilter(basic_details_groupd39fd), ...nullFilter(contact_details_groupa1911), ...nullFilter(address_details_groupb72f4), ...nullFilter(identity_details_group6a6fe), ...nullFilter(employment_details_group89cd6), ...nullFilter(compensation_details_groupf9ef1), ...nullFilter(bank_details820cd), ...nullFilter(emergency_contact_group73988), ...nullFilter(skills_education_groupcb53a), ...nullFilter(skills_group36679), ...nullFilter(skill9f89a), ...nullFilter(education_group70757), ...nullFilter(education2393a), ...nullFilter(cert_groupedb63), ...nullFilter(certificationc7d06), ...nullFilter(family_detail_group800b7), ...nullFilter(famly_detailsb06ea) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"33f6e2420861444f9f9472b55f6a8358",
              controlId:"8bc69d6bf9f64385abdd380859ed6240"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (dynamicactionsa8358Props.ssKey !== '' && dynamicactionsa8358Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = dynamicactionsa8358Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Details Updated Successfully', 'success',false);
    // refreshElement
    //riseListen
    // for group
    settotal_employees_tablee694eProps((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsa8358((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSave6240_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsa8358((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newemployees_v1?.dynamicactions?.button_update,dynamicactionsa8358Props?.dynamicActionRule?.button_update,])

 if (button_updated6240?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='flat-utility'
          disabled= {button_updated6240?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
