...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "33f6e2420861444f9f9472b55f6a8358");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactionsa8358Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel33d39((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel33d39?.isDisabled==null)
      {
        setcancel33d39((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_updated6240((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_updated6240?.isDisabled==null)
      {
        setbutton_updated6240((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("save")){
        setsavec5b7c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(savec5b7c?.isDisabled==null)
      {
        setsavec5b7c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactionsa8358Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactionsa8358Ref.current?.setSearchParams();
    dynamicactionsa8358Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactionsa8358Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactionsa8358Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactionsa8358) && Object.keys(dynamicactionsa8358)?.length>0)
      {
        setdynamicactionsa8358({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactionsa8358Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
