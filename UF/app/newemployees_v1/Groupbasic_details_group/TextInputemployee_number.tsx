...
const TextInputemployee_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.emp_number"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|619366ccec19457e956f09f17acd39fd|74f2e1be2f96474ea7d521fda975afe9"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'emp_number',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emp_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, emp_number: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basic_details_groupd39fd?.emp_number == "" || basic_details_groupd39fd?.emp_number == undefined){
      basic_details_groupd39fd.emp_number = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basic_details_groupd39fd?.emp_number);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,emp_number:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.emp_number !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basic_details_groupd39fd?.emp_number);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "74f2e1be2f96474ea7d521fda975afe9"
      );
      ...
      //     controlId: "74f2e1be2f96474ea7d521fda975afe9",
      //     isTable: false,
      //     from:"TextInputemployee_number",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'emp_number', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'emp_number',type:'text'};
      //   type={
      //     name:'emp_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emp_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emp_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.emp_number.type
      //   }
      ...
      //   type={
      //     name:'emp_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emp_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emp_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.emp_number.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basic_details_groupd39fd?.emp_number)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "74f2e1be2f96474ea7d521fda975afe9") {
        handleChange({target:{value:basic_details_groupd39fdRef?.current?.emp_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "74f2e1be2f96474ea7d521fda975afe9") {
        handleBlur({target:{value:basic_details_groupd39fdRef?.current?.emp_number||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupd39fd((pre:any)=>({...pre,emp_number:dfd_employees_v1Props.data[0]?.emp_number}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (employee_number5afe9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basic_details_groupd39fd?.emp_number||""}
         disabled= {employee_number5afe9?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.emp_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputemployee_number
