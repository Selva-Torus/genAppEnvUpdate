...
const TextInputlast_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.last_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|619366ccec19457e956f09f17acd39fd|3fe27c18cd4a4120901dec3fa3d74ac4"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'last_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,last_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, last_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basic_details_groupd39fd?.last_name == "" || basic_details_groupd39fd?.last_name == undefined){
      basic_details_groupd39fd.last_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basic_details_groupd39fd?.last_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,last_name:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.last_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basic_details_groupd39fd?.last_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3fe27c18cd4a4120901dec3fa3d74ac4"
      );
      ...
      //     controlId: "3fe27c18cd4a4120901dec3fa3d74ac4",
      //     isTable: false,
      //     from:"TextInputlast_name",
      ...
      //   type={
      //     name:'last_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.last_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.last_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.last_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basic_details_groupd39fd?.last_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "3fe27c18cd4a4120901dec3fa3d74ac4") {
        handleChange({target:{value:basic_details_groupd39fdRef?.current?.last_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3fe27c18cd4a4120901dec3fa3d74ac4") {
        handleBlur({target:{value:basic_details_groupd39fdRef?.current?.last_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupd39fd((pre:any)=>({...pre,last_name:dfd_employees_v1Props.data[0]?.last_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (last_name74ac4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basic_details_groupd39fd?.last_name||""}
         disabled= {last_name74ac4?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. Doe'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.last_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputlast_name
