...
const TextInputemployee_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|619366ccec19457e956f09f17acd39fd|719c7d0183524fe0a201be0d0c0c3e12"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employee_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, employee_code: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basic_details_groupd39fd?.employee_code == "" || basic_details_groupd39fd?.employee_code == undefined){
      basic_details_groupd39fd.employee_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basic_details_groupd39fd?.employee_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employee_code:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.employee_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basic_details_groupd39fd?.employee_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "719c7d0183524fe0a201be0d0c0c3e12"
      );
      ...
      //     controlId: "719c7d0183524fe0a201be0d0c0c3e12",
      //     isTable: false,
      //     from:"TextInputemployee_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'employee_code', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'employee_code',type:'text'};
      //   type={
      //     name:'employee_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.employee_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.employee_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.employee_code.type
      //   }
      ...
      //   type={
      //     name:'employee_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.employee_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.employee_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.employee_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basic_details_groupd39fd?.employee_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "719c7d0183524fe0a201be0d0c0c3e12") {
        handleChange({target:{value:basic_details_groupd39fdRef?.current?.employee_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "719c7d0183524fe0a201be0d0c0c3e12") {
        handleBlur({target:{value:basic_details_groupd39fdRef?.current?.employee_code||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupd39fd((pre:any)=>({...pre,employee_code:dfd_employees_v1Props.data[0]?.employee_code}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (employee_codec3e12?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basic_details_groupd39fd?.employee_code||""}
         disabled= {employee_codec3e12?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. EMP001'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Employeee Code"
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.employee_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputemployee_code
