...
const Dropdowngender = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "d5d3f6bfbbb7462caebb29d4326af587"
    );
  ...
  const handlechange = async(value: any) => {
    ...
    if(value.length>0){
      ...
      setbasic_details_groupd39fd((prev: any) => ({ ...prev,genderaf587: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basic_details_groupd39fd;
      delete temp.gender;
      delete temp.GENDER;
      delete temp.genderaf587;
      setbasic_details_groupd39fd(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,gender:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basic_details_groupd39fd.gender){
      if(Array.isArray(dfd_gendercombo_v1Props)){
        if(dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupd39fd.gender)){
          setdropdownValue([dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupd39fd.gender)?.gender])
          tempValue=dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupd39fd.gender)?.gender
        }else{
          setdropdownValue([basic_details_groupd39fd.gender])
          tempValue=basic_details_groupd39fd.gender
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{gender:basic_details_groupd39fd.gender}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupd39fd.gender)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupd39fd.gender)?.gender ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupd39fd.gender)?.gender
      }else{
        setdropdownValue([basic_details_groupd39fd.gender])
        tempValue=basic_details_groupd39fd.gender
      }   
    ...
    if(genderaf587?.trigger == true)
    {
      await handlechange(tempValue)
      setgenderaf587((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basic_details_groupd39fd.gender, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_gendercombo_v1Props) && dfd_gendercombo_v1Props?.length == 1){
    // setbasic_details_groupd39fd((pre:any)=>({...pre,gender:dfd_gendercombo_v1Props[0]?.gender}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasic_details_groupd39fd((prev: any) => ({ ...prev, gender: getMapperDetailsBindValues[value],GENDER: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasic_details_groupd39fd((prev: any) => ({ ...prev, gender: '', genderaf587: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,gender:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(basic_details_groupd39fd?.gender == "" || basic_details_groupd39fd?.gender == undefined){
      basic_details_groupd39fd.gender = "";
      const validate:any = v.safeParse(schema, basic_details_groupd39fd?.gender);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,gender:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.gender !== ""){
    const validate:any = v.safeParse(schema, basic_details_groupd39fd?.gender);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,gender:"invalid"}}));
  ...
  useEffect(() => { basic_details_groupd39fdRef.current = basic_details_groupd39fd; }, [basic_details_groupd39fd]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "d5d3f6bfbbb7462caebb29d4326af587") {
        handleClick(basic_details_groupd39fdRef?.current?.genderaf587?basic_details_groupd39fdRef?.current?.genderaf587:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "d5d3f6bfbbb7462caebb29d4326af587");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasic_details_groupd39fd((pre:any)=>({...pre,gender:""}))
    else
      setInitialCount(1)
  },[genderaf587?.refresh])
  

  if (genderaf587?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {genderaf587?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={basic_details_groupd39fd?.genderaf587 ? [basic_details_groupd39fd?.genderaf587] : (basic_details_groupd39fd?.gender ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.gender ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
