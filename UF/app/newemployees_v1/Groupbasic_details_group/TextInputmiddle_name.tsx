...
const TextInputmiddle_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|619366ccec19457e956f09f17acd39fd|300a01602dde443397a78740ee10e1fe"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,middle_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, middle_name: +e.target.value }));
    }
    else{
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, middle_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "300a01602dde443397a78740ee10e1fe"
      );
      ...
      //     controlId: "300a01602dde443397a78740ee10e1fe",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "300a01602dde443397a78740ee10e1fe") {
        handleChange({target:{value:basic_details_groupd39fdRef?.current?.middle_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "300a01602dde443397a78740ee10e1fe") {
        handleBlur({target:{value:basic_details_groupd39fdRef?.current?.middle_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupd39fd((pre:any)=>({...pre,middle_name:dfd_employees_v1Props.data[0]?.middle_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (middle_name0e1fe?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupd39fd?.middle_name||""}
         disabled= {middle_name0e1fe?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.middle_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
