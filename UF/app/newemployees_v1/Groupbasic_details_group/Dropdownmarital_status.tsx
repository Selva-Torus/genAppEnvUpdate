 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "e9aa5e194d28411dac29bd4bd323f841"
    );
  ...
  },[marital_status3f841?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbasic_details_groupd39fd((prev: any) => ({ ...prev,marital_status3f841: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basic_details_groupd39fd;
      delete temp.marital_status;
      delete temp.MARITAL_STATUS;
      delete temp.marital_status3f841;
      setbasic_details_groupd39fd(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,marital_status:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basic_details_groupd39fd.marital_status){
      if(Array.isArray(dfd_maritalstatuscombo_v1Props)){
        if(dfd_maritalstatuscombo_v1Props?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)){
          setdropdownValue([dfd_maritalstatuscombo_v1Props?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)?.marital_status])
          tempValue=dfd_maritalstatuscombo_v1Props?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)?.marital_status
        }else{
          setdropdownValue([basic_details_groupd39fd.marital_status])
          tempValue=basic_details_groupd39fd.marital_status
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{marital_status:basic_details_groupd39fd.marital_status}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)?.marital_status ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.marital_status === basic_details_groupd39fd.marital_status)?.marital_status
      }else{
        setdropdownValue([basic_details_groupd39fd.marital_status])
        tempValue=basic_details_groupd39fd.marital_status
      }   
    ...
    if(marital_status3f841?.trigger == true)
    {
      await handlechange(tempValue)
      setmarital_status3f841((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basic_details_groupd39fd.marital_status, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_maritalstatuscombo_v1Props) && dfd_maritalstatuscombo_v1Props?.length == 1){
    // setbasic_details_groupd39fd((pre:any)=>({...pre,marital_status:dfd_maritalstatuscombo_v1Props[0]?.marital_status}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasic_details_groupd39fd((prev: any) => ({ ...prev, marital_status: getMapperDetailsBindValues[value],MARITAL_STATUS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasic_details_groupd39fd((prev: any) => ({ ...prev, marital_status: '', marital_status3f841: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,marital_status:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(basic_details_groupd39fd?.marital_status == "" || basic_details_groupd39fd?.marital_status == undefined){
      basic_details_groupd39fd.marital_status = "";
      const validate:any = v.safeParse(schema, basic_details_groupd39fd?.marital_status);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,marital_status:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.marital_status !== ""){
    const validate:any = v.safeParse(schema, basic_details_groupd39fd?.marital_status);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,marital_status:"invalid"}}));
  ...
  useEffect(() => { basic_details_groupd39fdRef.current = basic_details_groupd39fd; }, [basic_details_groupd39fd]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "e9aa5e194d28411dac29bd4bd323f841") {
        handleClick(basic_details_groupd39fdRef?.current?.marital_status3f841?basic_details_groupd39fdRef?.current?.marital_status3f841:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "e9aa5e194d28411dac29bd4bd323f841");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasic_details_groupd39fd((pre:any)=>({...pre,marital_status:""}))
    else
      setInitialCount(1)
  },[marital_status3f841?.refresh])
  

  if (marital_status3f841?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {marital_status3f841?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={basic_details_groupd39fd?.marital_status3f841 ? [basic_details_groupd39fd?.marital_status3f841] : (basic_details_groupd39fd?.marital_status ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.marital_status ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
