 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "6739e5ee988540b084bcf2eb161a73f9"
    );
  ...
  },[blood_groupa73f9?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbasic_details_groupd39fd((prev: any) => ({ ...prev,blood_groupa73f9: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basic_details_groupd39fd;
      delete temp.blood_group;
      delete temp.BLOOD_GROUP;
      delete temp.blood_groupa73f9;
      setbasic_details_groupd39fd(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,blood_group:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basic_details_groupd39fd.blood_group){
      if(Array.isArray(dfd_bloodgroupcombo_v1Props)){
        if(dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)){
          setdropdownValue([dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)?.blood_group])
          tempValue=dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)?.blood_group
        }else{
          setdropdownValue([basic_details_groupd39fd.blood_group])
          tempValue=basic_details_groupd39fd.blood_group
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{blood_group:basic_details_groupd39fd.blood_group}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)?.blood_group ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupd39fd.blood_group)?.blood_group
      }else{
        setdropdownValue([basic_details_groupd39fd.blood_group])
        tempValue=basic_details_groupd39fd.blood_group
      }   
    ...
    if(blood_groupa73f9?.trigger == true)
    {
      await handlechange(tempValue)
      setblood_groupa73f9((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basic_details_groupd39fd.blood_group, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_bloodgroupcombo_v1Props) && dfd_bloodgroupcombo_v1Props?.length == 1){
    // setbasic_details_groupd39fd((pre:any)=>({...pre,blood_group:dfd_bloodgroupcombo_v1Props[0]?.blood_group}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasic_details_groupd39fd((prev: any) => ({ ...prev, blood_group: getMapperDetailsBindValues[value],BLOOD_GROUP: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasic_details_groupd39fd((prev: any) => ({ ...prev, blood_group: '', blood_groupa73f9: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,blood_group:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(basic_details_groupd39fd?.blood_group == "" || basic_details_groupd39fd?.blood_group == undefined){
      basic_details_groupd39fd.blood_group = "";
      const validate:any = v.safeParse(schema, basic_details_groupd39fd?.blood_group);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,blood_group:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.blood_group !== ""){
    const validate:any = v.safeParse(schema, basic_details_groupd39fd?.blood_group);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,blood_group:"invalid"}}));
  ...
  useEffect(() => { basic_details_groupd39fdRef.current = basic_details_groupd39fd; }, [basic_details_groupd39fd]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "6739e5ee988540b084bcf2eb161a73f9") {
        handleClick(basic_details_groupd39fdRef?.current?.blood_groupa73f9?basic_details_groupd39fdRef?.current?.blood_groupa73f9:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "6739e5ee988540b084bcf2eb161a73f9");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasic_details_groupd39fd((pre:any)=>({...pre,blood_group:""}))
    else
      setInitialCount(1)
  },[blood_groupa73f9?.refresh])
  

  if (blood_groupa73f9?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {blood_groupa73f9?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={basic_details_groupd39fd?.blood_groupa73f9 ? [basic_details_groupd39fd?.blood_groupa73f9] : (basic_details_groupd39fd?.blood_group ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.blood_group ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
