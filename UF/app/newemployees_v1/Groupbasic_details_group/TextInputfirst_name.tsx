...
const TextInputfirst_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.first_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|619366ccec19457e956f09f17acd39fd|66747d9f50ea477a87dd180d7b45aa4b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'first_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,first_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupd39fd((prev: any) => ({ ...prev, first_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basic_details_groupd39fd?.first_name == "" || basic_details_groupd39fd?.first_name == undefined){
      basic_details_groupd39fd.first_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basic_details_groupd39fd?.first_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,first_name:"invalid"}}));
        }
    }else if(basic_details_groupd39fd?.first_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basic_details_groupd39fd?.first_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "66747d9f50ea477a87dd180d7b45aa4b"
      );
      ...
      //     controlId: "66747d9f50ea477a87dd180d7b45aa4b",
      //     isTable: false,
      //     from:"TextInputfirst_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'first_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'first_name',type:'text'};
      //   type={
      //     name:'first_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.first_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.first_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.first_name.type
      //   }
      ...
      //   type={
      //     name:'first_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.first_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.first_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.first_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basic_details_groupd39fd?.first_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "66747d9f50ea477a87dd180d7b45aa4b") {
        handleChange({target:{value:basic_details_groupd39fdRef?.current?.first_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "66747d9f50ea477a87dd180d7b45aa4b") {
        handleBlur({target:{value:basic_details_groupd39fdRef?.current?.first_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupd39fd((pre:any)=>({...pre,first_name:dfd_employees_v1Props.data[0]?.first_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (first_name5aa4b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupd39fd?.first_name||""}
         disabled= {first_name5aa4b?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.first_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputfirst_name
