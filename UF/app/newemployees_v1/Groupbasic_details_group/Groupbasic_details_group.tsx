...
const Groupbasic_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "619366ccec19457e956f09f17acd39fd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbasic_details_groupd39fdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("basic_details")){
        setbasic_details3de86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(basic_details3de86?.isDisabled==null)
      {
        setbasic_details3de86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("employee_code")){
        setemployee_codec3e12((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employee_codec3e12?.isDisabled==null)
      {
        setemployee_codec3e12((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("employee_number")){
        setemployee_number5afe9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employee_number5afe9?.isDisabled==null)
      {
        setemployee_number5afe9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("first_name")){
        setfirst_name5aa4b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(first_name5aa4b?.isDisabled==null)
      {
        setfirst_name5aa4b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("middle_name")){
        setmiddle_name0e1fe((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(middle_name0e1fe?.isDisabled==null)
      {
        setmiddle_name0e1fe((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("last_name")){
        setlast_name74ac4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(last_name74ac4?.isDisabled==null)
      {
        setlast_name74ac4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("preferred_name")){
        setpreferred_namecb5c3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(preferred_namecb5c3?.isDisabled==null)
      {
        setpreferred_namecb5c3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("gender")){
        setgenderaf587((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(genderaf587?.isDisabled==null)
      {
        setgenderaf587((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("blood_group")){
        setblood_groupa73f9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(blood_groupa73f9?.isDisabled==null)
      {
        setblood_groupa73f9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("date_of_birth")){
        setdate_of_birth84db9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(date_of_birth84db9?.isDisabled==null)
      {
        setdate_of_birth84db9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("marital_status")){
        setmarital_status3f841((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(marital_status3f841?.isDisabled==null)
      {
        setmarital_status3f841((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const basic_details_groupd39fdRef = useRef<any>(null);
  const handleClearSearch = () => {
    basic_details_groupd39fdRef.current?.setSearchParams();
    basic_details_groupd39fdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(basic_details_groupd39fd) && Object.keys(basic_details_groupd39fd)?.length>0)
      {
        setbasic_details_groupd39fd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [basic_details_groupd39fdProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"basic_details_group"}))
        }}
    >
          {allowedControls.includes("basic_details") ?<Textbasic_details   /* 3de86 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("employee_code") ?<TextInputemployee_code   /* c3e12 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("employee_number") ?<TextInputemployee_number   /* 5afe9 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("first_name") ?<TextInputfirst_name   /* 5aa4b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("middle_name") ?<TextInputmiddle_name   /* 0e1fe */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("last_name") ?<TextInputlast_name   /* 74ac4 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("preferred_name") ?<TextInputpreferred_name   /* cb5c3 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("gender") ?<Dropdowngender   /* af587 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("blood_group") ?<Dropdownblood_group   /* a73f9 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("date_of_birth") ?<DatePickerdate_of_birth   /* 84db9 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("marital_status") ?<Dropdownmarital_status   /* 3f841 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
