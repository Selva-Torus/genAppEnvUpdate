...
const Groupcert_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {certification_icone672f, setcertification_icone672f}= useContext(TotalContext) as TotalContextProps;
  const {cert_text784fe, setcert_text784fe}= useContext(TotalContext) as TotalContextProps;
  const {cert_desa8bdc, setcert_desa8bdc}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1de0c329b6e44f19bbe77d52cefedb63");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcert_groupedb63Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("certification_icon")){
        setcertification_icone672f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(certification_icone672f?.isDisabled==null)
      {
        setcertification_icone672f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_text")){
        setcert_text784fe((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_text784fe?.isDisabled==null)
      {
        setcert_text784fe((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_des")){
        setcert_desa8bdc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_desa8bdc?.isDisabled==null)
      {
        setcert_desa8bdc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("certification")){
        setcertificationc7d06((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(certificationc7d06?.isDisabled==null)
      {
        setcertificationc7d06((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['certification_icon'] = certification_icone672f,
        codeStates['setcertification_icon'] = setcertification_icone672f,
        codeStates['cert_text'] = cert_text784fe,
        codeStates['setcert_text'] = setcert_text784fe,
        codeStates['cert_des'] = cert_desa8bdc,
        codeStates['setcert_des'] = setcert_desa8bdc,
  ...
  const cert_groupedb63Ref = useRef<any>(null);
  const handleClearSearch = () => {
    cert_groupedb63Ref.current?.setSearchParams();
    cert_groupedb63Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cert_groupedb63) && Object.keys(cert_groupedb63)?.length>0)
      {
        setcert_groupedb63({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cert_groupedb63Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"cert_group"}))
        }}
    >
        ...
        {allowedControls.includes("certification_icon")?<Iconcertification_icon /* e672f */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("cert_text") ?<Textcert_text   /* 784fe */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("cert_des") ?<Textcert_des   /* a8bdc */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
