...
const DatePickerprobation_end_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,probation_end: "invalid"}}))
    setemployment_details_group89cd6((prev: any) => ({ ...prev, probation_end: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,probation_end:undefined}}));
  if (!date) {
    setemployment_details_group89cd6((prev: any) => ({ ...prev, probation_end: null }));
    return;
  }
  const now = new Date();
  ...
  setemployment_details_group89cd6((prev: any) => ({ ...prev, probation_end: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(employment_details_group89cd6?.probation_end == "" || employment_details_group89cd6?.probation_end == undefined){
        const result = v.safeParse(schema, employment_details_group89cd6?.probation_end || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "9764e52e562c461297e639ab734d4862"
      );
...
useEffect(()=>{
 ...
 },[probation_end_dated4862?.refresh])

useEffect(()=>{
  if(!employment_details_group89cd6?.probation_end){ 
...
if (probation_end_dated4862?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 7`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={employment_details_group89cd6?.probation_end}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {probation_end_dated4862?.isDisabled ? true : false}
      disabled= {probation_end_dated4862?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Probation End Date"
      dateValidation=""
      validationState={validate?.newEmployees_v1?.probation_end ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerprobation_end_date
