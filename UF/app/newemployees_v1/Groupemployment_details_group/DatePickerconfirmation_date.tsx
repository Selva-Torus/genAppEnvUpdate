...
const DatePickerconfirmation_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,confirmation_date: "invalid"}}))
    setemployment_details_group89cd6((prev: any) => ({ ...prev, confirmation_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,confirmation_date:undefined}}));
  if (!date) {
    setemployment_details_group89cd6((prev: any) => ({ ...prev, confirmation_date: null }));
    return;
  }
  const now = new Date();
  ...
  setemployment_details_group89cd6((prev: any) => ({ ...prev, confirmation_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(employment_details_group89cd6?.confirmation_date == "" || employment_details_group89cd6?.confirmation_date == undefined){
        const result = v.safeParse(schema, employment_details_group89cd6?.confirmation_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "61a23e1310884256af25437b299264bb"
      );
...
useEffect(()=>{
 ...
 },[confirmation_date264bb?.refresh])

useEffect(()=>{
  if(!employment_details_group89cd6?.confirmation_date){ 
...
if (confirmation_date264bb?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `19 / 25`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={employment_details_group89cd6?.confirmation_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {confirmation_date264bb?.isDisabled ? true : false}
      disabled= {confirmation_date264bb?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Confirmation Date"
      dateValidation=""
      validationState={validate?.newEmployees_v1?.confirmation_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerconfirmation_date
