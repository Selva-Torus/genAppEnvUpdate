 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "f9e4c0286be3446e925c88281130a7c5"
    );
  ...
  },[employment_type0a7c5?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setemployment_details_group89cd6((prev: any) => ({ ...prev,employment_type0a7c5: value }))
      setIsRequredData(false)
    }else{
      let temp:any = employment_details_group89cd6;
      delete temp.employment_type;
      delete temp.EMPLOYMENT_TYPE;
      delete temp.employment_type0a7c5;
      setemployment_details_group89cd6(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employment_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(employment_details_group89cd6.employment_type){
      if(Array.isArray(dfd_employmenttypecombo_v1Props)){
        if(dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)){
          setdropdownValue([dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)?.employment_type])
          tempValue=dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)?.employment_type
        }else{
          setdropdownValue([employment_details_group89cd6.employment_type])
          tempValue=employment_details_group89cd6.employment_type
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{employment_type:employment_details_group89cd6.employment_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)?.employment_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group89cd6.employment_type)?.employment_type
      }else{
        setdropdownValue([employment_details_group89cd6.employment_type])
        tempValue=employment_details_group89cd6.employment_type
      }   
    ...
    if(employment_type0a7c5?.trigger == true)
    {
      await handlechange(tempValue)
      setemployment_type0a7c5((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[employment_details_group89cd6.employment_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employmenttypecombo_v1Props) && dfd_employmenttypecombo_v1Props?.length == 1){
    // setemployment_details_group89cd6((pre:any)=>({...pre,employment_type:dfd_employmenttypecombo_v1Props[0]?.employment_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setemployment_details_group89cd6((prev: any) => ({ ...prev, employment_type: getMapperDetailsBindValues[value],EMPLOYMENT_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setemployment_details_group89cd6((prev: any) => ({ ...prev, employment_type: '', employment_type0a7c5: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employment_type:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(employment_details_group89cd6?.employment_type == "" || employment_details_group89cd6?.employment_type == undefined){
      employment_details_group89cd6.employment_type = "";
      const validate:any = v.safeParse(schema, employment_details_group89cd6?.employment_type);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employment_type:"invalid"}}));
        }
    }else if(employment_details_group89cd6?.employment_type !== ""){
    const validate:any = v.safeParse(schema, employment_details_group89cd6?.employment_type);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,employment_type:"invalid"}}));
  ...
  useEffect(() => { employment_details_group89cd6Ref.current = employment_details_group89cd6; }, [employment_details_group89cd6]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "f9e4c0286be3446e925c88281130a7c5") {
        handleClick(employment_details_group89cd6Ref?.current?.employment_type0a7c5?employment_details_group89cd6Ref?.current?.employment_type0a7c5:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "f9e4c0286be3446e925c88281130a7c5");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group89cd6((pre:any)=>({...pre,employment_type:""}))
    else
      setInitialCount(1)
  },[employment_type0a7c5?.refresh])
  

  if (employment_type0a7c5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {employment_type0a7c5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={employment_details_group89cd6?.employment_type0a7c5 ? [employment_details_group89cd6?.employment_type0a7c5] : (employment_details_group89cd6?.employment_type ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.employment_type ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
