...
const Dropdownwork_mode = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [work_modeOptions, setwork_modeOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `emp_work_mode`
    let mapperText: string =  `emp_work_mode`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "04f20ced9de14becbd9535e032c83dbf"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_empworkmodecombo_v1Props.dstKey){
      dstKey = dfd_empworkmodecombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_empworkmodecombo_v1Props && !dfd_empworkmodecombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[work_mode83dbf?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setemployment_details_group89cd6((prev: any) => ({ ...prev,work_mode83dbf: value }))
      setIsRequredData(false)
    }else{
      let temp:any = employment_details_group89cd6;
      delete temp.work_mode;
      delete temp.WORK_MODE;
      delete temp.work_mode83dbf;
      setemployment_details_group89cd6(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_mode:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(employment_details_group89cd6.work_mode){
      if(Array.isArray(dfd_empworkmodecombo_v1Props)){
        if(dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)){
          setdropdownValue([dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)?.emp_work_mode])
          tempValue=dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)?.emp_work_mode
        }else{
          setdropdownValue([employment_details_group89cd6.work_mode])
          tempValue=employment_details_group89cd6.work_mode
        }
      }else{
        let dstKey:string = dfd_empworkmodecombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_work_mode:employment_details_group89cd6.work_mode}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)?.emp_work_mode ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group89cd6.work_mode)?.emp_work_mode
      }else{
        setdropdownValue([employment_details_group89cd6.work_mode])
        tempValue=employment_details_group89cd6.work_mode
      }   
    ...
    if(work_mode83dbf?.trigger == true)
    {
      await handlechange(tempValue)
      setwork_mode83dbf((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[employment_details_group89cd6.work_mode, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_empworkmodecombo_v1Props) && dfd_empworkmodecombo_v1Props?.length == 1){
    // setemployment_details_group89cd6((pre:any)=>({...pre,work_mode:dfd_empworkmodecombo_v1Props[0]?.work_mode}))
    }
  },[dfd_empworkmodecombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setemployment_details_group89cd6((prev: any) => ({ ...prev, work_mode: getMapperDetailsBindValues[value],WORK_MODE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setemployment_details_group89cd6((prev: any) => ({ ...prev, work_mode: '', work_mode83dbf: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_mode:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.emp_work_mode==getMapperDetailsBindValues[value] && items?.emp_work_mode==value)) || {}
    selected.current={
      ...selectedObj||{},
      emp_work_mode:value,
      emp_work_mode:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(employment_details_group89cd6?.work_mode == "" || employment_details_group89cd6?.work_mode == undefined){
      employment_details_group89cd6.work_mode = "";
      const validate:any = v.safeParse(schema, employment_details_group89cd6?.work_mode);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_mode:"invalid"}}));
        }
    }else if(employment_details_group89cd6?.work_mode !== ""){
    const validate:any = v.safeParse(schema, employment_details_group89cd6?.work_mode);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,work_mode:"invalid"}}));
  ...
  useEffect(() => { employment_details_group89cd6Ref.current = employment_details_group89cd6; }, [employment_details_group89cd6]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "04f20ced9de14becbd9535e032c83dbf") {
        handleClick(employment_details_group89cd6Ref?.current?.work_mode83dbf?employment_details_group89cd6Ref?.current?.work_mode83dbf:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "04f20ced9de14becbd9535e032c83dbf");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group89cd6((pre:any)=>({...pre,work_mode:""}))
    else
      setInitialCount(1)
  },[work_mode83dbf?.refresh])
  

  if (work_mode83dbf?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {work_mode83dbf?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={employment_details_group89cd6?.work_mode83dbf ? [employment_details_group89cd6?.work_mode83dbf] : (employment_details_group89cd6?.work_mode ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.work_mode ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownwork_mode;
