...
const Dropdowntime_zone = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(employment_details_group89cd6?.timezone=="" || employment_details_group89cd6?.timezone==undefined || employment_details_group89cd6?.timezone==null ){
    setSelectedItem("");
  }
  },[employment_details_group89cd6?.timezone])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "143f5a610fef49deaa528d5f35a89cd6",
        "68a680a95892479a8b7e274ad27c76a9"
      );
      ...
      let staticValueProps : any[] = [
  {
    ...
    "value": {
      ...
      setemployment_details_group89cd6((prev: any) => ({ ...prev, timezone: staticTextValue, time_zonec76a9: value}))
         setIsRequredData(false)
    } else {
       setemployment_details_group89cd6((prev: any) => ({ ...prev, timezone: '', time_zonec76a9: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,timezone:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(employment_details_group89cd6?.timezone == "" || employment_details_group89cd6?.timezone == undefined){
      employment_details_group89cd6.timezone = "";
      const validate:any = v.safeParse(schema, employment_details_group89cd6?.timezone);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,timezone:"invalid"}}));
        }
    }else if(employment_details_group89cd6?.timezone !== ""){
    const validate:any = v.safeParse(schema, employment_details_group89cd6?.timezone);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,timezone:"invalid"}}));
  ...
  useEffect(() => { employment_details_group89cd6Ref.current = employment_details_group89cd6; }, [employment_details_group89cd6]);
    useEffect(()=>{
        if(!employment_details_group89cd6?.timezone)
        { 
       ...
       const handler = (id:any) => {
          if (id === "68a680a95892479a8b7e274ad27c76a9") {
        handleClick(employment_details_group89cd6Ref?.current?.time_zonec76a9?employment_details_group89cd6Ref?.current?.time_zonec76a9:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "68a680a95892479a8b7e274ad27c76a9");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group89cd6((pre:any)=>({...pre,timezone:""}))
    else
      setInitialCount(1)
  },[time_zonec76a9?.refresh])
  

  if (time_zonec76a9?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {time_zonec76a9?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            employment_details_group89cd6?.time_zonec76a9 ? [employment_details_group89cd6?.time_zonec76a9] :
                employment_details_group89cd6?.timezone ? employment_details_group89cd6?.timezone : []
            }
        onChange={handleClick} 
        validationState={validate?.newEmployees_v1?.timezone ? "invalid" : undefined}
        errorMessage={error}
      /> 
    </div>
  );
...
export default Dropdowntime_zone;
