...
const Dropdownreporting_mmanager_id = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(employment_details_group89cd6?.reporting_manager_id=="" || employment_details_group89cd6?.reporting_manager_id==undefined || employment_details_group89cd6?.reporting_manager_id==null ){
    setSelectedItem("");
  }
  },[employment_details_group89cd6?.reporting_manager_id])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "143f5a610fef49deaa528d5f35a89cd6",
        "6616578c3f674a4fad1fcfef3ed99b9a"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reporting_mmanager_id99b9a?.refresh])
      ...
      let staticValueProps : any[] = [
  {
    ...
    "value": {
      ...
      setemployment_details_group89cd6((prev: any) => ({ ...prev, reporting_manager_id: staticTextValue, reporting_mmanager_id99b9a: value}))
         setIsRequredData(false)
    } else {
       setemployment_details_group89cd6((prev: any) => ({ ...prev, reporting_manager_id: '', reporting_mmanager_id99b9a: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,reporting_manager_id:undefined}}));
   
    // static
    selected.current={
      reporting_mmanager_id:value
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(employment_details_group89cd6?.reporting_manager_id == "" || employment_details_group89cd6?.reporting_manager_id == undefined){
      employment_details_group89cd6.reporting_manager_id = "";
      const validate:any = v.safeParse(schema, employment_details_group89cd6?.reporting_manager_id);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,reporting_manager_id:"invalid"}}));
        }
    }else if(employment_details_group89cd6?.reporting_manager_id !== ""){
    const validate:any = v.safeParse(schema, employment_details_group89cd6?.reporting_manager_id);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,reporting_manager_id:"invalid"}}));
  ...
  useEffect(() => { employment_details_group89cd6Ref.current = employment_details_group89cd6; }, [employment_details_group89cd6]);
    useEffect(()=>{
        if(!employment_details_group89cd6?.reporting_manager_id)
        { 
       ...
       const handler = (id:any) => {
          if (id === "6616578c3f674a4fad1fcfef3ed99b9a") {
        handleClick(employment_details_group89cd6Ref?.current?.reporting_mmanager_id99b9a?employment_details_group89cd6Ref?.current?.reporting_mmanager_id99b9a:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "6616578c3f674a4fad1fcfef3ed99b9a");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group89cd6((pre:any)=>({...pre,reporting_manager_id:""}))
    else
      setInitialCount(1)
  },[reporting_mmanager_id99b9a?.refresh])
  

  if (reporting_mmanager_id99b9a?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {reporting_mmanager_id99b9a?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            employment_details_group89cd6?.reporting_mmanager_id99b9a ? [employment_details_group89cd6?.reporting_mmanager_id99b9a] :
                employment_details_group89cd6?.reporting_manager_id ? employment_details_group89cd6?.reporting_manager_id : []
            }
        onChange={handleClick} 
        validationState={validate?.newEmployees_v1?.reporting_manager_id ? "invalid" : undefined}
        errorMessage={error}
      /> 
    </div>
  );
...
export default Dropdownreporting_mmanager_id;
