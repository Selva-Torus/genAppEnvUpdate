...
const TextInputpostal_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.postal_code"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|bba8ca5638ef4ca7b59fe895038b72f4|aff71cd8bc0c488eb502ea1de089a646"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'postal_code',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,postal_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddress_details_groupb72f4((prev: any) => ({ ...prev, postal_code: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(address_details_groupb72f4?.postal_code == "" || address_details_groupb72f4?.postal_code == undefined){
      address_details_groupb72f4.postal_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, address_details_groupb72f4?.postal_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,postal_code:"invalid"}}));
        }
    }else if(address_details_groupb72f4?.postal_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +address_details_groupb72f4?.postal_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "aff71cd8bc0c488eb502ea1de089a646"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1",
      //     componentId: "bba8ca5638ef4ca7b59fe895038b72f4",
      //     controlId: "aff71cd8bc0c488eb502ea1de089a646",
      //     isTable: false,
      //     from:"TextInputpostal_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'postal_code', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'postal_code',type:'text'};
      //   type={
      //     name:'postal_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.postal_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.postal_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.postal_code.type
      //   }
      ...
      //   type={
      //     name:'postal_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.postal_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.postal_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.postal_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!address_details_groupb72f4?.postal_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "aff71cd8bc0c488eb502ea1de089a646") {
        handleChange({target:{value:address_details_groupb72f4Ref?.current?.postal_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "aff71cd8bc0c488eb502ea1de089a646") {
        handleBlur({target:{value:address_details_groupb72f4Ref?.current?.postal_code||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setaddress_details_groupb72f4((pre:any)=>({...pre,postal_code:dfd_employees_v1Props.data[0]?.postal_code}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (postal_code9a646?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={address_details_groupb72f4?.postal_code||""}
         disabled= {postal_code9a646?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.postal_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpostal_code
