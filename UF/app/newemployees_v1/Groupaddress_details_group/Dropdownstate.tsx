...
const Dropdownstate = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    //getSourceValue
    code = address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"].toUpperCase()];
    getSourceFilterColumn = "";
    searchFilterData[getSourceFilterColumn] = code;
    //copySourceValue
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "4669b33e2b824047b51d74f52f5198ea"
    );
  ...
  try{
    ...
    //getFormData
    if(!address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"]] && Array.isArray(address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"]])){
  ...
  useEffect(() => {
    const fetchGetDropdownData = async () => {
      //getFormData
      if(Array.isArray(address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"]])){
        address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"]] = undefined;
      }
      if (!address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"]]) {
        let temp: any = address_details_groupb72f4
        delete temp.state
        setaddress_details_groupb72f4(temp)
      }
  ...
  },[state198ea?.refresh,address_details_groupb72f4[UOmapperData["90a0b6ebf16d409b92e040f323266635"]["source"].toUpperCase()]])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaddress_details_groupb72f4((prev: any) => ({ ...prev,state198ea: value }))
      setIsRequredData(false)
    }else{
      let temp:any = address_details_groupb72f4;
      delete temp.state;
      delete temp.STATE;
      delete temp.state198ea;
      setaddress_details_groupb72f4(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,state:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(address_details_groupb72f4.state){
      if(Array.isArray(dfd_statecombo_v1Props)){
        if(dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_groupb72f4.state)){
          setdropdownValue([dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_groupb72f4.state)?.state_name])
          tempValue=dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_groupb72f4.state)?.state_name
        }else{
          setdropdownValue([address_details_groupb72f4.state])
          tempValue=address_details_groupb72f4.state
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{state_name:address_details_groupb72f4.state}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_groupb72f4.state)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_groupb72f4.state)?.state_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_groupb72f4.state)?.state_name
      }else{
        setdropdownValue([address_details_groupb72f4.state])
        tempValue=address_details_groupb72f4.state
      }   
    ...
    if(state198ea?.trigger == true)
    {
      await handlechange(tempValue)
      setstate198ea((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[address_details_groupb72f4.state, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_statecombo_v1Props) && dfd_statecombo_v1Props?.length == 1){
    // setaddress_details_groupb72f4((pre:any)=>({...pre,state:dfd_statecombo_v1Props[0]?.state}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaddress_details_groupb72f4((prev: any) => ({ ...prev, state: getMapperDetailsBindValues[value],STATE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaddress_details_groupb72f4((prev: any) => ({ ...prev, state: '', state198ea: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,state:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(address_details_groupb72f4?.state == "" || address_details_groupb72f4?.state == undefined){
      address_details_groupb72f4.state = "";
      const validate:any = v.safeParse(schema, address_details_groupb72f4?.state);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,state:"invalid"}}));
        }
    }else if(address_details_groupb72f4?.state !== ""){
    const validate:any = v.safeParse(schema, address_details_groupb72f4?.state);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,state:"invalid"}}));
  ...
  useEffect(() => { address_details_groupb72f4Ref.current = address_details_groupb72f4; }, [address_details_groupb72f4]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "4669b33e2b824047b51d74f52f5198ea") {
        handleClick(address_details_groupb72f4Ref?.current?.state198ea?address_details_groupb72f4Ref?.current?.state198ea:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "4669b33e2b824047b51d74f52f5198ea");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaddress_details_groupb72f4((pre:any)=>({...pre,state:""}))
    else
      setInitialCount(1)
  },[state198ea?.refresh])
  

  if (state198ea?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {state198ea?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={address_details_groupb72f4?.state198ea ? [address_details_groupb72f4?.state198ea] : (address_details_groupb72f4?.state ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.state ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
