...
const Groupaddress_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {address_details051da, setaddress_details051da}= useContext(TotalContext) as TotalContextProps;
  const {country66635, setcountry66635}= useContext(TotalContext) as TotalContextProps;
  const {state198ea, setstate198ea}= useContext(TotalContext) as TotalContextProps;
  const {city05e71, setcity05e71}= useContext(TotalContext) as TotalContextProps;
  const {postal_code9a646, setpostal_code9a646}= useContext(TotalContext) as TotalContextProps;
  const {nationality39fcd, setnationality39fcd}= useContext(TotalContext) as TotalContextProps;
  const {address_line15e52e, setaddress_line15e52e}= useContext(TotalContext) as TotalContextProps;
  const {address_line20e4f1, setaddress_line20e4f1}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bba8ca5638ef4ca7b59fe895038b72f4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddress_details_groupb72f4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("address_details")){
        setaddress_details051da((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address_details051da?.isDisabled==null)
      {
        setaddress_details051da((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("country")){
        setcountry66635((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(country66635?.isDisabled==null)
      {
        setcountry66635((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("state")){
        setstate198ea((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(state198ea?.isDisabled==null)
      {
        setstate198ea((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("city")){
        setcity05e71((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(city05e71?.isDisabled==null)
      {
        setcity05e71((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("postal_code")){
        setpostal_code9a646((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(postal_code9a646?.isDisabled==null)
      {
        setpostal_code9a646((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("nationality")){
        setnationality39fcd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(nationality39fcd?.isDisabled==null)
      {
        setnationality39fcd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address_line1")){
        setaddress_line15e52e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address_line15e52e?.isDisabled==null)
      {
        setaddress_line15e52e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address_line2")){
        setaddress_line20e4f1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address_line20e4f1?.isDisabled==null)
      {
        setaddress_line20e4f1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['address_details'] = address_details051da,
        codeStates['setaddress_details'] = setaddress_details051da,
        codeStates['country'] = country66635,
        codeStates['setcountry'] = setcountry66635,
        codeStates['state'] = state198ea,
        codeStates['setstate'] = setstate198ea,
        codeStates['city'] = city05e71,
        codeStates['setcity'] = setcity05e71,
        codeStates['postal_code'] = postal_code9a646,
        codeStates['setpostal_code'] = setpostal_code9a646,
        codeStates['nationality'] = nationality39fcd,
        codeStates['setnationality'] = setnationality39fcd,
        codeStates['address_line1'] = address_line15e52e,
        codeStates['setaddress_line1'] = setaddress_line15e52e,
        codeStates['address_line2'] = address_line20e4f1,
        codeStates['setaddress_line2'] = setaddress_line20e4f1,
  ...
  const address_details_groupb72f4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    address_details_groupb72f4Ref.current?.setSearchParams();
    address_details_groupb72f4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(address_details_groupb72f4) && Object.keys(address_details_groupb72f4)?.length>0)
      {
        setaddress_details_groupb72f4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [address_details_groupb72f4Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"address_details_group"}))
        }}
    >
          {allowedControls.includes("address_details") ?<Textaddress_details   /* 051da */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("country") ?<Dropdowncountry   /* 66635 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("state") ?<Dropdownstate   /* 198ea */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("city") ?<TextInputcity   /* 05e71 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("postal_code") ?<TextInputpostal_code   /* 9a646 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("nationality") ?<TextInputnationality   /* 39fcd */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address_line1") ?<TextInputaddress_line1   /* 5e52e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address_line2") ?<TextInputaddress_line2   /* 0e4f1 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
