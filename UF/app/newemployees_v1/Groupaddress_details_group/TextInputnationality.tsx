...
const TextInputnationality = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.nationality"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|bba8ca5638ef4ca7b59fe895038b72f4|58c7921f0a3248d1afb4a5be69839fcd"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'nationality',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,nationality:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddress_details_groupb72f4((prev: any) => ({ ...prev, nationality: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(address_details_groupb72f4?.nationality == "" || address_details_groupb72f4?.nationality == undefined){
      address_details_groupb72f4.nationality = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, address_details_groupb72f4?.nationality);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,nationality:"invalid"}}));
        }
    }else if(address_details_groupb72f4?.nationality !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +address_details_groupb72f4?.nationality);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "58c7921f0a3248d1afb4a5be69839fcd"
      );
      ...
      //     controlId: "58c7921f0a3248d1afb4a5be69839fcd",
      //     isTable: false,
      //     from:"TextInputnationality",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'nationality', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'nationality',type:'text'};
      //   type={
      //     name:'nationality',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.nationality.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.nationality.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.nationality.type
      //   }
      ...
      //   type={
      //     name:'nationality',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.nationality.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.nationality.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.nationality.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!address_details_groupb72f4?.nationality)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "58c7921f0a3248d1afb4a5be69839fcd") {
        handleChange({target:{value:address_details_groupb72f4Ref?.current?.nationality||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "58c7921f0a3248d1afb4a5be69839fcd") {
        handleBlur({target:{value:address_details_groupb72f4Ref?.current?.nationality||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setaddress_details_groupb72f4((pre:any)=>({...pre,nationality:dfd_employees_v1Props.data[0]?.nationality}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (nationality39fcd?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={address_details_groupb72f4?.nationality||""}
         disabled= {nationality39fcd?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.nationality ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputnationality
