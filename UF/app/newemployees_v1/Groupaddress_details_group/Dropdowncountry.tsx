...
const Dropdowncountry = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `country_name`
    let mapperText: string =  `country_name`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      controlData,
      "bba8ca5638ef4ca7b59fe895038b72f4",
      "90a0b6ebf16d409b92e040f323266635"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_countrycombo_v1Props.dstKey){
      dstKey = dfd_countrycombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_countrycombo_v1Props && !dfd_countrycombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[country66635?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaddress_details_groupb72f4((prev: any) => ({ ...prev,country66635: value }))
      setIsRequredData(false)
    }else{
      let temp:any = address_details_groupb72f4;
      delete temp.country;
      delete temp.COUNTRY;
      delete temp.country66635;
      setaddress_details_groupb72f4(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,country:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(address_details_groupb72f4.country){
      if(Array.isArray(dfd_countrycombo_v1Props)){
        if(dfd_countrycombo_v1Props?.find((item: any) => item.country_name === address_details_groupb72f4.country)){
          setdropdownValue([dfd_countrycombo_v1Props?.find((item: any) => item.country_name === address_details_groupb72f4.country)?.country_name])
          tempValue=dfd_countrycombo_v1Props?.find((item: any) => item.country_name === address_details_groupb72f4.country)?.country_name
        }else{
          setdropdownValue([address_details_groupb72f4.country])
          tempValue=address_details_groupb72f4.country
        }
      }else{
        let dstKey:string = dfd_countrycombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{country_name:address_details_groupb72f4.country}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.country_name === address_details_groupb72f4.country)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.country_name === address_details_groupb72f4.country)?.country_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.country_name === address_details_groupb72f4.country)?.country_name
      }else{
        setdropdownValue([address_details_groupb72f4.country])
        tempValue=address_details_groupb72f4.country
      }   
    ...
    if(country66635?.trigger == true)
    {
      await handlechange(tempValue)
      setcountry66635((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[address_details_groupb72f4.country, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_countrycombo_v1Props) && dfd_countrycombo_v1Props?.length == 1){
    // setaddress_details_groupb72f4((pre:any)=>({...pre,country:dfd_countrycombo_v1Props[0]?.country}))
    }
  },[dfd_countrycombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaddress_details_groupb72f4((prev: any) => ({ ...prev, country: getMapperDetailsBindValues[value],COUNTRY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaddress_details_groupb72f4((prev: any) => ({ ...prev, country: '', country66635: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,country:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.country_name==getMapperDetailsBindValues[value] && items?.country_name==value)) || {}
    selected.current={
      ...selectedObj||{},
      country_name:value,
      country_name:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(address_details_groupb72f4?.country == "" || address_details_groupb72f4?.country == undefined){
      address_details_groupb72f4.country = "";
      const validate:any = v.safeParse(schema, address_details_groupb72f4?.country);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,country:"invalid"}}));
        }
    }else if(address_details_groupb72f4?.country !== ""){
    const validate:any = v.safeParse(schema, address_details_groupb72f4?.country);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,country:"invalid"}}));
  ...
  const address_details_groupb72f4Ref = useRef<any>(address_details_groupb72f4);
  useEffect(() => { address_details_groupb72f4Ref.current = address_details_groupb72f4; }, [address_details_groupb72f4]);
    useEffect(()=>{
        if(!address_details_groupb72f4?.country)
        { 
          setaddress_details_groupb72f4Props((pre:any)=>({...pre,required:true}))
          setIsRequredData(true)
        }
        if(validateRefetch.init!=0)
          handleBlur()
       const handler = (id:any) => {
          if (id === "90a0b6ebf16d409b92e040f323266635") {
        handleClick(address_details_groupb72f4Ref?.current?.country66635?address_details_groupb72f4Ref?.current?.country66635:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "90a0b6ebf16d409b92e040f323266635");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaddress_details_groupb72f4((pre:any)=>({...pre,country:""}))
    else
      setInitialCount(1)
  },[country66635?.refresh])
  

  if (country66635?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {country66635?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={address_details_groupb72f4?.country66635 ? [address_details_groupb72f4?.country66635] : (address_details_groupb72f4?.country ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.country ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
