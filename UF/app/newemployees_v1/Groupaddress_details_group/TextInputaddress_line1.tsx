...
const TextInputaddress_line1 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.address_line1"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|bba8ca5638ef4ca7b59fe895038b72f4|6c04178b0efe41b1b2d2dd199105e52e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'address_line1',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,address_line1:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddress_details_groupb72f4((prev: any) => ({ ...prev, address_line1: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(address_details_groupb72f4?.address_line1 == "" || address_details_groupb72f4?.address_line1 == undefined){
      address_details_groupb72f4.address_line1 = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, address_details_groupb72f4?.address_line1);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,address_line1:"invalid"}}));
        }
    }else if(address_details_groupb72f4?.address_line1 !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +address_details_groupb72f4?.address_line1);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6c04178b0efe41b1b2d2dd199105e52e"
      );
      ...
      //     controlId: "6c04178b0efe41b1b2d2dd199105e52e",
      //     isTable: false,
      //     from:"TextInputaddress_line1",
      ...
      //   type={
      //     name:'address_line1',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.address_line1.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.address_line1.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.address_line1.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!address_details_groupb72f4?.address_line1)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "6c04178b0efe41b1b2d2dd199105e52e") {
        handleChange({target:{value:address_details_groupb72f4Ref?.current?.address_line1||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6c04178b0efe41b1b2d2dd199105e52e") {
        handleBlur({target:{value:address_details_groupb72f4Ref?.current?.address_line1||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setaddress_details_groupb72f4((pre:any)=>({...pre,address_line1:dfd_employees_v1Props.data[0]?.address_line1}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (address_line15e52e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 16`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={address_details_groupb72f4?.address_line1||""}
         disabled= {address_line15e52e?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. House / Building / Street'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Address Line 1"
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.address_line1 ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputaddress_line1
