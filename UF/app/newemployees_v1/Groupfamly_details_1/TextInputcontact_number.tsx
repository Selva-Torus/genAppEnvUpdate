...
const TextInputcontact_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setfamly_detailsb06ea_1((prev: any) => ({ ...prev, contact_number: +e.target.value }));
    }
    else{
    setfamly_detailsb06ea_1((prev: any) => ({ ...prev, contact_number: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "627c1968a94d44119f97b45f0b3a4deb") {
        handleChange({target:{value:famly_detailsb06ea_1Ref?.current?.contact_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "627c1968a94d44119f97b45f0b3a4deb") {
        handleBlur({target:{value:famly_detailsb06ea_1Ref?.current?.contact_number||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb06ea_1((pre:any)=>({...pre,contact_number:dfd_familyjson_v1Props.data[0]?.contact_number}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb06ea_1?.contact_number||""}
