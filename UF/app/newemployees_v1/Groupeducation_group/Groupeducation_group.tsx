...
const Groupeducation_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0e21ed1ce8764b4c9d543be411f70757");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    seteducation_group70757Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("education_icon")){
        seteducation_icon6d7d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education_icon6d7d6?.isDisabled==null)
      {
        seteducation_icon6d7d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ed_his_text")){
        seted_his_textf2805((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ed_his_textf2805?.isDisabled==null)
      {
        seted_his_textf2805((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ed_des")){
        seted_des29068((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ed_des29068?.isDisabled==null)
      {
        seted_des29068((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("education")){
        seteducation2393a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education2393a?.isDisabled==null)
      {
        seteducation2393a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['education_icon'] = education_icon6d7d6,
        codeStates['seteducation_icon'] = seteducation_icon6d7d6,
        codeStates['ed_his_text'] = ed_his_textf2805,
        codeStates['seted_his_text'] = seted_his_textf2805,
        codeStates['ed_des'] = ed_des29068,
        codeStates['seted_des'] = seted_des29068,
  ...
  const education_group70757Ref = useRef<any>(null);
  const handleClearSearch = () => {
    education_group70757Ref.current?.setSearchParams();
    education_group70757Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(education_group70757) && Object.keys(education_group70757)?.length>0)
      {
        seteducation_group70757({})
      }
    }else 
      prevRefreshRef.current= true
  }, [education_group70757Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"education_group"}))
        }}
    >
        ...
        {allowedControls.includes("education_icon")?<Iconeducation_icon /* 6d7d6 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("ed_his_text") ?<Texted_his_text   /* f2805 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("ed_des") ?<Texted_des   /* 29068 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
