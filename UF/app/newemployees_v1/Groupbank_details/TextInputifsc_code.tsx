...
const TextInputifsc_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|603ab808ed8d45ebb1bfd8c9a83820cd|07023a0c56cf4627941700ca3d47da39"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,bank_ifsc:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbank_details820cd((prev: any) => ({ ...prev, bank_ifsc: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(bank_details820cd?.bank_ifsc == "" || bank_details820cd?.bank_ifsc == undefined){
      bank_details820cd.bank_ifsc = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, bank_details820cd?.bank_ifsc);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,bank_ifsc:"invalid"}}));
        }
    }else if(bank_details820cd?.bank_ifsc !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +bank_details820cd?.bank_ifsc);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "07023a0c56cf4627941700ca3d47da39"
      );
      ...
      //     controlId: "07023a0c56cf4627941700ca3d47da39",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!bank_details820cd?.bank_ifsc)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "07023a0c56cf4627941700ca3d47da39") {
        handleChange({target:{value:bank_details820cdRef?.current?.bank_ifsc||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "07023a0c56cf4627941700ca3d47da39") {
        handleBlur({target:{value:bank_details820cdRef?.current?.bank_ifsc||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbank_details820cd((pre:any)=>({...pre,bank_ifsc:dfd_employees_v1Props.data[0]?.bank_ifsc}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (ifsc_code7da39?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={bank_details820cd?.bank_ifsc||""}
         disabled= {ifsc_code7da39?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.bank_ifsc ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
