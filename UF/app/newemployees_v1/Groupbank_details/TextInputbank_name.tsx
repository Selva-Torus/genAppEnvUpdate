...
const TextInputbank_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|603ab808ed8d45ebb1bfd8c9a83820cd|62d26ed40109415db0c15fd7f7559ea1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,bank_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbank_details820cd((prev: any) => ({ ...prev, bank_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(bank_details820cd?.bank_name == "" || bank_details820cd?.bank_name == undefined){
      bank_details820cd.bank_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, bank_details820cd?.bank_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,bank_name:"invalid"}}));
        }
    }else if(bank_details820cd?.bank_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +bank_details820cd?.bank_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "62d26ed40109415db0c15fd7f7559ea1"
      );
      ...
      //     controlId: "62d26ed40109415db0c15fd7f7559ea1",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!bank_details820cd?.bank_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "62d26ed40109415db0c15fd7f7559ea1") {
        handleChange({target:{value:bank_details820cdRef?.current?.bank_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "62d26ed40109415db0c15fd7f7559ea1") {
        handleBlur({target:{value:bank_details820cdRef?.current?.bank_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbank_details820cd((pre:any)=>({...pre,bank_name:dfd_employees_v1Props.data[0]?.bank_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (bank_name59ea1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={bank_details820cd?.bank_name||""}
         disabled= {bank_name59ea1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.bank_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
