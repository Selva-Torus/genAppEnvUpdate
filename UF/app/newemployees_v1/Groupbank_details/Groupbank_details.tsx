...
const Groupbank_details = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "603ab808ed8d45ebb1bfd8c9a83820cd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbank_details820cdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("bank_details")){
        setbank_detailsc079f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_detailsc079f?.isDisabled==null)
      {
        setbank_detailsc079f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bank_name")){
        setbank_name59ea1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_name59ea1?.isDisabled==null)
      {
        setbank_name59ea1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bank_account_number")){
        setbank_account_number9f88c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_account_number9f88c?.isDisabled==null)
      {
        setbank_account_number9f88c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ifsc_code")){
        setifsc_code7da39((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ifsc_code7da39?.isDisabled==null)
      {
        setifsc_code7da39((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const bank_details820cdRef = useRef<any>(null);
  const handleClearSearch = () => {
    bank_details820cdRef.current?.setSearchParams();
    bank_details820cdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(bank_details820cd) && Object.keys(bank_details820cd)?.length>0)
      {
        setbank_details820cd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [bank_details820cdProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"bank_details"}))
        }}
    >
          {allowedControls.includes("bank_details") ?<Textbank_details   /* c079f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("bank_name") ?<TextInputbank_name   /* 59ea1 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("bank_account_number") ?<TextInputbank_account_number   /* 9f88c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ifsc_code") ?<TextInputifsc_code   /* 7da39 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
