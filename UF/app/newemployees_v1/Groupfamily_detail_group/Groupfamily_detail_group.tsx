...
const Groupfamily_detail_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2bfc28e4b8f0480d8115a27401a800b7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfamily_detail_group800b7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("family_icon")){
        setfamily_iconafa72((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_iconafa72?.isDisabled==null)
      {
        setfamily_iconafa72((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fam_text")){
        setfam_textcf8be((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fam_textcf8be?.isDisabled==null)
      {
        setfam_textcf8be((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_des")){
        setfamily_desb62f4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_desb62f4?.isDisabled==null)
      {
        setfamily_desb62f4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("famly_details")){
        setfamly_detailsb06ea((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(famly_detailsb06ea?.isDisabled==null)
      {
        setfamly_detailsb06ea((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['family_icon'] = family_iconafa72,
        codeStates['setfamily_icon'] = setfamily_iconafa72,
        codeStates['fam_text'] = fam_textcf8be,
        codeStates['setfam_text'] = setfam_textcf8be,
        codeStates['family_des'] = family_desb62f4,
        codeStates['setfamily_des'] = setfamily_desb62f4,
  ...
  const family_detail_group800b7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    family_detail_group800b7Ref.current?.setSearchParams();
    family_detail_group800b7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(family_detail_group800b7) && Object.keys(family_detail_group800b7)?.length>0)
      {
        setfamily_detail_group800b7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [family_detail_group800b7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"family_detail_group"}))
        }}
    >
        ...
        {allowedControls.includes("family_icon")?<Iconfamily_icon /* afa72 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("fam_text") ?<Textfam_text   /* cf8be */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("family_des") ?<Textfamily_des   /* b62f4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
