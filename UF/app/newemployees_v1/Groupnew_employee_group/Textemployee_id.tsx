...
const Textemployee_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {new_empe0326, setnew_empe0326}= useContext(TotalContext) as TotalContextProps;
  ...
  const {basic_divider2a0e2, setbasic_divider2a0e2}= useContext(TotalContext) as TotalContextProps;
  ...
  const {contact_details_divider70e71, setcontact_details_divider70e71}= useContext(TotalContext) as TotalContextProps;
  ...
  const {add_dividerea333, setadd_dividerea333}= useContext(TotalContext) as TotalContextProps;
  ...
  const {comp_divider6b1fb, setcomp_divider6b1fb}= useContext(TotalContext) as TotalContextProps;
  ...
  const {emergency_divider6af77, setemergency_divider6af77}= useContext(TotalContext) as TotalContextProps;
  ...
  const {skill_divider2aaad, setskill_divider2aaad}= useContext(TotalContext) as TotalContextProps;
  ...
  const {search_divider2e6c7, setsearch_divider2e6c7}= useContext(TotalContext) as TotalContextProps;
  const {employee_idb808d, setemployee_idb808d}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_employees_v1Props && !dfd_employees_v1Props.hasLogicCenter) {
        ...
        setnew_employee_group42d78((pre: any) => {
          return { ...pre, employee_id: api_paginationData.data.records[0]?.employee_id }
        })
        }
      }
      else{
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !new_employee_group42d78.employee_id){
        setnew_employee_group42d78((pre:any)=>({...pre,employee_id:dfd_employees_v1Props[0]?.employee_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employee_idb808d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !new_employee_group42d78.employee_id){
    setnew_employee_group42d78((pre:any)=>({...pre,employee_id:dfd_employees_v1Props[0]?.employee_id}));
  }
  },[dfd_employees_v1Props])

  if (employee_idb808d?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 4`,gridRow: `315 / 316`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.employee_id : (new_employee_group42d78?.employee_id || ""))}
</Text>
  </div>
  )
}
