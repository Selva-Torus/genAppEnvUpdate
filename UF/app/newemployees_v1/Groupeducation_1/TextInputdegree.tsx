...
const TextInputdegree = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    seteducation2393a_1((prev: any) => ({ ...prev, degree: +e.target.value }));
    }
    else{
    seteducation2393a_1((prev: any) => ({ ...prev, degree: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "3272d5674802438c894a68ffc300bb4f") {
        handleChange({target:{value:education2393a_1Ref?.current?.degree||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3272d5674802438c894a68ffc300bb4f") {
        handleBlur({target:{value:education2393a_1Ref?.current?.degree||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation2393a_1((pre:any)=>({...pre,degree:dfd_educationjson_v1Props.data[0]?.degree}));
    }
  }
