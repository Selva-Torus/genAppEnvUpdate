...
const TextInputrelationship = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setfamly_detailsb06ea_2((prev: any) => ({ ...prev, relationship: +e.target.value }));
    }
    else{
    setfamly_detailsb06ea_2((prev: any) => ({ ...prev, relationship: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6f135c8780e241f28aedd9c0b2ed6894") {
        handleChange({target:{value:famly_detailsb06ea_2Ref?.current?.relationship||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6f135c8780e241f28aedd9c0b2ed6894") {
        handleBlur({target:{value:famly_detailsb06ea_2Ref?.current?.relationship||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb06ea_2((pre:any)=>({...pre,relationship:dfd_familyjson_v1Props.data[0]?.relationship}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb06ea_2?.relationship||""}
