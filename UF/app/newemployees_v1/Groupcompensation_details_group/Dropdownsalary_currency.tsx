...
const Dropdownsalary_currency = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const {compensation_detailsa4288, setcompensation_detailsa4288}= useContext(TotalContext) as TotalContextProps;
  const {current_salary14311, setcurrent_salary14311}= useContext(TotalContext) as TotalContextProps;
  const {annual_ctcff223, setannual_ctcff223}= useContext(TotalContext) as TotalContextProps;
  const {salary_currency1bc50, setsalary_currency1bc50}= useContext(TotalContext) as TotalContextProps;
  ...
  const [salary_currencyOptions, setsalary_currencyOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `currency_no`
    ...
    const orchestrationData :any = getControlOrchestrationData(
      controlData,
      "22e037f622ba40aba20a68c1047f9ef1",
      "a0c285bacf0a471eb1d6bfa0a391bc50"
    );
  ...
  try{
    ...
    if(!value){
      ...
      setsalary_currencyOptions(temp);
  ...
  },[salary_currency1bc50?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setcompensation_details_groupf9ef1((prev: any) => ({ ...prev,salary_currency1bc50: value }))
      setIsRequredData(false)
    }else{
      let temp:any = compensation_details_groupf9ef1;
      delete temp.salary_currency;
      delete temp.SALARY_CURRENCY;
      delete temp.salary_currency1bc50;
      setcompensation_details_groupf9ef1(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,salary_currency:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(compensation_details_groupf9ef1.salary_currency){
      if(Array.isArray(dfd_currencycombo_v1Props)){
        if(dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)){
          setdropdownValue([dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)?.currency_no])
          tempValue=dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)?.currency_no
        }else{
          setdropdownValue([compensation_details_groupf9ef1.salary_currency])
          tempValue=compensation_details_groupf9ef1.salary_currency
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{currency_code:compensation_details_groupf9ef1.salary_currency}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)?.currency_no ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_groupf9ef1.salary_currency)?.currency_no
      }else{
        setdropdownValue([compensation_details_groupf9ef1.salary_currency])
        tempValue=compensation_details_groupf9ef1.salary_currency
      }   
    ...
    if(salary_currency1bc50?.trigger == true)
    {
      await handlechange(tempValue)
      setsalary_currency1bc50((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[compensation_details_groupf9ef1.salary_currency, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_currencycombo_v1Props) && dfd_currencycombo_v1Props?.length == 1){
    // setcompensation_details_groupf9ef1((pre:any)=>({...pre,salary_currency:dfd_currencycombo_v1Props[0]?.salary_currency}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, salary_currency: getMapperDetailsBindValues[value],SALARY_CURRENCY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, salary_currency: '', salary_currency1bc50: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,salary_currency:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.currency_code==getMapperDetailsBindValues[value] && items?.currency_no==value)) || {}
    selected.current={
      ...selectedObj||{},
      currency_no:value,
    ...
    if (customecode != '') {
      let codeStates: any = {}      
        ...
        codeStates['compensation_details'] = compensation_detailsa4288,
        codeStates['setcompensation_details'] = setcompensation_detailsa4288,
        codeStates['current_salary'] = current_salary14311,
        codeStates['setcurrent_salary'] = setcurrent_salary14311,
        codeStates['annual_ctc'] = annual_ctcff223,
        codeStates['setannual_ctc'] = setannual_ctcff223,
        codeStates['salary_currency'] = salary_currency1bc50,
        codeStates['setsalary_currency'] = setsalary_currency1bc50,
  ...
  const handleBlur = async () => {
    //validation
    if(compensation_details_groupf9ef1?.salary_currency == "" || compensation_details_groupf9ef1?.salary_currency == undefined){
      compensation_details_groupf9ef1.salary_currency = "";
      const validate:any = v.safeParse(schema, compensation_details_groupf9ef1?.salary_currency);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,salary_currency:"invalid"}}));
        }
    }else if(compensation_details_groupf9ef1?.salary_currency !== ""){
    const validate:any = v.safeParse(schema, compensation_details_groupf9ef1?.salary_currency);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,salary_currency:"invalid"}}));
  ...
  const compensation_details_groupf9ef1Ref = useRef<any>(compensation_details_groupf9ef1);
  useEffect(() => { compensation_details_groupf9ef1Ref.current = compensation_details_groupf9ef1; }, [compensation_details_groupf9ef1]);
    useEffect(()=>{
        if(!compensation_details_groupf9ef1?.salary_currency)
        { 
          setcompensation_details_groupf9ef1Props((pre:any)=>({...pre,required:true}))
          setIsRequredData(true)
        }
        if(validateRefetch.init!=0)
          handleBlur()
       const handler = (id:any) => {
          if (id === "a0c285bacf0a471eb1d6bfa0a391bc50") {
        handleClick(compensation_details_groupf9ef1Ref?.current?.salary_currency1bc50?compensation_details_groupf9ef1Ref?.current?.salary_currency1bc50:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "a0c285bacf0a471eb1d6bfa0a391bc50");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setcompensation_details_groupf9ef1((pre:any)=>({...pre,salary_currency:""}))
    else
      setInitialCount(1)
  },[salary_currency1bc50?.refresh])
  

  if (salary_currency1bc50?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {salary_currency1bc50?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={salary_currencyOptions}
        onLoadMore={loadMore}
        ...
        value={compensation_details_groupf9ef1?.salary_currency1bc50 ? [compensation_details_groupf9ef1?.salary_currency1bc50] : (compensation_details_groupf9ef1?.salary_currency ? dropdownValue : [])}
        validationState={validate?.newEmployees_v1?.salary_currency ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownsalary_currency;
