...
const TextInputannual_ctc = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|22e037f622ba40aba20a68c1047f9ef1|103296ccf75645a487bbb8aed86ff223"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,annual_ctc:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, annual_ctc: +e.target.value }));
    }
    else{
    setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, annual_ctc: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "103296ccf75645a487bbb8aed86ff223"
      );
      ...
      //     controlId: "103296ccf75645a487bbb8aed86ff223",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "103296ccf75645a487bbb8aed86ff223") {
        handleChange({target:{value:compensation_details_groupf9ef1Ref?.current?.annual_ctc||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "103296ccf75645a487bbb8aed86ff223") {
        handleBlur({target:{value:compensation_details_groupf9ef1Ref?.current?.annual_ctc||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcompensation_details_groupf9ef1((pre:any)=>({...pre,annual_ctc:dfd_employees_v1Props.data[0]?.annual_ctc}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (annual_ctcff223?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_details_groupf9ef1?.annual_ctc||""}
         disabled= {annual_ctcff223?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.annual_ctc ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
