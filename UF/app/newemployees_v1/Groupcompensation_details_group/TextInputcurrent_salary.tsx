...
const TextInputcurrent_salary = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|22e037f622ba40aba20a68c1047f9ef1|72e57f20e0fb4ba1ae22bd3262314311"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,current_salary:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, current_salary: +e.target.value }));
    }
    else{
    setcompensation_details_groupf9ef1((prev: any) => ({ ...prev, current_salary: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "72e57f20e0fb4ba1ae22bd3262314311"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1",
      //     componentId: "22e037f622ba40aba20a68c1047f9ef1",
      //     controlId: "72e57f20e0fb4ba1ae22bd3262314311",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "72e57f20e0fb4ba1ae22bd3262314311") {
        handleChange({target:{value:compensation_details_groupf9ef1Ref?.current?.current_salary||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "72e57f20e0fb4ba1ae22bd3262314311") {
        handleBlur({target:{value:compensation_details_groupf9ef1Ref?.current?.current_salary||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcompensation_details_groupf9ef1((pre:any)=>({...pre,current_salary:dfd_employees_v1Props.data[0]?.current_salary}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (current_salary14311?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_details_groupf9ef1?.current_salary||""}
         disabled= {current_salary14311?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.current_salary ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
