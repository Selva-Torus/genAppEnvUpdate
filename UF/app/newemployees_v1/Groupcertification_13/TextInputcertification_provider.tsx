...
const TextInputcertification_provider = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|981857295fec44a6a863f123e83c7d06|d2b3b7dfe0f14b159d7e9b3d5de33172"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,certification_provider:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcertificationc7d06_13((prev: any) => ({ ...prev, certification_provider: +e.target.value }));
    }
    else{
    setcertificationc7d06_13((prev: any) => ({ ...prev, certification_provider: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d2b3b7dfe0f14b159d7e9b3d5de33172"
      );
      ...
      //     controlId: "d2b3b7dfe0f14b159d7e9b3d5de33172",
      //     isTable: false,
  ...
  const certificationc7d06_13Ref = useRef<any>(certificationc7d06_13);
  useEffect(() => { certificationc7d06_13Ref.current = certificationc7d06_13; }, [certificationc7d06_13]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "d2b3b7dfe0f14b159d7e9b3d5de33172") {
        handleChange({target:{value:certificationc7d06_13Ref?.current?.certification_provider||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d2b3b7dfe0f14b159d7e9b3d5de33172") {
        handleBlur({target:{value:certificationc7d06_13Ref?.current?.certification_provider||""}});
      }
    };
  ...
  if(dfd_certifyjson_v1Props?.setSearchFilters && dfd_certifyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_certifyjson_v1Props.data) && dfd_certifyjson_v1Props.data.length > 0){
      setcertificationc7d06_13((pre:any)=>({...pre,certification_provider:dfd_certifyjson_v1Props.data[0]?.certification_provider}));
    }
  }
  },[dfd_certifyjson_v1Props?.setSearchFilters])
  if (certification_provider33172?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={certificationc7d06_13?.certification_provider||""}
         disabled= {certification_provider33172?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='AWS'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.certification_provider ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
