...
const TextInputspecialization = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    seteducation2393a_14((prev: any) => ({ ...prev, specialization: +e.target.value }));
    }
    else{
    seteducation2393a_14((prev: any) => ({ ...prev, specialization: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "69d4edf7441d42bb9d6595e230ecd52e") {
        handleChange({target:{value:education2393a_14Ref?.current?.specialization||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "69d4edf7441d42bb9d6595e230ecd52e") {
        handleBlur({target:{value:education2393a_14Ref?.current?.specialization||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation2393a_14((pre:any)=>({...pre,specialization:dfd_educationjson_v1Props.data[0]?.specialization}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={education2393a_14?.specialization||""}
