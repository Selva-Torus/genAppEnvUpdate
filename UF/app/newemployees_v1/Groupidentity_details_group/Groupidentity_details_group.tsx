...
const Groupidentity_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {identity_detailsd3925, setidentity_detailsd3925}= useContext(TotalContext) as TotalContextProps;
  const {national_id30319, setnational_id30319}= useContext(TotalContext) as TotalContextProps;
  const {passport_number9b4e5, setpassport_number9b4e5}= useContext(TotalContext) as TotalContextProps;
  const {biometric_id11ad8, setbiometric_id11ad8}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "125377fdc3004c1894d23350f816a6fe");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setidentity_details_group6a6feProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("identity_details")){
        setidentity_detailsd3925((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(identity_detailsd3925?.isDisabled==null)
      {
        setidentity_detailsd3925((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("national_id")){
        setnational_id30319((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(national_id30319?.isDisabled==null)
      {
        setnational_id30319((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("passport_number")){
        setpassport_number9b4e5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(passport_number9b4e5?.isDisabled==null)
      {
        setpassport_number9b4e5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("biometric_id")){
        setbiometric_id11ad8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(biometric_id11ad8?.isDisabled==null)
      {
        setbiometric_id11ad8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['identity_details'] = identity_detailsd3925,
        codeStates['setidentity_details'] = setidentity_detailsd3925,
        codeStates['national_id'] = national_id30319,
        codeStates['setnational_id'] = setnational_id30319,
        codeStates['passport_number'] = passport_number9b4e5,
        codeStates['setpassport_number'] = setpassport_number9b4e5,
        codeStates['biometric_id'] = biometric_id11ad8,
        codeStates['setbiometric_id'] = setbiometric_id11ad8,
  ...
  const identity_details_group6a6feRef = useRef<any>(null);
  const handleClearSearch = () => {
    identity_details_group6a6feRef.current?.setSearchParams();
    identity_details_group6a6feRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(identity_details_group6a6fe) && Object.keys(identity_details_group6a6fe)?.length>0)
      {
        setidentity_details_group6a6fe({})
      }
    }else 
      prevRefreshRef.current= true
  }, [identity_details_group6a6feProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"identity_details_group"}))
        }}
    >
          {allowedControls.includes("identity_details") ?<Textidentity_details   /* d3925 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("national_id") ?<TextInputnational_id   /* 30319 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("passport_number") ?<TextInputpassport_number   /* 9b4e5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("biometric_id") ?<TextInputbiometric_id   /* 11ad8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
