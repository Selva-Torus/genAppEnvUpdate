...
const TextInputpassport_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|125377fdc3004c1894d23350f816a6fe|b5cf0e19d2dd4bcfaa3fb7831a99b4e5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,passport_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setidentity_details_group6a6fe((prev: any) => ({ ...prev, passport_number: +e.target.value }));
    }
    else{
    setidentity_details_group6a6fe((prev: any) => ({ ...prev, passport_number: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "125377fdc3004c1894d23350f816a6fe",
        "b5cf0e19d2dd4bcfaa3fb7831a99b4e5"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1",
      //     componentId: "125377fdc3004c1894d23350f816a6fe",
      //     controlId: "b5cf0e19d2dd4bcfaa3fb7831a99b4e5",
      //     isTable: false,
  ...
  const identity_details_group6a6feRef = useRef<any>(identity_details_group6a6fe);
  useEffect(() => { identity_details_group6a6feRef.current = identity_details_group6a6fe; }, [identity_details_group6a6fe]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "b5cf0e19d2dd4bcfaa3fb7831a99b4e5") {
        handleChange({target:{value:identity_details_group6a6feRef?.current?.passport_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b5cf0e19d2dd4bcfaa3fb7831a99b4e5") {
        handleBlur({target:{value:identity_details_group6a6feRef?.current?.passport_number||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setidentity_details_group6a6fe((pre:any)=>({...pre,passport_number:dfd_employees_v1Props.data[0]?.passport_number}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (passport_number9b4e5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={identity_details_group6a6fe?.passport_number||""}
         disabled= {passport_number9b4e5?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.passport_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
