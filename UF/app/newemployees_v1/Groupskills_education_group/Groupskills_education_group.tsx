...
import Groupskills_group  from "../Groupskills_group/Groupskills_group";
import Groupeducation_group  from "../Groupeducation_group/Groupeducation_group";
import Groupcert_group  from "../Groupcert_group/Groupcert_group";
import Groupfamily_detail_group  from "../Groupfamily_detail_group/Groupfamily_detail_group";
...
import Textskills_education  from "./Textskills_education";
import Dividerfamily_divider  from "./Dividerfamily_divider";
...
const Groupskills_education_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {skills_educationb5256, setskills_educationb5256}= useContext(TotalContext) as TotalContextProps;
  ...
  const {family_divider948ae, setfamily_divider948ae}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7191df944fc64227b6ae630ba4dcb53a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setskills_education_groupcb53aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("skills_education")){
        setskills_educationb5256((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_educationb5256?.isDisabled==null)
      {
        setskills_educationb5256((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skills_group")){
        setskills_group36679((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_group36679?.isDisabled==null)
      {
        setskills_group36679((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("education_group")){
        seteducation_group70757((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education_group70757?.isDisabled==null)
      {
        seteducation_group70757((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_divider")){
        setfamily_divider948ae((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_divider948ae?.isDisabled==null)
      {
        setfamily_divider948ae((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_group")){
        setcert_groupedb63((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_groupedb63?.isDisabled==null)
      {
        setcert_groupedb63((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_detail_group")){
        setfamily_detail_group800b7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_detail_group800b7?.isDisabled==null)
      {
        setfamily_detail_group800b7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['skills_education'] = skills_educationb5256,
        codeStates['setskills_education'] = setskills_educationb5256,
        ...
        codeStates['family_divider'] = family_divider948ae,
        codeStates['setfamily_divider'] = setfamily_divider948ae,
  ...
  const skills_education_groupcb53aRef = useRef<any>(null);
  const handleClearSearch = () => {
    skills_education_groupcb53aRef.current?.setSearchParams();
    skills_education_groupcb53aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(skills_education_groupcb53a) && Object.keys(skills_education_groupcb53a)?.length>0)
      {
        setskills_education_groupcb53a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [skills_education_groupcb53aProps?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"skills_education_group"}))
        }}
    >
        {allowedComponent.includes("skills_group")  &&<Groupskills_group  
        ...
        {allowedComponent.includes("education_group")  &&<Groupeducation_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("cert_group")  &&<Groupcert_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("family_detail_group")  &&<Groupfamily_detail_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("skills_education") ?<Textskills_education   /* b5256 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("family_divider") ?<Dividerfamily_divider   /* 948ae */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupskills_education_group
