...
const TextInputproficiency_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newEmployees:AFVK:v1|77b5700fc0f34498a59bda374939f89a|ba1e68e9a10f4eeba589207ba0dfea56"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newEmployees_v1:{...pre?.newEmployees_v1,proficiency_level:undefined}}));
    if(dynamicStateandType.type=="number"){
    setskill9f89a_13((prev: any) => ({ ...prev, proficiency_level: +e.target.value }));
    }
    else{
    setskill9f89a_13((prev: any) => ({ ...prev, proficiency_level: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ba1e68e9a10f4eeba589207ba0dfea56"
      );
      ...
      //     controlId: "ba1e68e9a10f4eeba589207ba0dfea56",
      //     isTable: false,
  ...
  const skill9f89a_13Ref = useRef<any>(skill9f89a_13);
  useEffect(() => { skill9f89a_13Ref.current = skill9f89a_13; }, [skill9f89a_13]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "ba1e68e9a10f4eeba589207ba0dfea56") {
        handleChange({target:{value:skill9f89a_13Ref?.current?.proficiency_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ba1e68e9a10f4eeba589207ba0dfea56") {
        handleBlur({target:{value:skill9f89a_13Ref?.current?.proficiency_level||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskill9f89a_13((pre:any)=>({...pre,proficiency_level:dfd_skilljson_v1Props.data[0]?.proficiency_level}));
    }
  }
  },[dfd_skilljson_v1Props?.setSearchFilters])
  if (proficiency_levelfea56?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skill9f89a_13?.proficiency_level||""}
         disabled= {proficiency_levelfea56?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='Expert'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newEmployees_v1?.proficiency_level ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
