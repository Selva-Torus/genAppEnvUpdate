...
const Cardpendingcard = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setmanager_group41477((pre:any)=>({...pre,pendingcard:dfd_card_data_v1Props[0]?.pendingcard}));
    }
  ...
  if (pendingcard727e3?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `11 / 15`,gridRow: `1 / 29`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        label="Pending"
        disabled= {pendingcard727e3?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {manager_group41477?.pendingcard?manager_group41477?.pendingcard:"0"}
      </Card>
      </div>
  )
}

export default Cardpendingcard
