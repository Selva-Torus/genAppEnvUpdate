...
const Carddailyexpenses = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setmanager_group41477((pre:any)=>({...pre,dailyexpenses:dfd_card_data_v1Props[0]?.dailyexpenses}));
    }
  ...
  if (dailyexpensese7cda?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `16 / 20`,gridRow: `1 / 29`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        disabled= {dailyexpensese7cda?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {manager_group41477?.dailyexpenses?manager_group41477?.dailyexpenses:"0"}
      </Card>
      </div>
  )
}

export default Carddailyexpenses
