...
const Cardapprovedcard = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setmanager_group41477((pre:any)=>({...pre,approvedcard:dfd_card_data_v1Props[0]?.approvedcard}));
    }
  ...
  if (approvedcard75ed7?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 5`,gridRow: `1 / 29`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        icon="MdCheck"
        label="Approved"
        disabled= {approvedcard75ed7?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {manager_group41477?.approvedcard?manager_group41477?.approvedcard:"0"}
      </Card>
      </div>
  )
}

export default Cardapprovedcard
