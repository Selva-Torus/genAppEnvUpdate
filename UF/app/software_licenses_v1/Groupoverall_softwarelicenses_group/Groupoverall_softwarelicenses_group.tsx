...
import Groupsoftware_licenses_table  from "../Groupsoftware_licenses_table/Groupsoftware_licenses_table";
...
const Groupoverall_softwarelicenses_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupOverallSoftwarelicensesGroup',
    token
  );
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverall_softwarelicenses_group04cbaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("icon_text_group")){
        seticon_text_group44cf7({...icon_text_group44cf7,isDisabled:true});

    }else
    {
      if(icon_text_group44cf7?.isDisabled==null)
      {
        seticon_text_group44cf7({...icon_text_group44cf7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("search")){
        setsearchb475f({...searchb475f,isDisabled:true});

    }else
    {
      if(searchb475f?.isDisabled==null)
      {
        setsearchb475f({...searchb475f,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_license")){
        setadd_license9d3d9({...add_license9d3d9,isDisabled:true});

    }else
    {
      if(add_license9d3d9?.isDisabled==null)
      {
        setadd_license9d3d9({...add_license9d3d9,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("software_licenses_table")){
        setsoftware_licenses_table75a5d({...software_licenses_table75a5d,isDisabled:true});

    }else
    {
      if(software_licenses_table75a5d?.isDisabled==null)
      {
        setsoftware_licenses_table75a5d({...software_licenses_table75a5d,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const overall_softwarelicenses_group04cbaRef = useRef<any>(null);
  const handleClearSearch = () => {
    overall_softwarelicenses_group04cbaRef.current?.setSearchParams();
    overall_softwarelicenses_group04cbaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overall_softwarelicenses_group04cba) && Object.keys(overall_softwarelicenses_group04cba)?.length>0)
      {
        setoverall_softwarelicenses_group04cba({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overall_softwarelicenses_group04cbaProps?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedComponent.includes("software_licenses_table")  &&<Groupsoftware_licenses_table  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupoverall_softwarelicenses_group
