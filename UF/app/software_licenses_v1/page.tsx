...
const Groupoverall_softwarelicenses_group = dynamic(() => import("./Groupoverall_softwarelicenses_group/Groupoverall_softwarelicenses_group"), { ssr: false });


export default function PageSoftwareLicensesV1() {
  ...
  const [checkoverall_softwarelicenses_group,setCheckoverall_softwarelicenses_group,]=useState<boolean>(false);
  const [checkicon_text_group,setCheckicon_text_group,]=useState<boolean>(false);
  const [checksoftware_licenses_table,setChecksoftware_licenses_table,]=useState<boolean>(false);
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setassetsoftwarelicenses_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    await assetsoftwarelicenses_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'overall_softwarelicenses_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckoverall_softwarelicenses_group(true)
            }
            if(nodes?.groupName == 'icon_text_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckicon_text_group(true)
            }
            if(nodes?.groupName == 'software_licenses_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setChecksoftware_licenses_table(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setassetsoftwarelicenses_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(assetsoftwarelicenses_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(assetsoftwarelicenses_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkoverall_softwarelicenses_group && initialLoad &&<Groupoverall_softwarelicenses_group
