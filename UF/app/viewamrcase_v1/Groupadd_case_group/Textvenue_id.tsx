...
const Textvenue_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {venue_id063aaProps, setvenue_id063aaProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setadd_case_groupbe1de((pre: any) => ({
          ...pre,
          venue_id: venue_id063aaProps?.filteredData?.length > 0
            ? venue_id063aaProps?.filteredData[0]?.venue_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_groupbe1de.venue_id){
        setadd_case_groupbe1de((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[venue_id063aa?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_groupbe1de.venue_id){
    setadd_case_groupbe1de((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!venue_id063aaProps?.filterProps) return;
    handleMapperValue(venue_id063aaProps?.filterProps,venue_id063aaProps?.filterFlag);
  },[venue_id063aaProps?.filterProps])

  if (venue_id063aa?.isHidden) {
    return <></>
  }
