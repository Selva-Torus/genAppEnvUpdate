...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,last_payment_date:undefined}}));
  if (!date) {
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, last_payment_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_groupe3c1b((prev: any) => ({ ...prev, last_payment_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "a6d37ea659b5466d937ddb35fd2500eb"
      );
...
useEffect(()=>{
 ...
 },[last_payment_date500eb?.refresh])
...
if (last_payment_date500eb?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_groupe3c1b?.last_payment_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {last_payment_date500eb?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Last Payment"
      dateValidation=""
      validationState={validate?.viewAMRCase_v1?.last_payment_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
