...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,dob:undefined}}));
  if (!date) {
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, dob: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_groupe3c1b((prev: any) => ({ ...prev, dob: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "ce8271aab70d4771807c5239ad3ea900"
      );
...
return (
  ...
  style={{gridColumn: `13 / 25`,gridRow: `25 / 37`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_groupe3c1b?.dob}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {dobea900?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="DOB"
      dateValidation=""
      validationState={validate?.viewAMRCase_v1?.dob ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
