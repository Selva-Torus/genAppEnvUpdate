...
const TextInputssn_masked = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|91b13e4c1ee41bd2cc4315ebf2ce3c1b|6bf082464c1f70a37c9251000eb24ce0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,ssn_masked:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, ssn_masked: +e.target.value }));
    }
    else{
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, ssn_masked: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6bf082464c1f70a37c9251000eb24ce0"
      );
      ...
      //     controlId: "6bf082464c1f70a37c9251000eb24ce0",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6bf082464c1f70a37c9251000eb24ce0") {
        handleChange({target:{value:case_information_groupe3c1bRef?.current?.ssn_masked||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6bf082464c1f70a37c9251000eb24ce0") {
        handleBlur({target:{value:case_information_groupe3c1bRef?.current?.ssn_masked||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_groupe3c1b((pre:any)=>({...pre,ssn_masked:dfd_addcase_v1Props.data[0]?.ssn_masked}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (ssn_masked24ce0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_groupe3c1b?.ssn_masked||""}
         disabled= {ssn_masked24ce0?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="SSN"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.ssn_masked ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
