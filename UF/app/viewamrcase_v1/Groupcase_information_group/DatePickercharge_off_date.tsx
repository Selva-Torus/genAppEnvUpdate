...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,charge_off_date:undefined}}));
  if (!date) {
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, charge_off_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_groupe3c1b((prev: any) => ({ ...prev, charge_off_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "325bf460277c8f7209e22dde293d3231"
      );
...
if (charge_off_dated3231?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_groupe3c1b?.charge_off_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {charge_off_dated3231?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Charge Off Date"
      dateValidation=""
      validationState={validate?.viewAMRCase_v1?.charge_off_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
