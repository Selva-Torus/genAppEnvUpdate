...
const TextInputcase_display_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|91b13e4c1ee41bd2cc4315ebf2ce3c1b|38e19068a74c8ef33a04b3d728cb53b9"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,case_display_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, case_display_id: +e.target.value }));
    }
    else{
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, case_display_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "38e19068a74c8ef33a04b3d728cb53b9"
      );
      ...
      //     controlId: "38e19068a74c8ef33a04b3d728cb53b9",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "38e19068a74c8ef33a04b3d728cb53b9") {
        handleChange({target:{value:case_information_groupe3c1bRef?.current?.case_display_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "38e19068a74c8ef33a04b3d728cb53b9") {
        handleBlur({target:{value:case_information_groupe3c1bRef?.current?.case_display_id||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_groupe3c1b((pre:any)=>({...pre,case_display_id:dfd_addcase_v1Props.data[0]?.case_display_id}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (case_display_idb53b9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_groupe3c1b?.case_display_id||""}
         disabled= {case_display_idb53b9?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.case_display_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
