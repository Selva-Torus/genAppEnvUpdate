...
const TextInputcreditor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|91b13e4c1ee41bd2cc4315ebf2ce3c1b|758ac9656611b7a92dd2bff831262479"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,creditor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, creditor_name: +e.target.value }));
    }
    else{
    setcase_information_groupe3c1b((prev: any) => ({ ...prev, creditor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "758ac9656611b7a92dd2bff831262479"
      );
      ...
      //     controlId: "758ac9656611b7a92dd2bff831262479",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "758ac9656611b7a92dd2bff831262479") {
        handleChange({target:{value:case_information_groupe3c1bRef?.current?.creditor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "758ac9656611b7a92dd2bff831262479") {
        handleBlur({target:{value:case_information_groupe3c1bRef?.current?.creditor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_groupe3c1b((pre:any)=>({...pre,creditor_name:dfd_addcase_v1Props.data[0]?.creditor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (creditor_name62479?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_groupe3c1b?.creditor_name||""}
         disabled= {creditor_name62479?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Creditor"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.creditor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
