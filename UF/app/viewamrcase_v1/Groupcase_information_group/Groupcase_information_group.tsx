...
const Groupcase_information_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "91b13e4c1ee41bd2cc4315ebf2ce3c1b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcase_information_groupe3c1bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("case_info_text")){
        setcase_info_textd4267((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_info_textd4267?.isDisabled==null)
      {
        setcase_info_textd4267((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_name")){
        setdebtor_name83b58((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debtor_name83b58?.isDisabled==null)
      {
        setdebtor_name83b58((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_display_id")){
        setcase_display_idb53b9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_display_idb53b9?.isDisabled==null)
      {
        setcase_display_idb53b9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ssn_masked")){
        setssn_masked24ce0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ssn_masked24ce0?.isDisabled==null)
      {
        setssn_masked24ce0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dob")){
        setdobea900((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dobea900?.isDisabled==null)
      {
        setdobea900((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address")){
        setaddress4e81d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address4e81d?.isDisabled==null)
      {
        setaddress4e81d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("creditor_name")){
        setcreditor_name62479((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(creditor_name62479?.isDisabled==null)
      {
        setcreditor_name62479((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("charge_off_date")){
        setcharge_off_dated3231((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(charge_off_dated3231?.isDisabled==null)
      {
        setcharge_off_dated3231((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("last_payment_date")){
        setlast_payment_date500eb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(last_payment_date500eb?.isDisabled==null)
      {
        setlast_payment_date500eb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("divider")){
        setdivider772d9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(divider772d9?.isDisabled==null)
      {
        setdivider772d9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("card_group")){
        setcard_group7fa83((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(card_group7fa83?.isDisabled==null)
      {
        setcard_group7fa83((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("sol_expiry_date")){
        setsol_expiry_date69782((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(sol_expiry_date69782?.isDisabled==null)
      {
        setsol_expiry_date69782((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const case_information_groupe3c1bRef = useRef<any>(null);
  const handleClearSearch = () => {
    case_information_groupe3c1bRef.current?.setSearchParams();
    case_information_groupe3c1bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(case_information_groupe3c1b) && Object.keys(case_information_groupe3c1b)?.length>0)
      {
        setcase_information_groupe3c1b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [case_information_groupe3c1bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"case_information_group"}))
        }}
    >
        {allowedComponent.includes("card_group")  &&<Groupcard_group  
          ...
          {allowedControls.includes("case_info_text") ?<Textcase_info_text   /* d4267 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("debtor_name") ?<TextInputdebtor_name   /* 83b58 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("case_display_id") ?<TextInputcase_display_id   /* b53b9 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ssn_masked") ?<TextInputssn_masked   /* 24ce0 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dob") ?<DatePickerdob   /* ea900 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address") ?<TextAreaaddress   /* 4e81d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("creditor_name") ?<TextInputcreditor_name   /* 62479 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("charge_off_date") ?<DatePickercharge_off_date   /* d3231 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("last_payment_date") ?<DatePickerlast_payment_date   /* 500eb */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("divider") ?<Dividerdivider   /* 772d9 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("sol_expiry_date") ?<DatePickersol_expiry_date   /* 69782 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
