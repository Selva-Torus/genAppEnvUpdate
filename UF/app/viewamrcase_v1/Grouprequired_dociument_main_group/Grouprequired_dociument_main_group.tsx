...
const Grouprequired_dociument_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {reeq_doc_text78b91, setreeq_doc_text78b91}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e09e8d8465e490cd6eef74512046f022");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_main_group6f022Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reeq_doc_text")){
        setreeq_doc_text78b91((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reeq_doc_text78b91?.isDisabled==null)
      {
        setreeq_doc_text78b91((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_table")){
        setdoc_table8bfa1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_table8bfa1?.isDisabled==null)
      {
        setdoc_table8bfa1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['reeq_doc_text'] = reeq_doc_text78b91,
        codeStates['setreeq_doc_text'] = setreeq_doc_text78b91,
  ...
  const required_dociument_main_group6f022Ref = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_main_group6f022Ref.current?.setSearchParams();
    required_dociument_main_group6f022Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_main_group6f022) && Object.keys(required_dociument_main_group6f022)?.length>0)
      {
        setrequired_dociument_main_group6f022({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_main_group6f022Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_main_group"}))
        }}
    >
        {allowedComponent.includes("doc_table")  &&<Groupdoc_table  
          ...
          {allowedControls.includes("reeq_doc_text") ?<Textreeq_doc_text   /* 78b91 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
