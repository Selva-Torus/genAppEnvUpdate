...
const Groupintrest_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {intrest_text58114, setintrest_text58114}= useContext(TotalContext) as TotalContextProps;
  const {interest42832, setinterest42832}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "053d86bd199841ae8e8974e864644b4d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setintrest_group44b4dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("intrest_text")){
        setintrest_text58114((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_text58114?.isDisabled==null)
      {
        setintrest_text58114((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("interest")){
        setinterest42832((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(interest42832?.isDisabled==null)
      {
        setinterest42832((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['intrest_text'] = intrest_text58114,
        codeStates['setintrest_text'] = setintrest_text58114,
        codeStates['interest'] = interest42832,
        codeStates['setinterest'] = setinterest42832,
  ...
  const intrest_group44b4dRef = useRef<any>(null);
  const handleClearSearch = () => {
    intrest_group44b4dRef.current?.setSearchParams();
    intrest_group44b4dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(intrest_group44b4d) && Object.keys(intrest_group44b4d)?.length>0)
      {
        setintrest_group44b4d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [intrest_group44b4dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"intrest_group"}))
        }}
    >
          {allowedControls.includes("intrest_text") ?<Textintrest_text   /* 58114 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("interest") ?<Textinterest   /* 42832 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
