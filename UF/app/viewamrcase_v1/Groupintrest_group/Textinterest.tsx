...
const Textinterest = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {interest42832Props, setinterest42832Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setintrest_group44b4d((pre: any) => ({
          ...pre,
          interest: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.interest
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setintrest_group44b4d((pre: any) => ({
          ...pre,
          interest: interest42832Props?.filteredData?.length > 0
            ? interest42832Props?.filteredData[0]?.interest
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !intrest_group44b4d.interest){
        setintrest_group44b4d((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[interest42832?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !intrest_group44b4d.interest){
    setintrest_group44b4d((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!interest42832Props?.filterProps) return;
    handleMapperValue(interest42832Props?.filterProps,interest42832Props?.filterFlag);
  },[interest42832Props?.filterProps])

  if (interest42832?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.interest : (intrest_group44b4d?.interest || ""))}
</Text>
  </div>
  )
}

export default Textinterest
