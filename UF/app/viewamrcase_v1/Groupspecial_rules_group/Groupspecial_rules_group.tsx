...
const Groupspecial_rules_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {special_rules_textda90e, setspecial_rules_textda90e}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "71cc1b2ebb434b32851abca4d9af22ab");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setspecial_rules_groupf22abProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("special_rules_text")){
        setspecial_rules_textda90e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_textda90e?.isDisabled==null)
      {
        setspecial_rules_textda90e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules96aec((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules96aec?.isDisabled==null)
      {
        setspecial_rules96aec((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['special_rules_text'] = special_rules_textda90e,
        codeStates['setspecial_rules_text'] = setspecial_rules_textda90e,
  ...
  const special_rules_groupf22abRef = useRef<any>(null);
  const handleClearSearch = () => {
    special_rules_groupf22abRef.current?.setSearchParams();
    special_rules_groupf22abRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(special_rules_groupf22ab) && Object.keys(special_rules_groupf22ab)?.length>0)
      {
        setspecial_rules_groupf22ab({})
      }
    }else 
      prevRefreshRef.current= true
  }, [special_rules_groupf22abProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"special_rules_group"}))
        }}
    >
        {allowedComponent.includes("special_rules")  &&<Groupspecial_rules  
          ...
          {allowedControls.includes("special_rules_text") ?<Textspecial_rules_text   /* da90e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
