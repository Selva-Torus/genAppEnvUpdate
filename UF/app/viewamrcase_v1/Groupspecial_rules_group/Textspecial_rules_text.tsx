...
const Textspecial_rules_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {special_rules_textda90eProps, setspecial_rules_textda90eProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[special_rules_textda90e?.refresh])

  if (special_rules_textda90e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Special Rules")}
</Text>
  </div>
  )
}

export default Textspecial_rules_text
