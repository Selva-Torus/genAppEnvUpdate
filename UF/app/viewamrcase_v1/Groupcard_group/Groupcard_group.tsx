...
const Groupcard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9a6c4b50656c43ea962745eefe87fa83");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcard_group7fa83Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_group")){
        setprincipal_groupde6dd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_groupde6dd?.isDisabled==null)
      {
        setprincipal_groupde6dd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("intrest_group")){
        setintrest_group44b4d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_group44b4d?.isDisabled==null)
      {
        setintrest_group44b4d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees_group")){
        setfees_groupee523((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_groupee523?.isDisabled==null)
      {
        setfees_groupee523((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_group")){
        settotal_groupd3e06((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_groupd3e06?.isDisabled==null)
      {
        settotal_groupd3e06((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const card_group7fa83Ref = useRef<any>(null);
  const handleClearSearch = () => {
    card_group7fa83Ref.current?.setSearchParams();
    card_group7fa83Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card_group7fa83) && Object.keys(card_group7fa83)?.length>0)
      {
        setcard_group7fa83({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card_group7fa83Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"card_group"}))
        }}
    >
