...
const Groupdoc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "3c62b53b67123edde99bdc3fa168bfa1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_table8bfa1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_id394f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(attachment_id394f7?.isDisabled==null)
      {
        setattachment_id394f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_namef31ac((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_namef31ac?.isDisabled==null)
      {
        setdoc_namef31ac((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("view_button")){
        setview_button04be7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(view_button04be7?.isDisabled==null)
      {
        setview_button04be7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const doc_table8bfa1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_table8bfa1Ref.current?.setSearchParams();
    doc_table8bfa1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_table8bfa1) && Object.keys(doc_table8bfa1)?.length>0)
      {
        setdoc_table8bfa1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_table8bfa1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"doc_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tabledoc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_table8bfa1Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
