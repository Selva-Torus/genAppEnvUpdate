...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "cbf1d6fbe4a9c9ad9a90ec19104394f7",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "049c45d11e7867246e36c84d09bf31ac",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
...
const Tabledoc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, doc_table8bfa1 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, doc_table8bfa1 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, doc_table8bfa1 } = latestLockStateRef.current
    /// setdoc_table8bfa1Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(doc_table8bfa1.map((item: any) => item.doc_instance_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      doc_table8bfa1.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = doc_table8bfa1
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.doc_instance_id) ? item.doc_instance_id : null
          )
          .filter(Boolean)

        setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        throw err
      }
    }
    let index = Number(ids[ids.length - 1])
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(viewamrcase_v1Props.length > 0){
      for(let i=0;i< viewamrcase_v1Props.length;i++){
        if(viewamrcase_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:DocTable:AFVK:v1"){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["account_id"]:doc_table8bfa1Props?.presetValues?.account_id}
    ...
    if(!doc_table8bfa1Props?.presetValues || Object.keys(doc_table8bfa1Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '3c62b53b67123edde99bdc3fa168bfa1',
                elementId: '3c62b53b67123edde99bdc3fa168bfa1'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setdoc_table8bfa1(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in viewamrcase_v1?.doc_table && viewamrcase_v1?.doc_table[controller]?.itsHaveArtifact== true)
      {
        if(viewamrcase_v1?.doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(viewamrcase_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,doc_table:filteredData},{doc_table:viewamrcase_v1?.doc_table})
    ...
    if(await handleSecurity("view_button") &&nodeName?.toLowerCase()=="buttonview_button"){
      ...
      return (
        <React.Fragment>
        <Buttonview_button mainData={filteredData} lockedData={tempLockedData} controlData={controlData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} onSelectLock={setLockMode} rowIndex={index} currentSelectedIds={doc_table8bfa1Props?.selectedIds} tableName={tableName}/>
        </React.Fragment>
      )
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(doc_table8bfa1))
    {
      GetTableDetails()
    }
  }, [doc_table8bfa1])
  ...
  useEffect(() => {
    ...
    if (doc_table8bfa1Props.filterInitalLoad) return;

    const filterControllers = doc_table8bfa1Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['doc_table8bfa1']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['doc_table8bfa1']).filter((k: string) => jsonEntry['doc_table8bfa1'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(doc_table8bfa1Props.filterProps);
      return;
    }
    ...
    setdoc_table8bfa1Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(doc_table8bfa1Props.filterProps);

  }, [
    doc_table8bfa1Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    doc_table8bfa1Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(doc_table8bfa1Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(doc_table8bfa1Props.searchFilter) || {};
...
}, [doc_table8bfa1Props.searchFilter])
  ...
  useEffect(() => {
    if (doc_table8bfa1Props?.clearData === true) {
      if (myLockedIdsRef.current.length) {
        myLockedIdsRef.current.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setdoc_table8bfa1Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [doc_table8bfa1Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  }, [doc_table8bfa1Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = doc_table8bfa1Props?.selectedIds[doc_table8bfa1Props?.selectedIds?.length-1]
    if(Array.isArray(doc_table8bfa1) && doc_table8bfa1.length>0)
    {
      let data = doc_table8bfa1.find((data:any)=>(data?.doc_instance_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "doc_instance_id",
  ...
  useEffect(() => {
    if (doc_table8bfa1Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (doc_table8bfa1Props?.selectedIds?.length == 0){
      handleOnRowClick({},doc_table8bfa1Props?.selectedIds)
    }
  }, [doc_table8bfa1Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setdoc_table8bfa1Props((pre:any)=>({...pre, selectedIds:[row?.doc_instance_id]}))  
    onButtonSecurityHandle(data)
    ...
    let filterProps2:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData2 = await getFilterProps(filterProps2,{...add_case_groupbe1de,...header_groupc587e,...required_dociument_main_group6f022,...case_information_groupe3c1b,...card_group7fa83,...principal_groupde6dd,...intrest_group44b4d,...fees_groupee523,...total_groupd3e06,...venue_details_group1d734,...checklist_main_group32240,...checklist_tablee7dea,...special_rules_groupf22ab,...special_rules96aec,...doc_table8bfa1});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(doc_table8bfa1?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&doc_table8bfa1?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](doc_table8bfa1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_table8bfa1?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&doc_table8bfa1?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](doc_table8bfa1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_table8bfa1?.at(newIndex) ,currectPage:paginationData?.page});
      ...
      >
        <PageDocumentviewerpage2/>
          ...
          <div
            ...
            id={"3c62b53b67123edde99bdc3fa168bfa1"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableSorting={true}
              disable={disable}
              selectedIds={doc_table8bfa1Props?.selectedIds}  
