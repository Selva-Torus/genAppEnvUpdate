...
const Groupchecklist_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {valid_checklist_text6c6d2, setvalid_checklist_text6c6d2}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5a896a4ea58076cad94a32fd55832240");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_main_group32240Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valid_checklist_text")){
        setvalid_checklist_text6c6d2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_checklist_text6c6d2?.isDisabled==null)
      {
        setvalid_checklist_text6c6d2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_table")){
        setchecklist_tablee7dea((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_tablee7dea?.isDisabled==null)
      {
        setchecklist_tablee7dea((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['valid_checklist_text'] = valid_checklist_text6c6d2,
        codeStates['setvalid_checklist_text'] = setvalid_checklist_text6c6d2,
  ...
  const checklist_main_group32240Ref = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_main_group32240Ref.current?.setSearchParams();
    checklist_main_group32240Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_main_group32240) && Object.keys(checklist_main_group32240)?.length>0)
      {
        setchecklist_main_group32240({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_main_group32240Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_main_group"}))
        }}
    >
        {allowedComponent.includes("checklist_table")  &&<Groupchecklist_table  
          ...
          {allowedControls.includes("valid_checklist_text") ?<Textvalid_checklist_text   /* 6c6d2 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
