...
const Groupfees_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {fees_text67815, setfees_text67815}= useContext(TotalContext) as TotalContextProps;
  const {fees9a14f, setfees9a14f}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c99960c617444e578e1b97241e2ee523");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfees_groupee523Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("fees_text")){
        setfees_text67815((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_text67815?.isDisabled==null)
      {
        setfees_text67815((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees")){
        setfees9a14f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees9a14f?.isDisabled==null)
      {
        setfees9a14f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['fees_text'] = fees_text67815,
        codeStates['setfees_text'] = setfees_text67815,
        codeStates['fees'] = fees9a14f,
        codeStates['setfees'] = setfees9a14f,
  ...
  const fees_groupee523Ref = useRef<any>(null);
  const handleClearSearch = () => {
    fees_groupee523Ref.current?.setSearchParams();
    fees_groupee523Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(fees_groupee523) && Object.keys(fees_groupee523)?.length>0)
      {
        setfees_groupee523({})
      }
    }else 
      prevRefreshRef.current= true
  }, [fees_groupee523Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"fees_group"}))
        }}
    >
          {allowedControls.includes("fees_text") ?<Textfees_text   /* 67815 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("fees") ?<Textfees   /* 9a14f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
