...
const Textfees = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {fees9a14fProps, setfees9a14fProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setfees_groupee523((pre: any) => ({
          ...pre,
          fees: fees9a14fProps?.filteredData?.length > 0
            ? fees9a14fProps?.filteredData[0]?.fees
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !fees_groupee523.fees){
        setfees_groupee523((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !fees_groupee523.fees){
    setfees_groupee523((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!fees9a14fProps?.filterProps) return;
    handleMapperValue(fees9a14fProps?.filterProps,fees9a14fProps?.filterFlag);
  },[fees9a14fProps?.filterProps])

  if (fees9a14f?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.fees : (fees_groupee523?.fees || ""))}
</Text>
  </div>
  )
}
