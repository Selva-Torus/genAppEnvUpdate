...
export default function PageViewamrcaseV1({ onReady }: { onReady?: () => void } = {}) {
    ...
    async function addcase_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewamrcase_v1Props.length > 0){
          for(let i=0;i< viewamrcase_v1Props.length;i++){
            if(viewamrcase_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1"){
  ...
  useEffect(()=>{
    ...
    async function doctypenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewamrcase_v1Props.length > 0){
          for(let i=0;i< viewamrcase_v1Props.length;i++){
            if(viewamrcase_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:docTypeNameCombo:AFVK:v1"){
              // delete viewamrcase_v1Props[i].DFDkey;
              let temp=structuredClone(viewamrcase_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function amrchecklistcombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewamrcase_v1Props.length > 0){
          for(let i=0;i< viewamrcase_v1Props.length;i++){
            if(viewamrcase_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:amrChecklistCombo:AFVK:v1"){
              // delete viewamrcase_v1Props[i].DFDkey;
              let temp=structuredClone(viewamrcase_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function venuespecialrules_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let venuespecialrules_v1Body:te_refreshDto={
        ...
        if (encryptionFlagPage) {          
          venuespecialrules_v1Body["dpdKey"] = encryptionDpd;
          venuespecialrules_v1Body["method"] = encryptionMethod;
        }
        if(viewamrcase_v1Props.length > 0){
          for(let i=0;i< viewamrcase_v1Props.length;i++){
            if(viewamrcase_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:venueSpecialRules:AFVK:v1"){
              // delete viewamrcase_v1Props[i].DFDkey;
              let temp=structuredClone(viewamrcase_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
          ...
          venuespecialrules_v1Body['filterData'] = filterData;
        }
        const venuespecialrules_v1Data:any=await AxiosService.post("/te/eventEmitter",venuespecialrules_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=venuespecialrules_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(venuespecialrules_v1Data?.data?.dataset === 'Bulk Data Processing'){
          if(filterData.length>0){
            const api_paginationBody: api_paginationDto = {
          key: dstKey,
          count:parseInt(pagination?.count) || 10,
        ...
        setdfd_venuespecialrules_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_venuespecialrules_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (venuespecialrules_v1Data?.data?.dataset) {
           setdfd_venuespecialrules_v1Props(
              Array.isArray(venuespecialrules_v1Data?.data?.dataset?.data)
                 ? venuespecialrules_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
                    Object.entries(obj || {}).map(([key, value]) => [
                      key.toLowerCase(),
                      value
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.venuespecialrules_v1) {
      venuespecialrules_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.venuespecialrules_v1= true
  },[refetch?.venuespecialrules_v1])
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    const { getAftfactLevelRule } = await import("../utils/evaluateDecisionTable");
    let result :any =await getAftfactLevelRule(rule,data,allRuleData)
    setviewamrcase_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
    ...
    await venuespecialrules_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setviewamrcase_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(viewamrcase_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(viewamrcase_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
