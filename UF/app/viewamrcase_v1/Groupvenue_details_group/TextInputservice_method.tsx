...
const TextInputservice_method = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|7d99e3f90c038210537a6cdcd341d734|f20a38a447f82244684bd3fcc8f80ec2"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,service_method:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group1d734((prev: any) => ({ ...prev, service_method: +e.target.value }));
    }
    else{
    setvenue_details_group1d734((prev: any) => ({ ...prev, service_method: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f20a38a447f82244684bd3fcc8f80ec2"
      );
      ...
      //     controlId: "f20a38a447f82244684bd3fcc8f80ec2",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "f20a38a447f82244684bd3fcc8f80ec2") {
        handleChange({target:{value:venue_details_group1d734Ref?.current?.service_method||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f20a38a447f82244684bd3fcc8f80ec2") {
        handleBlur({target:{value:venue_details_group1d734Ref?.current?.service_method||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group1d734((pre:any)=>({...pre,service_method:dfd_addcase_v1Props.data[0]?.service_method}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (service_method80ec2?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group1d734?.service_method||""}
         disabled= {service_method80ec2?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.service_method ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
