...
const TextInputjudge_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.judge_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|7d99e3f90c038210537a6cdcd341d734|2519f544f5a7514b1b6d3c848905abc6"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'judge_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,judge_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group1d734((prev: any) => ({ ...prev, judge_name: +e.target.value }));
    }
    else{
    setvenue_details_group1d734((prev: any) => ({ ...prev, judge_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "2519f544f5a7514b1b6d3c848905abc6"
      );
      ...
      //     controlId: "2519f544f5a7514b1b6d3c848905abc6",
      //     isTable: false,
      //     from:"TextInputjudge_name",
      ...
      //   type={
      //     name:'judge_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "2519f544f5a7514b1b6d3c848905abc6") {
        handleChange({target:{value:venue_details_group1d734Ref?.current?.judge_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2519f544f5a7514b1b6d3c848905abc6") {
        handleBlur({target:{value:venue_details_group1d734Ref?.current?.judge_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group1d734((pre:any)=>({...pre,judge_name:dfd_addcase_v1Props.data[0]?.judge_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (judge_name5abc6?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group1d734?.judge_name||""}
         disabled= {judge_name5abc6?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Judge"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.judge_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputjudge_name
