...
const TextInputfiling_fee = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.filing_fee"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|7d99e3f90c038210537a6cdcd341d734|82d91b993fc8a4ebbaa62ab35a27fab8"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'filing_fee',type:"number"})
  ...
  const {county40b75, setcounty40b75}= useContext(TotalContext) as TotalContextProps;
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,filing_fee:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group1d734((prev: any) => ({ ...prev, filing_fee: +e.target.value }));
    }
    else{
    setvenue_details_group1d734((prev: any) => ({ ...prev, filing_fee: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['county'] = county40b75,
        codeStates['setcounty'] = setcounty40b75,
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "7d99e3f90c038210537a6cdcd341d734",
        "82d91b993fc8a4ebbaa62ab35a27fab8"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1",
      //     componentId: "7d99e3f90c038210537a6cdcd341d734",
      //     controlId: "82d91b993fc8a4ebbaa62ab35a27fab8",
      //     isTable: false,
      //     from:"TextInputfiling_fee",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'filing_fee', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'filing_fee',type:'text'};
      //   type={
      //     name:'filing_fee',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.filing_fee.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.filing_fee.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.filing_fee.type
      //   }
      ...
      //   type={
      //     name:'filing_fee',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filing_fee.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filing_fee.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filing_fee.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const venue_details_group1d734Ref = useRef<any>(venue_details_group1d734);
  useEffect(() => { venue_details_group1d734Ref.current = venue_details_group1d734; }, [venue_details_group1d734]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "82d91b993fc8a4ebbaa62ab35a27fab8") {
        handleChange({target:{value:venue_details_group1d734Ref?.current?.filing_fee||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "82d91b993fc8a4ebbaa62ab35a27fab8") {
        handleBlur({target:{value:venue_details_group1d734Ref?.current?.filing_fee||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group1d734((pre:any)=>({...pre,filing_fee:dfd_addcase_v1Props.data[0]?.filing_fee}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (filing_fee7fab8?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 25`,gridRow: `72 / 84`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group1d734?.filing_fee||""}
         disabled= {filing_fee7fab8?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Filing Fee"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.filing_fee ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputfiling_fee
