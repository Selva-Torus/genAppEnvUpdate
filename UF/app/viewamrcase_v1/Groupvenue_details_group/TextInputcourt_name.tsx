...
const TextInputcourt_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|7d99e3f90c038210537a6cdcd341d734|eb40eee0cbb624147ca47f228c427e21"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,court_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group1d734((prev: any) => ({ ...prev, court_name: +e.target.value }));
    }
    else{
    setvenue_details_group1d734((prev: any) => ({ ...prev, court_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "eb40eee0cbb624147ca47f228c427e21"
      );
      ...
      //     controlId: "eb40eee0cbb624147ca47f228c427e21",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "eb40eee0cbb624147ca47f228c427e21") {
        handleChange({target:{value:venue_details_group1d734Ref?.current?.court_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "eb40eee0cbb624147ca47f228c427e21") {
        handleBlur({target:{value:venue_details_group1d734Ref?.current?.court_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group1d734((pre:any)=>({...pre,court_name:dfd_addcase_v1Props.data[0]?.court_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (court_name27e21?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group1d734?.court_name||""}
         disabled= {court_name27e21?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Court"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.court_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
