...
const TextInputstate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCase:AFVK:v1|7d99e3f90c038210537a6cdcd341d734|974b8f90eab355ee22bc94a005d8a16f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCase_v1:{...pre?.viewAMRCase_v1,state:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group1d734((prev: any) => ({ ...prev, state: +e.target.value }));
    }
    else{
    setvenue_details_group1d734((prev: any) => ({ ...prev, state: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "974b8f90eab355ee22bc94a005d8a16f"
      );
      ...
      //     controlId: "974b8f90eab355ee22bc94a005d8a16f",
      //     isTable: false,
      //     from:"TextInputstate",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "974b8f90eab355ee22bc94a005d8a16f") {
        handleChange({target:{value:venue_details_group1d734Ref?.current?.state||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "974b8f90eab355ee22bc94a005d8a16f") {
        handleBlur({target:{value:venue_details_group1d734Ref?.current?.state||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group1d734((pre:any)=>({...pre,state:dfd_addcase_v1Props.data[0]?.state}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (state8a16f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 25`,gridRow: `10 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group1d734?.state||""}
         disabled= {state8a16f?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="State"
      errorMessage={error}
        validationState={validate?.viewAMRCase_v1?.state ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
