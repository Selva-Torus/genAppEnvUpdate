...
const Textprincipal = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {principald89b4Props, setprincipald89b4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setprincipal_groupde6dd((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setprincipal_groupde6dd((pre: any) => ({
          ...pre,
          principal: principald89b4Props?.filteredData?.length > 0
            ? principald89b4Props?.filteredData[0]?.principal
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !principal_groupde6dd.principal){
        setprincipal_groupde6dd((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[principald89b4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !principal_groupde6dd.principal){
    setprincipal_groupde6dd((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!principald89b4Props?.filterProps) return;
    handleMapperValue(principald89b4Props?.filterProps,principald89b4Props?.filterFlag);
  },[principald89b4Props?.filterProps])

  if (principald89b4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.principal : (principal_groupde6dd?.principal || ""))}
</Text>
  </div>
  )
}
