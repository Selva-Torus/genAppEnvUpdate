...
const Groupprincipal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {principal_text73a3d, setprincipal_text73a3d}= useContext(TotalContext) as TotalContextProps;
  const {principald89b4, setprincipald89b4}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c9f2b4b331dc4cfbb4738a6e7ddde6dd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprincipal_groupde6ddProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_text")){
        setprincipal_text73a3d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_text73a3d?.isDisabled==null)
      {
        setprincipal_text73a3d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("principal")){
        setprincipald89b4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principald89b4?.isDisabled==null)
      {
        setprincipald89b4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['principal_text'] = principal_text73a3d,
        codeStates['setprincipal_text'] = setprincipal_text73a3d,
        codeStates['principal'] = principald89b4,
        codeStates['setprincipal'] = setprincipald89b4,
  ...
  const principal_groupde6ddRef = useRef<any>(null);
  const handleClearSearch = () => {
    principal_groupde6ddRef.current?.setSearchParams();
    principal_groupde6ddRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(principal_groupde6dd) && Object.keys(principal_groupde6dd)?.length>0)
      {
        setprincipal_groupde6dd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [principal_groupde6ddProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"principal_group"}))
        }}
    >
          {allowedControls.includes("principal_text") ?<Textprincipal_text   /* 73a3d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("principal") ?<Textprincipal   /* d89b4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
