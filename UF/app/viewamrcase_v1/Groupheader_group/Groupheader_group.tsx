...
const Groupheader_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6579af9497d034811b9314c80cbc587e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setheader_groupc587eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("button_back")){
        setbutton_back1a912((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_back1a912?.isDisabled==null)
      {
        setbutton_back1a912((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("craete_header_text")){
        setcraete_header_texte958c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(craete_header_texte958c?.isDisabled==null)
      {
        setcraete_header_texte958c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const header_groupc587eRef = useRef<any>(null);
  const handleClearSearch = () => {
    header_groupc587eRef.current?.setSearchParams();
    header_groupc587eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(header_groupc587e) && Object.keys(header_groupc587e)?.length>0)
      {
        setheader_groupc587e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [header_groupc587eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcase_v1((pre:any)=>({...pre,_selectedGroup_:"header_group"}))
        }}
    >
        {        ((ruleData?.length>0 && "button_back" in ButtonGoRuleData)?ButtonGoRuleData["button_back"]:true) && 
          ...
          {allowedControls.includes("craete_header_text") ?<Textcraete_header_text   /* e958c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
