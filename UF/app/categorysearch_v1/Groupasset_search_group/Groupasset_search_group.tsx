...
const Groupasset_search_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7e7048961cc614a06ec648d85d3d84d5");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_search_groupd84d5Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("advancesearch")){
        setadvancesearchfab99({...advancesearchfab99,isDisabled:true});

    }else
    {
      if(advancesearchfab99?.isDisabled==null)
      {
        setadvancesearchfab99({...advancesearchfab99,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_search_groupd84d5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    asset_search_groupd84d5Ref.current?.setSearchParams();
    asset_search_groupd84d5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_search_groupd84d5) && Object.keys(asset_search_groupd84d5)?.length>0)
      {
        setasset_search_groupd84d5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_search_groupd84d5Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("advancesearch")?<AdvancedSearchadvancesearch /* fab99 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
