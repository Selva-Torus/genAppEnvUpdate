...
const Groupasset_search_group = dynamic(() => import("./Groupasset_search_group/Groupasset_search_group"), { ssr: false });


export default function PageCategorysearchV1() {
  ...
  const {categorysearch_v1, setcategorysearch_v1} = useContext(TotalContext) as TotalContextProps;
  const {categorysearch_v1Props, setcategorysearch_v1Props} = useContext(TotalContext) as TotalContextProps;
  const [checkasset_search_group,setCheckasset_search_group,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetcategory_v1DFD(pagination:any): Promise<void>{
        ...
        if(categorysearch_v1Props.length > 0){
          for(let i=0;i< categorysearch_v1Props.length;i++){
            if(categorysearch_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetCategory:AFVK:v1"){
              // delete categorysearch_v1Props[i].DFDkey;
              let temp=structuredClone(categorysearch_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setcategorysearch_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const { fetchBatchData } = await import("../utils/Orchestration");
    const data: any = await fetchBatchData(
      'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:categorySearch:AFVK:v1',
      [user],
      'pageCategorysearchV1',
      token
    )
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:categorySearch:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
    ...
    await assetcategory_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'asset_search_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckasset_search_group(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setcategorysearch_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(categorysearch_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(categorysearch_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkasset_search_group && initialLoad &&<Groupasset_search_group
