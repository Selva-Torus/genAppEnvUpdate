...
const Groupdoc_attached_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fd72de83de24cd5c26d4606276bb9604");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_attached_groupb9604Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("table_group")){
        settable_groupefcb8({...table_groupefcb8,isDisabled:true});

    }else
    {
      if(table_groupefcb8?.isDisabled==null)
      {
        settable_groupefcb8({...table_groupefcb8,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("documentuploadpanel")){
        setdocumentuploadpanel643f7({...documentuploadpanel643f7,isDisabled:true});

    }else
    {
      if(documentuploadpanel643f7?.isDisabled==null)
      {
        setdocumentuploadpanel643f7({...documentuploadpanel643f7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_add_doc")){
        setbutton_add_doc24b11({...button_add_doc24b11,isDisabled:true});

    }else
    {
      if(button_add_doc24b11?.isDisabled==null)
      {
        setbutton_add_doc24b11({...button_add_doc24b11,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("acat_id")){
        setacat_idf572e({...acat_idf572e,isDisabled:true});

    }else
    {
      if(acat_idf572e?.isDisabled==null)
      {
        setacat_idf572e({...acat_idf572e,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const doc_attached_groupb9604Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_attached_groupb9604Ref.current?.setSearchParams();
    doc_attached_groupb9604Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_attached_groupb9604) && Object.keys(doc_attached_groupb9604)?.length>0)
      {
        setdoc_attached_groupb9604({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_attached_groupb9604Props?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("documentuploadpanel") ?<DocumentUploadPaneldocumentuploadpanel   /* 643f7 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {        ((ruleData?.length>0 && "button_add_doc" in ButtonGoRuleData)?ButtonGoRuleData["button_add_doc"]:true) && 
          ...
          {allowedControls.includes("acat_id") ?<Textacat_id   /* f572e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
