...
const Textacat_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_categorydoctable_v1Props) && dfd_categorydoctable_v1Props && !table_groupefcb8.acat_id){
        settable_groupefcb8((pre:any)=>({...pre,acat_id:dfd_categorydoctable_v1Props[0]?.acat_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[acat_ida2d51?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_categorydoctable_v1Props) && !table_groupefcb8.acat_id){
    settable_groupefcb8((pre:any)=>({...pre,acat_id:dfd_categorydoctable_v1Props[0]?.acat_id}));
  }
  },[dfd_categorydoctable_v1Props])

  if (acat_ida2d51?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.acat_id : (table_groupefcb8?.acat_id || ""))}
</Text>
  </div>
  )
}
