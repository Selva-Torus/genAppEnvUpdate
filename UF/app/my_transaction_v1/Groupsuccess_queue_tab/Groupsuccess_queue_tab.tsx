 ...
 /////////////
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(success_queue_tabef582) &&
        Object.keys(success_queue_tabef582)?.length > 0
      ) {
        setsuccess_queue_tabef582({})
      }
    } else prevRefreshRef.current = true
  }, [success_queue_tabef582Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"success_queue_tab"}))
        }}
    >
