 ...
 /////////////
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(view_all_tab4a963) &&
        Object.keys(view_all_tab4a963)?.length > 0
      ) {
        setview_all_tab4a963({})
      }
    } else prevRefreshRef.current = true
  }, [view_all_tab4a963Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"view_all_tab"}))
        }}
    >
