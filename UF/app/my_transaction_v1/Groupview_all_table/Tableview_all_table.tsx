...
const Tableview_all_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, view_all_tablec9e87 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, view_all_tablec9e87 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, view_all_tablec9e87 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      view_all_tablec9e87.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.uuid){
          selectedData?.push(item)
          postIds.push(item.uuid)
      ...
      setview_all_tablec9e87Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,uuid:filtertedData[i]?.uuid}
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    ...
    return <RowActionComponent
      ...
      currentSelectedIds={view_all_tablec9e87Props?.selectedIds}
      skipUnlockRef={skipUnlockRef}
      tableName={tableName}
    />
  }, [allData, setRefetch, encryptionFlagCompData, view_all_tablec9e87Props?.selectedIds]);
////////////////////////
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (view_all_tablec9e87Props.filterInitalLoad) return;

    const filterControllers = view_all_tablec9e87Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['view_all_tablec9e87']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['view_all_tablec9e87']).filter((k: string) => jsonEntry['view_all_tablec9e87'][k] === false)
      : [];
    ...
    setview_all_tablec9e87Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(view_all_tablec9e87Props.filterProps);

  }, [
    view_all_tablec9e87Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(view_all_tablec9e87Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(view_all_tablec9e87Props.searchFilter) || {};
...
}, [view_all_tablec9e87Props.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = view_all_tablec9e87.find((data:any)=>(data?.uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "uuid",
        value: data["uuid"],
