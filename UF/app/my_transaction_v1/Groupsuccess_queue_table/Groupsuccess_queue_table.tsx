 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("product_code_success_queue")){
        setproduct_code_success_queue7c209((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(product_code_success_queue7c209?.isDisabled==null)
      {
        setproduct_code_success_queue7c209((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("channel_name_success_queue")){
        setchannel_name_success_queueeddaf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(channel_name_success_queueeddaf?.isDisabled==null)
      {
        setchannel_name_success_queueeddaf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_success_queue")){
        setuuid_success_queuec805b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_success_queuec805b?.isDisabled==null)
      {
        setuuid_success_queuec805b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_operational_pending")){
        setdr_account_operational_pending10a49((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_operational_pending10a49?.isDisabled==null)
      {
        setdr_account_operational_pending10a49((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    ...
    if(orchestrationData?.data?.readableControls.includes("cr_amount_success_queue")){
        setcr_amount_success_queueb80d4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_success_queueb80d4?.isDisabled==null)
      {
        setcr_amount_success_queueb80d4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_info_success_queue")){
        setremittance_info_success_queue2f950((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_info_success_queue2f950?.isDisabled==null)
      {
        setremittance_info_success_queue2f950((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status_success_queue")){
        setstatus_success_queue019a2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(status_success_queue019a2?.isDisabled==null)
      {
        setstatus_success_queue019a2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(success_queue_table63aae) &&
        Object.keys(success_queue_table63aae)?.length > 0
      ) {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"success_queue_table"}))
        }}
    >
