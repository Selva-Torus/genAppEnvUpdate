 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("product_code_failure_queue")){
        setproduct_code_failure_queue12297((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(product_code_failure_queue12297?.isDisabled==null)
      {
        setproduct_code_failure_queue12297((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("channel_name_failure_queue")){
        setchannel_name_failure_queue42953((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(channel_name_failure_queue42953?.isDisabled==null)
      {
        setchannel_name_failure_queue42953((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_failure_queue")){
        setuuid_failure_queue03c86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_failure_queue03c86?.isDisabled==null)
      {
        setuuid_failure_queue03c86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_failure_queue")){
        setdr_account_failure_queuef9d2d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_failure_queuef9d2d?.isDisabled==null)
      {
        setdr_account_failure_queuef9d2d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    ...
    if(orchestrationData?.data?.readableControls.includes("cr_amount_failure_queue")){
        setcr_amount_failure_queue57c4d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_failure_queue57c4d?.isDisabled==null)
      {
        setcr_amount_failure_queue57c4d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_info_failure_queue")){
        setremittance_info_failure_queue09d7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_info_failure_queue09d7a?.isDisabled==null)
      {
        setremittance_info_failure_queue09d7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status_failure_queue")){
        setstatus_failure_queue0aef8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(status_failure_queue0aef8?.isDisabled==null)
      {
        setstatus_failure_queue0aef8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(failure_queue_tablea476f) &&
        Object.keys(failure_queue_tablea476f)?.length > 0
      ) {
