...
const Grouptran_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  const buttonReadyRef = useRef<((id: string) => void) | null>(null);
  const triggerGuardRef = useRef(false);
  useEffect(() => {
    return () => {
      if (buttonReadyRef.current) {
        eventBus.off("buttonReady", buttonReadyRef.current);
        buttonReadyRef.current = null;
      }
    };
  }, []);
 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tran_tab_group")){
        settran_tab_group08b64((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tran_tab_group08b64?.isDisabled==null)
      {
        settran_tab_group08b64((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      const handler = (id: string) => {
        if (id === "searchcc244" && !triggerGuardRef.current) {
          triggerGuardRef.current = true;
          eventBus.off("buttonReady", handler);
          buttonReadyRef.current = null;
          eventBus.emit("triggerButton", "searchcc244");
        }
      };
      buttonReadyRef.current = handler;
      eventBus.on("buttonReady", handler);
      eventBus.emit("triggerButton", "searchcc244");
      // triggerExternalButtonClick
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(tran_main_group1dc7f) &&
        Object.keys(tran_main_group1dc7f)?.length > 0
      ) {
        settran_main_group1dc7f({})
      }
    } else prevRefreshRef.current = true
  }, [tran_main_group1dc7fProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"tran_main_group"}))
        }}
    >
