 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("product_code_return_queue")){
        setproduct_code_return_queuee5e11((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(product_code_return_queuee5e11?.isDisabled==null)
      {
        setproduct_code_return_queuee5e11((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("channel_name_return_queue")){
        setchannel_name_return_queuebdabb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(channel_name_return_queuebdabb?.isDisabled==null)
      {
        setchannel_name_return_queuebdabb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_return_queue")){
        setuuid_return_queue958c9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_return_queue958c9?.isDisabled==null)
      {
        setuuid_return_queue958c9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_return_queue")){
        setdr_account_return_queuee94b2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_return_queuee94b2?.isDisabled==null)
      {
        setdr_account_return_queuee94b2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    ...
    if(orchestrationData?.data?.readableControls.includes("cr_amount_return_queue")){
        setcr_amount_return_queue13fec((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_return_queue13fec?.isDisabled==null)
      {
        setcr_amount_return_queue13fec((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_info_return_queue")){
        setremittance_info_return_queuef37f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_info_return_queuef37f7?.isDisabled==null)
      {
        setremittance_info_return_queuef37f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status_return_queue")){
        setstatus_return_queue95903((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(status_return_queue95903?.isDisabled==null)
      {
        setstatus_return_queue95903((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(return_queue_table267f0) &&
        Object.keys(return_queue_table267f0)?.length > 0
      ) {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"return_queue_table"}))
        }}
    >
