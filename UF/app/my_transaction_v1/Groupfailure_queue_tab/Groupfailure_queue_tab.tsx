 ...
 /////////////
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(failure_queue_tab69f01) &&
        Object.keys(failure_queue_tab69f01)?.length > 0
      ) {
        setfailure_queue_tab69f01({})
      }
    } else prevRefreshRef.current = true
  }, [failure_queue_tab69f01Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransaction_v1((pre:any)=>({...pre,_selectedGroup_:"failure_queue_tab"}))
        }}
    >
