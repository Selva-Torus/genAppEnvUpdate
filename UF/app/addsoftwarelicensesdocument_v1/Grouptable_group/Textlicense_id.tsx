...
const Textlicense_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_softwaredoctable_v1Props && !dfd_softwaredoctable_v1Props.hasLogicCenter) {
        const api_paginationData: any = await AxiosService.post('/UF/pagination',
          {
            key: dfd_softwaredoctable_v1Props.dstKey,
            page: 1,
            count: 1
          },
          {
        ...
        settable_group7bc52((pre: any) => {
          return { ...pre, license_id: api_paginationData.data.records[0]?.license_id }
        })
        }
      }
      else{
      if(Array.isArray(dfd_softwaredoctable_v1Props) && dfd_softwaredoctable_v1Props && !table_group7bc52.license_id){
        settable_group7bc52((pre:any)=>({...pre,license_id:dfd_softwaredoctable_v1Props[0]?.license_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[license_idd34c8?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_softwaredoctable_v1Props) && !table_group7bc52.license_id){
    settable_group7bc52((pre:any)=>({...pre,license_id:dfd_softwaredoctable_v1Props[0]?.license_id}));
  }
  },[dfd_softwaredoctable_v1Props])

  if (license_idd34c8?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `7 / 25`,gridRow: `1 / 7`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.license_id : (table_group7bc52?.license_id || ""))}
</Text>
  </div>
  )
}
