...
const Textproduct_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_softwaredoctable_v1Props) && dfd_softwaredoctable_v1Props && !table_group7bc52.product_name){
        settable_group7bc52((pre:any)=>({...pre,product_name:dfd_softwaredoctable_v1Props[0]?.product_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[product_name405f8?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_softwaredoctable_v1Props) && !table_group7bc52.product_name){
    settable_group7bc52((pre:any)=>({...pre,product_name:dfd_softwaredoctable_v1Props[0]?.product_name}));
  }
  },[dfd_softwaredoctable_v1Props])

  if (product_name405f8?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.product_name : (table_group7bc52?.product_name || ""))}
</Text>
  </div>
  )
}
