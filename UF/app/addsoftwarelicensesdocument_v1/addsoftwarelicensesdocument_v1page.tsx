...
export default function PageAddsoftwarelicensesdocumentV1() {
  ...
  const [checksoftware_licenses_doc_table,setChecksoftware_licenses_doc_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      softwaredoctable_v1:false,
    });
    async function softwaredoctable_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let softwaredoctable_v1Body:te_refreshDto={
        ...
        if (encryptionFlagPage) {          
          softwaredoctable_v1Body["dpdKey"] = encryptionDpd;
          softwaredoctable_v1Body["method"] = encryptionMethod;
        }
        if(addsoftwarelicensesdocument_v1Props.length > 0){
          ...
          softwaredoctable_v1Body['filterData'] = filterData;
        }
        const softwaredoctable_v1Data:any=await AxiosService.post("/te/eventEmitter",softwaredoctable_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=softwaredoctable_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(softwaredoctable_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_softwaredoctable_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_softwaredoctable_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (softwaredoctable_v1Data?.data?.dataset) {
           setdfd_softwaredoctable_v1Props(
              Array.isArray(softwaredoctable_v1Data?.data?.dataset?.data)
                 ? softwaredoctable_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.softwaredoctable_v1) {
      softwaredoctable_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.softwaredoctable_v1= true
  },[refetch?.softwaredoctable_v1])
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    const { getAftfactLevelRule } = await import("../utils/evaluateDecisionTable");
    let result :any =await getAftfactLevelRule(rule,data,allRuleData)
    setaddsoftwarelicensesdocument_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    const data: any = await fetchBatchData(
      ...
      [user],
      'pageAddsoftwarelicensesdocumentV1',
      token
    )
    ...
    if (token) {
      ...
      try {
        ...
        try{
    await softwaredoctable_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'software_licenses_doc_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setChecksoftware_licenses_doc_table(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaddsoftwarelicensesdocument_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(addsoftwarelicensesdocument_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(addsoftwarelicensesdocument_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
