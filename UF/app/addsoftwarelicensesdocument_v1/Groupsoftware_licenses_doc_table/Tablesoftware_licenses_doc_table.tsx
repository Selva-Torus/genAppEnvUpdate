...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "e22fa71ef87e279ba79b15232261138d",
    "name": "Attachment Id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "52b9c8ad83615fdebb7fb5872173dcd4",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "82d7663f13633b64eeb1d0fa258698d2",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "85dfa068d4b13098635c2cacffb0acd9",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "c111f5c67f5e4c92afe4c22d2070c4db",
    "name": "Created By",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "a5e9d57592215705ddce865ad37265b6",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      if (ids.length == 0) {
        let keys:any
        setsoftware_licenses_doc_table265b6Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      software_licenses_doc_table265b6.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.license_id)
      ...
      //////////
      //////////
      setsoftware_licenses_doc_table265b6Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      setsoftware_licenses_doc_table265b6Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setsoftware_licenses_doc_table265b6Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(addsoftwarelicensesdocument_v1Props.length > 0){
      for(let i=0;i< addsoftwarelicensesdocument_v1Props.length;i++){
        if(addsoftwarelicensesdocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:softwareDocTable:AFVK:v1"){
          // delete addsoftwarelicensesdocument_v1Props[i].DFDkey;
          let temp=structuredClone(addsoftwarelicensesdocument_v1Props[i])
          delete temp?.DFDkey
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["license_id"]:software_licenses_doc_table265b6Props?.presetValues?.license_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!software_licenses_doc_table265b6Props?.presetValues || Object.keys(software_licenses_doc_table265b6Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addSoftwareLicensesDocument:AFVK:v1:UO', 
                nodeId: 'a5e9d57592215705ddce865ad37265b6',
                elementId: 'a5e9d57592215705ddce865ad37265b6'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addSoftwareLicensesDocument:AFVK:v1',
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setsoftware_licenses_doc_table265b6(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in addsoftwarelicensesdocument_v1?.software_licenses_doc_table && addsoftwarelicensesdocument_v1?.software_licenses_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(addsoftwarelicensesdocument_v1?.software_licenses_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(addsoftwarelicensesdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,software_licenses_doc_table:filteredData},{software_licenses_doc_table:addsoftwarelicensesdocument_v1?.software_licenses_doc_table})
          if(result?.software_licenses_doc_table?.[controller]?.show==true)
            return true
          else
            return false
        }
    ...
    if(await handleSecurity("bt_delete") &&nodeName?.toLowerCase()=="buttonbt_delete"){
      let tempLockedData:any = 
      {
        ...
        data:filteredData,
        primaryKeys:[filteredData?.license_id],
        lockMode: needLockingAndRule,
        ttl: needLockingAndRule?.ttl
      }
      return (
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(software_licenses_doc_table265b6))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (software_licenses_doc_table265b6Props.filterInitalLoad) return;

    const filterControllers = software_licenses_doc_table265b6Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['software_licenses_doc_table265b6']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['software_licenses_doc_table265b6']).filter((k: string) => jsonEntry['software_licenses_doc_table265b6'][k] === false)
      : [];
    ...
    setsoftware_licenses_doc_table265b6Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(software_licenses_doc_table265b6Props.filterProps);

  }, [
    software_licenses_doc_table265b6Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    software_licenses_doc_table265b6Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(software_licenses_doc_table265b6Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(software_licenses_doc_table265b6Props.searchFilter) || {};
...
}, [software_licenses_doc_table265b6Props.searchFilter])
  ...
  useEffect(() => {
    if (software_licenses_doc_table265b6Props?.clearData === true) {
      setsoftware_licenses_doc_table265b6([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setsoftware_licenses_doc_table265b6Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [software_licenses_doc_table265b6Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [software_licenses_doc_table265b6Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = software_licenses_doc_table265b6Props?.selectedIds[software_licenses_doc_table265b6Props?.selectedIds?.length-1]
    if(Array.isArray(software_licenses_doc_table265b6) && software_licenses_doc_table265b6.length>0)
    {
      let data = software_licenses_doc_table265b6[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "license_id",
  ...
  useEffect(() => {
    if (software_licenses_doc_table265b6Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (software_licenses_doc_table265b6Props?.selectedIds?.length == 0){
      handleOnRowClick({},software_licenses_doc_table265b6Props?.selectedIds)
    }
  }, [software_licenses_doc_table265b6Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    let copyFormhandlerData :any = {}
          //bindtran
    const pkCol = translatedColumns.find((col:any) => col.dfdName === "license_id"); // Find the column whose dfdName matches
    const result = allData.find((item:any) => item["license_id"] === data[pkCol?.id || "license_id"]);
          let bindData2 = filterByKeys(flattenKeepInner(allData[ids?.at(-1)]),document_viewer_group9a6ecProps?.controls);
          setdocument_viewer_group9a6ec((pre:any)=>({...pre, ...bindData2}))
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(software_licenses_doc_table265b6?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&software_licenses_doc_table265b6?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](software_licenses_doc_table265b6?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: software_licenses_doc_table265b6?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&software_licenses_doc_table265b6?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](software_licenses_doc_table265b6?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: software_licenses_doc_table265b6?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (software_licenses_doc_table265b6?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"a5e9d57592215705ddce865ad37265b6"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={software_licenses_doc_table265b6Props?.selectedIds}  
...
export default Tablesoftware_licenses_doc_table
