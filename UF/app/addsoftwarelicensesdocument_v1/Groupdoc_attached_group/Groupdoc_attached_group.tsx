...
const Groupdoc_attached_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ab4de620bb1c73cb8d7a6b98836c3d26");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_attached_groupc3d26Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("table_group")){
        settable_group7bc52({...table_group7bc52,isDisabled:true});

    }else
    {
      if(table_group7bc52?.isDisabled==null)
      {
        settable_group7bc52({...table_group7bc52,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("documentuploadpanel")){
        setdocumentuploadpanela3e1b({...documentuploadpanela3e1b,isDisabled:true});

    }else
    {
      if(documentuploadpanela3e1b?.isDisabled==null)
      {
        setdocumentuploadpanela3e1b({...documentuploadpanela3e1b,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_add_doc")){
        setbutton_add_doc7c1b7({...button_add_doc7c1b7,isDisabled:true});

    }else
    {
      if(button_add_doc7c1b7?.isDisabled==null)
      {
        setbutton_add_doc7c1b7({...button_add_doc7c1b7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("license_id")){
        setlicense_idbc5e1({...license_idbc5e1,isDisabled:true});

    }else
    {
      if(license_idbc5e1?.isDisabled==null)
      {
        setlicense_idbc5e1({...license_idbc5e1,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const doc_attached_groupc3d26Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_attached_groupc3d26Ref.current?.setSearchParams();
    doc_attached_groupc3d26Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_attached_groupc3d26) && Object.keys(doc_attached_groupc3d26)?.length>0)
      {
        setdoc_attached_groupc3d26({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_attached_groupc3d26Props?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("documentuploadpanel") ?<DocumentUploadPaneldocumentuploadpanel   /* a3e1b */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {        ((ruleData?.length>0 && "button_add_doc" in ButtonGoRuleData)?ButtonGoRuleData["button_add_doc"]:true) && 
          ...
          {allowedControls.includes("license_id") ?<Textlicense_id   /* bc5e1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
