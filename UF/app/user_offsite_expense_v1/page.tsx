...
import Groupoffsite_expense_user_table  from "./Groupoffsite_expense_user_table/Groupoffsite_expense_user_table";


export default function PageUserOffsiteExpenseV1() {
  const { isDark, isHighContrast, bgStyle, textStyle } : { isDark: boolean; isHighContrast: boolean; bgStyle: string; textStyle: string } = useTheme();
  const screenName:string = "user offsite expense";
  ...
  const {offsite_expense_user_table_v1Props, setoffsite_expense_user_table_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [checkoffsite_expense_user_table,setCheckoffsite_expense_user_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function claim_table_data_v1(pagination:any): Promise<void>{
        ...
        if(offsite_expense_user_table_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< offsite_expense_user_table_v1Props.length;i++){
            if(offsite_expense_user_table_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claim_table_data:AFVK:v1"){
              delete offsite_expense_user_table_v1Props[i].DFDkey;
              filterData.push(offsite_expense_user_table_v1Props[i])
            }           
          }
  ...
  return (
    ...
    {securityData[accessProfile]?.allowedGroups?.includes("offsite_expense_user_table") && initialLoad &&<Groupoffsite_expense_user_table 
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
