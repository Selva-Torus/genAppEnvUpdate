...
import PageMaintenancesearchpage2 from '@/app/maintenancesearch_v1/maintenancesearch_v1page';
...
const Buttonsearch = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['maintenancesearch_v1'] = maintenancesearch_v1Props,
        codeStates['setmaintenancesearch_v1'] = setmaintenancesearch_v1Props,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "d0743d34889f40948c244ce53b57f293"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "d0743d34889f40948c244ce53b57f293") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "search7f293");
    return () => {
  ...
  useEffect(()=>{
    if (!search7f293?.trigger) return;
      if(search7f293?.trigger){
      setsearch7f293((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setoverall_maintenance_group04cba((prev: any) => ({ ...prev, search: true }));
        //onClick

    // showArtifactAsModal
    let filterProps2:any =  [];
    let filterData2 = await getFilterProps(filterProps2,overall_maintenance_group04cba);
    setmaintenancesearch_v1Props([...filterData2 ]);
    setShowProfileAsModalOpen2(true);
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setoverall_maintenance_group04cba((prev: any) => ({ ...prev, search: false }));
 ...
 if (search7f293?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `17 / 21`,gridRow: `1 / 8`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showProfileAsModalOpen2 && hiddenModalForTrigger && (
          <div style={{ display: 'none' }}>
            <PageMaintenancesearchpage2/>
          </div>
        )}
      <Modal 
        ...
        showOverlay = {true}
        position = {"center"}
        modalName = "maintenancesearch"
        className='w-[40%] h-[] bg-gray-50 overflow-auto'
      >
        {!hiddenModalForTrigger && <PageMaintenancesearchpage2/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          disabled= {search7f293?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
