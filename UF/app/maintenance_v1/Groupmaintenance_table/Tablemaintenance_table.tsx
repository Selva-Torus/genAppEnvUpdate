...
let defaultColumns:any = [
  ...
  {
    "id": "maint_id",
    "nodeid": "07dac0fea5ed468884365783bb5d22c1",
    "name": "maint_id",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.maintenance_table && artifactRuleState?.maintenance_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.maintenance_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,maintenance_table:filteredData},{maintenance_table:artifactRuleState?.maintenance_table})
          if(result?.maintenance_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const Tablemaintenance_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setmaintenance_table75a5dProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setmaintenance_table75a5dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1:UO', 
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( maintenance_table75a5dProps?.presetValues&&Object.keys(maintenance_table75a5dProps?.presetValues).length > 0) {
          filtertedData = [maintenance_table75a5dProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setmaintenance_table75a5d(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={assetmaintenance_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (maintenance_table75a5dProps.filterInitalLoad) return;

    const filterControllers = maintenance_table75a5dProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['maintenance_table75a5d']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['maintenance_table75a5d']).filter((k: string) => jsonEntry['maintenance_table75a5d'][k] === false)
      : [];
    ...
    setmaintenance_table75a5dProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(maintenance_table75a5dProps.filterProps);

  }, [
    maintenance_table75a5dProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    maintenance_table75a5dProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(maintenance_table75a5dProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(maintenance_table75a5dProps.searchFilter) || {};
...
}, [maintenance_table75a5dProps.searchFilter])
  ...
  useEffect(() => {
    if (maintenance_table75a5dProps?.clearData === true) {
      setmaintenance_table75a5d([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setmaintenance_table75a5dProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [maintenance_table75a5dProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [maintenance_table75a5dProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = maintenance_table75a5dProps?.selectedIds[maintenance_table75a5dProps?.selectedIds?.length-1]
    if(Array.isArray(maintenance_table75a5d) && maintenance_table75a5d.length>0)
    {
      let data = maintenance_table75a5d[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "maint_id",
        value: data["maint_id"],
  ...
  useEffect(() => {
    if (maintenance_table75a5dProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (maintenance_table75a5dProps?.selectedIds?.length == 0){
      handleOnRowClick({},maintenance_table75a5dProps?.selectedIds)
    }
  }, [maintenance_table75a5dProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(maintenance_table75a5d?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&maintenance_table75a5d?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](maintenance_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: maintenance_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&maintenance_table75a5d?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](maintenance_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: maintenance_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (maintenance_table75a5d?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={maintenance_table75a5dProps?.selectedIds}  
...
export default Tablemaintenance_table
