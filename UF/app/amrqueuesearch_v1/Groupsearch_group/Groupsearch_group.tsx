...
const Groupsearch_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "48494de4ad3c18b8fe7daca69e7defa1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsearch_groupdefa1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("advancesearch")){
        setadvancesearch6059f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(advancesearch6059f?.isDisabled==null)
      {
        setadvancesearch6059f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const search_groupdefa1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    search_groupdefa1Ref.current?.setSearchParams();
    search_groupdefa1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(search_groupdefa1) && Object.keys(search_groupdefa1)?.length>0)
      {
        setsearch_groupdefa1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [search_groupdefa1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrqueuesearch_v1((pre:any)=>({...pre,_selectedGroup_:"search_group"}))
        }}
    >
        {allowedControls.includes("advancesearch")?<AdvancedSearchadvancesearch /* 6059f */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
