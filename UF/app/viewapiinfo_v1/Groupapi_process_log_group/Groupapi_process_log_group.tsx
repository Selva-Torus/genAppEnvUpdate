...
import Textapi_process_log_text  from "./Textapi_process_log_text";
...
const Groupapi_process_log_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {api_process_log_texte6f38, setapi_process_log_texte6f38}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "440e7a155547424ca070c954e1a192b0");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_process_log_group192b0Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_process_log_text")){
        setapi_process_log_texte6f38((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_process_log_texte6f38?.isDisabled==null)
      {
        setapi_process_log_texte6f38((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_process_log_table")){
        setapi_process_log_table5904eProps((pre:any)=>({...pre,...api_process_log_table5904e,isDisabled:true}));

    }else
    {
      if(api_process_log_table5904e?.isDisabled==null)
      {
        setapi_process_log_table5904eProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['api_process_log_text'] = api_process_log_texte6f38,
        codeStates['setapi_process_log_text'] = setapi_process_log_texte6f38,
  ...
  const api_process_log_group192b0Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_process_log_group192b0Ref.current?.setSearchParams();
    api_process_log_group192b0Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_process_log_group192b0) &&
        Object.keys(api_process_log_group192b0)?.length > 0
      ) {
        setapi_process_log_group192b0({})
      }
    } else prevRefreshRef.current = true
  }, [api_process_log_group192b0Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewapiinfo_v1((pre:any)=>({...pre,_selectedGroup_:"api_process_log_group"}))
        }}
    >
        {allowedComponent.includes("api_process_log_table")  &&<Groupapi_process_log_table  
          ...
          {allowedControls.includes("api_process_log_text") ?<Textapi_process_log_text   /* e6f38 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
