...
const Groupvob_api_info_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e5c37efc039748ec91f5c6659265fc53");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvob_api_info_group5fc53Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("group")){
        setgroup1f4ba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group1f4ba?.isDisabled==null)
      {
        setgroup1f4ba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_info_group")){
        setapi_info_group6ad41((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_info_group6ad41?.isDisabled==null)
      {
        setapi_info_group6ad41((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const vob_api_info_group5fc53Ref = useRef<any>(null);
  const handleClearSearch = () => {
    vob_api_info_group5fc53Ref.current?.setSearchParams();
    vob_api_info_group5fc53Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(vob_api_info_group5fc53) &&
        Object.keys(vob_api_info_group5fc53)?.length > 0
      ) {
        setvob_api_info_group5fc53({})
      }
    } else prevRefreshRef.current = true
  }, [vob_api_info_group5fc53Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewapiinfo_v1((pre:any)=>({...pre,_selectedGroup_:"vob_api_info_group"}))
        }}
    >
        ...
        {allowedComponent.includes("api_info_group")  &&<Groupapi_info_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupvob_api_info_group
