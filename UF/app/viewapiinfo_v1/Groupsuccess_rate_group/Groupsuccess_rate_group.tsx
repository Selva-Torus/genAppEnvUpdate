...
import Cardsuccess_rate_card  from "./Cardsuccess_rate_card";
...
const Groupsuccess_rate_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9eee764ad4484e4f8d086e2b923b6598");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsuccess_rate_groupb6598Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("success_rate_card")){
        setsuccess_rate_card0eba7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(success_rate_card0eba7?.isDisabled==null)
      {
        setsuccess_rate_card0eba7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const success_rate_groupb6598Ref = useRef<any>(null);
  const handleClearSearch = () => {
    success_rate_groupb6598Ref.current?.setSearchParams();
    success_rate_groupb6598Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(success_rate_groupb6598) &&
        Object.keys(success_rate_groupb6598)?.length > 0
      ) {
        setsuccess_rate_groupb6598({})
      }
    } else prevRefreshRef.current = true
  }, [success_rate_groupb6598Props?.refresh])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-2xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewapiinfo_v1((pre:any)=>({...pre,_selectedGroup_:"success_rate_group"}))
        }}
    >
        {allowedControls.includes("success_rate_card") ?<Cardsuccess_rate_card  /* 0eba7 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}

export default Groupsuccess_rate_group
