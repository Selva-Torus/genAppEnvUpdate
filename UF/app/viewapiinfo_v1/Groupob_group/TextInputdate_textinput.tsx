...
const TextInputdate_textinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:Trs:AFGK:TOB:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.release_date"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:Trs:AFGK:TOB:AFK:viewApiInfo:AFVK:v1|58264eecca5f4b09982e0f561e876678|e71f7103230c46e59ad78f0aff1b262e"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewApiInfo_v1:{...pre?.viewApiInfo_v1,release_date:undefined}}));
    if(dynamicStateandType.type=="number"){
    setob_group76678((prev: any) => ({ ...prev, release_date: +e.target.value }));
    }
    else{
    setob_group76678((prev: any) => ({ ...prev, release_date: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e71f7103230c46e59ad78f0aff1b262e"
      );
      ...
      //     controlId: "e71f7103230c46e59ad78f0aff1b262e",
      //     isTable: false,
      //     from:"TextInputdate_textinput",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e71f7103230c46e59ad78f0aff1b262e") {
        handleChange({target:{value:ob_group76678Ref?.current?.release_date||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e71f7103230c46e59ad78f0aff1b262e") {
        handleBlur({target:{value:ob_group76678Ref?.current?.release_date||""}});
      }
    };
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setob_group76678((pre:any)=>({...pre,release_date:dfd_mongo_api_repository_dfd_v1Props.data[0]?.release_date}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (date_textinputb262e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 25`,gridRow: `58 / 67`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ob_group76678?.release_date||""}
         disabled= {date_textinputb262e?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.viewApiInfo_v1?.release_date ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdate_textinput
