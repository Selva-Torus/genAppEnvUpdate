...
const TextInputpath_textinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:Trs:AFGK:TOB:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.server_url"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:Trs:AFGK:TOB:AFK:viewApiInfo:AFVK:v1|58264eecca5f4b09982e0f561e876678|43a626bbcf48444b9b741236b62ec3d3"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewApiInfo_v1:{...pre?.viewApiInfo_v1,server_url:undefined}}));
    if(dynamicStateandType.type=="number"){
    setob_group76678((prev: any) => ({ ...prev, server_url: +e.target.value }));
    }
    else{
    setob_group76678((prev: any) => ({ ...prev, server_url: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "43a626bbcf48444b9b741236b62ec3d3"
      );
      ...
      //     controlId: "43a626bbcf48444b9b741236b62ec3d3",
      //     isTable: false,
      //     from:"TextInputpath_textinput",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "43a626bbcf48444b9b741236b62ec3d3") {
        handleChange({target:{value:ob_group76678Ref?.current?.server_url||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "43a626bbcf48444b9b741236b62ec3d3") {
        handleBlur({target:{value:ob_group76678Ref?.current?.server_url||""}});
      }
    };
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setob_group76678((pre:any)=>({...pre,server_url:dfd_mongo_api_repository_dfd_v1Props.data[0]?.server_url}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (path_textinputec3d3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 25`,gridRow: `69 / 78`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ob_group76678?.server_url||""}
         disabled= {path_textinputec3d3?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.viewApiInfo_v1?.server_url ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpath_textinput
