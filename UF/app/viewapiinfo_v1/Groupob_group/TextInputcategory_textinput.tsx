...
const TextInputcategory_textinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:Trs:AFGK:TOB:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.api_category"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:Trs:AFGK:TOB:AFK:viewApiInfo:AFVK:v1|58264eecca5f4b09982e0f561e876678|738b03f1813d47ad8addf61bb20e77d9"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewApiInfo_v1:{...pre?.viewApiInfo_v1,api_category:undefined}}));
    if(dynamicStateandType.type=="number"){
    setob_group76678((prev: any) => ({ ...prev, api_category: +e.target.value }));
    }
    else{
    setob_group76678((prev: any) => ({ ...prev, api_category: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "738b03f1813d47ad8addf61bb20e77d9"
      );
      ...
      //     controlId: "738b03f1813d47ad8addf61bb20e77d9",
      //     isTable: false,
      //     from:"TextInputcategory_textinput",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "738b03f1813d47ad8addf61bb20e77d9") {
        handleChange({target:{value:ob_group76678Ref?.current?.api_category||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "738b03f1813d47ad8addf61bb20e77d9") {
        handleBlur({target:{value:ob_group76678Ref?.current?.api_category||""}});
      }
    };
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setob_group76678((pre:any)=>({...pre,api_category:dfd_mongo_api_repository_dfd_v1Props.data[0]?.api_category}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (category_textinpute77d9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 25`,gridRow: `47 / 56`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ob_group76678?.api_category||""}
         disabled= {category_textinpute77d9?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.viewApiInfo_v1?.api_category ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcategory_textinput
