...
const TextInputapi_name_textinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:Trs:AFGK:TOB:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.apiname"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:Trs:AFGK:TOB:AFK:viewApiInfo:AFVK:v1|58264eecca5f4b09982e0f561e876678|490a1d62421f45b0857080c4a964e4bf"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewApiInfo_v1:{...pre?.viewApiInfo_v1,apiname:undefined}}));
    if(dynamicStateandType.type=="number"){
    setob_group76678((prev: any) => ({ ...prev, apiname: +e.target.value }));
    }
    else{
    setob_group76678((prev: any) => ({ ...prev, apiname: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "490a1d62421f45b0857080c4a964e4bf"
      );
      ...
      //     controlId: "490a1d62421f45b0857080c4a964e4bf",
      //     isTable: false,
      //     from:"TextInputapi_name_textinput",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "490a1d62421f45b0857080c4a964e4bf") {
        handleChange({target:{value:ob_group76678Ref?.current?.apiname||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "490a1d62421f45b0857080c4a964e4bf") {
        handleBlur({target:{value:ob_group76678Ref?.current?.apiname||""}});
      }
    };
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setob_group76678((pre:any)=>({...pre,apiname:dfd_mongo_api_repository_dfd_v1Props.data[0]?.apiname}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (api_name_textinput4e4bf?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 25`,gridRow: `13 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ob_group76678?.apiname||""}
         disabled= {api_name_textinput4e4bf?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.viewApiInfo_v1?.apiname ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputapi_name_textinput
