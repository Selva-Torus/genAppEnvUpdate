...
import Grouptotal_calls_group  from "../Grouptotal_calls_group/Grouptotal_calls_group";
import Groupsuccess_rate_group  from "../Groupsuccess_rate_group/Groupsuccess_rate_group";
import Grouperror_rate_group  from "../Grouperror_rate_group/Grouperror_rate_group";
import Groupob_group  from "../Groupob_group/Groupob_group";
...
import PieChartsrate_piechart  from "./PieChartsrate_piechart";
...
const Groupapi_info_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "91e5ee3dbbdf431bb11b4c1d64f6ad41");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_info_group6ad41Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_calls_group")){
        settotal_calls_group76982((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_calls_group76982?.isDisabled==null)
      {
        settotal_calls_group76982((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("success_rate_group")){
        setsuccess_rate_groupb6598((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(success_rate_groupb6598?.isDisabled==null)
      {
        setsuccess_rate_groupb6598((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("error_rate_group")){
        seterror_rate_group773d1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(error_rate_group773d1?.isDisabled==null)
      {
        seterror_rate_group773d1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ob_group")){
        setob_group76678((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(ob_group76678?.isDisabled==null)
      {
        setob_group76678((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("rate_piechart")){
        setrate_piechartbb394((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rate_piechartbb394?.isDisabled==null)
      {
        setrate_piechartbb394((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_process_log_group")){
        setapi_process_log_group192b0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_process_log_group192b0?.isDisabled==null)
      {
        setapi_process_log_group192b0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['rate_piechart'] = rate_piechartbb394,
        codeStates['setrate_piechart'] = setrate_piechartbb394,
  ...
  const api_info_group6ad41Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_info_group6ad41Ref.current?.setSearchParams();
    api_info_group6ad41Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_info_group6ad41) &&
        Object.keys(api_info_group6ad41)?.length > 0
      ) {
        setapi_info_group6ad41({})
      }
    } else prevRefreshRef.current = true
  }, [api_info_group6ad41Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewapiinfo_v1((pre:any)=>({...pre,_selectedGroup_:"api_info_group"}))
        }}
    >
        {allowedComponent.includes("total_calls_group")  &&<Grouptotal_calls_group  
        ...
        {allowedComponent.includes("success_rate_group")  &&<Groupsuccess_rate_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("error_rate_group")  &&<Grouperror_rate_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("ob_group")  &&<Groupob_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("api_process_log_group")  &&<Groupapi_process_log_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedControls.includes("rate_piechart") ?<PieChartsrate_piechart /* bb394 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
