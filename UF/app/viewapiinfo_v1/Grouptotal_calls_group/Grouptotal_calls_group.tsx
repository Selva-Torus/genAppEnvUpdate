...
import Cardtotal_calls_card  from "./Cardtotal_calls_card";
...
const Grouptotal_calls_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a83e715c6c354f86be421229e3776982");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_calls_group76982Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_calls_card")){
        settotal_calls_cardce82b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_calls_cardce82b?.isDisabled==null)
      {
        settotal_calls_cardce82b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const total_calls_group76982Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_calls_group76982Ref.current?.setSearchParams();
    total_calls_group76982Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(total_calls_group76982) &&
        Object.keys(total_calls_group76982)?.length > 0
      ) {
        settotal_calls_group76982({})
      }
    } else prevRefreshRef.current = true
  }, [total_calls_group76982Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-2xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewapiinfo_v1((pre:any)=>({...pre,_selectedGroup_:"total_calls_group"}))
        }}
    >
        {allowedControls.includes("total_calls_card") ?<Cardtotal_calls_card  /* ce82b */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}

export default Grouptotal_calls_group
