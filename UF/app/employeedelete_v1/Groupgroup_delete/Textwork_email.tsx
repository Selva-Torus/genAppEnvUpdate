...
const Textwork_email = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_employees_v1Props && !dfd_employees_v1Props.hasLogicCenter) {
        ...
        setgroup_delete68104((pre: any) => {
          return { ...pre, work_email: api_paginationData.data.records[0]?.work_email }
        })
        }
      }
      else{
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !group_delete68104.work_email){
        setgroup_delete68104((pre:any)=>({...pre,work_email:dfd_employees_v1Props[0]?.work_email}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[work_email16f90?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !group_delete68104.work_email){
    setgroup_delete68104((pre:any)=>({...pre,work_email:dfd_employees_v1Props[0]?.work_email}));
  }
  },[dfd_employees_v1Props])

  if (work_email16f90?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.work_email : (group_delete68104?.work_email || ""))}
</Text>
  </div>
  )
}

export default Textwork_email
