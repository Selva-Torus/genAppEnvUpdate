...
const Textemployment_type = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_employees_v1Props && !dfd_employees_v1Props.hasLogicCenter) {
        ...
        setgroup_delete68104((pre: any) => {
          return { ...pre, employment_type: api_paginationData.data.records[0]?.employment_type }
        })
        }
      }
      else{
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !group_delete68104.employment_type){
        setgroup_delete68104((pre:any)=>({...pre,employment_type:dfd_employees_v1Props[0]?.employment_type}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employment_type48765?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !group_delete68104.employment_type){
    setgroup_delete68104((pre:any)=>({...pre,employment_type:dfd_employees_v1Props[0]?.employment_type}));
  }
  },[dfd_employees_v1Props])

  if (employment_type48765?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `28 / 33`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.employment_type : (group_delete68104?.employment_type || ""))}
</Text>
  </div>
  )
}

export default Textemployment_type
