...
const Textfull_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !group_delete68104.full_name){
        setgroup_delete68104((pre:any)=>({...pre,full_name:dfd_employees_v1Props[0]?.full_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[full_name89c4f?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !group_delete68104.full_name){
    setgroup_delete68104((pre:any)=>({...pre,full_name:dfd_employees_v1Props[0]?.full_name}));
  }
  },[dfd_employees_v1Props])

  if (full_name89c4f?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.full_name : (group_delete68104?.full_name || ""))}
</Text>
  </div>
  )
}
