...
const Textemployee_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !group_delete68104.employee_code){
        setgroup_delete68104((pre:any)=>({...pre,employee_code:dfd_employees_v1Props[0]?.employee_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employee_code89740?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !group_delete68104.employee_code){
    setgroup_delete68104((pre:any)=>({...pre,employee_code:dfd_employees_v1Props[0]?.employee_code}));
  }
  },[dfd_employees_v1Props])

  if (employee_code89740?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.employee_code : (group_delete68104?.employee_code || ""))}
</Text>
  </div>
  )
}
