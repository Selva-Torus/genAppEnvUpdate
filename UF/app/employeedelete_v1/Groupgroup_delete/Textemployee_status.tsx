...
const Textemployee_status = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_employees_v1Props && !dfd_employees_v1Props.hasLogicCenter) {
        ...
        setgroup_delete68104((pre: any) => {
          return { ...pre, employee_status: api_paginationData.data.records[0]?.employee_status }
        })
        }
      }
      else{
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !group_delete68104.employee_status){
        setgroup_delete68104((pre:any)=>({...pre,employee_status:dfd_employees_v1Props[0]?.employee_status}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employee_status8c982?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !group_delete68104.employee_status){
    setgroup_delete68104((pre:any)=>({...pre,employee_status:dfd_employees_v1Props[0]?.employee_status}));
  }
  },[dfd_employees_v1Props])

  if (employee_status8c982?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.employee_status : (group_delete68104?.employee_status || ""))}
</Text>
  </div>
  )
}

export default Textemployee_status
