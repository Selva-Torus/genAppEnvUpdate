...
export default function PagePendingfillingsearchV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function pendingfilingsdashboard_v1DFD(pagination:any): Promise<void>{
        ...
        if(pendingfillingsearch_v1Props.length > 0){
          for(let i=0;i< pendingfillingsearch_v1Props.length;i++){
            if(pendingfillingsearch_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:pendingFilingsDashboard:AFVK:v1"){
              // delete pendingfillingsearch_v1Props[i].DFDkey;
              let temp=structuredClone(pendingfillingsearch_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setpendingfillingsearch_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    const introspectParams = encryptionFlagPage
      ? {
          ...
          method: encryptionMethod,
          key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:pendingFillingSearch:AFVK:v1"
        }
      : { key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:pendingFillingSearch:AFVK:v1" }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setpendingfillingsearch_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(pendingfillingsearch_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(pendingfillingsearch_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setpendingfillingsearch_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
