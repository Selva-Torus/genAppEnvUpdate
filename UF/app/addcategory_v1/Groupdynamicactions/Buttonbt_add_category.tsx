...
const Buttonbt_add_category = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "2c0aa58125154c86a086d77092d240df"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("bt_add_category" in dynamicactions13884Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions13884Props?.dynamicActionRule?.bt_add_category,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("bt_add_category" in addcategory_v1?.dynamicactions && addcategory_v1?.dynamicactions["bt_add_category"]?.itsHaveArtifact== true)
      {
        setShowFlag(addcategory_v1?.dynamicactions["bt_add_category"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addcategory_v1?.dynamicactions?.bt_add_category?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "bt_add_category240df") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "2c0aa58125154c86a086d77092d240df") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "bt_add_category240df");
    return () => {
  ...
  useEffect(()=>{
    if (!bt_add_category240df?.trigger) return;
      if(bt_add_category240df?.trigger){
      setbt_add_category240df((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[bt_add_category240df?.trigger])

  useEffect(()=>{
    if(bt_add_category240df?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[bt_add_category240df?.refresh])
  ...
  async function handleSave40df_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const dynamicactions13884Props = dynamicactions13884PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.addCategory_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(dynamicactions13884Ref.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1|7ed190238b52498484f3735c45150108"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1|7ed190238b52498484f3735c45150108"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "2c0aa58125154c86a086d77092d240df",
  "type": "button",
  "name": "bt_add_category",
  "label": "bt_add_category",
  "sequence": 1,
  "children": [
    {
      "id": "2c0aa58125154c86a086d77092d240df.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "2c0aa58125154c86a086d77092d240df.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "2c0aa58125154c86a086d77092d240df.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "2c0aa58125154c86a086d77092d240df.1.1.1.1.1",
                  ...
                  "children": [
                    {
                      "id": "3de40d278fed40108fe057ba6413e4ac.1.1.1.1.1.1",
                      "value": "",
                      "type": "screen",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1|7ed190238b52498484f3735c45150108"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1";
      sourceId+= "|"+"1e732150ff2740f495eee7f741a13884";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1"+"|"+"1e732150ff2740f495eee7f741a13884"+"|"+eventProperty.id;
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions13884Ref.current, ...nullFilter(category_groupe3ebd), ...nullFilter(category_information_groupfb68a), ...nullFilter(category_configuration_group5d6af) });
    let reworkKeys:any[]=[];
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"1e732150ff2740f495eee7f741a13884",
              controlId:"2c0aa58125154c86a086d77092d240df"
            };
            if (encryptionFlagCont) {
        ...
        te_saveBody.controlName = "bt_add_category";
        if(mainData?.upId)
        {
        ...
        if(dynamicactions13884?.upId){
          te_saveBody['upId']= dynamicactions13884?.upId;
        }
        if(dynamicactions13884?.upid){
          te_saveBody['upId']= dynamicactions13884?.upid;
        }
        ...
        if (dynamicactions13884Props.ssKey !== '' && dynamicactions13884Props.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactions13884Props.ssKey;
    ...
    //infoMsg
    toast('New Category Added Successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
    setcategory_table3e4acProps((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions13884((prev: any) => ({ ...prev, bt_add_category: true }));
        //onClick

    //saveHandler
    await handleSave40df_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions13884((prev: any) => ({ ...prev, bt_add_category: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addcategory_v1?.dynamicactions?.bt_add_category,dynamicactions13884Props?.dynamicActionRule?.bt_add_category,])

 if (bt_add_category240df?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {bt_add_category240df?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Add Category")}
...
export default Buttonbt_add_category
