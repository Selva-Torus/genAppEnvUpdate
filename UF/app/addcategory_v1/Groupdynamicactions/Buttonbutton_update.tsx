 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "41cdad18843e4d3eb6db535023d74a1f"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactions13884Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions13884Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in addcategory_v1?.dynamicactions && addcategory_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(addcategory_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addcategory_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_update74a1f") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "41cdad18843e4d3eb6db535023d74a1f") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_update74a1f");
    return () => {
  ...
  useEffect(()=>{
    if (!button_update74a1f?.trigger) return;
      if(button_update74a1f?.trigger){
      setbutton_update74a1f((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_update74a1f?.trigger])

  useEffect(()=>{
    if(button_update74a1f?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_update74a1f?.refresh])
  ...
  async function handleSave4a1f_1_1_1(){
  ...
  return val === 'invalid';
    ...
    try{
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategoryModify:AFVK:v1|7e34f665759a433b8349b40358d7811d"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategoryModify:AFVK:v1|7e34f665759a433b8349b40358d7811d"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactions13884?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "41cdad18843e4d3eb6db535023d74a1f",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "41cdad18843e4d3eb6db535023d74a1f.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "41cdad18843e4d3eb6db535023d74a1f.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "41cdad18843e4d3eb6db535023d74a1f.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "41cdad18843e4d3eb6db535023d74a1f.1.1.1.1.1",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:addCategoryModify:AFVK:v1|7e34f665759a433b8349b40358d7811d"
          ]
        }
      ]
    }
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions13884?.upId? [dynamicactions13884?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions13884?.upId? [dynamicactions13884?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions13884?.upId? [dynamicactions13884?.upId ] : dynamicactions13884?.trs_process_id? [dynamicactions13884?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"41cdad18843e4d3eb6db535023d74a1f"
            };
            if (encryptionFlagCont) {
    ...
    te_eventEmitterBody["ssKey"] = dynamicactions13884Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Category Updated Successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions13884((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSave4a1f_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions13884((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addcategory_v1?.dynamicactions?.button_update,dynamicactions13884Props?.dynamicActionRule?.button_update,])

 if (button_update74a1f?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_update74a1f?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Update")}
