...
const TextInputcategory_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addCategory:AFVK:v1|9da39b1e9c6c4d9fba3f89ab536fb68a|d6c3f5c1e8714689b623241aa3560d4a"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCategory_v1:{...pre?.addCategory_v1,category_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcategory_information_groupfb68a((prev: any) => ({ ...prev, category_code: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(category_information_groupfb68a?.category_code == "" || category_information_groupfb68a?.category_code == undefined){
      category_information_groupfb68a.category_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, category_information_groupfb68a?.category_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addCategory_v1:{...pre?.addCategory_v1,category_code:"invalid"}}));
        }
    }else if(category_information_groupfb68a?.category_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +category_information_groupfb68a?.category_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d6c3f5c1e8714689b623241aa3560d4a"
      );
      ...
      //     controlId: "d6c3f5c1e8714689b623241aa3560d4a",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!category_information_groupfb68a?.category_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "d6c3f5c1e8714689b623241aa3560d4a") {
        handleChange({target:{value:category_information_groupfb68aRef?.current?.category_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d6c3f5c1e8714689b623241aa3560d4a") {
        handleBlur({target:{value:category_information_groupfb68aRef?.current?.category_code||""}});
      }
    };
  ...
  if(dfd_assetcategory_v1Props?.setSearchFilters && dfd_assetcategory_v1Props?.data)
  {
    if(Array.isArray(dfd_assetcategory_v1Props.data) && dfd_assetcategory_v1Props.data.length > 0){
      setcategory_information_groupfb68a((pre:any)=>({...pre,category_code:dfd_assetcategory_v1Props.data[0]?.category_code}));
    }
  }
  },[dfd_assetcategory_v1Props?.setSearchFilters])
  if (category_code60d4a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={category_information_groupfb68a?.category_code||""}
         disabled= {category_code60d4a?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addCategory_v1?.category_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
