...
import Buttonbt_view  from './Buttonbt_view'
...
let defaultColumns:any = [
  {
    "id": "employee_id",
    "nodeid": "09a559083c284059b15d6b8f08f78a7b",
    "name": "Employee ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "employee_code",
    "nodeid": "62d07641c6cf40a0a4dc9847b795b249",
    "name": "Employee Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "emp_number",
    "nodeid": "7c1d88dc595a4e04a403d207b8f23fed",
  ...
  {
    "id": "full_name",
    "nodeid": "157ae867098d4953b62b7a4b7d826270",
    "name": "Employee Name",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "79aa4f45442345d9ba41456eafb33de5",
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "reporting_manager_id"
  },
  {
    "id": "available_leave",
    "nodeid": "b6b9857ffe99488bbf75cb927200dc93",
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setleave_req_tablesbb32bProps((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      leave_req_tablesbb32b.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setleave_req_tablesbb32bProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setleave_req_tablesbb32bProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setleave_req_tablesbb32bProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '064106bafb7f4c4dbd44fb6df4cbb32b',
                elementId: '064106bafb7f4c4dbd44fb6df4cbb32b'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( leave_req_tablesbb32bProps?.presetValues&&Object.keys(leave_req_tablesbb32bProps?.presetValues).length > 0) {
          filtertedData = [leave_req_tablesbb32bProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setleave_req_tablesbb32b(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in leaverequest_v1?.leave_req_tables && leaverequest_v1?.leave_req_tables[controller]?.itsHaveArtifact== true)
      {
        if(leaverequest_v1?.leave_req_tables[controller])
        {
          let result :any = await getAftfactLevelRule(leaverequest_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,leave_req_tables:filteredData},{leave_req_tables:leaverequest_v1?.leave_req_tables})
          if(result?.leave_req_tables?.[controller]?.show==true)
            return true
          else
            return false
        }
    ...
    if(await handleSecurity("bt_view") &&nodeName?.toLowerCase()=="buttonbt_view"){
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (leave_req_tablesbb32bProps.filterInitalLoad) return;

    const filterControllers = leave_req_tablesbb32bProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['leave_req_tablesbb32b']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['leave_req_tablesbb32b']).filter((k: string) => jsonEntry['leave_req_tablesbb32b'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(leave_req_tablesbb32bProps.filterProps);
      return;
    }
    ...
    setleave_req_tablesbb32bProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(leave_req_tablesbb32bProps.filterProps);

  }, [
    leave_req_tablesbb32bProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    leave_req_tablesbb32bProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(leave_req_tablesbb32bProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(leave_req_tablesbb32bProps.searchFilter) || {};
...
}, [leave_req_tablesbb32bProps.searchFilter])
  ...
  useEffect(() => {
    if (leave_req_tablesbb32bProps?.clearData === true) {
      setleave_req_tablesbb32b([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setleave_req_tablesbb32bProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [leave_req_tablesbb32bProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [leave_req_tablesbb32bProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = leave_req_tablesbb32bProps?.selectedIds[leave_req_tablesbb32bProps?.selectedIds?.length-1]
    if(Array.isArray(leave_req_tablesbb32b) && leave_req_tablesbb32b.length>0)
    {
      let data = leave_req_tablesbb32b[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "leave_req_id",
  ...
  useEffect(() => {
    if (leave_req_tablesbb32bProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (leave_req_tablesbb32bProps?.selectedIds?.length == 0){
      handleOnRowClick({},leave_req_tablesbb32bProps?.selectedIds)
    }
  }, [leave_req_tablesbb32bProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(leave_req_tablesbb32b?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&leave_req_tablesbb32b?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](leave_req_tablesbb32b?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: leave_req_tablesbb32b?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&leave_req_tablesbb32b?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](leave_req_tablesbb32b?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: leave_req_tablesbb32b?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (leave_req_tablesbb32b?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"064106bafb7f4c4dbd44fb6df4cbb32b"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableSorting={true}
              selectedIds={leave_req_tablesbb32bProps?.selectedIds}  
...
export default Tableleave_req_tables
