...
import Tableapproval_pending_table  from './Tableapproval_pending_table';  
...
const Groupapproval_pending_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1fcbb0e3650b4dc5bcd0cb678fce294f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapproval_pending_tablee294fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_req_id")){
        setleave_req_id8fe41((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_req_id8fe41?.isDisabled==null)
      {
        setleave_req_id8fe41((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_request_number")){
        setleave_request_numbera0793((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_request_numbera0793?.isDisabled==null)
      {
        setleave_request_numbera0793((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("full_name")){
        setfull_name831de((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(full_name831de?.isDisabled==null)
      {
        setfull_name831de((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("start_date")){
        setstart_dated19f6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(start_dated19f6?.isDisabled==null)
      {
        setstart_dated19f6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("end_date")){
        setend_datefce01((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(end_datefce01?.isDisabled==null)
      {
        setend_datefce01((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("days_requested")){
        setdays_requestedaba8e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(days_requestedaba8e?.isDisabled==null)
      {
        setdays_requestedaba8e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_event_process_status")){
        settrs_event_process_status66324((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_event_process_status66324?.isDisabled==null)
      {
        settrs_event_process_status66324((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("view_bt")){
        setview_bt2dca3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(view_bt2dca3?.isDisabled==null)
      {
        setview_bt2dca3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const approval_pending_tablee294fRef = useRef<any>(null);
  const handleClearSearch = () => {
    approval_pending_tablee294fRef.current?.setSearchParams();
    approval_pending_tablee294fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approval_pending_tablee294f) && Object.keys(approval_pending_tablee294f)?.length>0)
      {
        setapproval_pending_tablee294f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approval_pending_tablee294fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setleaverequest_v1((pre:any)=>({...pre,_selectedGroup_:"approval_pending_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tableapproval_pending_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={approval_pending_tablee294fRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupapproval_pending_table
