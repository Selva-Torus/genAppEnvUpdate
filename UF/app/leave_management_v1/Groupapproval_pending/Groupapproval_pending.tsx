...
import Groupleave_approval_pending_group  from "../Groupleave_approval_pending_group/Groupleave_approval_pending_group";
...
const Groupapproval_pending = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "92279a5e24744b97896fd4ec0b3e1c0b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapproval_pendinge1c0bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_approval_pending_group")){
        setleave_approval_pending_group05215((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_approval_pending_group05215?.isDisabled==null)
      {
        setleave_approval_pending_group05215((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tab_groupfe908?.tab_group === "approval_pending"){
    handleOnChange()
    }
  ...
  const approval_pendinge1c0bRef = useRef<any>(null);
  const handleClearSearch = () => {
    approval_pendinge1c0bRef.current?.setSearchParams();
    approval_pendinge1c0bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approval_pendinge1c0b) && Object.keys(approval_pendinge1c0b)?.length>0)
      {
        setapproval_pendinge1c0b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approval_pendinge1c0bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setleaverequest_v1((pre:any)=>({...pre,_selectedGroup_:"approval_pending"}))
        }}
    >
        {allowedComponent.includes("leave_approval_pending_group")  &&<Groupleave_approval_pending_group  
...
export default Groupapproval_pending
