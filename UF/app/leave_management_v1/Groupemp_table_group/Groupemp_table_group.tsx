...
import Groupleave_req_tables  from "../Groupleave_req_tables/Groupleave_req_tables";
...
const Groupemp_table_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a0f4b2add31845c0aed8108315f0a9cd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemp_table_group0a9cdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_req_tables")){
        setleave_req_tablesbb32b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_req_tablesbb32b?.isDisabled==null)
      {
        setleave_req_tablesbb32b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const emp_table_group0a9cdRef = useRef<any>(null);
  const handleClearSearch = () => {
    emp_table_group0a9cdRef.current?.setSearchParams();
    emp_table_group0a9cdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(emp_table_group0a9cd) && Object.keys(emp_table_group0a9cd)?.length>0)
      {
        setemp_table_group0a9cd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [emp_table_group0a9cdProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setleaverequest_v1((pre:any)=>({...pre,_selectedGroup_:"emp_table_group"}))
        }}
    >
        {allowedComponent.includes("leave_req_tables")  &&<Groupleave_req_tables  
...
export default Groupemp_table_group
