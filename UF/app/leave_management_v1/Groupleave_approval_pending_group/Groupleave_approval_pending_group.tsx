...
import Groupapproval_pending_table  from "../Groupapproval_pending_table/Groupapproval_pending_table";
...
const Groupleave_approval_pending_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupLeaveApprovalPendingGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0330b15bcd0b4827ab636ee038005215");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setleave_approval_pending_group05215Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("approval_pending_table")){
        setapproval_pending_tablee294f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approval_pending_tablee294f?.isDisabled==null)
      {
        setapproval_pending_tablee294f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const leave_approval_pending_group05215Ref = useRef<any>(null);
  const handleClearSearch = () => {
    leave_approval_pending_group05215Ref.current?.setSearchParams();
    leave_approval_pending_group05215Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(leave_approval_pending_group05215) && Object.keys(leave_approval_pending_group05215)?.length>0)
      {
        setleave_approval_pending_group05215({})
      }
    }else 
      prevRefreshRef.current= true
  }, [leave_approval_pending_group05215Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setleaverequest_v1((pre:any)=>({...pre,_selectedGroup_:"leave_approval_pending_group"}))
        }}
    >
        {allowedComponent.includes("approval_pending_table")  &&<Groupapproval_pending_table  
...
export default Groupleave_approval_pending_group
