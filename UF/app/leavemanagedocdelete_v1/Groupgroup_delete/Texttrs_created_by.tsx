...
const Texttrs_created_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavemanagedoctable_v1Props) && dfd_leavemanagedoctable_v1Props && !group_delete617ef.trs_created_by){
        setgroup_delete617ef((pre:any)=>({...pre,trs_created_by:dfd_leavemanagedoctable_v1Props[0]?.trs_created_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[trs_created_by3ee0e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavemanagedoctable_v1Props) && !group_delete617ef.trs_created_by){
    setgroup_delete617ef((pre:any)=>({...pre,trs_created_by:dfd_leavemanagedoctable_v1Props[0]?.trs_created_by}));
  }
  },[dfd_leavemanagedoctable_v1Props])

  if (trs_created_by3ee0e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.trs_created_by : (group_delete617ef?.trs_created_by || ""))}
</Text>
  </div>
  )
}
