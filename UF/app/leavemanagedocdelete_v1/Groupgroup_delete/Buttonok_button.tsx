 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "802b79cad398f7f47a05491b82e0c2f2"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_button0c2f2") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "802b79cad398f7f47a05491b82e0c2f2") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_button0c2f2");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_button0c2f2?.trigger) return;
      if(ok_button0c2f2?.trigger){
      setok_button0c2f2((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_button0c2f2?.trigger])

  useEffect(()=>{
    if(ok_button0c2f2?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_button0c2f2?.refresh])
  ...
  async function handleSavec2f2_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_delete617efProps = group_delete617efPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.leaveManageDocDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_delete617efRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete617ef?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "802b79cad398f7f47a05491b82e0c2f2",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "802b79cad398f7f47a05491b82e0c2f2.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "802b79cad398f7f47a05491b82e0c2f2.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "802b79cad398f7f47a05491b82e0c2f2.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "802b79cad398f7f47a05491b82e0c2f2.1.1.1.1.1",
                  "eventContext": "rise",
                  "value": "",
                  "type": "handlerNode",
                  "name": "closeHandler",
            ...
            {
              "id": "802b79cad398f7f47a05491b82e0c2f2.1.1.1.2",
              ...
              "children": [
                {
                  "id": "858e60aa8d764228bc44c6400a849f40.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leaveManageDocDelete:AFVK:v1";
      sourceId+= "|"+"3bf1bf0f4a4e01cabdd93ee0d51617ef";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leaveManageDocDelete:AFVK:v1"+"|"+"3bf1bf0f4a4e01cabdd93ee0d51617ef"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete617ef?.upId? [group_delete617ef?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete617ef?.upId? [group_delete617ef?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete617ef?.upId? [group_delete617ef?.upId ] : group_delete617ef?.trs_process_id? [group_delete617ef?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete617efRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"3bf1bf0f4a4e01cabdd93ee0d51617ef",
              controlId:"802b79cad398f7f47a05491b82e0c2f2"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete617efProps.ssKey !== '' && group_delete617efProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete617efProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete617ef((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSavec2f2_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete617ef((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  return (
    <div
      ...
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="!text-white !rounded-md !bg-[#108DDA]"
          onClick={handleClick}
          view='action'
          disabled= {ok_button0c2f2?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
