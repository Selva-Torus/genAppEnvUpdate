...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavemanagedoctable_v1Props) && dfd_leavemanagedoctable_v1Props && !group_delete617ef.doc_name){
        setgroup_delete617ef((pre:any)=>({...pre,doc_name:dfd_leavemanagedoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_name2d223?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavemanagedoctable_v1Props) && !group_delete617ef.doc_name){
    setgroup_delete617ef((pre:any)=>({...pre,doc_name:dfd_leavemanagedoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_leavemanagedoctable_v1Props])

  if (doc_name2d223?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete617ef?.doc_name || ""))}
</Text>
  </div>
  )
}
