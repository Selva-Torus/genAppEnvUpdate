...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavemanagedoctable_v1Props) && dfd_leavemanagedoctable_v1Props && !group_delete617ef.doc_group){
        setgroup_delete617ef((pre:any)=>({...pre,doc_group:dfd_leavemanagedoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_group3263c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavemanagedoctable_v1Props) && !group_delete617ef.doc_group){
    setgroup_delete617ef((pre:any)=>({...pre,doc_group:dfd_leavemanagedoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_leavemanagedoctable_v1Props])

  if (doc_group3263c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_delete617ef?.doc_group || ""))}
</Text>
  </div>
  )
}
