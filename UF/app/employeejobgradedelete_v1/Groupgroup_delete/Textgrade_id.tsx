...
const Textgrade_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props && !group_deletea1de0.grade_id){
        setgroup_deletea1de0((pre:any)=>({...pre,grade_id:dfd_jobgrade_v1Props[0]?.grade_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[grade_id65c54?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobgrade_v1Props) && !group_deletea1de0.grade_id){
    setgroup_deletea1de0((pre:any)=>({...pre,grade_id:dfd_jobgrade_v1Props[0]?.grade_id}));
  }
  },[dfd_jobgrade_v1Props])

  if (grade_id65c54?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.grade_id : (group_deletea1de0?.grade_id || ""))}
</Text>
  </div>
  )
}
