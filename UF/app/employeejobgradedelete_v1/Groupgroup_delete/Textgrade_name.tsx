...
const Textgrade_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props && !group_deletea1de0.grade_name){
        setgroup_deletea1de0((pre:any)=>({...pre,grade_name:dfd_jobgrade_v1Props[0]?.grade_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[grade_nameaa0a4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobgrade_v1Props) && !group_deletea1de0.grade_name){
    setgroup_deletea1de0((pre:any)=>({...pre,grade_name:dfd_jobgrade_v1Props[0]?.grade_name}));
  }
  },[dfd_jobgrade_v1Props])

  if (grade_nameaa0a4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.grade_name : (group_deletea1de0?.grade_name || ""))}
</Text>
  </div>
  )
}
