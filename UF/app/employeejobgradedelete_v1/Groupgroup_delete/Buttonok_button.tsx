 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "2a322b3cb4194d75253311ded87504a2"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_button504a2") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "2a322b3cb4194d75253311ded87504a2") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_button504a2");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_button504a2?.trigger) return;
      if(ok_button504a2?.trigger){
      setok_button504a2((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_button504a2?.trigger])

  useEffect(()=>{
    if(ok_button504a2?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_button504a2?.refresh])
  ...
  async function handleSave04a2_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_deletea1de0Props = group_deletea1de0PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.employeeJobGradeDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_deletea1de0Ref.current);
      ...
      let lockedKeysLength:number;
        if(group_deletea1de0?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "2a322b3cb4194d75253311ded87504a2",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "2a322b3cb4194d75253311ded87504a2.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "2a322b3cb4194d75253311ded87504a2.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "2a322b3cb4194d75253311ded87504a2.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobGradeDelete:AFVK:v1";
      sourceId+= "|"+"25d78f163c6cf2b29df63021f43a1de0";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobGradeDelete:AFVK:v1"+"|"+"25d78f163c6cf2b29df63021f43a1de0"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_deletea1de0?.upId? [group_deletea1de0?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_deletea1de0?.upId? [group_deletea1de0?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_deletea1de0?.upId? [group_deletea1de0?.upId ] : group_deletea1de0?.trs_process_id? [group_deletea1de0?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_deletea1de0Ref.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"25d78f163c6cf2b29df63021f43a1de0",
              controlId:"2a322b3cb4194d75253311ded87504a2"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_deletea1de0Props.ssKey !== '' && group_deletea1de0Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_deletea1de0Props.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_deletea1de0((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave04a2_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_deletea1de0((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_button504a2?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
