...
const Textgrade_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobgrade_v1Props && !dfd_jobgrade_v1Props.hasLogicCenter) {
        ...
        setgroup_deletea1de0((pre: any) => {
          return { ...pre, grade_code: api_paginationData.data.records[0]?.grade_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props && !group_deletea1de0.grade_code){
        setgroup_deletea1de0((pre:any)=>({...pre,grade_code:dfd_jobgrade_v1Props[0]?.grade_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[grade_code812e5?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobgrade_v1Props) && !group_deletea1de0.grade_code){
    setgroup_deletea1de0((pre:any)=>({...pre,grade_code:dfd_jobgrade_v1Props[0]?.grade_code}));
  }
  },[dfd_jobgrade_v1Props])

  if (grade_code812e5?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.grade_code : (group_deletea1de0?.grade_code || ""))}
</Text>
  </div>
  )
}

export default Textgrade_code
