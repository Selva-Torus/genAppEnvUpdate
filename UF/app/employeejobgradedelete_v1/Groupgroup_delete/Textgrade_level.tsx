...
const Textgrade_level = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobgrade_v1Props && !dfd_jobgrade_v1Props.hasLogicCenter) {
        ...
        setgroup_deletea1de0((pre: any) => {
          return { ...pre, grade_level: api_paginationData.data.records[0]?.grade_level }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props && !group_deletea1de0.grade_level){
        setgroup_deletea1de0((pre:any)=>({...pre,grade_level:dfd_jobgrade_v1Props[0]?.grade_level}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[grade_level40e36?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobgrade_v1Props) && !group_deletea1de0.grade_level){
    setgroup_deletea1de0((pre:any)=>({...pre,grade_level:dfd_jobgrade_v1Props[0]?.grade_level}));
  }
  },[dfd_jobgrade_v1Props])

  if (grade_level40e36?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.grade_level : (group_deletea1de0?.grade_level || ""))}
</Text>
  </div>
  )
}

export default Textgrade_level
