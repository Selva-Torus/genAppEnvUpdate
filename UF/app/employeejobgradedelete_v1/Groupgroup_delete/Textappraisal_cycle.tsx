...
const Textappraisal_cycle = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobgrade_v1Props && !dfd_jobgrade_v1Props.hasLogicCenter) {
        ...
        setgroup_deletea1de0((pre: any) => {
          return { ...pre, appraisal_cycle: api_paginationData.data.records[0]?.appraisal_cycle }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props && !group_deletea1de0.appraisal_cycle){
        setgroup_deletea1de0((pre:any)=>({...pre,appraisal_cycle:dfd_jobgrade_v1Props[0]?.appraisal_cycle}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[appraisal_cycle961f2?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobgrade_v1Props) && !group_deletea1de0.appraisal_cycle){
    setgroup_deletea1de0((pre:any)=>({...pre,appraisal_cycle:dfd_jobgrade_v1Props[0]?.appraisal_cycle}));
  }
  },[dfd_jobgrade_v1Props])

  if (appraisal_cycle961f2?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.appraisal_cycle : (group_deletea1de0?.appraisal_cycle || ""))}
</Text>
  </div>
  )
}

export default Textappraisal_cycle
