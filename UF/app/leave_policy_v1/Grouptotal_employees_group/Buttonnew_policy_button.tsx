...
const Buttonnew_policy_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "77cc14b52229e4177539488c8a7ed37f"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "new_policy_buttoned37f") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "77cc14b52229e4177539488c8a7ed37f") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "new_policy_buttoned37f");
    return () => {
  ...
  useEffect(()=>{
    if (!new_policy_buttoned37f?.trigger) return;
      if(new_policy_buttoned37f?.trigger){
      setnew_policy_buttoned37f((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[new_policy_buttoned37f?.trigger])

  useEffect(()=>{
    setShowProfileAsModalOpen2(false)
    if(new_policy_buttoned37f?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[new_policy_buttoned37f?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        settotal_employees_group93757((prev: any) => ({ ...prev, new_policy_button: true }));
        //onClick
    ...
    //enableElement
    setsave5c873((prev: any) => ({ ...prev, isDisabled: false }));
    //disableElement
    setbutton_update83771((prev: any) => ({ ...prev, isDisabled: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        settotal_employees_group93757((prev: any) => ({ ...prev, new_policy_button: false }));
 ...
 if (new_policy_buttoned37f?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      >
        {!hiddenModalForTrigger && <PageNewleavepolicypage2/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='normal'
          disabled= {new_policy_buttoned37f?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdAdd"
...
export default Buttonnew_policy_button
