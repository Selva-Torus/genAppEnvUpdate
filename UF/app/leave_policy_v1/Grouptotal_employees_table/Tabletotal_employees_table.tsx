...
let defaultColumns:any = [
  {
    "id": "policy_id",
    "nodeid": "5ff551bfedc8faa9fe93e52efa959787",
    "name": "Policy ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "policy_code",
    "nodeid": "f4d5792724c90cddb6f13f31e3a26cc7",
    "name": "Policy Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "policy_name",
    "nodeid": "566fd926aae61865af3c00aefe52dc31",
    "name": "Policy Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "leave_type",
    "nodeid": "80d613360eea1d70aed29ef59cb6ad16",
    "name": "Leave Type",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "days_per_year",
    "nodeid": "5ad1e1bd79921180e957eb7d465cee52",
    "name": "Days/Year",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "days_per_year"
  },
  {
    "id": "applicable_to",
    "nodeid": "86fd71ef4fe47463c2a77107951d9dcb",
    "name": "Applicable To",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "applicable_to"
  },
  {
    "id": "trs_event_process_status",
    "nodeid": "492f7169016b5a7a287222caedd42183",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tabletotal_employees_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {leavepolicy_v1Props, setleavepolicy_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    /// settotal_employees_table02f51Props
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settotal_employees_table02f51Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      total_employees_table02f51.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settotal_employees_table02f51Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settotal_employees_table02f51Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settotal_employees_table02f51Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(leavepolicy_v1Props.length > 0){
      for(let i=0;i< leavepolicy_v1Props.length;i++){
        if(leavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leavePolicy:AFVK:v1"){
          // delete leavepolicy_v1Props[i].DFDkey;
          let temp=structuredClone(leavepolicy_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leavePolicy:AFVK:v1:UO', 
                nodeId: '9fa367ce3d1da5763dd7401922002f51',
                elementId: '9fa367ce3d1da5763dd7401922002f51'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( total_employees_table02f51Props?.presetValues&&Object.keys(total_employees_table02f51Props?.presetValues).length > 0) {
          filtertedData = [total_employees_table02f51Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settotal_employees_table02f51(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={leavepolicy_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (total_employees_table02f51Props.filterInitalLoad) return;

    const filterControllers = total_employees_table02f51Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['total_employees_table02f51']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['total_employees_table02f51']).filter((k: string) => jsonEntry['total_employees_table02f51'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(total_employees_table02f51Props.filterProps);
      return;
    }
    ...
    settotal_employees_table02f51Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(total_employees_table02f51Props.filterProps);

  }, [
    total_employees_table02f51Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    total_employees_table02f51Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(total_employees_table02f51Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(total_employees_table02f51Props.searchFilter) || {};
...
}, [total_employees_table02f51Props.searchFilter])
  ...
  useEffect(() => {
    if (total_employees_table02f51Props?.clearData === true) {
      settotal_employees_table02f51([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settotal_employees_table02f51Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [total_employees_table02f51Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [total_employees_table02f51Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = total_employees_table02f51Props?.selectedIds[total_employees_table02f51Props?.selectedIds?.length-1]
    if(Array.isArray(total_employees_table02f51) && total_employees_table02f51.length>0)
    {
      let data = total_employees_table02f51[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (total_employees_table02f51Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (total_employees_table02f51Props?.selectedIds?.length == 0){
      handleOnRowClick({},total_employees_table02f51Props?.selectedIds)
    }
  }, [total_employees_table02f51Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(total_employees_table02f51?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&total_employees_table02f51?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](total_employees_table02f51?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table02f51?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&total_employees_table02f51?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](total_employees_table02f51?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table02f51?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (total_employees_table02f51?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"9fa367ce3d1da5763dd7401922002f51"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableActions={true}
              tableSorting={true}
              selectedIds={total_employees_table02f51Props?.selectedIds}  
