...
const TextInputreturned_at = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1|b50b5e3a7b314f70bda35687ca3e035e|properties.returned_at"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|472c369f56f3afb6e920bdd86cc136e4|537d1361eb7a466caea44ebd7e7ecafb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'returned_at',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,returned_at:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_details_group136e4((prev: any) => ({ ...prev, returned_at: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_details_group136e4?.returned_at == "" || assignment_details_group136e4?.returned_at == undefined){
      assignment_details_group136e4.returned_at = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_details_group136e4?.returned_at);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,returned_at:"invalid"}}));
        }
    }else if(assignment_details_group136e4?.returned_at !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_details_group136e4?.returned_at);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "537d1361eb7a466caea44ebd7e7ecafb"
      );
      ...
      //     controlId: "537d1361eb7a466caea44ebd7e7ecafb",
      //     isTable: false,
      //     from:"TextInputreturned_at",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'returned_at', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'returned_at',type:'text'};
      //   type={
      //     name:'returned_at',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.returned_at.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.returned_at.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.returned_at.type
      //   }
      ...
      //   type={
      //     name:'returned_at',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.returned_at.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.returned_at.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.returned_at.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_details_group136e4?.returned_at)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "537d1361eb7a466caea44ebd7e7ecafb") {
        handleChange({target:{value:assignment_details_group136e4Ref?.current?.returned_at||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "537d1361eb7a466caea44ebd7e7ecafb") {
        handleBlur({target:{value:assignment_details_group136e4Ref?.current?.returned_at||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_details_group136e4((pre:any)=>({...pre,returned_at:dfd_assetassignments_v1Props.data[0]?.returned_at}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (returned_atecafb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_details_group136e4?.returned_at||""}
         disabled= {returned_atecafb?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.returned_at ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputreturned_at
