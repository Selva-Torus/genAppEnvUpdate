...
const TextInputapproval_status = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1|b50b5e3a7b314f70bda35687ca3e035e|properties.approval_status"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|472c369f56f3afb6e920bdd86cc136e4|73f80dac33ad47c28c48d331c08f07b0"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'approval_status',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,approval_status:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_details_group136e4((prev: any) => ({ ...prev, approval_status: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_details_group136e4?.approval_status == "" || assignment_details_group136e4?.approval_status == undefined){
      assignment_details_group136e4.approval_status = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_details_group136e4?.approval_status);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,approval_status:"invalid"}}));
        }
    }else if(assignment_details_group136e4?.approval_status !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_details_group136e4?.approval_status);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "73f80dac33ad47c28c48d331c08f07b0"
      );
      ...
      //     controlId: "73f80dac33ad47c28c48d331c08f07b0",
      //     isTable: false,
      //     from:"TextInputapproval_status",
      ...
      //   type={
      //     name:'approval_status',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approval_status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approval_status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approval_status.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_details_group136e4?.approval_status)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "73f80dac33ad47c28c48d331c08f07b0") {
        handleChange({target:{value:assignment_details_group136e4Ref?.current?.approval_status||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "73f80dac33ad47c28c48d331c08f07b0") {
        handleBlur({target:{value:assignment_details_group136e4Ref?.current?.approval_status||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_details_group136e4((pre:any)=>({...pre,approval_status:dfd_assetassignments_v1Props.data[0]?.approval_status}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (approval_statusf07b0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_details_group136e4?.approval_status||""}
         disabled= {approval_statusf07b0?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.approval_status ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputapproval_status
