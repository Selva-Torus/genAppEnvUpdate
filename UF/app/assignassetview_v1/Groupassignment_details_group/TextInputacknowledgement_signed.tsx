...
const TextInputacknowledgement_signed = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1|b50b5e3a7b314f70bda35687ca3e035e|properties.acknowledgement_signed"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|472c369f56f3afb6e920bdd86cc136e4|c4101e735fd343fa8c307bffa835ee58"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'acknowledgement_signed',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,acknowledgement_signed:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_details_group136e4((prev: any) => ({ ...prev, acknowledgement_signed: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_details_group136e4?.acknowledgement_signed == "" || assignment_details_group136e4?.acknowledgement_signed == undefined){
      assignment_details_group136e4.acknowledgement_signed = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_details_group136e4?.acknowledgement_signed);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,acknowledgement_signed:"invalid"}}));
        }
    }else if(assignment_details_group136e4?.acknowledgement_signed !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_details_group136e4?.acknowledgement_signed);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c4101e735fd343fa8c307bffa835ee58"
      );
      ...
      //     controlId: "c4101e735fd343fa8c307bffa835ee58",
      //     isTable: false,
      //     from:"TextInputacknowledgement_signed",
      ...
      //   type={
      //     name:'acknowledgement_signed',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.acknowledgement_signed.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.acknowledgement_signed.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.acknowledgement_signed.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_details_group136e4?.acknowledgement_signed)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "c4101e735fd343fa8c307bffa835ee58") {
        handleChange({target:{value:assignment_details_group136e4Ref?.current?.acknowledgement_signed||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c4101e735fd343fa8c307bffa835ee58") {
        handleBlur({target:{value:assignment_details_group136e4Ref?.current?.acknowledgement_signed||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_details_group136e4((pre:any)=>({...pre,acknowledgement_signed:dfd_assetassignments_v1Props.data[0]?.acknowledgement_signed}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (acknowledgement_signed5ee58?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_details_group136e4?.acknowledgement_signed||""}
         disabled= {acknowledgement_signed5ee58?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Acknowledgement Signed"
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.acknowledgement_signed ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputacknowledgement_signed
