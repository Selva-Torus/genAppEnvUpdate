...
const TextInputapproved_by = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|472c369f56f3afb6e920bdd86cc136e4|528c6ed0c340d9d0f9240cd1a962b89c"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,approved_by:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_details_group136e4((prev: any) => ({ ...prev, approved_by: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_details_group136e4?.approved_by == "" || assignment_details_group136e4?.approved_by == undefined){
      assignment_details_group136e4.approved_by = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_details_group136e4?.approved_by);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,approved_by:"invalid"}}));
        }
    }else if(assignment_details_group136e4?.approved_by !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_details_group136e4?.approved_by);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "528c6ed0c340d9d0f9240cd1a962b89c"
      );
      ...
      //     controlId: "528c6ed0c340d9d0f9240cd1a962b89c",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_details_group136e4?.approved_by)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "528c6ed0c340d9d0f9240cd1a962b89c") {
        handleChange({target:{value:assignment_details_group136e4Ref?.current?.approved_by||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "528c6ed0c340d9d0f9240cd1a962b89c") {
        handleBlur({target:{value:assignment_details_group136e4Ref?.current?.approved_by||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_details_group136e4((pre:any)=>({...pre,approved_by:dfd_assetassignments_v1Props.data[0]?.approved_by}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (approved_by2b89c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_details_group136e4?.approved_by||""}
         disabled= {approved_by2b89c?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.approved_by ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
