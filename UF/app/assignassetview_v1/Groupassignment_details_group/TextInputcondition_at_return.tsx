...
const TextInputcondition_at_return = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1|b50b5e3a7b314f70bda35687ca3e035e|properties.condition_at_return"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|472c369f56f3afb6e920bdd86cc136e4|0acfb20582a6454bb0eea1883511d3c7"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'condition_at_return',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,condition_at_return:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_details_group136e4((prev: any) => ({ ...prev, condition_at_return: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_details_group136e4?.condition_at_return == "" || assignment_details_group136e4?.condition_at_return == undefined){
      assignment_details_group136e4.condition_at_return = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_details_group136e4?.condition_at_return);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,condition_at_return:"invalid"}}));
        }
    }else if(assignment_details_group136e4?.condition_at_return !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_details_group136e4?.condition_at_return);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "0acfb20582a6454bb0eea1883511d3c7"
      );
      ...
      //     controlId: "0acfb20582a6454bb0eea1883511d3c7",
      //     isTable: false,
      //     from:"TextInputcondition_at_return",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'condition_at_return', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'condition_at_return',type:'text'};
      //   type={
      //     name:'condition_at_return',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_return.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_return.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_return.type
      //   }
      ...
      //   type={
      //     name:'condition_at_return',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_return.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_return.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_return.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_details_group136e4?.condition_at_return)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "0acfb20582a6454bb0eea1883511d3c7") {
        handleChange({target:{value:assignment_details_group136e4Ref?.current?.condition_at_return||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "0acfb20582a6454bb0eea1883511d3c7") {
        handleBlur({target:{value:assignment_details_group136e4Ref?.current?.condition_at_return||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_details_group136e4((pre:any)=>({...pre,condition_at_return:dfd_assetassignments_v1Props.data[0]?.condition_at_return}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (condition_at_return1d3c7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_details_group136e4?.condition_at_return||""}
         disabled= {condition_at_return1d3c7?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Condition At Return"
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.condition_at_return ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcondition_at_return
