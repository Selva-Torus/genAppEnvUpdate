...
const TextInputassignment_status = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|19079e66a48845fc878544720241057b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'assignment_status',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assignment_status:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, assignment_status: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.assignment_status == "" || assignment_information_groupc96e9?.assignment_status == undefined){
      assignment_information_groupc96e9.assignment_status = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.assignment_status);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assignment_status:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.assignment_status !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.assignment_status);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "19079e66a48845fc878544720241057b"
      );
      ...
      //     controlId: "19079e66a48845fc878544720241057b",
      //     isTable: false,
      //     from:"TextInputassignment_status",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'assignment_status', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'assignment_status',type:'text'};
      //   type={
      //     name:'assignment_status',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.assignment_status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.assignment_status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.assignment_status.type
      //   }
      ...
      //   type={
      //     name:'assignment_status',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assignment_status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assignment_status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assignment_status.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.assignment_status)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "19079e66a48845fc878544720241057b") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.assignment_status||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "19079e66a48845fc878544720241057b") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.assignment_status||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,assignment_status:dfd_assetassignments_v1Props.data[0]?.assignment_status}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (assignment_status1057b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.assignment_status||""}
         disabled= {assignment_status1057b?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.assignment_status ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputassignment_status
