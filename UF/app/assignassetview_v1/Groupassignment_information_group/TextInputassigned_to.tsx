...
const TextInputassigned_to = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|ca2739cedc1df0f38f9d29305cfad6a1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assigned_to:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, assigned_to: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.assigned_to == "" || assignment_information_groupc96e9?.assigned_to == undefined){
      assignment_information_groupc96e9.assigned_to = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.assigned_to);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assigned_to:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.assigned_to !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.assigned_to);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ca2739cedc1df0f38f9d29305cfad6a1"
      );
      ...
      //     controlId: "ca2739cedc1df0f38f9d29305cfad6a1",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.assigned_to)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "ca2739cedc1df0f38f9d29305cfad6a1") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.assigned_to||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ca2739cedc1df0f38f9d29305cfad6a1") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.assigned_to||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,assigned_to:dfd_assetassignments_v1Props.data[0]?.assigned_to}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (assigned_toad6a1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.assigned_to||""}
         disabled= {assigned_toad6a1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.assigned_to ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
