...
const TextInputexpected_return_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|9eb0785694af44658af504655f9df53d"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'expected_return_date',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,expected_return_date:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, expected_return_date: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.expected_return_date == "" || assignment_information_groupc96e9?.expected_return_date == undefined){
      assignment_information_groupc96e9.expected_return_date = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.expected_return_date);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,expected_return_date:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.expected_return_date !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.expected_return_date);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9eb0785694af44658af504655f9df53d"
      );
      ...
      //     controlId: "9eb0785694af44658af504655f9df53d",
      //     isTable: false,
      //     from:"TextInputexpected_return_date",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'expected_return_date', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'expected_return_date',type:'text'};
      //   type={
      //     name:'expected_return_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expected_return_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expected_return_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expected_return_date.type
      //   }
      ...
      //   type={
      //     name:'expected_return_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expected_return_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expected_return_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expected_return_date.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.expected_return_date)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "9eb0785694af44658af504655f9df53d") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.expected_return_date||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9eb0785694af44658af504655f9df53d") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.expected_return_date||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,expected_return_date:dfd_assetassignments_v1Props.data[0]?.expected_return_date}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (expected_return_datedf53d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.expected_return_date||""}
         disabled= {expected_return_datedf53d?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.expected_return_date ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputexpected_return_date
