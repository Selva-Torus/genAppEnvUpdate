...
const TextInputassigned_at = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|01462531f7ef4bd5b1f6eda5296ca20b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'assigned_date',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assigned_date:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, assigned_date: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.assigned_date == "" || assignment_information_groupc96e9?.assigned_date == undefined){
      assignment_information_groupc96e9.assigned_date = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.assigned_date);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,assigned_date:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.assigned_date !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.assigned_date);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "01462531f7ef4bd5b1f6eda5296ca20b"
      );
      ...
      //     controlId: "01462531f7ef4bd5b1f6eda5296ca20b",
      //     isTable: false,
      //     from:"TextInputassigned_at",
      ...
      //   type={
      //     name:'assigned_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assigned_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assigned_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.assigned_date.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.assigned_date)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "01462531f7ef4bd5b1f6eda5296ca20b") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.assigned_date||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "01462531f7ef4bd5b1f6eda5296ca20b") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.assigned_date||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,assigned_date:dfd_assetassignments_v1Props.data[0]?.assigned_date}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (assigned_atca20b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.assigned_date||""}
         disabled= {assigned_atca20b?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.assigned_date ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputassigned_at
