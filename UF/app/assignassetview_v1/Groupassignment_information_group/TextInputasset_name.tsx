...
const TextInputasset_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|4e9f59e2d6544d96bf43c1f9a3d39101"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,asset_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, asset_name: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.asset_name == "" || assignment_information_groupc96e9?.asset_name == undefined){
      assignment_information_groupc96e9.asset_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.asset_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,asset_name:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.asset_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.asset_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "4e9f59e2d6544d96bf43c1f9a3d39101"
      );
      ...
      //     controlId: "4e9f59e2d6544d96bf43c1f9a3d39101",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.asset_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "4e9f59e2d6544d96bf43c1f9a3d39101") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.asset_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4e9f59e2d6544d96bf43c1f9a3d39101") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.asset_name||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,asset_name:dfd_assetassignments_v1Props.data[0]?.asset_name}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (asset_name39101?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.asset_name||""}
         disabled= {asset_name39101?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.asset_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
