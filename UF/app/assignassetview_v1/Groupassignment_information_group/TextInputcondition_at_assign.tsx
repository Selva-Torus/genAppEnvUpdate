...
const TextInputcondition_at_assign = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetView:AFVK:v1|8b8be57613e6374a30114f7b757c96e9|100c2d1b28734bdb8d04e0ebccbf6852"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'condition_at_assign',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,condition_at_assign:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_groupc96e9((prev: any) => ({ ...prev, condition_at_assign: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_groupc96e9?.condition_at_assign == "" || assignment_information_groupc96e9?.condition_at_assign == undefined){
      assignment_information_groupc96e9.condition_at_assign = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_groupc96e9?.condition_at_assign);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAssetView_v1:{...pre?.assignAssetView_v1,condition_at_assign:"invalid"}}));
        }
    }else if(assignment_information_groupc96e9?.condition_at_assign !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_groupc96e9?.condition_at_assign);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "100c2d1b28734bdb8d04e0ebccbf6852"
      );
      ...
      //     controlId: "100c2d1b28734bdb8d04e0ebccbf6852",
      //     isTable: false,
      //     from:"TextInputcondition_at_assign",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'condition_at_assign', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'condition_at_assign',type:'text'};
      //   type={
      //     name:'condition_at_assign',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_assign.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_assign.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.condition_at_assign.type
      //   }
      ...
      //   type={
      //     name:'condition_at_assign',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_assign.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_assign.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.condition_at_assign.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_groupc96e9?.condition_at_assign)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "100c2d1b28734bdb8d04e0ebccbf6852") {
        handleChange({target:{value:assignment_information_groupc96e9Ref?.current?.condition_at_assign||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "100c2d1b28734bdb8d04e0ebccbf6852") {
        handleBlur({target:{value:assignment_information_groupc96e9Ref?.current?.condition_at_assign||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_groupc96e9((pre:any)=>({...pre,condition_at_assign:dfd_assetassignments_v1Props.data[0]?.condition_at_assign}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (condition_at_assignf6852?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={assignment_information_groupc96e9?.condition_at_assign||""}
         disabled= {condition_at_assignf6852?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Condition At Assign"
      errorMessage={error}
        validationState={validate?.assignAssetView_v1?.condition_at_assign ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcondition_at_assign
