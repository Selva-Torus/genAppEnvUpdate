...
import GroupUser_Grp  from "./GroupUser_Grp/GroupUser_Grp";


export default function PageUserHomeScreenV1() {
  ...
  const {dashboard_for_user_v1Props, setdashboard_for_user_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [checkuser_grp,setCheckuser_grp,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function card_data_v1(pagination:any): Promise<void>{
        ...
        if(dashboard_for_user_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< dashboard_for_user_v1Props.length;i++){
            if(dashboard_for_user_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:card_data:AFVK:v1"){
              delete dashboard_for_user_v1Props[i].DFDkey;
              filterData.push(dashboard_for_user_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function claim_table_data_v1(pagination:any): Promise<void>{
        ...
        if(dashboard_for_user_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< dashboard_for_user_v1Props.length;i++){
            if(dashboard_for_user_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claim_table_data:AFVK:v1"){
              delete dashboard_for_user_v1Props[i].DFDkey;
              filterData.push(dashboard_for_user_v1Props[i])
            }           
          }
  ...
  return (
    ...
    {securityData[accessProfile]?.allowedGroups?.includes("user_grp") && initialLoad &&<GroupUser_Grp 
