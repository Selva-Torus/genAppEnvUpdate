...
const Cardpendingcard = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setuser_grpd6690((pre:any)=>({...pre,pendingcard:dfd_card_data_v1Props[0]?.pendingcard}));
    }
  ...
  if (pendingcardee3c0?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `11 / 15`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        label="Pending"
        disabled= {pendingcardee3c0?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {user_grpd6690?.pendingcard?user_grpd6690?.pendingcard:"0"}
      </Card>
      </div>
  )
}
