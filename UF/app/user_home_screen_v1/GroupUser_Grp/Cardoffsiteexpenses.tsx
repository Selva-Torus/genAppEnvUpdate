...
const Cardoffsiteexpenses = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setuser_grpd6690((pre:any)=>({...pre,offsiteexpenses:dfd_card_data_v1Props[0]?.offsiteexpenses}));
    }
  ...
  if (offsiteexpensescd925?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `21 / 25`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        disabled= {offsiteexpensescd925?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {user_grpd6690?.offsiteexpenses?user_grpd6690?.offsiteexpenses:"0"}
      </Card>
      </div>
  )
}
