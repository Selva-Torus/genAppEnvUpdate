...
const Cardapprovedcard = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  useEffect(() => {
    if(Array.isArray(dfd_card_data_v1Props)){
      setuser_grpd6690((pre:any)=>({...pre,approvedcard:dfd_card_data_v1Props[0]?.approvedcard}));
    }
  ...
  if (approvedcardc5971?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 5`,gridRow: `1 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
        style={style}
        className=""
        theme="info"
        ...
        icon="MdCheck"
        label="Approved"
        disabled= {approvedcardc5971?.isDisabled ? true : false}
        onClick={handleClick}  
        contentAlign={"center"}
        >
      {user_grpd6690?.approvedcard?user_grpd6690?.approvedcard:"0"}
      </Card>
      </div>
  )
}
