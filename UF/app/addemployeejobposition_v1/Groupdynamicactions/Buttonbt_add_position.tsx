...
const Buttonbt_add_position = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "bc8c6d8c1be89615cf1115d40c35f98e"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("bt_add_position" in dynamicactions76c44Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions76c44Props?.dynamicActionRule?.bt_add_position,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("bt_add_position" in addemployeejobposition_v1?.dynamicactions && addemployeejobposition_v1?.dynamicactions["bt_add_position"]?.itsHaveArtifact== true)
      {
        setShowFlag(addemployeejobposition_v1?.dynamicactions["bt_add_position"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addemployeejobposition_v1?.dynamicactions?.bt_add_position?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "bt_add_position5f98e") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "bc8c6d8c1be89615cf1115d40c35f98e") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "bt_add_position5f98e");
    return () => {
  ...
  useEffect(()=>{
    if (!bt_add_position5f98e?.trigger) return;
      if(bt_add_position5f98e?.trigger){
      setbt_add_position5f98e((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[bt_add_position5f98e?.trigger])

  useEffect(()=>{
    if(bt_add_position5f98e?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[bt_add_position5f98e?.refresh])
  ...
  async function handleSavef98e_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const dynamicactions76c44Props = dynamicactions76c44PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.addEmployeeJobPosition_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(dynamicactions76c44Ref.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobPosition:AFVK:v1|f6531064c0a4c041cb9aba696def7e4b"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobPosition:AFVK:v1|f6531064c0a4c041cb9aba696def7e4b"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "bc8c6d8c1be89615cf1115d40c35f98e",
  "type": "button",
  "name": "bt_add_position",
  "label": "bt_add_position",
  "sequence": 1,
  "children": [
    {
      "id": "bc8c6d8c1be89615cf1115d40c35f98e.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "bc8c6d8c1be89615cf1115d40c35f98e.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "bc8c6d8c1be89615cf1115d40c35f98e.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Job Position Added Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobPosition:AFVK:v1|f6531064c0a4c041cb9aba696def7e4b"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1";
      sourceId+= "|"+"60586a3d869b2025697ef30c8e476c44";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1"+"|"+"60586a3d869b2025697ef30c8e476c44"+"|"+eventProperty.id;
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions76c44Ref.current, ...nullFilter(overall_groupae38a), ...nullFilter(position_information_group5335b), ...nullFilter(compensation_benfits_groupb46e6) });
    let reworkKeys:any[]=[];
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"60586a3d869b2025697ef30c8e476c44",
              controlId:"bc8c6d8c1be89615cf1115d40c35f98e"
            };
            if (encryptionFlagCont) {
        ...
        te_saveBody.controlName = "bt_add_position";
        if(mainData?.upId)
        {
        ...
        if(dynamicactions76c44?.upId){
          te_saveBody['upId']= dynamicactions76c44?.upId;
        }
        if(dynamicactions76c44?.upid){
          te_saveBody['upId']= dynamicactions76c44?.upid;
        }
        ...
        if (dynamicactions76c44Props.ssKey !== '' && dynamicactions76c44Props.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactions76c44Props.ssKey;
    ...
    //infoMsg
    toast('Job Position Added Successfully', 'success',false);
    }catch(err:any){
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions76c44((prev: any) => ({ ...prev, bt_add_position: true }));
        //onClick

    //saveHandler
    await handleSavef98e_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions76c44((prev: any) => ({ ...prev, bt_add_position: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addemployeejobposition_v1?.dynamicactions?.bt_add_position,dynamicactions76c44Props?.dynamicActionRule?.bt_add_position,])

 if (bt_add_position5f98e?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {bt_add_position5f98e?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Add Position")}
...
export default Buttonbt_add_position
