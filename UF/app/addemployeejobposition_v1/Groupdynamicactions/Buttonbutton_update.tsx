 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "9ddb66f44cea14fcc4e7fb9e29794589"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactions76c44Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions76c44Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in addemployeejobposition_v1?.dynamicactions && addemployeejobposition_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(addemployeejobposition_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addemployeejobposition_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_update94589") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "9ddb66f44cea14fcc4e7fb9e29794589") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_update94589");
    return () => {
  ...
  useEffect(()=>{
    if (!button_update94589?.trigger) return;
      if(button_update94589?.trigger){
      setbutton_update94589((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_update94589?.trigger])

  useEffect(()=>{
    if(button_update94589?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_update94589?.refresh])
  ...
  async function handleSave4589_1_1_1(){
  ...
  const hasInvalidField = Object.values(currentValidate?.addEmployeeJobPosition_v1 || {}).some(val => {
    ...
    try{
      ...
      let lockedKeysLength:number;
        if(dynamicactions76c44?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "9ddb66f44cea14fcc4e7fb9e29794589",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "9ddb66f44cea14fcc4e7fb9e29794589.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "9ddb66f44cea14fcc4e7fb9e29794589.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "9ddb66f44cea14fcc4e7fb9e29794589.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Job Position Updated Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions76c44?.upId? [dynamicactions76c44?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions76c44?.upId? [dynamicactions76c44?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions76c44?.upId? [dynamicactions76c44?.upId ] : dynamicactions76c44?.trs_process_id? [dynamicactions76c44?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"9ddb66f44cea14fcc4e7fb9e29794589"
            };
            if (encryptionFlagCont) {
    ...
    te_eventEmitterBody["ssKey"] = dynamicactions76c44Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Job Position Updated Successfully', 'success',true);
    }catch(err:any){
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions76c44((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSave4589_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions76c44((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addemployeejobposition_v1?.dynamicactions?.button_update,dynamicactions76c44Props?.dynamicActionRule?.button_update,])

 if (button_update94589?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_update94589?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Update")}
