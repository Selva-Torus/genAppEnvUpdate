...
const TextInputsalary_range_max = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|f1469b256669687453aa33ae83eb46e6|b81ed8a72dce53221b4f8759d48ebe1d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,salary_range_max:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, salary_range_max: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_groupb46e6?.salary_range_max == "" || compensation_benfits_groupb46e6?.salary_range_max == undefined){
      compensation_benfits_groupb46e6.salary_range_max = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_groupb46e6?.salary_range_max);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,salary_range_max:"invalid"}}));
        }
    }else if(compensation_benfits_groupb46e6?.salary_range_max !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_groupb46e6?.salary_range_max);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b81ed8a72dce53221b4f8759d48ebe1d"
      );
      ...
      //     controlId: "b81ed8a72dce53221b4f8759d48ebe1d",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_groupb46e6?.salary_range_max)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "b81ed8a72dce53221b4f8759d48ebe1d") {
        handleChange({target:{value:compensation_benfits_groupb46e6Ref?.current?.salary_range_max||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b81ed8a72dce53221b4f8759d48ebe1d") {
        handleBlur({target:{value:compensation_benfits_groupb46e6Ref?.current?.salary_range_max||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,salary_range_max:dfd_jobpositions_v1Props.data[0]?.salary_range_max}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (salary_range_maxebe1d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_groupb46e6?.salary_range_max||""}
         disabled= {salary_range_maxebe1d?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.salary_range_max ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
