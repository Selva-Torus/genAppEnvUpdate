 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "113864fe91c940868e1b4b3edad18094"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,is_open:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[is_open18094?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props?.length == 1){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,is_open:dfd_jobpositions_v1Props[0]?.is_open}))
    }
  },[dfd_jobpositions_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, is_open: checked }));
  ...
  return (
    <div 
      ...
      <Switch
        ...
        headerText="Is Open"
        headerPosition="top"
        disabled= {is_open18094?.isDisabled ? true : false}
        checkedContent="Yes"                                                                                                                                             
        uncheckedContent="No"
        checked={compensation_benfits_groupb46e6?.is_open || false} 
