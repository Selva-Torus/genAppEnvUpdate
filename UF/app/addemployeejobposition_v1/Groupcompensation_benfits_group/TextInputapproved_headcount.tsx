...
const TextInputapproved_headcount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|f1469b256669687453aa33ae83eb46e6|3f20271d13ff462f8d49258d9b942f81"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,approved_headcount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, approved_headcount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_groupb46e6?.approved_headcount == "" || compensation_benfits_groupb46e6?.approved_headcount == undefined){
      compensation_benfits_groupb46e6.approved_headcount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_groupb46e6?.approved_headcount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,approved_headcount:"invalid"}}));
        }
    }else if(compensation_benfits_groupb46e6?.approved_headcount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_groupb46e6?.approved_headcount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3f20271d13ff462f8d49258d9b942f81"
      );
      ...
      //     controlId: "3f20271d13ff462f8d49258d9b942f81",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_groupb46e6?.approved_headcount)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "3f20271d13ff462f8d49258d9b942f81") {
        handleChange({target:{value:compensation_benfits_groupb46e6Ref?.current?.approved_headcount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3f20271d13ff462f8d49258d9b942f81") {
        handleBlur({target:{value:compensation_benfits_groupb46e6Ref?.current?.approved_headcount||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,approved_headcount:dfd_jobpositions_v1Props.data[0]?.approved_headcount}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (approved_headcount42f81?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_groupb46e6?.approved_headcount||""}
         disabled= {approved_headcount42f81?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.approved_headcount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
