 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "c247b3932cbf42ab9704cdb8794fe60a"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,travel_required:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[travel_requiredfe60a?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props?.length == 1){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,travel_required:dfd_jobpositions_v1Props[0]?.travel_required}))
    }
  },[dfd_jobpositions_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, travel_required: checked }));
  ...
  if (travel_requiredfe60a?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="top"
        disabled= {travel_requiredfe60a?.isDisabled ? true : false}
        checkedContent="Yes"                                                                                                                                             
        uncheckedContent="No"
        checked={compensation_benfits_groupb46e6?.travel_required || false} 
