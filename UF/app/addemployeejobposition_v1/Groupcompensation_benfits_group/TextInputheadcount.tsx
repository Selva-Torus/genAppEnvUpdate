...
const TextInputheadcount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|f1469b256669687453aa33ae83eb46e6|437466b1349c47a495bdebc6d245aefa"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,headcount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, headcount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_groupb46e6?.headcount == "" || compensation_benfits_groupb46e6?.headcount == undefined){
      compensation_benfits_groupb46e6.headcount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_groupb46e6?.headcount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,headcount:"invalid"}}));
        }
    }else if(compensation_benfits_groupb46e6?.headcount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_groupb46e6?.headcount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "437466b1349c47a495bdebc6d245aefa"
      );
      ...
      //     controlId: "437466b1349c47a495bdebc6d245aefa",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_groupb46e6?.headcount)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "437466b1349c47a495bdebc6d245aefa") {
        handleChange({target:{value:compensation_benfits_groupb46e6Ref?.current?.headcount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "437466b1349c47a495bdebc6d245aefa") {
        handleBlur({target:{value:compensation_benfits_groupb46e6Ref?.current?.headcount||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,headcount:dfd_jobpositions_v1Props.data[0]?.headcount}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (headcount5aefa?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_groupb46e6?.headcount||""}
         disabled= {headcount5aefa?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.headcount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
