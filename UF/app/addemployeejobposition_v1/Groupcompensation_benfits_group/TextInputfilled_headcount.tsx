...
const TextInputfilled_headcount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|f1469b256669687453aa33ae83eb46e6|958d4bc63f7c4283bce9aa2a266049fc"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,filled_headcount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_groupb46e6((prev: any) => ({ ...prev, filled_headcount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_groupb46e6?.filled_headcount == "" || compensation_benfits_groupb46e6?.filled_headcount == undefined){
      compensation_benfits_groupb46e6.filled_headcount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_groupb46e6?.filled_headcount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,filled_headcount:"invalid"}}));
        }
    }else if(compensation_benfits_groupb46e6?.filled_headcount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_groupb46e6?.filled_headcount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "958d4bc63f7c4283bce9aa2a266049fc"
      );
      ...
      //     controlId: "958d4bc63f7c4283bce9aa2a266049fc",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_groupb46e6?.filled_headcount)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "958d4bc63f7c4283bce9aa2a266049fc") {
        handleChange({target:{value:compensation_benfits_groupb46e6Ref?.current?.filled_headcount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "958d4bc63f7c4283bce9aa2a266049fc") {
        handleBlur({target:{value:compensation_benfits_groupb46e6Ref?.current?.filled_headcount||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_groupb46e6((pre:any)=>({...pre,filled_headcount:dfd_jobpositions_v1Props.data[0]?.filled_headcount}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (filled_headcount049fc?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_groupb46e6?.filled_headcount||""}
         disabled= {filled_headcount049fc?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.filled_headcount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
