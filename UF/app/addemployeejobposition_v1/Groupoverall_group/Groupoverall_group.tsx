...
const Groupoverall_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "de6b6221165720e8e55628a046aae38a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverall_groupae38aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("position_information_group")){
        setposition_information_group5335b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(position_information_group5335b?.isDisabled==null)
      {
        setposition_information_group5335b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("compensation_benfits_group")){
        setcompensation_benfits_groupb46e6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(compensation_benfits_groupb46e6?.isDisabled==null)
      {
        setcompensation_benfits_groupb46e6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactions76c44((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactions76c44?.isDisabled==null)
      {
        setdynamicactions76c44((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("position_id")){
        setposition_id8e1e3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(position_id8e1e3?.isDisabled==null)
      {
        setposition_id8e1e3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['position_id'] = position_id8e1e3,
        codeStates['setposition_id'] = setposition_id8e1e3,
  ...
  const overall_groupae38aRef = useRef<any>(null);
  const handleClearSearch = () => {
    overall_groupae38aRef.current?.setSearchParams();
    overall_groupae38aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overall_groupae38a) && Object.keys(overall_groupae38a)?.length>0)
      {
        setoverall_groupae38a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overall_groupae38aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddemployeejobposition_v1((pre:any)=>({...pre,_selectedGroup_:"overall_group"}))
        }}
    >
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("position_id") ?<Textposition_id   /* 8e1e3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
