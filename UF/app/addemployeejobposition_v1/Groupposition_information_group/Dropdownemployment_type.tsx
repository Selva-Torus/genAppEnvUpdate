 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "21a76db93d104c5e9302810c62d9bb76"
    );
  ...
  },[employment_type9bb76?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setposition_information_group5335b((prev: any) => ({ ...prev,employment_type9bb76: value }))
      setIsRequredData(false)
    }else{
      let temp:any = position_information_group5335b;
      delete temp.employment_type;
      delete temp.EMPLOYMENT_TYPE;
      delete temp.employment_type9bb76;
      setposition_information_group5335b(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,employment_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(position_information_group5335b.employment_type){
      if(Array.isArray(dfd_employmenttypecombo_v1Props)){
        if(dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)){
          setdropdownValue([dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)?.employment_type])
          tempValue=dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)?.employment_type
        }else{
          setdropdownValue([position_information_group5335b.employment_type])
          tempValue=position_information_group5335b.employment_type
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{employment_type:position_information_group5335b.employment_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)?.employment_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.employment_type === position_information_group5335b.employment_type)?.employment_type
      }else{
        setdropdownValue([position_information_group5335b.employment_type])
        tempValue=position_information_group5335b.employment_type
      }   
    ...
    if(employment_type9bb76?.trigger == true)
    {
      await handlechange(tempValue)
      setemployment_type9bb76((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[position_information_group5335b.employment_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employmenttypecombo_v1Props) && dfd_employmenttypecombo_v1Props?.length == 1){
    // setposition_information_group5335b((pre:any)=>({...pre,employment_type:dfd_employmenttypecombo_v1Props[0]?.employment_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setposition_information_group5335b((prev: any) => ({ ...prev, employment_type: getMapperDetailsBindValues[value],EMPLOYMENT_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setposition_information_group5335b((prev: any) => ({ ...prev, employment_type: '', employment_type9bb76: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,employment_type:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(position_information_group5335b?.employment_type == "" || position_information_group5335b?.employment_type == undefined){
      position_information_group5335b.employment_type = "";
      const validate:any = v.safeParse(schema, position_information_group5335b?.employment_type);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,employment_type:"invalid"}}));
        }
    }else if(position_information_group5335b?.employment_type !== ""){
    const validate:any = v.safeParse(schema, position_information_group5335b?.employment_type);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,employment_type:"invalid"}}));
  ...
  useEffect(() => { position_information_group5335bRef.current = position_information_group5335b; }, [position_information_group5335b]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "21a76db93d104c5e9302810c62d9bb76") {
        handleClick(position_information_group5335bRef?.current?.employment_type9bb76?position_information_group5335bRef?.current?.employment_type9bb76:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "21a76db93d104c5e9302810c62d9bb76");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setposition_information_group5335b((pre:any)=>({...pre,employment_type:""}))
    else
      setInitialCount(1)
  },[employment_type9bb76?.refresh])
  

  if (employment_type9bb76?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {employment_type9bb76?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={position_information_group5335b?.employment_type9bb76 ? [position_information_group5335b?.employment_type9bb76] : (position_information_group5335b?.employment_type ? dropdownValue : [])}
        validationState={validate?.addEmployeeJobPosition_v1?.employment_type ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
