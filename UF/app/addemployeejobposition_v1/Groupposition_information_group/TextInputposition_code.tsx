...
const TextInputposition_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|b2c06e29b62cff17bf1e2725d255335b|e7b5d526ed2a20dc3f3cb87e3a3a4553"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,position_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group5335b((prev: any) => ({ ...prev, position_code: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(position_information_group5335b?.position_code == "" || position_information_group5335b?.position_code == undefined){
      position_information_group5335b.position_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, position_information_group5335b?.position_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,position_code:"invalid"}}));
        }
    }else if(position_information_group5335b?.position_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +position_information_group5335b?.position_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e7b5d526ed2a20dc3f3cb87e3a3a4553"
      );
      ...
      //     controlId: "e7b5d526ed2a20dc3f3cb87e3a3a4553",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!position_information_group5335b?.position_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "e7b5d526ed2a20dc3f3cb87e3a3a4553") {
        handleChange({target:{value:position_information_group5335bRef?.current?.position_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e7b5d526ed2a20dc3f3cb87e3a3a4553") {
        handleBlur({target:{value:position_information_group5335bRef?.current?.position_code||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group5335b((pre:any)=>({...pre,position_code:dfd_jobpositions_v1Props.data[0]?.position_code}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (position_codea4553?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={position_information_group5335b?.position_code||""}
         disabled= {position_codea4553?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.position_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
