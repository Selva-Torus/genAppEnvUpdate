...
const TextInputjob_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobPosition:AFVK:v1|b2c06e29b62cff17bf1e2725d255335b|a6165846447f4cb089017b81de177c64"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobPosition_v1:{...pre?.addEmployeeJobPosition_v1,job_level:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group5335b((prev: any) => ({ ...prev, job_level: +e.target.value }));
    }
    else{
    setposition_information_group5335b((prev: any) => ({ ...prev, job_level: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a6165846447f4cb089017b81de177c64"
      );
      ...
      //     controlId: "a6165846447f4cb089017b81de177c64",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a6165846447f4cb089017b81de177c64") {
        handleChange({target:{value:position_information_group5335bRef?.current?.job_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a6165846447f4cb089017b81de177c64") {
        handleBlur({target:{value:position_information_group5335bRef?.current?.job_level||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group5335b((pre:any)=>({...pre,job_level:dfd_jobpositions_v1Props.data[0]?.job_level}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (job_level77c64?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={position_information_group5335b?.job_level||""}
         disabled= {job_level77c64?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobPosition_v1?.job_level ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
