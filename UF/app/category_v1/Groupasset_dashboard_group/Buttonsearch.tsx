...
import PageCategorysearchpage2 from '@/app/categorysearch_v1/categorysearch_v1page';
...
const Buttonsearch = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['categorysearch_v1'] = categorysearch_v1Props,
        codeStates['setcategorysearch_v1'] = setcategorysearch_v1Props,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "78155ad3071c40a780c1192402f48da3"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "78155ad3071c40a780c1192402f48da3") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "search48da3");
    return () => {
  ...
  useEffect(()=>{
    if (!search48da3?.trigger) return;
      if(search48da3?.trigger){
      setsearch48da3((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setasset_dashboard_group485d3((prev: any) => ({ ...prev, search: true }));
        //onClick

    // showArtifactAsModal
    let filterProps2:any =  [];
    let filterData2 = await getFilterProps(filterProps2,asset_dashboard_group485d3);
    setcategorysearch_v1Props([...filterData2 ]);
    setShowProfileAsModalOpen2(true);
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setasset_dashboard_group485d3((prev: any) => ({ ...prev, search: false }));
 ...
 if (search48da3?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `17 / 21`,gridRow: `21 / 28`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showProfileAsModalOpen2 && hiddenModalForTrigger && (
          <div style={{ display: 'none' }}>
            <PageCategorysearchpage2/>
          </div>
        )}
      <Modal 
        ...
        showOverlay = {true}
        position = {"top-right"}
        modalName = "categorysearch"
        className='w-[40%] h-[] bg-gray-50 overflow-auto'
      >
        {!hiddenModalForTrigger && <PageCategorysearchpage2/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          disabled= {search48da3?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
