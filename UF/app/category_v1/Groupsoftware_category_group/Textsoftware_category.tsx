...
const Textsoftware_category = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetcategorycards_v1Props && !dfd_assetcategorycards_v1Props.hasLogicCenter) {
        ...
        setsoftware_category_group6e622((pre: any) => {
          return { ...pre, software_category: api_paginationData.data.records[0]?.software_category }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetcategorycards_v1Props) && dfd_assetcategorycards_v1Props && !software_category_group6e622.software_category){
        setsoftware_category_group6e622((pre:any)=>({...pre,software_category:dfd_assetcategorycards_v1Props[0]?.software_category}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[software_category5cc47?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategorycards_v1Props) && !software_category_group6e622.software_category){
    setsoftware_category_group6e622((pre:any)=>({...pre,software_category:dfd_assetcategorycards_v1Props[0]?.software_category}));
  }
  },[dfd_assetcategorycards_v1Props])

  if (software_category5cc47?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.software_category : (software_category_group6e622?.software_category || ""))}
</Text>
  </div>
  )
}

export default Textsoftware_category
