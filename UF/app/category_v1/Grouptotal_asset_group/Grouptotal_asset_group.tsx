...
import Dividertotal_category_divider  from "./Dividertotal_category_divider";
import Texttc_icon  from "./Texttc_icon";
import Texttotal_category_text  from "./Texttotal_category_text";
import Texttotal_categories  from "./Texttotal_categories";
...
const Grouptotal_asset_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "b5418aa8e9f6a9423edabaaf997fe2e6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_asset_groupfe2e6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_category_divider")){
        settotal_category_divider52a07({...total_category_divider52a07,isDisabled:true});

    }else
    {
      if(total_category_divider52a07?.isDisabled==null)
      {
        settotal_category_divider52a07({...total_category_divider52a07,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("tc_icon")){
        settc_icon1ed4f({...tc_icon1ed4f,isDisabled:true});

    }else
    {
      if(tc_icon1ed4f?.isDisabled==null)
      {
        settc_icon1ed4f({...tc_icon1ed4f,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_category_text")){
        settotal_category_textb2d2f({...total_category_textb2d2f,isDisabled:true});

    }else
    {
      if(total_category_textb2d2f?.isDisabled==null)
      {
        settotal_category_textb2d2f({...total_category_textb2d2f,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_categories")){
        settotal_categories55fa9({...total_categories55fa9,isDisabled:true});

    }else
    {
      if(total_categories55fa9?.isDisabled==null)
      {
        settotal_categories55fa9({...total_categories55fa9,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_category_divider'] = total_category_divider52a07,
        codeStates['settotal_category_divider'] = settotal_category_divider52a07,
        codeStates['tc_icon'] = tc_icon1ed4f,
        codeStates['settc_icon'] = settc_icon1ed4f,
        codeStates['total_category_text'] = total_category_textb2d2f,
        codeStates['settotal_category_text'] = settotal_category_textb2d2f,
        codeStates['total_categories'] = total_categories55fa9,
        codeStates['settotal_categories'] = settotal_categories55fa9,
  ...
  const total_asset_groupfe2e6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_asset_groupfe2e6Ref.current?.setSearchParams();
    total_asset_groupfe2e6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(total_asset_groupfe2e6) && Object.keys(total_asset_groupfe2e6)?.length>0)
      {
        settotal_asset_groupfe2e6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [total_asset_groupfe2e6Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("total_category_divider") ?<Dividertotal_category_divider   /* 52a07 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("tc_icon") ?<Texttc_icon   /* 1ed4f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_category_text") ?<Texttotal_category_text   /* b2d2f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_categories") ?<Texttotal_categories   /* 55fa9 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
