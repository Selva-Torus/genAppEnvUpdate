...
const Groupaddt__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "75e47dec9ce054f3edf561d726f6ba4f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddt__group6ba4fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("comments")){
        setcommentsc519d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(commentsc519d?.isDisabled==null)
      {
        setcommentsc519d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("improvements")){
        setimprovements56baa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(improvements56baa?.isDisabled==null)
      {
        setimprovements56baa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const addt__group6ba4fRef = useRef<any>(null);
  const handleClearSearch = () => {
    addt__group6ba4fRef.current?.setSearchParams();
    addt__group6ba4fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addt__group6ba4f) && Object.keys(addt__group6ba4f)?.length>0)
      {
        setaddt__group6ba4f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addt__group6ba4fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"addt__group"}))
        }}
    >
          {allowedControls.includes("comments") ?<Textcomments   /* c519d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("improvements") ?<TextAreaimprovements   /* 56baa */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
