...
const Buttonbutton_update = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {cancel6ca6c, setcancel6ca6c}= useContext(TotalContext) as TotalContextProps;
  const {button_updatedbacf, setbutton_updatedbacf}= useContext(TotalContext) as TotalContextProps;
  const {save624e5, setsave624e5}= useContext(TotalContext) as TotalContextProps;
  //////////////
  const pendingAutoSearch = useRef(false);
  // keep update group state in ref to access latest state value
  const dynamicactionsb315bRef = useRef(dynamicactionsb315b);
  useEffect(() => {
    dynamicactionsb315bRef.current = dynamicactionsb315b;
  ...
  const dynamicactionsb315bPropsRef = useRef(dynamicactionsb315bProps);
  useEffect(() => {
    dynamicactionsb315bPropsRef.current = dynamicactionsb315bProps;
  }, [dynamicactionsb315bProps]);
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['cancel'] = cancel6ca6c,
        codeStates['setcancel'] = setcancel6ca6c,
        codeStates['button_update'] = button_updatedbacf,
        codeStates['setbutton_update'] = setbutton_updatedbacf,
        codeStates['save'] = save624e5,
        codeStates['setsave'] = setsave624e5,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...dynamicactionsb315bRef.current};
      let parentRowSpan = 7;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "a55bc3c6b654bdf806e19b42105b315b",
        "67fe6c4879422658338be174a77dbacf"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    }else if(dynamicactionsb315bProps?.isHaveRule==true){
      if("button_update" in dynamicactionsb315bProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsb315bProps?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in viewperformancereview_v1?.dynamicactions && viewperformancereview_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(viewperformancereview_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(viewperformancereview_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_updatedbacf") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "67fe6c4879422658338be174a77dbacf") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_updatedbacf");
    return () => {
  ...
  useEffect(()=>{
    if (!button_updatedbacf?.trigger) return;
      if(button_updatedbacf?.trigger){
      setbutton_updatedbacf((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_updatedbacf?.trigger])

  useEffect(()=>{
    if(button_updatedbacf?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_updatedbacf?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsb315b((prev: any) => ({ ...prev, button_update: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsb315b((prev: any) => ({ ...prev, button_update: false }));
  ...
  }, [viewperformancereview_v1?.dynamicactions?.button_update,dynamicactionsb315bProps?.dynamicActionRule?.button_update,])

 if (button_updatedbacf?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_updatedbacf?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
