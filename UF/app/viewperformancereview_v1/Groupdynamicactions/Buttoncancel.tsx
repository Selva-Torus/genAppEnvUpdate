 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "af4bbebcdc013b143b6c6e40a576ca6c"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in dynamicactionsb315bProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsb315bProps?.dynamicActionRule?.cancel,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in viewperformancereview_v1?.dynamicactions && viewperformancereview_v1?.dynamicactions["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(viewperformancereview_v1?.dynamicactions["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(viewperformancereview_v1?.dynamicactions?.cancel?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "af4bbebcdc013b143b6c6e40a576ca6c") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "cancel6ca6c");
    return () => {
  ...
  useEffect(()=>{
    if (!cancel6ca6c?.trigger) return;
      if(cancel6ca6c?.trigger){
      setcancel6ca6c((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsb315b((prev: any) => ({ ...prev, cancel: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsb315b((prev: any) => ({ ...prev, cancel: false }));
  ...
  }, [viewperformancereview_v1?.dynamicactions?.cancel,dynamicactionsb315bProps?.dynamicActionRule?.cancel,])

 if (cancel6ca6c?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='outlined'
          disabled= {cancel6ca6c?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
