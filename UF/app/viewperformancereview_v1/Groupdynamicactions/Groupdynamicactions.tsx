...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a55bc3c6b654bdf806e19b42105b315b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactionsb315bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel6ca6c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel6ca6c?.isDisabled==null)
      {
        setcancel6ca6c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_updatedbacf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_updatedbacf?.isDisabled==null)
      {
        setbutton_updatedbacf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("save")){
        setsave624e5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(save624e5?.isDisabled==null)
      {
        setsave624e5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactionsb315bRef = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactionsb315bRef.current?.setSearchParams();
    dynamicactionsb315bRef.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactionsb315bProps((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactionsb315bProps((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactionsb315b) && Object.keys(dynamicactionsb315b)?.length>0)
      {
        setdynamicactionsb315b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactionsb315bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
