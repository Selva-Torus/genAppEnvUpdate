 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "6d9176c3dca21ca50883541bfb3624e5"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("save" in dynamicactionsb315bProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsb315bProps?.dynamicActionRule?.save,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("save" in viewperformancereview_v1?.dynamicactions && viewperformancereview_v1?.dynamicactions["save"]?.itsHaveArtifact== true)
      {
        setShowFlag(viewperformancereview_v1?.dynamicactions["save"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(viewperformancereview_v1?.dynamicactions?.save?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "6d9176c3dca21ca50883541bfb3624e5") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "save624e5");
    return () => {
  ...
  useEffect(()=>{
    if (!save624e5?.trigger) return;
      if(save624e5?.trigger){
      setsave624e5((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsb315b((prev: any) => ({ ...prev, save: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsb315b((prev: any) => ({ ...prev, save: false }));
  ...
  }, [viewperformancereview_v1?.dynamicactions?.save,dynamicactionsb315bProps?.dynamicActionRule?.save,])

 if (save624e5?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='normal'
          disabled= {save624e5?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
