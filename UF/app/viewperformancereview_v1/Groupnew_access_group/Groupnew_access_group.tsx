...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "01dacc2683a5ab6a81623e4326cc5a99");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_groupc5a99Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__group002d0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__group002d0?.isDisabled==null)
      {
        setaccess_req__group002d0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_group")){
        setvalid_group3a8ab((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_group3a8ab?.isDisabled==null)
      {
        setvalid_group3a8ab((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("addt__group")){
        setaddt__group6ba4f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(addt__group6ba4f?.isDisabled==null)
      {
        setaddt__group6ba4f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("addt__dts_group")){
        setaddt__dts_group613d2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(addt__dts_group613d2?.isDisabled==null)
      {
        setaddt__dts_group613d2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("completed_at_datepicker")){
        setcompleted_at_datepicker9e849((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(completed_at_datepicker9e849?.isDisabled==null)
      {
        setcompleted_at_datepicker9e849((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactionsb315b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactionsb315b?.isDisabled==null)
      {
        setdynamicactionsb315b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("review_id")){
        setreview_id0ac0f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(review_id0ac0f?.isDisabled==null)
      {
        setreview_id0ac0f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const new_access_groupc5a99Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_groupc5a99Ref.current?.setSearchParams();
    new_access_groupc5a99Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_groupc5a99) && Object.keys(new_access_groupc5a99)?.length>0)
      {
        setnew_access_groupc5a99({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_groupc5a99Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedControls.includes("completed_at_datepicker") ?<DatePickercompleted_at_datepicker   /* 9e849 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("review_id") ?<Textreview_id   /* 0ac0f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
