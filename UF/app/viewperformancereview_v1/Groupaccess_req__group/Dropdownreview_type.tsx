 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "b95299f9cefa52cd098109e1016d590c"
    );
  ...
  },[review_typed590c?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group002d0((prev: any) => ({ ...prev,review_typed590c: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group002d0;
      delete temp.review_type;
      delete temp.REVIEW_TYPE;
      delete temp.review_typed590c;
      setaccess_req__group002d0(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group002d0.review_type){
      if(Array.isArray(dfd_reviewtypecombo_v1Props)){
        if(dfd_reviewtypecombo_v1Props?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)){
          setdropdownValue([dfd_reviewtypecombo_v1Props?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)?.perf_review_type])
          tempValue=dfd_reviewtypecombo_v1Props?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)?.perf_review_type
        }else{
          setdropdownValue([access_req__group002d0.review_type])
          tempValue=access_req__group002d0.review_type
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{perf_review_type:access_req__group002d0.review_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)?.perf_review_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.perf_review_type === access_req__group002d0.review_type)?.perf_review_type
      }else{
        setdropdownValue([access_req__group002d0.review_type])
        tempValue=access_req__group002d0.review_type
      }   
    ...
    if(review_typed590c?.trigger == true)
    {
      await handlechange(tempValue)
      setreview_typed590c((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group002d0.review_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_reviewtypecombo_v1Props) && dfd_reviewtypecombo_v1Props?.length == 1){
    // setaccess_req__group002d0((pre:any)=>({...pre,review_type:dfd_reviewtypecombo_v1Props[0]?.review_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group002d0((prev: any) => ({ ...prev, review_type: getMapperDetailsBindValues[value],REVIEW_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group002d0((prev: any) => ({ ...prev, review_type: '', review_typed590c: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_type:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group002d0?.review_type == "" || access_req__group002d0?.review_type == undefined){
      access_req__group002d0.review_type = "";
      const validate:any = v.safeParse(schema, access_req__group002d0?.review_type);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_type:"invalid"}}));
        }
    }else if(access_req__group002d0?.review_type !== ""){
    const validate:any = v.safeParse(schema, access_req__group002d0?.review_type);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_type:"invalid"}}));
  ...
  useEffect(() => { access_req__group002d0Ref.current = access_req__group002d0; }, [access_req__group002d0]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "b95299f9cefa52cd098109e1016d590c") {
        handleClick(access_req__group002d0Ref?.current?.review_typed590c?access_req__group002d0Ref?.current?.review_typed590c:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "b95299f9cefa52cd098109e1016d590c");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group002d0((pre:any)=>({...pre,review_type:""}))
    else
      setInitialCount(1)
  },[review_typed590c?.refresh])
  

  if (review_typed590c?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {review_typed590c?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group002d0?.review_typed590c ? [access_req__group002d0?.review_typed590c] : (access_req__group002d0?.review_type ? dropdownValue : [])}
        validationState={validate?.viewPerformanceReview_v1?.review_type ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
