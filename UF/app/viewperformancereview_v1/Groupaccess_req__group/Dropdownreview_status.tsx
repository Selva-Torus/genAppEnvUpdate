...
const Dropdownreview_status = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [review_statusOptions, setreview_statusOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `perf_review_status`
    let mapperText: string =  `perf_review_status`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "a300670acfb62cae2ffaa51b8302c6ef"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_reviewstatuscombo_v1Props.dstKey){
      dstKey = dfd_reviewstatuscombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_reviewstatuscombo_v1Props && !dfd_reviewstatuscombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[review_status2c6ef?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group002d0((prev: any) => ({ ...prev,review_status2c6ef: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group002d0;
      delete temp.review_status;
      delete temp.REVIEW_STATUS;
      delete temp.review_status2c6ef;
      setaccess_req__group002d0(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_status:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group002d0.review_status){
      if(Array.isArray(dfd_reviewstatuscombo_v1Props)){
        if(dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)){
          setdropdownValue([dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)?.perf_review_status])
          tempValue=dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)?.perf_review_status
        }else{
          setdropdownValue([access_req__group002d0.review_status])
          tempValue=access_req__group002d0.review_status
        }
      }else{
        let dstKey:string = dfd_reviewstatuscombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{perf_review_status:access_req__group002d0.review_status}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)?.perf_review_status ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__group002d0.review_status)?.perf_review_status
      }else{
        setdropdownValue([access_req__group002d0.review_status])
        tempValue=access_req__group002d0.review_status
      }   
    ...
    if(review_status2c6ef?.trigger == true)
    {
      await handlechange(tempValue)
      setreview_status2c6ef((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group002d0.review_status, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_reviewstatuscombo_v1Props) && dfd_reviewstatuscombo_v1Props?.length == 1){
    // setaccess_req__group002d0((pre:any)=>({...pre,review_status:dfd_reviewstatuscombo_v1Props[0]?.review_status}))
    }
  },[dfd_reviewstatuscombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group002d0((prev: any) => ({ ...prev, review_status: getMapperDetailsBindValues[value],REVIEW_STATUS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group002d0((prev: any) => ({ ...prev, review_status: '', review_status2c6ef: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_status:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.perf_review_status==getMapperDetailsBindValues[value] && items?.perf_review_status==value)) || {}
    selected.current={
      ...selectedObj||{},
      perf_review_status:value,
      perf_review_status:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group002d0?.review_status == "" || access_req__group002d0?.review_status == undefined){
      access_req__group002d0.review_status = "";
      const validate:any = v.safeParse(schema, access_req__group002d0?.review_status);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_status:"invalid"}}));
        }
    }else if(access_req__group002d0?.review_status !== ""){
    const validate:any = v.safeParse(schema, access_req__group002d0?.review_status);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,review_status:"invalid"}}));
  ...
  useEffect(() => { access_req__group002d0Ref.current = access_req__group002d0; }, [access_req__group002d0]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "a300670acfb62cae2ffaa51b8302c6ef") {
        handleClick(access_req__group002d0Ref?.current?.review_status2c6ef?access_req__group002d0Ref?.current?.review_status2c6ef:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "a300670acfb62cae2ffaa51b8302c6ef");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group002d0((pre:any)=>({...pre,review_status:""}))
    else
      setInitialCount(1)
  },[review_status2c6ef?.refresh])
  

  if (review_status2c6ef?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {review_status2c6ef?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={review_statusOptions}
        onLoadMore={loadMore}
        ...
        value={access_req__group002d0?.review_status2c6ef ? [access_req__group002d0?.review_status2c6ef] : (access_req__group002d0?.review_status ? dropdownValue : [])}
        validationState={validate?.viewPerformanceReview_v1?.review_status ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownreview_status;
