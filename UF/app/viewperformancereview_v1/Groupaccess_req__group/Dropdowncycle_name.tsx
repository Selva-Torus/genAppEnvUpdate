...
const Dropdowncycle_name = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [cycle_nameOptions, setcycle_nameOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `cycle_name`
    let mapperText: string =  `cycle_name`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "6ec5785023664b03b8cfc12171147492"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_cyclenamecombo_v1Props.dstKey){
      dstKey = dfd_cyclenamecombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_cyclenamecombo_v1Props && !dfd_cyclenamecombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[cycle_name47492?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group002d0((prev: any) => ({ ...prev,cycle_name47492: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group002d0;
      delete temp.cycle_name;
      delete temp.CYCLE_NAME;
      delete temp.cycle_name47492;
      setaccess_req__group002d0(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,cycle_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group002d0.cycle_name){
      if(Array.isArray(dfd_cyclenamecombo_v1Props)){
        if(dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)){
          setdropdownValue([dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)?.cycle_name])
          tempValue=dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)?.cycle_name
        }else{
          setdropdownValue([access_req__group002d0.cycle_name])
          tempValue=access_req__group002d0.cycle_name
        }
      }else{
        let dstKey:string = dfd_cyclenamecombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{cycle_name:access_req__group002d0.cycle_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)?.cycle_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__group002d0.cycle_name)?.cycle_name
      }else{
        setdropdownValue([access_req__group002d0.cycle_name])
        tempValue=access_req__group002d0.cycle_name
      }   
    ...
    if(cycle_name47492?.trigger == true)
    {
      await handlechange(tempValue)
      setcycle_name47492((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group002d0.cycle_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_cyclenamecombo_v1Props) && dfd_cyclenamecombo_v1Props?.length == 1){
    // setaccess_req__group002d0((pre:any)=>({...pre,cycle_name:dfd_cyclenamecombo_v1Props[0]?.cycle_name}))
    }
  },[dfd_cyclenamecombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group002d0((prev: any) => ({ ...prev, cycle_name: getMapperDetailsBindValues[value],CYCLE_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group002d0((prev: any) => ({ ...prev, cycle_name: '', cycle_name47492: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,cycle_name:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.cycle_name==getMapperDetailsBindValues[value] && items?.cycle_name==value)) || {}
    selected.current={
      ...selectedObj||{},
      cycle_name:value,
      cycle_name:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group002d0?.cycle_name == "" || access_req__group002d0?.cycle_name == undefined){
      access_req__group002d0.cycle_name = "";
      const validate:any = v.safeParse(schema, access_req__group002d0?.cycle_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,cycle_name:"invalid"}}));
        }
    }else if(access_req__group002d0?.cycle_name !== ""){
    const validate:any = v.safeParse(schema, access_req__group002d0?.cycle_name);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewPerformanceReview_v1:{...pre?.viewPerformanceReview_v1,cycle_name:"invalid"}}));
  ...
  useEffect(() => { access_req__group002d0Ref.current = access_req__group002d0; }, [access_req__group002d0]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "6ec5785023664b03b8cfc12171147492") {
        handleClick(access_req__group002d0Ref?.current?.cycle_name47492?access_req__group002d0Ref?.current?.cycle_name47492:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "6ec5785023664b03b8cfc12171147492");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group002d0((pre:any)=>({...pre,cycle_name:""}))
    else
      setInitialCount(1)
  },[cycle_name47492?.refresh])
  

  if (cycle_name47492?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {cycle_name47492?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group002d0?.cycle_name47492 ? [access_req__group002d0?.cycle_name47492] : (access_req__group002d0?.cycle_name ? dropdownValue : [])}
        validationState={validate?.viewPerformanceReview_v1?.cycle_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdowncycle_name;
