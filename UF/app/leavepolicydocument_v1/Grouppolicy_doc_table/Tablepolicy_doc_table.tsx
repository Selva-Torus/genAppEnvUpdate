...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "1d4abc9b2e56e8e61edc010a64ac40d7",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "43333c8c4a4e5c8b1cfb2e9fdcd3b631",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "6ab7a6ead73bff894344a08250c787f1",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "975f8891db4b720bcc893a1db40d5da5",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "881e99377d3e2598fe32532fdb6f9f2c",
    "name": "Created By",
    "meta": {
      "sort": true
    },
...
const Tablepolicy_doc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const unlockRecord = async (id: number) => {
    ...
    /// setpolicy_doc_table06d23Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(policy_doc_table06d23.map((item: any) => item.asset_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      policy_doc_table06d23.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = policy_doc_table06d23
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.asset_id) ? index.toString() : null
          )
          .filter(Boolean)

        setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        return
      }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setpolicy_doc_table06d23Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(leavepolicydocument_v1Props.length > 0){
      for(let i=0;i< leavepolicydocument_v1Props.length;i++){
        if(leavepolicydocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leavePolicyDocTable:AFVK:v1"){
          // delete leavepolicydocument_v1Props[i].DFDkey;
          let temp=structuredClone(leavepolicydocument_v1Props[i])
          delete temp?.DFDkey
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["asset_id"]:policy_doc_table06d23Props?.presetValues?.asset_id}
    ...
    if(!policy_doc_table06d23Props?.presetValues || Object.keys(policy_doc_table06d23Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leavePolicyDocument:AFVK:v1:UO', 
                nodeId: 'fa9dc617684d49ae4bf52f23d0806d23',
                elementId: 'fa9dc617684d49ae4bf52f23d0806d23'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leavePolicyDocument:AFVK:v1',
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setpolicy_doc_table06d23(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in leavepolicydocument_v1?.policy_doc_table && leavepolicydocument_v1?.policy_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(leavepolicydocument_v1?.policy_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(leavepolicydocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,policy_doc_table:filteredData},{policy_doc_table:leavepolicydocument_v1?.policy_doc_table})
          if(result?.policy_doc_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(policy_doc_table06d23))
    {
      GetTableDetails()
    }
  }, [policy_doc_table06d23])
  ...
  useEffect(() => {
    ...
    if (policy_doc_table06d23Props.filterInitalLoad) return;

    const filterControllers = policy_doc_table06d23Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['policy_doc_table06d23']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['policy_doc_table06d23']).filter((k: string) => jsonEntry['policy_doc_table06d23'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(policy_doc_table06d23Props.filterProps);
      return;
    }
    ...
    setpolicy_doc_table06d23Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(policy_doc_table06d23Props.filterProps);

  }, [
    policy_doc_table06d23Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    policy_doc_table06d23Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(policy_doc_table06d23Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(policy_doc_table06d23Props.searchFilter) || {};
...
}, [policy_doc_table06d23Props.searchFilter])
  ...
  useEffect(() => {
    if (policy_doc_table06d23Props?.clearData === true) {
      if (lastLockedDataRef.current?.primaryKeys?.length) {
        lastLockedDataRef.current.primaryKeys.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setpolicy_doc_table06d23Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [policy_doc_table06d23Props?.clearData])
  useEffect(() => {
  ...
  }, [policy_doc_table06d23Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = policy_doc_table06d23Props?.selectedIds[policy_doc_table06d23Props?.selectedIds?.length-1]
    if(Array.isArray(policy_doc_table06d23) && policy_doc_table06d23.length>0)
    {
      let data = policy_doc_table06d23[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "asset_id",
        value: data["asset_id"],
  ...
  useEffect(() => {
    if (policy_doc_table06d23Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (policy_doc_table06d23Props?.selectedIds?.length == 0){
      handleOnRowClick({},policy_doc_table06d23Props?.selectedIds)
    }
  }, [policy_doc_table06d23Props?.selectedIds])
    ...
    let filterProps4:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData4 = await getFilterProps(filterProps4,{...doc_attached_group1c693,...table_group973f0,...policy_doc_table06d23});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(policy_doc_table06d23?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&policy_doc_table06d23?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](policy_doc_table06d23?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: policy_doc_table06d23?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&policy_doc_table06d23?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](policy_doc_table06d23?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: policy_doc_table06d23?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (policy_doc_table06d23?.isHidden) {
    return <></>
  }
  return(
      ...
      >
        <PageHrmdocumentviewerpage4/>
          ...
          <div
            ...
            id={"fa9dc617684d49ae4bf52f23d0806d23"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableSorting={true}
              selectedIds={policy_doc_table06d23Props?.selectedIds}  
...
export default Tablepolicy_doc_table
