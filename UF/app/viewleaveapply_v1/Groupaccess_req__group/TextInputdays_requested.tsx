...
const TextInputdays_requested = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewLeaveApply:AFVK:v1|1adc3ad97f24278a74ffd028cc8578e5|669dcce3f127e548d6e5f1243c74683c"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewLeaveApply_v1:{...pre?.viewLeaveApply_v1,days_requested:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group578e5((prev: any) => ({ ...prev, days_requested: +e.target.value }));
    }
    else{
    setaccess_req__group578e5((prev: any) => ({ ...prev, days_requested: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "669dcce3f127e548d6e5f1243c74683c"
      );
      ...
      //     controlId: "669dcce3f127e548d6e5f1243c74683c",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "669dcce3f127e548d6e5f1243c74683c") {
        handleChange({target:{value:access_req__group578e5Ref?.current?.days_requested||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "669dcce3f127e548d6e5f1243c74683c") {
        handleBlur({target:{value:access_req__group578e5Ref?.current?.days_requested||""}});
      }
    };
  ...
  if(dfd_applyleave_v1Props?.setSearchFilters && dfd_applyleave_v1Props?.data)
  {
    if(Array.isArray(dfd_applyleave_v1Props.data) && dfd_applyleave_v1Props.data.length > 0){
      setaccess_req__group578e5((pre:any)=>({...pre,days_requested:dfd_applyleave_v1Props.data[0]?.days_requested}));
    }
  }
  },[dfd_applyleave_v1Props?.setSearchFilters])
  if (days_requested4683c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `11 / 16`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={access_req__group578e5?.days_requested||""}
         disabled= {days_requested4683c?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.viewLeaveApply_v1?.days_requested ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
