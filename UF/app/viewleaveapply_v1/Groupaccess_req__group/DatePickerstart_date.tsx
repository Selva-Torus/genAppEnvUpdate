...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewLeaveApply_v1:{...pre?.viewLeaveApply_v1,start_date:undefined}}));
  if (!date) {
    setaccess_req__group578e5((prev: any) => ({ ...prev, start_date: null }));
    return;
  }
  const now = new Date();
  ...
  setaccess_req__group578e5((prev: any) => ({ ...prev, start_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "864c113daa028555e792d5e5fe48bb1a"
      );
...
return (
  ...
  style={{gridColumn: `1 / 6`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={access_req__group578e5?.start_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {start_date8bb1a?.isDisabled ? true : false}
      disabled= {start_date8bb1a?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Start Date"
      dateValidation=""
      validationState={validate?.viewLeaveApply_v1?.start_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
