...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1d5ffcd34b4d8daedd0cfcd4af28a441");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_group8a441Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__group578e5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__group578e5?.isDisabled==null)
      {
        setaccess_req__group578e5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emp_avail_group")){
        setemp_avail_groupeb48f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emp_avail_groupeb48f?.isDisabled==null)
      {
        setemp_avail_groupeb48f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_balance_group")){
        setleave_balance_group98af0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_balance_group98af0?.isDisabled==null)
      {
        setleave_balance_group98af0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_det_group")){
        setapp_det_group5b97e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_det_group5b97e?.isDisabled==null)
      {
        setapp_det_group5b97e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_group")){
        setaudit_group2b7ff((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_group2b7ff?.isDisabled==null)
      {
        setaudit_group2b7ff((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_req_id")){
        setleave_req_id64a28((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_req_id64a28?.isDisabled==null)
      {
        setleave_req_id64a28((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['leave_req_id'] = leave_req_id64a28,
        codeStates['setleave_req_id'] = setleave_req_id64a28,
  ...
  const new_access_group8a441Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_group8a441Ref.current?.setSearchParams();
    new_access_group8a441Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_group8a441) && Object.keys(new_access_group8a441)?.length>0)
      {
        setnew_access_group8a441({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_group8a441Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleaveapply_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("audit_group")  &&<Groupaudit_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("leave_req_id") ?<Textleave_req_id   /* 64a28 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
