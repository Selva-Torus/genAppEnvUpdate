...
const TextAreaapproval_comments = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "32a83d9f120cb5eb75c395ca1550cf33"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[approval_comments0cf33?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_group5b97e((pre:any)=>({...pre,approval_comments:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "32a83d9f120cb5eb75c395ca1550cf33") {
        handleChange({target:{value:app_det_group5b97eRef?.current?.approval_comments||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "32a83d9f120cb5eb75c395ca1550cf33") {
        handleBlur({target:{value:app_det_group5b97eRef?.current?.approval_comments||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,viewLeaveApply_v1:{...pre?.viewLeaveApply_v1,approval_comments:undefined}}));
    setapp_det_group5b97e((prev: any) => ({ ...prev, approval_comments: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (approval_comments0cf33?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {approval_comments0cf33?.isDisabled ? true : false}
      placeholder = {'Approved for the requested access..'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Approval Comments"
      pin = {'brick-brick'}
      value = { app_det_group5b97e?.approval_comments != null && typeof app_det_group5b97e?.approval_comments =='object' ? Object.keys(app_det_group5b97e?.approval_comments)?.length ?  JSON.stringify(app_det_group5b97e?.approval_comments,null ,2):"" : app_det_group5b97e?.approval_comments||""}
      errorMessage={error}
      validationState={validate?.viewLeaveApply_v1?.approval_comments ? "invalid" : undefined}
    />
  </div>
  )
}
