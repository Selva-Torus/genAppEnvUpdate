...
const Groupapp_det_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "66258595c70576c32feda1795395b97e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapp_det_group5b97eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("approve_group")){
        setapprove_group4d845((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approve_group4d845?.isDisabled==null)
      {
        setapprove_group4d845((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approval_comments")){
        setapproval_comments0cf33((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approval_comments0cf33?.isDisabled==null)
      {
        setapproval_comments0cf33((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_reason")){
        setreject_reason79401((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_reason79401?.isDisabled==null)
      {
        setreject_reason79401((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cancellation_reason")){
        setcancellation_reasone9400((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancellation_reasone9400?.isDisabled==null)
      {
        setcancellation_reasone9400((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const app_det_group5b97eRef = useRef<any>(null);
  const handleClearSearch = () => {
    app_det_group5b97eRef.current?.setSearchParams();
    app_det_group5b97eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(app_det_group5b97e) && Object.keys(app_det_group5b97e)?.length>0)
      {
        setapp_det_group5b97e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [app_det_group5b97eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleaveapply_v1((pre:any)=>({...pre,_selectedGroup_:"app_det_group"}))
        }}
    >
        ...
        {allowedControls.includes("approval_comments") ?<TextAreaapproval_comments   /* 0cf33 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("reject_reason") ?<TextAreareject_reason   /* 79401 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("cancellation_reason") ?<TextAreacancellation_reason   /* e9400 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
