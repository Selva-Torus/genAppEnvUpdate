...
const TextAreareject_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "fb571f96e092f100d0e51946cd979401"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reject_reason79401?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_group5b97e((pre:any)=>({...pre,rejection_reason:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "fb571f96e092f100d0e51946cd979401") {
        handleChange({target:{value:app_det_group5b97eRef?.current?.rejection_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "fb571f96e092f100d0e51946cd979401") {
        handleBlur({target:{value:app_det_group5b97eRef?.current?.rejection_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,viewLeaveApply_v1:{...pre?.viewLeaveApply_v1,rejection_reason:undefined}}));
    setapp_det_group5b97e((prev: any) => ({ ...prev, rejection_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (reject_reason79401?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `9 / 17`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {reject_reason79401?.isDisabled ? true : false}
      placeholder = {'Enter rejection reason if any...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Rejection Reason"
      pin = {'brick-brick'}
      value = { app_det_group5b97e?.rejection_reason != null && typeof app_det_group5b97e?.rejection_reason =='object' ? Object.keys(app_det_group5b97e?.rejection_reason)?.length ?  JSON.stringify(app_det_group5b97e?.rejection_reason,null ,2):"" : app_det_group5b97e?.rejection_reason||""}
      errorMessage={error}
      validationState={validate?.viewLeaveApply_v1?.rejection_reason ? "invalid" : undefined}
    />
  </div>
  )
}
