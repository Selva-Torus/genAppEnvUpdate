...
import Buttonbuttondfgdf  from "./Buttonbuttondfgdf";
...
const Groupgggg = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {buttondfgdf29503, setbuttondfgdf29503}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "79b678e9ac7e4a34be664213a18721e2");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgggg721e2Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("buttondfgdf")){
        setbuttondfgdf29503({...buttondfgdf29503,isDisabled:true});

    }else
    {
      if(buttondfgdf29503?.isDisabled==null)
      {
        setbuttondfgdf29503({...buttondfgdf29503,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['buttondfgdf'] = buttondfgdf29503,
        codeStates['setbuttondfgdf'] = setbuttondfgdf29503,
  ...
  const gggg721e2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    gggg721e2Ref.current?.setSearchParams();
    gggg721e2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(gggg721e2) && Object.keys(gggg721e2)?.length>0)
      {
        setgggg721e2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [gggg721e2Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdynamicaction_v1((pre:any)=>({...pre,_selectedGroup_:"gggg"}))
        }}
    >
        {        ((ruleData?.length>0 && "buttondfgdf" in ButtonGoRuleData)?ButtonGoRuleData["buttondfgdf"]:true) && 
          allowedControls.includes("buttondfgdf")  ?            <Buttonbuttondfgdf tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}

export default Groupgggg
