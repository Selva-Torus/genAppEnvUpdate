...
const Buttonbuttondsdfsd = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {buttondsdfsd487f1, setbuttondsdfsd487f1}= useContext(TotalContext) as TotalContextProps;
  //////////////
  const pendingAutoSearch = useRef(false);
  // keep update group state in ref to access latest state value
  const xbxvvcv42015Ref = useRef(xbxvvcv42015);
  useEffect(() => {
    xbxvvcv42015Ref.current = xbxvvcv42015;
  ...
  const xbxvvcv42015PropsRef = useRef(xbxvvcv42015Props);
  useEffect(() => {
    xbxvvcv42015PropsRef.current = xbxvvcv42015Props;
  }, [xbxvvcv42015Props]);
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['buttondsdfsd'] = buttondsdfsd487f1,
        codeStates['setbuttondsdfsd'] = setbuttondsdfsd487f1,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...xbxvvcv42015Ref.current};
      let parentRowSpan = 36;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "4671cad28b25439e99e6be638b642015",
        "507daefe58b04e528ca7cbb06bc487f1"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "buttondsdfsd487f1") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "507daefe58b04e528ca7cbb06bc487f1") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttondsdfsd487f1");
    return () => {
  ...
  useEffect(()=>{
    if (!buttondsdfsd487f1?.trigger) return;
      if(buttondsdfsd487f1?.trigger){
      setbuttondsdfsd487f1((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttondsdfsd487f1?.trigger])

  useEffect(()=>{
    if(buttondsdfsd487f1?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setxbxvvcv42015((prev: any) => ({ ...prev, buttondsdfsd: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setxbxvvcv42015((prev: any) => ({ ...prev, buttondsdfsd: false }));
 ...
 if (buttondsdfsd487f1?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `8 / 10`,gridRow: `17 / 27`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="   "
          onClick={handleClick}
          view='action'
          disabled= {buttondsdfsd487f1?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("")}
...
export default Buttonbuttondsdfsd
