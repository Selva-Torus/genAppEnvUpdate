...
const Buttonxcvxvxc = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "0728f4a5bb03465ea385a3e72bae4e1f"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "0728f4a5bb03465ea385a3e72bae4e1f") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "xcvxvxce4e1f");
    return () => {
  ...
  useEffect(()=>{
    if (!xcvxvxce4e1f?.trigger) return;
      if(xcvxvxce4e1f?.trigger){
      setxcvxvxce4e1f((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[xcvxvxce4e1f?.trigger])

  useEffect(()=>{
    if(xcvxvxce4e1f?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        settab_group03bf3((prev: any) => ({ ...prev, xcvxvxc: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        settab_group03bf3((prev: any) => ({ ...prev, xcvxvxc: false }));
 ...
 if (xcvxvxce4e1f?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `21 / 25`,gridRow: `3 / 14`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="   "
          onClick={handleClick}
          view='action'
          disabled= {xcvxvxce4e1f?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("hhh")}
