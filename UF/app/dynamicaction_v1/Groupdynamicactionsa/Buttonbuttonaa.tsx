...
const Buttonbuttonaa = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {buttoncc181d5, setbuttoncc181d5}= useContext(TotalContext) as TotalContextProps;
  const {buttonbb596df, setbuttonbb596df}= useContext(TotalContext) as TotalContextProps;
  const {buttonaaf8012, setbuttonaaf8012}= useContext(TotalContext) as TotalContextProps;
  ...
  const dynamicactionsa32986Ref = useRef(dynamicactionsa32986);
  useEffect(() => {
    dynamicactionsa32986Ref.current = dynamicactionsa32986;
  ...
  const dynamicactionsa32986PropsRef = useRef(dynamicactionsa32986Props);
  useEffect(() => {
    dynamicactionsa32986PropsRef.current = dynamicactionsa32986Props;
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['buttoncc'] = buttoncc181d5,
        codeStates['setbuttoncc'] = setbuttoncc181d5,
        codeStates['buttonbb'] = buttonbb596df,
        codeStates['setbuttonbb'] = setbuttonbb596df,
        codeStates['buttonaa'] = buttonaaf8012,
        codeStates['setbuttonaa'] = setbuttonaaf8012,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...dynamicactionsa32986Ref.current};
      let parentRowSpan = 21;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "c2e6e0e5e6b444448a1880187c432986",
        "e5fde8517db4466f93ffac08abcf8012"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    }else if(dynamicactionsa32986Props?.isHaveRule==true){
      if("buttonaa" in dynamicactionsa32986Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsa32986Props?.dynamicActionRule?.buttonaa,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttonaa" in dynamicaction_v1?.dynamicactionsa && dynamicaction_v1?.dynamicactionsa["buttonaa"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactionsa["buttonaa"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactionsa?.buttonaa?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "e5fde8517db4466f93ffac08abcf8012") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttonaaf8012");
    return () => {
  ...
  useEffect(()=>{
    if (!buttonaaf8012?.trigger) return;
      if(buttonaaf8012?.trigger){
      setbuttonaaf8012((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttonaaf8012?.trigger])

  useEffect(()=>{
    if(buttonaaf8012?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[buttonaaf8012?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttonaa: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttonaa: false }));
  ...
  }, [dynamicaction_v1?.dynamicactionsa?.buttonaa,dynamicactionsa32986Props?.dynamicActionRule?.buttonaa,])

 if (buttonaaf8012?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttonaaf8012?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttonaa")}
