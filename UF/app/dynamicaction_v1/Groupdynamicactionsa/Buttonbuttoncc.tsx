...
const Buttonbuttoncc = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "fa2228ff86a84ded8147a0820af181d5"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("buttoncc" in dynamicactionsa32986Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsa32986Props?.dynamicActionRule?.buttoncc,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttoncc" in dynamicaction_v1?.dynamicactionsa && dynamicaction_v1?.dynamicactionsa["buttoncc"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactionsa["buttoncc"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactionsa?.buttoncc?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "fa2228ff86a84ded8147a0820af181d5") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttoncc181d5");
    return () => {
  ...
  useEffect(()=>{
    if (!buttoncc181d5?.trigger) return;
      if(buttoncc181d5?.trigger){
      setbuttoncc181d5((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttoncc181d5?.trigger])

  useEffect(()=>{
    if(buttoncc181d5?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[buttoncc181d5?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttoncc: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttoncc: false }));
  ...
  }, [dynamicaction_v1?.dynamicactionsa?.buttoncc,dynamicactionsa32986Props?.dynamicActionRule?.buttoncc,])

 if (buttoncc181d5?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttoncc181d5?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttoncc")}
