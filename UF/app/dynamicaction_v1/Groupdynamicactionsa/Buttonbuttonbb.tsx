...
const Buttonbuttonbb = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "c25782a9ff2448e2af5c07087c9596df"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("buttonbb" in dynamicactionsa32986Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsa32986Props?.dynamicActionRule?.buttonbb,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttonbb" in dynamicaction_v1?.dynamicactionsa && dynamicaction_v1?.dynamicactionsa["buttonbb"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactionsa["buttonbb"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactionsa?.buttonbb?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "c25782a9ff2448e2af5c07087c9596df") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttonbb596df");
    return () => {
  ...
  useEffect(()=>{
    if (!buttonbb596df?.trigger) return;
      if(buttonbb596df?.trigger){
      setbuttonbb596df((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttonbb596df?.trigger])

  useEffect(()=>{
    if(buttonbb596df?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttonbb: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsa32986((prev: any) => ({ ...prev, buttonbb: false }));
  ...
  }, [dynamicaction_v1?.dynamicactionsa?.buttonbb,dynamicactionsa32986Props?.dynamicActionRule?.buttonbb,])

 if (buttonbb596df?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttonbb596df?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttonbb")}
