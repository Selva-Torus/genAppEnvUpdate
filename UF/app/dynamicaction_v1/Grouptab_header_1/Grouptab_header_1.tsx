...
import Groupgggg  from "../Groupgggg/Groupgggg";
...
const Grouptab_header_1 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bcae40dc8c1f4d1dbbb50b53ed919fae");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settab_header_119faeProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("gggg")){
        setgggg721e2({...gggg721e2,isDisabled:true});

    }else
    {
      if(gggg721e2?.isDisabled==null)
      {
        setgggg721e2({...gggg721e2,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const handleOnChange=()=>{
      setxcvxvxce4e1f((prev: any) => ({ ...prev, isDisabled: true }));
      setrrrrb3f0c((prev: any) => ({ ...prev, isHidden: false }));

  }
  useEffect(()=>{
    if(tab_group03bf3?.tab_group === "tab_header_1"){
    handleOnChange()
    }
  ...
  const tab_header_119faeRef = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header_119faeRef.current?.setSearchParams();
    tab_header_119faeRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_header_119fae) && Object.keys(tab_header_119fae)?.length>0)
      {
        settab_header_119fae({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_header_119faeProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdynamicaction_v1((pre:any)=>({...pre,_selectedGroup_:"tab_header_1"}))
        }}
    >
        {allowedComponent.includes("gggg")  &&<Groupgggg  
