...
const Buttonbuttond = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "08b3a08b936842d799a800e2cea0665e"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("buttond" in dynamicactionsc9120Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsc9120Props?.dynamicActionRule?.buttond,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttond" in dynamicaction_v1?.dynamicactions && dynamicaction_v1?.dynamicactions["buttond"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactions["buttond"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactions?.buttond?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "08b3a08b936842d799a800e2cea0665e") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttond0665e");
    return () => {
  ...
  useEffect(()=>{
    if (!buttond0665e?.trigger) return;
      if(buttond0665e?.trigger){
      setbuttond0665e((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttond0665e?.trigger])

  useEffect(()=>{
    if(buttond0665e?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttond: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttond: false }));
  ...
  }, [dynamicaction_v1?.dynamicactions?.buttond,dynamicactionsc9120Props?.dynamicActionRule?.buttond,])

 if (buttond0665e?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttond0665e?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttond")}
