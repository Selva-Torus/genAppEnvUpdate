...
const Buttonbuttonc = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "66a4902ad51840e5a90a6fe4e1d177ba"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("buttonc" in dynamicactionsc9120Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsc9120Props?.dynamicActionRule?.buttonc,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttonc" in dynamicaction_v1?.dynamicactions && dynamicaction_v1?.dynamicactions["buttonc"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactions["buttonc"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactions?.buttonc?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "66a4902ad51840e5a90a6fe4e1d177ba") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttonc177ba");
    return () => {
  ...
  useEffect(()=>{
    if (!buttonc177ba?.trigger) return;
      if(buttonc177ba?.trigger){
      setbuttonc177ba((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttonc177ba?.trigger])

  useEffect(()=>{
    if(buttonc177ba?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[buttonc177ba?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttonc: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttonc: false }));
  ...
  }, [dynamicaction_v1?.dynamicactions?.buttonc,dynamicactionsc9120Props?.dynamicActionRule?.buttonc,])

 if (buttonc177ba?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttonc177ba?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttonc")}
