...
const Buttonbuttona = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "60f5ae4f9a7c44069566a4946a80ee1f"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("buttona" in dynamicactionsc9120Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsc9120Props?.dynamicActionRule?.buttona,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("buttona" in dynamicaction_v1?.dynamicactions && dynamicaction_v1?.dynamicactions["buttona"]?.itsHaveArtifact== true)
      {
        setShowFlag(dynamicaction_v1?.dynamicactions["buttona"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(dynamicaction_v1?.dynamicactions?.buttona?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "60f5ae4f9a7c44069566a4946a80ee1f") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "buttona0ee1f");
    return () => {
  ...
  useEffect(()=>{
    if (!buttona0ee1f?.trigger) return;
      if(buttona0ee1f?.trigger){
      setbuttona0ee1f((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[buttona0ee1f?.trigger])

  useEffect(()=>{
    if(buttona0ee1f?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[buttona0ee1f?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttona: true }));
        //onClick

      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsc9120((prev: any) => ({ ...prev, buttona: false }));
  ...
  }, [dynamicaction_v1?.dynamicactions?.buttona,dynamicactionsc9120Props?.dynamicActionRule?.buttona,])

 if (buttona0ee1f?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {buttona0ee1f?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("buttona")}
