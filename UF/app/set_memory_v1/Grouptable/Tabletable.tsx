...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "6a7cf578d3e246a2b0c604ef0cf2114e",
    "name": "country",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "state",
    "nodeid": "edcd5d56a6f54125a5cf16b16e4bbeaf",
    "name": "state",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "city",
    "nodeid": "0cb7707117da4c93ba745191b376731c",
    "name": "city",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {setmemoryvaluesevent_v1Props, setsetmemoryvaluesevent_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, tabled5efd })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, tabled5efd }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, tabled5efd } = latestLockStateRef.current
    /// settabled5efdProps
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settabled5efdProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      tabled5efd.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settabled5efdProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settabled5efdProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settabled5efdProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(setmemoryvaluesevent_v1Props.length > 0){
      for(let i=0;i< setmemoryvaluesevent_v1Props.length;i++){
        if(setmemoryvaluesevent_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:set_db_node:AFVK:v1"){
          // delete setmemoryvaluesevent_v1Props[i].DFDkey;
          let temp=structuredClone(setmemoryvaluesevent_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:setMemoryValuesevent:AFVK:v1:UO', 
                nodeId: 'dc074ef9142b4d079fb88a89e3cd5efd',
                elementId: 'dc074ef9142b4d079fb88a89e3cd5efd'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( tabled5efdProps?.presetValues&&Object.keys(tabled5efdProps?.presetValues).length > 0) {
          filtertedData = [tabled5efdProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settabled5efd(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (tabled5efdProps.filterInitalLoad) return;

    const filterControllers = tabled5efdProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['tabled5efd']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['tabled5efd']).filter((k: string) => jsonEntry['tabled5efd'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(tabled5efdProps.filterProps);
      return;
    }
    ...
    settabled5efdProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(tabled5efdProps.filterProps);

  }, [
    tabled5efdProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    tabled5efdProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(tabled5efdProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(tabled5efdProps.searchFilter) || {};
  ...
  useEffect(() => {
    if (tabled5efdProps?.clearData === true) {
      settabled5efd([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settabled5efdProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [tabled5efdProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = tabled5efdProps?.selectedIds[tabled5efdProps?.selectedIds?.length-1]
    if(Array.isArray(tabled5efd) && tabled5efd.length>0)
    {
      let data = tabled5efd.find((data:any)=>(data?.my_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "my_id",
        value: data["my_id"],
  ...
  useEffect(() => {
    if (tabled5efdProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (tabled5efdProps?.selectedIds?.length == 0){
      handleOnRowClick({},tabled5efdProps?.selectedIds)
    }
  }, [tabled5efdProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(tabled5efd?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&tabled5efd?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](tabled5efd?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tabled5efd?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&tabled5efd?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](tabled5efd?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tabled5efd?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"dc074ef9142b4d079fb88a89e3cd5efd"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={tabled5efdProps?.selectedIds}  
