...
export default function PageSetMemoryV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const screenName:string = "set_memory";
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      set_db_node_v1:false,
    });
    async function set_db_node_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let set_db_node_v1Body:te_refreshDto={
          key: "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:set_db_node:AFVK:v1"+":",
          refreshFlag: "Y",
          count:parseInt(pagination?.count) || 10,
          page:parseInt(pagination?.page) || 1
        }
        if (encryptionFlagPage) {          
          set_db_node_v1Body["dpdKey"] = encryptionDpd;
          set_db_node_v1Body["method"] = encryptionMethod;
        }
        if(setmemoryvaluesevent_v1Props.length > 0){
          ...
          set_db_node_v1Body['filterData'] = filterData;
        }
        const set_db_node_v1Data:any=await AxiosService.post("/te/eventEmitter",set_db_node_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=set_db_node_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(set_db_node_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_set_db_node_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_set_db_node_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (set_db_node_v1Data?.data?.dataset) {
           setdfd_set_db_node_v1Props(
              Array.isArray(set_db_node_v1Data?.data?.dataset?.data)
                 ? set_db_node_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.set_db_node_v1) {
      set_db_node_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.set_db_node_v1= true
  },[refetch?.set_db_node_v1])
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    const { getAftfactLevelRule } = await import("../utils/evaluateDecisionTable");
    let result :any =await getAftfactLevelRule(rule,data,allRuleData)
    setsetmemoryvaluesevent_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:setMemoryValuesevent:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
      ...
      try {
        ...
        try{
    await set_db_node_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setsetmemoryvaluesevent_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(setmemoryvaluesevent_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(setmemoryvaluesevent_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setsetmemoryvaluesevent_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
