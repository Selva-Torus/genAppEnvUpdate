...
export default function PageAddcategoryviewV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetcategory_v1DFD(pagination:any): Promise<void>{
        ...
        if(addcategoryview_v1Props.length > 0){
          for(let i=0;i< addcategoryview_v1Props.length;i++){
            if(addcategoryview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetCategory:AFVK:v1"){
              // delete addcategoryview_v1Props[i].DFDkey;
              let temp=structuredClone(addcategoryview_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setaddcategoryview_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaddcategoryview_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(addcategoryview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(addcategoryview_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
