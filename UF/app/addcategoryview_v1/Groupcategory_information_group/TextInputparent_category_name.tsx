...
const TextInputparent_category_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetCategory:AFVK:v1|22ba40e3f56441559478608632cef203|properties.parent_category_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addCategoryView:AFVK:v1|7f3089aeea63747b5a2d6a8984ca040a|7488481c06504442b1a55f1fe239a67f"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'parent_category_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCategoryView_v1:{...pre?.addCategoryView_v1,parent_category_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcategory_information_groupa040a((prev: any) => ({ ...prev, parent_category_name: +e.target.value }));
    }
    else{
    setcategory_information_groupa040a((prev: any) => ({ ...prev, parent_category_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "7488481c06504442b1a55f1fe239a67f"
      );
      ...
      //     controlId: "7488481c06504442b1a55f1fe239a67f",
      //     isTable: false,
      //     from:"TextInputparent_category_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'parent_category_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'parent_category_name',type:'text'};
      //   type={
      //     name:'parent_category_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_category_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_category_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_category_name.type
      //   }
      ...
      //   type={
      //     name:'parent_category_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_category_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_category_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_category_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "7488481c06504442b1a55f1fe239a67f") {
        handleChange({target:{value:category_information_groupa040aRef?.current?.parent_category_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "7488481c06504442b1a55f1fe239a67f") {
        handleBlur({target:{value:category_information_groupa040aRef?.current?.parent_category_name||""}});
      }
    };
  ...
  if(dfd_assetcategory_v1Props?.setSearchFilters && dfd_assetcategory_v1Props?.data)
  {
    if(Array.isArray(dfd_assetcategory_v1Props.data) && dfd_assetcategory_v1Props.data.length > 0){
      setcategory_information_groupa040a((pre:any)=>({...pre,parent_category_name:dfd_assetcategory_v1Props.data[0]?.parent_category_name}));
    }
  }
  },[dfd_assetcategory_v1Props?.setSearchFilters])
  if (parent_category_name9a67f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 13`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={category_information_groupa040a?.parent_category_name||""}
         disabled= {parent_category_name9a67f?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addCategoryView_v1?.parent_category_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputparent_category_name
