...
const TextInputasset_prefix = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addCategoryView:AFVK:v1|7f3089aeea63747b5a2d6a8984ca040a|88ef54421ad6ee73e3d0a2483dd5007a"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCategoryView_v1:{...pre?.addCategoryView_v1,asset_prefix:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcategory_information_groupa040a((prev: any) => ({ ...prev, asset_prefix: +e.target.value }));
    }
    else{
    setcategory_information_groupa040a((prev: any) => ({ ...prev, asset_prefix: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "88ef54421ad6ee73e3d0a2483dd5007a"
      );
      ...
      //     controlId: "88ef54421ad6ee73e3d0a2483dd5007a",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "88ef54421ad6ee73e3d0a2483dd5007a") {
        handleChange({target:{value:category_information_groupa040aRef?.current?.asset_prefix||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "88ef54421ad6ee73e3d0a2483dd5007a") {
        handleBlur({target:{value:category_information_groupa040aRef?.current?.asset_prefix||""}});
      }
    };
  ...
  if(dfd_assetcategory_v1Props?.setSearchFilters && dfd_assetcategory_v1Props?.data)
  {
    if(Array.isArray(dfd_assetcategory_v1Props.data) && dfd_assetcategory_v1Props.data.length > 0){
      setcategory_information_groupa040a((pre:any)=>({...pre,asset_prefix:dfd_assetcategory_v1Props.data[0]?.asset_prefix}));
    }
  }
  },[dfd_assetcategory_v1Props?.setSearchFilters])
  if (asset_prefix5007a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={category_information_groupa040a?.asset_prefix||""}
         disabled= {asset_prefix5007a?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addCategoryView_v1?.asset_prefix ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
