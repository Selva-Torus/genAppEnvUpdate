...
import Groupdoc_type_table  from "../Groupdoc_type_table/Groupdoc_type_table";
...
const Grouprequired_dociument_header_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupRequiredDociumentHeaderGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0de924089fe5453e99378c2c571e39c8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_header_groupe39c8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("doc_type_table")){
        setdoc_type_tablebe9fa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_type_tablebe9fa?.isDisabled==null)
      {
        setdoc_type_tablebe9fa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const required_dociument_header_groupe39c8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_header_groupe39c8Ref.current?.setSearchParams();
    required_dociument_header_groupe39c8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_header_groupe39c8) && Object.keys(required_dociument_header_groupe39c8)?.length>0)
      {
        setrequired_dociument_header_groupe39c8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_header_groupe39c8Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_header_group"}))
        }}
    >
        {allowedComponent.includes("doc_type_table")  &&<Groupdoc_type_table  
...
export default Grouprequired_dociument_header_group
