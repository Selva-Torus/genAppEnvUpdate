...
const Groupgeorgia_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warning_iconc4eea, setwarning_iconc4eea}= useContext(TotalContext) as TotalContextProps;
  const {georgia_sol_text6238e, setgeorgia_sol_text6238e}= useContext(TotalContext) as TotalContextProps;
  const {special_rules031f4, setspecial_rules031f4}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ab5a2814dea24a6d9e6c988df9e0fa18");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgia_group0fa18Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warning_icon")){
        setwarning_iconc4eea((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warning_iconc4eea?.isDisabled==null)
      {
        setwarning_iconc4eea((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgia_sol_text")){
        setgeorgia_sol_text6238e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgia_sol_text6238e?.isDisabled==null)
      {
        setgeorgia_sol_text6238e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules031f4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules031f4?.isDisabled==null)
      {
        setspecial_rules031f4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warning_icon'] = warning_iconc4eea,
        codeStates['setwarning_icon'] = setwarning_iconc4eea,
        codeStates['georgia_sol_text'] = georgia_sol_text6238e,
        codeStates['setgeorgia_sol_text'] = setgeorgia_sol_text6238e,
        codeStates['special_rules'] = special_rules031f4,
        codeStates['setspecial_rules'] = setspecial_rules031f4,
  ...
  const georgia_group0fa18Ref = useRef<any>(null);
  const handleClearSearch = () => {
    georgia_group0fa18Ref.current?.setSearchParams();
    georgia_group0fa18Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgia_group0fa18) && Object.keys(georgia_group0fa18)?.length>0)
      {
        setgeorgia_group0fa18({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgia_group0fa18Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgia_group"}))
        }}
    >
        {allowedControls.includes("warning_icon")?<Iconwarning_icon /* c4eea */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgia_sol_text") ?<Textgeorgia_sol_text   /* 6238e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("special_rules") ?<Textspecial_rules   /* 031f4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
