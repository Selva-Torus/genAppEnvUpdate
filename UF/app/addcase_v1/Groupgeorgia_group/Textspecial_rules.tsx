...
const Textspecial_rules = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {special_rules031f4Props, setspecial_rules031f4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[special_rules031f4?.refresh])

  if (special_rules031f4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Georgia statute of limitations for open accounts is 6 years from the date of last payment.")}
</Text>
  </div>
  )
}
