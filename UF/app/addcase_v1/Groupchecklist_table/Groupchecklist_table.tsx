...
const Groupchecklist_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "71f0e8dd493f4db6b887d87b4d0198e1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_table198e1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("checklist_item_id")){
        setchecklist_item_id255b0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_item_id255b0?.isDisabled==null)
      {
        setchecklist_item_id255b0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("item_name")){
        setitem_nameeed39((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(item_nameeed39?.isDisabled==null)
      {
        setitem_nameeed39((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("is_complete")){
        setis_completeb8c69((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(is_completeb8c69?.isDisabled==null)
      {
        setis_completeb8c69((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const checklist_table198e1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_table198e1Ref.current?.setSearchParams();
    checklist_table198e1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_table198e1) && Object.keys(checklist_table198e1)?.length>0)
      {
        setchecklist_table198e1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_table198e1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablechecklist_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={checklist_table198e1Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
