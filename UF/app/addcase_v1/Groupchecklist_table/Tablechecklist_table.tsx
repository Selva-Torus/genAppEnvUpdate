...
let defaultColumns:any = [
  {
    ...
    "nodeid": "afd688793a204abcb645a04da05255b0",
  ...
  {
    "id": "item_name",
    "nodeid": "4584df795daf4a71a6b570825cceed39",
    "name": "Item Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "is_complete",
    "nodeid": "022adba7e41c42a09ab706a95c0b8c69",
    "name": "Is Complete",
    "meta": {
      "sort": true
    },
...
const Tablechecklist_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {checklist_item_id255b0, setchecklist_item_id255b0}= useContext(TotalContext) as TotalContextProps  
  const {item_nameeed39, setitem_nameeed39}= useContext(TotalContext) as TotalContextProps  
  const {is_completeb8c69, setis_completeb8c69}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "71f0e8dd493f4db6b887d87b4d0198e1",
      );
  ...
  const lockRecord = async (id: number) => {
    try {
      const res = await AxiosService.post(
        '/UF/lock',
        { tableName: 'checklist_items', key:"checklist_item_id",value:id},
        {
          headers: {
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, checklist_table198e1 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, checklist_table198e1 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, checklist_table198e1 } = latestLockStateRef.current
    /// setchecklist_table198e1Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(checklist_table198e1.map((item: any) => item.checklist_item_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      try {
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.checklist_item_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
        ...
        if (previousLockedId === row.checklist_item_id) {
          return
        }

        await lockRecord(row.checklist_item_id)

        if (previousLockedId && previousLockedId !== row.checklist_item_id) {
          await unlockRecord(previousLockedId)
        }

      checklist_table198e1.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.checklist_item_id){
          selectedData?.push(item)
          postIds.push(item.checklist_item_id)
      ...
      //////////
      //////////
        setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      checklist_table198e1.filter((item:any,id:number)=>{
        if (ids.includes(item.checklist_item_id)){
          selectedData?.push(item)
          postIds.push(item.checklist_item_id) 
          processIds.push(item?.trs_process_id)
        } 
      ...
      setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = checklist_table198e1
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.checklist_item_id) ? item.checklist_item_id : null
          )
          .filter(Boolean)

        setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        throw err
      }
    }
    let index = Number(ids[ids.length - 1])
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['checklist_item_id'] = checklist_item_id255b0,
        codeStates['setchecklist_item_id'] = setchecklist_item_id255b0,
        codeStates['item_name'] = item_nameeed39,
        codeStates['setitem_name'] = setitem_nameeed39,
        codeStates['is_complete'] = is_completeb8c69,
        codeStates['setis_complete'] = setis_completeb8c69,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setchecklist_table198e1Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1:UO', 
                nodeId: '71f0e8dd493f4db6b887d87b4d0198e1',
                elementId: '71f0e8dd493f4db6b887d87b4d0198e1'
              },
            }
          }else
          {
      ...
      if (api_pagination?.data?.records.length > 0) {
        ...
        result.checklist_item_id = emp?.checklist_item_id;
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( checklist_table198e1Props?.presetValues&&Object.keys(checklist_table198e1Props?.presetValues).length > 0) {
          filtertedData = [checklist_table198e1Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setchecklist_table198e1(api_pagination?.data?.records||[])
        }
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,checklist_item_id:filtertedData[i]?.checklist_item_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.checklist_item_id==item?.checklist_item_id)
        {
          index=i
          row=data
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (checklist_table198e1Props.filterInitalLoad) return;

    const filterControllers = checklist_table198e1Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['checklist_table198e1']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['checklist_table198e1']).filter((k: string) => jsonEntry['checklist_table198e1'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(checklist_table198e1Props.filterProps);
      return;
    }
    ...
    setchecklist_table198e1Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(checklist_table198e1Props.filterProps);

  }, [
    checklist_table198e1Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    checklist_table198e1Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(checklist_table198e1Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(checklist_table198e1Props.searchFilter) || {};
...
}, [checklist_table198e1Props.searchFilter])
  ...
  useEffect(() => {
    if (checklist_table198e1Props?.clearData === true) {
      if (myLockedIdsRef.current.length) {
        myLockedIdsRef.current.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setchecklist_table198e1Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [checklist_table198e1Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [checklist_table198e1Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = checklist_table198e1Props?.selectedIds[checklist_table198e1Props?.selectedIds?.length-1]
    if(Array.isArray(checklist_table198e1) && checklist_table198e1.length>0)
    {
      let data = checklist_table198e1.find((data:any)=>(data?.checklist_item_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "checklist_item_id",
        value: data["checklist_item_id"],
        parentData: data
      })
    }
  }
  useEffect(() => {
    if (checklist_table198e1Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (checklist_table198e1Props?.selectedIds?.length == 0){
      handleOnRowClick({},checklist_table198e1Props?.selectedIds)
    }
  }, [checklist_table198e1Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.checklist_item_id==data?.checklist_item_id)
        {
          index=i
          row=item
        }
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(checklist_table198e1?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&checklist_table198e1?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](checklist_table198e1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_table198e1?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&checklist_table198e1?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](checklist_table198e1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_table198e1?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (checklist_table198e1?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"71f0e8dd493f4db6b887d87b4d0198e1"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableSelection={true}
              disable={disable}
              selectedIds={checklist_table198e1Props?.selectedIds}  
