...
const Groupgeorgias_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a80b3abb0dbf4486aa4eb364aa6945fd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgias_group945fdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warnings_icon")){
        setwarnings_icon63b3d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warnings_icon63b3d?.isDisabled==null)
      {
        setwarnings_icon63b3d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_sol_text")){
        setgeorgias_sol_textea2e1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_sol_textea2e1?.isDisabled==null)
      {
        setgeorgias_sol_textea2e1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("specials_rules")){
        setspecials_rulesb4b8b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(specials_rulesb4b8b?.isDisabled==null)
      {
        setspecials_rulesb4b8b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warnings_icon'] = warnings_icon63b3d,
        codeStates['setwarnings_icon'] = setwarnings_icon63b3d,
        codeStates['georgias_sol_text'] = georgias_sol_textea2e1,
        codeStates['setgeorgias_sol_text'] = setgeorgias_sol_textea2e1,
        codeStates['specials_rules'] = specials_rulesb4b8b,
        codeStates['setspecials_rules'] = setspecials_rulesb4b8b,
  ...
  const georgias_group945fdRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgias_group945fdRef.current?.setSearchParams();
    georgias_group945fdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgias_group945fd) && Object.keys(georgias_group945fd)?.length>0)
      {
        setgeorgias_group945fd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgias_group945fdProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgias_group"}))
        }}
    >
        {allowedControls.includes("warnings_icon")?<Iconwarnings_icon /* 63b3d */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgias_sol_text") ?<Textgeorgias_sol_text   /* ea2e1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("specials_rules") ?<Textspecials_rules   /* b4b8b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
