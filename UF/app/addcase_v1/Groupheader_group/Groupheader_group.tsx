...
const Groupheader_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {craete_header_textace67, setcraete_header_textace67}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "99d08a120ea1483ebb442d34e384878f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setheader_group4878fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("craete_header_text")){
        setcraete_header_textace67((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(craete_header_textace67?.isDisabled==null)
      {
        setcraete_header_textace67((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['craete_header_text'] = craete_header_textace67,
        codeStates['setcraete_header_text'] = setcraete_header_textace67,
  ...
  const header_group4878fRef = useRef<any>(null);
  const handleClearSearch = () => {
    header_group4878fRef.current?.setSearchParams();
    header_group4878fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(header_group4878f) && Object.keys(header_group4878f)?.length>0)
      {
        setheader_group4878f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [header_group4878fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"header_group"}))
        }}
    >
          {allowedControls.includes("craete_header_text") ?<Textcraete_header_text   /* ace67 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
