...
const TextInputcreditor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|765fc890b6a2413b897557a765428f6f|d5fb97fc1bdc44c89aa19ea063ef8de4"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,creditor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group28f6f((prev: any) => ({ ...prev, creditor_name: +e.target.value }));
    }
    else{
    setcase_information_group28f6f((prev: any) => ({ ...prev, creditor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d5fb97fc1bdc44c89aa19ea063ef8de4"
      );
      ...
      //     controlId: "d5fb97fc1bdc44c89aa19ea063ef8de4",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "d5fb97fc1bdc44c89aa19ea063ef8de4") {
        handleChange({target:{value:case_information_group28f6fRef?.current?.creditor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d5fb97fc1bdc44c89aa19ea063ef8de4") {
        handleBlur({target:{value:case_information_group28f6fRef?.current?.creditor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group28f6f((pre:any)=>({...pre,creditor_name:dfd_addcase_v1Props.data[0]?.creditor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (creditor_namef8de4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group28f6f?.creditor_name||""}
         disabled= {creditor_namef8de4?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Creditor"
      errorMessage={error}
        validationState={validate?.addCase_v1?.creditor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
