...
const Dropdownpriority_name = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [priority_nameOptions, setpriority_nameOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `priority_name`
    let mapperText: string =  `priority_name`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "81a3b93088324324ada44f83129bcbd5"
    );
  ...
  },[priority_namebcbd5?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setcase_information_group28f6f((prev: any) => ({ ...prev,priority_namebcbd5: value }))
      setIsRequredData(false)
    }else{
      let temp:any = case_information_group28f6f;
      delete temp.priority_name;
      delete temp.PRIORITY_NAME;
      delete temp.priority_namebcbd5;
      setcase_information_group28f6f(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,priority_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(case_information_group28f6f.priority_name){
      if(Array.isArray(dfd_prioritycombo_v1Props)){
        if(dfd_prioritycombo_v1Props?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)){
          setdropdownValue([dfd_prioritycombo_v1Props?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)?.priority_name])
          tempValue=dfd_prioritycombo_v1Props?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)?.priority_name
        }else{
          setdropdownValue([case_information_group28f6f.priority_name])
          tempValue=case_information_group28f6f.priority_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{priority_name:case_information_group28f6f.priority_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)?.priority_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.priority_name === case_information_group28f6f.priority_name)?.priority_name
      }else{
        setdropdownValue([case_information_group28f6f.priority_name])
        tempValue=case_information_group28f6f.priority_name
      }   
    ...
    if(priority_namebcbd5?.trigger == true)
    {
      await handlechange(tempValue)
      setpriority_namebcbd5((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[case_information_group28f6f.priority_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_prioritycombo_v1Props) && dfd_prioritycombo_v1Props?.length == 1){
    // setcase_information_group28f6f((pre:any)=>({...pre,priority_name:dfd_prioritycombo_v1Props[0]?.priority_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setcase_information_group28f6f((prev: any) => ({ ...prev, priority_name: getMapperDetailsBindValues[value],PRIORITY_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setcase_information_group28f6f((prev: any) => ({ ...prev, priority_name: '', priority_namebcbd5: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,priority_name:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.priority_name==getMapperDetailsBindValues[value] && items?.priority_name==value)) || {}
    selected.current={
      ...selectedObj||{},
      priority_name:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  useEffect(() => { case_information_group28f6fRef.current = case_information_group28f6f; }, [case_information_group28f6f]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "81a3b93088324324ada44f83129bcbd5") {
        handleClick(case_information_group28f6fRef?.current?.priority_namebcbd5?case_information_group28f6fRef?.current?.priority_namebcbd5:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "81a3b93088324324ada44f83129bcbd5");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setcase_information_group28f6f((pre:any)=>({...pre,priority_name:""}))
    else
      setInitialCount(1)
  },[priority_namebcbd5?.refresh])
  

  if (priority_namebcbd5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {priority_namebcbd5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={priority_nameOptions}
        onLoadMore={loadMore}
        isLoadingMore={isLoadingMore}
        placeholder={keyset("Select Priority")} 
        filterable={true} 
        hasClear={true} 
        onChange={handlechange} 
        value={case_information_group28f6f?.priority_namebcbd5 ? [case_information_group28f6f?.priority_namebcbd5] : (case_information_group28f6f?.priority_name ? dropdownValue : [])}
        validationState={validate?.addCase_v1?.priority_name ? "invalid" : undefined}
        />
    </div>
  );
};

export default Dropdownpriority_name;
