...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,sla_wait_start_time:undefined}}));
  if (!date) {
    setcase_information_group28f6f((prev: any) => ({ ...prev, sla_wait_start_time: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group28f6f((prev: any) => ({ ...prev, sla_wait_start_time: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "75895bc179494c448933a907df22fb95"
      );
...
useEffect(()=>{
  setcase_information_group28f6fProps((pre:any)=>({...pre,validation:true}))
 },[sla_wait_start_time2fb95?.refresh])
...
if (sla_wait_start_time2fb95?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group28f6f?.sla_wait_start_time}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {sla_wait_start_time2fb95?.isDisabled ? true : false}
      disabled= {sla_wait_start_time2fb95?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="SLA Start Time"
      dateValidation=""
      validationState={validate?.addCase_v1?.sla_wait_start_time ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
