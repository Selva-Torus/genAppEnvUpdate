...
const Textgeorsgiass_sosl_texts = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {georsgiass_sosl_texts14d14Props, setgeorsgiass_sosl_texts14d14Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[georsgiass_sosl_texts14d14?.refresh])

  if (georsgiass_sosl_texts14d14?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `5 / 25`,gridRow: `2 / 10`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!font-bold"
  variant="header-1"
  color="primary"
>
      {keyset("Florida SOL: 5 years")}
</Text>
  </div>
  )
}

export default Textgeorsgiass_sosl_texts
