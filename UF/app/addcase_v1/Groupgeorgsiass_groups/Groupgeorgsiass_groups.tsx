...
const Groupgeorgsiass_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warningssss_iconsbdae5, setwarningssss_iconsbdae5}= useContext(TotalContext) as TotalContextProps;
  const {georsgiass_sosl_texts14d14, setgeorsgiass_sosl_texts14d14}= useContext(TotalContext) as TotalContextProps;
  const {spesciasls_ruless22364, setspesciasls_ruless22364}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f0cd38dbc4c84e3f90246f2cebfb044a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgsiass_groupsb044aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningssss_icons")){
        setwarningssss_iconsbdae5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningssss_iconsbdae5?.isDisabled==null)
      {
        setwarningssss_iconsbdae5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georsgiass_sosl_texts")){
        setgeorsgiass_sosl_texts14d14((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georsgiass_sosl_texts14d14?.isDisabled==null)
      {
        setgeorsgiass_sosl_texts14d14((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("spesciasls_ruless")){
        setspesciasls_ruless22364((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(spesciasls_ruless22364?.isDisabled==null)
      {
        setspesciasls_ruless22364((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningssss_icons'] = warningssss_iconsbdae5,
        codeStates['setwarningssss_icons'] = setwarningssss_iconsbdae5,
        codeStates['georsgiass_sosl_texts'] = georsgiass_sosl_texts14d14,
        codeStates['setgeorsgiass_sosl_texts'] = setgeorsgiass_sosl_texts14d14,
        codeStates['spesciasls_ruless'] = spesciasls_ruless22364,
        codeStates['setspesciasls_ruless'] = setspesciasls_ruless22364,
  ...
  const georgsiass_groupsb044aRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgsiass_groupsb044aRef.current?.setSearchParams();
    georgsiass_groupsb044aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgsiass_groupsb044a) && Object.keys(georgsiass_groupsb044a)?.length>0)
      {
        setgeorgsiass_groupsb044a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgsiass_groupsb044aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgsiass_groups"}))
        }}
    >
        {allowedControls.includes("warningssss_icons")?<Iconwarningssss_icons /* bdae5 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georsgiass_sosl_texts") ?<Textgeorsgiass_sosl_texts   /* 14d14 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("spesciasls_ruless") ?<Textspesciasls_ruless   /* 22364 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
