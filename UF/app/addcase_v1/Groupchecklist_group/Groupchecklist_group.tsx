...
const Groupchecklist_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "656f7ed5f3c14176a061fe9237932b3d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_group32b3dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("checklist_table")){
        setchecklist_table198e1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_table198e1?.isDisabled==null)
      {
        setchecklist_table198e1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const checklist_group32b3dRef = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_group32b3dRef.current?.setSearchParams();
    checklist_group32b3dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_group32b3d) && Object.keys(checklist_group32b3d)?.length>0)
      {
        setchecklist_group32b3d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_group32b3dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_group"}))
        }}
    >
...
export default Groupchecklist_group
