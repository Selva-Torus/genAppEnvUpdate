...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2116bf27bdb84365b00562aa66b094c3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions094c3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel_button")){
        setcancel_button70ef3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel_button70ef3?.isDisabled==null)
      {
        setcancel_button70ef3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("update")){
        setupdate294f0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(update294f0?.isDisabled==null)
      {
        setupdate294f0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("submit")){
        setsubmit0112f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(submit0112f?.isDisabled==null)
      {
        setsubmit0112f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions094c3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions094c3Ref.current?.setSearchParams();
    dynamicactions094c3Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions094c3Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions094c3Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions094c3) && Object.keys(dynamicactions094c3)?.length>0)
      {
        setdynamicactions094c3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions094c3Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
        ...
        {        ((ruleData?.length>0 && "submit" in ButtonGoRuleData)?ButtonGoRuleData["submit"]:true) && 
