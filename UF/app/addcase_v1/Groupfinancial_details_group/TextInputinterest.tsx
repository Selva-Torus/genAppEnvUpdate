...
const TextInputinterest = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.interest"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|564f1b6503aa4531afd30f4a41852f47|2d6414bf5cc640c7b940eb9306f81968"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'interest',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,interest:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_group52f47((prev: any) => ({ ...prev, interest: +e.target.value }));
    }
    else{
    setfinancial_details_group52f47((prev: any) => ({ ...prev, interest: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "2d6414bf5cc640c7b940eb9306f81968"
      );
      ...
      //     controlId: "2d6414bf5cc640c7b940eb9306f81968",
      //     isTable: false,
      //     from:"TextInputinterest",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'interest', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'interest',type:'text'};
      //   type={
      //     name:'interest',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.interest.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.interest.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.interest.type
      //   }
      ...
      //   type={
      //     name:'interest',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.interest.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.interest.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.interest.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "2d6414bf5cc640c7b940eb9306f81968") {
        handleChange({target:{value:financial_details_group52f47Ref?.current?.interest||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2d6414bf5cc640c7b940eb9306f81968") {
        handleBlur({target:{value:financial_details_group52f47Ref?.current?.interest||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_group52f47((pre:any)=>({...pre,interest:dfd_addcase_v1Props.data[0]?.interest}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (interest81968?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 11`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={financial_details_group52f47?.interest||""}
         disabled= {interest81968?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        view='normal'
        contentAlign={"left"}
        ...
        headerText="Interest"
      errorMessage={error}
        validationState={validate?.addCase_v1?.interest ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputinterest
