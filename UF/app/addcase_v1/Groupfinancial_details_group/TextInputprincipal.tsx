...
const TextInputprincipal = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.principal"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|564f1b6503aa4531afd30f4a41852f47|1072412c049f4d55bbbf8b8ffc0c3ba6"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'principal',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,principal:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_group52f47((prev: any) => ({ ...prev, principal: +e.target.value }));
    }
    else{
    setfinancial_details_group52f47((prev: any) => ({ ...prev, principal: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "1072412c049f4d55bbbf8b8ffc0c3ba6"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1",
      //     componentId: "564f1b6503aa4531afd30f4a41852f47",
      //     controlId: "1072412c049f4d55bbbf8b8ffc0c3ba6",
      //     isTable: false,
      //     from:"TextInputprincipal",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'principal', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'principal',type:'text'};
      //   type={
      //     name:'principal',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.principal.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.principal.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.principal.type
      //   }
      ...
      //   type={
      //     name:'principal',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.principal.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.principal.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.principal.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const financial_details_group52f47Ref = useRef<any>(financial_details_group52f47);
  useEffect(() => { financial_details_group52f47Ref.current = financial_details_group52f47; }, [financial_details_group52f47]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "1072412c049f4d55bbbf8b8ffc0c3ba6") {
        handleChange({target:{value:financial_details_group52f47Ref?.current?.principal||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "1072412c049f4d55bbbf8b8ffc0c3ba6") {
        handleBlur({target:{value:financial_details_group52f47Ref?.current?.principal||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_group52f47((pre:any)=>({...pre,principal:dfd_addcase_v1Props.data[0]?.principal}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (principalc3ba6?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 6`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={financial_details_group52f47?.principal||""}
         disabled= {principalc3ba6?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.addCase_v1?.principal ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputprincipal
