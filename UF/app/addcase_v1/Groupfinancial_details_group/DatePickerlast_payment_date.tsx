...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,last_payment_date:undefined}}));
  if (!date) {
    setfinancial_details_group52f47((prev: any) => ({ ...prev, last_payment_date: null }));
    return;
  }
  const now = new Date();
  ...
  setfinancial_details_group52f47((prev: any) => ({ ...prev, last_payment_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "8be4bd31438e4f90bd2d5587bfb23905"
      );
...
useEffect(()=>{
 ...
 },[last_payment_date23905?.refresh])
...
if (last_payment_date23905?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `8 / 16`,gridRow: `10 / 22`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={financial_details_group52f47?.last_payment_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {last_payment_date23905?.isDisabled ? true : false}
      disabled= {last_payment_date23905?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Last Payment Date"
      dateValidation=""
      validationState={validate?.addCase_v1?.last_payment_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
