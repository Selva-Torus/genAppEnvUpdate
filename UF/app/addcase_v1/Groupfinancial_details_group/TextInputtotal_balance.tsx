...
const TextInputtotal_balance = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|564f1b6503aa4531afd30f4a41852f47|9050fb44d93e4c229baff2da932d15a0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,total_balance:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_group52f47((prev: any) => ({ ...prev, total_balance: +e.target.value }));
    }
    else{
    setfinancial_details_group52f47((prev: any) => ({ ...prev, total_balance: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9050fb44d93e4c229baff2da932d15a0"
      );
      ...
      //     controlId: "9050fb44d93e4c229baff2da932d15a0",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9050fb44d93e4c229baff2da932d15a0") {
        handleChange({target:{value:financial_details_group52f47Ref?.current?.total_balance||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9050fb44d93e4c229baff2da932d15a0") {
        handleBlur({target:{value:financial_details_group52f47Ref?.current?.total_balance||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_group52f47((pre:any)=>({...pre,total_balance:dfd_addcase_v1Props.data[0]?.total_balance}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (total_balanced15a0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={financial_details_group52f47?.total_balance||""}
         disabled= {total_balanced15a0?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.addCase_v1?.total_balance ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
