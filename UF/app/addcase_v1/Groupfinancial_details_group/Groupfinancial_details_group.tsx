...
const Groupfinancial_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "564f1b6503aa4531afd30f4a41852f47");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfinancial_details_group52f47Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("financial_dtls_text")){
        setfinancial_dtls_text3b122((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(financial_dtls_text3b122?.isDisabled==null)
      {
        setfinancial_dtls_text3b122((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("charge_off_date")){
        setcharge_off_datef15ef((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(charge_off_datef15ef?.isDisabled==null)
      {
        setcharge_off_datef15ef((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("last_payment_date")){
        setlast_payment_date23905((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(last_payment_date23905?.isDisabled==null)
      {
        setlast_payment_date23905((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_balance")){
        settotal_balanced15a0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balanced15a0?.isDisabled==null)
      {
        settotal_balanced15a0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("principal")){
        setprincipalc3ba6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principalc3ba6?.isDisabled==null)
      {
        setprincipalc3ba6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("interest")){
        setinterest81968((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(interest81968?.isDisabled==null)
      {
        setinterest81968((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees")){
        setfees0f99a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees0f99a?.isDisabled==null)
      {
        setfees0f99a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const financial_details_group52f47Ref = useRef<any>(null);
  const handleClearSearch = () => {
    financial_details_group52f47Ref.current?.setSearchParams();
    financial_details_group52f47Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(financial_details_group52f47) && Object.keys(financial_details_group52f47)?.length>0)
      {
        setfinancial_details_group52f47({})
      }
    }else 
      prevRefreshRef.current= true
  }, [financial_details_group52f47Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"financial_details_group"}))
        }}
    >
          {allowedControls.includes("financial_dtls_text") ?<Textfinancial_dtls_text   /* 3b122 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("charge_off_date") ?<DatePickercharge_off_date   /* f15ef */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("last_payment_date") ?<DatePickerlast_payment_date   /* 23905 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("total_balance") ?<TextInputtotal_balance   /* d15a0 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("principal") ?<TextInputprincipal   /* c3ba6 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("interest") ?<TextInputinterest   /* 81968 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("fees") ?<TextInputfees   /* 0f99a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
