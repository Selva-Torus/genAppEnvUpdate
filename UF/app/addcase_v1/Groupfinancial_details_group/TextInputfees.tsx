...
const TextInputfees = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.fees"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|564f1b6503aa4531afd30f4a41852f47|da70e32985ec478a83ca12fbe470f99a"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'fees',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,fees:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_group52f47((prev: any) => ({ ...prev, fees: +e.target.value }));
    }
    else{
    setfinancial_details_group52f47((prev: any) => ({ ...prev, fees: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "da70e32985ec478a83ca12fbe470f99a"
      );
      ...
      //     controlId: "da70e32985ec478a83ca12fbe470f99a",
      //     isTable: false,
      //     from:"TextInputfees",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'fees', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'fees',type:'text'};
      //   type={
      //     name:'fees',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.fees.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.fees.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.fees.type
      //   }
      ...
      //   type={
      //     name:'fees',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.fees.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.fees.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.fees.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "da70e32985ec478a83ca12fbe470f99a") {
        handleChange({target:{value:financial_details_group52f47Ref?.current?.fees||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "da70e32985ec478a83ca12fbe470f99a") {
        handleBlur({target:{value:financial_details_group52f47Ref?.current?.fees||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_group52f47((pre:any)=>({...pre,fees:dfd_addcase_v1Props.data[0]?.fees}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (fees0f99a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `11 / 16`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={financial_details_group52f47?.fees||""}
         disabled= {fees0f99a?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        view='normal'
        contentAlign={"left"}
        ...
        headerText="Fees"
      errorMessage={error}
        validationState={validate?.addCase_v1?.fees ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
