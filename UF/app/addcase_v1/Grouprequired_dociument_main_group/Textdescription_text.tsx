...
const Textdescription_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {description_textb45efProps, setdescription_textb45efProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[description_textb45ef?.refresh])

  if (description_textb45ef?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `97 / 103`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset("All required documents must be uploaded before submitting to the AMR Queue.")}
</Text>
  </div>
  )
}

export default Textdescription_text
