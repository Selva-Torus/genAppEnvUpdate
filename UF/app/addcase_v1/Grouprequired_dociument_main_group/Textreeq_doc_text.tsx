...
const Textreeq_doc_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {reeq_doc_text9e8f8Props, setreeq_doc_text9e8f8Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reeq_doc_text9e8f8?.refresh])

  if (reeq_doc_text9e8f8?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Required Documents")}
</Text>
  </div>
  )
}

export default Textreeq_doc_text
