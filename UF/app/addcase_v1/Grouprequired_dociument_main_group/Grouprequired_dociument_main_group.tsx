...
import Grouprequired_dociument_header_group  from "../Grouprequired_dociument_header_group/Grouprequired_dociument_header_group";
...
const Grouprequired_dociument_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {reeq_doc_text9e8f8, setreeq_doc_text9e8f8}= useContext(TotalContext) as TotalContextProps;
  ...
  const {description_textb45ef, setdescription_textb45ef}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5b2a911b73ac493aba792ab00d804e92");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_main_group04e92Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reeq_doc_text")){
        setreeq_doc_text9e8f8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reeq_doc_text9e8f8?.isDisabled==null)
      {
        setreeq_doc_text9e8f8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("required_dociument_header_group")){
        setrequired_dociument_header_groupe39c8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(required_dociument_header_groupe39c8?.isDisabled==null)
      {
        setrequired_dociument_header_groupe39c8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("description_text")){
        setdescription_textb45ef((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(description_textb45ef?.isDisabled==null)
      {
        setdescription_textb45ef((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['reeq_doc_text'] = reeq_doc_text9e8f8,
        codeStates['setreeq_doc_text'] = setreeq_doc_text9e8f8,
        ...
        codeStates['description_text'] = description_textb45ef,
        codeStates['setdescription_text'] = setdescription_textb45ef,
  ...
  const required_dociument_main_group04e92Ref = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_main_group04e92Ref.current?.setSearchParams();
    required_dociument_main_group04e92Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_main_group04e92) && Object.keys(required_dociument_main_group04e92)?.length>0)
      {
        setrequired_dociument_main_group04e92({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_main_group04e92Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_main_group"}))
        }}
    >
        {allowedComponent.includes("required_dociument_header_group")  &&<Grouprequired_dociument_header_group  
          ...
          {allowedControls.includes("reeq_doc_text") ?<Textreeq_doc_text   /* 9e8f8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("description_text") ?<Textdescription_text   /* b45ef */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
