...
const TextInputssn_masked = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|bc50558abc744bbaae03288432278a70|0530e75123d74668b2a4b9f58c9273fe"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,ssn_masked:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdebtor_information_group78a70((prev: any) => ({ ...prev, ssn_masked: +e.target.value }));
    }
    else{
    setdebtor_information_group78a70((prev: any) => ({ ...prev, ssn_masked: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "0530e75123d74668b2a4b9f58c9273fe"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1",
      //     componentId: "bc50558abc744bbaae03288432278a70",
      //     controlId: "0530e75123d74668b2a4b9f58c9273fe",
      //     isTable: false,
  ...
  const debtor_information_group78a70Ref = useRef<any>(debtor_information_group78a70);
  useEffect(() => { debtor_information_group78a70Ref.current = debtor_information_group78a70; }, [debtor_information_group78a70]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "0530e75123d74668b2a4b9f58c9273fe") {
        handleChange({target:{value:debtor_information_group78a70Ref?.current?.ssn_masked||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "0530e75123d74668b2a4b9f58c9273fe") {
        handleBlur({target:{value:debtor_information_group78a70Ref?.current?.ssn_masked||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setdebtor_information_group78a70((pre:any)=>({...pre,ssn_masked:dfd_addcase_v1Props.data[0]?.ssn_masked}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (ssn_masked273fe?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={debtor_information_group78a70?.ssn_masked||""}
         disabled= {ssn_masked273fe?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="SSN"
      errorMessage={error}
        validationState={validate?.addCase_v1?.ssn_masked ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
