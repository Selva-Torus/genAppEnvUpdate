...
const Groupdebtor_information_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bc50558abc744bbaae03288432278a70");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdebtor_information_group78a70Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("debt_info_text")){
        setdebt_info_text9078e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debt_info_text9078e?.isDisabled==null)
      {
        setdebt_info_text9078e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_name")){
        setdebtor_namea5e3f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debtor_namea5e3f?.isDisabled==null)
      {
        setdebtor_namea5e3f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ssn_masked")){
        setssn_masked273fe((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ssn_masked273fe?.isDisabled==null)
      {
        setssn_masked273fe((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dob")){
        setdobdba19((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dobdba19?.isDisabled==null)
      {
        setdobdba19((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address")){
        setaddressa3e42((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(addressa3e42?.isDisabled==null)
      {
        setaddressa3e42((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const debtor_information_group78a70Ref = useRef<any>(null);
  const handleClearSearch = () => {
    debtor_information_group78a70Ref.current?.setSearchParams();
    debtor_information_group78a70Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(debtor_information_group78a70) && Object.keys(debtor_information_group78a70)?.length>0)
      {
        setdebtor_information_group78a70({})
      }
    }else 
      prevRefreshRef.current= true
  }, [debtor_information_group78a70Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"debtor_information_group"}))
        }}
    >
          {allowedControls.includes("debt_info_text") ?<Textdebt_info_text   /* 9078e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("debtor_name") ?<TextInputdebtor_name   /* a5e3f */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ssn_masked") ?<TextInputssn_masked   /* 273fe */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dob") ?<DatePickerdob   /* dba19 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address") ?<TextAreaaddress   /* a3e42 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
