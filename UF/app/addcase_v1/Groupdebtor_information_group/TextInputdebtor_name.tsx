...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|bc50558abc744bbaae03288432278a70|4eeabbf73e014a689d9d434a19ba5e3f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,debtor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdebtor_information_group78a70((prev: any) => ({ ...prev, debtor_name: +e.target.value }));
    }
    else{
    setdebtor_information_group78a70((prev: any) => ({ ...prev, debtor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "4eeabbf73e014a689d9d434a19ba5e3f"
      );
      ...
      //     controlId: "4eeabbf73e014a689d9d434a19ba5e3f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "4eeabbf73e014a689d9d434a19ba5e3f") {
        handleChange({target:{value:debtor_information_group78a70Ref?.current?.debtor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4eeabbf73e014a689d9d434a19ba5e3f") {
        handleBlur({target:{value:debtor_information_group78a70Ref?.current?.debtor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setdebtor_information_group78a70((pre:any)=>({...pre,debtor_name:dfd_addcase_v1Props.data[0]?.debtor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (debtor_namea5e3f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={debtor_information_group78a70?.debtor_name||""}
         disabled= {debtor_namea5e3f?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addCase_v1?.debtor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
