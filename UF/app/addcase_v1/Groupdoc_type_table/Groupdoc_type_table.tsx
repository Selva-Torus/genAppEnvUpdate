...
import Tabledoc_type_table  from './Tabledoc_type_table';  
...
const Groupdoc_type_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2835a4a5d4754169ba15a5e8d16be9fa");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_type_tablebe9faProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("doc_type_id")){
        setdoc_type_idaf61f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_type_idaf61f?.isDisabled==null)
      {
        setdoc_type_idaf61f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_type_name")){
        setdoc_type_name949dd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_type_name949dd?.isDisabled==null)
      {
        setdoc_type_name949dd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uploader")){
        setuploaderdff25((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(uploaderdff25?.isDisabled==null)
      {
        setuploaderdff25((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_reference_url")){
        setdoc_reference_urld9056((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_reference_urld9056?.isDisabled==null)
      {
        setdoc_reference_urld9056((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const doc_type_tablebe9faRef = useRef<any>(null);
  const handleClearSearch = () => {
    doc_type_tablebe9faRef.current?.setSearchParams();
    doc_type_tablebe9faRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_type_tablebe9fa) && Object.keys(doc_type_tablebe9fa)?.length>0)
      {
        setdoc_type_tablebe9fa({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_type_tablebe9faProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"doc_type_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tabledoc_type_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_type_tablebe9faRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupdoc_type_table
