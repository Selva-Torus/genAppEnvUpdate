 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "61db3c5f33304debbbf66a2976fb3e9d"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setvenue_details_group17ac6((pre:any)=>({...pre,efiling_required:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[efiling_requiredb3e9d?.refresh])

  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setvenue_details_group17ac6((prev: any) => ({ ...prev, efiling_required: checked }));
  ...
  if (efiling_requiredb3e9d?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="left"
        disabled= {efiling_requiredb3e9d?.isDisabled ? true : false}
        checked={venue_details_group17ac6?.efiling_required || false} 
