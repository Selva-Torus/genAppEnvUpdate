...
const TextInputcounty = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|98c070f07547402d854c337bbb817ac6|78f2c49d24394dbcb42b9a49734f4404"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,county:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group17ac6((prev: any) => ({ ...prev, county: +e.target.value }));
    }
    else{
    setvenue_details_group17ac6((prev: any) => ({ ...prev, county: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "78f2c49d24394dbcb42b9a49734f4404"
      );
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "78f2c49d24394dbcb42b9a49734f4404") {
        handleChange({target:{value:venue_details_group17ac6Ref?.current?.county||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "78f2c49d24394dbcb42b9a49734f4404") {
        handleBlur({target:{value:venue_details_group17ac6Ref?.current?.county||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group17ac6((pre:any)=>({...pre,county:dfd_addcase_v1Props.data[0]?.county}));
    }
  }
