...
const TextInputjudge_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|98c070f07547402d854c337bbb817ac6|a88306fc6c3048e6803394e503578f03"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,judge_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group17ac6((prev: any) => ({ ...prev, judge_name: +e.target.value }));
    }
    else{
    setvenue_details_group17ac6((prev: any) => ({ ...prev, judge_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a88306fc6c3048e6803394e503578f03"
      );
      ...
      //     controlId: "a88306fc6c3048e6803394e503578f03",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a88306fc6c3048e6803394e503578f03") {
        handleChange({target:{value:venue_details_group17ac6Ref?.current?.judge_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a88306fc6c3048e6803394e503578f03") {
        handleBlur({target:{value:venue_details_group17ac6Ref?.current?.judge_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group17ac6((pre:any)=>({...pre,judge_name:dfd_addcase_v1Props.data[0]?.judge_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (judge_name78f03?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group17ac6?.judge_name||""}
         disabled= {judge_name78f03?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Judge"
      errorMessage={error}
        validationState={validate?.addCase_v1?.judge_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
