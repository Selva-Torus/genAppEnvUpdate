...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,sol_expiry_date:undefined}}));
  if (!date) {
    setvenue_details_group17ac6((prev: any) => ({ ...prev, sol_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvenue_details_group17ac6((prev: any) => ({ ...prev, sol_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "99f6812f74e742a89369584d34d8639c"
      );
...
useEffect(()=>{
  setvenue_details_group17ac6Props((pre:any)=>({...pre,validation:true}))
 },[sol_expiry_date8639c?.refresh])
...
if (sol_expiry_date8639c?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 8`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={venue_details_group17ac6?.sol_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {sol_expiry_date8639c?.isDisabled ? true : false}
      disabled= {sol_expiry_date8639c?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="SOL Expiry Date"
      dateValidation=""
      validationState={validate?.addCase_v1?.sol_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
