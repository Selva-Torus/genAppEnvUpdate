...
const Dropdownservice_method = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(venue_details_group17ac6?.service_method=="" || venue_details_group17ac6?.service_method==undefined || venue_details_group17ac6?.service_method==null ){
    setSelectedItem("");
  }
  },[venue_details_group17ac6?.service_method])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "98c070f07547402d854c337bbb817ac6",
        "8688ad9e831640539f05bd8a41adeae7"
      );
  ...
  },
  {
    ...
    "value": {
      ...
      setvenue_details_group17ac6((prev: any) => ({ ...prev, service_method: staticTextValue, service_methoddeae7: value}))
         setIsRequredData(false)
    } else {
       setvenue_details_group17ac6((prev: any) => ({ ...prev, service_method: '', service_methoddeae7: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,service_method:undefined}}));
  ...
  useEffect(() => { venue_details_group17ac6Ref.current = venue_details_group17ac6; }, [venue_details_group17ac6]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "8688ad9e831640539f05bd8a41adeae7") {
        handleClick(venue_details_group17ac6Ref?.current?.service_methoddeae7?venue_details_group17ac6Ref?.current?.service_methoddeae7:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "8688ad9e831640539f05bd8a41adeae7");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setvenue_details_group17ac6((pre:any)=>({...pre,service_method:""}))
    else
      setInitialCount(1)
  },[service_methoddeae7?.refresh])
  

  if (service_methoddeae7?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        className=""
        placeholder={keyset("Select Service Method")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {service_methoddeae7?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            venue_details_group17ac6?.service_methoddeae7 ? [venue_details_group17ac6?.service_methoddeae7] :
                venue_details_group17ac6?.service_method ? venue_details_group17ac6?.service_method : []
            }
        onChange={handleClick} 
        validationState={validate?.addCase_v1?.service_method ? "invalid" : undefined}
      /> 
    </div>
  );
};

export default Dropdownservice_method;
