...
import Dropdownservice_method  from "./Dropdownservice_method";
...
const Groupvenue_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "98c070f07547402d854c337bbb817ac6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvenue_details_group17ac6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("ven_name_text")){
        setven_name_textef3ac((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ven_name_textef3ac?.isDisabled==null)
      {
        setven_name_textef3ac((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("state")){
        setstate752e3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(state752e3?.isDisabled==null)
      {
        setstate752e3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("country")){
        setcountryf4404((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(countryf4404?.isDisabled==null)
      {
        setcountryf4404((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_name")){
        setcourt_namef21b5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_namef21b5?.isDisabled==null)
      {
        setcourt_namef21b5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("judge_name")){
        setjudge_name78f03((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(judge_name78f03?.isDisabled==null)
      {
        setjudge_name78f03((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("sol_expiry_date")){
        setsol_expiry_date8639c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(sol_expiry_date8639c?.isDisabled==null)
      {
        setsol_expiry_date8639c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("filing_fee")){
        setfiling_fee389dd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(filing_fee389dd?.isDisabled==null)
      {
        setfiling_fee389dd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("service_method")){
        setservice_methoddeae7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(service_methoddeae7?.isDisabled==null)
      {
        setservice_methoddeae7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("efiling_system")){
        setefiling_systemc00e5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(efiling_systemc00e5?.isDisabled==null)
      {
        setefiling_systemc00e5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("efiling_required")){
        setefiling_requiredb3e9d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(efiling_requiredb3e9d?.isDisabled==null)
      {
        setefiling_requiredb3e9d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const venue_details_group17ac6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    venue_details_group17ac6Ref.current?.setSearchParams();
    venue_details_group17ac6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(venue_details_group17ac6) && Object.keys(venue_details_group17ac6)?.length>0)
      {
        setvenue_details_group17ac6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [venue_details_group17ac6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"venue_details_group"}))
        }}
    >
          {allowedControls.includes("ven_name_text") ?<Textven_name_text   /* ef3ac */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("state") ?<TextInputstate   /* 752e3 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("country") ?<TextInputcountry   /* f4404 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("court_name") ?<TextInputcourt_name   /* f21b5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("judge_name") ?<TextInputjudge_name   /* 78f03 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("sol_expiry_date") ?<DatePickersol_expiry_date   /* 8639c */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("filing_fee") ?<TextInputfiling_fee   /* 389dd */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("service_method") ?<Dropdownservice_method   /* deae7 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("efiling_system") ?<Dropdownefiling_system   /* c00e5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("efiling_required")?<Switchefiling_required  /* b3e9d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
