...
const TextInputstate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|98c070f07547402d854c337bbb817ac6|f8de56cfd7924a51ac8f55b0d5f752e3"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,state:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group17ac6((prev: any) => ({ ...prev, state: +e.target.value }));
    }
    else{
    setvenue_details_group17ac6((prev: any) => ({ ...prev, state: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f8de56cfd7924a51ac8f55b0d5f752e3"
      );
      ...
      //     controlId: "f8de56cfd7924a51ac8f55b0d5f752e3",
      //     isTable: false,
      //     from:"TextInputstate",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "f8de56cfd7924a51ac8f55b0d5f752e3") {
        handleChange({target:{value:venue_details_group17ac6Ref?.current?.state||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f8de56cfd7924a51ac8f55b0d5f752e3") {
        handleBlur({target:{value:venue_details_group17ac6Ref?.current?.state||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group17ac6((pre:any)=>({...pre,state:dfd_addcase_v1Props.data[0]?.state}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (state752e3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group17ac6?.state||""}
         disabled= {state752e3?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="State"
      errorMessage={error}
        validationState={validate?.addCase_v1?.state ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
