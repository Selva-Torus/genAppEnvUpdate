...
const Textgeorgias_sosl_texts = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {georgias_sosl_texts1ccd5Props, setgeorgias_sosl_texts1ccd5Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[georgias_sosl_texts1ccd5?.refresh])

  if (georgias_sosl_texts1ccd5?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Wage Garnishment Restrictions")}
</Text>
  </div>
  )
}

export default Textgeorgias_sosl_texts
