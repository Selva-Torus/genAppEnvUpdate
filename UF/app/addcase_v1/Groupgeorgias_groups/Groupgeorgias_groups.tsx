...
const Groupgeorgias_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warningss_iconsb27c3, setwarningss_iconsb27c3}= useContext(TotalContext) as TotalContextProps;
  const {georgias_sosl_texts1ccd5, setgeorgias_sosl_texts1ccd5}= useContext(TotalContext) as TotalContextProps;
  const {specials_rulesscabb6, setspecials_rulesscabb6}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0c8d236fdde94299b6a1d02933b6f85f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgias_groups6f85fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningss_icons")){
        setwarningss_iconsb27c3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningss_iconsb27c3?.isDisabled==null)
      {
        setwarningss_iconsb27c3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_sosl_texts")){
        setgeorgias_sosl_texts1ccd5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_sosl_texts1ccd5?.isDisabled==null)
      {
        setgeorgias_sosl_texts1ccd5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("specials_ruless")){
        setspecials_rulesscabb6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(specials_rulesscabb6?.isDisabled==null)
      {
        setspecials_rulesscabb6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningss_icons'] = warningss_iconsb27c3,
        codeStates['setwarningss_icons'] = setwarningss_iconsb27c3,
        codeStates['georgias_sosl_texts'] = georgias_sosl_texts1ccd5,
        codeStates['setgeorgias_sosl_texts'] = setgeorgias_sosl_texts1ccd5,
        codeStates['specials_ruless'] = specials_rulesscabb6,
        codeStates['setspecials_ruless'] = setspecials_rulesscabb6,
  ...
  const georgias_groups6f85fRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgias_groups6f85fRef.current?.setSearchParams();
    georgias_groups6f85fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgias_groups6f85f) && Object.keys(georgias_groups6f85f)?.length>0)
      {
        setgeorgias_groups6f85f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgias_groups6f85fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgias_groups"}))
        }}
    >
        {allowedControls.includes("warningss_icons")?<Iconwarningss_icons /* b27c3 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgias_sosl_texts") ?<Textgeorgias_sosl_texts   /* 1ccd5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("specials_ruless") ?<Textspecials_ruless   /* cabb6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
