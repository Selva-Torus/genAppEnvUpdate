...
const Textspecials_ruless = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {specials_rulesscabb6Props, setspecials_rulesscabb6Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[specials_rulesscabb6?.refresh])

  if (specials_rulesscabb6?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `12 / 37`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!p-1"
  variant="subheader-3"
  color="primary"
>
      {keyset("Georgia law limits disposable earnings to 25% or less depending on debtor's circumstances.")}
</Text>
  </div>
  )
}

export default Textspecials_ruless
