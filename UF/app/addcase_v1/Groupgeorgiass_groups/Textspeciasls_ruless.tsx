...
const Textspeciasls_ruless = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {speciasls_rulessc3d56Props, setspeciasls_rulessc3d56Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[speciasls_rulessc3d56?.refresh])

  if (speciasls_rulessc3d56?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `12 / 51`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!p-1"
  variant="subheader-3"
  color="primary"
>
      {keyset("Automatically identifies debts related to open accounts in Texas and validates whether the claim falls within the 4-year statute of limitations before initiating legal or collection actions")}
</Text>
  </div>
  )
}

export default Textspeciasls_ruless
