...
import Iconwarningsss_icons  from "./Iconwarningsss_icons";
import Textgeorgiass_sosl_texts  from "./Textgeorgiass_sosl_texts";
import Textspeciasls_ruless  from "./Textspeciasls_ruless";
...
const Groupgeorgiass_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warningsss_iconse38db, setwarningsss_iconse38db}= useContext(TotalContext) as TotalContextProps;
  const {georgiass_sosl_texts52ad5, setgeorgiass_sosl_texts52ad5}= useContext(TotalContext) as TotalContextProps;
  const {speciasls_rulessc3d56, setspeciasls_rulessc3d56}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1988aca9424b4e23a27fa93e67486a87");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgiass_groups86a87Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningsss_icons")){
        setwarningsss_iconse38db((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningsss_iconse38db?.isDisabled==null)
      {
        setwarningsss_iconse38db((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgiass_sosl_texts")){
        setgeorgiass_sosl_texts52ad5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgiass_sosl_texts52ad5?.isDisabled==null)
      {
        setgeorgiass_sosl_texts52ad5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("speciasls_ruless")){
        setspeciasls_rulessc3d56((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(speciasls_rulessc3d56?.isDisabled==null)
      {
        setspeciasls_rulessc3d56((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningsss_icons'] = warningsss_iconse38db,
        codeStates['setwarningsss_icons'] = setwarningsss_iconse38db,
        codeStates['georgiass_sosl_texts'] = georgiass_sosl_texts52ad5,
        codeStates['setgeorgiass_sosl_texts'] = setgeorgiass_sosl_texts52ad5,
        codeStates['speciasls_ruless'] = speciasls_rulessc3d56,
        codeStates['setspeciasls_ruless'] = setspeciasls_rulessc3d56,
  ...
  const georgiass_groups86a87Ref = useRef<any>(null);
  const handleClearSearch = () => {
    georgiass_groups86a87Ref.current?.setSearchParams();
    georgiass_groups86a87Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgiass_groups86a87) && Object.keys(georgiass_groups86a87)?.length>0)
      {
        setgeorgiass_groups86a87({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgiass_groups86a87Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setaddcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgiass_groups"}))
        }}
    >
        {allowedControls.includes("warningsss_icons")?<Iconwarningsss_icons /* e38db */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgiass_sosl_texts") ?<Textgeorgiass_sosl_texts   /* 52ad5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("speciasls_ruless") ?<Textspeciasls_ruless   /* c3d56 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgeorgiass_groups
