...
const Textcreditor_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {creditor_idf6f71Props, setcreditor_idf6f71Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setadd_case_groupeb161((pre: any) => ({
          ...pre,
          creditor_id: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.creditor_id
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setadd_case_groupeb161((pre: any) => ({
          ...pre,
          creditor_id: creditor_idf6f71Props?.filteredData?.length > 0
            ? creditor_idf6f71Props?.filteredData[0]?.creditor_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_groupeb161.creditor_id){
        setadd_case_groupeb161((pre:any)=>({...pre,creditor_id:dfd_addcase_v1Props[0]?.creditor_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[creditor_idf6f71?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_groupeb161.creditor_id){
    setadd_case_groupeb161((pre:any)=>({...pre,creditor_id:dfd_addcase_v1Props[0]?.creditor_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!creditor_idf6f71Props?.filterProps) return;
    handleMapperValue(creditor_idf6f71Props?.filterProps,creditor_idf6f71Props?.filterFlag);
  },[creditor_idf6f71Props?.filterProps])

  if (creditor_idf6f71?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `16 / 18`,gridRow: `8 / 9`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
...
export default Textcreditor_id
