...
const TextArearemarks_textarea = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "cabf4518f6a147d3a0ffaa03a4ceb161",
        "4dc7cfa77e3c497f93e26fa1e06dc753"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[remarks_textareadc753?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setadd_case_groupeb161((pre:any)=>({...pre,remarks_textarea:""}))
    }else 
      prevRefreshRef.current= true
  },[remarks_textareadc753?.refresh])

  const add_case_groupeb161Ref = useRef<any>(add_case_groupeb161);
  useEffect(() => { add_case_groupeb161Ref.current = add_case_groupeb161; }, [add_case_groupeb161]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "4dc7cfa77e3c497f93e26fa1e06dc753") {
        handleChange({target:{value:add_case_groupeb161Ref?.current?.remarks_textarea||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4dc7cfa77e3c497f93e26fa1e06dc753") {
        handleBlur({target:{value:add_case_groupeb161Ref?.current?.remarks_textarea||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,addCase_v1:{...pre?.addCase_v1,remarks_textarea:undefined}}));
    if(dynamicStateandType.type=="number"){
    setadd_case_groupeb161((prev: any) => ({ ...prev, remarks_textarea: +e?.target?.value }));
    }
    else{
    setadd_case_groupeb161((prev: any) => ({ ...prev, remarks_textarea: e?.target?.value }));
    }
  ...
  if (remarks_textareadc753?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 25`,gridRow: `444 / 461`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {remarks_textareadc753?.isDisabled ? true : false}
      placeholder = {'Enter any additional remarks or notes'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Remarks"
      pin = {'brick-brick'}
      value = { add_case_groupeb161?.remarks_textarea != null && typeof add_case_groupeb161?.remarks_textarea =='object' ? Object.keys(add_case_groupeb161?.remarks_textarea)?.length ?  JSON.stringify(add_case_groupeb161?.remarks_textarea,null ,2):"" : add_case_groupeb161?.remarks_textarea||""}
      errorMessage={error}
      validationState={validate?.addCase_v1?.remarks_textarea ? "invalid" : undefined}
    />
  </div>
  )
}
