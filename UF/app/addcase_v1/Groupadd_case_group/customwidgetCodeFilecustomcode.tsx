...
const DocumentUploader = React.forwardRef(
  (
    {
      dropzoneOptions,
      value = [],
    ...
    const [files, setFiles] = React.useState<Drag_file>(
      ...
      setFiles((prevFiles: any) => {
        ...
        if (singleSelect) {
          if (typeof onChange === 'function') {
            onChange([
              { file: filesWithUrls[0].file, url: filesWithUrls[0].url }
            ])
          }
          ...
          if (typeof onChange === 'function') {
            onChange(
              updatedFiles.map((f: any) => ({ file: f.file, url: f.url }))
            )
          }
          return updatedFiles
        }
      ...
      return (
        <div className='h-full w-full p-4' {...getRootProps()}>
          ...
          <div className='flex h-full flex-col gap-4'>
            ...
            <div
              className={`
              flex min-h-[180px] flex-col items-center
            ...
            {/* File List */}
            {files?.length > 0 && (
              <div className='flex max-h-[200px] flex-col gap-2 overflow-y-auto scrollbar-thin'>
                <span
    ...
    const uploaderElement = (
      ...
      >
        <div
          className={`flex  w-full ${fillContainer ? 'h-full w-full' : ''}`}
          {...getRootProps()}
        >
          {viewType == 'modal' ? (
            <div
              className={`flex  w-full ${
                fillContainer ? 'h-full w-full' : 'justify-start'
              } ${className}`}
            >
              <Button
                pin='round-round'
          ...
          <Modal
            ...
            title={
              ...
              >
                ...
                <div
                  className={`flex flex-col ${getBorderRadiusClass(
                    branding.borderRadius
                  )}`}
                >
                  <span
...
const Viewer = ({ file, url, closeFn }: any) => {
  ...
  return (
    <>
      {file && (
        <div key={file.name} className=' overflow-hidden rounded-md border p-2'>
          ...
          {(file.type ===
            ...
            sheetData.length > 0 && (
              <div className='max-h-[53vh] overflow-auto rounded-md border p-2'>
                <table className='w-full table-auto border-collapse border border-gray-300 text-sm'>
                  <tbody>
                    {sheetData.map((row, i) => (
                      <tr
                        key={i}
                        className={i === 0 ? 'bg-gray-100 font-bold' : ''}
                      >
...
const customwidgetCodeFilecustomcode = (props: any) => {
  const uploaderRef = useRef<{ open: () => void; close: () => void }>(null)
  const { lockedData, setLockedData } = useContext(
    TotalContext
  ) as TotalContextProps
  useEffect(() => {
    if (props?.customwidget?.open == true) {
      uploaderRef.current?.open()
    }
  }, [props?.customwidget?.open])

  const [docs, setDocs] = useState({})
  console.log(lockedData)

  const [files, setFiles] = useState<any[]>([])
  const handleFileSet = async (e: any) => {
    setFiles((pre: any) => ({
      ...pre,
      [props?.customwidget?.mainData?.doc_type_id || '55555']: e
    }))

    const currentValue: any = e
    let fileBody: any = currentValue.map((item: any) => item?.file)
    const formData = new FormData()
    fileBody.forEach((file: File) => {
      formData.append('file', file)
    })
    formData.append('context', e)
    ...
    let temp = props?.dynamicactions?.doc_type_table || []
    if (
      temp.findIndex(
        (item: any) =>
          item?.doc_type_id == props?.customwidget?.mainData?.doc_type_id
      ) != -1
    ) {
      temp.map((item: any, idx: number) => {
        if (item?.doc_type_id == props?.customwidget?.mainData?.doc_type_id) {
          temp[idx] = { ...temp[idx], doc_reference_url: res.data.imageUrl }
        }
      })
    } else {
      temp.push({
        doc_type_id: props?.customwidget?.mainData?.doc_type_id,
        doc_reference_url: res.data.imageUrl
      })
    }
    props?.setdynamicactions((pre: any) => ({
      ...pre,
      doc_type_table: temp
    }))
  }
console.log(props?.financial_details_group);

useEffect(()=>{

    props?.setfinancial_details_group((pre:any)=>({...pre,
        total_balance:(+pre?.fees) + (+pre?.interest) + (+pre?.principal)
    }))

},[ props?.financial_details_group?.fees,
    props?.financial_details_group?.principal,
    props?.financial_details_group?.interest
])

  useEffect(() => {
    if (lockedData?.selectedData?.length > 0) {
      let temp: any = []
      lockedData?.selectedData?.map((item: any) => {
        temp.push({
      ...
      props?.setdynamicactions((pre: any) => ({
        ...pre,
        checklist_table: temp
      }))
    } else {
    }
  }, [lockedData])
//   console.log(props?.dynamicactions)

  return (
    <div>
      {props?.customwidget?.open == true ? (
        <div className='w-[2px]'>
          <DocumentUploader
            ref={uploaderRef}
            id='customwidget-document-uploader'
            value={
              files?.[props?.customwidget?.mainData?.doc_type_id || '55555']
            }
            onChange={(e: any) => {
              handleFileSet(e)
            }}
            onClose={() => {
              props?.setcustomwidget((pre: any) => ({ ...pre, open: false }))
            }}
            viewType='modal'
            singleSelect={false}
          />
...
export default customwidgetCodeFilecustomcode 
