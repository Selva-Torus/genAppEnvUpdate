...
const Textaccount_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {account_id7e64eProps, setaccount_id7e64eProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setadd_case_groupeb161((pre: any) => ({
          ...pre,
          account_id: account_id7e64eProps?.filteredData?.length > 0
            ? account_id7e64eProps?.filteredData[0]?.account_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_groupeb161.account_id){
        setadd_case_groupeb161((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[account_id7e64e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_groupeb161.account_id){
    setadd_case_groupeb161((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!account_id7e64eProps?.filterProps) return;
    handleMapperValue(account_id7e64eProps?.filterProps,account_id7e64eProps?.filterFlag);
  },[account_id7e64eProps?.filterProps])

  if (account_id7e64e?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `23 / 25`,gridRow: `346 / 347`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
