...
export default function PageDocumentviewerV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      ...
      assigndoctable_v1:false,
      categorydoctable_v1:false,
    });
    async function assetdoctable_v1DFD(pagination:any): Promise<void>{
        ...
        if(documentviewer_v1Props.length > 0){
          for(let i=0;i< documentviewer_v1Props.length;i++){
            if(documentviewer_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDocTable:AFVK:v1"){
  ...
  useEffect(()=>{
    ...
    async function assigndoctable_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let assigndoctable_v1Body:te_refreshDto={
        ...
        if (encryptionFlagPage) {          
          assigndoctable_v1Body["dpdKey"] = encryptionDpd;
          assigndoctable_v1Body["method"] = encryptionMethod;
        }
        if(documentviewer_v1Props.length > 0){
          for(let i=0;i< documentviewer_v1Props.length;i++){
            if(documentviewer_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assignDocTable:AFVK:v1"){
              // delete documentviewer_v1Props[i].DFDkey;
              let temp=structuredClone(documentviewer_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
          ...
          assigndoctable_v1Body['filterData'] = filterData;
        }
        const assigndoctable_v1Data:any=await AxiosService.post("/te/eventEmitter",assigndoctable_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=assigndoctable_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(assigndoctable_v1Data?.data?.dataset === 'Bulk Data Processing'){
          if(filterData.length>0){
            const api_paginationBody: api_paginationDto = {
          key: dstKey,
          count:parseInt(pagination?.count) || 10,
        ...
        setdfd_assigndoctable_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_assigndoctable_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (assigndoctable_v1Data?.data?.dataset) {
           setdfd_assigndoctable_v1Props(
              Array.isArray(assigndoctable_v1Data?.data?.dataset?.data)
                 ? assigndoctable_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
                    Object.entries(obj || {}).map(([key, value]) => [
                      key.toLowerCase(),
                      value
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.assigndoctable_v1) {
      assigndoctable_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.assigndoctable_v1= true
  },[refetch?.assigndoctable_v1])
    async function categorydoctable_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let categorydoctable_v1Body:te_refreshDto={
        ...
        if (encryptionFlagPage) {          
          categorydoctable_v1Body["dpdKey"] = encryptionDpd;
          categorydoctable_v1Body["method"] = encryptionMethod;
        }
        if(documentviewer_v1Props.length > 0){
          for(let i=0;i< documentviewer_v1Props.length;i++){
            if(documentviewer_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:categoryDocTable:AFVK:v1"){
              // delete documentviewer_v1Props[i].DFDkey;
              let temp=structuredClone(documentviewer_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
          ...
          categorydoctable_v1Body['filterData'] = filterData;
        }
        const categorydoctable_v1Data:any=await AxiosService.post("/te/eventEmitter",categorydoctable_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=categorydoctable_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(categorydoctable_v1Data?.data?.dataset === 'Bulk Data Processing'){
          if(filterData.length>0){
            const api_paginationBody: api_paginationDto = {
          key: dstKey,
          count:parseInt(pagination?.count) || 10,
        ...
        setdfd_categorydoctable_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_categorydoctable_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (categorydoctable_v1Data?.data?.dataset) {
           setdfd_categorydoctable_v1Props(
              Array.isArray(categorydoctable_v1Data?.data?.dataset?.data)
                 ? categorydoctable_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
                    Object.entries(obj || {}).map(([key, value]) => [
                      key.toLowerCase(),
                      value
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.categorydoctable_v1) {
      categorydoctable_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.categorydoctable_v1= true
  },[refetch?.categorydoctable_v1])
  ...
  async function securityCheck(): Promise<void> {
    ...
    const data: any = await fetchBatchData(
      'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:documentViewer:AFVK:v1',
      [user],
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:documentViewer:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
    ...
    await assigndoctable_v1DFD(pagination)
    await categorydoctable_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setdocumentviewer_v1(allRuleData)
  }, [])
