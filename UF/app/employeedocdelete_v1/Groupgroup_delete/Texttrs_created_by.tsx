...
const Texttrs_created_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employeedoctable_v1Props) && dfd_employeedoctable_v1Props && !group_delete6f37f.trs_created_by){
        setgroup_delete6f37f((pre:any)=>({...pre,trs_created_by:dfd_employeedoctable_v1Props[0]?.trs_created_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[trs_created_by4392c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employeedoctable_v1Props) && !group_delete6f37f.trs_created_by){
    setgroup_delete6f37f((pre:any)=>({...pre,trs_created_by:dfd_employeedoctable_v1Props[0]?.trs_created_by}));
  }
  },[dfd_employeedoctable_v1Props])

  if (trs_created_by4392c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.trs_created_by : (group_delete6f37f?.trs_created_by || ""))}
</Text>
  </div>
  )
}
