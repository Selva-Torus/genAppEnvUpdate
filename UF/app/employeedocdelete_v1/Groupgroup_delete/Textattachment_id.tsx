...
const Textattachment_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employeedoctable_v1Props) && dfd_employeedoctable_v1Props && !group_delete6f37f.attachment_id){
        setgroup_delete6f37f((pre:any)=>({...pre,attachment_id:dfd_employeedoctable_v1Props[0]?.attachment_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[attachment_id662fa?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employeedoctable_v1Props) && !group_delete6f37f.attachment_id){
    setgroup_delete6f37f((pre:any)=>({...pre,attachment_id:dfd_employeedoctable_v1Props[0]?.attachment_id}));
  }
  },[dfd_employeedoctable_v1Props])

  if (attachment_id662fa?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.attachment_id : (group_delete6f37f?.attachment_id || ""))}
</Text>
  </div>
  )
}
