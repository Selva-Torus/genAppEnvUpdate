 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "69be76271ad813049b5b8e69584c9ba6"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonc9ba6") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "69be76271ad813049b5b8e69584c9ba6") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonc9ba6");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonc9ba6?.trigger) return;
      if(ok_buttonc9ba6?.trigger){
      setok_buttonc9ba6((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonc9ba6?.trigger])

  useEffect(()=>{
    if(ok_buttonc9ba6?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonc9ba6?.refresh])
  ...
  async function handleSave9ba6_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_delete6f37fProps = group_delete6f37fPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.employeeDocDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_delete6f37fRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete6f37f?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "69be76271ad813049b5b8e69584c9ba6",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "69be76271ad813049b5b8e69584c9ba6.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "69be76271ad813049b5b8e69584c9ba6.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "69be76271ad813049b5b8e69584c9ba6.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "69be76271ad813049b5b8e69584c9ba6.1.1.1.2",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeDocDelete:AFVK:v1";
      sourceId+= "|"+"61db4e2c83132ca9f61c25fb39e6f37f";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeDocDelete:AFVK:v1"+"|"+"61db4e2c83132ca9f61c25fb39e6f37f"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete6f37f?.upId? [group_delete6f37f?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete6f37f?.upId? [group_delete6f37f?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete6f37f?.upId? [group_delete6f37f?.upId ] : group_delete6f37f?.trs_process_id? [group_delete6f37f?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete6f37fRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"61db4e2c83132ca9f61c25fb39e6f37f",
              controlId:"69be76271ad813049b5b8e69584c9ba6"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete6f37fProps.ssKey !== '' && group_delete6f37fProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete6f37fProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete6f37f((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave9ba6_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete6f37f((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='normal'
          disabled= {ok_buttonc9ba6?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
