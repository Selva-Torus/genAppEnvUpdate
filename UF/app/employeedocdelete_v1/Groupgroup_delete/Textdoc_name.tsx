...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employeedoctable_v1Props) && dfd_employeedoctable_v1Props && !group_delete6f37f.doc_name){
        setgroup_delete6f37f((pre:any)=>({...pre,doc_name:dfd_employeedoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_name5ec80?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employeedoctable_v1Props) && !group_delete6f37f.doc_name){
    setgroup_delete6f37f((pre:any)=>({...pre,doc_name:dfd_employeedoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_employeedoctable_v1Props])

  if (doc_name5ec80?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete6f37f?.doc_name || ""))}
</Text>
  </div>
  )
}
