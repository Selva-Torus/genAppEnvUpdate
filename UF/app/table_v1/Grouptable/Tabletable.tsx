...
let defaultColumns:any = [
  {
    "id": "expense_name",
    "nodeid": "c11ff4dfaaac4c768f6a33ddd85eac5c",
    "name": "Expense Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cetegory",
    "nodeid": "6945ee7b4ec141099f83cce00bccd65c",
    "name": "Cetegory",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {expense_nameeac5c, setexpense_nameeac5c}= useContext(TotalContext) as TotalContextProps  
  const {cetegorycd65c, setcetegorycd65c}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "3b14a2643a394afe85afafd6b235ca87",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, table5ca87 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, table5ca87 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, table5ca87 } = latestLockStateRef.current
    /// settable5ca87Props
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settable5ca87Props((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.claim_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      table5ca87.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.claim_id){
      ...
      settable5ca87Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['expense_name'] = expense_nameeac5c,
        codeStates['setexpense_name'] = setexpense_nameeac5c,
        codeStates['cetegory'] = cetegorycd65c,
        codeStates['setcetegory'] = setcetegorycd65c,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable5ca87Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:Table:AFVK:v1:UO', 
                nodeId: '3b14a2643a394afe85afafd6b235ca87',
                elementId: '3b14a2643a394afe85afafd6b235ca87'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table5ca87Props?.presetValues&&Object.keys(table5ca87Props?.presetValues).length > 0) {
          filtertedData = [table5ca87Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable5ca87(api_pagination?.data?.records||[])
        }
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,claim_id:filtertedData[i]?.claim_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.claim_id==item?.claim_id)
        {
          index=i
          row=data
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table5ca87Props.filterInitalLoad) return;

    const filterControllers = table5ca87Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table5ca87']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table5ca87']).filter((k: string) => jsonEntry['table5ca87'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(table5ca87Props.filterProps);
      return;
    }
    ...
    settable5ca87Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table5ca87Props.filterProps);

  }, [
    table5ca87Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table5ca87Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table5ca87Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table5ca87Props.searchFilter) || {};
  ...
  useEffect(() => {
    if (table5ca87Props?.clearData === true) {
      settable5ca87([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settable5ca87Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table5ca87Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [table5ca87Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = table5ca87Props?.selectedIds[table5ca87Props?.selectedIds?.length-1]
    if(Array.isArray(table5ca87) && table5ca87.length>0)
    {
      let data = table5ca87.find((data:any)=>(data?.claim_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "claim_id",
        value: data["claim_id"],
  ...
  useEffect(() => {
    if (table5ca87Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (table5ca87Props?.selectedIds?.length == 0){
      handleOnRowClick({},table5ca87Props?.selectedIds)
    }
  }, [table5ca87Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.claim_id==data?.claim_id)
        {
          index=i
          row=item
        }
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table5ca87?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table5ca87?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table5ca87?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table5ca87?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table5ca87?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table5ca87?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table5ca87?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"3b14a2643a394afe85afafd6b235ca87"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={table5ca87Props?.selectedIds}  
