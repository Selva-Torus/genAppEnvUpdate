...
const Groupinitiate_asset_disposal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setinitiate_asset_disposal_groupdb5a7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("disposal_details_group")){
        setdisposal_details_groupe1b0c({...disposal_details_groupe1b0c,isDisabled:true});

    }else
    {
      if(disposal_details_groupe1b0c?.isDisabled==null)
      {
        setdisposal_details_groupe1b0c({...disposal_details_groupe1b0c,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("compliance_financial_group")){
        setcompliance_financial_group1f9bc({...compliance_financial_group1f9bc,isDisabled:true});

    }else
    {
      if(compliance_financial_group1f9bc?.isDisabled==null)
      {
        setcompliance_financial_group1f9bc({...compliance_financial_group1f9bc,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactions9a7ff({...dynamicactions9a7ff,isDisabled:true});

    }else
    {
      if(dynamicactions9a7ff?.isDisabled==null)
      {
        setdynamicactions9a7ff({...dynamicactions9a7ff,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const initiate_asset_disposal_groupdb5a7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    initiate_asset_disposal_groupdb5a7Ref.current?.setSearchParams();
    initiate_asset_disposal_groupdb5a7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(initiate_asset_disposal_groupdb5a7) && Object.keys(initiate_asset_disposal_groupdb5a7)?.length>0)
      {
        setinitiate_asset_disposal_groupdb5a7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [initiate_asset_disposal_groupdb5a7Props?.refresh,token])
