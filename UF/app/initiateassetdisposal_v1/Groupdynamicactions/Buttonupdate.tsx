 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "deef8c9b6c0a4c8a872db09fb8b92b0c"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("update" in dynamicactions9a7ffProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions9a7ffProps?.dynamicActionRule?.update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("update" in initiateassetdisposal_v1?.dynamicactions && initiateassetdisposal_v1?.dynamicactions["update"]?.itsHaveArtifact== true)
      {
        setShowFlag(initiateassetdisposal_v1?.dynamicactions["update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(initiateassetdisposal_v1?.dynamicactions?.update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "deef8c9b6c0a4c8a872db09fb8b92b0c") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "update92b0c");
    return () => {
  ...
  useEffect(()=>{
    if (!update92b0c?.trigger) return;
      if(update92b0c?.trigger){
      setupdate92b0c((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  async function handleSave2b0c_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const dynamicactions9a7ffProps = dynamicactions9a7ffPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.initiateAssetDisposal_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(dynamicactions9a7ffRef.current);
      ...
      let lockedKeysLength:number;
        if(dynamicactions9a7ff?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "deef8c9b6c0a4c8a872db09fb8b92b0c",
  "type": "button",
  "name": "update",
  "label": "update",
  "sequence": 1,
  "children": [
    {
      "id": "deef8c9b6c0a4c8a872db09fb8b92b0c.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "deef8c9b6c0a4c8a872db09fb8b92b0c.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "deef8c9b6c0a4c8a872db09fb8b92b0c.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Disposal Updated Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:initiateAssetDisposal:AFVK:v1";
      sourceId+= "|"+"fe8f3e5f71864cebb471d21fc069a7ff";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:initiateAssetDisposal:AFVK:v1"+"|"+"fe8f3e5f71864cebb471d21fc069a7ff"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions9a7ff?.upId? [dynamicactions9a7ff?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions9a7ff?.upId? [dynamicactions9a7ff?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions9a7ff?.upId? [dynamicactions9a7ff?.upId ] : dynamicactions9a7ff?.trs_process_id? [dynamicactions9a7ff?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "update"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions9a7ffRef.current, ...nullFilter(initiate_asset_disposal_groupdb5a7), ...nullFilter(disposal_details_groupe1b0c), ...nullFilter(compliance_financial_group1f9bc) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"fe8f3e5f71864cebb471d21fc069a7ff",
              controlId:"deef8c9b6c0a4c8a872db09fb8b92b0c"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (dynamicactions9a7ffProps.ssKey !== '' && dynamicactions9a7ffProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = dynamicactions9a7ffProps.ssKey;          
    }
    ...
    //infoMsg
    toast('Disposal Updated Successfully', 'success',false);
    }
    catch(err:any)
    {
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions9a7ff((prev: any) => ({ ...prev, update: true }));
        //onClick

    //eventEmitter
    await handleSave2b0c_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions9a7ff((prev: any) => ({ ...prev, update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [initiateassetdisposal_v1?.dynamicactions?.update,dynamicactions9a7ffProps?.dynamicActionRule?.update,])

 if (update92b0c?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {update92b0c?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
