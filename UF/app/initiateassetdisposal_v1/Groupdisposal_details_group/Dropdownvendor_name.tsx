...
const Dropdownvendor_name = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(disposal_details_groupe1b0c?.vendor_name=="" || disposal_details_groupe1b0c?.vendor_name==undefined || disposal_details_groupe1b0c?.vendor_name==null ){
    setSelectedItem("");
  }
  },[disposal_details_groupe1b0c?.vendor_name])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "2693374bb2d64ed88d121dd7c5ee1b0c",
        "901c06c330684e0fafdd9f82927abdbb"
      );
      ...
      let staticValueProps : any[] = [
  {
    ...
    "value": {
      ...
      setdisposal_details_groupe1b0c((prev: any) => ({ ...prev, vendor_name: staticTextValue, vendor_nameabdbb: value}))
         setIsRequredData(false)
    } else {
       setdisposal_details_groupe1b0c((prev: any) => ({ ...prev, vendor_name: '', vendor_nameabdbb: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,initiateAssetDisposal_v1:{...pre?.initiateAssetDisposal_v1,vendor_name:undefined}}));
  ...
  useEffect(() => { disposal_details_groupe1b0cRef.current = disposal_details_groupe1b0c; }, [disposal_details_groupe1b0c]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "901c06c330684e0fafdd9f82927abdbb") {
        handleClick(disposal_details_groupe1b0cRef?.current?.vendor_nameabdbb?disposal_details_groupe1b0cRef?.current?.vendor_nameabdbb:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "901c06c330684e0fafdd9f82927abdbb");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setdisposal_details_groupe1b0c((pre:any)=>({...pre,vendor_name:""}))
    else
      setInitialCount(1)
  },[vendor_nameabdbb?.refresh])

  if (vendor_nameabdbb?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {vendor_nameabdbb?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            disposal_details_groupe1b0c?.vendor_nameabdbb ? [disposal_details_groupe1b0c?.vendor_nameabdbb] :
                disposal_details_groupe1b0c?.vendor_name ? disposal_details_groupe1b0c?.vendor_name : []
            }
        onChange={handleClick} 
        validationState={validate?.initiateAssetDisposal_v1?.vendor_name ? "invalid" : undefined}
      /> 
    </div>
  );
};
