...
const TextInputdisposal_value = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.disposal_value"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:initiateAssetDisposal:AFVK:v1|16048a3a05ac4926a046632180f1f9bc|c575f701e896479aa7ef6886642d21f4"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'disposal_value',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,initiateAssetDisposal_v1:{...pre?.initiateAssetDisposal_v1,disposal_value:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, disposal_value: +e.target.value }));
    }
    else{
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, disposal_value: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c575f701e896479aa7ef6886642d21f4"
      );
      ...
      //     controlId: "c575f701e896479aa7ef6886642d21f4",
      //     isTable: false,
      //     from:"TextInputdisposal_value",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'disposal_value', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'disposal_value',type:'text'};
      //   type={
      //     name:'disposal_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_value.type
      //   }
      ...
      //   type={
      //     name:'disposal_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_value.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c575f701e896479aa7ef6886642d21f4") {
        handleChange({target:{value:compliance_financial_group1f9bcRef?.current?.disposal_value||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c575f701e896479aa7ef6886642d21f4") {
        handleBlur({target:{value:compliance_financial_group1f9bcRef?.current?.disposal_value||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_group1f9bc((pre:any)=>({...pre,disposal_value:dfd_assetdisposal_v1Props.data[0]?.disposal_value}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (disposal_valued21f4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_group1f9bc?.disposal_value||""}
         disabled= {disposal_valued21f4?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Disposal Value"
      errorMessage={error}
        validationState={validate?.initiateAssetDisposal_v1?.disposal_value ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdisposal_value
