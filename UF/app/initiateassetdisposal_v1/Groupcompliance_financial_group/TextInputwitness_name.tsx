...
const TextInputwitness_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.witness_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:initiateAssetDisposal:AFVK:v1|16048a3a05ac4926a046632180f1f9bc|08860b08948b4f49bfa416fa0c6ac8f7"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'witness_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,initiateAssetDisposal_v1:{...pre?.initiateAssetDisposal_v1,witness_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, witness_name: +e.target.value }));
    }
    else{
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, witness_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "08860b08948b4f49bfa416fa0c6ac8f7"
      );
      ...
      //     controlId: "08860b08948b4f49bfa416fa0c6ac8f7",
      //     isTable: false,
      //     from:"TextInputwitness_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'witness_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'witness_name',type:'text'};
      //   type={
      //     name:'witness_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.witness_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.witness_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.witness_name.type
      //   }
      ...
      //   type={
      //     name:'witness_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.witness_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.witness_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.witness_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "08860b08948b4f49bfa416fa0c6ac8f7") {
        handleChange({target:{value:compliance_financial_group1f9bcRef?.current?.witness_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "08860b08948b4f49bfa416fa0c6ac8f7") {
        handleBlur({target:{value:compliance_financial_group1f9bcRef?.current?.witness_name||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_group1f9bc((pre:any)=>({...pre,witness_name:dfd_assetdisposal_v1Props.data[0]?.witness_name}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (witness_nameac8f7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_group1f9bc?.witness_name||""}
         disabled= {witness_nameac8f7?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Witness Name"
      errorMessage={error}
        validationState={validate?.initiateAssetDisposal_v1?.witness_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputwitness_name
