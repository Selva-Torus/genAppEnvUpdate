...
const TextInputdisposal_cost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.disposal_cost"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:initiateAssetDisposal:AFVK:v1|16048a3a05ac4926a046632180f1f9bc|b4473b36d41240478112e8a46d9031f6"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'disposal_cost',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,initiateAssetDisposal_v1:{...pre?.initiateAssetDisposal_v1,disposal_cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, disposal_cost: +e.target.value }));
    }
    else{
    setcompliance_financial_group1f9bc((prev: any) => ({ ...prev, disposal_cost: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b4473b36d41240478112e8a46d9031f6"
      );
      ...
      //     controlId: "b4473b36d41240478112e8a46d9031f6",
      //     isTable: false,
      //     from:"TextInputdisposal_cost",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'disposal_cost', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'disposal_cost',type:'text'};
      //   type={
      //     name:'disposal_cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_cost.type
      //   }
      ...
      //   type={
      //     name:'disposal_cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_cost.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b4473b36d41240478112e8a46d9031f6") {
        handleChange({target:{value:compliance_financial_group1f9bcRef?.current?.disposal_cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b4473b36d41240478112e8a46d9031f6") {
        handleBlur({target:{value:compliance_financial_group1f9bcRef?.current?.disposal_cost||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_group1f9bc((pre:any)=>({...pre,disposal_cost:dfd_assetdisposal_v1Props.data[0]?.disposal_cost}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (disposal_cost031f6?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_group1f9bc?.disposal_cost||""}
         disabled= {disposal_cost031f6?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Disposal Cost"
      errorMessage={error}
        validationState={validate?.initiateAssetDisposal_v1?.disposal_cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdisposal_cost
