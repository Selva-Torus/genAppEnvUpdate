...
const Groupaddt__dts_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1821c4e38fd97308ea72ccd29820d865");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddt__dts_group0d865Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("add_inf")){
        setadd_inf7c94d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_inf7c94d?.isDisabled==null)
      {
        setadd_inf7c94d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("notes")){
        setnotes14713((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(notes14713?.isDisabled==null)
      {
        setnotes14713((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_dts")){
        setadd_dts8a152((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_dts8a152?.isDisabled==null)
      {
        setadd_dts8a152((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const addt__dts_group0d865Ref = useRef<any>(null);
  const handleClearSearch = () => {
    addt__dts_group0d865Ref.current?.setSearchParams();
    addt__dts_group0d865Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addt__dts_group0d865) && Object.keys(addt__dts_group0d865)?.length>0)
      {
        setaddt__dts_group0d865({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addt__dts_group0d865Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewbackgroundcheck_v1((pre:any)=>({...pre,_selectedGroup_:"addt__dts_group"}))
        }}
    >
          {allowedControls.includes("add_inf") ?<Textadd_inf   /* 7c94d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("notes") ?<TextAreanotes   /* 14713 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("add_dts") ?<TextAreaadd_dts   /* 8a152 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
