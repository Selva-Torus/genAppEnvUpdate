...
const TextAreanotes = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "50aa3aa62bd247a58c201bc273d14713"
      );
  ...
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setaddt__dts_group0d865((pre:any)=>({...pre,notes:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "50aa3aa62bd247a58c201bc273d14713") {
        handleChange({target:{value:addt__dts_group0d865Ref?.current?.notes||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "50aa3aa62bd247a58c201bc273d14713") {
        handleBlur({target:{value:addt__dts_group0d865Ref?.current?.notes||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newBackgroundCheck_v1:{...pre?.newBackgroundCheck_v1,notes:undefined}}));
    setaddt__dts_group0d865((prev: any) => ({ ...prev, notes: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (notes14713?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `7 / 24`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {notes14713?.isDisabled ? true : false}
      placeholder = {'type here...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Notes"
      pin = {'brick-brick'}
      value = { addt__dts_group0d865?.notes != null && typeof addt__dts_group0d865?.notes =='object' ? Object.keys(addt__dts_group0d865?.notes)?.length ?  JSON.stringify(addt__dts_group0d865?.notes,null ,2):"" : addt__dts_group0d865?.notes||""}
      errorMessage={error}
      validationState={validate?.newBackgroundCheck_v1?.notes ? "invalid" : undefined}
    />
  </div>
  )
}
