...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,newBackgroundCheck_v1:{...pre?.newBackgroundCheck_v1,completed_date:undefined}}));
  if (!date) {
    setaccess_req__groupdd45d((prev: any) => ({ ...prev, completed_date: null }));
    return;
  }
  const now = new Date();
  ...
  setaccess_req__groupdd45d((prev: any) => ({ ...prev, completed_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "ba8ce099ff764be78ea20b5c50f8c01c"
      );
...
useEffect(()=>{
  setaccess_req__groupdd45dProps((pre:any)=>({...pre,validation:true}))
 },[completed_date8c01c?.refresh])
...
if (completed_date8c01c?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 7`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={access_req__groupdd45d?.completed_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {completed_date8c01c?.isDisabled ? true : false}
      disabled= {completed_date8c01c?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Completed Date"
      dateValidation=""
      validationState={validate?.newBackgroundCheck_v1?.completed_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
