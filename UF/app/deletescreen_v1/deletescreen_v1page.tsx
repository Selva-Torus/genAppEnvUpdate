...
export default function PageDeletescreenV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assets_v1DFD(pagination:any): Promise<void>{
        ...
        if(deletescreen_v1Props.length > 0){
          for(let i=0;i< deletescreen_v1Props.length;i++){
            if(deletescreen_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1"){
              // delete deletescreen_v1Props[i].DFDkey;
              let temp=structuredClone(deletescreen_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setdeletescreen_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:deleteScreen:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setdeletescreen_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(deletescreen_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(deletescreen_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
