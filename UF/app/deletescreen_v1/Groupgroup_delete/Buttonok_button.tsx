...
const Buttonok_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "bc56a31e9cea48a5aeb4bbcf93658a95"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_button58a95") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "bc56a31e9cea48a5aeb4bbcf93658a95") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_button58a95");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_button58a95?.trigger) return;
      if(ok_button58a95?.trigger){
      setok_button58a95((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_button58a95?.trigger])

  useEffect(()=>{
    if(ok_button58a95?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_button58a95?.refresh])
  ...
  async function handleSave8a95_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete3c02fProps = group_delete3c02fPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.deleteScreen_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete3c02fRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete3c02f?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "bc56a31e9cea48a5aeb4bbcf93658a95",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "bc56a31e9cea48a5aeb4bbcf93658a95.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "bc56a31e9cea48a5aeb4bbcf93658a95.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "bc56a31e9cea48a5aeb4bbcf93658a95.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "bc56a31e9cea48a5aeb4bbcf93658a95.1.1.1.2",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:deleteScreen:AFVK:v1";
      sourceId+= "|"+"58a9b96c02f241298beab7b73d03c02f";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:deleteScreen:AFVK:v1"+"|"+"58a9b96c02f241298beab7b73d03c02f"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete3c02f?.upId? [group_delete3c02f?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3c02f?.upId? [group_delete3c02f?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3c02f?.upId? [group_delete3c02f?.upId ] : group_delete3c02f?.trs_process_id? [group_delete3c02f?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete3c02fRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"58a9b96c02f241298beab7b73d03c02f",
              controlId:"bc56a31e9cea48a5aeb4bbcf93658a95"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete3c02fProps.ssKey !== '' && group_delete3c02fProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete3c02fProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete3c02f((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave8a95_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete3c02f((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (ok_button58a95?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `20 / 25`,gridRow: `42 / 49`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="   "
          onClick={handleClick}
          view='action'
          disabled= {ok_button58a95?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
