...
const Textasset_type = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assets_v1Props && !dfd_assets_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3c02f((pre: any) => {
          return { ...pre, asset_type: api_paginationData.data.records[0]?.asset_type }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assets_v1Props) && dfd_assets_v1Props && !group_delete3c02f.asset_type){
        setgroup_delete3c02f((pre:any)=>({...pre,asset_type:dfd_assets_v1Props[0]?.asset_type}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[asset_typebe078?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assets_v1Props) && !group_delete3c02f.asset_type){
    setgroup_delete3c02f((pre:any)=>({...pre,asset_type:dfd_assets_v1Props[0]?.asset_type}));
  }
  },[dfd_assets_v1Props])

  if (asset_typebe078?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.asset_type : (group_delete3c02f?.asset_type || ""))}
</Text>
  </div>
  )
}

export default Textasset_type
