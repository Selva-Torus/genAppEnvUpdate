...
const Textasset_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assets_v1Props) && dfd_assets_v1Props && !group_delete3c02f.asset_id){
        setgroup_delete3c02f((pre:any)=>({...pre,asset_id:dfd_assets_v1Props[0]?.asset_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[asset_id4d81b?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assets_v1Props) && !group_delete3c02f.asset_id){
    setgroup_delete3c02f((pre:any)=>({...pre,asset_id:dfd_assets_v1Props[0]?.asset_id}));
  }
  },[dfd_assets_v1Props])

  if (asset_id4d81b?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.asset_id : (group_delete3c02f?.asset_id || ""))}
</Text>
  </div>
  )
}
