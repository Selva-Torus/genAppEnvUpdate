...
const Textasset_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assets_v1Props) && dfd_assets_v1Props && !group_delete3c02f.asset_name){
        setgroup_delete3c02f((pre:any)=>({...pre,asset_name:dfd_assets_v1Props[0]?.asset_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[asset_named7764?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assets_v1Props) && !group_delete3c02f.asset_name){
    setgroup_delete3c02f((pre:any)=>({...pre,asset_name:dfd_assets_v1Props[0]?.asset_name}));
  }
  },[dfd_assets_v1Props])

  if (asset_named7764?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.asset_name : (group_delete3c02f?.asset_name || ""))}
</Text>
  </div>
  )
}
