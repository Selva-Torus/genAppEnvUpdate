...
const Textcategory_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assets_v1Props) && dfd_assets_v1Props && !group_delete3c02f.category_name){
        setgroup_delete3c02f((pre:any)=>({...pre,category_name:dfd_assets_v1Props[0]?.category_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[category_nameb3bdb?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assets_v1Props) && !group_delete3c02f.category_name){
    setgroup_delete3c02f((pre:any)=>({...pre,category_name:dfd_assets_v1Props[0]?.category_name}));
  }
  },[dfd_assets_v1Props])

  if (category_nameb3bdb?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.category_name : (group_delete3c02f?.category_name || ""))}
</Text>
  </div>
  )
}
