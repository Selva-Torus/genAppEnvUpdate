...
const Textlocation = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assets_v1Props && !dfd_assets_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3c02f((pre: any) => {
          return { ...pre, location: api_paginationData.data.records[0]?.location }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assets_v1Props) && dfd_assets_v1Props && !group_delete3c02f.location){
        setgroup_delete3c02f((pre:any)=>({...pre,location:dfd_assets_v1Props[0]?.location}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[location0b4e4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assets_v1Props) && !group_delete3c02f.location){
    setgroup_delete3c02f((pre:any)=>({...pre,location:dfd_assets_v1Props[0]?.location}));
  }
  },[dfd_assets_v1Props])

  if (location0b4e4?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `31 / 36`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.location : (group_delete3c02f?.location || ""))}
</Text>
  </div>
  )
}
