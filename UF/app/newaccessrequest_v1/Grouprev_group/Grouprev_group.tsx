...
const Grouprev_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {access_revok8cc13, setaccess_revok8cc13}= useContext(TotalContext) as TotalContextProps;
  const {prov_inf_des3629c, setprov_inf_des3629c}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "252f66569bbebca8a48b3fd9c85a6a87");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrev_groupa6a87Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_revok")){
        setaccess_revok8cc13((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_revok8cc13?.isDisabled==null)
      {
        setaccess_revok8cc13((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_inf_des")){
        setprov_inf_des3629c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf_des3629c?.isDisabled==null)
      {
        setprov_inf_des3629c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['access_revok'] = access_revok8cc13,
        codeStates['setaccess_revok'] = setaccess_revok8cc13,
        codeStates['prov_inf_des'] = prov_inf_des3629c,
        codeStates['setprov_inf_des'] = setprov_inf_des3629c,
  ...
  const rev_groupa6a87Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rev_groupa6a87Ref.current?.setSearchParams();
    rev_groupa6a87Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rev_groupa6a87) && Object.keys(rev_groupa6a87)?.length>0)
      {
        setrev_groupa6a87({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rev_groupa6a87Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"rev_group"}))
        }}
    >
          {allowedControls.includes("access_revok") ?<Textaccess_revok   /* 8cc13 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("prov_inf_des") ?<Textprov_inf_des   /* 3629c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
