 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "35a812d848224213a71ccebd87464168"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("save" in dynamicactions820e8Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions820e8Props?.dynamicActionRule?.save,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("save" in newaccessrequest_v1?.dynamicactions && newaccessrequest_v1?.dynamicactions["save"]?.itsHaveArtifact== true)
      {
        setShowFlag(newaccessrequest_v1?.dynamicactions["save"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newaccessrequest_v1?.dynamicactions?.save?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "35a812d848224213a71ccebd87464168") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "save64168");
    return () => {
  ...
  useEffect(()=>{
    if (!save64168?.trigger) return;
      if(save64168?.trigger){
      setsave64168((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  async function handleSave4168_1_1_1(){
  ...
  const hasInvalidField = Object.values(currentValidate?.newAccessRequest_v1 || {}).some(val => {
    ...
    try{
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|df4fd23f80574a0d9ec4fb6fbd383638"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|df4fd23f80574a0d9ec4fb6fbd383638"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "35a812d848224213a71ccebd87464168",
  "type": "button",
  "name": "save",
  "label": "save",
  "sequence": 1,
  "children": [
    {
      "id": "35a812d848224213a71ccebd87464168.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "35a812d848224213a71ccebd87464168.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "35a812d848224213a71ccebd87464168.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "35a812d848224213a71ccebd87464168.1.1.1.2",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|df4fd23f80574a0d9ec4fb6fbd383638"
          ]
        }
      ]
    }
  ...
  // saveHandler
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"35a812d848224213a71ccebd87464168"
            };
            if (encryptionFlagCont) {
        ...
        if(dynamicactions820e8?.upId){
          te_saveBody['upId']= dynamicactions820e8?.upId;
        }
        if(dynamicactions820e8?.upid){
          te_saveBody['upId']= dynamicactions820e8?.upid;
        }
        ...
        if (dynamicactions820e8Props.ssKey !== '' && dynamicactions820e8Props.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactions820e8Props.ssKey;
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions820e8((prev: any) => ({ ...prev, save: true }));
        //onClick

    //saveHandler
    await handleSave4168_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions820e8((prev: any) => ({ ...prev, save: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newaccessrequest_v1?.dynamicactions?.save,dynamicactions820e8Props?.dynamicActionRule?.save,])

 if (save64168?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='normal'
          disabled= {save64168?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
