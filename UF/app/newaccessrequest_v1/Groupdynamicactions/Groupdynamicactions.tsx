...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9b6c7ea332084d03bf447831f39820e8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions820e8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancelec530((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancelec530?.isDisabled==null)
      {
        setcancelec530((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_updated86f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_updated86f7?.isDisabled==null)
      {
        setbutton_updated86f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("save")){
        setsave64168((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(save64168?.isDisabled==null)
      {
        setsave64168((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions820e8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions820e8Ref.current?.setSearchParams();
    dynamicactions820e8Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions820e8Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions820e8Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions820e8) && Object.keys(dynamicactions820e8)?.length>0)
      {
        setdynamicactions820e8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions820e8Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
