...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,valid_from: "invalid"}}))
    setvalid_groupec21c((prev: any) => ({ ...prev, valid_from: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,valid_from:undefined}}));
  if (!date) {
    setvalid_groupec21c((prev: any) => ({ ...prev, valid_from: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_groupec21c((prev: any) => ({ ...prev, valid_from: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_groupec21c?.valid_from == "" || valid_groupec21c?.valid_from == undefined){
        const result = v.safeParse(schema, valid_groupec21c?.valid_from || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "85f6cfc7c667edc789a22670c6a0917c"
      );
...
useEffect(()=>{
 ...
 },[valid_from0917c?.refresh])

useEffect(()=>{
  if(!valid_groupec21c?.valid_from){ 
...
return (
  ...
  style={{gridColumn: `1 / 9`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_groupec21c?.valid_from}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {valid_from0917c?.isDisabled ? true : false}
      disabled= {valid_from0917c?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Valid From"
      dateValidation=""
      validationState={validate?.newAccessRequest_v1?.valid_from ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
