...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,valid_to: "invalid"}}))
    setvalid_groupec21c((prev: any) => ({ ...prev, valid_to: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,valid_to:undefined}}));
  if (!date) {
    setvalid_groupec21c((prev: any) => ({ ...prev, valid_to: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_groupec21c((prev: any) => ({ ...prev, valid_to: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_groupec21c?.valid_to == "" || valid_groupec21c?.valid_to == undefined){
        const result = v.safeParse(schema, valid_groupec21c?.valid_to || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "c7b7a6f8d07eae4120cb5f7af4606467"
      );
...
useEffect(()=>{
 ...
 },[valid_to06467?.refresh])

useEffect(()=>{
  if(!valid_groupec21c?.valid_to){ 
...
return (
  ...
  style={{gridColumn: `9 / 17`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_groupec21c?.valid_to}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {valid_to06467?.isDisabled ? true : false}
      disabled= {valid_to06467?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Valid To"
      dateValidation=""
      validationState={validate?.newAccessRequest_v1?.valid_to ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
