 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "59d601ae77a6a76898190e92c56cc6ef"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setvalid_groupec21c((pre:any)=>({...pre,access_review_required:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[access_review_req_switchcc6ef?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props?.length == 1){
      setvalid_groupec21c((pre:any)=>({...pre,access_review_required:dfd_addaccessrequestmodify_v1Props[0]?.access_review_required}))
    }
  },[dfd_addaccessrequestmodify_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setvalid_groupec21c((prev: any) => ({ ...prev, access_review_required: checked }));
  ...
  if (access_review_req_switchcc6ef?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="top"
        disabled= {access_review_req_switchcc6ef?.isDisabled ? true : false}
        content=" "
        checked={valid_groupec21c?.access_review_required || false} 
