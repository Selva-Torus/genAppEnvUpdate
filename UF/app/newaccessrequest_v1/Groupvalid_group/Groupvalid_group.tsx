...
const Groupvalid_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "56a365d2f50867ba2f88da8e214ec21c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalid_groupec21cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("validity_details")){
        setvalidity_detailsa3730((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(validity_detailsa3730?.isDisabled==null)
      {
        setvalidity_detailsa3730((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_from")){
        setvalid_from0917c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_from0917c?.isDisabled==null)
      {
        setvalid_from0917c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_to")){
        setvalid_to06467((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_to06467?.isDisabled==null)
      {
        setvalid_to06467((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_review_req_switch")){
        setaccess_review_req_switchcc6ef((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_review_req_switchcc6ef?.isDisabled==null)
      {
        setaccess_review_req_switchcc6ef((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_expiry_date")){
        setaccess_expiry_date729dc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_expiry_date729dc?.isDisabled==null)
      {
        setaccess_expiry_date729dc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const valid_groupec21cRef = useRef<any>(null);
  const handleClearSearch = () => {
    valid_groupec21cRef.current?.setSearchParams();
    valid_groupec21cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(valid_groupec21c) && Object.keys(valid_groupec21c)?.length>0)
      {
        setvalid_groupec21c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [valid_groupec21cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"valid_group"}))
        }}
    >
          {allowedControls.includes("validity_details") ?<Textvalidity_details   /* a3730 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("valid_from") ?<DatePickervalid_from   /* 0917c */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("valid_to") ?<DatePickervalid_to   /* 06467 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("access_review_req_switch")?<Switchaccess_review_req_switch  /* cc6ef */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("access_expiry_date") ?<DatePickeraccess_expiry_date   /* 729dc */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
