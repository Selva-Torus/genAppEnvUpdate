...
const TextAreabusiness_justification = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ce8a290653d385ff81936894dd98767b"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[business_justification8767b?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setbusiness_just__group75edc((pre:any)=>({...pre,business_justification:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ce8a290653d385ff81936894dd98767b") {
        handleChange({target:{value:business_just__group75edcRef?.current?.business_justification||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ce8a290653d385ff81936894dd98767b") {
        handleBlur({target:{value:business_just__group75edcRef?.current?.business_justification||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,business_justification:undefined}}));
    setbusiness_just__group75edc((prev: any) => ({ ...prev, business_justification: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (business_justification8767b?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `7 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {business_justification8767b?.isDisabled ? true : false}
      placeholder = {'Provide detailed business justification for this access request...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Business Justification"
      pin = {'brick-brick'}
      value = { business_just__group75edc?.business_justification != null && typeof business_just__group75edc?.business_justification =='object' ? Object.keys(business_just__group75edc?.business_justification)?.length ?  JSON.stringify(business_just__group75edc?.business_justification,null ,2):"" : business_just__group75edc?.business_justification||""}
      errorMessage={error}
      validationState={validate?.newAccessRequest_v1?.business_justification ? "invalid" : undefined}
    />
  </div>
  )
}
