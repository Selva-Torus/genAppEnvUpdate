...
const TextAreareject_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "8bab233f52ecff8f1b0b169bc96ea43d",
        "e57bb984ed6f7ccdd9c576dd0b71b936"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reject_reason1b936?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_inf_groupea43d((pre:any)=>({...pre,rejection_reason:""}))
    }else 
      prevRefreshRef.current= true
  },[reject_reason1b936?.refresh])

  const app_inf_groupea43dRef = useRef<any>(app_inf_groupea43d);
  useEffect(() => { app_inf_groupea43dRef.current = app_inf_groupea43d; }, [app_inf_groupea43d]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "e57bb984ed6f7ccdd9c576dd0b71b936") {
        handleChange({target:{value:app_inf_groupea43dRef?.current?.rejection_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e57bb984ed6f7ccdd9c576dd0b71b936") {
        handleBlur({target:{value:app_inf_groupea43dRef?.current?.rejection_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,rejection_reason:undefined}}));
    setapp_inf_groupea43d((prev: any) => ({ ...prev, rejection_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (reject_reason1b936?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `7 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {reject_reason1b936?.isDisabled ? true : false}
      placeholder = {'Enter rejection reason if any...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Rejection Reason"
      pin = {'brick-brick'}
      value = { app_inf_groupea43d?.rejection_reason != null && typeof app_inf_groupea43d?.rejection_reason =='object' ? Object.keys(app_inf_groupea43d?.rejection_reason)?.length ?  JSON.stringify(app_inf_groupea43d?.rejection_reason,null ,2):"" : app_inf_groupea43d?.rejection_reason||""}
      errorMessage={error}
      validationState={validate?.newAccessRequest_v1?.rejection_reason ? "invalid" : undefined}
    />
  </div>
  )
}
