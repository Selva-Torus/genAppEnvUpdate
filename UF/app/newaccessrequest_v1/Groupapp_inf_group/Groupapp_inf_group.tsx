...
const Groupapp_inf_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {app_infa651b, setapp_infa651b}= useContext(TotalContext) as TotalContextProps;
  const {approval_comments2f3a8, setapproval_comments2f3a8}= useContext(TotalContext) as TotalContextProps;
  const {reject_reason1b936, setreject_reason1b936}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8bab233f52ecff8f1b0b169bc96ea43d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapp_inf_groupea43dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_inf")){
        setapp_infa651b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_infa651b?.isDisabled==null)
      {
        setapp_infa651b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approval_comments")){
        setapproval_comments2f3a8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approval_comments2f3a8?.isDisabled==null)
      {
        setapproval_comments2f3a8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_reason")){
        setreject_reason1b936((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_reason1b936?.isDisabled==null)
      {
        setreject_reason1b936((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['app_inf'] = app_infa651b,
        codeStates['setapp_inf'] = setapp_infa651b,
        codeStates['approval_comments'] = approval_comments2f3a8,
        codeStates['setapproval_comments'] = setapproval_comments2f3a8,
        codeStates['reject_reason'] = reject_reason1b936,
        codeStates['setreject_reason'] = setreject_reason1b936,
  ...
  const app_inf_groupea43dRef = useRef<any>(null);
  const handleClearSearch = () => {
    app_inf_groupea43dRef.current?.setSearchParams();
    app_inf_groupea43dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(app_inf_groupea43d) && Object.keys(app_inf_groupea43d)?.length>0)
      {
        setapp_inf_groupea43d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [app_inf_groupea43dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"app_inf_group"}))
        }}
    >
          {allowedControls.includes("app_inf") ?<Textapp_inf   /* a651b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("approval_comments") ?<TextAreaapproval_comments   /* 2f3a8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("reject_reason") ?<TextAreareject_reason   /* 1b936 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
