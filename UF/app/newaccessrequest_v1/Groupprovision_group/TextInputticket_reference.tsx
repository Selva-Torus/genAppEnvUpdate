...
const TextInputticket_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|a03f43d59623a24f595dce6a88b4e2a2|57994ec94921ca113d3a0618d950daa0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,ticket_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setprovision_group4e2a2((prev: any) => ({ ...prev, ticket_reference: +e.target.value }));
    }
    else{
    setprovision_group4e2a2((prev: any) => ({ ...prev, ticket_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "57994ec94921ca113d3a0618d950daa0"
      );
      ...
      //     controlId: "57994ec94921ca113d3a0618d950daa0",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "57994ec94921ca113d3a0618d950daa0") {
        handleChange({target:{value:provision_group4e2a2Ref?.current?.ticket_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "57994ec94921ca113d3a0618d950daa0") {
        handleBlur({target:{value:provision_group4e2a2Ref?.current?.ticket_reference||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setprovision_group4e2a2((pre:any)=>({...pre,ticket_reference:dfd_addaccessrequestmodify_v1Props.data[0]?.ticket_reference}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (ticket_reference0daa0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={provision_group4e2a2?.ticket_reference||""}
         disabled= {ticket_reference0daa0?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newAccessRequest_v1?.ticket_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
