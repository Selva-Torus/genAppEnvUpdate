...
const TextInputprovisioning_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|a03f43d59623a24f595dce6a88b4e2a2|d68ffc87c5bbdf9e2793df8aef5d433b"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,provisioning_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setprovision_group4e2a2((prev: any) => ({ ...prev, provisioning_reference: +e.target.value }));
    }
    else{
    setprovision_group4e2a2((prev: any) => ({ ...prev, provisioning_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d68ffc87c5bbdf9e2793df8aef5d433b"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1",
      //     componentId: "a03f43d59623a24f595dce6a88b4e2a2",
      //     controlId: "d68ffc87c5bbdf9e2793df8aef5d433b",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "d68ffc87c5bbdf9e2793df8aef5d433b") {
        handleChange({target:{value:provision_group4e2a2Ref?.current?.provisioning_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d68ffc87c5bbdf9e2793df8aef5d433b") {
        handleBlur({target:{value:provision_group4e2a2Ref?.current?.provisioning_reference||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setprovision_group4e2a2((pre:any)=>({...pre,provisioning_reference:dfd_addaccessrequestmodify_v1Props.data[0]?.provisioning_reference}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (provisioning_referenced433b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={provision_group4e2a2?.provisioning_reference||""}
         disabled= {provisioning_referenced433b?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newAccessRequest_v1?.provisioning_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
