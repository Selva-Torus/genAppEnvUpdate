...
const Textaccess_req_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[access_req_id8cf02?.refresh])

  if (access_req_id8cf02?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 4`,gridRow: `152 / 153`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.access_req_id : (new_access_group1e8f3?.access_req_id || ""))}
</Text>
  </div>
  )
}
