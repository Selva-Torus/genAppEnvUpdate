...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,revoked_at:undefined}}));
  if (!date) {
    setrevocation_groupc3044((prev: any) => ({ ...prev, revoked_at: null }));
    return;
  }
  const now = new Date();
  ...
  setrevocation_groupc3044((prev: any) => ({ ...prev, revoked_at: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "55b396c5d06e31078837a472dd3c9732"
      );
...
useEffect(()=>{
  setrevocation_groupc3044Props((pre:any)=>({...pre,validation:true}))
 },[revok_atc9732?.refresh])
...
return (
  ...
  style={{gridColumn: `13 / 25`,gridRow: `9 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={revocation_groupc3044?.revoked_at}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {revok_atc9732?.isDisabled ? true : false}
      disabled= {revok_atc9732?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Revoked At"
      dateValidation=""
      validationState={validate?.newAccessRequest_v1?.revoked_at ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
