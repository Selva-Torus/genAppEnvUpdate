...
const Grouprevocation_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "79c8ea2f4c06beb1534cb019bebc3044");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrevocation_groupc3044Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rev_group")){
        setrev_groupa6a87((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(rev_groupa6a87?.isDisabled==null)
      {
        setrev_groupa6a87((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revoked_by")){
        setrevoked_by47ca5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revoked_by47ca5?.isDisabled==null)
      {
        setrevoked_by47ca5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revok_at")){
        setrevok_atc9732((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revok_atc9732?.isDisabled==null)
      {
        setrevok_atc9732((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revoked_reason")){
        setrevoked_reason43c9c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revoked_reason43c9c?.isDisabled==null)
      {
        setrevoked_reason43c9c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const revocation_groupc3044Ref = useRef<any>(null);
  const handleClearSearch = () => {
    revocation_groupc3044Ref.current?.setSearchParams();
    revocation_groupc3044Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(revocation_groupc3044) && Object.keys(revocation_groupc3044)?.length>0)
      {
        setrevocation_groupc3044({})
      }
    }else 
      prevRefreshRef.current= true
  }, [revocation_groupc3044Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"revocation_group"}))
        }}
    >
        ...
        {allowedControls.includes("revoked_by") ?<TextInputrevoked_by   /* 47ca5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("revok_at") ?<DatePickerrevok_at   /* c9732 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("revoked_reason") ?<TextArearevoked_reason   /* 43c9c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
