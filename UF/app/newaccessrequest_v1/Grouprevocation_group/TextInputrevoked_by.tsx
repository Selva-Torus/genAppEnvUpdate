...
const TextInputrevoked_by = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|79c8ea2f4c06beb1534cb019bebc3044|fc3c140b9b29ca976ef7e040e2147ca5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,revoked_by:undefined}}));
    if(dynamicStateandType.type=="number"){
    setrevocation_groupc3044((prev: any) => ({ ...prev, revoked_by: +e.target.value }));
    }
    else{
    setrevocation_groupc3044((prev: any) => ({ ...prev, revoked_by: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "fc3c140b9b29ca976ef7e040e2147ca5"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1",
      //     componentId: "79c8ea2f4c06beb1534cb019bebc3044",
      //     controlId: "fc3c140b9b29ca976ef7e040e2147ca5",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "fc3c140b9b29ca976ef7e040e2147ca5") {
        handleChange({target:{value:revocation_groupc3044Ref?.current?.revoked_by||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "fc3c140b9b29ca976ef7e040e2147ca5") {
        handleBlur({target:{value:revocation_groupc3044Ref?.current?.revoked_by||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setrevocation_groupc3044((pre:any)=>({...pre,revoked_by:dfd_addaccessrequestmodify_v1Props.data[0]?.revoked_by}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (revoked_by47ca5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={revocation_groupc3044?.revoked_by||""}
         disabled= {revoked_by47ca5?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newAccessRequest_v1?.revoked_by ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
