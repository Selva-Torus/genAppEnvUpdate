...
const Groupprov_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {prov_infa6c26, setprov_infa6c26}= useContext(TotalContext) as TotalContextProps;
  const {prov_inf_des4a871, setprov_inf_des4a871}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cfc8f82186645c11f5d42de95303b4eb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprov_group3b4ebProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prov_inf")){
        setprov_infa6c26((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_infa6c26?.isDisabled==null)
      {
        setprov_infa6c26((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_inf_des")){
        setprov_inf_des4a871((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf_des4a871?.isDisabled==null)
      {
        setprov_inf_des4a871((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['prov_inf'] = prov_infa6c26,
        codeStates['setprov_inf'] = setprov_infa6c26,
        codeStates['prov_inf_des'] = prov_inf_des4a871,
        codeStates['setprov_inf_des'] = setprov_inf_des4a871,
  ...
  const prov_group3b4ebRef = useRef<any>(null);
  const handleClearSearch = () => {
    prov_group3b4ebRef.current?.setSearchParams();
    prov_group3b4ebRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(prov_group3b4eb) && Object.keys(prov_group3b4eb)?.length>0)
      {
        setprov_group3b4eb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [prov_group3b4ebProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"prov_group"}))
        }}
    >
          {allowedControls.includes("prov_inf") ?<Textprov_inf   /* a6c26 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("prov_inf_des") ?<Textprov_inf_des   /* 4a871 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
