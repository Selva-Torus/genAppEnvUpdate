 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "c3b7a37240a0ccb4b5976ec4f505eb9a"
    );
  ...
  },[access_level5eb9a?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group3a221((prev: any) => ({ ...prev,access_level5eb9a: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group3a221;
      delete temp.access_level;
      delete temp.ACCESS_LEVEL;
      delete temp.access_level5eb9a;
      setaccess_req__group3a221(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,access_level:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group3a221.access_level){
      if(Array.isArray(dfd_accesslevelcombo_v1Props)){
        if(dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)){
          setdropdownValue([dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)?.document_access_scope])
          tempValue=dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)?.document_access_scope
        }else{
          setdropdownValue([access_req__group3a221.access_level])
          tempValue=access_req__group3a221.access_level
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{document_access_scope:access_req__group3a221.access_level}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)?.document_access_scope ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group3a221.access_level)?.document_access_scope
      }else{
        setdropdownValue([access_req__group3a221.access_level])
        tempValue=access_req__group3a221.access_level
      }   
    ...
    if(access_level5eb9a?.trigger == true)
    {
      await handlechange(tempValue)
      setaccess_level5eb9a((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group3a221.access_level, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_accesslevelcombo_v1Props) && dfd_accesslevelcombo_v1Props?.length == 1){
    // setaccess_req__group3a221((pre:any)=>({...pre,access_level:dfd_accesslevelcombo_v1Props[0]?.access_level}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group3a221((prev: any) => ({ ...prev, access_level: getMapperDetailsBindValues[value],ACCESS_LEVEL: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group3a221((prev: any) => ({ ...prev, access_level: '', access_level5eb9a: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,access_level:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group3a221?.access_level == "" || access_req__group3a221?.access_level == undefined){
      access_req__group3a221.access_level = "";
      const validate:any = v.safeParse(schema, access_req__group3a221?.access_level);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,access_level:"invalid"}}));
        }
    }else if(access_req__group3a221?.access_level !== ""){
    const validate:any = v.safeParse(schema, access_req__group3a221?.access_level);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,access_level:"invalid"}}));
  ...
  useEffect(() => { access_req__group3a221Ref.current = access_req__group3a221; }, [access_req__group3a221]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "c3b7a37240a0ccb4b5976ec4f505eb9a") {
        handleClick(access_req__group3a221Ref?.current?.access_level5eb9a?access_req__group3a221Ref?.current?.access_level5eb9a:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "c3b7a37240a0ccb4b5976ec4f505eb9a");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group3a221((pre:any)=>({...pre,access_level:""}))
    else
      setInitialCount(1)
  },[access_level5eb9a?.refresh])
  

  if (access_level5eb9a?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {access_level5eb9a?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group3a221?.access_level5eb9a ? [access_req__group3a221?.access_level5eb9a] : (access_req__group3a221?.access_level ? dropdownValue : [])}
        validationState={validate?.newAccessRequest_v1?.access_level ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
