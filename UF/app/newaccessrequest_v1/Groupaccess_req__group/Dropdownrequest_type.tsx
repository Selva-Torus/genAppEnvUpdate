 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "720cadfd3884a71ab1cb593cf67ade40"
    );
  ...
  },[request_typeade40?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group3a221((prev: any) => ({ ...prev,request_typeade40: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group3a221;
      delete temp.request_type;
      delete temp.REQUEST_TYPE;
      delete temp.request_typeade40;
      setaccess_req__group3a221(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group3a221.request_type){
      if(Array.isArray(dfd_requesttypecombo_v1Props)){
        if(dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)){
          setdropdownValue([dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)?.emp_request_type])
          tempValue=dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)?.emp_request_type
        }else{
          setdropdownValue([access_req__group3a221.request_type])
          tempValue=access_req__group3a221.request_type
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_request_type:access_req__group3a221.request_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)?.emp_request_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__group3a221.request_type)?.emp_request_type
      }else{
        setdropdownValue([access_req__group3a221.request_type])
        tempValue=access_req__group3a221.request_type
      }   
    ...
    if(request_typeade40?.trigger == true)
    {
      await handlechange(tempValue)
      setrequest_typeade40((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group3a221.request_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_requesttypecombo_v1Props) && dfd_requesttypecombo_v1Props?.length == 1){
    // setaccess_req__group3a221((pre:any)=>({...pre,request_type:dfd_requesttypecombo_v1Props[0]?.request_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group3a221((prev: any) => ({ ...prev, request_type: getMapperDetailsBindValues[value],REQUEST_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group3a221((prev: any) => ({ ...prev, request_type: '', request_typeade40: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_type:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group3a221?.request_type == "" || access_req__group3a221?.request_type == undefined){
      access_req__group3a221.request_type = "";
      const validate:any = v.safeParse(schema, access_req__group3a221?.request_type);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_type:"invalid"}}));
        }
    }else if(access_req__group3a221?.request_type !== ""){
    const validate:any = v.safeParse(schema, access_req__group3a221?.request_type);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_type:"invalid"}}));
  ...
  useEffect(() => { access_req__group3a221Ref.current = access_req__group3a221; }, [access_req__group3a221]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "720cadfd3884a71ab1cb593cf67ade40") {
        handleClick(access_req__group3a221Ref?.current?.request_typeade40?access_req__group3a221Ref?.current?.request_typeade40:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "720cadfd3884a71ab1cb593cf67ade40");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group3a221((pre:any)=>({...pre,request_type:""}))
    else
      setInitialCount(1)
  },[request_typeade40?.refresh])
  

  if (request_typeade40?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {request_typeade40?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group3a221?.request_typeade40 ? [access_req__group3a221?.request_typeade40] : (access_req__group3a221?.request_type ? dropdownValue : [])}
        validationState={validate?.newAccessRequest_v1?.request_type ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
