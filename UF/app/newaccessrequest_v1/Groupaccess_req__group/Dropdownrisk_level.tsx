 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "206a947141dc9bd880cb19ea0bacb9f7"
    );
  ...
  },[risk_levelcb9f7?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group3a221((prev: any) => ({ ...prev,risk_levelcb9f7: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group3a221;
      delete temp.risk_level;
      delete temp.RISK_LEVEL;
      delete temp.risk_levelcb9f7;
      setaccess_req__group3a221(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,risk_level:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group3a221.risk_level){
      if(Array.isArray(dfd_risklevelcombo_v1Props)){
        if(dfd_risklevelcombo_v1Props?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)){
          setdropdownValue([dfd_risklevelcombo_v1Props?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)?.risk_level])
          tempValue=dfd_risklevelcombo_v1Props?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)?.risk_level
        }else{
          setdropdownValue([access_req__group3a221.risk_level])
          tempValue=access_req__group3a221.risk_level
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{risk_level:access_req__group3a221.risk_level}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)?.risk_level ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.risk_level === access_req__group3a221.risk_level)?.risk_level
      }else{
        setdropdownValue([access_req__group3a221.risk_level])
        tempValue=access_req__group3a221.risk_level
      }   
    ...
    if(risk_levelcb9f7?.trigger == true)
    {
      await handlechange(tempValue)
      setrisk_levelcb9f7((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group3a221.risk_level, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_risklevelcombo_v1Props) && dfd_risklevelcombo_v1Props?.length == 1){
    // setaccess_req__group3a221((pre:any)=>({...pre,risk_level:dfd_risklevelcombo_v1Props[0]?.risk_level}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group3a221((prev: any) => ({ ...prev, risk_level: getMapperDetailsBindValues[value],RISK_LEVEL: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group3a221((prev: any) => ({ ...prev, risk_level: '', risk_levelcb9f7: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,risk_level:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group3a221?.risk_level == "" || access_req__group3a221?.risk_level == undefined){
      access_req__group3a221.risk_level = "";
      const validate:any = v.safeParse(schema, access_req__group3a221?.risk_level);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,risk_level:"invalid"}}));
        }
    }else if(access_req__group3a221?.risk_level !== ""){
    const validate:any = v.safeParse(schema, access_req__group3a221?.risk_level);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,risk_level:"invalid"}}));
  ...
  useEffect(() => { access_req__group3a221Ref.current = access_req__group3a221; }, [access_req__group3a221]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "206a947141dc9bd880cb19ea0bacb9f7") {
        handleClick(access_req__group3a221Ref?.current?.risk_levelcb9f7?access_req__group3a221Ref?.current?.risk_levelcb9f7:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "206a947141dc9bd880cb19ea0bacb9f7");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group3a221((pre:any)=>({...pre,risk_level:""}))
    else
      setInitialCount(1)
  },[risk_levelcb9f7?.refresh])
  

  if (risk_levelcb9f7?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {risk_levelcb9f7?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group3a221?.risk_levelcb9f7 ? [access_req__group3a221?.risk_levelcb9f7] : (access_req__group3a221?.risk_level ? dropdownValue : [])}
        validationState={validate?.newAccessRequest_v1?.risk_level ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
