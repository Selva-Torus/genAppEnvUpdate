...
const TextInputsystem_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addAccessRequestModify:AFVK:v1|21d269e30ee840848c3f8d33112a6ffd|properties.system_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|728f2a80659a170f2ab5070e9423a221|8fbe46d3f82be33381e49b11c8655638"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'system_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,system_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group3a221((prev: any) => ({ ...prev, system_name: +e.target.value }));
    }
    else{
    setaccess_req__group3a221((prev: any) => ({ ...prev, system_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "8fbe46d3f82be33381e49b11c8655638"
      );
      ...
      //     controlId: "8fbe46d3f82be33381e49b11c8655638",
      //     isTable: false,
      //     from:"TextInputsystem_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'system_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'system_name',type:'text'};
      //   type={
      //     name:'system_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.system_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.system_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.system_name.type
      //   }
      ...
      //   type={
      //     name:'system_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.system_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.system_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.system_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "8fbe46d3f82be33381e49b11c8655638") {
        handleChange({target:{value:access_req__group3a221Ref?.current?.system_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "8fbe46d3f82be33381e49b11c8655638") {
        handleBlur({target:{value:access_req__group3a221Ref?.current?.system_name||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setaccess_req__group3a221((pre:any)=>({...pre,system_name:dfd_addaccessrequestmodify_v1Props.data[0]?.system_name}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (system_name55638?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group3a221?.system_name||""}
         disabled= {system_name55638?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. CI/CD Pipeline (Jenkins)'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="System Name"
      errorMessage={error}
        validationState={validate?.newAccessRequest_v1?.system_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputsystem_name
