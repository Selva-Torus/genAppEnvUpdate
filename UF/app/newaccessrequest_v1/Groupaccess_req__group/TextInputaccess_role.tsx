...
const TextInputaccess_role = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addAccessRequestModify:AFVK:v1|21d269e30ee840848c3f8d33112a6ffd|properties.access_role"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1|728f2a80659a170f2ab5070e9423a221|bb67a4a2fef8b11bd3ff7b4c51953ace"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'access_role',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,access_role:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group3a221((prev: any) => ({ ...prev, access_role: +e.target.value }));
    }
    else{
    setaccess_req__group3a221((prev: any) => ({ ...prev, access_role: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "bb67a4a2fef8b11bd3ff7b4c51953ace"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequest:AFVK:v1",
      //     componentId: "728f2a80659a170f2ab5070e9423a221",
      //     controlId: "bb67a4a2fef8b11bd3ff7b4c51953ace",
      //     isTable: false,
      //     from:"TextInputaccess_role",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'access_role', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'access_role',type:'text'};
      //   type={
      //     name:'access_role',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.access_role.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.access_role.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.access_role.type
      //   }
      ...
      //   type={
      //     name:'access_role',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.access_role.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.access_role.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.access_role.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "bb67a4a2fef8b11bd3ff7b4c51953ace") {
        handleChange({target:{value:access_req__group3a221Ref?.current?.access_role||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "bb67a4a2fef8b11bd3ff7b4c51953ace") {
        handleBlur({target:{value:access_req__group3a221Ref?.current?.access_role||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setaccess_req__group3a221((pre:any)=>({...pre,access_role:dfd_addaccessrequestmodify_v1Props.data[0]?.access_role}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (access_role53ace?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group3a221?.access_role||""}
         disabled= {access_role53ace?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. Developer'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Access Role"
      errorMessage={error}
        validationState={validate?.newAccessRequest_v1?.access_role ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputaccess_role
