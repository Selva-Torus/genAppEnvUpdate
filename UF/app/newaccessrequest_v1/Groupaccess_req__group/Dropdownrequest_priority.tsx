 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "66a078faababa58737f0f9b6a8a55c8e"
    );
  ...
  },[request_priority55c8e?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group3a221((prev: any) => ({ ...prev,request_priority55c8e: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group3a221;
      delete temp.request_priority;
      delete temp.REQUEST_PRIORITY;
      delete temp.request_priority55c8e;
      setaccess_req__group3a221(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_priority:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group3a221.request_priority){
      if(Array.isArray(dfd_requestprioritycombo_v1Props)){
        if(dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)){
          setdropdownValue([dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)?.emp_request_priority])
          tempValue=dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)?.emp_request_priority
        }else{
          setdropdownValue([access_req__group3a221.request_priority])
          tempValue=access_req__group3a221.request_priority
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_request_priority:access_req__group3a221.request_priority}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)?.emp_request_priority ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group3a221.request_priority)?.emp_request_priority
      }else{
        setdropdownValue([access_req__group3a221.request_priority])
        tempValue=access_req__group3a221.request_priority
      }   
    ...
    if(request_priority55c8e?.trigger == true)
    {
      await handlechange(tempValue)
      setrequest_priority55c8e((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group3a221.request_priority, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_requestprioritycombo_v1Props) && dfd_requestprioritycombo_v1Props?.length == 1){
    // setaccess_req__group3a221((pre:any)=>({...pre,request_priority:dfd_requestprioritycombo_v1Props[0]?.request_priority}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group3a221((prev: any) => ({ ...prev, request_priority: getMapperDetailsBindValues[value],REQUEST_PRIORITY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group3a221((prev: any) => ({ ...prev, request_priority: '', request_priority55c8e: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_priority:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group3a221?.request_priority == "" || access_req__group3a221?.request_priority == undefined){
      access_req__group3a221.request_priority = "";
      const validate:any = v.safeParse(schema, access_req__group3a221?.request_priority);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_priority:"invalid"}}));
        }
    }else if(access_req__group3a221?.request_priority !== ""){
    const validate:any = v.safeParse(schema, access_req__group3a221?.request_priority);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequest_v1:{...pre?.newAccessRequest_v1,request_priority:"invalid"}}));
  ...
  useEffect(() => { access_req__group3a221Ref.current = access_req__group3a221; }, [access_req__group3a221]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "66a078faababa58737f0f9b6a8a55c8e") {
        handleClick(access_req__group3a221Ref?.current?.request_priority55c8e?access_req__group3a221Ref?.current?.request_priority55c8e:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "66a078faababa58737f0f9b6a8a55c8e");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group3a221((pre:any)=>({...pre,request_priority:""}))
    else
      setInitialCount(1)
  },[request_priority55c8e?.refresh])
  

  if (request_priority55c8e?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {request_priority55c8e?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group3a221?.request_priority55c8e ? [access_req__group3a221?.request_priority55c8e] : (access_req__group3a221?.request_priority ? dropdownValue : [])}
        validationState={validate?.newAccessRequest_v1?.request_priority ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
