...
const Groupaudit_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1a6efcedf7893269b3b06ce51d4c16c3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaudit_groupc16c3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("audit_info")){
        setaudit_info99d74((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_info99d74?.isDisabled==null)
      {
        setaudit_info99d74((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_inf_textarea")){
        setaudit_inf_textarea0e485((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_inf_textarea0e485?.isDisabled==null)
      {
        setaudit_inf_textarea0e485((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const audit_groupc16c3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    audit_groupc16c3Ref.current?.setSearchParams();
    audit_groupc16c3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(audit_groupc16c3) && Object.keys(audit_groupc16c3)?.length>0)
      {
        setaudit_groupc16c3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [audit_groupc16c3Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"audit_group"}))
        }}
    >
          {allowedControls.includes("audit_info") ?<Textaudit_info   /* 99d74 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("audit_inf_textarea") ?<TextAreaaudit_inf_textarea   /* 0e485 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
