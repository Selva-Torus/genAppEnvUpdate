...
const Grouptable = dynamic(() => import("./Grouptable/Grouptable"), { ssr: false });


export default function PageCheckerV1() {
  ...
  const screenName:string = "checker";
  ...
  const {checker_v1Props, setchecker_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function userdfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(checker_v1Props.length > 0){
          for(let i=0;i< checker_v1Props.length;i++){
            if(checker_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TGW01:AFGK:TGW004:AFK:userdfd:AFVK:v1"){
              // delete checker_v1Props[i].DFDkey;
              let temp=structuredClone(checker_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setchecker_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setchecker_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(checker_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(checker_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setchecker_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
