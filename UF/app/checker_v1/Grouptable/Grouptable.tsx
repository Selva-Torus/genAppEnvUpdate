...
const Grouptable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c090d85b024a4ed38d2fdfe1f67cb5d6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settablecb5d6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("user_id")){
        setuser_id8eaea({...user_id8eaea,isDisabled:true});

    }else
    {
      if(user_id8eaea?.isDisabled==null)
      {
        setuser_id8eaea({...user_id8eaea,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("name")){
        setname36041({...name36041,isDisabled:true});

    }else
    {
      if(name36041?.isDisabled==null)
      {
        setname36041({...name36041,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const tablecb5d6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tablecb5d6Ref.current?.setSearchParams();
    tablecb5d6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tablecb5d6) && Object.keys(tablecb5d6)?.length>0)
      {
        settablecb5d6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tablecb5d6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setchecker_v1((pre:any)=>({...pre,_selectedGroup_:"table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tabletable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={tablecb5d6Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
