...
let defaultColumns:any = [
  {
    "id": "user_id",
    "nodeid": "6e1221cb65664327bc9ec489fd48eaea",
    "name": "user_id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "name",
    "nodeid": "e54ba4ea82104f39a1da7e162d036041",
    "name": "name",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {user_id8eaea, setuser_id8eaea}= useContext(TotalContext) as TotalContextProps  
  const {name36041, setname36041}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "c090d85b024a4ed38d2fdfe1f67cb5d6",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        //bindtran
        keys={}       
        Object.keys(group7fd3f).map((item: any) => {
          keys[item] = null
        }) 
        setgroup7fd3f(keys)
        keys={}       
        Object.keys(group7fd3fProps?.presetValues).map((item: any) => {
          keys[item] = null
        }) 
        setgroup7fd3fProps((pre:any)=>({...pre.presetValues,...keys}))
        settablecb5d6Props((pre:any)=>({...pre, selectedIds:[]}))
                  //bindtran
        setgroup7fd3f({})
        setgroup7fd3fProps({...group7fd3fProps,presetValues:{}})
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      tablecb5d6.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.user_id)
      ...
      //////////
                  //bindtran
          setgroup7fd3f((pre:any)=>({...pre, ...flattenKeepInner(allData[ids?.at(-1)])}))
          setgroup7fd3fProps({...group7fd3fProps,presetValues:allData[ids?.at(-1)]})
      //////////
      settablecb5d6Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      settablecb5d6Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  //bindtran
        setgroup7fd3f((pre:any)=>({...pre, ...flattenKeepInner(allData[ids.at(-1)])}))
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['user_id'] = user_id8eaea,
        codeStates['setuser_id'] = setuser_id8eaea,
        codeStates['name'] = name36041,
        codeStates['setname'] = setname36041,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settablecb5d6Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:checker:AFVK:v1:UO', 
                nodeId: 'c090d85b024a4ed38d2fdfe1f67cb5d6',
                elementId: 'c090d85b024a4ed38d2fdfe1f67cb5d6'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( tablecb5d6Props?.presetValues&&Object.keys(tablecb5d6Props?.presetValues).length > 0) {
          filtertedData = [tablecb5d6Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settablecb5d6(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (tablecb5d6Props.filterInitalLoad) return;

    const filterControllers = tablecb5d6Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['tablecb5d6']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['tablecb5d6']).filter((k: string) => jsonEntry['tablecb5d6'][k] === false)
      : [];
    ...
    settablecb5d6Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(tablecb5d6Props.filterProps);

  }, [
    tablecb5d6Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    tablecb5d6Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(tablecb5d6Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(tablecb5d6Props.searchFilter) || {};
  ...
  useEffect(() => {
    if (tablecb5d6Props?.clearData === true) {
      settablecb5d6([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      settablecb5d6Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [tablecb5d6Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [tablecb5d6Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = tablecb5d6Props?.selectedIds[tablecb5d6Props?.selectedIds?.length-1]
    if(Array.isArray(tablecb5d6) && tablecb5d6.length>0)
    {
      let data = tablecb5d6[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "user_id",
        value: data["user_id"],
  ...
  useEffect(() => {
    if (tablecb5d6Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (tablecb5d6Props?.selectedIds?.length == 0){
      handleOnRowClick({},tablecb5d6Props?.selectedIds)
    }
  }, [tablecb5d6Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(tablecb5d6?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&tablecb5d6?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](tablecb5d6?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tablecb5d6?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&tablecb5d6?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](tablecb5d6?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tablecb5d6?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"c090d85b024a4ed38d2fdfe1f67cb5d6"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableSelection={true}
              selectedIds={tablecb5d6Props?.selectedIds}  
