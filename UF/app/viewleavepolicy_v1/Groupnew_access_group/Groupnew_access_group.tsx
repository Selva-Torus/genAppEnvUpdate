...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bcc9d860deb49fbfb4819f688c4193d2");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_group193d2Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__groupc57b7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__groupc57b7?.isDisabled==null)
      {
        setaccess_req__groupc57b7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_inf_group")){
        setapp_inf_group60e94((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_inf_group60e94?.isDisabled==null)
      {
        setapp_inf_group60e94((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_group")){
        setvalid_group60f4e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_group60f4e?.isDisabled==null)
      {
        setvalid_group60f4e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("business_just__group")){
        setbusiness_just__group4dcdb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_just__group4dcdb?.isDisabled==null)
      {
        setbusiness_just__group4dcdb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("provision_group")){
        setprovision_group68072((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(provision_group68072?.isDisabled==null)
      {
        setprovision_group68072((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("policy_id")){
        setpolicy_id76189((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(policy_id76189?.isDisabled==null)
      {
        setpolicy_id76189((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['policy_id'] = policy_id76189,
        codeStates['setpolicy_id'] = setpolicy_id76189,
  ...
  const new_access_group193d2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_group193d2Ref.current?.setSearchParams();
    new_access_group193d2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_group193d2) && Object.keys(new_access_group193d2)?.length>0)
      {
        setnew_access_group193d2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_group193d2Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("provision_group")  &&<Groupprovision_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("policy_id") ?<Textpolicy_id   /* 76189 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
