...
import Textleave_rules  from "./Textleave_rules";
...
const Groupleave_rule_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {leave_rules5433b, setleave_rules5433b}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "03c30074ba0df031ec861c30b531e665");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setleave_rule_group1e665Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_rules")){
        setleave_rules5433b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_rules5433b?.isDisabled==null)
      {
        setleave_rules5433b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['leave_rules'] = leave_rules5433b,
        codeStates['setleave_rules'] = setleave_rules5433b,
  ...
  const leave_rule_group1e665Ref = useRef<any>(null);
  const handleClearSearch = () => {
    leave_rule_group1e665Ref.current?.setSearchParams();
    leave_rule_group1e665Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(leave_rule_group1e665) && Object.keys(leave_rule_group1e665)?.length>0)
      {
        setleave_rule_group1e665({})
      }
    }else 
      prevRefreshRef.current= true
  }, [leave_rule_group1e665Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"leave_rule_group"}))
        }}
    >
          {allowedControls.includes("leave_rules") ?<Textleave_rules   /* 5433b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupleave_rule_group
