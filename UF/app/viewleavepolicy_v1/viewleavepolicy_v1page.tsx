...
export default function PageViewleavepolicyV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function addleavepolicymodify_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewleavepolicy_v1Props.length > 0){
          for(let i=0;i< viewleavepolicy_v1Props.length;i++){
            if(viewleavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addLeavePolicyModify:AFVK:v1"){
              // delete viewleavepolicy_v1Props[i].DFDkey;
              let temp=structuredClone(viewleavepolicy_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function leavetypecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewleavepolicy_v1Props.length > 0){
          for(let i=0;i< viewleavepolicy_v1Props.length;i++){
            if(viewleavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leaveTypeCombo:AFVK:v1"){
              // delete viewleavepolicy_v1Props[i].DFDkey;
              let temp=structuredClone(viewleavepolicy_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function accrualfrequencycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewleavepolicy_v1Props.length > 0){
          for(let i=0;i< viewleavepolicy_v1Props.length;i++){
            if(viewleavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:accrualFrequencyCombo:AFVK:v1"){
              // delete viewleavepolicy_v1Props[i].DFDkey;
              let temp=structuredClone(viewleavepolicy_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function gendercombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewleavepolicy_v1Props.length > 0){
          for(let i=0;i< viewleavepolicy_v1Props.length;i++){
            if(viewleavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:genderCombo:AFVK:v1"){
              // delete viewleavepolicy_v1Props[i].DFDkey;
              let temp=structuredClone(viewleavepolicy_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function statecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewleavepolicy_v1Props.length > 0){
          for(let i=0;i< viewleavepolicy_v1Props.length;i++){
            if(viewleavepolicy_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:stateCombo:AFVK:v1"){
              // delete viewleavepolicy_v1Props[i].DFDkey;
              let temp=structuredClone(viewleavepolicy_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setviewleavepolicy_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setviewleavepolicy_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(viewleavepolicy_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(viewleavepolicy_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setviewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
