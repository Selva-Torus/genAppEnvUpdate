...
const TextInputcarry_forward_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addLeavePolicyModify:AFVK:v1|4e279f3010e947cda8cc341d6c0ac371|properties.carry_forward_days"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewLeavePolicy:AFVK:v1|844ad10a3f3090d34b205744d9d60f4e|0d571d0b4a16a8af2c1a022ceb7069b8"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'carry_forward_days',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,carry_forward_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalid_group60f4e((prev: any) => ({ ...prev, carry_forward_days: +e.target.value }));
    }
    else{
    setvalid_group60f4e((prev: any) => ({ ...prev, carry_forward_days: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "0d571d0b4a16a8af2c1a022ceb7069b8"
      );
      ...
      //     controlId: "0d571d0b4a16a8af2c1a022ceb7069b8",
      //     isTable: false,
      //     from:"TextInputcarry_forward_days",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'carry_forward_days', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'carry_forward_days',type:'text'};
      //   type={
      //     name:'carry_forward_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_days.type
      //   }
      ...
      //   type={
      //     name:'carry_forward_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_days.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "0d571d0b4a16a8af2c1a022ceb7069b8") {
        handleChange({target:{value:valid_group60f4eRef?.current?.carry_forward_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "0d571d0b4a16a8af2c1a022ceb7069b8") {
        handleBlur({target:{value:valid_group60f4eRef?.current?.carry_forward_days||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setvalid_group60f4e((pre:any)=>({...pre,carry_forward_days:dfd_addleavepolicymodify_v1Props.data[0]?.carry_forward_days}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (carry_forward_days069b8?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 16`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={valid_group60f4e?.carry_forward_days||""}
         disabled= {carry_forward_days069b8?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Carry Forward Days"
      errorMessage={error}
        validationState={validate?.viewLeavePolicy_v1?.carry_forward_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcarry_forward_days
