...
const TextInputmax_consecutive_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addLeavePolicyModify:AFVK:v1|4e279f3010e947cda8cc341d6c0ac371|properties.max_consecutive_days"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewLeavePolicy:AFVK:v1|844ad10a3f3090d34b205744d9d60f4e|9548e87c0375cb295622375c35447fe8"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'max_consecutive_days',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,max_consecutive_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalid_group60f4e((prev: any) => ({ ...prev, max_consecutive_days: +e.target.value }));
    }
    else{
    setvalid_group60f4e((prev: any) => ({ ...prev, max_consecutive_days: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9548e87c0375cb295622375c35447fe8"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewLeavePolicy:AFVK:v1",
      //     componentId: "844ad10a3f3090d34b205744d9d60f4e",
      //     controlId: "9548e87c0375cb295622375c35447fe8",
      //     isTable: false,
      //     from:"TextInputmax_consecutive_days",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'max_consecutive_days', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'max_consecutive_days',type:'text'};
      //   type={
      //     name:'max_consecutive_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.max_consecutive_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.max_consecutive_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.max_consecutive_days.type
      //   }
      ...
      //   type={
      //     name:'max_consecutive_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.max_consecutive_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.max_consecutive_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.max_consecutive_days.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9548e87c0375cb295622375c35447fe8") {
        handleChange({target:{value:valid_group60f4eRef?.current?.max_consecutive_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9548e87c0375cb295622375c35447fe8") {
        handleBlur({target:{value:valid_group60f4eRef?.current?.max_consecutive_days||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setvalid_group60f4e((pre:any)=>({...pre,max_consecutive_days:dfd_addleavepolicymodify_v1Props.data[0]?.max_consecutive_days}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (max_consecutive_days47fe8?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={valid_group60f4e?.max_consecutive_days||""}
         disabled= {max_consecutive_days47fe8?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Max Consecutive Days"
      errorMessage={error}
        validationState={validate?.viewLeavePolicy_v1?.max_consecutive_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmax_consecutive_days
