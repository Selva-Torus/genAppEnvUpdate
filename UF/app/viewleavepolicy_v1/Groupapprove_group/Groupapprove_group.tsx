...
import Textadv_conf  from "./Textadv_conf";
...
const Groupapprove_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {adv_conf3740b, setadv_conf3740b}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1113cff415c1bdcccb8cc9f8d2827e47");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapprove_group27e47Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("adv_conf")){
        setadv_conf3740b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(adv_conf3740b?.isDisabled==null)
      {
        setadv_conf3740b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['adv_conf'] = adv_conf3740b,
        codeStates['setadv_conf'] = setadv_conf3740b,
  ...
  const approve_group27e47Ref = useRef<any>(null);
  const handleClearSearch = () => {
    approve_group27e47Ref.current?.setSearchParams();
    approve_group27e47Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approve_group27e47) && Object.keys(approve_group27e47)?.length>0)
      {
        setapprove_group27e47({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approve_group27e47Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"approve_group"}))
        }}
    >
          {allowedControls.includes("adv_conf") ?<Textadv_conf   /* 3740b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupapprove_group
