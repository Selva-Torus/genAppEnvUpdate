...
const Groupaccess_req__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c2ec4bac028db14075caf13cd28c57b7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaccess_req__groupc57b7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("basic_inf")){
        setbasic_infa3474((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(basic_infa3474?.isDisabled==null)
      {
        setbasic_infa3474((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("policy_code")){
        setpolicy_code4f041((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(policy_code4f041?.isDisabled==null)
      {
        setpolicy_code4f041((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("policy_name")){
        setpolicy_name62419((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(policy_name62419?.isDisabled==null)
      {
        setpolicy_name62419((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_type")){
        setleave_typeb4f0b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_typeb4f0b?.isDisabled==null)
      {
        setleave_typeb4f0b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatusbefa6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(statusbefa6?.isDisabled==null)
      {
        setstatusbefa6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const access_req__groupc57b7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    access_req__groupc57b7Ref.current?.setSearchParams();
    access_req__groupc57b7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(access_req__groupc57b7) && Object.keys(access_req__groupc57b7)?.length>0)
      {
        setaccess_req__groupc57b7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [access_req__groupc57b7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"access_req__group"}))
        }}
    >
          {allowedControls.includes("basic_inf") ?<Textbasic_inf   /* a3474 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("policy_code") ?<TextInputpolicy_code   /* 4f041 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("policy_name") ?<TextInputpolicy_name   /* 62419 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("leave_type") ?<Dropdownleave_type   /* b4f0b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("status") ?<Dropdownstatus   /* befa6 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
