...
const TextInputpolicy_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewLeavePolicy:AFVK:v1|c2ec4bac028db14075caf13cd28c57b7|7ddd1ec4567069a166ba25e93a44f041"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,policy_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__groupc57b7((prev: any) => ({ ...prev, policy_code: +e.target.value }));
    }
    else{
    setaccess_req__groupc57b7((prev: any) => ({ ...prev, policy_code: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "7ddd1ec4567069a166ba25e93a44f041"
      );
      ...
      //     controlId: "7ddd1ec4567069a166ba25e93a44f041",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "7ddd1ec4567069a166ba25e93a44f041") {
        handleChange({target:{value:access_req__groupc57b7Ref?.current?.policy_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "7ddd1ec4567069a166ba25e93a44f041") {
        handleBlur({target:{value:access_req__groupc57b7Ref?.current?.policy_code||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setaccess_req__groupc57b7((pre:any)=>({...pre,policy_code:dfd_addleavepolicymodify_v1Props.data[0]?.policy_code}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (policy_code4f041?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__groupc57b7?.policy_code||""}
         disabled= {policy_code4f041?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewLeavePolicy_v1?.policy_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
