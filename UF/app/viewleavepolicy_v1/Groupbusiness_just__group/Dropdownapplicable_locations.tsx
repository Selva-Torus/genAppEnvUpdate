 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "6aa1c1da7b678f3ffdf2795383f81924"
    );
  ...
  },[applicable_locations81924?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbusiness_just__group4dcdb((prev: any) => ({ ...prev,applicable_locations81924: value }))
      setIsRequredData(false)
    }else{
      let temp:any = business_just__group4dcdb;
      delete temp.applicable_locations;
      delete temp.APPLICABLE_LOCATIONS;
      delete temp.applicable_locations81924;
      setbusiness_just__group4dcdb(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,applicable_locations:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(business_just__group4dcdb.applicable_locations){
      if(Array.isArray(dfd_statecombo_v1Props)){
        if(dfd_statecombo_v1Props?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)){
          setdropdownValue([dfd_statecombo_v1Props?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)?.state_name])
          tempValue=dfd_statecombo_v1Props?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)?.state_name
        }else{
          setdropdownValue([business_just__group4dcdb.applicable_locations])
          tempValue=business_just__group4dcdb.applicable_locations
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{state_name:business_just__group4dcdb.applicable_locations}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)?.state_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.state_name === business_just__group4dcdb.applicable_locations)?.state_name
      }else{
        setdropdownValue([business_just__group4dcdb.applicable_locations])
        tempValue=business_just__group4dcdb.applicable_locations
      }   
    ...
    if(applicable_locations81924?.trigger == true)
    {
      await handlechange(tempValue)
      setapplicable_locations81924((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[business_just__group4dcdb.applicable_locations, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_statecombo_v1Props) && dfd_statecombo_v1Props?.length == 1){
    // setbusiness_just__group4dcdb((pre:any)=>({...pre,applicable_locations:dfd_statecombo_v1Props[0]?.applicable_locations}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbusiness_just__group4dcdb((prev: any) => ({ ...prev, applicable_locations: getMapperDetailsBindValues[value],APPLICABLE_LOCATIONS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbusiness_just__group4dcdb((prev: any) => ({ ...prev, applicable_locations: '', applicable_locations81924: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,applicable_locations:undefined}}));
  ...
  useEffect(() => { business_just__group4dcdbRef.current = business_just__group4dcdb; }, [business_just__group4dcdb]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "6aa1c1da7b678f3ffdf2795383f81924") {
        handleClick(business_just__group4dcdbRef?.current?.applicable_locations81924?business_just__group4dcdbRef?.current?.applicable_locations81924:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "6aa1c1da7b678f3ffdf2795383f81924");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbusiness_just__group4dcdb((pre:any)=>({...pre,applicable_locations:""}))
    else
      setInitialCount(1)
  },[applicable_locations81924?.refresh])
  

  if (applicable_locations81924?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {applicable_locations81924?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={business_just__group4dcdb?.applicable_locations81924 ? [business_just__group4dcdb?.applicable_locations81924] : (business_just__group4dcdb?.applicable_locations ? dropdownValue : [])}
        validationState={validate?.viewLeavePolicy_v1?.applicable_locations ? "invalid" : undefined}
        />
    </div>
  );
};
