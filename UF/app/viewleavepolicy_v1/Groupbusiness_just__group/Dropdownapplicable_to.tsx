...
const Dropdownapplicable_to = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(business_just__group4dcdb?.applicable_to=="" || business_just__group4dcdb?.applicable_to==undefined || business_just__group4dcdb?.applicable_to==null ){
    setSelectedItem("");
  }
  },[business_just__group4dcdb?.applicable_to])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "80d7f9a18588f84ae819f7204a74dcdb",
        "900bf5fca8592e271ecfc82fe3fd7cba"
      );
  ...
  },
  {
    ...
    "value": {
      ...
      setbusiness_just__group4dcdb((prev: any) => ({ ...prev, applicable_to: staticTextValue, applicable_tod7cba: value}))
         setIsRequredData(false)
    } else {
       setbusiness_just__group4dcdb((prev: any) => ({ ...prev, applicable_to: '', applicable_tod7cba: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewLeavePolicy_v1:{...pre?.viewLeavePolicy_v1,applicable_to:undefined}}));
  ...
  useEffect(() => { business_just__group4dcdbRef.current = business_just__group4dcdb; }, [business_just__group4dcdb]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "900bf5fca8592e271ecfc82fe3fd7cba") {
        handleClick(business_just__group4dcdbRef?.current?.applicable_tod7cba?business_just__group4dcdbRef?.current?.applicable_tod7cba:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "900bf5fca8592e271ecfc82fe3fd7cba");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbusiness_just__group4dcdb((pre:any)=>({...pre,applicable_to:""}))
    else
      setInitialCount(1)
  },[applicable_tod7cba?.refresh])
  

  if (applicable_tod7cba?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {applicable_tod7cba?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            business_just__group4dcdb?.applicable_tod7cba ? [business_just__group4dcdb?.applicable_tod7cba] :
                business_just__group4dcdb?.applicable_to ? business_just__group4dcdb?.applicable_to : []
            }
        onChange={handleClick} 
        validationState={validate?.viewLeavePolicy_v1?.applicable_to ? "invalid" : undefined}
      /> 
    </div>
  );
};
