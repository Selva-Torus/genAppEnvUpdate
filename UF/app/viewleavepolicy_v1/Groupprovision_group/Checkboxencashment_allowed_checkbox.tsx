...
const Checkboxencashment_allowed_checkbox = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "fc4b0f0a03ab0f28fa005d9538bab660"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[encashment_allowed_checkboxab660?.refresh])

  useEffect(()=>{
    if (provision_group68072?.encashment_allowed === undefined) {
      setprovision_group68072((prev: any) => ({ ...prev, encashment_allowed: false }));
    }
  },[])
  ...
  const handleChange=async(checked:boolean)=>{
    ...
    setprovision_group68072((prev: any) => ({ ...prev, encashment_allowed: checked}));
    }catch (err: any) {
  ...
  if (encashment_allowed_checkboxab660?.isHidden) {
    return <></>;
  }
  return (
    <div 
       style={{gridColumn: `13 / 24`,gridRow: `12 / 18`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Checkbox 
      className=""
      value={provision_group68072?.encashment_allowed||false}
      checked={provision_group68072?.encashment_allowed||false}
      disabled= {encashment_allowed_checkboxab660?.isDisabled ? true : false}
      content = {'Encashment'}
