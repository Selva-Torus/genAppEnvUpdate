...
export default function PageAddemployeejobgradeV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function jobgrade_v1DFD(pagination:any): Promise<void>{
        ...
        if(addemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< addemployeejobgrade_v1Props.length;i++){
            if(addemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobGrade:AFVK:v1"){
              // delete addemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(addemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function currencycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(addemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< addemployeejobgrade_v1Props.length;i++){
            if(addemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:currencyCombo:AFVK:v1"){
              // delete addemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(addemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function appraisalcyclecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(addemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< addemployeejobgrade_v1Props.length;i++){
            if(addemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:appraisalCycleCombo:AFVK:v1"){
              // delete addemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(addemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setaddemployeejobgrade_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaddemployeejobgrade_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(addemployeejobgrade_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(addemployeejobgrade_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setaddemployeejobgrade_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
