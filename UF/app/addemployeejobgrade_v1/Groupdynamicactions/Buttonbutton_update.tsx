 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "9a3b28ff649f1898bc0d6d08dea4ce10"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactions7e8c7Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions7e8c7Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in addemployeejobgrade_v1?.dynamicactions && addemployeejobgrade_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(addemployeejobgrade_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addemployeejobgrade_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_update4ce10") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "9a3b28ff649f1898bc0d6d08dea4ce10") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_update4ce10");
    return () => {
  ...
  useEffect(()=>{
    if (!button_update4ce10?.trigger) return;
      if(button_update4ce10?.trigger){
      setbutton_update4ce10((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_update4ce10?.trigger])

  useEffect(()=>{
    if(button_update4ce10?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_update4ce10?.refresh])
  ...
  async function handleSavece10_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const dynamicactions7e8c7Props = dynamicactions7e8c7PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.addEmployeeJobGrade_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(dynamicactions7e8c7Ref.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobGradeModify:AFVK:v1|b8e3a884453776e3ae295fcb9ff4ceb9"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobGradeModify:AFVK:v1|b8e3a884453776e3ae295fcb9ff4ceb9"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactions7e8c7?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "9a3b28ff649f1898bc0d6d08dea4ce10",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "9a3b28ff649f1898bc0d6d08dea4ce10.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "9a3b28ff649f1898bc0d6d08dea4ce10.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "9a3b28ff649f1898bc0d6d08dea4ce10.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "9a3b28ff649f1898bc0d6d08dea4ce10.1.1.1.1.1",
                  ...
                  "children": [
                    {
                      "id": "ff40997343dd428ae50a99eca449c094.1.1.1.1.1.1",
                      "value": "",
                      "type": "screen",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Job Grade Updated Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addJobGradeModify:AFVK:v1|b8e3a884453776e3ae295fcb9ff4ceb9"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1";
      sourceId+= "|"+"1da03bd8ba298eeb685f249866d7e8c7";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1"+"|"+"1da03bd8ba298eeb685f249866d7e8c7"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions7e8c7?.upId? [dynamicactions7e8c7?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions7e8c7?.upId? [dynamicactions7e8c7?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions7e8c7?.upId? [dynamicactions7e8c7?.upId ] : dynamicactions7e8c7?.trs_process_id? [dynamicactions7e8c7?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions7e8c7Ref.current, ...nullFilter(overall_groupfc238), ...nullFilter(grade_information_groupddd50), ...nullFilter(compensation_benfits_group49b64), ...nullFilter(hr_policies_group0f880) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"1da03bd8ba298eeb685f249866d7e8c7",
              controlId:"9a3b28ff649f1898bc0d6d08dea4ce10"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (dynamicactions7e8c7Props.ssKey !== '' && dynamicactions7e8c7Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = dynamicactions7e8c7Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Job Grade Updated Successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
    settotal_employees_table9c094Props((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions7e8c7((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSavece10_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions7e8c7((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addemployeejobgrade_v1?.dynamicactions?.button_update,dynamicactions7e8c7Props?.dynamicActionRule?.button_update,])

 if (button_update4ce10?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_update4ce10?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Update")}
