...
const TextInputprobation_period_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1|3daad9f45b7c4612a93064fc26a0f880|f1bdd179079d4d12916812241eacf854"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,probation_period_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    sethr_policies_group0f880((prev: any) => ({ ...prev, probation_period_days: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(hr_policies_group0f880?.probation_period_days == "" || hr_policies_group0f880?.probation_period_days == undefined){
      hr_policies_group0f880.probation_period_days = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, hr_policies_group0f880?.probation_period_days);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,probation_period_days:"invalid"}}));
        }
    }else if(hr_policies_group0f880?.probation_period_days !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +hr_policies_group0f880?.probation_period_days);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f1bdd179079d4d12916812241eacf854"
      );
      ...
      //     controlId: "f1bdd179079d4d12916812241eacf854",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!hr_policies_group0f880?.probation_period_days)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "f1bdd179079d4d12916812241eacf854") {
        handleChange({target:{value:hr_policies_group0f880Ref?.current?.probation_period_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f1bdd179079d4d12916812241eacf854") {
        handleBlur({target:{value:hr_policies_group0f880Ref?.current?.probation_period_days||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      sethr_policies_group0f880((pre:any)=>({...pre,probation_period_days:dfd_jobgrade_v1Props.data[0]?.probation_period_days}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (probation_period_dayscf854?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={hr_policies_group0f880?.probation_period_days||""}
         disabled= {probation_period_dayscf854?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobGrade_v1?.probation_period_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
