...
const TextInputmin_salary = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1|d1c13cefdc89a0d6a6ad217393049b64|3532acef14f0503306033fc4e6c807a5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,min_salary:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_group49b64((prev: any) => ({ ...prev, min_salary: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_group49b64?.min_salary == "" || compensation_benfits_group49b64?.min_salary == undefined){
      compensation_benfits_group49b64.min_salary = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_group49b64?.min_salary);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,min_salary:"invalid"}}));
        }
    }else if(compensation_benfits_group49b64?.min_salary !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_group49b64?.min_salary);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3532acef14f0503306033fc4e6c807a5"
      );
      ...
      //     controlId: "3532acef14f0503306033fc4e6c807a5",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_group49b64?.min_salary)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "3532acef14f0503306033fc4e6c807a5") {
        handleChange({target:{value:compensation_benfits_group49b64Ref?.current?.min_salary||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3532acef14f0503306033fc4e6c807a5") {
        handleBlur({target:{value:compensation_benfits_group49b64Ref?.current?.min_salary||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setcompensation_benfits_group49b64((pre:any)=>({...pre,min_salary:dfd_jobgrade_v1Props.data[0]?.min_salary}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (min_salary807a5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_group49b64?.min_salary||""}
         disabled= {min_salary807a5?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobGrade_v1?.min_salary ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
