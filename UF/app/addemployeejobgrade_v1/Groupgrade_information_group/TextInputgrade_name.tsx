...
const TextInputgrade_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1|4991c2d5fafb91b54d151c9dc22ddd50|afdc7ad0802766de5bfdeaea341da657"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,grade_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrade_information_groupddd50((prev: any) => ({ ...prev, grade_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(grade_information_groupddd50?.grade_name == "" || grade_information_groupddd50?.grade_name == undefined){
      grade_information_groupddd50.grade_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, grade_information_groupddd50?.grade_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,grade_name:"invalid"}}));
        }
    }else if(grade_information_groupddd50?.grade_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +grade_information_groupddd50?.grade_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "afdc7ad0802766de5bfdeaea341da657"
      );
      ...
      //     controlId: "afdc7ad0802766de5bfdeaea341da657",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!grade_information_groupddd50?.grade_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "afdc7ad0802766de5bfdeaea341da657") {
        handleChange({target:{value:grade_information_groupddd50Ref?.current?.grade_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "afdc7ad0802766de5bfdeaea341da657") {
        handleBlur({target:{value:grade_information_groupddd50Ref?.current?.grade_name||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setgrade_information_groupddd50((pre:any)=>({...pre,grade_name:dfd_jobgrade_v1Props.data[0]?.grade_name}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (grade_nameda657?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={grade_information_groupddd50?.grade_name||""}
         disabled= {grade_nameda657?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobGrade_v1?.grade_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
