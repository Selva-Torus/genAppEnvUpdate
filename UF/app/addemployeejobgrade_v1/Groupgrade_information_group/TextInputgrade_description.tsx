...
const TextInputgrade_description = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1|4991c2d5fafb91b54d151c9dc22ddd50|52db15472ab148598d93f560237702d3"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,grade_description:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrade_information_groupddd50((prev: any) => ({ ...prev, grade_description: +e.target.value }));
    }
    else{
    setgrade_information_groupddd50((prev: any) => ({ ...prev, grade_description: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "52db15472ab148598d93f560237702d3"
      );
      ...
      //     controlId: "52db15472ab148598d93f560237702d3",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "52db15472ab148598d93f560237702d3") {
        handleChange({target:{value:grade_information_groupddd50Ref?.current?.grade_description||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "52db15472ab148598d93f560237702d3") {
        handleBlur({target:{value:grade_information_groupddd50Ref?.current?.grade_description||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setgrade_information_groupddd50((pre:any)=>({...pre,grade_description:dfd_jobgrade_v1Props.data[0]?.grade_description}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (grade_description702d3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={grade_information_groupddd50?.grade_description||""}
         disabled= {grade_description702d3?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobGrade_v1?.grade_description ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
