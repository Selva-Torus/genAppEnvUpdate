...
const TextInputgrade_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:addEmployeeJobGrade:AFVK:v1|4991c2d5fafb91b54d151c9dc22ddd50|248cb740c90a01934ced03b5c5d96f59"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,grade_level:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrade_information_groupddd50((prev: any) => ({ ...prev, grade_level: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(grade_information_groupddd50?.grade_level == "" || grade_information_groupddd50?.grade_level == undefined){
      grade_information_groupddd50.grade_level = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, grade_information_groupddd50?.grade_level);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,addEmployeeJobGrade_v1:{...pre?.addEmployeeJobGrade_v1,grade_level:"invalid"}}));
        }
    }else if(grade_information_groupddd50?.grade_level !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +grade_information_groupddd50?.grade_level);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "248cb740c90a01934ced03b5c5d96f59"
      );
      ...
      //     controlId: "248cb740c90a01934ced03b5c5d96f59",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!grade_information_groupddd50?.grade_level)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "248cb740c90a01934ced03b5c5d96f59") {
        handleChange({target:{value:grade_information_groupddd50Ref?.current?.grade_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "248cb740c90a01934ced03b5c5d96f59") {
        handleBlur({target:{value:grade_information_groupddd50Ref?.current?.grade_level||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setgrade_information_groupddd50((pre:any)=>({...pre,grade_level:dfd_jobgrade_v1Props.data[0]?.grade_level}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (grade_level96f59?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={grade_information_groupddd50?.grade_level||""}
         disabled= {grade_level96f59?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addEmployeeJobGrade_v1?.grade_level ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
