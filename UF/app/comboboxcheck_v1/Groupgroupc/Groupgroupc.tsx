...
const Groupgroupc = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "89af691d3ef44b43bdff103159e0c048");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroupc0c048Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text2342")){
        settext234245d6a({...text234245d6a,isDisabled:true});

    }else
    {
      if(text234245d6a?.isDisabled==null)
      {
  ...
  const groupc0c048Ref = useRef<any>(null);
  const handleClearSearch = () => {
    groupc0c048Ref.current?.setSearchParams();
    groupc0c048Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupc0c048) && Object.keys(groupc0c048)?.length>0)
      {
        setgroupc0c048({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupc0c048Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setcomboboxcheck_v1((pre:any)=>({...pre,_selectedGroup_:"groupc"}))
        }}
    >
