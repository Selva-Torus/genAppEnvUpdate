...
export default function ComboBoxcomboboxa({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9034c26e35274693b074b63d5902ee09"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    ...
    if (isFromEvent) {
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({country:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.country_code, item])).values()
        );
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    setgroup123488888((pre:any)=>({...pre,comboboxa:data?.country_code}))
    setselectedData(data?.country)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.country_code == data?.country_code && items?.country == data?.country)) || {};  
    try{
  ...
  useEffect(() => { group123488888Ref.current = group123488888; }, [group123488888]);
    ...
    const handler = (id:any) => {
      if (id === "9034c26e35274693b074b63d5902ee09") {
        handleOnUpdate({
          value:selectedData,
          text:group123488888Ref?.current?.comboboxa||""
        });
      }
    };
  ...
  useEffect(()=>{
    if(group123488888?.comboboxa=="")
    {
      setselectedData('')
    }else
    {
      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.country==selectedData))?.country || group123488888?.comboboxa;
      setselectedData(formBindedData)
    }
  },[group123488888?.comboboxa])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {comboboxa2ee09?.isDisabled ? true : false}
        needTooltip={true}  
        tooltipProps={{title:"234234",placement:"top-end"}}
        contentAlign={"left"}
        onBlur={handleBlur}
        isStatic={false}
