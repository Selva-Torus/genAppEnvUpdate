...
const TextInputtextinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,comboboxcheck_v1:{...pre?.comboboxcheck_v1,textinput:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup123488888((prev: any) => ({ ...prev, textinput: +e.target.value }));
    }
    else{
    setgroup123488888((prev: any) => ({ ...prev, textinput: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "94df667eedef45f9a31fd502979aee10"
      );
      ...
      //     controlId: "94df667eedef45f9a31fd502979aee10",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "94df667eedef45f9a31fd502979aee10") {
        handleChange({target:{value:group123488888Ref?.current?.textinput||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "94df667eedef45f9a31fd502979aee10") {
        handleBlur({target:{value:group123488888Ref?.current?.textinput||""}});
      }
    };
  ...
  if (textinputaee10?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 13`,gridRow: `196 / 206`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group123488888?.textinput||""}
         disabled= {textinputaee10?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.comboboxcheck_v1?.textinput ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
