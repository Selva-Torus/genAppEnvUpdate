...
export default function ComboBoxcomboboxb({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e126f0c8edf947329c18f017f0a9056e"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    ...
    if (isFromEvent) {
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({state:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.state_code, item])).values()
        );
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    setgroup123488888((pre:any)=>({...pre,comboboxb:data?.state_code}))
    setselectedData(data?.state)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.state_code == data?.state_code && items?.state == data?.state)) || {};  
    try{
  ...
  useEffect(() => { group123488888Ref.current = group123488888; }, [group123488888]);
    ...
    const handler = (id:any) => {
      if (id === "e126f0c8edf947329c18f017f0a9056e") {
        handleOnUpdate({
          value:selectedData,
          text:group123488888Ref?.current?.comboboxb||""
        });
      }
    };
    ...
    const handleCustomDataFilter=async()=>{
      await delay(500);
      setselectedData("")
      setSearch("")
      setgroup123488888((pre:any)=>({...pre,comboboxb:""}))
      seteventFilterData({[UOmapperData["9034c26e35274693b074b63d5902ee09"]["text"]]:group123488888[UOmapperData["9034c26e35274693b074b63d5902ee09"]["source"]]})
      getDropdownData(10,1,"",true,"",{[UOmapperData["9034c26e35274693b074b63d5902ee09"]["text"]]:group123488888[UOmapperData["9034c26e35274693b074b63d5902ee09"]["source"]]})
      prevRefreshRef.current= true
    }
    //getFormData
    useEffect(()=>{
      handleCustomDataFilter()
      if(!group123488888[UOmapperData["9034c26e35274693b074b63d5902ee09"]["source"]])
        setDynamicDFDData([])
    },[group123488888[UOmapperData["9034c26e35274693b074b63d5902ee09"]["source"]]])

  useEffect(()=>{
    if(group123488888?.comboboxb=="")
    {
      setselectedData('')
    }else
    {
      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.state==selectedData))?.state || group123488888?.comboboxb;
      setselectedData(formBindedData)
    }
  },[group123488888?.comboboxb])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {comboboxb9056e?.isDisabled ? true : false}
        contentAlign={"center"}
        onBlur={handleBlur}
        isStatic={false}
