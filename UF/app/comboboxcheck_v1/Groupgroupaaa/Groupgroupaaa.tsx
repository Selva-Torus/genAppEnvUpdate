...
import Texttext23423  from "./Texttext23423";
...
const Groupgroupaaa = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8697f9b3e58447adb118a72343497733");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroupaaa97733Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text23423")){
        settext23423bb984({...text23423bb984,isDisabled:true});

    }else
    {
      if(text23423bb984?.isDisabled==null)
      {
        settext23423bb984({...text23423bb984,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const groupaaa97733Ref = useRef<any>(null);
  const handleClearSearch = () => {
    groupaaa97733Ref.current?.setSearchParams();
    groupaaa97733Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupaaa97733) && Object.keys(groupaaa97733)?.length>0)
      {
        setgroupaaa97733({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupaaa97733Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setcomboboxcheck_v1((pre:any)=>({...pre,_selectedGroup_:"groupaaa"}))
        }}
    >
          {allowedControls.includes("text23423") ?<Texttext23423   /* bb984 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgroupaaa
