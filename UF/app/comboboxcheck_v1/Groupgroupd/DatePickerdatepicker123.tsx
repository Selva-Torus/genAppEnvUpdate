...
const DatePickerdatepicker123 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,comboboxcheck_v1:{...pre?.comboboxcheck_v1,datepicker123:undefined}}));
  if (!date) {
    setgroupd487a8((prev: any) => ({ ...prev, datepicker123: null }));
    return;
  }
  const now = new Date();
  ...
  setgroupd487a8((prev: any) => ({ ...prev, datepicker123: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "a4fcf000188844c184d176620e8487a8",
        "b51d364eb7144887be5f3efbe26acd53"
      );
...
useEffect(()=>{
  setgroupd487a8Props((pre:any)=>({...pre,validation:true}))
 },[datepicker123acd53?.refresh])
...
return (
  <div 
  style={{gridColumn: `8 / 23`,gridRow: `41 / 51`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={groupd487a8?.datepicker123}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {datepicker123acd53?.isDisabled ? true : false}
      disabled= {datepicker123acd53?.isDisabled ? true : false}
      contentAlign={"center"}
      dateValidation=""
      validationState={validate?.comboboxcheck_v1?.datepicker123 ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerdatepicker123
