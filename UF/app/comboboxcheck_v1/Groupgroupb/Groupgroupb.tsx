...
const Groupgroupb = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d670347ac1ed46919c46dbfee548f3d7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroupb8f3d7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text1231")){
        settext1231c2aa3({...text1231c2aa3,isDisabled:true});

    }else
    {
      if(text1231c2aa3?.isDisabled==null)
      {
  ...
  const groupb8f3d7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    groupb8f3d7Ref.current?.setSearchParams();
    groupb8f3d7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupb8f3d7) && Object.keys(groupb8f3d7)?.length>0)
      {
        setgroupb8f3d7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupb8f3d7Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}  ${comboboxcheck_v1?._selectedGroup_=="groupb" ? 'border-2 border-solid !border-[#ff943d]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setcomboboxcheck_v1((pre:any)=>({...pre,_selectedGroup_:"groupb"}))
        }}
    >
