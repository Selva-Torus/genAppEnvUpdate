...
const Groupgroup1234 = dynamic(() => import("./Groupgroup1234/Groupgroup1234"), { ssr: false });


export default function PageComboboxcheckV1() {
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setcomboboxcheck_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    await country_code_dfd_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'group1234' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckgroup1234(true)
            }
            if(nodes?.groupName == 'state' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckstate(true)
            }
            if(nodes?.groupName == 'groupaaa' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setcomboboxcheck_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(comboboxcheck_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(comboboxcheck_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setcomboboxcheck_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkgroup1234 && initialLoad &&<Groupgroup1234
