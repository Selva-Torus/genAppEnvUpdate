...
const TextInputcategory_total_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:Group_Array_dsd:AFVK:v1|de2bf16ff1b543958a63ec1f5fde63e7|items.properties.employee_detail.properties.total_amount"
      ],
      "targetKey": "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:GroupArray:AFVK:v1|05177fac499640d4bf45a199a95494e0|bc4d3d30a6c44ca798c0adba83c49375"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'total_amount',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,GroupArray_v1:{...pre?.GroupArray_v1,total_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrouparray494e0_3((prev: any) => ({ ...prev, total_amount: +e.target.value }));
    }
    else{
    setgrouparray494e0_3((prev: any) => ({ ...prev, total_amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "bc4d3d30a6c44ca798c0adba83c49375"
      );
      ...
      //     controlId: "bc4d3d30a6c44ca798c0adba83c49375",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "bc4d3d30a6c44ca798c0adba83c49375") {
        handleChange({target:{value:grouparray494e0_3Ref?.current?.total_amount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "bc4d3d30a6c44ca798c0adba83c49375") {
        handleBlur({target:{value:grouparray494e0_3Ref?.current?.total_amount||""}});
      }
    };
  ...
  if(dfd_group_array_dsd_v1Props?.setSearchFilters && dfd_group_array_dsd_v1Props?.data)
  {
    if(Array.isArray(dfd_group_array_dsd_v1Props.data) && dfd_group_array_dsd_v1Props.data.length > 0){
      setgrouparray494e0_3((pre:any)=>({...pre,total_amount:dfd_group_array_dsd_v1Props.data[0]?.total_amount}));
    }
  }
  },[dfd_group_array_dsd_v1Props?.setSearchFilters])
  if (category_total_amount49375?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `10 / 17`,gridRow: `95 / 111`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={grouparray494e0_3?.total_amount||""}
         disabled= {category_total_amount49375?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Amount"
      errorMessage={error}
        validationState={validate?.GroupArray_v1?.total_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
