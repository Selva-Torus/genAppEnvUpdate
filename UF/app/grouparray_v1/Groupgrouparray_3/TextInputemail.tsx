...
const TextInputemail = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:Group_Array_dsd:AFVK:v1|de2bf16ff1b543958a63ec1f5fde63e7|items.properties.employee_detail.properties.email_id"
      ],
      "targetKey": "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:GroupArray:AFVK:v1|05177fac499640d4bf45a199a95494e0|3b8d8479e3734bb7b76d452b62d0c3ca"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'email_id',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,GroupArray_v1:{...pre?.GroupArray_v1,email_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrouparray494e0_3((prev: any) => ({ ...prev, email_id: +e.target.value }));
    }
    else{
    setgrouparray494e0_3((prev: any) => ({ ...prev, email_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3b8d8479e3734bb7b76d452b62d0c3ca"
      );
      ...
      //     controlId: "3b8d8479e3734bb7b76d452b62d0c3ca",
      //     isTable: false,
      //     from:"TextInputemail",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'email_id', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'email_id',type:'text'};
      //   type={
      //     name:'email_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.email_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.email_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.email_id.type
      //   }
      ...
      //   type={
      //     name:'email_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.email_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.email_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.email_id.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const grouparray494e0_3Ref = useRef<any>(grouparray494e0_3);
  useEffect(() => { grouparray494e0_3Ref.current = grouparray494e0_3; }, [grouparray494e0_3]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "3b8d8479e3734bb7b76d452b62d0c3ca") {
        handleChange({target:{value:grouparray494e0_3Ref?.current?.email_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3b8d8479e3734bb7b76d452b62d0c3ca") {
        handleBlur({target:{value:grouparray494e0_3Ref?.current?.email_id||""}});
      }
    };
  ...
  if(dfd_group_array_dsd_v1Props?.setSearchFilters && dfd_group_array_dsd_v1Props?.data)
  {
    if(Array.isArray(dfd_group_array_dsd_v1Props.data) && dfd_group_array_dsd_v1Props.data.length > 0){
      setgrouparray494e0_3((pre:any)=>({...pre,email_id:dfd_group_array_dsd_v1Props.data[0]?.email_id}));
    }
  }
  },[dfd_group_array_dsd_v1Props?.setSearchFilters])
  if (email0c3ca?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `10 / 17`,gridRow: `33 / 50`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={grouparray494e0_3?.email_id||""}
         disabled= {email0c3ca?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Email ID"
      errorMessage={error}
        validationState={validate?.GroupArray_v1?.email_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
