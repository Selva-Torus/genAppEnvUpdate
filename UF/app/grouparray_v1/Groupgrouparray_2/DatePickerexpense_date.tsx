...
const handleUpdate = async(date: any) => {
  ...
  if (!date) {
    setgrouparray494e0_2((prev: any) => ({ ...prev, expense_date: null }));
    return;
  }
  const now = new Date();
  ...
  setgrouparray494e0_2((prev: any) => ({ ...prev, expense_date: isoDate }))
  }catch (err: any) {
...
useEffect(()=>{
  setgrouparray494e0_2Props((pre:any)=>({...pre,validation:true}))
...
return (
  ...
  style={{gridColumn: `10 / 17`,gridRow: `57 / 72`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={grouparray494e0_2?.expense_date}
