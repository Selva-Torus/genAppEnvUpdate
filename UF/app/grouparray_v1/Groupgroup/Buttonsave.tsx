...
const Buttonsave = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "0f8f7fd491ce4714853a3661642445f0"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "0f8f7fd491ce4714853a3661642445f0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "save445f0");
    return () => {
  ...
  useEffect(()=>{
    if (!save445f0?.trigger) return;
      if(save445f0?.trigger){
      setsave445f0((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  async function handleSave45f0_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group84b9cProps = group84b9cPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.GroupArray_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group84b9cRef.current);
  ...
  "id": "0f8f7fd491ce4714853a3661642445f0",
  "type": "button",
  "name": "save",
  "label": "save",
  "sequence": 1,
  "children": [
    {
      "id": "0f8f7fd491ce4714853a3661642445f0.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "0f8f7fd491ce4714853a3661642445f0.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "0f8f7fd491ce4714853a3661642445f0.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
      ...
      let sourceId : string = "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:GroupArray:AFVK:v1";
      sourceId+= "|"+"8a9a1198a9e0460bb61bd440d3784b9c";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:GroupArray:AFVK:v1"+"|"+"8a9a1198a9e0460bb61bd440d3784b9c"+"|"+eventProperty.id;
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group84b9cRef.current, ...nullFilter(grouparray494e0) });
    let reworkKeys:any[]=[];
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"8a9a1198a9e0460bb61bd440d3784b9c",
              controlId:"0f8f7fd491ce4714853a3661642445f0"
            };
            if (encryptionFlagCont) {
        ...
        if(group84b9c?.upId){
          te_saveBody['upId']= group84b9c?.upId;
        }
        if(group84b9c?.upid){
          te_saveBody['upId']= group84b9c?.upid;
        }
        ...
        if (group84b9cProps.ssKey !== '' && group84b9cProps.ssKey !== undefined) {
          te_saveBody["ssKey"] = group84b9cProps.ssKey;
    ...
    toast('Data saved successfully', 'success',true,'');
      ...
      if(Array.isArray(group84b9c)){
        setRefetch((pre: any) => !pre)    
        // needClearValue
        let keys: any = {};
        group84b9c.map((item: any) => {
          keys[item] = '';
        })
        setgroup84b9c(keys);
        ...
        // needClearValue
        let keys: any = {};
        Object.keys(group84b9c).map((item: any) => {
          keys[item] = '';
        })
        setgroup84b9c(keys);
      }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{
      setIsProcessing(true);
        setgroup84b9c((prev: any) => ({ ...prev, save: true }));
        //onClick

    //saveHandler
    await handleSave45f0_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup84b9c((prev: any) => ({ ...prev, save: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (save445f0?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `12 / 15`,gridRow: `201 / 222`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className=""
          ...
          disabled= {save445f0?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Save")}
