...
import Buttonadd_asset  from "./Buttonadd_asset";
...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fdf4dead03104e5b91db9e096191077f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions1077fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel407b1({...cancel407b1,isDisabled:true});

    }else
    {
      if(cancel407b1?.isDisabled==null)
      {
        setcancel407b1({...cancel407b1,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_update10522({...button_update10522,isDisabled:true});

    }else
    {
      if(button_update10522?.isDisabled==null)
      {
        setbutton_update10522({...button_update10522,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_asset")){
        setadd_asset1b88e({...add_asset1b88e,isDisabled:true});

    }else
    {
      if(add_asset1b88e?.isDisabled==null)
      {
        setadd_asset1b88e({...add_asset1b88e,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions1077fRef = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions1077fRef.current?.setSearchParams();
    dynamicactions1077fRef.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions1077fProps((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions1077fProps((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions1077f) && Object.keys(dynamicactions1077f)?.length>0)
      {
        setdynamicactions1077f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions1077fProps?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {        ((ruleData?.length>0 && "add_asset" in ButtonGoRuleData)?ButtonGoRuleData["add_asset"]:true) && 
          allowedControls.includes("add_asset")  ?            <Buttonadd_asset tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}
