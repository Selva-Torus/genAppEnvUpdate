...
const TextInputasset_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.asset_code"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|660280a194cb44e488d6a1381cedeeeb|75807d9a874444cb8fc70346721aa68d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupdeeeb((prev: any) => ({ ...prev, asset_code: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(asset_info_groupdeeeb?.asset_code == "" || asset_info_groupdeeeb?.asset_code == undefined){
      asset_info_groupdeeeb.asset_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, asset_info_groupdeeeb?.asset_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_code:"invalid"}}));
        }
    }else if(asset_info_groupdeeeb?.asset_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +asset_info_groupdeeeb?.asset_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "75807d9a874444cb8fc70346721aa68d"
      );
      ...
      //     controlId: "75807d9a874444cb8fc70346721aa68d",
      //     isTable: false,
      //     from:"TextInputasset_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'asset_code', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'asset_code',type:'text'};
      //   type={
      //     name:'asset_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_code.type
      //   }
      ...
      //   type={
      //     name:'asset_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!asset_info_groupdeeeb?.asset_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "75807d9a874444cb8fc70346721aa68d") {
        handleChange({target:{value:asset_info_groupdeeebRef?.current?.asset_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "75807d9a874444cb8fc70346721aa68d") {
        handleBlur({target:{value:asset_info_groupdeeebRef?.current?.asset_code||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupdeeeb((pre:any)=>({...pre,asset_code:dfd_assets_v1Props.data[0]?.asset_code}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_codeaa68d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupdeeeb?.asset_code||""}
         disabled= {asset_codeaa68d?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.asset_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputasset_code
