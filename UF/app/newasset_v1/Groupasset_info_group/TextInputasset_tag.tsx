...
const TextInputasset_tag = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|660280a194cb44e488d6a1381cedeeeb|088e9f765c754e079d479816d28cb5cb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'asset_tag',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_tag:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupdeeeb((prev: any) => ({ ...prev, asset_tag: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(asset_info_groupdeeeb?.asset_tag == "" || asset_info_groupdeeeb?.asset_tag == undefined){
      asset_info_groupdeeeb.asset_tag = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, asset_info_groupdeeeb?.asset_tag);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_tag:"invalid"}}));
        }
    }else if(asset_info_groupdeeeb?.asset_tag !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +asset_info_groupdeeeb?.asset_tag);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "088e9f765c754e079d479816d28cb5cb"
      );
      ...
      //     controlId: "088e9f765c754e079d479816d28cb5cb",
      //     isTable: false,
      //     from:"TextInputasset_tag",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'asset_tag', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'asset_tag',type:'text'};
      //   type={
      //     name:'asset_tag',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_tag.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_tag.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_tag.type
      //   }
      ...
      //   type={
      //     name:'asset_tag',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_tag.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_tag.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_tag.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!asset_info_groupdeeeb?.asset_tag)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "088e9f765c754e079d479816d28cb5cb") {
        handleChange({target:{value:asset_info_groupdeeebRef?.current?.asset_tag||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "088e9f765c754e079d479816d28cb5cb") {
        handleBlur({target:{value:asset_info_groupdeeebRef?.current?.asset_tag||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupdeeeb((pre:any)=>({...pre,asset_tag:dfd_assets_v1Props.data[0]?.asset_tag}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_tagcb5cb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupdeeeb?.asset_tag||""}
         disabled= {asset_tagcb5cb?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.asset_tag ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputasset_tag
