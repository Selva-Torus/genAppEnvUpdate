...
const TextInputmanufacturer = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.manufacturer"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|660280a194cb44e488d6a1381cedeeeb|36da98192ca54a8087b151c4084b8d3f"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'manufacturer',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,manufacturer:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupdeeeb((prev: any) => ({ ...prev, manufacturer: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(asset_info_groupdeeeb?.manufacturer == "" || asset_info_groupdeeeb?.manufacturer == undefined){
      asset_info_groupdeeeb.manufacturer = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, asset_info_groupdeeeb?.manufacturer);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,manufacturer:"invalid"}}));
        }
    }else if(asset_info_groupdeeeb?.manufacturer !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +asset_info_groupdeeeb?.manufacturer);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "36da98192ca54a8087b151c4084b8d3f"
      );
      ...
      //     controlId: "36da98192ca54a8087b151c4084b8d3f",
      //     isTable: false,
      //     from:"TextInputmanufacturer",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'manufacturer', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'manufacturer',type:'text'};
      //   type={
      //     name:'manufacturer',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.manufacturer.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.manufacturer.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.manufacturer.type
      //   }
      ...
      //   type={
      //     name:'manufacturer',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.manufacturer.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.manufacturer.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.manufacturer.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!asset_info_groupdeeeb?.manufacturer)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "36da98192ca54a8087b151c4084b8d3f") {
        handleChange({target:{value:asset_info_groupdeeebRef?.current?.manufacturer||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "36da98192ca54a8087b151c4084b8d3f") {
        handleBlur({target:{value:asset_info_groupdeeebRef?.current?.manufacturer||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupdeeeb((pre:any)=>({...pre,manufacturer:dfd_assets_v1Props.data[0]?.manufacturer}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (manufacturerb8d3f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupdeeeb?.manufacturer||""}
         disabled= {manufacturerb8d3f?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.manufacturer ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmanufacturer
