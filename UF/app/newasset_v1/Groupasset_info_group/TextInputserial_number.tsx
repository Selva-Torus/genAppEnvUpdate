...
const TextInputserial_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.serial_number"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|660280a194cb44e488d6a1381cedeeeb|266c428bdd4747c3b77888a9bc3a45cf"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'serial_number',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,serial_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupdeeeb((prev: any) => ({ ...prev, serial_number: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(asset_info_groupdeeeb?.serial_number == "" || asset_info_groupdeeeb?.serial_number == undefined){
      asset_info_groupdeeeb.serial_number = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, asset_info_groupdeeeb?.serial_number);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,serial_number:"invalid"}}));
        }
    }else if(asset_info_groupdeeeb?.serial_number !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +asset_info_groupdeeeb?.serial_number);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "266c428bdd4747c3b77888a9bc3a45cf"
      );
      ...
      //     controlId: "266c428bdd4747c3b77888a9bc3a45cf",
      //     isTable: false,
      //     from:"TextInputserial_number",
      ...
      //   type={
      //     name:'serial_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.serial_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.serial_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.serial_number.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!asset_info_groupdeeeb?.serial_number)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "266c428bdd4747c3b77888a9bc3a45cf") {
        handleChange({target:{value:asset_info_groupdeeebRef?.current?.serial_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "266c428bdd4747c3b77888a9bc3a45cf") {
        handleBlur({target:{value:asset_info_groupdeeebRef?.current?.serial_number||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupdeeeb((pre:any)=>({...pre,serial_number:dfd_assets_v1Props.data[0]?.serial_number}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (serial_numbera45cf?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupdeeeb?.serial_number||""}
         disabled= {serial_numbera45cf?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.serial_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputserial_number
