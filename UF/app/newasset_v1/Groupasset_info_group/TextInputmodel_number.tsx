...
const TextInputmodel_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.model_number"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|660280a194cb44e488d6a1381cedeeeb|cc27af44a715499293d659331bf32271"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'model_number',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,model_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupdeeeb((prev: any) => ({ ...prev, model_number: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(asset_info_groupdeeeb?.model_number == "" || asset_info_groupdeeeb?.model_number == undefined){
      asset_info_groupdeeeb.model_number = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, asset_info_groupdeeeb?.model_number);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,model_number:"invalid"}}));
        }
    }else if(asset_info_groupdeeeb?.model_number !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +asset_info_groupdeeeb?.model_number);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "cc27af44a715499293d659331bf32271"
      );
      ...
      //     controlId: "cc27af44a715499293d659331bf32271",
      //     isTable: false,
      //     from:"TextInputmodel_number",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'model_number', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'model_number',type:'text'};
      //   type={
      //     name:'model_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.model_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.model_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.model_number.type
      //   }
      ...
      //   type={
      //     name:'model_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.model_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.model_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.model_number.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!asset_info_groupdeeeb?.model_number)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "cc27af44a715499293d659331bf32271") {
        handleChange({target:{value:asset_info_groupdeeebRef?.current?.model_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "cc27af44a715499293d659331bf32271") {
        handleBlur({target:{value:asset_info_groupdeeebRef?.current?.model_number||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupdeeeb((pre:any)=>({...pre,model_number:dfd_assets_v1Props.data[0]?.model_number}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (model_number32271?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupdeeeb?.model_number||""}
         disabled= {model_number32271?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.model_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmodel_number
