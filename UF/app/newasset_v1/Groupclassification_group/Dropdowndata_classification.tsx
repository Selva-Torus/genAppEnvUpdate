...
const Dropdowndata_classification = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [data_classificationOptions, setdata_classificationOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `data_classification`
    let mapperText: string =  `data_classification`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "a87932b93a494278a79cbc9e07045708"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_assetdataclassificationcombo_v1Props.dstKey){
      dstKey = dfd_assetdataclassificationcombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_assetdataclassificationcombo_v1Props && !dfd_assetdataclassificationcombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  try{
    ...
    if(!value){
      ...
      setdata_classificationOptions(temp);
  ...
  },[data_classification45708?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setclassification_group3c6b3((prev: any) => ({ ...prev, data_classification: getMapperDetailsBindValues[value],data_classification45708: value,DATA_CLASSIFICATION: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = classification_group3c6b3;
      delete temp.data_classification;
      delete temp.DATA_CLASSIFICATION;
      delete temp.data_classification45708;
      setclassification_group3c6b3(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,data_classification:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(classification_group3c6b3.data_classification){
      if(Array.isArray(dfd_assetdataclassificationcombo_v1Props)){
        if(dfd_assetdataclassificationcombo_v1Props?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)){
          setdropdownValue([dfd_assetdataclassificationcombo_v1Props?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)?.data_classification])
          tempValue=dfd_assetdataclassificationcombo_v1Props?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)?.data_classification
        }else{
          setdropdownValue([classification_group3c6b3.data_classification])
          tempValue=classification_group3c6b3.data_classification
        }
      }else{
        let dstKey:string = dfd_assetdataclassificationcombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{data_classification:classification_group3c6b3.data_classification}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)?.data_classification ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.data_classification === classification_group3c6b3.data_classification)?.data_classification
      }else{
        setdropdownValue([classification_group3c6b3.data_classification])
        tempValue=classification_group3c6b3.data_classification
      }   
    ...
    if(data_classification45708?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setdata_classification45708((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[classification_group3c6b3.data_classification, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_assetdataclassificationcombo_v1Props) && dfd_assetdataclassificationcombo_v1Props?.length == 1){
    // setclassification_group3c6b3((pre:any)=>({...pre,data_classification:dfd_assetdataclassificationcombo_v1Props[0]?.data_classification}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setclassification_group3c6b3((prev: any) => ({ ...prev, data_classification: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setclassification_group3c6b3((prev: any) => ({ ...prev, data_classification: '', data_classification45708: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,data_classification:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.data_classification==getMapperDetailsBindValues[value] && items?.data_classification==value)) || {}
    selected.current={
      ...selectedObj||{},
      data_classification:value,
      data_classification:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
      if(classification_group3c6b3?.data_classification == "" || classification_group3c6b3?.data_classification == undefined){
      classification_group3c6b3.data_classification = "";
      const validate:any = v.safeParse(schema, classification_group3c6b3?.data_classification);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,data_classification:"invalid"}}));
        }
    }else if(classification_group3c6b3?.data_classification !== ""){
    const validate:any = v.safeParse(schema, classification_group3c6b3?.data_classification);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,data_classification:"invalid"}}));
  ...
  useEffect(() => { classification_group3c6b3Ref.current = classification_group3c6b3; }, [classification_group3c6b3]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "a87932b93a494278a79cbc9e07045708") {
        handleClick(classification_group3c6b3Ref?.current?.data_classification45708?classification_group3c6b3Ref?.current?.data_classification45708:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "a87932b93a494278a79cbc9e07045708");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setclassification_group3c6b3((pre:any)=>({...pre,data_classification:""}))
    else
      setInitialCount(1)
  },[data_classification45708?.refresh])

  if (data_classification45708?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {data_classification45708?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={data_classificationOptions}
        onLoadMore={loadMore}
        ...
        value={classification_group3c6b3?.data_classification45708 ? [classification_group3c6b3?.data_classification45708] : (classification_group3c6b3?.data_classification ? dropdownValue : [])}
        validationState={validate?.newAsset_v1?.data_classification ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdowndata_classification;
