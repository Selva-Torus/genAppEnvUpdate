...
const Dropdownclassification = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `classification`
    let mapperText: string =  `classification`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "0380813be5e94025b540525c20c8722b"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_assetclassificationcombo_v1Props.dstKey){
      dstKey = dfd_assetclassificationcombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_assetclassificationcombo_v1Props && !dfd_assetclassificationcombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[classification8722b?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setclassification_group3c6b3((prev: any) => ({ ...prev, classification: getMapperDetailsBindValues[value],classification8722b: value,CLASSIFICATION: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = classification_group3c6b3;
      delete temp.classification;
      delete temp.CLASSIFICATION;
      delete temp.classification8722b;
      setclassification_group3c6b3(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,classification:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(classification_group3c6b3.classification){
      if(Array.isArray(dfd_assetclassificationcombo_v1Props)){
        if(dfd_assetclassificationcombo_v1Props?.find((item: any) => item.classification === classification_group3c6b3.classification)){
          setdropdownValue([dfd_assetclassificationcombo_v1Props?.find((item: any) => item.classification === classification_group3c6b3.classification)?.classification])
          tempValue=dfd_assetclassificationcombo_v1Props?.find((item: any) => item.classification === classification_group3c6b3.classification)?.classification
        }else{
          setdropdownValue([classification_group3c6b3.classification])
          tempValue=classification_group3c6b3.classification
        }
      }else{
        let dstKey:string = dfd_assetclassificationcombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{classification:classification_group3c6b3.classification}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.classification === classification_group3c6b3.classification)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.classification === classification_group3c6b3.classification)?.classification ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.classification === classification_group3c6b3.classification)?.classification
      }else{
        setdropdownValue([classification_group3c6b3.classification])
        tempValue=classification_group3c6b3.classification
      }   
    ...
    if(classification8722b?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setclassification8722b((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[classification_group3c6b3.classification, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_assetclassificationcombo_v1Props) && dfd_assetclassificationcombo_v1Props?.length == 1){
    // setclassification_group3c6b3((pre:any)=>({...pre,classification:dfd_assetclassificationcombo_v1Props[0]?.classification}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setclassification_group3c6b3((prev: any) => ({ ...prev, classification: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setclassification_group3c6b3((prev: any) => ({ ...prev, classification: '', classification8722b: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,classification:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.classification==getMapperDetailsBindValues[value] && items?.classification==value)) || {}
    selected.current={
      ...selectedObj||{},
      classification:value,
      classification:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
      if(classification_group3c6b3?.classification == "" || classification_group3c6b3?.classification == undefined){
      classification_group3c6b3.classification = "";
      const validate:any = v.safeParse(schema, classification_group3c6b3?.classification);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,classification:"invalid"}}));
        }
    }else if(classification_group3c6b3?.classification !== ""){
    const validate:any = v.safeParse(schema, classification_group3c6b3?.classification);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,classification:"invalid"}}));
  ...
  useEffect(() => { classification_group3c6b3Ref.current = classification_group3c6b3; }, [classification_group3c6b3]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "0380813be5e94025b540525c20c8722b") {
        handleClick(classification_group3c6b3Ref?.current?.classification8722b?classification_group3c6b3Ref?.current?.classification8722b:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "0380813be5e94025b540525c20c8722b");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setclassification_group3c6b3((pre:any)=>({...pre,classification:""}))
    else
      setInitialCount(1)
  },[classification8722b?.refresh])

  if (classification8722b?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {classification8722b?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={classificationOptions}
        onLoadMore={loadMore}
        ...
        value={classification_group3c6b3?.classification8722b ? [classification_group3c6b3?.classification8722b] : (classification_group3c6b3?.classification ? dropdownValue : [])}
        validationState={validate?.newAsset_v1?.classification ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
