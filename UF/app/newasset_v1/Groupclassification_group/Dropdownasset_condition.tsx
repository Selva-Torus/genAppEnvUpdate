...
const Dropdownasset_condition = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [asset_conditionOptions, setasset_conditionOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "15af2eb1cd694498a90d6534891414c5"
    );
  ...
  try{
    ...
    if(!value){
      ...
      setasset_conditionOptions(temp);
  ...
  },[asset_condition414c5?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setclassification_group3c6b3((prev: any) => ({ ...prev, asset_condition: getMapperDetailsBindValues[value],asset_condition414c5: value,ASSET_CONDITION: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = classification_group3c6b3;
      delete temp.asset_condition;
      delete temp.ASSET_CONDITION;
      delete temp.asset_condition414c5;
      setclassification_group3c6b3(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_condition:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(classification_group3c6b3.asset_condition){
      if(Array.isArray(dfd_assetconditioncombo_v1Props)){
        if(dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)){
          setdropdownValue([dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)?.asset_condition])
          tempValue=dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)?.asset_condition
        }else{
          setdropdownValue([classification_group3c6b3.asset_condition])
          tempValue=classification_group3c6b3.asset_condition
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{asset_condition:classification_group3c6b3.asset_condition}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)?.asset_condition ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.asset_condition === classification_group3c6b3.asset_condition)?.asset_condition
      }else{
        setdropdownValue([classification_group3c6b3.asset_condition])
        tempValue=classification_group3c6b3.asset_condition
      }   
    ...
    if(asset_condition414c5?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setasset_condition414c5((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[classification_group3c6b3.asset_condition, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_assetconditioncombo_v1Props) && dfd_assetconditioncombo_v1Props?.length == 1){
    // setclassification_group3c6b3((pre:any)=>({...pre,asset_condition:dfd_assetconditioncombo_v1Props[0]?.asset_condition}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setclassification_group3c6b3((prev: any) => ({ ...prev, asset_condition: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setclassification_group3c6b3((prev: any) => ({ ...prev, asset_condition: '', asset_condition414c5: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_condition:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
      if(classification_group3c6b3?.asset_condition == "" || classification_group3c6b3?.asset_condition == undefined){
      classification_group3c6b3.asset_condition = "";
      const validate:any = v.safeParse(schema, classification_group3c6b3?.asset_condition);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_condition:"invalid"}}));
        }
    }else if(classification_group3c6b3?.asset_condition !== ""){
    const validate:any = v.safeParse(schema, classification_group3c6b3?.asset_condition);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,asset_condition:"invalid"}}));
  ...
  useEffect(() => { classification_group3c6b3Ref.current = classification_group3c6b3; }, [classification_group3c6b3]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "15af2eb1cd694498a90d6534891414c5") {
        handleClick(classification_group3c6b3Ref?.current?.asset_condition414c5?classification_group3c6b3Ref?.current?.asset_condition414c5:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "15af2eb1cd694498a90d6534891414c5");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setclassification_group3c6b3((pre:any)=>({...pre,asset_condition:""}))
    else
      setInitialCount(1)
  },[asset_condition414c5?.refresh])

  if (asset_condition414c5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {asset_condition414c5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={asset_conditionOptions}
        onLoadMore={loadMore}
        ...
        value={classification_group3c6b3?.asset_condition414c5 ? [classification_group3c6b3?.asset_condition414c5] : (classification_group3c6b3?.asset_condition ? dropdownValue : [])}
        validationState={validate?.newAsset_v1?.asset_condition ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownasset_condition;
