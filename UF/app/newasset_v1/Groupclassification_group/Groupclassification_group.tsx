...
import Dropdownclassification  from "./Dropdownclassification";
import Dropdowndata_classification  from "./Dropdowndata_classification";
import Dropdownownership_type  from "./Dropdownownership_type";
import Dropdownlifecycle_stage  from "./Dropdownlifecycle_stage";
import Dropdownasset_condition  from "./Dropdownasset_condition";
import Dropdownrisk_level  from "./Dropdownrisk_level";
import Checkboxis_critical  from "./Checkboxis_critical";
...
const Groupclassification_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "43f361b4c0a74138ba1001e580d3c6b3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setclassification_group3c6b3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("classification_text")){
        setclassification_text9bbdf({...classification_text9bbdf,isDisabled:true});

    }else
    {
      if(classification_text9bbdf?.isDisabled==null)
      {
        setclassification_text9bbdf({...classification_text9bbdf,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("classification")){
        setclassification8722b({...classification8722b,isDisabled:true});

    }else
    {
      if(classification8722b?.isDisabled==null)
      {
        setclassification8722b({...classification8722b,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("data_classification")){
        setdata_classification45708({...data_classification45708,isDisabled:true});

    }else
    {
      if(data_classification45708?.isDisabled==null)
      {
        setdata_classification45708({...data_classification45708,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ownership_type")){
        setownership_type1a506({...ownership_type1a506,isDisabled:true});

    }else
    {
      if(ownership_type1a506?.isDisabled==null)
      {
        setownership_type1a506({...ownership_type1a506,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("lifecycle_stage")){
        setlifecycle_stage1446e({...lifecycle_stage1446e,isDisabled:true});

    }else
    {
      if(lifecycle_stage1446e?.isDisabled==null)
      {
        setlifecycle_stage1446e({...lifecycle_stage1446e,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("asset_condition")){
        setasset_condition414c5({...asset_condition414c5,isDisabled:true});

    }else
    {
      if(asset_condition414c5?.isDisabled==null)
      {
        setasset_condition414c5({...asset_condition414c5,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("risk_level")){
        setrisk_levelf1e8c({...risk_levelf1e8c,isDisabled:true});

    }else
    {
      if(risk_levelf1e8c?.isDisabled==null)
      {
        setrisk_levelf1e8c({...risk_levelf1e8c,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("is_critical")){
        setis_critical0f006({...is_critical0f006,isDisabled:true});

    }else
    {
      if(is_critical0f006?.isDisabled==null)
      {
        setis_critical0f006({...is_critical0f006,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const classification_group3c6b3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    classification_group3c6b3Ref.current?.setSearchParams();
    classification_group3c6b3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(classification_group3c6b3) && Object.keys(classification_group3c6b3)?.length>0)
      {
        setclassification_group3c6b3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [classification_group3c6b3Props?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("classification") ?<Dropdownclassification   /* 8722b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("data_classification") ?<Dropdowndata_classification   /* 45708 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("ownership_type") ?<Dropdownownership_type   /* 1a506 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("lifecycle_stage") ?<Dropdownlifecycle_stage   /* 1446e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("asset_condition") ?<Dropdownasset_condition   /* 414c5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("risk_level") ?<Dropdownrisk_level   /* f1e8c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("is_critical") ?<Checkboxis_critical   /* 0f006 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
