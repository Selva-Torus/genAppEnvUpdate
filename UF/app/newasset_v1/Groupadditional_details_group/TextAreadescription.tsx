...
const TextAreadescription = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "951df6a3e4e742638860c1c075709f58"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[description09f58?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setadditional_details_group8c616((pre:any)=>({...pre,description:""}))
    }else 
  ...
  const handleChange = async(e: any) => {
    ...
    setadditional_details_group8c616((prev: any) => ({ ...prev, description: e?.target?.value }));
    }catch (err: any) {
  ...
  if (description09f58?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `9 / 25`,gridRow: `8 / 20`, gap:``, height: `100%`}} >
    <TextArea
      className="!rounded-md"
      onFocus={handleFocus}
      onChange={handleChange}
      onBlur={handleBlur}
      disabled= {description09f58?.isDisabled ? true : false}
      placeholder = {'type here...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Description"
      pin = {'brick-brick'}
      value = { additional_details_group8c616?.description != null && typeof additional_details_group8c616?.description =='object' ? Object.keys(additional_details_group8c616?.description)?.length ?  JSON.stringify(additional_details_group8c616?.description,null ,2):"" : additional_details_group8c616?.description||""}
    />
  </div>
  )
}
