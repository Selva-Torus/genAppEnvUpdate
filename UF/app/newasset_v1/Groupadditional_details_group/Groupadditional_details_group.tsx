...
const Groupadditional_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "751dbf74dd7a4f09bb2709de1d68c616");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadditional_details_group8c616Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("additional_details_text")){
        setadditional_details_text21426({...additional_details_text21426,isDisabled:true});

    }else
    {
      if(additional_details_text21426?.isDisabled==null)
      {
        setadditional_details_text21426({...additional_details_text21426,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("location")){
        setlocation2ff4b({...location2ff4b,isDisabled:true});

    }else
    {
      if(location2ff4b?.isDisabled==null)
      {
        setlocation2ff4b({...location2ff4b,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("description")){
        setdescription09f58({...description09f58,isDisabled:true});

    }else
    {
      if(description09f58?.isDisabled==null)
      {
        setdescription09f58({...description09f58,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const additional_details_group8c616Ref = useRef<any>(null);
  const handleClearSearch = () => {
    additional_details_group8c616Ref.current?.setSearchParams();
    additional_details_group8c616Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(additional_details_group8c616) && Object.keys(additional_details_group8c616)?.length>0)
      {
        setadditional_details_group8c616({})
      }
    }else 
      prevRefreshRef.current= true
  }, [additional_details_group8c616Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("additional_details_text") ?<Textadditional_details_text   /* 21426 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("location") ?<TextInputlocation   /* 2ff4b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("description") ?<TextAreadescription   /* 09f58 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
