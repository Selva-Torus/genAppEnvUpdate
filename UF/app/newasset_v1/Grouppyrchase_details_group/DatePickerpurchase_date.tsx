...
const DatePickerpurchase_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,purchase_date: "invalid"}}))
    setpyrchase_details_group76407((prev: any) => ({ ...prev, purchase_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,purchase_date:undefined}}));
  if (!date) {
    setpyrchase_details_group76407((prev: any) => ({ ...prev, purchase_date: null }));
    return;
  }
  ...
  setpyrchase_details_group76407((prev: any) => ({ ...prev, purchase_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(pyrchase_details_group76407?.purchase_date == "" || pyrchase_details_group76407?.purchase_date == undefined){
        const result = v.safeParse(schema, pyrchase_details_group76407?.purchase_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "895a337a026249ea9b7243224aac1162"
      );
...
useEffect(()=>{
 ...
 },[purchase_datec1162?.refresh])

useEffect(()=>{
  if(!pyrchase_details_group76407?.purchase_date){ 
...
if (purchase_datec1162?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      ...
      //label={keyset("")}
      value={pyrchase_details_group76407?.purchase_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {purchase_datec1162?.isDisabled ? true : false}
      disabled= {purchase_datec1162?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Purchase Date"
      dateValidation=""
      validationState={validate?.newAsset_v1?.purchase_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerpurchase_date
