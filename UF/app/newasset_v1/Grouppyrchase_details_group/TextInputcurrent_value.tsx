...
const TextInputcurrent_value = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.current_value"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|ee1d91659fbf473bb30f690c00976407|d5b12e6d329c4010b2875274c128f6cd"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'current_value',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,current_value:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_group76407((prev: any) => ({ ...prev, current_value: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(pyrchase_details_group76407?.current_value == "" || pyrchase_details_group76407?.current_value == undefined){
      pyrchase_details_group76407.current_value = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, pyrchase_details_group76407?.current_value);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,current_value:"invalid"}}));
        }
    }else if(pyrchase_details_group76407?.current_value !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +pyrchase_details_group76407?.current_value);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d5b12e6d329c4010b2875274c128f6cd"
      );
      ...
      //     controlId: "d5b12e6d329c4010b2875274c128f6cd",
      //     isTable: false,
      //     from:"TextInputcurrent_value",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'current_value', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'current_value',type:'text'};
      //   type={
      //     name:'current_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_value.type
      //   }
      ...
      //   type={
      //     name:'current_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_value.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!pyrchase_details_group76407?.current_value)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "d5b12e6d329c4010b2875274c128f6cd") {
        handleChange({target:{value:pyrchase_details_group76407Ref?.current?.current_value||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d5b12e6d329c4010b2875274c128f6cd") {
        handleBlur({target:{value:pyrchase_details_group76407Ref?.current?.current_value||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_group76407((pre:any)=>({...pre,current_value:dfd_assets_v1Props.data[0]?.current_value}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (current_value8f6cd?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={pyrchase_details_group76407?.current_value||""}
         disabled= {current_value8f6cd?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAsset_v1?.current_value ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcurrent_value
