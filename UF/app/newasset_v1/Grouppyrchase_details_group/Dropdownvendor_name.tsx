 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "680c71f3c16e4369932b9c4c64e4190d"
    );
  ...
  },[vendor_name4190d?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setpyrchase_details_group76407((prev: any) => ({ ...prev, vendor_name: getMapperDetailsBindValues[value],vendor_name4190d: value,VENDOR_NAME: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = pyrchase_details_group76407;
      delete temp.vendor_name;
      delete temp.VENDOR_NAME;
      delete temp.vendor_name4190d;
      setpyrchase_details_group76407(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,vendor_name:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(pyrchase_details_group76407.vendor_name){
      if(Array.isArray(dfd_vendornamecombo_v1Props)){
        if(dfd_vendornamecombo_v1Props?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)){
          setdropdownValue([dfd_vendornamecombo_v1Props?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)?.vendor_name])
          tempValue=dfd_vendornamecombo_v1Props?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)?.vendor_name
        }else{
          setdropdownValue([pyrchase_details_group76407.vendor_name])
          tempValue=pyrchase_details_group76407.vendor_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{vendor_name:pyrchase_details_group76407.vendor_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)?.vendor_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.vendor_name === pyrchase_details_group76407.vendor_name)?.vendor_name
      }else{
        setdropdownValue([pyrchase_details_group76407.vendor_name])
        tempValue=pyrchase_details_group76407.vendor_name
      }   
    ...
    if(vendor_name4190d?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setvendor_name4190d((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[pyrchase_details_group76407.vendor_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_vendornamecombo_v1Props) && dfd_vendornamecombo_v1Props?.length == 1){
    // setpyrchase_details_group76407((pre:any)=>({...pre,vendor_name:dfd_vendornamecombo_v1Props[0]?.vendor_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setpyrchase_details_group76407((prev: any) => ({ ...prev, vendor_name: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setpyrchase_details_group76407((prev: any) => ({ ...prev, vendor_name: '', vendor_name4190d: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,vendor_name:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
      if(pyrchase_details_group76407?.vendor_name == "" || pyrchase_details_group76407?.vendor_name == undefined){
      pyrchase_details_group76407.vendor_name = "";
      const validate:any = v.safeParse(schema, pyrchase_details_group76407?.vendor_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,vendor_name:"invalid"}}));
        }
    }else if(pyrchase_details_group76407?.vendor_name !== ""){
    const validate:any = v.safeParse(schema, pyrchase_details_group76407?.vendor_name);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,vendor_name:"invalid"}}));
  ...
  useEffect(() => { pyrchase_details_group76407Ref.current = pyrchase_details_group76407; }, [pyrchase_details_group76407]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "680c71f3c16e4369932b9c4c64e4190d") {
        handleClick(pyrchase_details_group76407Ref?.current?.vendor_name4190d?pyrchase_details_group76407Ref?.current?.vendor_name4190d:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "680c71f3c16e4369932b9c4c64e4190d");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setpyrchase_details_group76407((pre:any)=>({...pre,vendor_name:""}))
    else
      setInitialCount(1)
  },[vendor_name4190d?.refresh])

  if (vendor_name4190d?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {vendor_name4190d?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={pyrchase_details_group76407?.vendor_name4190d ? [pyrchase_details_group76407?.vendor_name4190d] : (pyrchase_details_group76407?.vendor_name ? dropdownValue : [])}
        validationState={validate?.newAsset_v1?.vendor_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
