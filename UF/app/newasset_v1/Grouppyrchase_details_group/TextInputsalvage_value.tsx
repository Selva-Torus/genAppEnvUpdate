...
const TextInputsalvage_value = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.salvage_value"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|ee1d91659fbf473bb30f690c00976407|b91fca5c42954f9a830b75a8495f1995"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'salvage_value',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,salvage_value:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_group76407((prev: any) => ({ ...prev, salvage_value: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(pyrchase_details_group76407?.salvage_value == "" || pyrchase_details_group76407?.salvage_value == undefined){
      pyrchase_details_group76407.salvage_value = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, pyrchase_details_group76407?.salvage_value);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,salvage_value:"invalid"}}));
        }
    }else if(pyrchase_details_group76407?.salvage_value !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +pyrchase_details_group76407?.salvage_value);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b91fca5c42954f9a830b75a8495f1995"
      );
      ...
      //     controlId: "b91fca5c42954f9a830b75a8495f1995",
      //     isTable: false,
      //     from:"TextInputsalvage_value",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'salvage_value', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'salvage_value',type:'text'};
      //   type={
      //     name:'salvage_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.salvage_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.salvage_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.salvage_value.type
      //   }
      ...
      //   type={
      //     name:'salvage_value',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salvage_value.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salvage_value.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salvage_value.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!pyrchase_details_group76407?.salvage_value)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "b91fca5c42954f9a830b75a8495f1995") {
        handleChange({target:{value:pyrchase_details_group76407Ref?.current?.salvage_value||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b91fca5c42954f9a830b75a8495f1995") {
        handleBlur({target:{value:pyrchase_details_group76407Ref?.current?.salvage_value||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_group76407((pre:any)=>({...pre,salvage_value:dfd_assets_v1Props.data[0]?.salvage_value}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (salvage_valuef1995?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `34 / 46`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={pyrchase_details_group76407?.salvage_value||""}
         disabled= {salvage_valuef1995?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Salvage Value"
      errorMessage={error}
        validationState={validate?.newAsset_v1?.salvage_value ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputsalvage_value
