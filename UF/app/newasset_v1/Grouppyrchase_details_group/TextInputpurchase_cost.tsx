...
const TextInputpurchase_cost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.purchase_cost"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAsset:AFVK:v1|ee1d91659fbf473bb30f690c00976407|91e9d79ef20846558184240ad4eff91e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'purchase_cost',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,purchase_cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_group76407((prev: any) => ({ ...prev, purchase_cost: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(pyrchase_details_group76407?.purchase_cost == "" || pyrchase_details_group76407?.purchase_cost == undefined){
      pyrchase_details_group76407.purchase_cost = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, pyrchase_details_group76407?.purchase_cost);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAsset_v1:{...pre?.newAsset_v1,purchase_cost:"invalid"}}));
        }
    }else if(pyrchase_details_group76407?.purchase_cost !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +pyrchase_details_group76407?.purchase_cost);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "91e9d79ef20846558184240ad4eff91e"
      );
      ...
      //     controlId: "91e9d79ef20846558184240ad4eff91e",
      //     isTable: false,
      //     from:"TextInputpurchase_cost",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'purchase_cost', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'purchase_cost',type:'text'};
      //   type={
      //     name:'purchase_cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.purchase_cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.purchase_cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.purchase_cost.type
      //   }
      ...
      //   type={
      //     name:'purchase_cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.purchase_cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.purchase_cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.purchase_cost.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!pyrchase_details_group76407?.purchase_cost)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "91e9d79ef20846558184240ad4eff91e") {
        handleChange({target:{value:pyrchase_details_group76407Ref?.current?.purchase_cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "91e9d79ef20846558184240ad4eff91e") {
        handleBlur({target:{value:pyrchase_details_group76407Ref?.current?.purchase_cost||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_group76407((pre:any)=>({...pre,purchase_cost:dfd_assets_v1Props.data[0]?.purchase_cost}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (purchase_costff91e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={pyrchase_details_group76407?.purchase_cost||""}
         disabled= {purchase_costff91e?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Purchase Cost"
      errorMessage={error}
        validationState={validate?.newAsset_v1?.purchase_cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpurchase_cost
