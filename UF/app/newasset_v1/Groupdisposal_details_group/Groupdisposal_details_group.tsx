...
const Groupdisposal_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2cc91251b32d441abfbad386965affa1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdisposal_details_groupaffa1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("disposal_details_text")){
        setdisposal_details_text65909({...disposal_details_text65909,isDisabled:true});

    }else
    {
      if(disposal_details_text65909?.isDisabled==null)
      {
        setdisposal_details_text65909({...disposal_details_text65909,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("disposal_method")){
        setdisposal_methodd33dc({...disposal_methodd33dc,isDisabled:true});

    }else
    {
      if(disposal_methodd33dc?.isDisabled==null)
      {
        setdisposal_methodd33dc({...disposal_methodd33dc,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("disposal_date")){
        setdisposal_date920f2({...disposal_date920f2,isDisabled:true});

    }else
    {
      if(disposal_date920f2?.isDisabled==null)
      {
        setdisposal_date920f2({...disposal_date920f2,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("disposal_ref")){
        setdisposal_ref075d5({...disposal_ref075d5,isDisabled:true});

    }else
    {
      if(disposal_ref075d5?.isDisabled==null)
      {
        setdisposal_ref075d5({...disposal_ref075d5,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const disposal_details_groupaffa1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    disposal_details_groupaffa1Ref.current?.setSearchParams();
    disposal_details_groupaffa1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(disposal_details_groupaffa1) && Object.keys(disposal_details_groupaffa1)?.length>0)
      {
        setdisposal_details_groupaffa1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [disposal_details_groupaffa1Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("disposal_details_text") ?<Textdisposal_details_text   /* 65909 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("disposal_method") ?<Dropdowndisposal_method   /* d33dc */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("disposal_date") ?<DatePickerdisposal_date   /* 920f2 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("disposal_ref") ?<TextInputdisposal_ref   /* 075d5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
