...
const TextInputtextinput5 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:ffff:AFVK:v1|aaa21094dc8b49d0be2621f7ea87f2ed|09ee622be5f24ed9ae7648fdb31daae3"
    }
  ],
  ...
  "v.maxValue(55)"
    ...
    const schema : any  = v.pipe(    v.number(),
    v.minValue(9),
    v.maxValue(55),
)
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,my_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup7f2ed((prev: any) => ({ ...prev, my_id: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(group7f2ed?.my_id == "" || group7f2ed?.my_id == undefined){
          setError('');
          setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,my_id:undefined}}));
    }else if(group7f2ed?.my_id !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +group7f2ed?.my_id);
        }
        else{
          validate = v.safeParse(schema, group7f2ed?.my_id);
        }
    if(!validate.success){
      ...
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,my_id:"invalid"}}));
    }else{
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,my_id:undefined}}));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "09ee622be5f24ed9ae7648fdb31daae3"
      );
      ...
      //     controlId: "09ee622be5f24ed9ae7648fdb31daae3",
      //     isTable: false,
      //     from:"TextInputtextinput5",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "09ee622be5f24ed9ae7648fdb31daae3") {
        handleChange({target:{value:group7f2edRef?.current?.my_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "09ee622be5f24ed9ae7648fdb31daae3") {
        handleBlur({target:{value:group7f2edRef?.current?.my_id||""}});
      }
    };
  ...
  if(dfd_country_code_dfd_v1Props?.setSearchFilters && dfd_country_code_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_country_code_dfd_v1Props.data) && dfd_country_code_dfd_v1Props.data.length > 0){
      setgroup7f2ed((pre:any)=>({...pre,my_id:dfd_country_code_dfd_v1Props.data[0]?.my_id}));
    }
  }
  },[dfd_country_code_dfd_v1Props?.setSearchFilters])
  if (textinput5daae3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 10`,gridRow: `2 / 15`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group7f2ed?.my_id||""}
         disabled= {textinput5daae3?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ffff_v1?.my_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtextinput5
