...
const TextInputtextinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setgroup7f2ed((prev: any) => ({ ...prev, textinput: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(group7f2ed?.textinput == "" || group7f2ed?.textinput == undefined){
      group7f2ed.textinput = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, group7f2ed?.textinput);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput:"invalid"}}));
        }
    }else if(group7f2ed?.textinput !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +group7f2ed?.textinput);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "477f903a175c4dfbb6efa5e6b90d0435"
      );
      ...
      //     controlId: "477f903a175c4dfbb6efa5e6b90d0435",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "477f903a175c4dfbb6efa5e6b90d0435") {
        handleChange({target:{value:group7f2edRef?.current?.textinput||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "477f903a175c4dfbb6efa5e6b90d0435") {
        handleBlur({target:{value:group7f2edRef?.current?.textinput||""}});
      }
    };
  ...
  if (textinputd0435?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 5`,gridRow: `2 / 15`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group7f2ed?.textinput||""}
         disabled= {textinputd0435?.isDisabled ? true : false}
        pin='brick-brick'     
