...
const TextInputtextinput3 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'textinput3',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput3:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup7f2ed((prev: any) => ({ ...prev, textinput3: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(group7f2ed?.textinput3 == "" || group7f2ed?.textinput3 == undefined){
          setError('');
          setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput3:undefined}}));
    }else if(group7f2ed?.textinput3 !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +group7f2ed?.textinput3);
        }
        else{
          validate = v.safeParse(schema, group7f2ed?.textinput3);
        }
    if(!validate.success){
      ...
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput3:"invalid"}}));
    }else{
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput3:undefined}}));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e9ba62847d6a4f5a8b3328e10bf8ac83"
      );
      ...
      //     controlId: "e9ba62847d6a4f5a8b3328e10bf8ac83",
      //     isTable: false,
      //     from:"TextInputtextinput3",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'textinput3', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'textinput3',type:'text'};
      //   type={
      //     name:'textinput3',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput3.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput3.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput3.type
      //   }
      ...
      //   type={
      //     name:'textinput3',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput3.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput3.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput3.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e9ba62847d6a4f5a8b3328e10bf8ac83") {
        handleChange({target:{value:group7f2edRef?.current?.textinput3||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e9ba62847d6a4f5a8b3328e10bf8ac83") {
        handleBlur({target:{value:group7f2edRef?.current?.textinput3||""}});
      }
    };
  ...
  if (textinput38ac83?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 5`,gridRow: `72 / 90`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group7f2ed?.textinput3||""}
         disabled= {textinput38ac83?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ffff_v1?.textinput3 ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtextinput3
