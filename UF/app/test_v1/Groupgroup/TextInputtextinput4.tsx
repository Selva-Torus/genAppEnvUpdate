...
const TextInputtextinput4 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'textinput4',type:"text"})
  ...
  schemaArray = [
  "v.any()",
  "v.regex(/^[678]\\d{9}$/, 'must be regex format')"
] ;
    const schema : any  = v.pipe(    v.any(),
    v.regex(/^[678]\d{9}$/, 'must be regex format'),
)
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput4:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup7f2ed((prev: any) => ({ ...prev, textinput4: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(group7f2ed?.textinput4 == "" || group7f2ed?.textinput4 == undefined){
          setError('');
          setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput4:undefined}}));
    }else if(group7f2ed?.textinput4 !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +group7f2ed?.textinput4);
        }
        else{
          validate = v.safeParse(schema, group7f2ed?.textinput4);
        }
    if(!validate.success){
      ...
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput4:"invalid"}}));
    }else{
      setError('');
      setValidate((pre:any)=>({...pre,ffff_v1:{...pre?.ffff_v1,textinput4:undefined}}));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "74c73798c9bd433a81a7edcac8855cca"
      );
      ...
      //     controlId: "74c73798c9bd433a81a7edcac8855cca",
      //     isTable: false,
      //     from:"TextInputtextinput4",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'textinput4', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'textinput4',type:'text'};
      //   type={
      //     name:'textinput4',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput4.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput4.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.textinput4.type
      //   }
      ...
      //   type={
      //     name:'textinput4',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput4.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput4.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.textinput4.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "74c73798c9bd433a81a7edcac8855cca") {
        handleChange({target:{value:group7f2edRef?.current?.textinput4||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "74c73798c9bd433a81a7edcac8855cca") {
        handleBlur({target:{value:group7f2edRef?.current?.textinput4||""}});
      }
    };
  ...
  if (textinput455cca?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 5`,gridRow: `98 / 116`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group7f2ed?.textinput4||""}
         disabled= {textinput455cca?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ffff_v1?.textinput4 ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtextinput4
