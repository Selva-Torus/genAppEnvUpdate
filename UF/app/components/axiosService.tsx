...
AxiosService.interceptors.request.use(
  async (config) => {
     if (config.data && ['post', 'put', 'patch'].includes(config.method || '') && config.data.dpdKey && config.data.method ) {
      ...
      let ciphertext : any;
      if(method == "vault"){
        const wrapped = await localEncrypt({ value: config.data, context: "ct005_gss_rtgs_v1" })
        ciphertext = await encryptData(dpdKey, wrapped)
      }else{
        ciphertext = await clientEncrypt(dpdKey, method, config.data, "ct005_gss_rtgs_v1")
      }
  ...
  async(response:any) => {
    if ( response.data.dpdKey && response.data.method ) {
      ...
      if(method == "vault"){
        ...
        response.data = await clientDecrypt(dpdKey, method, response.data, "ct005_gss_rtgs_v1")
      }
    }
    return response
  },
