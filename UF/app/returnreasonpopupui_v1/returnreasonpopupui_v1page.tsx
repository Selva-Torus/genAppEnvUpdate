...
export default function PageReturnreasonpopupuiV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function returnreasondfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(returnreasonpopupui_v1Props.length > 0){
          for(let i=0;i< returnreasonpopupui_v1Props.length;i++){
            if(returnreasonpopupui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:returnReasonDfd:AFVK:v1"){
              // delete returnreasonpopupui_v1Props[i].DFDkey;
              let temp=structuredClone(returnreasonpopupui_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setreturnreasonpopupui_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    const introspectParams = encryptionFlagPage
      ? {
          ...
          method: encryptionMethod,
          key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:returnReasonPopUpUi:AFVK:v1"
        }
      : { key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:returnReasonPopUpUi:AFVK:v1" }
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:returnReasonPopUpUi:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setreturnreasonpopupui_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(returnreasonpopupui_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(returnreasonpopupui_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setreturnreasonpopupui_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
