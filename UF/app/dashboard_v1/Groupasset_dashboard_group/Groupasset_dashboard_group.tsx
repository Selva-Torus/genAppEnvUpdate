...
const Groupasset_dashboard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d3b15240a883824258901918a0e4bbfe");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_dashboard_group4bbfeProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("amr_queue_group")){
        setamr_queue_groupc92ca((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(amr_queue_groupc92ca?.isDisabled==null)
      {
        setamr_queue_groupc92ca((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_file_group")){
        setpending_file_groupffe32((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_file_groupffe32?.isDisabled==null)
      {
        setpending_file_groupffe32((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("service_pending_group")){
        setservice_pending_group7ba93((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(service_pending_group7ba93?.isDisabled==null)
      {
        setservice_pending_group7ba93((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("slas_at_risk_group")){
        setslas_at_risk_group23eb4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(slas_at_risk_group23eb4?.isDisabled==null)
      {
        setslas_at_risk_group23eb4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection_group")){
        setcourt_rejection_groupc9d54((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_groupc9d54?.isDisabled==null)
      {
        setcourt_rejection_groupc9d54((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("collected_mtd_group")){
        setcollected_mtd_group7b7b5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(collected_mtd_group7b7b5?.isDisabled==null)
      {
        setcollected_mtd_group7b7b5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_dashboard_group4bbfeRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_dashboard_group4bbfeRef.current?.setSearchParams();
    asset_dashboard_group4bbfeRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_dashboard_group4bbfe) && Object.keys(asset_dashboard_group4bbfe)?.length>0)
      {
        setasset_dashboard_group4bbfe({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_dashboard_group4bbfeProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-md ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"asset_dashboard_group"}))
        }}
    >
