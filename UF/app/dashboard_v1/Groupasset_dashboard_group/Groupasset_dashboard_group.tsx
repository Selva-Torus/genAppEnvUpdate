...
import Groupmaintenance_due_group  from "../Groupmaintenance_due_group/Groupmaintenance_due_group";
import Groupwarranty_expiring_group  from "../Groupwarranty_expiring_group/Groupwarranty_expiring_group";
import Groupsoftware_licenses_group  from "../Groupsoftware_licenses_group/Groupsoftware_licenses_group";
import Grouppending_disposal_group  from "../Grouppending_disposal_group/Grouppending_disposal_group";
...
const Groupasset_dashboard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "b5cd49f3cb284a9c980e2670ba94d6cb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_dashboard_group4d6cbProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_asset_group")){
        settotal_asset_group69aa9({...total_asset_group69aa9,isDisabled:true});

    }else
    {
      if(total_asset_group69aa9?.isDisabled==null)
      {
        settotal_asset_group69aa9({...total_asset_group69aa9,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("maintenance_due_group")){
        setmaintenance_due_group704ca({...maintenance_due_group704ca,isDisabled:true});

    }else
    {
      if(maintenance_due_group704ca?.isDisabled==null)
      {
        setmaintenance_due_group704ca({...maintenance_due_group704ca,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("warranty_expiring_group")){
        setwarranty_expiring_groupb5bd4({...warranty_expiring_groupb5bd4,isDisabled:true});

    }else
    {
      if(warranty_expiring_groupb5bd4?.isDisabled==null)
      {
        setwarranty_expiring_groupb5bd4({...warranty_expiring_groupb5bd4,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("software_licenses_group")){
        setsoftware_licenses_group4beb5({...software_licenses_group4beb5,isDisabled:true});

    }else
    {
      if(software_licenses_group4beb5?.isDisabled==null)
      {
        setsoftware_licenses_group4beb5({...software_licenses_group4beb5,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_disposal_group")){
        setpending_disposal_group2580d({...pending_disposal_group2580d,isDisabled:true});

    }else
    {
      if(pending_disposal_group2580d?.isDisabled==null)
      {
        setpending_disposal_group2580d({...pending_disposal_group2580d,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_dashboard_group4d6cbRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_dashboard_group4d6cbRef.current?.setSearchParams();
    asset_dashboard_group4d6cbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_dashboard_group4d6cb) && Object.keys(asset_dashboard_group4d6cb)?.length>0)
      {
        setasset_dashboard_group4d6cb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_dashboard_group4d6cbProps?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedComponent.includes("maintenance_due_group")  &&<Groupmaintenance_due_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("warranty_expiring_group")  &&<Groupwarranty_expiring_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("software_licenses_group")  &&<Groupsoftware_licenses_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("pending_disposal_group")  &&<Grouppending_disposal_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
