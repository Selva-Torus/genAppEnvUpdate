...
import Groupamr_queue_group  from "../Groupamr_queue_group/Groupamr_queue_group";
import Grouppending_file_group  from "../Grouppending_file_group/Grouppending_file_group";
import Groupservice_pending_group  from "../Groupservice_pending_group/Groupservice_pending_group";
import Groupslas_at_risk_group  from "../Groupslas_at_risk_group/Groupslas_at_risk_group";
import Groupcourt_rejection_group  from "../Groupcourt_rejection_group/Groupcourt_rejection_group";
import Groupcollected_mtd_group  from "../Groupcollected_mtd_group/Groupcollected_mtd_group";
...
const Groupasset_dashboard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e5dbd51c011620ec72a6fe778f51aa03");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_dashboard_group1aa03Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("amr_queue_group")){
        setamr_queue_group3c082((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(amr_queue_group3c082?.isDisabled==null)
      {
        setamr_queue_group3c082((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_file_group")){
        setpending_file_group2128c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_file_group2128c?.isDisabled==null)
      {
        setpending_file_group2128c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("service_pending_group")){
        setservice_pending_group8c0ca((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(service_pending_group8c0ca?.isDisabled==null)
      {
        setservice_pending_group8c0ca((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("slas_at_risk_group")){
        setslas_at_risk_group1f8c0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(slas_at_risk_group1f8c0?.isDisabled==null)
      {
        setslas_at_risk_group1f8c0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection_group")){
        setcourt_rejection_groupdf57a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_groupdf57a?.isDisabled==null)
      {
        setcourt_rejection_groupdf57a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("collected_mtd_group")){
        setcollected_mtd_group0f074((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(collected_mtd_group0f074?.isDisabled==null)
      {
        setcollected_mtd_group0f074((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_dashboard_group1aa03Ref = useRef<any>(null);
  const handleClearSearch = () => {
    asset_dashboard_group1aa03Ref.current?.setSearchParams();
    asset_dashboard_group1aa03Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_dashboard_group1aa03) && Object.keys(asset_dashboard_group1aa03)?.length>0)
      {
        setasset_dashboard_group1aa03({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_dashboard_group1aa03Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-md ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"asset_dashboard_group"}))
        }}
    >
        {allowedComponent.includes("amr_queue_group")  &&<Groupamr_queue_group  
        ...
        {allowedComponent.includes("pending_file_group")  &&<Grouppending_file_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("service_pending_group")  &&<Groupservice_pending_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("slas_at_risk_group")  &&<Groupslas_at_risk_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("court_rejection_group")  &&<Groupcourt_rejection_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("collected_mtd_group")  &&<Groupcollected_mtd_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
