...
const Groupct006_af_uf_ufws_lap_lap_amrqueuetable_v1 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupCt006AfUfUfwsLapLapAmrqueuetableV1',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0e1ad8ea6a024087a1a766d19f38a797");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("group")){
        setgroup28176((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(group28176?.isDisabled==null)
      {
        setgroup28176((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797Ref = useRef<any>(null);
  const handleClearSearch = () => {
    ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797Ref.current?.setSearchParams();
    ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797) && Object.keys(ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797)?.length>0)
      {
        setct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797({})
      }
    }else 
      prevRefreshRef.current= true
  }, [ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"ct006_af_uf_ufws_lap_lap_amrqueuetable_v1"}))
        }}
    >
