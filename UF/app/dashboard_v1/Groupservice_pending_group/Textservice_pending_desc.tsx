...
const Textservice_pending_desc = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {service_pending_descabf48Props, setservice_pending_descabf48Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[service_pending_descabf48?.refresh])

  if (service_pending_descabf48?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Cases awaiting service")}
</Text>
  </div>
  )
}

export default Textservice_pending_desc
