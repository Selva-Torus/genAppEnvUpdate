...
let defaultColumns:any = [
  {
    "id": "asset_id",
    "nodeid": "47e31d92e3f240c793693add10d29a99",
    "name": "Asset ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "asset_tag",
    "nodeid": "fd9269b4db6249bd87bcfea92efe74f0",
    "name": "Asset Tag",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "asset_name",
    "nodeid": "d7d79b5ffc764127b0a2780b92e13b83",
    "name": "Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "category",
    "nodeid": "8eaa24e151074a63b8a64b80a972ca3f",
    "name": "Category",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "serial_no",
    "nodeid": "e735f83e9d454c288b6b59c361e3f2a9",
    "name": "Serial No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "assigned_to",
    "nodeid": "a03e4d17753c430fbea9dcb965c28cfb",
    "name": "Assigned To",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "location",
    "nodeid": "985cfa485b7f49f5b5161533a1e7cca5",
    "name": "Location",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "warranty_expiry",
    "nodeid": "4a34ed799a994857a5384e0826dd159c",
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "c133231606c5425da9f090f1a0fe3168",
      );
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setwarrenty_expiring_tablee3168Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setwarrenty_expiring_tablee3168Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'c133231606c5425da9f090f1a0fe3168',
                elementId: 'c133231606c5425da9f090f1a0fe3168'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( warrenty_expiring_tablee3168Props?.presetValues&&Object.keys(warrenty_expiring_tablee3168Props?.presetValues).length > 0) {
          filtertedData = [warrenty_expiring_tablee3168Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setwarrenty_expiring_tablee3168(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (warrenty_expiring_tablee3168Props.filterInitalLoad) return;

    const filterControllers = warrenty_expiring_tablee3168Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['warrenty_expiring_tablee3168']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['warrenty_expiring_tablee3168']).filter((k: string) => jsonEntry['warrenty_expiring_tablee3168'][k] === false)
      : [];
    ...
    setwarrenty_expiring_tablee3168Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(warrenty_expiring_tablee3168Props.filterProps);

  }, [
    warrenty_expiring_tablee3168Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    warrenty_expiring_tablee3168Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(warrenty_expiring_tablee3168Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(warrenty_expiring_tablee3168Props.searchFilter) || {};
...
}, [warrenty_expiring_tablee3168Props.searchFilter])
  ...
  useEffect(() => {
    if (warrenty_expiring_tablee3168Props?.clearData === true) {
      setwarrenty_expiring_tablee3168([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setwarrenty_expiring_tablee3168Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [warrenty_expiring_tablee3168Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [warrenty_expiring_tablee3168Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = warrenty_expiring_tablee3168Props?.selectedIds[warrenty_expiring_tablee3168Props?.selectedIds?.length-1]
    if(Array.isArray(warrenty_expiring_tablee3168) && warrenty_expiring_tablee3168.length>0)
    {
      let data = warrenty_expiring_tablee3168[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (warrenty_expiring_tablee3168Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (warrenty_expiring_tablee3168Props?.selectedIds?.length == 0){
      handleOnRowClick({},warrenty_expiring_tablee3168Props?.selectedIds)
    }
  }, [warrenty_expiring_tablee3168Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(warrenty_expiring_tablee3168?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&warrenty_expiring_tablee3168?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](warrenty_expiring_tablee3168?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: warrenty_expiring_tablee3168?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&warrenty_expiring_tablee3168?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](warrenty_expiring_tablee3168?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: warrenty_expiring_tablee3168?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (warrenty_expiring_tablee3168?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"c133231606c5425da9f090f1a0fe3168"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={warrenty_expiring_tablee3168Props?.selectedIds}  
...
export default Tablewarrenty_expiring_table
