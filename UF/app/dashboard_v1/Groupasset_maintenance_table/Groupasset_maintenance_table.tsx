...
import Tableasset_maintenance_table  from './Tableasset_maintenance_table';  
...
const Groupasset_maintenance_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8c4ebc6912b743a0baa96754e416cdf1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_maintenance_table6cdf1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("ref")){
        setref2b838({...ref2b838,isDisabled:true});

    }else
    {
      if(ref2b838?.isDisabled==null)
      {
        setref2b838({...ref2b838,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("asset_name")){
        setasset_name4d2a5({...asset_name4d2a5,isDisabled:true});

    }else
    {
      if(asset_name4d2a5?.isDisabled==null)
      {
        setasset_name4d2a5({...asset_name4d2a5,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("maint_type")){
        setmaint_typee7fac({...maint_typee7fac,isDisabled:true});

    }else
    {
      if(maint_typee7fac?.isDisabled==null)
      {
        setmaint_typee7fac({...maint_typee7fac,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("description")){
        setdescriptionb10c9({...descriptionb10c9,isDisabled:true});

    }else
    {
      if(descriptionb10c9?.isDisabled==null)
      {
        setdescriptionb10c9({...descriptionb10c9,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vendor_name")){
        setvendor_name73ece({...vendor_name73ece,isDisabled:true});

    }else
    {
      if(vendor_name73ece?.isDisabled==null)
      {
        setvendor_name73ece({...vendor_name73ece,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("scheduled_date")){
        setscheduled_date67fe7({...scheduled_date67fe7,isDisabled:true});

    }else
    {
      if(scheduled_date67fe7?.isDisabled==null)
      {
        setscheduled_date67fe7({...scheduled_date67fe7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cost")){
        setcost3b16a({...cost3b16a,isDisabled:true});

    }else
    {
      if(cost3b16a?.isDisabled==null)
      {
        setcost3b16a({...cost3b16a,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatusc3912({...statusc3912,isDisabled:true});

    }else
    {
      if(statusc3912?.isDisabled==null)
      {
        setstatusc3912({...statusc3912,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("maint_id")){
        setmaint_ida5387({...maint_ida5387,isDisabled:true});

    }else
    {
      if(maint_ida5387?.isDisabled==null)
      {
        setmaint_ida5387({...maint_ida5387,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const asset_maintenance_table6cdf1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    asset_maintenance_table6cdf1Ref.current?.setSearchParams();
    asset_maintenance_table6cdf1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_maintenance_table6cdf1) && Object.keys(asset_maintenance_table6cdf1)?.length>0)
      {
        setasset_maintenance_table6cdf1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_maintenance_table6cdf1Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableasset_maintenance_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={asset_maintenance_table6cdf1Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupasset_maintenance_table
