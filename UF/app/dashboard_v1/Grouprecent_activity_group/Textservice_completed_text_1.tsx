...
const Textservice_completed_text_1 = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {service_completed_text_197211Props, setservice_completed_text_197211Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[service_completed_text_197211?.refresh])

  if (service_completed_text_197211?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `45 / 50`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-1"
  color="primary"
>
      {keyset("Service completed — Personal service")}
</Text>
  </div>
  )
}

export default Textservice_completed_text_1
