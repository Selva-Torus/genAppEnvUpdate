...
const Textamr_queued_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {amr_queued_textb4f27Props, setamr_queued_textb4f27Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[amr_queued_textb4f27?.refresh])

  if (amr_queued_textb4f27?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 15`,gridRow: `7 / 12`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!text-[#71717A] "
  variant="code-inline-1"
>
      {keyset("◉ 10:47 AM  #CAS-2024-0147")}
</Text>
  </div>
  )
}

export default Textamr_queued_text
