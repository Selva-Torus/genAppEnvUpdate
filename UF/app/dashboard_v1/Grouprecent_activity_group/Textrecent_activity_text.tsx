...
const Textrecent_activity_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {recent_activity_text25b7bProps, setrecent_activity_text25b7bProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[recent_activity_text25b7b?.refresh])

  if (recent_activity_text25b7b?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 8`,gridRow: `1 / 6`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!font-bold"
  variant="subheader-2"
  color="primary"
>
      {keyset("Recent Activity")}
</Text>
  </div>
  )
}

export default Textrecent_activity_text
