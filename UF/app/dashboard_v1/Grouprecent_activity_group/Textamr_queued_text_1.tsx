...
const Textamr_queued_text_1 = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {amr_queued_text_1dc178Props, setamr_queued_text_1dc178Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[amr_queued_text_1dc178?.refresh])

  if (amr_queued_text_1dc178?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `13 / 18`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-1"
  color="primary"
>
      {keyset("AMR queued — Marcus T. Williams — Affidavit re-notarization required")}
</Text>
  </div>
  )
}

export default Textamr_queued_text_1
