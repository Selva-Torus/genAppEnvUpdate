...
const Textpending_file_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {pending_file_text86c2aProps, setpending_file_text86c2aProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[pending_file_text86c2a?.refresh])

  if (pending_file_text86c2a?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("PENDING FILINGS")}
</Text>
  </div>
  )
}

export default Textpending_file_text
