...
const Textcourt_rejection = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {court_rejectionff779Props, setcourt_rejectionff779Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_cardsdashboard_v1Props && !dfd_cardsdashboard_v1Props.hasLogicCenter) {
        ...
        setcourt_rejection_groupdf57a((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setcourt_rejection_groupdf57a((pre: any) => ({
          ...pre,
          court_rejections: court_rejectionff779Props?.filteredData?.length > 0
            ? court_rejectionff779Props?.filteredData[0]?.court_rejections
            : "0"
        }))
      }else if(Array.isArray(dfd_cardsdashboard_v1Props) && dfd_cardsdashboard_v1Props && !court_rejection_groupdf57a.court_rejections){
        setcourt_rejection_groupdf57a((pre:any)=>({...pre,court_rejections:dfd_cardsdashboard_v1Props[0]?.court_rejections}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[court_rejectionff779?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_cardsdashboard_v1Props) && !court_rejection_groupdf57a.court_rejections){
    setcourt_rejection_groupdf57a((pre:any)=>({...pre,court_rejections:dfd_cardsdashboard_v1Props[0]?.court_rejections}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!court_rejectionff779Props?.filterProps) return;
    handleMapperValue(court_rejectionff779Props?.filterProps,court_rejectionff779Props?.filterFlag);
  },[court_rejectionff779Props?.filterProps])

  if (court_rejectionff779?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.court_rejections : (court_rejection_groupdf57a?.court_rejections || ""))}
</Text>
  </div>
  )
}
