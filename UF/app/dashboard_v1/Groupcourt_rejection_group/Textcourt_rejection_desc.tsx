...
const Textcourt_rejection_desc = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {court_rejection_desc6f72fProps, setcourt_rejection_desc6f72fProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[court_rejection_desc6f72f?.refresh])

  if (court_rejection_desc6f72f?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("This week need fixes")}
</Text>
  </div>
  )
}

export default Textcourt_rejection_desc
