...
const Groupcourt_rejection_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5d29dd48cc206fa8503de83efa9c9d54");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcourt_rejection_groupc9d54Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("court_rejection_text")){
        setcourt_rejection_text06a86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_text06a86?.isDisabled==null)
      {
        setcourt_rejection_text06a86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("icon")){
        seticona5abd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(icona5abd?.isDisabled==null)
      {
        seticona5abd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection")){
        setcourt_rejection2460a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection2460a?.isDisabled==null)
      {
        setcourt_rejection2460a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection_desc")){
        setcourt_rejection_desc7c63b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_desc7c63b?.isDisabled==null)
      {
        setcourt_rejection_desc7c63b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['court_rejection_text'] = court_rejection_text06a86,
        codeStates['setcourt_rejection_text'] = setcourt_rejection_text06a86,
        codeStates['icon'] = icona5abd,
        codeStates['seticon'] = seticona5abd,
        codeStates['court_rejection'] = court_rejection2460a,
        codeStates['setcourt_rejection'] = setcourt_rejection2460a,
        codeStates['court_rejection_desc'] = court_rejection_desc7c63b,
        codeStates['setcourt_rejection_desc'] = setcourt_rejection_desc7c63b,
  ...
  const court_rejection_groupc9d54Ref = useRef<any>(null);
  const handleClearSearch = () => {
    court_rejection_groupc9d54Ref.current?.setSearchParams();
    court_rejection_groupc9d54Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(court_rejection_groupc9d54) && Object.keys(court_rejection_groupc9d54)?.length>0)
      {
        setcourt_rejection_groupc9d54({})
      }
    }else 
      prevRefreshRef.current= true
  }, [court_rejection_groupc9d54Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !pl-3 !rounded-2xl !bg-white ${isDark ? 'text-white' : 'text-black'}  ${dashboard_v1?._selectedGroup_=="court_rejection_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"court_rejection_group"}))
        }}
    >
          {allowedControls.includes("court_rejection_text") ?<Textcourt_rejection_text   /* 06a86 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("icon")?<Iconicon /* a5abd */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("court_rejection") ?<Textcourt_rejection   /* 2460a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("court_rejection_desc") ?<Textcourt_rejection_desc   /* 7c63b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
