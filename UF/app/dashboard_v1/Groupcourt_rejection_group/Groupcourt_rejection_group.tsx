...
import Textcourt_rejection  from "./Textcourt_rejection";
import Textcourt_rejection_desc  from "./Textcourt_rejection_desc";
...
const Groupcourt_rejection_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "22d56a77560b7876e1bd59db9c6df57a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcourt_rejection_groupdf57aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("court_rejection_text")){
        setcourt_rejection_textc9a86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_textc9a86?.isDisabled==null)
      {
        setcourt_rejection_textc9a86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("icon")){
        seticon87359((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(icon87359?.isDisabled==null)
      {
        seticon87359((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection")){
        setcourt_rejectionff779((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejectionff779?.isDisabled==null)
      {
        setcourt_rejectionff779((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_rejection_desc")){
        setcourt_rejection_desc6f72f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_rejection_desc6f72f?.isDisabled==null)
      {
        setcourt_rejection_desc6f72f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['court_rejection_text'] = court_rejection_textc9a86,
        codeStates['setcourt_rejection_text'] = setcourt_rejection_textc9a86,
        codeStates['icon'] = icon87359,
        codeStates['seticon'] = seticon87359,
        codeStates['court_rejection'] = court_rejectionff779,
        codeStates['setcourt_rejection'] = setcourt_rejectionff779,
        codeStates['court_rejection_desc'] = court_rejection_desc6f72f,
        codeStates['setcourt_rejection_desc'] = setcourt_rejection_desc6f72f,
  ...
  const court_rejection_groupdf57aRef = useRef<any>(null);
  const handleClearSearch = () => {
    court_rejection_groupdf57aRef.current?.setSearchParams();
    court_rejection_groupdf57aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(court_rejection_groupdf57a) && Object.keys(court_rejection_groupdf57a)?.length>0)
      {
        setcourt_rejection_groupdf57a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [court_rejection_groupdf57aProps?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !pl-3 !rounded-2xl !bg-white ${isDark ? 'text-white' : 'text-black'}  ${newdashboard_v1?._selectedGroup_=="court_rejection_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"court_rejection_group"}))
        }}
    >
          {allowedControls.includes("court_rejection_text") ?<Textcourt_rejection_text   /* c9a86 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("icon")?<Iconicon /* 87359 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("court_rejection") ?<Textcourt_rejection   /* ff779 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("court_rejection_desc") ?<Textcourt_rejection_desc   /* 6f72f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupcourt_rejection_group
