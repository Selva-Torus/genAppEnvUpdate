...
import Grouppending_fillings_group  from "../Grouppending_fillings_group/Grouppending_fillings_group";
...
const Groupct006_af_uf_ufws_lap_lap_pendingfilingtable_v1 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupCt006AfUfUfwsLapLapPendingfilingtableV1',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7420d4ad7818489c8ba59fcf938ff8da");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8daProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("pending_fillings_group")){
        setpending_fillings_groupb1568((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_fillings_groupb1568?.isDisabled==null)
      {
        setpending_fillings_groupb1568((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8daRef = useRef<any>(null);
  const handleClearSearch = () => {
    ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8daRef.current?.setSearchParams();
    ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8daRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da) && Object.keys(ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da)?.length>0)
      {
        setct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da({})
      }
    }else 
      prevRefreshRef.current= true
  }, [ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8daProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1"}))
        }}
    >
        {allowedComponent.includes("pending_fillings_group")  &&<Grouppending_fillings_group  
...
export default Groupct006_af_uf_ufws_lap_lap_pendingfilingtable_v1
