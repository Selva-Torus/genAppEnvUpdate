...
const Labeltotal_value_label = ({encryptionFlagCompData,setIsProcessing}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: "cd2bedd58a5546819956f0cb77f9d783",
        controlId: "16bd2508093944eaae03e6925ecd1dce",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  settotal_value_group9d783((pre:any)=>({...pre,total_value_label:""}));
},[total_value_labeld1dce?.refresh])


const handleClick =async(e:any)=>{
  try{
    setIsProcessing(true);
  settotal_value_group9d783((prev: any) => ({ ...prev, total_value_label: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (total_value_labeld1dce?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `6 / 16`,gridRow: `1 / 8`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className="p-1 !text-white font-bold"
        disabled= {total_value_labeld1dce?.isDisabled ? true : false}
        theme="clear"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labeltotal_value_label
