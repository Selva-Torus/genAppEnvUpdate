...
import Icontotal_value_icon  from "./Icontotal_value_icon";
import Labeltotal_value_label  from "./Labeltotal_value_label";
import Texttotal_amount  from "./Texttotal_amount";
...
const Grouptotal_value_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cd2bedd58a5546819956f0cb77f9d783");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_value_group9d783Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_value_icon")){
        settotal_value_icon16d5d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_value_icon16d5d?.isDisabled==null)
      {
        settotal_value_icon16d5d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_value_label")){
        settotal_value_labeld1dce((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_value_labeld1dce?.isDisabled==null)
      {
        settotal_value_labeld1dce((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_amount")){
        settotal_amount94c0b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_amount94c0b?.isDisabled==null)
      {
        settotal_amount94c0b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_value_icon'] = total_value_icon16d5d,
        codeStates['settotal_value_icon'] = settotal_value_icon16d5d,
        codeStates['total_value_label'] = total_value_labeld1dce,
        codeStates['settotal_value_label'] = settotal_value_labeld1dce,
  ...
  const total_value_group9d783Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_value_group9d783Ref.current?.setSearchParams();
    total_value_group9d783Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(total_value_group9d783) &&
        Object.keys(total_value_group9d783)?.length > 0
      ) {
        settotal_value_group9d783({})
      }
    } else prevRefreshRef.current = true
  }, [total_value_group9d783Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"total_value_group"}))
        }}
    >
        {allowedControls.includes("total_value_icon")?<Icontotal_value_icon /* 16d5d */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("total_value_label")?<Labeltotal_value_label   /* d1dce */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_amount") ?<Texttotal_amount   /* 94c0b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouptotal_value_group
