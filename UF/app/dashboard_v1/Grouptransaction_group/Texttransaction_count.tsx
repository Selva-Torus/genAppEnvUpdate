...
const Texttransaction_count = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_transactioncountvphdashboard_v1Props && !dfd_transactioncountvphdashboard_v1Props.hasLogicCenter) {
        ...
        const api_paginationData: any = await AxiosService.post('/UF/pagination',
          {
            key: dfd_transactioncountvphdashboard_v1Props.dstKey,
            page: 1,
            count: 1,
        ...
        settransaction_group6c6f2((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        settransaction_group6c6f2((pre: any) => ({
          ...pre,
          total_count: transaction_countc3fd5Props?.filteredData?.length > 0
            ? transaction_countc3fd5Props?.filteredData[0]?.total_count
            : "0"
        }))
      }else if(Array.isArray(dfd_transactioncountvphdashboard_v1Props) && dfd_transactioncountvphdashboard_v1Props && !transaction_group6c6f2.total_count){
        settransaction_group6c6f2((pre:any)=>({...pre,total_count:dfd_transactioncountvphdashboard_v1Props[0]?.total_count}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[transaction_countc3fd5?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_transactioncountvphdashboard_v1Props) && !transaction_group6c6f2.total_count){
    settransaction_group6c6f2((pre:any)=>({...pre,total_count:dfd_transactioncountvphdashboard_v1Props[0]?.total_count}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!transaction_countc3fd5Props?.filterProps) return;
    handleMapperValue(transaction_countc3fd5Props?.filterProps,transaction_countc3fd5Props?.filterFlag);
  },[transaction_countc3fd5Props?.filterProps])

  if (transaction_countc3fd5?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_count : (transaction_group6c6f2?.total_count || ""))}
</Text>
  </div>
  )
}

export default Texttransaction_count
