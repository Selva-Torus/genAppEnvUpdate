...
const Labeltransactions_label = ({encryptionFlagCompData,setIsProcessing}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: "793a6b161bb2485c995842c573d6c6f2",
        controlId: "27bc28afcdef44208b6f8c6ccecf64b1",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  settransaction_group6c6f2((pre:any)=>({...pre,transactions_label:""}));
},[transactions_labelf64b1?.refresh])


const handleClick =async(e:any)=>{
  try{
    setIsProcessing(true);
  settransaction_group6c6f2((prev: any) => ({ ...prev, transactions_label: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (transactions_labelf64b1?.isHidden) {
    return <></>
  }  
  ...
  return (
    <div 
      ...
      <Label 
        ...
        disabled= {transactions_labelf64b1?.isDisabled ? true : false}
        theme="clear"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labeltransactions_label
