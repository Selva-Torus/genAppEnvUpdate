...
let defaultColumns:any = [
  {
    "id": "case_display_id",
    "nodeid": "b595eb6955af40e2a530a63dab832ae4",
    "name": "Case ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "debtor_name",
    "nodeid": "1f1d19199c3246a2bb3ce02e0f6cd77b",
    "name": "Debtor Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "creditor_name",
    "nodeid": "2c8909211e654a52ba489282ab5b14ce",
    "name": "Creditor Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "full_name",
    "nodeid": "8b353b211f624b70bd444df8662592f3",
  ...
  {
    "id": "total_balance",
    "nodeid": "21498734d7694eed92706259dd898c00",
    "name": "Total Balance",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "court_name",
    "nodeid": "6f4f3dec38264ad0b97715d28084b97b",
    "name": "Venue",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "priority_name",
    "nodeid": "0b960ed1afbe4a53b4358044592c61c4",
    "name": "Priority",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "status_name",
    "nodeid": "4fdf88ede74c4363830cfcc1daac590e",
    "name": "Status",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "f21d87c794f34c04870c1f3543b4e909",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData,onSelectLock,currentSelectedIds,skipUnlockRef,tableName}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.pending_fillings_table && artifactRuleState?.pending_fillings_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.pending_fillings_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,pending_fillings_table:filteredData},{pending_fillings_table:artifactRuleState?.pending_fillings_table})
          if(result?.pending_fillings_table?.[controller]?.show==true)
            return true
          else
            return false
        }
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "ff94621c3e014341b5c0038c37011279",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, pending_fillings_table11279 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, pending_fillings_table11279 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, pending_fillings_table11279 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setpending_fillings_table11279Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      pending_fillings_table11279.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setpending_fillings_table11279Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setpending_fillings_table11279Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setpending_fillings_table11279Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'ff94621c3e014341b5c0038c37011279',
                elementId: 'ff94621c3e014341b5c0038c37011279'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( pending_fillings_table11279Props?.presetValues&&Object.keys(pending_fillings_table11279Props?.presetValues).length > 0) {
          filtertedData = [pending_fillings_table11279Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setpending_fillings_table11279(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    ...
    return <RowActionComponent
      ...
      currentSelectedIds={pending_fillings_table11279Props?.selectedIds}
      skipUnlockRef={skipUnlockRef}
      tableName={tableName}
    />
  }, [allData, setRefetch, encryptionFlagCompData, pending_fillings_table11279Props?.selectedIds]);
////////////////////////
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (pending_fillings_table11279Props.filterInitalLoad) return;

    const filterControllers = pending_fillings_table11279Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['pending_fillings_table11279']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['pending_fillings_table11279']).filter((k: string) => jsonEntry['pending_fillings_table11279'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(pending_fillings_table11279Props.filterProps);
      return;
    }
    ...
    setpending_fillings_table11279Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(pending_fillings_table11279Props.filterProps);

  }, [
    pending_fillings_table11279Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    pending_fillings_table11279Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(pending_fillings_table11279Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(pending_fillings_table11279Props.searchFilter) || {};
...
}, [pending_fillings_table11279Props.searchFilter])
  ...
  useEffect(() => {
    if (pending_fillings_table11279Props?.clearData === true) {
      setpending_fillings_table11279([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setpending_fillings_table11279Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [pending_fillings_table11279Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [pending_fillings_table11279Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = pending_fillings_table11279Props?.selectedIds[pending_fillings_table11279Props?.selectedIds?.length-1]
    if(Array.isArray(pending_fillings_table11279) && pending_fillings_table11279.length>0)
    {
      let data = pending_fillings_table11279.find((data:any)=>(data?.account_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "account_id",
  ...
  useEffect(() => {
    if (pending_fillings_table11279Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (pending_fillings_table11279Props?.selectedIds?.length == 0){
      handleOnRowClick({},pending_fillings_table11279Props?.selectedIds)
    }
  }, [pending_fillings_table11279Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(pending_fillings_table11279?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&pending_fillings_table11279?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](pending_fillings_table11279?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: pending_fillings_table11279?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&pending_fillings_table11279?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](pending_fillings_table11279?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: pending_fillings_table11279?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (pending_fillings_table11279?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"ff94621c3e014341b5c0038c37011279"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableSorting={true}
              disable={disable}
              selectedIds={pending_fillings_table11279Props?.selectedIds}  
...
export default Tablepending_fillings_table
