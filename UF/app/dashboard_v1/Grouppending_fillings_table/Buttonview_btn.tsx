...
import PageViewcasepage16 from '@/app/viewcase_v1/viewcase_v1page';
...
const Buttonview_btn = ({ mainData,lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData,onSelectLock,rowIndex,currentSelectedIds,skipUnlockRef,tableName}: { mainData:any,lockedData:any,setLockedData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any,onSelectLock?:any,rowIndex?:number,currentSelectedIds?:string[],skipUnlockRef?:React.MutableRefObject<boolean>,tableName?:string}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "51f53410bad3497aa31969832cc88178"
      );
  ...
  const handleClick=async()=>{
    ...
    //bindTran
    // For group or table
    let bindData10 = filterByKeys(mainData,debtor_information_groupdfa55Props?.controls);
    setdebtor_information_groupdfa55(bindData10||{})
    setdebtor_information_groupdfa55Props({...debtor_information_groupdfa55Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData12 = filterByKeys(mainData,financial_details_grouped0d9Props?.controls);
    setfinancial_details_grouped0d9(bindData12||{})
    setfinancial_details_grouped0d9Props({...financial_details_grouped0d9Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData14 = filterByKeys(mainData,financial_details_grouped0d9Props?.controls);
    setfinancial_details_grouped0d9(bindData14||{})
    setfinancial_details_grouped0d9Props({...financial_details_grouped0d9Props,presetValues:{...(mainData||{})}})
    // showArtifactAsModal
    let filterProps16:any =  [];
    let filterData16 = await getFilterProps(filterProps16,mainData);
    setviewcase_v1Props([...filterData16 ]);
  setAssetDataReady(false);          
    setShowProfileAsModalOpen16(true);
    //bindTran
    // For group or table
    let bindData18 = filterByKeys(mainData,doc_tablee79c7Props?.controls);
    setdoc_tablee79c7(bindData18||{})
    setdoc_tablee79c7Props({...doc_tablee79c7Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData20 = filterByKeys(mainData,doc_tablee79c7Props?.controls);
    setdoc_tablee79c7(bindData20||{})
    setdoc_tablee79c7Props({...doc_tablee79c7Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData22 = filterByKeys(mainData,required_dociument_main_group3eb5bProps?.controls);
    setrequired_dociument_main_group3eb5b(bindData22||{})
    setrequired_dociument_main_group3eb5bProps({...required_dociument_main_group3eb5bProps,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData24 = filterByKeys(mainData,georgias_groupbac01Props?.controls);
    setgeorgias_groupbac01(bindData24||{})
    setgeorgias_groupbac01Props({...georgias_groupbac01Props,presetValues:{...(mainData||{})}})
  ...
  return (
    ...
    >
      ...
      <Modal 
        ...
        onClose={() => setShowProfileAsModalOpen16(false)}
        title="View Case"
        variant="header-1"
      ...
      >
        <PageViewcasepage16  onReady={handleAssetPageReady}/>
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          disabled= {view_btn88178?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
