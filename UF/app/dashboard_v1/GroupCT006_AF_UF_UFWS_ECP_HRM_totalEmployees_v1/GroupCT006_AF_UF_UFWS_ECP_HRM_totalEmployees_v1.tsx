...
import Groupemployee_table_group  from "../Groupemployee_table_group/Groupemployee_table_group";
...
const Groupct006_af_uf_ufws_ecp_hrm_totalemployees_v1 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupCt006AfUfUfwsEcpHrmTotalemployeesV1',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "db7ed2aadef847b3b5ac1704e75f547f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("employee_table_group")){
        setemployee_table_group55008((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employee_table_group55008?.isDisabled==null)
      {
        setemployee_table_group55008((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547fRef = useRef<any>(null);
  const handleClearSearch = () => {
    ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547fRef.current?.setSearchParams();
    ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547f) && Object.keys(ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547f)?.length>0)
      {
        setct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [ct006_af_uf_ufws_ecp_hrm_totalemployees_v1f547fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"ct006_af_uf_ufws_ecp_hrm_totalemployees_v1"}))
        }}
    >
        {allowedComponent.includes("employee_table_group")  &&<Groupemployee_table_group  
...
export default Groupct006_af_uf_ufws_ecp_hrm_totalemployees_v1
