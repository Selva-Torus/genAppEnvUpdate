...
const Groupasset_table_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "4e9b81968e40440ab3ebce3b5756fffa");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_table_group6fffaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("asset_table")){
        setasset_table6082a({...asset_table6082a,isDisabled:true});

    }else
    {
      if(asset_table6082a?.isDisabled==null)
      {
        setasset_table6082a({...asset_table6082a,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_table_group6fffaRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_table_group6fffaRef.current?.setSearchParams();
    asset_table_group6fffaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_table_group6fffa) && Object.keys(asset_table_group6fffa)?.length>0)
      {
        setasset_table_group6fffa({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_table_group6fffaProps?.refresh,token])
...
export default Groupasset_table_group
