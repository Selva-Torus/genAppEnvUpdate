...
import Dividerwarranty_expiring_divider  from "./Dividerwarranty_expiring_divider";
import Textwarranty_expiring_text  from "./Textwarranty_expiring_text";
import Textwarranty_expiring  from "./Textwarranty_expiring";
import Textwarranty_description  from "./Textwarranty_description";
...
const Groupwarranty_expiring_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  let code:any = `settable_group((pre)=>({status:"warranty_expiring_group"}))`;
  let idx = "";
  let item = "";
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setwarranty_expiring_groupb5bd4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warranty_expiring_divider")){
        setwarranty_expiring_dividerf7152({...warranty_expiring_dividerf7152,isDisabled:true});

    }else
    {
      if(warranty_expiring_dividerf7152?.isDisabled==null)
      {
        setwarranty_expiring_dividerf7152({...warranty_expiring_dividerf7152,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("warranty_expiring_text")){
        setwarranty_expiring_texteee70({...warranty_expiring_texteee70,isDisabled:true});

    }else
    {
      if(warranty_expiring_texteee70?.isDisabled==null)
      {
        setwarranty_expiring_texteee70({...warranty_expiring_texteee70,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("warranty_expiring")){
        setwarranty_expiringfa250({...warranty_expiringfa250,isDisabled:true});

    }else
    {
      if(warranty_expiringfa250?.isDisabled==null)
      {
        setwarranty_expiringfa250({...warranty_expiringfa250,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("warranty_description")){
        setwarranty_description49c85({...warranty_description49c85,isDisabled:true});

    }else
    {
      if(warranty_description49c85?.isDisabled==null)
      {
        setwarranty_description49c85({...warranty_description49c85,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warranty_expiring_divider'] = warranty_expiring_dividerf7152,
        codeStates['setwarranty_expiring_divider'] = setwarranty_expiring_dividerf7152,
        codeStates['warranty_expiring_text'] = warranty_expiring_texteee70,
        codeStates['setwarranty_expiring_text'] = setwarranty_expiring_texteee70,
        codeStates['warranty_expiring'] = warranty_expiringfa250,
        codeStates['setwarranty_expiring'] = setwarranty_expiringfa250,
        codeStates['warranty_description'] = warranty_description49c85,
        codeStates['setwarranty_description'] = setwarranty_description49c85,
  ...
  const warranty_expiring_groupb5bd4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    warranty_expiring_groupb5bd4Ref.current?.setSearchParams();
    warranty_expiring_groupb5bd4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(warranty_expiring_groupb5bd4) && Object.keys(warranty_expiring_groupb5bd4)?.length>0)
      {
        setwarranty_expiring_groupb5bd4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [warranty_expiring_groupb5bd4Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("warranty_expiring_divider") ?<Dividerwarranty_expiring_divider   /* f7152 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("warranty_expiring_text") ?<Textwarranty_expiring_text   /* eee70 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("warranty_expiring") ?<Textwarranty_expiring   /* fa250 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("warranty_description") ?<Textwarranty_description   /* 49c85 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupwarranty_expiring_group
