...
const Textwarranty_expiring = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setwarranty_expiring_groupb5bd4((pre: any) => {
          return { ...pre, warranty_expiring: api_paginationData.data.records[0]?.warranty_expiring }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !warranty_expiring_groupb5bd4.warranty_expiring){
        setwarranty_expiring_groupb5bd4((pre:any)=>({...pre,warranty_expiring:dfd_assetdashboard_v1Props[0]?.warranty_expiring}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[warranty_expiringfa250?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !warranty_expiring_groupb5bd4.warranty_expiring){
    setwarranty_expiring_groupb5bd4((pre:any)=>({...pre,warranty_expiring:dfd_assetdashboard_v1Props[0]?.warranty_expiring}));
  }
  },[dfd_assetdashboard_v1Props])

  if (warranty_expiringfa250?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.warranty_expiring : (warranty_expiring_groupb5bd4?.warranty_expiring || ""))}
</Text>
  </div>
  )
}

export default Textwarranty_expiring
