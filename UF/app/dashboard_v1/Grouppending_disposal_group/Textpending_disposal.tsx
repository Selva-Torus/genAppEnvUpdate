...
const Textpending_disposal = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setpending_disposal_group2580d((pre: any) => {
          return { ...pre, pending_disposal: api_paginationData.data.records[0]?.pending_disposal }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !pending_disposal_group2580d.pending_disposal){
        setpending_disposal_group2580d((pre:any)=>({...pre,pending_disposal:dfd_assetdashboard_v1Props[0]?.pending_disposal}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[pending_disposal38551?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !pending_disposal_group2580d.pending_disposal){
    setpending_disposal_group2580d((pre:any)=>({...pre,pending_disposal:dfd_assetdashboard_v1Props[0]?.pending_disposal}));
  }
  },[dfd_assetdashboard_v1Props])

  if (pending_disposal38551?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.pending_disposal : (pending_disposal_group2580d?.pending_disposal || ""))}
</Text>
  </div>
  )
}

export default Textpending_disposal
