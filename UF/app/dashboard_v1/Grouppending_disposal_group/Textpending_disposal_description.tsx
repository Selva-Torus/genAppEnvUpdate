...
const Textpending_disposal_description = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setpending_disposal_group2580d((pre: any) => {
          return { ...pre, pending_disposal_description: api_paginationData.data.records[0]?.pending_disposal_description }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !pending_disposal_group2580d.pending_disposal_description){
        setpending_disposal_group2580d((pre:any)=>({...pre,pending_disposal_description:dfd_assetdashboard_v1Props[0]?.pending_disposal_description}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[pending_disposal_descriptionceadc?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !pending_disposal_group2580d.pending_disposal_description){
    setpending_disposal_group2580d((pre:any)=>({...pre,pending_disposal_description:dfd_assetdashboard_v1Props[0]?.pending_disposal_description}));
  }
  },[dfd_assetdashboard_v1Props])

  if (pending_disposal_descriptionceadc?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.pending_disposal_description : (pending_disposal_group2580d?.pending_disposal_description || ""))}
</Text>
  </div>
  )
}

export default Textpending_disposal_description
