...
import Textemp_head_text  from "./Textemp_head_text";
...
const Groupemp_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {emp_head_textf66e8, setemp_head_textf66e8}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e06c112417824b42bbdd9e9ae965e40b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemp_group5e40bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("emp_head_text")){
        setemp_head_textf66e8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emp_head_textf66e8?.isDisabled==null)
      {
        setemp_head_textf66e8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['emp_head_text'] = emp_head_textf66e8,
        codeStates['setemp_head_text'] = setemp_head_textf66e8,
  ...
  const emp_group5e40bRef = useRef<any>(null);
  const handleClearSearch = () => {
    emp_group5e40bRef.current?.setSearchParams();
    emp_group5e40bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(emp_group5e40b) && Object.keys(emp_group5e40b)?.length>0)
      {
        setemp_group5e40b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [emp_group5e40bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"emp_group"}))
        }}
    >
          {allowedControls.includes("emp_head_text") ?<Textemp_head_text   /* f66e8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
