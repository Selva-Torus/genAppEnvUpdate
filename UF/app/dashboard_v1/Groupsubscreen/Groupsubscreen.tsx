...
import GroupCT006_AF_UF_UFWS_LAP_LAP_amrQueueTable_v1  from "../GroupCT006_AF_UF_UFWS_LAP_LAP_amrQueueTable_v1/GroupCT006_AF_UF_UFWS_LAP_LAP_amrQueueTable_v1";
import GroupCT006_AF_UF_UFWS_LAP_LAP_pendingFilingTable_v1  from "../GroupCT006_AF_UF_UFWS_LAP_LAP_pendingFilingTable_v1/GroupCT006_AF_UF_UFWS_LAP_LAP_pendingFilingTable_v1";
...
const Groupsubscreen = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "09a165b7494b38fcb00ec2c047ce9ab5");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsubscreene9ab5Props((pre:any)=>({...pre,isHaveRule:true}))
    let schemaFlag:any = evaluateDecisionTable(orchestrationData?.data?.rule.nodes,{},{...decodedTokenObj,...table_group112bd});
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("ct006_af_uf_ufws_lap_lap_amrqueuetable_v1")){
        setct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797?.isDisabled==null)
      {
        setct006_af_uf_ufws_lap_lap_amrqueuetable_v18a797((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1")){
        setct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da?.isDisabled==null)
      {
        setct006_af_uf_ufws_lap_lap_pendingfilingtable_v1ff8da((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const subscreene9ab5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    subscreene9ab5Ref.current?.setSearchParams();
    subscreene9ab5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(subscreene9ab5) && Object.keys(subscreene9ab5)?.length>0)
      {
        setsubscreene9ab5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [subscreene9ab5Props?.refresh,token,
table_group112bd[uoMapperData["576e82efe1bb7979dc64ab024f377d60"]["source"]]
])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"subscreen"}))
        }}
    >
        {(showFlag == "ct006_af_uf_ufws_lap_lap_amrqueuetable_v1") &&allowedComponent.includes("ct006_af_uf_ufws_lap_lap_amrqueuetable_v1")  &&<GroupCT006_AF_UF_UFWS_LAP_LAP_amrQueueTable_v1  
        ...
        {(showFlag == "ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1") &&allowedComponent.includes("ct006_af_uf_ufws_lap_lap_pendingfilingtable_v1")  &&<GroupCT006_AF_UF_UFWS_LAP_LAP_pendingFilingTable_v1  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData} 
          setTableData={setTableData}
