...
import Groupasset_maintenance_table  from "../Groupasset_maintenance_table/Groupasset_maintenance_table";
...
const Groupasset_maintenance_table_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupAssetMaintenanceTableGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bf36377a25574c61861baf3bea2e042b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_maintenance_table_groupe042bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("asset_maintenance_table")){
        setasset_maintenance_table6cdf1({...asset_maintenance_table6cdf1,isDisabled:true});

    }else
    {
      if(asset_maintenance_table6cdf1?.isDisabled==null)
      {
        setasset_maintenance_table6cdf1({...asset_maintenance_table6cdf1,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_maintenance_table_groupe042bRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_maintenance_table_groupe042bRef.current?.setSearchParams();
    asset_maintenance_table_groupe042bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_maintenance_table_groupe042b) && Object.keys(asset_maintenance_table_groupe042b)?.length>0)
      {
        setasset_maintenance_table_groupe042b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_maintenance_table_groupe042bProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("asset_maintenance_table")  &&<Groupasset_maintenance_table  
...
export default Groupasset_maintenance_table_group
