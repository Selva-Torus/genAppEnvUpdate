...
export default function ComboBoxproduct_combobox({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "438df3179fed48388c0f8daf6e37ef64"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    let dstKey0:string = dfd_productdashboard_v1Props.dstKey;
    if (isFromEvent) {
      let paginationBody={}
      if(isFromEvent)
      {
        let temp="CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:productDashboard:AFVK:v1:"
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({e_product:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.product_code + '|' + item.e_product, item])).values()
        );
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        settab_grpe63f4((pre:any)=>({...pre,product_code:records[0]?.product_code}))
        setselectedData(records[0]?.e_product)
      }else
      {
        let temp:any = [...dynamicDFDData,...records]
        const unique = Array.from(
      ...
      if(prevRefreshRef.current==false) // prevent onload data get
        setDynamicDFDData(dfd_productdashboard_v1Props );
    }
  }
//setSearchFilters
async function handleSearch0(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), product_combobox: value})
    ...
    "nodeBasedData": [
      {
        "nodeId": "ccfeff99b47a41a1b1fc2a6a55c13247",
        "object": {
          "properties.trs_product_code": "438df3179fed48388c0f8daf6e37ef64"
        }
      }
    ]
  }
...
async function handleSearch1(value:any) {
   ...
   let spreadedValues:any = [
      {
        ...
        nodeBasedData: [                          {
                            "nodeId":"96586de5c9854f02aee370579b276c2a",
                            "object":{
                              ...temp
                            }
                          },
    ...
    "nodeBasedData": [
      {
        "nodeId": "96586de5c9854f02aee370579b276c2a",
        "object": {
          "properties.name": "438df3179fed48388c0f8daf6e37ef64"
        }
      }
    ]
  }
  ...
  filterData=nullFilter(filterData);
  if (piechart9dde7Props?.filterProps?.length) {
    piechart9dde7Props.filterProps.forEach((existingItem: any) => {
      const matchIndex = filterData.findIndex(
        (f: any) => f.nodeId === existingItem.nodeId
      );
      if (matchIndex >= 0) {
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  setpiechart9dde7Props((prev: any) => {
  let mergedData: any;
...
//setSearchFilters
async function handleSearch3(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), product_combobox: value})
  let temp: any = {}, filteredData: any = [], filterFlag = true;
  tab_grpe63f4Props?.needToSpread?.map((keys:any)=>{
    delete mainData?.[keys]
...
//setSearchFilters
async function handleSearch4(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), product_combobox: value})
  let temp: any = {}, filteredData: any = [], filterFlag = true;
  tab_grpe63f4Props?.needToSpread?.map((keys:any)=>{
    delete mainData?.[keys]
   ...
   let spreadedValues:any = [
      {
        ...
        nodeBasedData: [                          {
                            "nodeId":"0de07f85d9a444c5a3caae508fb938ae",
                            "object":{
                              ...temp
                            }
                          },
  ...
  filterData=nullFilter(filterData);
  if (barchart84262Props?.filterProps?.length) {
    barchart84262Props.filterProps.forEach((existingItem: any) => {
      const matchIndex = filterData.findIndex(
        (f: any) => f.nodeId === existingItem.nodeId
      );
      if (matchIndex >= 0) {
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  setbarchart84262Props((prev: any) => {
  let mergedData: any;
...
//setSearchFilters
async function handleSearch5(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), product_combobox: value})
  let temp: any = {}, filteredData: any = [], filterFlag = true;
  tab_grpe63f4Props?.needToSpread?.map((keys:any)=>{
    delete mainData?.[keys]
    ...
    "nodeBasedData": [
      {
        "nodeId": "c2a320f6a30140a487ed20c46f1763dd",
        "object": {
          "properties.trs_process_code": "438df3179fed48388c0f8daf6e37ef64"
        }
      }
    ]
  }
  ...
  filterData=nullFilter(filterData);
  if (transaction_tablef4f34Props?.filterProps?.length) {
    transaction_tablef4f34Props.filterProps.forEach((existingItem: any) => {
      const matchIndex = filterData.findIndex(
        (f: any) => f.nodeId === existingItem.nodeId
      );
      if (matchIndex >= 0) {
  ...
  settransaction_tablef4f34Props((prev: any) => {
  let mergedData: any;
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    settab_grpe63f4((pre:any)=>({...pre,product_combobox:data?.product_code}))
    setselectedData(data?.e_product)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.product_code == data?.product_code && items?.e_product == data?.e_product)) || {};  
    try{
    setIsProcessing(true);
    let filterValue = data?.product_code;

    await  handleSearch0(filterValue);
    await  handleSearch1(filterValue);
    await  handleSearch2(filterValue);
    await  handleSearch3(filterValue);
    await  handleSearch4(filterValue);
    await  handleSearch5(filterValue);
  ...
  useEffect(() => { tab_grpe63f4Ref.current = tab_grpe63f4; }, [tab_grpe63f4]);
    ...
    const handler = (id:any) => {
      if (id === "438df3179fed48388c0f8daf6e37ef64") {
        handleOnUpdate({
          value:selectedData,
          text:tab_grpe63f4Ref?.current?.product_combobox||""
        });
      }
    };
    ...
    useEffect(()=>{
    if(product_combobox7ef64?.trigger===undefined || !product_combobox7ef64?.trigger) return;
    handleOnUpdate({product_combobox:tab_grpe63f4Ref?.current?.product_combobox||""});
  },[product_combobox7ef64?.trigger])

  useEffect(()=>{
    if(tab_grpe63f4?.product_combobox=="")
    {
      setselectedData('')
    }else
    {

      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.e_product==(selectedData||tab_grpe63f4?.product_combobox)))?.e_product || tab_grpe63f4?.product_combobox;
      setselectedData(formBindedData)
    }
  },[tab_grpe63f4?.product_combobox])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {product_combobox7ef64?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
