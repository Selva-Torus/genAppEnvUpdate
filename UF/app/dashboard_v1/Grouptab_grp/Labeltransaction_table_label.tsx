...
const Labeltransaction_table_label = ({encryptionFlagCompData,setIsProcessing}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "1d69d4e989fa4169ac019e0b3c09d37f",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  settab_grpe63f4((pre:any)=>({...pre,transaction_table_label:""}));
},[transaction_table_label9d37f?.refresh])


const handleClick =async(e:any)=>{
  try{
    setIsProcessing(true);
  settab_grpe63f4((prev: any) => ({ ...prev, transaction_table_label: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (transaction_table_label9d37f?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `1 / 6`,gridRow: `136 / 145`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {transaction_table_label9d37f?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labeltransaction_table_label
