...
export default function ComboBoxcurrency_combobox({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a15a7f68c3bc49daa1aa666fe64fbbfc"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    let dstKey0:string = dfd_currencydashboard_v1Props.dstKey;
    if (isFromEvent) {
      let paginationBody={}
      if(isFromEvent)
      {
        let temp="CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:currencyDashboard:AFVK:v1:"
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({currency_desc:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.currency_code + '|' + item.currency_desc, item])).values()
        );
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        settab_grpe63f4((pre:any)=>({...pre,currency_code:records[0]?.currency_code}))
        setselectedData(records[0]?.currency_desc)
      }else
      {
        let temp:any = [...dynamicDFDData,...records]
        const unique = Array.from(
      ...
      if(prevRefreshRef.current==false) // prevent onload data get
        setDynamicDFDData(dfd_currencydashboard_v1Props );
    }
  }
//setSearchFilters
async function handleSearch0(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), currency_combobox: value})
    ...
    "nodeBasedData": [
      {
        "nodeId": "ccfeff99b47a41a1b1fc2a6a55c13247",
        "object": {
          "properties.cr_currency": "a15a7f68c3bc49daa1aa666fe64fbbfc"
        }
      }
    ]
  }
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    settab_grpe63f4((pre:any)=>({...pre,currency_combobox:data?.currency_code}))
    setselectedData(data?.currency_desc)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.currency_code == data?.currency_code && items?.currency_desc == data?.currency_desc)) || {};  
    try{
    setIsProcessing(true);
    let filterValue = data?.currency_code;
  ...
  useEffect(() => { tab_grpe63f4Ref.current = tab_grpe63f4; }, [tab_grpe63f4]);
    ...
    const handler = (id:any) => {
      if (id === "a15a7f68c3bc49daa1aa666fe64fbbfc") {
        handleOnUpdate({
          value:selectedData,
          text:tab_grpe63f4Ref?.current?.currency_combobox||""
        });
      }
    };
    ...
    useEffect(()=>{
    if(currency_comboboxfbbfc?.trigger===undefined || !currency_comboboxfbbfc?.trigger) return;
    handleOnUpdate({currency_combobox:tab_grpe63f4Ref?.current?.currency_combobox||""});
  },[currency_comboboxfbbfc?.trigger])

  useEffect(()=>{
    if(tab_grpe63f4?.currency_combobox=="")
    {
      setselectedData('')
    }else
    {

      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.currency_desc==(selectedData||tab_grpe63f4?.currency_combobox)))?.currency_desc || tab_grpe63f4?.currency_combobox;
      setselectedData(formBindedData)
    }
  },[tab_grpe63f4?.currency_combobox])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {currency_comboboxfbbfc?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        onBlur={handleBlur}
        isStatic={false}
        placeholder={"Search currency_desc..."}
        value={selectedData}
        onChange={handleOnUpdate}

        toSave="currency_code"
        toDisplay="currency_desc"
        isArray={false}
        isMultiple={false}
