...
export default function ComboBoxprocess_category_combobox({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "cb717ae102b04801b33b236b5e3bb731"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    let dstKey0:string = dfd_processcategorydashboard_v1Props.dstKey;
    if (isFromEvent) {
      let paginationBody={}
      if(isFromEvent)
      {
        let temp="CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:processCategoryDashboard:AFVK:v1:"
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({process_type:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.process_type + '|' + item.process_type, item])).values()
        );
      ...
      if(prevRefreshRef.current==false) // prevent onload data get
        setDynamicDFDData(dfd_processcategorydashboard_v1Props );
    }
  }
//setSearchFilters
async function handleSearch0(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), process_category_combobox: value})
    ...
    "nodeBasedData": [
      {
        "nodeId": "ccfeff99b47a41a1b1fc2a6a55c13247",
        "object": {
          "properties.process_type": "cb717ae102b04801b33b236b5e3bb731"
        }
      }
    ]
  }
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  return {
    ...
    filterControllers: {
      ...
      process_category_comboboxbb731: true
    }
  };
  });
  }
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    settab_grpe63f4((pre:any)=>({...pre,process_category_combobox:data?.process_type}))
    setselectedData(data?.process_type)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.process_type == data?.process_type && items?.process_type == data?.process_type)) || {};  
    try{
    setIsProcessing(true);
    let filterValue = data?.process_type;
  ...
  useEffect(() => { tab_grpe63f4Ref.current = tab_grpe63f4; }, [tab_grpe63f4]);
    ...
    const handler = (id:any) => {
      if (id === "cb717ae102b04801b33b236b5e3bb731") {
        handleOnUpdate({
          value:selectedData,
          text:tab_grpe63f4Ref?.current?.process_category_combobox||""
        });
      }
    };
    ...
    useEffect(()=>{
    if(process_category_comboboxbb731?.trigger===undefined || !process_category_comboboxbb731?.trigger) return;
    handleOnUpdate({process_category_combobox:tab_grpe63f4Ref?.current?.process_category_combobox||""});
  },[process_category_comboboxbb731?.trigger])

  useEffect(()=>{
    if(tab_grpe63f4?.process_category_combobox=="")
    {
      setselectedData('')
    }else
    {

      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.process_type==(selectedData||tab_grpe63f4?.process_category_combobox)))?.process_type || tab_grpe63f4?.process_category_combobox;
      setselectedData(formBindedData)
    }
  },[tab_grpe63f4?.process_category_combobox])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {process_category_comboboxbb731?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
