...
export default function ComboBoxchannel_combobox({encryptionFlagCompData,setIsProcessing,controlData}:any) { 
 ...
 /////////////
  ...
  const handleOrchestration=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9dc2591b08b04b8c9d28963697b95649"
      );
  ...
  const getDropdownData = async(count?:any, page: number = 1,searchValue?:string,isFromEvent?:boolean,fromWhere?:string,eventFilterDataParam? : any)=>{
    let dstKey0:string = dfd_channeldashboard_v1Props.dstKey;
    if (isFromEvent) {
      let paginationBody={}
      if(isFromEvent)
      {
        let temp="CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:channelDashboard:AFVK:v1:"
      ...
      if(searchValue!=""||isFromEvent)
      {
        ...
        let tempSearchFilter=nullFilter({channel:searchValue,...getFromDataFilter})
      ...
      else if(fromWhere=="onScroll")
      {
        ...
        const unique = Array.from(
          new Map(temp.map((item:any) => [item.channel + '|' + item.channel, item])).values()
        );
      ...
      if(prevRefreshRef.current==false) // prevent onload data get
        setDynamicDFDData(dfd_channeldashboard_v1Props );
    }
  }
//setSearchFilters
async function handleSearch0(value:any) {
  let mainData: any =  nullFilter({...structuredClone(tab_grpe63f4), channel_combobox: value})
    ...
    "nodeBasedData": [
      {
        "nodeId": "4694b4cd0ea34d0db77992bbda019635",
        "object": {
          "properties.channel_name": "9dc2591b08b04b8c9d28963697b95649"
        }
      }
    ]
  }
    ...
    "nodeBasedData": [
      {
        "nodeId": "0de07f85d9a444c5a3caae508fb938ae",
        "object": {
          "properties.name": "9dc2591b08b04b8c9d28963697b95649"
        }
      }
    ]
  }
...
if(te_refresh.data.dataset=== "Bulk Data Processing"){
  ...
  const handleOnUpdate=async(data:any)=>{
    ...
    settab_grpe63f4((pre:any)=>({...pre,channel_combobox:data?.channel}))
    setselectedData(data?.channel)
  ///////////

    let selectedObj=dynamicDFDData?.find((items:any)=>(items?.channel == data?.channel && items?.channel == data?.channel)) || {};  
    try{
    setIsProcessing(true);
    let filterValue = data?.channel;
  ...
  useEffect(() => { tab_grpe63f4Ref.current = tab_grpe63f4; }, [tab_grpe63f4]);
    ...
    const handler = (id:any) => {
      if (id === "9dc2591b08b04b8c9d28963697b95649") {
        handleOnUpdate({
          value:selectedData,
          text:tab_grpe63f4Ref?.current?.channel_combobox||""
        });
      }
    };
    ...
    useEffect(()=>{
    if(channel_combobox95649?.trigger===undefined || !channel_combobox95649?.trigger) return;
    handleOnUpdate({channel_combobox:tab_grpe63f4Ref?.current?.channel_combobox||""});
  },[channel_combobox95649?.trigger])

  useEffect(()=>{
    if(tab_grpe63f4?.channel_combobox=="")
    {
      setselectedData('')
    }else
    {

      let formBindedData:any=dynamicDFDData?.find((item:any)=>(item?.channel==(selectedData||tab_grpe63f4?.channel_combobox)))?.channel || tab_grpe63f4?.channel_combobox;
      setselectedData(formBindedData)
    }
  },[tab_grpe63f4?.channel_combobox])
  ...
  <div 
    style={{
      ...
      //style props
        ...
        search={search}
        setSearch={setSearch}
        disabled= {channel_combobox95649?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
