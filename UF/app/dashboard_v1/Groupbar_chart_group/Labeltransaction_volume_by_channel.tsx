...
const Labeltransaction_volume_by_channel = ({encryptionFlagCompData,setIsProcessing}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: "f986da5046b749b3ae592e940b7737a3",
        controlId: "60444ce0a3b148909d6e230cbbb2b08e",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setbar_chart_group737a3((pre:any)=>({...pre,transaction_volume_by_channel:""}));
},[transaction_volume_by_channel2b08e?.refresh])


const handleClick =async(e:any)=>{
  try{
    setIsProcessing(true);
  setbar_chart_group737a3((prev: any) => ({ ...prev, transaction_volume_by_channel: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (transaction_volume_by_channel2b08e?.isHidden) {
    return <></>
  }  
  ...
  return (
    <div 
      ...
      <Label 
        className=""
        disabled= {transaction_volume_by_channel2b08e?.isDisabled ? true : false}
        theme="clear"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labeltransaction_volume_by_channel
