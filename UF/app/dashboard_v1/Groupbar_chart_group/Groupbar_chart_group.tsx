...
import Labeltransaction_volume_by_channel  from "./Labeltransaction_volume_by_channel";
...
const Groupbar_chart_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f986da5046b749b3ae592e940b7737a3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbar_chart_group737a3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("transaction_volume_by_channel")){
        settransaction_volume_by_channel2b08e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(transaction_volume_by_channel2b08e?.isDisabled==null)
      {
        settransaction_volume_by_channel2b08e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("barchart")){
        setbarchart84262((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(barchart84262?.isDisabled==null)
      {
        setbarchart84262((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['transaction_volume_by_channel'] = transaction_volume_by_channel2b08e,
        codeStates['settransaction_volume_by_channel'] = settransaction_volume_by_channel2b08e,
  ...
  const bar_chart_group737a3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    bar_chart_group737a3Ref.current?.setSearchParams();
    bar_chart_group737a3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(bar_chart_group737a3) &&
        Object.keys(bar_chart_group737a3)?.length > 0
      ) {
        setbar_chart_group737a3({})
      }
    } else prevRefreshRef.current = true
  }, [bar_chart_group737a3Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"bar_chart_group"}))
        }}
    >
        {allowedControls.includes("transaction_volume_by_channel")?<Labeltransaction_volume_by_channel   /* 2b08e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("barchart") ?<BarChartsbarchart /* 84262 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
