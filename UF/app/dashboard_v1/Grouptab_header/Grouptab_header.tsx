...
import Grouptab_grp  from "../Grouptab_grp/Grouptab_grp";
...
const Grouptab_header = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a44897ca4cff442cbf31a731e4104820");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settab_header04820Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tab_grp")){
        settab_grpe63f4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tab_grpe63f4?.isDisabled==null)
      {
        settab_grpe63f4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tab_group65b41?.tab_group === "tab_header"){
    handleOnChange()
    }
  ...
  const tab_header04820Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header04820Ref.current?.setSearchParams();
    tab_header04820Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(tab_header04820) &&
        Object.keys(tab_header04820)?.length > 0
      ) {
        settab_header04820({})
      }
    } else prevRefreshRef.current = true
  }, [tab_header04820Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"tab_header"}))
        }}
    >
        {allowedComponent.includes("tab_grp")  &&<Grouptab_grp  
...
export default Grouptab_header
