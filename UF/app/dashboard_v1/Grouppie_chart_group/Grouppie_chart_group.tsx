...
import Labelproduct_organation_label  from "./Labelproduct_organation_label";
...
const Grouppie_chart_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7169a37581d049d395f6f3949db15067");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setpie_chart_group15067Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("product_organation_label")){
        setproduct_organation_label2e07d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(product_organation_label2e07d?.isDisabled==null)
      {
        setproduct_organation_label2e07d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("piechart")){
        setpiechart9dde7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(piechart9dde7?.isDisabled==null)
      {
        setpiechart9dde7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const handleOnChange=()=>{
      ...
      settransaction_tablef4f34((prev:any) => ({ ...prev, ...pie_chart_group15067 }));
  ...
  const pie_chart_group15067Ref = useRef<any>(null);
  const handleClearSearch = () => {
    pie_chart_group15067Ref.current?.setSearchParams();
    pie_chart_group15067Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(pie_chart_group15067) &&
        Object.keys(pie_chart_group15067)?.length > 0
      ) {
        setpie_chart_group15067({})
      }
    } else prevRefreshRef.current = true
  }, [pie_chart_group15067Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"pie_chart_group"}))
        }}
    >
        {allowedControls.includes("product_organation_label")?<Labelproduct_organation_label   /* 2e07d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("piechart") ?<PieChartspiechart /* 9dde7 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouppie_chart_group
