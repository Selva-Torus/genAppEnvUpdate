...
const Texttotal_emp_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        settotal_employees_group69aa9((pre: any) => {
          return { ...pre, new_hires: api_paginationData.data.records[0]?.new_hires }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !total_employees_group69aa9.new_hires){
        settotal_employees_group69aa9((pre:any)=>({...pre,new_hires:dfd_hrmdashboard_v1Props[0]?.new_hires}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_emp_text34ab7?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !total_employees_group69aa9.new_hires){
    settotal_employees_group69aa9((pre:any)=>({...pre,new_hires:dfd_hrmdashboard_v1Props[0]?.new_hires}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (total_emp_text34ab7?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.new_hires : (total_employees_group69aa9?.new_hires || ""))}
</Text>
  </div>
  )
}

export default Texttotal_emp_text
