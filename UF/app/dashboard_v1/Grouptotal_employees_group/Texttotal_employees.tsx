...
const Texttotal_employees = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        settotal_employees_group69aa9((pre: any) => {
          return { ...pre, total_employees: api_paginationData.data.records[0]?.total_employees }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !total_employees_group69aa9.total_employees){
        settotal_employees_group69aa9((pre:any)=>({...pre,total_employees:dfd_hrmdashboard_v1Props[0]?.total_employees}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_employees9e45d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !total_employees_group69aa9.total_employees){
    settotal_employees_group69aa9((pre:any)=>({...pre,total_employees:dfd_hrmdashboard_v1Props[0]?.total_employees}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (total_employees9e45d?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_employees : (total_employees_group69aa9?.total_employees || ""))}
</Text>
  </div>
  )
}

export default Texttotal_employees
