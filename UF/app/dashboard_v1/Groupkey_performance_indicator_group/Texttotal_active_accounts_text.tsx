...
const Texttotal_active_accounts_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {total_active_accounts_texted4d7Props, settotal_active_accounts_texted4d7Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_active_accounts_texted4d7?.refresh])

  if (total_active_accounts_texted4d7?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 17`,gridRow: `11 / 16`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-1"
  color="primary"
>
      {keyset("🗂️ Total Active Accounts")}
</Text>
  </div>
  )
}

export default Texttotal_active_accounts_text
