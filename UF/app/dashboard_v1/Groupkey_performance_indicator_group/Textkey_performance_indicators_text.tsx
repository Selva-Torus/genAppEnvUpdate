...
const Textkey_performance_indicators_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {key_performance_indicators_text4f6dbProps, setkey_performance_indicators_text4f6dbProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[key_performance_indicators_text4f6db?.refresh])

  if (key_performance_indicators_text4f6db?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 19`,gridRow: `1 / 6`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!font-bold"
  variant="subheader-2"
  color="primary"
>
      {keyset("Key Performance Indicators")}
</Text>
  </div>
  )
}

export default Textkey_performance_indicators_text
