...
const Textavg_days_to_judgment_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {avg_days_to_judgment_text82b69Props, setavg_days_to_judgment_text82b69Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[avg_days_to_judgment_text82b69?.refresh])

  if (avg_days_to_judgment_text82b69?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 17`,gridRow: `22 / 27`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-1"
  color="primary"
>
      {keyset("⏱️ Avg Days to Judgment")}
</Text>
  </div>
  )
}

export default Textavg_days_to_judgment_text
