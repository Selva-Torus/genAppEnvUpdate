 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "faa851870cc0487e9d506abe6b02f9d0"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "faa851870cc0487e9d506abe6b02f9d0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "add_btn2f9d0");
    return () => {
  ...
  useEffect(()=>{
    if (!add_btn2f9d0?.trigger) return;
      if(add_btn2f9d0?.trigger){
      setadd_btn2f9d0((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[add_btn2f9d0?.trigger])

  useEffect(()=>{
    if(add_btn2f9d0?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[add_btn2f9d0?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup28176((prev: any) => ({ ...prev, add_btn: true }));
        //onClick
    ...
    routes.push(getRouteScreenDetails('CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1', 'addcase_v1'));
      ...
      setIsProcessing(false);
        setgroup28176((prev: any) => ({ ...prev, add_btn: false }));
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='outlined'
          disabled= {add_btn2f9d0?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdAdd"
