...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "10760b8009a647e5879cb6456d228176");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup28176Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("search_btn")){
        setsearch_btn4c2ed((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(search_btn4c2ed?.isDisabled==null)
      {
        setsearch_btn4c2ed((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_btn")){
        setadd_btn2f9d0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_btn2f9d0?.isDisabled==null)
      {
        setadd_btn2f9d0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text")){
        settextc2337((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(textc2337?.isDisabled==null)
      {
        settextc2337((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("table")){
        settable852e3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(table852e3?.isDisabled==null)
      {
        settable852e3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const group28176Ref = useRef<any>(null);
  const handleClearSearch = () => {
    group28176Ref.current?.setSearchParams();
    group28176Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group28176) && Object.keys(group28176)?.length>0)
      {
        setgroup28176({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group28176Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
        ...
        {        ((ruleData?.length>0 && "add_btn" in ButtonGoRuleData)?ButtonGoRuleData["add_btn"]:true) && 
          ...
          {allowedControls.includes("text") ?<Texttext   /* c2337 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
