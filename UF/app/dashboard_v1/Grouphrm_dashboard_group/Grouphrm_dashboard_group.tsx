...
import Grouppending_access_req_group  from "../Grouppending_access_req_group/Grouppending_access_req_group";
...
const Grouphrm_dashboard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    sethrm_dashboard_group4d6cbProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_employees_group")){
        settotal_employees_group69aa9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_employees_group69aa9?.isDisabled==null)
      {
        settotal_employees_group69aa9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_access_req_group")){
        setpending_access_req_groupb5bd4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_access_req_groupb5bd4?.isDisabled==null)
      {
        setpending_access_req_groupb5bd4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_requests_group")){
        setleave_requests_group4beb5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_requests_group4beb5?.isDisabled==null)
      {
        setleave_requests_group4beb5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("onboarding_group")){
        setonboarding_group2580d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(onboarding_group2580d?.isDisabled==null)
      {
        setonboarding_group2580d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const hrm_dashboard_group4d6cbRef = useRef<any>(null);
  const handleClearSearch = () => {
    hrm_dashboard_group4d6cbRef.current?.setSearchParams();
    hrm_dashboard_group4d6cbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(hrm_dashboard_group4d6cb) && Object.keys(hrm_dashboard_group4d6cb)?.length>0)
      {
        sethrm_dashboard_group4d6cb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [hrm_dashboard_group4d6cbProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-md ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"hrm_dashboard_group"}))
        }}
    >
        ...
        {allowedComponent.includes("pending_access_req_group")  &&<Grouppending_access_req_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
