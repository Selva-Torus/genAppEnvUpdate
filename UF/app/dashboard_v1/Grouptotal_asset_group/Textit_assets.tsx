...
const Textit_assets = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        settotal_asset_group69aa9((pre: any) => {
          return { ...pre, it_assets: api_paginationData.data.records[0]?.it_assets }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !total_asset_group69aa9.it_assets){
        settotal_asset_group69aa9((pre:any)=>({...pre,it_assets:dfd_assetdashboard_v1Props[0]?.it_assets}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[it_assets6051f?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !total_asset_group69aa9.it_assets){
    settotal_asset_group69aa9((pre:any)=>({...pre,it_assets:dfd_assetdashboard_v1Props[0]?.it_assets}));
  }
  },[dfd_assetdashboard_v1Props])

  if (it_assets6051f?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.it_assets : (total_asset_group69aa9?.it_assets || ""))}
</Text>
  </div>
  )
}

export default Textit_assets
