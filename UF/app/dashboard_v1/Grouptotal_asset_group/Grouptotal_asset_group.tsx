...
import Dividertotal_asset_divider  from "./Dividertotal_asset_divider";
import Texttotal_assest_text  from "./Texttotal_assest_text";
import Texttotal_assest  from "./Texttotal_assest";
import Textit_assets  from "./Textit_assets";
...
const Grouptotal_asset_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  let code:any = `settable_group((pre)=>({status:"total_asset_group"}))`;
  let idx = "";
  let item = "";
 ...
 /////////////
  ...
  const {total_asset_dividerd84da, settotal_asset_dividerd84da}= useContext(TotalContext) as TotalContextProps;
  const {total_assest_text88ed3, settotal_assest_text88ed3}= useContext(TotalContext) as TotalContextProps;
  const {total_assest9e45d, settotal_assest9e45d}= useContext(TotalContext) as TotalContextProps;
  const {it_assets6051f, setit_assets6051f}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "110392e92dd940ed958bab8b3d169aa9");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_asset_group69aa9Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_asset_divider")){
        settotal_asset_dividerd84da({...total_asset_dividerd84da,isDisabled:true});

    }else
    {
      if(total_asset_dividerd84da?.isDisabled==null)
      {
        settotal_asset_dividerd84da({...total_asset_dividerd84da,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_assest_text")){
        settotal_assest_text88ed3({...total_assest_text88ed3,isDisabled:true});

    }else
    {
      if(total_assest_text88ed3?.isDisabled==null)
      {
        settotal_assest_text88ed3({...total_assest_text88ed3,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_assest")){
        settotal_assest9e45d({...total_assest9e45d,isDisabled:true});

    }else
    {
      if(total_assest9e45d?.isDisabled==null)
      {
        settotal_assest9e45d({...total_assest9e45d,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("it_assets")){
        setit_assets6051f({...it_assets6051f,isDisabled:true});

    }else
    {
      if(it_assets6051f?.isDisabled==null)
      {
        setit_assets6051f({...it_assets6051f,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_asset_divider'] = total_asset_dividerd84da,
        codeStates['settotal_asset_divider'] = settotal_asset_dividerd84da,
        codeStates['total_assest_text'] = total_assest_text88ed3,
        codeStates['settotal_assest_text'] = settotal_assest_text88ed3,
        codeStates['total_assest'] = total_assest9e45d,
        codeStates['settotal_assest'] = settotal_assest9e45d,
        codeStates['it_assets'] = it_assets6051f,
        codeStates['setit_assets'] = setit_assets6051f,
  ...
  const total_asset_group69aa9Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_asset_group69aa9Ref.current?.setSearchParams();
    total_asset_group69aa9Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(total_asset_group69aa9) && Object.keys(total_asset_group69aa9)?.length>0)
      {
        settotal_asset_group69aa9({})
      }
    }else 
      prevRefreshRef.current= true
  }, [total_asset_group69aa9Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("total_asset_divider") ?<Dividertotal_asset_divider   /* d84da */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_assest_text") ?<Texttotal_assest_text   /* 88ed3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_assest") ?<Texttotal_assest   /* 9e45d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("it_assets") ?<Textit_assets   /* 6051f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouptotal_asset_group
