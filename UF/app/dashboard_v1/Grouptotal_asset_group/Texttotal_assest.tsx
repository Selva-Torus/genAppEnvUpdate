...
const Texttotal_assest = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        settotal_asset_group69aa9((pre: any) => {
          return { ...pre, total_assets: api_paginationData.data.records[0]?.total_assets }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !total_asset_group69aa9.total_assets){
        settotal_asset_group69aa9((pre:any)=>({...pre,total_assets:dfd_assetdashboard_v1Props[0]?.total_assets}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_assest9e45d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !total_asset_group69aa9.total_assets){
    settotal_asset_group69aa9((pre:any)=>({...pre,total_assets:dfd_assetdashboard_v1Props[0]?.total_assets}));
  }
  },[dfd_assetdashboard_v1Props])

  if (total_assest9e45d?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_assets : (total_asset_group69aa9?.total_assets || ""))}
</Text>
  </div>
  )
}

export default Texttotal_assest
