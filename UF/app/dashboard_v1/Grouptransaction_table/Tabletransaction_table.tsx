...
let defaultColumns:any = [
  {
    "id": "value_date",
    "nodeid": "0335d36f570c4113aaed5ca914626f4c",
    "name": "Value Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_account",
    "nodeid": "633518a5126344d497b16fd0208e2a30",
    "name": "Dr Account",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_name",
    "nodeid": "6260757ab13c47abad37ac1fac259632",
    "name": "Dr Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_currency",
    "nodeid": "9a202409a6bf4fab84e94da006439803",
    "name": "Dr Currency",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_amount",
    "nodeid": "35ea7619941642258b98d7fd6efc3a1f",
    "name": "Dr Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_account",
    "nodeid": "ac01bdc3d878418bbe65d06dcb6cf8bb",
    "name": "Cr Account",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_name",
    "nodeid": "20c49fe681b14eaf9f56a8114ea8be31",
    "name": "Cr Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_currency",
    "nodeid": "2b60fdb653c64610979414c19405fe15",
    "name": "Cr Currency",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_amount",
    "nodeid": "8939d6556cca42488cdcf8117d8a66de",
    "name": "Cr Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "uuid",
    "nodeid": "e141a519da2142518a2a5325f2c9822d",
    "name": "UUID",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "daa7b624a7c04780af6ff0c1f49f4f34",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, transaction_tablef4f34 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, transaction_tablef4f34 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, transaction_tablef4f34 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settransaction_tablef4f34Props((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.vgphstm_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      transaction_tablef4f34.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.vgphstm_id){
          selectedData?.push(item)
          postIds.push(item.vgphstm_id)
      ...
      //////////
      //////////
        settransaction_tablef4f34Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
    else if(needLockingAndRule.lockMode==='Multi'){
      // its for ui level selected list show for multi select
      transaction_tablef4f34.filter((item:any,id:number)=>{
        if (ids.includes(item.vgphstm_id)){
          selectedData?.push(item)
          postIds.push(item.vgphstm_id) 
          processIds.push(item?.trs_process_id)
        } 
      ...
      settransaction_tablef4f34Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settransaction_tablef4f34Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false,sortingDetails:any={}){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:globalDashboard:AFVK:v1:UO', 
                nodeId: 'daa7b624a7c04780af6ff0c1f49f4f34',
                elementId: 'daa7b624a7c04780af6ff0c1f49f4f34'
              },
              sortingDetails
            }
          }else
      ...
      if (api_pagination?.data?.records.length > 0) {
        ...
        result.vgphstm_id = emp?.vgphstm_id;
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( transaction_tablef4f34Props?.presetValues&&Object.keys(transaction_tablef4f34Props?.presetValues).length > 0) {
          filtertedData = [transaction_tablef4f34Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settransaction_tablef4f34(api_pagination?.data?.records||[])
        }
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,vgphstm_id:filtertedData[i]?.vgphstm_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.vgphstm_id==item?.vgphstm_id)
        {
          index=i
          row=data
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (transaction_tablef4f34Props.filterInitalLoad) return;

    const filterControllers = transaction_tablef4f34Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['transaction_tablef4f34']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['transaction_tablef4f34']).filter((k: string) => jsonEntry['transaction_tablef4f34'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(transaction_tablef4f34Props.filterProps);
      return;
    }
    ...
    settransaction_tablef4f34Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(transaction_tablef4f34Props.filterProps);

  }, [
    transaction_tablef4f34Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    transaction_tablef4f34Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(transaction_tablef4f34Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(transaction_tablef4f34Props.searchFilter) || {};
...
}, [transaction_tablef4f34Props.searchFilter])
  ...
  useEffect(() => {
    if (transaction_tablef4f34Props?.clearData === true) {
      settransaction_tablef4f34([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settransaction_tablef4f34Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [transaction_tablef4f34Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = transaction_tablef4f34Props?.selectedIds[transaction_tablef4f34Props?.selectedIds?.length-1]
    if(Array.isArray(transaction_tablef4f34) && transaction_tablef4f34.length>0)
    {
      let data = transaction_tablef4f34.find((data:any)=>(data?.vgphstm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_id",
        value: data["vgphstm_id"],
        parentData: data
      })
    }
  }
  useEffect(() => {
    if (transaction_tablef4f34Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (transaction_tablef4f34Props?.selectedIds?.length == 0){
      handleOnRowClick({},transaction_tablef4f34Props?.selectedIds)
    }
  }, [transaction_tablef4f34Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.vgphstm_id==data?.vgphstm_id)
        {
          index=i
          row=item
        }
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(transaction_tablef4f34?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&transaction_tablef4f34?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](transaction_tablef4f34?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: transaction_tablef4f34?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&transaction_tablef4f34?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](transaction_tablef4f34?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: transaction_tablef4f34?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  //////
  ////
  if (transaction_tablef4f34?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"daa7b624a7c04780af6ff0c1f49f4f34"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={transaction_tablef4f34Props?.selectedIds}  
...
export default Tabletransaction_table
