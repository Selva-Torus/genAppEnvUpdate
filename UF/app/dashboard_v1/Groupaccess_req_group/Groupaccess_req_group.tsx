...
import Groupacc_group  from "../Groupacc_group/Groupacc_group";
...
const Groupaccess_req_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {search_button3eaa0, setsearch_button3eaa0}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fb006904fad64aa08c1a1655751b1258");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaccess_req_groupb1258Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("acc_group")){
        setacc_group3b167((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(acc_group3b167?.isDisabled==null)
      {
        setacc_group3b167((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("search_button")){
        setsearch_button3eaa0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(search_button3eaa0?.isDisabled==null)
      {
        setsearch_button3eaa0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_req_table")){
        setaccess_req_tablec5aac((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req_tablec5aac?.isDisabled==null)
      {
        setaccess_req_tablec5aac((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['search_button'] = search_button3eaa0,
        codeStates['setsearch_button'] = setsearch_button3eaa0,
  ...
  const access_req_groupb1258Ref = useRef<any>(null);
  const handleClearSearch = () => {
    access_req_groupb1258Ref.current?.setSearchParams();
    access_req_groupb1258Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(access_req_groupb1258) && Object.keys(access_req_groupb1258)?.length>0)
      {
        setaccess_req_groupb1258({})
      }
    }else 
      prevRefreshRef.current= true
  }, [access_req_groupb1258Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"access_req_group"}))
        }}
    >
        {allowedComponent.includes("acc_group")  &&<Groupacc_group  
