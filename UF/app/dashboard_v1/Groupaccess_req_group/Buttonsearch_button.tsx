...
const Buttonsearch_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const access_req_groupb1258Ref = useRef(access_req_groupb1258);
  useEffect(() => {
    access_req_groupb1258Ref.current = access_req_groupb1258;
    if (!pendingAutoSearch.current) return;
      pendingAutoSearch.current = false;
      handleClick(false);
  }, [access_req_groupb1258]);
  
  //group props in ref to access latest props value
  const access_req_groupb1258PropsRef = useRef(access_req_groupb1258Props);
  useEffect(() => {
    access_req_groupb1258PropsRef.current = access_req_groupb1258Props;
  }, [access_req_groupb1258Props]);
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...access_req_groupb1258Ref.current};
      let parentRowSpan = 108;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "fb006904fad64aa08c1a1655751b1258",
        "b2c59729832343389a1a6ddb0d83eaa0"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "search_button3eaa0") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "b2c59729832343389a1a6ddb0d83eaa0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "search_button3eaa0");
    return () => {
  ...
  useEffect(()=>{
    if (!search_button3eaa0?.trigger) return;
      if(search_button3eaa0?.trigger){
      setsearch_button3eaa0((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[search_button3eaa0?.trigger])

  useEffect(()=>{
    if(search_button3eaa0?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[search_button3eaa0?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setaccess_req_groupb1258((prev: any) => ({ ...prev, search_button: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setaccess_req_groupb1258((prev: any) => ({ ...prev, search_button: false }));
 ...
 if (search_button3eaa0?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          disabled= {search_button3eaa0?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
