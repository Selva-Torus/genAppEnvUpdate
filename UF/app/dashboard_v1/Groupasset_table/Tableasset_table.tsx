...
let defaultColumns:any = [
  {
    "id": "asset_id",
    "nodeid": "de7cd6def4204a3b9fd945b17a1bd7bd",
    "name": "Asset ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "asset_tag",
    "nodeid": "16d08f7ef362453a8c03b81b5dfafbdd",
    "name": "Asset Tag",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "asset_name",
    "nodeid": "30f5edc3b9164d339c2731891ad1ef31",
    "name": "Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "category",
    "nodeid": "4400264213724381a4640bf77b92a9d0",
    "name": "Category",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "serial_no",
    "nodeid": "7063d471457547b3b91c622d3bb3915b",
    "name": "Serial No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "assigned_to",
    "nodeid": "6c8b4bf8afb54c56b9b8fe10a5ee3252",
    "name": "Assigned To",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "location",
    "nodeid": "beeb73d955534090b4d07721f56ee1a6",
    "name": "Location",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "warranty_expiry",
    "nodeid": "a54a057ec5574be691c73037f00c5b88",
...
const Tableasset_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {asset_idbd7bd, setasset_idbd7bd}= useContext(TotalContext) as TotalContextProps  
  const {asset_tagafbdd, setasset_tagafbdd}= useContext(TotalContext) as TotalContextProps  
  const {asset_name1ef31, setasset_name1ef31}= useContext(TotalContext) as TotalContextProps  
  const {category2a9d0, setcategory2a9d0}= useContext(TotalContext) as TotalContextProps  
  const {serial_no3915b, setserial_no3915b}= useContext(TotalContext) as TotalContextProps  
  const {assigned_toe3252, setassigned_toe3252}= useContext(TotalContext) as TotalContextProps  
  const {locationee1a6, setlocationee1a6}= useContext(TotalContext) as TotalContextProps  
  const {warranty_expiryc5b88, setwarranty_expiryc5b88}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "5f0852ed9bbd40e5b6d5a79d5ae6082a",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['asset_id'] = asset_idbd7bd,
        codeStates['setasset_id'] = setasset_idbd7bd,
        codeStates['asset_tag'] = asset_tagafbdd,
        codeStates['setasset_tag'] = setasset_tagafbdd,
        codeStates['asset_name'] = asset_name1ef31,
        codeStates['setasset_name'] = setasset_name1ef31,
        codeStates['category'] = category2a9d0,
        codeStates['setcategory'] = setcategory2a9d0,
        codeStates['serial_no'] = serial_no3915b,
        codeStates['setserial_no'] = setserial_no3915b,
        codeStates['assigned_to'] = assigned_toe3252,
        codeStates['setassigned_to'] = setassigned_toe3252,
        codeStates['location'] = locationee1a6,
        codeStates['setlocation'] = setlocationee1a6,
        codeStates['warranty_expiry'] = warranty_expiryc5b88,
        codeStates['setwarranty_expiry'] = setwarranty_expiryc5b88,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setasset_table6082aProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setasset_table6082aProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '5f0852ed9bbd40e5b6d5a79d5ae6082a',
                elementId: '5f0852ed9bbd40e5b6d5a79d5ae6082a'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( asset_table6082aProps?.presetValues&&Object.keys(asset_table6082aProps?.presetValues).length > 0) {
          filtertedData = [asset_table6082aProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setasset_table6082a(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (asset_table6082aProps.filterInitalLoad) return;

    const filterControllers = asset_table6082aProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['asset_table6082a']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['asset_table6082a']).filter((k: string) => jsonEntry['asset_table6082a'][k] === false)
      : [];
    ...
    setasset_table6082aProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(asset_table6082aProps.filterProps);

  }, [
    asset_table6082aProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    asset_table6082aProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(asset_table6082aProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(asset_table6082aProps.searchFilter) || {};
...
}, [asset_table6082aProps.searchFilter])
  ...
  useEffect(() => {
    if (asset_table6082aProps?.clearData === true) {
      setasset_table6082a([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setasset_table6082aProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [asset_table6082aProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [asset_table6082aProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = asset_table6082aProps?.selectedIds[asset_table6082aProps?.selectedIds?.length-1]
    if(Array.isArray(asset_table6082a) && asset_table6082a.length>0)
    {
      let data = asset_table6082a[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (asset_table6082aProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (asset_table6082aProps?.selectedIds?.length == 0){
      handleOnRowClick({},asset_table6082aProps?.selectedIds)
    }
  }, [asset_table6082aProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(asset_table6082a?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&asset_table6082a?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](asset_table6082a?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_table6082a?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&asset_table6082a?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](asset_table6082a?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_table6082a?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"5f0852ed9bbd40e5b6d5a79d5ae6082a"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={asset_table6082aProps?.selectedIds}  
