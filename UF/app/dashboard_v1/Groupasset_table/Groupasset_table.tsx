...
const Groupasset_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5f0852ed9bbd40e5b6d5a79d5ae6082a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_table6082aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("asset_id")){
        setasset_idbd7bd({...asset_idbd7bd,isDisabled:true});

    }else
    {
      if(asset_idbd7bd?.isDisabled==null)
      {
        setasset_idbd7bd({...asset_idbd7bd,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("asset_tag")){
        setasset_tagafbdd({...asset_tagafbdd,isDisabled:true});

    }else
    {
      if(asset_tagafbdd?.isDisabled==null)
      {
        setasset_tagafbdd({...asset_tagafbdd,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("asset_name")){
        setasset_name1ef31({...asset_name1ef31,isDisabled:true});

    }else
    {
      if(asset_name1ef31?.isDisabled==null)
      {
        setasset_name1ef31({...asset_name1ef31,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("category")){
        setcategory2a9d0({...category2a9d0,isDisabled:true});

    }else
    {
      if(category2a9d0?.isDisabled==null)
      {
        setcategory2a9d0({...category2a9d0,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("serial_no")){
        setserial_no3915b({...serial_no3915b,isDisabled:true});

    }else
    {
      if(serial_no3915b?.isDisabled==null)
      {
        setserial_no3915b({...serial_no3915b,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("assigned_to")){
        setassigned_toe3252({...assigned_toe3252,isDisabled:true});

    }else
    {
      if(assigned_toe3252?.isDisabled==null)
      {
        setassigned_toe3252({...assigned_toe3252,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("location")){
        setlocationee1a6({...locationee1a6,isDisabled:true});

    }else
    {
      if(locationee1a6?.isDisabled==null)
      {
        setlocationee1a6({...locationee1a6,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("warranty_expiry")){
        setwarranty_expiryc5b88({...warranty_expiryc5b88,isDisabled:true});

    }else
    {
      if(warranty_expiryc5b88?.isDisabled==null)
      {
        setwarranty_expiryc5b88({...warranty_expiryc5b88,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const asset_table6082aRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_table6082aRef.current?.setSearchParams();
    asset_table6082aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_table6082a) && Object.keys(asset_table6082a)?.length>0)
      {
        setasset_table6082a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_table6082aProps?.refresh,token])
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableasset_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={asset_table6082aRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
