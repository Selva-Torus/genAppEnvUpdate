...
const Textcollected_mtd_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {collected_mtd_textf5ff3Props, setcollected_mtd_textf5ff3Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[collected_mtd_textf5ff3?.refresh])

  if (collected_mtd_textf5ff3?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("COLLECTED MTD")}
</Text>
  </div>
  )
}

export default Textcollected_mtd_text
