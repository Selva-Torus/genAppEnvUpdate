...
const Groupcollected_mtd_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0ee8c05f0f4074f50dd2908f2920f074");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcollected_mtd_group0f074Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("collected_mtd_text")){
        setcollected_mtd_textf5ff3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(collected_mtd_textf5ff3?.isDisabled==null)
      {
        setcollected_mtd_textf5ff3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("icon")){
        seticonb9347((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(iconb9347?.isDisabled==null)
      {
        seticonb9347((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("collected_mtd")){
        setcollected_mtd65ae0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(collected_mtd65ae0?.isDisabled==null)
      {
        setcollected_mtd65ae0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("collected_mtd_desc")){
        setcollected_mtd_desc21bcb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(collected_mtd_desc21bcb?.isDisabled==null)
      {
        setcollected_mtd_desc21bcb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['collected_mtd_text'] = collected_mtd_textf5ff3,
        codeStates['setcollected_mtd_text'] = setcollected_mtd_textf5ff3,
        codeStates['icon'] = iconb9347,
        codeStates['seticon'] = seticonb9347,
        codeStates['collected_mtd'] = collected_mtd65ae0,
        codeStates['setcollected_mtd'] = setcollected_mtd65ae0,
        codeStates['collected_mtd_desc'] = collected_mtd_desc21bcb,
        codeStates['setcollected_mtd_desc'] = setcollected_mtd_desc21bcb,
  ...
  const collected_mtd_group0f074Ref = useRef<any>(null);
  const handleClearSearch = () => {
    collected_mtd_group0f074Ref.current?.setSearchParams();
    collected_mtd_group0f074Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(collected_mtd_group0f074) && Object.keys(collected_mtd_group0f074)?.length>0)
      {
        setcollected_mtd_group0f074({})
      }
    }else 
      prevRefreshRef.current= true
  }, [collected_mtd_group0f074Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !pl-3 !rounded-2xl !bg-white ${isDark ? 'text-white' : 'text-black'}  ${newdashboard_v1?._selectedGroup_=="collected_mtd_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"collected_mtd_group"}))
        }}
    >
          {allowedControls.includes("collected_mtd_text") ?<Textcollected_mtd_text   /* f5ff3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("icon")?<Iconicon /* b9347 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("collected_mtd") ?<Textcollected_mtd   /* 65ae0 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("collected_mtd_desc") ?<Textcollected_mtd_desc   /* 21bcb */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
