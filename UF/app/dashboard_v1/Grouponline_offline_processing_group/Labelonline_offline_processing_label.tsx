...
const Labelonline_offline_processing_label = ({encryptionFlagCompData,setIsProcessing}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: "aa33d04d7fe24f9f9825667f7a57ad24",
        controlId: "994f992bd24a4e7da984c0230c99cdc2",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setonline_offline_processing_group7ad24((pre:any)=>({...pre,online_offline_processing_label:""}));
},[online_offline_processing_label9cdc2?.refresh])


const handleClick =async(e:any)=>{
  try{
    setIsProcessing(true);
  setonline_offline_processing_group7ad24((prev: any) => ({ ...prev, online_offline_processing_label: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (online_offline_processing_label9cdc2?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `6 / 22`,gridRow: `1 / 8`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className="p-1 !text-white font-bold"
        disabled= {online_offline_processing_label9cdc2?.isDisabled ? true : false}
        theme="clear"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labelonline_offline_processing_label
