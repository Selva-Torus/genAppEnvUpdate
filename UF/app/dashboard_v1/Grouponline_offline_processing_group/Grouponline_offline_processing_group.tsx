...
import Icononline_offline_processing_icon  from "./Icononline_offline_processing_icon";
import Labelonline_offline_processing_label  from "./Labelonline_offline_processing_label";
import Textonline_offline_process  from "./Textonline_offline_process";
...
const Grouponline_offline_processing_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupOnlineOfflineProcessingGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "aa33d04d7fe24f9f9825667f7a57ad24");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setonline_offline_processing_group7ad24Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("online_offline_processing_icon")){
        setonline_offline_processing_icon87fd3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(online_offline_processing_icon87fd3?.isDisabled==null)
      {
        setonline_offline_processing_icon87fd3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("online_offline_processing_label")){
        setonline_offline_processing_label9cdc2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(online_offline_processing_label9cdc2?.isDisabled==null)
      {
        setonline_offline_processing_label9cdc2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("online_offline_process")){
        setonline_offline_process41265((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(online_offline_process41265?.isDisabled==null)
      {
        setonline_offline_process41265((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['online_offline_processing_icon'] = online_offline_processing_icon87fd3,
        codeStates['setonline_offline_processing_icon'] = setonline_offline_processing_icon87fd3,
        codeStates['online_offline_processing_label'] = online_offline_processing_label9cdc2,
        codeStates['setonline_offline_processing_label'] = setonline_offline_processing_label9cdc2,
  ...
  const online_offline_processing_group7ad24Ref = useRef<any>(null);
  const handleClearSearch = () => {
    online_offline_processing_group7ad24Ref.current?.setSearchParams();
    online_offline_processing_group7ad24Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(online_offline_processing_group7ad24) &&
        Object.keys(online_offline_processing_group7ad24)?.length > 0
      ) {
        setonline_offline_processing_group7ad24({})
      }
    } else prevRefreshRef.current = true
  }, [online_offline_processing_group7ad24Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"online_offline_processing_group"}))
        }}
    >
        {allowedControls.includes("online_offline_processing_icon")?<Icononline_offline_processing_icon /* 87fd3 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("online_offline_processing_label")?<Labelonline_offline_processing_label   /* 9cdc2 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("online_offline_process") ?<Textonline_offline_process   /* 41265 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouponline_offline_processing_group
