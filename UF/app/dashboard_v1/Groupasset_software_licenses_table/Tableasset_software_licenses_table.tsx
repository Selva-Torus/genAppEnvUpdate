...
let defaultColumns:any = [
  {
    "id": "product_name",
    "nodeid": "497fab67f95e431087b97ed2d05ff649",
    "name": "Software",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "license_type",
    "nodeid": "45cffe6ba0534ec88cb57951aa3e6826",
    "name": "License Type",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "seats_total",
    "nodeid": "e5ca9703a0e340f29ae2098419128de1",
    "name": "Total Seats",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "seats_used",
    "nodeid": "466ea6d3f0c742a29c898983eab897a3",
    "name": "Allocated",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "expiry_date",
    "nodeid": "ac904970cc4d48e8bd87143421e7d6c7",
    "name": "Expiry",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cost",
    "nodeid": "82a773e3266d4299bfdd01458626edbe",
    "name": "Cost",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "license_id",
    "nodeid": "ae49f522952a4e2fb677581accdc0471",
    "name": "license_id",
    "meta": {
      "sort": true
    },
...
const Tableasset_software_licenses_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {product_nameff649, setproduct_nameff649}= useContext(TotalContext) as TotalContextProps  
  const {license_typee6826, setlicense_typee6826}= useContext(TotalContext) as TotalContextProps  
  const {seats_total28de1, setseats_total28de1}= useContext(TotalContext) as TotalContextProps  
  const {seats_used897a3, setseats_used897a3}= useContext(TotalContext) as TotalContextProps  
  const {expiry_date7d6c7, setexpiry_date7d6c7}= useContext(TotalContext) as TotalContextProps  
  const {cost6edbe, setcost6edbe}= useContext(TotalContext) as TotalContextProps  
  const {license_idc0471, setlicense_idc0471}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "fe36cd354c674daea2f0e1b269713758",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['product_name'] = product_nameff649,
        codeStates['setproduct_name'] = setproduct_nameff649,
        codeStates['license_type'] = license_typee6826,
        codeStates['setlicense_type'] = setlicense_typee6826,
        codeStates['seats_total'] = seats_total28de1,
        codeStates['setseats_total'] = setseats_total28de1,
        codeStates['seats_used'] = seats_used897a3,
        codeStates['setseats_used'] = setseats_used897a3,
        codeStates['expiry_date'] = expiry_date7d6c7,
        codeStates['setexpiry_date'] = setexpiry_date7d6c7,
        codeStates['cost'] = cost6edbe,
        codeStates['setcost'] = setcost6edbe,
        codeStates['license_id'] = license_idc0471,
        codeStates['setlicense_id'] = setlicense_idc0471,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setasset_software_licenses_table13758Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setasset_software_licenses_table13758Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'fe36cd354c674daea2f0e1b269713758',
                elementId: 'fe36cd354c674daea2f0e1b269713758'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( asset_software_licenses_table13758Props?.presetValues&&Object.keys(asset_software_licenses_table13758Props?.presetValues).length > 0) {
          filtertedData = [asset_software_licenses_table13758Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setasset_software_licenses_table13758(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (asset_software_licenses_table13758Props.filterInitalLoad) return;

    const filterControllers = asset_software_licenses_table13758Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['asset_software_licenses_table13758']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['asset_software_licenses_table13758']).filter((k: string) => jsonEntry['asset_software_licenses_table13758'][k] === false)
      : [];
    ...
    setasset_software_licenses_table13758Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(asset_software_licenses_table13758Props.filterProps);

  }, [
    asset_software_licenses_table13758Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    asset_software_licenses_table13758Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(asset_software_licenses_table13758Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(asset_software_licenses_table13758Props.searchFilter) || {};
...
}, [asset_software_licenses_table13758Props.searchFilter])
  ...
  useEffect(() => {
    if (asset_software_licenses_table13758Props?.clearData === true) {
      setasset_software_licenses_table13758([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setasset_software_licenses_table13758Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [asset_software_licenses_table13758Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [asset_software_licenses_table13758Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = asset_software_licenses_table13758Props?.selectedIds[asset_software_licenses_table13758Props?.selectedIds?.length-1]
    if(Array.isArray(asset_software_licenses_table13758) && asset_software_licenses_table13758.length>0)
    {
      let data = asset_software_licenses_table13758[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "license_id",
  ...
  useEffect(() => {
    if (asset_software_licenses_table13758Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (asset_software_licenses_table13758Props?.selectedIds?.length == 0){
      handleOnRowClick({},asset_software_licenses_table13758Props?.selectedIds)
    }
  }, [asset_software_licenses_table13758Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(asset_software_licenses_table13758?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&asset_software_licenses_table13758?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](asset_software_licenses_table13758?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_software_licenses_table13758?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&asset_software_licenses_table13758?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](asset_software_licenses_table13758?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_software_licenses_table13758?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (asset_software_licenses_table13758?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"fe36cd354c674daea2f0e1b269713758"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={asset_software_licenses_table13758Props?.selectedIds}  
...
export default Tableasset_software_licenses_table
