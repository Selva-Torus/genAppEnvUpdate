...
import Tableasset_software_licenses_table  from './Tableasset_software_licenses_table';  
...
const Groupasset_software_licenses_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupAssetSoftwareLicensesTable',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fe36cd354c674daea2f0e1b269713758");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_software_licenses_table13758Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("product_name")){
        setproduct_nameff649({...product_nameff649,isDisabled:true});

    }else
    {
      if(product_nameff649?.isDisabled==null)
      {
        setproduct_nameff649({...product_nameff649,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("license_type")){
        setlicense_typee6826({...license_typee6826,isDisabled:true});

    }else
    {
      if(license_typee6826?.isDisabled==null)
      {
        setlicense_typee6826({...license_typee6826,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("seats_total")){
        setseats_total28de1({...seats_total28de1,isDisabled:true});

    }else
    {
      if(seats_total28de1?.isDisabled==null)
      {
        setseats_total28de1({...seats_total28de1,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("seats_used")){
        setseats_used897a3({...seats_used897a3,isDisabled:true});

    }else
    {
      if(seats_used897a3?.isDisabled==null)
      {
        setseats_used897a3({...seats_used897a3,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("expiry_date")){
        setexpiry_date7d6c7({...expiry_date7d6c7,isDisabled:true});

    }else
    {
      if(expiry_date7d6c7?.isDisabled==null)
      {
        setexpiry_date7d6c7({...expiry_date7d6c7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cost")){
        setcost6edbe({...cost6edbe,isDisabled:true});

    }else
    {
      if(cost6edbe?.isDisabled==null)
      {
        setcost6edbe({...cost6edbe,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("license_id")){
        setlicense_idc0471({...license_idc0471,isDisabled:true});

    }else
    {
      if(license_idc0471?.isDisabled==null)
      {
        setlicense_idc0471({...license_idc0471,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const asset_software_licenses_table13758Ref = useRef<any>(null);
  const handleClearSearch = () => {
    asset_software_licenses_table13758Ref.current?.setSearchParams();
    asset_software_licenses_table13758Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_software_licenses_table13758) && Object.keys(asset_software_licenses_table13758)?.length>0)
      {
        setasset_software_licenses_table13758({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_software_licenses_table13758Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableasset_software_licenses_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={asset_software_licenses_table13758Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupasset_software_licenses_table
