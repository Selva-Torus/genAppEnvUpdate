...
const Groupgdb_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8b988746e8524f30949c4bbfc355384d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgdb_group5384dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tab_group")){
        settab_group65b41((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tab_group65b41?.isDisabled==null)
      {
        settab_group65b41((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const gdb_group5384dRef = useRef<any>(null);
  const handleClearSearch = () => {
    gdb_group5384dRef.current?.setSearchParams();
    gdb_group5384dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(gdb_group5384d) &&
        Object.keys(gdb_group5384d)?.length > 0
      ) {
        setgdb_group5384d({})
      }
    } else prevRefreshRef.current = true
  }, [gdb_group5384dProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setglobaldashboard_v1((pre:any)=>({...pre,_selectedGroup_:"gdb_group"}))
        }}
    >
