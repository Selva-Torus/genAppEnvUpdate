...
import Grouptotal_employee_table  from "../Grouptotal_employee_table/Grouptotal_employee_table";
...
const Groupemployee_table_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1159bed8870343229c7ee5a629a55008");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemployee_table_group55008Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("emp_group")){
        setemp_group5e40b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emp_group5e40b?.isDisabled==null)
      {
        setemp_group5e40b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button")){
        setbutton48432((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button48432?.isDisabled==null)
      {
        setbutton48432((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_employee_table")){
        settotal_employee_tablee4e9d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_employee_tablee4e9d?.isDisabled==null)
      {
        settotal_employee_tablee4e9d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const employee_table_group55008Ref = useRef<any>(null);
  const handleClearSearch = () => {
    employee_table_group55008Ref.current?.setSearchParams();
    employee_table_group55008Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(employee_table_group55008) && Object.keys(employee_table_group55008)?.length>0)
      {
        setemployee_table_group55008({})
      }
    }else 
      prevRefreshRef.current= true
  }, [employee_table_group55008Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"employee_table_group"}))
        }}
    >
        ...
        {allowedComponent.includes("total_employee_table")  &&<Grouptotal_employee_table  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupemployee_table_group
