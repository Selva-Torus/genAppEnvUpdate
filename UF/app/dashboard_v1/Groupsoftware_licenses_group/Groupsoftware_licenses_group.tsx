...
import Dividersoftware_licenses_divider  from "./Dividersoftware_licenses_divider";
import Textsoftware_licenses_text  from "./Textsoftware_licenses_text";
import Textsoftware_licenses  from "./Textsoftware_licenses";
import Textlicenses_near_expiry  from "./Textlicenses_near_expiry";
...
const Groupsoftware_licenses_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "19c9f5f542eb45b58a3d9ef54a34beb5");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsoftware_licenses_group4beb5Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("software_licenses_divider")){
        setsoftware_licenses_divider9376b({...software_licenses_divider9376b,isDisabled:true});

    }else
    {
      if(software_licenses_divider9376b?.isDisabled==null)
      {
        setsoftware_licenses_divider9376b({...software_licenses_divider9376b,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("software_licenses_text")){
        setsoftware_licenses_text92f3e({...software_licenses_text92f3e,isDisabled:true});

    }else
    {
      if(software_licenses_text92f3e?.isDisabled==null)
      {
        setsoftware_licenses_text92f3e({...software_licenses_text92f3e,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("software_licenses")){
        setsoftware_licenses46b7c({...software_licenses46b7c,isDisabled:true});

    }else
    {
      if(software_licenses46b7c?.isDisabled==null)
      {
        setsoftware_licenses46b7c({...software_licenses46b7c,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("licenses_near_expiry")){
        setlicenses_near_expiry46af3({...licenses_near_expiry46af3,isDisabled:true});

    }else
    {
      if(licenses_near_expiry46af3?.isDisabled==null)
      {
        setlicenses_near_expiry46af3({...licenses_near_expiry46af3,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['software_licenses_divider'] = software_licenses_divider9376b,
        codeStates['setsoftware_licenses_divider'] = setsoftware_licenses_divider9376b,
        codeStates['software_licenses_text'] = software_licenses_text92f3e,
        codeStates['setsoftware_licenses_text'] = setsoftware_licenses_text92f3e,
        codeStates['software_licenses'] = software_licenses46b7c,
        codeStates['setsoftware_licenses'] = setsoftware_licenses46b7c,
        codeStates['licenses_near_expiry'] = licenses_near_expiry46af3,
        codeStates['setlicenses_near_expiry'] = setlicenses_near_expiry46af3,
  ...
  const software_licenses_group4beb5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    software_licenses_group4beb5Ref.current?.setSearchParams();
    software_licenses_group4beb5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(software_licenses_group4beb5) && Object.keys(software_licenses_group4beb5)?.length>0)
      {
        setsoftware_licenses_group4beb5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [software_licenses_group4beb5Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("software_licenses_divider") ?<Dividersoftware_licenses_divider   /* 9376b */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("software_licenses_text") ?<Textsoftware_licenses_text   /* 92f3e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("software_licenses") ?<Textsoftware_licenses   /* 46b7c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("licenses_near_expiry") ?<Textlicenses_near_expiry   /* 46af3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupsoftware_licenses_group
