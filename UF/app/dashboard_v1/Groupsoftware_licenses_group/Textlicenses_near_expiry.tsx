...
const Textlicenses_near_expiry = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setsoftware_licenses_group4beb5((pre: any) => {
          return { ...pre, licenses_near_expiry: api_paginationData.data.records[0]?.licenses_near_expiry }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !software_licenses_group4beb5.licenses_near_expiry){
        setsoftware_licenses_group4beb5((pre:any)=>({...pre,licenses_near_expiry:dfd_assetdashboard_v1Props[0]?.licenses_near_expiry}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[licenses_near_expiry46af3?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !software_licenses_group4beb5.licenses_near_expiry){
    setsoftware_licenses_group4beb5((pre:any)=>({...pre,licenses_near_expiry:dfd_assetdashboard_v1Props[0]?.licenses_near_expiry}));
  }
  },[dfd_assetdashboard_v1Props])

  if (licenses_near_expiry46af3?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.licenses_near_expiry : (software_licenses_group4beb5?.licenses_near_expiry || ""))}
</Text>
  </div>
  )
}

export default Textlicenses_near_expiry
