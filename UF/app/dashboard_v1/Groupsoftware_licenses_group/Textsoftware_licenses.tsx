...
const Textsoftware_licenses = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setsoftware_licenses_group4beb5((pre: any) => {
          return { ...pre, software_licenses: api_paginationData.data.records[0]?.software_licenses }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !software_licenses_group4beb5.software_licenses){
        setsoftware_licenses_group4beb5((pre:any)=>({...pre,software_licenses:dfd_assetdashboard_v1Props[0]?.software_licenses}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[software_licenses46b7c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !software_licenses_group4beb5.software_licenses){
    setsoftware_licenses_group4beb5((pre:any)=>({...pre,software_licenses:dfd_assetdashboard_v1Props[0]?.software_licenses}));
  }
  },[dfd_assetdashboard_v1Props])

  if (software_licenses46b7c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.software_licenses : (software_licenses_group4beb5?.software_licenses || ""))}
</Text>
  </div>
  )
}

export default Textsoftware_licenses
