...
const Textleave_requests = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        setleave_requests_group4beb5((pre: any) => {
          return { ...pre, total_leave_requests: api_paginationData.data.records[0]?.total_leave_requests }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !leave_requests_group4beb5.total_leave_requests){
        setleave_requests_group4beb5((pre:any)=>({...pre,total_leave_requests:dfd_hrmdashboard_v1Props[0]?.total_leave_requests}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[leave_requests46b7c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !leave_requests_group4beb5.total_leave_requests){
    setleave_requests_group4beb5((pre:any)=>({...pre,total_leave_requests:dfd_hrmdashboard_v1Props[0]?.total_leave_requests}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (leave_requests46b7c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_leave_requests : (leave_requests_group4beb5?.total_leave_requests || ""))}
</Text>
  </div>
  )
}

export default Textleave_requests
