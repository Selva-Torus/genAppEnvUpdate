...
import Textleave_req_text  from "./Textleave_req_text";
import Iconleave_icon  from "./Iconleave_icon";
import Textleave_requests  from "./Textleave_requests";
import Textleave_des_text  from "./Textleave_des_text";
...
const Groupleave_requests_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setleave_requests_group4beb5Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_req_text")){
        setleave_req_text92f3e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_req_text92f3e?.isDisabled==null)
      {
        setleave_req_text92f3e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_icon")){
        setleave_iconce916((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_iconce916?.isDisabled==null)
      {
        setleave_iconce916((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_requests")){
        setleave_requests46b7c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_requests46b7c?.isDisabled==null)
      {
        setleave_requests46b7c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_des_text")){
        setleave_des_text39ee0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_des_text39ee0?.isDisabled==null)
      {
        setleave_des_text39ee0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['leave_req_text'] = leave_req_text92f3e,
        codeStates['setleave_req_text'] = setleave_req_text92f3e,
        codeStates['leave_icon'] = leave_iconce916,
        codeStates['setleave_icon'] = setleave_iconce916,
        codeStates['leave_requests'] = leave_requests46b7c,
        codeStates['setleave_requests'] = setleave_requests46b7c,
        codeStates['leave_des_text'] = leave_des_text39ee0,
        codeStates['setleave_des_text'] = setleave_des_text39ee0,
  ...
  const leave_requests_group4beb5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    leave_requests_group4beb5Ref.current?.setSearchParams();
    leave_requests_group4beb5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(leave_requests_group4beb5) && Object.keys(leave_requests_group4beb5)?.length>0)
      {
        setleave_requests_group4beb5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [leave_requests_group4beb5Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 pl-3 rounded-2xl bg-white border border-gray-100 transition-all duration-300 ease-in-out cursor-pointer ${isDark ? 'text-white' : 'text-black'}  ${hrmdashboard_v1?._selectedGroup_=="leave_requests_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"leave_requests_group"}))
        }}
    >
          {allowedControls.includes("leave_req_text") ?<Textleave_req_text   /* 92f3e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("leave_icon")?<Iconleave_icon /* ce916 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("leave_requests") ?<Textleave_requests   /* 46b7c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("leave_des_text") ?<Textleave_des_text   /* 39ee0 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
