...
const Textamr_queue_desc = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {amr_queue_descafe0eProps, setamr_queue_descafe0eProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[amr_queue_descafe0e?.refresh])

  if (amr_queue_descafe0e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Accounts Pending review")}
</Text>
  </div>
  )
}

export default Textamr_queue_desc
