...
const Textamr_queue = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {amr_queue5b620Props, setamr_queue5b620Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_cardsdashboard_v1Props && !dfd_cardsdashboard_v1Props.hasLogicCenter) {
        ...
        setamr_queue_groupc92ca((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setamr_queue_groupc92ca((pre: any) => ({
          ...pre,
          amr_queue: amr_queue5b620Props?.filteredData?.length > 0
            ? amr_queue5b620Props?.filteredData[0]?.amr_queue
            : "0"
        }))
      }else if(Array.isArray(dfd_cardsdashboard_v1Props) && dfd_cardsdashboard_v1Props && !amr_queue_groupc92ca.amr_queue){
        setamr_queue_groupc92ca((pre:any)=>({...pre,amr_queue:dfd_cardsdashboard_v1Props[0]?.amr_queue}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[amr_queue5b620?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_cardsdashboard_v1Props) && !amr_queue_groupc92ca.amr_queue){
    setamr_queue_groupc92ca((pre:any)=>({...pre,amr_queue:dfd_cardsdashboard_v1Props[0]?.amr_queue}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!amr_queue5b620Props?.filterProps) return;
    handleMapperValue(amr_queue5b620Props?.filterProps,amr_queue5b620Props?.filterFlag);
  },[amr_queue5b620Props?.filterProps])

  if (amr_queue5b620?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.amr_queue : (amr_queue_groupc92ca?.amr_queue || ""))}
</Text>
  </div>
  )
}
