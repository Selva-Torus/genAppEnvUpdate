...
const Textstatus = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {status77d60Props, setstatus77d60Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  if (status77d60?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 2`,gridRow: `114 / 115`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.status : (table_group112bd?.status || ""))}
</Text>
  </div>
  )
}
