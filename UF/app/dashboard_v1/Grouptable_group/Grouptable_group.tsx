...
const Grouptable_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bcd1435a872149b1a43ec3cfdb194010");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settable_group94010Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("subscreen")){
        setsubscreen99589({...subscreen99589,isDisabled:true});

    }else
    {
      if(subscreen99589?.isDisabled==null)
      {
        setsubscreen99589({...subscreen99589,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatusf4240({...statusf4240,isDisabled:true});

    }else
    {
      if(statusf4240?.isDisabled==null)
      {
        setstatusf4240({...statusf4240,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const table_group94010Ref = useRef<any>(null);
  const handleClearSearch = () => {
    table_group94010Ref.current?.setSearchParams();
    table_group94010Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(table_group94010) && Object.keys(table_group94010)?.length>0)
      {
        settable_group94010({})
      }
    }else 
      prevRefreshRef.current= true
  }, [table_group94010Props?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("status") ?<TextInputstatus   /* f4240 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
