...
const Grouptable_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "19c25e7435d84f47b274d4aef2fe0a6f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settable_groupe0a6fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("subscreen")){
        setsubscreen1c010((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(subscreen1c010?.isDisabled==null)
      {
        setsubscreen1c010((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatuscae93((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(statuscae93?.isDisabled==null)
      {
        setstatuscae93((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const table_groupe0a6fRef = useRef<any>(null);
  const handleClearSearch = () => {
    table_groupe0a6fRef.current?.setSearchParams();
    table_groupe0a6fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(table_groupe0a6f) && Object.keys(table_groupe0a6f)?.length>0)
      {
        settable_groupe0a6f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [table_groupe0a6fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"table_group"}))
        }}
    >
        ...
        {allowedControls.includes("status") ?<TextInputstatus   /* cae93 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
