...
import Groupkey_performance_indicator_group  from "../Groupkey_performance_indicator_group/Groupkey_performance_indicator_group";
import Grouprecent_activity_group  from "../Grouprecent_activity_group/Grouprecent_activity_group";
...
const Groupoverall_key_performance_indicators = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupOverallKeyPerformanceIndicators',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9f06d24ac680416291a36152c4fc2711");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverall_key_performance_indicatorsc2711Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("key_performance_indicator_group")){
        setkey_performance_indicator_groupf9eaf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(key_performance_indicator_groupf9eaf?.isDisabled==null)
      {
        setkey_performance_indicator_groupf9eaf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("recent_activity_group")){
        setrecent_activity_group91db6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(recent_activity_group91db6?.isDisabled==null)
      {
        setrecent_activity_group91db6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const overall_key_performance_indicatorsc2711Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overall_key_performance_indicatorsc2711Ref.current?.setSearchParams();
    overall_key_performance_indicatorsc2711Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overall_key_performance_indicatorsc2711) && Object.keys(overall_key_performance_indicatorsc2711)?.length>0)
      {
        setoverall_key_performance_indicatorsc2711({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overall_key_performance_indicatorsc2711Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"overall_key_performance_indicators"}))
        }}
    >
        {allowedComponent.includes("key_performance_indicator_group")  &&<Groupkey_performance_indicator_group  
        ...
        {allowedComponent.includes("recent_activity_group")  &&<Grouprecent_activity_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupoverall_key_performance_indicators
