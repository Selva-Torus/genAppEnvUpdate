...
import Groupaccess_req_group  from "../Groupaccess_req_group/Groupaccess_req_group";
...
const Groupct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupCt006AfUfUfwsEcpHrmPendingaccessrequestV1',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "4630706d651f4432bc9da967e50adcfe");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfeProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req_group")){
        setaccess_req_groupb1258((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req_groupb1258?.isDisabled==null)
      {
        setaccess_req_groupb1258((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfeRef = useRef<any>(null);
  const handleClearSearch = () => {
    ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfeRef.current?.setSearchParams();
    ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfeRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfe) && Object.keys(ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfe)?.length>0)
      {
        setct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfe({})
      }
    }else 
      prevRefreshRef.current= true
  }, [ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1adcfeProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"ct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1"}))
        }}
    >
        {allowedComponent.includes("access_req_group")  &&<Groupaccess_req_group  
...
export default Groupct006_af_uf_ufws_ecp_hrm_pendingaccessrequest_v1
