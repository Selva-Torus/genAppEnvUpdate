...
import Grouppending_fillings_table  from "../Grouppending_fillings_table/Grouppending_fillings_table";
...
const Grouppending_fillings_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ef5e02f1b52f43c191afd8d3a7bb1568");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setpending_fillings_groupb1568Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("search_btn")){
        setsearch_btn15268((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(search_btn15268?.isDisabled==null)
      {
        setsearch_btn15268((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_btn")){
        setadd_btn707cf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_btn707cf?.isDisabled==null)
      {
        setadd_btn707cf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text")){
        settext3ced2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(text3ced2?.isDisabled==null)
      {
        settext3ced2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_fillings_table")){
        setpending_fillings_table11279((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_fillings_table11279?.isDisabled==null)
      {
        setpending_fillings_table11279((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const pending_fillings_groupb1568Ref = useRef<any>(null);
  const handleClearSearch = () => {
    pending_fillings_groupb1568Ref.current?.setSearchParams();
    pending_fillings_groupb1568Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(pending_fillings_groupb1568) && Object.keys(pending_fillings_groupb1568)?.length>0)
      {
        setpending_fillings_groupb1568({})
      }
    }else 
      prevRefreshRef.current= true
  }, [pending_fillings_groupb1568Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"pending_fillings_group"}))
        }}
    >
        {allowedComponent.includes("pending_fillings_table")  &&<Grouppending_fillings_table  
        ...
        {        ((ruleData?.length>0 && "add_btn" in ButtonGoRuleData)?ButtonGoRuleData["add_btn"]:true) && 
          ...
          {allowedControls.includes("text") ?<Texttext   /* 3ced2 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouppending_fillings_group
