...
import PagePendingfillingsearchpage2 from '@/app/pendingfillingsearch_v1/pendingfillingsearch_v1page';
...
const Buttonsearch_btn = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {pendingfillingsearch_v1Props, setpendingfillingsearch_v1Props}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['pendingfillingsearch_v1'] = pendingfillingsearch_v1Props,
        codeStates['setpendingfillingsearch_v1'] = setpendingfillingsearch_v1Props,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "7cdeeb53811742ebb5ea86d1c4015268"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "search_btn15268") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "7cdeeb53811742ebb5ea86d1c4015268") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "search_btn15268");
    return () => {
  ...
  useEffect(()=>{
    if (!search_btn15268?.trigger) return;
      if(search_btn15268?.trigger){
      setsearch_btn15268((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[search_btn15268?.trigger])

  useEffect(()=>{
    setShowProfileAsModalOpen2(false)
    if(search_btn15268?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[search_btn15268?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setpending_fillings_groupb1568((prev: any) => ({ ...prev, search_btn: true }));
        //onClick
    ...
    setpendingfillingsearch_v1Props([...filterData2 ]);
    setAssetDataReady(false);
    setShowProfileAsModalOpen2(true);
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setpending_fillings_groupb1568((prev: any) => ({ ...prev, search_btn: false }));
      ...
      >
        {showProfileAsModalOpen2 && hiddenModalForTrigger && (
          <div style={{ display: 'none' }}>
            <PagePendingfillingsearchpage2 onReady={handleAssetPageReady}/>
          </div>
        )}
      <Modal 
        ...
        title="Search Pending Filling Case"
        variant="header-1"
        ready={assetDataReady}
        showOverlay = {true}
        position = {"top-right"}
        modalName = "pendingfillingsearch"
        className='w-[] h-[] bg-gray-50 overflow-auto'
      >
        {!hiddenModalForTrigger && <PagePendingfillingsearchpage2  onReady={handleAssetPageReady}/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          disabled= {search_btn15268?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdSearch"
