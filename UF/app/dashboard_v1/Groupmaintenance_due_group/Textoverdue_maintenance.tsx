...
const Textoverdue_maintenance = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setmaintenance_due_group704ca((pre: any) => {
          return { ...pre, overdue_maintenance: api_paginationData.data.records[0]?.overdue_maintenance }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !maintenance_due_group704ca.overdue_maintenance){
        setmaintenance_due_group704ca((pre:any)=>({...pre,overdue_maintenance:dfd_assetdashboard_v1Props[0]?.overdue_maintenance}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[overdue_maintenancef7357?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !maintenance_due_group704ca.overdue_maintenance){
    setmaintenance_due_group704ca((pre:any)=>({...pre,overdue_maintenance:dfd_assetdashboard_v1Props[0]?.overdue_maintenance}));
  }
  },[dfd_assetdashboard_v1Props])

  if (overdue_maintenancef7357?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.overdue_maintenance : (maintenance_due_group704ca?.overdue_maintenance || ""))}
</Text>
  </div>
  )
}

export default Textoverdue_maintenance
