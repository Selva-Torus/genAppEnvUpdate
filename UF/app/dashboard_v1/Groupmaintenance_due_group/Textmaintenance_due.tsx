...
const Textmaintenance_due = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetdashboard_v1Props && !dfd_assetdashboard_v1Props.hasLogicCenter) {
        ...
        setmaintenance_due_group704ca((pre: any) => {
          return { ...pre, maintenance_due: api_paginationData.data.records[0]?.maintenance_due }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetdashboard_v1Props) && dfd_assetdashboard_v1Props && !maintenance_due_group704ca.maintenance_due){
        setmaintenance_due_group704ca((pre:any)=>({...pre,maintenance_due:dfd_assetdashboard_v1Props[0]?.maintenance_due}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[maintenance_due09ab9?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdashboard_v1Props) && !maintenance_due_group704ca.maintenance_due){
    setmaintenance_due_group704ca((pre:any)=>({...pre,maintenance_due:dfd_assetdashboard_v1Props[0]?.maintenance_due}));
  }
  },[dfd_assetdashboard_v1Props])

  if (maintenance_due09ab9?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.maintenance_due : (maintenance_due_group704ca?.maintenance_due || ""))}
</Text>
  </div>
  )
}

export default Textmaintenance_due
