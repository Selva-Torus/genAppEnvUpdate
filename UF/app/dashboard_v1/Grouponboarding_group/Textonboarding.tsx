...
const Textonboarding = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        setonboarding_group2580d((pre: any) => {
          return { ...pre, background_checks: api_paginationData.data.records[0]?.background_checks }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !onboarding_group2580d.background_checks){
        setonboarding_group2580d((pre:any)=>({...pre,background_checks:dfd_hrmdashboard_v1Props[0]?.background_checks}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[onboarding38551?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !onboarding_group2580d.background_checks){
    setonboarding_group2580d((pre:any)=>({...pre,background_checks:dfd_hrmdashboard_v1Props[0]?.background_checks}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (onboarding38551?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.background_checks : (onboarding_group2580d?.background_checks || ""))}
</Text>
  </div>
  )
}
