...
import Textonboarding_text  from "./Textonboarding_text";
...
const Grouponboarding_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setonboarding_group2580dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("onboarding_text")){
        setonboarding_text3d356((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(onboarding_text3d356?.isDisabled==null)
      {
        setonboarding_text3d356((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_icon")){
        setleave_icon64f7b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_icon64f7b?.isDisabled==null)
      {
        setleave_icon64f7b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("onboarding")){
        setonboarding38551((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(onboarding38551?.isDisabled==null)
      {
        setonboarding38551((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("board_des_text")){
        setboard_des_text94799((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(board_des_text94799?.isDisabled==null)
      {
        setboard_des_text94799((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['onboarding_text'] = onboarding_text3d356,
        codeStates['setonboarding_text'] = setonboarding_text3d356,
        codeStates['leave_icon'] = leave_icon64f7b,
        codeStates['setleave_icon'] = setleave_icon64f7b,
        codeStates['onboarding'] = onboarding38551,
        codeStates['setonboarding'] = setonboarding38551,
        codeStates['board_des_text'] = board_des_text94799,
        codeStates['setboard_des_text'] = setboard_des_text94799,
  ...
  const onboarding_group2580dRef = useRef<any>(null);
  const handleClearSearch = () => {
    onboarding_group2580dRef.current?.setSearchParams();
    onboarding_group2580dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(onboarding_group2580d) && Object.keys(onboarding_group2580d)?.length>0)
      {
        setonboarding_group2580d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [onboarding_group2580dProps?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 pl-3 rounded-2xl bg-white border border-gray-100 transition-all duration-300 ease-in-out cursor-pointer ${isDark ? 'text-white' : 'text-black'}  ${hrmdashboard_v1?._selectedGroup_=="onboarding_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"onboarding_group"}))
        }}
    >
          {allowedControls.includes("onboarding_text") ?<Textonboarding_text   /* 3d356 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("leave_icon")?<Iconleave_icon /* 64f7b */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("onboarding") ?<Textonboarding   /* 38551 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("board_des_text") ?<Textboard_des_text   /* 94799 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
