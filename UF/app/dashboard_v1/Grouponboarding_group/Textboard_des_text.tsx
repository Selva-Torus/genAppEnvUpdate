...
const Textboard_des_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        setonboarding_group2580d((pre: any) => {
          return { ...pre, background_check_text: api_paginationData.data.records[0]?.background_check_text }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !onboarding_group2580d.background_check_text){
        setonboarding_group2580d((pre:any)=>({...pre,background_check_text:dfd_hrmdashboard_v1Props[0]?.background_check_text}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[board_des_text94799?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !onboarding_group2580d.background_check_text){
    setonboarding_group2580d((pre:any)=>({...pre,background_check_text:dfd_hrmdashboard_v1Props[0]?.background_check_text}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (board_des_text94799?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `15 / 21`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.background_check_text : (onboarding_group2580d?.background_check_text || ""))}
</Text>
  </div>
  )
}
