...
import Textpending_access_req_text  from "./Textpending_access_req_text";
import Iconaccess_icon  from "./Iconaccess_icon";
import Textpending_access_req  from "./Textpending_access_req";
import Textaccess_des_text  from "./Textaccess_des_text";
...
const Grouppending_access_req_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  let code:any = `settable_group((pre)=>({status:"pending_access_req_group"}))`;
  let idx = "";
  let item = "";
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "027e06378b9d4644bebb875afedb5bd4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setpending_access_req_groupb5bd4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("pending_access_req_text")){
        setpending_access_req_texteee70((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_access_req_texteee70?.isDisabled==null)
      {
        setpending_access_req_texteee70((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_icon")){
        setaccess_icon3d558((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_icon3d558?.isDisabled==null)
      {
        setaccess_icon3d558((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pending_access_req")){
        setpending_access_reqfa250((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pending_access_reqfa250?.isDisabled==null)
      {
        setpending_access_reqfa250((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_des_text")){
        setaccess_des_text941a0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_des_text941a0?.isDisabled==null)
      {
        setaccess_des_text941a0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['pending_access_req_text'] = pending_access_req_texteee70,
        codeStates['setpending_access_req_text'] = setpending_access_req_texteee70,
        codeStates['access_icon'] = access_icon3d558,
        codeStates['setaccess_icon'] = setaccess_icon3d558,
        codeStates['pending_access_req'] = pending_access_reqfa250,
        codeStates['setpending_access_req'] = setpending_access_reqfa250,
        codeStates['access_des_text'] = access_des_text941a0,
        codeStates['setaccess_des_text'] = setaccess_des_text941a0,
  ...
  const pending_access_req_groupb5bd4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    pending_access_req_groupb5bd4Ref.current?.setSearchParams();
    pending_access_req_groupb5bd4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(pending_access_req_groupb5bd4) && Object.keys(pending_access_req_groupb5bd4)?.length>0)
      {
        setpending_access_req_groupb5bd4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [pending_access_req_groupb5bd4Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 pl-3 rounded-2xl bg-white border border-gray-100 transition-all duration-300 ease-in-out cursor-pointer ${isDark ? 'text-white' : 'text-black'}  ${hrmdashboard_v1?._selectedGroup_=="pending_access_req_group" ?'border-2 border-solid !border-[var(--selection-color)]': ''}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"pending_access_req_group"}))
        }}
    >
          {allowedControls.includes("pending_access_req_text") ?<Textpending_access_req_text   /* eee70 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("access_icon")?<Iconaccess_icon /* 3d558 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("pending_access_req") ?<Textpending_access_req   /* fa250 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("access_des_text") ?<Textaccess_des_text   /* 941a0 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouppending_access_req_group
