...
const Textpending_access_req = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_hrmdashboard_v1Props && !dfd_hrmdashboard_v1Props.hasLogicCenter) {
        ...
        setpending_access_req_groupb5bd4((pre: any) => {
          return { ...pre, pending_approval_count: api_paginationData.data.records[0]?.pending_approval_count }
        })
        }
      }
      else{
      if(Array.isArray(dfd_hrmdashboard_v1Props) && dfd_hrmdashboard_v1Props && !pending_access_req_groupb5bd4.pending_approval_count){
        setpending_access_req_groupb5bd4((pre:any)=>({...pre,pending_approval_count:dfd_hrmdashboard_v1Props[0]?.pending_approval_count}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[pending_access_reqfa250?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_hrmdashboard_v1Props) && !pending_access_req_groupb5bd4.pending_approval_count){
    setpending_access_req_groupb5bd4((pre:any)=>({...pre,pending_approval_count:dfd_hrmdashboard_v1Props[0]?.pending_approval_count}));
  }
  },[dfd_hrmdashboard_v1Props])

  if (pending_access_reqfa250?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.pending_approval_count : (pending_access_req_groupb5bd4?.pending_approval_count || ""))}
</Text>
  </div>
  )
}

export default Textpending_access_req
