...
import Tableasset_disposal_table  from './Tableasset_disposal_table';  
...
const Groupasset_disposal_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8f7a89b6389644efbec4f443f3f440cd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_disposal_table440cdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("asset_name")){
        setasset_name251dd({...asset_name251dd,isDisabled:true});

    }else
    {
      if(asset_name251dd?.isDisabled==null)
      {
        setasset_name251dd({...asset_name251dd,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("disposal_method")){
        setdisposal_method84ead({...disposal_method84ead,isDisabled:true});

    }else
    {
      if(disposal_method84ead?.isDisabled==null)
      {
        setdisposal_method84ead({...disposal_method84ead,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reason")){
        setreason441ae({...reason441ae,isDisabled:true});

    }else
    {
      if(reason441ae?.isDisabled==null)
      {
        setreason441ae({...reason441ae,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("current_value")){
        setcurrent_value047d6({...current_value047d6,isDisabled:true});

    }else
    {
      if(current_value047d6?.isDisabled==null)
      {
        setcurrent_value047d6({...current_value047d6,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("data_wiped")){
        setdata_wiped9be82({...data_wiped9be82,isDisabled:true});

    }else
    {
      if(data_wiped9be82?.isDisabled==null)
      {
        setdata_wiped9be82({...data_wiped9be82,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatus00e2a({...status00e2a,isDisabled:true});

    }else
    {
      if(status00e2a?.isDisabled==null)
      {
        setstatus00e2a({...status00e2a,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const asset_disposal_table440cdRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_disposal_table440cdRef.current?.setSearchParams();
    asset_disposal_table440cdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_disposal_table440cd) && Object.keys(asset_disposal_table440cd)?.length>0)
      {
        setasset_disposal_table440cd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_disposal_table440cdProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableasset_disposal_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={asset_disposal_table440cdRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupasset_disposal_table
