...
import Textacc_r_text  from "./Textacc_r_text";
...
const Groupacc_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {acc_r_texta9806, setacc_r_texta9806}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "85dda6ae71e141ca8756e2750953b167");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setacc_group3b167Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("acc_r_text")){
        setacc_r_texta9806((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(acc_r_texta9806?.isDisabled==null)
      {
        setacc_r_texta9806((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['acc_r_text'] = acc_r_texta9806,
        codeStates['setacc_r_text'] = setacc_r_texta9806,
  ...
  const acc_group3b167Ref = useRef<any>(null);
  const handleClearSearch = () => {
    acc_group3b167Ref.current?.setSearchParams();
    acc_group3b167Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(acc_group3b167) && Object.keys(acc_group3b167)?.length>0)
      {
        setacc_group3b167({})
      }
    }else 
      prevRefreshRef.current= true
  }, [acc_group3b167Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          sethrmdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"acc_group"}))
        }}
    >
          {allowedControls.includes("acc_r_text") ?<Textacc_r_text   /* a9806 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupacc_group
