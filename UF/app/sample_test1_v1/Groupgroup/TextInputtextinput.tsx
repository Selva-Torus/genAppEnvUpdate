...
const TextInputtextinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,sample_test1_v1:{...pre?.sample_test1_v1,textinput:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup0843e((prev: any) => ({ ...prev, textinput: +e.target.value }));
    }
    else{
    setgroup0843e((prev: any) => ({ ...prev, textinput: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "db527597a21b46beae18929a3710843e",
        "2505346f3a0b40198a91523d32f55664"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:sample_test1:AFVK:v1",
      //     componentId: "db527597a21b46beae18929a3710843e",
      //     controlId: "2505346f3a0b40198a91523d32f55664",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "2505346f3a0b40198a91523d32f55664") {
        handleChange({target:{value:group0843eRef?.current?.textinput||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2505346f3a0b40198a91523d32f55664") {
        handleBlur({target:{value:group0843eRef?.current?.textinput||""}});
      }
    };
  ...
  if (textinput55664?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 18`,gridRow: `54 / 64`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group0843e?.textinput||""}
         disabled= {textinput55664?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.sample_test1_v1?.textinput ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
