...
export default function PageSampleTest1V1() {
  ...
  const {sample_test1_v1, setsample_test1_v1} = useContext(TotalContext) as TotalContextProps;
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setsample_test1_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const { fetchBatchData } = await import("../utils/Orchestration");
    const data: any = await fetchBatchData(
      'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:sample_test1:AFVK:v1',
      [user],
      'pageSampleTest1V1',
      token
    )
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setsample_test1_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(sample_test1_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(sample_test1_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setsample_test1_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
