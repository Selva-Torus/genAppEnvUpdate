...
const Textcycle_type = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_performancecycle_v1Props && !dfd_performancecycle_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebe86e((pre: any) => {
          return { ...pre, cycle_type: api_paginationData.data.records[0]?.cycle_type }
        })
        }
      }
      else{
      if(Array.isArray(dfd_performancecycle_v1Props) && dfd_performancecycle_v1Props && !group_deletebe86e.cycle_type){
        setgroup_deletebe86e((pre:any)=>({...pre,cycle_type:dfd_performancecycle_v1Props[0]?.cycle_type}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cycle_type89f52?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancecycle_v1Props) && !group_deletebe86e.cycle_type){
    setgroup_deletebe86e((pre:any)=>({...pre,cycle_type:dfd_performancecycle_v1Props[0]?.cycle_type}));
  }
  },[dfd_performancecycle_v1Props])

  if (cycle_type89f52?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.cycle_type : (group_deletebe86e?.cycle_type || ""))}
</Text>
  </div>
  )
}

export default Textcycle_type
