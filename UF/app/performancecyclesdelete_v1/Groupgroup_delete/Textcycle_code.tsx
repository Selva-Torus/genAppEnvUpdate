...
const Textcycle_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_performancecycle_v1Props && !dfd_performancecycle_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebe86e((pre: any) => {
          return { ...pre, cycle_code: api_paginationData.data.records[0]?.cycle_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_performancecycle_v1Props) && dfd_performancecycle_v1Props && !group_deletebe86e.cycle_code){
        setgroup_deletebe86e((pre:any)=>({...pre,cycle_code:dfd_performancecycle_v1Props[0]?.cycle_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cycle_code5f073?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancecycle_v1Props) && !group_deletebe86e.cycle_code){
    setgroup_deletebe86e((pre:any)=>({...pre,cycle_code:dfd_performancecycle_v1Props[0]?.cycle_code}));
  }
  },[dfd_performancecycle_v1Props])

  if (cycle_code5f073?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.cycle_code : (group_deletebe86e?.cycle_code || ""))}
</Text>
  </div>
  )
}

export default Textcycle_code
