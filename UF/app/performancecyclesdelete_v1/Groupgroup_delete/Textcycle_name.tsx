...
const Textcycle_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_performancecycle_v1Props) && dfd_performancecycle_v1Props && !group_deletebe86e.cycle_name){
        setgroup_deletebe86e((pre:any)=>({...pre,cycle_name:dfd_performancecycle_v1Props[0]?.cycle_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cycle_name6a018?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancecycle_v1Props) && !group_deletebe86e.cycle_name){
    setgroup_deletebe86e((pre:any)=>({...pre,cycle_name:dfd_performancecycle_v1Props[0]?.cycle_name}));
  }
  },[dfd_performancecycle_v1Props])

  if (cycle_name6a018?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.cycle_name : (group_deletebe86e?.cycle_name || ""))}
</Text>
  </div>
  )
}
