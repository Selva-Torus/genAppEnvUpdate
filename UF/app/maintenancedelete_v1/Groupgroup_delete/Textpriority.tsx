...
const Textpriority = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetmaintenance_v1Props && !dfd_assetmaintenance_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3f77f((pre: any) => {
          return { ...pre, priority: api_paginationData.data.records[0]?.priority }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.priority){
        setgroup_delete3f77f((pre:any)=>({...pre,priority:dfd_assetmaintenance_v1Props[0]?.priority}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[priority1b975?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.priority){
    setgroup_delete3f77f((pre:any)=>({...pre,priority:dfd_assetmaintenance_v1Props[0]?.priority}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (priority1b975?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `22 / 28`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.priority : (group_delete3f77f?.priority || ""))}
</Text>
  </div>
  )
}

export default Textpriority
