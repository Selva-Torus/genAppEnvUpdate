...
const Textperformed_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetmaintenance_v1Props && !dfd_assetmaintenance_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3f77f((pre: any) => {
          return { ...pre, performed_by: api_paginationData.data.records[0]?.performed_by }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.performed_by){
        setgroup_delete3f77f((pre:any)=>({...pre,performed_by:dfd_assetmaintenance_v1Props[0]?.performed_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[performed_byc179b?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.performed_by){
    setgroup_delete3f77f((pre:any)=>({...pre,performed_by:dfd_assetmaintenance_v1Props[0]?.performed_by}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (performed_byc179b?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `36 / 42`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.performed_by : (group_delete3f77f?.performed_by || ""))}
</Text>
  </div>
  )
}

export default Textperformed_by
