...
const Textschedule_date = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_assetmaintenance_v1Props && !dfd_assetmaintenance_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3f77f((pre: any) => {
          return { ...pre, scheduled_date: api_paginationData.data.records[0]?.scheduled_date }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.scheduled_date){
        setgroup_delete3f77f((pre:any)=>({...pre,scheduled_date:dfd_assetmaintenance_v1Props[0]?.scheduled_date}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[schedule_dateef711?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.scheduled_date){
    setgroup_delete3f77f((pre:any)=>({...pre,scheduled_date:dfd_assetmaintenance_v1Props[0]?.scheduled_date}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (schedule_dateef711?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `29 / 35`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.scheduled_date : (group_delete3f77f?.scheduled_date || ""))}
</Text>
  </div>
  )
}

export default Textschedule_date
