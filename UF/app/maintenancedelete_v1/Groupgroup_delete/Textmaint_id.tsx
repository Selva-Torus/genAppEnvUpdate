...
const Textmaint_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.maint_id){
        setgroup_delete3f77f((pre:any)=>({...pre,maint_id:dfd_assetmaintenance_v1Props[0]?.maint_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[maint_id927de?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.maint_id){
    setgroup_delete3f77f((pre:any)=>({...pre,maint_id:dfd_assetmaintenance_v1Props[0]?.maint_id}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (maint_id927de?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 3`,gridRow: `49 / 50`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.maint_id : (group_delete3f77f?.maint_id || ""))}
</Text>
  </div>
  )
}
