 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "3ebf51922b660dc6120cc50a827d1793"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttond1793") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "3ebf51922b660dc6120cc50a827d1793") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttond1793");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttond1793?.trigger) return;
      if(ok_buttond1793?.trigger){
      setok_buttond1793((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttond1793?.trigger])

  useEffect(()=>{
    if(ok_buttond1793?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttond1793?.refresh])
  ...
  async function handleSave1793_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete3f77fProps = group_delete3f77fPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.maintenanceDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete3f77fRef.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenanceModify:AFVK:v1|d7f3324cb46d4a319d2fd37b2815a555"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenanceModify:AFVK:v1|d7f3324cb46d4a319d2fd37b2815a555"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(group_delete3f77f?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "3ebf51922b660dc6120cc50a827d1793",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "3ebf51922b660dc6120cc50a827d1793.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "3ebf51922b660dc6120cc50a827d1793.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "3ebf51922b660dc6120cc50a827d1793.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "3ebf51922b660dc6120cc50a827d1793.1.1.1.2",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenanceModify:AFVK:v1|d7f3324cb46d4a319d2fd37b2815a555"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:maintenanceDelete:AFVK:v1";
      sourceId+= "|"+"f5e5ab6658f380cb72c7f8b9a483f77f";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:maintenanceDelete:AFVK:v1"+"|"+"f5e5ab6658f380cb72c7f8b9a483f77f"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete3f77f?.upId? [group_delete3f77f?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3f77f?.upId? [group_delete3f77f?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3f77f?.upId? [group_delete3f77f?.upId ] : group_delete3f77f?.trs_process_id? [group_delete3f77f?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete3f77fRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"f5e5ab6658f380cb72c7f8b9a483f77f",
              controlId:"3ebf51922b660dc6120cc50a827d1793"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete3f77fProps.ssKey !== '' && group_delete3f77fProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete3f77fProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete3f77f((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave1793_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete3f77f((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_buttond1793?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
