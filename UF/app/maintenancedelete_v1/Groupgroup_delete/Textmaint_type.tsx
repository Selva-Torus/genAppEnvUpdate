...
const Textmaint_type = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetmaintenance_v1Props && !dfd_assetmaintenance_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3f77f((pre: any) => {
          return { ...pre, maint_type: api_paginationData.data.records[0]?.maint_type }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.maint_type){
        setgroup_delete3f77f((pre:any)=>({...pre,maint_type:dfd_assetmaintenance_v1Props[0]?.maint_type}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[maint_typefc524?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.maint_type){
    setgroup_delete3f77f((pre:any)=>({...pre,maint_type:dfd_assetmaintenance_v1Props[0]?.maint_type}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (maint_typefc524?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `15 / 21`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.maint_type : (group_delete3f77f?.maint_type || ""))}
</Text>
  </div>
  )
}

export default Textmaint_type
