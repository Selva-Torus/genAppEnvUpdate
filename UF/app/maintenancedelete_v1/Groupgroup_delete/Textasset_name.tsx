...
const Textasset_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetmaintenance_v1Props) && dfd_assetmaintenance_v1Props && !group_delete3f77f.asset_name){
        setgroup_delete3f77f((pre:any)=>({...pre,asset_name:dfd_assetmaintenance_v1Props[0]?.asset_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[asset_name9f8b1?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetmaintenance_v1Props) && !group_delete3f77f.asset_name){
    setgroup_delete3f77f((pre:any)=>({...pre,asset_name:dfd_assetmaintenance_v1Props[0]?.asset_name}));
  }
  },[dfd_assetmaintenance_v1Props])

  if (asset_name9f8b1?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `8 / 14`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.asset_name : (group_delete3f77f?.asset_name || ""))}
</Text>
  </div>
  )
}
