...
export default function PageMaintenancedeleteV1() {
  ...
  const {maintenancedelete_v1Props, setmaintenancedelete_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetmaintenance_v1DFD(pagination:any): Promise<void>{
        ...
        if(maintenancedelete_v1Props.length > 0){
          for(let i=0;i< maintenancedelete_v1Props.length;i++){
            if(maintenancedelete_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1"){
              // delete maintenancedelete_v1Props[i].DFDkey;
              let temp=structuredClone(maintenancedelete_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setmaintenancedelete_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:maintenanceDelete:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setmaintenancedelete_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(maintenancedelete_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(maintenancedelete_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
