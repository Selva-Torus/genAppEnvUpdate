...
export default function PageRequestFormV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const screenName:string = "request form";
  ...
  const {request_form_v1Props, setrequest_form_v1Props} = useContext(TotalContext) as TotalContextProps;
  const [checkgroup,setCheckgroup,]=useState<boolean>(false);
  const [checkgroup_two,setCheckgroup_two,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function claims_dfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(request_form_v1Props.length > 0){
          for(let i=0;i< request_form_v1Props.length;i++){
            if(request_form_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:claims_dfd:AFVK:v1"){
              // delete request_form_v1Props[i].DFDkey;
              let temp=structuredClone(request_form_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setrequest_form_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:Request_form:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
    ...
    await claims_dfd_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'group_two' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckgroup_two(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setrequest_form_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(request_form_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(request_form_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setrequest_form_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
