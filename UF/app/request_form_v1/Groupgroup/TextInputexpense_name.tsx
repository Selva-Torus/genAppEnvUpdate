...
const TextInputexpense_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:claims_dfd:AFVK:v1|1143fba1b035410d80b91c8996ee6d3a|responses.200.content.application/json.schema.items.properties.expense_name"
      ],
      "targetKey": "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:Request_form:AFVK:v1|5a69b1d62572431ab2933ca7cf0571d2|0a202d94f20b43c0b683fa29c60136a1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,Request_form_v1:{...pre?.Request_form_v1,expense_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroup571d2((prev: any) => ({ ...prev, expense_name: +e.target.value }));
    }
    else{
    setgroup571d2((prev: any) => ({ ...prev, expense_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "0a202d94f20b43c0b683fa29c60136a1"
      );
      ...
      //     controlId: "0a202d94f20b43c0b683fa29c60136a1",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "0a202d94f20b43c0b683fa29c60136a1") {
        handleChange({target:{value:group571d2Ref?.current?.expense_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "0a202d94f20b43c0b683fa29c60136a1") {
        handleBlur({target:{value:group571d2Ref?.current?.expense_name||""}});
      }
    };
  ...
  if(dfd_claims_dfd_v1Props?.setSearchFilters && dfd_claims_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_dfd_v1Props.data) && dfd_claims_dfd_v1Props.data.length > 0){
      setgroup571d2((pre:any)=>({...pre,expense_name:dfd_claims_dfd_v1Props.data[0]?.expense_name}));
    }
  }
  },[dfd_claims_dfd_v1Props?.setSearchFilters])
  if (expense_name136a1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `5 / 20`,gridRow: `19 / 34`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={group571d2?.expense_name||""}
         disabled= {expense_name136a1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.Request_form_v1?.expense_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
