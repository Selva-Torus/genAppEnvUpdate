...
const Buttonsave = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {saveec42b, setsaveec42b}= useContext(TotalContext) as TotalContextProps;
  //////////////
  const pendingAutoSearch = useRef(false);
  const preloadDone = useRef(false);
  // keep update group state in ref to access latest state value
  const group_two6135cRef = useRef(group_two6135c);
  useEffect(() => {
    group_two6135cRef.current = group_two6135c;
  ...
  const group_two6135cPropsRef = useRef(group_two6135cProps);
  useEffect(() => {
    group_two6135cPropsRef.current = group_two6135cProps;
  }, [group_two6135cProps]);
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['save'] = saveec42b,
        codeStates['setsave'] = setsaveec42b,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...group_two6135cRef.current};
      let parentRowSpan = 18;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "490e834d7bef401a8ed8a6a9ebb6135c",
        "7ad064a4534a45e68842944eacdec42b"
      );
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "7ad064a4534a45e68842944eacdec42b") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "saveec42b");
    return () => {
  ...
  useEffect(()=>{
    if (!saveec42b?.trigger) return;
      if(saveec42b?.trigger){
      setsaveec42b((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  async function handleSavec42b_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_two6135cProps = group_two6135cPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.Request_form_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_two6135cRef.current);
      ...
      let tagetKey:string="CK:CT001:FNGK:AF:FNK:PF-PFD:CATK:TAM:AFGK:TA:AFK:Request_form_default_event:AFVK:v1|4414bc997bdd4eaa99a8f9b22a8cd453"
      let uf_getPFDetails:any={
        key: "CK:CT001:FNGK:AF:FNK:PF-PFD:CATK:TAM:AFGK:TA:AFK:Request_form_default_event:AFVK:v1|4414bc997bdd4eaa99a8f9b22a8cd453"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(group_two6135c?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "7ad064a4534a45e68842944eacdec42b",
  "type": "button",
  "name": "save",
  "label": "save",
  "sequence": 1,
  "children": [
    {
      "id": "7ad064a4534a45e68842944eacdec42b.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "7ad064a4534a45e68842944eacdec42b.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "7ad064a4534a45e68842944eacdec42b.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
          ...
          "targetKey": [
            "CK:CT001:FNGK:AF:FNK:PF-PFD:CATK:TAM:AFGK:TA:AFK:Request_form_default_event:AFVK:v1|4414bc997bdd4eaa99a8f9b22a8cd453",
            "CK:CT001:FNGK:AF:FNK:PF-PFD:CATK:TAM:AFGK:TA:AFK:Request_PF:AFVK:v1|3666f177cebd4a4d9938b915cecb75ca"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:Request_form:AFVK:v1";
      sourceId+= "|"+"490e834d7bef401a8ed8a6a9ebb6135c";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:Request_form:AFVK:v1"+"|"+"490e834d7bef401a8ed8a6a9ebb6135c"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_two6135c?.upId? [group_two6135c?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_two6135c?.upId? [group_two6135c?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_two6135c?.upId? [group_two6135c?.upId ] : group_two6135c?.trs_process_id? [group_two6135c?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "save"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_two6135cRef.current, ...nullFilter(group571d2) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"490e834d7bef401a8ed8a6a9ebb6135c",
              controlId:"7ad064a4534a45e68842944eacdec42b"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_two6135cProps.ssKey !== '' && group_two6135cProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_two6135cProps.ssKey;          
    }
    te_eventEmitterBody["lock"] = actionLockData;
    if (group_two6135cProps?.tableName) {
      te_eventEmitterBody["tableName"] = group_two6135cProps?.tableName;
    }
    if (encryptionFlagCont) {
    ...
    toast(te_eventEmitter?.data?.data?.comments, 'warning')
    ///////////////////////

    //infoMsg
    toast('Data saved successfully', 'success',false,'code-2');
    }catch(err:any){
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{
      setIsProcessing(true);
        setgroup_two6135c((prev: any) => ({ ...prev, save: true }));
        //onClick

    //eventEmitter
    await handleSavec42b_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_two6135c((prev: any) => ({ ...prev, save: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (saveec42b?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `6 / 22`,gridRow: `4 / 14`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className=""
          ...
          disabled= {saveec42b?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Save")}
