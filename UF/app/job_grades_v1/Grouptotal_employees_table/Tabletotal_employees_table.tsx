...
let defaultColumns:any = [
  {
    "id": "grade_id",
    "nodeid": "c5cb2e14ecc8c205ac3f0ec0e5f09291",
    "name": "grade_id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "grade_code",
    "nodeid": "d256f7c52bfd5a625d91d846ee81f507",
    "name": "Grade Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "grade_name",
    "nodeid": "e53bc1abf638afdef64ff3f6879d440c",
    "name": "Grade Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "grade_level",
    "nodeid": "30c8bd6a92cd5105befecca0e26d01e1",
    "name": "Grade Level",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "salary_range",
    "nodeid": "fa3fd85b4194cc56dea712f22c07d11a",
    "name": "Salary Range",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "salary_range"
  },
  {
    "id": "promotion_eligible",
    "nodeid": "581b6b27034d1be52399b609772c98f8",
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "promotion_eligible"
  },
  {
    "id": "overtime_eligible",
    "nodeid": "b762bd283712a8cb72a47069182ff5a4",
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.total_employees_table && artifactRuleState?.total_employees_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.total_employees_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,total_employees_table:filteredData},{total_employees_table:artifactRuleState?.total_employees_table})
          if(result?.total_employees_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const Tabletotal_employees_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {employeejobgrades_v1Props, setemployeejobgrades_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    /// settotal_employees_table9c094Props
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settotal_employees_table9c094Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      total_employees_table9c094.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settotal_employees_table9c094Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settotal_employees_table9c094Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settotal_employees_table9c094Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(employeejobgrades_v1Props.length > 0){
      for(let i=0;i< employeejobgrades_v1Props.length;i++){
        if(employeejobgrades_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobGrade:AFVK:v1"){
          // delete employeejobgrades_v1Props[i].DFDkey;
          let temp=structuredClone(employeejobgrades_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobGrades:AFVK:v1:UO', 
                nodeId: 'ff40997343dd428ae50a99eca449c094',
                elementId: 'ff40997343dd428ae50a99eca449c094'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( total_employees_table9c094Props?.presetValues&&Object.keys(total_employees_table9c094Props?.presetValues).length > 0) {
          filtertedData = [total_employees_table9c094Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settotal_employees_table9c094(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={employeejobgrades_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (total_employees_table9c094Props.filterInitalLoad) return;

    const filterControllers = total_employees_table9c094Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['total_employees_table9c094']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['total_employees_table9c094']).filter((k: string) => jsonEntry['total_employees_table9c094'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(total_employees_table9c094Props.filterProps);
      return;
    }
    ...
    settotal_employees_table9c094Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(total_employees_table9c094Props.filterProps);

  }, [
    total_employees_table9c094Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    total_employees_table9c094Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(total_employees_table9c094Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(total_employees_table9c094Props.searchFilter) || {};
...
}, [total_employees_table9c094Props.searchFilter])
  ...
  useEffect(() => {
    if (total_employees_table9c094Props?.clearData === true) {
      settotal_employees_table9c094([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settotal_employees_table9c094Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [total_employees_table9c094Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [total_employees_table9c094Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = total_employees_table9c094Props?.selectedIds[total_employees_table9c094Props?.selectedIds?.length-1]
    if(Array.isArray(total_employees_table9c094) && total_employees_table9c094.length>0)
    {
      let data = total_employees_table9c094[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (total_employees_table9c094Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (total_employees_table9c094Props?.selectedIds?.length == 0){
      handleOnRowClick({},total_employees_table9c094Props?.selectedIds)
    }
  }, [total_employees_table9c094Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(total_employees_table9c094?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&total_employees_table9c094?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](total_employees_table9c094?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table9c094?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&total_employees_table9c094?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](total_employees_table9c094?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table9c094?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (total_employees_table9c094?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"ff40997343dd428ae50a99eca449c094"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={total_employees_table9c094Props?.selectedIds}  
...
export default Tabletotal_employees_table
