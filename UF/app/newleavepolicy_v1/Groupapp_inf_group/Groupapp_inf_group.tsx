...
const Groupapp_inf_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "90fb798a8b9499384ff751e486d2196e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapp_inf_group2196eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("approve_group")){
        setapprove_group0167c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approve_group0167c?.isDisabled==null)
      {
        setapprove_group0167c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("accrual_rules_dynamicjsonform")){
        setaccrual_rules_dynamicjsonform8866d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(accrual_rules_dynamicjsonform8866d?.isDisabled==null)
      {
        setaccrual_rules_dynamicjsonform8866d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("policy_rules_dynamicjsonform")){
        setpolicy_rules_dynamicjsonform2062a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(policy_rules_dynamicjsonform2062a?.isDisabled==null)
      {
        setpolicy_rules_dynamicjsonform2062a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_dts_dynamicjsonform")){
        setadd_dts_dynamicjsonform65fd5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_dts_dynamicjsonform65fd5?.isDisabled==null)
      {
        setadd_dts_dynamicjsonform65fd5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const app_inf_group2196eRef = useRef<any>(null);
  const handleClearSearch = () => {
    app_inf_group2196eRef.current?.setSearchParams();
    app_inf_group2196eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(app_inf_group2196e) && Object.keys(app_inf_group2196e)?.length>0)
      {
        setapp_inf_group2196e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [app_inf_group2196eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"app_inf_group"}))
        }}
    >
        ...
        {allowedControls.includes("accrual_rules_dynamicjsonform") ?<DynamicJsonFormaccrual_rules_dynamicjsonform   /* 8866d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("policy_rules_dynamicjsonform") ?<DynamicJsonFormpolicy_rules_dynamicjsonform   /* 2062a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("add_dts_dynamicjsonform") ?<DynamicJsonFormadd_dts_dynamicjsonform   /* 65fd5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
