...
const Groupapprove_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {adv_conf0a664, setadv_conf0a664}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "32f5ff8966b774685028a9ba52c0167c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapprove_group0167cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("adv_conf")){
        setadv_conf0a664((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(adv_conf0a664?.isDisabled==null)
      {
        setadv_conf0a664((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['adv_conf'] = adv_conf0a664,
        codeStates['setadv_conf'] = setadv_conf0a664,
  ...
  const approve_group0167cRef = useRef<any>(null);
  const handleClearSearch = () => {
    approve_group0167cRef.current?.setSearchParams();
    approve_group0167cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approve_group0167c) && Object.keys(approve_group0167c)?.length>0)
      {
        setapprove_group0167c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approve_group0167cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"approve_group"}))
        }}
    >
          {allowedControls.includes("adv_conf") ?<Textadv_conf   /* 0a664 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
