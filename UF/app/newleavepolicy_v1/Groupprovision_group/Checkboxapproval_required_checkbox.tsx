...
const Checkboxapproval_required_checkbox = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "b3dabcadf4914e9bbad284238ccf9d82"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[approval_required_checkboxf9d82?.refresh])

  useEffect(()=>{
    if (provision_groupc3fca?.approval_required === undefined) {
      setprovision_groupc3fca((prev: any) => ({ ...prev, approval_required: false }));
    }
  },[])
  ...
  const handleChange=async(checked:boolean)=>{
    ...
    setprovision_groupc3fca((prev: any) => ({ ...prev, approval_required: checked}));
    }catch (err: any) {
  ...
  if (approval_required_checkboxf9d82?.isHidden) {
    return <></>;
  }
  return (
    <div 
       style={{gridColumn: `1 / 11`,gridRow: `12 / 18`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Checkbox 
      className=""
      value={provision_groupc3fca?.approval_required||false}
      checked={provision_groupc3fca?.approval_required||false}
      disabled= {approval_required_checkboxf9d82?.isDisabled ? true : false}
