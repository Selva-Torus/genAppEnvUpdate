...
const TextInputmax_consecutive_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|7e3c285562b42eca97c3f153fae5c57c|ef6fa84eb4f942ee99652a5946c3d6d6"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,max_consecutive_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalid_group5c57c((prev: any) => ({ ...prev, max_consecutive_days: +e.target.value }));
    }
    else{
    setvalid_group5c57c((prev: any) => ({ ...prev, max_consecutive_days: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ef6fa84eb4f942ee99652a5946c3d6d6"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1",
      //     componentId: "7e3c285562b42eca97c3f153fae5c57c",
      //     controlId: "ef6fa84eb4f942ee99652a5946c3d6d6",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ef6fa84eb4f942ee99652a5946c3d6d6") {
        handleChange({target:{value:valid_group5c57cRef?.current?.max_consecutive_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ef6fa84eb4f942ee99652a5946c3d6d6") {
        handleBlur({target:{value:valid_group5c57cRef?.current?.max_consecutive_days||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setvalid_group5c57c((pre:any)=>({...pre,max_consecutive_days:dfd_addleavepolicymodify_v1Props.data[0]?.max_consecutive_days}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (max_consecutive_days3d6d6?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={valid_group5c57c?.max_consecutive_days||""}
         disabled= {max_consecutive_days3d6d6?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.newLeavePolicy_v1?.max_consecutive_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
