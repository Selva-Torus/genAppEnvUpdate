...
const TextInputcarry_forward_expiry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addLeavePolicyModify:AFVK:v1|4e279f3010e947cda8cc341d6c0ac371|properties.carry_forward_expiry"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|7e3c285562b42eca97c3f153fae5c57c|97ff79075eaa42ca982c7a050b2448cb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'carry_forward_expiry',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,carry_forward_expiry:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalid_group5c57c((prev: any) => ({ ...prev, carry_forward_expiry: +e.target.value }));
    }
    else{
    setvalid_group5c57c((prev: any) => ({ ...prev, carry_forward_expiry: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "97ff79075eaa42ca982c7a050b2448cb"
      );
      ...
      //     controlId: "97ff79075eaa42ca982c7a050b2448cb",
      //     isTable: false,
      //     from:"TextInputcarry_forward_expiry",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'carry_forward_expiry', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'carry_forward_expiry',type:'text'};
      //   type={
      //     name:'carry_forward_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.carry_forward_expiry.type
      //   }
      ...
      //   type={
      //     name:'carry_forward_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.carry_forward_expiry.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "97ff79075eaa42ca982c7a050b2448cb") {
        handleChange({target:{value:valid_group5c57cRef?.current?.carry_forward_expiry||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "97ff79075eaa42ca982c7a050b2448cb") {
        handleBlur({target:{value:valid_group5c57cRef?.current?.carry_forward_expiry||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setvalid_group5c57c((pre:any)=>({...pre,carry_forward_expiry:dfd_addleavepolicymodify_v1Props.data[0]?.carry_forward_expiry}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (carry_forward_expiry448cb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `16 / 25`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={valid_group5c57c?.carry_forward_expiry||""}
         disabled= {carry_forward_expiry448cb?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.newLeavePolicy_v1?.carry_forward_expiry ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcarry_forward_expiry
