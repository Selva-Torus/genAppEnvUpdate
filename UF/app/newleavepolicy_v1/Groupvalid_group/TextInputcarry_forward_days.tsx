...
const TextInputcarry_forward_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|7e3c285562b42eca97c3f153fae5c57c|5de78f765f534541b67621039fd3f3d5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,carry_forward_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalid_group5c57c((prev: any) => ({ ...prev, carry_forward_days: +e.target.value }));
    }
    else{
    setvalid_group5c57c((prev: any) => ({ ...prev, carry_forward_days: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5de78f765f534541b67621039fd3f3d5"
      );
      ...
      //     controlId: "5de78f765f534541b67621039fd3f3d5",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5de78f765f534541b67621039fd3f3d5") {
        handleChange({target:{value:valid_group5c57cRef?.current?.carry_forward_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5de78f765f534541b67621039fd3f3d5") {
        handleBlur({target:{value:valid_group5c57cRef?.current?.carry_forward_days||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setvalid_group5c57c((pre:any)=>({...pre,carry_forward_days:dfd_addleavepolicymodify_v1Props.data[0]?.carry_forward_days}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (carry_forward_days3f3d5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={valid_group5c57c?.carry_forward_days||""}
         disabled= {carry_forward_days3f3d5?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.newLeavePolicy_v1?.carry_forward_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
