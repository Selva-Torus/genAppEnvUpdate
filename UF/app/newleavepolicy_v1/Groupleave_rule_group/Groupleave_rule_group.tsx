...
const Groupleave_rule_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {leave_rules13299, setleave_rules13299}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9b07d1e2a791c557e82fed77f7ef75c0");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setleave_rule_groupf75c0Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("leave_rules")){
        setleave_rules13299((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_rules13299?.isDisabled==null)
      {
        setleave_rules13299((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['leave_rules'] = leave_rules13299,
        codeStates['setleave_rules'] = setleave_rules13299,
  ...
  const leave_rule_groupf75c0Ref = useRef<any>(null);
  const handleClearSearch = () => {
    leave_rule_groupf75c0Ref.current?.setSearchParams();
    leave_rule_groupf75c0Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(leave_rule_groupf75c0) && Object.keys(leave_rule_groupf75c0)?.length>0)
      {
        setleave_rule_groupf75c0({})
      }
    }else 
      prevRefreshRef.current= true
  }, [leave_rule_groupf75c0Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"leave_rule_group"}))
        }}
    >
          {allowedControls.includes("leave_rules") ?<Textleave_rules   /* 13299 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
