...
const TextInputpolicy_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|ea310355e51365a74a46ea7eaf9ae6e3|670d53d2b25c41e3a22ed37ab3555cd1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,policy_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__groupae6e3((prev: any) => ({ ...prev, policy_name: +e.target.value }));
    }
    else{
    setaccess_req__groupae6e3((prev: any) => ({ ...prev, policy_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "670d53d2b25c41e3a22ed37ab3555cd1"
      );
      ...
      //     controlId: "670d53d2b25c41e3a22ed37ab3555cd1",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "670d53d2b25c41e3a22ed37ab3555cd1") {
        handleChange({target:{value:access_req__groupae6e3Ref?.current?.policy_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "670d53d2b25c41e3a22ed37ab3555cd1") {
        handleBlur({target:{value:access_req__groupae6e3Ref?.current?.policy_name||""}});
      }
    };
  ...
  if(dfd_addleavepolicymodify_v1Props?.setSearchFilters && dfd_addleavepolicymodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addleavepolicymodify_v1Props.data) && dfd_addleavepolicymodify_v1Props.data.length > 0){
      setaccess_req__groupae6e3((pre:any)=>({...pre,policy_name:dfd_addleavepolicymodify_v1Props.data[0]?.policy_name}));
    }
  }
  },[dfd_addleavepolicymodify_v1Props?.setSearchFilters])
  if (policy_name55cd1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__groupae6e3?.policy_name||""}
         disabled= {policy_name55cd1?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.newLeavePolicy_v1?.policy_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
