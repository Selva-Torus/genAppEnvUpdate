 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "1dffe24893169e6ed89731404875c873"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("save" in dynamicactionsd8c40Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsd8c40Props?.dynamicActionRule?.save,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("save" in newleavepolicy_v1?.dynamicactions && newleavepolicy_v1?.dynamicactions["save"]?.itsHaveArtifact== true)
      {
        setShowFlag(newleavepolicy_v1?.dynamicactions["save"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newleavepolicy_v1?.dynamicactions?.save?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "1dffe24893169e6ed89731404875c873") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "save5c873");
    return () => {
  ...
  useEffect(()=>{
    if (!save5c873?.trigger) return;
      if(save5c873?.trigger){
      setsave5c873((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  async function handleSavec873_1_1_1(){
  ...
  const hasInvalidField = Object.values(currentValidate?.newLeavePolicy_v1 || {}).some(val => {
    ...
    try{
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|af9806fd59ff953bc4b7e758c863d617"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|af9806fd59ff953bc4b7e758c863d617"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "1dffe24893169e6ed89731404875c873",
  "type": "button",
  "name": "save",
  "label": "save",
  "sequence": 1,
  "children": [
    {
      "id": "1dffe24893169e6ed89731404875c873.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "1dffe24893169e6ed89731404875c873.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "1dffe24893169e6ed89731404875c873.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "1dffe24893169e6ed89731404875c873.1.1.1.2",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:newLeavePolicy:AFVK:v1|af9806fd59ff953bc4b7e758c863d617"
          ]
        }
      ]
    }
  ...
  // saveHandler
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"1dffe24893169e6ed89731404875c873"
            };
            if (encryptionFlagCont) {
        ...
        if(dynamicactionsd8c40?.upId){
          te_saveBody['upId']= dynamicactionsd8c40?.upId;
        }
        if(dynamicactionsd8c40?.upid){
          te_saveBody['upId']= dynamicactionsd8c40?.upid;
        }
        ...
        if (dynamicactionsd8c40Props.ssKey !== '' && dynamicactionsd8c40Props.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactionsd8c40Props.ssKey;
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsd8c40((prev: any) => ({ ...prev, save: true }));
        //onClick

    //saveHandler
    await handleSavec873_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsd8c40((prev: any) => ({ ...prev, save: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newleavepolicy_v1?.dynamicactions?.save,dynamicactionsd8c40Props?.dynamicActionRule?.save,])

 if (save5c873?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='normal'
          disabled= {save5c873?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
