...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fde9e81841e505a060bcbca3ce5d8c40");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactionsd8c40Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel18d41((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel18d41?.isDisabled==null)
      {
        setcancel18d41((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_update83771((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_update83771?.isDisabled==null)
      {
        setbutton_update83771((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("save")){
        setsave5c873((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(save5c873?.isDisabled==null)
      {
        setsave5c873((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactionsd8c40Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactionsd8c40Ref.current?.setSearchParams();
    dynamicactionsd8c40Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactionsd8c40Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactionsd8c40Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactionsd8c40) && Object.keys(dynamicactionsd8c40)?.length>0)
      {
        setdynamicactionsd8c40({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactionsd8c40Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
