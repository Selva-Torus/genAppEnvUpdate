...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "14fea52dceb8ebd6b2b1e82f5cc86c35");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_group86c35Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__groupae6e3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__groupae6e3?.isDisabled==null)
      {
        setaccess_req__groupae6e3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_inf_group")){
        setapp_inf_group2196e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_inf_group2196e?.isDisabled==null)
      {
        setapp_inf_group2196e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_group")){
        setvalid_group5c57c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_group5c57c?.isDisabled==null)
      {
        setvalid_group5c57c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("business_just__group")){
        setbusiness_just__groupd6ebd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_just__groupd6ebd?.isDisabled==null)
      {
        setbusiness_just__groupd6ebd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("provision_group")){
        setprovision_groupc3fca((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(provision_groupc3fca?.isDisabled==null)
      {
        setprovision_groupc3fca((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("policy_id")){
        setpolicy_id895de((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(policy_id895de?.isDisabled==null)
      {
        setpolicy_id895de((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactionsd8c40((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactionsd8c40?.isDisabled==null)
      {
        setdynamicactionsd8c40((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['policy_id'] = policy_id895de,
        codeStates['setpolicy_id'] = setpolicy_id895de,
  ...
  const new_access_group86c35Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_group86c35Ref.current?.setSearchParams();
    new_access_group86c35Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_group86c35) && Object.keys(new_access_group86c35)?.length>0)
      {
        setnew_access_group86c35({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_group86c35Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("policy_id") ?<Textpolicy_id   /* 895de */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
