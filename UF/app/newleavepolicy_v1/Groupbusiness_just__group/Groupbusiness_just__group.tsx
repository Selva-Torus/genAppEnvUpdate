...
const Groupbusiness_just__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5f21bd74f1d1a9a4e352b1be510d6ebd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbusiness_just__groupd6ebdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_rules")){
        setapp_rules6b502((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_rules6b502?.isDisabled==null)
      {
        setapp_rules6b502((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("applicable_to")){
        setapplicable_toc8db1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(applicable_toc8db1?.isDisabled==null)
      {
        setapplicable_toc8db1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("gender_applicability")){
        setgender_applicability02992((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(gender_applicability02992?.isDisabled==null)
      {
        setgender_applicability02992((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("applicable_locations")){
        setapplicable_locations79a76((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(applicable_locations79a76?.isDisabled==null)
      {
        setapplicable_locations79a76((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const business_just__groupd6ebdRef = useRef<any>(null);
  const handleClearSearch = () => {
    business_just__groupd6ebdRef.current?.setSearchParams();
    business_just__groupd6ebdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(business_just__groupd6ebd) && Object.keys(business_just__groupd6ebd)?.length>0)
      {
        setbusiness_just__groupd6ebd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [business_just__groupd6ebdProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewleavepolicy_v1((pre:any)=>({...pre,_selectedGroup_:"business_just__group"}))
        }}
    >
          {allowedControls.includes("app_rules") ?<Textapp_rules   /* 6b502 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("applicable_to") ?<Dropdownapplicable_to   /* c8db1 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("gender_applicability") ?<Dropdowngender_applicability   /* 02992 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("applicable_locations") ?<Dropdownapplicable_locations   /* 79a76 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
