...
const Dropdownapplicable_to = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(business_just__groupd6ebd?.applicable_to=="" || business_just__groupd6ebd?.applicable_to==undefined || business_just__groupd6ebd?.applicable_to==null ){
    setSelectedItem("");
  }
  },[business_just__groupd6ebd?.applicable_to])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "5f21bd74f1d1a9a4e352b1be510d6ebd",
        "015e6f8301ba44f9881b0f0af99c8db1"
      );
  ...
  },
  {
    ...
    "value": {
      ...
      setbusiness_just__groupd6ebd((prev: any) => ({ ...prev, applicable_to: staticTextValue, applicable_toc8db1: value}))
         setIsRequredData(false)
    } else {
       setbusiness_just__groupd6ebd((prev: any) => ({ ...prev, applicable_to: '', applicable_toc8db1: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,applicable_to:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(business_just__groupd6ebd?.applicable_to == "" || business_just__groupd6ebd?.applicable_to == undefined){
      business_just__groupd6ebd.applicable_to = "";
      const validate:any = v.safeParse(schema, business_just__groupd6ebd?.applicable_to);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,applicable_to:"invalid"}}));
        }
    }else if(business_just__groupd6ebd?.applicable_to !== ""){
    const validate:any = v.safeParse(schema, business_just__groupd6ebd?.applicable_to);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,applicable_to:"invalid"}}));
  ...
  useEffect(() => { business_just__groupd6ebdRef.current = business_just__groupd6ebd; }, [business_just__groupd6ebd]);
    useEffect(()=>{
        if(!business_just__groupd6ebd?.applicable_to)
        { 
       ...
       const handler = (id:any) => {
          if (id === "015e6f8301ba44f9881b0f0af99c8db1") {
        handleClick(business_just__groupd6ebdRef?.current?.applicable_toc8db1?business_just__groupd6ebdRef?.current?.applicable_toc8db1:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "015e6f8301ba44f9881b0f0af99c8db1");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbusiness_just__groupd6ebd((pre:any)=>({...pre,applicable_to:""}))
    else
      setInitialCount(1)
  },[applicable_toc8db1?.refresh])
  

  if (applicable_toc8db1?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {applicable_toc8db1?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            business_just__groupd6ebd?.applicable_toc8db1 ? [business_just__groupd6ebd?.applicable_toc8db1] :
                business_just__groupd6ebd?.applicable_to ? business_just__groupd6ebd?.applicable_to : []
            }
        onChange={handleClick} 
        validationState={validate?.newLeavePolicy_v1?.applicable_to ? "invalid" : undefined}
        errorMessage={error}
      /> 
    </div>
  );
...
export default Dropdownapplicable_to;
