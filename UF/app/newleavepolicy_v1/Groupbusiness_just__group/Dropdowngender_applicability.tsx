 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "83f4b4fba9a141b0a0e205a988602992"
    );
  ...
  },[gender_applicability02992?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbusiness_just__groupd6ebd((prev: any) => ({ ...prev,gender_applicability02992: value }))
      setIsRequredData(false)
    }else{
      let temp:any = business_just__groupd6ebd;
      delete temp.gender_applicability;
      delete temp.GENDER_APPLICABILITY;
      delete temp.gender_applicability02992;
      setbusiness_just__groupd6ebd(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,gender_applicability:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(business_just__groupd6ebd.gender_applicability){
      if(Array.isArray(dfd_gendercombo_v1Props)){
        if(dfd_gendercombo_v1Props?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)){
          setdropdownValue([dfd_gendercombo_v1Props?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)?.gender])
          tempValue=dfd_gendercombo_v1Props?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)?.gender
        }else{
          setdropdownValue([business_just__groupd6ebd.gender_applicability])
          tempValue=business_just__groupd6ebd.gender_applicability
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{gender:business_just__groupd6ebd.gender_applicability}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)?.gender ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.gender === business_just__groupd6ebd.gender_applicability)?.gender
      }else{
        setdropdownValue([business_just__groupd6ebd.gender_applicability])
        tempValue=business_just__groupd6ebd.gender_applicability
      }   
    ...
    if(gender_applicability02992?.trigger == true)
    {
      await handlechange(tempValue)
      setgender_applicability02992((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[business_just__groupd6ebd.gender_applicability, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_gendercombo_v1Props) && dfd_gendercombo_v1Props?.length == 1){
    // setbusiness_just__groupd6ebd((pre:any)=>({...pre,gender_applicability:dfd_gendercombo_v1Props[0]?.gender_applicability}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbusiness_just__groupd6ebd((prev: any) => ({ ...prev, gender_applicability: getMapperDetailsBindValues[value],GENDER_APPLICABILITY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbusiness_just__groupd6ebd((prev: any) => ({ ...prev, gender_applicability: '', gender_applicability02992: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,gender_applicability:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(business_just__groupd6ebd?.gender_applicability == "" || business_just__groupd6ebd?.gender_applicability == undefined){
      business_just__groupd6ebd.gender_applicability = "";
      const validate:any = v.safeParse(schema, business_just__groupd6ebd?.gender_applicability);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,gender_applicability:"invalid"}}));
        }
    }else if(business_just__groupd6ebd?.gender_applicability !== ""){
    const validate:any = v.safeParse(schema, business_just__groupd6ebd?.gender_applicability);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newLeavePolicy_v1:{...pre?.newLeavePolicy_v1,gender_applicability:"invalid"}}));
  ...
  useEffect(() => { business_just__groupd6ebdRef.current = business_just__groupd6ebd; }, [business_just__groupd6ebd]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "83f4b4fba9a141b0a0e205a988602992") {
        handleClick(business_just__groupd6ebdRef?.current?.gender_applicability02992?business_just__groupd6ebdRef?.current?.gender_applicability02992:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "83f4b4fba9a141b0a0e205a988602992");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbusiness_just__groupd6ebd((pre:any)=>({...pre,gender_applicability:""}))
    else
      setInitialCount(1)
  },[gender_applicability02992?.refresh])
  

  if (gender_applicability02992?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {gender_applicability02992?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={business_just__groupd6ebd?.gender_applicability02992 ? [business_just__groupd6ebd?.gender_applicability02992] : (business_just__groupd6ebd?.gender_applicability ? dropdownValue : [])}
        validationState={validate?.newLeavePolicy_v1?.gender_applicability ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
