 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "c2ed01e6feaabe76c218c01ec04e6d7f"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttone6d7f") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "c2ed01e6feaabe76c218c01ec04e6d7f") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttone6d7f");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttone6d7f?.trigger) return;
      if(ok_buttone6d7f?.trigger){
      setok_buttone6d7f((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttone6d7f?.trigger])

  useEffect(()=>{
    if(ok_buttone6d7f?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttone6d7f?.refresh])
  ...
  async function handleSave6d7f_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete10eb3Props = group_delete10eb3PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.assetDocDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete10eb3Ref.current);
      ...
      let lockedKeysLength:number;
        if(group_delete10eb3?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "c2ed01e6feaabe76c218c01ec04e6d7f",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "c2ed01e6feaabe76c218c01ec04e6d7f.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "c2ed01e6feaabe76c218c01ec04e6d7f.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "c2ed01e6feaabe76c218c01ec04e6d7f.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "c2ed01e6feaabe76c218c01ec04e6d7f.1.1.1.2",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDocDelete:AFVK:v1";
      sourceId+= "|"+"64ffca7df7aa52a114926f0c91610eb3";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDocDelete:AFVK:v1"+"|"+"64ffca7df7aa52a114926f0c91610eb3"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete10eb3?.upId? [group_delete10eb3?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete10eb3?.upId? [group_delete10eb3?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete10eb3?.upId? [group_delete10eb3?.upId ] : group_delete10eb3?.trs_process_id? [group_delete10eb3?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete10eb3Ref.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"64ffca7df7aa52a114926f0c91610eb3",
              controlId:"c2ed01e6feaabe76c218c01ec04e6d7f"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete10eb3Props.ssKey !== '' && group_delete10eb3Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete10eb3Props.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete10eb3((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave6d7f_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete10eb3((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_buttone6d7f?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Delete")}
