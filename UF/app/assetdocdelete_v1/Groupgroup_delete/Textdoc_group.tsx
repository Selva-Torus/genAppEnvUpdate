...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetdoctable_v1Props) && dfd_assetdoctable_v1Props && !group_delete10eb3.doc_group){
        setgroup_delete10eb3((pre:any)=>({...pre,doc_group:dfd_assetdoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_group82055?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdoctable_v1Props) && !group_delete10eb3.doc_group){
    setgroup_delete10eb3((pre:any)=>({...pre,doc_group:dfd_assetdoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_assetdoctable_v1Props])

  if (doc_group82055?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_delete10eb3?.doc_group || ""))}
</Text>
  </div>
  )
}
