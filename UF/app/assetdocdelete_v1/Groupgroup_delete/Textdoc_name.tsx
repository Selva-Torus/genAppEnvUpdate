...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetdoctable_v1Props) && dfd_assetdoctable_v1Props && !group_delete10eb3.doc_name){
        setgroup_delete10eb3((pre:any)=>({...pre,doc_name:dfd_assetdoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_name1f607?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetdoctable_v1Props) && !group_delete10eb3.doc_name){
    setgroup_delete10eb3((pre:any)=>({...pre,doc_name:dfd_assetdoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_assetdoctable_v1Props])

  if (doc_name1f607?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete10eb3?.doc_name || ""))}
</Text>
  </div>
  )
}
