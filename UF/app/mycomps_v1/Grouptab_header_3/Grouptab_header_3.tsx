...
const Grouptab_header_3 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "62837663252c4201b93ba864a3970ce9");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settab_header_370ce9Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("groupa")){
        setgroupa1825e({...groupa1825e,isDisabled:true});

    }else
    {
      if(groupa1825e?.isDisabled==null)
      {
        setgroupa1825e({...groupa1825e,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tab_group4b1a3?.tab_group === "tab_header_3"){
    handleOnChange()
    }
  ...
  const tab_header_370ce9Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header_370ce9Ref.current?.setSearchParams();
    tab_header_370ce9Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_header_370ce9) && Object.keys(tab_header_370ce9)?.length>0)
      {
        settab_header_370ce9({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_header_370ce9Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmycomps_v1((pre:any)=>({...pre,_selectedGroup_:"tab_header_3"}))
        }}
    >
...
export default Grouptab_header_3
