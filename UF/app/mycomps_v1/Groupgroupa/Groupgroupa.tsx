...
const Groupgroupa = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a6e3673ee0a84b2094492a95cef1825e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroupa1825eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("textinput")){
        settextinput5c43e({...textinput5c43e,isDisabled:true});

    }else
    {
      if(textinput5c43e?.isDisabled==null)
      {
        settextinput5c43e({...textinput5c43e,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("textarea")){
        settextarea1bbf8({...textarea1bbf8,isDisabled:true});

    }else
    {
      if(textarea1bbf8?.isDisabled==null)
      {
        settextarea1bbf8({...textarea1bbf8,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const groupa1825eRef = useRef<any>(null);
  const handleClearSearch = () => {
    groupa1825eRef.current?.setSearchParams();
    groupa1825eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupa1825e) && Object.keys(groupa1825e)?.length>0)
      {
        setgroupa1825e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupa1825eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmycomps_v1((pre:any)=>({...pre,_selectedGroup_:"groupa"}))
        }}
    >
        {allowedControls.includes("textinput") ?<TextInputtextinput   /* 5c43e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("textarea") ?<TextAreatextarea   /* 1bbf8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
