...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "442de4c61b794cb988d18a74e745635d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup5635dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("combobox")){
        setcomboboxb40b7({...comboboxb40b7,isDisabled:true});

    }else
    {
      if(comboboxb40b7?.isDisabled==null)
      {
        setcomboboxb40b7({...comboboxb40b7,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("grouparray")){
        setgrouparray81c1a({...grouparray81c1a,isDisabled:true});

    }else
    {
      if(grouparray81c1a?.isDisabled==null)
      {
        setgrouparray81c1a({...grouparray81c1a,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pininput")){
        setpininputdfec6({...pininputdfec6,isDisabled:true});

    }else
    {
      if(pininputdfec6?.isDisabled==null)
      {
        setpininputdfec6({...pininputdfec6,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text_to_speech")){
        settext_to_speechdcae4({...text_to_speechdcae4,isDisabled:true});

    }else
    {
      if(text_to_speechdcae4?.isDisabled==null)
      {
        settext_to_speechdcae4({...text_to_speechdcae4,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("tab_group")){
        settab_group4b1a3({...tab_group4b1a3,isDisabled:true});

    }else
    {
      if(tab_group4b1a3?.isDisabled==null)
      {
        settab_group4b1a3({...tab_group4b1a3,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const group5635dRef = useRef<any>(null);
  const handleClearSearch = () => {
    group5635dRef.current?.setSearchParams();
    group5635dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group5635d) && Object.keys(group5635d)?.length>0)
      {
        setgroup5635d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group5635dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmycomps_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
        ...
        {allowedComponent.includes("tab_group")  &&<Grouptab_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedControls.includes("combobox") ?<ComboBoxcombobox /* b40b7 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("pininput") ?<PinInputpininput   /* dfec6 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("text_to_speech") ?<TextToSpeechOutputtext_to_speech   /* dcae4 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
