...
export default function PageMycompsV1() {
  ...
  const screenName:string = "mycomps";
  ...
  const [checkgroupa,setCheckgroupa,]=useState<boolean>(false);
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setmycomps_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    await country_code_dfd_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'groupa' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckgroupa(true)
            }
          })
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setmycomps_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(mycomps_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
tab_group:tab_group4b1a3.tab_group,      }
      handleArtfactRule(mycomps_v1?._artfactPFRule_,data,allRuleData)
    }
  },[tab_group4b1a3.tab_group,])

  const parentRef:any = useRef(null);
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setmycomps_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
