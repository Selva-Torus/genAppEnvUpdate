...
const Grouptab_header_2 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a55c0d682fe64798b676b2351f92515d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settab_header_22515dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("groupb")){
        setgroupb5565e({...groupb5565e,isDisabled:true});

    }else
    {
      if(groupb5565e?.isDisabled==null)
      {
        setgroupb5565e({...groupb5565e,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const tab_header_22515dRef = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header_22515dRef.current?.setSearchParams();
    tab_header_22515dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_header_22515d) && Object.keys(tab_header_22515d)?.length>0)
      {
        settab_header_22515d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_header_22515dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmycomps_v1((pre:any)=>({...pre,_selectedGroup_:"tab_header_2"}))
        }}
    >
