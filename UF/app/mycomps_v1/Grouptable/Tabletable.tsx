...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "53ff46129f5a4b2aa1f3d8ab50d961dc",
    "name": "country",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {mycomps_v1Props, setmycomps_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settable050ebProps((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      table050eb.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.ggg)
      ...
      //////////
      //////////
      settable050ebProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      settable050ebProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    setSelectedPaginationData(checkedData)
            // infoMsg
  if (decodedTokenObj?.roleName === "Torus") {
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable050ebProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(mycomps_v1Props.length > 0){
      for(let i=0;i< mycomps_v1Props.length;i++){
        if(mycomps_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TGW01:AFGK:TGW004:AFK:country_code_dfd:AFVK:v1"){
          // delete mycomps_v1Props[i].DFDkey;
          let temp=structuredClone(mycomps_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:mycomps:AFVK:v1:UO', 
                nodeId: 'a81a9487cfe24022a2cdc0b2d36050eb',
                elementId: 'a81a9487cfe24022a2cdc0b2d36050eb'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table050ebProps?.presetValues&&Object.keys(table050ebProps?.presetValues).length > 0) {
          filtertedData = [table050ebProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable050eb(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in mycomps_v1?.table && mycomps_v1?.table[controller]?.itsHaveArtifact== true)
      {
        if(mycomps_v1?.table[controller])
        {
          let result :any = await getAftfactLevelRule(mycomps_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,table:filteredData},{table:mycomps_v1?.table})
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table050ebProps.filterInitalLoad) return;

    const filterControllers = table050ebProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table050eb']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table050eb']).filter((k: string) => jsonEntry['table050eb'][k] === false)
      : [];
    ...
    settable050ebProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table050ebProps.filterProps);

  }, [
    table050ebProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table050ebProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table050ebProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table050ebProps.searchFilter) || {};
  ...
  useEffect(() => {
    if (table050ebProps?.clearData === true) {
      settable050eb([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      settable050ebProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table050ebProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [table050ebProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = table050ebProps?.selectedIds[table050ebProps?.selectedIds?.length-1]
    if(Array.isArray(table050eb) && table050eb.length>0)
    {
      let data = table050eb[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "ggg",
        value: data["ggg"],
  ...
  useEffect(() => {
    if (table050ebProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (table050ebProps?.selectedIds?.length == 0){
      handleOnRowClick({},table050ebProps?.selectedIds)
    }
  }, [table050ebProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table050eb?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table050eb?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table050eb?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table050eb?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table050eb?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table050eb?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table050eb?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"a81a9487cfe24022a2cdc0b2d36050eb"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableSelection={true}
              selectedIds={table050ebProps?.selectedIds}  
