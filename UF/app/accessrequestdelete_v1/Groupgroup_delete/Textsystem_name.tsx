...
const Textsystem_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addaccessrequestmodify_v1Props && !dfd_addaccessrequestmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete39e95((pre: any) => {
          return { ...pre, system_name: api_paginationData.data.records[0]?.system_name }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props && !group_delete39e95.system_name){
        setgroup_delete39e95((pre:any)=>({...pre,system_name:dfd_addaccessrequestmodify_v1Props[0]?.system_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[system_named477d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && !group_delete39e95.system_name){
    setgroup_delete39e95((pre:any)=>({...pre,system_name:dfd_addaccessrequestmodify_v1Props[0]?.system_name}));
  }
  },[dfd_addaccessrequestmodify_v1Props])

  if (system_named477d?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.system_name : (group_delete39e95?.system_name || ""))}
</Text>
  </div>
  )
}

export default Textsystem_name
