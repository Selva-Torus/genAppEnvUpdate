...
const Textaccess_role = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addaccessrequestmodify_v1Props && !dfd_addaccessrequestmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete39e95((pre: any) => {
          return { ...pre, access_role: api_paginationData.data.records[0]?.access_role }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props && !group_delete39e95.access_role){
        setgroup_delete39e95((pre:any)=>({...pre,access_role:dfd_addaccessrequestmodify_v1Props[0]?.access_role}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[access_rolef3310?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && !group_delete39e95.access_role){
    setgroup_delete39e95((pre:any)=>({...pre,access_role:dfd_addaccessrequestmodify_v1Props[0]?.access_role}));
  }
  },[dfd_addaccessrequestmodify_v1Props])

  if (access_rolef3310?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.access_role : (group_delete39e95?.access_role || ""))}
</Text>
  </div>
  )
}

export default Textaccess_role
