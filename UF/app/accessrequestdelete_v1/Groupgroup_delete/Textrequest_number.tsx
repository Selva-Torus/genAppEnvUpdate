...
const Textrequest_number = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addaccessrequestmodify_v1Props && !dfd_addaccessrequestmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete39e95((pre: any) => {
          return { ...pre, request_number: api_paginationData.data.records[0]?.request_number }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props && !group_delete39e95.request_number){
        setgroup_delete39e95((pre:any)=>({...pre,request_number:dfd_addaccessrequestmodify_v1Props[0]?.request_number}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[request_numbera8283?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && !group_delete39e95.request_number){
    setgroup_delete39e95((pre:any)=>({...pre,request_number:dfd_addaccessrequestmodify_v1Props[0]?.request_number}));
  }
  },[dfd_addaccessrequestmodify_v1Props])

  if (request_numbera8283?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.request_number : (group_delete39e95?.request_number || ""))}
</Text>
  </div>
  )
}

export default Textrequest_number
