...
const Textaccess_req_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props && !group_delete39e95.access_req_id){
        setgroup_delete39e95((pre:any)=>({...pre,access_req_id:dfd_addaccessrequestmodify_v1Props[0]?.access_req_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[access_req_id2b2a7?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && !group_delete39e95.access_req_id){
    setgroup_delete39e95((pre:any)=>({...pre,access_req_id:dfd_addaccessrequestmodify_v1Props[0]?.access_req_id}));
  }
  },[dfd_addaccessrequestmodify_v1Props])

  if (access_req_id2b2a7?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 5`,gridRow: `45 / 46`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.access_req_id : (group_delete39e95?.access_req_id || ""))}
</Text>
  </div>
  )
}
