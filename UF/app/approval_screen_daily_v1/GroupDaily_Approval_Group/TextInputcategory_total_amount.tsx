...
const TextInputcategory_total_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Approval_Screen_Daily:AFVK:v1|91dcb5fe99014c21add21a4d00669531|ee09b38e61cf413a9a60d57d5a5cf2e2"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=``;
     if (code != '') {
      let codeStates: any = {};
      codeStates['daily_approval_group']  = {...daily_approval_group69531,category_total_amount:newInputValue},
    ...
    setdaily_approval_group69531((prev: any) => ({ ...prev, category_total_amount: +e.target.value }));
    }
    else{
    setdaily_approval_group69531((prev: any) => ({ ...prev, category_total_amount: e.target.value }));
    }
  ...
  if(dfd_claims_v1Props?.setSearchFilters && dfd_claims_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_v1Props.data) && dfd_claims_v1Props.data.length > 0){
      setdaily_approval_group69531((pre:any)=>({...pre,category_total_amount:dfd_claims_v1Props.data[0]?.category_total_amount}));
    }
  }
  },[dfd_claims_v1Props?.setSearchFilters])
  if (category_total_amountcf2e2?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 23`,gridRow: `51 / 70`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={daily_approval_group69531?.category_total_amount||""}
         disabled= {category_total_amountcf2e2?.isDisabled ? true : false}
        pin='brick-brick'     
