...
const TextInputclaim_category = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Approval_Screen_Daily:AFVK:v1|91dcb5fe99014c21add21a4d00669531|0cc243821fee47f695a7cb783a846dd0"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=``;
     if (code != '') {
      let codeStates: any = {};
      codeStates['daily_approval_group']  = {...daily_approval_group69531,claim_category:newInputValue},
    ...
    setdaily_approval_group69531((prev: any) => ({ ...prev, claim_category: +e.target.value }));
    }
    else{
    setdaily_approval_group69531((prev: any) => ({ ...prev, claim_category: e.target.value }));
    }
  ...
  if(dfd_claims_v1Props?.setSearchFilters && dfd_claims_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_v1Props.data) && dfd_claims_v1Props.data.length > 0){
      setdaily_approval_group69531((pre:any)=>({...pre,claim_category:dfd_claims_v1Props.data[0]?.claim_category}));
    }
  }
  },[dfd_claims_v1Props?.setSearchFilters])
  if (claim_category46dd0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `10 / 16`,gridRow: `51 / 71`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={daily_approval_group69531?.claim_category||""}
         disabled= {claim_category46dd0?.isDisabled ? true : false}
        pin='brick-brick'     
