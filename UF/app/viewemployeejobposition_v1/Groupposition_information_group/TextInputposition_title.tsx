...
const TextInputposition_title = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|c7b866caed721de00fbfdfc1f8967802|4b93b3fefcebcf01943afbc269f6e1ab"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,position_title:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group67802((prev: any) => ({ ...prev, position_title: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(position_information_group67802?.position_title == "" || position_information_group67802?.position_title == undefined){
      position_information_group67802.position_title = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, position_information_group67802?.position_title);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,position_title:"invalid"}}));
        }
    }else if(position_information_group67802?.position_title !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +position_information_group67802?.position_title);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "4b93b3fefcebcf01943afbc269f6e1ab"
      );
      ...
      //     controlId: "4b93b3fefcebcf01943afbc269f6e1ab",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!position_information_group67802?.position_title)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "4b93b3fefcebcf01943afbc269f6e1ab") {
        handleChange({target:{value:position_information_group67802Ref?.current?.position_title||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4b93b3fefcebcf01943afbc269f6e1ab") {
        handleBlur({target:{value:position_information_group67802Ref?.current?.position_title||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group67802((pre:any)=>({...pre,position_title:dfd_jobpositions_v1Props.data[0]?.position_title}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (position_title6e1ab?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={position_information_group67802?.position_title||""}
         disabled= {position_title6e1ab?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.position_title ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
