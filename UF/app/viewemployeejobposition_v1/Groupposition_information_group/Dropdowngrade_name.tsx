 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "58c851fe96e876f030ca4d9322411b6f"
    );
  ...
  },[grade_name11b6f?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setposition_information_group67802((prev: any) => ({ ...prev,grade_name11b6f: value }))
      setIsRequredData(false)
    }else{
      let temp:any = position_information_group67802;
      delete temp.grade_code_name;
      delete temp.GRADE_CODE_NAME;
      delete temp.grade_name11b6f;
      setposition_information_group67802(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,grade_code_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(position_information_group67802.grade_code_name){
      if(Array.isArray(dfd_gradenamecombo_v1Props)){
        if(dfd_gradenamecombo_v1Props?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)){
          setdropdownValue([dfd_gradenamecombo_v1Props?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)?.grade_code_name])
          tempValue=dfd_gradenamecombo_v1Props?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)?.grade_code_name
        }else{
          setdropdownValue([position_information_group67802.grade_code_name])
          tempValue=position_information_group67802.grade_code_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{grade_name:position_information_group67802.grade_code_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)?.grade_code_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.grade_name === position_information_group67802.grade_code_name)?.grade_code_name
      }else{
        setdropdownValue([position_information_group67802.grade_code_name])
        tempValue=position_information_group67802.grade_code_name
      }   
    ...
    if(grade_name11b6f?.trigger == true)
    {
      await handlechange(tempValue)
      setgrade_name11b6f((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[position_information_group67802.grade_code_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_gradenamecombo_v1Props) && dfd_gradenamecombo_v1Props?.length == 1){
    // setposition_information_group67802((pre:any)=>({...pre,grade_code_name:dfd_gradenamecombo_v1Props[0]?.grade_code_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setposition_information_group67802((prev: any) => ({ ...prev, grade_code_name: getMapperDetailsBindValues[value],GRADE_CODE_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setposition_information_group67802((prev: any) => ({ ...prev, grade_code_name: '', grade_name11b6f: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,grade_code_name:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(position_information_group67802?.grade_code_name == "" || position_information_group67802?.grade_code_name == undefined){
      position_information_group67802.grade_code_name = "";
      const validate:any = v.safeParse(schema, position_information_group67802?.grade_code_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,grade_code_name:"invalid"}}));
        }
    }else if(position_information_group67802?.grade_code_name !== ""){
    const validate:any = v.safeParse(schema, position_information_group67802?.grade_code_name);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,grade_code_name:"invalid"}}));
  ...
  useEffect(() => { position_information_group67802Ref.current = position_information_group67802; }, [position_information_group67802]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "58c851fe96e876f030ca4d9322411b6f") {
        handleClick(position_information_group67802Ref?.current?.grade_name11b6f?position_information_group67802Ref?.current?.grade_name11b6f:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "58c851fe96e876f030ca4d9322411b6f");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setposition_information_group67802((pre:any)=>({...pre,grade_code_name:""}))
    else
      setInitialCount(1)
  },[grade_name11b6f?.refresh])
  

  if (grade_name11b6f?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {grade_name11b6f?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={position_information_group67802?.grade_name11b6f ? [position_information_group67802?.grade_name11b6f] : (position_information_group67802?.grade_code_name ? dropdownValue : [])}
        validationState={validate?.viewEmployeeJobPosition_v1?.grade_code_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
