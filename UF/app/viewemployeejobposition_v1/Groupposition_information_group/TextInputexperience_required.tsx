...
const TextInputexperience_required = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|c7b866caed721de00fbfdfc1f8967802|1d75fa88709dbd2dc86afad5ee4d886e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'experience_required',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,experience_required:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group67802((prev: any) => ({ ...prev, experience_required: +e.target.value }));
    }
    else{
    setposition_information_group67802((prev: any) => ({ ...prev, experience_required: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "1d75fa88709dbd2dc86afad5ee4d886e"
      );
      ...
      //     controlId: "1d75fa88709dbd2dc86afad5ee4d886e",
      //     isTable: false,
      //     from:"TextInputexperience_required",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'experience_required', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'experience_required',type:'text'};
      //   type={
      //     name:'experience_required',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.experience_required.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.experience_required.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.experience_required.type
      //   }
      ...
      //   type={
      //     name:'experience_required',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.experience_required.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.experience_required.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.experience_required.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "1d75fa88709dbd2dc86afad5ee4d886e") {
        handleChange({target:{value:position_information_group67802Ref?.current?.experience_required||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "1d75fa88709dbd2dc86afad5ee4d886e") {
        handleBlur({target:{value:position_information_group67802Ref?.current?.experience_required||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group67802((pre:any)=>({...pre,experience_required:dfd_jobpositions_v1Props.data[0]?.experience_required}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (experience_requiredd886e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `23 / 37`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={position_information_group67802?.experience_required||""}
         disabled= {experience_requiredd886e?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.experience_required ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputexperience_required
