...
const TextInputjob_family = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1|ffc1d63fd7a3455d8438315430a6dcce|properties.job_family"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|c7b866caed721de00fbfdfc1f8967802|251eaf38c1a722ac11a5db3c08debc1e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'job_family',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,job_family:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group67802((prev: any) => ({ ...prev, job_family: +e.target.value }));
    }
    else{
    setposition_information_group67802((prev: any) => ({ ...prev, job_family: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "251eaf38c1a722ac11a5db3c08debc1e"
      );
      ...
      //     controlId: "251eaf38c1a722ac11a5db3c08debc1e",
      //     isTable: false,
      //     from:"TextInputjob_family",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'job_family', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'job_family',type:'text'};
      //   type={
      //     name:'job_family',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.job_family.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.job_family.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.job_family.type
      //   }
      ...
      //   type={
      //     name:'job_family',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.job_family.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.job_family.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.job_family.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "251eaf38c1a722ac11a5db3c08debc1e") {
        handleChange({target:{value:position_information_group67802Ref?.current?.job_family||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "251eaf38c1a722ac11a5db3c08debc1e") {
        handleBlur({target:{value:position_information_group67802Ref?.current?.job_family||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group67802((pre:any)=>({...pre,job_family:dfd_jobpositions_v1Props.data[0]?.job_family}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (job_familyebc1e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `23 / 37`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={position_information_group67802?.job_family||""}
         disabled= {job_familyebc1e?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.job_family ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputjob_family
