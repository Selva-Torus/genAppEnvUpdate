...
const TextInputdescription = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1|ffc1d63fd7a3455d8438315430a6dcce|properties.description"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|c7b866caed721de00fbfdfc1f8967802|5fdcce62cda412f5065297749baf7b05"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'description',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,description:undefined}}));
    if(dynamicStateandType.type=="number"){
    setposition_information_group67802((prev: any) => ({ ...prev, description: +e.target.value }));
    }
    else{
    setposition_information_group67802((prev: any) => ({ ...prev, description: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5fdcce62cda412f5065297749baf7b05"
      );
      ...
      //     controlId: "5fdcce62cda412f5065297749baf7b05",
      //     isTable: false,
      //     from:"TextInputdescription",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'description', type: 'number'});
      }
      ...
      //   type={
      //     name:'description',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.description.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.description.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.description.type
      //   }
      ...
      //   type={
      //     name:'description',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.description.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.description.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.description.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5fdcce62cda412f5065297749baf7b05") {
        handleChange({target:{value:position_information_group67802Ref?.current?.description||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5fdcce62cda412f5065297749baf7b05") {
        handleBlur({target:{value:position_information_group67802Ref?.current?.description||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setposition_information_group67802((pre:any)=>({...pre,description:dfd_jobpositions_v1Props.data[0]?.description}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (descriptionf7b05?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `8 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={position_information_group67802?.description||""}
         disabled= {descriptionf7b05?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.description ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
