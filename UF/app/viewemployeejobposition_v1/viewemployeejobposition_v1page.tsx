...
export default function PageViewemployeejobpositionV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function employmenttypecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewemployeejobposition_v1Props.length > 0){
          for(let i=0;i< viewemployeejobposition_v1Props.length;i++){
            if(viewemployeejobposition_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employmentTypeCombo:AFVK:v1"){
              // delete viewemployeejobposition_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobposition_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function vacancystatuscombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewemployeejobposition_v1Props.length > 0){
          for(let i=0;i< viewemployeejobposition_v1Props.length;i++){
            if(viewemployeejobposition_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:vacancyStatusCombo:AFVK:v1"){
              // delete viewemployeejobposition_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobposition_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function gradenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewemployeejobposition_v1Props.length > 0){
          for(let i=0;i< viewemployeejobposition_v1Props.length;i++){
            if(viewemployeejobposition_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:gradeNameCombo:AFVK:v1"){
              // delete viewemployeejobposition_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobposition_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function jobpositions_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewemployeejobposition_v1Props.length > 0){
          for(let i=0;i< viewemployeejobposition_v1Props.length;i++){
            if(viewemployeejobposition_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1"){
              // delete viewemployeejobposition_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobposition_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setviewemployeejobposition_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setviewemployeejobposition_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(viewemployeejobposition_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(viewemployeejobposition_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setviewemployeejobposition_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
