 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "605e3450c023b1b9ba47e6a851ca2944"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,remote_allowed:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[remote_alloweda2944?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props?.length == 1){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,remote_allowed:dfd_jobpositions_v1Props[0]?.remote_allowed}))
    }
  },[dfd_jobpositions_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, remote_allowed: checked }));
  ...
  if (remote_alloweda2944?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="top"
        disabled= {remote_alloweda2944?.isDisabled ? true : false}
        checkedContent="Yes"                                                                                                                                             
        uncheckedContent="No"
        checked={compensation_benfits_group0d8fe?.remote_allowed || false} 
