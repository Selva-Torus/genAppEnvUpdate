...
const TextInputapproved_headcount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1|ffc1d63fd7a3455d8438315430a6dcce|properties.approved_headcount"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|2a5944178febfa18e4be375d2920d8fe|f01f8efdcf678fcea2cfdab936f53c3b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'approved_headcount',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,approved_headcount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, approved_headcount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_group0d8fe?.approved_headcount == "" || compensation_benfits_group0d8fe?.approved_headcount == undefined){
      compensation_benfits_group0d8fe.approved_headcount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_group0d8fe?.approved_headcount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,approved_headcount:"invalid"}}));
        }
    }else if(compensation_benfits_group0d8fe?.approved_headcount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_group0d8fe?.approved_headcount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f01f8efdcf678fcea2cfdab936f53c3b"
      );
      ...
      //     controlId: "f01f8efdcf678fcea2cfdab936f53c3b",
      //     isTable: false,
      //     from:"TextInputapproved_headcount",
      ...
      //   type={
      //     name:'approved_headcount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approved_headcount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approved_headcount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.approved_headcount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_group0d8fe?.approved_headcount)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "f01f8efdcf678fcea2cfdab936f53c3b") {
        handleChange({target:{value:compensation_benfits_group0d8feRef?.current?.approved_headcount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f01f8efdcf678fcea2cfdab936f53c3b") {
        handleBlur({target:{value:compensation_benfits_group0d8feRef?.current?.approved_headcount||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,approved_headcount:dfd_jobpositions_v1Props.data[0]?.approved_headcount}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (approved_headcount53c3b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `9 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={compensation_benfits_group0d8fe?.approved_headcount||""}
         disabled= {approved_headcount53c3b?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Approved Headcount"
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.approved_headcount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputapproved_headcount
