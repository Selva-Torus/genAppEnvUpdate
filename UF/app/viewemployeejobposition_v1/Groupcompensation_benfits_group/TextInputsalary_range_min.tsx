...
const TextInputsalary_range_min = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|2a5944178febfa18e4be375d2920d8fe|128c2bc358c5aa3db616b6df98a31c0f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,salary_range_min:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, salary_range_min: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_group0d8fe?.salary_range_min == "" || compensation_benfits_group0d8fe?.salary_range_min == undefined){
      compensation_benfits_group0d8fe.salary_range_min = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_group0d8fe?.salary_range_min);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,salary_range_min:"invalid"}}));
        }
    }else if(compensation_benfits_group0d8fe?.salary_range_min !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_group0d8fe?.salary_range_min);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "128c2bc358c5aa3db616b6df98a31c0f"
      );
      ...
      //     controlId: "128c2bc358c5aa3db616b6df98a31c0f",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_group0d8fe?.salary_range_min)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "128c2bc358c5aa3db616b6df98a31c0f") {
        handleChange({target:{value:compensation_benfits_group0d8feRef?.current?.salary_range_min||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "128c2bc358c5aa3db616b6df98a31c0f") {
        handleBlur({target:{value:compensation_benfits_group0d8feRef?.current?.salary_range_min||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,salary_range_min:dfd_jobpositions_v1Props.data[0]?.salary_range_min}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (salary_range_min31c0f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_group0d8fe?.salary_range_min||""}
         disabled= {salary_range_min31c0f?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.salary_range_min ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
