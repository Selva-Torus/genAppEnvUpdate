...
const Switchtravel_required = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "e303ab1b5ee0adcf457801267f9ee204"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,travel_required:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[travel_requiredee204?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props?.length == 1){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,travel_required:dfd_jobpositions_v1Props[0]?.travel_required}))
    }
  },[dfd_jobpositions_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, travel_required: checked }));
  ...
  if (travel_requiredee204?.isHidden) {
    return <></>
  }
  return (
    <div 
      className=""
      style={{gridColumn: `17 / 21`,gridRow: `24 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Switch
        className=""
        contentAlign={"left"}
        headerText="Travel Required"
        headerPosition="top"
        disabled= {travel_requiredee204?.isDisabled ? true : false}
        checkedContent="Yes"                                                                                                                                             
        uncheckedContent="No"
        checked={compensation_benfits_group0d8fe?.travel_required || false} 
...
export default Switchtravel_required
