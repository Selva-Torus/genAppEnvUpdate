...
const TextInputsalary_range_max = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1|ffc1d63fd7a3455d8438315430a6dcce|properties.salary_range_max"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|2a5944178febfa18e4be375d2920d8fe|dea97d91ac9824b9f32891bee28b8794"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'salary_range_max',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,salary_range_max:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, salary_range_max: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_group0d8fe?.salary_range_max == "" || compensation_benfits_group0d8fe?.salary_range_max == undefined){
      compensation_benfits_group0d8fe.salary_range_max = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_group0d8fe?.salary_range_max);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,salary_range_max:"invalid"}}));
        }
    }else if(compensation_benfits_group0d8fe?.salary_range_max !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_group0d8fe?.salary_range_max);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "dea97d91ac9824b9f32891bee28b8794"
      );
      ...
      //     controlId: "dea97d91ac9824b9f32891bee28b8794",
      //     isTable: false,
      //     from:"TextInputsalary_range_max",
      ...
      //   type={
      //     name:'salary_range_max',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salary_range_max.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salary_range_max.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.salary_range_max.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_group0d8fe?.salary_range_max)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "dea97d91ac9824b9f32891bee28b8794") {
        handleChange({target:{value:compensation_benfits_group0d8feRef?.current?.salary_range_max||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "dea97d91ac9824b9f32891bee28b8794") {
        handleBlur({target:{value:compensation_benfits_group0d8feRef?.current?.salary_range_max||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,salary_range_max:dfd_jobpositions_v1Props.data[0]?.salary_range_max}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (salary_range_maxb8794?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `9 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={compensation_benfits_group0d8fe?.salary_range_max||""}
         disabled= {salary_range_maxb8794?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.salary_range_max ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputsalary_range_max
