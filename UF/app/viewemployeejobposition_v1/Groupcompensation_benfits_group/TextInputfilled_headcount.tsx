...
const TextInputfilled_headcount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1|ffc1d63fd7a3455d8438315430a6dcce|properties.filled_headcount"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobPosition:AFVK:v1|2a5944178febfa18e4be375d2920d8fe|53669d058d2e4d6207ec0c01b8435c2c"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'filled_headcount',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,filled_headcount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_group0d8fe((prev: any) => ({ ...prev, filled_headcount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_group0d8fe?.filled_headcount == "" || compensation_benfits_group0d8fe?.filled_headcount == undefined){
      compensation_benfits_group0d8fe.filled_headcount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_group0d8fe?.filled_headcount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobPosition_v1:{...pre?.viewEmployeeJobPosition_v1,filled_headcount:"invalid"}}));
        }
    }else if(compensation_benfits_group0d8fe?.filled_headcount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_group0d8fe?.filled_headcount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "53669d058d2e4d6207ec0c01b8435c2c"
      );
      ...
      //     controlId: "53669d058d2e4d6207ec0c01b8435c2c",
      //     isTable: false,
      //     from:"TextInputfilled_headcount",
      ...
      //   type={
      //     name:'filled_headcount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filled_headcount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filled_headcount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.filled_headcount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_group0d8fe?.filled_headcount)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "53669d058d2e4d6207ec0c01b8435c2c") {
        handleChange({target:{value:compensation_benfits_group0d8feRef?.current?.filled_headcount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "53669d058d2e4d6207ec0c01b8435c2c") {
        handleBlur({target:{value:compensation_benfits_group0d8feRef?.current?.filled_headcount||""}});
      }
    };
  ...
  if(dfd_jobpositions_v1Props?.setSearchFilters && dfd_jobpositions_v1Props?.data)
  {
    if(Array.isArray(dfd_jobpositions_v1Props.data) && dfd_jobpositions_v1Props.data.length > 0){
      setcompensation_benfits_group0d8fe((pre:any)=>({...pre,filled_headcount:dfd_jobpositions_v1Props.data[0]?.filled_headcount}));
    }
  }
  },[dfd_jobpositions_v1Props?.setSearchFilters])
  if (filled_headcount35c2c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `24 / 38`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={compensation_benfits_group0d8fe?.filled_headcount||""}
         disabled= {filled_headcount35c2c?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Filled Headcount"
      errorMessage={error}
        validationState={validate?.viewEmployeeJobPosition_v1?.filled_headcount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputfilled_headcount
