...
const Textrejected_requests = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_individualleavereqcards_v1Props && !dfd_individualleavereqcards_v1Props.hasLogicCenter) {
        ...
        setonboarding_group4ab1e((pre: any) => {
          return { ...pre, rejected_requests: api_paginationData.data.records[0]?.rejected_requests }
        })
        }
      }
      else{
      if(Array.isArray(dfd_individualleavereqcards_v1Props) && dfd_individualleavereqcards_v1Props && !onboarding_group4ab1e.rejected_requests){
        setonboarding_group4ab1e((pre:any)=>({...pre,rejected_requests:dfd_individualleavereqcards_v1Props[0]?.rejected_requests}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[rejected_requestsf6330?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_individualleavereqcards_v1Props) && !onboarding_group4ab1e.rejected_requests){
    setonboarding_group4ab1e((pre:any)=>({...pre,rejected_requests:dfd_individualleavereqcards_v1Props[0]?.rejected_requests}));
  }
  },[dfd_individualleavereqcards_v1Props])

  if (rejected_requestsf6330?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.rejected_requests : (onboarding_group4ab1e?.rejected_requests || ""))}
</Text>
  </div>
  )
}

export default Textrejected_requests
