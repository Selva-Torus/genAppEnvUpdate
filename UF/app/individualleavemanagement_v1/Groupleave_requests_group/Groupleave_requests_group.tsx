...
import Iconapp_icon  from "./Iconapp_icon";
import Textapproved_text  from "./Textapproved_text";
import Textapproved_this_month  from "./Textapproved_this_month";
import Textday_text  from "./Textday_text";
...
const Groupleave_requests_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e11d9677efd34bc2733dbda4d2fb9aaf");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setleave_requests_groupb9aafProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("divider")){
        setdividerdb068((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dividerdb068?.isDisabled==null)
      {
        setdividerdb068((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_icon")){
        setapp_iconeccb1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_iconeccb1?.isDisabled==null)
      {
        setapp_iconeccb1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approved_text")){
        setapproved_text01cff((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approved_text01cff?.isDisabled==null)
      {
        setapproved_text01cff((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approved_this_month")){
        setapproved_this_month268e6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approved_this_month268e6?.isDisabled==null)
      {
        setapproved_this_month268e6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("day_text")){
        setday_text5965c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(day_text5965c?.isDisabled==null)
      {
        setday_text5965c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['divider'] = dividerdb068,
        codeStates['setdivider'] = setdividerdb068,
        codeStates['app_icon'] = app_iconeccb1,
        codeStates['setapp_icon'] = setapp_iconeccb1,
        codeStates['approved_text'] = approved_text01cff,
        codeStates['setapproved_text'] = setapproved_text01cff,
        codeStates['approved_this_month'] = approved_this_month268e6,
        codeStates['setapproved_this_month'] = setapproved_this_month268e6,
        codeStates['day_text'] = day_text5965c,
        codeStates['setday_text'] = setday_text5965c,
  ...
  const leave_requests_groupb9aafRef = useRef<any>(null);
  const handleClearSearch = () => {
    leave_requests_groupb9aafRef.current?.setSearchParams();
    leave_requests_groupb9aafRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(leave_requests_groupb9aaf) && Object.keys(leave_requests_groupb9aaf)?.length>0)
      {
        setleave_requests_groupb9aaf({})
      }
    }else 
      prevRefreshRef.current= true
  }, [leave_requests_groupb9aafProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !rounded-md ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setindividualleavemanagement_v1((pre:any)=>({...pre,_selectedGroup_:"leave_requests_group"}))
        }}
    >
        {allowedControls.includes("divider") ?<Dividerdivider   /* db068 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("app_icon")?<Iconapp_icon /* eccb1 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("approved_text") ?<Textapproved_text   /* 01cff */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("approved_this_month") ?<Textapproved_this_month   /* 268e6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("day_text") ?<Textday_text   /* 5965c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupleave_requests_group
