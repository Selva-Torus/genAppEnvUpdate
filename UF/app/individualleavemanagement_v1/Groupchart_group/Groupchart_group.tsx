...
import Textchart_text  from "./Textchart_text";
...
const Groupchart_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9c92deb8c0df4c54b1f9e6a4ba7dd9ff");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchart_groupdd9ffProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("chart_text")){
        setchart_textfde29((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(chart_textfde29?.isDisabled==null)
      {
        setchart_textfde29((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("piechart")){
        setpiechart31095((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(piechart31095?.isDisabled==null)
      {
        setpiechart31095((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['chart_text'] = chart_textfde29,
        codeStates['setchart_text'] = setchart_textfde29,
        codeStates['piechart'] = piechart31095,
        codeStates['setpiechart'] = setpiechart31095,
  ...
  const chart_groupdd9ffRef = useRef<any>(null);
  const handleClearSearch = () => {
    chart_groupdd9ffRef.current?.setSearchParams();
    chart_groupdd9ffRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(chart_groupdd9ff) && Object.keys(chart_groupdd9ff)?.length>0)
      {
        setchart_groupdd9ff({})
      }
    }else 
      prevRefreshRef.current= true
  }, [chart_groupdd9ffProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setindividualleavemanagement_v1((pre:any)=>({...pre,_selectedGroup_:"chart_group"}))
        }}
    >
          {allowedControls.includes("chart_text") ?<Textchart_text   /* fde29 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("piechart") ?<PieChartspiechart /* 31095 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupchart_group
