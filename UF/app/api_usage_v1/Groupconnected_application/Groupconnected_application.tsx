...
const Groupconnected_application = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cc4cb5d37dab4218b60ceaba56b19ab2");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setconnected_application19ab2Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_name")){
        setapp_namedc4c5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(app_namedc4c5?.isDisabled==null)
      {
        setapp_namedc4c5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("tppname")){
        settppname5b032((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tppname5b032?.isDisabled==null)
      {
        settppname5b032((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("type")){
        settyped4eac((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(typed4eac?.isDisabled==null)
      {
        settyped4eac((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status_value")){
        setstatus_value3beb3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(status_value3beb3?.isDisabled==null)
      {
        setstatus_value3beb3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(connected_application19ab2) &&
        Object.keys(connected_application19ab2)?.length > 0
      ) {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"connected_application"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableconnected_application headerButtonsRenders={renderBUttons}
        ...
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={connected_application19ab2Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
