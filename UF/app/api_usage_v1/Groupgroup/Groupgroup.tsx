...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ed35f907cd59456a97cdbdca7d1657d5");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup657d5Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("icon5675")){
        seticon5675ee8ba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(icon5675ee8ba?.isDisabled==null)
      {
        seticon5675ee8ba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text4545")){
        settext454513feb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text454513feb?.isDisabled==null)
      {
        settext454513feb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text45645")){
        settext4564580602((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text4564580602?.isDisabled==null)
      {
        settext4564580602((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['icon5675'] = icon5675ee8ba,
        codeStates['seticon5675'] = seticon5675ee8ba,
        codeStates['text4545'] = text454513feb,
        codeStates['settext4545'] = settext454513feb,
        codeStates['text45645'] = text4564580602,
        codeStates['settext45645'] = settext4564580602,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(group657d5) &&
        Object.keys(group657d5)?.length > 0
      ) {
        setgroup657d5({})
      }
    } else prevRefreshRef.current = true
  }, [group657d5Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
        {allowedControls.includes("icon5675")?<Iconicon5675 /* ee8ba */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("text4545") ?<Texttext4545   /* 13feb */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("text45645") ?<Texttext45645   /* 80602 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
