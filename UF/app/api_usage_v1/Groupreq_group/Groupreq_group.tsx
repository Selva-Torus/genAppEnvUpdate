...
const Groupreq_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "338a929827b441b58a690143f2cdf5e7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setreq_groupdf5e7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_requests")){
        settotal_requests06c5a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_requests06c5a?.isDisabled==null)
      {
        settotal_requests06c5a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("tot_req_icon")){
        settot_req_icon1fa8f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tot_req_icon1fa8f?.isDisabled==null)
      {
        settot_req_icon1fa8f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_requests'] = total_requests06c5a,
        codeStates['settotal_requests'] = settotal_requests06c5a,
        codeStates['tot_req_icon'] = tot_req_icon1fa8f,
        codeStates['settot_req_icon'] = settot_req_icon1fa8f,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(req_groupdf5e7) &&
        Object.keys(req_groupdf5e7)?.length > 0
      ) {
        setreq_groupdf5e7({})
      }
    } else prevRefreshRef.current = true
  }, [req_groupdf5e7Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"req_group"}))
        }}
    >
        {allowedControls.includes("total_requests") ?<Cardtotal_requests  /* 06c5a */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("tot_req_icon")?<Icontot_req_icon /* 1fa8f */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}
