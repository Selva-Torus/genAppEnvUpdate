...
const Cardtotal_requests = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const {total_requests06c5aProps, settotal_requests06c5aProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean):Promise<void>=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "ae67ce02a3c242d4abcbadfe09706c5a"
      );
    ...
    try{
      if ("hasLogicCenter" in dfd_mongodb_maindashboard_dfd_v1Props && !dfd_mongodb_maindashboard_dfd_v1Props.hasLogicCenter) {
        ...
        setreq_groupdf5e7((pre: any) => ({
          ...pre,
          total_requests: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.total_requests
            : "0"
        }))
      }
      else{
        if(filterFlag){
          setreq_groupdf5e7((pre: any) => ({
            ...pre,
            total_requests: total_requests06c5aProps?.filteredData?.length > 0
              ? total_requests06c5aProps?.filteredData[0]?.total_requests
              : "0"
          }))
        }else if(Array.isArray(dfd_mongodb_maindashboard_dfd_v1Props) && dfd_mongodb_maindashboard_dfd_v1Props && !req_groupdf5e7.total_requests){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!total_requests06c5aProps?.filterProps) return;
    handleMapperDetails(total_requests06c5aProps?.filterProps,total_requests06c5aProps?.filterFlag);
  },[total_requests06c5aProps?.filterProps])
