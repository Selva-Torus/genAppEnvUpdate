...
const Groupgroup454 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "98c73da3ec924b46a87aa54c7119ff98");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup4549ff98Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("icon5645")){
        seticon56454d73c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(icon56454d73c?.isDisabled==null)
      {
        seticon56454d73c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text45645")){
        settext45645c95a7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text45645c95a7?.isDisabled==null)
      {
        settext45645c95a7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text23523")){
        settext23523e7140((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text23523e7140?.isDisabled==null)
      {
        settext23523e7140((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['icon5645'] = icon56454d73c,
        codeStates['seticon5645'] = seticon56454d73c,
        codeStates['text45645'] = text45645c95a7,
        codeStates['settext45645'] = settext45645c95a7,
        codeStates['text23523'] = text23523e7140,
        codeStates['settext23523'] = settext23523e7140,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(group4549ff98) &&
        Object.keys(group4549ff98)?.length > 0
      ) {
        setgroup4549ff98({})
      }
    } else prevRefreshRef.current = true
  }, [group4549ff98Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md rounded-xl  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group454"}))
        }}
    >
        {allowedControls.includes("icon5645")?<Iconicon5645 /* 4d73c */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("text45645") ?<Texttext45645   /* c95a7 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("text23523") ?<Texttext23523   /* e7140 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
