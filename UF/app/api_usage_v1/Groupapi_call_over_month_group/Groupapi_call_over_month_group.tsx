...
const Groupapi_call_over_month_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "136595a6c5b44c8990100d7e837ccb80");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_call_over_month_groupccb80Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_over_month_linechart")){
        setapi_call_over_month_linecharte59b1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_call_over_month_linecharte59b1?.isDisabled==null)
      {
        setapi_call_over_month_linecharte59b1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['api_call_over_month_linechart'] = api_call_over_month_linecharte59b1,
        codeStates['setapi_call_over_month_linechart'] = setapi_call_over_month_linecharte59b1,
  ...
  const api_call_over_month_groupccb80Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_month_groupccb80Ref.current?.setSearchParams();
    api_call_over_month_groupccb80Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_call_over_month_groupccb80) &&
        Object.keys(api_call_over_month_groupccb80)?.length > 0
      ) {
        setapi_call_over_month_groupccb80({})
      }
    } else prevRefreshRef.current = true
  }, [api_call_over_month_groupccb80Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_call_over_month_group"}))
        }}
    >
        {allowedControls.includes("api_call_over_month_linechart") ?<LineChartsapi_call_over_month_linechart /* e59b1 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
