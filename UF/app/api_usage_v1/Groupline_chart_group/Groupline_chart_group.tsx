...
const Groupline_chart_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2150f24b9f8d42f4be5c49f18b9adc5c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setline_chart_groupadc5cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_hours")){
        setapi_call_hours6d062((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_call_hours6d062?.isDisabled==null)
      {
        setapi_call_hours6d062((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_call_hours_dropdown")){
        setapi_call_hours_dropdown14e0a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_call_hours_dropdown14e0a?.isDisabled==null)
      {
        setapi_call_hours_dropdown14e0a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_call_over_frequency_subscreen")){
        setapi_call_over_frequency_subscreenb8acc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_call_over_frequency_subscreenb8acc?.isDisabled==null)
      {
        setapi_call_over_frequency_subscreenb8acc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(line_chart_groupadc5c) &&
        Object.keys(line_chart_groupadc5c)?.length > 0
      ) {
        setline_chart_groupadc5c({})
      }
    } else prevRefreshRef.current = true
  }, [line_chart_groupadc5cProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"line_chart_group"}))
        }}
    >
        {allowedComponent.includes("api_call_over_frequency_subscreen")  &&<Groupapi_call_over_frequency_subscreen  
          ...
          {allowedControls.includes("api_call_hours") ?<Textapi_call_hours   /* 6d062 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("api_call_hours_dropdown") ?<Dropdownapi_call_hours_dropdown   /* 14e0a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
