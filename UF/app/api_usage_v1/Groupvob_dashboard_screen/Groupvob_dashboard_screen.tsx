...
const Groupvob_dashboard_screen = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "b659f0676d0243baa1258468e019ce49");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvob_dashboard_screen9ce49Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_usage_group")){
        setapi_usage_group868b4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_usage_group868b4?.isDisabled==null)
      {
        setapi_usage_group868b4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("req_group")){
        setreq_groupdf5e7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(req_groupdf5e7?.isDisabled==null)
      {
        setreq_groupdf5e7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("active_group")){
        setactive_group31e18((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(active_group31e18?.isDisabled==null)
      {
        setactive_group31e18((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_api_calls_group")){
        settotal_api_calls_groupd4dee((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_api_calls_groupd4dee?.isDisabled==null)
      {
        settotal_api_calls_groupd4dee((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("most_group")){
        setmost_groupc5ce0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(most_groupc5ce0?.isDisabled==null)
      {
        setmost_groupc5ce0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("line_chart_group")){
        setline_chart_groupadc5c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(line_chart_groupadc5c?.isDisabled==null)
      {
        setline_chart_groupadc5c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_used_api_group")){
        settotal_used_api_groupcd37d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(total_used_api_groupcd37d?.isDisabled==null)
      {
        settotal_used_api_groupcd37d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("list_of_register_tpp_group")){
        setlist_of_register_tpp_groupbe9d5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(list_of_register_tpp_groupbe9d5?.isDisabled==null)
      {
        setlist_of_register_tpp_groupbe9d5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_repo_table")){
        setapi_repo_table162e4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_repo_table162e4?.isDisabled==null)
      {
        setapi_repo_table162e4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group123")){
        setgroup1233a04c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group1233a04c?.isDisabled==null)
      {
        setgroup1233a04c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(vob_dashboard_screen9ce49) &&
        Object.keys(vob_dashboard_screen9ce49)?.length > 0
      ) {
        setvob_dashboard_screen9ce49({})
      }
    } else prevRefreshRef.current = true
  }, [vob_dashboard_screen9ce49Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"vob_dashboard_screen"}))
        }}
    >
