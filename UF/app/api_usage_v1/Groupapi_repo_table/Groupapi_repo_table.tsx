...
const Groupapi_repo_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "3d7c3aaaa9fe448e9630ca3878a162e4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_repo_table162e4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_repository")){
        setapi_repositoryb1ab8Props((pre:any)=>({...pre,...api_repositoryb1ab8,isDisabled:true}));

    }else
    {
      if(api_repositoryb1ab8?.isDisabled==null)
      {
        setapi_repositoryb1ab8Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_repo_table162e4) &&
        Object.keys(api_repo_table162e4)?.length > 0
      ) {
        setapi_repo_table162e4({})
      }
    } else prevRefreshRef.current = true
  }, [api_repo_table162e4Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_repo_table"}))
        }}
    >
