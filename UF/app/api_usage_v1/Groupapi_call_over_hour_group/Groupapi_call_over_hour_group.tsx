...
const Groupapi_call_over_hour_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {api_call_over_hour_linechart01342, setapi_call_over_hour_linechart01342}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "da6fe2227cd6460b9f34f9b88512febf");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_call_over_hour_group2febfProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_over_hour_linechart")){
        setapi_call_over_hour_linechart01342((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_call_over_hour_linechart01342?.isDisabled==null)
      {
        setapi_call_over_hour_linechart01342((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['api_call_over_hour_linechart'] = api_call_over_hour_linechart01342,
        codeStates['setapi_call_over_hour_linechart'] = setapi_call_over_hour_linechart01342,
  ...
  const api_call_over_hour_group2febfRef = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_hour_group2febfRef.current?.setSearchParams();
    api_call_over_hour_group2febfRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_call_over_hour_group2febf) &&
        Object.keys(api_call_over_hour_group2febf)?.length > 0
      ) {
        setapi_call_over_hour_group2febf({})
      }
    } else prevRefreshRef.current = true
  }, [api_call_over_hour_group2febfProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_call_over_hour_group"}))
        }}
    >
        {allowedControls.includes("api_call_over_hour_linechart") ?<LineChartsapi_call_over_hour_linechart /* 01342 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
