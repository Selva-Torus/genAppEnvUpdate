...
export default function LineChartsapi_call_over_hour_linechart({ 
  ...
  const {api_call_over_hour_linechart01342Props, setapi_call_over_hour_linechart01342Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean)=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
      controlData,
      "da6fe2227cd6460b9f34f9b88512febf",
      "9c6cdd1d93d84363be5982a456001342"
    ); 
    ...
    if (code != '') {
      ...
      try{
        if ("hasLogicCenter" in dfd_mongo_linechart_dfd_v1Props && !dfd_mongo_linechart_dfd_v1Props.hasLogicCenter) {
          ...
          setapi_call_over_hour_group2febf((pre: any) => ({
            ...pre,
        ...
        if(filterFlag){
          setapi_call_over_hour_group2febf((pre: any) => ({
            ...pre,
            name: api_call_over_hour_linechart01342Props?.filteredData?.length > 0
              ? api_call_over_hour_linechart01342Props?.filteredData[0]?.name
              : "0"
          }))
          setData(api_call_over_hour_linechart01342Props?.filteredData);
        }else if(Array.isArray(dfd_mongo_linechart_dfd_v1Props) && dfd_mongo_linechart_dfd_v1Props && !api_call_over_hour_group2febf.name){
          setData(dfd_mongo_linechart_dfd_v1Props);
          setapi_call_over_hour_group2febf((pre:any)=>({...pre,name:dfd_mongo_linechart_dfd_v1Props[0]?.name}));
        }
      }
      }catch(err){
        console.log(err)
  ...
  useEffect(() => {
    handleMapperDetails();
  },[api_call_over_hour_linechart01342?.refresh])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!api_call_over_hour_linechart01342Props?.filterProps) return;
    handleMapperDetails(api_call_over_hour_linechart01342Props?.filterProps,api_call_over_hour_linechart01342Props?.filterFlag);
  },[api_call_over_hour_linechart01342Props?.filterProps])

  if (api_call_over_hour_linechart01342?.isHidden) {
    return <></>
  }
  return (
    <div
