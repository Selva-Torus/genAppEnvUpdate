...
const Groupmost_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e46460169e424963a78d04eee5ec5ce0");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setmost_groupc5ce0Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("error_rate")){
        seterror_rate72497((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(error_rate72497?.isDisabled==null)
      {
        seterror_rate72497((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("error_rate_icon")){
        seterror_rate_icon89a9a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(error_rate_icon89a9a?.isDisabled==null)
      {
        seterror_rate_icon89a9a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['error_rate'] = error_rate72497,
        codeStates['seterror_rate'] = seterror_rate72497,
        codeStates['error_rate_icon'] = error_rate_icon89a9a,
        codeStates['seterror_rate_icon'] = seterror_rate_icon89a9a,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(most_groupc5ce0) &&
        Object.keys(most_groupc5ce0)?.length > 0
      ) {
        setmost_groupc5ce0({})
      }
    } else prevRefreshRef.current = true
  }, [most_groupc5ce0Props?.refresh])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://tdps3api.toruslowcode.com/torus/9.1/CT009/resources/images/newwwwww3.jpg')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"most_group"}))
        }}
    >
        {allowedControls.includes("error_rate") ?<Carderror_rate  /* 72497 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("error_rate_icon")?<Iconerror_rate_icon /* 89a9a */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}
