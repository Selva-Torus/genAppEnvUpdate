...
const Carderror_rate = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const {error_rate72497Props, seterror_rate72497Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean):Promise<void>=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "e46460169e424963a78d04eee5ec5ce0",
        "54043c1682304c12af6d8074e9972497"
      );
    ...
    try{
      if ("hasLogicCenter" in dfd_mongodb_maindashboard_dfd_v1Props && !dfd_mongodb_maindashboard_dfd_v1Props.hasLogicCenter) {
        ...
        setmost_groupc5ce0((pre: any) => ({
          ...pre,
          error: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.error
            : "0"
        }))
      }
      else{
        if(filterFlag){
          setmost_groupc5ce0((pre: any) => ({
            ...pre,
            error: error_rate72497Props?.filteredData?.length > 0
              ? error_rate72497Props?.filteredData[0]?.error
              : "0"
          }))
        }else if(Array.isArray(dfd_mongodb_maindashboard_dfd_v1Props) && dfd_mongodb_maindashboard_dfd_v1Props && !most_groupc5ce0.error){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!error_rate72497Props?.filterProps) return;
    handleMapperDetails(error_rate72497Props?.filterProps,error_rate72497Props?.filterFlag);
  },[error_rate72497Props?.filterProps])
  ...
  return (
    <div 
    style={{gridColumn: `7 / 23`,gridRow: `1 / 18`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
