...
const Groupgroup79679 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "56f0779d27dc48b69c22ce9fc508bff3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup796798bff3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("icon8698")){
        seticon86986b1a3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(icon86986b1a3?.isDisabled==null)
      {
        seticon86986b1a3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text34572547")){
        settext3457254757f70((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text3457254757f70?.isDisabled==null)
      {
        settext3457254757f70((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text")){
        settext2668f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(text2668f?.isDisabled==null)
      {
        settext2668f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['icon8698'] = icon86986b1a3,
        codeStates['seticon8698'] = seticon86986b1a3,
        codeStates['text34572547'] = text3457254757f70,
        codeStates['settext34572547'] = settext3457254757f70,
        codeStates['text'] = text2668f,
        codeStates['settext'] = settext2668f,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(group796798bff3) &&
        Object.keys(group796798bff3)?.length > 0
      ) {
        setgroup796798bff3({})
      }
    } else prevRefreshRef.current = true
  }, [group796798bff3Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group79679"}))
        }}
    >
        {allowedControls.includes("icon8698")?<Iconicon8698 /* 6b1a3 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("text34572547") ?<Texttext34572547   /* 57f70 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("text") ?<Texttext   /* 2668f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
