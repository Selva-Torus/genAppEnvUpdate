...
const Groupapi_repository = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "82ecab6738f749949432f786075b1ab8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_repositoryb1ab8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("apiname")){
        setapinamecccc2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(apinamecccc2?.isDisabled==null)
      {
        setapinamecccc2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("version")){
        setversion33b3f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(version33b3f?.isDisabled==null)
      {
        setversion33b3f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatuscd1e6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(statuscd1e6?.isDisabled==null)
      {
        setstatuscd1e6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("api_category")){
        setapi_category0905e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_category0905e?.isDisabled==null)
      {
        setapi_category0905e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("release_date")){
        setrelease_date1939f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(release_date1939f?.isDisabled==null)
      {
        setrelease_date1939f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("view_log")){
        setview_log82d2f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(view_log82d2f?.isDisabled==null)
      {
        setview_log82d2f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_repositoryb1ab8) &&
        Object.keys(api_repositoryb1ab8)?.length > 0
      ) {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_repository"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableapi_repository headerButtonsRenders={renderBUttons}
        ...
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={api_repositoryb1ab8Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
