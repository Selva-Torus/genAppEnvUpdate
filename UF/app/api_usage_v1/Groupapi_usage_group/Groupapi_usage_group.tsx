...
const Groupapi_usage_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "add0bd981a004738b56543151c0868b4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_usage_group868b4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_usage_overview")){
        setapi_usage_overviewecc9e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_usage_overviewecc9e?.isDisabled==null)
      {
        setapi_usage_overviewecc9e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['api_usage_overview'] = api_usage_overviewecc9e,
        codeStates['setapi_usage_overview'] = setapi_usage_overviewecc9e,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_usage_group868b4) &&
        Object.keys(api_usage_group868b4)?.length > 0
      ) {
        setapi_usage_group868b4({})
      }
    } else prevRefreshRef.current = true
  }, [api_usage_group868b4Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-2xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_usage_group"}))
        }}
    >
          {allowedControls.includes("api_usage_overview") ?<Textapi_usage_overview   /* ecc9e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
