...
const Groupgroup123 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e7a22cb9c1da4891b40a25f8e5a3a04c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup1233a04cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("consent_lifecycles")){
        setconsent_lifecycles11691((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(consent_lifecycles11691?.isDisabled==null)
      {
        setconsent_lifecycles11691((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group454")){
        setgroup4549ff98((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group4549ff98?.isDisabled==null)
      {
        setgroup4549ff98((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group")){
        setgroup657d5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group657d5?.isDisabled==null)
      {
        setgroup657d5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group6576")){
        setgroup6576622ab((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group6576622ab?.isDisabled==null)
      {
        setgroup6576622ab((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group79679")){
        setgroup796798bff3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group796798bff3?.isDisabled==null)
      {
        setgroup796798bff3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['consent_lifecycles'] = consent_lifecycles11691,
        codeStates['setconsent_lifecycles'] = setconsent_lifecycles11691,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(group1233a04c) &&
        Object.keys(group1233a04c)?.length > 0
      ) {
        setgroup1233a04c({})
      }
    } else prevRefreshRef.current = true
  }, [group1233a04cProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group123"}))
        }}
    >
        ...
        {allowedComponent.includes("group79679")  &&<Groupgroup79679  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("consent_lifecycles") ?<Textconsent_lifecycles   /* 11691 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
