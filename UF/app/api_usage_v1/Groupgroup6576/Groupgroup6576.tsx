...
const Groupgroup6576 = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1d3a3c21edab4f3a88dd5a1631d622ab");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup6576622abProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("icon234234")){
        seticon234234e2c9a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(icon234234e2c9a?.isDisabled==null)
      {
        seticon234234e2c9a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("textwrwer")){
        settextwrwer0e4e1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(textwrwer0e4e1?.isDisabled==null)
      {
        settextwrwer0e4e1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("textwerwer")){
        settextwerweraf6e8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(textwerweraf6e8?.isDisabled==null)
      {
        settextwerweraf6e8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['icon234234'] = icon234234e2c9a,
        codeStates['seticon234234'] = seticon234234e2c9a,
        codeStates['textwrwer'] = textwrwer0e4e1,
        codeStates['settextwrwer'] = settextwrwer0e4e1,
        codeStates['textwerwer'] = textwerweraf6e8,
        codeStates['settextwerwer'] = settextwerweraf6e8,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(group6576622ab) &&
        Object.keys(group6576622ab)?.length > 0
      ) {
        setgroup6576622ab({})
      }
    } else prevRefreshRef.current = true
  }, [group6576622abProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"group6576"}))
        }}
    >
        {allowedControls.includes("icon234234")?<Iconicon234234 /* e2c9a */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("textwrwer") ?<Texttextwrwer   /* 0e4e1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("textwerwer") ?<Texttextwerwer   /* af6e8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
