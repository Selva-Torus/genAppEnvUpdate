...
const Groupapi_call_over_week_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {week_linechart22a2f, setweek_linechart22a2f}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f713427b8d9440d6b0c11c2dd23987fe");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapi_call_over_week_group987feProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("week_linechart")){
        setweek_linechart22a2f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(week_linechart22a2f?.isDisabled==null)
      {
        setweek_linechart22a2f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['week_linechart'] = week_linechart22a2f,
        codeStates['setweek_linechart'] = setweek_linechart22a2f,
  ...
  const api_call_over_week_group987feRef = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_week_group987feRef.current?.setSearchParams();
    api_call_over_week_group987feRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(api_call_over_week_group987fe) &&
        Object.keys(api_call_over_week_group987fe)?.length > 0
      ) {
        setapi_call_over_week_group987fe({})
      }
    } else prevRefreshRef.current = true
  }, [api_call_over_week_group987feProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"api_call_over_week_group"}))
        }}
    >
        {allowedControls.includes("week_linechart") ?<LineChartsweek_linechart /* 22a2f */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
