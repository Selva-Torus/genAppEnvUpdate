...
export default function LineChartsweek_linechart({ 
  ...
  const {week_linechart22a2fProps, setweek_linechart22a2fProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean)=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
      controlData,
      "f713427b8d9440d6b0c11c2dd23987fe",
      "05f062402dc44d81a7164b2485822a2f"
    ); 
    ...
    if (code != '') {
      ...
      try{
        if ("hasLogicCenter" in dfd_mongo_linechart_dfd_v1Props && !dfd_mongo_linechart_dfd_v1Props.hasLogicCenter) {
          ...
          setapi_call_over_week_group987fe((pre: any) => ({
            ...pre,
        ...
        if(filterFlag){
          setapi_call_over_week_group987fe((pre: any) => ({
            ...pre,
            name: week_linechart22a2fProps?.filteredData?.length > 0
              ? week_linechart22a2fProps?.filteredData[0]?.name
              : "0"
          }))
          setData(week_linechart22a2fProps?.filteredData);
        }else if(Array.isArray(dfd_mongo_linechart_dfd_v1Props) && dfd_mongo_linechart_dfd_v1Props && !api_call_over_week_group987fe.name){
          setData(dfd_mongo_linechart_dfd_v1Props);
          setapi_call_over_week_group987fe((pre:any)=>({...pre,name:dfd_mongo_linechart_dfd_v1Props[0]?.name}));
        }
      }
      }catch(err){
        console.log(err)
  ...
  // setSearchFilters
  useEffect(() => {
    if (!week_linechart22a2fProps?.filterProps) return;
    handleMapperDetails(week_linechart22a2fProps?.filterProps,week_linechart22a2fProps?.filterFlag);
  },[week_linechart22a2fProps?.filterProps])

  if (week_linechart22a2f?.isHidden) {
    return <></>
  }
  return (
    <div
