...
const Cardmost_used_apis = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const {most_used_apis2686bProps, setmost_used_apis2686bProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean):Promise<void>=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "6f9c80070d3f414787075e5de4fd4dee",
        "2f451e7451a94c6495db7b14bef2686b"
      );
    ...
    try{
      if ("hasLogicCenter" in dfd_mongodb_maindashboard_dfd_v1Props && !dfd_mongodb_maindashboard_dfd_v1Props.hasLogicCenter) {
        ...
        settotal_api_calls_groupd4dee((pre: any) => ({
          ...pre,
          most_used_apis: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.most_used_apis
            : "0"
        }))
      }
      else{
        if(filterFlag){
          settotal_api_calls_groupd4dee((pre: any) => ({
            ...pre,
            most_used_apis: most_used_apis2686bProps?.filteredData?.length > 0
              ? most_used_apis2686bProps?.filteredData[0]?.most_used_apis
              : "0"
          }))
        }else if(Array.isArray(dfd_mongodb_maindashboard_dfd_v1Props) && dfd_mongodb_maindashboard_dfd_v1Props && !total_api_calls_groupd4dee.most_used_apis){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!most_used_apis2686bProps?.filterProps) return;
    handleMapperDetails(most_used_apis2686bProps?.filterProps,most_used_apis2686bProps?.filterFlag);
  },[most_used_apis2686bProps?.filterProps])
