...
const Grouptotal_api_calls_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6f9c80070d3f414787075e5de4fd4dee");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_api_calls_groupd4deeProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("most_used_apis")){
        setmost_used_apis2686b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(most_used_apis2686b?.isDisabled==null)
      {
        setmost_used_apis2686b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("most_used_api_icon")){
        setmost_used_api_icon6560d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(most_used_api_icon6560d?.isDisabled==null)
      {
        setmost_used_api_icon6560d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['most_used_apis'] = most_used_apis2686b,
        codeStates['setmost_used_apis'] = setmost_used_apis2686b,
        codeStates['most_used_api_icon'] = most_used_api_icon6560d,
        codeStates['setmost_used_api_icon'] = setmost_used_api_icon6560d,
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(total_api_calls_groupd4dee) &&
        Object.keys(total_api_calls_groupd4dee)?.length > 0
      ) {
        settotal_api_calls_groupd4dee({})
      }
    } else prevRefreshRef.current = true
  }, [total_api_calls_groupd4deeProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"total_api_calls_group"}))
        }}
    >
        {allowedControls.includes("most_used_apis") ?<Cardmost_used_apis  /* 2686b */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("most_used_api_icon")?<Iconmost_used_api_icon /* 6560d */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}
