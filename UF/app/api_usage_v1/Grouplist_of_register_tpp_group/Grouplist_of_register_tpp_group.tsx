...
const Grouplist_of_register_tpp_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e5ace6874e2f43d493c10954d23be9d5");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setlist_of_register_tpp_groupbe9d5Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("connected_application")){
        setconnected_application19ab2Props((pre:any)=>({...pre,...connected_application19ab2,isDisabled:true}));

    }else
    {
      if(connected_application19ab2?.isDisabled==null)
      {
        setconnected_application19ab2Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(list_of_register_tpp_groupbe9d5) &&
        Object.keys(list_of_register_tpp_groupbe9d5)?.length > 0
      ) {
        setlist_of_register_tpp_groupbe9d5({})
      }
    } else prevRefreshRef.current = true
  }, [list_of_register_tpp_groupbe9d5Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapiusagedashboard_v1((pre:any)=>({...pre,_selectedGroup_:"list_of_register_tpp_group"}))
        }}
    >
