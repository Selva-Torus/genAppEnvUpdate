...
const Buttonbutton_update = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const dynamicactions956baRef = useRef(dynamicactions956ba);
  useEffect(() => {
    dynamicactions956baRef.current = dynamicactions956ba;
  ...
  const dynamicactions956baPropsRef = useRef(dynamicactions956baProps);
  useEffect(() => {
    dynamicactions956baPropsRef.current = dynamicactions956baProps;
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...dynamicactions956baRef.current};
      let parentRowSpan = 8;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "fc36e97cb2554510a1b8d282b5a956ba",
        "926ec6edcfda442db95fc6e89abdc4e0"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    }else if(dynamicactions956baProps?.isHaveRule==true){
      if("button_update" in dynamicactions956baProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions956baProps?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in assignasset_v1?.dynamicactions && assignasset_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(assignasset_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(assignasset_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_updatedc4e0") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "926ec6edcfda442db95fc6e89abdc4e0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_updatedc4e0");
    return () => {
  ...
  useEffect(()=>{
    if (!button_updatedc4e0?.trigger) return;
      if(button_updatedc4e0?.trigger){
      setbutton_updatedc4e0((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_updatedc4e0?.trigger])

  useEffect(()=>{
    if(button_updatedc4e0?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_updatedc4e0?.refresh])
  ...
  async function handleSavec4e0_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const dynamicactions956baProps = dynamicactions956baPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.assignAsset_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(dynamicactions956baRef.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assignAssetModify:AFVK:v1|82c772edca1542c2b3db7231dfcc8050"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assignAssetModify:AFVK:v1|82c772edca1542c2b3db7231dfcc8050"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactions956ba?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "926ec6edcfda442db95fc6e89abdc4e0",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "926ec6edcfda442db95fc6e89abdc4e0.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "926ec6edcfda442db95fc6e89abdc4e0.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "926ec6edcfda442db95fc6e89abdc4e0.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Updated Assigned Asset Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
            ...
            {
              "id": "926ec6edcfda442db95fc6e89abdc4e0.1.1.1.2",
              ...
              "children": [
                {
                  "id": "356fd7ddf8ed4df7909d896283975a5d.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assignAssetModify:AFVK:v1|82c772edca1542c2b3db7231dfcc8050"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAsset:AFVK:v1";
      sourceId+= "|"+"fc36e97cb2554510a1b8d282b5a956ba";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAsset:AFVK:v1"+"|"+"fc36e97cb2554510a1b8d282b5a956ba"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions956ba?.upId? [dynamicactions956ba?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions956ba?.upId? [dynamicactions956ba?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions956ba?.upId? [dynamicactions956ba?.upId ] : dynamicactions956ba?.trs_process_id? [dynamicactions956ba?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions956baRef.current, ...nullFilter(assign_asset_groupdb5a7), ...nullFilter(assignment_information_group5d144), ...nullFilter(assignment_details_group7f60d) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"fc36e97cb2554510a1b8d282b5a956ba",
              controlId:"926ec6edcfda442db95fc6e89abdc4e0"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (dynamicactions956baProps.ssKey !== '' && dynamicactions956baProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = dynamicactions956baProps.ssKey;          
    }
    ...
    //infoMsg
    toast('Updated Assigned Asset Successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
    setassignments_table75a5dProps((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions956ba((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSavec4e0_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions956ba((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [assignasset_v1?.dynamicactions?.button_update,dynamicactions956baProps?.dynamicActionRule?.button_update,])

 if (button_updatedc4e0?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_updatedc4e0?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
