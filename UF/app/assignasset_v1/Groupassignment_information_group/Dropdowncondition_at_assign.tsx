...
const Dropdowncondition_at_assign = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [condition_at_assignOptions, setcondition_at_assignOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `asset_condition`
    let mapperText: string =  `asset_condition`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "5b443277b5f1422e91a1137e44427aff"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_assetconditioncombo_v1Props.dstKey){
      dstKey = dfd_assetconditioncombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_assetconditioncombo_v1Props && !dfd_assetconditioncombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  try{
    ...
    if(!value){
      ...
      setcondition_at_assignOptions(temp);
  ...
  },[condition_at_assign27aff?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setassignment_information_group5d144((prev: any) => ({ ...prev, condition_at_assign: getMapperDetailsBindValues[value],condition_at_assign27aff: value,CONDITION_AT_ASSIGN: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = assignment_information_group5d144;
      delete temp.condition_at_assign;
      delete temp.CONDITION_AT_ASSIGN;
      delete temp.condition_at_assign27aff;
      setassignment_information_group5d144(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,condition_at_assign:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(assignment_information_group5d144.condition_at_assign){
      if(Array.isArray(dfd_assetconditioncombo_v1Props)){
        if(dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)){
          setdropdownValue([dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)?.asset_condition])
          tempValue=dfd_assetconditioncombo_v1Props?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)?.asset_condition
        }else{
          setdropdownValue([assignment_information_group5d144.condition_at_assign])
          tempValue=assignment_information_group5d144.condition_at_assign
        }
      }else{
        let dstKey:string = dfd_assetconditioncombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{asset_condition:assignment_information_group5d144.condition_at_assign}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)?.asset_condition ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.asset_condition === assignment_information_group5d144.condition_at_assign)?.asset_condition
      }else{
        setdropdownValue([assignment_information_group5d144.condition_at_assign])
        tempValue=assignment_information_group5d144.condition_at_assign
      }   
    ...
    if(condition_at_assign27aff?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setcondition_at_assign27aff((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[assignment_information_group5d144.condition_at_assign, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_assetconditioncombo_v1Props) && dfd_assetconditioncombo_v1Props?.length == 1){
    // setassignment_information_group5d144((pre:any)=>({...pre,condition_at_assign:dfd_assetconditioncombo_v1Props[0]?.condition_at_assign}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setassignment_information_group5d144((prev: any) => ({ ...prev, condition_at_assign: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setassignment_information_group5d144((prev: any) => ({ ...prev, condition_at_assign: '', condition_at_assign27aff: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,condition_at_assign:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.asset_condition==getMapperDetailsBindValues[value] && items?.asset_condition==value)) || {}
    selected.current={
      ...selectedObj||{},
      asset_condition:value,
      asset_condition:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
      if(assignment_information_group5d144?.condition_at_assign == "" || assignment_information_group5d144?.condition_at_assign == undefined){
      assignment_information_group5d144.condition_at_assign = "";
      const validate:any = v.safeParse(schema, assignment_information_group5d144?.condition_at_assign);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,condition_at_assign:"invalid"}}));
        }
    }else if(assignment_information_group5d144?.condition_at_assign !== ""){
    const validate:any = v.safeParse(schema, assignment_information_group5d144?.condition_at_assign);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,condition_at_assign:"invalid"}}));
  ...
  useEffect(() => { assignment_information_group5d144Ref.current = assignment_information_group5d144; }, [assignment_information_group5d144]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "5b443277b5f1422e91a1137e44427aff") {
        handleClick(assignment_information_group5d144Ref?.current?.condition_at_assign27aff?assignment_information_group5d144Ref?.current?.condition_at_assign27aff:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "5b443277b5f1422e91a1137e44427aff");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setassignment_information_group5d144((pre:any)=>({...pre,condition_at_assign:""}))
    else
      setInitialCount(1)
  },[condition_at_assign27aff?.refresh])

  if (condition_at_assign27aff?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {condition_at_assign27aff?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={condition_at_assignOptions}
        onLoadMore={loadMore}
        ...
        value={assignment_information_group5d144?.condition_at_assign27aff ? [assignment_information_group5d144?.condition_at_assign27aff] : (assignment_information_group5d144?.condition_at_assign ? dropdownValue : [])}
        validationState={validate?.assignAsset_v1?.condition_at_assign ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdowncondition_at_assign;
