...
const TextInputassigned_by = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAsset:AFVK:v1|913d50f029c84864b01c8a333a75d144|9f3166b31ccb4dd8bd6088889d8c4563"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,assigned_by:undefined}}));
    if(dynamicStateandType.type=="number"){
    setassignment_information_group5d144((prev: any) => ({ ...prev, assigned_by: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(assignment_information_group5d144?.assigned_by == "" || assignment_information_group5d144?.assigned_by == undefined){
      assignment_information_group5d144.assigned_by = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, assignment_information_group5d144?.assigned_by);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,assignAsset_v1:{...pre?.assignAsset_v1,assigned_by:"invalid"}}));
        }
    }else if(assignment_information_group5d144?.assigned_by !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +assignment_information_group5d144?.assigned_by);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9f3166b31ccb4dd8bd6088889d8c4563"
      );
      ...
      //     controlId: "9f3166b31ccb4dd8bd6088889d8c4563",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!assignment_information_group5d144?.assigned_by)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "9f3166b31ccb4dd8bd6088889d8c4563") {
        handleChange({target:{value:assignment_information_group5d144Ref?.current?.assigned_by||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9f3166b31ccb4dd8bd6088889d8c4563") {
        handleBlur({target:{value:assignment_information_group5d144Ref?.current?.assigned_by||""}});
      }
    };
  ...
  if(dfd_assetassignments_v1Props?.setSearchFilters && dfd_assetassignments_v1Props?.data)
  {
    if(Array.isArray(dfd_assetassignments_v1Props.data) && dfd_assetassignments_v1Props.data.length > 0){
      setassignment_information_group5d144((pre:any)=>({...pre,assigned_by:dfd_assetassignments_v1Props.data[0]?.assigned_by}));
    }
  }
  },[dfd_assetassignments_v1Props?.setSearchFilters])
  if (assigned_byc4563?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `9 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={assignment_information_group5d144?.assigned_by||""}
         disabled= {assigned_byc4563?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assignAsset_v1?.assigned_by ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
