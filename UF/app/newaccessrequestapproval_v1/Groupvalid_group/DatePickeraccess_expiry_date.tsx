...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,access_expiry_date: "invalid"}}))
    setvalid_groupec09d((prev: any) => ({ ...prev, access_expiry_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,access_expiry_date:undefined}}));
  if (!date) {
    setvalid_groupec09d((prev: any) => ({ ...prev, access_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_groupec09d((prev: any) => ({ ...prev, access_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_groupec09d?.access_expiry_date == "" || valid_groupec09d?.access_expiry_date == undefined){
        const result = v.safeParse(schema, valid_groupec09d?.access_expiry_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "45dd686c9fc0f1803bfa090445f1529d"
      );
...
useEffect(()=>{
 ...
 },[access_expiry_date1529d?.refresh])

useEffect(()=>{
  if(!valid_groupec09d?.access_expiry_date){ 
...
if (access_expiry_date1529d?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_groupec09d?.access_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {access_expiry_date1529d?.isDisabled ? true : false}
      disabled= {access_expiry_date1529d?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Access Expiry Date"
      dateValidation=""
      validationState={validate?.newAccessRequestApproval_v1?.access_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
