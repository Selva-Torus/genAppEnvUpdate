...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,valid_from: "invalid"}}))
    setvalid_groupec09d((prev: any) => ({ ...prev, valid_from: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,valid_from:undefined}}));
  if (!date) {
    setvalid_groupec09d((prev: any) => ({ ...prev, valid_from: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_groupec09d((prev: any) => ({ ...prev, valid_from: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_groupec09d?.valid_from == "" || valid_groupec09d?.valid_from == undefined){
        const result = v.safeParse(schema, valid_groupec09d?.valid_from || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "899c11a87df5b14352751abfe3fe715b"
      );
...
useEffect(()=>{
 ...
 },[valid_frome715b?.refresh])

useEffect(()=>{
  if(!valid_groupec09d?.valid_from){ 
...
return (
  ...
  style={{gridColumn: `1 / 9`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_groupec09d?.valid_from}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {valid_frome715b?.isDisabled ? true : false}
      disabled= {valid_frome715b?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Valid From"
      dateValidation=""
      validationState={validate?.newAccessRequestApproval_v1?.valid_from ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
