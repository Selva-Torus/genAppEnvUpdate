...
import Textprov_inf  from "./Textprov_inf";
...
const Groupprov_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {prov_inf00981, setprov_inf00981}= useContext(TotalContext) as TotalContextProps;
  const {prov_inf_des18d0b, setprov_inf_des18d0b}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fdb43f892038da01ccf1456a94b33ef1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprov_group33ef1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prov_inf")){
        setprov_inf00981((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf00981?.isDisabled==null)
      {
        setprov_inf00981((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_inf_des")){
        setprov_inf_des18d0b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf_des18d0b?.isDisabled==null)
      {
        setprov_inf_des18d0b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['prov_inf'] = prov_inf00981,
        codeStates['setprov_inf'] = setprov_inf00981,
        codeStates['prov_inf_des'] = prov_inf_des18d0b,
        codeStates['setprov_inf_des'] = setprov_inf_des18d0b,
  ...
  const prov_group33ef1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    prov_group33ef1Ref.current?.setSearchParams();
    prov_group33ef1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(prov_group33ef1) && Object.keys(prov_group33ef1)?.length>0)
      {
        setprov_group33ef1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [prov_group33ef1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"prov_group"}))
        }}
    >
          {allowedControls.includes("prov_inf") ?<Textprov_inf   /* 00981 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("prov_inf_des") ?<Textprov_inf_des   /* 18d0b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupprov_group
