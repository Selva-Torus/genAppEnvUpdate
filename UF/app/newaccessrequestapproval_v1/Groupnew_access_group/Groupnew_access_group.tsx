...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "73190eff3aec96dc73cbdde00f789009");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_group89009Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__groupf6698((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__groupf6698?.isDisabled==null)
      {
        setaccess_req__groupf6698((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("business_just__group")){
        setbusiness_just__group5c7fc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_just__group5c7fc?.isDisabled==null)
      {
        setbusiness_just__group5c7fc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_group")){
        setvalid_groupec09d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_groupec09d?.isDisabled==null)
      {
        setvalid_groupec09d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_inf_group")){
        setapp_inf_group4185e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_inf_group4185e?.isDisabled==null)
      {
        setapp_inf_group4185e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("provision_group")){
        setprovision_groupf5a52((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(provision_groupf5a52?.isDisabled==null)
      {
        setprovision_groupf5a52((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revocation_group")){
        setrevocation_group9c6ae((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revocation_group9c6ae?.isDisabled==null)
      {
        setrevocation_group9c6ae((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_group")){
        setaudit_group270d9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_group270d9?.isDisabled==null)
      {
        setaudit_group270d9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactions51364((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactions51364?.isDisabled==null)
      {
        setdynamicactions51364((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("access_req_id")){
        setaccess_req_id2369c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req_id2369c?.isDisabled==null)
      {
        setaccess_req_id2369c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['access_req_id'] = access_req_id2369c,
        codeStates['setaccess_req_id'] = setaccess_req_id2369c,
  ...
  const new_access_group89009Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_group89009Ref.current?.setSearchParams();
    new_access_group89009Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_group89009) && Object.keys(new_access_group89009)?.length>0)
      {
        setnew_access_group89009({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_group89009Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("access_req_id") ?<Textaccess_req_id   /* 2369c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
