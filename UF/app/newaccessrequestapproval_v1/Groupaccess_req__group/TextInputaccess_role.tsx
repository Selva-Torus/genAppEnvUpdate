...
const TextInputaccess_role = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequestApproval:AFVK:v1|243f73b730937821efe0461c690f6698|affd56aba4533a9f54ca9a07d4c058b8"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,access_role:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__groupf6698((prev: any) => ({ ...prev, access_role: +e.target.value }));
    }
    else{
    setaccess_req__groupf6698((prev: any) => ({ ...prev, access_role: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "affd56aba4533a9f54ca9a07d4c058b8"
      );
      ...
      //     controlId: "affd56aba4533a9f54ca9a07d4c058b8",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "affd56aba4533a9f54ca9a07d4c058b8") {
        handleChange({target:{value:access_req__groupf6698Ref?.current?.access_role||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "affd56aba4533a9f54ca9a07d4c058b8") {
        handleBlur({target:{value:access_req__groupf6698Ref?.current?.access_role||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setaccess_req__groupf6698((pre:any)=>({...pre,access_role:dfd_addaccessrequestmodify_v1Props.data[0]?.access_role}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (access_role058b8?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__groupf6698?.access_role||""}
         disabled= {access_role058b8?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAccessRequestApproval_v1?.access_role ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
