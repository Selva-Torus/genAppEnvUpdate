...
const Dropdownrequest_priority = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [request_priorityOptions, setrequest_priorityOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `emp_request_priority`
    let mapperText: string =  `emp_request_priority`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "fcd1767263bd4b98d6813f5134bbdba9"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_requestprioritycombo_v1Props.dstKey){
      dstKey = dfd_requestprioritycombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_requestprioritycombo_v1Props && !dfd_requestprioritycombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  try{
    ...
    if(!value){
      ...
      setrequest_priorityOptions(temp);
  ...
  },[request_prioritybdba9?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__groupf6698((prev: any) => ({ ...prev,request_prioritybdba9: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__groupf6698;
      delete temp.request_priority;
      delete temp.REQUEST_PRIORITY;
      delete temp.request_prioritybdba9;
      setaccess_req__groupf6698(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_priority:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__groupf6698.request_priority){
      if(Array.isArray(dfd_requestprioritycombo_v1Props)){
        if(dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)){
          setdropdownValue([dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)?.emp_request_priority])
          tempValue=dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)?.emp_request_priority
        }else{
          setdropdownValue([access_req__groupf6698.request_priority])
          tempValue=access_req__groupf6698.request_priority
        }
      }else{
        let dstKey:string = dfd_requestprioritycombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_request_priority:access_req__groupf6698.request_priority}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)?.emp_request_priority ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__groupf6698.request_priority)?.emp_request_priority
      }else{
        setdropdownValue([access_req__groupf6698.request_priority])
        tempValue=access_req__groupf6698.request_priority
      }   
    ...
    if(request_prioritybdba9?.trigger == true)
    {
      await handlechange(tempValue)
      setrequest_prioritybdba9((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__groupf6698.request_priority, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_requestprioritycombo_v1Props) && dfd_requestprioritycombo_v1Props?.length == 1){
    // setaccess_req__groupf6698((pre:any)=>({...pre,request_priority:dfd_requestprioritycombo_v1Props[0]?.request_priority}))
    }
  },[dfd_requestprioritycombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__groupf6698((prev: any) => ({ ...prev, request_priority: getMapperDetailsBindValues[value],REQUEST_PRIORITY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__groupf6698((prev: any) => ({ ...prev, request_priority: '', request_prioritybdba9: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_priority:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.emp_request_priority==getMapperDetailsBindValues[value] && items?.emp_request_priority==value)) || {}
    selected.current={
      ...selectedObj||{},
      emp_request_priority:value,
      emp_request_priority:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__groupf6698?.request_priority == "" || access_req__groupf6698?.request_priority == undefined){
      access_req__groupf6698.request_priority = "";
      const validate:any = v.safeParse(schema, access_req__groupf6698?.request_priority);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_priority:"invalid"}}));
        }
    }else if(access_req__groupf6698?.request_priority !== ""){
    const validate:any = v.safeParse(schema, access_req__groupf6698?.request_priority);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_priority:"invalid"}}));
  ...
  useEffect(() => { access_req__groupf6698Ref.current = access_req__groupf6698; }, [access_req__groupf6698]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "fcd1767263bd4b98d6813f5134bbdba9") {
        handleClick(access_req__groupf6698Ref?.current?.request_prioritybdba9?access_req__groupf6698Ref?.current?.request_prioritybdba9:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "fcd1767263bd4b98d6813f5134bbdba9");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__groupf6698((pre:any)=>({...pre,request_priority:""}))
    else
      setInitialCount(1)
  },[request_prioritybdba9?.refresh])
  

  if (request_prioritybdba9?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {request_prioritybdba9?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={request_priorityOptions}
        onLoadMore={loadMore}
        ...
        value={access_req__groupf6698?.request_prioritybdba9 ? [access_req__groupf6698?.request_prioritybdba9] : (access_req__groupf6698?.request_priority ? dropdownValue : [])}
        validationState={validate?.newAccessRequestApproval_v1?.request_priority ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownrequest_priority;
