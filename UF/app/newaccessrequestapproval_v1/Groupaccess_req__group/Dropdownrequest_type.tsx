...
const Dropdownrequest_type = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [request_typeOptions, setrequest_typeOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `emp_request_type`
    let mapperText: string =  `emp_request_type`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "f8078d2d2ac33c40a0d2f2ea35a7ec60"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_requesttypecombo_v1Props.dstKey){
      dstKey = dfd_requesttypecombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_requesttypecombo_v1Props && !dfd_requesttypecombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[request_type7ec60?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__groupf6698((prev: any) => ({ ...prev,request_type7ec60: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__groupf6698;
      delete temp.request_type;
      delete temp.REQUEST_TYPE;
      delete temp.request_type7ec60;
      setaccess_req__groupf6698(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__groupf6698.request_type){
      if(Array.isArray(dfd_requesttypecombo_v1Props)){
        if(dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)){
          setdropdownValue([dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)?.emp_request_type])
          tempValue=dfd_requesttypecombo_v1Props?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)?.emp_request_type
        }else{
          setdropdownValue([access_req__groupf6698.request_type])
          tempValue=access_req__groupf6698.request_type
        }
      }else{
        let dstKey:string = dfd_requesttypecombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_request_type:access_req__groupf6698.request_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)?.emp_request_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_request_type === access_req__groupf6698.request_type)?.emp_request_type
      }else{
        setdropdownValue([access_req__groupf6698.request_type])
        tempValue=access_req__groupf6698.request_type
      }   
    ...
    if(request_type7ec60?.trigger == true)
    {
      await handlechange(tempValue)
      setrequest_type7ec60((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__groupf6698.request_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_requesttypecombo_v1Props) && dfd_requesttypecombo_v1Props?.length == 1){
    // setaccess_req__groupf6698((pre:any)=>({...pre,request_type:dfd_requesttypecombo_v1Props[0]?.request_type}))
    }
  },[dfd_requesttypecombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__groupf6698((prev: any) => ({ ...prev, request_type: getMapperDetailsBindValues[value],REQUEST_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__groupf6698((prev: any) => ({ ...prev, request_type: '', request_type7ec60: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_type:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.emp_request_type==getMapperDetailsBindValues[value] && items?.emp_request_type==value)) || {}
    selected.current={
      ...selectedObj||{},
      emp_request_type:value,
      emp_request_type:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__groupf6698?.request_type == "" || access_req__groupf6698?.request_type == undefined){
      access_req__groupf6698.request_type = "";
      const validate:any = v.safeParse(schema, access_req__groupf6698?.request_type);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_type:"invalid"}}));
        }
    }else if(access_req__groupf6698?.request_type !== ""){
    const validate:any = v.safeParse(schema, access_req__groupf6698?.request_type);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,request_type:"invalid"}}));
  ...
  useEffect(() => { access_req__groupf6698Ref.current = access_req__groupf6698; }, [access_req__groupf6698]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "f8078d2d2ac33c40a0d2f2ea35a7ec60") {
        handleClick(access_req__groupf6698Ref?.current?.request_type7ec60?access_req__groupf6698Ref?.current?.request_type7ec60:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "f8078d2d2ac33c40a0d2f2ea35a7ec60");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__groupf6698((pre:any)=>({...pre,request_type:""}))
    else
      setInitialCount(1)
  },[request_type7ec60?.refresh])
  

  if (request_type7ec60?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {request_type7ec60?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={request_typeOptions}
        onLoadMore={loadMore}
        ...
        value={access_req__groupf6698?.request_type7ec60 ? [access_req__groupf6698?.request_type7ec60] : (access_req__groupf6698?.request_type ? dropdownValue : [])}
        validationState={validate?.newAccessRequestApproval_v1?.request_type ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownrequest_type;
