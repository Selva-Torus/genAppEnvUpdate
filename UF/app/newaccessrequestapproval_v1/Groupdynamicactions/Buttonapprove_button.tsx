...
const Buttonapprove_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "7cc7a9eebea4076bc432e1d243f2a6ba"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("approve_button" in dynamicactions51364Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions51364Props?.dynamicActionRule?.approve_button,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("approve_button" in newaccessrequestapproval_v1?.dynamicactions && newaccessrequestapproval_v1?.dynamicactions["approve_button"]?.itsHaveArtifact== true)
      {
        setShowFlag(newaccessrequestapproval_v1?.dynamicactions["approve_button"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newaccessrequestapproval_v1?.dynamicactions?.approve_button?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "approve_button2a6ba") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "7cc7a9eebea4076bc432e1d243f2a6ba") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "approve_button2a6ba");
    return () => {
  ...
  useEffect(()=>{
    if (!approve_button2a6ba?.trigger) return;
      if(approve_button2a6ba?.trigger){
      setapprove_button2a6ba((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[approve_button2a6ba?.trigger])

  useEffect(()=>{
    if(approve_button2a6ba?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[approve_button2a6ba?.refresh])
  ...
  async function handleSavea6ba_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const dynamicactions51364Props = dynamicactions51364PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.newAccessRequestApproval_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(dynamicactions51364Ref.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:approveNewAccessRequest:AFVK:v1|46fa19eb79084ae6bcbacf4dcae74a12"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:approveNewAccessRequest:AFVK:v1|46fa19eb79084ae6bcbacf4dcae74a12"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactions51364?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "7cc7a9eebea4076bc432e1d243f2a6ba",
  "type": "button",
  "name": "approve_button",
  ...
  "children": [
    {
      "id": "7cc7a9eebea4076bc432e1d243f2a6ba.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "7cc7a9eebea4076bc432e1d243f2a6ba.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "7cc7a9eebea4076bc432e1d243f2a6ba.1.1.1.1",
              ...
              "children": [
                {
                  "id": "6b7c6238e3f84271b65c7f36e733ced6.1.1.1.1.1",
                  "value": "",
                  "type": "screen",
            ...
            {
              "id": "7cc7a9eebea4076bc432e1d243f2a6ba.1.1.1.2",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "7cc7a9eebea4076bc432e1d243f2a6ba.1.1.1.2.1",
                  "eventContext": "rise",
                  "value": "",
                  "type": "handlerNode",
                  "name": "closeHandler",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:approveNewAccessRequest:AFVK:v1|46fa19eb79084ae6bcbacf4dcae74a12"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequestApproval:AFVK:v1";
      sourceId+= "|"+"987efbf92f26f9cdd3f8f9c04f851364";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequestApproval:AFVK:v1"+"|"+"987efbf92f26f9cdd3f8f9c04f851364"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions51364?.upId? [dynamicactions51364?.upId ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName : "approve_button"
      }
      }else{
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions51364?.upId? [dynamicactions51364?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions51364?.upId? [dynamicactions51364?.upId ] : dynamicactions51364?.trs_process_id? [dynamicactions51364?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "approve_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions51364Ref.current, ...nullFilter(new_access_group89009), ...nullFilter(access_req__groupf6698), ...nullFilter(business_just__group5c7fc), ...nullFilter(valid_groupec09d), ...nullFilter(app_inf_group4185e), ...nullFilter(approve_group6b509), ...nullFilter(provision_groupf5a52), ...nullFilter(prov_group33ef1), ...nullFilter(revocation_group9c6ae), ...nullFilter(rev_group4b1cb), ...nullFilter(audit_group270d9) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"987efbf92f26f9cdd3f8f9c04f851364",
              controlId:"7cc7a9eebea4076bc432e1d243f2a6ba"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (dynamicactions51364Props.ssKey !== '' && dynamicactions51364Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = dynamicactions51364Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Request Approved', 'success',true);
    // closeHandler   
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions51364((prev: any) => ({ ...prev, approve_button: true }));
        //onClick

    //eventEmitter
    await handleSavea6ba_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions51364((prev: any) => ({ ...prev, approve_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newaccessrequestapproval_v1?.dynamicactions?.approve_button,dynamicactions51364Props?.dynamicActionRule?.approve_button,])

 if (approve_button2a6ba?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {approve_button2a6ba?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("Approved")}
