...
import Buttonreject_button  from "./Buttonreject_button";
...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "987efbf92f26f9cdd3f8f9c04f851364");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions51364Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel6a7f9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel6a7f9?.isDisabled==null)
      {
        setcancel6a7f9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approve_button")){
        setapprove_button2a6ba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approve_button2a6ba?.isDisabled==null)
      {
        setapprove_button2a6ba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_button")){
        setreject_buttone810c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_buttone810c?.isDisabled==null)
      {
        setreject_buttone810c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions51364Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions51364Ref.current?.setSearchParams();
    dynamicactions51364Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions51364Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions51364Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions51364) && Object.keys(dynamicactions51364)?.length>0)
      {
        setdynamicactions51364({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions51364Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
        ...
        {        ((ruleData?.length>0 && "approve_button" in ButtonGoRuleData)?ButtonGoRuleData["approve_button"]:true) && 
          allowedControls.includes("approve_button")  ?            <Buttonapprove_button tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
        {        ((ruleData?.length>0 && "reject_button" in ButtonGoRuleData)?ButtonGoRuleData["reject_button"]:true) && 
          allowedControls.includes("reject_button")  ?            <Buttonreject_button tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}
