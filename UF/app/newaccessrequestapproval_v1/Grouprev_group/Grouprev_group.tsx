...
const Grouprev_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {access_revok2382e, setaccess_revok2382e}= useContext(TotalContext) as TotalContextProps;
  const {prov_inf_desa5498, setprov_inf_desa5498}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "43c0d08a419a26059f9c43fe2934b1cb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrev_group4b1cbProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_revok")){
        setaccess_revok2382e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_revok2382e?.isDisabled==null)
      {
        setaccess_revok2382e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_inf_des")){
        setprov_inf_desa5498((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf_desa5498?.isDisabled==null)
      {
        setprov_inf_desa5498((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['access_revok'] = access_revok2382e,
        codeStates['setaccess_revok'] = setaccess_revok2382e,
        codeStates['prov_inf_des'] = prov_inf_desa5498,
        codeStates['setprov_inf_des'] = setprov_inf_desa5498,
  ...
  const rev_group4b1cbRef = useRef<any>(null);
  const handleClearSearch = () => {
    rev_group4b1cbRef.current?.setSearchParams();
    rev_group4b1cbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rev_group4b1cb) && Object.keys(rev_group4b1cb)?.length>0)
      {
        setrev_group4b1cb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rev_group4b1cbProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"rev_group"}))
        }}
    >
          {allowedControls.includes("access_revok") ?<Textaccess_revok   /* 2382e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("prov_inf_des") ?<Textprov_inf_des   /* a5498 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
