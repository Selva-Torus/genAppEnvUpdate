...
const TextArearevoked_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "face6752f69ced0292bd99cffa9f5ff7"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[revoked_reasonf5ff7?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setrevocation_group9c6ae((pre:any)=>({...pre,revoked_reason:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "face6752f69ced0292bd99cffa9f5ff7") {
        handleChange({target:{value:revocation_group9c6aeRef?.current?.revoked_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "face6752f69ced0292bd99cffa9f5ff7") {
        handleBlur({target:{value:revocation_group9c6aeRef?.current?.revoked_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,revoked_reason:undefined}}));
    setrevocation_group9c6ae((prev: any) => ({ ...prev, revoked_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (revoked_reasonf5ff7?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 25`,gridRow: `21 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {revoked_reasonf5ff7?.isDisabled ? true : false}
      placeholder = {'Enter reason for revocation...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Revoked Reason"
      pin = {'brick-brick'}
      value = { revocation_group9c6ae?.revoked_reason != null && typeof revocation_group9c6ae?.revoked_reason =='object' ? Object.keys(revocation_group9c6ae?.revoked_reason)?.length ?  JSON.stringify(revocation_group9c6ae?.revoked_reason,null ,2):"" : revocation_group9c6ae?.revoked_reason||""}
      errorMessage={error}
      validationState={validate?.newAccessRequestApproval_v1?.revoked_reason ? "invalid" : undefined}
    />
  </div>
  )
}
