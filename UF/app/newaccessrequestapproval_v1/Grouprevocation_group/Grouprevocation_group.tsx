...
const Grouprevocation_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "46f10e735b86fb544c49256e2979c6ae");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrevocation_group9c6aeProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rev_group")){
        setrev_group4b1cb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(rev_group4b1cb?.isDisabled==null)
      {
        setrev_group4b1cb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revoked_by")){
        setrevoked_by7a13e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revoked_by7a13e?.isDisabled==null)
      {
        setrevoked_by7a13e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revok_at")){
        setrevok_atb5109((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revok_atb5109?.isDisabled==null)
      {
        setrevok_atb5109((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("revoked_reason")){
        setrevoked_reasonf5ff7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(revoked_reasonf5ff7?.isDisabled==null)
      {
        setrevoked_reasonf5ff7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const revocation_group9c6aeRef = useRef<any>(null);
  const handleClearSearch = () => {
    revocation_group9c6aeRef.current?.setSearchParams();
    revocation_group9c6aeRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(revocation_group9c6ae) && Object.keys(revocation_group9c6ae)?.length>0)
      {
        setrevocation_group9c6ae({})
      }
    }else 
      prevRefreshRef.current= true
  }, [revocation_group9c6aeProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"revocation_group"}))
        }}
    >
        ...
        {allowedControls.includes("revoked_by") ?<TextInputrevoked_by   /* 7a13e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("revok_at") ?<DatePickerrevok_at   /* b5109 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("revoked_reason") ?<TextArearevoked_reason   /* f5ff7 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
