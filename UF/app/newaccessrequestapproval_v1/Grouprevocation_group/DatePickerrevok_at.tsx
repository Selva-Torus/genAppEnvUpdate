...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,revoked_at:undefined}}));
  if (!date) {
    setrevocation_group9c6ae((prev: any) => ({ ...prev, revoked_at: null }));
    return;
  }
  const now = new Date();
  ...
  setrevocation_group9c6ae((prev: any) => ({ ...prev, revoked_at: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "1d9cab306c46185cc7ea1aec8a8b5109"
      );
...
useEffect(()=>{
  setrevocation_group9c6aeProps((pre:any)=>({...pre,validation:true}))
 },[revok_atb5109?.refresh])
...
return (
  ...
  style={{gridColumn: `13 / 25`,gridRow: `9 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={revocation_group9c6ae?.revoked_at}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {revok_atb5109?.isDisabled ? true : false}
      disabled= {revok_atb5109?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Revoked At"
      dateValidation=""
      validationState={validate?.newAccessRequestApproval_v1?.revoked_at ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
