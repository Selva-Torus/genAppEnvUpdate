...
const TextAreaapproval_comments = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ef46ebc722128dfe8cc5d96826676b6a"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[approval_comments76b6a?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_inf_group4185e((pre:any)=>({...pre,approval_comments:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ef46ebc722128dfe8cc5d96826676b6a") {
        handleChange({target:{value:app_inf_group4185eRef?.current?.approval_comments||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ef46ebc722128dfe8cc5d96826676b6a") {
        handleBlur({target:{value:app_inf_group4185eRef?.current?.approval_comments||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,approval_comments:undefined}}));
    setapp_inf_group4185e((prev: any) => ({ ...prev, approval_comments: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (approval_comments76b6a?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `9 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {approval_comments76b6a?.isDisabled ? true : false}
      placeholder = {'Approved for the requested access..'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Approval Comments"
      pin = {'brick-brick'}
      value = { app_inf_group4185e?.approval_comments != null && typeof app_inf_group4185e?.approval_comments =='object' ? Object.keys(app_inf_group4185e?.approval_comments)?.length ?  JSON.stringify(app_inf_group4185e?.approval_comments,null ,2):"" : app_inf_group4185e?.approval_comments||""}
      errorMessage={error}
      validationState={validate?.newAccessRequestApproval_v1?.approval_comments ? "invalid" : undefined}
    />
  </div>
  )
}
