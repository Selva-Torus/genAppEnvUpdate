...
const Groupapp_inf_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {approval_comments76b6a, setapproval_comments76b6a}= useContext(TotalContext) as TotalContextProps;
  const {reject_reason8ce39, setreject_reason8ce39}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "270889a41f58b855b98caa0e2fa4185e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapp_inf_group4185eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("approve_group")){
        setapprove_group6b509((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approve_group6b509?.isDisabled==null)
      {
        setapprove_group6b509((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approval_comments")){
        setapproval_comments76b6a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approval_comments76b6a?.isDisabled==null)
      {
        setapproval_comments76b6a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_reason")){
        setreject_reason8ce39((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_reason8ce39?.isDisabled==null)
      {
        setreject_reason8ce39((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['approval_comments'] = approval_comments76b6a,
        codeStates['setapproval_comments'] = setapproval_comments76b6a,
        codeStates['reject_reason'] = reject_reason8ce39,
        codeStates['setreject_reason'] = setreject_reason8ce39,
  ...
  const app_inf_group4185eRef = useRef<any>(null);
  const handleClearSearch = () => {
    app_inf_group4185eRef.current?.setSearchParams();
    app_inf_group4185eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(app_inf_group4185e) && Object.keys(app_inf_group4185e)?.length>0)
      {
        setapp_inf_group4185e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [app_inf_group4185eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"app_inf_group"}))
        }}
    >
        ...
        {allowedControls.includes("approval_comments") ?<TextAreaapproval_comments   /* 76b6a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("reject_reason") ?<TextAreareject_reason   /* 8ce39 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
