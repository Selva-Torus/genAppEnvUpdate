...
const TextAreareject_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "270889a41f58b855b98caa0e2fa4185e",
        "f7b805fc8674bc488300abff7408ce39"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reject_reason8ce39?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_inf_group4185e((pre:any)=>({...pre,rejection_reason:""}))
    }else 
      prevRefreshRef.current= true
  },[reject_reason8ce39?.refresh])

  const app_inf_group4185eRef = useRef<any>(app_inf_group4185e);
  useEffect(() => { app_inf_group4185eRef.current = app_inf_group4185e; }, [app_inf_group4185e]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "f7b805fc8674bc488300abff7408ce39") {
        handleChange({target:{value:app_inf_group4185eRef?.current?.rejection_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f7b805fc8674bc488300abff7408ce39") {
        handleBlur({target:{value:app_inf_group4185eRef?.current?.rejection_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,rejection_reason:undefined}}));
    setapp_inf_group4185e((prev: any) => ({ ...prev, rejection_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (reject_reason8ce39?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `9 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {reject_reason8ce39?.isDisabled ? true : false}
      placeholder = {'Enter rejection reason if any...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Rejection Reason"
      pin = {'brick-brick'}
      value = { app_inf_group4185e?.rejection_reason != null && typeof app_inf_group4185e?.rejection_reason =='object' ? Object.keys(app_inf_group4185e?.rejection_reason)?.length ?  JSON.stringify(app_inf_group4185e?.rejection_reason,null ,2):"" : app_inf_group4185e?.rejection_reason||""}
      errorMessage={error}
      validationState={validate?.newAccessRequestApproval_v1?.rejection_reason ? "invalid" : undefined}
    />
  </div>
  )
}
