...
const Groupbusiness_just__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "094d38e13590c3fd6878c96afd55c7fc");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbusiness_just__group5c7fcProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("business_justify")){
        setbusiness_justifye613d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_justifye613d?.isDisabled==null)
      {
        setbusiness_justifye613d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("business_justification")){
        setbusiness_justification20f3a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_justification20f3a?.isDisabled==null)
      {
        setbusiness_justification20f3a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("additional_details")){
        setadditional_details11c25((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(additional_details11c25?.isDisabled==null)
      {
        setadditional_details11c25((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const business_just__group5c7fcRef = useRef<any>(null);
  const handleClearSearch = () => {
    business_just__group5c7fcRef.current?.setSearchParams();
    business_just__group5c7fcRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(business_just__group5c7fc) && Object.keys(business_just__group5c7fc)?.length>0)
      {
        setbusiness_just__group5c7fc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [business_just__group5c7fcProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"business_just__group"}))
        }}
    >
          {allowedControls.includes("business_justify") ?<Textbusiness_justify   /* e613d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("business_justification") ?<TextAreabusiness_justification   /* 20f3a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("additional_details") ?<TextAreaadditional_details   /* 11c25 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
