...
const TextAreaaudit_inf_textarea = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "d3bf1a0105214b7d2dcb016d3cd270d9",
        "e5de1c12ffee6c0acf099e5d48727797"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[audit_inf_textarea27797?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setaudit_group270d9((pre:any)=>({...pre,audit_metadata:""}))
    }else 
      prevRefreshRef.current= true
  },[audit_inf_textarea27797?.refresh])

  const audit_group270d9Ref = useRef<any>(audit_group270d9);
  useEffect(() => { audit_group270d9Ref.current = audit_group270d9; }, [audit_group270d9]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "e5de1c12ffee6c0acf099e5d48727797") {
        handleChange({target:{value:audit_group270d9Ref?.current?.audit_metadata||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e5de1c12ffee6c0acf099e5d48727797") {
        handleBlur({target:{value:audit_group270d9Ref?.current?.audit_metadata||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,audit_metadata:undefined}}));
    setaudit_group270d9((prev: any) => ({ ...prev, audit_metadata: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (audit_inf_textarea27797?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 25`,gridRow: `7 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {audit_inf_textarea27797?.isDisabled ? true : false}
      placeholder = {'--'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Audit Metadata"
      pin = {'brick-brick'}
      value = { audit_group270d9?.audit_metadata != null && typeof audit_group270d9?.audit_metadata =='object' ? Object.keys(audit_group270d9?.audit_metadata)?.length ?  JSON.stringify(audit_group270d9?.audit_metadata,null ,2):"" : audit_group270d9?.audit_metadata||""}
      errorMessage={error}
      validationState={validate?.newAccessRequestApproval_v1?.audit_metadata ? "invalid" : undefined}
    />
  </div>
  )
}
