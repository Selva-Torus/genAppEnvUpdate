...
import Textaudit_info  from "./Textaudit_info";
import TextAreaaudit_inf_textarea  from "./TextAreaaudit_inf_textarea";
...
const Groupaudit_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {audit_info18bfa, setaudit_info18bfa}= useContext(TotalContext) as TotalContextProps;
  const {audit_inf_textarea27797, setaudit_inf_textarea27797}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d3bf1a0105214b7d2dcb016d3cd270d9");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaudit_group270d9Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("audit_info")){
        setaudit_info18bfa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_info18bfa?.isDisabled==null)
      {
        setaudit_info18bfa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_inf_textarea")){
        setaudit_inf_textarea27797((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_inf_textarea27797?.isDisabled==null)
      {
        setaudit_inf_textarea27797((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['audit_info'] = audit_info18bfa,
        codeStates['setaudit_info'] = setaudit_info18bfa,
        codeStates['audit_inf_textarea'] = audit_inf_textarea27797,
        codeStates['setaudit_inf_textarea'] = setaudit_inf_textarea27797,
  ...
  const audit_group270d9Ref = useRef<any>(null);
  const handleClearSearch = () => {
    audit_group270d9Ref.current?.setSearchParams();
    audit_group270d9Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(audit_group270d9) && Object.keys(audit_group270d9)?.length>0)
      {
        setaudit_group270d9({})
      }
    }else 
      prevRefreshRef.current= true
  }, [audit_group270d9Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewaccessrequestapproval_v1((pre:any)=>({...pre,_selectedGroup_:"audit_group"}))
        }}
    >
          {allowedControls.includes("audit_info") ?<Textaudit_info   /* 18bfa */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("audit_inf_textarea") ?<TextAreaaudit_inf_textarea   /* 27797 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
