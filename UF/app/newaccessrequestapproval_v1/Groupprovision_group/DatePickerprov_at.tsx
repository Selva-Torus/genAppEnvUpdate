...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,provisioned_at:undefined}}));
  if (!date) {
    setprovision_groupf5a52((prev: any) => ({ ...prev, provisioned_at: null }));
    return;
  }
  const now = new Date();
  ...
  setprovision_groupf5a52((prev: any) => ({ ...prev, provisioned_at: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "196b92f8cd8e5e11d030d88e1e445563"
      );
...
useEffect(()=>{
  setprovision_groupf5a52Props((pre:any)=>({...pre,validation:true}))
 },[prov_at45563?.refresh])
...
return (
  ...
  style={{gridColumn: `13 / 25`,gridRow: `9 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={provision_groupf5a52?.provisioned_at}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {prov_at45563?.isDisabled ? true : false}
      disabled= {prov_at45563?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Provisioned At"
      dateValidation=""
      validationState={validate?.newAccessRequestApproval_v1?.provisioned_at ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
