...
const TextInputprovisioning_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newAccessRequestApproval:AFVK:v1|59c6acf3719d24a70ca6d96fd9af5a52|9641d12420fe1cfb39a2ce1cfaed3793"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAccessRequestApproval_v1:{...pre?.newAccessRequestApproval_v1,provisioning_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setprovision_groupf5a52((prev: any) => ({ ...prev, provisioning_reference: +e.target.value }));
    }
    else{
    setprovision_groupf5a52((prev: any) => ({ ...prev, provisioning_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9641d12420fe1cfb39a2ce1cfaed3793"
      );
      ...
      //     controlId: "9641d12420fe1cfb39a2ce1cfaed3793",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9641d12420fe1cfb39a2ce1cfaed3793") {
        handleChange({target:{value:provision_groupf5a52Ref?.current?.provisioning_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9641d12420fe1cfb39a2ce1cfaed3793") {
        handleBlur({target:{value:provision_groupf5a52Ref?.current?.provisioning_reference||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setprovision_groupf5a52((pre:any)=>({...pre,provisioning_reference:dfd_addaccessrequestmodify_v1Props.data[0]?.provisioning_reference}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (provisioning_referenced3793?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 13`,gridRow: `24 / 36`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={provision_groupf5a52?.provisioning_reference||""}
         disabled= {provisioning_referenced3793?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newAccessRequestApproval_v1?.provisioning_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
