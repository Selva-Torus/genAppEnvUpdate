...
const TextInputmiddle_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.middle_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|5d92c67029c108d489235a37078e03ea|3239ab64f684c7e9b4a6d93a17002f8e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'middle_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,middle_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, middle_name: +e.target.value }));
    }
    else{
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, middle_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3239ab64f684c7e9b4a6d93a17002f8e"
      );
      ...
      //     controlId: "3239ab64f684c7e9b4a6d93a17002f8e",
      //     isTable: false,
      //     from:"TextInputmiddle_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'middle_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'middle_name',type:'text'};
      //   type={
      //     name:'middle_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.middle_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.middle_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.middle_name.type
      //   }
      ...
      //   type={
      //     name:'middle_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.middle_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.middle_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.middle_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "3239ab64f684c7e9b4a6d93a17002f8e") {
        handleChange({target:{value:basic_details_groupe03eaRef?.current?.middle_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3239ab64f684c7e9b4a6d93a17002f8e") {
        handleBlur({target:{value:basic_details_groupe03eaRef?.current?.middle_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupe03ea((pre:any)=>({...pre,middle_name:dfd_employees_v1Props.data[0]?.middle_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (middle_name02f8e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupe03ea?.middle_name||""}
         disabled= {middle_name02f8e?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.middle_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmiddle_name
