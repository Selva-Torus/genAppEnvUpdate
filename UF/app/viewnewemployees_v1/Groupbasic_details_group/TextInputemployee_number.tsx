...
const TextInputemployee_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|5d92c67029c108d489235a37078e03ea|e683e1c2b9926943210852a26c18b9ab"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,emp_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, emp_number: +e.target.value }));
    }
    else{
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, emp_number: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e683e1c2b9926943210852a26c18b9ab"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "5d92c67029c108d489235a37078e03ea",
      //     controlId: "e683e1c2b9926943210852a26c18b9ab",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e683e1c2b9926943210852a26c18b9ab") {
        handleChange({target:{value:basic_details_groupe03eaRef?.current?.emp_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e683e1c2b9926943210852a26c18b9ab") {
        handleBlur({target:{value:basic_details_groupe03eaRef?.current?.emp_number||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupe03ea((pre:any)=>({...pre,emp_number:dfd_employees_v1Props.data[0]?.emp_number}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (employee_number8b9ab?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupe03ea?.emp_number||""}
         disabled= {employee_number8b9ab?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.emp_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
