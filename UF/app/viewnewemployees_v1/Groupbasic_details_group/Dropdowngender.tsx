 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "1274918fa69cfc9164cd0151acb91e7d"
    );
  ...
  const handlechange = async(value: any) => {
    ...
    if(value.length>0){
      ...
      setbasic_details_groupe03ea((prev: any) => ({ ...prev,gender91e7d: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basic_details_groupe03ea;
      delete temp.gender;
      delete temp.GENDER;
      delete temp.gender91e7d;
      setbasic_details_groupe03ea(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,gender:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basic_details_groupe03ea.gender){
      if(Array.isArray(dfd_gendercombo_v1Props)){
        if(dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupe03ea.gender)){
          setdropdownValue([dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupe03ea.gender)?.gender])
          tempValue=dfd_gendercombo_v1Props?.find((item: any) => item.gender === basic_details_groupe03ea.gender)?.gender
        }else{
          setdropdownValue([basic_details_groupe03ea.gender])
          tempValue=basic_details_groupe03ea.gender
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{gender:basic_details_groupe03ea.gender}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupe03ea.gender)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupe03ea.gender)?.gender ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.gender === basic_details_groupe03ea.gender)?.gender
      }else{
        setdropdownValue([basic_details_groupe03ea.gender])
        tempValue=basic_details_groupe03ea.gender
      }   
    ...
    if(gender91e7d?.trigger == true)
    {
      await handlechange(tempValue)
      setgender91e7d((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basic_details_groupe03ea.gender, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_gendercombo_v1Props) && dfd_gendercombo_v1Props?.length == 1){
    // setbasic_details_groupe03ea((pre:any)=>({...pre,gender:dfd_gendercombo_v1Props[0]?.gender}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasic_details_groupe03ea((prev: any) => ({ ...prev, gender: getMapperDetailsBindValues[value],GENDER: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasic_details_groupe03ea((prev: any) => ({ ...prev, gender: '', gender91e7d: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,gender:undefined}}));
  ...
  useEffect(() => { basic_details_groupe03eaRef.current = basic_details_groupe03ea; }, [basic_details_groupe03ea]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "1274918fa69cfc9164cd0151acb91e7d") {
        handleClick(basic_details_groupe03eaRef?.current?.gender91e7d?basic_details_groupe03eaRef?.current?.gender91e7d:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "1274918fa69cfc9164cd0151acb91e7d");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasic_details_groupe03ea((pre:any)=>({...pre,gender:""}))
    else
      setInitialCount(1)
  },[gender91e7d?.refresh])
  

  if (gender91e7d?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {gender91e7d?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={basic_details_groupe03ea?.gender91e7d ? [basic_details_groupe03ea?.gender91e7d] : (basic_details_groupe03ea?.gender ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.gender ? "invalid" : undefined}
        />
    </div>
  );
};
