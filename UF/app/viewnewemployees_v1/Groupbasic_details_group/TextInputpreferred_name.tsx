...
const TextInputpreferred_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|5d92c67029c108d489235a37078e03ea|3c679120fbac5c341c2af123c6cffc33"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,preferred_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, preferred_name: +e.target.value }));
    }
    else{
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, preferred_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3c679120fbac5c341c2af123c6cffc33"
      );
      ...
      //     controlId: "3c679120fbac5c341c2af123c6cffc33",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "3c679120fbac5c341c2af123c6cffc33") {
        handleChange({target:{value:basic_details_groupe03eaRef?.current?.preferred_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3c679120fbac5c341c2af123c6cffc33") {
        handleBlur({target:{value:basic_details_groupe03eaRef?.current?.preferred_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupe03ea((pre:any)=>({...pre,preferred_name:dfd_employees_v1Props.data[0]?.preferred_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (preferred_nameffc33?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupe03ea?.preferred_name||""}
         disabled= {preferred_nameffc33?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.preferred_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
