...
const Dropdownblood_group = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [blood_groupOptions, setblood_groupOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `blood_group`
    let mapperText: string =  `blood_group`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "b37f73917889ed4fd65c819d96007520"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_bloodgroupcombo_v1Props.dstKey){
      dstKey = dfd_bloodgroupcombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_bloodgroupcombo_v1Props && !dfd_bloodgroupcombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[blood_group07520?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbasic_details_groupe03ea((prev: any) => ({ ...prev,blood_group07520: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basic_details_groupe03ea;
      delete temp.blood_group;
      delete temp.BLOOD_GROUP;
      delete temp.blood_group07520;
      setbasic_details_groupe03ea(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,blood_group:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basic_details_groupe03ea.blood_group){
      if(Array.isArray(dfd_bloodgroupcombo_v1Props)){
        if(dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)){
          setdropdownValue([dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)?.blood_group])
          tempValue=dfd_bloodgroupcombo_v1Props?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)?.blood_group
        }else{
          setdropdownValue([basic_details_groupe03ea.blood_group])
          tempValue=basic_details_groupe03ea.blood_group
        }
      }else{
        let dstKey:string = dfd_bloodgroupcombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{blood_group:basic_details_groupe03ea.blood_group}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)?.blood_group ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.blood_group === basic_details_groupe03ea.blood_group)?.blood_group
      }else{
        setdropdownValue([basic_details_groupe03ea.blood_group])
        tempValue=basic_details_groupe03ea.blood_group
      }   
    ...
    if(blood_group07520?.trigger == true)
    {
      await handlechange(tempValue)
      setblood_group07520((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basic_details_groupe03ea.blood_group, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_bloodgroupcombo_v1Props) && dfd_bloodgroupcombo_v1Props?.length == 1){
    // setbasic_details_groupe03ea((pre:any)=>({...pre,blood_group:dfd_bloodgroupcombo_v1Props[0]?.blood_group}))
    }
  },[dfd_bloodgroupcombo_v1Props])
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasic_details_groupe03ea((prev: any) => ({ ...prev, blood_group: getMapperDetailsBindValues[value],BLOOD_GROUP: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasic_details_groupe03ea((prev: any) => ({ ...prev, blood_group: '', blood_group07520: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,blood_group:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.blood_group==getMapperDetailsBindValues[value] && items?.blood_group==value)) || {}
    selected.current={
      ...selectedObj||{},
      blood_group:value,
      blood_group:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  useEffect(() => { basic_details_groupe03eaRef.current = basic_details_groupe03ea; }, [basic_details_groupe03ea]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "b37f73917889ed4fd65c819d96007520") {
        handleClick(basic_details_groupe03eaRef?.current?.blood_group07520?basic_details_groupe03eaRef?.current?.blood_group07520:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "b37f73917889ed4fd65c819d96007520");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasic_details_groupe03ea((pre:any)=>({...pre,blood_group:""}))
    else
      setInitialCount(1)
  },[blood_group07520?.refresh])
  

  if (blood_group07520?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {blood_group07520?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={blood_groupOptions}
        onLoadMore={loadMore}
        ...
        value={basic_details_groupe03ea?.blood_group07520 ? [basic_details_groupe03ea?.blood_group07520] : (basic_details_groupe03ea?.blood_group ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.blood_group ? "invalid" : undefined}
        />
    </div>
  );
};

export default Dropdownblood_group;
