...
const TextInputfirst_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|5d92c67029c108d489235a37078e03ea|86c3ebb197c9796481b55a6066a60ab5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,first_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, first_name: +e.target.value }));
    }
    else{
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, first_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "86c3ebb197c9796481b55a6066a60ab5"
      );
      ...
      //     controlId: "86c3ebb197c9796481b55a6066a60ab5",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "86c3ebb197c9796481b55a6066a60ab5") {
        handleChange({target:{value:basic_details_groupe03eaRef?.current?.first_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "86c3ebb197c9796481b55a6066a60ab5") {
        handleBlur({target:{value:basic_details_groupe03eaRef?.current?.first_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupe03ea((pre:any)=>({...pre,first_name:dfd_employees_v1Props.data[0]?.first_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (first_name60ab5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupe03ea?.first_name||""}
         disabled= {first_name60ab5?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.first_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
