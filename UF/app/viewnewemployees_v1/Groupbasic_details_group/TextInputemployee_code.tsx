...
const TextInputemployee_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|5d92c67029c108d489235a37078e03ea|add23e4b1ebe0dc97e204e4bcef06972"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,employee_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, employee_code: +e.target.value }));
    }
    else{
    setbasic_details_groupe03ea((prev: any) => ({ ...prev, employee_code: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "add23e4b1ebe0dc97e204e4bcef06972"
      );
      ...
      //     controlId: "add23e4b1ebe0dc97e204e4bcef06972",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "add23e4b1ebe0dc97e204e4bcef06972") {
        handleChange({target:{value:basic_details_groupe03eaRef?.current?.employee_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "add23e4b1ebe0dc97e204e4bcef06972") {
        handleBlur({target:{value:basic_details_groupe03eaRef?.current?.employee_code||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbasic_details_groupe03ea((pre:any)=>({...pre,employee_code:dfd_employees_v1Props.data[0]?.employee_code}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (employee_code06972?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basic_details_groupe03ea?.employee_code||""}
         disabled= {employee_code06972?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.employee_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
