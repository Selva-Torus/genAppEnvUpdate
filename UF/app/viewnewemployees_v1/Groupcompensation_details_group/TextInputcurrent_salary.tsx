...
const TextInputcurrent_salary = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.current_salary"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|3db7c5c9c639f4a1925d30dc69b83106|899d330d9630db1370eead95ef7d261e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'current_salary',type:"number"})
  ...
  const {compensation_details3c118, setcompensation_details3c118}= useContext(TotalContext) as TotalContextProps;
  const {current_salaryd261e, setcurrent_salaryd261e}= useContext(TotalContext) as TotalContextProps;
  const {annual_ctc36c70, setannual_ctc36c70}= useContext(TotalContext) as TotalContextProps;
  const {salary_currencyf668b, setsalary_currencyf668b}= useContext(TotalContext) as TotalContextProps;
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,current_salary:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_details_group83106((prev: any) => ({ ...prev, current_salary: +e.target.value }));
    }
    else{
    setcompensation_details_group83106((prev: any) => ({ ...prev, current_salary: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['compensation_details'] = compensation_details3c118,
        codeStates['setcompensation_details'] = setcompensation_details3c118,
        codeStates['current_salary'] = current_salaryd261e,
        codeStates['setcurrent_salary'] = setcurrent_salaryd261e,
        codeStates['annual_ctc'] = annual_ctc36c70,
        codeStates['setannual_ctc'] = setannual_ctc36c70,
        codeStates['salary_currency'] = salary_currencyf668b,
        codeStates['setsalary_currency'] = setsalary_currencyf668b,
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "3db7c5c9c639f4a1925d30dc69b83106",
        "899d330d9630db1370eead95ef7d261e"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "3db7c5c9c639f4a1925d30dc69b83106",
      //     controlId: "899d330d9630db1370eead95ef7d261e",
      //     isTable: false,
      //     from:"TextInputcurrent_salary",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'current_salary', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'current_salary',type:'text'};
      //   type={
      //     name:'current_salary',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_salary.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_salary.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.current_salary.type
      //   }
      ...
      //   type={
      //     name:'current_salary',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_salary.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_salary.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.current_salary.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const compensation_details_group83106Ref = useRef<any>(compensation_details_group83106);
  useEffect(() => { compensation_details_group83106Ref.current = compensation_details_group83106; }, [compensation_details_group83106]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "899d330d9630db1370eead95ef7d261e") {
        handleChange({target:{value:compensation_details_group83106Ref?.current?.current_salary||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "899d330d9630db1370eead95ef7d261e") {
        handleBlur({target:{value:compensation_details_group83106Ref?.current?.current_salary||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcompensation_details_group83106((pre:any)=>({...pre,current_salary:dfd_employees_v1Props.data[0]?.current_salary}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (current_salaryd261e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={compensation_details_group83106?.current_salary||""}
         disabled= {current_salaryd261e?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.current_salary ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcurrent_salary
