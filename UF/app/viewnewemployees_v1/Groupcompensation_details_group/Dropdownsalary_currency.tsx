 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "71ce9a9f414ce694b2033a84e22f668b"
    );
  ...
  },[salary_currencyf668b?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setcompensation_details_group83106((prev: any) => ({ ...prev,salary_currencyf668b: value }))
      setIsRequredData(false)
    }else{
      let temp:any = compensation_details_group83106;
      delete temp.salary_currency;
      delete temp.SALARY_CURRENCY;
      delete temp.salary_currencyf668b;
      setcompensation_details_group83106(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,salary_currency:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(compensation_details_group83106.salary_currency){
      if(Array.isArray(dfd_currencycombo_v1Props)){
        if(dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)){
          setdropdownValue([dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)?.currency_no])
          tempValue=dfd_currencycombo_v1Props?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)?.currency_no
        }else{
          setdropdownValue([compensation_details_group83106.salary_currency])
          tempValue=compensation_details_group83106.salary_currency
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{currency_code:compensation_details_group83106.salary_currency}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)?.currency_no ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.currency_code === compensation_details_group83106.salary_currency)?.currency_no
      }else{
        setdropdownValue([compensation_details_group83106.salary_currency])
        tempValue=compensation_details_group83106.salary_currency
      }   
    ...
    if(salary_currencyf668b?.trigger == true)
    {
      await handlechange(tempValue)
      setsalary_currencyf668b((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[compensation_details_group83106.salary_currency, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_currencycombo_v1Props) && dfd_currencycombo_v1Props?.length == 1){
    // setcompensation_details_group83106((pre:any)=>({...pre,salary_currency:dfd_currencycombo_v1Props[0]?.salary_currency}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setcompensation_details_group83106((prev: any) => ({ ...prev, salary_currency: getMapperDetailsBindValues[value],SALARY_CURRENCY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setcompensation_details_group83106((prev: any) => ({ ...prev, salary_currency: '', salary_currencyf668b: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,salary_currency:undefined}}));
  ...
  useEffect(() => { compensation_details_group83106Ref.current = compensation_details_group83106; }, [compensation_details_group83106]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "71ce9a9f414ce694b2033a84e22f668b") {
        handleClick(compensation_details_group83106Ref?.current?.salary_currencyf668b?compensation_details_group83106Ref?.current?.salary_currencyf668b:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "71ce9a9f414ce694b2033a84e22f668b");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setcompensation_details_group83106((pre:any)=>({...pre,salary_currency:""}))
    else
      setInitialCount(1)
  },[salary_currencyf668b?.refresh])
  

  if (salary_currencyf668b?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {salary_currencyf668b?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={compensation_details_group83106?.salary_currencyf668b ? [compensation_details_group83106?.salary_currencyf668b] : (compensation_details_group83106?.salary_currency ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.salary_currency ? "invalid" : undefined}
        />
    </div>
  );
};
