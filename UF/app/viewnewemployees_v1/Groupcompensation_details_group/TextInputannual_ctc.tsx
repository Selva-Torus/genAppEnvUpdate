...
const TextInputannual_ctc = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.annual_ctc"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|3db7c5c9c639f4a1925d30dc69b83106|95b701f4bbb046d195b0c1ee98a36c70"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'annual_ctc',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,annual_ctc:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_details_group83106((prev: any) => ({ ...prev, annual_ctc: +e.target.value }));
    }
    else{
    setcompensation_details_group83106((prev: any) => ({ ...prev, annual_ctc: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "95b701f4bbb046d195b0c1ee98a36c70"
      );
      ...
      //     controlId: "95b701f4bbb046d195b0c1ee98a36c70",
      //     isTable: false,
      //     from:"TextInputannual_ctc",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'annual_ctc', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'annual_ctc',type:'text'};
      //   type={
      //     name:'annual_ctc',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.annual_ctc.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.annual_ctc.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.annual_ctc.type
      //   }
      ...
      //   type={
      //     name:'annual_ctc',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.annual_ctc.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.annual_ctc.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.annual_ctc.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "95b701f4bbb046d195b0c1ee98a36c70") {
        handleChange({target:{value:compensation_details_group83106Ref?.current?.annual_ctc||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "95b701f4bbb046d195b0c1ee98a36c70") {
        handleBlur({target:{value:compensation_details_group83106Ref?.current?.annual_ctc||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcompensation_details_group83106((pre:any)=>({...pre,annual_ctc:dfd_employees_v1Props.data[0]?.annual_ctc}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (annual_ctc36c70?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_details_group83106?.annual_ctc||""}
         disabled= {annual_ctc36c70?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. 6000000'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Annual CTC"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.annual_ctc ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputannual_ctc
