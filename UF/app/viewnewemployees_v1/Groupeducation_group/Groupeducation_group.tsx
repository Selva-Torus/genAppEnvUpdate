...
import Groupeducation  from "../Groupeducation/Groupeducation";
...
import Iconeducation_icon  from "./Iconeducation_icon";
import Texted_his_text  from "./Texted_his_text";
import Texted_des  from "./Texted_des";
...
const Groupeducation_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {education_icon99a7a, seteducation_icon99a7a}= useContext(TotalContext) as TotalContextProps;
  const {ed_his_textc52be, seted_his_textc52be}= useContext(TotalContext) as TotalContextProps;
  const {ed_des3936c, seted_des3936c}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "777d70f852658c4e00217974b3ccd288");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    seteducation_groupcd288Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("education_icon")){
        seteducation_icon99a7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education_icon99a7a?.isDisabled==null)
      {
        seteducation_icon99a7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ed_his_text")){
        seted_his_textc52be((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ed_his_textc52be?.isDisabled==null)
      {
        seted_his_textc52be((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ed_des")){
        seted_des3936c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ed_des3936c?.isDisabled==null)
      {
        seted_des3936c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("education")){
        seteducation28de7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education28de7?.isDisabled==null)
      {
        seteducation28de7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['education_icon'] = education_icon99a7a,
        codeStates['seteducation_icon'] = seteducation_icon99a7a,
        codeStates['ed_his_text'] = ed_his_textc52be,
        codeStates['seted_his_text'] = seted_his_textc52be,
        codeStates['ed_des'] = ed_des3936c,
        codeStates['seted_des'] = seted_des3936c,
  ...
  const education_groupcd288Ref = useRef<any>(null);
  const handleClearSearch = () => {
    education_groupcd288Ref.current?.setSearchParams();
    education_groupcd288Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(education_groupcd288) && Object.keys(education_groupcd288)?.length>0)
      {
        seteducation_groupcd288({})
      }
    }else 
      prevRefreshRef.current= true
  }, [education_groupcd288Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"education_group"}))
        }}
    >
        {allowedComponent.includes("education")  &&<Groupeducation  
        ...
        {allowedControls.includes("education_icon")?<Iconeducation_icon /* 99a7a */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("ed_his_text") ?<Texted_his_text   /* c52be */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("ed_des") ?<Texted_des   /* 3936c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupeducation_group
