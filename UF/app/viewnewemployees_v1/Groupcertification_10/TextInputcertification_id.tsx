...
const TextInputcertification_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setcertification02740_10((prev: any) => ({ ...prev, certification_id: +e.target.value }));
    }
    else{
    setcertification02740_10((prev: any) => ({ ...prev, certification_id: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5862d01b481b15a411b5bfb8582a2576") {
        handleChange({target:{value:certification02740_10Ref?.current?.certification_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5862d01b481b15a411b5bfb8582a2576") {
        handleBlur({target:{value:certification02740_10Ref?.current?.certification_id||""}});
      }
    };
  ...
  if(dfd_certifyjson_v1Props?.setSearchFilters && dfd_certifyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_certifyjson_v1Props.data) && dfd_certifyjson_v1Props.data.length > 0){
      setcertification02740_10((pre:any)=>({...pre,certification_id:dfd_certifyjson_v1Props.data[0]?.certification_id}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={certification02740_10?.certification_id||""}
