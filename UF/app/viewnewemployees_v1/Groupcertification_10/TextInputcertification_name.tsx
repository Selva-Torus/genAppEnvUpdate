...
const TextInputcertification_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setcertification02740_10((prev: any) => ({ ...prev, certification_name: +e.target.value }));
    }
    else{
    setcertification02740_10((prev: any) => ({ ...prev, certification_name: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "731a89929c4aaa1c3a57ef9b565a8b69") {
        handleChange({target:{value:certification02740_10Ref?.current?.certification_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "731a89929c4aaa1c3a57ef9b565a8b69") {
        handleBlur({target:{value:certification02740_10Ref?.current?.certification_name||""}});
      }
    };
  ...
  if(dfd_certifyjson_v1Props?.setSearchFilters && dfd_certifyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_certifyjson_v1Props.data) && dfd_certifyjson_v1Props.data.length > 0){
      setcertification02740_10((pre:any)=>({...pre,certification_name:dfd_certifyjson_v1Props.data[0]?.certification_name}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={certification02740_10?.certification_name||""}
