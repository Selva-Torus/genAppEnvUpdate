...
const TextInputskill_category = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:skillJson:AFVK:v1|f2a01cd22fda4fcdae3e61b66777a17d|properties.skills_json.items.properties.skill_category"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|7cba9e840637995c1112c096726d0dba|929f902a655b54e0a3946f3c8497ff29"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'skill_category',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,skill_category:undefined}}));
    if(dynamicStateandType.type=="number"){
    setskilld0dba_13((prev: any) => ({ ...prev, skill_category: +e.target.value }));
    }
    else{
    setskilld0dba_13((prev: any) => ({ ...prev, skill_category: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "929f902a655b54e0a3946f3c8497ff29"
      );
      ...
      //     controlId: "929f902a655b54e0a3946f3c8497ff29",
      //     isTable: false,
      //     from:"TextInputskill_category",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'skill_category', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'skill_category',type:'text'};
      //   type={
      //     name:'skill_category',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.skill_category.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.skill_category.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.skill_category.type
      //   }
      ...
      //   type={
      //     name:'skill_category',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.skill_category.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.skill_category.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.skill_category.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "929f902a655b54e0a3946f3c8497ff29") {
        handleChange({target:{value:skilld0dba_13Ref?.current?.skill_category||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "929f902a655b54e0a3946f3c8497ff29") {
        handleBlur({target:{value:skilld0dba_13Ref?.current?.skill_category||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskilld0dba_13((pre:any)=>({...pre,skill_category:dfd_skilljson_v1Props.data[0]?.skill_category}));
    }
  }
  },[dfd_skilljson_v1Props?.setSearchFilters])
  if (skill_category7ff29?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skilld0dba_13?.skill_category||""}
         disabled= {skill_category7ff29?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.skill_category ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputskill_category
