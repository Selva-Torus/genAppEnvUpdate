...
const TextInputyears_of_experience = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setskilld0dba_13((prev: any) => ({ ...prev, years_of_experience: +e.target.value }));
    }
    else{
    setskilld0dba_13((prev: any) => ({ ...prev, years_of_experience: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "f8766ff22fac8201778f4a086a35d72c") {
        handleChange({target:{value:skilld0dba_13Ref?.current?.years_of_experience||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f8766ff22fac8201778f4a086a35d72c") {
        handleBlur({target:{value:skilld0dba_13Ref?.current?.years_of_experience||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskilld0dba_13((pre:any)=>({...pre,years_of_experience:dfd_skilljson_v1Props.data[0]?.years_of_experience}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skilld0dba_13?.years_of_experience||""}
