...
const TextInputfamily_member_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:familyJson:AFVK:v1|c4aa677c03bb47c88fbb0a7f37a48045|properties.family_details_json.items.properties.member_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|951803869bda3c5307631ba51c6b4eb8|6b1ca5e4681a9f27c73a98402dfdb442"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'member_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,member_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, member_name: +e.target.value }));
    }
    else{
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, member_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6b1ca5e4681a9f27c73a98402dfdb442"
      );
      ...
      //     controlId: "6b1ca5e4681a9f27c73a98402dfdb442",
      //     isTable: false,
      //     from:"TextInputfamily_member_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'member_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'member_name',type:'text'};
      //   type={
      //     name:'member_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.member_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.member_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.member_name.type
      //   }
      ...
      //   type={
      //     name:'member_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.member_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.member_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.member_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6b1ca5e4681a9f27c73a98402dfdb442") {
        handleChange({target:{value:famly_detailsb4eb8_10Ref?.current?.member_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6b1ca5e4681a9f27c73a98402dfdb442") {
        handleBlur({target:{value:famly_detailsb4eb8_10Ref?.current?.member_name||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb4eb8_10((pre:any)=>({...pre,member_name:dfd_familyjson_v1Props.data[0]?.member_name}));
    }
  }
  },[dfd_familyjson_v1Props?.setSearchFilters])
  if (family_member_namedb442?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb4eb8_10?.member_name||""}
         disabled= {family_member_namedb442?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='Albert'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Family Member Name"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.member_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputfamily_member_name
