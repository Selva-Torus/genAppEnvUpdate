...
const TextInputrelationship = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|951803869bda3c5307631ba51c6b4eb8|5c5e9f76fdcf1649047a127b47701fb3"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,relationship:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, relationship: +e.target.value }));
    }
    else{
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, relationship: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5c5e9f76fdcf1649047a127b47701fb3"
      );
      ...
      //     controlId: "5c5e9f76fdcf1649047a127b47701fb3",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5c5e9f76fdcf1649047a127b47701fb3") {
        handleChange({target:{value:famly_detailsb4eb8_10Ref?.current?.relationship||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5c5e9f76fdcf1649047a127b47701fb3") {
        handleBlur({target:{value:famly_detailsb4eb8_10Ref?.current?.relationship||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb4eb8_10((pre:any)=>({...pre,relationship:dfd_familyjson_v1Props.data[0]?.relationship}));
    }
  }
  },[dfd_familyjson_v1Props?.setSearchFilters])
  if (relationship01fb3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb4eb8_10?.relationship||""}
         disabled= {relationship01fb3?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='father'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.relationship ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
