...
const TextInputoccupation = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:familyJson:AFVK:v1|c4aa677c03bb47c88fbb0a7f37a48045|properties.family_details_json.items.properties.occupation"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|951803869bda3c5307631ba51c6b4eb8|f17b8c2ca631bcc1bd734213e28c7375"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'occupation',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,occupation:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, occupation: +e.target.value }));
    }
    else{
    setfamly_detailsb4eb8_10((prev: any) => ({ ...prev, occupation: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f17b8c2ca631bcc1bd734213e28c7375"
      );
      ...
      //     controlId: "f17b8c2ca631bcc1bd734213e28c7375",
      //     isTable: false,
      //     from:"TextInputoccupation",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'occupation', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'occupation',type:'text'};
      //   type={
      //     name:'occupation',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.occupation.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.occupation.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.occupation.type
      //   }
      ...
      //   type={
      //     name:'occupation',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.occupation.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.occupation.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.occupation.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const famly_detailsb4eb8_10Ref = useRef<any>(famly_detailsb4eb8_10);
  useEffect(() => { famly_detailsb4eb8_10Ref.current = famly_detailsb4eb8_10; }, [famly_detailsb4eb8_10]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "f17b8c2ca631bcc1bd734213e28c7375") {
        handleChange({target:{value:famly_detailsb4eb8_10Ref?.current?.occupation||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f17b8c2ca631bcc1bd734213e28c7375") {
        handleBlur({target:{value:famly_detailsb4eb8_10Ref?.current?.occupation||""}});
      }
    };
  ...
  if(dfd_familyjson_v1Props?.setSearchFilters && dfd_familyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_familyjson_v1Props.data) && dfd_familyjson_v1Props.data.length > 0){
      setfamly_detailsb4eb8_10((pre:any)=>({...pre,occupation:dfd_familyjson_v1Props.data[0]?.occupation}));
    }
  }
  },[dfd_familyjson_v1Props?.setSearchFilters])
  if (occupationc7375?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={famly_detailsb4eb8_10?.occupation||""}
         disabled= {occupationc7375?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.occupation ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputoccupation
