...
const TextInputproficiency_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:skillJson:AFVK:v1|f2a01cd22fda4fcdae3e61b66777a17d|properties.skills_json.items.properties.proficiency_level"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|7cba9e840637995c1112c096726d0dba|ff12ed1a5b7bc48705ce03ac070f783d"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'proficiency_level',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,proficiency_level:undefined}}));
    if(dynamicStateandType.type=="number"){
    setskilld0dba_12((prev: any) => ({ ...prev, proficiency_level: +e.target.value }));
    }
    else{
    setskilld0dba_12((prev: any) => ({ ...prev, proficiency_level: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ff12ed1a5b7bc48705ce03ac070f783d"
      );
      ...
      //     controlId: "ff12ed1a5b7bc48705ce03ac070f783d",
      //     isTable: false,
      //     from:"TextInputproficiency_level",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'proficiency_level', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'proficiency_level',type:'text'};
      //   type={
      //     name:'proficiency_level',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.proficiency_level.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.proficiency_level.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.proficiency_level.type
      //   }
      ...
      //   type={
      //     name:'proficiency_level',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.proficiency_level.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.proficiency_level.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.proficiency_level.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const skilld0dba_12Ref = useRef<any>(skilld0dba_12);
  useEffect(() => { skilld0dba_12Ref.current = skilld0dba_12; }, [skilld0dba_12]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "ff12ed1a5b7bc48705ce03ac070f783d") {
        handleChange({target:{value:skilld0dba_12Ref?.current?.proficiency_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ff12ed1a5b7bc48705ce03ac070f783d") {
        handleBlur({target:{value:skilld0dba_12Ref?.current?.proficiency_level||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskilld0dba_12((pre:any)=>({...pre,proficiency_level:dfd_skilljson_v1Props.data[0]?.proficiency_level}));
    }
  }
  },[dfd_skilljson_v1Props?.setSearchFilters])
  if (proficiency_levelf783d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skilld0dba_12?.proficiency_level||""}
         disabled= {proficiency_levelf783d?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='Expert'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Proficiency Level"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.proficiency_level ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputproficiency_level
