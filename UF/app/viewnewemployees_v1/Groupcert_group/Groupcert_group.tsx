...
import Groupcertification  from "../Groupcertification/Groupcertification";
...
import Iconcertification_icon  from "./Iconcertification_icon";
import Textcert_text  from "./Textcert_text";
import Textcert_des  from "./Textcert_des";
...
const Groupcert_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {certification_icon1c1d6, setcertification_icon1c1d6}= useContext(TotalContext) as TotalContextProps;
  const {cert_text8e531, setcert_text8e531}= useContext(TotalContext) as TotalContextProps;
  const {cert_desa6515, setcert_desa6515}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ebe8120308c866f4de925d453c23be86");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcert_group3be86Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("certification_icon")){
        setcertification_icon1c1d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(certification_icon1c1d6?.isDisabled==null)
      {
        setcertification_icon1c1d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_text")){
        setcert_text8e531((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_text8e531?.isDisabled==null)
      {
        setcert_text8e531((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_des")){
        setcert_desa6515((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_desa6515?.isDisabled==null)
      {
        setcert_desa6515((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("certification")){
        setcertification02740((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(certification02740?.isDisabled==null)
      {
        setcertification02740((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['certification_icon'] = certification_icon1c1d6,
        codeStates['setcertification_icon'] = setcertification_icon1c1d6,
        codeStates['cert_text'] = cert_text8e531,
        codeStates['setcert_text'] = setcert_text8e531,
        codeStates['cert_des'] = cert_desa6515,
        codeStates['setcert_des'] = setcert_desa6515,
  ...
  const cert_group3be86Ref = useRef<any>(null);
  const handleClearSearch = () => {
    cert_group3be86Ref.current?.setSearchParams();
    cert_group3be86Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cert_group3be86) && Object.keys(cert_group3be86)?.length>0)
      {
        setcert_group3be86({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cert_group3be86Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"cert_group"}))
        }}
    >
        {allowedComponent.includes("certification")  &&<Groupcertification  
        ...
        {allowedControls.includes("certification_icon")?<Iconcertification_icon /* 1c1d6 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("cert_text") ?<Textcert_text   /* 8e531 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("cert_des") ?<Textcert_des   /* a6515 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupcert_group
