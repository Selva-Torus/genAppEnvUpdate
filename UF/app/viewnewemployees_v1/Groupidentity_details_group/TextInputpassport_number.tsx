...
const TextInputpassport_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.passport_number"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c7be01d5914725bd0a1b63e5b4120918|b22b5203251665a2f670c39859cabca3"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'passport_number',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,passport_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setidentity_details_group20918((prev: any) => ({ ...prev, passport_number: +e.target.value }));
    }
    else{
    setidentity_details_group20918((prev: any) => ({ ...prev, passport_number: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "c7be01d5914725bd0a1b63e5b4120918",
        "b22b5203251665a2f670c39859cabca3"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "c7be01d5914725bd0a1b63e5b4120918",
      //     controlId: "b22b5203251665a2f670c39859cabca3",
      //     isTable: false,
      //     from:"TextInputpassport_number",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'passport_number', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'passport_number',type:'text'};
      //   type={
      //     name:'passport_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.passport_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.passport_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.passport_number.type
      //   }
      ...
      //   type={
      //     name:'passport_number',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.passport_number.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.passport_number.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.passport_number.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const identity_details_group20918Ref = useRef<any>(identity_details_group20918);
  useEffect(() => { identity_details_group20918Ref.current = identity_details_group20918; }, [identity_details_group20918]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "b22b5203251665a2f670c39859cabca3") {
        handleChange({target:{value:identity_details_group20918Ref?.current?.passport_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b22b5203251665a2f670c39859cabca3") {
        handleBlur({target:{value:identity_details_group20918Ref?.current?.passport_number||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setidentity_details_group20918((pre:any)=>({...pre,passport_number:dfd_employees_v1Props.data[0]?.passport_number}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (passport_numberabca3?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={identity_details_group20918?.passport_number||""}
         disabled= {passport_numberabca3?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.passport_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpassport_number
