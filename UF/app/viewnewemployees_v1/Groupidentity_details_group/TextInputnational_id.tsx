...
const TextInputnational_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.national_id"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c7be01d5914725bd0a1b63e5b4120918|b1b6e998ee1edde22ae726d2530d5e7a"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'national_id',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,national_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setidentity_details_group20918((prev: any) => ({ ...prev, national_id: +e.target.value }));
    }
    else{
    setidentity_details_group20918((prev: any) => ({ ...prev, national_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b1b6e998ee1edde22ae726d2530d5e7a"
      );
      ...
      //     controlId: "b1b6e998ee1edde22ae726d2530d5e7a",
      //     isTable: false,
      //     from:"TextInputnational_id",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'national_id', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'national_id',type:'text'};
      //   type={
      //     name:'national_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.national_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.national_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.national_id.type
      //   }
      ...
      //   type={
      //     name:'national_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.national_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.national_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.national_id.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b1b6e998ee1edde22ae726d2530d5e7a") {
        handleChange({target:{value:identity_details_group20918Ref?.current?.national_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b1b6e998ee1edde22ae726d2530d5e7a") {
        handleBlur({target:{value:identity_details_group20918Ref?.current?.national_id||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setidentity_details_group20918((pre:any)=>({...pre,national_id:dfd_employees_v1Props.data[0]?.national_id}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (national_idd5e7a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 25`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={identity_details_group20918?.national_id||""}
         disabled= {national_idd5e7a?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.national_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputnational_id
