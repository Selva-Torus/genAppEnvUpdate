...
const TextInputbiometric_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.biometric_id"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c7be01d5914725bd0a1b63e5b4120918|de205e187c43e276204b2f411496d969"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'biometric_id',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,biometric_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setidentity_details_group20918((prev: any) => ({ ...prev, biometric_id: +e.target.value }));
    }
    else{
    setidentity_details_group20918((prev: any) => ({ ...prev, biometric_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "de205e187c43e276204b2f411496d969"
      );
      ...
      //     controlId: "de205e187c43e276204b2f411496d969",
      //     isTable: false,
      //     from:"TextInputbiometric_id",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'biometric_id', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'biometric_id',type:'text'};
      //   type={
      //     name:'biometric_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.biometric_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.biometric_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.biometric_id.type
      //   }
      ...
      //   type={
      //     name:'biometric_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.biometric_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.biometric_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.biometric_id.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "de205e187c43e276204b2f411496d969") {
        handleChange({target:{value:identity_details_group20918Ref?.current?.biometric_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "de205e187c43e276204b2f411496d969") {
        handleBlur({target:{value:identity_details_group20918Ref?.current?.biometric_id||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setidentity_details_group20918((pre:any)=>({...pre,biometric_id:dfd_employees_v1Props.data[0]?.biometric_id}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (biometric_id6d969?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={identity_details_group20918?.biometric_id||""}
         disabled= {biometric_id6d969?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. BIO12345'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Biometric ID"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.biometric_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputbiometric_id
