...
import Textidentity_details  from "./Textidentity_details";
import TextInputnational_id  from "./TextInputnational_id";
import TextInputpassport_number  from "./TextInputpassport_number";
import TextInputbiometric_id  from "./TextInputbiometric_id";
...
const Groupidentity_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {identity_details4e5e6, setidentity_details4e5e6}= useContext(TotalContext) as TotalContextProps;
  const {national_idd5e7a, setnational_idd5e7a}= useContext(TotalContext) as TotalContextProps;
  const {passport_numberabca3, setpassport_numberabca3}= useContext(TotalContext) as TotalContextProps;
  const {biometric_id6d969, setbiometric_id6d969}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c7be01d5914725bd0a1b63e5b4120918");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setidentity_details_group20918Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("identity_details")){
        setidentity_details4e5e6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(identity_details4e5e6?.isDisabled==null)
      {
        setidentity_details4e5e6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("national_id")){
        setnational_idd5e7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(national_idd5e7a?.isDisabled==null)
      {
        setnational_idd5e7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("passport_number")){
        setpassport_numberabca3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(passport_numberabca3?.isDisabled==null)
      {
        setpassport_numberabca3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("biometric_id")){
        setbiometric_id6d969((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(biometric_id6d969?.isDisabled==null)
      {
        setbiometric_id6d969((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['identity_details'] = identity_details4e5e6,
        codeStates['setidentity_details'] = setidentity_details4e5e6,
        codeStates['national_id'] = national_idd5e7a,
        codeStates['setnational_id'] = setnational_idd5e7a,
        codeStates['passport_number'] = passport_numberabca3,
        codeStates['setpassport_number'] = setpassport_numberabca3,
        codeStates['biometric_id'] = biometric_id6d969,
        codeStates['setbiometric_id'] = setbiometric_id6d969,
  ...
  const identity_details_group20918Ref = useRef<any>(null);
  const handleClearSearch = () => {
    identity_details_group20918Ref.current?.setSearchParams();
    identity_details_group20918Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(identity_details_group20918) && Object.keys(identity_details_group20918)?.length>0)
      {
        setidentity_details_group20918({})
      }
    }else 
      prevRefreshRef.current= true
  }, [identity_details_group20918Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"identity_details_group"}))
        }}
    >
          {allowedControls.includes("identity_details") ?<Textidentity_details   /* 4e5e6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("national_id") ?<TextInputnational_id   /* d5e7a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("passport_number") ?<TextInputpassport_number   /* abca3 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("biometric_id") ?<TextInputbiometric_id   /* 6d969 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupidentity_details_group
