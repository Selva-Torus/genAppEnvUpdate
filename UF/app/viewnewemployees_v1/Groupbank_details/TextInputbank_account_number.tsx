...
const TextInputbank_account_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c6c4a90399cc465584f67c08d3ef21b7|703df73abfbf66c78aae53e692dcdb89"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,bank_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_account: +e.target.value }));
    }
    else{
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_account: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "703df73abfbf66c78aae53e692dcdb89"
      );
      ...
      //     controlId: "703df73abfbf66c78aae53e692dcdb89",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "703df73abfbf66c78aae53e692dcdb89") {
        handleChange({target:{value:bank_detailsf21b7Ref?.current?.bank_account||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "703df73abfbf66c78aae53e692dcdb89") {
        handleBlur({target:{value:bank_detailsf21b7Ref?.current?.bank_account||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbank_detailsf21b7((pre:any)=>({...pre,bank_account:dfd_employees_v1Props.data[0]?.bank_account}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (bank_account_numbercdb89?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={bank_detailsf21b7?.bank_account||""}
         disabled= {bank_account_numbercdb89?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.bank_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
