...
const TextInputifsc_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.bank_ifsc"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c6c4a90399cc465584f67c08d3ef21b7|6220e0e801b5ef87cc442cd4efa249a2"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'bank_ifsc',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,bank_ifsc:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_ifsc: +e.target.value }));
    }
    else{
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_ifsc: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "c6c4a90399cc465584f67c08d3ef21b7",
        "6220e0e801b5ef87cc442cd4efa249a2"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "c6c4a90399cc465584f67c08d3ef21b7",
      //     controlId: "6220e0e801b5ef87cc442cd4efa249a2",
      //     isTable: false,
      //     from:"TextInputifsc_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'bank_ifsc', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'bank_ifsc',type:'text'};
      //   type={
      //     name:'bank_ifsc',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.bank_ifsc.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.bank_ifsc.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.bank_ifsc.type
      //   }
      ...
      //   type={
      //     name:'bank_ifsc',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.bank_ifsc.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.bank_ifsc.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.bank_ifsc.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const bank_detailsf21b7Ref = useRef<any>(bank_detailsf21b7);
  useEffect(() => { bank_detailsf21b7Ref.current = bank_detailsf21b7; }, [bank_detailsf21b7]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "6220e0e801b5ef87cc442cd4efa249a2") {
        handleChange({target:{value:bank_detailsf21b7Ref?.current?.bank_ifsc||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6220e0e801b5ef87cc442cd4efa249a2") {
        handleBlur({target:{value:bank_detailsf21b7Ref?.current?.bank_ifsc||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbank_detailsf21b7((pre:any)=>({...pre,bank_ifsc:dfd_employees_v1Props.data[0]?.bank_ifsc}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (ifsc_code249a2?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={bank_detailsf21b7?.bank_ifsc||""}
         disabled= {ifsc_code249a2?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.bank_ifsc ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputifsc_code
