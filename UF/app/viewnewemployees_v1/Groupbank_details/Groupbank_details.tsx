...
import Textbank_details  from "./Textbank_details";
import TextInputbank_name  from "./TextInputbank_name";
import TextInputbank_account_number  from "./TextInputbank_account_number";
import TextInputifsc_code  from "./TextInputifsc_code";
...
const Groupbank_details = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {bank_details91264, setbank_details91264}= useContext(TotalContext) as TotalContextProps;
  const {bank_name78ddc, setbank_name78ddc}= useContext(TotalContext) as TotalContextProps;
  const {bank_account_numbercdb89, setbank_account_numbercdb89}= useContext(TotalContext) as TotalContextProps;
  const {ifsc_code249a2, setifsc_code249a2}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c6c4a90399cc465584f67c08d3ef21b7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbank_detailsf21b7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("bank_details")){
        setbank_details91264((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_details91264?.isDisabled==null)
      {
        setbank_details91264((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bank_name")){
        setbank_name78ddc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_name78ddc?.isDisabled==null)
      {
        setbank_name78ddc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bank_account_number")){
        setbank_account_numbercdb89((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bank_account_numbercdb89?.isDisabled==null)
      {
        setbank_account_numbercdb89((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ifsc_code")){
        setifsc_code249a2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ifsc_code249a2?.isDisabled==null)
      {
        setifsc_code249a2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['bank_details'] = bank_details91264,
        codeStates['setbank_details'] = setbank_details91264,
        codeStates['bank_name'] = bank_name78ddc,
        codeStates['setbank_name'] = setbank_name78ddc,
        codeStates['bank_account_number'] = bank_account_numbercdb89,
        codeStates['setbank_account_number'] = setbank_account_numbercdb89,
        codeStates['ifsc_code'] = ifsc_code249a2,
        codeStates['setifsc_code'] = setifsc_code249a2,
  ...
  const bank_detailsf21b7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    bank_detailsf21b7Ref.current?.setSearchParams();
    bank_detailsf21b7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(bank_detailsf21b7) && Object.keys(bank_detailsf21b7)?.length>0)
      {
        setbank_detailsf21b7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [bank_detailsf21b7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"bank_details"}))
        }}
    >
          {allowedControls.includes("bank_details") ?<Textbank_details   /* 91264 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("bank_name") ?<TextInputbank_name   /* 78ddc */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("bank_account_number") ?<TextInputbank_account_number   /* cdb89 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ifsc_code") ?<TextInputifsc_code   /* 249a2 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupbank_details
