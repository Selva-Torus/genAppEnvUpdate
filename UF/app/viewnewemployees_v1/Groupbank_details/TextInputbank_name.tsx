...
const TextInputbank_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employees:AFVK:v1|73c2a20a785b4361a729fb7c45a12258|properties.bank_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|c6c4a90399cc465584f67c08d3ef21b7|36653001a2dcb118c16fdc9029e78ddc"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,bank_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_name: +e.target.value }));
    }
    else{
    setbank_detailsf21b7((prev: any) => ({ ...prev, bank_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "36653001a2dcb118c16fdc9029e78ddc"
      );
      ...
      //     controlId: "36653001a2dcb118c16fdc9029e78ddc",
      //     isTable: false,
      //     from:"TextInputbank_name",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "36653001a2dcb118c16fdc9029e78ddc") {
        handleChange({target:{value:bank_detailsf21b7Ref?.current?.bank_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "36653001a2dcb118c16fdc9029e78ddc") {
        handleBlur({target:{value:bank_detailsf21b7Ref?.current?.bank_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setbank_detailsf21b7((pre:any)=>({...pre,bank_name:dfd_employees_v1Props.data[0]?.bank_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (bank_name78ddc?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={bank_detailsf21b7?.bank_name||""}
         disabled= {bank_name78ddc?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.bank_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputbank_name
