...
const TextInputcertification_provider = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:certifyJson:AFVK:v1|dcf2ab0ce86249d59782805341cb8b09|properties.certifications_json.items.properties.certification_provider"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|4b33772a57a21ae60ade6f0e5dd02740|dba4039d951e10692749b1a3beb427b4"
    }
  ],
  ...
  "schemaData": {
    "type": "string",
    "x-pg-type": "text",
    "x-expression": "c ->> 'certification_provider'"
  }
}
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'certification_provider',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,certification_provider:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcertification02740_14((prev: any) => ({ ...prev, certification_provider: +e.target.value }));
    }
    else{
    setcertification02740_14((prev: any) => ({ ...prev, certification_provider: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "dba4039d951e10692749b1a3beb427b4"
      );
      ...
      //     controlId: "dba4039d951e10692749b1a3beb427b4",
      //     isTable: false,
      //     from:"TextInputcertification_provider",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'certification_provider', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'certification_provider',type:'text'};
      //   type={
      //     name:'certification_provider',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.certification_provider.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.certification_provider.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.certification_provider.type
      //   }
      ...
      //   type={
      //     name:'certification_provider',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.certification_provider.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.certification_provider.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.certification_provider.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const certification02740_14Ref = useRef<any>(certification02740_14);
  useEffect(() => { certification02740_14Ref.current = certification02740_14; }, [certification02740_14]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "dba4039d951e10692749b1a3beb427b4") {
        handleChange({target:{value:certification02740_14Ref?.current?.certification_provider||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "dba4039d951e10692749b1a3beb427b4") {
        handleBlur({target:{value:certification02740_14Ref?.current?.certification_provider||""}});
      }
    };
  ...
  if(dfd_certifyjson_v1Props?.setSearchFilters && dfd_certifyjson_v1Props?.data)
  {
    if(Array.isArray(dfd_certifyjson_v1Props.data) && dfd_certifyjson_v1Props.data.length > 0){
      setcertification02740_14((pre:any)=>({...pre,certification_provider:dfd_certifyjson_v1Props.data[0]?.certification_provider}));
    }
  }
  },[dfd_certifyjson_v1Props?.setSearchFilters])
  if (certification_provider427b4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `1 / 15`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={certification02740_14?.certification_provider||""}
         disabled= {certification_provider427b4?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='AWS'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Certification Provider"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.certification_provider ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcertification_provider
