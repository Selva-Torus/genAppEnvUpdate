...
const TextInputskill_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setskilld0dba_8((prev: any) => ({ ...prev, skill_name: +e.target.value }));
    }
    else{
    setskilld0dba_8((prev: any) => ({ ...prev, skill_name: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "4c4ee1766e778a26e96b2930739649e5") {
        handleChange({target:{value:skilld0dba_8Ref?.current?.skill_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4c4ee1766e778a26e96b2930739649e5") {
        handleBlur({target:{value:skilld0dba_8Ref?.current?.skill_name||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskilld0dba_8((pre:any)=>({...pre,skill_name:dfd_skilljson_v1Props.data[0]?.skill_name}));
    }
  }
