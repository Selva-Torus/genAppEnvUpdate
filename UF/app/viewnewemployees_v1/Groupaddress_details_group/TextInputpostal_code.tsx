...
const TextInputpostal_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|f05017610109e63bcf3eb18c12175e08|da374939eba21efd8518e5c7043a6518"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,postal_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddress_details_group75e08((prev: any) => ({ ...prev, postal_code: +e.target.value }));
    }
    else{
    setaddress_details_group75e08((prev: any) => ({ ...prev, postal_code: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "da374939eba21efd8518e5c7043a6518"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "f05017610109e63bcf3eb18c12175e08",
      //     controlId: "da374939eba21efd8518e5c7043a6518",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "da374939eba21efd8518e5c7043a6518") {
        handleChange({target:{value:address_details_group75e08Ref?.current?.postal_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "da374939eba21efd8518e5c7043a6518") {
        handleBlur({target:{value:address_details_group75e08Ref?.current?.postal_code||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setaddress_details_group75e08((pre:any)=>({...pre,postal_code:dfd_employees_v1Props.data[0]?.postal_code}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (postal_codea6518?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={address_details_group75e08?.postal_code||""}
         disabled= {postal_codea6518?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.postal_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
