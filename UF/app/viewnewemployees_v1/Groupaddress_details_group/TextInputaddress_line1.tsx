...
const TextInputaddress_line1 = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|f05017610109e63bcf3eb18c12175e08|9260ef442fa4e4c3de20a9127ab1ef14"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,address_line1:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddress_details_group75e08((prev: any) => ({ ...prev, address_line1: +e.target.value }));
    }
    else{
    setaddress_details_group75e08((prev: any) => ({ ...prev, address_line1: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9260ef442fa4e4c3de20a9127ab1ef14"
      );
      ...
      //     controlId: "9260ef442fa4e4c3de20a9127ab1ef14",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9260ef442fa4e4c3de20a9127ab1ef14") {
        handleChange({target:{value:address_details_group75e08Ref?.current?.address_line1||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9260ef442fa4e4c3de20a9127ab1ef14") {
        handleBlur({target:{value:address_details_group75e08Ref?.current?.address_line1||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setaddress_details_group75e08((pre:any)=>({...pre,address_line1:dfd_employees_v1Props.data[0]?.address_line1}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (address_line11ef14?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={address_details_group75e08?.address_line1||""}
         disabled= {address_line11ef14?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.address_line1 ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
