 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      controlData,
      "f05017610109e63bcf3eb18c12175e08",
      "fba74ea28f8d406de4d2fa1feb7c3b74"
    );
  ...
  const handlechange = async(value: any) => {
    ...
    if(value.length>0){
      ...
      setaddress_details_group75e08((prev: any) => ({ ...prev,statec3b74: value }))
      setIsRequredData(false)
    }else{
      let temp:any = address_details_group75e08;
      delete temp.state;
      delete temp.STATE;
      delete temp.statec3b74;
      setaddress_details_group75e08(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,state:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(address_details_group75e08.state){
      if(Array.isArray(dfd_statecombo_v1Props)){
        if(dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_group75e08.state)){
          setdropdownValue([dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_group75e08.state)?.state_name])
          tempValue=dfd_statecombo_v1Props?.find((item: any) => item.state_name === address_details_group75e08.state)?.state_name
        }else{
          setdropdownValue([address_details_group75e08.state])
          tempValue=address_details_group75e08.state
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{state_name:address_details_group75e08.state}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_group75e08.state)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_group75e08.state)?.state_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.state_name === address_details_group75e08.state)?.state_name
      }else{
        setdropdownValue([address_details_group75e08.state])
        tempValue=address_details_group75e08.state
      }   
    ...
    if(statec3b74?.trigger == true)
    {
      await handlechange(tempValue)
      setstatec3b74((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[address_details_group75e08.state, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_statecombo_v1Props) && dfd_statecombo_v1Props?.length == 1){
    // setaddress_details_group75e08((pre:any)=>({...pre,state:dfd_statecombo_v1Props[0]?.state}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaddress_details_group75e08((prev: any) => ({ ...prev, state: getMapperDetailsBindValues[value],STATE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaddress_details_group75e08((prev: any) => ({ ...prev, state: '', statec3b74: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,state:undefined}}));
  ...
  const address_details_group75e08Ref = useRef<any>(address_details_group75e08);
  useEffect(() => { address_details_group75e08Ref.current = address_details_group75e08; }, [address_details_group75e08]);
    useEffect(()=>{
        handleBlur()
       const handler = (id:any) => {
          if (id === "fba74ea28f8d406de4d2fa1feb7c3b74") {
        handleClick(address_details_group75e08Ref?.current?.statec3b74?address_details_group75e08Ref?.current?.statec3b74:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "fba74ea28f8d406de4d2fa1feb7c3b74");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaddress_details_group75e08((pre:any)=>({...pre,state:""}))
    else
      setInitialCount(1)
  },[statec3b74?.refresh])
  

  if (statec3b74?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {statec3b74?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={address_details_group75e08?.statec3b74 ? [address_details_group75e08?.statec3b74] : (address_details_group75e08?.state ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.state ? "invalid" : undefined}
        />
    </div>
  );
};
