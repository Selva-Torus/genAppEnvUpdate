...
const TextInputwork_email = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|4b650f7d35bd022d03a10ffade93ff3d|b4827514287887c642884558ac8c12b1"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'work_email',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,work_email:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_group3ff3d((prev: any) => ({ ...prev, work_email: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_group3ff3d?.work_email == "" || contact_details_group3ff3d?.work_email == undefined){
      contact_details_group3ff3d.work_email = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_group3ff3d?.work_email);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,work_email:"invalid"}}));
        }
    }else if(contact_details_group3ff3d?.work_email !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_group3ff3d?.work_email);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b4827514287887c642884558ac8c12b1"
      );
      ...
      //     controlId: "b4827514287887c642884558ac8c12b1",
      //     isTable: false,
      //     from:"TextInputwork_email",
      ...
      //   type={
      //     name:'work_email',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.work_email.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.work_email.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.work_email.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_group3ff3d?.work_email)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "b4827514287887c642884558ac8c12b1") {
        handleChange({target:{value:contact_details_group3ff3dRef?.current?.work_email||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b4827514287887c642884558ac8c12b1") {
        handleBlur({target:{value:contact_details_group3ff3dRef?.current?.work_email||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_group3ff3d((pre:any)=>({...pre,work_email:dfd_employees_v1Props.data[0]?.work_email}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (work_emailc12b1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_group3ff3d?.work_email||""}
         disabled= {work_emailc12b1?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='e.g. johndoe@gsstvl.com'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Work Email"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.work_email ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputwork_email
