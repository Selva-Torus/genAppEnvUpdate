...
const Groupcontact_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "4b650f7d35bd022d03a10ffade93ff3d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcontact_details_group3ff3dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("contact_details")){
        setcontact_detailsf9e7f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(contact_detailsf9e7f?.isDisabled==null)
      {
        setcontact_detailsf9e7f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("personal_email")){
        setpersonal_email720ff((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(personal_email720ff?.isDisabled==null)
      {
        setpersonal_email720ff((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("work_email")){
        setwork_emailc12b1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(work_emailc12b1?.isDisabled==null)
      {
        setwork_emailc12b1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("alternate_mobile_number")){
        setalternate_mobile_number4806d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(alternate_mobile_number4806d?.isDisabled==null)
      {
        setalternate_mobile_number4806d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("mobile_number")){
        setmobile_number8bfe0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(mobile_number8bfe0?.isDisabled==null)
      {
        setmobile_number8bfe0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("linkedin_profile")){
        setlinkedin_profiledece8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(linkedin_profiledece8?.isDisabled==null)
      {
        setlinkedin_profiledece8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const contact_details_group3ff3dRef = useRef<any>(null);
  const handleClearSearch = () => {
    contact_details_group3ff3dRef.current?.setSearchParams();
    contact_details_group3ff3dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(contact_details_group3ff3d) && Object.keys(contact_details_group3ff3d)?.length>0)
      {
        setcontact_details_group3ff3d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [contact_details_group3ff3dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"contact_details_group"}))
        }}
    >
          {allowedControls.includes("contact_details") ?<Textcontact_details   /* f9e7f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("personal_email") ?<TextInputpersonal_email   /* 720ff */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("work_email") ?<TextInputwork_email   /* c12b1 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("alternate_mobile_number") ?<TextInputalternate_mobile_number   /* 4806d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("mobile_number") ?<TextInputmobile_number   /* 8bfe0 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("linkedin_profile") ?<TextInputlinkedin_profile   /* dece8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
