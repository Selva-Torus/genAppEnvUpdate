...
const TextInputpersonal_email = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|4b650f7d35bd022d03a10ffade93ff3d|9fe9bcb9bf86ce88b396fc3a847720ff"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,personal_email:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_group3ff3d((prev: any) => ({ ...prev, personal_email: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_group3ff3d?.personal_email == "" || contact_details_group3ff3d?.personal_email == undefined){
      contact_details_group3ff3d.personal_email = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_group3ff3d?.personal_email);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,personal_email:"invalid"}}));
        }
    }else if(contact_details_group3ff3d?.personal_email !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_group3ff3d?.personal_email);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9fe9bcb9bf86ce88b396fc3a847720ff"
      );
      ...
      //     controlId: "9fe9bcb9bf86ce88b396fc3a847720ff",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_group3ff3d?.personal_email)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "9fe9bcb9bf86ce88b396fc3a847720ff") {
        handleChange({target:{value:contact_details_group3ff3dRef?.current?.personal_email||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9fe9bcb9bf86ce88b396fc3a847720ff") {
        handleBlur({target:{value:contact_details_group3ff3dRef?.current?.personal_email||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_group3ff3d((pre:any)=>({...pre,personal_email:dfd_employees_v1Props.data[0]?.personal_email}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (personal_email720ff?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_group3ff3d?.personal_email||""}
         disabled= {personal_email720ff?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.personal_email ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
