...
const TextInputmobile_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|4b650f7d35bd022d03a10ffade93ff3d|06ae0148a58bc5fb38160a39da78bfe0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,mobile:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcontact_details_group3ff3d((prev: any) => ({ ...prev, mobile: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(contact_details_group3ff3d?.mobile == "" || contact_details_group3ff3d?.mobile == undefined){
      contact_details_group3ff3d.mobile = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, contact_details_group3ff3d?.mobile);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,mobile:"invalid"}}));
        }
    }else if(contact_details_group3ff3d?.mobile !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +contact_details_group3ff3d?.mobile);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "06ae0148a58bc5fb38160a39da78bfe0"
      );
      ...
      //     controlId: "06ae0148a58bc5fb38160a39da78bfe0",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!contact_details_group3ff3d?.mobile)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "06ae0148a58bc5fb38160a39da78bfe0") {
        handleChange({target:{value:contact_details_group3ff3dRef?.current?.mobile||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "06ae0148a58bc5fb38160a39da78bfe0") {
        handleBlur({target:{value:contact_details_group3ff3dRef?.current?.mobile||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setcontact_details_group3ff3d((pre:any)=>({...pre,mobile:dfd_employees_v1Props.data[0]?.mobile}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (mobile_number8bfe0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={contact_details_group3ff3d?.mobile||""}
         disabled= {mobile_number8bfe0?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.mobile ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
