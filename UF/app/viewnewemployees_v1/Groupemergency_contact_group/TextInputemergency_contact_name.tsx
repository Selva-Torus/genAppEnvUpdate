...
const TextInputemergency_contact_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|9db89c055c68bd75e3b4f60e26ed1907|443ac62b843c0a72573bd4def01b6689"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,emergency_contact_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setemergency_contact_groupd1907((prev: any) => ({ ...prev, emergency_contact_name: +e.target.value }));
    }
    else{
    setemergency_contact_groupd1907((prev: any) => ({ ...prev, emergency_contact_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "443ac62b843c0a72573bd4def01b6689"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1",
      //     componentId: "9db89c055c68bd75e3b4f60e26ed1907",
      //     controlId: "443ac62b843c0a72573bd4def01b6689",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "443ac62b843c0a72573bd4def01b6689") {
        handleChange({target:{value:emergency_contact_groupd1907Ref?.current?.emergency_contact_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "443ac62b843c0a72573bd4def01b6689") {
        handleBlur({target:{value:emergency_contact_groupd1907Ref?.current?.emergency_contact_name||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setemergency_contact_groupd1907((pre:any)=>({...pre,emergency_contact_name:dfd_employees_v1Props.data[0]?.emergency_contact_name}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (emergency_contact_nameb6689?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={emergency_contact_groupd1907?.emergency_contact_name||""}
         disabled= {emergency_contact_nameb6689?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.emergency_contact_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
