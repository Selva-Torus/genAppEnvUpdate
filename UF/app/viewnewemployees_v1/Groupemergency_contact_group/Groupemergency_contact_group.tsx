...
const Groupemergency_contact_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9db89c055c68bd75e3b4f60e26ed1907");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemergency_contact_groupd1907Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("emergency_contact")){
        setemergency_contact8156b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact8156b?.isDisabled==null)
      {
        setemergency_contact8156b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emergency_contact_name")){
        setemergency_contact_nameb6689((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact_nameb6689?.isDisabled==null)
      {
        setemergency_contact_nameb6689((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emergency_contact_phone")){
        setemergency_contact_phone6c6a0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emergency_contact_phone6c6a0?.isDisabled==null)
      {
        setemergency_contact_phone6c6a0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("relationship")){
        setrelationship1b848((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(relationship1b848?.isDisabled==null)
      {
        setrelationship1b848((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const emergency_contact_groupd1907Ref = useRef<any>(null);
  const handleClearSearch = () => {
    emergency_contact_groupd1907Ref.current?.setSearchParams();
    emergency_contact_groupd1907Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(emergency_contact_groupd1907) && Object.keys(emergency_contact_groupd1907)?.length>0)
      {
        setemergency_contact_groupd1907({})
      }
    }else 
      prevRefreshRef.current= true
  }, [emergency_contact_groupd1907Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"emergency_contact_group"}))
        }}
    >
          {allowedControls.includes("emergency_contact") ?<Textemergency_contact   /* 8156b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("emergency_contact_name") ?<TextInputemergency_contact_name   /* b6689 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("emergency_contact_phone") ?<TextInputemergency_contact_phone   /* 6c6a0 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("relationship") ?<Dropdownrelationship   /* 1b848 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
