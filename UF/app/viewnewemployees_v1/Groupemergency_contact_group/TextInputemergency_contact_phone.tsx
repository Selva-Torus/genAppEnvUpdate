...
const TextInputemergency_contact_phone = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|9db89c055c68bd75e3b4f60e26ed1907|85dcc5f7c78fc196cbd9d3ca7536c6a0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,emergency_contact_phone:undefined}}));
    if(dynamicStateandType.type=="number"){
    setemergency_contact_groupd1907((prev: any) => ({ ...prev, emergency_contact_phone: +e.target.value }));
    }
    else{
    setemergency_contact_groupd1907((prev: any) => ({ ...prev, emergency_contact_phone: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "85dcc5f7c78fc196cbd9d3ca7536c6a0"
      );
      ...
      //     controlId: "85dcc5f7c78fc196cbd9d3ca7536c6a0",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "85dcc5f7c78fc196cbd9d3ca7536c6a0") {
        handleChange({target:{value:emergency_contact_groupd1907Ref?.current?.emergency_contact_phone||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "85dcc5f7c78fc196cbd9d3ca7536c6a0") {
        handleBlur({target:{value:emergency_contact_groupd1907Ref?.current?.emergency_contact_phone||""}});
      }
    };
  ...
  if(dfd_employees_v1Props?.setSearchFilters && dfd_employees_v1Props?.data)
  {
    if(Array.isArray(dfd_employees_v1Props.data) && dfd_employees_v1Props.data.length > 0){
      setemergency_contact_groupd1907((pre:any)=>({...pre,emergency_contact_phone:dfd_employees_v1Props.data[0]?.emergency_contact_phone}));
    }
  }
  },[dfd_employees_v1Props?.setSearchFilters])
  if (emergency_contact_phone6c6a0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={emergency_contact_groupd1907?.emergency_contact_phone||""}
         disabled= {emergency_contact_phone6c6a0?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.emergency_contact_phone ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
