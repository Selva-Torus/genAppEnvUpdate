 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "b1f78025d665590cca51712af049321e"
    );
  ...
  },[employment_type9321e?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setemployment_details_group2c703((prev: any) => ({ ...prev,employment_type9321e: value }))
      setIsRequredData(false)
    }else{
      let temp:any = employment_details_group2c703;
      delete temp.employment_type;
      delete temp.EMPLOYMENT_TYPE;
      delete temp.employment_type9321e;
      setemployment_details_group2c703(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,employment_type:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(employment_details_group2c703.employment_type){
      if(Array.isArray(dfd_employmenttypecombo_v1Props)){
        if(dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)){
          setdropdownValue([dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)?.employment_type])
          tempValue=dfd_employmenttypecombo_v1Props?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)?.employment_type
        }else{
          setdropdownValue([employment_details_group2c703.employment_type])
          tempValue=employment_details_group2c703.employment_type
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{employment_type:employment_details_group2c703.employment_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)?.employment_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.employment_type === employment_details_group2c703.employment_type)?.employment_type
      }else{
        setdropdownValue([employment_details_group2c703.employment_type])
        tempValue=employment_details_group2c703.employment_type
      }   
    ...
    if(employment_type9321e?.trigger == true)
    {
      await handlechange(tempValue)
      setemployment_type9321e((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[employment_details_group2c703.employment_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employmenttypecombo_v1Props) && dfd_employmenttypecombo_v1Props?.length == 1){
    // setemployment_details_group2c703((pre:any)=>({...pre,employment_type:dfd_employmenttypecombo_v1Props[0]?.employment_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setemployment_details_group2c703((prev: any) => ({ ...prev, employment_type: getMapperDetailsBindValues[value],EMPLOYMENT_TYPE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setemployment_details_group2c703((prev: any) => ({ ...prev, employment_type: '', employment_type9321e: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,employment_type:undefined}}));
  ...
  useEffect(() => { employment_details_group2c703Ref.current = employment_details_group2c703; }, [employment_details_group2c703]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "b1f78025d665590cca51712af049321e") {
        handleClick(employment_details_group2c703Ref?.current?.employment_type9321e?employment_details_group2c703Ref?.current?.employment_type9321e:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "b1f78025d665590cca51712af049321e");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group2c703((pre:any)=>({...pre,employment_type:""}))
    else
      setInitialCount(1)
  },[employment_type9321e?.refresh])
  

  if (employment_type9321e?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {employment_type9321e?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={employment_details_group2c703?.employment_type9321e ? [employment_details_group2c703?.employment_type9321e] : (employment_details_group2c703?.employment_type ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.employment_type ? "invalid" : undefined}
        />
    </div>
  );
};
