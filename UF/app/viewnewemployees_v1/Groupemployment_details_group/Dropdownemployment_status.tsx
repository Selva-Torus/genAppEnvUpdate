 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "43554a40474c87bb086f22ffc9178e5b"
    );
  ...
  },[employment_status78e5b?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setemployment_details_group2c703((prev: any) => ({ ...prev,employment_status78e5b: value }))
      setIsRequredData(false)
    }else{
      let temp:any = employment_details_group2c703;
      delete temp.employee_status;
      delete temp.EMPLOYEE_STATUS;
      delete temp.employment_status78e5b;
      setemployment_details_group2c703(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,employee_status:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(employment_details_group2c703.employee_status){
      if(Array.isArray(dfd_employeestatuscombo_v1Props)){
        if(dfd_employeestatuscombo_v1Props?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)){
          setdropdownValue([dfd_employeestatuscombo_v1Props?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)?.employee_status])
          tempValue=dfd_employeestatuscombo_v1Props?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)?.employee_status
        }else{
          setdropdownValue([employment_details_group2c703.employee_status])
          tempValue=employment_details_group2c703.employee_status
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{employee_status:employment_details_group2c703.employee_status}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)?.employee_status ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.employee_status === employment_details_group2c703.employee_status)?.employee_status
      }else{
        setdropdownValue([employment_details_group2c703.employee_status])
        tempValue=employment_details_group2c703.employee_status
      }   
    ...
    if(employment_status78e5b?.trigger == true)
    {
      await handlechange(tempValue)
      setemployment_status78e5b((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[employment_details_group2c703.employee_status, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employeestatuscombo_v1Props) && dfd_employeestatuscombo_v1Props?.length == 1){
    // setemployment_details_group2c703((pre:any)=>({...pre,employee_status:dfd_employeestatuscombo_v1Props[0]?.employee_status}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setemployment_details_group2c703((prev: any) => ({ ...prev, employee_status: getMapperDetailsBindValues[value],EMPLOYEE_STATUS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setemployment_details_group2c703((prev: any) => ({ ...prev, employee_status: '', employment_status78e5b: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,employee_status:undefined}}));
  ...
  useEffect(() => { employment_details_group2c703Ref.current = employment_details_group2c703; }, [employment_details_group2c703]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "43554a40474c87bb086f22ffc9178e5b") {
        handleClick(employment_details_group2c703Ref?.current?.employment_status78e5b?employment_details_group2c703Ref?.current?.employment_status78e5b:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "43554a40474c87bb086f22ffc9178e5b");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group2c703((pre:any)=>({...pre,employee_status:""}))
    else
      setInitialCount(1)
  },[employment_status78e5b?.refresh])
  

  if (employment_status78e5b?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {employment_status78e5b?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={employment_details_group2c703?.employment_status78e5b ? [employment_details_group2c703?.employment_status78e5b] : (employment_details_group2c703?.employee_status ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.employee_status ? "invalid" : undefined}
        />
    </div>
  );
};
