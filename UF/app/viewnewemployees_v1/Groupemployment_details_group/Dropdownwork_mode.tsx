 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "c3fcfc04512ac69995451924e21fea3c"
    );
  ...
  },[work_modefea3c?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setemployment_details_group2c703((prev: any) => ({ ...prev,work_modefea3c: value }))
      setIsRequredData(false)
    }else{
      let temp:any = employment_details_group2c703;
      delete temp.work_mode;
      delete temp.WORK_MODE;
      delete temp.work_modefea3c;
      setemployment_details_group2c703(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,work_mode:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(employment_details_group2c703.work_mode){
      if(Array.isArray(dfd_empworkmodecombo_v1Props)){
        if(dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)){
          setdropdownValue([dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)?.emp_work_mode])
          tempValue=dfd_empworkmodecombo_v1Props?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)?.emp_work_mode
        }else{
          setdropdownValue([employment_details_group2c703.work_mode])
          tempValue=employment_details_group2c703.work_mode
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_work_mode:employment_details_group2c703.work_mode}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)?.emp_work_mode ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_work_mode === employment_details_group2c703.work_mode)?.emp_work_mode
      }else{
        setdropdownValue([employment_details_group2c703.work_mode])
        tempValue=employment_details_group2c703.work_mode
      }   
    ...
    if(work_modefea3c?.trigger == true)
    {
      await handlechange(tempValue)
      setwork_modefea3c((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[employment_details_group2c703.work_mode, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_empworkmodecombo_v1Props) && dfd_empworkmodecombo_v1Props?.length == 1){
    // setemployment_details_group2c703((pre:any)=>({...pre,work_mode:dfd_empworkmodecombo_v1Props[0]?.work_mode}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setemployment_details_group2c703((prev: any) => ({ ...prev, work_mode: getMapperDetailsBindValues[value],WORK_MODE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setemployment_details_group2c703((prev: any) => ({ ...prev, work_mode: '', work_modefea3c: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,work_mode:undefined}}));
  ...
  const employment_details_group2c703Ref = useRef<any>(employment_details_group2c703);
  useEffect(() => { employment_details_group2c703Ref.current = employment_details_group2c703; }, [employment_details_group2c703]);
    useEffect(()=>{
        handleBlur()
       const handler = (id:any) => {
          if (id === "c3fcfc04512ac69995451924e21fea3c") {
        handleClick(employment_details_group2c703Ref?.current?.work_modefea3c?employment_details_group2c703Ref?.current?.work_modefea3c:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "c3fcfc04512ac69995451924e21fea3c");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group2c703((pre:any)=>({...pre,work_mode:""}))
    else
      setInitialCount(1)
  },[work_modefea3c?.refresh])
  

  if (work_modefea3c?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {work_modefea3c?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={employment_details_group2c703?.work_modefea3c ? [employment_details_group2c703?.work_modefea3c] : (employment_details_group2c703?.work_mode ? dropdownValue : [])}
        validationState={validate?.viewNewEmployees_v1?.work_mode ? "invalid" : undefined}
        />
    </div>
  );
};
