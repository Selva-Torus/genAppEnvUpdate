...
import Textemployment_details  from "./Textemployment_details";
import Dropdownemployment_type  from "./Dropdownemployment_type";
import Dropdownemployment_status  from "./Dropdownemployment_status";
import DatePickerhire_date  from "./DatePickerhire_date";
import DatePickerconfirmation_date  from "./DatePickerconfirmation_date";
import DatePickerprobation_end_date  from "./DatePickerprobation_end_date";
import Dropdownwork_mode  from "./Dropdownwork_mode";
import Dropdowntime_zone  from "./Dropdowntime_zone";
import Dropdownhr_manager_id  from "./Dropdownhr_manager_id";
import Dropdownreporting_mmanager_id  from "./Dropdownreporting_mmanager_id";
...
const Groupemployment_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "47ab0503c3ef42c41f62b7cc6712c703");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemployment_details_group2c703Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("employment_details")){
        setemployment_details91e0a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employment_details91e0a?.isDisabled==null)
      {
        setemployment_details91e0a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("employment_type")){
        setemployment_type9321e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employment_type9321e?.isDisabled==null)
      {
        setemployment_type9321e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("employment_status")){
        setemployment_status78e5b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(employment_status78e5b?.isDisabled==null)
      {
        setemployment_status78e5b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("hire_date")){
        sethire_date31217((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(hire_date31217?.isDisabled==null)
      {
        sethire_date31217((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("confirmation_date")){
        setconfirmation_date350d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(confirmation_date350d6?.isDisabled==null)
      {
        setconfirmation_date350d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("probation_end_date")){
        setprobation_end_datefb160((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(probation_end_datefb160?.isDisabled==null)
      {
        setprobation_end_datefb160((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("work_mode")){
        setwork_modefea3c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(work_modefea3c?.isDisabled==null)
      {
        setwork_modefea3c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("time_zone")){
        settime_zoneba768((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(time_zoneba768?.isDisabled==null)
      {
        settime_zoneba768((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("hr_manager_id")){
        sethr_manager_idec19f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(hr_manager_idec19f?.isDisabled==null)
      {
        sethr_manager_idec19f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reporting_mmanager_id")){
        setreporting_mmanager_id7f8b5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reporting_mmanager_id7f8b5?.isDisabled==null)
      {
        setreporting_mmanager_id7f8b5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const employment_details_group2c703Ref = useRef<any>(null);
  const handleClearSearch = () => {
    employment_details_group2c703Ref.current?.setSearchParams();
    employment_details_group2c703Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(employment_details_group2c703) && Object.keys(employment_details_group2c703)?.length>0)
      {
        setemployment_details_group2c703({})
      }
    }else 
      prevRefreshRef.current= true
  }, [employment_details_group2c703Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"employment_details_group"}))
        }}
    >
          {allowedControls.includes("employment_details") ?<Textemployment_details   /* 91e0a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("employment_type") ?<Dropdownemployment_type   /* 9321e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("employment_status") ?<Dropdownemployment_status   /* 78e5b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("hire_date") ?<DatePickerhire_date   /* 31217 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("confirmation_date") ?<DatePickerconfirmation_date   /* 350d6 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("probation_end_date") ?<DatePickerprobation_end_date   /* fb160 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("work_mode") ?<Dropdownwork_mode   /* fea3c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("time_zone") ?<Dropdowntime_zone   /* ba768 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("hr_manager_id") ?<Dropdownhr_manager_id   /* ec19f */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("reporting_mmanager_id") ?<Dropdownreporting_mmanager_id   /* 7f8b5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}

export default Groupemployment_details_group
