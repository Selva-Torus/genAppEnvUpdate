...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,probation_end:undefined}}));
  if (!date) {
    setemployment_details_group2c703((prev: any) => ({ ...prev, probation_end: null }));
    return;
  }
  const now = new Date();
  ...
  setemployment_details_group2c703((prev: any) => ({ ...prev, probation_end: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "b0cc7721fecf36bb1e94149ff38fb160"
      );
...
useEffect(()=>{
 ...
 },[probation_end_datefb160?.refresh])
...
if (probation_end_datefb160?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 7`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={employment_details_group2c703?.probation_end}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {probation_end_datefb160?.isDisabled ? true : false}
      disabled= {probation_end_datefb160?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Probation End Date"
      dateValidation=""
      validationState={validate?.viewNewEmployees_v1?.probation_end ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
