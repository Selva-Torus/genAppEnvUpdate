...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,confirmation_date:undefined}}));
  if (!date) {
    setemployment_details_group2c703((prev: any) => ({ ...prev, confirmation_date: null }));
    return;
  }
  const now = new Date();
  ...
  setemployment_details_group2c703((prev: any) => ({ ...prev, confirmation_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "7b61c441f755d0f419134f36229350d6"
      );
...
useEffect(()=>{
 ...
 },[confirmation_date350d6?.refresh])
...
if (confirmation_date350d6?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `19 / 25`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={employment_details_group2c703?.confirmation_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {confirmation_date350d6?.isDisabled ? true : false}
      disabled= {confirmation_date350d6?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Confirmation Date"
      dateValidation=""
      validationState={validate?.viewNewEmployees_v1?.confirmation_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
