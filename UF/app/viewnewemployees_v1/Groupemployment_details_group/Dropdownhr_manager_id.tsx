...
const Dropdownhr_manager_id = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(employment_details_group2c703?.hr_manager_id=="" || employment_details_group2c703?.hr_manager_id==undefined || employment_details_group2c703?.hr_manager_id==null ){
    setSelectedItem("");
  }
  },[employment_details_group2c703?.hr_manager_id])
  const handleMapperValue=async()=>{
    try{
      const orchestrationData :any = getControlOrchestrationData(
        controlData,
        "47ab0503c3ef42c41f62b7cc6712c703",
        "eed4823ab3a39d89271b73273dbec19f"
      );
      ...
      let staticValueProps : any[] = [
  {
    ...
    "value": {
      ...
      setemployment_details_group2c703((prev: any) => ({ ...prev, hr_manager_id: staticTextValue, hr_manager_idec19f: value}))
         setIsRequredData(false)
    } else {
       setemployment_details_group2c703((prev: any) => ({ ...prev, hr_manager_id: '', hr_manager_idec19f: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,hr_manager_id:undefined}}));
  ...
  useEffect(() => { employment_details_group2c703Ref.current = employment_details_group2c703; }, [employment_details_group2c703]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "eed4823ab3a39d89271b73273dbec19f") {
        handleClick(employment_details_group2c703Ref?.current?.hr_manager_idec19f?employment_details_group2c703Ref?.current?.hr_manager_idec19f:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "eed4823ab3a39d89271b73273dbec19f");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setemployment_details_group2c703((pre:any)=>({...pre,hr_manager_id:""}))
    else
      setInitialCount(1)
  },[hr_manager_idec19f?.refresh])
  

  if (hr_manager_idec19f?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {hr_manager_idec19f?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={
            employment_details_group2c703?.hr_manager_idec19f ? [employment_details_group2c703?.hr_manager_idec19f] :
                employment_details_group2c703?.hr_manager_id ? employment_details_group2c703?.hr_manager_id : []
            }
        onChange={handleClick} 
        validationState={validate?.viewNewEmployees_v1?.hr_manager_id ? "invalid" : undefined}
      /> 
    </div>
  );
};
