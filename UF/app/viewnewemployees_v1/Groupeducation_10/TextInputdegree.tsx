...
const TextInputdegree = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:educationJson:AFVK:v1|172c23b4894e4d33a2784d2bd3e0c409|properties.education_history_json.items.properties.degree"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|b0ce34f22c49cc7e38365c0ce5828de7|eb263b57736571ed66adf3910849b174"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'degree',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,degree:undefined}}));
    if(dynamicStateandType.type=="number"){
    seteducation28de7_10((prev: any) => ({ ...prev, degree: +e.target.value }));
    }
    else{
    seteducation28de7_10((prev: any) => ({ ...prev, degree: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "eb263b57736571ed66adf3910849b174"
      );
      ...
      //     controlId: "eb263b57736571ed66adf3910849b174",
      //     isTable: false,
      //     from:"TextInputdegree",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'degree', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'degree',type:'text'};
      //   type={
      //     name:'degree',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.degree.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.degree.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.degree.type
      //   }
      ...
      //   type={
      //     name:'degree',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.degree.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.degree.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.degree.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "eb263b57736571ed66adf3910849b174") {
        handleChange({target:{value:education28de7_10Ref?.current?.degree||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "eb263b57736571ed66adf3910849b174") {
        handleBlur({target:{value:education28de7_10Ref?.current?.degree||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation28de7_10((pre:any)=>({...pre,degree:dfd_educationjson_v1Props.data[0]?.degree}));
    }
  }
  },[dfd_educationjson_v1Props?.setSearchFilters])
  if (degree9b174?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={education28de7_10?.degree||""}
         disabled= {degree9b174?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='B.E'      
        view='normal'
        contentAlign={"left"}
        ...
        headerText="Degree"
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.degree ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdegree
