...
const Groupskills_education_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c606202c2905f553e1f9bfe4080d1667");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setskills_education_groupd1667Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("skills_education")){
        setskills_education3793d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_education3793d?.isDisabled==null)
      {
        setskills_education3793d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skills_group")){
        setskills_group92cc8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_group92cc8?.isDisabled==null)
      {
        setskills_group92cc8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("education_group")){
        seteducation_groupcd288((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(education_groupcd288?.isDisabled==null)
      {
        seteducation_groupcd288((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_divider")){
        setfamily_divider0014b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_divider0014b?.isDisabled==null)
      {
        setfamily_divider0014b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cert_group")){
        setcert_group3be86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cert_group3be86?.isDisabled==null)
      {
        setcert_group3be86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_detail_group")){
        setfamily_detail_group496b3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_detail_group496b3?.isDisabled==null)
      {
        setfamily_detail_group496b3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['skills_education'] = skills_education3793d,
        codeStates['setskills_education'] = setskills_education3793d,
        ...
        codeStates['family_divider'] = family_divider0014b,
        codeStates['setfamily_divider'] = setfamily_divider0014b,
  ...
  const skills_education_groupd1667Ref = useRef<any>(null);
  const handleClearSearch = () => {
    skills_education_groupd1667Ref.current?.setSearchParams();
    skills_education_groupd1667Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(skills_education_groupd1667) && Object.keys(skills_education_groupd1667)?.length>0)
      {
        setskills_education_groupd1667({})
      }
    }else 
      prevRefreshRef.current= true
  }, [skills_education_groupd1667Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"skills_education_group"}))
        }}
    >
        ...
        {allowedComponent.includes("family_detail_group")  &&<Groupfamily_detail_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("skills_education") ?<Textskills_education   /* 3793d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("family_divider") ?<Dividerfamily_divider   /* 0014b */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
