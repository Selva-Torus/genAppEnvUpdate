...
const TextInputproficiency_level = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setskilld0dba_1((prev: any) => ({ ...prev, proficiency_level: +e.target.value }));
    }
    else{
    setskilld0dba_1((prev: any) => ({ ...prev, proficiency_level: e.target.value }));
    }
  ...
  const skilld0dba_1Ref = useRef<any>(skilld0dba_1);
  useEffect(() => { skilld0dba_1Ref.current = skilld0dba_1; }, [skilld0dba_1]);
  useEffect(()=>{
      handleMapperValue();
    ...
    const handlerChange = (id:any) => {
      if (id === "ff12ed1a5b7bc48705ce03ac070f783d") {
        handleChange({target:{value:skilld0dba_1Ref?.current?.proficiency_level||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ff12ed1a5b7bc48705ce03ac070f783d") {
        handleBlur({target:{value:skilld0dba_1Ref?.current?.proficiency_level||""}});
      }
    };
  ...
  if(dfd_skilljson_v1Props?.setSearchFilters && dfd_skilljson_v1Props?.data)
  {
    if(Array.isArray(dfd_skilljson_v1Props.data) && dfd_skilljson_v1Props.data.length > 0){
      setskilld0dba_1((pre:any)=>({...pre,proficiency_level:dfd_skilljson_v1Props.data[0]?.proficiency_level}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={skilld0dba_1?.proficiency_level||""}
