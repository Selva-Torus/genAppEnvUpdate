...
const TextInputinstitution_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:educationJson:AFVK:v1|172c23b4894e4d33a2784d2bd3e0c409|properties.education_history_json.items.properties.institution_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewNewEmployees:AFVK:v1|b0ce34f22c49cc7e38365c0ce5828de7|94b88182608f4ea7ac4987a0912e6f0c"
    }
  ],
  ...
  "schemaData": {
    "type": "string",
    "x-pg-type": "text",
    "x-expression": "ed ->> 'institution_name'"
  }
}
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'institution_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewNewEmployees_v1:{...pre?.viewNewEmployees_v1,institution_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    seteducation28de7_12((prev: any) => ({ ...prev, institution_name: +e.target.value }));
    }
    else{
    seteducation28de7_12((prev: any) => ({ ...prev, institution_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "94b88182608f4ea7ac4987a0912e6f0c"
      );
      ...
      //     controlId: "94b88182608f4ea7ac4987a0912e6f0c",
      //     isTable: false,
      //     from:"TextInputinstitution_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'institution_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'institution_name',type:'text'};
      //   type={
      //     name:'institution_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.institution_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.institution_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.institution_name.type
      //   }
      ...
      //   type={
      //     name:'institution_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.institution_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.institution_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.institution_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const education28de7_12Ref = useRef<any>(education28de7_12);
  useEffect(() => { education28de7_12Ref.current = education28de7_12; }, [education28de7_12]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "94b88182608f4ea7ac4987a0912e6f0c") {
        handleChange({target:{value:education28de7_12Ref?.current?.institution_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "94b88182608f4ea7ac4987a0912e6f0c") {
        handleBlur({target:{value:education28de7_12Ref?.current?.institution_name||""}});
      }
    };
  ...
  if(dfd_educationjson_v1Props?.setSearchFilters && dfd_educationjson_v1Props?.data)
  {
    if(Array.isArray(dfd_educationjson_v1Props.data) && dfd_educationjson_v1Props.data.length > 0){
      seteducation28de7_12((pre:any)=>({...pre,institution_name:dfd_educationjson_v1Props.data[0]?.institution_name}));
    }
  }
  },[dfd_educationjson_v1Props?.setSearchFilters])
  if (institution_namee6f0c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `1 / 13`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={education28de7_12?.institution_name||""}
         disabled= {institution_namee6f0c?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewNewEmployees_v1?.institution_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputinstitution_name
