...
import Groupfamly_details  from "../Groupfamly_details/Groupfamly_details";
...
import Iconfamily_icon  from "./Iconfamily_icon";
import Textfam_text  from "./Textfam_text";
import Textfamily_des  from "./Textfamily_des";
...
const Groupfamily_detail_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {family_icon128d6, setfamily_icon128d6}= useContext(TotalContext) as TotalContextProps;
  const {fam_text4ee51, setfam_text4ee51}= useContext(TotalContext) as TotalContextProps;
  const {family_desd7843, setfamily_desd7843}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "70c180a8d35dedf703c98c7ab62496b3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfamily_detail_group496b3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("family_icon")){
        setfamily_icon128d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_icon128d6?.isDisabled==null)
      {
        setfamily_icon128d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fam_text")){
        setfam_text4ee51((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fam_text4ee51?.isDisabled==null)
      {
        setfam_text4ee51((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("family_des")){
        setfamily_desd7843((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(family_desd7843?.isDisabled==null)
      {
        setfamily_desd7843((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("famly_details")){
        setfamly_detailsb4eb8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(famly_detailsb4eb8?.isDisabled==null)
      {
        setfamly_detailsb4eb8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['family_icon'] = family_icon128d6,
        codeStates['setfamily_icon'] = setfamily_icon128d6,
        codeStates['fam_text'] = fam_text4ee51,
        codeStates['setfam_text'] = setfam_text4ee51,
        codeStates['family_des'] = family_desd7843,
        codeStates['setfamily_des'] = setfamily_desd7843,
  ...
  const family_detail_group496b3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    family_detail_group496b3Ref.current?.setSearchParams();
    family_detail_group496b3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(family_detail_group496b3) && Object.keys(family_detail_group496b3)?.length>0)
      {
        setfamily_detail_group496b3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [family_detail_group496b3Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"family_detail_group"}))
        }}
    >
        {allowedComponent.includes("famly_details")  &&<Groupfamly_details  
        ...
        {allowedControls.includes("family_icon")?<Iconfamily_icon /* 128d6 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("fam_text") ?<Textfam_text   /* 4ee51 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("family_des") ?<Textfamily_des   /* d7843 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupfamily_detail_group
