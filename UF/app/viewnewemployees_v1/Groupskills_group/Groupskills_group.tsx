...
const Groupskills_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e32afbf567b63024ae1573641c992cc8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setskills_group92cc8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("skills_icon")){
        setskills_icon4cbbc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skills_icon4cbbc?.isDisabled==null)
      {
        setskills_icon4cbbc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill_text")){
        setskill_textb79c7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skill_textb79c7?.isDisabled==null)
      {
        setskill_textb79c7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill_des")){
        setskill_desb14d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skill_desb14d6?.isDisabled==null)
      {
        setskill_desb14d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("skill")){
        setskilld0dba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(skilld0dba?.isDisabled==null)
      {
        setskilld0dba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['skills_icon'] = skills_icon4cbbc,
        codeStates['setskills_icon'] = setskills_icon4cbbc,
        codeStates['skill_text'] = skill_textb79c7,
        codeStates['setskill_text'] = setskill_textb79c7,
        codeStates['skill_des'] = skill_desb14d6,
        codeStates['setskill_des'] = setskill_desb14d6,
  ...
  const skills_group92cc8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    skills_group92cc8Ref.current?.setSearchParams();
    skills_group92cc8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(skills_group92cc8) && Object.keys(skills_group92cc8)?.length>0)
      {
        setskills_group92cc8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [skills_group92cc8Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewnewemployees_v1((pre:any)=>({...pre,_selectedGroup_:"skills_group"}))
        }}
    >
        ...
        {allowedControls.includes("skills_icon")?<Iconskills_icon /* 4cbbc */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("skill_text") ?<Textskill_text   /* b79c7 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("skill_des") ?<Textskill_des   /* b14d6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
