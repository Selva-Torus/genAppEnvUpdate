...
const Textemployee_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_employees_v1Props && !dfd_employees_v1Props.hasLogicCenter) {
        ...
        setnew_employee_groupdf01f((pre: any) => {
          return { ...pre, employee_id: api_paginationData.data.records[0]?.employee_id }
        })
        }
      }
      else{
      if(Array.isArray(dfd_employees_v1Props) && dfd_employees_v1Props && !new_employee_groupdf01f.employee_id){
        setnew_employee_groupdf01f((pre:any)=>({...pre,employee_id:dfd_employees_v1Props[0]?.employee_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employee_ida5f8c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employees_v1Props) && !new_employee_groupdf01f.employee_id){
    setnew_employee_groupdf01f((pre:any)=>({...pre,employee_id:dfd_employees_v1Props[0]?.employee_id}));
  }
  },[dfd_employees_v1Props])

  if (employee_ida5f8c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.employee_id : (new_employee_groupdf01f?.employee_id || ""))}
</Text>
  </div>
  )
}
