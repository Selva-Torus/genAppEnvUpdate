 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "c768cc3ab03ff74fe59ac325ffac63df"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonc63df") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "c768cc3ab03ff74fe59ac325ffac63df") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonc63df");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonc63df?.trigger) return;
      if(ok_buttonc63df?.trigger){
      setok_buttonc63df((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonc63df?.trigger])

  useEffect(()=>{
    if(ok_buttonc63df?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonc63df?.refresh])
  ...
  async function handleSave63df_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete8ee3bProps = group_delete8ee3bPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.assignDocDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete8ee3bRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete8ee3b?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "c768cc3ab03ff74fe59ac325ffac63df",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "c768cc3ab03ff74fe59ac325ffac63df.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "c768cc3ab03ff74fe59ac325ffac63df.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "c768cc3ab03ff74fe59ac325ffac63df.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "c768cc3ab03ff74fe59ac325ffac63df.1.1.1.2",
              ...
              "children": [
                {
                  "id": "1cff839b6eae7947a9fa183da14392d0.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignDocDelete:AFVK:v1";
      sourceId+= "|"+"25c9d48e9a305596f358cc08bd38ee3b";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignDocDelete:AFVK:v1"+"|"+"25c9d48e9a305596f358cc08bd38ee3b"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete8ee3b?.upId? [group_delete8ee3b?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete8ee3b?.upId? [group_delete8ee3b?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete8ee3b?.upId? [group_delete8ee3b?.upId ] : group_delete8ee3b?.trs_process_id? [group_delete8ee3b?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete8ee3bRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"25c9d48e9a305596f358cc08bd38ee3b",
              controlId:"c768cc3ab03ff74fe59ac325ffac63df"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete8ee3bProps.ssKey !== '' && group_delete8ee3bProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete8ee3bProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete8ee3b((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave63df_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete8ee3b((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_buttonc63df?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("Delete")}
