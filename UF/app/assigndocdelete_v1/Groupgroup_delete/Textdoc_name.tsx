...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assigndoctable_v1Props) && dfd_assigndoctable_v1Props && !group_delete8ee3b.doc_name){
        setgroup_delete8ee3b((pre:any)=>({...pre,doc_name:dfd_assigndoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_name19bda?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assigndoctable_v1Props) && !group_delete8ee3b.doc_name){
    setgroup_delete8ee3b((pre:any)=>({...pre,doc_name:dfd_assigndoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_assigndoctable_v1Props])

  if (doc_name19bda?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete8ee3b?.doc_name || ""))}
</Text>
  </div>
  )
}
