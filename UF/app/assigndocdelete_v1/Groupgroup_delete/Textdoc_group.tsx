...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assigndoctable_v1Props) && dfd_assigndoctable_v1Props && !group_delete8ee3b.doc_group){
        setgroup_delete8ee3b((pre:any)=>({...pre,doc_group:dfd_assigndoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_group796b8?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assigndoctable_v1Props) && !group_delete8ee3b.doc_group){
    setgroup_delete8ee3b((pre:any)=>({...pre,doc_group:dfd_assigndoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_assigndoctable_v1Props])

  if (doc_group796b8?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_delete8ee3b?.doc_group || ""))}
</Text>
  </div>
  )
}
