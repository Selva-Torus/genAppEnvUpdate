...
import Groupgrp  from "./Groupgrp/Groupgrp";


export default function PageManagerDailyExpenseV1() {
  const { isDark, isHighContrast, bgStyle, textStyle } : { isDark: boolean; isHighContrast: boolean; bgStyle: string; textStyle: string } = useTheme();
  const screenName:string = "manager daily expense";
  ...
  const {daily_expense_manager_table_v1Props, setdaily_expense_manager_table_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [checkgrp,setCheckgrp,]=useState<boolean>(false);
  const [checkdaily_expense_manager_table,setCheckdaily_expense_manager_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function claim_table_data_v1(pagination:any): Promise<void>{
        ...
        if(daily_expense_manager_table_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< daily_expense_manager_table_v1Props.length;i++){
            if(daily_expense_manager_table_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claim_table_data:AFVK:v1"){
              delete daily_expense_manager_table_v1Props[i].DFDkey;
              filterData.push(daily_expense_manager_table_v1Props[i])
            }           
          }
  ...
  return (
    ...
    {securityData[accessProfile]?.allowedGroups?.includes("grp") && initialLoad &&<Groupgrp 
