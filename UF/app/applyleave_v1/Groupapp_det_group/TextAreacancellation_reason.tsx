...
const TextAreacancellation_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ace07d4dad2a4cbb975cbe6a1894824f"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cancellation_reason4824f?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_groupe1335((pre:any)=>({...pre,cancellation_reason:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ace07d4dad2a4cbb975cbe6a1894824f") {
        handleChange({target:{value:app_det_groupe1335Ref?.current?.cancellation_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ace07d4dad2a4cbb975cbe6a1894824f") {
        handleBlur({target:{value:app_det_groupe1335Ref?.current?.cancellation_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,cancellation_reason:undefined}}));
    setapp_det_groupe1335((prev: any) => ({ ...prev, cancellation_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (cancellation_reason4824f?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `17 / 25`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {cancellation_reason4824f?.isDisabled ? true : false}
      placeholder = {'Enter cancellation reason....'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Cancellation Reason"
      pin = {'brick-brick'}
      value = { app_det_groupe1335?.cancellation_reason != null && typeof app_det_groupe1335?.cancellation_reason =='object' ? Object.keys(app_det_groupe1335?.cancellation_reason)?.length ?  JSON.stringify(app_det_groupe1335?.cancellation_reason,null ,2):"" : app_det_groupe1335?.cancellation_reason||""}
      errorMessage={error}
      validationState={validate?.applyLeave_v1?.cancellation_reason ? "invalid" : undefined}
    />
  </div>
  )
}
