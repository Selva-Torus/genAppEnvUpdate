...
const TextAreaapproval_comments = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "0d798c00244645dfb90dae0520760355"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[approval_comments60355?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_groupe1335((pre:any)=>({...pre,approval_comments:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "0d798c00244645dfb90dae0520760355") {
        handleChange({target:{value:app_det_groupe1335Ref?.current?.approval_comments||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "0d798c00244645dfb90dae0520760355") {
        handleBlur({target:{value:app_det_groupe1335Ref?.current?.approval_comments||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,approval_comments:undefined}}));
    setapp_det_groupe1335((prev: any) => ({ ...prev, approval_comments: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (approval_comments60355?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {approval_comments60355?.isDisabled ? true : false}
      placeholder = {'Approved for the requested access..'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Approval Comments"
      pin = {'brick-brick'}
      value = { app_det_groupe1335?.approval_comments != null && typeof app_det_groupe1335?.approval_comments =='object' ? Object.keys(app_det_groupe1335?.approval_comments)?.length ?  JSON.stringify(app_det_groupe1335?.approval_comments,null ,2):"" : app_det_groupe1335?.approval_comments||""}
      errorMessage={error}
      validationState={validate?.applyLeave_v1?.approval_comments ? "invalid" : undefined}
    />
  </div>
  )
}
