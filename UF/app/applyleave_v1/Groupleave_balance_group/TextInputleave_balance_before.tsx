...
const TextInputleave_balance_before = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'leave_balance_before',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,leave_balance_before:undefined}}));
    if(dynamicStateandType.type=="number"){
    setleave_balance_group2b19a((prev: any) => ({ ...prev, leave_balance_before: +e.target.value }));
    }
    else{
    setleave_balance_group2b19a((prev: any) => ({ ...prev, leave_balance_before: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5201871f475a4db18d9860bf1e7d2d96"
      );
      ...
      //     controlId: "5201871f475a4db18d9860bf1e7d2d96",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5201871f475a4db18d9860bf1e7d2d96") {
        handleChange({target:{value:leave_balance_group2b19aRef?.current?.leave_balance_before||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5201871f475a4db18d9860bf1e7d2d96") {
        handleBlur({target:{value:leave_balance_group2b19aRef?.current?.leave_balance_before||""}});
      }
    };
  ...
  if (leave_balance_befored2d96?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={leave_balance_group2b19a?.leave_balance_before||""}
         disabled= {leave_balance_befored2d96?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.applyLeave_v1?.leave_balance_before ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
