...
const Textleave_req_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {leave_req_id501ec, setleave_req_id501ec}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[leave_req_id501ec?.refresh])

  if (leave_req_id501ec?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 4`,gridRow: `111 / 112`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.leave_req_id : (new_access_group9bde0?.leave_req_id || ""))}
</Text>
  </div>
  )
}
