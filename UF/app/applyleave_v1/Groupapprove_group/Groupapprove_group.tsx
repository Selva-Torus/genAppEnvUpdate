...
const Groupapprove_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {app_det13c3a, setapp_det13c3a}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "894ae5180dbf4fe08e8339d8ab61e00a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapprove_group1e00aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_det")){
        setapp_det13c3a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_det13c3a?.isDisabled==null)
      {
        setapp_det13c3a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['app_det'] = app_det13c3a,
        codeStates['setapp_det'] = setapp_det13c3a,
  ...
  const approve_group1e00aRef = useRef<any>(null);
  const handleClearSearch = () => {
    approve_group1e00aRef.current?.setSearchParams();
    approve_group1e00aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approve_group1e00a) && Object.keys(approve_group1e00a)?.length>0)
      {
        setapprove_group1e00a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approve_group1e00aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleave_v1((pre:any)=>({...pre,_selectedGroup_:"approve_group"}))
        }}
    >
          {allowedControls.includes("app_det") ?<Textapp_det   /* 13c3a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
