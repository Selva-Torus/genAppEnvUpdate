...
const Groupaudit_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7a7282a89b174fd8be15136e78aa0703");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaudit_groupa0703Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("add_info")){
        setadd_info70aa5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_info70aa5?.isDisabled==null)
      {
        setadd_info70aa5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_inf_textarea")){
        setadd_inf_textareab8ae8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_inf_textareab8ae8?.isDisabled==null)
      {
        setadd_inf_textareab8ae8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['add_info'] = add_info70aa5,
        codeStates['setadd_info'] = setadd_info70aa5,
        codeStates['add_inf_textarea'] = add_inf_textareab8ae8,
        codeStates['setadd_inf_textarea'] = setadd_inf_textareab8ae8,
  ...
  const audit_groupa0703Ref = useRef<any>(null);
  const handleClearSearch = () => {
    audit_groupa0703Ref.current?.setSearchParams();
    audit_groupa0703Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(audit_groupa0703) && Object.keys(audit_groupa0703)?.length>0)
      {
        setaudit_groupa0703({})
      }
    }else 
      prevRefreshRef.current= true
  }, [audit_groupa0703Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleave_v1((pre:any)=>({...pre,_selectedGroup_:"audit_group"}))
        }}
    >
          {allowedControls.includes("add_info") ?<Textadd_info   /* 70aa5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("add_inf_textarea") ?<TextAreaadd_inf_textarea   /* b8ae8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
