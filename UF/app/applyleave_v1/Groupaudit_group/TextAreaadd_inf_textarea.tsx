...
const TextAreaadd_inf_textarea = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "7a7282a89b174fd8be15136e78aa0703",
        "000d36f3995d48179fd813bbf68b8ae8"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[add_inf_textareab8ae8?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setaudit_groupa0703((pre:any)=>({...pre,add_inf_textarea:""}))
    }else 
      prevRefreshRef.current= true
  },[add_inf_textareab8ae8?.refresh])

  const audit_groupa0703Ref = useRef<any>(audit_groupa0703);
  useEffect(() => { audit_groupa0703Ref.current = audit_groupa0703; }, [audit_groupa0703]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "000d36f3995d48179fd813bbf68b8ae8") {
        handleChange({target:{value:audit_groupa0703Ref?.current?.add_inf_textarea||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "000d36f3995d48179fd813bbf68b8ae8") {
        handleBlur({target:{value:audit_groupa0703Ref?.current?.add_inf_textarea||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,add_inf_textarea:undefined}}));
    setaudit_groupa0703((prev: any) => ({ ...prev, add_inf_textarea: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (add_inf_textareab8ae8?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 25`,gridRow: `8 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {add_inf_textareab8ae8?.isDisabled ? true : false}
      placeholder = {'--'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Additional Details"
      pin = {'brick-brick'}
      value = { audit_groupa0703?.add_inf_textarea != null && typeof audit_groupa0703?.add_inf_textarea =='object' ? Object.keys(audit_groupa0703?.add_inf_textarea)?.length ?  JSON.stringify(audit_groupa0703?.add_inf_textarea,null ,2):"" : audit_groupa0703?.add_inf_textarea||""}
      errorMessage={error}
      validationState={validate?.applyLeave_v1?.add_inf_textarea ? "invalid" : undefined}
    />
  </div>
  )
}
