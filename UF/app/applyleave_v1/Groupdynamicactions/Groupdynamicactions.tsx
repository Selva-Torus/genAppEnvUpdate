...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a78e3f0cdef141126f39f959ea8e55b7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactionse55b7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel4567f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel4567f?.isDisabled==null)
      {
        setcancel4567f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("apply")){
        setapply6811c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(apply6811c?.isDisabled==null)
      {
        setapply6811c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactionse55b7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactionse55b7Ref.current?.setSearchParams();
    dynamicactionse55b7Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactionse55b7Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactionse55b7Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactionse55b7) && Object.keys(dynamicactionse55b7)?.length>0)
      {
        setdynamicactionse55b7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactionse55b7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleave_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
        ...
        {        ((ruleData?.length>0 && "apply" in ButtonGoRuleData)?ButtonGoRuleData["apply"]:true) && 
          allowedControls.includes("apply")  ?            <Buttonapply tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}
