...
const TextAreahandover_notes = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6d371a3d5db9b4c9bb2e268926b78e9b"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[handover_notes78e9b?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setemp_avail_group21476((pre:any)=>({...pre,handover_notes:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6d371a3d5db9b4c9bb2e268926b78e9b") {
        handleChange({target:{value:emp_avail_group21476Ref?.current?.handover_notes||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6d371a3d5db9b4c9bb2e268926b78e9b") {
        handleBlur({target:{value:emp_avail_group21476Ref?.current?.handover_notes||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,handover_notes:undefined}}));
    setemp_avail_group21476((prev: any) => ({ ...prev, handover_notes: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (handover_notes78e9b?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `10 / 25`,gridRow: `8 / 27`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {handover_notes78e9b?.isDisabled ? true : false}
      placeholder = {'e.g. Specify tasks to be covered and designated persons'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Handover Notes"
      pin = {'brick-brick'}
      value = { emp_avail_group21476?.handover_notes != null && typeof emp_avail_group21476?.handover_notes =='object' ? Object.keys(emp_avail_group21476?.handover_notes)?.length ?  JSON.stringify(emp_avail_group21476?.handover_notes,null ,2):"" : emp_avail_group21476?.handover_notes||""}
      errorMessage={error}
      validationState={validate?.applyLeave_v1?.handover_notes ? "invalid" : undefined}
    />
  </div>
  )
}
