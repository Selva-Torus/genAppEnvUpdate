...
export default function PageApplyleaveV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function employeenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleave_v1Props.length > 0){
          for(let i=0;i< applyleave_v1Props.length;i++){
            if(applyleave_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employeeNameCombo:AFVK:v1"){
              // delete applyleave_v1Props[i].DFDkey;
              let temp=structuredClone(applyleave_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function policynamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleave_v1Props.length > 0){
          for(let i=0;i< applyleave_v1Props.length;i++){
            if(applyleave_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:policyNameCombo:AFVK:v1"){
              // delete applyleave_v1Props[i].DFDkey;
              let temp=structuredClone(applyleave_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function leavereasoncategorycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleave_v1Props.length > 0){
          for(let i=0;i< applyleave_v1Props.length;i++){
            if(applyleave_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leaveReasonCategoryCombo:AFVK:v1"){
              // delete applyleave_v1Props[i].DFDkey;
              let temp=structuredClone(applyleave_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setapplyleave_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setapplyleave_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(applyleave_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(applyleave_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setapplyleave_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
