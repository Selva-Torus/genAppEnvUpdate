...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,start_date: "invalid"}}))
    setaccess_req__group23855((prev: any) => ({ ...prev, start_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,start_date:undefined}}));
  if (!date) {
    setaccess_req__group23855((prev: any) => ({ ...prev, start_date: null }));
    return;
  }
  const now = new Date();
  ...
  setaccess_req__group23855((prev: any) => ({ ...prev, start_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(access_req__group23855?.start_date == "" || access_req__group23855?.start_date == undefined){
        const result = v.safeParse(schema, access_req__group23855?.start_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "0785f7940ac5499daa513e6c30322dc6"
      );
...
useEffect(()=>{
 ...
 },[start_date22dc6?.refresh])

useEffect(()=>{
  if(!access_req__group23855?.start_date){ 
...
if (start_date22dc6?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 6`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={access_req__group23855?.start_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {start_date22dc6?.isDisabled ? true : false}
      disabled= {start_date22dc6?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Start Date"
      dateValidation=""
      validationState={validate?.applyLeave_v1?.start_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
