...
const TextInputdays_requested = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'days_requested',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,days_requested:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group23855((prev: any) => ({ ...prev, days_requested: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(access_req__group23855?.days_requested == "" || access_req__group23855?.days_requested == undefined){
      access_req__group23855.days_requested = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, access_req__group23855?.days_requested);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,applyLeave_v1:{...pre?.applyLeave_v1,days_requested:"invalid"}}));
        }
    }else if(access_req__group23855?.days_requested !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +access_req__group23855?.days_requested);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ab28f7f1abcc44cc9a3cde4786284d0e"
      );
      ...
      //     controlId: "ab28f7f1abcc44cc9a3cde4786284d0e",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!access_req__group23855?.days_requested)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "ab28f7f1abcc44cc9a3cde4786284d0e") {
        handleChange({target:{value:access_req__group23855Ref?.current?.days_requested||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ab28f7f1abcc44cc9a3cde4786284d0e") {
        handleBlur({target:{value:access_req__group23855Ref?.current?.days_requested||""}});
      }
    };
  ...
  if (days_requested84d0e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group23855?.days_requested||""}
         disabled= {days_requested84d0e?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.applyLeave_v1?.days_requested ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
