...
const Buttonok_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "34d93b13ee9d56945f87b2e9268c8577"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonc8577") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "34d93b13ee9d56945f87b2e9268c8577") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonc8577");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonc8577?.trigger) return;
      if(ok_buttonc8577?.trigger){
      setok_buttonc8577((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonc8577?.trigger])

  useEffect(()=>{
    if(ok_buttonc8577?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonc8577?.refresh])
  ...
  async function handleSave8577_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete3c2cdProps = group_delete3c2cdPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.categoryDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete3c2cdRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete3c2cd?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "34d93b13ee9d56945f87b2e9268c8577",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "34d93b13ee9d56945f87b2e9268c8577.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "34d93b13ee9d56945f87b2e9268c8577.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "34d93b13ee9d56945f87b2e9268c8577.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "34d93b13ee9d56945f87b2e9268c8577.1.1.1.1.1",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:categoryDelete:AFVK:v1";
      sourceId+= "|"+"0f6f1c2688e8f3e9b08f7d717613c2cd";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:categoryDelete:AFVK:v1"+"|"+"0f6f1c2688e8f3e9b08f7d717613c2cd"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete3c2cd?.upId? [group_delete3c2cd?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3c2cd?.upId? [group_delete3c2cd?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete3c2cd?.upId? [group_delete3c2cd?.upId ] : group_delete3c2cd?.trs_process_id? [group_delete3c2cd?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete3c2cdRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"0f6f1c2688e8f3e9b08f7d717613c2cd",
              controlId:"34d93b13ee9d56945f87b2e9268c8577"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete3c2cdProps.ssKey !== '' && group_delete3c2cdProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete3c2cdProps.ssKey;          
    }
    ...
    //infoMsg
    toast('Category Deleted successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete3c2cd((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave8577_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete3c2cd((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (ok_buttonc8577?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `20 / 25`,gridRow: `42 / 48`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="   "
          onClick={handleClick}
          view='action'
          disabled= {ok_buttonc8577?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
