...
const Textdepreciation_method = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetcategory_v1Props && !dfd_assetcategory_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3c2cd((pre: any) => {
          return { ...pre, depreciation_method: api_paginationData.data.records[0]?.depreciation_method }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetcategory_v1Props) && dfd_assetcategory_v1Props && !group_delete3c2cd.depreciation_method){
        setgroup_delete3c2cd((pre:any)=>({...pre,depreciation_method:dfd_assetcategory_v1Props[0]?.depreciation_method}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[depreciation_method0e872?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategory_v1Props) && !group_delete3c2cd.depreciation_method){
    setgroup_delete3c2cd((pre:any)=>({...pre,depreciation_method:dfd_assetcategory_v1Props[0]?.depreciation_method}));
  }
  },[dfd_assetcategory_v1Props])

  if (depreciation_method0e872?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `19 / 24`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.depreciation_method : (group_delete3c2cd?.depreciation_method || ""))}
</Text>
  </div>
  )
}

export default Textdepreciation_method
