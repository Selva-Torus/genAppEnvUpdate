...
const Textcategory_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetcategory_v1Props && !dfd_assetcategory_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3c2cd((pre: any) => {
          return { ...pre, category_code: api_paginationData.data.records[0]?.category_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetcategory_v1Props) && dfd_assetcategory_v1Props && !group_delete3c2cd.category_code){
        setgroup_delete3c2cd((pre:any)=>({...pre,category_code:dfd_assetcategory_v1Props[0]?.category_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[category_codef16a8?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategory_v1Props) && !group_delete3c2cd.category_code){
    setgroup_delete3c2cd((pre:any)=>({...pre,category_code:dfd_assetcategory_v1Props[0]?.category_code}));
  }
  },[dfd_assetcategory_v1Props])

  if (category_codef16a8?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `7 / 12`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.category_code : (group_delete3c2cd?.category_code || ""))}
</Text>
  </div>
  )
}

export default Textcategory_code
