...
const Textacat_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetcategory_v1Props) && dfd_assetcategory_v1Props && !group_delete3c2cd.acat_id){
        setgroup_delete3c2cd((pre:any)=>({...pre,acat_id:dfd_assetcategory_v1Props[0]?.acat_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[acat_id9127b?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategory_v1Props) && !group_delete3c2cd.acat_id){
    setgroup_delete3c2cd((pre:any)=>({...pre,acat_id:dfd_assetcategory_v1Props[0]?.acat_id}));
  }
  },[dfd_assetcategory_v1Props])

  if (acat_id9127b?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 3`,gridRow: `42 / 43`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.acat_id : (group_delete3c2cd?.acat_id || ""))}
</Text>
  </div>
  )
}
