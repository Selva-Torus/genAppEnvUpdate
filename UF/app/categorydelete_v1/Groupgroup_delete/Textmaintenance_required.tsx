...
const Textmaintenance_required = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetcategory_v1Props && !dfd_assetcategory_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3c2cd((pre: any) => {
          return { ...pre, maintenance_required: api_paginationData.data.records[0]?.maintenance_required }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetcategory_v1Props) && dfd_assetcategory_v1Props && !group_delete3c2cd.maintenance_required){
        setgroup_delete3c2cd((pre:any)=>({...pre,maintenance_required:dfd_assetcategory_v1Props[0]?.maintenance_required}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[maintenance_required336be?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategory_v1Props) && !group_delete3c2cd.maintenance_required){
    setgroup_delete3c2cd((pre:any)=>({...pre,maintenance_required:dfd_assetcategory_v1Props[0]?.maintenance_required}));
  }
  },[dfd_assetcategory_v1Props])

  if (maintenance_required336be?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.maintenance_required : (group_delete3c2cd?.maintenance_required || ""))}
</Text>
  </div>
  )
}

export default Textmaintenance_required
