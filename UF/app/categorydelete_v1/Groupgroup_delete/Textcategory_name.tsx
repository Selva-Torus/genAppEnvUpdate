...
const Textcategory_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetcategory_v1Props) && dfd_assetcategory_v1Props && !group_delete3c2cd.category_name){
        setgroup_delete3c2cd((pre:any)=>({...pre,category_name:dfd_assetcategory_v1Props[0]?.category_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[category_namecbc0b?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetcategory_v1Props) && !group_delete3c2cd.category_name){
    setgroup_delete3c2cd((pre:any)=>({...pre,category_name:dfd_assetcategory_v1Props[0]?.category_name}));
  }
  },[dfd_assetcategory_v1Props])

  if (category_namecbc0b?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.category_name : (group_delete3c2cd?.category_name || ""))}
</Text>
  </div>
  )
}
