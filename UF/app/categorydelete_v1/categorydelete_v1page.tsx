...
export default function PageCategorydeleteV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetcategory_v1DFD(pagination:any): Promise<void>{
        ...
        if(categorydelete_v1Props.length > 0){
          for(let i=0;i< categorydelete_v1Props.length;i++){
            if(categorydelete_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetCategory:AFVK:v1"){
              // delete categorydelete_v1Props[i].DFDkey;
              let temp=structuredClone(categorydelete_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setcategorydelete_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:categoryDelete:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setcategorydelete_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(categorydelete_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(categorydelete_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
