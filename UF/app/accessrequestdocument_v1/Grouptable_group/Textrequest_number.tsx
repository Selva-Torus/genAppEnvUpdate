...
const Textrequest_number = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_accessreqdoctable_v1Props) && dfd_accessreqdoctable_v1Props && !table_groupbcd33.request_number){
        settable_groupbcd33((pre:any)=>({...pre,request_number:dfd_accessreqdoctable_v1Props[0]?.request_number}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[request_numberd4d19?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_accessreqdoctable_v1Props) && !table_groupbcd33.request_number){
    settable_groupbcd33((pre:any)=>({...pre,request_number:dfd_accessreqdoctable_v1Props[0]?.request_number}));
  }
  },[dfd_accessreqdoctable_v1Props])

  if (request_numberd4d19?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.request_number : (table_groupbcd33?.request_number || ""))}
</Text>
  </div>
  )
}
