...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "0ae36fc2fac830d0bccc842437a3a32b",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "1d6f463a98e9d570ba634b66939b39db",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "9344ebbd7206df1eafebc68e420972ff",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "3d59ffb93ced9107c36df626c8e62e44",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "917314f52e1c71a8b6a039500a913bbc",
    "name": "Created By",
    "meta": {
      "sort": true
    },
...
const Tablerequest_doc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const unlockRecord = async (id: number) => {
    ...
    /// setrequest_doc_tablea9098Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(request_doc_tablea9098.map((item: any) => item.access_req_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      request_doc_tablea9098.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = request_doc_tablea9098
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.access_req_id) ? index.toString() : null
          )
          .filter(Boolean)

        setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        return
      }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrequest_doc_tablea9098Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(accessrequestdocument_v1Props.length > 0){
      for(let i=0;i< accessrequestdocument_v1Props.length;i++){
        if(accessrequestdocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:accessReqDocTable:AFVK:v1"){
          // delete accessrequestdocument_v1Props[i].DFDkey;
          let temp=structuredClone(accessrequestdocument_v1Props[i])
          delete temp?.DFDkey
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["employee_id"]:request_doc_tablea9098Props?.presetValues?.employee_id}
    ...
    if(!request_doc_tablea9098Props?.presetValues || Object.keys(request_doc_tablea9098Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:accessRequestDocument:AFVK:v1:UO', 
                nodeId: 'd902a9ff8e44f6c60f7ec1f0f80a9098',
                elementId: 'd902a9ff8e44f6c60f7ec1f0f80a9098'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:accessRequestDocument:AFVK:v1',
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrequest_doc_tablea9098(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in accessrequestdocument_v1?.request_doc_table && accessrequestdocument_v1?.request_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(accessrequestdocument_v1?.request_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(accessrequestdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,request_doc_table:filteredData},{request_doc_table:accessrequestdocument_v1?.request_doc_table})
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(request_doc_tablea9098))
    {
      GetTableDetails()
    }
  }, [request_doc_tablea9098])
  ...
  useEffect(() => {
    ...
    if (request_doc_tablea9098Props.filterInitalLoad) return;

    const filterControllers = request_doc_tablea9098Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['request_doc_tablea9098']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['request_doc_tablea9098']).filter((k: string) => jsonEntry['request_doc_tablea9098'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(request_doc_tablea9098Props.filterProps);
      return;
    }
    ...
    setrequest_doc_tablea9098Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(request_doc_tablea9098Props.filterProps);

  }, [
    request_doc_tablea9098Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    request_doc_tablea9098Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(request_doc_tablea9098Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(request_doc_tablea9098Props.searchFilter) || {};
...
}, [request_doc_tablea9098Props.searchFilter])
  ...
  useEffect(() => {
    if (request_doc_tablea9098Props?.clearData === true) {
      if (lastLockedDataRef.current?.primaryKeys?.length) {
        lastLockedDataRef.current.primaryKeys.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrequest_doc_tablea9098Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [request_doc_tablea9098Props?.clearData])
  useEffect(() => {
  ...
  }, [request_doc_tablea9098Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = request_doc_tablea9098Props?.selectedIds[request_doc_tablea9098Props?.selectedIds?.length-1]
    if(Array.isArray(request_doc_tablea9098) && request_doc_tablea9098.length>0)
    {
      let data = request_doc_tablea9098[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "access_req_id",
  ...
  useEffect(() => {
    if (request_doc_tablea9098Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (request_doc_tablea9098Props?.selectedIds?.length == 0){
      handleOnRowClick({},request_doc_tablea9098Props?.selectedIds)
    }
  }, [request_doc_tablea9098Props?.selectedIds])
    ...
    let filterProps4:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData4 = await getFilterProps(filterProps4,{...doc_attached_groupd9ca3,...table_groupbcd33,...request_doc_tablea9098});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(request_doc_tablea9098?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&request_doc_tablea9098?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](request_doc_tablea9098?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: request_doc_tablea9098?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&request_doc_tablea9098?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](request_doc_tablea9098?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: request_doc_tablea9098?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (request_doc_tablea9098?.isHidden) {
    return <></>
  }
  return(
      ...
      >
        <PageHrmdocumentviewerpage4/>
          ...
          <div
            ...
            id={"d902a9ff8e44f6c60f7ec1f0f80a9098"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={request_doc_tablea9098Props?.selectedIds}  
