...
export default function PageCombodropV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const screenName:string = "combodrop";
  ...
  const {combo_dropdown_v1Props, setcombo_dropdown_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function chart_data_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        ...
        if(combo_dropdown_v1Props.length > 0){
          for(let i=0;i< combo_dropdown_v1Props.length;i++){
            if(combo_dropdown_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:chart_data:AFVK:v1"){
              // delete combo_dropdown_v1Props[i].DFDkey;
              let temp=structuredClone(combo_dropdown_v1Props[i])
              delete temp?.DFDkey
        ...
        if(chart_data_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_chart_data_v1Props(api_paginationData?.data?.records || []);
          ...
          setdfd_chart_data_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (chart_data_v1Data?.data?.dataset) {
           setdfd_chart_data_v1Props(
              Array.isArray(chart_data_v1Data?.data?.dataset?.data)
                 ? chart_data_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.chart_data_v1) {
      chart_data_v1DFD(paginationData)
    }else 
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setcombo_dropdown_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:combo_dropdown:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
      ...
      try {
        ...
        try{
    await chart_data_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setcombo_dropdown_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(combo_dropdown_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(combo_dropdown_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setcombo_dropdown_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
