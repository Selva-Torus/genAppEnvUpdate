...
const TextInputgrade_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobGrade:AFVK:v1|873dabfd41dff5447b4ce1edcd9162a8|c9828be7c1253d1564b9477accef4177"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'grade_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,grade_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrade_information_group162a8((prev: any) => ({ ...prev, grade_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(grade_information_group162a8?.grade_name == "" || grade_information_group162a8?.grade_name == undefined){
      grade_information_group162a8.grade_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, grade_information_group162a8?.grade_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,grade_name:"invalid"}}));
        }
    }else if(grade_information_group162a8?.grade_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +grade_information_group162a8?.grade_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c9828be7c1253d1564b9477accef4177"
      );
      ...
      //     controlId: "c9828be7c1253d1564b9477accef4177",
      //     isTable: false,
      //     from:"TextInputgrade_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'grade_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'grade_name',type:'text'};
      //   type={
      //     name:'grade_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.grade_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.grade_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.grade_name.type
      //   }
      ...
      //   type={
      //     name:'grade_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.grade_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.grade_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.grade_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!grade_information_group162a8?.grade_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "c9828be7c1253d1564b9477accef4177") {
        handleChange({target:{value:grade_information_group162a8Ref?.current?.grade_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c9828be7c1253d1564b9477accef4177") {
        handleBlur({target:{value:grade_information_group162a8Ref?.current?.grade_name||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setgrade_information_group162a8((pre:any)=>({...pre,grade_name:dfd_jobgrade_v1Props.data[0]?.grade_name}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (grade_namef4177?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `7 / 21`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={grade_information_group162a8?.grade_name||""}
         disabled= {grade_namef4177?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Grade Name"
      errorMessage={error}
        validationState={validate?.viewEmployeeJobGrade_v1?.grade_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputgrade_name
