...
const TextInputgrade_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobGrade:AFVK:v1|873dabfd41dff5447b4ce1edcd9162a8|f6a5eca1a1f11f669da097dd038edd22"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,grade_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgrade_information_group162a8((prev: any) => ({ ...prev, grade_code: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(grade_information_group162a8?.grade_code == "" || grade_information_group162a8?.grade_code == undefined){
      grade_information_group162a8.grade_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, grade_information_group162a8?.grade_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,grade_code:"invalid"}}));
        }
    }else if(grade_information_group162a8?.grade_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +grade_information_group162a8?.grade_code);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f6a5eca1a1f11f669da097dd038edd22"
      );
      ...
      //     controlId: "f6a5eca1a1f11f669da097dd038edd22",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!grade_information_group162a8?.grade_code)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "f6a5eca1a1f11f669da097dd038edd22") {
        handleChange({target:{value:grade_information_group162a8Ref?.current?.grade_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f6a5eca1a1f11f669da097dd038edd22") {
        handleBlur({target:{value:grade_information_group162a8Ref?.current?.grade_code||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setgrade_information_group162a8((pre:any)=>({...pre,grade_code:dfd_jobgrade_v1Props.data[0]?.grade_code}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (grade_codeedd22?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={grade_information_group162a8?.grade_code||""}
         disabled= {grade_codeedd22?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobGrade_v1?.grade_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
