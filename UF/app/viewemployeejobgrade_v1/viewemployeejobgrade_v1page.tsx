...
export default function PageViewemployeejobgradeV1() {
  ...
  const screenName:string = "job grades";
  ...
  const [checkgrade_information_group,setCheckgrade_information_group,]=useState<boolean>(false);
  const [checkcompensation_benfits_group,setCheckcompensation_benfits_group,]=useState<boolean>(false);
  const [checkhr_policies_group,setCheckhr_policies_group,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      ...
      appraisalcyclecombo_v1:false,
    });
    async function jobgrade_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let jobgrade_v1Body:te_refreshDto={
        ...
        if (encryptionFlagPage) {          
          jobgrade_v1Body["dpdKey"] = encryptionDpd;
          jobgrade_v1Body["method"] = encryptionMethod;
        }
        if(viewemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< viewemployeejobgrade_v1Props.length;i++){
            if(viewemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobGrade:AFVK:v1"){
              // delete viewemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
            }           
          }
          jobgrade_v1Body['filterData'] = filterData;
        }
        const jobgrade_v1Data:any=await AxiosService.post("/te/eventEmitter",jobgrade_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=jobgrade_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(jobgrade_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_jobgrade_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_jobgrade_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (jobgrade_v1Data?.data?.dataset) {
           setdfd_jobgrade_v1Props(
              Array.isArray(jobgrade_v1Data?.data?.dataset?.data)
                 ? jobgrade_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.jobgrade_v1) {
      jobgrade_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.jobgrade_v1= true
  },[refetch?.jobgrade_v1])
    async function currencycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(viewemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< viewemployeejobgrade_v1Props.length;i++){
            if(viewemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:currencyCombo:AFVK:v1"){
              // delete viewemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function appraisalcyclecombo_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let appraisalcyclecombo_v1Body:te_refreshDto={
          key: "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:appraisalCycleCombo:AFVK:v1"+":",
          refreshFlag: "Y",
          count:parseInt(pagination?.count) || 10,
          page:parseInt(pagination?.page) || 1
        }
        if (encryptionFlagPage) {          
          appraisalcyclecombo_v1Body["dpdKey"] = encryptionDpd;
          appraisalcyclecombo_v1Body["method"] = encryptionMethod;
        }
        if(viewemployeejobgrade_v1Props.length > 0){
          for(let i=0;i< viewemployeejobgrade_v1Props.length;i++){
            if(viewemployeejobgrade_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:appraisalCycleCombo:AFVK:v1"){
              // delete viewemployeejobgrade_v1Props[i].DFDkey;
              let temp=structuredClone(viewemployeejobgrade_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
          ...
          appraisalcyclecombo_v1Body['filterData'] = filterData;
        }
        const appraisalcyclecombo_v1Data:any=await AxiosService.post("/te/eventEmitter",appraisalcyclecombo_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=appraisalcyclecombo_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(appraisalcyclecombo_v1Data?.data?.dataset === 'Bulk Data Processing'){
          if(filterData.length>0){
            const api_paginationBody: api_paginationDto = {
          key: dstKey,
          count:parseInt(pagination?.count) || 10,
        ...
        setdfd_appraisalcyclecombo_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_appraisalcyclecombo_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (appraisalcyclecombo_v1Data?.data?.dataset) {
           setdfd_appraisalcyclecombo_v1Props(
              Array.isArray(appraisalcyclecombo_v1Data?.data?.dataset?.data)
                 ? appraisalcyclecombo_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
                    Object.entries(obj || {}).map(([key, value]) => [
                      key.toLowerCase(),
                      value
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.appraisalcyclecombo_v1) {
      appraisalcyclecombo_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.appraisalcyclecombo_v1= true
  },[refetch?.appraisalcyclecombo_v1])
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    const { getAftfactLevelRule } = await import("../utils/evaluateDecisionTable");
    let result :any =await getAftfactLevelRule(rule,data,allRuleData)
    setviewemployeejobgrade_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      ...
      try {
        ...
        try{
    await jobgrade_v1DFD(pagination)
    await currencycombo_v1DFD(pagination)
    await appraisalcyclecombo_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'grade_information_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckgrade_information_group(true)
            }
            ...
            if(nodes?.groupName == 'hr_policies_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckhr_policies_group(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setviewemployeejobgrade_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(viewemployeejobgrade_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(viewemployeejobgrade_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setviewemployeejobgrade_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
