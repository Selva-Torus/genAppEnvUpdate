 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "79dd19963cd315c51626e9cac088e1f2"
    );
  ...
  },[appraisal_cycle8e1f2?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      sethr_policies_groupa0e79((prev: any) => ({ ...prev,appraisal_cycle8e1f2: value }))
      setIsRequredData(false)
    }else{
      let temp:any = hr_policies_groupa0e79;
      delete temp.appraisal_cycle;
      delete temp.APPRAISAL_CYCLE;
      delete temp.appraisal_cycle8e1f2;
      sethr_policies_groupa0e79(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,appraisal_cycle:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(hr_policies_groupa0e79.appraisal_cycle){
      if(Array.isArray(dfd_appraisalcyclecombo_v1Props)){
        if(dfd_appraisalcyclecombo_v1Props?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)){
          setdropdownValue([dfd_appraisalcyclecombo_v1Props?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)?.appraisal_cycle])
          tempValue=dfd_appraisalcyclecombo_v1Props?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)?.appraisal_cycle
        }else{
          setdropdownValue([hr_policies_groupa0e79.appraisal_cycle])
          tempValue=hr_policies_groupa0e79.appraisal_cycle
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{appraisal_cycle:hr_policies_groupa0e79.appraisal_cycle}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)?.appraisal_cycle ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.appraisal_cycle === hr_policies_groupa0e79.appraisal_cycle)?.appraisal_cycle
      }else{
        setdropdownValue([hr_policies_groupa0e79.appraisal_cycle])
        tempValue=hr_policies_groupa0e79.appraisal_cycle
      }   
    ...
    if(appraisal_cycle8e1f2?.trigger == true)
    {
      await handlechange(tempValue)
      setappraisal_cycle8e1f2((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[hr_policies_groupa0e79.appraisal_cycle, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_appraisalcyclecombo_v1Props) && dfd_appraisalcyclecombo_v1Props?.length == 1){
    // sethr_policies_groupa0e79((pre:any)=>({...pre,appraisal_cycle:dfd_appraisalcyclecombo_v1Props[0]?.appraisal_cycle}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      sethr_policies_groupa0e79((prev: any) => ({ ...prev, appraisal_cycle: getMapperDetailsBindValues[value],APPRAISAL_CYCLE: getMapperDetails}))
         setIsRequredData(false)
    } else {
       sethr_policies_groupa0e79((prev: any) => ({ ...prev, appraisal_cycle: '', appraisal_cycle8e1f2: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,appraisal_cycle:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(hr_policies_groupa0e79?.appraisal_cycle == "" || hr_policies_groupa0e79?.appraisal_cycle == undefined){
      hr_policies_groupa0e79.appraisal_cycle = "";
      const validate:any = v.safeParse(schema, hr_policies_groupa0e79?.appraisal_cycle);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,appraisal_cycle:"invalid"}}));
        }
    }else if(hr_policies_groupa0e79?.appraisal_cycle !== ""){
    const validate:any = v.safeParse(schema, hr_policies_groupa0e79?.appraisal_cycle);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,appraisal_cycle:"invalid"}}));
  ...
  useEffect(() => { hr_policies_groupa0e79Ref.current = hr_policies_groupa0e79; }, [hr_policies_groupa0e79]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "79dd19963cd315c51626e9cac088e1f2") {
        handleClick(hr_policies_groupa0e79Ref?.current?.appraisal_cycle8e1f2?hr_policies_groupa0e79Ref?.current?.appraisal_cycle8e1f2:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "79dd19963cd315c51626e9cac088e1f2");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     sethr_policies_groupa0e79((pre:any)=>({...pre,appraisal_cycle:""}))
    else
      setInitialCount(1)
  },[appraisal_cycle8e1f2?.refresh])
  

  if (appraisal_cycle8e1f2?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {appraisal_cycle8e1f2?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={hr_policies_groupa0e79?.appraisal_cycle8e1f2 ? [hr_policies_groupa0e79?.appraisal_cycle8e1f2] : (hr_policies_groupa0e79?.appraisal_cycle ? dropdownValue : [])}
        validationState={validate?.viewEmployeeJobGrade_v1?.appraisal_cycle ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
