...
const TextInputprobation_period_days = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobGrade:AFVK:v1|c496957cdcf54d02b06872f0f45f2a70|properties.probation_period_days"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobGrade:AFVK:v1|e32f597b364718eea74271361f6a0e79|185e1bd8465b15292e1533497adb3f51"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'probation_period_days',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,probation_period_days:undefined}}));
    if(dynamicStateandType.type=="number"){
    sethr_policies_groupa0e79((prev: any) => ({ ...prev, probation_period_days: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(hr_policies_groupa0e79?.probation_period_days == "" || hr_policies_groupa0e79?.probation_period_days == undefined){
      hr_policies_groupa0e79.probation_period_days = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, hr_policies_groupa0e79?.probation_period_days);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,probation_period_days:"invalid"}}));
        }
    }else if(hr_policies_groupa0e79?.probation_period_days !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +hr_policies_groupa0e79?.probation_period_days);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "185e1bd8465b15292e1533497adb3f51"
      );
      ...
      //     controlId: "185e1bd8465b15292e1533497adb3f51",
      //     isTable: false,
      //     from:"TextInputprobation_period_days",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'probation_period_days', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'probation_period_days',type:'text'};
      //   type={
      //     name:'probation_period_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.probation_period_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.probation_period_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.probation_period_days.type
      //   }
      ...
      //   type={
      //     name:'probation_period_days',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.probation_period_days.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.probation_period_days.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.probation_period_days.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!hr_policies_groupa0e79?.probation_period_days)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "185e1bd8465b15292e1533497adb3f51") {
        handleChange({target:{value:hr_policies_groupa0e79Ref?.current?.probation_period_days||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "185e1bd8465b15292e1533497adb3f51") {
        handleBlur({target:{value:hr_policies_groupa0e79Ref?.current?.probation_period_days||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      sethr_policies_groupa0e79((pre:any)=>({...pre,probation_period_days:dfd_jobgrade_v1Props.data[0]?.probation_period_days}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (probation_period_daysb3f51?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `8 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={hr_policies_groupa0e79?.probation_period_days||""}
         disabled= {probation_period_daysb3f51?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Probation Period Days (in Days)"
      errorMessage={error}
        validationState={validate?.viewEmployeeJobGrade_v1?.probation_period_days ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputprobation_period_days
