...
const TextInputmin_salary = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewEmployeeJobGrade:AFVK:v1|ff5ef8668484fc5fdc076b33084a044d|6e03a24965146369e659aa9041e22457"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'min_salary',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,min_salary:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompensation_benfits_groupa044d((prev: any) => ({ ...prev, min_salary: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(compensation_benfits_groupa044d?.min_salary == "" || compensation_benfits_groupa044d?.min_salary == undefined){
      compensation_benfits_groupa044d.min_salary = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, compensation_benfits_groupa044d?.min_salary);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewEmployeeJobGrade_v1:{...pre?.viewEmployeeJobGrade_v1,min_salary:"invalid"}}));
        }
    }else if(compensation_benfits_groupa044d?.min_salary !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +compensation_benfits_groupa044d?.min_salary);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6e03a24965146369e659aa9041e22457"
      );
      ...
      //     controlId: "6e03a24965146369e659aa9041e22457",
      //     isTable: false,
      //     from:"TextInputmin_salary",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'min_salary', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'min_salary',type:'text'};
      //   type={
      //     name:'min_salary',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.min_salary.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.min_salary.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.min_salary.type
      //   }
      ...
      //   type={
      //     name:'min_salary',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.min_salary.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.min_salary.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.min_salary.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!compensation_benfits_groupa044d?.min_salary)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "6e03a24965146369e659aa9041e22457") {
        handleChange({target:{value:compensation_benfits_groupa044dRef?.current?.min_salary||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6e03a24965146369e659aa9041e22457") {
        handleBlur({target:{value:compensation_benfits_groupa044dRef?.current?.min_salary||""}});
      }
    };
  ...
  if(dfd_jobgrade_v1Props?.setSearchFilters && dfd_jobgrade_v1Props?.data)
  {
    if(Array.isArray(dfd_jobgrade_v1Props.data) && dfd_jobgrade_v1Props.data.length > 0){
      setcompensation_benfits_groupa044d((pre:any)=>({...pre,min_salary:dfd_jobgrade_v1Props.data[0]?.min_salary}));
    }
  }
  },[dfd_jobgrade_v1Props?.setSearchFilters])
  if (min_salary22457?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compensation_benfits_groupa044d?.min_salary||""}
         disabled= {min_salary22457?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewEmployeeJobGrade_v1?.min_salary ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputmin_salary
