 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "f592407a8efb1d338f2a2e01fcb28dff"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setcompensation_benfits_groupa044d((pre:any)=>({...pre,promotion_eligible:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[promotion_eligible28dff?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_jobgrade_v1Props) && dfd_jobgrade_v1Props?.length == 1){
      setcompensation_benfits_groupa044d((pre:any)=>({...pre,promotion_eligible:dfd_jobgrade_v1Props[0]?.promotion_eligible}))
    }
  },[dfd_jobgrade_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setcompensation_benfits_groupa044d((prev: any) => ({ ...prev, promotion_eligible: checked }));
  ...
  if (promotion_eligible28dff?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="top"
        disabled= {promotion_eligible28dff?.isDisabled ? true : false}
        checkedContent="Yes"                                                                                                                                             
        uncheckedContent="No"
        checked={compensation_benfits_groupa044d?.promotion_eligible || false} 
