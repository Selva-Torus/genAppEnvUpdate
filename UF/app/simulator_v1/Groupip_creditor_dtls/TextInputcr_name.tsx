...
const TextInputcr_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,cr_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_name: +e.target.value }));
    }
    else{
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "7e4c2c50f0ff4bc09017b7d1f2989142"
      );
      ...
      //     controlId: "7e4c2c50f0ff4bc09017b7d1f2989142",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "7e4c2c50f0ff4bc09017b7d1f2989142") {
        handleChange({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "7e4c2c50f0ff4bc09017b7d1f2989142") {
        handleBlur({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_name||""}});
      }
    };
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={ip_creditor_dtls1ade4?.cr_name||""}
         disabled= {cr_name89142?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.cr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
