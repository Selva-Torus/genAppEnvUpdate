...
const TextInputcr_account = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,cr_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_account: +e.target.value }));
    }
    else{
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_account: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5711ffd6f24a4880bb3d5db643bb0c70"
      );
      ...
      //     controlId: "5711ffd6f24a4880bb3d5db643bb0c70",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5711ffd6f24a4880bb3d5db643bb0c70") {
        handleChange({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_account||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5711ffd6f24a4880bb3d5db643bb0c70") {
        handleBlur({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_account||""}});
      }
    };
  ...
  if (cr_accountb0c70?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `7 / 12`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ip_creditor_dtls1ade4?.cr_account||""}
         disabled= {cr_accountb0c70?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.cr_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
