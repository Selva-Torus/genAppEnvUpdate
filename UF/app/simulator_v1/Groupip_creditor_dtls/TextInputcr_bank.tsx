...
const TextInputcr_bank = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cr_bank',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,cr_bank:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_bank: +e.target.value }));
    }
    else{
    setip_creditor_dtls1ade4((prev: any) => ({ ...prev, cr_bank: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "2e291be1d3304d83918b4451f84e46ca"
      );
      ...
      //     controlId: "2e291be1d3304d83918b4451f84e46ca",
      //     isTable: false,
      //     from:"TextInputcr_bank",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cr_bank', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cr_bank',type:'text'};
      //   type={
      //     name:'cr_bank',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank.type
      //   }
      ...
      //   type={
      //     name:'cr_bank',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "2e291be1d3304d83918b4451f84e46ca") {
        handleChange({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_bank||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2e291be1d3304d83918b4451f84e46ca") {
        handleBlur({target:{value:ip_creditor_dtls1ade4Ref?.current?.cr_bank||""}});
      }
    };
  ...
  if (cr_banke46ca?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 25`,gridRow: `19 / 24`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ip_creditor_dtls1ade4?.cr_bank||""}
         disabled= {cr_banke46ca?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.cr_bank ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcr_bank
