...
import Groupsimulator_tab_group  from "../Groupsimulator_tab_group/Groupsimulator_tab_group";
...
const Groupsimulator_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "93a06ea5fcb64330aa38284284f0541e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsimulator_main_group0541eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("simulator_tab_group")){
        setsimulator_tab_groupfd732((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(simulator_tab_groupfd732?.isDisabled==null)
      {
        setsimulator_tab_groupfd732((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const simulator_main_group0541eRef = useRef<any>(null);
  const handleClearSearch = () => {
    simulator_main_group0541eRef.current?.setSearchParams();
    simulator_main_group0541eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(simulator_main_group0541e) && Object.keys(simulator_main_group0541e)?.length>0)
      {
        setsimulator_main_group0541e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [simulator_main_group0541eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setsimulatorprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"simulator_main_group"}))
        }}
    >
        {allowedComponent.includes("simulator_tab_group")  &&<Groupsimulator_tab_group  
...
export default Groupsimulator_main_group
