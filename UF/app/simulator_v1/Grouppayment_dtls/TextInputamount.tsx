...
const TextInputamount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpayment_dtls30132((prev: any) => ({ ...prev, amount: +e.target.value }));
    }
    else{
    setpayment_dtls30132((prev: any) => ({ ...prev, amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "641c2dd86b6348dca6a5cca638a07414"
      );
      ...
      //     controlId: "641c2dd86b6348dca6a5cca638a07414",
      //     isTable: false,
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'amount', type: 'number'});
      }
      ...
      //   type={
      //     name:'amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.amount.type
      //   }
      ...
      //   type={
      //     name:'amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "641c2dd86b6348dca6a5cca638a07414") {
        handleChange({target:{value:payment_dtls30132Ref?.current?.amount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "641c2dd86b6348dca6a5cca638a07414") {
        handleBlur({target:{value:payment_dtls30132Ref?.current?.amount||""}});
      }
    };
  ...
  if (amount07414?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 25`,gridRow: `13 / 18`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={payment_dtls30132?.amount||""}
         disabled= {amount07414?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
