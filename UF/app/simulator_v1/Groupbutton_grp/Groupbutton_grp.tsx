...
import Buttonsubmit_ip  from "./Buttonsubmit_ip";
...
const Groupbutton_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7c32dd49a6594766b9b4f718e9f7b9b7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbutton_grp7b9b7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("submit_ip")){
        setsubmit_ip98bbf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(submit_ip98bbf?.isDisabled==null)
      {
        setsubmit_ip98bbf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const button_grp7b9b7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    button_grp7b9b7Ref.current?.setSearchParams();
    button_grp7b9b7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(button_grp7b9b7) && Object.keys(button_grp7b9b7)?.length>0)
      {
        setbutton_grp7b9b7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [button_grp7b9b7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setsimulatorprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"button_grp"}))
        }}
    >
        {        ((ruleData?.length>0 && "submit_ip" in ButtonGoRuleData)?ButtonGoRuleData["submit_ip"]:true) && 
          allowedControls.includes("submit_ip")  ?            <Buttonsubmit_ip tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}

export default Groupbutton_grp
