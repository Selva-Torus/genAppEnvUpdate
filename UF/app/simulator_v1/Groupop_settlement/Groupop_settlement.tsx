...
import Groupop_settlement_grp  from "../Groupop_settlement_grp/Groupop_settlement_grp";
...
const Groupop_settlement = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "59a3eba6f37d45099492c40f55ee399b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setop_settlemente399bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("op_settlement_grp")){
        setop_settlement_grpb706d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(op_settlement_grpb706d?.isDisabled==null)
      {
        setop_settlement_grpb706d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(simulator_tab_groupfd732?.simulator_tab_group === "op_settlement"){
    handleOnChange()
    }
  ...
  const op_settlemente399bRef = useRef<any>(null);
  const handleClearSearch = () => {
    op_settlemente399bRef.current?.setSearchParams();
    op_settlemente399bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(op_settlemente399b) && Object.keys(op_settlemente399b)?.length>0)
      {
        setop_settlemente399b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [op_settlemente399bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setsimulatorprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"op_settlement"}))
        }}
    >
        {allowedComponent.includes("op_settlement_grp")  &&<Groupop_settlement_grp  
...
export default Groupop_settlement
