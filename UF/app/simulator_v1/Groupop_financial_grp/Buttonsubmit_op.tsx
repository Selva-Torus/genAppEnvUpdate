...
const Buttonsubmit_op = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const op_financial_grp8a39aRef = useRef(op_financial_grp8a39a);
  useEffect(() => {
    op_financial_grp8a39aRef.current = op_financial_grp8a39a;
  ...
  const op_financial_grp8a39aPropsRef = useRef(op_financial_grp8a39aProps);
  useEffect(() => {
    op_financial_grp8a39aPropsRef.current = op_financial_grp8a39aProps;
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...op_financial_grp8a39aRef.current};
      let parentRowSpan = 70;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "1687ee1f949f41fdbe50d7088248a39a",
        "d747095e02c64729bb337dadb98cf1e2"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "submit_opcf1e2") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "d747095e02c64729bb337dadb98cf1e2") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "submit_opcf1e2");
    return () => {
  ...
  useEffect(()=>{
    if (!submit_opcf1e2?.trigger) return;
      if(submit_opcf1e2?.trigger){
      setsubmit_opcf1e2((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[submit_opcf1e2?.trigger])

  useEffect(()=>{
    if(submit_opcf1e2?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[submit_opcf1e2?.refresh])
  ...
  async function handleSavef1e2_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const op_financial_grp8a39aProps = op_financial_grp8a39aPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.simulatorProcessUi_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(op_financial_grp8a39aRef.current);
      ...
      let tagetKey:string="CK:CT005:FNGK:AF:FNK:PF-PFD:CATK:GSS:AFGK:RTGS:AFK:simulatorProcess:AFVK:v1|89259b2d20b944a98b636ae4f0822fe7"
      let uf_getPFDetails:any={
        key: "CK:CT005:FNGK:AF:FNK:PF-PFD:CATK:GSS:AFGK:RTGS:AFK:simulatorProcess:AFVK:v1|89259b2d20b944a98b636ae4f0822fe7"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(op_financial_grp8a39a?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "d747095e02c64729bb337dadb98cf1e2",
  "type": "button",
  "name": "submit_op",
  "label": "submit_op",
  "sequence": 1,
  "children": [
    {
      "id": "d747095e02c64729bb337dadb98cf1e2.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "d747095e02c64729bb337dadb98cf1e2.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "d747095e02c64729bb337dadb98cf1e2.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
          ...
          "targetKey": [
            "CK:CT005:FNGK:AF:FNK:PF-PFD:CATK:GSS:AFGK:RTGS:AFK:simulatorProcess:AFVK:v1|89259b2d20b944a98b636ae4f0822fe7"
          ]
        }
      ]
    }
      ...
      sourceId+= "|"+"1687ee1f949f41fdbe50d7088248a39a";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:simulatorProcessUi:AFVK:v1"+"|"+"1687ee1f949f41fdbe50d7088248a39a"+"|"+eventProperty.id;
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : op_financial_grp8a39a?.upId? [op_financial_grp8a39a?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : op_financial_grp8a39a?.upId? [op_financial_grp8a39a?.upId ] : op_financial_grp8a39a?.trs_process_id? [op_financial_grp8a39a?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "submit_op"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...op_financial_grp8a39aRef.current, ...nullFilter(simulator_main_group0541e), ...nullFilter(simulator_tab_groupfd732), ...nullFilter(op_financial4735b), ...nullFilter(op_settlemente399b), ...nullFilter(op_settlement_grpb706d), ...nullFilter(ip_financial66005), ...nullFilter(ip_debtor_dtls8143c), ...nullFilter(ip_creditor_dtls1ade4), ...nullFilter(payment_dtls30132), ...nullFilter(addionl_info43014), ...nullFilter(button_grp7b9b7) });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"1687ee1f949f41fdbe50d7088248a39a",
              controlId:"d747095e02c64729bb337dadb98cf1e2"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (op_financial_grp8a39aProps.ssKey !== '' && op_financial_grp8a39aProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = op_financial_grp8a39aProps.ssKey;          
    }
    te_eventEmitterBody["lock"] = actionLockData;
    if (op_financial_grp8a39aProps?.tableName) {
      te_eventEmitterBody["tableName"] = op_financial_grp8a39aProps?.tableName;
    }
    if (encryptionFlagCont) {
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setop_financial_grp8a39a((prev: any) => ({ ...prev, submit_op: true }));
        //onClick

    //eventEmitter
    await handleSavef1e2_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setop_financial_grp8a39a((prev: any) => ({ ...prev, submit_op: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (submit_opcf1e2?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `22 / 25`,gridRow: `57 / 67`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='action'
          disabled= {submit_opcf1e2?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdOutlineSave"
...
export default Buttonsubmit_op
