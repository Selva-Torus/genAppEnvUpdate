...
const TextInputproduct_code_op = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'product_code_op',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,product_code_op:undefined}}));
    if(dynamicStateandType.type=="number"){
    setop_financial_grp8a39a((prev: any) => ({ ...prev, product_code_op: +e.target.value }));
    }
    else{
    setop_financial_grp8a39a((prev: any) => ({ ...prev, product_code_op: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a2a289723fe043c38da47ef22408fcb1"
      );
      ...
      //     controlId: "a2a289723fe043c38da47ef22408fcb1",
      //     isTable: false,
      //     from:"TextInputproduct_code_op",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'product_code_op', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'product_code_op',type:'text'};
      //   type={
      //     name:'product_code_op',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_code_op.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_code_op.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_code_op.type
      //   }
      ...
      //   type={
      //     name:'product_code_op',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_code_op.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_code_op.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_code_op.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a2a289723fe043c38da47ef22408fcb1") {
        handleChange({target:{value:op_financial_grp8a39aRef?.current?.product_code_op||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a2a289723fe043c38da47ef22408fcb1") {
        handleBlur({target:{value:op_financial_grp8a39aRef?.current?.product_code_op||""}});
      }
    };
  ...
  if (product_code_op8fcb1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `2 / 10`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
        {isRequredData && <span style={{ color: 'red' }}>*</span>}
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-lg"
        label={keyset("Product Code")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={op_financial_grp8a39a?.product_code_op||""}
        disabled={ true }
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.product_code_op ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputproduct_code_op
