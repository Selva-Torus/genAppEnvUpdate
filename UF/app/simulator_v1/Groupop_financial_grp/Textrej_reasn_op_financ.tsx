...
const Textrej_reasn_op_financ = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {rej_reasn_op_financ13f05Props, setrej_reasn_op_financ13f05Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[rej_reasn_op_financ13f05?.refresh])

  if (rej_reasn_op_financ13f05?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 6`,gridRow: `48 / 55`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset("Reject Reason")}
</Text>
  </div>
  )
}

export default Textrej_reasn_op_financ
