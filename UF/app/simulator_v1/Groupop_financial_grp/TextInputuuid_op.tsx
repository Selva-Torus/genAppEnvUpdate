...
const TextInputuuid_op = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'uuid_op',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,uuid_op:undefined}}));
    if(dynamicStateandType.type=="number"){
    setop_financial_grp8a39a((prev: any) => ({ ...prev, uuid_op: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(op_financial_grp8a39a?.uuid_op == "" || op_financial_grp8a39a?.uuid_op == undefined){
      op_financial_grp8a39a.uuid_op = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, op_financial_grp8a39a?.uuid_op);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,uuid_op:"invalid"}}));
        }
    }else if(op_financial_grp8a39a?.uuid_op !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +op_financial_grp8a39a?.uuid_op);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "910a728a350146389d957c2b7974c851"
      );
      ...
      //     controlId: "910a728a350146389d957c2b7974c851",
      //     isTable: false,
      //     from:"TextInputuuid_op",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'uuid_op', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'uuid_op',type:'text'};
      //   type={
      //     name:'uuid_op',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.uuid_op.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.uuid_op.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.uuid_op.type
      //   }
      ...
      //   type={
      //     name:'uuid_op',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.uuid_op.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.uuid_op.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.uuid_op.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!op_financial_grp8a39a?.uuid_op)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "910a728a350146389d957c2b7974c851") {
        handleChange({target:{value:op_financial_grp8a39aRef?.current?.uuid_op||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "910a728a350146389d957c2b7974c851") {
        handleBlur({target:{value:op_financial_grp8a39aRef?.current?.uuid_op||""}});
      }
    };
  ...
  if (uuid_op4c851?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `29 / 37`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={op_financial_grp8a39a?.uuid_op||""}
         disabled= {uuid_op4c851?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.uuid_op ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputuuid_op
