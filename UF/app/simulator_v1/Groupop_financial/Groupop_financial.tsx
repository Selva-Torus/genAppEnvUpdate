...
import Groupop_financial_grp  from "../Groupop_financial_grp/Groupop_financial_grp";
...
const Groupop_financial = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "94fd595ab7564b53adc085e5ad24735b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setop_financial4735bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("op_financial_grp")){
        setop_financial_grp8a39a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(op_financial_grp8a39a?.isDisabled==null)
      {
        setop_financial_grp8a39a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(simulator_tab_groupfd732?.simulator_tab_group === "op_financial"){
    handleOnChange()
    }
  ...
  const op_financial4735bRef = useRef<any>(null);
  const handleClearSearch = () => {
    op_financial4735bRef.current?.setSearchParams();
    op_financial4735bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(op_financial4735b) && Object.keys(op_financial4735b)?.length>0)
      {
        setop_financial4735b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [op_financial4735bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setsimulatorprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"op_financial"}))
        }}
    >
        {allowedComponent.includes("op_financial_grp")  &&<Groupop_financial_grp  
...
export default Groupop_financial
