...
const TextInputop_setl_message_type = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'op_setl_message_type',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,op_setl_message_type:undefined}}));
    if(dynamicStateandType.type=="number"){
    setop_settlement_grpb706d((prev: any) => ({ ...prev, op_setl_message_type: +e.target.value }));
    }
    else{
    setop_settlement_grpb706d((prev: any) => ({ ...prev, op_setl_message_type: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "fa0cd32090dd435fb9bdc56eccc41552"
      );
      ...
      //     controlId: "fa0cd32090dd435fb9bdc56eccc41552",
      //     isTable: false,
      //     from:"TextInputop_setl_message_type",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'op_setl_message_type', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'op_setl_message_type',type:'text'};
      //   type={
      //     name:'op_setl_message_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_message_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_message_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_message_type.type
      //   }
      ...
      //   type={
      //     name:'op_setl_message_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_message_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_message_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_message_type.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "fa0cd32090dd435fb9bdc56eccc41552") {
        handleChange({target:{value:op_settlement_grpb706dRef?.current?.op_setl_message_type||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "fa0cd32090dd435fb9bdc56eccc41552") {
        handleBlur({target:{value:op_settlement_grpb706dRef?.current?.op_setl_message_type||""}});
      }
    };
  ...
  if (op_setl_message_type41552?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={op_settlement_grpb706d?.op_setl_message_type||""}
        disabled={ true }
        pin='brick-brick'     
        placeholder='CAMT054'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.op_setl_message_type ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputop_setl_message_type
