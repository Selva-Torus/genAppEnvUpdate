...
const TextInputop_setl_product_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'op_setl_product_code',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,op_setl_product_code:undefined}}));
    if(dynamicStateandType.type=="number"){
    setop_settlement_grpb706d((prev: any) => ({ ...prev, op_setl_product_code: +e.target.value }));
    }
    else{
    setop_settlement_grpb706d((prev: any) => ({ ...prev, op_setl_product_code: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "3870d2929b8a4e889d8f3bd710f63258"
      );
      ...
      //     controlId: "3870d2929b8a4e889d8f3bd710f63258",
      //     isTable: false,
      //     from:"TextInputop_setl_product_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'op_setl_product_code', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'op_setl_product_code',type:'text'};
      //   type={
      //     name:'op_setl_product_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_product_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_product_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.op_setl_product_code.type
      //   }
      ...
      //   type={
      //     name:'op_setl_product_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_product_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_product_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.op_setl_product_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "3870d2929b8a4e889d8f3bd710f63258") {
        handleChange({target:{value:op_settlement_grpb706dRef?.current?.op_setl_product_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3870d2929b8a4e889d8f3bd710f63258") {
        handleBlur({target:{value:op_settlement_grpb706dRef?.current?.op_setl_product_code||""}});
      }
    };
  ...
  if (op_setl_product_code63258?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `1 / 10`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={op_settlement_grpb706d?.op_setl_product_code||""}
        disabled={ true }
        pin='brick-brick'     
        placeholder='RTGS_KEEQB'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.op_setl_product_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputop_setl_product_code
