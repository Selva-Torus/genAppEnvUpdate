...
const TextInputdr_bank = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_bank',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,dr_bank:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_bank: +e.target.value }));
    }
    else{
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_bank: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "318ab60f259b4c5ea160ebf1a948143c",
        "c42a70c93b194e0fbc08b7f9500e5943"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:simulatorProcessUi:AFVK:v1",
      //     componentId: "318ab60f259b4c5ea160ebf1a948143c",
      //     controlId: "c42a70c93b194e0fbc08b7f9500e5943",
      //     isTable: false,
      //     from:"TextInputdr_bank",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'dr_bank', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'dr_bank',type:'text'};
      //   type={
      //     name:'dr_bank',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_bank.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_bank.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_bank.type
      //   }
      ...
      //   type={
      //     name:'dr_bank',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_bank.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_bank.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_bank.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const ip_debtor_dtls8143cRef = useRef<any>(ip_debtor_dtls8143c);
  useEffect(() => { ip_debtor_dtls8143cRef.current = ip_debtor_dtls8143c; }, [ip_debtor_dtls8143c]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "c42a70c93b194e0fbc08b7f9500e5943") {
        handleChange({target:{value:ip_debtor_dtls8143cRef?.current?.dr_bank||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c42a70c93b194e0fbc08b7f9500e5943") {
        handleBlur({target:{value:ip_debtor_dtls8143cRef?.current?.dr_bank||""}});
      }
    };
  ...
  if (dr_banke5943?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `14 / 18`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ip_debtor_dtls8143c?.dr_bank||""}
         disabled= {dr_banke5943?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.dr_bank ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdr_bank
