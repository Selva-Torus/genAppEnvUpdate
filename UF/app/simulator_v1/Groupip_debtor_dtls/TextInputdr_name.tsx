...
const TextInputdr_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,dr_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_name: +e.target.value }));
    }
    else{
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "115c518f17eb411cbe43c627c879810f"
      );
      ...
      //     controlId: "115c518f17eb411cbe43c627c879810f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "115c518f17eb411cbe43c627c879810f") {
        handleChange({target:{value:ip_debtor_dtls8143cRef?.current?.dr_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "115c518f17eb411cbe43c627c879810f") {
        handleBlur({target:{value:ip_debtor_dtls8143cRef?.current?.dr_name||""}});
      }
    };
  ...
  if (dr_name9810f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `9 / 13`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ip_debtor_dtls8143c?.dr_name||""}
         disabled= {dr_name9810f?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.dr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
