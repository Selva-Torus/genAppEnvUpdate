...
const TextInputdr_account = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,dr_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_account: +e.target.value }));
    }
    else{
    setip_debtor_dtls8143c((prev: any) => ({ ...prev, dr_account: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a99ddd96dbb94d3f9df4b56422250944"
      );
      ...
      //     controlId: "a99ddd96dbb94d3f9df4b56422250944",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a99ddd96dbb94d3f9df4b56422250944") {
        handleChange({target:{value:ip_debtor_dtls8143cRef?.current?.dr_account||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a99ddd96dbb94d3f9df4b56422250944") {
        handleBlur({target:{value:ip_debtor_dtls8143cRef?.current?.dr_account||""}});
      }
    };
  ...
  if (dr_account50944?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `6 / 25`,gridRow: `4 / 8`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={ip_debtor_dtls8143c?.dr_account||""}
         disabled= {dr_account50944?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.dr_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
