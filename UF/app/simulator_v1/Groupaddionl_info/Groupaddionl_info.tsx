...
import Textremittance_lbl  from "./Textremittance_lbl";
...
const Groupaddionl_info = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {addtional_info60a69, setaddtional_info60a69}= useContext(TotalContext) as TotalContextProps;
  const {remittance_infod7394, setremittance_infod7394}= useContext(TotalContext) as TotalContextProps;
  const {remittance_lbl3a5c2, setremittance_lbl3a5c2}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "10790abd287249dda5368dc7ddd43014");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddionl_info43014Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("addtional_info")){
        setaddtional_info60a69((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(addtional_info60a69?.isDisabled==null)
      {
        setaddtional_info60a69((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_info")){
        setremittance_infod7394((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_infod7394?.isDisabled==null)
      {
        setremittance_infod7394((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_lbl")){
        setremittance_lbl3a5c2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_lbl3a5c2?.isDisabled==null)
      {
        setremittance_lbl3a5c2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['addtional_info'] = addtional_info60a69,
        codeStates['setaddtional_info'] = setaddtional_info60a69,
        codeStates['remittance_info'] = remittance_infod7394,
        codeStates['setremittance_info'] = setremittance_infod7394,
        codeStates['remittance_lbl'] = remittance_lbl3a5c2,
        codeStates['setremittance_lbl'] = setremittance_lbl3a5c2,
  ...
  const addionl_info43014Ref = useRef<any>(null);
  const handleClearSearch = () => {
    addionl_info43014Ref.current?.setSearchParams();
    addionl_info43014Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addionl_info43014) && Object.keys(addionl_info43014)?.length>0)
      {
        setaddionl_info43014({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addionl_info43014Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-3 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setsimulatorprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"addionl_info"}))
        }}
    >
          {allowedControls.includes("addtional_info") ?<Textaddtional_info   /* 60a69 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("remittance_info") ?<TextInputremittance_info   /* d7394 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("remittance_lbl") ?<Textremittance_lbl   /* 3a5c2 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupaddionl_info
