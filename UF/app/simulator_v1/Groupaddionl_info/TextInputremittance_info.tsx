...
const TextInputremittance_info = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,simulatorProcessUi_v1:{...pre?.simulatorProcessUi_v1,remittance_info:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaddionl_info43014((prev: any) => ({ ...prev, remittance_info: +e.target.value }));
    }
    else{
    setaddionl_info43014((prev: any) => ({ ...prev, remittance_info: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "10790abd287249dda5368dc7ddd43014",
        "a1d3f56f692c496e8db05bdff3cd7394"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:simulatorProcessUi:AFVK:v1",
      //     componentId: "10790abd287249dda5368dc7ddd43014",
      //     controlId: "a1d3f56f692c496e8db05bdff3cd7394",
      //     isTable: false,
  ...
  const addionl_info43014Ref = useRef<any>(addionl_info43014);
  useEffect(() => { addionl_info43014Ref.current = addionl_info43014; }, [addionl_info43014]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "a1d3f56f692c496e8db05bdff3cd7394") {
        handleChange({target:{value:addionl_info43014Ref?.current?.remittance_info||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a1d3f56f692c496e8db05bdff3cd7394") {
        handleBlur({target:{value:addionl_info43014Ref?.current?.remittance_info||""}});
      }
    };
  ...
  if (remittance_infod7394?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={addionl_info43014?.remittance_info||""}
         disabled= {remittance_infod7394?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.simulatorProcessUi_v1?.remittance_info ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
