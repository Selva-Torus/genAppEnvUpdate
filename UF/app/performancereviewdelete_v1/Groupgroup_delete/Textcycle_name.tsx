...
const Textcycle_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addperformancereviewmodify_v1Props && !dfd_addperformancereviewmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3ee3b((pre: any) => {
          return { ...pre, cycle_name: api_paginationData.data.records[0]?.cycle_name }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && dfd_addperformancereviewmodify_v1Props && !group_delete3ee3b.cycle_name){
        setgroup_delete3ee3b((pre:any)=>({...pre,cycle_name:dfd_addperformancereviewmodify_v1Props[0]?.cycle_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cycle_namecf2b5?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && !group_delete3ee3b.cycle_name){
    setgroup_delete3ee3b((pre:any)=>({...pre,cycle_name:dfd_addperformancereviewmodify_v1Props[0]?.cycle_name}));
  }
  },[dfd_addperformancereviewmodify_v1Props])

  if (cycle_namecf2b5?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.cycle_name : (group_delete3ee3b?.cycle_name || ""))}
</Text>
  </div>
  )
}

export default Textcycle_name
