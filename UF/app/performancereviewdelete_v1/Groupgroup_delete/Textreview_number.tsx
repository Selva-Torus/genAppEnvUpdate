...
const Textreview_number = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addperformancereviewmodify_v1Props && !dfd_addperformancereviewmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3ee3b((pre: any) => {
          return { ...pre, review_number: api_paginationData.data.records[0]?.review_number }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && dfd_addperformancereviewmodify_v1Props && !group_delete3ee3b.review_number){
        setgroup_delete3ee3b((pre:any)=>({...pre,review_number:dfd_addperformancereviewmodify_v1Props[0]?.review_number}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[review_number5cbc0?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && !group_delete3ee3b.review_number){
    setgroup_delete3ee3b((pre:any)=>({...pre,review_number:dfd_addperformancereviewmodify_v1Props[0]?.review_number}));
  }
  },[dfd_addperformancereviewmodify_v1Props])

  if (review_number5cbc0?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `11 / 16`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.review_number : (group_delete3ee3b?.review_number || ""))}
</Text>
  </div>
  )
}

export default Textreview_number
