...
const Textreview_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[review_id5d984?.refresh])

  if (review_id5d984?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 3`,gridRow: `46 / 47`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.review_id : (group_delete3ee3b?.review_id || ""))}
</Text>
  </div>
  )
}
