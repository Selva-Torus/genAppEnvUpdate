...
const Textreview_status = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_addperformancereviewmodify_v1Props && !dfd_addperformancereviewmodify_v1Props.hasLogicCenter) {
        ...
        setgroup_delete3ee3b((pre: any) => {
          return { ...pre, review_status: api_paginationData.data.records[0]?.review_status }
        })
        }
      }
      else{
      if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && dfd_addperformancereviewmodify_v1Props && !group_delete3ee3b.review_status){
        setgroup_delete3ee3b((pre:any)=>({...pre,review_status:dfd_addperformancereviewmodify_v1Props[0]?.review_status}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[review_status9db4f?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addperformancereviewmodify_v1Props) && !group_delete3ee3b.review_status){
    setgroup_delete3ee3b((pre:any)=>({...pre,review_status:dfd_addperformancereviewmodify_v1Props[0]?.review_status}));
  }
  },[dfd_addperformancereviewmodify_v1Props])

  if (review_status9db4f?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `29 / 34`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.review_status : (group_delete3ee3b?.review_status || ""))}
</Text>
  </div>
  )
}

export default Textreview_status
