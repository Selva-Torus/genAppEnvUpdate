...
const Texttrs_created_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavepolicydoctable_v1Props) && dfd_leavepolicydoctable_v1Props && !group_delete34b87.trs_created_by){
        setgroup_delete34b87((pre:any)=>({...pre,trs_created_by:dfd_leavepolicydoctable_v1Props[0]?.trs_created_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[trs_created_by2301e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicydoctable_v1Props) && !group_delete34b87.trs_created_by){
    setgroup_delete34b87((pre:any)=>({...pre,trs_created_by:dfd_leavepolicydoctable_v1Props[0]?.trs_created_by}));
  }
  },[dfd_leavepolicydoctable_v1Props])

  if (trs_created_by2301e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.trs_created_by : (group_delete34b87?.trs_created_by || ""))}
</Text>
  </div>
  )
}
