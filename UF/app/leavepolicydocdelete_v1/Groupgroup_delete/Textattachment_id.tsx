...
const Textattachment_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavepolicydoctable_v1Props) && dfd_leavepolicydoctable_v1Props && !group_delete34b87.attachment_id){
        setgroup_delete34b87((pre:any)=>({...pre,attachment_id:dfd_leavepolicydoctable_v1Props[0]?.attachment_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[attachment_idb53af?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicydoctable_v1Props) && !group_delete34b87.attachment_id){
    setgroup_delete34b87((pre:any)=>({...pre,attachment_id:dfd_leavepolicydoctable_v1Props[0]?.attachment_id}));
  }
  },[dfd_leavepolicydoctable_v1Props])

  if (attachment_idb53af?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.attachment_id : (group_delete34b87?.attachment_id || ""))}
</Text>
  </div>
  )
}
