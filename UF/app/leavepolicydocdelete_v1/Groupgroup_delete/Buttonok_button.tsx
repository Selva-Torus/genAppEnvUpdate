 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "c774fd6a41008dbdd61a0fe3a75435a9"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_button435a9") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "c774fd6a41008dbdd61a0fe3a75435a9") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_button435a9");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_button435a9?.trigger) return;
      if(ok_button435a9?.trigger){
      setok_button435a9((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_button435a9?.trigger])

  useEffect(()=>{
    if(ok_button435a9?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_button435a9?.refresh])
  ...
  async function handleSave35a9_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_delete34b87Props = group_delete34b87PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.leavePolicyDocDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_delete34b87Ref.current);
      ...
      let lockedKeysLength:number;
        if(group_delete34b87?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "c774fd6a41008dbdd61a0fe3a75435a9",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "c774fd6a41008dbdd61a0fe3a75435a9.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "c774fd6a41008dbdd61a0fe3a75435a9.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "c774fd6a41008dbdd61a0fe3a75435a9.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "c774fd6a41008dbdd61a0fe3a75435a9.1.1.1.2",
              ...
              "children": [
                {
                  "id": "fa9dc617684d49ae4bf52f23d0806d23.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leavePolicyDocDelete:AFVK:v1";
      sourceId+= "|"+"8478877420f1cbcccf3342f2aef34b87";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leavePolicyDocDelete:AFVK:v1"+"|"+"8478877420f1cbcccf3342f2aef34b87"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete34b87?.upId? [group_delete34b87?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete34b87?.upId? [group_delete34b87?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete34b87?.upId? [group_delete34b87?.upId ] : group_delete34b87?.trs_process_id? [group_delete34b87?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete34b87Ref.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"8478877420f1cbcccf3342f2aef34b87",
              controlId:"c774fd6a41008dbdd61a0fe3a75435a9"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete34b87Props.ssKey !== '' && group_delete34b87Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete34b87Props.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete34b87((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave35a9_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete34b87((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_button435a9?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
