...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavepolicydoctable_v1Props) && dfd_leavepolicydoctable_v1Props && !group_delete34b87.doc_name){
        setgroup_delete34b87((pre:any)=>({...pre,doc_name:dfd_leavepolicydoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_namec14df?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicydoctable_v1Props) && !group_delete34b87.doc_name){
    setgroup_delete34b87((pre:any)=>({...pre,doc_name:dfd_leavepolicydoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_leavepolicydoctable_v1Props])

  if (doc_namec14df?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete34b87?.doc_name || ""))}
</Text>
  </div>
  )
}
