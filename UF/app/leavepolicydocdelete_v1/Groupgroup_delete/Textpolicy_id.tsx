...
const Textpolicy_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavepolicydoctable_v1Props) && dfd_leavepolicydoctable_v1Props && !group_delete34b87.policy_id){
        setgroup_delete34b87((pre:any)=>({...pre,policy_id:dfd_leavepolicydoctable_v1Props[0]?.policy_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[policy_idb60b9?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicydoctable_v1Props) && !group_delete34b87.policy_id){
    setgroup_delete34b87((pre:any)=>({...pre,policy_id:dfd_leavepolicydoctable_v1Props[0]?.policy_id}));
  }
  },[dfd_leavepolicydoctable_v1Props])

  if (policy_idb60b9?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.policy_id : (group_delete34b87?.policy_id || ""))}
</Text>
  </div>
  )
}
