...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,addLicense_v1:{...pre?.addLicense_v1,purchase_date:undefined}}));
  if (!date) {
    setvalidity_financial_details_grouped4a1((prev: any) => ({ ...prev, purchase_date: null }));
    return;
  }
  ...
  setvalidity_financial_details_grouped4a1((prev: any) => ({ ...prev, purchase_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "29574182d4e740b9a5af49fdfd7bfe70"
      );
...
useEffect(()=>{
 ...
 },[purchase_datebfe70?.refresh])

useEffect(()=>{
  handleBlur();
},[validateRefetch.value])

if (purchase_datebfe70?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={validity_financial_details_grouped4a1?.purchase_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {purchase_datebfe70?.isDisabled ? true : false}
      disabled= {purchase_datebfe70?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Purchase Date"
      dateValidation=""
      validationState={validate?.addLicense_v1?.purchase_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
