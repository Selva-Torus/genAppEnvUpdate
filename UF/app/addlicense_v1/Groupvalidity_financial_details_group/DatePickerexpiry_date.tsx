...
const DatePickerexpiry_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,addLicense_v1:{...pre?.addLicense_v1,expiry_date:undefined}}));
  if (!date) {
    setvalidity_financial_details_grouped4a1((prev: any) => ({ ...prev, expiry_date: null }));
    return;
  }
  ...
  setvalidity_financial_details_grouped4a1((prev: any) => ({ ...prev, expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "f33c1b073e734a708b7f0c574db5c034"
      );
...
useEffect(()=>{
 ...
 },[expiry_date5c034?.refresh])

useEffect(()=>{
  handleBlur();
},[validateRefetch.value])

if (expiry_date5c034?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `9 / 17`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={validity_financial_details_grouped4a1?.expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {expiry_date5c034?.isDisabled ? true : false}
      disabled= {expiry_date5c034?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Expiry Date"
      dateValidation=""
      validationState={validate?.addLicense_v1?.expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerexpiry_date
