...
const TextInputseats_used = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.seats_used"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicense:AFVK:v1|eec8a7bd5fff4c538b0760c4f4eb5d91|a98bda2f46c34b5780117e62cd5dd434"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'seats_used',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicense_v1:{...pre?.addLicense_v1,seats_used:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_configuration_groupb5d91((prev: any) => ({ ...prev, seats_used: +e.target.value }));
    }
    else{
    setlicense_configuration_groupb5d91((prev: any) => ({ ...prev, seats_used: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a98bda2f46c34b5780117e62cd5dd434"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicense:AFVK:v1",
      //     componentId: "eec8a7bd5fff4c538b0760c4f4eb5d91",
      //     controlId: "a98bda2f46c34b5780117e62cd5dd434",
      //     isTable: false,
      //     from:"TextInputseats_used",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'seats_used', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'seats_used',type:'text'};
      //   type={
      //     name:'seats_used',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.seats_used.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.seats_used.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.seats_used.type
      //   }
      ...
      //   type={
      //     name:'seats_used',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.seats_used.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.seats_used.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.seats_used.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const license_configuration_groupb5d91Ref = useRef<any>(license_configuration_groupb5d91);
  useEffect(() => { license_configuration_groupb5d91Ref.current = license_configuration_groupb5d91; }, [license_configuration_groupb5d91]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleBlur();
    const handlerChange = (id:any) => {
      if (id === "a98bda2f46c34b5780117e62cd5dd434") {
        handleChange({target:{value:license_configuration_groupb5d91Ref?.current?.seats_used||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a98bda2f46c34b5780117e62cd5dd434") {
        handleBlur({target:{value:license_configuration_groupb5d91Ref?.current?.seats_used||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_configuration_groupb5d91((pre:any)=>({...pre,seats_used:dfd_assetsoftwarelicenses_v1Props.data[0]?.seats_used}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (seats_useddd434?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_configuration_groupb5d91?.seats_used||""}
         disabled= {seats_useddd434?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Used Licenses"
      errorMessage={error}
        validationState={validate?.addLicense_v1?.seats_used ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputseats_used
