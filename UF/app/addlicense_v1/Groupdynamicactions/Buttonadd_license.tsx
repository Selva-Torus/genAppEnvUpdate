 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "479bcc53da9a474e9b1345feda13b16e"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("add_license" in dynamicactions67d98Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions67d98Props?.dynamicActionRule?.add_license,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("add_license" in addlicense_v1?.dynamicactions && addlicense_v1?.dynamicactions["add_license"]?.itsHaveArtifact== true)
      {
        setShowFlag(addlicense_v1?.dynamicactions["add_license"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(addlicense_v1?.dynamicactions?.add_license?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "add_license3b16e") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "479bcc53da9a474e9b1345feda13b16e") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "add_license3b16e");
    return () => {
  ...
  useEffect(()=>{
    if (!add_license3b16e?.trigger) return;
      if(add_license3b16e?.trigger){
      setadd_license3b16e((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[add_license3b16e?.trigger])

  useEffect(()=>{
    if(add_license3b16e?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[add_license3b16e?.refresh])
  ...
  async function handleSaveb16e_1_1_1(){
  ...
  return val === 'invalid';
    ...
    try{
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|f39995a0457a430ea32df22e39eea3d6"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|f39995a0457a430ea32df22e39eea3d6"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "479bcc53da9a474e9b1345feda13b16e",
  "type": "button",
  "name": "add_license",
  "label": "add_license",
  "sequence": 1,
  "children": [
    {
      "id": "479bcc53da9a474e9b1345feda13b16e.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "479bcc53da9a474e9b1345feda13b16e.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "479bcc53da9a474e9b1345feda13b16e.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "License Added Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|f39995a0457a430ea32df22e39eea3d6"
          ]
        }
      ]
    }
  ...
  // saveHandler
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"479bcc53da9a474e9b1345feda13b16e"
            };
            if (encryptionFlagCont) {
        ...
        te_saveBody.controlName = "add_license";
        if(mainData?.upId)
        {
        ...
        if(dynamicactions67d98?.upId){
          te_saveBody['upId']= dynamicactions67d98?.upId;
        }
        if(dynamicactions67d98?.upid){
          te_saveBody['upId']= dynamicactions67d98?.upid;
        }
        ...
        if (dynamicactions67d98Props.ssKey !== '' && dynamicactions67d98Props.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactions67d98Props.ssKey;
    ...
    //infoMsg
    toast('License Added Successfully', 'success',false);
    }
    catch(err:any)
    {
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions67d98((prev: any) => ({ ...prev, add_license: true }));
        //onClick

    //saveHandler
    await handleSaveb16e_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions67d98((prev: any) => ({ ...prev, add_license: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [addlicense_v1?.dynamicactions?.add_license,dynamicactions67d98Props?.dynamicActionRule?.add_license,])

 if (add_license3b16e?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {add_license3b16e?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
