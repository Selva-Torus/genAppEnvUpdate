...
const Dropdownlicense_type = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const [license_typeOptions, setlicense_typeOptions] = useState<string[]>([]);
  let category : string
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `license_type`
    let mapperText: string =  `license_type`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "b37dcf96f9bc423c91c7bc55108ae36b"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_licensetypecombo_v1Props.dstKey){
      dstKey = dfd_licensetypecombo_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_licensetypecombo_v1Props && !dfd_licensetypecombo_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[license_typeae36b?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setlicense_information_groupfae34((prev: any) => ({ ...prev, license_type: getMapperDetailsBindValues[value],license_typeae36b: value,LICENSE_TYPE: getMapperDetails }))
      setIsRequredData(false)
    }else{
      let temp:any = license_information_groupfae34;
      delete temp.license_type;
      delete temp.LICENSE_TYPE;
      delete temp.license_typeae36b;
      setlicense_information_groupfae34(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,addLicense_v1:{...pre?.addLicense_v1,license_type:undefined}}));
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(license_information_groupfae34.license_type){
      if(Array.isArray(dfd_licensetypecombo_v1Props)){
        if(dfd_licensetypecombo_v1Props?.find((item: any) => item.license_type === license_information_groupfae34.license_type)){
          setdropdownValue([dfd_licensetypecombo_v1Props?.find((item: any) => item.license_type === license_information_groupfae34.license_type)?.license_type])
          tempValue=dfd_licensetypecombo_v1Props?.find((item: any) => item.license_type === license_information_groupfae34.license_type)?.license_type
        }else{
          setdropdownValue([license_information_groupfae34.license_type])
          tempValue=license_information_groupfae34.license_type
        }
      }else{
        let dstKey:string = dfd_licensetypecombo_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          key:dstKey,
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{license_type:license_information_groupfae34.license_type}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.license_type === license_information_groupfae34.license_type)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.license_type === license_information_groupfae34.license_type)?.license_type ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.license_type === license_information_groupfae34.license_type)?.license_type
      }else{
        setdropdownValue([license_information_groupfae34.license_type])
        tempValue=license_information_groupfae34.license_type
      }   
    ...
    if(license_typeae36b?.trigger!=null)
    {
      handleClick(tempValue)
    }
    else{
      setlicense_typeae36b((pre:any)=>({...pre,trigger:null}))
    }
  }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[license_information_groupfae34.license_type, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_licensetypecombo_v1Props) && dfd_licensetypecombo_v1Props?.length == 1){
    // setlicense_information_groupfae34((pre:any)=>({...pre,license_type:dfd_licensetypecombo_v1Props[0]?.license_type}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setlicense_information_groupfae34((prev: any) => ({ ...prev, license_type: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setlicense_information_groupfae34((prev: any) => ({ ...prev, license_type: '', license_typeae36b: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,addLicense_v1:{...pre?.addLicense_v1,license_type:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.license_type==getMapperDetailsBindValues[value] && items?.license_type==value)) || {}
    selected.current={
      ...selectedObj||{},
      license_type:value,
      license_type:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  useEffect(() => { license_information_groupfae34Ref.current = license_information_groupfae34; }, [license_information_groupfae34]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "b37dcf96f9bc423c91c7bc55108ae36b") {
        handleClick(license_information_groupfae34Ref?.current?.license_typeae36b?license_information_groupfae34Ref?.current?.license_typeae36b:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "b37dcf96f9bc423c91c7bc55108ae36b");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setlicense_information_groupfae34((pre:any)=>({...pre,license_type:""}))
    else
      setInitialCount(1)
  },[license_typeae36b?.refresh])

  if (license_typeae36b?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {license_typeae36b?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={license_typeOptions}
        onLoadMore={loadMore}
        ...
        value={license_information_groupfae34?.license_typeae36b ? [license_information_groupfae34?.license_typeae36b] : (license_information_groupfae34?.license_type ? dropdownValue : [])}
        validationState={validate?.addLicense_v1?.license_type ? "invalid" : undefined}
        />
    </div>
  );
};

export default Dropdownlicense_type;
