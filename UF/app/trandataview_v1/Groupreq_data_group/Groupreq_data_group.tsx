 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("req_jsonviewer")){
        setreq_jsonviewerc80ab((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(req_jsonviewerc80ab?.isDisabled==null)
      {
        setreq_jsonviewerc80ab((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(req_data_group8d4d7) &&
        Object.keys(req_data_group8d4d7)?.length > 0
      ) {
        setreq_data_group8d4d7({})
      }
    } else prevRefreshRef.current = true
  }, [req_data_group8d4d7Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settrandataview_v1((pre:any)=>({...pre,_selectedGroup_:"req_data_group"}))
        }}
    >
