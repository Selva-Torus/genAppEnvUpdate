 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("res_jsonviewer")){
        setres_jsonviewer9d6d1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(res_jsonviewer9d6d1?.isDisabled==null)
      {
        setres_jsonviewer9d6d1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(res_data_group9d75a) &&
        Object.keys(res_data_group9d75a)?.length > 0
      ) {
        setres_data_group9d75a({})
      }
    } else prevRefreshRef.current = true
  }, [res_data_group9d75aProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settrandataview_v1((pre:any)=>({...pre,_selectedGroup_:"res_data_group"}))
        }}
    >
