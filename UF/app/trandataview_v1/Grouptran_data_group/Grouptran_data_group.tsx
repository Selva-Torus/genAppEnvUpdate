 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("divider_top")){
        setdivider_topf46a0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(divider_topf46a0?.isDisabled==null)
      {
        setdivider_topf46a0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("req_data_group")){
        setreq_data_group8d4d7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(req_data_group8d4d7?.isDisabled==null)
      {
        setreq_data_group8d4d7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("res_data_group")){
        setres_data_group9d75a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(res_data_group9d75a?.isDisabled==null)
      {
        setres_data_group9d75a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("divider_bottom")){
        setdivider_bottom6920d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(divider_bottom6920d?.isDisabled==null)
      {
        setdivider_bottom6920d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cancel_btn")){
        setcancel_btn5e840((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cancel_btn5e840?.isDisabled==null)
      {
        setcancel_btn5e840((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(tran_data_group84f25) &&
        Object.keys(tran_data_group84f25)?.length > 0
      ) {
        settran_data_group84f25({})
      }
    } else prevRefreshRef.current = true
  }, [tran_data_group84f25Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settrandataview_v1((pre:any)=>({...pre,_selectedGroup_:"tran_data_group"}))
        }}
    >
