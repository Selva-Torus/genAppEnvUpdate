...
const TextInputstatus = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.trs_status"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:transactionSearch:AFVK:v1|526f0e58d5454270aca67c481a99066f|d9a26bb02dd843dbabb84b9db334bd75"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,trs_status:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, trs_status: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, trs_status: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerBlur = (id:any) => {
      if (id === "d9a26bb02dd843dbabb84b9db334bd75") {
        handleBlur({target:{value:main_group9066fRef?.current?.trs_status||""}});
      }
    };
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,trs_status:dfd_transaction_v1Props.data[0]?.trs_status}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `51 / 65`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.trs_status||""}
        ...
        headerText="Status"
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.trs_status ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
