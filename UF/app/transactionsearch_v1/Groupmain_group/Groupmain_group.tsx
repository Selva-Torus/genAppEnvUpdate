 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("divider_top")){
        setdivider_top0354b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(divider_top0354b?.isDisabled==null)
      {
        setdivider_top0354b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_date")){
        settrs_created_date2cea8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(trs_created_date2cea8?.isDisabled==null)
      {
        settrs_created_date2cea8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_account_no")){
        setdebtor_account_no963e4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(debtor_account_no963e4?.isDisabled==null)
      {
        setdebtor_account_no963e4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_name")){
        setdebtor_namee2d9f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(debtor_namee2d9f?.isDisabled==null)
      {
        setdebtor_namee2d9f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("creditor_account_no")){
        setcreditor_account_noca692((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(creditor_account_noca692?.isDisabled==null)
      {
        setcreditor_account_noca692((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("payment_currency")){
        setpayment_currency703d2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(payment_currency703d2?.isDisabled==null)
      {
        setpayment_currency703d2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("payment_amount")){
        setpayment_amount042b1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(payment_amount042b1?.isDisabled==null)
      {
        setpayment_amount042b1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid")){
        setuuid29c9f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid29c9f?.isDisabled==null)
      {
        setuuid29c9f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("channel")){
        setchannel26e83((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(channel26e83?.isDisabled==null)
      {
        setchannel26e83((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("processstatus")){
        setprocessstatus134a1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(processstatus134a1?.isDisabled==null)
      {
        setprocessstatus134a1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
        setstatus4bd75((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(status4bd75?.isDisabled==null)
      {
        setstatus4bd75((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("divider_bottom")){
        setdivider_bottom72ab5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(divider_bottom72ab5?.isDisabled==null)
      {
        setdivider_bottom72ab5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("search")){
        setsearch0e695((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(search0e695?.isDisabled==null)
      {
        setsearch0e695((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("clear")){
        setcleareddfa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cleareddfa?.isDisabled==null)
      {
        setcleareddfa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(main_group9066f) &&
        Object.keys(main_group9066f)?.length > 0
      ) {
        setmain_group9066f({})
      }
    } else prevRefreshRef.current = true
  }, [main_group9066fProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionsearch_v1((pre:any)=>({...pre,_selectedGroup_:"main_group"}))
        }}
    >
