...
const Dropdownprocessstatus = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: any) => {
 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    let mapperValue: string =  `process_status`
    let mapperText: string =  `process_status`
    bindtranValue = value;
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "fb9f9c7eff1f4a9f9272cca158e134a1"
    );
    if(orchestrationData?.data?.code)
    {
      setAllCode(orchestrationData?.data?.code)
    }
    if(dfd_processstatuscombosearch_v1Props.dstKey){
      dstKey = dfd_processstatuscombosearch_v1Props.dstKey
    }else{
      dstKey=orchestrationData?.data?.dfdKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    }
    if (!value && "hasLogicCenter" in dfd_processstatuscombosearch_v1Props && !dfd_processstatuscombosearch_v1Props.hasLogicCenter && !dfdFlag) {
  ...
  },[processstatus134a1?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setmain_group9066f((prev: any) => ({ ...prev,processstatus134a1: value }))
      setIsRequredData(false)
    }else{
      let temp:any = main_group9066f;
      delete temp.processstatus;
      delete temp.PROCESSSTATUS;
      delete temp.processstatus134a1;
      setmain_group9066f(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,processstatus:undefined}}));
    ...
    if(main_group9066f.processstatus){
      if(Array.isArray(dfd_processstatuscombosearch_v1Props)){
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{process_status:main_group9066f.processstatus}
        },
        {
          headers: {
            'Content-Type': 'application/json',
    ...
    if(processstatus134a1?.trigger == true)
    {
      await handlechange(tempValue)
      setprocessstatus134a1((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[main_group9066f.processstatus, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_processstatuscombosearch_v1Props) && dfd_processstatuscombosearch_v1Props?.length == 1){
    // setmain_group9066f((pre:any)=>({...pre,processstatus:dfd_processstatuscombosearch_v1Props[0]?.processstatus}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setmain_group9066f((prev: any) => ({ ...prev, processstatus: getMapperDetailsBindValues[value],PROCESSSTATUS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setmain_group9066f((prev: any) => ({ ...prev, processstatus: '', processstatus134a1: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,processstatus:undefined}}));
    ...
    //dynamic 
    let selectedObj=dfData?.find((items:any)=>(items?.process_status==getMapperDetailsBindValues[value] && items?.process_status==value)) || {}
    selected.current={
      ...selectedObj||{},
      process_status:getMapperDetailsBindValues[value]
    }
    customecode = allCode
    if (customecode != '') {
      let codeStates: any = {}      
  ...
  useEffect(() => {
    if(initialCount!=0)
     setmain_group9066f((pre:any)=>({...pre,processstatus:""}))
    else
      setInitialCount(1)
  },[processstatus134a1?.refresh])
  

  if (processstatus134a1?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        static={true}
        staticProps={processstatusOptions}
        onLoadMore={loadMore}
...
export default Dropdownprocessstatus;
