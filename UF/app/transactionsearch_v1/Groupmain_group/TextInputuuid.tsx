...
const TextInputuuid = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.vgphstm_uuid"
      ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,vgphstm_uuid:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, vgphstm_uuid: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, vgphstm_uuid: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "145647b72a0f4d5baea6bcf29cb29c9f") {
        handleChange({target:{value:main_group9066fRef?.current?.vgphstm_uuid||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "145647b72a0f4d5baea6bcf29cb29c9f") {
        handleBlur({target:{value:main_group9066fRef?.current?.vgphstm_uuid||""}});
      }
    };
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,vgphstm_uuid:dfd_transaction_v1Props.data[0]?.vgphstm_uuid}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `36 / 50`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.vgphstm_uuid||""}
        ...
        headerText="UUID"
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.vgphstm_uuid ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
