...
export default function PageTransactionsearchV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  useEffect(()=>{
    ...
    async function forexcurrencydropdowndfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(transactionsearch_v1Props.length > 0){
          for(let i=0;i< transactionsearch_v1Props.length;i++){
            if(transactionsearch_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:forexCurrencyDropDownDfd:AFVK:v1"){
              // delete transactionsearch_v1Props[i].DFDkey;
              let temp=structuredClone(transactionsearch_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionSearch:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
