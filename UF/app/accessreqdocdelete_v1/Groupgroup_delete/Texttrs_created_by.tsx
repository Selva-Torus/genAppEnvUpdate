...
const Texttrs_created_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_accessreqdoctable_v1Props) && dfd_accessreqdoctable_v1Props && !group_deletee7496.trs_created_by){
        setgroup_deletee7496((pre:any)=>({...pre,trs_created_by:dfd_accessreqdoctable_v1Props[0]?.trs_created_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[trs_created_byfc11e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_accessreqdoctable_v1Props) && !group_deletee7496.trs_created_by){
    setgroup_deletee7496((pre:any)=>({...pre,trs_created_by:dfd_accessreqdoctable_v1Props[0]?.trs_created_by}));
  }
  },[dfd_accessreqdoctable_v1Props])

  if (trs_created_byfc11e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.trs_created_by : (group_deletee7496?.trs_created_by || ""))}
</Text>
  </div>
  )
}
