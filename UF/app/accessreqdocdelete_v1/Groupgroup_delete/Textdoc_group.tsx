...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_accessreqdoctable_v1Props) && dfd_accessreqdoctable_v1Props && !group_deletee7496.doc_group){
        setgroup_deletee7496((pre:any)=>({...pre,doc_group:dfd_accessreqdoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_groupd3c6e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_accessreqdoctable_v1Props) && !group_deletee7496.doc_group){
    setgroup_deletee7496((pre:any)=>({...pre,doc_group:dfd_accessreqdoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_accessreqdoctable_v1Props])

  if (doc_groupd3c6e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_deletee7496?.doc_group || ""))}
</Text>
  </div>
  )
}
