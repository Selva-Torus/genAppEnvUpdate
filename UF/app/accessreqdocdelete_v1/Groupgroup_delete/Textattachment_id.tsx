...
const Textattachment_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_accessreqdoctable_v1Props) && dfd_accessreqdoctable_v1Props && !group_deletee7496.attachment_id){
        setgroup_deletee7496((pre:any)=>({...pre,attachment_id:dfd_accessreqdoctable_v1Props[0]?.attachment_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[attachment_ideb2da?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_accessreqdoctable_v1Props) && !group_deletee7496.attachment_id){
    setgroup_deletee7496((pre:any)=>({...pre,attachment_id:dfd_accessreqdoctable_v1Props[0]?.attachment_id}));
  }
  },[dfd_accessreqdoctable_v1Props])

  if (attachment_ideb2da?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.attachment_id : (group_deletee7496?.attachment_id || ""))}
</Text>
  </div>
  )
}
