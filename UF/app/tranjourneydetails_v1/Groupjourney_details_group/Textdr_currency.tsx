...
const Textdr_currency = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {dr_currency9c8a2Props, setdr_currency9c8a2Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          dr_currency: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.dr_currency
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          dr_currency: dr_currency9c8a2Props?.filteredData?.length > 0
            ? dr_currency9c8a2Props?.filteredData[0]?.dr_currency
            : "0"
        }))
      }else if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.dr_currency){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!dr_currency9c8a2Props?.filterProps) return;
    handleMapperValue(dr_currency9c8a2Props?.filterProps,dr_currency9c8a2Props?.filterFlag);
  },[dr_currency9c8a2Props?.filterProps])

  if (dr_currency9c8a2?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `50 / 58`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
...
export default Textdr_currency
