...
const Texttransaction_date_time = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {transaction_date_time14856Props, settransaction_date_time14856Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          trs_created_date: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.trs_created_date
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          trs_created_date: transaction_date_time14856Props?.filteredData?.length > 0
            ? transaction_date_time14856Props?.filteredData[0]?.trs_created_date
            : "0"
        }))
      }else if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.trs_created_date){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!transaction_date_time14856Props?.filterProps) return;
    handleMapperValue(transaction_date_time14856Props?.filterProps,transaction_date_time14856Props?.filterFlag);
  },[transaction_date_time14856Props?.filterProps])
...
return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `14 / 22`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
