...
const Textcredit_account = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {credit_account0d1f4Props, setcredit_account0d1f4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          cr_account: credit_account0d1f4Props?.filteredData?.length > 0
            ? credit_account0d1f4Props?.filteredData[0]?.cr_account
            : "0"
        }))
  ...
  // setSearchFilters
  useEffect(() => {
    if (!credit_account0d1f4Props?.filterProps) return;
    handleMapperValue(credit_account0d1f4Props?.filterProps,credit_account0d1f4Props?.filterFlag);
  },[credit_account0d1f4Props?.filterProps])
