 ...
 /////////////
  ...
  /////////////
    ...
    if(orchestrationData?.data?.readableControls.includes("debit_account")){
        setdebit_account36b40((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(debit_account36b40?.isDisabled==null)
      {
        setdebit_account36b40((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    ...
    if(orchestrationData?.data?.readableControls.includes("currency")){
        setcurrency9c8a2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(currency9c8a2?.isDisabled==null)
      {
        setcurrency9c8a2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("credit_account")){
        setcredit_account0d1f4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(credit_account0d1f4?.isDisabled==null)
      {
        setcredit_account0d1f4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    ...
    if(orchestrationData?.data?.readableControls.includes("transaction_reference_label")){
        settransaction_reference_labelb1ca9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(transaction_reference_labelb1ca9?.isDisabled==null)
      {
        settransaction_reference_labelb1ca9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("amount")){
        setamount01416((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(amount01416?.isDisabled==null)
      {
        setamount01416((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("transaction_reference")){
        settransaction_reference500d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(transaction_reference500d6?.isDisabled==null)
      {
        settransaction_reference500d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(journey_details_groupd9a0e) &&
        Object.keys(journey_details_groupd9a0e)?.length > 0
      ) {
        setjourney_details_groupd9a0e({})
      }
    } else prevRefreshRef.current = true
  }, [journey_details_groupd9a0eProps?.refresh])
