...
const Textdebit_account = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {debit_account36b40Props, setdebit_account36b40Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          dr_account: debit_account36b40Props?.filteredData?.length > 0
            ? debit_account36b40Props?.filteredData[0]?.dr_account
            : "0"
        }))
  ...
  // setSearchFilters
  useEffect(() => {
    if (!debit_account36b40Props?.filterProps) return;
    handleMapperValue(debit_account36b40Props?.filterProps,debit_account36b40Props?.filterFlag);
  },[debit_account36b40Props?.filterProps])
