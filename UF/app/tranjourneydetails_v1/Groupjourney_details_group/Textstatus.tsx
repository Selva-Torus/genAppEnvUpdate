...
const Textstatus = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {status88bc7Props, setstatus88bc7Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_journey_v1Props && !dfd_journey_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          trs_status: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.trs_status
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          trs_status: status88bc7Props?.filteredData?.length > 0
            ? status88bc7Props?.filteredData[0]?.trs_status
            : "0"
        }))
      }else if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.trs_status){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_status:dfd_journey_v1Props[0]?.trs_status}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_journey_v1Props) && !journey_details_groupd9a0e.trs_status){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_status:dfd_journey_v1Props[0]?.trs_status}));
  }
  },[dfd_journey_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!status88bc7Props?.filterProps) return;
    handleMapperValue(status88bc7Props?.filterProps,status88bc7Props?.filterFlag);
  },[status88bc7Props?.filterProps])
...
return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `14 / 23`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.trs_status : (journey_details_groupd9a0e?.trs_status || ""))}
</Text>
  </div>
  )
}
