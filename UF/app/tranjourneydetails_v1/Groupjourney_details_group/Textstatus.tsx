    ...
    try{
      if ("hasLogicCenter" in dfd_journey_v1Props && !dfd_journey_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          result: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.result
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          result: status88bc7Props?.filteredData?.length > 0
            ? status88bc7Props?.filteredData[0]?.result
            : "0"
        }))
      }else if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.result){
