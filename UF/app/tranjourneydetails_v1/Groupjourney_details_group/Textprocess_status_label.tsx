...
const Textprocess_status_label = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {process_status_labelb1ca9Props, setprocess_status_labelb1ca9Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[process_status_labelb1ca9?.refresh])

  if (process_status_labelb1ca9?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `60 / 68`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset("Process Status")}
</Text>
  </div>
  )
}

export default Textprocess_status_label
