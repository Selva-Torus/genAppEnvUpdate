...
const Textcr_account = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {cr_account0d1f4Props, setcr_account0d1f4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          cr_account: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.cr_account
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          cr_account: cr_account0d1f4Props?.filteredData?.length > 0
            ? cr_account0d1f4Props?.filteredData[0]?.cr_account
            : "0"
        }))
      }else if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.cr_account){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!cr_account0d1f4Props?.filterProps) return;
    handleMapperValue(cr_account0d1f4Props?.filterProps,cr_account0d1f4Props?.filterFlag);
  },[cr_account0d1f4Props?.filterProps])

  if (cr_account0d1f4?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `50 / 58`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
