...
const Textcurrency = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {currency9c8a2Props, setcurrency9c8a2Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          dr_currency: currency9c8a2Props?.filteredData?.length > 0
            ? currency9c8a2Props?.filteredData[0]?.dr_currency
            : "0"
        }))
  ...
  // setSearchFilters
  useEffect(() => {
    if (!currency9c8a2Props?.filterProps) return;
    handleMapperValue(currency9c8a2Props?.filterProps,currency9c8a2Props?.filterFlag);
  },[currency9c8a2Props?.filterProps])
