...
const Textprocessed_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {processed_byd2b69Props, setprocessed_byd2b69Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_journey_v1Props && !dfd_journey_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          processing_system: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.processing_system
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          processing_system: processed_byd2b69Props?.filteredData?.length > 0
            ? processed_byd2b69Props?.filteredData[0]?.processing_system
            : "0"
        }))
      }else if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.processing_system){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!processed_byd2b69Props?.filterProps) return;
    handleMapperValue(processed_byd2b69Props?.filterProps,processed_byd2b69Props?.filterFlag);
  },[processed_byd2b69Props?.filterProps])
