...
const Texttransaction_reference = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {transaction_reference500d6Props, settransaction_reference500d6Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          tran_reference: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.tran_reference
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setjourney_details_groupd9a0e((pre: any) => ({
          ...pre,
          tran_reference: transaction_reference500d6Props?.filteredData?.length > 0
            ? transaction_reference500d6Props?.filteredData[0]?.tran_reference
            : "0"
        }))
      }else if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.tran_reference){
  ...
  // setSearchFilters
  useEffect(() => {
    if (!transaction_reference500d6Props?.filterProps) return;
    handleMapperValue(transaction_reference500d6Props?.filterProps,transaction_reference500d6Props?.filterFlag);
  },[transaction_reference500d6Props?.filterProps])
