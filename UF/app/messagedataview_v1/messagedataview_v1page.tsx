...
export default function PageMessagedataviewV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:messageDataView:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setmessagedataview_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])
