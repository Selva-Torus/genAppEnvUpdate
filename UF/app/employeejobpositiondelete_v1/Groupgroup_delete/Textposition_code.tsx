...
const Textposition_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobpositions_v1Props && !dfd_jobpositions_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebeb3a((pre: any) => {
          return { ...pre, position_code: api_paginationData.data.records[0]?.position_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props && !group_deletebeb3a.position_code){
        setgroup_deletebeb3a((pre:any)=>({...pre,position_code:dfd_jobpositions_v1Props[0]?.position_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[position_codea4c8a?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobpositions_v1Props) && !group_deletebeb3a.position_code){
    setgroup_deletebeb3a((pre:any)=>({...pre,position_code:dfd_jobpositions_v1Props[0]?.position_code}));
  }
  },[dfd_jobpositions_v1Props])

  if (position_codea4c8a?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `8 / 14`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.position_code : (group_deletebeb3a?.position_code || ""))}
</Text>
  </div>
  )
}

export default Textposition_code
