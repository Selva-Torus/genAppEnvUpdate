...
const Textvacancy_status = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobpositions_v1Props && !dfd_jobpositions_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebeb3a((pre: any) => {
          return { ...pre, vacancy_status: api_paginationData.data.records[0]?.vacancy_status }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props && !group_deletebeb3a.vacancy_status){
        setgroup_deletebeb3a((pre:any)=>({...pre,vacancy_status:dfd_jobpositions_v1Props[0]?.vacancy_status}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[vacancy_status0fe05?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobpositions_v1Props) && !group_deletebeb3a.vacancy_status){
    setgroup_deletebeb3a((pre:any)=>({...pre,vacancy_status:dfd_jobpositions_v1Props[0]?.vacancy_status}));
  }
  },[dfd_jobpositions_v1Props])

  if (vacancy_status0fe05?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `36 / 42`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.vacancy_status : (group_deletebeb3a?.vacancy_status || ""))}
</Text>
  </div>
  )
}

export default Textvacancy_status
