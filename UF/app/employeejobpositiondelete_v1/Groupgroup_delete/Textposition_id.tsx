...
const Textposition_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props && !group_deletebeb3a.position_id){
        setgroup_deletebeb3a((pre:any)=>({...pre,position_id:dfd_jobpositions_v1Props[0]?.position_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[position_idebcb1?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobpositions_v1Props) && !group_deletebeb3a.position_id){
    setgroup_deletebeb3a((pre:any)=>({...pre,position_id:dfd_jobpositions_v1Props[0]?.position_id}));
  }
  },[dfd_jobpositions_v1Props])

  if (position_idebcb1?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.position_id : (group_deletebeb3a?.position_id || ""))}
</Text>
  </div>
  )
}
