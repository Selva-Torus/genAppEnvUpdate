...
const Textposition_title = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobpositions_v1Props && !dfd_jobpositions_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebeb3a((pre: any) => {
          return { ...pre, position_title: api_paginationData.data.records[0]?.position_title }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props && !group_deletebeb3a.position_title){
        setgroup_deletebeb3a((pre:any)=>({...pre,position_title:dfd_jobpositions_v1Props[0]?.position_title}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[position_titleee3e1?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobpositions_v1Props) && !group_deletebeb3a.position_title){
    setgroup_deletebeb3a((pre:any)=>({...pre,position_title:dfd_jobpositions_v1Props[0]?.position_title}));
  }
  },[dfd_jobpositions_v1Props])

  if (position_titleee3e1?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `15 / 21`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.position_title : (group_deletebeb3a?.position_title || ""))}
</Text>
  </div>
  )
}

export default Textposition_title
