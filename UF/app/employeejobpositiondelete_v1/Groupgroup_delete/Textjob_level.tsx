...
const Textjob_level = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_jobpositions_v1Props && !dfd_jobpositions_v1Props.hasLogicCenter) {
        ...
        setgroup_deletebeb3a((pre: any) => {
          return { ...pre, job_level: api_paginationData.data.records[0]?.job_level }
        })
        }
      }
      else{
      if(Array.isArray(dfd_jobpositions_v1Props) && dfd_jobpositions_v1Props && !group_deletebeb3a.job_level){
        setgroup_deletebeb3a((pre:any)=>({...pre,job_level:dfd_jobpositions_v1Props[0]?.job_level}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[job_level29550?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_jobpositions_v1Props) && !group_deletebeb3a.job_level){
    setgroup_deletebeb3a((pre:any)=>({...pre,job_level:dfd_jobpositions_v1Props[0]?.job_level}));
  }
  },[dfd_jobpositions_v1Props])

  if (job_level29550?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.job_level : (group_deletebeb3a?.job_level || ""))}
</Text>
  </div>
  )
}

export default Textjob_level
