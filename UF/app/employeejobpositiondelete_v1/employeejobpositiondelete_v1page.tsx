...
export default function PageEmployeejobpositiondeleteV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function jobpositions_v1DFD(pagination:any): Promise<void>{
        ...
        if(employeejobpositiondelete_v1Props.length > 0){
          for(let i=0;i< employeejobpositiondelete_v1Props.length;i++){
            if(employeejobpositiondelete_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1"){
              // delete employeejobpositiondelete_v1Props[i].DFDkey;
              let temp=structuredClone(employeejobpositiondelete_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setemployeejobpositiondelete_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    const data: any = await fetchBatchData(
      ...
      [user],
      'pageEmployeejobpositiondeleteV1',
      token
    )
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobPositionDelete:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setemployeejobpositiondelete_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(employeejobpositiondelete_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(employeejobpositiondelete_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setemployeejobpositiondelete_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
