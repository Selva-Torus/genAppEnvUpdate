...
import Groupspecial_rules  from "../Groupspecial_rules/Groupspecial_rules";
...
import Textspecial_rules_text  from "./Textspecial_rules_text";
...
const Groupspecial_rules_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {special_rules_text24a29, setspecial_rules_text24a29}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8a5e51ea4dd12a97fbedd9f459e7ce9f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setspecial_rules_group7ce9fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("special_rules_text")){
        setspecial_rules_text24a29((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_text24a29?.isDisabled==null)
      {
        setspecial_rules_text24a29((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules7f109((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules7f109?.isDisabled==null)
      {
        setspecial_rules7f109((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['special_rules_text'] = special_rules_text24a29,
        codeStates['setspecial_rules_text'] = setspecial_rules_text24a29,
  ...
  const special_rules_group7ce9fRef = useRef<any>(null);
  const handleClearSearch = () => {
    special_rules_group7ce9fRef.current?.setSearchParams();
    special_rules_group7ce9fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(special_rules_group7ce9f) && Object.keys(special_rules_group7ce9f)?.length>0)
      {
        setspecial_rules_group7ce9f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [special_rules_group7ce9fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"special_rules_group"}))
        }}
    >
        {allowedComponent.includes("special_rules")  &&<Groupspecial_rules  
          ...
          {allowedControls.includes("special_rules_text") ?<Textspecial_rules_text   /* 24a29 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupspecial_rules_group
