...
const TextInputssn_masked = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|2723b3f0ae705c0f66d6e06787248f3c|6b5dc77a613a14839b7fa2b2218c4424"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,ssn_masked:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group48f3c((prev: any) => ({ ...prev, ssn_masked: +e.target.value }));
    }
    else{
    setcase_information_group48f3c((prev: any) => ({ ...prev, ssn_masked: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6b5dc77a613a14839b7fa2b2218c4424"
      );
      ...
      //     controlId: "6b5dc77a613a14839b7fa2b2218c4424",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6b5dc77a613a14839b7fa2b2218c4424") {
        handleChange({target:{value:case_information_group48f3cRef?.current?.ssn_masked||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6b5dc77a613a14839b7fa2b2218c4424") {
        handleBlur({target:{value:case_information_group48f3cRef?.current?.ssn_masked||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group48f3c((pre:any)=>({...pre,ssn_masked:dfd_addcase_v1Props.data[0]?.ssn_masked}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (ssn_maskedc4424?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group48f3c?.ssn_masked||""}
         disabled= {ssn_maskedc4424?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="SSN"
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.ssn_masked ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
