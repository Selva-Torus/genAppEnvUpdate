...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,last_payment_date:undefined}}));
  if (!date) {
    setcase_information_group48f3c((prev: any) => ({ ...prev, last_payment_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group48f3c((prev: any) => ({ ...prev, last_payment_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "2ecf458ff9cbf33794f6ab40ded43bdb"
      );
...
useEffect(()=>{
 ...
 },[last_payment_date43bdb?.refresh])
...
if (last_payment_date43bdb?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group48f3c?.last_payment_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {last_payment_date43bdb?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Last Payment"
      dateValidation=""
      validationState={validate?.viewAMRCasePgGraph_v1?.last_payment_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
