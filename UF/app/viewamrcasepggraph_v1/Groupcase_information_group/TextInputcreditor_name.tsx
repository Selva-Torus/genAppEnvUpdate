...
const TextInputcreditor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|2723b3f0ae705c0f66d6e06787248f3c|9437db52d6231cc9f6c390752e5a9b98"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,creditor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group48f3c((prev: any) => ({ ...prev, creditor_name: +e.target.value }));
    }
    else{
    setcase_information_group48f3c((prev: any) => ({ ...prev, creditor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9437db52d6231cc9f6c390752e5a9b98"
      );
      ...
      //     controlId: "9437db52d6231cc9f6c390752e5a9b98",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9437db52d6231cc9f6c390752e5a9b98") {
        handleChange({target:{value:case_information_group48f3cRef?.current?.creditor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9437db52d6231cc9f6c390752e5a9b98") {
        handleBlur({target:{value:case_information_group48f3cRef?.current?.creditor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group48f3c((pre:any)=>({...pre,creditor_name:dfd_addcase_v1Props.data[0]?.creditor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (creditor_namea9b98?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group48f3c?.creditor_name||""}
         disabled= {creditor_namea9b98?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
        ...
        headerText="Creditor"
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.creditor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
