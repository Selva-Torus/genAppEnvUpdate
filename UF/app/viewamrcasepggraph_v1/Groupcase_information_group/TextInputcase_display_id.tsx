...
const TextInputcase_display_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.case_display_id"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|2723b3f0ae705c0f66d6e06787248f3c|734b8ceaa2aaa5d2e95384b48abd1272"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,case_display_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group48f3c((prev: any) => ({ ...prev, case_display_id: +e.target.value }));
    }
    else{
    setcase_information_group48f3c((prev: any) => ({ ...prev, case_display_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "734b8ceaa2aaa5d2e95384b48abd1272"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1",
      //     componentId: "2723b3f0ae705c0f66d6e06787248f3c",
      //     controlId: "734b8ceaa2aaa5d2e95384b48abd1272",
      //     isTable: false,
      //     from:"TextInputcase_display_id",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'case_display_id', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'case_display_id',type:'text'};
      //   type={
      //     name:'case_display_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.case_display_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.case_display_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.case_display_id.type
      //   }
      ...
      //   type={
      //     name:'case_display_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.case_display_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.case_display_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.case_display_id.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const case_information_group48f3cRef = useRef<any>(case_information_group48f3c);
  useEffect(() => { case_information_group48f3cRef.current = case_information_group48f3c; }, [case_information_group48f3c]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "734b8ceaa2aaa5d2e95384b48abd1272") {
        handleChange({target:{value:case_information_group48f3cRef?.current?.case_display_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "734b8ceaa2aaa5d2e95384b48abd1272") {
        handleBlur({target:{value:case_information_group48f3cRef?.current?.case_display_id||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group48f3c((pre:any)=>({...pre,case_display_id:dfd_addcase_v1Props.data[0]?.case_display_id}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (case_display_idd1272?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `11 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={case_information_group48f3c?.case_display_id||""}
         disabled= {case_display_idd1272?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Case Display ID"
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.case_display_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcase_display_id
