...
const Groupcase_information_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2723b3f0ae705c0f66d6e06787248f3c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcase_information_group48f3cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("case_info_text")){
        setcase_info_text1e3db((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_info_text1e3db?.isDisabled==null)
      {
        setcase_info_text1e3db((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_name")){
        setdebtor_name2af58((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debtor_name2af58?.isDisabled==null)
      {
        setdebtor_name2af58((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_display_id")){
        setcase_display_idd1272((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_display_idd1272?.isDisabled==null)
      {
        setcase_display_idd1272((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ssn_masked")){
        setssn_maskedc4424((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ssn_maskedc4424?.isDisabled==null)
      {
        setssn_maskedc4424((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dob")){
        setdob29785((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dob29785?.isDisabled==null)
      {
        setdob29785((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address")){
        setaddress70906((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address70906?.isDisabled==null)
      {
        setaddress70906((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("creditor_name")){
        setcreditor_namea9b98((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(creditor_namea9b98?.isDisabled==null)
      {
        setcreditor_namea9b98((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("charge_off_date")){
        setcharge_off_date763d0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(charge_off_date763d0?.isDisabled==null)
      {
        setcharge_off_date763d0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("last_payment_date")){
        setlast_payment_date43bdb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(last_payment_date43bdb?.isDisabled==null)
      {
        setlast_payment_date43bdb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("divider")){
        setdividere3c67((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dividere3c67?.isDisabled==null)
      {
        setdividere3c67((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("card_group")){
        setcard_group4c709((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(card_group4c709?.isDisabled==null)
      {
        setcard_group4c709((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("sol_expiry_date")){
        setsol_expiry_date4dfe3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(sol_expiry_date4dfe3?.isDisabled==null)
      {
        setsol_expiry_date4dfe3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const case_information_group48f3cRef = useRef<any>(null);
  const handleClearSearch = () => {
    case_information_group48f3cRef.current?.setSearchParams();
    case_information_group48f3cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(case_information_group48f3c) && Object.keys(case_information_group48f3c)?.length>0)
      {
        setcase_information_group48f3c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [case_information_group48f3cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"case_information_group"}))
        }}
    >
        {allowedComponent.includes("card_group")  &&<Groupcard_group  
          ...
          {allowedControls.includes("case_info_text") ?<Textcase_info_text   /* 1e3db */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("debtor_name") ?<TextInputdebtor_name   /* 2af58 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("case_display_id") ?<TextInputcase_display_id   /* d1272 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ssn_masked") ?<TextInputssn_masked   /* c4424 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dob") ?<DatePickerdob   /* 29785 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address") ?<TextAreaaddress   /* 70906 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("creditor_name") ?<TextInputcreditor_name   /* a9b98 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("charge_off_date") ?<DatePickercharge_off_date   /* 763d0 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("last_payment_date") ?<DatePickerlast_payment_date   /* 43bdb */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("divider") ?<Dividerdivider   /* e3c67 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("sol_expiry_date") ?<DatePickersol_expiry_date   /* 4dfe3 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
