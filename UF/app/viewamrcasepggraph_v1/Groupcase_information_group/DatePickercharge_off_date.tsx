...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,charge_off_date:undefined}}));
  if (!date) {
    setcase_information_group48f3c((prev: any) => ({ ...prev, charge_off_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group48f3c((prev: any) => ({ ...prev, charge_off_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "e4da207d9d6989e443cdcf3eafb763d0"
      );
...
if (charge_off_date763d0?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group48f3c?.charge_off_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {charge_off_date763d0?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Charge Off Date"
      dateValidation=""
      validationState={validate?.viewAMRCasePgGraph_v1?.charge_off_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
