...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|2723b3f0ae705c0f66d6e06787248f3c|1bb9a0687c36a356da097a283422af58"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,debtor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group48f3c((prev: any) => ({ ...prev, debtor_name: +e.target.value }));
    }
    else{
    setcase_information_group48f3c((prev: any) => ({ ...prev, debtor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "1bb9a0687c36a356da097a283422af58"
      );
      ...
      //     controlId: "1bb9a0687c36a356da097a283422af58",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "1bb9a0687c36a356da097a283422af58") {
        handleChange({target:{value:case_information_group48f3cRef?.current?.debtor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "1bb9a0687c36a356da097a283422af58") {
        handleBlur({target:{value:case_information_group48f3cRef?.current?.debtor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group48f3c((pre:any)=>({...pre,debtor_name:dfd_addcase_v1Props.data[0]?.debtor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (debtor_name2af58?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group48f3c?.debtor_name||""}
         disabled= {debtor_name2af58?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Debtor"
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.debtor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
