...
const Groupintrest_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5c7aebe727c98f2da84fdc84a5165c3b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setintrest_group65c3bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("intrest_text")){
        setintrest_text3a919((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_text3a919?.isDisabled==null)
      {
        setintrest_text3a919((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("interest")){
        setinterest783c4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(interest783c4?.isDisabled==null)
      {
        setinterest783c4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['intrest_text'] = intrest_text3a919,
        codeStates['setintrest_text'] = setintrest_text3a919,
        codeStates['interest'] = interest783c4,
        codeStates['setinterest'] = setinterest783c4,
  ...
  const intrest_group65c3bRef = useRef<any>(null);
  const handleClearSearch = () => {
    intrest_group65c3bRef.current?.setSearchParams();
    intrest_group65c3bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(intrest_group65c3b) && Object.keys(intrest_group65c3b)?.length>0)
      {
        setintrest_group65c3b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [intrest_group65c3bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"intrest_group"}))
        }}
    >
          {allowedControls.includes("intrest_text") ?<Textintrest_text   /* 3a919 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("interest") ?<Textinterest   /* 783c4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
