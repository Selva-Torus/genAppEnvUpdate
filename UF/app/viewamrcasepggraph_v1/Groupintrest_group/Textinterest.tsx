...
const Textinterest = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {interest783c4Props, setinterest783c4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setintrest_group65c3b((pre: any) => ({
          ...pre,
          interest: interest783c4Props?.filteredData?.length > 0
            ? interest783c4Props?.filteredData[0]?.interest
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !intrest_group65c3b.interest){
        setintrest_group65c3b((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[interest783c4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !intrest_group65c3b.interest){
    setintrest_group65c3b((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!interest783c4Props?.filterProps) return;
    handleMapperValue(interest783c4Props?.filterProps,interest783c4Props?.filterFlag);
  },[interest783c4Props?.filterProps])

  if (interest783c4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.interest : (intrest_group65c3b?.interest || ""))}
</Text>
  </div>
  )
}
