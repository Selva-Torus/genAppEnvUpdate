...
const Grouptotal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "4eccf92ecee29b2dcd47ce40be3c52d3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_groupc52d3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_balance_text")){
        settotal_balance_text0b418((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balance_text0b418?.isDisabled==null)
      {
        settotal_balance_text0b418((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_balance")){
        settotal_balancee7e5f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balancee7e5f?.isDisabled==null)
      {
        settotal_balancee7e5f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_balance_text'] = total_balance_text0b418,
        codeStates['settotal_balance_text'] = settotal_balance_text0b418,
        codeStates['total_balance'] = total_balancee7e5f,
        codeStates['settotal_balance'] = settotal_balancee7e5f,
  ...
  const total_groupc52d3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_groupc52d3Ref.current?.setSearchParams();
    total_groupc52d3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(total_groupc52d3) && Object.keys(total_groupc52d3)?.length>0)
      {
        settotal_groupc52d3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [total_groupc52d3Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"total_group"}))
        }}
    >
          {allowedControls.includes("total_balance_text") ?<Texttotal_balance_text   /* 0b418 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_balance") ?<Texttotal_balance   /* e7e5f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
