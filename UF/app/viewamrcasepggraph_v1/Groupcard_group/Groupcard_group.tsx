...
import Groupprincipal_group  from "../Groupprincipal_group/Groupprincipal_group";
import Groupintrest_group  from "../Groupintrest_group/Groupintrest_group";
import Groupfees_group  from "../Groupfees_group/Groupfees_group";
import Grouptotal_group  from "../Grouptotal_group/Grouptotal_group";
...
const Groupcard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "23ca8a0b60e0f64b61482307b8e4c709");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcard_group4c709Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_group")){
        setprincipal_group42235((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_group42235?.isDisabled==null)
      {
        setprincipal_group42235((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("intrest_group")){
        setintrest_group65c3b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_group65c3b?.isDisabled==null)
      {
        setintrest_group65c3b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees_group")){
        setfees_group8c4a6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_group8c4a6?.isDisabled==null)
      {
        setfees_group8c4a6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_group")){
        settotal_groupc52d3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_groupc52d3?.isDisabled==null)
      {
        settotal_groupc52d3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const card_group4c709Ref = useRef<any>(null);
  const handleClearSearch = () => {
    card_group4c709Ref.current?.setSearchParams();
    card_group4c709Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card_group4c709) && Object.keys(card_group4c709)?.length>0)
      {
        setcard_group4c709({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card_group4c709Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"card_group"}))
        }}
    >
        {allowedComponent.includes("principal_group")  &&<Groupprincipal_group  
        ...
        {allowedComponent.includes("intrest_group")  &&<Groupintrest_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("fees_group")  &&<Groupfees_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("total_group")  &&<Grouptotal_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupcard_group
