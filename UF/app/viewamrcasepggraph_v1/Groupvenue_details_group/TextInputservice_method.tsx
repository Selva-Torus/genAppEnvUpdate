...
const TextInputservice_method = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|57c10704bc2bf3a621d11bfa47051614|ea370f7a065821127ddb0437b2648ac6"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,service_method:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group51614((prev: any) => ({ ...prev, service_method: +e.target.value }));
    }
    else{
    setvenue_details_group51614((prev: any) => ({ ...prev, service_method: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ea370f7a065821127ddb0437b2648ac6"
      );
      ...
      //     controlId: "ea370f7a065821127ddb0437b2648ac6",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ea370f7a065821127ddb0437b2648ac6") {
        handleChange({target:{value:venue_details_group51614Ref?.current?.service_method||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ea370f7a065821127ddb0437b2648ac6") {
        handleBlur({target:{value:venue_details_group51614Ref?.current?.service_method||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group51614((pre:any)=>({...pre,service_method:dfd_specialrulessurerealdb_v1Props.data[0]?.service_method}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (service_method48ac6?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group51614?.service_method||""}
         disabled= {service_method48ac6?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.service_method ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
