...
const TextInputfiling_fee = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:specialRulesSurerealDB:AFVK:v1|5c1ed2d414f740ab820856297865df26|items.properties.venue.items.properties.filing_fee"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRCasePgGraph:AFVK:v1|57c10704bc2bf3a621d11bfa47051614|870a661283bf5fac1c49f9d1a8c144cd"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,filing_fee:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group51614((prev: any) => ({ ...prev, filing_fee: +e.target.value }));
    }
    else{
    setvenue_details_group51614((prev: any) => ({ ...prev, filing_fee: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "870a661283bf5fac1c49f9d1a8c144cd"
      );
      ...
      //     controlId: "870a661283bf5fac1c49f9d1a8c144cd",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "870a661283bf5fac1c49f9d1a8c144cd") {
        handleChange({target:{value:venue_details_group51614Ref?.current?.filing_fee||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "870a661283bf5fac1c49f9d1a8c144cd") {
        handleBlur({target:{value:venue_details_group51614Ref?.current?.filing_fee||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group51614((pre:any)=>({...pre,filing_fee:dfd_specialrulessurerealdb_v1Props.data[0]?.filing_fee}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (filing_fee144cd?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group51614?.filing_fee||""}
         disabled= {filing_fee144cd?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
      ...
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.filing_fee ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
