...
const TextInputstate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRCasePgGraph_v1:{...pre?.viewAMRCasePgGraph_v1,state:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group51614((prev: any) => ({ ...prev, state: +e.target.value }));
    }
    else{
    setvenue_details_group51614((prev: any) => ({ ...prev, state: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "983a9b1951051a092c93ab26d34a7e09"
      );
      ...
      //     controlId: "983a9b1951051a092c93ab26d34a7e09",
      //     isTable: false,
      //     from:"TextInputstate",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "983a9b1951051a092c93ab26d34a7e09") {
        handleChange({target:{value:venue_details_group51614Ref?.current?.state||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "983a9b1951051a092c93ab26d34a7e09") {
        handleBlur({target:{value:venue_details_group51614Ref?.current?.state||""}});
      }
    };
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group51614?.state||""}
         disabled= {statea7e09?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="State"
      errorMessage={error}
        validationState={validate?.viewAMRCasePgGraph_v1?.state ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
