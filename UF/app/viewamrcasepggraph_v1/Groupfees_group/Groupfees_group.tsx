...
const Groupfees_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a847e489607d2f2310e70a332338c4a6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfees_group8c4a6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("fees_text")){
        setfees_text4a6b6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_text4a6b6?.isDisabled==null)
      {
        setfees_text4a6b6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees")){
        setfeesab1fe((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(feesab1fe?.isDisabled==null)
      {
        setfeesab1fe((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['fees_text'] = fees_text4a6b6,
        codeStates['setfees_text'] = setfees_text4a6b6,
        codeStates['fees'] = feesab1fe,
        codeStates['setfees'] = setfeesab1fe,
  ...
  const fees_group8c4a6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    fees_group8c4a6Ref.current?.setSearchParams();
    fees_group8c4a6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(fees_group8c4a6) && Object.keys(fees_group8c4a6)?.length>0)
      {
        setfees_group8c4a6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [fees_group8c4a6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"fees_group"}))
        }}
    >
          {allowedControls.includes("fees_text") ?<Textfees_text   /* 4a6b6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("fees") ?<Textfees   /* ab1fe */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
