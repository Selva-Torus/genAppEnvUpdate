...
const Groupdoc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "040ce6f6cfa9a14028814480bdf9c4f7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_table9c4f7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_idcc3f4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(attachment_idcc3f4?.isDisabled==null)
      {
        setattachment_idcc3f4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_name909a3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_name909a3?.isDisabled==null)
      {
        setdoc_name909a3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("view_button")){
        setview_button22ede((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(view_button22ede?.isDisabled==null)
      {
        setview_button22ede((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const doc_table9c4f7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_table9c4f7Ref.current?.setSearchParams();
    doc_table9c4f7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_table9c4f7) && Object.keys(doc_table9c4f7)?.length>0)
      {
        setdoc_table9c4f7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_table9c4f7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"doc_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tabledoc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_table9c4f7Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
