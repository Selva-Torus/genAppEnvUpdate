...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "4fcac96943872980c63009da3afcc3f4",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "73589a4405b6bc6c4b55d8018da909a3",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
...
const Tabledoc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, doc_table9c4f7 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, doc_table9c4f7 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, doc_table9c4f7 } = latestLockStateRef.current
    /// setdoc_table9c4f7Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(doc_table9c4f7.map((item: any) => item.doc_instance_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      doc_table9c4f7.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = doc_table9c4f7
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.doc_instance_id) ? item.doc_instance_id : null
          )
          .filter(Boolean)

        setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        throw err
      }
    }
    let index = Number(ids[ids.length - 1])
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["account_id"]:doc_table9c4f7Props?.presetValues?.account_id}
    ...
    if(!doc_table9c4f7Props?.presetValues || Object.keys(doc_table9c4f7Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '040ce6f6cfa9a14028814480bdf9c4f7',
                elementId: '040ce6f6cfa9a14028814480bdf9c4f7'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setdoc_table9c4f7(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in viewamrcasepggraph_v1?.doc_table && viewamrcasepggraph_v1?.doc_table[controller]?.itsHaveArtifact== true)
      {
        if(viewamrcasepggraph_v1?.doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(viewamrcasepggraph_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,doc_table:filteredData},{doc_table:viewamrcasepggraph_v1?.doc_table})
    ...
    if(await handleSecurity("view_button") &&nodeName?.toLowerCase()=="buttonview_button"){
      ...
      return (
        <React.Fragment>
        <Buttonview_button mainData={filteredData} lockedData={tempLockedData} controlData={controlData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} onSelectLock={setLockMode} rowIndex={index} currentSelectedIds={doc_table9c4f7Props?.selectedIds} tableName={tableName}/>
        </React.Fragment>
      )
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(doc_table9c4f7))
    {
      GetTableDetails()
    }
  }, [doc_table9c4f7])
  ...
  useEffect(() => {
    ...
    if (doc_table9c4f7Props.filterInitalLoad) return;

    const filterControllers = doc_table9c4f7Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['doc_table9c4f7']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['doc_table9c4f7']).filter((k: string) => jsonEntry['doc_table9c4f7'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(doc_table9c4f7Props.filterProps);
      return;
    }
    ...
    setdoc_table9c4f7Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(doc_table9c4f7Props.filterProps);

  }, [
    doc_table9c4f7Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    doc_table9c4f7Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(doc_table9c4f7Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(doc_table9c4f7Props.searchFilter) || {};
...
}, [doc_table9c4f7Props.searchFilter])
  ...
  useEffect(() => {
    if (doc_table9c4f7Props?.clearData === true) {
      if (myLockedIdsRef.current.length) {
        myLockedIdsRef.current.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setdoc_table9c4f7Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [doc_table9c4f7Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  }, [doc_table9c4f7Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = doc_table9c4f7Props?.selectedIds[doc_table9c4f7Props?.selectedIds?.length-1]
    if(Array.isArray(doc_table9c4f7) && doc_table9c4f7.length>0)
    {
      let data = doc_table9c4f7.find((data:any)=>(data?.doc_instance_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "doc_instance_id",
  ...
  useEffect(() => {
    if (doc_table9c4f7Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (doc_table9c4f7Props?.selectedIds?.length == 0){
      handleOnRowClick({},doc_table9c4f7Props?.selectedIds)
    }
  }, [doc_table9c4f7Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setdoc_table9c4f7Props((pre:any)=>({...pre, selectedIds:[row?.doc_instance_id]}))  
    onButtonSecurityHandle(data)
    ...
    let filterProps2:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData2 = await getFilterProps(filterProps2,{...add_case_grouped126,...header_groupbd8a8,...required_dociument_main_group255d1,...case_information_group48f3c,...card_group4c709,...principal_group42235,...intrest_group65c3b,...fees_group8c4a6,...total_groupc52d3,...venue_details_group51614,...checklist_main_group2b466,...checklist_tablecafb0,...special_rules_group7ce9f,...special_rules7f109,...doc_table9c4f7});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(doc_table9c4f7?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&doc_table9c4f7?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](doc_table9c4f7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_table9c4f7?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&doc_table9c4f7?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](doc_table9c4f7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_table9c4f7?.at(newIndex) ,currectPage:paginationData?.page});
      ...
      >
        <PageDocumentviewerpage2/>
          ...
          <div
            ...
            id={"040ce6f6cfa9a14028814480bdf9c4f7"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableSorting={true}
              disable={disable}
              selectedIds={doc_table9c4f7Props?.selectedIds}  
