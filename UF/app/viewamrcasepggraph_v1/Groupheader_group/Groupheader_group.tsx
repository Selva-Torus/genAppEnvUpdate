...
import Buttonback_bt  from "./Buttonback_bt";
...
const Groupheader_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f0b7ddc761cdd7f387b468de821bd8a8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setheader_groupbd8a8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("back_bt")){
        setback_bt22897((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(back_bt22897?.isDisabled==null)
      {
        setback_bt22897((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("craete_header_text")){
        setcraete_header_textae5d6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(craete_header_textae5d6?.isDisabled==null)
      {
        setcraete_header_textae5d6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const header_groupbd8a8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    header_groupbd8a8Ref.current?.setSearchParams();
    header_groupbd8a8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(header_groupbd8a8) && Object.keys(header_groupbd8a8)?.length>0)
      {
        setheader_groupbd8a8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [header_groupbd8a8Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"header_group"}))
        }}
    >
        {        ((ruleData?.length>0 && "back_bt" in ButtonGoRuleData)?ButtonGoRuleData["back_bt"]:true) && 
          allowedControls.includes("back_bt")  ?            <Buttonback_bt tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
          {allowedControls.includes("craete_header_text") ?<Textcraete_header_text   /* ae5d6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
