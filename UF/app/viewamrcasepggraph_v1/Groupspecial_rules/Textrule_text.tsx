...
const Textrule_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setspecial_rules7f109((pre: any) => ({
          ...pre,
          text: rule_textdb2d4Props?.filteredData?.length > 0
            ? rule_textdb2d4Props?.filteredData[0]?.text
            : "0"
        }))
  ...
  // setSearchFilters
  useEffect(() => {
    if (!rule_textdb2d4Props?.filterProps) return;
    handleMapperValue(rule_textdb2d4Props?.filterProps,rule_textdb2d4Props?.filterFlag);
  },[rule_textdb2d4Props?.filterProps])
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.text : (special_rules7f109?.text || ""))}
</Text>
  </div>
  )
}
