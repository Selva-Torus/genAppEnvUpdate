...
import Textprincipal_text  from "./Textprincipal_text";
...
const Groupprincipal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {principal_text6883f, setprincipal_text6883f}= useContext(TotalContext) as TotalContextProps;
  const {principal65ef3, setprincipal65ef3}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "226b227eebc7cf76b4b768fba7b42235");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprincipal_group42235Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_text")){
        setprincipal_text6883f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_text6883f?.isDisabled==null)
      {
        setprincipal_text6883f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("principal")){
        setprincipal65ef3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal65ef3?.isDisabled==null)
      {
        setprincipal65ef3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['principal_text'] = principal_text6883f,
        codeStates['setprincipal_text'] = setprincipal_text6883f,
        codeStates['principal'] = principal65ef3,
        codeStates['setprincipal'] = setprincipal65ef3,
  ...
  const principal_group42235Ref = useRef<any>(null);
  const handleClearSearch = () => {
    principal_group42235Ref.current?.setSearchParams();
    principal_group42235Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(principal_group42235) && Object.keys(principal_group42235)?.length>0)
      {
        setprincipal_group42235({})
      }
    }else 
      prevRefreshRef.current= true
  }, [principal_group42235Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"principal_group"}))
        }}
    >
          {allowedControls.includes("principal_text") ?<Textprincipal_text   /* 6883f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("principal") ?<Textprincipal   /* 65ef3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupprincipal_group
