...
const Textprincipal = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {principal65ef3Props, setprincipal65ef3Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setprincipal_group42235((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setprincipal_group42235((pre: any) => ({
          ...pre,
          principal: principal65ef3Props?.filteredData?.length > 0
            ? principal65ef3Props?.filteredData[0]?.principal
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !principal_group42235.principal){
        setprincipal_group42235((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[principal65ef3?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !principal_group42235.principal){
    setprincipal_group42235((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!principal65ef3Props?.filterProps) return;
    handleMapperValue(principal65ef3Props?.filterProps,principal65ef3Props?.filterFlag);
  },[principal65ef3Props?.filterProps])

  if (principal65ef3?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.principal : (principal_group42235?.principal || ""))}
</Text>
  </div>
  )
}
