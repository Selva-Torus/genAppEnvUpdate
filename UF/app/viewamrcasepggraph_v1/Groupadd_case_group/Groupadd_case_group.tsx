...
const Groupadd_case_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {account_idcc45c, setaccount_idcc45c}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c58a7bc033dc6192e018b5b5350ed126");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadd_case_grouped126Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("header_group")){
        setheader_groupbd8a8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(header_groupbd8a8?.isDisabled==null)
      {
        setheader_groupbd8a8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("required_dociument_main_group")){
        setrequired_dociument_main_group255d1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(required_dociument_main_group255d1?.isDisabled==null)
      {
        setrequired_dociument_main_group255d1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_information_group")){
        setcase_information_group48f3c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_information_group48f3c?.isDisabled==null)
      {
        setcase_information_group48f3c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_details_group")){
        setvenue_details_group51614((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_details_group51614?.isDisabled==null)
      {
        setvenue_details_group51614((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_main_group")){
        setchecklist_main_group2b466((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_main_group2b466?.isDisabled==null)
      {
        setchecklist_main_group2b466((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules_group")){
        setspecial_rules_group7ce9f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_group7ce9f?.isDisabled==null)
      {
        setspecial_rules_group7ce9f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("account_id")){
        setaccount_idcc45c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(account_idcc45c?.isDisabled==null)
      {
        setaccount_idcc45c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['account_id'] = account_idcc45c,
        codeStates['setaccount_id'] = setaccount_idcc45c,
  ...
  const add_case_grouped126Ref = useRef<any>(null);
  const handleClearSearch = () => {
    add_case_grouped126Ref.current?.setSearchParams();
    add_case_grouped126Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(add_case_grouped126) && Object.keys(add_case_grouped126)?.length>0)
      {
        setadd_case_grouped126({})
      }
    }else 
      prevRefreshRef.current= true
  }, [add_case_grouped126Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"add_case_group"}))
        }}
    >
        ...
        {allowedComponent.includes("special_rules_group")  &&<Groupspecial_rules_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("account_id") ?<Textaccount_id   /* cc45c */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
