...
const Textaccount_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {account_idcc45cProps, setaccount_idcc45cProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setadd_case_grouped126((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setadd_case_grouped126((pre: any) => ({
          ...pre,
          account_id: account_idcc45cProps?.filteredData?.length > 0
            ? account_idcc45cProps?.filteredData[0]?.account_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_grouped126.account_id){
        setadd_case_grouped126((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[account_idcc45c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_grouped126.account_id){
    setadd_case_grouped126((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!account_idcc45cProps?.filterProps) return;
    handleMapperValue(account_idcc45cProps?.filterProps,account_idcc45cProps?.filterFlag);
  },[account_idcc45cProps?.filterProps])

  if (account_idcc45c?.isHidden) {
    return <></>
  }
