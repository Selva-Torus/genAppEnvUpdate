...
const Grouprequired_dociument_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {reeq_doc_text64f93, setreeq_doc_text64f93}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "65ddb42ea96f742dd7330f57b1a255d1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_main_group255d1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reeq_doc_text")){
        setreeq_doc_text64f93((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reeq_doc_text64f93?.isDisabled==null)
      {
        setreeq_doc_text64f93((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_table")){
        setdoc_table9c4f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_table9c4f7?.isDisabled==null)
      {
        setdoc_table9c4f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['reeq_doc_text'] = reeq_doc_text64f93,
        codeStates['setreeq_doc_text'] = setreeq_doc_text64f93,
  ...
  const required_dociument_main_group255d1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_main_group255d1Ref.current?.setSearchParams();
    required_dociument_main_group255d1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_main_group255d1) && Object.keys(required_dociument_main_group255d1)?.length>0)
      {
        setrequired_dociument_main_group255d1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_main_group255d1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrcasepggraph_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_main_group"}))
        }}
    >
        {allowedComponent.includes("doc_table")  &&<Groupdoc_table  
          ...
          {allowedControls.includes("reeq_doc_text") ?<Textreeq_doc_text   /* 64f93 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
