...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "47357f59404ce7f3fe7daa84cbdeddae",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "91ac6b2978270b3b5b03dcf235c5887d",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "3d8cc1fc7cb2961eac6447ec4dd81621",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "9e0b54ac422027cf7fd21644c49e5a4e",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "e4571c05a0c6a10cacae7a56ce60e881",
    "name": "Created By",
    "meta": {
      "sort": true
    },
...
const Tableemployee_doc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const unlockRecord = async (id: number) => {
    ...
    /// setemployee_doc_table78f2fProps
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(employee_doc_table78f2f.map((item: any) => item.asset_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      employee_doc_table78f2f.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = employee_doc_table78f2f
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.asset_id) ? index.toString() : null
          )
          .filter(Boolean)

        setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        return
      }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setemployee_doc_table78f2fProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(backgroundcheckdocument_v1Props.length > 0){
      for(let i=0;i< backgroundcheckdocument_v1Props.length;i++){
        if(backgroundcheckdocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:backgroundCheckDocTable:AFVK:v1"){
          // delete backgroundcheckdocument_v1Props[i].DFDkey;
          let temp=structuredClone(backgroundcheckdocument_v1Props[i])
          delete temp?.DFDkey
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["asset_id"]:employee_doc_table78f2fProps?.presetValues?.asset_id}
    ...
    if(!employee_doc_table78f2fProps?.presetValues || Object.keys(employee_doc_table78f2fProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:backgroundCheckDocument:AFVK:v1:UO', 
                nodeId: 'dcfab41f782d61d24821f9774c078f2f',
                elementId: 'dcfab41f782d61d24821f9774c078f2f'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:backgroundCheckDocument:AFVK:v1',
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setemployee_doc_table78f2f(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in backgroundcheckdocument_v1?.employee_doc_table && backgroundcheckdocument_v1?.employee_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(backgroundcheckdocument_v1?.employee_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(backgroundcheckdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,employee_doc_table:filteredData},{employee_doc_table:backgroundcheckdocument_v1?.employee_doc_table})
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(employee_doc_table78f2f))
    {
      GetTableDetails()
    }
  }, [employee_doc_table78f2f])
  ...
  useEffect(() => {
    ...
    if (employee_doc_table78f2fProps.filterInitalLoad) return;

    const filterControllers = employee_doc_table78f2fProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['employee_doc_table78f2f']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['employee_doc_table78f2f']).filter((k: string) => jsonEntry['employee_doc_table78f2f'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(employee_doc_table78f2fProps.filterProps);
      return;
    }
    ...
    setemployee_doc_table78f2fProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(employee_doc_table78f2fProps.filterProps);

  }, [
    employee_doc_table78f2fProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    employee_doc_table78f2fProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(employee_doc_table78f2fProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(employee_doc_table78f2fProps.searchFilter) || {};
...
}, [employee_doc_table78f2fProps.searchFilter])
  ...
  useEffect(() => {
    if (employee_doc_table78f2fProps?.clearData === true) {
      if (lastLockedDataRef.current?.primaryKeys?.length) {
        lastLockedDataRef.current.primaryKeys.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setemployee_doc_table78f2fProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [employee_doc_table78f2fProps?.clearData])
  useEffect(() => {
  ...
  }, [employee_doc_table78f2fProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = employee_doc_table78f2fProps?.selectedIds[employee_doc_table78f2fProps?.selectedIds?.length-1]
    if(Array.isArray(employee_doc_table78f2f) && employee_doc_table78f2f.length>0)
    {
      let data = employee_doc_table78f2f[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "asset_id",
        value: data["asset_id"],
  ...
  useEffect(() => {
    if (employee_doc_table78f2fProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (employee_doc_table78f2fProps?.selectedIds?.length == 0){
      handleOnRowClick({},employee_doc_table78f2fProps?.selectedIds)
    }
  }, [employee_doc_table78f2fProps?.selectedIds])
    ...
    let filterProps4:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData4 = await getFilterProps(filterProps4,{...doc_attached_groupb4f40,...table_groupb0ef9,...employee_doc_table78f2f});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(employee_doc_table78f2f?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&employee_doc_table78f2f?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](employee_doc_table78f2f?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: employee_doc_table78f2f?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&employee_doc_table78f2f?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](employee_doc_table78f2f?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: employee_doc_table78f2f?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (employee_doc_table78f2f?.isHidden) {
    return <></>
  }
  return(
      ...
      >
        <PageHrmdocumentviewerpage4/>
          ...
          <div
            ...
            id={"dcfab41f782d61d24821f9774c078f2f"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={employee_doc_table78f2fProps?.selectedIds}  
