...
const Groupemployee_doc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "dcfab41f782d61d24821f9774c078f2f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemployee_doc_table78f2fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_ideddae((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(attachment_ideddae?.isDisabled==null)
      {
        setattachment_ideddae((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_group")){
        setdoc_group5887d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_group5887d?.isDisabled==null)
      {
        setdoc_group5887d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_name81621((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_name81621?.isDisabled==null)
      {
        setdoc_name81621((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_date")){
        settrs_created_datee5a4e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_datee5a4e?.isDisabled==null)
      {
        settrs_created_datee5a4e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_by")){
        settrs_created_by0e881((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_by0e881?.isDisabled==null)
      {
        settrs_created_by0e881((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bt_delete")){
        setbt_delete5b212((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bt_delete5b212?.isDisabled==null)
      {
        setbt_delete5b212((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const employee_doc_table78f2fRef = useRef<any>(null);
  const handleClearSearch = () => {
    employee_doc_table78f2fRef.current?.setSearchParams();
    employee_doc_table78f2fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(employee_doc_table78f2f) && Object.keys(employee_doc_table78f2f)?.length>0)
      {
        setemployee_doc_table78f2f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [employee_doc_table78f2fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setbackgroundcheckdocument_v1((pre:any)=>({...pre,_selectedGroup_:"employee_doc_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tableemployee_doc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={employee_doc_table78f2fRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
