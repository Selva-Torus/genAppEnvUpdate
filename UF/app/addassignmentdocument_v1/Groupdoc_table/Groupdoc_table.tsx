...
const Groupdoc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1cff839b6eae7947a9fa183da14392d0");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_table392d0Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_id0c7b6({...attachment_id0c7b6,isDisabled:true});

    }else
    {
      if(attachment_id0c7b6?.isDisabled==null)
      {
        setattachment_id0c7b6({...attachment_id0c7b6,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_group")){
        setdoc_group8e81a({...doc_group8e81a,isDisabled:true});

    }else
    {
      if(doc_group8e81a?.isDisabled==null)
      {
        setdoc_group8e81a({...doc_group8e81a,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_nameb994a({...doc_nameb994a,isDisabled:true});

    }else
    {
      if(doc_nameb994a?.isDisabled==null)
      {
        setdoc_nameb994a({...doc_nameb994a,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_date")){
        settrs_created_date26a21({...trs_created_date26a21,isDisabled:true});

    }else
    {
      if(trs_created_date26a21?.isDisabled==null)
      {
        settrs_created_date26a21({...trs_created_date26a21,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_by")){
        settrs_created_by95da2({...trs_created_by95da2,isDisabled:true});

    }else
    {
      if(trs_created_by95da2?.isDisabled==null)
      {
        settrs_created_by95da2({...trs_created_by95da2,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bt_delete")){
        setbt_deletea6263({...bt_deletea6263,isDisabled:true});

    }else
    {
      if(bt_deletea6263?.isDisabled==null)
      {
        setbt_deletea6263({...bt_deletea6263,isDisabled:false});
      }
    }
  //////////////
  }
  ...
  const doc_table392d0Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_table392d0Ref.current?.setSearchParams();
    doc_table392d0Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_table392d0) && Object.keys(doc_table392d0)?.length>0)
      {
        setdoc_table392d0({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_table392d0Props?.refresh,token])
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tabledoc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_table392d0Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
