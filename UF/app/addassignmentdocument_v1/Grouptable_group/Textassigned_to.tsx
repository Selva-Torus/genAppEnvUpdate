...
const Textassigned_to = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assigndoctable_v1Props) && dfd_assigndoctable_v1Props && !table_group75a5e.assigned_to){
        settable_group75a5e((pre:any)=>({...pre,assigned_to:dfd_assigndoctable_v1Props[0]?.assigned_to}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[assigned_to72696?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assigndoctable_v1Props) && !table_group75a5e.assigned_to){
    settable_group75a5e((pre:any)=>({...pre,assigned_to:dfd_assigndoctable_v1Props[0]?.assigned_to}));
  }
  },[dfd_assigndoctable_v1Props])

  if (assigned_to72696?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `5 / 25`,gridRow: `8 / 14`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.assigned_to : (table_group75a5e?.assigned_to || ""))}
</Text>
  </div>
  )
}
