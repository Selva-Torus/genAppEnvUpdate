...
import Groupaddt__group  from "../Groupaddt__group/Groupaddt__group";
...
import DatePickercompleted_at_datepicker  from "./DatePickercompleted_at_datepicker";
...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "37cea112d9c2947958360e56f6afa034");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_groupfa034Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__groupdb1de((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__groupdb1de?.isDisabled==null)
      {
        setaccess_req__groupdb1de((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("valid_group")){
        setvalid_groupb4569((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_groupb4569?.isDisabled==null)
      {
        setvalid_groupb4569((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("addt__group")){
        setaddt__group82d26((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(addt__group82d26?.isDisabled==null)
      {
        setaddt__group82d26((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("addt__dts_group")){
        setaddt__dts_group7fd81((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(addt__dts_group7fd81?.isDisabled==null)
      {
        setaddt__dts_group7fd81((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("completed_at_datepicker")){
        setcompleted_at_datepicker4163e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(completed_at_datepicker4163e?.isDisabled==null)
      {
        setcompleted_at_datepicker4163e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactions31354((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactions31354?.isDisabled==null)
      {
        setdynamicactions31354((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("review_id")){
        setreview_id1f0d1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(review_id1f0d1?.isDisabled==null)
      {
        setreview_id1f0d1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const new_access_groupfa034Ref = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_groupfa034Ref.current?.setSearchParams();
    new_access_groupfa034Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_groupfa034) && Object.keys(new_access_groupfa034)?.length>0)
      {
        setnew_access_groupfa034({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_groupfa034Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("valid_group")  &&<Groupvalid_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("addt__group")  &&<Groupaddt__group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedControls.includes("completed_at_datepicker") ?<DatePickercompleted_at_datepicker   /* 4163e */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("review_id") ?<Textreview_id   /* 1f0d1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
