 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "68cb46b6b2ebfec8569a2c9d87f1b289"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactions31354Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions31354Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in newperformancereview_v1?.dynamicactions && newperformancereview_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(newperformancereview_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newperformancereview_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_update1b289") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "68cb46b6b2ebfec8569a2c9d87f1b289") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_update1b289");
    return () => {
  ...
  useEffect(()=>{
    if (!button_update1b289?.trigger) return;
      if(button_update1b289?.trigger){
      setbutton_update1b289((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_update1b289?.trigger])

  useEffect(()=>{
    if(button_update1b289?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_update1b289?.refresh])
  ...
  async function handleSaveb289_1_1_1(){
  ...
  const hasInvalidField = Object.values(currentValidate?.newPerformanceReview_v1 || {}).some(val => {
    ...
    try{
      ...
      let lockedKeysLength:number;
        if(dynamicactions31354?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "68cb46b6b2ebfec8569a2c9d87f1b289",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "68cb46b6b2ebfec8569a2c9d87f1b289.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "68cb46b6b2ebfec8569a2c9d87f1b289.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "68cb46b6b2ebfec8569a2c9d87f1b289.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "68cb46b6b2ebfec8569a2c9d87f1b289.1.1.1.1.1",
                  "eventContext": "rise",
                  "value": "",
                  "type": "handlerNode",
                  "name": "closeHandler",
            ...
            {
              "id": "68cb46b6b2ebfec8569a2c9d87f1b289.1.1.1.2",
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions31354?.upId? [dynamicactions31354?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions31354?.upId? [dynamicactions31354?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions31354?.upId? [dynamicactions31354?.upId ] : dynamicactions31354?.trs_process_id? [dynamicactions31354?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"68cb46b6b2ebfec8569a2c9d87f1b289"
            };
            if (encryptionFlagCont) {
    ...
    te_eventEmitterBody["ssKey"] = dynamicactions31354Props.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions31354((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSaveb289_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions31354((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newperformancereview_v1?.dynamicactions?.button_update,dynamicactions31354Props?.dynamicActionRule?.button_update,])

 if (button_update1b289?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_update1b289?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
