...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8e89aca3256313c5db76b7b1e3731354");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions31354Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel5aa73((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel5aa73?.isDisabled==null)
      {
        setcancel5aa73((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("button_update")){
        setbutton_update1b289((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(button_update1b289?.isDisabled==null)
      {
        setbutton_update1b289((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("save")){
        setsave02eeb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(save02eeb?.isDisabled==null)
      {
        setsave02eeb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions31354Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions31354Ref.current?.setSearchParams();
    dynamicactions31354Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions31354Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions31354Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions31354) && Object.keys(dynamicactions31354)?.length>0)
      {
        setdynamicactions31354({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions31354Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
