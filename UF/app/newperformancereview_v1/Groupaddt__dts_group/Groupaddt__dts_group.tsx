...
const Groupaddt__dts_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "23b0089287ff44468b7a08377e67fd81");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddt__dts_group7fd81Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("add_inf")){
        setadd_inf6c4b2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_inf6c4b2?.isDisabled==null)
      {
        setadd_inf6c4b2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_dts")){
        setadd_dts40b3d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_dts40b3d?.isDisabled==null)
      {
        setadd_dts40b3d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const addt__dts_group7fd81Ref = useRef<any>(null);
  const handleClearSearch = () => {
    addt__dts_group7fd81Ref.current?.setSearchParams();
    addt__dts_group7fd81Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addt__dts_group7fd81) && Object.keys(addt__dts_group7fd81)?.length>0)
      {
        setaddt__dts_group7fd81({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addt__dts_group7fd81Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"addt__dts_group"}))
        }}
    >
          {allowedControls.includes("add_inf") ?<Textadd_inf   /* 6c4b2 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("add_dts") ?<TextAreaadd_dts   /* 40b3d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
