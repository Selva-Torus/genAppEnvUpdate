 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "8678948175a445f999f504cfc5213305"
    );
  ...
  },[full_name13305?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__groupdb1de((prev: any) => ({ ...prev,full_name13305: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__groupdb1de;
      delete temp.full_name;
      delete temp.FULL_NAME;
      delete temp.full_name13305;
      setaccess_req__groupdb1de(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,full_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__groupdb1de.full_name){
      if(Array.isArray(dfd_employeenamecombo_v1Props)){
        if(dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)){
          setdropdownValue([dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)?.full_name])
          tempValue=dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)?.full_name
        }else{
          setdropdownValue([access_req__groupdb1de.full_name])
          tempValue=access_req__groupdb1de.full_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{full_name:access_req__groupdb1de.full_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)?.full_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__groupdb1de.full_name)?.full_name
      }else{
        setdropdownValue([access_req__groupdb1de.full_name])
        tempValue=access_req__groupdb1de.full_name
      }   
    ...
    if(full_name13305?.trigger == true)
    {
      await handlechange(tempValue)
      setfull_name13305((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__groupdb1de.full_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employeenamecombo_v1Props) && dfd_employeenamecombo_v1Props?.length == 1){
    // setaccess_req__groupdb1de((pre:any)=>({...pre,full_name:dfd_employeenamecombo_v1Props[0]?.full_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__groupdb1de((prev: any) => ({ ...prev, full_name: getMapperDetailsBindValues[value],FULL_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__groupdb1de((prev: any) => ({ ...prev, full_name: '', full_name13305: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,full_name:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__groupdb1de?.full_name == "" || access_req__groupdb1de?.full_name == undefined){
      access_req__groupdb1de.full_name = "";
      const validate:any = v.safeParse(schema, access_req__groupdb1de?.full_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,full_name:"invalid"}}));
        }
    }else if(access_req__groupdb1de?.full_name !== ""){
    const validate:any = v.safeParse(schema, access_req__groupdb1de?.full_name);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,full_name:"invalid"}}));
  ...
  useEffect(() => { access_req__groupdb1deRef.current = access_req__groupdb1de; }, [access_req__groupdb1de]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "8678948175a445f999f504cfc5213305") {
        handleClick(access_req__groupdb1deRef?.current?.full_name13305?access_req__groupdb1deRef?.current?.full_name13305:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "8678948175a445f999f504cfc5213305");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__groupdb1de((pre:any)=>({...pre,full_name:""}))
    else
      setInitialCount(1)
  },[full_name13305?.refresh])
  

  if (full_name13305?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {full_name13305?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__groupdb1de?.full_name13305 ? [access_req__groupdb1de?.full_name13305] : (access_req__groupdb1de?.full_name ? dropdownValue : [])}
        validationState={validate?.newPerformanceReview_v1?.full_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
