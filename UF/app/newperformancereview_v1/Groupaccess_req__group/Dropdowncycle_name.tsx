 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "5759a849598d4f409bf9ebbec66855a5"
    );
  ...
  },[cycle_name855a5?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__groupdb1de((prev: any) => ({ ...prev,cycle_name855a5: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__groupdb1de;
      delete temp.cycle_name;
      delete temp.CYCLE_NAME;
      delete temp.cycle_name855a5;
      setaccess_req__groupdb1de(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,cycle_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__groupdb1de.cycle_name){
      if(Array.isArray(dfd_cyclenamecombo_v1Props)){
        if(dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)){
          setdropdownValue([dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)?.cycle_name])
          tempValue=dfd_cyclenamecombo_v1Props?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)?.cycle_name
        }else{
          setdropdownValue([access_req__groupdb1de.cycle_name])
          tempValue=access_req__groupdb1de.cycle_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{cycle_name:access_req__groupdb1de.cycle_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)?.cycle_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.cycle_name === access_req__groupdb1de.cycle_name)?.cycle_name
      }else{
        setdropdownValue([access_req__groupdb1de.cycle_name])
        tempValue=access_req__groupdb1de.cycle_name
      }   
    ...
    if(cycle_name855a5?.trigger == true)
    {
      await handlechange(tempValue)
      setcycle_name855a5((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__groupdb1de.cycle_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_cyclenamecombo_v1Props) && dfd_cyclenamecombo_v1Props?.length == 1){
    // setaccess_req__groupdb1de((pre:any)=>({...pre,cycle_name:dfd_cyclenamecombo_v1Props[0]?.cycle_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__groupdb1de((prev: any) => ({ ...prev, cycle_name: getMapperDetailsBindValues[value],CYCLE_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__groupdb1de((prev: any) => ({ ...prev, cycle_name: '', cycle_name855a5: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,cycle_name:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__groupdb1de?.cycle_name == "" || access_req__groupdb1de?.cycle_name == undefined){
      access_req__groupdb1de.cycle_name = "";
      const validate:any = v.safeParse(schema, access_req__groupdb1de?.cycle_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,cycle_name:"invalid"}}));
        }
    }else if(access_req__groupdb1de?.cycle_name !== ""){
    const validate:any = v.safeParse(schema, access_req__groupdb1de?.cycle_name);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,cycle_name:"invalid"}}));
  ...
  useEffect(() => { access_req__groupdb1deRef.current = access_req__groupdb1de; }, [access_req__groupdb1de]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "5759a849598d4f409bf9ebbec66855a5") {
        handleClick(access_req__groupdb1deRef?.current?.cycle_name855a5?access_req__groupdb1deRef?.current?.cycle_name855a5:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "5759a849598d4f409bf9ebbec66855a5");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__groupdb1de((pre:any)=>({...pre,cycle_name:""}))
    else
      setInitialCount(1)
  },[cycle_name855a5?.refresh])
  

  if (cycle_name855a5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {cycle_name855a5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__groupdb1de?.cycle_name855a5 ? [access_req__groupdb1de?.cycle_name855a5] : (access_req__groupdb1de?.cycle_name ? dropdownValue : [])}
        validationState={validate?.newPerformanceReview_v1?.cycle_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
