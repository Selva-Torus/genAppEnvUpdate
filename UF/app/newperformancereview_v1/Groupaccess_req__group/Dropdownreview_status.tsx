 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "84204320c977413c9c4a24b48144ba57"
    );
  ...
  },[review_status4ba57?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__groupdb1de((prev: any) => ({ ...prev,review_status4ba57: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__groupdb1de;
      delete temp.review_status;
      delete temp.REVIEW_STATUS;
      delete temp.review_status4ba57;
      setaccess_req__groupdb1de(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,review_status:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__groupdb1de.review_status){
      if(Array.isArray(dfd_reviewstatuscombo_v1Props)){
        if(dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)){
          setdropdownValue([dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)?.perf_review_status])
          tempValue=dfd_reviewstatuscombo_v1Props?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)?.perf_review_status
        }else{
          setdropdownValue([access_req__groupdb1de.review_status])
          tempValue=access_req__groupdb1de.review_status
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{perf_review_status:access_req__groupdb1de.review_status}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)?.perf_review_status ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.perf_review_status === access_req__groupdb1de.review_status)?.perf_review_status
      }else{
        setdropdownValue([access_req__groupdb1de.review_status])
        tempValue=access_req__groupdb1de.review_status
      }   
    ...
    if(review_status4ba57?.trigger == true)
    {
      await handlechange(tempValue)
      setreview_status4ba57((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__groupdb1de.review_status, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_reviewstatuscombo_v1Props) && dfd_reviewstatuscombo_v1Props?.length == 1){
    // setaccess_req__groupdb1de((pre:any)=>({...pre,review_status:dfd_reviewstatuscombo_v1Props[0]?.review_status}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__groupdb1de((prev: any) => ({ ...prev, review_status: getMapperDetailsBindValues[value],REVIEW_STATUS: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__groupdb1de((prev: any) => ({ ...prev, review_status: '', review_status4ba57: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,review_status:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__groupdb1de?.review_status == "" || access_req__groupdb1de?.review_status == undefined){
      access_req__groupdb1de.review_status = "";
      const validate:any = v.safeParse(schema, access_req__groupdb1de?.review_status);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,review_status:"invalid"}}));
        }
    }else if(access_req__groupdb1de?.review_status !== ""){
    const validate:any = v.safeParse(schema, access_req__groupdb1de?.review_status);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,review_status:"invalid"}}));
  ...
  useEffect(() => { access_req__groupdb1deRef.current = access_req__groupdb1de; }, [access_req__groupdb1de]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "84204320c977413c9c4a24b48144ba57") {
        handleClick(access_req__groupdb1deRef?.current?.review_status4ba57?access_req__groupdb1deRef?.current?.review_status4ba57:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "84204320c977413c9c4a24b48144ba57");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__groupdb1de((pre:any)=>({...pre,review_status:""}))
    else
      setInitialCount(1)
  },[review_status4ba57?.refresh])
  

  if (review_status4ba57?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {review_status4ba57?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__groupdb1de?.review_status4ba57 ? [access_req__groupdb1de?.review_status4ba57] : (access_req__groupdb1de?.review_status ? dropdownValue : [])}
        validationState={validate?.newPerformanceReview_v1?.review_status ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
