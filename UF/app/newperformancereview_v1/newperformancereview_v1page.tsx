...
export default function PageNewperformancereviewV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function addperformancereviewmodify_v1DFD(pagination:any): Promise<void>{
        ...
        if(newperformancereview_v1Props.length > 0){
          for(let i=0;i< newperformancereview_v1Props.length;i++){
            if(newperformancereview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceReviewModify:AFVK:v1"){
              // delete newperformancereview_v1Props[i].DFDkey;
              let temp=structuredClone(newperformancereview_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function employeenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(newperformancereview_v1Props.length > 0){
          for(let i=0;i< newperformancereview_v1Props.length;i++){
            if(newperformancereview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employeeNameCombo:AFVK:v1"){
              // delete newperformancereview_v1Props[i].DFDkey;
              let temp=structuredClone(newperformancereview_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function cyclenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(newperformancereview_v1Props.length > 0){
          for(let i=0;i< newperformancereview_v1Props.length;i++){
            if(newperformancereview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:cycleNameCombo:AFVK:v1"){
              // delete newperformancereview_v1Props[i].DFDkey;
              let temp=structuredClone(newperformancereview_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function reviewstatuscombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(newperformancereview_v1Props.length > 0){
          for(let i=0;i< newperformancereview_v1Props.length;i++){
            if(newperformancereview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:reviewStatusCombo:AFVK:v1"){
              // delete newperformancereview_v1Props[i].DFDkey;
              let temp=structuredClone(newperformancereview_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function reviewtypecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(newperformancereview_v1Props.length > 0){
          for(let i=0;i< newperformancereview_v1Props.length;i++){
            if(newperformancereview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:reviewTypeCombo:AFVK:v1"){
              // delete newperformancereview_v1Props[i].DFDkey;
              let temp=structuredClone(newperformancereview_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setnewperformancereview_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setnewperformancereview_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(newperformancereview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(newperformancereview_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setnewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
