...
import TextAreaimprovements  from "./TextAreaimprovements";
...
const Groupaddt__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {commentsd3579, setcommentsd3579}= useContext(TotalContext) as TotalContextProps;
  const {improvementsd073c, setimprovementsd073c}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "91c60bc2456be5dafe724ba519f82d26");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddt__group82d26Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("comments")){
        setcommentsd3579((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(commentsd3579?.isDisabled==null)
      {
        setcommentsd3579((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("improvements")){
        setimprovementsd073c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(improvementsd073c?.isDisabled==null)
      {
        setimprovementsd073c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['comments'] = commentsd3579,
        codeStates['setcomments'] = setcommentsd3579,
        codeStates['improvements'] = improvementsd073c,
        codeStates['setimprovements'] = setimprovementsd073c,
  ...
  const addt__group82d26Ref = useRef<any>(null);
  const handleClearSearch = () => {
    addt__group82d26Ref.current?.setSearchParams();
    addt__group82d26Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addt__group82d26) && Object.keys(addt__group82d26)?.length>0)
      {
        setaddt__group82d26({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addt__group82d26Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewperformancereview_v1((pre:any)=>({...pre,_selectedGroup_:"addt__group"}))
        }}
    >
          {allowedControls.includes("comments") ?<Textcomments   /* d3579 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("improvements") ?<TextAreaimprovements   /* d073c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}

export default Groupaddt__group
