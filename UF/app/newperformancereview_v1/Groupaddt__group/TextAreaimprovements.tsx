...
const TextAreaimprovements = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "91c60bc2456be5dafe724ba519f82d26",
        "935727345aa4488f8bea6d2c081d073c"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[improvementsd073c?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setaddt__group82d26((pre:any)=>({...pre,improvements:""}))
    }else 
      prevRefreshRef.current= true
  },[improvementsd073c?.refresh])

  const addt__group82d26Ref = useRef<any>(addt__group82d26);
  useEffect(() => { addt__group82d26Ref.current = addt__group82d26; }, [addt__group82d26]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "935727345aa4488f8bea6d2c081d073c") {
        handleChange({target:{value:addt__group82d26Ref?.current?.improvements||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "935727345aa4488f8bea6d2c081d073c") {
        handleBlur({target:{value:addt__group82d26Ref?.current?.improvements||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,newPerformanceReview_v1:{...pre?.newPerformanceReview_v1,improvements:undefined}}));
    setaddt__group82d26((prev: any) => ({ ...prev, improvements: e?.target?.value }));
    try{
    }catch (err: any) {
...
return (
  ...
  style={{gridColumn: `1 / 25`,gridRow: `7 / 24`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {improvementsd073c?.isDisabled ? true : false}
      placeholder = {'type here...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Improvements"
      pin = {'brick-brick'}
      value = { addt__group82d26?.improvements != null && typeof addt__group82d26?.improvements =='object' ? Object.keys(addt__group82d26?.improvements)?.length ?  JSON.stringify(addt__group82d26?.improvements,null ,2):"" : addt__group82d26?.improvements||""}
      errorMessage={error}
      validationState={validate?.newPerformanceReview_v1?.improvements ? "invalid" : undefined}
    />
  </div>
  )
}
