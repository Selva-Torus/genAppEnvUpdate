...
const Buttonschedule = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const dynamicactions8672dRef = useRef(dynamicactions8672d);
  useEffect(() => {
    dynamicactions8672dRef.current = dynamicactions8672d;
  ...
  const dynamicactions8672dPropsRef = useRef(dynamicactions8672dProps);
  useEffect(() => {
    dynamicactions8672dPropsRef.current = dynamicactions8672dProps;
  ...
  const handleMapper=async (data?:any) => {
    try{     
      data = {...data,...dynamicactions8672dRef.current};
      let parentRowSpan = 10;
      const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "4edd4e4a1df7412b875b53d143d8672d",
        "d628ec7437604cff879a668d048133fb"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    }else if(dynamicactions8672dProps?.isHaveRule==true){
      if("schedule" in dynamicactions8672dProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions8672dProps?.dynamicActionRule?.schedule,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("schedule" in logmaintenance_v1?.dynamicactions && logmaintenance_v1?.dynamicactions["schedule"]?.itsHaveArtifact== true)
      {
        setShowFlag(logmaintenance_v1?.dynamicactions["schedule"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(logmaintenance_v1?.dynamicactions?.schedule?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "d628ec7437604cff879a668d048133fb") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "schedule133fb");
    return () => {
  ...
  useEffect(()=>{
    if (!schedule133fb?.trigger) return;
      if(schedule133fb?.trigger){
      setschedule133fb((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[schedule133fb?.trigger])

  useEffect(()=>{
    if(schedule133fb?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[schedule133fb?.refresh])
  ...
  async function handleSave33fb_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const dynamicactions8672dProps = dynamicactions8672dPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.logMaintenance_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(dynamicactions8672dRef.current);
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|f5a6cd2800cc470dadf1b607de22f9db"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|f5a6cd2800cc470dadf1b607de22f9db"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
      let eventProperty :any = {
  "id": "d628ec7437604cff879a668d048133fb",
  "type": "button",
  "name": "schedule",
  "label": "schedule",
  "sequence": 1,
  "children": [
    {
      "id": "d628ec7437604cff879a668d048133fb.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "d628ec7437604cff879a668d048133fb.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "saveHandler",
          ...
          "children": [
            {
              "id": "d628ec7437604cff879a668d048133fb.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Add Schedule Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|f5a6cd2800cc470dadf1b607de22f9db"
          ]
        }
      ]
    }
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:logMaintenance:AFVK:v1";
      sourceId+= "|"+"4edd4e4a1df7412b875b53d143d8672d";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:logMaintenance:AFVK:v1"+"|"+"4edd4e4a1df7412b875b53d143d8672d"+"|"+eventProperty.id;
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...dynamicactions8672dRef.current, ...nullFilter(maintenance_groupdb5a7), ...nullFilter(maintenance_information_groupea3ac), ...nullFilter(execution_details_group591cd) });
    let reworkKeys:any[]=[];
    ...
    if("_groupArrays_" in reworkedObject )
    {
      ...
      if (uf_getPFDetails.key != undefined) {
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"4edd4e4a1df7412b875b53d143d8672d",
              controlId:"d628ec7437604cff879a668d048133fb"
            };
            if (encryptionFlagCont) {
        ...
        te_saveBody.controlName = "schedule";
        if(mainData?.upId)
        {
        ...
        if(dynamicactions8672d?.upId){
          te_saveBody['upId']= dynamicactions8672d?.upId;
        }
        if(dynamicactions8672d?.upid){
          te_saveBody['upId']= dynamicactions8672d?.upid;
        }
        ...
        if (dynamicactions8672dProps.ssKey !== '' && dynamicactions8672dProps.ssKey !== undefined) {
          te_saveBody["ssKey"] = dynamicactions8672dProps.ssKey;
    ...
    //infoMsg
    toast('Add Schedule Successfully', 'success',true);
    }
    catch(err:any)
    {
      savedData.current = {};
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions8672d((prev: any) => ({ ...prev, schedule: true }));
        //onClick

    //saveHandler
    await handleSave33fb_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions8672d((prev: any) => ({ ...prev, schedule: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [logmaintenance_v1?.dynamicactions?.schedule,dynamicactions8672dProps?.dynamicActionRule?.schedule,])

 if (schedule133fb?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {schedule133fb?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
