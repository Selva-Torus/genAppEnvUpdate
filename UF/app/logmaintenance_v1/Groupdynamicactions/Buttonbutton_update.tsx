 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "503acb50d28042d4b7101e687f95cdad"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactions8672dProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions8672dProps?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in logmaintenance_v1?.dynamicactions && logmaintenance_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(logmaintenance_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(logmaintenance_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_update5cdad") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "503acb50d28042d4b7101e687f95cdad") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_update5cdad");
    return () => {
  ...
  useEffect(()=>{
    if (!button_update5cdad?.trigger) return;
      if(button_update5cdad?.trigger){
      setbutton_update5cdad((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_update5cdad?.trigger])

  useEffect(()=>{
    if(button_update5cdad?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_update5cdad?.refresh])
  ...
  async function handleSavecdad_1_1_1(){
  ...
  return val === 'invalid';
    ...
    try{
      ...
      let lockedKeysLength:number;
        if(dynamicactions8672d?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "503acb50d28042d4b7101e687f95cdad",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "503acb50d28042d4b7101e687f95cdad.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "503acb50d28042d4b7101e687f95cdad.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "503acb50d28042d4b7101e687f95cdad.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Schedule Updated Successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
            ...
            {
              "id": "503acb50d28042d4b7101e687f95cdad.1.1.1.2",
              ...
              "children": [
                {
                  ...
                  "value": "",
                  "type": "screen",
                  "name": "assetMaintenance.v1|maintenance_table",
                  "label": "assetMaintenance.v1|maintenance_table",
                  "key": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|maintenance_table",
                  "elementType": "group",
                  "groupType": "table",
                  "sequence": "1.1.1.2.1",
                  "children": []
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactions8672d?.upId? [dynamicactions8672d?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions8672d?.upId? [dynamicactions8672d?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactions8672d?.upId? [dynamicactions8672d?.upId ] : dynamicactions8672d?.trs_process_id? [dynamicactions8672d?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"503acb50d28042d4b7101e687f95cdad"
            };
            if (encryptionFlagCont) {
    ...
    te_eventEmitterBody["ssKey"] = dynamicactions8672dProps.ssKey;          
    }
    ...
    //infoMsg
    toast('Schedule Updated Successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
    setmaintenance_table75a5dProps((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions8672d((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSavecdad_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions8672d((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [logmaintenance_v1?.dynamicactions?.button_update,dynamicactions8672dProps?.dynamicActionRule?.button_update,])

 if (button_update5cdad?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_update5cdad?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
