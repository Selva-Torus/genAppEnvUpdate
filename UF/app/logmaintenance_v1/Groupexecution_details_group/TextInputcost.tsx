...
const TextInputcost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|a3386808f53949eebff60bf867dcc7a3|properties.cost"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:logMaintenance:AFVK:v1|6f620f2c13924269ac67da12e7f591cd|a64dcd05af544eddb5e904c7d5e35190"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cost',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setexecution_details_group591cd((prev: any) => ({ ...prev, cost: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(execution_details_group591cd?.cost == "" || execution_details_group591cd?.cost == undefined){
      execution_details_group591cd.cost = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, execution_details_group591cd?.cost);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,cost:"invalid"}}));
        }
    }else if(execution_details_group591cd?.cost !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +execution_details_group591cd?.cost);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a64dcd05af544eddb5e904c7d5e35190"
      );
      ...
      //     controlId: "a64dcd05af544eddb5e904c7d5e35190",
      //     isTable: false,
      //     from:"TextInputcost",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cost', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cost',type:'text'};
      //   type={
      //     name:'cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cost.type
      //   }
      ...
      //   type={
      //     name:'cost',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cost.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cost.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cost.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!execution_details_group591cd?.cost)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "a64dcd05af544eddb5e904c7d5e35190") {
        handleChange({target:{value:execution_details_group591cdRef?.current?.cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a64dcd05af544eddb5e904c7d5e35190") {
        handleBlur({target:{value:execution_details_group591cdRef?.current?.cost||""}});
      }
    };
  ...
  if(dfd_assetmaintenance_v1Props?.setSearchFilters && dfd_assetmaintenance_v1Props?.data)
  {
    if(Array.isArray(dfd_assetmaintenance_v1Props.data) && dfd_assetmaintenance_v1Props.data.length > 0){
      setexecution_details_group591cd((pre:any)=>({...pre,cost:dfd_assetmaintenance_v1Props.data[0]?.cost}));
    }
  }
  },[dfd_assetmaintenance_v1Props?.setSearchFilters])
  if (cost35190?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={execution_details_group591cd?.cost||""}
         disabled= {cost35190?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.logMaintenance_v1?.cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
