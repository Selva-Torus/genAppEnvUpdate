...
const TextInputvendor_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|a3386808f53949eebff60bf867dcc7a3|properties.vendor_reference"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:logMaintenance:AFVK:v1|6f620f2c13924269ac67da12e7f591cd|2b264b4a9fe6456e8d441afb049fa982"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'vendor_reference',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,vendor_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setexecution_details_group591cd((prev: any) => ({ ...prev, vendor_reference: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(execution_details_group591cd?.vendor_reference == "" || execution_details_group591cd?.vendor_reference == undefined){
      execution_details_group591cd.vendor_reference = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, execution_details_group591cd?.vendor_reference);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,vendor_reference:"invalid"}}));
        }
    }else if(execution_details_group591cd?.vendor_reference !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +execution_details_group591cd?.vendor_reference);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "2b264b4a9fe6456e8d441afb049fa982"
      );
      ...
      //     controlId: "2b264b4a9fe6456e8d441afb049fa982",
      //     isTable: false,
      //     from:"TextInputvendor_reference",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'vendor_reference', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'vendor_reference',type:'text'};
      //   type={
      //     name:'vendor_reference',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vendor_reference.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vendor_reference.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vendor_reference.type
      //   }
      ...
      //   type={
      //     name:'vendor_reference',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vendor_reference.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vendor_reference.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vendor_reference.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!execution_details_group591cd?.vendor_reference)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "2b264b4a9fe6456e8d441afb049fa982") {
        handleChange({target:{value:execution_details_group591cdRef?.current?.vendor_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2b264b4a9fe6456e8d441afb049fa982") {
        handleBlur({target:{value:execution_details_group591cdRef?.current?.vendor_reference||""}});
      }
    };
  ...
  if(dfd_assetmaintenance_v1Props?.setSearchFilters && dfd_assetmaintenance_v1Props?.data)
  {
    if(Array.isArray(dfd_assetmaintenance_v1Props.data) && dfd_assetmaintenance_v1Props.data.length > 0){
      setexecution_details_group591cd((pre:any)=>({...pre,vendor_reference:dfd_assetmaintenance_v1Props.data[0]?.vendor_reference}));
    }
  }
  },[dfd_assetmaintenance_v1Props?.setSearchFilters])
  if (vendor_referencefa982?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={execution_details_group591cd?.vendor_reference||""}
         disabled= {vendor_referencefa982?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Vendor Reference"
      errorMessage={error}
        validationState={validate?.logMaintenance_v1?.vendor_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputvendor_reference
