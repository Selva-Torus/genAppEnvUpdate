...
const TextInputdowntime_hours = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1|a3386808f53949eebff60bf867dcc7a3|properties.downtime_hours"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:logMaintenance:AFVK:v1|6f620f2c13924269ac67da12e7f591cd|e583cd66b961410598acc99c1ba721c7"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'downtime_hours',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,downtime_hours:undefined}}));
    if(dynamicStateandType.type=="number"){
    setexecution_details_group591cd((prev: any) => ({ ...prev, downtime_hours: +e.target.value }));
  ...
  const handleBlur=async (e?:any) => {
      let validate:any
      if(execution_details_group591cd?.downtime_hours == "" || execution_details_group591cd?.downtime_hours == undefined){
      execution_details_group591cd.downtime_hours = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, execution_details_group591cd?.downtime_hours);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,downtime_hours:"invalid"}}));
        }
    }else if(execution_details_group591cd?.downtime_hours !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +execution_details_group591cd?.downtime_hours);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e583cd66b961410598acc99c1ba721c7"
      );
      ...
      //     controlId: "e583cd66b961410598acc99c1ba721c7",
      //     isTable: false,
      //     from:"TextInputdowntime_hours",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'downtime_hours', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'downtime_hours',type:'text'};
      //   type={
      //     name:'downtime_hours',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.downtime_hours.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.downtime_hours.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.downtime_hours.type
      //   }
      ...
      //   type={
      //     name:'downtime_hours',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.downtime_hours.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.downtime_hours.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.downtime_hours.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!execution_details_group591cd?.downtime_hours)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "e583cd66b961410598acc99c1ba721c7") {
        handleChange({target:{value:execution_details_group591cdRef?.current?.downtime_hours||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e583cd66b961410598acc99c1ba721c7") {
        handleBlur({target:{value:execution_details_group591cdRef?.current?.downtime_hours||""}});
      }
    };
  ...
  if(dfd_assetmaintenance_v1Props?.setSearchFilters && dfd_assetmaintenance_v1Props?.data)
  {
    if(Array.isArray(dfd_assetmaintenance_v1Props.data) && dfd_assetmaintenance_v1Props.data.length > 0){
      setexecution_details_group591cd((pre:any)=>({...pre,downtime_hours:dfd_assetmaintenance_v1Props.data[0]?.downtime_hours}));
    }
  }
  },[dfd_assetmaintenance_v1Props?.setSearchFilters])
  if (downtime_hours721c7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className=""
        label={keyset("Downtime Hours")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={execution_details_group591cd?.downtime_hours||""}
         disabled= {downtime_hours721c7?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Downtime Hours"
      errorMessage={error}
        validationState={validate?.logMaintenance_v1?.downtime_hours ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdowntime_hours
