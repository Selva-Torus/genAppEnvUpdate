...
const DatePickerscheduled_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,scheduled_date: "invalid"}}))
    setmaintenance_information_groupea3ac((prev: any) => ({ ...prev, scheduled_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,logMaintenance_v1:{...pre?.logMaintenance_v1,scheduled_date:undefined}}));
  if (!date) {
    setmaintenance_information_groupea3ac((prev: any) => ({ ...prev, scheduled_date: null }));
    return;
  }
  ...
  setmaintenance_information_groupea3ac((prev: any) => ({ ...prev, scheduled_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(maintenance_information_groupea3ac?.scheduled_date == "" || maintenance_information_groupea3ac?.scheduled_date == undefined){
        const result = v.safeParse(schema, maintenance_information_groupea3ac?.scheduled_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "f6d0387b601b4908ae806b58f2683e9d"
      );
...
useEffect(()=>{
 ...
 },[scheduled_date83e9d?.refresh])

useEffect(()=>{
  if(!maintenance_information_groupea3ac?.scheduled_date){ 
...
if (scheduled_date83e9d?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      ...
      //label={keyset("")}
      value={maintenance_information_groupea3ac?.scheduled_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {scheduled_date83e9d?.isDisabled ? true : false}
      disabled= {scheduled_date83e9d?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Scheduled Date"
      dateValidation=""
      validationState={validate?.logMaintenance_v1?.scheduled_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerscheduled_date
