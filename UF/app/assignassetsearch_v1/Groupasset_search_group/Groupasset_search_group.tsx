...
const Groupasset_search_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "dfee2b5fd26b613feaa59c7429c75d0d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setasset_search_group75d0dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("advancesearch")){
        setadvancesearch9256e({...advancesearch9256e,isDisabled:true});

    }else
    {
      if(advancesearch9256e?.isDisabled==null)
      {
        setadvancesearch9256e({...advancesearch9256e,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const asset_search_group75d0dRef = useRef<any>(null);
  const handleClearSearch = () => {
    asset_search_group75d0dRef.current?.setSearchParams();
    asset_search_group75d0dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(asset_search_group75d0d) && Object.keys(asset_search_group75d0d)?.length>0)
      {
        setasset_search_group75d0d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [asset_search_group75d0dProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("advancesearch")?<AdvancedSearchadvancesearch /* 9256e */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
