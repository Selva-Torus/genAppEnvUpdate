...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,valid_to: "invalid"}}))
    setvalid_group6c83b((prev: any) => ({ ...prev, valid_to: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,valid_to:undefined}}));
  if (!date) {
    setvalid_group6c83b((prev: any) => ({ ...prev, valid_to: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_group6c83b((prev: any) => ({ ...prev, valid_to: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_group6c83b?.valid_to == "" || valid_group6c83b?.valid_to == undefined){
        const result = v.safeParse(schema, valid_group6c83b?.valid_to || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "91209634f78d9bb819bc92e9441e6a41"
      );
...
useEffect(()=>{
 ...
 },[valid_toe6a41?.refresh])

useEffect(()=>{
  if(!valid_group6c83b?.valid_to){ 
...
return (
  ...
  style={{gridColumn: `9 / 17`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_group6c83b?.valid_to}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {valid_toe6a41?.isDisabled ? true : false}
      disabled= {valid_toe6a41?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Valid To"
      dateValidation=""
      validationState={validate?.viewAccessRequest_v1?.valid_to ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
