 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      "a2702aa8d9fe62a9a4f4d9da8fc0077c"
    );
  ...
  useEffect(() => {
    if(prevRefreshRef.current)
      setvalid_group6c83b((pre:any)=>({...pre,access_review_required:null}))
    else
      prevRefreshRef.current=true
    handleMapperValue()
  },[access_review_req_switch0077c?.refresh])

  useEffect(() => {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props) && dfd_addaccessrequestmodify_v1Props?.length == 1){
      setvalid_group6c83b((pre:any)=>({...pre,access_review_required:dfd_addaccessrequestmodify_v1Props[0]?.access_review_required}))
    }
  },[dfd_addaccessrequestmodify_v1Props])
  const handleChange = async (checked: boolean,comingRule:any={}) => {
    try{
    setIsProcessing(true);
    setvalid_group6c83b((prev: any) => ({ ...prev, access_review_required: checked }));
  ...
  if (access_review_req_switch0077c?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Switch
        ...
        headerPosition="top"
        disabled= {access_review_req_switch0077c?.isDisabled ? true : false}
        content=" "
        checked={valid_group6c83b?.access_review_required || false} 
