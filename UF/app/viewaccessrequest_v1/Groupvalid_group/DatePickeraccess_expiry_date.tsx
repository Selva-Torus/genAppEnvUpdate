...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_expiry_date: "invalid"}}))
    setvalid_group6c83b((prev: any) => ({ ...prev, access_expiry_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_expiry_date:undefined}}));
  if (!date) {
    setvalid_group6c83b((prev: any) => ({ ...prev, access_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_group6c83b((prev: any) => ({ ...prev, access_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_group6c83b?.access_expiry_date == "" || valid_group6c83b?.access_expiry_date == undefined){
        const result = v.safeParse(schema, valid_group6c83b?.access_expiry_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "9752ab4f1e4c10a0e561aa8123dea669"
      );
...
useEffect(()=>{
 ...
 },[access_expiry_dateea669?.refresh])

useEffect(()=>{
  if(!valid_group6c83b?.access_expiry_date){ 
...
if (access_expiry_dateea669?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_group6c83b?.access_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {access_expiry_dateea669?.isDisabled ? true : false}
      disabled= {access_expiry_dateea669?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Access Expiry Date"
      dateValidation=""
      validationState={validate?.viewAccessRequest_v1?.access_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
