...
const Groupbusiness_just__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5b347cd1b3cfeab295dc168432b2c68d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbusiness_just__group2c68dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("business_justify")){
        setbusiness_justify89ef9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_justify89ef9?.isDisabled==null)
      {
        setbusiness_justify89ef9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("business_justification")){
        setbusiness_justification899e2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(business_justification899e2?.isDisabled==null)
      {
        setbusiness_justification899e2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("additional_details")){
        setadditional_detailscbef3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(additional_detailscbef3?.isDisabled==null)
      {
        setadditional_detailscbef3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const business_just__group2c68dRef = useRef<any>(null);
  const handleClearSearch = () => {
    business_just__group2c68dRef.current?.setSearchParams();
    business_just__group2c68dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(business_just__group2c68d) && Object.keys(business_just__group2c68d)?.length>0)
      {
        setbusiness_just__group2c68d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [business_just__group2c68dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"business_just__group"}))
        }}
    >
          {allowedControls.includes("business_justify") ?<Textbusiness_justify   /* 89ef9 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("business_justification") ?<TextAreabusiness_justification   /* 899e2 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("additional_details") ?<TextAreaadditional_details   /* cbef3 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
