...
const TextAreareject_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "a2fbf0d4123fb9632097ff4b9715ad5c",
        "fbf9d7865f24556e20c24809f991b1db"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[reject_reason1b1db?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_inf_group5ad5c((pre:any)=>({...pre,rejection_reason:""}))
    }else 
      prevRefreshRef.current= true
  },[reject_reason1b1db?.refresh])

  const app_inf_group5ad5cRef = useRef<any>(app_inf_group5ad5c);
  useEffect(() => { app_inf_group5ad5cRef.current = app_inf_group5ad5c; }, [app_inf_group5ad5c]);
  useEffect(()=>{
      handleMapperValue();
    if(validateRefetch.init!=0)
      handleValidate();
    const handlerChange = (id:any) => {
      if (id === "fbf9d7865f24556e20c24809f991b1db") {
        handleChange({target:{value:app_inf_group5ad5cRef?.current?.rejection_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "fbf9d7865f24556e20c24809f991b1db") {
        handleBlur({target:{value:app_inf_group5ad5cRef?.current?.rejection_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,rejection_reason:undefined}}));
    setapp_inf_group5ad5c((prev: any) => ({ ...prev, rejection_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (reject_reason1b1db?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `7 / 36`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {reject_reason1b1db?.isDisabled ? true : false}
      placeholder = {'Enter rejection reason if any...'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Rejection Reason"
      pin = {'brick-brick'}
      value = { app_inf_group5ad5c?.rejection_reason != null && typeof app_inf_group5ad5c?.rejection_reason =='object' ? Object.keys(app_inf_group5ad5c?.rejection_reason)?.length ?  JSON.stringify(app_inf_group5ad5c?.rejection_reason,null ,2):"" : app_inf_group5ad5c?.rejection_reason||""}
      errorMessage={error}
      validationState={validate?.viewAccessRequest_v1?.rejection_reason ? "invalid" : undefined}
    />
  </div>
  )
}
