...
const Groupprov_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {prov_inf0c656, setprov_inf0c656}= useContext(TotalContext) as TotalContextProps;
  const {prov_inf_des7e23f, setprov_inf_des7e23f}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0af38320458e5fce14e670b2cf0ce05f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprov_groupce05fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prov_inf")){
        setprov_inf0c656((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf0c656?.isDisabled==null)
      {
        setprov_inf0c656((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_inf_des")){
        setprov_inf_des7e23f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_inf_des7e23f?.isDisabled==null)
      {
        setprov_inf_des7e23f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['prov_inf'] = prov_inf0c656,
        codeStates['setprov_inf'] = setprov_inf0c656,
        codeStates['prov_inf_des'] = prov_inf_des7e23f,
        codeStates['setprov_inf_des'] = setprov_inf_des7e23f,
  ...
  const prov_groupce05fRef = useRef<any>(null);
  const handleClearSearch = () => {
    prov_groupce05fRef.current?.setSearchParams();
    prov_groupce05fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(prov_groupce05f) && Object.keys(prov_groupce05f)?.length>0)
      {
        setprov_groupce05f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [prov_groupce05fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"prov_group"}))
        }}
    >
          {allowedControls.includes("prov_inf") ?<Textprov_inf   /* 0c656 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("prov_inf_des") ?<Textprov_inf_des   /* 7e23f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
