...
const TextInputsystem_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1|009cd55942c4a593e51245660f2580cf|54f6fd03938f2cd5f744cc87b81c0d6b"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,system_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group580cf((prev: any) => ({ ...prev, system_name: +e.target.value }));
    }
    else{
    setaccess_req__group580cf((prev: any) => ({ ...prev, system_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "54f6fd03938f2cd5f744cc87b81c0d6b"
      );
      ...
      //     controlId: "54f6fd03938f2cd5f744cc87b81c0d6b",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "54f6fd03938f2cd5f744cc87b81c0d6b") {
        handleChange({target:{value:access_req__group580cfRef?.current?.system_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "54f6fd03938f2cd5f744cc87b81c0d6b") {
        handleBlur({target:{value:access_req__group580cfRef?.current?.system_name||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setaccess_req__group580cf((pre:any)=>({...pre,system_name:dfd_addaccessrequestmodify_v1Props.data[0]?.system_name}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (system_namec0d6b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group580cf?.system_name||""}
         disabled= {system_namec0d6b?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewAccessRequest_v1?.system_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
