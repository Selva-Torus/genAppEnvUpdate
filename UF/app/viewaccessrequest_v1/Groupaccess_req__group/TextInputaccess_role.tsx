...
const TextInputaccess_role = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1|009cd55942c4a593e51245660f2580cf|4530da03dd9fe8a75ac1eee8d465bf05"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_role:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group580cf((prev: any) => ({ ...prev, access_role: +e.target.value }));
    }
    else{
    setaccess_req__group580cf((prev: any) => ({ ...prev, access_role: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "4530da03dd9fe8a75ac1eee8d465bf05"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1",
      //     componentId: "009cd55942c4a593e51245660f2580cf",
      //     controlId: "4530da03dd9fe8a75ac1eee8d465bf05",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "4530da03dd9fe8a75ac1eee8d465bf05") {
        handleChange({target:{value:access_req__group580cfRef?.current?.access_role||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "4530da03dd9fe8a75ac1eee8d465bf05") {
        handleBlur({target:{value:access_req__group580cfRef?.current?.access_role||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setaccess_req__group580cf((pre:any)=>({...pre,access_role:dfd_addaccessrequestmodify_v1Props.data[0]?.access_role}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (access_role5bf05?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__group580cf?.access_role||""}
         disabled= {access_role5bf05?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewAccessRequest_v1?.access_role ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
