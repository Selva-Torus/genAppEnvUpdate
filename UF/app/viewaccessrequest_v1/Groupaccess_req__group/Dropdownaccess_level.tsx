 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "821ffa79d4acc5c88e5b27799c64f3fa"
    );
  ...
  },[access_level4f3fa?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group580cf((prev: any) => ({ ...prev,access_level4f3fa: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group580cf;
      delete temp.access_level;
      delete temp.ACCESS_LEVEL;
      delete temp.access_level4f3fa;
      setaccess_req__group580cf(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_level:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group580cf.access_level){
      if(Array.isArray(dfd_accesslevelcombo_v1Props)){
        if(dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)){
          setdropdownValue([dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)?.document_access_scope])
          tempValue=dfd_accesslevelcombo_v1Props?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)?.document_access_scope
        }else{
          setdropdownValue([access_req__group580cf.access_level])
          tempValue=access_req__group580cf.access_level
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{document_access_scope:access_req__group580cf.access_level}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)?.document_access_scope ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.document_access_scope === access_req__group580cf.access_level)?.document_access_scope
      }else{
        setdropdownValue([access_req__group580cf.access_level])
        tempValue=access_req__group580cf.access_level
      }   
    ...
    if(access_level4f3fa?.trigger == true)
    {
      await handlechange(tempValue)
      setaccess_level4f3fa((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group580cf.access_level, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_accesslevelcombo_v1Props) && dfd_accesslevelcombo_v1Props?.length == 1){
    // setaccess_req__group580cf((pre:any)=>({...pre,access_level:dfd_accesslevelcombo_v1Props[0]?.access_level}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group580cf((prev: any) => ({ ...prev, access_level: getMapperDetailsBindValues[value],ACCESS_LEVEL: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group580cf((prev: any) => ({ ...prev, access_level: '', access_level4f3fa: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_level:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group580cf?.access_level == "" || access_req__group580cf?.access_level == undefined){
      access_req__group580cf.access_level = "";
      const validate:any = v.safeParse(schema, access_req__group580cf?.access_level);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_level:"invalid"}}));
        }
    }else if(access_req__group580cf?.access_level !== ""){
    const validate:any = v.safeParse(schema, access_req__group580cf?.access_level);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,access_level:"invalid"}}));
  ...
  useEffect(() => { access_req__group580cfRef.current = access_req__group580cf; }, [access_req__group580cf]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "821ffa79d4acc5c88e5b27799c64f3fa") {
        handleClick(access_req__group580cfRef?.current?.access_level4f3fa?access_req__group580cfRef?.current?.access_level4f3fa:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "821ffa79d4acc5c88e5b27799c64f3fa");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group580cf((pre:any)=>({...pre,access_level:""}))
    else
      setInitialCount(1)
  },[access_level4f3fa?.refresh])
  

  if (access_level4f3fa?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {access_level4f3fa?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group580cf?.access_level4f3fa ? [access_req__group580cf?.access_level4f3fa] : (access_req__group580cf?.access_level ? dropdownValue : [])}
        validationState={validate?.viewAccessRequest_v1?.access_level ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
