 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "1dcfbca6ec4dc468a593ca8ed23c3cd8"
    );
  ...
  },[full_namec3cd8?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group580cf((prev: any) => ({ ...prev,full_namec3cd8: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group580cf;
      delete temp.employee_name;
      delete temp.EMPLOYEE_NAME;
      delete temp.full_namec3cd8;
      setaccess_req__group580cf(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,employee_name:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group580cf.employee_name){
      if(Array.isArray(dfd_employeenamecombo_v1Props)){
        if(dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__group580cf.employee_name)){
          setdropdownValue([dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__group580cf.employee_name)?.full_name])
          tempValue=dfd_employeenamecombo_v1Props?.find((item: any) => item.full_name === access_req__group580cf.employee_name)?.full_name
        }else{
          setdropdownValue([access_req__group580cf.employee_name])
          tempValue=access_req__group580cf.employee_name
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{full_name:access_req__group580cf.employee_name}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__group580cf.employee_name)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__group580cf.employee_name)?.full_name ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.full_name === access_req__group580cf.employee_name)?.full_name
      }else{
        setdropdownValue([access_req__group580cf.employee_name])
        tempValue=access_req__group580cf.employee_name
      }   
    ...
    if(full_namec3cd8?.trigger == true)
    {
      await handlechange(tempValue)
      setfull_namec3cd8((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group580cf.employee_name, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_employeenamecombo_v1Props) && dfd_employeenamecombo_v1Props?.length == 1){
    // setaccess_req__group580cf((pre:any)=>({...pre,employee_name:dfd_employeenamecombo_v1Props[0]?.employee_name}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group580cf((prev: any) => ({ ...prev, employee_name: getMapperDetailsBindValues[value],EMPLOYEE_NAME: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group580cf((prev: any) => ({ ...prev, employee_name: '', full_namec3cd8: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,employee_name:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group580cf?.employee_name == "" || access_req__group580cf?.employee_name == undefined){
      access_req__group580cf.employee_name = "";
      const validate:any = v.safeParse(schema, access_req__group580cf?.employee_name);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,employee_name:"invalid"}}));
        }
    }else if(access_req__group580cf?.employee_name !== ""){
    const validate:any = v.safeParse(schema, access_req__group580cf?.employee_name);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,employee_name:"invalid"}}));
  ...
  useEffect(() => { access_req__group580cfRef.current = access_req__group580cf; }, [access_req__group580cf]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "1dcfbca6ec4dc468a593ca8ed23c3cd8") {
        handleClick(access_req__group580cfRef?.current?.full_namec3cd8?access_req__group580cfRef?.current?.full_namec3cd8:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "1dcfbca6ec4dc468a593ca8ed23c3cd8");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group580cf((pre:any)=>({...pre,employee_name:""}))
    else
      setInitialCount(1)
  },[full_namec3cd8?.refresh])
  

  if (full_namec3cd8?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {full_namec3cd8?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group580cf?.full_namec3cd8 ? [access_req__group580cf?.full_namec3cd8] : (access_req__group580cf?.employee_name ? dropdownValue : [])}
        validationState={validate?.viewAccessRequest_v1?.employee_name ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
