 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "667cb74260f7b964bf8fb4157d62dac4"
    );
  ...
  },[request_priority2dac4?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setaccess_req__group580cf((prev: any) => ({ ...prev,request_priority2dac4: value }))
      setIsRequredData(false)
    }else{
      let temp:any = access_req__group580cf;
      delete temp.request_priority;
      delete temp.REQUEST_PRIORITY;
      delete temp.request_priority2dac4;
      setaccess_req__group580cf(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,request_priority:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(access_req__group580cf.request_priority){
      if(Array.isArray(dfd_requestprioritycombo_v1Props)){
        if(dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)){
          setdropdownValue([dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)?.emp_request_priority])
          tempValue=dfd_requestprioritycombo_v1Props?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)?.emp_request_priority
        }else{
          setdropdownValue([access_req__group580cf.request_priority])
          tempValue=access_req__group580cf.request_priority
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{emp_request_priority:access_req__group580cf.request_priority}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)?.emp_request_priority ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.emp_request_priority === access_req__group580cf.request_priority)?.emp_request_priority
      }else{
        setdropdownValue([access_req__group580cf.request_priority])
        tempValue=access_req__group580cf.request_priority
      }   
    ...
    if(request_priority2dac4?.trigger == true)
    {
      await handlechange(tempValue)
      setrequest_priority2dac4((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[access_req__group580cf.request_priority, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_requestprioritycombo_v1Props) && dfd_requestprioritycombo_v1Props?.length == 1){
    // setaccess_req__group580cf((pre:any)=>({...pre,request_priority:dfd_requestprioritycombo_v1Props[0]?.request_priority}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setaccess_req__group580cf((prev: any) => ({ ...prev, request_priority: getMapperDetailsBindValues[value],REQUEST_PRIORITY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setaccess_req__group580cf((prev: any) => ({ ...prev, request_priority: '', request_priority2dac4: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,request_priority:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
    if(access_req__group580cf?.request_priority == "" || access_req__group580cf?.request_priority == undefined){
      access_req__group580cf.request_priority = "";
      const validate:any = v.safeParse(schema, access_req__group580cf?.request_priority);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,request_priority:"invalid"}}));
        }
    }else if(access_req__group580cf?.request_priority !== ""){
    const validate:any = v.safeParse(schema, access_req__group580cf?.request_priority);
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,request_priority:"invalid"}}));
  ...
  useEffect(() => { access_req__group580cfRef.current = access_req__group580cf; }, [access_req__group580cf]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "667cb74260f7b964bf8fb4157d62dac4") {
        handleClick(access_req__group580cfRef?.current?.request_priority2dac4?access_req__group580cfRef?.current?.request_priority2dac4:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "667cb74260f7b964bf8fb4157d62dac4");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setaccess_req__group580cf((pre:any)=>({...pre,request_priority:""}))
    else
      setInitialCount(1)
  },[request_priority2dac4?.refresh])
  

  if (request_priority2dac4?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {request_priority2dac4?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={access_req__group580cf?.request_priority2dac4 ? [access_req__group580cf?.request_priority2dac4] : (access_req__group580cf?.request_priority ? dropdownValue : [])}
        validationState={validate?.viewAccessRequest_v1?.request_priority ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
