...
const Groupprovision_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5f37cc88bca7ddd3a92f1469f1de166a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprovision_groupe166aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prov_group")){
        setprov_groupce05f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_groupce05f?.isDisabled==null)
      {
        setprov_groupce05f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("provisioning_status")){
        setprovisioning_statusa0dcf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(provisioning_statusa0dcf?.isDisabled==null)
      {
        setprovisioning_statusa0dcf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("prov_at")){
        setprov_at8b178((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(prov_at8b178?.isDisabled==null)
      {
        setprov_at8b178((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("provisioning_reference")){
        setprovisioning_reference8d6a1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(provisioning_reference8d6a1?.isDisabled==null)
      {
        setprovisioning_reference8d6a1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ticket_reference")){
        setticket_reference4e55b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ticket_reference4e55b?.isDisabled==null)
      {
        setticket_reference4e55b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const provision_groupe166aRef = useRef<any>(null);
  const handleClearSearch = () => {
    provision_groupe166aRef.current?.setSearchParams();
    provision_groupe166aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(provision_groupe166a) && Object.keys(provision_groupe166a)?.length>0)
      {
        setprovision_groupe166a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [provision_groupe166aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"provision_group"}))
        }}
    >
        ...
        {allowedControls.includes("provisioning_status") ?<Dropdownprovisioning_status   /* a0dcf */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("prov_at") ?<DatePickerprov_at   /* 8b178 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("provisioning_reference") ?<TextInputprovisioning_reference   /* 8d6a1 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ticket_reference") ?<TextInputticket_reference   /* 4e55b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
