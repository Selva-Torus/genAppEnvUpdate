...
const TextInputprovisioning_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addAccessRequestModify:AFVK:v1|21d269e30ee840848c3f8d33112a6ffd|properties.provisioning_reference"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1|5f37cc88bca7ddd3a92f1469f1de166a|82b792ddee326cbc3b3eae002748d6a1"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'provisioning_reference',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,provisioning_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setprovision_groupe166a((prev: any) => ({ ...prev, provisioning_reference: +e.target.value }));
    }
    else{
    setprovision_groupe166a((prev: any) => ({ ...prev, provisioning_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "82b792ddee326cbc3b3eae002748d6a1"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1",
      //     componentId: "5f37cc88bca7ddd3a92f1469f1de166a",
      //     controlId: "82b792ddee326cbc3b3eae002748d6a1",
      //     isTable: false,
      //     from:"TextInputprovisioning_reference",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'provisioning_reference', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'provisioning_reference',type:'text'};
      //   type={
      //     name:'provisioning_reference',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.provisioning_reference.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.provisioning_reference.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.provisioning_reference.type
      //   }
      ...
      //   type={
      //     name:'provisioning_reference',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.provisioning_reference.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.provisioning_reference.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.provisioning_reference.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const provision_groupe166aRef = useRef<any>(provision_groupe166a);
  useEffect(() => { provision_groupe166aRef.current = provision_groupe166a; }, [provision_groupe166a]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "82b792ddee326cbc3b3eae002748d6a1") {
        handleChange({target:{value:provision_groupe166aRef?.current?.provisioning_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "82b792ddee326cbc3b3eae002748d6a1") {
        handleBlur({target:{value:provision_groupe166aRef?.current?.provisioning_reference||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setprovision_groupe166a((pre:any)=>({...pre,provisioning_reference:dfd_addaccessrequestmodify_v1Props.data[0]?.provisioning_reference}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (provisioning_reference8d6a1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 13`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={provision_groupe166a?.provisioning_reference||""}
         disabled= {provisioning_reference8d6a1?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Provisioning Reference"
      errorMessage={error}
        validationState={validate?.viewAccessRequest_v1?.provisioning_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputprovisioning_reference
