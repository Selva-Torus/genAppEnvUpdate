...
const TextInputticket_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:viewAccessRequest:AFVK:v1|5f37cc88bca7ddd3a92f1469f1de166a|e6185b3974e6093dc85cd4f8c214e55b"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAccessRequest_v1:{...pre?.viewAccessRequest_v1,ticket_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setprovision_groupe166a((prev: any) => ({ ...prev, ticket_reference: +e.target.value }));
    }
    else{
    setprovision_groupe166a((prev: any) => ({ ...prev, ticket_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e6185b3974e6093dc85cd4f8c214e55b"
      );
      ...
      //     controlId: "e6185b3974e6093dc85cd4f8c214e55b",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e6185b3974e6093dc85cd4f8c214e55b") {
        handleChange({target:{value:provision_groupe166aRef?.current?.ticket_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e6185b3974e6093dc85cd4f8c214e55b") {
        handleBlur({target:{value:provision_groupe166aRef?.current?.ticket_reference||""}});
      }
    };
  ...
  if(dfd_addaccessrequestmodify_v1Props?.setSearchFilters && dfd_addaccessrequestmodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addaccessrequestmodify_v1Props.data) && dfd_addaccessrequestmodify_v1Props.data.length > 0){
      setprovision_groupe166a((pre:any)=>({...pre,ticket_reference:dfd_addaccessrequestmodify_v1Props.data[0]?.ticket_reference}));
    }
  }
  },[dfd_addaccessrequestmodify_v1Props?.setSearchFilters])
  if (ticket_reference4e55b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={provision_groupe166a?.ticket_reference||""}
         disabled= {ticket_reference4e55b?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.viewAccessRequest_v1?.ticket_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
