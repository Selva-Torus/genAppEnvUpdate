...
const Groupaudit_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {audit_info7b666, setaudit_info7b666}= useContext(TotalContext) as TotalContextProps;
  const {audit_inf_textareacaa14, setaudit_inf_textareacaa14}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e79e6e98bbba059044203d52dd3dea6a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaudit_groupdea6aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("audit_info")){
        setaudit_info7b666((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_info7b666?.isDisabled==null)
      {
        setaudit_info7b666((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_inf_textarea")){
        setaudit_inf_textareacaa14((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_inf_textareacaa14?.isDisabled==null)
      {
        setaudit_inf_textareacaa14((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['audit_info'] = audit_info7b666,
        codeStates['setaudit_info'] = setaudit_info7b666,
        codeStates['audit_inf_textarea'] = audit_inf_textareacaa14,
        codeStates['setaudit_inf_textarea'] = setaudit_inf_textareacaa14,
  ...
  const audit_groupdea6aRef = useRef<any>(null);
  const handleClearSearch = () => {
    audit_groupdea6aRef.current?.setSearchParams();
    audit_groupdea6aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(audit_groupdea6a) && Object.keys(audit_groupdea6a)?.length>0)
      {
        setaudit_groupdea6a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [audit_groupdea6aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:"audit_group"}))
        }}
    >
          {allowedControls.includes("audit_info") ?<Textaudit_info   /* 7b666 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("audit_inf_textarea") ?<TextAreaaudit_inf_textarea   /* caa14 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
