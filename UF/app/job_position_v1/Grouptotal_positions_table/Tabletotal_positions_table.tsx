...
import Buttonad_doc  from './Buttonad_doc'
...
let defaultColumns:any = [
  {
    "id": "position_id",
    "nodeid": "b35b16a109f2255c92fde5b6913d92e4",
    "name": "Position Id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "position_code",
    "nodeid": "2283eaa6e22b04800bd0bc03e8bbd881",
    "name": "Position Code",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "position_code"
  },
  {
    "id": "position_title",
    "nodeid": "664dea9e824db5c377fda5a4fbbda9c0",
    "name": "Position Title",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "position_title"
  },
  {
    "id": "grade_name",
    "nodeid": "cc9b09c13f15e02c55bcca5289e6c5bb",
    "name": "Grade Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "employment_type",
    "nodeid": "330bbac43cc629143e08fb6659616321",
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "employment_type"
  },
  {
    "id": "experience_required",
    "nodeid": "8f4308388335f151f21b7a316eecde5a",
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "experience_required"
  },
  {
    "id": "job_level",
    "nodeid": "93d49df73cff6e5dfc9b8bfd212e1aaa",
    "name": "Job Level",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.total_positions_table && artifactRuleState?.total_positions_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.total_positions_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,total_positions_table:filteredData},{total_positions_table:artifactRuleState?.total_positions_table})
          if(result?.total_positions_table?.[controller]?.show==true)
            return true
          else
            return false
        }
  ...
  useEffect(() => {
    async function loadPopupData() {
      ...
      let ad_doc:any = await handleSecurity("ad_doc") || false
      const content = (
        ...
        {
        ad_doc&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonad_doc'))&&(<Buttonad_doc mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />)}
        </div>
      );
...
const Tabletotal_positions_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {employeejobpositions_v1Props, setemployeejobpositions_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    /// settotal_positions_table22a59Props
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settotal_positions_table22a59Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      //////////
      //////////
        settotal_positions_table22a59Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settotal_positions_table22a59Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settotal_positions_table22a59Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(employeejobpositions_v1Props.length > 0){
      for(let i=0;i< employeejobpositions_v1Props.length;i++){
        if(employeejobpositions_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1"){
          // delete employeejobpositions_v1Props[i].DFDkey;
          let temp=structuredClone(employeejobpositions_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobPositions:AFVK:v1:UO', 
                nodeId: 'be36788337963d8ca3d8befe9e222a59',
                elementId: 'be36788337963d8ca3d8befe9e222a59'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( total_positions_table22a59Props?.presetValues&&Object.keys(total_positions_table22a59Props?.presetValues).length > 0) {
          filtertedData = [total_positions_table22a59Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settotal_positions_table22a59(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={employeejobpositions_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (total_positions_table22a59Props.filterInitalLoad) return;

    const filterControllers = total_positions_table22a59Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['total_positions_table22a59']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['total_positions_table22a59']).filter((k: string) => jsonEntry['total_positions_table22a59'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(total_positions_table22a59Props.filterProps);
      return;
    }
    ...
    settotal_positions_table22a59Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(total_positions_table22a59Props.filterProps);

  }, [
    total_positions_table22a59Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    total_positions_table22a59Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(total_positions_table22a59Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(total_positions_table22a59Props.searchFilter) || {};
...
}, [total_positions_table22a59Props.searchFilter])
  ...
  useEffect(() => {
    if (total_positions_table22a59Props?.clearData === true) {
      settotal_positions_table22a59([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settotal_positions_table22a59Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [total_positions_table22a59Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [total_positions_table22a59Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = total_positions_table22a59Props?.selectedIds[total_positions_table22a59Props?.selectedIds?.length-1]
    if(Array.isArray(total_positions_table22a59) && total_positions_table22a59.length>0)
    {
      let data = total_positions_table22a59[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (total_positions_table22a59Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (total_positions_table22a59Props?.selectedIds?.length == 0){
      handleOnRowClick({},total_positions_table22a59Props?.selectedIds)
    }
  }, [total_positions_table22a59Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(total_positions_table22a59?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&total_positions_table22a59?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](total_positions_table22a59?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_positions_table22a59?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&total_positions_table22a59?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](total_positions_table22a59?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_positions_table22a59?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (total_positions_table22a59?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"be36788337963d8ca3d8befe9e222a59"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={total_positions_table22a59Props?.selectedIds}  
...
export default Tabletotal_positions_table
