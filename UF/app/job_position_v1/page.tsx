...
const Grouptotal_position_group = dynamic(() => import("./Grouptotal_position_group/Grouptotal_position_group"), { ssr: false });


export default function PageJobPositionV1() {
  ...
  const [checktotal_position_group,setChecktotal_position_group,]=useState<boolean>(false);
  const [checktotal_positions_table,setChecktotal_positions_table,]=useState<boolean>(false);
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setemployeejobpositions_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    await jobpositions_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'total_position_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setChecktotal_position_group(true)
            }
            if(nodes?.groupName == 'total_positions_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setChecktotal_positions_table(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setemployeejobpositions_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(employeejobpositions_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(employeejobpositions_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setemployeejobpositions_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checktotal_position_group && initialLoad &&<Grouptotal_position_group
