 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "e6e09b7a2b69ad7ad853c70367706375"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_add06375") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "e6e09b7a2b69ad7ad853c70367706375") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_add06375");
    return () => {
  ...
  useEffect(()=>{
    if (!button_add06375?.trigger) return;
      if(button_add06375?.trigger){
      setbutton_add06375((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_add06375?.trigger])

  useEffect(()=>{
    setShowProfileAsModalOpen2(false)
    if(button_add06375?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_add06375?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        settotal_position_group79a63((prev: any) => ({ ...prev, button_add: true }));
        //onClick
    ...
    //enableElement
    setbt_add_position5f98e((prev: any) => ({ ...prev, isDisabled: false }));
    //disableElement
    setbutton_update94589((prev: any) => ({ ...prev, isDisabled: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        settotal_position_group79a63((prev: any) => ({ ...prev, button_add: false }));
 ...
 if (button_add06375?.isHidden) {
  ...
  return (
    <div
      ...
      >
        {!hiddenModalForTrigger && <PageAddemployeejobpositionpage2/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='normal'
          disabled= {button_add06375?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdAdd"
          iconDisplay='Start with Icon'
        >
          {keyset("New Job Position")}
        </Button>}
      </div>
