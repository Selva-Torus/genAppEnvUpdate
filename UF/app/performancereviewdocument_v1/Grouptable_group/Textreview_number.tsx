...
const Textreview_number = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_performancereviewdoctable_v1Props && !dfd_performancereviewdoctable_v1Props.hasLogicCenter) {
        ...
        settable_group106c4((pre: any) => {
          return { ...pre, review_number: api_paginationData.data.records[0]?.review_number }
        })
        }
      }
      else{
      if(Array.isArray(dfd_performancereviewdoctable_v1Props) && dfd_performancereviewdoctable_v1Props && !table_group106c4.review_number){
        settable_group106c4((pre:any)=>({...pre,review_number:dfd_performancereviewdoctable_v1Props[0]?.review_number}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[review_number1cdea?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancereviewdoctable_v1Props) && !table_group106c4.review_number){
    settable_group106c4((pre:any)=>({...pre,review_number:dfd_performancereviewdoctable_v1Props[0]?.review_number}));
  }
  },[dfd_performancereviewdoctable_v1Props])

  if (review_number1cdea?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.review_number : (table_group106c4?.review_number || ""))}
</Text>
  </div>
  )
}
