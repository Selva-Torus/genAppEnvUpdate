...
const Textfull_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_performancereviewdoctable_v1Props) && dfd_performancereviewdoctable_v1Props && !table_group106c4.full_name){
        settable_group106c4((pre:any)=>({...pre,full_name:dfd_performancereviewdoctable_v1Props[0]?.full_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[full_namebcb8e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancereviewdoctable_v1Props) && !table_group106c4.full_name){
    settable_group106c4((pre:any)=>({...pre,full_name:dfd_performancereviewdoctable_v1Props[0]?.full_name}));
  }
  },[dfd_performancereviewdoctable_v1Props])

  if (full_namebcb8e?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.full_name : (table_group106c4?.full_name || ""))}
</Text>
  </div>
  )
}
