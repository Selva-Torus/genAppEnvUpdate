...
const Groupemployee_doc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bbef3aa28e652fe589f2867ed76d849d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setemployee_doc_tabled849dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_idbfc84((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(attachment_idbfc84?.isDisabled==null)
      {
        setattachment_idbfc84((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_group")){
        setdoc_group9bffc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_group9bffc?.isDisabled==null)
      {
        setdoc_group9bffc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_namef061c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_namef061c?.isDisabled==null)
      {
        setdoc_namef061c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_date")){
        settrs_created_date19289((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_date19289?.isDisabled==null)
      {
        settrs_created_date19289((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_by")){
        settrs_created_by90878((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_by90878?.isDisabled==null)
      {
        settrs_created_by90878((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("bt_delete")){
        setbt_delete676ed((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(bt_delete676ed?.isDisabled==null)
      {
        setbt_delete676ed((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const employee_doc_tabled849dRef = useRef<any>(null);
  const handleClearSearch = () => {
    employee_doc_tabled849dRef.current?.setSearchParams();
    employee_doc_tabled849dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(employee_doc_tabled849d) && Object.keys(employee_doc_tabled849d)?.length>0)
      {
        setemployee_doc_tabled849d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [employee_doc_tabled849dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setperformancereviewdocument_v1((pre:any)=>({...pre,_selectedGroup_:"employee_doc_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tableemployee_doc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={employee_doc_tabled849dRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
