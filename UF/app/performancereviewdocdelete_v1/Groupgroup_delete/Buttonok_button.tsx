...
const Buttonok_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "0aaf1c50778e796cb698c1e34f703deb"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_button03deb") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "0aaf1c50778e796cb698c1e34f703deb") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_button03deb");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_button03deb?.trigger) return;
      if(ok_button03deb?.trigger){
      setok_button03deb((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_button03deb?.trigger])

  useEffect(()=>{
    if(ok_button03deb?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_button03deb?.refresh])
  ...
  async function handleSave3deb_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_delete4384fProps = group_delete4384fPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.performanceReviewDocDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_delete4384fRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete4384f?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "0aaf1c50778e796cb698c1e34f703deb",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "0aaf1c50778e796cb698c1e34f703deb.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "0aaf1c50778e796cb698c1e34f703deb.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "0aaf1c50778e796cb698c1e34f703deb.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "0aaf1c50778e796cb698c1e34f703deb.1.1.1.1.1",
                  "eventContext": "rise",
                  "value": "",
                  "type": "handlerNode",
                  "name": "closeHandler",
            ...
            {
              "id": "0aaf1c50778e796cb698c1e34f703deb.1.1.1.2",
              ...
              "children": [
                {
                  "id": "bbef3aa28e652fe589f2867ed76d849d.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:performanceReviewDocDelete:AFVK:v1";
      sourceId+= "|"+"d5583e9a87a5b88918cf8b5fa7d4384f";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:performanceReviewDocDelete:AFVK:v1"+"|"+"d5583e9a87a5b88918cf8b5fa7d4384f"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete4384f?.upId? [group_delete4384f?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete4384f?.upId? [group_delete4384f?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete4384f?.upId? [group_delete4384f?.upId ] : group_delete4384f?.trs_process_id? [group_delete4384f?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete4384fRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"d5583e9a87a5b88918cf8b5fa7d4384f",
              controlId:"0aaf1c50778e796cb698c1e34f703deb"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete4384fProps.ssKey !== '' && group_delete4384fProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete4384fProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete4384f((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave3deb_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete4384f((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (ok_button03deb?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `19 / 25`,gridRow: `45 / 51`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_button03deb?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
