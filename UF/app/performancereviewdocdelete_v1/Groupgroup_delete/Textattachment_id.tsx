...
const Textattachment_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_performancereviewdoctable_v1Props) && dfd_performancereviewdoctable_v1Props && !group_delete4384f.attachment_id){
        setgroup_delete4384f((pre:any)=>({...pre,attachment_id:dfd_performancereviewdoctable_v1Props[0]?.attachment_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[attachment_id0a460?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancereviewdoctable_v1Props) && !group_delete4384f.attachment_id){
    setgroup_delete4384f((pre:any)=>({...pre,attachment_id:dfd_performancereviewdoctable_v1Props[0]?.attachment_id}));
  }
  },[dfd_performancereviewdoctable_v1Props])

  if (attachment_id0a460?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.attachment_id : (group_delete4384f?.attachment_id || ""))}
</Text>
  </div>
  )
}
