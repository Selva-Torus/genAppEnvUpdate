...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_performancereviewdoctable_v1Props) && dfd_performancereviewdoctable_v1Props && !group_delete4384f.doc_group){
        setgroup_delete4384f((pre:any)=>({...pre,doc_group:dfd_performancereviewdoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_group35e3d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_performancereviewdoctable_v1Props) && !group_delete4384f.doc_group){
    setgroup_delete4384f((pre:any)=>({...pre,doc_group:dfd_performancereviewdoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_performancereviewdoctable_v1Props])

  if (doc_group35e3d?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_delete4384f?.doc_group || ""))}
</Text>
  </div>
  )
}
