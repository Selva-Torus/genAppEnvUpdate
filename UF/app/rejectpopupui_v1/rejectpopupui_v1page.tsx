...
export default function PageRejectpopupuiV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      rejectpopupdfd_v1:false,
    });
    async function rejectpopupdfd_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let rejectpopupdfd_v1Body:te_refreshDto={
          key: "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:rejectPopupDfd:AFVK:v1"+":",
          refreshFlag: "Y",
          count:parseInt(pagination?.count) || 10,
          page:parseInt(pagination?.page) || 1
        }
        if (encryptionFlagPage) {          
          rejectpopupdfd_v1Body["dpdKey"] = encryptionDpd;
          rejectpopupdfd_v1Body["method"] = encryptionMethod;
        }
        if(rejectpopupui_v1Props.length > 0){
          for(let i=0;i< rejectpopupui_v1Props.length;i++){
            if(rejectpopupui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:rejectPopupDfd:AFVK:v1"){
              // delete rejectpopupui_v1Props[i].DFDkey;
              let temp=structuredClone(rejectpopupui_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
            }           
          }
          rejectpopupdfd_v1Body['filterData'] = filterData;
        }
        const rejectpopupdfd_v1Data:any=await AxiosService.post("/te/eventEmitter",rejectpopupdfd_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        let dstKey:string=rejectpopupdfd_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(rejectpopupdfd_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_rejectpopupdfd_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_rejectpopupdfd_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (rejectpopupdfd_v1Data?.data?.dataset) {
           setdfd_rejectpopupdfd_v1Props(
              Array.isArray(rejectpopupdfd_v1Data?.data?.dataset?.data)
                 ? rejectpopupdfd_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.rejectpopupdfd_v1) {
      rejectpopupdfd_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.rejectpopupdfd_v1= true
  },[refetch?.rejectpopupdfd_v1])
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:rejectPopUpUi:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
      ...
      try {
        ...
        try{
    await rejectpopupdfd_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setrejectpopupui_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setrejectpopupui_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
