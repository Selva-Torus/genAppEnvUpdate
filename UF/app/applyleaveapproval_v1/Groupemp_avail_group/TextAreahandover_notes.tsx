...
const TextAreahandover_notes = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b7fa63c92031a4d99c76797f7bf899d2"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[handover_notes899d2?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setemp_avail_group11178((pre:any)=>({...pre,handover_notes:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b7fa63c92031a4d99c76797f7bf899d2") {
        handleChange({target:{value:emp_avail_group11178Ref?.current?.handover_notes||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b7fa63c92031a4d99c76797f7bf899d2") {
        handleBlur({target:{value:emp_avail_group11178Ref?.current?.handover_notes||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,handover_notes:undefined}}));
    setemp_avail_group11178((prev: any) => ({ ...prev, handover_notes: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (handover_notes899d2?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `10 / 25`,gridRow: `8 / 20`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {handover_notes899d2?.isDisabled ? true : false}
      placeholder = {'e.g. Specify tasks to be covered and designated persons'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Handover Notes"
      pin = {'brick-brick'}
      value = { emp_avail_group11178?.handover_notes != null && typeof emp_avail_group11178?.handover_notes =='object' ? Object.keys(emp_avail_group11178?.handover_notes)?.length ?  JSON.stringify(emp_avail_group11178?.handover_notes,null ,2):"" : emp_avail_group11178?.handover_notes||""}
      errorMessage={error}
      validationState={validate?.applyLeaveApproval_v1?.handover_notes ? "invalid" : undefined}
    />
  </div>
  )
}
