...
const TextInputcontact_during_leave = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:applyLeave:AFVK:v1|39645aa0f2064fb48ea2e65e5e5e702a|properties.contact_during_leave"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1|50258739a639fa916d88bf4ffd811178|3572eb50e0d9b6a059c0607998213a3c"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,contact_during_leave:undefined}}));
    if(dynamicStateandType.type=="number"){
    setemp_avail_group11178((prev: any) => ({ ...prev, contact_during_leave: +e.target.value }));
    }
    else{
    setemp_avail_group11178((prev: any) => ({ ...prev, contact_during_leave: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "50258739a639fa916d88bf4ffd811178",
        "3572eb50e0d9b6a059c0607998213a3c"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1",
      //     componentId: "50258739a639fa916d88bf4ffd811178",
      //     controlId: "3572eb50e0d9b6a059c0607998213a3c",
      //     isTable: false,
  ...
  const emp_avail_group11178Ref = useRef<any>(emp_avail_group11178);
  useEffect(() => { emp_avail_group11178Ref.current = emp_avail_group11178; }, [emp_avail_group11178]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "3572eb50e0d9b6a059c0607998213a3c") {
        handleChange({target:{value:emp_avail_group11178Ref?.current?.contact_during_leave||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "3572eb50e0d9b6a059c0607998213a3c") {
        handleBlur({target:{value:emp_avail_group11178Ref?.current?.contact_during_leave||""}});
      }
    };
  ...
  if(dfd_applyleave_v1Props?.setSearchFilters && dfd_applyleave_v1Props?.data)
  {
    if(Array.isArray(dfd_applyleave_v1Props.data) && dfd_applyleave_v1Props.data.length > 0){
      setemp_avail_group11178((pre:any)=>({...pre,contact_during_leave:dfd_applyleave_v1Props.data[0]?.contact_during_leave}));
    }
  }
  },[dfd_applyleave_v1Props?.setSearchFilters])
  if (contact_during_leave13a3c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={emp_avail_group11178?.contact_during_leave||""}
         disabled= {contact_during_leave13a3c?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.applyLeaveApproval_v1?.contact_during_leave ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
