...
import Groupemp_avail_group  from "../Groupemp_avail_group/Groupemp_avail_group";
import Groupleave_balance_group  from "../Groupleave_balance_group/Groupleave_balance_group";
import Groupapp_det_group  from "../Groupapp_det_group/Groupapp_det_group";
...
const Groupnew_access_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {leave_req_id66177, setleave_req_id66177}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "3eef8e0c64aec1113305fa92a7cc501f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setnew_access_groupc501fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("access_req__group")){
        setaccess_req__group7ac49((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(access_req__group7ac49?.isDisabled==null)
      {
        setaccess_req__group7ac49((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("emp_avail_group")){
        setemp_avail_group11178((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(emp_avail_group11178?.isDisabled==null)
      {
        setemp_avail_group11178((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_balance_group")){
        setleave_balance_group98e23((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_balance_group98e23?.isDisabled==null)
      {
        setleave_balance_group98e23((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("app_det_group")){
        setapp_det_groupe2c1b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_det_groupe2c1b?.isDisabled==null)
      {
        setapp_det_groupe2c1b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("audit_group")){
        setaudit_group087fd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(audit_group087fd?.isDisabled==null)
      {
        setaudit_group087fd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("leave_req_id")){
        setleave_req_id66177((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(leave_req_id66177?.isDisabled==null)
      {
        setleave_req_id66177((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactionsafd15((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactionsafd15?.isDisabled==null)
      {
        setdynamicactionsafd15((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['leave_req_id'] = leave_req_id66177,
        codeStates['setleave_req_id'] = setleave_req_id66177,
  ...
  const new_access_groupc501fRef = useRef<any>(null);
  const handleClearSearch = () => {
    new_access_groupc501fRef.current?.setSearchParams();
    new_access_groupc501fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(new_access_groupc501f) && Object.keys(new_access_groupc501f)?.length>0)
      {
        setnew_access_groupc501f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [new_access_groupc501fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleaveapproval_v1((pre:any)=>({...pre,_selectedGroup_:"new_access_group"}))
        }}
    >
        ...
        {allowedComponent.includes("emp_avail_group")  &&<Groupemp_avail_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("leave_balance_group")  &&<Groupleave_balance_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("app_det_group")  &&<Groupapp_det_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("leave_req_id") ?<Textleave_req_id   /* 66177 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
