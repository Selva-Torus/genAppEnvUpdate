...
const Textleave_req_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_applyleave_v1Props && !dfd_applyleave_v1Props.hasLogicCenter) {
        ...
        setnew_access_groupc501f((pre: any) => {
          return { ...pre, leave_req_id: api_paginationData.data.records[0]?.leave_req_id }
        })
        }
      }
      else{
      if(Array.isArray(dfd_applyleave_v1Props) && dfd_applyleave_v1Props && !new_access_groupc501f.leave_req_id){
        setnew_access_groupc501f((pre:any)=>({...pre,leave_req_id:dfd_applyleave_v1Props[0]?.leave_req_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[leave_req_id66177?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_applyleave_v1Props) && !new_access_groupc501f.leave_req_id){
    setnew_access_groupc501f((pre:any)=>({...pre,leave_req_id:dfd_applyleave_v1Props[0]?.leave_req_id}));
  }
  },[dfd_applyleave_v1Props])

  if (leave_req_id66177?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 4`,gridRow: `96 / 97`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.leave_req_id : (new_access_groupc501f?.leave_req_id || ""))}
</Text>
  </div>
  )
}
