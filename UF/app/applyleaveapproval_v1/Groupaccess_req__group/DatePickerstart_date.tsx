...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,start_date:undefined}}));
  if (!date) {
    setaccess_req__group7ac49((prev: any) => ({ ...prev, start_date: null }));
    return;
  }
  const now = new Date();
  ...
  setaccess_req__group7ac49((prev: any) => ({ ...prev, start_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "3a2ac2e80382f598631bd16c92634ff8"
      );
...
return (
  ...
  style={{gridColumn: `1 / 6`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={access_req__group7ac49?.start_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly=  {start_date34ff8?.isDisabled ? true : false}
      disabled= {start_date34ff8?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Start Date"
      dateValidation=""
      validationState={validate?.applyLeaveApproval_v1?.start_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
