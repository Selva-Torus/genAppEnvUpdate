...
const TextInputdays_requested = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:applyLeave:AFVK:v1|39645aa0f2064fb48ea2e65e5e5e702a|properties.days_requested"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1|e968590033094bbfaf1b81b7bf27ac49|414b91caddb6c98fbf2a73c8a0570ed8"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'days_requested',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,days_requested:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__group7ac49((prev: any) => ({ ...prev, days_requested: +e.target.value }));
    }
    else{
    setaccess_req__group7ac49((prev: any) => ({ ...prev, days_requested: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "414b91caddb6c98fbf2a73c8a0570ed8"
      );
      ...
      //     controlId: "414b91caddb6c98fbf2a73c8a0570ed8",
      //     isTable: false,
      //     from:"TextInputdays_requested",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'days_requested', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'days_requested',type:'text'};
      //   type={
      //     name:'days_requested',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.days_requested.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.days_requested.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.days_requested.type
      //   }
      ...
      //   type={
      //     name:'days_requested',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.days_requested.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.days_requested.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.days_requested.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "414b91caddb6c98fbf2a73c8a0570ed8") {
        handleChange({target:{value:access_req__group7ac49Ref?.current?.days_requested||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "414b91caddb6c98fbf2a73c8a0570ed8") {
        handleBlur({target:{value:access_req__group7ac49Ref?.current?.days_requested||""}});
      }
    };
  ...
  if(dfd_applyleave_v1Props?.setSearchFilters && dfd_applyleave_v1Props?.data)
  {
    if(Array.isArray(dfd_applyleave_v1Props.data) && dfd_applyleave_v1Props.data.length > 0){
      setaccess_req__group7ac49((pre:any)=>({...pre,days_requested:dfd_applyleave_v1Props.data[0]?.days_requested}));
    }
  }
  },[dfd_applyleave_v1Props?.setSearchFilters])
  if (days_requested70ed8?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `11 / 14`,gridRow: `20 / 32`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={access_req__group7ac49?.days_requested||""}
         disabled= {days_requested70ed8?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.applyLeaveApproval_v1?.days_requested ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdays_requested
