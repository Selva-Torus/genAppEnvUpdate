...
import Textapp_det  from "./Textapp_det";
...
const Groupapprove_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {app_deta0272, setapp_deta0272}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f1d291c8ba89af3eba5a307d4954086e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapprove_group4086eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_det")){
        setapp_deta0272((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(app_deta0272?.isDisabled==null)
      {
        setapp_deta0272((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['app_det'] = app_deta0272,
        codeStates['setapp_det'] = setapp_deta0272,
  ...
  const approve_group4086eRef = useRef<any>(null);
  const handleClearSearch = () => {
    approve_group4086eRef.current?.setSearchParams();
    approve_group4086eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(approve_group4086e) && Object.keys(approve_group4086e)?.length>0)
      {
        setapprove_group4086e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [approve_group4086eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleaveapproval_v1((pre:any)=>({...pre,_selectedGroup_:"approve_group"}))
        }}
    >
          {allowedControls.includes("app_det") ?<Textapp_det   /* a0272 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
