...
export default function PageApplyleaveapprovalV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function employeenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleaveapproval_v1Props.length > 0){
          for(let i=0;i< applyleaveapproval_v1Props.length;i++){
            if(applyleaveapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:employeeNameCombo:AFVK:v1"){
              // delete applyleaveapproval_v1Props[i].DFDkey;
              let temp=structuredClone(applyleaveapproval_v1Props[i])
              delete temp?.DFDkey
  ...
  useEffect(()=>{
    ...
    async function policynamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleaveapproval_v1Props.length > 0){
          for(let i=0;i< applyleaveapproval_v1Props.length;i++){
            if(applyleaveapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:policyNameCombo:AFVK:v1"){
              // delete applyleaveapproval_v1Props[i].DFDkey;
              let temp=structuredClone(applyleaveapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function leavereasoncategorycombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleaveapproval_v1Props.length > 0){
          for(let i=0;i< applyleaveapproval_v1Props.length;i++){
            if(applyleaveapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leaveReasonCategoryCombo:AFVK:v1"){
              // delete applyleaveapproval_v1Props[i].DFDkey;
              let temp=structuredClone(applyleaveapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function applyleave_v1DFD(pagination:any): Promise<void>{
        ...
        if(applyleaveapproval_v1Props.length > 0){
          for(let i=0;i< applyleaveapproval_v1Props.length;i++){
            if(applyleaveapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:applyLeave:AFVK:v1"){
              // delete applyleaveapproval_v1Props[i].DFDkey;
              let temp=structuredClone(applyleaveapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setapplyleaveapproval_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setapplyleaveapproval_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(applyleaveapproval_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(applyleaveapproval_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setapplyleaveapproval_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
