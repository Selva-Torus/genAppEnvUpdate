...
const TextInputleave_balance_after = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1|3854484090e6bf4736e73ef2adc98e23|1ff1106dfe82308b02e99ca65e9898d5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,leave_balance_afterd:undefined}}));
    if(dynamicStateandType.type=="number"){
    setleave_balance_group98e23((prev: any) => ({ ...prev, leave_balance_afterd: +e.target.value }));
    }
    else{
    setleave_balance_group98e23((prev: any) => ({ ...prev, leave_balance_afterd: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "1ff1106dfe82308b02e99ca65e9898d5"
      );
      ...
      //     controlId: "1ff1106dfe82308b02e99ca65e9898d5",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "1ff1106dfe82308b02e99ca65e9898d5") {
        handleChange({target:{value:leave_balance_group98e23Ref?.current?.leave_balance_afterd||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "1ff1106dfe82308b02e99ca65e9898d5") {
        handleBlur({target:{value:leave_balance_group98e23Ref?.current?.leave_balance_afterd||""}});
      }
    };
  ...
  if(dfd_applyleave_v1Props?.setSearchFilters && dfd_applyleave_v1Props?.data)
  {
    if(Array.isArray(dfd_applyleave_v1Props.data) && dfd_applyleave_v1Props.data.length > 0){
      setleave_balance_group98e23((pre:any)=>({...pre,leave_balance_afterd:dfd_applyleave_v1Props.data[0]?.leave_balance_afterd}));
    }
  }
  },[dfd_applyleave_v1Props?.setSearchFilters])
  if (leave_balance_after898d5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={leave_balance_group98e23?.leave_balance_afterd||""}
         disabled= {leave_balance_after898d5?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.applyLeaveApproval_v1?.leave_balance_afterd ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
