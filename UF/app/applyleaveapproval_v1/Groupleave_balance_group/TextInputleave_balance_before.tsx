...
const TextInputleave_balance_before = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:applyLeave:AFVK:v1|39645aa0f2064fb48ea2e65e5e5e702a|properties.leave_balance_before"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1|3854484090e6bf4736e73ef2adc98e23|b6dcab2ac2a2aa7004eb962e0f3044cf"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'leave_balance_before',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,leave_balance_before:undefined}}));
    if(dynamicStateandType.type=="number"){
    setleave_balance_group98e23((prev: any) => ({ ...prev, leave_balance_before: +e.target.value }));
    }
    else{
    setleave_balance_group98e23((prev: any) => ({ ...prev, leave_balance_before: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "3854484090e6bf4736e73ef2adc98e23",
        "b6dcab2ac2a2aa7004eb962e0f3044cf"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:applyLeaveApproval:AFVK:v1",
      //     componentId: "3854484090e6bf4736e73ef2adc98e23",
      //     controlId: "b6dcab2ac2a2aa7004eb962e0f3044cf",
      //     isTable: false,
      //     from:"TextInputleave_balance_before",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'leave_balance_before', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'leave_balance_before',type:'text'};
      //   type={
      //     name:'leave_balance_before',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.leave_balance_before.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.leave_balance_before.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.leave_balance_before.type
      //   }
      ...
      //   type={
      //     name:'leave_balance_before',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.leave_balance_before.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.leave_balance_before.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.leave_balance_before.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const leave_balance_group98e23Ref = useRef<any>(leave_balance_group98e23);
  useEffect(() => { leave_balance_group98e23Ref.current = leave_balance_group98e23; }, [leave_balance_group98e23]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "b6dcab2ac2a2aa7004eb962e0f3044cf") {
        handleChange({target:{value:leave_balance_group98e23Ref?.current?.leave_balance_before||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b6dcab2ac2a2aa7004eb962e0f3044cf") {
        handleBlur({target:{value:leave_balance_group98e23Ref?.current?.leave_balance_before||""}});
      }
    };
  ...
  if(dfd_applyleave_v1Props?.setSearchFilters && dfd_applyleave_v1Props?.data)
  {
    if(Array.isArray(dfd_applyleave_v1Props.data) && dfd_applyleave_v1Props.data.length > 0){
      setleave_balance_group98e23((pre:any)=>({...pre,leave_balance_before:dfd_applyleave_v1Props.data[0]?.leave_balance_before}));
    }
  }
  },[dfd_applyleave_v1Props?.setSearchFilters])
  if (leave_balance_before044cf?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 12`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={leave_balance_group98e23?.leave_balance_before||""}
         disabled= {leave_balance_before044cf?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='--'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Leave Balance Before"
      errorMessage={error}
        validationState={validate?.applyLeaveApproval_v1?.leave_balance_before ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputleave_balance_before
