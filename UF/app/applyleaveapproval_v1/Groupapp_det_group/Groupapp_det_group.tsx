...
const Groupapp_det_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c5a67b29a2c95698b35ba680df1e2c1b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setapp_det_groupe2c1bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("approve_group")){
        setapprove_group4086e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approve_group4086e?.isDisabled==null)
      {
        setapprove_group4086e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("approval_comments")){
        setapproval_commentsa4387((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(approval_commentsa4387?.isDisabled==null)
      {
        setapproval_commentsa4387((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_reason")){
        setreject_reasone8180((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_reasone8180?.isDisabled==null)
      {
        setreject_reasone8180((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cancellation_reason")){
        setcancellation_reasonc3fa8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancellation_reasonc3fa8?.isDisabled==null)
      {
        setcancellation_reasonc3fa8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const app_det_groupe2c1bRef = useRef<any>(null);
  const handleClearSearch = () => {
    app_det_groupe2c1bRef.current?.setSearchParams();
    app_det_groupe2c1bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(app_det_groupe2c1b) && Object.keys(app_det_groupe2c1b)?.length>0)
      {
        setapp_det_groupe2c1b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [app_det_groupe2c1bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setapplyleaveapproval_v1((pre:any)=>({...pre,_selectedGroup_:"app_det_group"}))
        }}
    >
        ...
        {allowedControls.includes("approval_comments") ?<TextAreaapproval_comments   /* a4387 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("reject_reason") ?<TextAreareject_reason   /* e8180 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("cancellation_reason") ?<TextAreacancellation_reason   /* c3fa8 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
