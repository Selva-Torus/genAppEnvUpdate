...
const TextAreaapproval_comments = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'approval_comments',type:"string"})
 ...
 /////////////
    ...
    try{
      ...
      if(orchestrationData?.data?.schemaData?.at(0).schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          ...
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_comments.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_comments.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_comments.type
        }
        setDynamicStateandType(type)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[approval_commentsa4387?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_groupe2c1b((pre:any)=>({...pre,approval_comments:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "494d9368f804c88e2426f483cc0a4387") {
        handleChange({target:{value:app_det_groupe2c1bRef?.current?.approval_comments||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "494d9368f804c88e2426f483cc0a4387") {
        handleBlur({target:{value:app_det_groupe2c1bRef?.current?.approval_comments||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,approval_comments:undefined}}));
    setapp_det_groupe2c1b((prev: any) => ({ ...prev, approval_comments: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (approval_commentsa4387?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `1 / 9`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {approval_commentsa4387?.isDisabled ? true : false}
      placeholder = {'Approved for the requested access..'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Approval Comments"
      pin = {'brick-brick'}
      value = { app_det_groupe2c1b?.approval_comments != null && typeof app_det_groupe2c1b?.approval_comments =='object' ? Object.keys(app_det_groupe2c1b?.approval_comments)?.length ?  JSON.stringify(app_det_groupe2c1b?.approval_comments,null ,2):"" : app_det_groupe2c1b?.approval_comments||""}
      errorMessage={error}
      validationState={validate?.applyLeaveApproval_v1?.approval_comments ? "invalid" : undefined}
    />
  </div>
  )
}

export default TextAreaapproval_comments
