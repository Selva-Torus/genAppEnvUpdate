...
const TextAreacancellation_reason = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a2eaa9d6815c706c329bea3a57ac3fa8"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[cancellation_reasonc3fa8?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setapp_det_groupe2c1b((pre:any)=>({...pre,cancellation_reason:""}))
    }else 
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a2eaa9d6815c706c329bea3a57ac3fa8") {
        handleChange({target:{value:app_det_groupe2c1bRef?.current?.cancellation_reason||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a2eaa9d6815c706c329bea3a57ac3fa8") {
        handleBlur({target:{value:app_det_groupe2c1bRef?.current?.cancellation_reason||""}});
      }
    };
  ...
  const handleChange = async(e: any) => {
    let validate:any;
    setError('');
    setValidate((pre:any)=>({...pre,applyLeaveApproval_v1:{...pre?.applyLeaveApproval_v1,cancellation_reason:undefined}}));
    setapp_det_groupe2c1b((prev: any) => ({ ...prev, cancellation_reason: e?.target?.value }));
    try{
    }catch (err: any) {
  ...
  if (cancellation_reasonc3fa8?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `17 / 25`,gridRow: `9 / 32`, gap:``, height: `100%`}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {cancellation_reasonc3fa8?.isDisabled ? true : false}
      placeholder = {'Enter cancellation reason....'}
      contentAlign={"left"}
      headerPosition='top'
      headerText="Cancellation Reason"
      pin = {'brick-brick'}
      value = { app_det_groupe2c1b?.cancellation_reason != null && typeof app_det_groupe2c1b?.cancellation_reason =='object' ? Object.keys(app_det_groupe2c1b?.cancellation_reason)?.length ?  JSON.stringify(app_det_groupe2c1b?.cancellation_reason,null ,2):"" : app_det_groupe2c1b?.cancellation_reason||""}
      errorMessage={error}
      validationState={validate?.applyLeaveApproval_v1?.cancellation_reason ? "invalid" : undefined}
    />
  </div>
  )
}
