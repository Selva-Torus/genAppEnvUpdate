 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "e2e94d5114d5e32ec008f6d07238ac85"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("reject_bt" in dynamicactionsafd15Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsafd15Props?.dynamicActionRule?.reject_bt,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("reject_bt" in applyleaveapproval_v1?.dynamicactions && applyleaveapproval_v1?.dynamicactions["reject_bt"]?.itsHaveArtifact== true)
      {
        setShowFlag(applyleaveapproval_v1?.dynamicactions["reject_bt"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(applyleaveapproval_v1?.dynamicactions?.reject_bt?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "reject_bt8ac85") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "e2e94d5114d5e32ec008f6d07238ac85") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "reject_bt8ac85");
    return () => {
  ...
  useEffect(()=>{
    if (!reject_bt8ac85?.trigger) return;
      if(reject_bt8ac85?.trigger){
      setreject_bt8ac85((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[reject_bt8ac85?.trigger])

  useEffect(()=>{
    if(reject_bt8ac85?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[reject_bt8ac85?.refresh])
  ...
  async function handleSaveac85_1_1_1(){
  ...
  "id": "e2e94d5114d5e32ec008f6d07238ac85",
  "type": "button",
  "name": "reject_bt",
  "label": "reject_bt",
  "sequence": 1,
  "children": [
    {
      "id": "e2e94d5114d5e32ec008f6d07238ac85.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "e2e94d5114d5e32ec008f6d07238ac85.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "e2e94d5114d5e32ec008f6d07238ac85.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "e2e94d5114d5e32ec008f6d07238ac85.1.1.1.2",
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"e2e94d5114d5e32ec008f6d07238ac85"
            };
            if (encryptionFlagCont) {
    ...
    //infoMsg
    toast('Leave Request Rejected', 'success',true);
    // refreshElement
    //riseListen
    // for group
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsafd15((prev: any) => ({ ...prev, reject_bt: true }));
        //onClick

    //eventEmitter
    await handleSaveac85_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsafd15((prev: any) => ({ ...prev, reject_bt: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [applyleaveapproval_v1?.dynamicactions?.reject_bt,dynamicactionsafd15Props?.dynamicActionRule?.reject_bt,])

 if (reject_bt8ac85?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="!bg-[#EF4444] hover:!bg-[#DC2626] !text-white !rounded-lg !font-bold"
          onClick={handleClick}
          view='normal'
          disabled= {reject_bt8ac85?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("Rejected")}
...
export default Buttonreject_bt
