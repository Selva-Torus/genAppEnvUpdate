...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "a3b7095e79db4b41ace02fe8c66f4ab0",
    "name": "country",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "state",
    "nodeid": "8205d35b9ced460780b59f4a7ab015fd",
    "name": "state",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "city",
    "nodeid": "8fc3dcf0e4a3487698c871806a1fc3b9",
    "name": "city",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "e3ede2a2fba04939ab2cf598fd23c3b1",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, table3c3b1 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, table3c3b1 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, table3c3b1 } = latestLockStateRef.current
    /// settable3c3b1Props
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settable3c3b1Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      table3c3b1.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settable3c3b1Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settable3c3b1Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable3c3b1Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:chart:AFVK:v1:UO', 
                nodeId: 'e3ede2a2fba04939ab2cf598fd23c3b1',
                elementId: 'e3ede2a2fba04939ab2cf598fd23c3b1'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table3c3b1Props?.presetValues&&Object.keys(table3c3b1Props?.presetValues).length > 0) {
          filtertedData = [table3c3b1Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable3c3b1(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table3c3b1Props.filterInitalLoad) return;

    const filterControllers = table3c3b1Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table3c3b1']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table3c3b1']).filter((k: string) => jsonEntry['table3c3b1'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(table3c3b1Props.filterProps);
      return;
    }
    ...
    settable3c3b1Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table3c3b1Props.filterProps);

  }, [
    table3c3b1Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table3c3b1Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table3c3b1Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table3c3b1Props.searchFilter) || {};
  ...
  useEffect(() => {
    if (table3c3b1Props?.clearData === true) {
      settable3c3b1([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settable3c3b1Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table3c3b1Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = table3c3b1Props?.selectedIds[table3c3b1Props?.selectedIds?.length-1]
    if(Array.isArray(table3c3b1) && table3c3b1.length>0)
    {
      let data = table3c3b1.find((data:any)=>(data?.my_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "my_id",
        value: data["my_id"],
  ...
  useEffect(() => {
    if (table3c3b1Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (table3c3b1Props?.selectedIds?.length == 0){
      handleOnRowClick({},table3c3b1Props?.selectedIds)
    }
  }, [table3c3b1Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table3c3b1?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table3c3b1?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table3c3b1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table3c3b1?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table3c3b1?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table3c3b1?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table3c3b1?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"e3ede2a2fba04939ab2cf598fd23c3b1"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={table3c3b1Props?.selectedIds}  
