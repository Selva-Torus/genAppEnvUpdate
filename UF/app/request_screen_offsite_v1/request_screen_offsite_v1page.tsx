...
import GroupRequest_Offsite_Group  from "./GroupRequest_Offsite_Group/GroupRequest_Offsite_Group";


export default function PageRequestScreenOffsiteV1() {
  const { isDark, isHighContrast, bgStyle, textStyle } : { isDark: boolean; isHighContrast: boolean; bgStyle: string; textStyle: string } = useTheme();
  const screenName:string = "user home screen";
  ...
  const [checkrequest_offsite_group,setCheckrequest_offsite_group,]=useState<boolean>(false);
  const [checkclaims_detail_table,setCheckclaims_detail_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function claims_v1(pagination:any): Promise<void>{
        ...
        if(request_screen_offsite_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< request_screen_offsite_v1Props.length;i++){
            if(request_screen_offsite_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claims:AFVK:v1"){
              delete request_screen_offsite_v1Props[i].DFDkey;
              filterData.push(request_screen_offsite_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function claims_detail_v1(pagination:any): Promise<void>{
        ...
        if(request_screen_offsite_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< request_screen_offsite_v1Props.length;i++){
            if(request_screen_offsite_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claims_detail:AFVK:v1"){
              delete request_screen_offsite_v1Props[i].DFDkey;
              filterData.push(request_screen_offsite_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function code_description_v1(pagination:any): Promise<void>{
        ...
        if(request_screen_offsite_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< request_screen_offsite_v1Props.length;i++){
            if(request_screen_offsite_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:code_description:AFVK:v1"){
              delete request_screen_offsite_v1Props[i].DFDkey;
              filterData.push(request_screen_offsite_v1Props[i])
            }           
          }
        ...
        if (code_description_v1Data?.data?.dataset) {
          setdfd_code_description_v1Props(code_description_v1Data?.data?.dataset?.data || []);
        }else{
         //////////////
        let dstKey:string=code_description_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        ...
        setdfd_code_description_v1Props(api_paginationData?.data?.records || []);
        }
      }
  useEffect(()=>{
    if (prevRefreshRef?.current?.code_description_v1) {
      code_description_v1(paginationData)
    }else 
      prevRefreshRef.current.code_description_v1= true
  },[refetch?.code_description_v1])
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Request_Screen_Offsite:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  "events": {}
        ...
        try{
          ...
          await code_description_v1(pagination)
        }catch(err:any)
        {
  ...
  return (
    ...
    {securityData[accessProfile]?.allowedGroups?.includes("request_offsite_group") && initialLoad &&<GroupRequest_Offsite_Group 
