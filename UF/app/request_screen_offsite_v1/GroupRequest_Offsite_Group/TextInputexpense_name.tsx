...
const TextInputexpense_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "code": "setrequest_offsite_group((prev) => ({ ...prev, claim_expense_type: \"offsite_expense\"}));\r\nconsole.log(\"claim_expense_type\");",
  "rule": {},
  "events": {},
  "mapper": [
    {
      ...
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Request_Screen_Offsite:AFVK:v1|4d39522ee30443598090fe9ac0c429cb|527c7ce2701a4e85917227cd8d9bf755"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=`setrequest_offsite_group((prev) => ({ ...prev, claim_expense_type: "offsite_expense"}));
console.log("claim_expense_type");`;
     if (code != '') {
      let codeStates: any = {};
      codeStates['request_offsite_group']  = {...request_offsite_group429cb,expense_name:newInputValue},
      codeStates['setrequest_offsite_group'] = setrequest_offsite_group429cb,
      codeStates['claims_detail_table']  = {...claims_detail_tablef8143,expense_name:newInputValue},
    ...
    setrequest_offsite_group429cb((prev: any) => ({ ...prev, expense_name: +e.target.value }));
    }
    else{
    setrequest_offsite_group429cb((prev: any) => ({ ...prev, expense_name: e.target.value }));
    }
  ...
  if(dfd_claims_v1Props?.setSearchFilters && dfd_claims_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_v1Props.data) && dfd_claims_v1Props.data.length > 0){
      setrequest_offsite_group429cb((pre:any)=>({...pre,expense_name:dfd_claims_v1Props.data[0]?.expense_name}));
    }
  }
  },[dfd_claims_v1Props?.setSearchFilters])
  if (expense_namebf755?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `5 / 20`,gridRow: `25 / 43`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={request_offsite_group429cb?.expense_name||""}
         disabled= {expense_namebf755?.isDisabled ? true : false}
        pin='brick-brick'     
