...
const handleUpdate = async(date: any) => {
  ...
  setrequest_offsite_group429cb((prev: any) => ({ ...prev, from_date: isoDate }))
}
...
useEffect(()=>{
 ...
 },[from_date6f9c3?.refresh])

  useEffect(()=>{
      handleBlur()
  },[validateRefetch.value])

if (from_date6f9c3?.isHidden) {
  return <></>
}

return (
  <div 
  style={{gridColumn: `5 / 11`,gridRow: `47 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={request_offsite_group429cb?.from_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      readOnly=  {from_date6f9c3?.isDisabled ? true : false}
      disabled= {from_date6f9c3?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
