...
const Textdoc_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && dfd_backgroundcheckdoctable_v1Props && !group_delete04302.doc_name){
        setgroup_delete04302((pre:any)=>({...pre,doc_name:dfd_backgroundcheckdoctable_v1Props[0]?.doc_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_name36a98?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && !group_delete04302.doc_name){
    setgroup_delete04302((pre:any)=>({...pre,doc_name:dfd_backgroundcheckdoctable_v1Props[0]?.doc_name}));
  }
  },[dfd_backgroundcheckdoctable_v1Props])

  if (doc_name36a98?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `8 / 24`,gridRow: `19 / 24`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_name : (group_delete04302?.doc_name || ""))}
</Text>
  </div>
  )
}
