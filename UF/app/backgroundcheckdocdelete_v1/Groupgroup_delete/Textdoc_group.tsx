...
const Textdoc_group = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && dfd_backgroundcheckdoctable_v1Props && !group_delete04302.doc_group){
        setgroup_delete04302((pre:any)=>({...pre,doc_group:dfd_backgroundcheckdoctable_v1Props[0]?.doc_group}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[doc_group89f93?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && !group_delete04302.doc_group){
    setgroup_delete04302((pre:any)=>({...pre,doc_group:dfd_backgroundcheckdoctable_v1Props[0]?.doc_group}));
  }
  },[dfd_backgroundcheckdoctable_v1Props])

  if (doc_group89f93?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.doc_group : (group_delete04302?.doc_group || ""))}
</Text>
  </div>
  )
}
