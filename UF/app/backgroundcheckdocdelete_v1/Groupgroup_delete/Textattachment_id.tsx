...
const Textattachment_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && dfd_backgroundcheckdoctable_v1Props && !group_delete04302.attachment_id){
        setgroup_delete04302((pre:any)=>({...pre,attachment_id:dfd_backgroundcheckdoctable_v1Props[0]?.attachment_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[attachment_id23a23?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_backgroundcheckdoctable_v1Props) && !group_delete04302.attachment_id){
    setgroup_delete04302((pre:any)=>({...pre,attachment_id:dfd_backgroundcheckdoctable_v1Props[0]?.attachment_id}));
  }
  },[dfd_backgroundcheckdoctable_v1Props])

  if (attachment_id23a23?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.attachment_id : (group_delete04302?.attachment_id || ""))}
</Text>
  </div>
  )
}
