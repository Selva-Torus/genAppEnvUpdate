...
const Buttonok_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "2bf6503088ed5dc4f11b1b567daba1e7"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonba1e7") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "2bf6503088ed5dc4f11b1b567daba1e7") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonba1e7");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonba1e7?.trigger) return;
      if(ok_buttonba1e7?.trigger){
      setok_buttonba1e7((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonba1e7?.trigger])

  useEffect(()=>{
    if(ok_buttonba1e7?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonba1e7?.refresh])
  ...
  async function handleSavea1e7_1_1_1(){
      

  setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    await delay(1000); 

    const group_delete04302Props = group_delete04302PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.backgroundCheckDocDelete_v1 || {}).some(val => {
    ...
    try{
      ...
      let mainData:any=structuredClone(group_delete04302Ref.current);
      ...
      let lockedKeysLength:number;
        if(group_delete04302?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "2bf6503088ed5dc4f11b1b567daba1e7",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "2bf6503088ed5dc4f11b1b567daba1e7.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "2bf6503088ed5dc4f11b1b567daba1e7.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "2bf6503088ed5dc4f11b1b567daba1e7.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "2bf6503088ed5dc4f11b1b567daba1e7.1.1.1.2",
              ...
              "children": [
                {
                  "id": "f56c544fe9d33cadb4368d9ad2bb42f3.1.1.1.2.1",
                  "value": "",
                  "type": "screen",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:backgroundCheckDocDelete:AFVK:v1";
      sourceId+= "|"+"1c41ce322e1e7891f96d939d82f04302";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:backgroundCheckDocDelete:AFVK:v1"+"|"+"1c41ce322e1e7891f96d939d82f04302"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete04302?.upId? [group_delete04302?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete04302?.upId? [group_delete04302?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete04302?.upId? [group_delete04302?.upId ] : group_delete04302?.trs_process_id? [group_delete04302?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete04302Ref.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"1c41ce322e1e7891f96d939d82f04302",
              controlId:"2bf6503088ed5dc4f11b1b567daba1e7"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete04302Props.ssKey !== '' && group_delete04302Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete04302Props.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete04302((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSavea1e7_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete04302((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
 ...
 if (ok_buttonba1e7?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `20 / 25`,gridRow: `36 / 43`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="!bg-[#DC2626] hover:!bg-[#B91C1C] !text-white !rounded-lg !font-bold"
          onClick={handleClick}
          view='normal'
          disabled= {ok_buttonba1e7?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
