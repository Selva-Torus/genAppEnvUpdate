...
const Groupadd_case_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {account_iddb411, setaccount_iddb411}= useContext(TotalContext) as TotalContextProps;
  const {venue_id85b23, setvenue_id85b23}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "eab16a1dfae32bbcbb962bfc5f54945a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadd_case_group4945aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("header_group")){
        setheader_groupf55cb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(header_groupf55cb?.isDisabled==null)
      {
        setheader_groupf55cb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("required_dociument_main_group")){
        setrequired_dociument_main_groupdfaaf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(required_dociument_main_groupdfaaf?.isDisabled==null)
      {
        setrequired_dociument_main_groupdfaaf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_information_group")){
        setcase_information_group40df6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_information_group40df6?.isDisabled==null)
      {
        setcase_information_group40df6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_details_group")){
        setvenue_details_group5f664((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_details_group5f664?.isDisabled==null)
      {
        setvenue_details_group5f664((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_main_group")){
        setchecklist_main_group2d71b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_main_group2d71b?.isDisabled==null)
      {
        setchecklist_main_group2d71b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules_group")){
        setspecial_rules_group47bec((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_group47bec?.isDisabled==null)
      {
        setspecial_rules_group47bec((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("account_id")){
        setaccount_iddb411((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(account_iddb411?.isDisabled==null)
      {
        setaccount_iddb411((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_id")){
        setvenue_id85b23((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_id85b23?.isDisabled==null)
      {
        setvenue_id85b23((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['account_id'] = account_iddb411,
        codeStates['setaccount_id'] = setaccount_iddb411,
        codeStates['venue_id'] = venue_id85b23,
        codeStates['setvenue_id'] = setvenue_id85b23,
  ...
  const add_case_group4945aRef = useRef<any>(null);
  const handleClearSearch = () => {
    add_case_group4945aRef.current?.setSearchParams();
    add_case_group4945aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(add_case_group4945a) && Object.keys(add_case_group4945a)?.length>0)
      {
        setadd_case_group4945a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [add_case_group4945aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"add_case_group"}))
        }}
    >
        ...
        {allowedComponent.includes("special_rules_group")  &&<Groupspecial_rules_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("account_id") ?<Textaccount_id   /* db411 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("venue_id") ?<Textvenue_id   /* 85b23 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
