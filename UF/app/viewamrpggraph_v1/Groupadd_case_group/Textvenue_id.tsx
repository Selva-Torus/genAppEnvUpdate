...
const Textvenue_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {venue_id85b23Props, setvenue_id85b23Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setadd_case_group4945a((pre: any) => ({
          ...pre,
          venue_id: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.venue_id
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setadd_case_group4945a((pre: any) => ({
          ...pre,
          venue_id: venue_id85b23Props?.filteredData?.length > 0
            ? venue_id85b23Props?.filteredData[0]?.venue_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_group4945a.venue_id){
        setadd_case_group4945a((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[venue_id85b23?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_group4945a.venue_id){
    setadd_case_group4945a((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!venue_id85b23Props?.filterProps) return;
    handleMapperValue(venue_id85b23Props?.filterProps,venue_id85b23Props?.filterFlag);
  },[venue_id85b23Props?.filterProps])

  if (venue_id85b23?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `4 / 6`,gridRow: `229 / 230`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
