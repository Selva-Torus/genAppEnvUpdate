...
const Groupchecklist_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {valid_checklist_text1abcc, setvalid_checklist_text1abcc}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9f03db2baa5f85949f4d5a9dc312d71b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_main_group2d71bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valid_checklist_text")){
        setvalid_checklist_text1abcc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_checklist_text1abcc?.isDisabled==null)
      {
        setvalid_checklist_text1abcc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_table")){
        setchecklist_tablec0934((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_tablec0934?.isDisabled==null)
      {
        setchecklist_tablec0934((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['valid_checklist_text'] = valid_checklist_text1abcc,
        codeStates['setvalid_checklist_text'] = setvalid_checklist_text1abcc,
  ...
  const checklist_main_group2d71bRef = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_main_group2d71bRef.current?.setSearchParams();
    checklist_main_group2d71bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_main_group2d71b) && Object.keys(checklist_main_group2d71b)?.length>0)
      {
        setchecklist_main_group2d71b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_main_group2d71bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_main_group"}))
        }}
    >
        {allowedComponent.includes("checklist_table")  &&<Groupchecklist_table  
          ...
          {allowedControls.includes("valid_checklist_text") ?<Textvalid_checklist_text   /* 1abcc */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
