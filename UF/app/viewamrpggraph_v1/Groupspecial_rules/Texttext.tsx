...
const Texttext = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {textc6918Props, settextc6918Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setspecial_rules3c582((pre: any) => ({
          ...pre,
          text: textc6918Props?.filteredData?.length > 0
            ? textc6918Props?.filteredData[0]?.text
            : "0"
        }))
      }else if(Array.isArray(dfd_specialrulessurerealdb_v1Props) && dfd_specialrulessurerealdb_v1Props && !special_rules3c582.text){
        setspecial_rules3c582((pre:any)=>({...pre,text:dfd_specialrulessurerealdb_v1Props[0]?.text}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_specialrulessurerealdb_v1Props) && !special_rules3c582.text){
    setspecial_rules3c582((pre:any)=>({...pre,text:dfd_specialrulessurerealdb_v1Props[0]?.text}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!textc6918Props?.filterProps) return;
    handleMapperValue(textc6918Props?.filterProps,textc6918Props?.filterFlag);
  },[textc6918Props?.filterProps])

  if (textc6918?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.text : (special_rules3c582?.text || ""))}
</Text>
  </div>
  )
}
