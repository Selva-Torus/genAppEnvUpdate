...
const Groupspecial_rules = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {dynamic_icona2832, setdynamic_icona2832}= useContext(TotalContext) as TotalContextProps;
  const {textc6918, settextc6918}= useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7b373f1cd501a63195aa046989f3c582");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setspecial_rules3c582Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("dynamic_icon")){
        setdynamic_icona2832((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamic_icona2832?.isDisabled==null)
      {
        setdynamic_icona2832((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("text")){
        settextc6918((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(textc6918?.isDisabled==null)
      {
        settextc6918((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['dynamic_icon'] = dynamic_icona2832,
        codeStates['setdynamic_icon'] = setdynamic_icona2832,
        codeStates['text'] = textc6918,
        codeStates['settext'] = settextc6918,
  ...
  const special_rules3c582Ref = useRef<any>(null);
  const handleClearSearch = () => {
    special_rules3c582Ref.current?.setSearchParams();
    special_rules3c582Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(special_rules3c582) && Object.keys(special_rules3c582)?.length>0)
      {
        setspecial_rules3c582({})
      }
    }else 
      prevRefreshRef.current= true
  }, [special_rules3c582Props?.refresh,token])
    ...
    <div key={idx}
      ...
      onClick={(e:any)=>{
        ...
        handleOnClick(item, idx);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"special_rules"+idx}))
      }}
    >
        {allowedControls.includes("dynamic_icon")?<Icondynamic_icon /* a2832 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("text") ?<Texttext   /* c6918 */ isDynamic={true } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
  )
  })}
  </>
