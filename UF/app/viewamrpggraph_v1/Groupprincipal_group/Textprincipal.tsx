...
const Textprincipal = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {principala34bdProps, setprincipala34bdProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setprincipal_group510ca((pre: any) => ({
          ...pre,
          principal: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.principal
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setprincipal_group510ca((pre: any) => ({
          ...pre,
          principal: principala34bdProps?.filteredData?.length > 0
            ? principala34bdProps?.filteredData[0]?.principal
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !principal_group510ca.principal){
        setprincipal_group510ca((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[principala34bd?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !principal_group510ca.principal){
    setprincipal_group510ca((pre:any)=>({...pre,principal:dfd_addcase_v1Props[0]?.principal}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!principala34bdProps?.filterProps) return;
    handleMapperValue(principala34bdProps?.filterProps,principala34bdProps?.filterFlag);
  },[principala34bdProps?.filterProps])

  if (principala34bd?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.principal : (principal_group510ca?.principal || ""))}
</Text>
  </div>
  )
}
