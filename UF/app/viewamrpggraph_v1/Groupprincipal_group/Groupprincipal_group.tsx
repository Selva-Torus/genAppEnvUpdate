...
const Groupprincipal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {principal_text9bbf4, setprincipal_text9bbf4}= useContext(TotalContext) as TotalContextProps;
  const {principala34bd, setprincipala34bd}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9bf2264c5895b740778e6c16c8c510ca");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprincipal_group510caProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_text")){
        setprincipal_text9bbf4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_text9bbf4?.isDisabled==null)
      {
        setprincipal_text9bbf4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("principal")){
        setprincipala34bd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principala34bd?.isDisabled==null)
      {
        setprincipala34bd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['principal_text'] = principal_text9bbf4,
        codeStates['setprincipal_text'] = setprincipal_text9bbf4,
        codeStates['principal'] = principala34bd,
        codeStates['setprincipal'] = setprincipala34bd,
  ...
  const principal_group510caRef = useRef<any>(null);
  const handleClearSearch = () => {
    principal_group510caRef.current?.setSearchParams();
    principal_group510caRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(principal_group510ca) && Object.keys(principal_group510ca)?.length>0)
      {
        setprincipal_group510ca({})
      }
    }else 
      prevRefreshRef.current= true
  }, [principal_group510caProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"principal_group"}))
        }}
    >
          {allowedControls.includes("principal_text") ?<Textprincipal_text   /* 9bbf4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("principal") ?<Textprincipal   /* a34bd */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
