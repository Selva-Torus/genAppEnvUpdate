...
let defaultColumns:any = [
  {
    ...
    "nodeid": "c93bb73dd0a6735d3aab93b230ef168d",
  ...
  {
    "id": "item_name",
    "nodeid": "24f91ed14e4e435b1ac55afb2d3dd097",
  ...
  {
    "id": "is_completed",
    "nodeid": "90f3951f590cd9c2025f303e1fdd3b5a",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tablechecklist_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {checklist_item_idf168d, setchecklist_item_idf168d}= useContext(TotalContext) as TotalContextProps  
  const {item_namedd097, setitem_namedd097}= useContext(TotalContext) as TotalContextProps  
  const {is_completedd3b5a, setis_completedd3b5a}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "85d1235d04a77af1f06ec21a9fec0934",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, checklist_tablec0934 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, checklist_tablec0934 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, checklist_tablec0934 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setchecklist_tablec0934Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      checklist_tablec0934.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setchecklist_tablec0934Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setchecklist_tablec0934Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['checklist_item_id'] = checklist_item_idf168d,
        codeStates['setchecklist_item_id'] = setchecklist_item_idf168d,
        codeStates['item_name'] = item_namedd097,
        codeStates['setitem_name'] = setitem_namedd097,
        codeStates['is_completed'] = is_completedd3b5a,
        codeStates['setis_completed'] = setis_completedd3b5a,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setchecklist_tablec0934Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["account_id"]:checklist_tablec0934Props?.presetValues?.account_id}
    ...
    if(!checklist_tablec0934Props?.presetValues || Object.keys(checklist_tablec0934Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '85d1235d04a77af1f06ec21a9fec0934',
                elementId: '85d1235d04a77af1f06ec21a9fec0934'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setchecklist_tablec0934(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(checklist_tablec0934))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (checklist_tablec0934Props.filterInitalLoad) return;

    const filterControllers = checklist_tablec0934Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['checklist_tablec0934']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['checklist_tablec0934']).filter((k: string) => jsonEntry['checklist_tablec0934'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(checklist_tablec0934Props.filterProps);
      return;
    }
    ...
    setchecklist_tablec0934Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(checklist_tablec0934Props.filterProps);

  }, [
    checklist_tablec0934Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    checklist_tablec0934Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(checklist_tablec0934Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(checklist_tablec0934Props.searchFilter) || {};
...
}, [checklist_tablec0934Props.searchFilter])
  ...
  useEffect(() => {
    if (checklist_tablec0934Props?.clearData === true) {
      setchecklist_tablec0934([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setchecklist_tablec0934Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [checklist_tablec0934Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [checklist_tablec0934Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = checklist_tablec0934Props?.selectedIds[checklist_tablec0934Props?.selectedIds?.length-1]
    if(Array.isArray(checklist_tablec0934) && checklist_tablec0934.length>0)
    {
      let data = checklist_tablec0934.find((data:any)=>(data?.checklist_status_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "checklist_status_id",
  ...
  useEffect(() => {
    if (checklist_tablec0934Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (checklist_tablec0934Props?.selectedIds?.length == 0){
      handleOnRowClick({},checklist_tablec0934Props?.selectedIds)
    }
  }, [checklist_tablec0934Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(checklist_tablec0934?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&checklist_tablec0934?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](checklist_tablec0934?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_tablec0934?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&checklist_tablec0934?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](checklist_tablec0934?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_tablec0934?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (checklist_tablec0934?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"85d1235d04a77af1f06ec21a9fec0934"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={checklist_tablec0934Props?.selectedIds}  
