...
const Groupchecklist_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "85d1235d04a77af1f06ec21a9fec0934");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_tablec0934Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("checklist_item_id")){
        setchecklist_item_idf168d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_item_idf168d?.isDisabled==null)
      {
        setchecklist_item_idf168d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("item_name")){
        setitem_namedd097((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(item_namedd097?.isDisabled==null)
      {
        setitem_namedd097((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("is_completed")){
        setis_completedd3b5a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(is_completedd3b5a?.isDisabled==null)
      {
        setis_completedd3b5a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const checklist_tablec0934Ref = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_tablec0934Ref.current?.setSearchParams();
    checklist_tablec0934Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_tablec0934) && Object.keys(checklist_tablec0934)?.length>0)
      {
        setchecklist_tablec0934({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_tablec0934Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablechecklist_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={checklist_tablec0934Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
