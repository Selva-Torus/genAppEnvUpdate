...
const Groupspecial_rules_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {special_rules_texta6d06, setspecial_rules_texta6d06}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a71e4125dcfcaeed1c3196cc80e47bec");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setspecial_rules_group47becProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("special_rules_text")){
        setspecial_rules_texta6d06((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_texta6d06?.isDisabled==null)
      {
        setspecial_rules_texta6d06((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules3c582((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules3c582?.isDisabled==null)
      {
        setspecial_rules3c582((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['special_rules_text'] = special_rules_texta6d06,
        codeStates['setspecial_rules_text'] = setspecial_rules_texta6d06,
  ...
  const special_rules_group47becRef = useRef<any>(null);
  const handleClearSearch = () => {
    special_rules_group47becRef.current?.setSearchParams();
    special_rules_group47becRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(special_rules_group47bec) && Object.keys(special_rules_group47bec)?.length>0)
      {
        setspecial_rules_group47bec({})
      }
    }else 
      prevRefreshRef.current= true
  }, [special_rules_group47becProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"special_rules_group"}))
        }}
    >
        {allowedComponent.includes("special_rules")  &&<Groupspecial_rules  
          ...
          {allowedControls.includes("special_rules_text") ?<Textspecial_rules_text   /* a6d06 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
