...
const Groupintrest_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {intrest_texte0e7e, setintrest_texte0e7e}= useContext(TotalContext) as TotalContextProps;
  const {interest5fac3, setinterest5fac3}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d74110d0594a68a1c13d3d0b22e1ba85");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setintrest_group1ba85Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("intrest_text")){
        setintrest_texte0e7e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_texte0e7e?.isDisabled==null)
      {
        setintrest_texte0e7e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("interest")){
        setinterest5fac3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(interest5fac3?.isDisabled==null)
      {
        setinterest5fac3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['intrest_text'] = intrest_texte0e7e,
        codeStates['setintrest_text'] = setintrest_texte0e7e,
        codeStates['interest'] = interest5fac3,
        codeStates['setinterest'] = setinterest5fac3,
  ...
  const intrest_group1ba85Ref = useRef<any>(null);
  const handleClearSearch = () => {
    intrest_group1ba85Ref.current?.setSearchParams();
    intrest_group1ba85Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(intrest_group1ba85) && Object.keys(intrest_group1ba85)?.length>0)
      {
        setintrest_group1ba85({})
      }
    }else 
      prevRefreshRef.current= true
  }, [intrest_group1ba85Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"intrest_group"}))
        }}
    >
          {allowedControls.includes("intrest_text") ?<Textintrest_text   /* e0e7e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("interest") ?<Textinterest   /* 5fac3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
