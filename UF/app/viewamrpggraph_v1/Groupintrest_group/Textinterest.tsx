...
const Textinterest = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {interest5fac3Props, setinterest5fac3Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setintrest_group1ba85((pre: any) => ({
          ...pre,
          interest: interest5fac3Props?.filteredData?.length > 0
            ? interest5fac3Props?.filteredData[0]?.interest
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !intrest_group1ba85.interest){
        setintrest_group1ba85((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[interest5fac3?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !intrest_group1ba85.interest){
    setintrest_group1ba85((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!interest5fac3Props?.filterProps) return;
    handleMapperValue(interest5fac3Props?.filterProps,interest5fac3Props?.filterFlag);
  },[interest5fac3Props?.filterProps])

  if (interest5fac3?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.interest : (intrest_group1ba85?.interest || ""))}
</Text>
  </div>
  )
}
