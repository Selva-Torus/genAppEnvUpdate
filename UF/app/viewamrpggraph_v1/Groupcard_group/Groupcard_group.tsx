...
const Groupcard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "04f5bff7ca08bae7cc3a2f610a600ce3");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcard_group00ce3Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_group")){
        setprincipal_group510ca((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_group510ca?.isDisabled==null)
      {
        setprincipal_group510ca((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("intrest_group")){
        setintrest_group1ba85((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_group1ba85?.isDisabled==null)
      {
        setintrest_group1ba85((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees_group")){
        setfees_groupbee4a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_groupbee4a?.isDisabled==null)
      {
        setfees_groupbee4a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_group")){
        settotal_group197f6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_group197f6?.isDisabled==null)
      {
        settotal_group197f6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const card_group00ce3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    card_group00ce3Ref.current?.setSearchParams();
    card_group00ce3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card_group00ce3) && Object.keys(card_group00ce3)?.length>0)
      {
        setcard_group00ce3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card_group00ce3Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"card_group"}))
        }}
    >
