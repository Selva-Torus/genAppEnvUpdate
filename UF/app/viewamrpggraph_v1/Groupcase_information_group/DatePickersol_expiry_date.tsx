...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,sol_expiry_date:undefined}}));
  if (!date) {
    setcase_information_group40df6((prev: any) => ({ ...prev, sol_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group40df6((prev: any) => ({ ...prev, sol_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "7ed5a00247126f350e457ce589c3d70d"
      );
...
if (sol_expiry_date3d70d?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `98 / 110`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group40df6?.sol_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {sol_expiry_date3d70d?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="SOL Expires"
      dateValidation=""
      validationState={validate?.viewAMRPgGraph_v1?.sol_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
