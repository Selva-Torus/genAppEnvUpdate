...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,charge_off_date:undefined}}));
  if (!date) {
    setcase_information_group40df6((prev: any) => ({ ...prev, charge_off_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group40df6((prev: any) => ({ ...prev, charge_off_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "17b2c8ec35a994a8e24cd082e2e4e80f"
      );
...
if (charge_off_date4e80f?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      ...
      //label={keyset("")}
      value={case_information_group40df6?.charge_off_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {charge_off_date4e80f?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Charge Off Date"
      dateValidation=""
      validationState={validate?.viewAMRPgGraph_v1?.charge_off_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
