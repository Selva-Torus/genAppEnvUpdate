...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,last_payment_date:undefined}}));
  if (!date) {
    setcase_information_group40df6((prev: any) => ({ ...prev, last_payment_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group40df6((prev: any) => ({ ...prev, last_payment_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "1f4ce553795910e5f71ac986298f6b2b"
      );
...
useEffect(()=>{
 ...
 },[last_payment_datef6b2b?.refresh])
...
if (last_payment_datef6b2b?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `13 / 25`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group40df6?.last_payment_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {last_payment_datef6b2b?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Last Payment"
      dateValidation=""
      validationState={validate?.viewAMRPgGraph_v1?.last_payment_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
