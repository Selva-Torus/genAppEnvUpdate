...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|29949970b12d2ab7344e422611d40df6|9ae314ef60cb8a713b8a327a65c12f82"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,debtor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group40df6((prev: any) => ({ ...prev, debtor_name: +e.target.value }));
    }
    else{
    setcase_information_group40df6((prev: any) => ({ ...prev, debtor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9ae314ef60cb8a713b8a327a65c12f82"
      );
      ...
      //     controlId: "9ae314ef60cb8a713b8a327a65c12f82",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9ae314ef60cb8a713b8a327a65c12f82") {
        handleChange({target:{value:case_information_group40df6Ref?.current?.debtor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9ae314ef60cb8a713b8a327a65c12f82") {
        handleBlur({target:{value:case_information_group40df6Ref?.current?.debtor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group40df6((pre:any)=>({...pre,debtor_name:dfd_addcase_v1Props.data[0]?.debtor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (debtor_name12f82?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group40df6?.debtor_name||""}
         disabled= {debtor_name12f82?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Debtor"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.debtor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
