...
const TextInputcase_display_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|29949970b12d2ab7344e422611d40df6|47dd2c6791d54c83a7a6d1131b9da9aa"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,case_display_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group40df6((prev: any) => ({ ...prev, case_display_id: +e.target.value }));
    }
    else{
    setcase_information_group40df6((prev: any) => ({ ...prev, case_display_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "47dd2c6791d54c83a7a6d1131b9da9aa"
      );
      ...
      //     controlId: "47dd2c6791d54c83a7a6d1131b9da9aa",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "47dd2c6791d54c83a7a6d1131b9da9aa") {
        handleChange({target:{value:case_information_group40df6Ref?.current?.case_display_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "47dd2c6791d54c83a7a6d1131b9da9aa") {
        handleBlur({target:{value:case_information_group40df6Ref?.current?.case_display_id||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group40df6((pre:any)=>({...pre,case_display_id:dfd_addcase_v1Props.data[0]?.case_display_id}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (case_display_idda9aa?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group40df6?.case_display_id||""}
         disabled= {case_display_idda9aa?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.case_display_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
