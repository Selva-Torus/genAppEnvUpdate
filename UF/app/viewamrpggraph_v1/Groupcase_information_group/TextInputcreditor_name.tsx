...
const TextInputcreditor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|29949970b12d2ab7344e422611d40df6|b02c29ffa36712cf2636674b99fb337f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,creditor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group40df6((prev: any) => ({ ...prev, creditor_name: +e.target.value }));
    }
    else{
    setcase_information_group40df6((prev: any) => ({ ...prev, creditor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b02c29ffa36712cf2636674b99fb337f"
      );
      ...
      //     controlId: "b02c29ffa36712cf2636674b99fb337f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b02c29ffa36712cf2636674b99fb337f") {
        handleChange({target:{value:case_information_group40df6Ref?.current?.creditor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b02c29ffa36712cf2636674b99fb337f") {
        handleBlur({target:{value:case_information_group40df6Ref?.current?.creditor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group40df6((pre:any)=>({...pre,creditor_name:dfd_addcase_v1Props.data[0]?.creditor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (creditor_nameb337f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group40df6?.creditor_name||""}
         disabled= {creditor_nameb337f?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Creditor"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.creditor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
