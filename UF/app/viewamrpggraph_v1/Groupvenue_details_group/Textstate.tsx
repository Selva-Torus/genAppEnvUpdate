...
const Textstate = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {state7419eProps, setstate7419eProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          state: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.state
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          state: state7419eProps?.filteredData?.length > 0
            ? state7419eProps?.filteredData[0]?.state
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.state){
        setvenue_details_group5f664((pre:any)=>({...pre,state:dfd_venuesurerealdb_v1Props[0]?.state}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.state){
    setvenue_details_group5f664((pre:any)=>({...pre,state:dfd_venuesurerealdb_v1Props[0]?.state}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!state7419eProps?.filterProps) return;
    handleMapperValue(state7419eProps?.filterProps,state7419eProps?.filterFlag);
  },[state7419eProps?.filterProps])

  if (state7419e?.isHidden) {
    return <></>
  }
