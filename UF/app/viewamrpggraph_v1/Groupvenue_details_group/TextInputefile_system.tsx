...
const TextInputefile_system = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:specialRulesSurerealDB:AFVK:v1|5c1ed2d414f740ab820856297865df26|items.properties.venue.items.properties.efile_system"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|bb2fc560566e3bb613de9b434655f664|33c6840d31848202f6c1aebfa4e4d3ab"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'efile_system',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,efile_system:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group5f664((prev: any) => ({ ...prev, efile_system: +e.target.value }));
    }
    else{
    setvenue_details_group5f664((prev: any) => ({ ...prev, efile_system: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "33c6840d31848202f6c1aebfa4e4d3ab"
      );
      ...
      //     controlId: "33c6840d31848202f6c1aebfa4e4d3ab",
      //     isTable: false,
      //     from:"TextInputefile_system",
      ...
      //   type={
      //     name:'efile_system',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.efile_system.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.efile_system.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.efile_system.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "33c6840d31848202f6c1aebfa4e4d3ab") {
        handleChange({target:{value:venue_details_group5f664Ref?.current?.efile_system||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "33c6840d31848202f6c1aebfa4e4d3ab") {
        handleBlur({target:{value:venue_details_group5f664Ref?.current?.efile_system||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group5f664((pre:any)=>({...pre,efile_system:dfd_specialrulessurerealdb_v1Props.data[0]?.efile_system}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (efile_system4d3ab?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group5f664?.efile_system||""}
         disabled= {efile_system4d3ab?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.efile_system ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputefile_system
