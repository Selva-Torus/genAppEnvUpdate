...
const Textcounty = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {countyb406dProps, setcountyb406dProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          county: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.county
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          county: countyb406dProps?.filteredData?.length > 0
            ? countyb406dProps?.filteredData[0]?.county
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.county){
        setvenue_details_group5f664((pre:any)=>({...pre,county:dfd_venuesurerealdb_v1Props[0]?.county}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.county){
    setvenue_details_group5f664((pre:any)=>({...pre,county:dfd_venuesurerealdb_v1Props[0]?.county}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!countyb406dProps?.filterProps) return;
    handleMapperValue(countyb406dProps?.filterProps,countyb406dProps?.filterFlag);
  },[countyb406dProps?.filterProps])

  if (countyb406d?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `29 / 35`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-white"
  variant="subheader-1"
  color="primary"
>
      {keyset(isDynamic ? item?.county : (venue_details_group5f664?.county || ""))}
</Text>
  </div>
  )
}

export default Textcounty
