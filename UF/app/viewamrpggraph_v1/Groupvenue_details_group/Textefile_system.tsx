...
const Textefile_system = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {efile_system553b3Props, setefile_system553b3Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          efile_system: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.efile_system
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          efile_system: efile_system553b3Props?.filteredData?.length > 0
            ? efile_system553b3Props?.filteredData[0]?.efile_system
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.efile_system){
        setvenue_details_group5f664((pre:any)=>({...pre,efile_system:dfd_venuesurerealdb_v1Props[0]?.efile_system}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[efile_system553b3?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.efile_system){
    setvenue_details_group5f664((pre:any)=>({...pre,efile_system:dfd_venuesurerealdb_v1Props[0]?.efile_system}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!efile_system553b3Props?.filterProps) return;
    handleMapperValue(efile_system553b3Props?.filterProps,efile_system553b3Props?.filterFlag);
  },[efile_system553b3Props?.filterProps])

  if (efile_system553b3?.isHidden) {
    return <></>
  }
...
export default Textefile_system
