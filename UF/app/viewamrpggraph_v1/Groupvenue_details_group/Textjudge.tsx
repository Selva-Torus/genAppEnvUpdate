...
const Textjudge = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {judge65dffProps, setjudge65dffProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          judge: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.judge
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          judge: judge65dffProps?.filteredData?.length > 0
            ? judge65dffProps?.filteredData[0]?.judge
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.judge){
        setvenue_details_group5f664((pre:any)=>({...pre,judge:dfd_venuesurerealdb_v1Props[0]?.judge}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.judge){
    setvenue_details_group5f664((pre:any)=>({...pre,judge:dfd_venuesurerealdb_v1Props[0]?.judge}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!judge65dffProps?.filterProps) return;
    handleMapperValue(judge65dffProps?.filterProps,judge65dffProps?.filterFlag);
  },[judge65dffProps?.filterProps])

  if (judge65dff?.isHidden) {
    return <></>
  }
