...
const Texttext_service_method = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {text_service_methodabd13Props, settext_service_methodabd13Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[text_service_methodabd13?.refresh])

  if (text_service_methodabd13?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Service Method")}
</Text>
  </div>
  )
}

export default Texttext_service_method
