...
const TextInputstate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|bb2fc560566e3bb613de9b434655f664|eb00161701ae9f6e4a8ddc52fb30a6f5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,state:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group5f664((prev: any) => ({ ...prev, state: +e.target.value }));
    }
    else{
    setvenue_details_group5f664((prev: any) => ({ ...prev, state: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "eb00161701ae9f6e4a8ddc52fb30a6f5"
      );
      ...
      //     controlId: "eb00161701ae9f6e4a8ddc52fb30a6f5",
      //     isTable: false,
      //     from:"TextInputstate",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "eb00161701ae9f6e4a8ddc52fb30a6f5") {
        handleChange({target:{value:venue_details_group5f664Ref?.current?.state||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "eb00161701ae9f6e4a8ddc52fb30a6f5") {
        handleBlur({target:{value:venue_details_group5f664Ref?.current?.state||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group5f664((pre:any)=>({...pre,state:dfd_specialrulessurerealdb_v1Props.data[0]?.state}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (state0a6f5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group5f664?.state||""}
         disabled= {state0a6f5?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="State"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.state ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
