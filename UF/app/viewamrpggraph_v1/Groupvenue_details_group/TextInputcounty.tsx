...
const TextInputcounty = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|bb2fc560566e3bb613de9b434655f664|fb15175c3e9e875e90d1357e1aac9b2f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,county:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group5f664((prev: any) => ({ ...prev, county: +e.target.value }));
    }
    else{
    setvenue_details_group5f664((prev: any) => ({ ...prev, county: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "fb15175c3e9e875e90d1357e1aac9b2f"
      );
      ...
      //     controlId: "fb15175c3e9e875e90d1357e1aac9b2f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "fb15175c3e9e875e90d1357e1aac9b2f") {
        handleChange({target:{value:venue_details_group5f664Ref?.current?.county||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "fb15175c3e9e875e90d1357e1aac9b2f") {
        handleBlur({target:{value:venue_details_group5f664Ref?.current?.county||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group5f664((pre:any)=>({...pre,county:dfd_specialrulessurerealdb_v1Props.data[0]?.county}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (countyc9b2f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group5f664?.county||""}
         disabled= {countyc9b2f?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Country"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.county ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
