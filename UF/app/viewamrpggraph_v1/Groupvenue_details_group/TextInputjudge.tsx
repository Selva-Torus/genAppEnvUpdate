...
const TextInputjudge = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:specialRulesSurerealDB:AFVK:v1|5c1ed2d414f740ab820856297865df26|items.properties.venue.items.properties.judge"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|bb2fc560566e3bb613de9b434655f664|bfcb95c26aa481595b9ec4e847f12cde"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'judge',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,judge:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group5f664((prev: any) => ({ ...prev, judge: +e.target.value }));
    }
    else{
    setvenue_details_group5f664((prev: any) => ({ ...prev, judge: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "bb2fc560566e3bb613de9b434655f664",
        "bfcb95c26aa481595b9ec4e847f12cde"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1",
      //     componentId: "bb2fc560566e3bb613de9b434655f664",
      //     controlId: "bfcb95c26aa481595b9ec4e847f12cde",
      //     isTable: false,
      //     from:"TextInputjudge",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'judge', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'judge',type:'text'};
      //   type={
      //     name:'judge',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.judge.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.judge.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.judge.type
      //   }
      ...
      //   type={
      //     name:'judge',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.judge.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const venue_details_group5f664Ref = useRef<any>(venue_details_group5f664);
  useEffect(() => { venue_details_group5f664Ref.current = venue_details_group5f664; }, [venue_details_group5f664]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "bfcb95c26aa481595b9ec4e847f12cde") {
        handleChange({target:{value:venue_details_group5f664Ref?.current?.judge||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "bfcb95c26aa481595b9ec4e847f12cde") {
        handleBlur({target:{value:venue_details_group5f664Ref?.current?.judge||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group5f664((pre:any)=>({...pre,judge:dfd_specialrulessurerealdb_v1Props.data[0]?.judge}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (judge12cde?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 25`,gridRow: `56 / 68`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group5f664?.judge||""}
         disabled= {judge12cde?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Judge"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.judge ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
