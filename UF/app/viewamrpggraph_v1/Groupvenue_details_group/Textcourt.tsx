...
const Textcourt = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {court02ff6Props, setcourt02ff6Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          court: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.court
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          court: court02ff6Props?.filteredData?.length > 0
            ? court02ff6Props?.filteredData[0]?.court
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.court){
        setvenue_details_group5f664((pre:any)=>({...pre,court:dfd_venuesurerealdb_v1Props[0]?.court}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.court){
    setvenue_details_group5f664((pre:any)=>({...pre,court:dfd_venuesurerealdb_v1Props[0]?.court}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!court02ff6Props?.filterProps) return;
    handleMapperValue(court02ff6Props?.filterProps,court02ff6Props?.filterFlag);
  },[court02ff6Props?.filterProps])

  if (court02ff6?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `46 / 52`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-white"
  variant="subheader-1"
  color="primary"
>
      {keyset(isDynamic ? item?.court : (venue_details_group5f664?.court || ""))}
</Text>
  </div>
  )
}

export default Textcourt
