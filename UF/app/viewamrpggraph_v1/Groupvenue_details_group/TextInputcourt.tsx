...
const TextInputcourt = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewAMRPgGraph:AFVK:v1|bb2fc560566e3bb613de9b434655f664|a6136b22b75d8afd178b24c51dd411e1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewAMRPgGraph_v1:{...pre?.viewAMRPgGraph_v1,court:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group5f664((prev: any) => ({ ...prev, court: +e.target.value }));
    }
    else{
    setvenue_details_group5f664((prev: any) => ({ ...prev, court: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a6136b22b75d8afd178b24c51dd411e1"
      );
      ...
      //     controlId: "a6136b22b75d8afd178b24c51dd411e1",
      //     isTable: false,
      //     from:"TextInputcourt",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a6136b22b75d8afd178b24c51dd411e1") {
        handleChange({target:{value:venue_details_group5f664Ref?.current?.court||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a6136b22b75d8afd178b24c51dd411e1") {
        handleBlur({target:{value:venue_details_group5f664Ref?.current?.court||""}});
      }
    };
  ...
  if(dfd_specialrulessurerealdb_v1Props?.setSearchFilters && dfd_specialrulessurerealdb_v1Props?.data)
  {
    if(Array.isArray(dfd_specialrulessurerealdb_v1Props.data) && dfd_specialrulessurerealdb_v1Props.data.length > 0){
      setvenue_details_group5f664((pre:any)=>({...pre,court:dfd_specialrulessurerealdb_v1Props.data[0]?.court}));
    }
  }
  },[dfd_specialrulessurerealdb_v1Props?.setSearchFilters])
  if (court411e1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group5f664?.court||""}
         disabled= {court411e1?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Court"
      errorMessage={error}
        validationState={validate?.viewAMRPgGraph_v1?.court ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
