...
const Textfiling_fee = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {filing_fee3e689Props, setfiling_fee3e689Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
    ...
    try{
      if ("hasLogicCenter" in dfd_venuesurerealdb_v1Props && !dfd_venuesurerealdb_v1Props.hasLogicCenter) {
        ...
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          filing_fee: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.filing_fee
            : "0"
        }))
      }
      else{
      if(filterFlag){
        setvenue_details_group5f664((pre: any) => ({
          ...pre,
          filing_fee: filing_fee3e689Props?.filteredData?.length > 0
            ? filing_fee3e689Props?.filteredData[0]?.filing_fee
            : "0"
        }))
      }else if(Array.isArray(dfd_venuesurerealdb_v1Props) && dfd_venuesurerealdb_v1Props && !venue_details_group5f664.filing_fee){
        setvenue_details_group5f664((pre:any)=>({...pre,filing_fee:dfd_venuesurerealdb_v1Props[0]?.filing_fee}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[filing_fee3e689?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_venuesurerealdb_v1Props) && !venue_details_group5f664.filing_fee){
    setvenue_details_group5f664((pre:any)=>({...pre,filing_fee:dfd_venuesurerealdb_v1Props[0]?.filing_fee}));
  }
  ...
  // setSearchFilters
  useEffect(() => {
    if (!filing_fee3e689Props?.filterProps) return;
    handleMapperValue(filing_fee3e689Props?.filterProps,filing_fee3e689Props?.filterFlag);
  },[filing_fee3e689Props?.filterProps])

  if (filing_fee3e689?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 25`,gridRow: `78 / 84`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className="!bg-white"
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.filing_fee : (venue_details_group5f664?.filing_fee || ""))}
</Text>
  </div>
  )
}

export default Textfiling_fee
