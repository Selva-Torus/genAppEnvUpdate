...
const Texttotal_balance = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {total_balance8ece0Props, settotal_balance8ece0Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        settotal_group197f6((pre: any) => ({
          ...pre,
          total_balance: total_balance8ece0Props?.filteredData?.length > 0
            ? total_balance8ece0Props?.filteredData[0]?.total_balance
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !total_group197f6.total_balance){
        settotal_group197f6((pre:any)=>({...pre,total_balance:dfd_addcase_v1Props[0]?.total_balance}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_balance8ece0?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !total_group197f6.total_balance){
    settotal_group197f6((pre:any)=>({...pre,total_balance:dfd_addcase_v1Props[0]?.total_balance}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!total_balance8ece0Props?.filterProps) return;
    handleMapperValue(total_balance8ece0Props?.filterProps,total_balance8ece0Props?.filterFlag);
  },[total_balance8ece0Props?.filterProps])

  if (total_balance8ece0?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_balance : (total_group197f6?.total_balance || ""))}
</Text>
  </div>
  )
}
