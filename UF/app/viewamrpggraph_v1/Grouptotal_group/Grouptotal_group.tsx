...
import Texttotal_balance_text  from "./Texttotal_balance_text";
import Texttotal_balance  from "./Texttotal_balance";
...
const Grouptotal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {total_balance_textc34b6, settotal_balance_textc34b6}= useContext(TotalContext) as TotalContextProps;
  const {total_balance8ece0, settotal_balance8ece0}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "aa110d72a53449ccd32229b5543197f6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_group197f6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_balance_text")){
        settotal_balance_textc34b6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balance_textc34b6?.isDisabled==null)
      {
        settotal_balance_textc34b6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_balance")){
        settotal_balance8ece0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balance8ece0?.isDisabled==null)
      {
        settotal_balance8ece0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_balance_text'] = total_balance_textc34b6,
        codeStates['settotal_balance_text'] = settotal_balance_textc34b6,
        codeStates['total_balance'] = total_balance8ece0,
        codeStates['settotal_balance'] = settotal_balance8ece0,
  ...
  const total_group197f6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_group197f6Ref.current?.setSearchParams();
    total_group197f6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(total_group197f6) && Object.keys(total_group197f6)?.length>0)
      {
        settotal_group197f6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [total_group197f6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"total_group"}))
        }}
    >
          {allowedControls.includes("total_balance_text") ?<Texttotal_balance_text   /* c34b6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_balance") ?<Texttotal_balance   /* 8ece0 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouptotal_group
