...
const Groupfees_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {fees_text6cb5e, setfees_text6cb5e}= useContext(TotalContext) as TotalContextProps;
  const {feesad465, setfeesad465}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a345cc474f937141267ae2a7600bee4a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfees_groupbee4aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("fees_text")){
        setfees_text6cb5e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_text6cb5e?.isDisabled==null)
      {
        setfees_text6cb5e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees")){
        setfeesad465((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(feesad465?.isDisabled==null)
      {
        setfeesad465((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['fees_text'] = fees_text6cb5e,
        codeStates['setfees_text'] = setfees_text6cb5e,
        codeStates['fees'] = feesad465,
        codeStates['setfees'] = setfeesad465,
  ...
  const fees_groupbee4aRef = useRef<any>(null);
  const handleClearSearch = () => {
    fees_groupbee4aRef.current?.setSearchParams();
    fees_groupbee4aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(fees_groupbee4a) && Object.keys(fees_groupbee4a)?.length>0)
      {
        setfees_groupbee4a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [fees_groupbee4aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewamrpggraph_v1((pre:any)=>({...pre,_selectedGroup_:"fees_group"}))
        }}
    >
          {allowedControls.includes("fees_text") ?<Textfees_text   /* 6cb5e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("fees") ?<Textfees   /* ad465 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
