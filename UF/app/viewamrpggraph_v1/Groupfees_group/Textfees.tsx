...
const Textfees = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {feesad465Props, setfeesad465Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setfees_groupbee4a((pre: any) => ({
          ...pre,
          fees: feesad465Props?.filteredData?.length > 0
            ? feesad465Props?.filteredData[0]?.fees
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !fees_groupbee4a.fees){
        setfees_groupbee4a((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !fees_groupbee4a.fees){
    setfees_groupbee4a((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!feesad465Props?.filterProps) return;
    handleMapperValue(feesad465Props?.filterProps,feesad465Props?.filterFlag);
  },[feesad465Props?.filterProps])

  if (feesad465?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.fees : (fees_groupbee4a?.fees || ""))}
</Text>
  </div>
  )
}
