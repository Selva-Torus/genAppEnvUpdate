...
const DatePickerfrom_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,from_date:undefined}))

  setoffsite_approval_group8d6cc((prev: any) => ({ ...prev, from_date: isoDate }))
}
...
useEffect(()=>{
 ...
 },[from_dated8c1b?.refresh])

  useEffect(()=>{
      handleBlur()
  },[validateRefetch.value])

if (from_dated8c1b?.isHidden) {
  return <></>
}

return (
  <div 
  style={{gridColumn: `2 / 11`,gridRow: `51 / 67`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={offsite_approval_group8d6cc?.from_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      readOnly=  {from_dated8c1b?.isDisabled ? true : false}
      disabled= {from_dated8c1b?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="From Date"
      validationState={validate?.from_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerfrom_date
