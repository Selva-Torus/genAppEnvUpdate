...
const TextInputclaim_category = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claims:AFVK:v1|d4d9550eca7f46129886b6c11168e6b5|items.properties.claim_category"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Approval_Screen_Offsite:AFVK:v1|279c2b7585c947c9abdc712d6a18d6cc|8fd8a11bdb4e401394b4a952db3ac401"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'claim_category',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=``;
     if (code != '') {
      let codeStates: any = {};
      codeStates['offsite_approval_group']  = {...offsite_approval_group8d6cc,claim_category:newInputValue},
      codeStates['setoffsite_approval_group'] = setoffsite_approval_group8d6cc,
      codeStates['claim_detail_table']  = {...claim_detail_table1835f,claim_category:newInputValue},
      codeStates['setclaim_detail_table'] = setclaim_detail_table1835f,
    codeExecution(code,codeStates);
    }  
    setError('');
    setValidate((pre:any)=>({...pre,claim_category:undefined}));
    if(dynamicStateandType.type=="number"){
    setoffsite_approval_group8d6cc((prev: any) => ({ ...prev, claim_category: +e.target.value }));
    }
    else{
    setoffsite_approval_group8d6cc((prev: any) => ({ ...prev, claim_category: e.target.value }));
    }
  ...
  if(dfd_claims_v1Props?.setSearchFilters && dfd_claims_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_v1Props.data) && dfd_claims_v1Props.data.length > 0){
      setoffsite_approval_group8d6cc((pre:any)=>({...pre,claim_category:dfd_claims_v1Props.data[0]?.claim_category}));
    }
  }
  },[dfd_claims_v1Props?.setSearchFilters])
  if (claim_categoryac401?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `2 / 11`,gridRow: `72 / 89`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={offsite_approval_group8d6cc?.claim_category||""}
         disabled= {claim_categoryac401?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerPosition='top'
        headerText="Category"
        validationState={validate?.claim_category ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputclaim_category
