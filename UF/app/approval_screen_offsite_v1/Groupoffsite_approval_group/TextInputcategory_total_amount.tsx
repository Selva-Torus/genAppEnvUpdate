...
const TextInputcategory_total_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Approval_Screen_Offsite:AFVK:v1|279c2b7585c947c9abdc712d6a18d6cc|4fd946aa2893443d95ef2da4060dd7c0"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=``;
     if (code != '') {
      let codeStates: any = {};
      codeStates['offsite_approval_group']  = {...offsite_approval_group8d6cc,category_total_amount:newInputValue},
      codeStates['setoffsite_approval_group'] = setoffsite_approval_group8d6cc,
      codeStates['claim_detail_table']  = {...claim_detail_table1835f,category_total_amount:newInputValue},
    ...
    setoffsite_approval_group8d6cc((prev: any) => ({ ...prev, category_total_amount: +e.target.value }));
    }
    else{
    setoffsite_approval_group8d6cc((prev: any) => ({ ...prev, category_total_amount: e.target.value }));
    }
  ...
  if(dfd_claims_v1Props?.setSearchFilters && dfd_claims_v1Props?.data)
  {
    if(Array.isArray(dfd_claims_v1Props.data) && dfd_claims_v1Props.data.length > 0){
      setoffsite_approval_group8d6cc((pre:any)=>({...pre,category_total_amount:dfd_claims_v1Props.data[0]?.category_total_amount}));
    }
  }
  },[dfd_claims_v1Props?.setSearchFilters])
  if (category_total_amountdd7c0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `12 / 22`,gridRow: `72 / 89`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={offsite_approval_group8d6cc?.category_total_amount||""}
         disabled= {category_total_amountdd7c0?.isDisabled ? true : false}
        pin='brick-brick'     
