...
export default function PageAssetdisposalviewV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetdisposal_v1DFD(pagination:any): Promise<void>{
        ...
        if(assetdisposalview_v1Props.length > 0){
          for(let i=0;i< assetdisposalview_v1Props.length;i++){
            if(assetdisposalview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1"){
              // delete assetdisposalview_v1Props[i].DFDkey;
              let temp=structuredClone(assetdisposalview_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setassetdisposalview_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setassetdisposalview_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(assetdisposalview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(assetdisposalview_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
