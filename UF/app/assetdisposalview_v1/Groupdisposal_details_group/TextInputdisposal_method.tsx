...
const TextInputdisposal_method = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.disposal_method"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|b3ad56a3082a709039df9dc7d32aa369|51daf51dfae24c5692414bac0daa093b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'disposal_method',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,disposal_method:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, disposal_method: +e.target.value }));
    }
    else{
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, disposal_method: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "51daf51dfae24c5692414bac0daa093b"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1",
      //     componentId: "b3ad56a3082a709039df9dc7d32aa369",
      //     controlId: "51daf51dfae24c5692414bac0daa093b",
      //     isTable: false,
      //     from:"TextInputdisposal_method",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'disposal_method', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'disposal_method',type:'text'};
      //   type={
      //     name:'disposal_method',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_method.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_method.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_method.type
      //   }
      ...
      //   type={
      //     name:'disposal_method',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_method.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_method.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_method.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const disposal_details_groupaa369Ref = useRef<any>(disposal_details_groupaa369);
  useEffect(() => { disposal_details_groupaa369Ref.current = disposal_details_groupaa369; }, [disposal_details_groupaa369]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleBlur();
    const handlerChange = (id:any) => {
      if (id === "51daf51dfae24c5692414bac0daa093b") {
        handleChange({target:{value:disposal_details_groupaa369Ref?.current?.disposal_method||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "51daf51dfae24c5692414bac0daa093b") {
        handleBlur({target:{value:disposal_details_groupaa369Ref?.current?.disposal_method||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setdisposal_details_groupaa369((pre:any)=>({...pre,disposal_method:dfd_assetdisposal_v1Props.data[0]?.disposal_method}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (disposal_methoda093b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={disposal_details_groupaa369?.disposal_method||""}
         disabled= {disposal_methoda093b?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Disposal Method"
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.disposal_method ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdisposal_method
