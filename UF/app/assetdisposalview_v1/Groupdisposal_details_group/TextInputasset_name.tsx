...
const TextInputasset_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.asset_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|b3ad56a3082a709039df9dc7d32aa369|e9ee90d13ac343b98303a041655298df"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,asset_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, asset_name: +e.target.value }));
    }
    else{
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, asset_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e9ee90d13ac343b98303a041655298df"
      );
      ...
      //     controlId: "e9ee90d13ac343b98303a041655298df",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e9ee90d13ac343b98303a041655298df") {
        handleChange({target:{value:disposal_details_groupaa369Ref?.current?.asset_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e9ee90d13ac343b98303a041655298df") {
        handleBlur({target:{value:disposal_details_groupaa369Ref?.current?.asset_name||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setdisposal_details_groupaa369((pre:any)=>({...pre,asset_name:dfd_assetdisposal_v1Props.data[0]?.asset_name}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (asset_name298df?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={disposal_details_groupaa369?.asset_name||""}
         disabled= {asset_name298df?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.asset_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
