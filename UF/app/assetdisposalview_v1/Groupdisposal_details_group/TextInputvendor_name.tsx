...
const TextInputvendor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.vendor_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|b3ad56a3082a709039df9dc7d32aa369|02d6091fb9fd455994997ab7bf75f557"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,vendor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, vendor_name: +e.target.value }));
    }
    else{
    setdisposal_details_groupaa369((prev: any) => ({ ...prev, vendor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "02d6091fb9fd455994997ab7bf75f557"
      );
      ...
      //     controlId: "02d6091fb9fd455994997ab7bf75f557",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "02d6091fb9fd455994997ab7bf75f557") {
        handleChange({target:{value:disposal_details_groupaa369Ref?.current?.vendor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "02d6091fb9fd455994997ab7bf75f557") {
        handleBlur({target:{value:disposal_details_groupaa369Ref?.current?.vendor_name||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setdisposal_details_groupaa369((pre:any)=>({...pre,vendor_name:dfd_assetdisposal_v1Props.data[0]?.vendor_name}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (vendor_name5f557?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={disposal_details_groupaa369?.vendor_name||""}
         disabled= {vendor_name5f557?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.vendor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
