...
const TextInputdata_wiped = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.data_wiped"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|2f1878a0c47c273ea728efb4f07e5dd8|a5fe17bbf0d94d079d8ab392668a4257"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'data_wiped',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,data_wiped:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, data_wiped: +e.target.value }));
    }
    else{
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, data_wiped: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a5fe17bbf0d94d079d8ab392668a4257"
      );
      ...
      //     controlId: "a5fe17bbf0d94d079d8ab392668a4257",
      //     isTable: false,
      //     from:"TextInputdata_wiped",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'data_wiped', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'data_wiped',type:'text'};
      //   type={
      //     name:'data_wiped',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wiped.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wiped.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wiped.type
      //   }
      ...
      //   type={
      //     name:'data_wiped',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wiped.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wiped.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wiped.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a5fe17bbf0d94d079d8ab392668a4257") {
        handleChange({target:{value:compliance_financial_groupe5dd8Ref?.current?.data_wiped||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a5fe17bbf0d94d079d8ab392668a4257") {
        handleBlur({target:{value:compliance_financial_groupe5dd8Ref?.current?.data_wiped||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_groupe5dd8((pre:any)=>({...pre,data_wiped:dfd_assetdisposal_v1Props.data[0]?.data_wiped}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (data_wipeda4257?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_groupe5dd8?.data_wiped||""}
         disabled= {data_wipeda4257?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.data_wiped ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdata_wiped
