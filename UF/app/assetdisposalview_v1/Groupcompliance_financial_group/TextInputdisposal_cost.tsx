...
const TextInputdisposal_cost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|2f1878a0c47c273ea728efb4f07e5dd8|591fa8039bea62484e0a14e7be123f44"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,disposal_cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, disposal_cost: +e.target.value }));
    }
    else{
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, disposal_cost: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "591fa8039bea62484e0a14e7be123f44"
      );
      ...
      //     controlId: "591fa8039bea62484e0a14e7be123f44",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "591fa8039bea62484e0a14e7be123f44") {
        handleChange({target:{value:compliance_financial_groupe5dd8Ref?.current?.disposal_cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "591fa8039bea62484e0a14e7be123f44") {
        handleBlur({target:{value:compliance_financial_groupe5dd8Ref?.current?.disposal_cost||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_groupe5dd8((pre:any)=>({...pre,disposal_cost:dfd_assetdisposal_v1Props.data[0]?.disposal_cost}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (disposal_cost23f44?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_groupe5dd8?.disposal_cost||""}
         disabled= {disposal_cost23f44?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.disposal_cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
