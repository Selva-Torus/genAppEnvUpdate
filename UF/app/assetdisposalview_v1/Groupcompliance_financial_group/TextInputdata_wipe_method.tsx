...
const TextInputdata_wipe_method = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1|1d36f55182b042ad946873239ea59ff0|properties.data_wipe_method"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|2f1878a0c47c273ea728efb4f07e5dd8|39295bc71f26468598c83cb7dc78923d"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'data_wipe_method',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,data_wipe_method:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, data_wipe_method: +e.target.value }));
    }
    else{
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, data_wipe_method: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "39295bc71f26468598c83cb7dc78923d"
      );
      ...
      //     controlId: "39295bc71f26468598c83cb7dc78923d",
      //     isTable: false,
      //     from:"TextInputdata_wipe_method",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'data_wipe_method', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'data_wipe_method',type:'text'};
      //   type={
      //     name:'data_wipe_method',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wipe_method.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wipe_method.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.data_wipe_method.type
      //   }
      ...
      //   type={
      //     name:'data_wipe_method',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wipe_method.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wipe_method.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.data_wipe_method.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "39295bc71f26468598c83cb7dc78923d") {
        handleChange({target:{value:compliance_financial_groupe5dd8Ref?.current?.data_wipe_method||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "39295bc71f26468598c83cb7dc78923d") {
        handleBlur({target:{value:compliance_financial_groupe5dd8Ref?.current?.data_wipe_method||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_groupe5dd8((pre:any)=>({...pre,data_wipe_method:dfd_assetdisposal_v1Props.data[0]?.data_wipe_method}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (data_wipe_method8923d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_groupe5dd8?.data_wipe_method||""}
         disabled= {data_wipe_method8923d?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Data Wipe Method"
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.data_wipe_method ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdata_wipe_method
