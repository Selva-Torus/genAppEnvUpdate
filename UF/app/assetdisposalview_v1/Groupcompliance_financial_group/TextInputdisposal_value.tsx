...
const TextInputdisposal_value = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|2f1878a0c47c273ea728efb4f07e5dd8|41335fe6d052a348ba2bad8e2a513578"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,disposal_value:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, disposal_value: +e.target.value }));
    }
    else{
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, disposal_value: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "41335fe6d052a348ba2bad8e2a513578"
      );
      ...
      //     controlId: "41335fe6d052a348ba2bad8e2a513578",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "41335fe6d052a348ba2bad8e2a513578") {
        handleChange({target:{value:compliance_financial_groupe5dd8Ref?.current?.disposal_value||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "41335fe6d052a348ba2bad8e2a513578") {
        handleBlur({target:{value:compliance_financial_groupe5dd8Ref?.current?.disposal_value||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_groupe5dd8((pre:any)=>({...pre,disposal_value:dfd_assetdisposal_v1Props.data[0]?.disposal_value}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (disposal_value13578?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_groupe5dd8?.disposal_value||""}
         disabled= {disposal_value13578?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.disposal_value ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
