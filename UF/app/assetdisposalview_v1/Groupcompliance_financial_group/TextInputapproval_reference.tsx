...
const TextInputapproval_reference = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposalView:AFVK:v1|2f1878a0c47c273ea728efb4f07e5dd8|c443496217b24942ed0739e4fdeb0a46"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,assetDisposalView_v1:{...pre?.assetDisposalView_v1,approval_reference:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, approval_reference: +e.target.value }));
    }
    else{
    setcompliance_financial_groupe5dd8((prev: any) => ({ ...prev, approval_reference: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c443496217b24942ed0739e4fdeb0a46"
      );
      ...
      //     controlId: "c443496217b24942ed0739e4fdeb0a46",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c443496217b24942ed0739e4fdeb0a46") {
        handleChange({target:{value:compliance_financial_groupe5dd8Ref?.current?.approval_reference||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c443496217b24942ed0739e4fdeb0a46") {
        handleBlur({target:{value:compliance_financial_groupe5dd8Ref?.current?.approval_reference||""}});
      }
    };
  ...
  if(dfd_assetdisposal_v1Props?.setSearchFilters && dfd_assetdisposal_v1Props?.data)
  {
    if(Array.isArray(dfd_assetdisposal_v1Props.data) && dfd_assetdisposal_v1Props.data.length > 0){
      setcompliance_financial_groupe5dd8((pre:any)=>({...pre,approval_reference:dfd_assetdisposal_v1Props.data[0]?.approval_reference}));
    }
  }
  },[dfd_assetdisposal_v1Props?.setSearchFilters])
  if (approval_referenceb0a46?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={compliance_financial_groupe5dd8?.approval_reference||""}
         disabled= {approval_referenceb0a46?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.assetDisposalView_v1?.approval_reference ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
