...
export default function PageEmployeesV1() {
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setemployees_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setemployees_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(employees_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(employees_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setemployees_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
