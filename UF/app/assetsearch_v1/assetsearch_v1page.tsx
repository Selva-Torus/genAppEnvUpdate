...
export default function PageAssetsearchV1() {
  ...
  const {assetsearch_v1, setassetsearch_v1} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assets_v1DFD(pagination:any): Promise<void>{
        ...
        if(assetsearch_v1Props.length > 0){
          for(let i=0;i< assetsearch_v1Props.length;i++){
            if(assetsearch_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1"){
              // delete assetsearch_v1Props[i].DFDkey;
              let temp=structuredClone(assetsearch_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setassetsearch_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const { fetchBatchData } = await import("../utils/Orchestration");
    const data: any = await fetchBatchData(
      'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetSearch:AFVK:v1',
      [user],
      'pageAssetsearchV1',
      token
    )
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetSearch:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setassetsearch_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(assetsearch_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(assetsearch_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
