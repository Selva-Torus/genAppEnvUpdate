...
export default function PageMakerV1() {
  ...
  const {maker_v1Props, setmaker_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function userdfd_v1DFD(pagination:any): Promise<void>{
        let filterData :any[] =[];
        let userdfd_v1Body:te_refreshDto={
          key: "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TGW01:AFGK:TGW004:AFK:userdfd:AFVK:v1"+":",
          refreshFlag: "Y",
        ...
        if(maker_v1Props.length > 0){
          for(let i=0;i< maker_v1Props.length;i++){
            if(maker_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TGW01:AFGK:TGW004:AFK:userdfd:AFVK:v1"){
              // delete maker_v1Props[i].DFDkey;
              let temp=structuredClone(maker_v1Props[i])
              delete temp?.DFDkey
        ...
        let dstKey:string=userdfd_v1Body?.key || ""
        dstKey=dstKey.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
        if(userdfd_v1Data?.data?.dataset === 'Bulk Data Processing'){
        ...
        setdfd_userdfd_v1Props(api_paginationData?.data?.records || []);
      }else{
          setdfd_userdfd_v1Props({ hasLogicCenter: false, dstKey: dstKey })
      }
      }else if (userdfd_v1Data?.data?.dataset) {
           setdfd_userdfd_v1Props(
              Array.isArray(userdfd_v1Data?.data?.dataset?.data)
                 ? userdfd_v1Data?.data.dataset.data.map((obj: any) =>
                  Object.fromEntries(
  ...
  useEffect(()=>{
    if (prevRefreshRef?.current?.userdfd_v1) {
      userdfd_v1DFD(paginationData)
    }else 
      prevRefreshRef.current.userdfd_v1= true
  },[refetch?.userdfd_v1])
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    const { getAftfactLevelRule } = await import("../utils/evaluateDecisionTable");
    let result :any =await getAftfactLevelRule(rule,data,allRuleData)
    setmaker_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      ...
      try {
        ...
        try{
    await userdfd_v1DFD(pagination)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setmaker_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(maker_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(maker_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setmaker_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
