...
const TextInputcountry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,maker_v1:{...pre?.maker_v1,country:undefined}}));
    if(dynamicStateandType.type=="number"){
    setgroupe0568((prev: any) => ({ ...prev, country: +e.target.value }));
    }
    else{
    setgroupe0568((prev: any) => ({ ...prev, country: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5643d3315c9042c1b0e62c12bf73b817"
      );
      ...
      //     controlId: "5643d3315c9042c1b0e62c12bf73b817",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5643d3315c9042c1b0e62c12bf73b817") {
        handleChange({target:{value:groupe0568Ref?.current?.country||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5643d3315c9042c1b0e62c12bf73b817") {
        handleBlur({target:{value:groupe0568Ref?.current?.country||""}});
      }
    };
  ...
  if (country3b817?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `10 / 16`,gridRow: `12 / 29`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
        {isRequredData && <span style={{ color: 'red' }}>*</span>}
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className=""
        label={keyset("country")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={groupe0568?.country||""}
         disabled= {country3b817?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.maker_v1?.country ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
