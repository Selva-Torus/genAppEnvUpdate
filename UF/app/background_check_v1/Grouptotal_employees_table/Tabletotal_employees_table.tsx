...
let defaultColumns:any = [
  {
    "id": "check_id",
    "nodeid": "460b0720818bd3d4011aab9e130c0499",
    "name": "Check No.",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "full_name",
    "nodeid": "bec2bd98141e792d1bb81bb4a8d8459f",
    "name": "Employee Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "check_type",
    "nodeid": "b55cc8b596e9d1ce4497c34e7b667f55",
    "name": "Check Type",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "initiated_date",
    "nodeid": "bf60cb9222ab863a2eef3e5d9f5d326f",
    "name": "Initiated Date",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "initiated_date"
  },
  {
    "id": "completed_date",
    "nodeid": "c88b036fffe94bab57d5ff69b59b46ea",
    "name": "Completed Date",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "completed_date"
  },
  {
    "id": "result",
    "nodeid": "27981cfada7c81d75b9b37c19f9428b3",
    "name": "Result",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "816a8788d5dfb59107d4fb3266434423",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tabletotal_employees_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {backgroundcheck_v1Props, setbackgroundcheck_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    /// settotal_employees_table379daProps
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settotal_employees_table379daProps((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      total_employees_table379da.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settotal_employees_table379daProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settotal_employees_table379daProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settotal_employees_table379daProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(backgroundcheck_v1Props.length > 0){
      for(let i=0;i< backgroundcheck_v1Props.length;i++){
        if(backgroundcheck_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:backgroundCheck:AFVK:v1"){
          // delete backgroundcheck_v1Props[i].DFDkey;
          let temp=structuredClone(backgroundcheck_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:backgroundCheck:AFVK:v1:UO', 
                nodeId: '33564afe78fbb2cdfc338e23720379da',
                elementId: '33564afe78fbb2cdfc338e23720379da'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( total_employees_table379daProps?.presetValues&&Object.keys(total_employees_table379daProps?.presetValues).length > 0) {
          filtertedData = [total_employees_table379daProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settotal_employees_table379da(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={backgroundcheck_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (total_employees_table379daProps.filterInitalLoad) return;

    const filterControllers = total_employees_table379daProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['total_employees_table379da']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['total_employees_table379da']).filter((k: string) => jsonEntry['total_employees_table379da'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(total_employees_table379daProps.filterProps);
      return;
    }
    ...
    settotal_employees_table379daProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(total_employees_table379daProps.filterProps);

  }, [
    total_employees_table379daProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    total_employees_table379daProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(total_employees_table379daProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(total_employees_table379daProps.searchFilter) || {};
...
}, [total_employees_table379daProps.searchFilter])
  ...
  useEffect(() => {
    if (total_employees_table379daProps?.clearData === true) {
      settotal_employees_table379da([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settotal_employees_table379daProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [total_employees_table379daProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [total_employees_table379daProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = total_employees_table379daProps?.selectedIds[total_employees_table379daProps?.selectedIds?.length-1]
    if(Array.isArray(total_employees_table379da) && total_employees_table379da.length>0)
    {
      let data = total_employees_table379da[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (total_employees_table379daProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (total_employees_table379daProps?.selectedIds?.length == 0){
      handleOnRowClick({},total_employees_table379daProps?.selectedIds)
    }
  }, [total_employees_table379daProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(total_employees_table379da?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&total_employees_table379da?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](total_employees_table379da?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table379da?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&total_employees_table379da?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](total_employees_table379da?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: total_employees_table379da?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (total_employees_table379da?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"33564afe78fbb2cdfc338e23720379da"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableActions={true}
              tableSorting={true}
              selectedIds={total_employees_table379daProps?.selectedIds}  
