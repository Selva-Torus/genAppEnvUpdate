...
const Groupaccess_req_group = dynamic(() => import("./Groupaccess_req_group/Groupaccess_req_group"), { ssr: false });


export default function PageAccessRequestV1() {
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setaccessrequest_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaccessrequest_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(accessrequest_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(accessrequest_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setaccessrequest_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkaccess_req_group && initialLoad &&<Groupaccess_req_group
