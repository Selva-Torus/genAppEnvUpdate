...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "064302e165aea74442d8cbc07385eb1e",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "61d16dbd7ed2fe66c1e681b6da783956",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "7e774b15caa26a7b75d91c9d13f5febb",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "525833290cd6f38179302ef9bbc09f2c",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "6a24e4d599051d4d544d9ff92483e3a0",
    "name": "Created By",
    "meta": {
      "sort": true
    },
...
const Tableemployee_doc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const unlockRecord = async (id: number) => {
    ...
    /// setemployee_doc_tableb42f3Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(employee_doc_tableb42f3.map((item: any) => item.asset_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      employee_doc_tableb42f3.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = employee_doc_tableb42f3
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.asset_id) ? index.toString() : null
          )
          .filter(Boolean)

        setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        return
      }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setemployee_doc_tableb42f3Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["asset_id"]:employee_doc_tableb42f3Props?.presetValues?.asset_id}
    ...
    if(!employee_doc_tableb42f3Props?.presetValues || Object.keys(employee_doc_tableb42f3Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeesDocument:AFVK:v1:UO', 
                nodeId: 'f56c544fe9d33cadb4368d9ad2bb42f3',
                elementId: 'f56c544fe9d33cadb4368d9ad2bb42f3'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setemployee_doc_tableb42f3(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in employeesdocument_v1?.employee_doc_table && employeesdocument_v1?.employee_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(employeesdocument_v1?.employee_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(employeesdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,employee_doc_table:filteredData},{employee_doc_table:employeesdocument_v1?.employee_doc_table})
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(employee_doc_tableb42f3))
    {
      GetTableDetails()
    }
  }, [employee_doc_tableb42f3])
  ...
  useEffect(() => {
    ...
    if (employee_doc_tableb42f3Props.filterInitalLoad) return;

    const filterControllers = employee_doc_tableb42f3Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['employee_doc_tableb42f3']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['employee_doc_tableb42f3']).filter((k: string) => jsonEntry['employee_doc_tableb42f3'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(employee_doc_tableb42f3Props.filterProps);
      return;
    }
    ...
    setemployee_doc_tableb42f3Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(employee_doc_tableb42f3Props.filterProps);

  }, [
    employee_doc_tableb42f3Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    employee_doc_tableb42f3Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(employee_doc_tableb42f3Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(employee_doc_tableb42f3Props.searchFilter) || {};
...
}, [employee_doc_tableb42f3Props.searchFilter])
  ...
  useEffect(() => {
    if (employee_doc_tableb42f3Props?.clearData === true) {
      if (lastLockedDataRef.current?.primaryKeys?.length) {
        lastLockedDataRef.current.primaryKeys.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setemployee_doc_tableb42f3Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [employee_doc_tableb42f3Props?.clearData])
  useEffect(() => {
  ...
  }, [employee_doc_tableb42f3Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = employee_doc_tableb42f3Props?.selectedIds[employee_doc_tableb42f3Props?.selectedIds?.length-1]
    if(Array.isArray(employee_doc_tableb42f3) && employee_doc_tableb42f3.length>0)
    {
      let data = employee_doc_tableb42f3[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "asset_id",
        value: data["asset_id"],
  ...
  useEffect(() => {
    if (employee_doc_tableb42f3Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (employee_doc_tableb42f3Props?.selectedIds?.length == 0){
      handleOnRowClick({},employee_doc_tableb42f3Props?.selectedIds)
    }
  }, [employee_doc_tableb42f3Props?.selectedIds])
    ...
    let filterProps4:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData4 = await getFilterProps(filterProps4,{...doc_attached_groupac2a0,...table_group034b1,...employee_doc_tableb42f3});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(employee_doc_tableb42f3?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&employee_doc_tableb42f3?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](employee_doc_tableb42f3?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: employee_doc_tableb42f3?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&employee_doc_tableb42f3?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](employee_doc_tableb42f3?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: employee_doc_tableb42f3?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (employee_doc_tableb42f3?.isHidden) {
    return <></>
  }
  return(
      ...
      >
        <PageHrmdocumentviewerpage4/>
          ...
          <div
            ...
            id={"f56c544fe9d33cadb4368d9ad2bb42f3"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={employee_doc_tableb42f3Props?.selectedIds}  
