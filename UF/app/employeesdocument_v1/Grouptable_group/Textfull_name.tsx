...
const Textfull_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_employeedoctable_v1Props) && dfd_employeedoctable_v1Props && !table_group034b1.full_name){
        settable_group034b1((pre:any)=>({...pre,full_name:dfd_employeedoctable_v1Props[0]?.full_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[full_namede77c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_employeedoctable_v1Props) && !table_group034b1.full_name){
    settable_group034b1((pre:any)=>({...pre,full_name:dfd_employeedoctable_v1Props[0]?.full_name}));
  }
  },[dfd_employeedoctable_v1Props])

  if (full_namede77c?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.full_name : (table_group034b1?.full_name || ""))}
</Text>
  </div>
  )
}
