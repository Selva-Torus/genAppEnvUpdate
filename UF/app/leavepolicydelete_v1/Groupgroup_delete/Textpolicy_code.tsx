...
const Textpolicy_code = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_leavepolicy_v1Props && !dfd_leavepolicy_v1Props.hasLogicCenter) {
        ...
        setgroup_delete40e71((pre: any) => {
          return { ...pre, policycode: api_paginationData.data.records[0]?.policycode }
        })
        }
      }
      else{
      if(Array.isArray(dfd_leavepolicy_v1Props) && dfd_leavepolicy_v1Props && !group_delete40e71.policycode){
        setgroup_delete40e71((pre:any)=>({...pre,policycode:dfd_leavepolicy_v1Props[0]?.policycode}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[policy_codeea843?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicy_v1Props) && !group_delete40e71.policycode){
    setgroup_delete40e71((pre:any)=>({...pre,policycode:dfd_leavepolicy_v1Props[0]?.policycode}));
  }
  },[dfd_leavepolicy_v1Props])

  if (policy_codeea843?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.policycode : (group_delete40e71?.policycode || ""))}
</Text>
  </div>
  )
}

export default Textpolicy_code
