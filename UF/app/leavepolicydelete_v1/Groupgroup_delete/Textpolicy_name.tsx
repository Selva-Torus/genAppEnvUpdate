...
const Textpolicy_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_leavepolicy_v1Props && !dfd_leavepolicy_v1Props.hasLogicCenter) {
        ...
        setgroup_delete40e71((pre: any) => {
          return { ...pre, policyname: api_paginationData.data.records[0]?.policyname }
        })
        }
      }
      else{
      if(Array.isArray(dfd_leavepolicy_v1Props) && dfd_leavepolicy_v1Props && !group_delete40e71.policyname){
        setgroup_delete40e71((pre:any)=>({...pre,policyname:dfd_leavepolicy_v1Props[0]?.policyname}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[policy_name3b3f3?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavepolicy_v1Props) && !group_delete40e71.policyname){
    setgroup_delete40e71((pre:any)=>({...pre,policyname:dfd_leavepolicy_v1Props[0]?.policyname}));
  }
  },[dfd_leavepolicy_v1Props])

  if (policy_name3b3f3?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.policyname : (group_delete40e71?.policyname || ""))}
</Text>
  </div>
  )
}
