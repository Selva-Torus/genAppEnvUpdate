...
const Grouptran_journey_group = dynamic(() => import("./Grouptran_journey_group/Grouptran_journey_group"), { ssr: false });

export default function PageTransactionjourneyV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:transactionJourney:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    settransactionjourney_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])
