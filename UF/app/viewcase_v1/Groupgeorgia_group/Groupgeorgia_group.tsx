...
import Iconwarning_icon  from "./Iconwarning_icon";
import Textgeorgia_sol_text  from "./Textgeorgia_sol_text";
...
const Groupgeorgia_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warning_icond1cd0, setwarning_icond1cd0}= useContext(TotalContext) as TotalContextProps;
  const {georgia_sol_text460ef, setgeorgia_sol_text460ef}= useContext(TotalContext) as TotalContextProps;
  const {special_rulesd6c6e, setspecial_rulesd6c6e}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bcf77c6298e50dd3730ad07b41da636c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgia_groupa636cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warning_icon")){
        setwarning_icond1cd0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warning_icond1cd0?.isDisabled==null)
      {
        setwarning_icond1cd0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgia_sol_text")){
        setgeorgia_sol_text460ef((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgia_sol_text460ef?.isDisabled==null)
      {
        setgeorgia_sol_text460ef((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rulesd6c6e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rulesd6c6e?.isDisabled==null)
      {
        setspecial_rulesd6c6e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warning_icon'] = warning_icond1cd0,
        codeStates['setwarning_icon'] = setwarning_icond1cd0,
        codeStates['georgia_sol_text'] = georgia_sol_text460ef,
        codeStates['setgeorgia_sol_text'] = setgeorgia_sol_text460ef,
        codeStates['special_rules'] = special_rulesd6c6e,
        codeStates['setspecial_rules'] = setspecial_rulesd6c6e,
  ...
  const georgia_groupa636cRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgia_groupa636cRef.current?.setSearchParams();
    georgia_groupa636cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgia_groupa636c) && Object.keys(georgia_groupa636c)?.length>0)
      {
        setgeorgia_groupa636c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgia_groupa636cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgia_group"}))
        }}
    >
        {allowedControls.includes("warning_icon")?<Iconwarning_icon /* d1cd0 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgia_sol_text") ?<Textgeorgia_sol_text   /* 460ef */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("special_rules") ?<Textspecial_rules   /* d6c6e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgeorgia_group
