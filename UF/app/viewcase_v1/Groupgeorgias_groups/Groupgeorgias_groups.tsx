...
import Iconwarningss_icons  from "./Iconwarningss_icons";
import Textgeorgias_sosl_texts  from "./Textgeorgias_sosl_texts";
import Textspecials_ruless  from "./Textspecials_ruless";
...
const Groupgeorgias_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warningss_icons476c3, setwarningss_icons476c3}= useContext(TotalContext) as TotalContextProps;
  const {georgias_sosl_texts3f264, setgeorgias_sosl_texts3f264}= useContext(TotalContext) as TotalContextProps;
  const {specials_rulesscc30f, setspecials_rulesscc30f}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e9b7439cf7907cafdc34bc831cdbf356");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgias_groupsbf356Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningss_icons")){
        setwarningss_icons476c3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningss_icons476c3?.isDisabled==null)
      {
        setwarningss_icons476c3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_sosl_texts")){
        setgeorgias_sosl_texts3f264((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_sosl_texts3f264?.isDisabled==null)
      {
        setgeorgias_sosl_texts3f264((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("specials_ruless")){
        setspecials_rulesscc30f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(specials_rulesscc30f?.isDisabled==null)
      {
        setspecials_rulesscc30f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningss_icons'] = warningss_icons476c3,
        codeStates['setwarningss_icons'] = setwarningss_icons476c3,
        codeStates['georgias_sosl_texts'] = georgias_sosl_texts3f264,
        codeStates['setgeorgias_sosl_texts'] = setgeorgias_sosl_texts3f264,
        codeStates['specials_ruless'] = specials_rulesscc30f,
        codeStates['setspecials_ruless'] = setspecials_rulesscc30f,
  ...
  const georgias_groupsbf356Ref = useRef<any>(null);
  const handleClearSearch = () => {
    georgias_groupsbf356Ref.current?.setSearchParams();
    georgias_groupsbf356Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgias_groupsbf356) && Object.keys(georgias_groupsbf356)?.length>0)
      {
        setgeorgias_groupsbf356({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgias_groupsbf356Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgias_groups"}))
        }}
    >
        {allowedControls.includes("warningss_icons")?<Iconwarningss_icons /* 476c3 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgias_sosl_texts") ?<Textgeorgias_sosl_texts   /* 3f264 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("specials_ruless") ?<Textspecials_ruless   /* cc30f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgeorgias_groups
