...
import Dividervenue_special_rules_dividersss  from "./Dividervenue_special_rules_dividersss";
...
const Groupvenue_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ec222c2350517802d6fc59b9deaa72d9");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvenue_groupa72d9Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rules_icon")){
        setrules_iconfb03f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(rules_iconfb03f?.isDisabled==null)
      {
        setrules_iconfb03f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_special_rules_text")){
        setvenue_special_rules_textecd92((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_special_rules_textecd92?.isDisabled==null)
      {
        setvenue_special_rules_textecd92((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules2001a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules2001a?.isDisabled==null)
      {
        setspecial_rules2001a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_special_rules_divider")){
        setvenue_special_rules_divider206ab((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_special_rules_divider206ab?.isDisabled==null)
      {
        setvenue_special_rules_divider206ab((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgia_group")){
        setgeorgia_groupa636c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgia_groupa636c?.isDisabled==null)
      {
        setgeorgia_groupa636c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_special_rules_dividers")){
        setvenue_special_rules_dividers28a68((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_special_rules_dividers28a68?.isDisabled==null)
      {
        setvenue_special_rules_dividers28a68((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_group")){
        setgeorgias_groupbac01((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_groupbac01?.isDisabled==null)
      {
        setgeorgias_groupbac01((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_special_rules_dividerss")){
        setvenue_special_rules_dividerss0c16e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_special_rules_dividerss0c16e?.isDisabled==null)
      {
        setvenue_special_rules_dividerss0c16e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_groups")){
        setgeorgias_groupsbf356((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_groupsbf356?.isDisabled==null)
      {
        setgeorgias_groupsbf356((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_special_rules_dividersss")){
        setvenue_special_rules_dividersssdcaab((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_special_rules_dividersssdcaab?.isDisabled==null)
      {
        setvenue_special_rules_dividersssdcaab((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgiass_groups")){
        setgeorgiass_groups9e4dd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgiass_groups9e4dd?.isDisabled==null)
      {
        setgeorgiass_groups9e4dd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venues_special_rules_dividerssss")){
        setvenues_special_rules_dividerssss44419((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venues_special_rules_dividerssss44419?.isDisabled==null)
      {
        setvenues_special_rules_dividerssss44419((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgsiass_groups")){
        setgeorgsiass_groups6bf7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgsiass_groups6bf7a?.isDisabled==null)
      {
        setgeorgsiass_groups6bf7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['rules_icon'] = rules_iconfb03f,
        codeStates['setrules_icon'] = setrules_iconfb03f,
        codeStates['venue_special_rules_text'] = venue_special_rules_textecd92,
        codeStates['setvenue_special_rules_text'] = setvenue_special_rules_textecd92,
        codeStates['special_rules'] = special_rules2001a,
        codeStates['setspecial_rules'] = setspecial_rules2001a,
        codeStates['venue_special_rules_divider'] = venue_special_rules_divider206ab,
        codeStates['setvenue_special_rules_divider'] = setvenue_special_rules_divider206ab,
        ...
        codeStates['venue_special_rules_dividers'] = venue_special_rules_dividers28a68,
        codeStates['setvenue_special_rules_dividers'] = setvenue_special_rules_dividers28a68,
        ...
        codeStates['venue_special_rules_dividerss'] = venue_special_rules_dividerss0c16e,
        codeStates['setvenue_special_rules_dividerss'] = setvenue_special_rules_dividerss0c16e,
        ...
        codeStates['venue_special_rules_dividersss'] = venue_special_rules_dividersssdcaab,
        codeStates['setvenue_special_rules_dividersss'] = setvenue_special_rules_dividersssdcaab,
        ...
        codeStates['venues_special_rules_dividerssss'] = venues_special_rules_dividerssss44419,
        codeStates['setvenues_special_rules_dividerssss'] = setvenues_special_rules_dividerssss44419,
  ...
  const venue_groupa72d9Ref = useRef<any>(null);
  const handleClearSearch = () => {
    venue_groupa72d9Ref.current?.setSearchParams();
    venue_groupa72d9Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(venue_groupa72d9) && Object.keys(venue_groupa72d9)?.length>0)
      {
        setvenue_groupa72d9({})
      }
    }else 
      prevRefreshRef.current= true
  }, [venue_groupa72d9Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-3 !rounded-2xl className=&#34;group border border-[#daad2f] rounded-xl p-6 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"venue_group"}))
        }}
    >
        ...
        {allowedComponent.includes("georgsiass_groups")  &&<Groupgeorgsiass_groups  
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedControls.includes("rules_icon")?<Iconrules_icon /* fb03f */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("venue_special_rules_text") ?<Textvenue_special_rules_text   /* ecd92 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("special_rules") ?<Textspecial_rules   /* 2001a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("venue_special_rules_divider") ?<Dividervenue_special_rules_divider   /* 206ab */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("venue_special_rules_dividers") ?<Dividervenue_special_rules_dividers   /* 28a68 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("venue_special_rules_dividerss") ?<Dividervenue_special_rules_dividerss   /* 0c16e */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("venue_special_rules_dividersss") ?<Dividervenue_special_rules_dividersss   /* dcaab */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("venues_special_rules_dividerssss") ?<Dividervenues_special_rules_dividerssss   /* 44419 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
