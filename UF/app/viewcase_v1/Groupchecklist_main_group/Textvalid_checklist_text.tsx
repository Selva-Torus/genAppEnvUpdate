...
const Textvalid_checklist_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {valid_checklist_text1428aProps, setvalid_checklist_text1428aProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[valid_checklist_text1428a?.refresh])

  if (valid_checklist_text1428a?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("AMR Validation Checklist")}
</Text>
  </div>
  )
}

export default Textvalid_checklist_text
