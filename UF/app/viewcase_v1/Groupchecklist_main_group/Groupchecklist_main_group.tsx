...
const Groupchecklist_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {valid_checklist_text1428a, setvalid_checklist_text1428a}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f97c6f41348675ec5d2736013b95b62e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_main_group5b62eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valid_checklist_text")){
        setvalid_checklist_text1428a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_checklist_text1428a?.isDisabled==null)
      {
        setvalid_checklist_text1428a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_table")){
        setchecklist_table45abc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_table45abc?.isDisabled==null)
      {
        setchecklist_table45abc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['valid_checklist_text'] = valid_checklist_text1428a,
        codeStates['setvalid_checklist_text'] = setvalid_checklist_text1428a,
  ...
  const checklist_main_group5b62eRef = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_main_group5b62eRef.current?.setSearchParams();
    checklist_main_group5b62eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_main_group5b62e) && Object.keys(checklist_main_group5b62e)?.length>0)
      {
        setchecklist_main_group5b62e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_main_group5b62eProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_main_group"}))
        }}
    >
