...
const Groupadd_case_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "f9a0177882553386d93285e61911f6e4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadd_case_group1f6e4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("header_group")){
        setheader_group3749a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(header_group3749a?.isDisabled==null)
      {
        setheader_group3749a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_id")){
        setvenue_idcb444((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_idcb444?.isDisabled==null)
      {
        setvenue_idcb444((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("creditor_id")){
        setcreditor_idb1867((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(creditor_idb1867?.isDisabled==null)
      {
        setcreditor_idb1867((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_information_group")){
        setcase_information_groupcec29((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_information_groupcec29?.isDisabled==null)
      {
        setcase_information_groupcec29((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_group")){
        setvenue_groupa72d9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_groupa72d9?.isDisabled==null)
      {
        setvenue_groupa72d9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_information_group")){
        setdebtor_information_groupdfa55((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debtor_information_groupdfa55?.isDisabled==null)
      {
        setdebtor_information_groupdfa55((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("financial_details_group")){
        setfinancial_details_grouped0d9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(financial_details_grouped0d9?.isDisabled==null)
      {
        setfinancial_details_grouped0d9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_details_group")){
        setvenue_details_group6a27a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_details_group6a27a?.isDisabled==null)
      {
        setvenue_details_group6a27a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("required_dociument_main_group")){
        setrequired_dociument_main_group3eb5b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(required_dociument_main_group3eb5b?.isDisabled==null)
      {
        setrequired_dociument_main_group3eb5b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_main_group")){
        setchecklist_main_group5b62e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_main_group5b62e?.isDisabled==null)
      {
        setchecklist_main_group5b62e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remarks_textarea")){
        setremarks_textarea15a62((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(remarks_textarea15a62?.isDisabled==null)
      {
        setremarks_textarea15a62((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("account_id")){
        setaccount_id4ecc7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(account_id4ecc7?.isDisabled==null)
      {
        setaccount_id4ecc7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const add_case_group1f6e4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    add_case_group1f6e4Ref.current?.setSearchParams();
    add_case_group1f6e4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(add_case_group1f6e4) && Object.keys(add_case_group1f6e4)?.length>0)
      {
        setadd_case_group1f6e4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [add_case_group1f6e4Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"add_case_group"}))
        }}
    >
        ...
        {allowedComponent.includes("checklist_main_group")  &&<Groupchecklist_main_group  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("venue_id") ?<Textvenue_id   /* cb444 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("creditor_id") ?<Textcreditor_id   /* b1867 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("remarks_textarea") ?<TextArearemarks_textarea   /* 15a62 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
          {allowedControls.includes("account_id") ?<Textaccount_id   /* 4ecc7 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
