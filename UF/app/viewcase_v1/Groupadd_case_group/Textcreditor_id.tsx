...
const Textcreditor_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {creditor_idb1867Props, setcreditor_idb1867Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setadd_case_group1f6e4((pre: any) => ({
          ...pre,
          creditor_id: creditor_idb1867Props?.filteredData?.length > 0
            ? creditor_idb1867Props?.filteredData[0]?.creditor_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_group1f6e4.creditor_id){
        setadd_case_group1f6e4((pre:any)=>({...pre,creditor_id:dfd_addcase_v1Props[0]?.creditor_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[creditor_idb1867?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_group1f6e4.creditor_id){
    setadd_case_group1f6e4((pre:any)=>({...pre,creditor_id:dfd_addcase_v1Props[0]?.creditor_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!creditor_idb1867Props?.filterProps) return;
    handleMapperValue(creditor_idb1867Props?.filterProps,creditor_idb1867Props?.filterFlag);
  },[creditor_idb1867Props?.filterProps])

  if (creditor_idb1867?.isHidden) {
    return <></>
  }
