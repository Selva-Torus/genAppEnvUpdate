...
const Textaccount_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {account_id4ecc7Props, setaccount_id4ecc7Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setadd_case_group1f6e4((pre: any) => ({
          ...pre,
          account_id: account_id4ecc7Props?.filteredData?.length > 0
            ? account_id4ecc7Props?.filteredData[0]?.account_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_group1f6e4.account_id){
        setadd_case_group1f6e4((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[account_id4ecc7?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_group1f6e4.account_id){
    setadd_case_group1f6e4((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!account_id4ecc7Props?.filterProps) return;
    handleMapperValue(account_id4ecc7Props?.filterProps,account_id4ecc7Props?.filterFlag);
  },[account_id4ecc7Props?.filterProps])

  if (account_id4ecc7?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `3 / 5`,gridRow: `440 / 441`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
