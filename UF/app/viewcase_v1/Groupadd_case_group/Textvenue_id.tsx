...
const Textvenue_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {venue_idcb444Props, setvenue_idcb444Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setadd_case_group1f6e4((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setadd_case_group1f6e4((pre: any) => ({
          ...pre,
          venue_id: venue_idcb444Props?.filteredData?.length > 0
            ? venue_idcb444Props?.filteredData[0]?.venue_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_group1f6e4.venue_id){
        setadd_case_group1f6e4((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[venue_idcb444?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_group1f6e4.venue_id){
    setadd_case_group1f6e4((pre:any)=>({...pre,venue_id:dfd_addcase_v1Props[0]?.venue_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!venue_idcb444Props?.filterProps) return;
    handleMapperValue(venue_idcb444Props?.filterProps,venue_idcb444Props?.filterFlag);
  },[venue_idcb444Props?.filterProps])

  if (venue_idcb444?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `14 / 16`,gridRow: `8 / 9`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
