...
const Groupgeorgiass_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e7147a2ca87141a4b29d629d4059e4dd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgiass_groups9e4ddProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningsss_icons")){
        setwarningsss_icons3be9c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningsss_icons3be9c?.isDisabled==null)
      {
        setwarningsss_icons3be9c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgiass_sosl_texts")){
        setgeorgiass_sosl_texts88097((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgiass_sosl_texts88097?.isDisabled==null)
      {
        setgeorgiass_sosl_texts88097((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("specials_ruless")){
        setspecials_ruless95686((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(specials_ruless95686?.isDisabled==null)
      {
        setspecials_ruless95686((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningsss_icons'] = warningsss_icons3be9c,
        codeStates['setwarningsss_icons'] = setwarningsss_icons3be9c,
        codeStates['georgiass_sosl_texts'] = georgiass_sosl_texts88097,
        codeStates['setgeorgiass_sosl_texts'] = setgeorgiass_sosl_texts88097,
        codeStates['specials_ruless'] = specials_ruless95686,
        codeStates['setspecials_ruless'] = setspecials_ruless95686,
  ...
  const georgiass_groups9e4ddRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgiass_groups9e4ddRef.current?.setSearchParams();
    georgiass_groups9e4ddRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgiass_groups9e4dd) && Object.keys(georgiass_groups9e4dd)?.length>0)
      {
        setgeorgiass_groups9e4dd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgiass_groups9e4ddProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgiass_groups"}))
        }}
    >
        {allowedControls.includes("warningsss_icons")?<Iconwarningsss_icons /* 3be9c */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgiass_sosl_texts") ?<Textgeorgiass_sosl_texts   /* 88097 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("specials_ruless") ?<Textspecials_ruless   /* 95686 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
