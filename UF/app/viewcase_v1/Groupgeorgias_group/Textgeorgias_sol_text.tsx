...
const Textgeorgias_sol_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {georgias_sol_text24265Props, setgeorgias_sol_text24265Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[georgias_sol_text24265?.refresh])

  if (georgias_sol_text24265?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Homestead Exemption Applies")}
</Text>
  </div>
  )
}

export default Textgeorgias_sol_text
