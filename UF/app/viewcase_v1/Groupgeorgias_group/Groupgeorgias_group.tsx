...
import Iconwarnings_icon  from "./Iconwarnings_icon";
import Textgeorgias_sol_text  from "./Textgeorgias_sol_text";
import Textspecials_rules  from "./Textspecials_rules";
...
const Groupgeorgias_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warnings_icon0adcc, setwarnings_icon0adcc}= useContext(TotalContext) as TotalContextProps;
  const {georgias_sol_text24265, setgeorgias_sol_text24265}= useContext(TotalContext) as TotalContextProps;
  const {specials_rules14021, setspecials_rules14021}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9faf3be54175738c68f33a05ef1bac01");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgias_groupbac01Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warnings_icon")){
        setwarnings_icon0adcc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warnings_icon0adcc?.isDisabled==null)
      {
        setwarnings_icon0adcc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georgias_sol_text")){
        setgeorgias_sol_text24265((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georgias_sol_text24265?.isDisabled==null)
      {
        setgeorgias_sol_text24265((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("specials_rules")){
        setspecials_rules14021((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(specials_rules14021?.isDisabled==null)
      {
        setspecials_rules14021((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warnings_icon'] = warnings_icon0adcc,
        codeStates['setwarnings_icon'] = setwarnings_icon0adcc,
        codeStates['georgias_sol_text'] = georgias_sol_text24265,
        codeStates['setgeorgias_sol_text'] = setgeorgias_sol_text24265,
        codeStates['specials_rules'] = specials_rules14021,
        codeStates['setspecials_rules'] = setspecials_rules14021,
  ...
  const georgias_groupbac01Ref = useRef<any>(null);
  const handleClearSearch = () => {
    georgias_groupbac01Ref.current?.setSearchParams();
    georgias_groupbac01Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgias_groupbac01) && Object.keys(georgias_groupbac01)?.length>0)
      {
        setgeorgias_groupbac01({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgias_groupbac01Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgias_group"}))
        }}
    >
        {allowedControls.includes("warnings_icon")?<Iconwarnings_icon /* 0adcc */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georgias_sol_text") ?<Textgeorgias_sol_text   /* 24265 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("specials_rules") ?<Textspecials_rules   /* 14021 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgeorgias_group
