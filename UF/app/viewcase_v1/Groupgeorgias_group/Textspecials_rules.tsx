...
const Textspecials_rules = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {specials_rules14021Props, setspecials_rules14021Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[specials_rules14021?.refresh])

  if (specials_rules14021?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Georgia homestead exemption may protect primary residence from judgment lien.")}
</Text>
  </div>
  )
}

export default Textspecials_rules
