...
const Grouprequired_dociument_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {reeq_doc_text732f8, setreeq_doc_text732f8}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "212592b2d4dc665ef98893833743eb5b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_main_group3eb5bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reeq_doc_text")){
        setreeq_doc_text732f8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reeq_doc_text732f8?.isDisabled==null)
      {
        setreeq_doc_text732f8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_table")){
        setdoc_tablee79c7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_tablee79c7?.isDisabled==null)
      {
        setdoc_tablee79c7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['reeq_doc_text'] = reeq_doc_text732f8,
        codeStates['setreeq_doc_text'] = setreeq_doc_text732f8,
  ...
  const required_dociument_main_group3eb5bRef = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_main_group3eb5bRef.current?.setSearchParams();
    required_dociument_main_group3eb5bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_main_group3eb5b) && Object.keys(required_dociument_main_group3eb5b)?.length>0)
      {
        setrequired_dociument_main_group3eb5b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_main_group3eb5bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_main_group"}))
        }}
    >
        {allowedComponent.includes("doc_table")  &&<Groupdoc_table  
          ...
          {allowedControls.includes("reeq_doc_text") ?<Textreeq_doc_text   /* 732f8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
