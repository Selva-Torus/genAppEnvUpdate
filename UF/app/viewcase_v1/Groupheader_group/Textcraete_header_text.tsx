...
const Textcraete_header_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {craete_header_text93eaa, setcraete_header_text93eaa}= useContext(TotalContext) as TotalContextProps;
  ...
  const {craete_header_text93eaaProps, setcraete_header_text93eaaProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[craete_header_text93eaa?.refresh])

  if (craete_header_text93eaa?.isHidden) {
    return <></>
  }
