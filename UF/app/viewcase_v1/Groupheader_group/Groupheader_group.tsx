...
const Groupheader_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5d2fb3d78950885a72b3789458d3749a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setheader_group3749aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("craete_header_text")){
        setcraete_header_text93eaa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(craete_header_text93eaa?.isDisabled==null)
      {
        setcraete_header_text93eaa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['craete_header_text'] = craete_header_text93eaa,
        codeStates['setcraete_header_text'] = setcraete_header_text93eaa,
  ...
  const header_group3749aRef = useRef<any>(null);
  const handleClearSearch = () => {
    header_group3749aRef.current?.setSearchParams();
    header_group3749aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(header_group3749a) && Object.keys(header_group3749a)?.length>0)
      {
        setheader_group3749a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [header_group3749aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"header_group"}))
        }}
    >
          {allowedControls.includes("craete_header_text") ?<Textcraete_header_text   /* 93eaa */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
