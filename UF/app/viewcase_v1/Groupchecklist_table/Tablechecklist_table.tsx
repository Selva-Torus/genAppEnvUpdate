...
let defaultColumns:any = [
  {
    ...
    "nodeid": "a2f072e261cf4946b4f26c12ef4611e6",
  ...
  {
    "id": "item_name",
    "nodeid": "61c0e898f45d4fe3a5533b3290bafc9a",
  ...
  {
    "id": "is_completed",
    "nodeid": "fa24eb4ef02144bf8d99a1879d66ef7a",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tablechecklist_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {checklist_item_id611e6, setchecklist_item_id611e6}= useContext(TotalContext) as TotalContextProps  
  const {item_nameafc9a, setitem_nameafc9a}= useContext(TotalContext) as TotalContextProps  
  const {is_completed6ef7a, setis_completed6ef7a}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "7fed27426c324e5f8ca49a40e0545abc",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, checklist_table45abc })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, checklist_table45abc }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, checklist_table45abc } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setchecklist_table45abcProps((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }


      checklist_table45abc.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setchecklist_table45abcProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setchecklist_table45abcProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['checklist_item_id'] = checklist_item_id611e6,
        codeStates['setchecklist_item_id'] = setchecklist_item_id611e6,
        codeStates['item_name'] = item_nameafc9a,
        codeStates['setitem_name'] = setitem_nameafc9a,
        codeStates['is_completed'] = is_completed6ef7a,
        codeStates['setis_completed'] = setis_completed6ef7a,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setchecklist_table45abcProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["account_id"]:checklist_table45abcProps?.presetValues?.account_id}
    ...
    if(!checklist_table45abcProps?.presetValues || Object.keys(checklist_table45abcProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '7fed27426c324e5f8ca49a40e0545abc',
                elementId: '7fed27426c324e5f8ca49a40e0545abc'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setchecklist_table45abc(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(checklist_table45abc))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (checklist_table45abcProps.filterInitalLoad) return;

    const filterControllers = checklist_table45abcProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['checklist_table45abc']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['checklist_table45abc']).filter((k: string) => jsonEntry['checklist_table45abc'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(checklist_table45abcProps.filterProps);
      return;
    }
    ...
    setchecklist_table45abcProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(checklist_table45abcProps.filterProps);

  }, [
    checklist_table45abcProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    checklist_table45abcProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(checklist_table45abcProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(checklist_table45abcProps.searchFilter) || {};
...
}, [checklist_table45abcProps.searchFilter])
  ...
  useEffect(() => {
    if (checklist_table45abcProps?.clearData === true) {
      setchecklist_table45abc([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setchecklist_table45abcProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [checklist_table45abcProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [checklist_table45abcProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = checklist_table45abcProps?.selectedIds[checklist_table45abcProps?.selectedIds?.length-1]
    if(Array.isArray(checklist_table45abc) && checklist_table45abc.length>0)
    {
      let data = checklist_table45abc.find((data:any)=>(data?.checklist_status_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "checklist_status_id",
  ...
  useEffect(() => {
    if (checklist_table45abcProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (checklist_table45abcProps?.selectedIds?.length == 0){
      handleOnRowClick({},checklist_table45abcProps?.selectedIds)
    }
  }, [checklist_table45abcProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(checklist_table45abc?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&checklist_table45abc?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](checklist_table45abc?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_table45abc?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&checklist_table45abc?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](checklist_table45abc?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: checklist_table45abc?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (checklist_table45abc?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"7fed27426c324e5f8ca49a40e0545abc"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={checklist_table45abcProps?.selectedIds}  
