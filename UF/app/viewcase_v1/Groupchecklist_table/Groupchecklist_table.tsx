...
const Groupchecklist_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7fed27426c324e5f8ca49a40e0545abc");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_table45abcProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("checklist_item_id")){
        setchecklist_item_id611e6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_item_id611e6?.isDisabled==null)
      {
        setchecklist_item_id611e6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("item_name")){
        setitem_nameafc9a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(item_nameafc9a?.isDisabled==null)
      {
        setitem_nameafc9a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("is_completed")){
        setis_completed6ef7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(is_completed6ef7a?.isDisabled==null)
      {
        setis_completed6ef7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const checklist_table45abcRef = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_table45abcRef.current?.setSearchParams();
    checklist_table45abcRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_table45abc) && Object.keys(checklist_table45abc)?.length>0)
      {
        setchecklist_table45abc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_table45abcProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablechecklist_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={checklist_table45abcRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
