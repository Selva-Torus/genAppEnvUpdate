...
const Groupdoc_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6b6d3099fa6a45dea7e98d27d7fe79c7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoc_tablee79c7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("attachment_id")){
        setattachment_id54469((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(attachment_id54469?.isDisabled==null)
      {
        setattachment_id54469((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
        setdoc_name513db((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_name513db?.isDisabled==null)
      {
        setdoc_name513db((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_by")){
        settrs_created_by736ad((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_by736ad?.isDisabled==null)
      {
        settrs_created_by736ad((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("trs_created_date")){
        settrs_created_datecfead((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(trs_created_datecfead?.isDisabled==null)
      {
        settrs_created_datecfead((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const doc_tablee79c7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doc_tablee79c7Ref.current?.setSearchParams();
    doc_tablee79c7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_tablee79c7) && Object.keys(doc_tablee79c7)?.length>0)
      {
        setdoc_tablee79c7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_tablee79c7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"doc_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tabledoc_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_tablee79c7Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
