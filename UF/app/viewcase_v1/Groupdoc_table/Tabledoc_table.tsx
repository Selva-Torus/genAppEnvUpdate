...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "2704256ce5014e2ab5fcb7db41354469",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "005c79990e964067978d6f0ac8c513db",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "d7d33f708d1f4958bbe4a490448736ad",
    "name": "Created By",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "8f8722c4e3424cc88eaed135a5acfead",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
...
const Tabledoc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {attachment_id54469, setattachment_id54469}= useContext(TotalContext) as TotalContextProps  
  const {doc_name513db, setdoc_name513db}= useContext(TotalContext) as TotalContextProps  
  const {trs_created_by736ad, settrs_created_by736ad}= useContext(TotalContext) as TotalContextProps  
  const {trs_created_datecfead, settrs_created_datecfead}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "6b6d3099fa6a45dea7e98d27d7fe79c7",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, doc_tablee79c7 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, doc_tablee79c7 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, doc_tablee79c7 } = latestLockStateRef.current
    /// setdoc_tablee79c7Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(doc_tablee79c7.map((item: any) => item.doc_instance_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      doc_tablee79c7.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = doc_tablee79c7
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.doc_instance_id) ? item.doc_instance_id : null
          )
          .filter(Boolean)

        setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        throw err
      }
    }
    let index = Number(ids[ids.length - 1])
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['attachment_id'] = attachment_id54469,
        codeStates['setattachment_id'] = setattachment_id54469,
        codeStates['doc_name'] = doc_name513db,
        codeStates['setdoc_name'] = setdoc_name513db,
        codeStates['trs_created_by'] = trs_created_by736ad,
        codeStates['settrs_created_by'] = settrs_created_by736ad,
        codeStates['trs_created_date'] = trs_created_datecfead,
        codeStates['settrs_created_date'] = settrs_created_datecfead,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["account_id"]:doc_tablee79c7Props?.presetValues?.account_id}
    ...
    if(!doc_tablee79c7Props?.presetValues || Object.keys(doc_tablee79c7Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1:UO', 
                nodeId: '6b6d3099fa6a45dea7e98d27d7fe79c7',
                elementId: '6b6d3099fa6a45dea7e98d27d7fe79c7'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setdoc_tablee79c7(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(doc_tablee79c7))
    {
      GetTableDetails()
    }
  }, [doc_tablee79c7])
  ...
  useEffect(() => {
    ...
    if (doc_tablee79c7Props.filterInitalLoad) return;

    const filterControllers = doc_tablee79c7Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['doc_tablee79c7']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['doc_tablee79c7']).filter((k: string) => jsonEntry['doc_tablee79c7'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(doc_tablee79c7Props.filterProps);
      return;
    }
    ...
    setdoc_tablee79c7Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(doc_tablee79c7Props.filterProps);

  }, [
    doc_tablee79c7Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    doc_tablee79c7Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(doc_tablee79c7Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(doc_tablee79c7Props.searchFilter) || {};
...
}, [doc_tablee79c7Props.searchFilter])
  ...
  useEffect(() => {
    if (doc_tablee79c7Props?.clearData === true) {
      if (myLockedIdsRef.current.length) {
        myLockedIdsRef.current.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setdoc_tablee79c7Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [doc_tablee79c7Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  }, [doc_tablee79c7Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = doc_tablee79c7Props?.selectedIds[doc_tablee79c7Props?.selectedIds?.length-1]
    if(Array.isArray(doc_tablee79c7) && doc_tablee79c7.length>0)
    {
      let data = doc_tablee79c7.find((data:any)=>(data?.doc_instance_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "doc_instance_id",
  ...
  useEffect(() => {
    if (doc_tablee79c7Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (doc_tablee79c7Props?.selectedIds?.length == 0){
      handleOnRowClick({},doc_tablee79c7Props?.selectedIds)
    }
  }, [doc_tablee79c7Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setdoc_tablee79c7Props((pre:any)=>({...pre, selectedIds:[row?.doc_instance_id]}))  
    onButtonSecurityHandle(data)
    ...
    let filterProps2:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData2 = await getFilterProps(filterProps2,{...add_case_group1f6e4,...header_group3749a,...case_information_groupcec29,...venue_groupa72d9,...georgia_groupa636c,...georgias_groupbac01,...georgias_groupsbf356,...georgiass_groups9e4dd,...georgsiass_groups6bf7a,...debtor_information_groupdfa55,...financial_details_grouped0d9,...venue_details_group6a27a,...required_dociument_main_group3eb5b,...checklist_main_group5b62e,...checklist_table45abc,...doc_tablee79c7});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(doc_tablee79c7?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&doc_tablee79c7?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](doc_tablee79c7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_tablee79c7?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&doc_tablee79c7?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](doc_tablee79c7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doc_tablee79c7?.at(newIndex) ,currectPage:paginationData?.page});
      ...
      >
        <PageDocumentviewerpage2/>
          ...
          <div
            ...
            id={"6b6d3099fa6a45dea7e98d27d7fe79c7"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableSorting={true}
              disable={disable}
              selectedIds={doc_tablee79c7Props?.selectedIds}  
