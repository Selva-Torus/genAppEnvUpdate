...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,charge_off_date:undefined}}));
  if (!date) {
    setfinancial_details_grouped0d9((prev: any) => ({ ...prev, charge_off_date: null }));
    return;
  }
  const now = new Date();
  ...
  setfinancial_details_grouped0d9((prev: any) => ({ ...prev, charge_off_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "63554701a6605ae042ec185840613a39"
      );
...
if (charge_off_date13a39?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 8`,gridRow: `10 / 22`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={financial_details_grouped0d9?.charge_off_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {charge_off_date13a39?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Charge Off Date"
      dateValidation=""
      validationState={validate?.viewCase_v1?.charge_off_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
