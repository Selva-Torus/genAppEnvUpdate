...
const TextInputprincipal = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|7371fc8250b615220e8d394f4c4ed0d9|ee422881cb0b6e97b2af43bedb06eb2a"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,principal:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_grouped0d9((prev: any) => ({ ...prev, principal: +e.target.value }));
    }
    else{
    setfinancial_details_grouped0d9((prev: any) => ({ ...prev, principal: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ee422881cb0b6e97b2af43bedb06eb2a"
      );
      ...
      //     controlId: "ee422881cb0b6e97b2af43bedb06eb2a",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ee422881cb0b6e97b2af43bedb06eb2a") {
        handleChange({target:{value:financial_details_grouped0d9Ref?.current?.principal||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ee422881cb0b6e97b2af43bedb06eb2a") {
        handleBlur({target:{value:financial_details_grouped0d9Ref?.current?.principal||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_grouped0d9((pre:any)=>({...pre,principal:dfd_addcase_v1Props.data[0]?.principal}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (principal6eb2a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={financial_details_grouped0d9?.principal||""}
         disabled= {principal6eb2a?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.principal ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
