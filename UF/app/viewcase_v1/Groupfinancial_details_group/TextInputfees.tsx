...
const TextInputfees = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|7371fc8250b615220e8d394f4c4ed0d9|87c4d7898c5ea3f8a8b642d2d2ab456c"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,fees:undefined}}));
    if(dynamicStateandType.type=="number"){
    setfinancial_details_grouped0d9((prev: any) => ({ ...prev, fees: +e.target.value }));
    }
    else{
    setfinancial_details_grouped0d9((prev: any) => ({ ...prev, fees: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "87c4d7898c5ea3f8a8b642d2d2ab456c"
      );
      ...
      //     controlId: "87c4d7898c5ea3f8a8b642d2d2ab456c",
      //     isTable: false,
      //     from:"TextInputfees",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "87c4d7898c5ea3f8a8b642d2d2ab456c") {
        handleChange({target:{value:financial_details_grouped0d9Ref?.current?.fees||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "87c4d7898c5ea3f8a8b642d2d2ab456c") {
        handleBlur({target:{value:financial_details_grouped0d9Ref?.current?.fees||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setfinancial_details_grouped0d9((pre:any)=>({...pre,fees:dfd_addcase_v1Props.data[0]?.fees}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (feesb456c?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={financial_details_grouped0d9?.fees||""}
         disabled= {feesb456c?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
        ...
        headerText="Fees"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.fees ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
