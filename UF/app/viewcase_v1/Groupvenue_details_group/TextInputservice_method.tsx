...
const TextInputservice_method = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,service_method:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, service_method: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, service_method: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "7fb3c7308ef94d7ea2ee3cf2ac11b411"
      );
      ...
      //     controlId: "7fb3c7308ef94d7ea2ee3cf2ac11b411",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "7fb3c7308ef94d7ea2ee3cf2ac11b411") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.service_method||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "7fb3c7308ef94d7ea2ee3cf2ac11b411") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.service_method||""}});
      }
    };
  ...
  if (service_method1b411?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.service_method||""}
         disabled= {service_method1b411?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.service_method ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
