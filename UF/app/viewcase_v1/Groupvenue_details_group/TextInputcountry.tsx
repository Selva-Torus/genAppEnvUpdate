...
const TextInputcountry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|c047eba261894ed1a6f57f313eb6a27a|abeed84434d4cc50a6d7a1f95d81983b"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,county:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, county: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, county: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "abeed84434d4cc50a6d7a1f95d81983b"
      );
      ...
      //     controlId: "abeed84434d4cc50a6d7a1f95d81983b",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "abeed84434d4cc50a6d7a1f95d81983b") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.county||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "abeed84434d4cc50a6d7a1f95d81983b") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.county||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6a27a((pre:any)=>({...pre,county:dfd_addcase_v1Props.data[0]?.county}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (country1983b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.county||""}
         disabled= {country1983b?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Country"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.county ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
