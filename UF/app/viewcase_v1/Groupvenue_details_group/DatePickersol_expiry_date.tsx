...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,sol_expiry_date:undefined}}));
  if (!date) {
    setvenue_details_group6a27a((prev: any) => ({ ...prev, sol_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvenue_details_group6a27a((prev: any) => ({ ...prev, sol_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "96309f080c89f4bd2efb7669fbdd5486"
      );
...
useEffect(()=>{
  setvenue_details_group6a27aProps((pre:any)=>({...pre,validation:true}))
 },[sol_expiry_dated5486?.refresh])
...
if (sol_expiry_dated5486?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 8`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={venue_details_group6a27a?.sol_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {sol_expiry_dated5486?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="SOL Expiry Date"
      dateValidation=""
      validationState={validate?.viewCase_v1?.sol_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
