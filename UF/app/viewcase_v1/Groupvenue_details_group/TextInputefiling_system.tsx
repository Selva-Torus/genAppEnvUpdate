...
const TextInputefiling_system = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,efiling_system:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, efiling_system: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, efiling_system: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "2920dce1d6d24a7ea77ef030b197d508"
      );
      ...
      //     controlId: "2920dce1d6d24a7ea77ef030b197d508",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "2920dce1d6d24a7ea77ef030b197d508") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.efiling_system||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "2920dce1d6d24a7ea77ef030b197d508") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.efiling_system||""}});
      }
    };
  ...
  if (efiling_system7d508?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.efiling_system||""}
         disabled= {efiling_system7d508?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.efiling_system ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
