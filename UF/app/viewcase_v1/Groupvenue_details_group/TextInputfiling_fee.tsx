...
const TextInputfiling_fee = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|c047eba261894ed1a6f57f313eb6a27a|e4368ffa4647c219258c936427089c7f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,filing_fee:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, filing_fee: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, filing_fee: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e4368ffa4647c219258c936427089c7f"
      );
      ...
      //     controlId: "e4368ffa4647c219258c936427089c7f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e4368ffa4647c219258c936427089c7f") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.filing_fee||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e4368ffa4647c219258c936427089c7f") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.filing_fee||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6a27a((pre:any)=>({...pre,filing_fee:dfd_addcase_v1Props.data[0]?.filing_fee}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (filing_fee89c7f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `8 / 13`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.filing_fee||""}
         disabled= {filing_fee89c7f?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.filing_fee ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
