...
const Groupvenue_details_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c047eba261894ed1a6f57f313eb6a27a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvenue_details_group6a27aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("ven_name_text")){
        setven_name_text0c8ba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ven_name_text0c8ba?.isDisabled==null)
      {
        setven_name_text0c8ba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("state")){
        setstate3010e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(state3010e?.isDisabled==null)
      {
        setstate3010e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("country")){
        setcountry1983b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(country1983b?.isDisabled==null)
      {
        setcountry1983b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("court_name")){
        setcourt_name41a77((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(court_name41a77?.isDisabled==null)
      {
        setcourt_name41a77((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("judge_name")){
        setjudge_name0a819((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(judge_name0a819?.isDisabled==null)
      {
        setjudge_name0a819((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("sol_expiry_date")){
        setsol_expiry_dated5486((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(sol_expiry_dated5486?.isDisabled==null)
      {
        setsol_expiry_dated5486((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("filing_fee")){
        setfiling_fee89c7f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(filing_fee89c7f?.isDisabled==null)
      {
        setfiling_fee89c7f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("service_method")){
        setservice_method1b411((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(service_method1b411?.isDisabled==null)
      {
        setservice_method1b411((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("efiling_system")){
        setefiling_system7d508((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(efiling_system7d508?.isDisabled==null)
      {
        setefiling_system7d508((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("efiling_required")){
        setefiling_required36d60((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(efiling_required36d60?.isDisabled==null)
      {
        setefiling_required36d60((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const venue_details_group6a27aRef = useRef<any>(null);
  const handleClearSearch = () => {
    venue_details_group6a27aRef.current?.setSearchParams();
    venue_details_group6a27aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(venue_details_group6a27a) && Object.keys(venue_details_group6a27a)?.length>0)
      {
        setvenue_details_group6a27a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [venue_details_group6a27aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"venue_details_group"}))
        }}
    >
          {allowedControls.includes("ven_name_text") ?<Textven_name_text   /* 0c8ba */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("state") ?<TextInputstate   /* 3010e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("country") ?<TextInputcountry   /* 1983b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("court_name") ?<TextInputcourt_name   /* 41a77 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("judge_name") ?<TextInputjudge_name   /* 0a819 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("sol_expiry_date") ?<DatePickersol_expiry_date   /* d5486 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("filing_fee") ?<TextInputfiling_fee   /* 89c7f */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("service_method") ?<TextInputservice_method   /* 1b411 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("efiling_system") ?<TextInputefiling_system   /* 7d508 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("efiling_required")?<Switchefiling_required  /* 36d60 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
