...
const TextInputjudge_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|c047eba261894ed1a6f57f313eb6a27a|1a201d0fc8e958a1f970c2bbe3d0a819"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,judge_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, judge_name: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, judge_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "1a201d0fc8e958a1f970c2bbe3d0a819"
      );
      ...
      //     controlId: "1a201d0fc8e958a1f970c2bbe3d0a819",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "1a201d0fc8e958a1f970c2bbe3d0a819") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.judge_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "1a201d0fc8e958a1f970c2bbe3d0a819") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.judge_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6a27a((pre:any)=>({...pre,judge_name:dfd_addcase_v1Props.data[0]?.judge_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (judge_name0a819?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `10 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.judge_name||""}
         disabled= {judge_name0a819?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Judge"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.judge_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
