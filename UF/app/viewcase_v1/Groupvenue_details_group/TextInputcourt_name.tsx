...
const TextInputcourt_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|c047eba261894ed1a6f57f313eb6a27a|9d55e36afc6bd8f40158ac1000a41a77"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,court_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6a27a((prev: any) => ({ ...prev, court_name: +e.target.value }));
    }
    else{
    setvenue_details_group6a27a((prev: any) => ({ ...prev, court_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9d55e36afc6bd8f40158ac1000a41a77"
      );
      ...
      //     controlId: "9d55e36afc6bd8f40158ac1000a41a77",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9d55e36afc6bd8f40158ac1000a41a77") {
        handleChange({target:{value:venue_details_group6a27aRef?.current?.court_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9d55e36afc6bd8f40158ac1000a41a77") {
        handleBlur({target:{value:venue_details_group6a27aRef?.current?.court_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6a27a((pre:any)=>({...pre,court_name:dfd_addcase_v1Props.data[0]?.court_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (court_name41a77?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6a27a?.court_name||""}
         disabled= {court_name41a77?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Court"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.court_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
