...
const TextInputqueue_position = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|19e9edfb1e7ccf7985bc9825117cec29|baef41145ab68936140422c91d0ceb8d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,queue_position:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupcec29((prev: any) => ({ ...prev, queue_position: +e.target.value }));
    }
    else{
    setcase_information_groupcec29((prev: any) => ({ ...prev, queue_position: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "baef41145ab68936140422c91d0ceb8d"
      );
      ...
      //     controlId: "baef41145ab68936140422c91d0ceb8d",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "baef41145ab68936140422c91d0ceb8d") {
        handleChange({target:{value:case_information_groupcec29Ref?.current?.queue_position||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "baef41145ab68936140422c91d0ceb8d") {
        handleBlur({target:{value:case_information_groupcec29Ref?.current?.queue_position||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_groupcec29((pre:any)=>({...pre,queue_position:dfd_addcase_v1Props.data[0]?.queue_position}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (queue_positionceb8d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_groupcec29?.queue_position||""}
         disabled= {queue_positionceb8d?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.queue_position ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
