...
const TextInputpriority_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'priority_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,priority_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupcec29((prev: any) => ({ ...prev, priority_name: +e.target.value }));
    }
    else{
    setcase_information_groupcec29((prev: any) => ({ ...prev, priority_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e0b54239fe76483389cc26c22dc449dd"
      );
      ...
      //     controlId: "e0b54239fe76483389cc26c22dc449dd",
      //     isTable: false,
      //     from:"TextInputpriority_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'priority_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'priority_name',type:'text'};
      //   type={
      //     name:'priority_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.priority_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.priority_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.priority_name.type
      //   }
      ...
      //   type={
      //     name:'priority_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.priority_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.priority_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.priority_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e0b54239fe76483389cc26c22dc449dd") {
        handleChange({target:{value:case_information_groupcec29Ref?.current?.priority_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e0b54239fe76483389cc26c22dc449dd") {
        handleBlur({target:{value:case_information_groupcec29Ref?.current?.priority_name||""}});
      }
    };
  ...
  if (priority_name449dd?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `25 / 37`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={case_information_groupcec29?.priority_name||""}
         disabled= {priority_name449dd?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Priority"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.priority_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpriority_name
