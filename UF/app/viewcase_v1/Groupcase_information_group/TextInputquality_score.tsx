...
const TextInputquality_score = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1|0b84a99163eb8dd425338976559dd501|properties.quality_score"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|19e9edfb1e7ccf7985bc9825117cec29|e5db34d6e0d4254536500f8a054dfaa9"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'quality_score',type:"number"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,quality_score:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_groupcec29((prev: any) => ({ ...prev, quality_score: +e.target.value }));
    }
    else{
    setcase_information_groupcec29((prev: any) => ({ ...prev, quality_score: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e5db34d6e0d4254536500f8a054dfaa9"
      );
      ...
      //     controlId: "e5db34d6e0d4254536500f8a054dfaa9",
      //     isTable: false,
      //     from:"TextInputquality_score",
      ...
      //   type={
      //     name:'quality_score',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.quality_score.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.quality_score.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.quality_score.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e5db34d6e0d4254536500f8a054dfaa9") {
        handleChange({target:{value:case_information_groupcec29Ref?.current?.quality_score||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e5db34d6e0d4254536500f8a054dfaa9") {
        handleBlur({target:{value:case_information_groupcec29Ref?.current?.quality_score||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_groupcec29((pre:any)=>({...pre,quality_score:dfd_addcase_v1Props.data[0]?.quality_score}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (quality_scoredfaa9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_groupcec29?.quality_score||""}
         disabled= {quality_scoredfaa9?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.quality_score ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputquality_score
