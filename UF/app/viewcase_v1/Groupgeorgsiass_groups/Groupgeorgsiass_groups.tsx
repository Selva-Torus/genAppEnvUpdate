...
import Iconwarningssss_icons  from "./Iconwarningssss_icons";
...
const Groupgeorgsiass_groups = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {warningssss_icons55275, setwarningssss_icons55275}= useContext(TotalContext) as TotalContextProps;
  const {georsgiass_sosl_texts15f5e, setgeorsgiass_sosl_texts15f5e}= useContext(TotalContext) as TotalContextProps;
  const {spesciasls_ruless910f1, setspesciasls_ruless910f1}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0506e20f2a374d9ca16712c8fb96bf7a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgeorgsiass_groups6bf7aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("warningssss_icons")){
        setwarningssss_icons55275((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(warningssss_icons55275?.isDisabled==null)
      {
        setwarningssss_icons55275((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("georsgiass_sosl_texts")){
        setgeorsgiass_sosl_texts15f5e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(georsgiass_sosl_texts15f5e?.isDisabled==null)
      {
        setgeorsgiass_sosl_texts15f5e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("spesciasls_ruless")){
        setspesciasls_ruless910f1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(spesciasls_ruless910f1?.isDisabled==null)
      {
        setspesciasls_ruless910f1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['warningssss_icons'] = warningssss_icons55275,
        codeStates['setwarningssss_icons'] = setwarningssss_icons55275,
        codeStates['georsgiass_sosl_texts'] = georsgiass_sosl_texts15f5e,
        codeStates['setgeorsgiass_sosl_texts'] = setgeorsgiass_sosl_texts15f5e,
        codeStates['spesciasls_ruless'] = spesciasls_ruless910f1,
        codeStates['setspesciasls_ruless'] = setspesciasls_ruless910f1,
  ...
  const georgsiass_groups6bf7aRef = useRef<any>(null);
  const handleClearSearch = () => {
    georgsiass_groups6bf7aRef.current?.setSearchParams();
    georgsiass_groups6bf7aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(georgsiass_groups6bf7a) && Object.keys(georgsiass_groups6bf7a)?.length>0)
      {
        setgeorgsiass_groups6bf7a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [georgsiass_groups6bf7aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"georgsiass_groups"}))
        }}
    >
        {allowedControls.includes("warningssss_icons")?<Iconwarningssss_icons /* 55275 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
          {allowedControls.includes("georsgiass_sosl_texts") ?<Textgeorsgiass_sosl_texts   /* 15f5e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("spesciasls_ruless") ?<Textspesciasls_ruless   /* 910f1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupgeorgsiass_groups
