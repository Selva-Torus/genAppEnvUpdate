...
import Textdebt_info_text  from "./Textdebt_info_text";
...
const Groupdebtor_information_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {debt_info_textb09b8, setdebt_info_textb09b8}= useContext(TotalContext) as TotalContextProps;
  const {debtor_namef7cac, setdebtor_namef7cac}= useContext(TotalContext) as TotalContextProps;
  const {ssn_masked26c46, setssn_masked26c46}= useContext(TotalContext) as TotalContextProps;
  const {dob0f0bf, setdob0f0bf}= useContext(TotalContext) as TotalContextProps;
  const {address22d72, setaddress22d72}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2945c3ed617347a7c830ad1f64edfa55");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdebtor_information_groupdfa55Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("debt_info_text")){
        setdebt_info_textb09b8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debt_info_textb09b8?.isDisabled==null)
      {
        setdebt_info_textb09b8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("debtor_name")){
        setdebtor_namef7cac((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(debtor_namef7cac?.isDisabled==null)
      {
        setdebtor_namef7cac((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ssn_masked")){
        setssn_masked26c46((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(ssn_masked26c46?.isDisabled==null)
      {
        setssn_masked26c46((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dob")){
        setdob0f0bf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dob0f0bf?.isDisabled==null)
      {
        setdob0f0bf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("address")){
        setaddress22d72((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(address22d72?.isDisabled==null)
      {
        setaddress22d72((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['debt_info_text'] = debt_info_textb09b8,
        codeStates['setdebt_info_text'] = setdebt_info_textb09b8,
        codeStates['debtor_name'] = debtor_namef7cac,
        codeStates['setdebtor_name'] = setdebtor_namef7cac,
        codeStates['ssn_masked'] = ssn_masked26c46,
        codeStates['setssn_masked'] = setssn_masked26c46,
        codeStates['dob'] = dob0f0bf,
        codeStates['setdob'] = setdob0f0bf,
        codeStates['address'] = address22d72,
        codeStates['setaddress'] = setaddress22d72,
  ...
  const debtor_information_groupdfa55Ref = useRef<any>(null);
  const handleClearSearch = () => {
    debtor_information_groupdfa55Ref.current?.setSearchParams();
    debtor_information_groupdfa55Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(debtor_information_groupdfa55) && Object.keys(debtor_information_groupdfa55)?.length>0)
      {
        setdebtor_information_groupdfa55({})
      }
    }else 
      prevRefreshRef.current= true
  }, [debtor_information_groupdfa55Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setviewcase_v1((pre:any)=>({...pre,_selectedGroup_:"debtor_information_group"}))
        }}
    >
          {allowedControls.includes("debt_info_text") ?<Textdebt_info_text   /* b09b8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("debtor_name") ?<TextInputdebtor_name   /* f7cac */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("ssn_masked") ?<TextInputssn_masked   /* 26c46 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dob") ?<DatePickerdob   /* 0f0bf */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("address") ?<TextAreaaddress   /* 22d72 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}

export default Groupdebtor_information_group
