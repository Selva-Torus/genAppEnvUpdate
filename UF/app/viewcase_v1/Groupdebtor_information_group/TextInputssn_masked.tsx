...
const TextInputssn_masked = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|2945c3ed617347a7c830ad1f64edfa55|79caa3d6674e6fc9e34d0ecc7c126c46"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,ssn_masked:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdebtor_information_groupdfa55((prev: any) => ({ ...prev, ssn_masked: +e.target.value }));
    }
    else{
    setdebtor_information_groupdfa55((prev: any) => ({ ...prev, ssn_masked: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "2945c3ed617347a7c830ad1f64edfa55",
        "79caa3d6674e6fc9e34d0ecc7c126c46"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1",
      //     componentId: "2945c3ed617347a7c830ad1f64edfa55",
      //     controlId: "79caa3d6674e6fc9e34d0ecc7c126c46",
      //     isTable: false,
  ...
  const debtor_information_groupdfa55Ref = useRef<any>(debtor_information_groupdfa55);
  useEffect(() => { debtor_information_groupdfa55Ref.current = debtor_information_groupdfa55; }, [debtor_information_groupdfa55]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "79caa3d6674e6fc9e34d0ecc7c126c46") {
        handleChange({target:{value:debtor_information_groupdfa55Ref?.current?.ssn_masked||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "79caa3d6674e6fc9e34d0ecc7c126c46") {
        handleBlur({target:{value:debtor_information_groupdfa55Ref?.current?.ssn_masked||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setdebtor_information_groupdfa55((pre:any)=>({...pre,ssn_masked:dfd_addcase_v1Props.data[0]?.ssn_masked}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (ssn_masked26c46?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `10 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={debtor_information_groupdfa55?.ssn_masked||""}
         disabled= {ssn_masked26c46?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="SSN"
      errorMessage={error}
        validationState={validate?.viewCase_v1?.ssn_masked ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
