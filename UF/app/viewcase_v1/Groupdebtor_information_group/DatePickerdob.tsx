...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,dob:undefined}}));
  if (!date) {
    setdebtor_information_groupdfa55((prev: any) => ({ ...prev, dob: null }));
    return;
  }
  const now = new Date();
  ...
  setdebtor_information_groupdfa55((prev: any) => ({ ...prev, dob: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "cfdc38b06f60edbe5ffe0462f460f0bf"
      );
...
useEffect(()=>{
  setdebtor_information_groupdfa55Props((pre:any)=>({...pre,validation:true}))
 },[dob0f0bf?.refresh])
...
return (
  ...
  style={{gridColumn: `1 / 13`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={debtor_information_groupdfa55?.dob}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {dob0f0bf?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Date Of Birth"
      dateValidation=""
      validationState={validate?.viewCase_v1?.dob ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
