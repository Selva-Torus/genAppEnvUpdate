...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:viewCase:AFVK:v1|2945c3ed617347a7c830ad1f64edfa55|a99ee037154cc5104e292026d8df7cac"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,viewCase_v1:{...pre?.viewCase_v1,debtor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdebtor_information_groupdfa55((prev: any) => ({ ...prev, debtor_name: +e.target.value }));
    }
    else{
    setdebtor_information_groupdfa55((prev: any) => ({ ...prev, debtor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a99ee037154cc5104e292026d8df7cac"
      );
      ...
      //     controlId: "a99ee037154cc5104e292026d8df7cac",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a99ee037154cc5104e292026d8df7cac") {
        handleChange({target:{value:debtor_information_groupdfa55Ref?.current?.debtor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a99ee037154cc5104e292026d8df7cac") {
        handleBlur({target:{value:debtor_information_groupdfa55Ref?.current?.debtor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setdebtor_information_groupdfa55((pre:any)=>({...pre,debtor_name:dfd_addcase_v1Props.data[0]?.debtor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (debtor_namef7cac?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={debtor_information_groupdfa55?.debtor_name||""}
         disabled= {debtor_namef7cac?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.viewCase_v1?.debtor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
