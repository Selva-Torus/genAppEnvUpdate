...
const TextInputcycle_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1|027910746c6343f7b637e85490c0c120|properties.cycle_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:newPerformanceCycle:AFVK:v1|071c2ad6b391e5265316b0c6f2dbdb89|7d07b03d4ba3ffad18e49c3dd3fe6684"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cycle_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,cycle_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setaccess_req__groupbdb89((prev: any) => ({ ...prev, cycle_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(access_req__groupbdb89?.cycle_name == "" || access_req__groupbdb89?.cycle_name == undefined){
      access_req__groupbdb89.cycle_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, access_req__groupbdb89?.cycle_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,cycle_name:"invalid"}}));
        }
    }else if(access_req__groupbdb89?.cycle_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +access_req__groupbdb89?.cycle_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "7d07b03d4ba3ffad18e49c3dd3fe6684"
      );
      ...
      //     controlId: "7d07b03d4ba3ffad18e49c3dd3fe6684",
      //     isTable: false,
      //     from:"TextInputcycle_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cycle_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cycle_name',type:'text'};
      //   type={
      //     name:'cycle_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cycle_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cycle_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cycle_name.type
      //   }
      ...
      //   type={
      //     name:'cycle_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cycle_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cycle_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cycle_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!access_req__groupbdb89?.cycle_name)
      {
    ...
    const handlerChange = (id:any) => {
      if (id === "7d07b03d4ba3ffad18e49c3dd3fe6684") {
        handleChange({target:{value:access_req__groupbdb89Ref?.current?.cycle_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "7d07b03d4ba3ffad18e49c3dd3fe6684") {
        handleBlur({target:{value:access_req__groupbdb89Ref?.current?.cycle_name||""}});
      }
    };
  ...
  if(dfd_addperformancecyclemodify_v1Props?.setSearchFilters && dfd_addperformancecyclemodify_v1Props?.data)
  {
    if(Array.isArray(dfd_addperformancecyclemodify_v1Props.data) && dfd_addperformancecyclemodify_v1Props.data.length > 0){
      setaccess_req__groupbdb89((pre:any)=>({...pre,cycle_name:dfd_addperformancecyclemodify_v1Props.data[0]?.cycle_name}));
    }
  }
  },[dfd_addperformancecyclemodify_v1Props?.setSearchFilters])
  if (cycle_namee6684?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={access_req__groupbdb89?.cycle_name||""}
         disabled= {cycle_namee6684?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.newPerformanceCycle_v1?.cycle_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcycle_name
