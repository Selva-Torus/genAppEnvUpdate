 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "5e66ffb2ca060bf6938ba82f5cd7df15"
    );
  ...
  },[review_frequency7df15?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setvalid_group071c1((prev: any) => ({ ...prev,review_frequency7df15: value }))
      setIsRequredData(false)
    }else{
      let temp:any = valid_group071c1;
      delete temp.review_frequency;
      delete temp.REVIEW_FREQUENCY;
      delete temp.review_frequency7df15;
      setvalid_group071c1(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,review_frequency:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(valid_group071c1.review_frequency){
      if(Array.isArray(dfd_reviewfrequencycombo_v1Props)){
        if(dfd_reviewfrequencycombo_v1Props?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)){
          setdropdownValue([dfd_reviewfrequencycombo_v1Props?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)?.perf_review_frequency])
          tempValue=dfd_reviewfrequencycombo_v1Props?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)?.perf_review_frequency
        }else{
          setdropdownValue([valid_group071c1.review_frequency])
          tempValue=valid_group071c1.review_frequency
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{perf_review_frequency:valid_group071c1.review_frequency}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)?.perf_review_frequency ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.perf_review_frequency === valid_group071c1.review_frequency)?.perf_review_frequency
      }else{
        setdropdownValue([valid_group071c1.review_frequency])
        tempValue=valid_group071c1.review_frequency
      }   
    ...
    if(review_frequency7df15?.trigger == true)
    {
      await handlechange(tempValue)
      setreview_frequency7df15((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[valid_group071c1.review_frequency, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_reviewfrequencycombo_v1Props) && dfd_reviewfrequencycombo_v1Props?.length == 1){
    // setvalid_group071c1((pre:any)=>({...pre,review_frequency:dfd_reviewfrequencycombo_v1Props[0]?.review_frequency}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setvalid_group071c1((prev: any) => ({ ...prev, review_frequency: getMapperDetailsBindValues[value],REVIEW_FREQUENCY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setvalid_group071c1((prev: any) => ({ ...prev, review_frequency: '', review_frequency7df15: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,review_frequency:undefined}}));
  ...
  const valid_group071c1Ref = useRef<any>(valid_group071c1);
  useEffect(() => { valid_group071c1Ref.current = valid_group071c1; }, [valid_group071c1]);
    useEffect(()=>{
        handleBlur()
       const handler = (id:any) => {
          if (id === "5e66ffb2ca060bf6938ba82f5cd7df15") {
        handleClick(valid_group071c1Ref?.current?.review_frequency7df15?valid_group071c1Ref?.current?.review_frequency7df15:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "5e66ffb2ca060bf6938ba82f5cd7df15");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setvalid_group071c1((pre:any)=>({...pre,review_frequency:""}))
    else
      setInitialCount(1)
  },[review_frequency7df15?.refresh])
  

  if (review_frequency7df15?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        className=""    
        disabled= {review_frequency7df15?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={valid_group071c1?.review_frequency7df15 ? [valid_group071c1?.review_frequency7df15] : (valid_group071c1?.review_frequency ? dropdownValue : [])}
        validationState={validate?.newPerformanceCycle_v1?.review_frequency ? "invalid" : undefined}
        />
    </div>
  );
};
