...
const handleUpdate = async(date: any) => {
  ...
  if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,start_date: "invalid"}}))
    setvalid_group071c1((prev: any) => ({ ...prev, start_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,newPerformanceCycle_v1:{...pre?.newPerformanceCycle_v1,start_date:undefined}}));
  if (!date) {
    setvalid_group071c1((prev: any) => ({ ...prev, start_date: null }));
    return;
  }
  const now = new Date();
  ...
  setvalid_group071c1((prev: any) => ({ ...prev, start_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    //validation
      if(valid_group071c1?.start_date == "" || valid_group071c1?.start_date == undefined){
        const result = v.safeParse(schema, valid_group071c1?.start_date || '');
      if (!result.success) {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "bb72d69a4d634d67d79c83c9a072419e"
      );
...
useEffect(()=>{
 ...
 },[start_date2419e?.refresh])

useEffect(()=>{
  if(!valid_group071c1?.start_date){ 
...
return (
  ...
  style={{gridColumn: `1 / 13`,gridRow: `7 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={valid_group071c1?.start_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {start_date2419e?.isDisabled ? true : false}
      disabled= {start_date2419e?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Start Date"
      dateValidation=""
      validationState={validate?.newPerformanceCycle_v1?.start_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
