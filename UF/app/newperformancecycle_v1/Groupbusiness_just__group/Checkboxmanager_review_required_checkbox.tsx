...
const Checkboxmanager_review_required_checkbox = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "17b7184054b8c8c45f79659d3aa2a721"
      );
  ...
  useEffect(()=>{
    handleMapperValue()
  },[manager_review_required_checkbox2a721?.refresh])

  useEffect(()=>{
    if (business_just__group04cc1?.manager_review_required === undefined) {
      setbusiness_just__group04cc1((prev: any) => ({ ...prev, manager_review_required: true }));
    }
  },[])
  ...
  const handleChange=async(checked:boolean)=>{
    ...
    setbusiness_just__group04cc1((prev: any) => ({ ...prev, manager_review_required: checked}));
    }catch (err: any) {
  ...
  if (manager_review_required_checkbox2a721?.isHidden) {
    return <></>;
  }
  return (
    <div 
       style={{gridColumn: `1 / 11`,gridRow: `20 / 26`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Checkbox 
      className=""
      value={business_just__group04cc1?.manager_review_required||false}
      checked={business_just__group04cc1?.manager_review_required||true}
      disabled= {manager_review_required_checkbox2a721?.isDisabled ? true : false}
