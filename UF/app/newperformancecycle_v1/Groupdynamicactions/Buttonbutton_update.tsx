 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "cfa26e365fcf0fba487e19fa7cdefceb"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("button_update" in dynamicactionsd94d3Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsd94d3Props?.dynamicActionRule?.button_update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("button_update" in newperformancecycle_v1?.dynamicactions && newperformancecycle_v1?.dynamicactions["button_update"]?.itsHaveArtifact== true)
      {
        setShowFlag(newperformancecycle_v1?.dynamicactions["button_update"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newperformancecycle_v1?.dynamicactions?.button_update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "button_updateefceb") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "cfa26e365fcf0fba487e19fa7cdefceb") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "button_updateefceb");
    return () => {
  ...
  useEffect(()=>{
    if (!button_updateefceb?.trigger) return;
      if(button_updateefceb?.trigger){
      setbutton_updateefceb((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[button_updateefceb?.trigger])

  useEffect(()=>{
    if(button_updateefceb?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[button_updateefceb?.refresh])
  ...
  async function handleSavefceb_1_1_1(){
  ...
  const hasInvalidField = Object.values(currentValidate?.newPerformanceCycle_v1 || {}).some(val => {
    ...
    try{
      ...
      let tagetKey:string="CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1|f6b3a6d6e9d31efa79a6fbb3fc9fa214"
      let uf_getPFDetails:any={
        key: "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1|f6b3a6d6e9d31efa79a6fbb3fc9fa214"
      };
      let uf_ifo:any;
      let lockedKeysLength:number;
        if(dynamicactionsd94d3?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "cfa26e365fcf0fba487e19fa7cdefceb",
  "type": "button",
  "name": "button_update",
  "label": "button_update",
  "sequence": 1,
  "children": [
    {
      "id": "cfa26e365fcf0fba487e19fa7cdefceb.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "cfa26e365fcf0fba487e19fa7cdefceb.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "cfa26e365fcf0fba487e19fa7cdefceb.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "cfa26e365fcf0fba487e19fa7cdefceb.1.1.1.1.1",
          ...
          "targetKey": [
            "CK:CT006:FNGK:AF:FNK:PF-PFD:CATK:ECP:AFGK:HRM:AFK:addPerformanceCycleModify:AFVK:v1|f6b3a6d6e9d31efa79a6fbb3fc9fa214"
          ]
        }
      ]
    }
      ...
      te_eventEmitterBody = {
        ...
        upId : dynamicactionsd94d3?.upId? [dynamicactionsd94d3?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactionsd94d3?.upId? [dynamicactionsd94d3?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : dynamicactionsd94d3?.upId? [dynamicactionsd94d3?.upId ] : dynamicactionsd94d3?.trs_process_id? [dynamicactionsd94d3?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "button_update"
      }
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"cfa26e365fcf0fba487e19fa7cdefceb"
            };
            if (encryptionFlagCont) {
    ...
    te_eventEmitterBody["ssKey"] = dynamicactionsd94d3Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Details Updated', 'warning',true);
    // refreshElement
    //riseListen
    // for group
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsd94d3((prev: any) => ({ ...prev, button_update: true }));
        //onClick

    //eventEmitter
    await handleSavefceb_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsd94d3((prev: any) => ({ ...prev, button_update: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [newperformancecycle_v1?.dynamicactions?.button_update,dynamicactionsd94d3Props?.dynamicActionRule?.button_update,])

 if (button_updateefceb?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {button_updateefceb?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
