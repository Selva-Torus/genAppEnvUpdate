 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "6c102126589a6210eb6c23d5a0cbad27"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in dynamicactionsd94d3Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactionsd94d3Props?.dynamicActionRule?.cancel,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in newperformancecycle_v1?.dynamicactions && newperformancecycle_v1?.dynamicactions["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(newperformancecycle_v1?.dynamicactions["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(newperformancecycle_v1?.dynamicactions?.cancel?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "6c102126589a6210eb6c23d5a0cbad27") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "cancelbad27");
    return () => {
  ...
  useEffect(()=>{
    if (!cancelbad27?.trigger) return;
      if(cancelbad27?.trigger){
      setcancelbad27((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactionsd94d3((prev: any) => ({ ...prev, cancel: true }));
        //onClick

    // closeHandler   
    eventBus.emit('closeModal', 'newperformancecycle');
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactionsd94d3((prev: any) => ({ ...prev, cancel: false }));
  ...
  }, [newperformancecycle_v1?.dynamicactions?.cancel,dynamicactionsd94d3Props?.dynamicActionRule?.cancel,])

 if (cancelbad27?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='outlined'
          disabled= {cancelbad27?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
