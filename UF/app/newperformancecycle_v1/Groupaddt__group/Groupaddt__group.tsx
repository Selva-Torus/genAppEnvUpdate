...
import Textadditional_information  from "./Textadditional_information";
...
const Groupaddt__group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "34d877ccaf2e420faa7ffe9e3c9284f6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setaddt__group284f6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("additional_information")){
        setadditional_information858dc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(additional_information858dc?.isDisabled==null)
      {
        setadditional_information858dc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("add_inf_textarea")){
        setadd_inf_textarea3dd1e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(add_inf_textarea3dd1e?.isDisabled==null)
      {
        setadd_inf_textarea3dd1e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const addt__group284f6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    addt__group284f6Ref.current?.setSearchParams();
    addt__group284f6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(addt__group284f6) && Object.keys(addt__group284f6)?.length>0)
      {
        setaddt__group284f6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [addt__group284f6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setnewperformancecycle_v1((pre:any)=>({...pre,_selectedGroup_:"addt__group"}))
        }}
    >
          {allowedControls.includes("additional_information") ?<Textadditional_information   /* 858dc */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("add_inf_textarea") ?<TextAreaadd_inf_textarea   /* 3dd1e */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
    </div>
 )
}
