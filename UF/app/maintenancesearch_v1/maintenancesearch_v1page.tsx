...
export default function PageMaintenancesearchV1() {
  ...
  const {maintenancesearch_v1Props, setmaintenancesearch_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetmaintenance_v1DFD(pagination:any): Promise<void>{
        ...
        if(maintenancesearch_v1Props.length > 0){
          for(let i=0;i< maintenancesearch_v1Props.length;i++){
            if(maintenancesearch_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetMaintenance:AFVK:v1"){
              // delete maintenancesearch_v1Props[i].DFDkey;
              let temp=structuredClone(maintenancesearch_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setmaintenancesearch_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:maintenanceSearch:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setmaintenancesearch_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(maintenancesearch_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(maintenancesearch_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
