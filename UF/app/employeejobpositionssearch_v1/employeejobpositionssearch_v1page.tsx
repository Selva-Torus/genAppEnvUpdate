...
export default function PageEmployeejobpositionssearchV1() {
  ...
  const {employeejobpositionssearch_v1, setemployeejobpositionssearch_v1} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function jobpositions_v1DFD(pagination:any): Promise<void>{
        ...
        if(employeejobpositionssearch_v1Props.length > 0){
          for(let i=0;i< employeejobpositionssearch_v1Props.length;i++){
            if(employeejobpositionssearch_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:jobPositions:AFVK:v1"){
              // delete employeejobpositionssearch_v1Props[i].DFDkey;
              let temp=structuredClone(employeejobpositionssearch_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setemployeejobpositionssearch_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const { fetchBatchData } = await import("../utils/Orchestration");
    const data: any = await fetchBatchData(
      'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobPositionsSearch:AFVK:v1',
      [user],
      'pageEmployeejobpositionssearchV1',
      token
    )
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:employeeJobPositionsSearch:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setemployeejobpositionssearch_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(employeejobpositionssearch_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(employeejobpositionssearch_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setemployeejobpositionssearch_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
