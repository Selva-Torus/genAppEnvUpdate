...
const Grouprtgs_lst_doc_list_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "50633997eba74d5a8b6e1d00b0e32147");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("file_name_rtgs_list")){
        setfile_name_rtgs_list06cd7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(file_name_rtgs_list06cd7?.isDisabled==null)
      {
        setfile_name_rtgs_list06cd7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("action_rtgs_list")){
        setaction_rtgs_listcf67e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(action_rtgs_listcf67e?.isDisabled==null)
      {
        setaction_rtgs_listcf67e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const rtgs_lst_doc_list_table32147Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_lst_doc_list_table32147Ref.current?.setSearchParams();
    rtgs_lst_doc_list_table32147Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_lst_doc_list_table32147) && Object.keys(rtgs_lst_doc_list_table32147)?.length>0)
      {
        setrtgs_lst_doc_list_table32147({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_lst_doc_list_table32147Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_lst_doc_list_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_lst_doc_list_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_lst_doc_list_table32147Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
