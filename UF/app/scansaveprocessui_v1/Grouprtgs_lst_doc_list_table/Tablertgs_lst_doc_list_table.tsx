...
let defaultColumns:any = [
  {
    ...
    "nodeid": "1611ed4193cb46519c61eade31706cd7",
    "name": "File Name",
    "meta": {
      "sort": true
    },
...
const Tablertgs_lst_doc_list_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const tableName = "tam_app_attachment"
 ...
 /////////////
  ...
  const {file_name_rtgs_list06cd7, setfile_name_rtgs_list06cd7}= useContext(TotalContext) as TotalContextProps  
  const {action_rtgs_listcf67e, setaction_rtgs_listcf67e}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "50633997eba74d5a8b6e1d00b0e32147",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_lst_doc_list_table32147 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_lst_doc_list_table32147 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_lst_doc_list_table32147 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.attachment_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      rtgs_lst_doc_list_table32147.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.attachment_id){
          selectedData?.push(item)
          postIds.push(item.attachment_id)
      ...
      //////////
      //////////
        setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
    else if(needLockingAndRule.lockMode==='Multi'){
      // its for ui level selected list show for multi select
      rtgs_lst_doc_list_table32147.filter((item:any,id:number)=>{
        if (ids.includes(item.attachment_id)){
          selectedData?.push(item)
          postIds.push(item.attachment_id) 
          processIds.push(item?.trs_process_id)
        } 
      ...
      setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['file_name_rtgs_list'] = file_name_rtgs_list06cd7,
        codeStates['setfile_name_rtgs_list'] = setfile_name_rtgs_list06cd7,
        codeStates['action_rtgs_list'] = action_rtgs_listcf67e,
        codeStates['setaction_rtgs_list'] = setaction_rtgs_listcf67e,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_lst_doc_list_table32147Props?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_lst_doc_list_table32147Props?.presetValues || Object.keys(rtgs_lst_doc_list_table32147Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '50633997eba74d5a8b6e1d00b0e32147',
                elementId: '50633997eba74d5a8b6e1d00b0e32147'
              },
            }
          }else
          {
      ...
      if (api_pagination?.data?.records.length > 0) {
        ...
        result.attachment_id = emp?.attachment_id;
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_lst_doc_list_table32147(api_pagination?.data?.records||[])
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,attachment_id:filtertedData[i]?.attachment_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.attachment_id==item?.attachment_id)
        {
          index=i
          row=data
        }
    ...
    if(await handleSecurity("action_rtgs_list") &&nodeName?.toLowerCase()=="buttonaction_rtgs_list"){
      let tempLockedData:any = 
      {
        ...
        data:filteredData,
        primaryKeys:[filteredData?.attachment_id],
        lockMode: needLockingAndRule,
        ttl: needLockingAndRule?.ttl
      }
      return (
        <React.Fragment>
        <Buttonaction_rtgs_list mainData={filteredData} lockedData={tempLockedData} controlData={controlData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} onSelectLock={setLockMode} rowIndex={index} currentSelectedIds={rtgs_lst_doc_list_table32147Props?.selectedIds} tableName={tableName}/>
        </React.Fragment>
      )
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_lst_doc_list_table32147))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_lst_doc_list_table32147Props.filterInitalLoad) return;

    const filterControllers = rtgs_lst_doc_list_table32147Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_lst_doc_list_table32147']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_lst_doc_list_table32147']).filter((k: string) => jsonEntry['rtgs_lst_doc_list_table32147'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_lst_doc_list_table32147Props.filterProps);
      return;
    }
    ...
    setrtgs_lst_doc_list_table32147Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_lst_doc_list_table32147Props.filterProps);

  }, [
    rtgs_lst_doc_list_table32147Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_lst_doc_list_table32147Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_lst_doc_list_table32147Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_lst_doc_list_table32147Props.searchFilter) || {};
...
}, [rtgs_lst_doc_list_table32147Props.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_lst_doc_list_table32147Props?.clearData === true) {
      setrtgs_lst_doc_list_table32147([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_lst_doc_list_table32147Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_lst_doc_list_table32147Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_lst_doc_list_table32147Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_lst_doc_list_table32147Props?.selectedIds[rtgs_lst_doc_list_table32147Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_lst_doc_list_table32147) && rtgs_lst_doc_list_table32147.length>0)
    {
      let data = rtgs_lst_doc_list_table32147.find((data:any)=>(data?.attachment_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "attachment_id",
  ...
  useEffect(() => {
    if (rtgs_lst_doc_list_table32147Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_lst_doc_list_table32147Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_lst_doc_list_table32147Props?.selectedIds)
    }
  }, [rtgs_lst_doc_list_table32147Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.attachment_id==data?.attachment_id)
        {
          index=i
          row=item
        }
    ...
    setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre, selectedIds:[row?.attachment_id]}))  
    onButtonSecurityHandle(data)
    let copyFormhandlerData :any = {}
          //bindtran
    const pkCol = translatedColumns.find((col:any) => col.dfdName === "attachment_id"); // Find the column whose dfdName matches
    const result = allData.find((item:any) => item["attachment_id"] === data[pkCol?.id || "attachment_id"]);
          let bindData2 = filterByKeys(flattenKeepInner(allData[index]),overallgroup01c61Props?.controls);
          setoverallgroup01c61((pre:any)=>({...pre, ...bindData2}))
          setoverallgroup01c61({...overallgroup01c61,presetValues:allData[index]})
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_lst_doc_list_table32147?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_lst_doc_list_table32147?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_lst_doc_list_table32147?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_lst_doc_list_table32147?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_lst_doc_list_table32147?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_lst_doc_list_table32147?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_lst_doc_list_table32147?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_lst_doc_list_table32147?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"50633997eba74d5a8b6e1d00b0e32147"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_lst_doc_list_table32147Props?.selectedIds}  
