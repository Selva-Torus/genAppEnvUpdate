...
const Grouprtgs_list_tble_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1eddc2d9a7f5498bbaad6d1ad85ab24b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_tble_groupab24bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_table")){
        setrtgs_list_table2926aProps((pre:any)=>({...pre,...rtgs_list_table2926a,isDisabled:true}));

    }else
    {
      if(rtgs_list_table2926a?.isDisabled==null)
      {
        setrtgs_list_table2926aProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const rtgs_list_tble_groupab24bRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_tble_groupab24bRef.current?.setSearchParams();
    rtgs_list_tble_groupab24bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_tble_groupab24b) && Object.keys(rtgs_list_tble_groupab24b)?.length>0)
      {
        setrtgs_list_tble_groupab24b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_tble_groupab24bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_tble_group"}))
        }}
    >
...
export default Grouprtgs_list_tble_group
