 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("commoninfo")){
        setcommoninfof4607((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(commoninfof4607?.isDisabled==null)
      {
        setcommoninfof4607((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("basicinfo")){
        setbasicinfo3d198((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(basicinfo3d198?.isDisabled==null)
      {
        setbasicinfo3d198((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("additionalinfo")){
        setadditionalinfod2894((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(additionalinfod2894?.isDisabled==null)
      {
        setadditionalinfod2894((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("listgroup")){
        setlistgroupdcdbd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(listgroupdcdbd?.isDisabled==null)
      {
        setlistgroupdcdbd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"allcontrols"}))
        }}
    >
