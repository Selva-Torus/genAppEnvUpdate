 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_grp")){
        setrtgs_list_grpcf7d8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_grpcf7d8?.isDisabled==null)
      {
        setrtgs_list_grpcf7d8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(control_tab_groupbc3e2?.control_tab_group === "rtgs_list"){
    handleOnChange()
    }
  },[control_tab_groupbc3e2?.control_tab_group])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list"}))
        }}
    >
