...
const Groupcmnt_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "aea4f243bf904f81aef727eddfde161c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcmnt_liste161cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_cmnts_list")){
        setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre,...rtgs_list_cmnts_list2148d,isDisabled:true}));

    }else
    {
      if(rtgs_list_cmnts_list2148d?.isDisabled==null)
      {
        setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(rtgs_list_tab_grp28533?.rtgs_list_tab_grp === "cmnt_list"){
    handleOnChange()
    }
  ...
  const cmnt_liste161cRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmnt_liste161cRef.current?.setSearchParams();
    cmnt_liste161cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmnt_liste161c) && Object.keys(cmnt_liste161c)?.length>0)
      {
        setcmnt_liste161c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmnt_liste161cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"cmnt_list"}))
        }}
    >
