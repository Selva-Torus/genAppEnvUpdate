 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("control_tab_group")){
        setcontrol_tab_groupbc3e2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(control_tab_groupbc3e2?.isDisabled==null)
      {
        setcontrol_tab_groupbc3e2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"controlgroup"}))
        }}
    >
