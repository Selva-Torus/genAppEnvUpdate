 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vldcode")){
        setvldcode0c0ce((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vldcode0c0ce?.isDisabled==null)
      {
        setvldcode0c0ce((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vldreason")){
        setvldreason2ef16((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vldreason2ef16?.isDisabled==null)
      {
        setvldreason2ef16((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"valdnlisttable"}))
        }}
    >
