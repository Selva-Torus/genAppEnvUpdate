...
const TablevaldnListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, valdnlisttable17ec7 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, valdnlisttable17ec7 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, valdnlisttable17ec7 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      valdnlisttable17ec7.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setvaldnlisttable17ec7Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setvaldnlisttable17ec7Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:valdnlisttable17ec7Props?.presetValues?.vgphstm_uuid}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (valdnlisttable17ec7Props.filterInitalLoad) return;

    const filterControllers = valdnlisttable17ec7Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['valdnlisttable17ec7']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['valdnlisttable17ec7']).filter((k: string) => jsonEntry['valdnlisttable17ec7'][k] === false)
      : [];
    ...
    setvaldnlisttable17ec7Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(valdnlisttable17ec7Props.filterProps);

  }, [
    valdnlisttable17ec7Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(valdnlisttable17ec7Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(valdnlisttable17ec7Props.searchFilter) || {};
...
}, [valdnlisttable17ec7Props.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = valdnlisttable17ec7.find((data:any)=>(data?.vgphtelm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
