 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_tble_group")){
        setrtgs_list_tble_groupab24b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_tble_groupab24b?.isDisabled==null)
      {
        setrtgs_list_tble_groupab24b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group")){
        setgroup05462((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group05462?.isDisabled==null)
      {
        setgroup05462((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_grp"}))
        }}
    >
