 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts")){
        setcmnts11ffa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cmnts11ffa?.isDisabled==null)
      {
        setcmnts11ffa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"cmntlisttable"}))
        }}
    >
