...
const TablecmntListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, cmntlisttable02d0e })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, cmntlisttable02d0e }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, cmntlisttable02d0e } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      cmntlisttable02d0e.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setcmntlisttable02d0eProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setcmntlisttable02d0eProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:cmntlisttable02d0eProps?.presetValues?.vgphstm_uuid}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(cmntlisttable02d0e))
    {
      GetTableDetails()
    }
  }, [cmntlisttable02d0e])
  ...
  useEffect(() => {
    ...
    if (cmntlisttable02d0eProps.filterInitalLoad) return;

    const filterControllers = cmntlisttable02d0eProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['cmntlisttable02d0e']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['cmntlisttable02d0e']).filter((k: string) => jsonEntry['cmntlisttable02d0e'][k] === false)
      : [];
    ...
    setcmntlisttable02d0eProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(cmntlisttable02d0eProps.filterProps);

  }, [
    cmntlisttable02d0eProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(cmntlisttable02d0eProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(cmntlisttable02d0eProps.searchFilter) || {};
...
}, [cmntlisttable02d0eProps.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = cmntlisttable02d0e.find((data:any)=>(data?.vgphtlm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtlm_id",
