...
const Groupdocumnt_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "eb850f94adb8418393c4c61a9603a31d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdocumnt_list3a31dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_lst_doc_list_table")){
        setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre,...rtgs_lst_doc_list_table32147,isDisabled:true}));

    }else
    {
      if(rtgs_lst_doc_list_table32147?.isDisabled==null)
      {
        setrtgs_lst_doc_list_table32147Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(rtgs_list_tab_grp28533?.rtgs_list_tab_grp === "documnt_list"){
    handleOnChange()
    }
  ...
  const documnt_list3a31dRef = useRef<any>(null);
  const handleClearSearch = () => {
    documnt_list3a31dRef.current?.setSearchParams();
    documnt_list3a31dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(documnt_list3a31d) && Object.keys(documnt_list3a31d)?.length>0)
      {
        setdocumnt_list3a31d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [documnt_list3a31dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"documnt_list"}))
        }}
    >
