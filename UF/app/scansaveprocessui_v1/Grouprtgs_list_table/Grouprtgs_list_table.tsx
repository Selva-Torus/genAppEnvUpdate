...
const Grouprtgs_list_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a736657afc324e0089b1e7744632926a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_table2926aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tran_id")){
        settran_id5f12f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tran_id5f12f?.isDisabled==null)
      {
        settran_id5f12f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_acnt_no")){
        setdr_acnt_no469c1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_acnt_no469c1?.isDisabled==null)
      {
        setdr_acnt_no469c1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_acnt_no")){
        setcr_acnt_nocb409((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_acnt_nocb409?.isDisabled==null)
      {
        setcr_acnt_nocb409((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("amnt")){
        setamntef7a4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(amntef7a4?.isDisabled==null)
      {
        setamntef7a4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_code")){
        setcr_bank_code24beb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_bank_code24beb?.isDisabled==null)
      {
        setcr_bank_code24beb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("created_by")){
        setcreated_bye14cd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(created_bye14cd?.isDisabled==null)
      {
        setcreated_bye14cd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("created_date")){
        setcreated_date14669((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(created_date14669?.isDisabled==null)
      {
        setcreated_date14669((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const rtgs_list_table2926aRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_table2926aRef.current?.setSearchParams();
    rtgs_list_table2926aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_table2926a) && Object.keys(rtgs_list_table2926a)?.length>0)
      {
        setrtgs_list_table2926a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_table2926aProps?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-1 pr-1 pl-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_list_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_table2926aRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
