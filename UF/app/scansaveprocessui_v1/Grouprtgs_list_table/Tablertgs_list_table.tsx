...
let defaultColumns:any = [
  {
    "id": "tran_id",
    "nodeid": "e2eec2717ed64a7ab5228f35ccf5f12f",
    "name": "Tran ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_acnt_no",
    "nodeid": "e1942377ef1946adb5fbc18ea7d469c1",
    "name": "Dr Account No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_acnt_no",
    "nodeid": "b1f631040e6943d2806e28e8efecb409",
    "name": "Cr Account No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "amnt",
    "nodeid": "ec5a857a786744c9bb97ba24889ef7a4",
    "name": "Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_bank_code",
    "nodeid": "0dbc0255d8604c7eb2c8741258e24beb",
    "name": "Cr Bank Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "created_by",
    "nodeid": "dca0608b363646278066083f132e14cd",
    "name": "Created By",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "created_date",
    "nodeid": "d21448ee80fc4879adc9c9f970b14669",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {tran_id5f12f, settran_id5f12f}= useContext(TotalContext) as TotalContextProps  
  const {dr_acnt_no469c1, setdr_acnt_no469c1}= useContext(TotalContext) as TotalContextProps  
  const {cr_acnt_nocb409, setcr_acnt_nocb409}= useContext(TotalContext) as TotalContextProps  
  const {amntef7a4, setamntef7a4}= useContext(TotalContext) as TotalContextProps  
  const {cr_bank_code24beb, setcr_bank_code24beb}= useContext(TotalContext) as TotalContextProps  
  const {created_bye14cd, setcreated_bye14cd}= useContext(TotalContext) as TotalContextProps  
  const {created_date14669, setcreated_date14669}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "a736657afc324e0089b1e7744632926a",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_table2926a })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_table2926a }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_table2926a } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_table2926aProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      rtgs_list_table2926a.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setrtgs_list_table2926aProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setrtgs_list_table2926aProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['tran_id'] = tran_id5f12f,
        codeStates['settran_id'] = settran_id5f12f,
        codeStates['dr_acnt_no'] = dr_acnt_no469c1,
        codeStates['setdr_acnt_no'] = setdr_acnt_no469c1,
        codeStates['cr_acnt_no'] = cr_acnt_nocb409,
        codeStates['setcr_acnt_no'] = setcr_acnt_nocb409,
        codeStates['amnt'] = amntef7a4,
        codeStates['setamnt'] = setamntef7a4,
        codeStates['cr_bank_code'] = cr_bank_code24beb,
        codeStates['setcr_bank_code'] = setcr_bank_code24beb,
        codeStates['created_by'] = created_bye14cd,
        codeStates['setcreated_by'] = setcreated_bye14cd,
        codeStates['created_date'] = created_date14669,
        codeStates['setcreated_date'] = setcreated_date14669,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_table2926aProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_table2926aProps?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_table2926aProps?.presetValues || Object.keys(rtgs_list_table2926aProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'a736657afc324e0089b1e7744632926a',
                elementId: 'a736657afc324e0089b1e7744632926a'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_table2926a(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_table2926a))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_table2926aProps.filterInitalLoad) return;

    const filterControllers = rtgs_list_table2926aProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_table2926a']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_table2926a']).filter((k: string) => jsonEntry['rtgs_list_table2926a'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_table2926aProps.filterProps);
      return;
    }
    ...
    setrtgs_list_table2926aProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_table2926aProps.filterProps);

  }, [
    rtgs_list_table2926aProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_table2926aProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_table2926aProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_table2926aProps.searchFilter) || {};
...
}, [rtgs_list_table2926aProps.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_table2926aProps?.clearData === true) {
      setrtgs_list_table2926a([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_table2926aProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_table2926aProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_table2926aProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_table2926aProps?.selectedIds[rtgs_list_table2926aProps?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_table2926a) && rtgs_list_table2926a.length>0)
    {
      let data = rtgs_list_table2926a.find((data:any)=>(data?.vgphstm_uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_uuid",
  ...
  useEffect(() => {
    if (rtgs_list_table2926aProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_table2926aProps?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_table2926aProps?.selectedIds)
    }
  }, [rtgs_list_table2926aProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_table2926a?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_table2926a?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_table2926a?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_table2926a?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_table2926a?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_table2926a?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_table2926a?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_table2926a?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"a736657afc324e0089b1e7744632926a"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_table2926aProps?.selectedIds}  
