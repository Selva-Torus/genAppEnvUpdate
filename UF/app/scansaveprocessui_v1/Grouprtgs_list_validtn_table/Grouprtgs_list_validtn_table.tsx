...
const Grouprtgs_list_validtn_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a296019fb2344177ad1369a2c3e84666");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_validtn_table84666Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vld_code_rtgs_lst")){
        setvld_code_rtgs_lsta5e1f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vld_code_rtgs_lsta5e1f?.isDisabled==null)
      {
        setvld_code_rtgs_lsta5e1f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vld_reason_rtgs_list")){
        setvld_reason_rtgs_listdd73b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vld_reason_rtgs_listdd73b?.isDisabled==null)
      {
        setvld_reason_rtgs_listdd73b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const rtgs_list_validtn_table84666Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_validtn_table84666Ref.current?.setSearchParams();
    rtgs_list_validtn_table84666Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_validtn_table84666) && Object.keys(rtgs_list_validtn_table84666)?.length>0)
      {
        setrtgs_list_validtn_table84666({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_validtn_table84666Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_validtn_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_list_validtn_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_validtn_table84666Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
