...
let defaultColumns:any = [
  {
    ...
    "nodeid": "01e9267e04ef4984a5f9b2088baa5e1f",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "b22ef5c952e549d480ab8286b11dd73b",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_validtn_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const tableName = "vgph_tran_error_log_main"
 ...
 /////////////
  ...
  const {vld_code_rtgs_lsta5e1f, setvld_code_rtgs_lsta5e1f}= useContext(TotalContext) as TotalContextProps  
  const {vld_reason_rtgs_listdd73b, setvld_reason_rtgs_listdd73b}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "a296019fb2344177ad1369a2c3e84666",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_validtn_table84666 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_validtn_table84666 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_validtn_table84666 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_validtn_table84666Props((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.vgphtelm_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      rtgs_list_validtn_table84666.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.vgphtelm_id){
          selectedData?.push(item)
          postIds.push(item.vgphtelm_id)
      ...
      //////////
      //////////
        setrtgs_list_validtn_table84666Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
    else if(needLockingAndRule.lockMode==='Multi'){
      // its for ui level selected list show for multi select
      rtgs_list_validtn_table84666.filter((item:any,id:number)=>{
        if (ids.includes(item.vgphtelm_id)){
          selectedData?.push(item)
          postIds.push(item.vgphtelm_id) 
          processIds.push(item?.trs_process_id)
        } 
      ...
      setrtgs_list_validtn_table84666Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['vld_code_rtgs_lst'] = vld_code_rtgs_lsta5e1f,
        codeStates['setvld_code_rtgs_lst'] = setvld_code_rtgs_lsta5e1f,
        codeStates['vld_reason_rtgs_list'] = vld_reason_rtgs_listdd73b,
        codeStates['setvld_reason_rtgs_list'] = setvld_reason_rtgs_listdd73b,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_validtn_table84666Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_validtn_table84666Props?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_validtn_table84666Props?.presetValues || Object.keys(rtgs_list_validtn_table84666Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'a296019fb2344177ad1369a2c3e84666',
                elementId: 'a296019fb2344177ad1369a2c3e84666'
              },
            }
          }else
          {
      ...
      if (api_pagination?.data?.records.length > 0) {
        ...
        result.vgphtelm_id = emp?.vgphtelm_id;
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_validtn_table84666(api_pagination?.data?.records||[])
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,vgphtelm_id:filtertedData[i]?.vgphtelm_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.vgphtelm_id==item?.vgphtelm_id)
        {
          index=i
          row=data
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_validtn_table84666))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_validtn_table84666Props.filterInitalLoad) return;

    const filterControllers = rtgs_list_validtn_table84666Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_validtn_table84666']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_validtn_table84666']).filter((k: string) => jsonEntry['rtgs_list_validtn_table84666'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_validtn_table84666Props.filterProps);
      return;
    }
    ...
    setrtgs_list_validtn_table84666Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_validtn_table84666Props.filterProps);

  }, [
    rtgs_list_validtn_table84666Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_validtn_table84666Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_validtn_table84666Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_validtn_table84666Props.searchFilter) || {};
...
}, [rtgs_list_validtn_table84666Props.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table84666Props?.clearData === true) {
      setrtgs_list_validtn_table84666([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_validtn_table84666Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_validtn_table84666Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_validtn_table84666Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_validtn_table84666Props?.selectedIds[rtgs_list_validtn_table84666Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_validtn_table84666) && rtgs_list_validtn_table84666.length>0)
    {
      let data = rtgs_list_validtn_table84666.find((data:any)=>(data?.vgphtelm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table84666Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_validtn_table84666Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_validtn_table84666Props?.selectedIds)
    }
  }, [rtgs_list_validtn_table84666Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.vgphtelm_id==data?.vgphtelm_id)
        {
          index=i
          row=item
        }
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_validtn_table84666?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_validtn_table84666?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_validtn_table84666?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table84666?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_validtn_table84666?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_validtn_table84666?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table84666?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_validtn_table84666?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"a296019fb2344177ad1369a2c3e84666"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_validtn_table84666Props?.selectedIds}  
