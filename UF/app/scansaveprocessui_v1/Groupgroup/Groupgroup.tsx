...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "393f83491c0a4f20afff9716a2805462");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup05462Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_tab_grp")){
        setrtgs_list_tab_grp28533((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_tab_grp28533?.isDisabled==null)
      {
        setrtgs_list_tab_grp28533((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const group05462Ref = useRef<any>(null);
  const handleClearSearch = () => {
    group05462Ref.current?.setSearchParams();
    group05462Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group05462) && Object.keys(group05462)?.length>0)
      {
        setgroup05462({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group05462Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
