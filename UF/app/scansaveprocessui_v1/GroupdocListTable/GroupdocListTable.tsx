 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("filename")){
        setfilename7c104((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(filename7c104?.isDisabled==null)
      {
        setfilename7c104((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("action")){
        setactionf530a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(actionf530a?.isDisabled==null)
      {
        setactionf530a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"doclisttable"}))
        }}
    >
