...
const TabledocListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, doclisttable56e97 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, doclisttable56e97 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, doclisttable56e97 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      doclisttable56e97.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setdoclisttable56e97Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setdoclisttable56e97Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:doclisttable56e97Props?.presetValues?.vgphstm_uuid}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(await handleSecurity("action") &&nodeName?.toLowerCase()=="buttonaction"){
      ...
      return (
        <React.Fragment>
        <Buttonaction mainData={filteredData} lockedData={tempLockedData} controlData={controlData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} onSelectLock={setLockMode} rowIndex={index} currentSelectedIds={doclisttable56e97Props?.selectedIds} tableName={tableName}/>
        </React.Fragment>
      )
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (doclisttable56e97Props.filterInitalLoad) return;

    const filterControllers = doclisttable56e97Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['doclisttable56e97']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['doclisttable56e97']).filter((k: string) => jsonEntry['doclisttable56e97'][k] === false)
      : [];
    ...
    setdoclisttable56e97Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(doclisttable56e97Props.filterProps);

  }, [
    doclisttable56e97Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(doclisttable56e97Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(doclisttable56e97Props.searchFilter) || {};
...
}, [doclisttable56e97Props.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = doclisttable56e97.find((data:any)=>(data?.attachment_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "attachment_id",
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setdoclisttable56e97Props((pre:any)=>({...pre, selectedIds:[row?.attachment_id]}))  
    onButtonSecurityHandle(data)
