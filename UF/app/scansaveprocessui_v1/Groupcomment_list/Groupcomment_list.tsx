 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmntlisttable")){
        setcmntlisttable02d0eProps((pre:any)=>({...pre,...cmntlisttable02d0e,isDisabled:true}));

    }else
    {
      if(cmntlisttable02d0e?.isDisabled==null)
      {
        setcmntlisttable02d0eProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(list_tab_groupd6905?.list_tab_group === "comment_list"){
    handleOnChange()
    }
  },[list_tab_groupd6905?.list_tab_group])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"comment_list"}))
        }}
    >
