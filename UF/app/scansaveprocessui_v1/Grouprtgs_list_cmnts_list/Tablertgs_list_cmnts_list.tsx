...
let defaultColumns:any = [
  {
    "id": "cmnts_rtgs_list",
    "nodeid": "b1172d9aa1c24944aa0225792ceee03b",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_cmnts_list = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {cmnts_rtgs_listee03b, setcmnts_rtgs_listee03b}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "d8f270b65a4145dc8f2f910ce942148d",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list2148d })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list2148d }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list2148d } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      rtgs_list_cmnts_list2148d.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['cmnts_rtgs_list'] = cmnts_rtgs_listee03b,
        codeStates['setcmnts_rtgs_list'] = setcmnts_rtgs_listee03b,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_cmnts_list2148dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_cmnts_list2148dProps?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_cmnts_list2148dProps?.presetValues || Object.keys(rtgs_list_cmnts_list2148dProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'd8f270b65a4145dc8f2f910ce942148d',
                elementId: 'd8f270b65a4145dc8f2f910ce942148d'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_cmnts_list2148d(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_cmnts_list2148d))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_cmnts_list2148dProps.filterInitalLoad) return;

    const filterControllers = rtgs_list_cmnts_list2148dProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_cmnts_list2148d']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_cmnts_list2148d']).filter((k: string) => jsonEntry['rtgs_list_cmnts_list2148d'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_cmnts_list2148dProps.filterProps);
      return;
    }
    ...
    setrtgs_list_cmnts_list2148dProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_cmnts_list2148dProps.filterProps);

  }, [
    rtgs_list_cmnts_list2148dProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_cmnts_list2148dProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_cmnts_list2148dProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_cmnts_list2148dProps.searchFilter) || {};
...
}, [rtgs_list_cmnts_list2148dProps.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_cmnts_list2148dProps?.clearData === true) {
      setrtgs_list_cmnts_list2148d([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_cmnts_list2148dProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_cmnts_list2148dProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_cmnts_list2148dProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_cmnts_list2148dProps?.selectedIds[rtgs_list_cmnts_list2148dProps?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_cmnts_list2148d) && rtgs_list_cmnts_list2148d.length>0)
    {
      let data = rtgs_list_cmnts_list2148d.find((data:any)=>(data?.vgphtlm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtlm_id",
  ...
  useEffect(() => {
    if (rtgs_list_cmnts_list2148dProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_cmnts_list2148dProps?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_cmnts_list2148dProps?.selectedIds)
    }
  }, [rtgs_list_cmnts_list2148dProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_cmnts_list2148d?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_cmnts_list2148d?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_cmnts_list2148d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list2148d?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_cmnts_list2148d?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_cmnts_list2148d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list2148d?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_cmnts_list2148d?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"d8f270b65a4145dc8f2f910ce942148d"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_cmnts_list2148dProps?.selectedIds}  
