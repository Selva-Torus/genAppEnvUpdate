 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("common_info")){
        setcommon_info3a458((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(common_info3a458?.isDisabled==null)
      {
        setcommon_info3a458((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account")){
        setdr_account27abb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account27abb?.isDisabled==null)
      {
        setdr_account27abb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name")){
        setdr_name84266((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_name84266?.isDisabled==null)
      {
        setdr_name84266((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("base_currency")){
        setbase_currencyb386d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(base_currencyb386d?.isDisabled==null)
      {
        setbase_currencyb386d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_cust_ac_sanc_lmt")){
        setdr_cust_ac_sanc_lmtb74f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_cust_ac_sanc_lmtb74f7?.isDisabled==null)
      {
        setdr_cust_ac_sanc_lmtb74f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_cust_ac_balance")){
        setdr_cust_ac_balance753dd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_cust_ac_balance753dd?.isDisabled==null)
      {
        setdr_cust_ac_balance753dd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-3 pr-4 pl-4 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"commoninfo"}))
        }}
    >
