...
const TextInputdr_cust_ac_balance = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|ff81fb16b4444da4b89336717f6753dd"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_cust_ac_balance',type:"number"})
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "ff81fb16b4444da4b89336717f6753dd") {
        handleChange({target:{value:commoninfof4607Ref?.current?.dr_cust_ac_balance||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "ff81fb16b4444da4b89336717f6753dd") {
        handleBlur({target:{value:commoninfof4607Ref?.current?.dr_cust_ac_balance||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setcommoninfof4607((pre:any)=>({...pre,dr_cust_ac_balance:dfd_scansaveprocessdfd_v1Props.data[0]?.dr_cust_ac_balance}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `22 / 34`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
