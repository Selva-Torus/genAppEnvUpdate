 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("scan" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.scan,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("scan" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["scan"]?.itsHaveArtifact== true)
      {
        ...
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.scan?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "81fd4695b5644dd8b8863c6eb6a31ce1") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "scan31ce1");
    return () => {
  ...
  }, [scansaveprocessui_v1?.button_group?.scan,button_group74f3eProps?.dynamicActionRule?.scan,control_tab_groupbc3e2])
