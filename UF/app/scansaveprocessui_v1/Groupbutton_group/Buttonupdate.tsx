 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("update" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.update,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("update" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["update"]?.itsHaveArtifact== true)
      {
        ...
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.update?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "f15c003da4df4213bd3f5535607ed7a9") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "updateed7a9");
    return () => {
  ...
  }, [scansaveprocessui_v1?.button_group?.update,button_group74f3eProps?.dynamicActionRule?.update,control_tab_groupbc3e2])
