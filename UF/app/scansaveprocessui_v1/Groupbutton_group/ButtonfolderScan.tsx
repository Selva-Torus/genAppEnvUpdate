 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("folderscan" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.folderscan,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("folderscan" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["folderscan"]?.itsHaveArtifact== true)
      {
        ...
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.folderscan?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "8561ae7dd8c84f67ba2cb9e7fa4f14e0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "folderscanf14e0");
    return () => {
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    setoverallgroup01c61((pre:any)=>({...pre,[UOmapperData['9b8e4edd435448ab98f68a59ea29df1d']['source']]:imgData4}));
  ...
  }, [scansaveprocessui_v1?.button_group?.folderscan,button_group74f3eProps?.dynamicActionRule?.folderscan,control_tab_groupbc3e2])
