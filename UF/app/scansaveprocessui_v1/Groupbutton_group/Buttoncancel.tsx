 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.cancel,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["cancel"]?.itsHaveArtifact== true)
      {
        ...
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.cancel?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "1606eaecab464786b4cc3669d5a2bf72") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "cancel2bf72");
    return () => {
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    let filterProps12: any =  [];
      let filterData12 = await getFilterProps(filterProps12,{...overallgroup01c61,...controlgroupda197,...control_tab_groupbc3e2,...rtgs_infofd0aa,...allcontrols71c54,...commoninfof4607,...basicinfo3d198,...additionalinfod2894,...listgroupdcdbd,...list_tab_groupd6905,...document_list38c6e,...validation_listae827,...comment_list72944,...rtgs_lista0a19,...rtgs_list_grpcf7d8,...rtgs_list_tble_groupab24b,...group05462,...rtgs_list_tab_grp28533,...documnt_list3a31d,...validtn_list10f93,...cmnt_liste161c,...button_group74f3e});
    ...
    setrtgs_list_table2926aProps((pre:any) => ({
      ...pre,
      clearData: true,  // Flag to clear allDataObject without re-fetching
      selectedIds: []
    }));
    ...
    setrtgs_lst_doc_list_table32147Props((pre:any) => ({
      ...pre,
      clearData: true,  // Flag to clear allDataObject without re-fetching
      selectedIds: []
    }));
    ...
    setrtgs_list_validtn_table84666Props((pre:any) => ({
      ...pre,
      clearData: true,  // Flag to clear allDataObject without re-fetching
      selectedIds: []
    }));
    ...
    setrtgs_list_cmnts_list2148dProps((pre:any) => ({
      ...pre,
      clearData: true,  // Flag to clear allDataObject without re-fetching
      selectedIds: []
    }));
  ...
  }, [scansaveprocessui_v1?.button_group?.cancel,button_group74f3eProps?.dynamicActionRule?.cancel,control_tab_groupbc3e2])
