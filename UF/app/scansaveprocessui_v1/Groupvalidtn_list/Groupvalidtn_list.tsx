...
const Groupvalidtn_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "3d257ea6aa0c428d9edc620736710f93");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalidtn_list10f93Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_validtn_table")){
        setrtgs_list_validtn_table84666Props((pre:any)=>({...pre,...rtgs_list_validtn_table84666,isDisabled:true}));

    }else
    {
      if(rtgs_list_validtn_table84666?.isDisabled==null)
      {
        setrtgs_list_validtn_table84666Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(rtgs_list_tab_grp28533?.rtgs_list_tab_grp === "validtn_list"){
    handleOnChange()
    }
  },[rtgs_list_tab_grp28533?.rtgs_list_tab_grp])
  ...
  const validtn_list10f93Ref = useRef<any>(null);
  const handleClearSearch = () => {
    validtn_list10f93Ref.current?.setSearchParams();
    validtn_list10f93Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validtn_list10f93) && Object.keys(validtn_list10f93)?.length>0)
      {
        setvalidtn_list10f93({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validtn_list10f93Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"validtn_list"}))
        }}
    >
