 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("list_tab_group")){
        setlist_tab_groupd6905((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(list_tab_groupd6905?.isDisabled==null)
      {
        setlist_tab_groupd6905((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"listgroup"}))
        }}
    >
