 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("controlgroup")){
        setcontrolgroupda197((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(controlgroupda197?.isDisabled==null)
      {
        setcontrolgroupda197((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("documentviewer")){
        setdocumentviewer9df1d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(documentviewer9df1d?.isDisabled==null)
      {
        setdocumentviewer9df1d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"overallgroup"}))
        }}
    >
