...
const TextInputbase_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.cr_amount"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|43669d503a94467982239c6659b3b226"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cr_amount',type:"number"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, cr_amount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.cr_amount == "" || basicinfo3d198?.cr_amount == undefined){
      basicinfo3d198.cr_amount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.cr_amount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_amount:"invalid"}}));
        }
    }else if(basicinfo3d198?.cr_amount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.cr_amount);
        }
        else{
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "43669d503a94467982239c6659b3b226") {
        handleChange({target:{value:basicinfo3d198Ref?.current?.cr_amount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "43669d503a94467982239c6659b3b226") {
        handleBlur({target:{value:basicinfo3d198Ref?.current?.cr_amount||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,cr_amount:dfd_scansaveprocessdfd_v1Props.data[0]?.cr_amount}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `35 / 47`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basicinfo3d198?.cr_amount||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.cr_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
