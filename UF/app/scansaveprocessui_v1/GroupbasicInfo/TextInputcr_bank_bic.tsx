...
const TextInputcr_bank_bic = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        ...
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_basic.properties.cr_bank_bic"
      ],
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c22b200afac143a4b305b1dc87c3d26f") {
        handleChange({target:{value:basicinfo3d198Ref?.current?.bic_code||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c22b200afac143a4b305b1dc87c3d26f") {
        handleBlur({target:{value:basicinfo3d198Ref?.current?.bic_code||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,bic_code:dfd_scansaveprocessdfd_v1Props.data[0]?.bic_code}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `22 / 34`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
