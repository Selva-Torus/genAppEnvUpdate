...
const TextInputforex_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|033d10c820374a4fa40de84cfeca58a5"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, dr_amount: +e.target.value }));
    }
    else{
    setbasicinfo3d198((prev: any) => ({ ...prev, dr_amount: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "033d10c820374a4fa40de84cfeca58a5") {
        handleChange({target:{value:basicinfo3d198Ref?.current?.dr_amount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "033d10c820374a4fa40de84cfeca58a5") {
        handleBlur({target:{value:basicinfo3d198Ref?.current?.dr_amount||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,dr_amount:dfd_scansaveprocessdfd_v1Props.data[0]?.dr_amount}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `35 / 47`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basicinfo3d198?.dr_amount||""}
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.dr_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
