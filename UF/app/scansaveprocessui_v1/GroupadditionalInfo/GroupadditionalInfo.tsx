 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("addtional_info")){
        setaddtional_info46cb8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(addtional_info46cb8?.isDisabled==null)
      {
        setaddtional_info46cb8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("signature_screen")){
        setsignature_screen413bb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(signature_screen413bb?.isDisabled==null)
      {
        setsignature_screen413bb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("remittance_info")){
        setremittance_infoba5e0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(remittance_infoba5e0?.isDisabled==null)
      {
        setremittance_infoba5e0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("additional_ref")){
        setadditional_reff63a3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(additional_reff63a3?.isDisabled==null)
      {
        setadditional_reff63a3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("customwidget")){
        setcustomwidgetd7e47((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(customwidgetd7e47?.isDisabled==null)
      {
        setcustomwidgetd7e47((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vgphstm_uuid")){
        setvgphstm_uuidcf6fc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vgphstm_uuidcf6fc?.isDisabled==null)
      {
        setvgphstm_uuidcf6fc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-3 pr-4 pl-4 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setscansaveprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"additionalinfo"}))
        }}
    >
        ...
        {allowedControls.includes("signature_screen") ?<CustomWidgetsignature_screen /* 413bb */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        ...
        {allowedControls.includes("vgphstm_uuid") ?<TextInputvgphstm_uuid   /* cf6fc */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
