...
const TextInputvgphstm_uuid = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,vgphstm_uuid:undefined}}));
    if(dynamicStateandType.type=="number"){
    setadditionalinfod2894((prev: any) => ({ ...prev, vgphstm_uuid: +e.target.value }));
    }
    else{
    setadditionalinfod2894((prev: any) => ({ ...prev, vgphstm_uuid: e.target.value }));
    }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "d8c63820961a4d69b4213aab865cf6fc") {
        handleChange({target:{value:additionalinfod2894Ref?.current?.vgphstm_uuid||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d8c63820961a4d69b4213aab865cf6fc") {
        handleBlur({target:{value:additionalinfod2894Ref?.current?.vgphstm_uuid||""}});
      }
    };
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={additionalinfod2894?.vgphstm_uuid||""}
         disabled= {vgphstm_uuidcf6fc?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.vgphstm_uuid ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
