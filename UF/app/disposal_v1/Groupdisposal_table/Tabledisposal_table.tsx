...
let defaultColumns:any = [
  {
    "id": "disposal_id",
    "nodeid": "f563558ead094d6aba4142d194946e83",
    "name": "Disposal ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "disposal_method",
    "nodeid": "60f1c7e6a70844fa878d5041447ba6cd",
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "disposal_method"
  },
  {
    "id": "disposal_date",
    "nodeid": "fb820ba3b8054c8ca8c47c6a912e0685",
    "name": "Disposal Date",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "witness_name"
  },
  {
    "id": "data_wipe_method",
    "nodeid": "db75d3aa1e724c64b1c06136b74c4b88",
    "name": "Data Wipe Method",
    "meta": {
      "sort": true
    },
    "className": "",
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.disposal_table && artifactRuleState?.disposal_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.disposal_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,disposal_table:filteredData},{disposal_table:artifactRuleState?.disposal_table})
          if(result?.disposal_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const Tabledisposal_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {assetdisposal_v1Props, setassetdisposal_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setdisposal_table75a5dProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdisposal_table75a5dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(assetdisposal_v1Props.length > 0){
      for(let i=0;i< assetdisposal_v1Props.length;i++){
        if(assetdisposal_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1"){
          // delete assetdisposal_v1Props[i].DFDkey;
          let temp=structuredClone(assetdisposal_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetDisposal:AFVK:v1:UO', 
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( disposal_table75a5dProps?.presetValues&&Object.keys(disposal_table75a5dProps?.presetValues).length > 0) {
          filtertedData = [disposal_table75a5dProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setdisposal_table75a5d(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={assetdisposal_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (disposal_table75a5dProps.filterInitalLoad) return;

    const filterControllers = disposal_table75a5dProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['disposal_table75a5d']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['disposal_table75a5d']).filter((k: string) => jsonEntry['disposal_table75a5d'][k] === false)
      : [];
    ...
    setdisposal_table75a5dProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(disposal_table75a5dProps.filterProps);

  }, [
    disposal_table75a5dProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    disposal_table75a5dProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(disposal_table75a5dProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(disposal_table75a5dProps.searchFilter) || {};
...
}, [disposal_table75a5dProps.searchFilter])
  ...
  useEffect(() => {
    if (disposal_table75a5dProps?.clearData === true) {
      setdisposal_table75a5d([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setdisposal_table75a5dProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [disposal_table75a5dProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [disposal_table75a5dProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = disposal_table75a5dProps?.selectedIds[disposal_table75a5dProps?.selectedIds?.length-1]
    if(Array.isArray(disposal_table75a5d) && disposal_table75a5d.length>0)
    {
      let data = disposal_table75a5d[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "disposal_id",
  ...
  useEffect(() => {
    if (disposal_table75a5dProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (disposal_table75a5dProps?.selectedIds?.length == 0){
      handleOnRowClick({},disposal_table75a5dProps?.selectedIds)
    }
  }, [disposal_table75a5dProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(disposal_table75a5d?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&disposal_table75a5d?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](disposal_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: disposal_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&disposal_table75a5d?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](disposal_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: disposal_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (disposal_table75a5d?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={disposal_table75a5dProps?.selectedIds}  
...
export default Tabledisposal_table
