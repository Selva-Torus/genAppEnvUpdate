...
const Groupoverall_disposal_group = dynamic(() => import("./Groupoverall_disposal_group/Groupoverall_disposal_group"), { ssr: false });


export default function PageDisposalV1() {
  ...
  const [checkoverall_disposal_group,setCheckoverall_disposal_group,]=useState<boolean>(false);
  const [checkicon_text_group,setCheckicon_text_group,]=useState<boolean>(false);
  const [checkdisposal_table,setCheckdisposal_table,]=useState<boolean>(false);
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setassetdisposal_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    await assetdisposal_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'overall_disposal_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckoverall_disposal_group(true)
            }
            if(nodes?.groupName == 'icon_text_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckicon_text_group(true)
            }
            if(nodes?.groupName == 'disposal_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckdisposal_table(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setassetdisposal_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(assetdisposal_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(assetdisposal_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkoverall_disposal_group && initialLoad &&<Groupoverall_disposal_group
