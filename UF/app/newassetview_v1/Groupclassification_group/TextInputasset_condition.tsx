...
const TextInputasset_condition = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.asset_condition"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|f0135763467c6ee90f680c9cf68d9d65|c98f89374f7e47858f43d56fee84d358"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'asset_condition',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,asset_condition:undefined}}));
    if(dynamicStateandType.type=="number"){
    setclassification_groupd9d65((prev: any) => ({ ...prev, asset_condition: +e.target.value }));
    }
    else{
    setclassification_groupd9d65((prev: any) => ({ ...prev, asset_condition: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c98f89374f7e47858f43d56fee84d358"
      );
      ...
      //     controlId: "c98f89374f7e47858f43d56fee84d358",
      //     isTable: false,
      //     from:"TextInputasset_condition",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'asset_condition', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'asset_condition',type:'text'};
      //   type={
      //     name:'asset_condition',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_condition.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_condition.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_condition.type
      //   }
      ...
      //   type={
      //     name:'asset_condition',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_condition.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_condition.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_condition.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c98f89374f7e47858f43d56fee84d358") {
        handleChange({target:{value:classification_groupd9d65Ref?.current?.asset_condition||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c98f89374f7e47858f43d56fee84d358") {
        handleBlur({target:{value:classification_groupd9d65Ref?.current?.asset_condition||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setclassification_groupd9d65((pre:any)=>({...pre,asset_condition:dfd_assets_v1Props.data[0]?.asset_condition}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_condition4d358?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={classification_groupd9d65?.asset_condition||""}
         disabled= {asset_condition4d358?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.asset_condition ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputasset_condition
