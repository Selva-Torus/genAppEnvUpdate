...
const TextInputclassification = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.classification"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|f0135763467c6ee90f680c9cf68d9d65|62b2dbf9e14a412ba696e33e14bf4888"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'classification',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,classification:undefined}}));
    if(dynamicStateandType.type=="number"){
    setclassification_groupd9d65((prev: any) => ({ ...prev, classification: +e.target.value }));
    }
    else{
    setclassification_groupd9d65((prev: any) => ({ ...prev, classification: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "62b2dbf9e14a412ba696e33e14bf4888"
      );
      ...
      //     controlId: "62b2dbf9e14a412ba696e33e14bf4888",
      //     isTable: false,
      //     from:"TextInputclassification",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'classification', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'classification',type:'text'};
      //   type={
      //     name:'classification',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.classification.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.classification.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.classification.type
      //   }
      ...
      //   type={
      //     name:'classification',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.classification.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.classification.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.classification.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "62b2dbf9e14a412ba696e33e14bf4888") {
        handleChange({target:{value:classification_groupd9d65Ref?.current?.classification||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "62b2dbf9e14a412ba696e33e14bf4888") {
        handleBlur({target:{value:classification_groupd9d65Ref?.current?.classification||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setclassification_groupd9d65((pre:any)=>({...pre,classification:dfd_assets_v1Props.data[0]?.classification}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (classificationf4888?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={classification_groupd9d65?.classification||""}
         disabled= {classificationf4888?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.classification ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputclassification
