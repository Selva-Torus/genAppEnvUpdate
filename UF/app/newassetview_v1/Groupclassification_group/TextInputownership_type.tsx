...
const TextInputownership_type = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.ownership_type"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|f0135763467c6ee90f680c9cf68d9d65|8c53d3e6eed54a469e74d6574c9783c2"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'ownership_type',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,ownership_type:undefined}}));
    if(dynamicStateandType.type=="number"){
    setclassification_groupd9d65((prev: any) => ({ ...prev, ownership_type: +e.target.value }));
    }
    else{
    setclassification_groupd9d65((prev: any) => ({ ...prev, ownership_type: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "8c53d3e6eed54a469e74d6574c9783c2"
      );
      ...
      //     controlId: "8c53d3e6eed54a469e74d6574c9783c2",
      //     isTable: false,
      //     from:"TextInputownership_type",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'ownership_type', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'ownership_type',type:'text'};
      //   type={
      //     name:'ownership_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.ownership_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.ownership_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.ownership_type.type
      //   }
      ...
      //   type={
      //     name:'ownership_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.ownership_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.ownership_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.ownership_type.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "8c53d3e6eed54a469e74d6574c9783c2") {
        handleChange({target:{value:classification_groupd9d65Ref?.current?.ownership_type||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "8c53d3e6eed54a469e74d6574c9783c2") {
        handleBlur({target:{value:classification_groupd9d65Ref?.current?.ownership_type||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setclassification_groupd9d65((pre:any)=>({...pre,ownership_type:dfd_assets_v1Props.data[0]?.ownership_type}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (ownership_type783c2?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={classification_groupd9d65?.ownership_type||""}
         disabled= {ownership_type783c2?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.ownership_type ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputownership_type
