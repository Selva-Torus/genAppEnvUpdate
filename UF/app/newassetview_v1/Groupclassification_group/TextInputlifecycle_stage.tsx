...
const TextInputlifecycle_stage = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.lifecycle_stage"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|f0135763467c6ee90f680c9cf68d9d65|e9a26238ad6949cc9856fb521c726be5"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'lifecycle_stage',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,lifecycle_stage:undefined}}));
    if(dynamicStateandType.type=="number"){
    setclassification_groupd9d65((prev: any) => ({ ...prev, lifecycle_stage: +e.target.value }));
    }
    else{
    setclassification_groupd9d65((prev: any) => ({ ...prev, lifecycle_stage: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "f0135763467c6ee90f680c9cf68d9d65",
        "e9a26238ad6949cc9856fb521c726be5"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1",
      //     componentId: "f0135763467c6ee90f680c9cf68d9d65",
      //     controlId: "e9a26238ad6949cc9856fb521c726be5",
      //     isTable: false,
      //     from:"TextInputlifecycle_stage",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'lifecycle_stage', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'lifecycle_stage',type:'text'};
      //   type={
      //     name:'lifecycle_stage',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.lifecycle_stage.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.lifecycle_stage.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.lifecycle_stage.type
      //   }
      ...
      //   type={
      //     name:'lifecycle_stage',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.lifecycle_stage.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.lifecycle_stage.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.lifecycle_stage.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const classification_groupd9d65Ref = useRef<any>(classification_groupd9d65);
  useEffect(() => { classification_groupd9d65Ref.current = classification_groupd9d65; }, [classification_groupd9d65]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleBlur();
    const handlerChange = (id:any) => {
      if (id === "e9a26238ad6949cc9856fb521c726be5") {
        handleChange({target:{value:classification_groupd9d65Ref?.current?.lifecycle_stage||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e9a26238ad6949cc9856fb521c726be5") {
        handleBlur({target:{value:classification_groupd9d65Ref?.current?.lifecycle_stage||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setclassification_groupd9d65((pre:any)=>({...pre,lifecycle_stage:dfd_assets_v1Props.data[0]?.lifecycle_stage}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (lifecycle_stage26be5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `8 / 20`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={classification_groupd9d65?.lifecycle_stage||""}
         disabled= {lifecycle_stage26be5?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Lifecycle Stage"
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.lifecycle_stage ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputlifecycle_stage
