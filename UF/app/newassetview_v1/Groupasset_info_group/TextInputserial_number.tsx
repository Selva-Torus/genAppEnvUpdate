...
const TextInputserial_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|13d6f9aca9f08ad0e2052fe4913cc113|09cc607b654d22910aea417e00867791"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,serial_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupcc113((prev: any) => ({ ...prev, serial_number: +e.target.value }));
    }
    else{
    setasset_info_groupcc113((prev: any) => ({ ...prev, serial_number: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "09cc607b654d22910aea417e00867791"
      );
      ...
      //     controlId: "09cc607b654d22910aea417e00867791",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "09cc607b654d22910aea417e00867791") {
        handleChange({target:{value:asset_info_groupcc113Ref?.current?.serial_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "09cc607b654d22910aea417e00867791") {
        handleBlur({target:{value:asset_info_groupcc113Ref?.current?.serial_number||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupcc113((pre:any)=>({...pre,serial_number:dfd_assets_v1Props.data[0]?.serial_number}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (serial_number67791?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupcc113?.serial_number||""}
         disabled= {serial_number67791?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.serial_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
