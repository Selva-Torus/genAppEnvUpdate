...
const TextInputasset_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|13d6f9aca9f08ad0e2052fe4913cc113|6c3428161e953fcdd36c73de4354044f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,asset_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_name: +e.target.value }));
    }
    else{
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6c3428161e953fcdd36c73de4354044f"
      );
      ...
      //     controlId: "6c3428161e953fcdd36c73de4354044f",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6c3428161e953fcdd36c73de4354044f") {
        handleChange({target:{value:asset_info_groupcc113Ref?.current?.asset_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6c3428161e953fcdd36c73de4354044f") {
        handleBlur({target:{value:asset_info_groupcc113Ref?.current?.asset_name||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupcc113((pre:any)=>({...pre,asset_name:dfd_assets_v1Props.data[0]?.asset_name}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_name4044f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupcc113?.asset_name||""}
         disabled= {asset_name4044f?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.asset_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
