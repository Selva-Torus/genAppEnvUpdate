...
const TextInputmodel_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|13d6f9aca9f08ad0e2052fe4913cc113|b40a54991147b06b77de31063a846a87"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,model_number:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupcc113((prev: any) => ({ ...prev, model_number: +e.target.value }));
    }
    else{
    setasset_info_groupcc113((prev: any) => ({ ...prev, model_number: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b40a54991147b06b77de31063a846a87"
      );
      ...
      //     controlId: "b40a54991147b06b77de31063a846a87",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "b40a54991147b06b77de31063a846a87") {
        handleChange({target:{value:asset_info_groupcc113Ref?.current?.model_number||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "b40a54991147b06b77de31063a846a87") {
        handleBlur({target:{value:asset_info_groupcc113Ref?.current?.model_number||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupcc113((pre:any)=>({...pre,model_number:dfd_assets_v1Props.data[0]?.model_number}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (model_number46a87?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupcc113?.model_number||""}
         disabled= {model_number46a87?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.model_number ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
