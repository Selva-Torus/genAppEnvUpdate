...
const TextInputasset_tag = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|13d6f9aca9f08ad0e2052fe4913cc113|36ecbb76205e26b33c6d0953dff665c1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,asset_tag:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_tag: +e.target.value }));
    }
    else{
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_tag: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "36ecbb76205e26b33c6d0953dff665c1"
      );
      ...
      //     controlId: "36ecbb76205e26b33c6d0953dff665c1",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "36ecbb76205e26b33c6d0953dff665c1") {
        handleChange({target:{value:asset_info_groupcc113Ref?.current?.asset_tag||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "36ecbb76205e26b33c6d0953dff665c1") {
        handleBlur({target:{value:asset_info_groupcc113Ref?.current?.asset_tag||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupcc113((pre:any)=>({...pre,asset_tag:dfd_assets_v1Props.data[0]?.asset_tag}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_tag665c1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupcc113?.asset_tag||""}
         disabled= {asset_tag665c1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.asset_tag ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
