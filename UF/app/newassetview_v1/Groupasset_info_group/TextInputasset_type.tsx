...
const TextInputasset_type = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.asset_type"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|13d6f9aca9f08ad0e2052fe4913cc113|c802a83bc02a452c8a5e400da52cdf86"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'asset_type',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,asset_type:undefined}}));
    if(dynamicStateandType.type=="number"){
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_type: +e.target.value }));
    }
    else{
    setasset_info_groupcc113((prev: any) => ({ ...prev, asset_type: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c802a83bc02a452c8a5e400da52cdf86"
      );
      ...
      //     controlId: "c802a83bc02a452c8a5e400da52cdf86",
      //     isTable: false,
      //     from:"TextInputasset_type",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'asset_type', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'asset_type',type:'text'};
      //   type={
      //     name:'asset_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.asset_type.type
      //   }
      ...
      //   type={
      //     name:'asset_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.asset_type.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c802a83bc02a452c8a5e400da52cdf86") {
        handleChange({target:{value:asset_info_groupcc113Ref?.current?.asset_type||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c802a83bc02a452c8a5e400da52cdf86") {
        handleBlur({target:{value:asset_info_groupcc113Ref?.current?.asset_type||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setasset_info_groupcc113((pre:any)=>({...pre,asset_type:dfd_assets_v1Props.data[0]?.asset_type}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (asset_typecdf86?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={asset_info_groupcc113?.asset_type||""}
         disabled= {asset_typecdf86?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.asset_type ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputasset_type
