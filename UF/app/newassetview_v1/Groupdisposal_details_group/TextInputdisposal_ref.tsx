...
const TextInputdisposal_ref = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.disposal_ref"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|dce7d099650788e566ce578dc2067f77|cac29209069d2c01021807401c535f4b"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'disposal_ref',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,disposal_ref:undefined}}));
    if(dynamicStateandType.type=="number"){
    setdisposal_details_group67f77((prev: any) => ({ ...prev, disposal_ref: +e.target.value }));
    }
    else{
    setdisposal_details_group67f77((prev: any) => ({ ...prev, disposal_ref: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "cac29209069d2c01021807401c535f4b"
      );
      ...
      //     controlId: "cac29209069d2c01021807401c535f4b",
      //     isTable: false,
      //     from:"TextInputdisposal_ref",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'disposal_ref', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'disposal_ref',type:'text'};
      //   type={
      //     name:'disposal_ref',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_ref.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_ref.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.disposal_ref.type
      //   }
      ...
      //   type={
      //     name:'disposal_ref',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_ref.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_ref.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.disposal_ref.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "cac29209069d2c01021807401c535f4b") {
        handleChange({target:{value:disposal_details_group67f77Ref?.current?.disposal_ref||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "cac29209069d2c01021807401c535f4b") {
        handleBlur({target:{value:disposal_details_group67f77Ref?.current?.disposal_ref||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setdisposal_details_group67f77((pre:any)=>({...pre,disposal_ref:dfd_assets_v1Props.data[0]?.disposal_ref}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (disposal_ref35f4b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={disposal_details_group67f77?.disposal_ref||""}
         disabled= {disposal_ref35f4b?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
        headerPosition='top'
        headerText="Disposal Reference"
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.disposal_ref ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdisposal_ref
