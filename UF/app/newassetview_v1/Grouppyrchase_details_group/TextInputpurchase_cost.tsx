...
const TextInputpurchase_cost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|50a619a1af5a843efc63dd18a4ac3900|bcdb9d46fc2062c0cd6de8803e6899f9"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,purchase_cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, purchase_cost: +e.target.value }));
    }
    else{
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, purchase_cost: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "bcdb9d46fc2062c0cd6de8803e6899f9"
      );
      ...
      //     controlId: "bcdb9d46fc2062c0cd6de8803e6899f9",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "bcdb9d46fc2062c0cd6de8803e6899f9") {
        handleChange({target:{value:pyrchase_details_groupc3900Ref?.current?.purchase_cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "bcdb9d46fc2062c0cd6de8803e6899f9") {
        handleBlur({target:{value:pyrchase_details_groupc3900Ref?.current?.purchase_cost||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_groupc3900((pre:any)=>({...pre,purchase_cost:dfd_assets_v1Props.data[0]?.purchase_cost}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (purchase_cost899f9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={pyrchase_details_groupc3900?.purchase_cost||""}
         disabled= {purchase_cost899f9?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.purchase_cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
