...
const TextInputcurrent_value = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|50a619a1af5a843efc63dd18a4ac3900|53876bda1a687c8dc30a562cb298e31d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,current_value:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, current_value: +e.target.value }));
    }
    else{
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, current_value: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "53876bda1a687c8dc30a562cb298e31d"
      );
      ...
      //     controlId: "53876bda1a687c8dc30a562cb298e31d",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "53876bda1a687c8dc30a562cb298e31d") {
        handleChange({target:{value:pyrchase_details_groupc3900Ref?.current?.current_value||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "53876bda1a687c8dc30a562cb298e31d") {
        handleBlur({target:{value:pyrchase_details_groupc3900Ref?.current?.current_value||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_groupc3900((pre:any)=>({...pre,current_value:dfd_assets_v1Props.data[0]?.current_value}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (current_value8e31d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `21 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={pyrchase_details_groupc3900?.current_value||""}
         disabled= {current_value8e31d?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.current_value ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
