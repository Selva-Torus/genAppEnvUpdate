...
const TextInputvendor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.vendor_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|50a619a1af5a843efc63dd18a4ac3900|d0cac4ffa0044baa833158231e51f183"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,vendor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, vendor_name: +e.target.value }));
    }
    else{
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, vendor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d0cac4ffa0044baa833158231e51f183"
      );
      ...
      //     controlId: "d0cac4ffa0044baa833158231e51f183",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "d0cac4ffa0044baa833158231e51f183") {
        handleChange({target:{value:pyrchase_details_groupc3900Ref?.current?.vendor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d0cac4ffa0044baa833158231e51f183") {
        handleBlur({target:{value:pyrchase_details_groupc3900Ref?.current?.vendor_name||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_groupc3900((pre:any)=>({...pre,vendor_name:dfd_assets_v1Props.data[0]?.vendor_name}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (vendor_name1f183?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={pyrchase_details_groupc3900?.vendor_name||""}
         disabled= {vendor_name1f183?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.vendor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
