...
const TextInputwarranty_expiry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assets:AFVK:v1|19752269565d4be2be63be1bd8cf4ff6|properties.warranty_expiry"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:newAssetView:AFVK:v1|50a619a1af5a843efc63dd18a4ac3900|86ffd12c224e4cf68ad1ca395b2e6615"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'warranty_expiry',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,newAssetView_v1:{...pre?.newAssetView_v1,warranty_expiry:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, warranty_expiry: +e.target.value }));
    }
    else{
    setpyrchase_details_groupc3900((prev: any) => ({ ...prev, warranty_expiry: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "86ffd12c224e4cf68ad1ca395b2e6615"
      );
      ...
      //     controlId: "86ffd12c224e4cf68ad1ca395b2e6615",
      //     isTable: false,
      //     from:"TextInputwarranty_expiry",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'warranty_expiry', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'warranty_expiry',type:'text'};
      //   type={
      //     name:'warranty_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.warranty_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.warranty_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.warranty_expiry.type
      //   }
      ...
      //   type={
      //     name:'warranty_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.warranty_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.warranty_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.warranty_expiry.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "86ffd12c224e4cf68ad1ca395b2e6615") {
        handleChange({target:{value:pyrchase_details_groupc3900Ref?.current?.warranty_expiry||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "86ffd12c224e4cf68ad1ca395b2e6615") {
        handleBlur({target:{value:pyrchase_details_groupc3900Ref?.current?.warranty_expiry||""}});
      }
    };
  ...
  if(dfd_assets_v1Props?.setSearchFilters && dfd_assets_v1Props?.data)
  {
    if(Array.isArray(dfd_assets_v1Props.data) && dfd_assets_v1Props.data.length > 0){
      setpyrchase_details_groupc3900((pre:any)=>({...pre,warranty_expiry:dfd_assets_v1Props.data[0]?.warranty_expiry}));
    }
  }
  },[dfd_assets_v1Props?.setSearchFilters])
  if (warranty_expirye6615?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={pyrchase_details_groupc3900?.warranty_expiry||""}
         disabled= {warranty_expirye6615?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.newAssetView_v1?.warranty_expiry ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputwarranty_expiry
