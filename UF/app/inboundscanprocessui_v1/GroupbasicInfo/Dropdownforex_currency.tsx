 ...
 /////////////
  ...
  const getDropdownData = async(value?:any, page: number = 1, skipAutoSet: boolean = false)=>{
    ...
    const orchestrationData :any = getControlOrchestrationData(
      ...
      "d75fe9e40ca7d39decfe6946bd110f51"
    );
  ...
  },[forex_currency10f51?.refresh])  

  const handlechange = async(value: any) => {
    isUserSelectionRef.current = true;
    if(value.length>0){
      await getDropdownData(value)
      setbasicinfoffb0a((prev: any) => ({ ...prev,forex_currency10f51: value }))
      setIsRequredData(false)
    }else{
      let temp:any = basicinfoffb0a;
      delete temp.cr_currency;
      delete temp.CR_CURRENCY;
      delete temp.forex_currency10f51;
      setbasicinfoffb0a(temp);
      getDropdownData(undefined, 1, true);
      setIsRequredData(true);
    }
     setError('')
    setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,cr_currency:undefined}}));
    await handleClick(value);
  };

    const fetchDropdownData = async()=>{
    let tempValue:any=""
    if(basicinfoffb0a.cr_currency){
      if(Array.isArray(dfd_forexcurrencydropdowndfd_v1Props)){
        if(dfd_forexcurrencydropdowndfd_v1Props?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)){
          setdropdownValue([dfd_forexcurrencydropdowndfd_v1Props?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)?.currency_code])
          tempValue=dfd_forexcurrencydropdowndfd_v1Props?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)?.currency_code
        }else{
          setdropdownValue([basicinfoffb0a.cr_currency])
          tempValue=basicinfoffb0a.cr_currency
        }
      }else{
        ...
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{currency_code:basicinfoffb0a.cr_currency}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)?.currency_code ])
        tempValue=api_paginationData?.data?.records?.find((item: any) => item.currency_code === basicinfoffb0a.cr_currency)?.currency_code
      }else{
        setdropdownValue([basicinfoffb0a.cr_currency])
        tempValue=basicinfoffb0a.cr_currency
      }   
    ...
    if(forex_currency10f51?.trigger == true)
    {
      await handlechange(tempValue)
      setforex_currency10f51((pre:any)=>({...pre,trigger:false}))
  ...
  useEffect(() => {
    ...
    fetchData();
  },[basicinfoffb0a.cr_currency, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_forexcurrencydropdowndfd_v1Props) && dfd_forexcurrencydropdowndfd_v1Props?.length == 1){
    // setbasicinfoffb0a((pre:any)=>({...pre,cr_currency:dfd_forexcurrencydropdowndfd_v1Props[0]?.cr_currency}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setbasicinfoffb0a((prev: any) => ({ ...prev, cr_currency: getMapperDetailsBindValues[value],CR_CURRENCY: getMapperDetails}))
         setIsRequredData(false)
    } else {
       setbasicinfoffb0a((prev: any) => ({ ...prev, cr_currency: '', forex_currency10f51: '' }))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,cr_currency:undefined}}));
  ...
  useEffect(() => { basicinfoffb0aRef.current = basicinfoffb0a; }, [basicinfoffb0a]);
    useEffect(()=>{
       ...
       const handler = (id:any) => {
          if (id === "d75fe9e40ca7d39decfe6946bd110f51") {
        handleClick(basicinfoffb0aRef?.current?.forex_currency10f51?basicinfoffb0aRef?.current?.forex_currency10f51:"");
          }
        };
        eventBus.on("triggerElement|onClick", handler);
        eventBus.emit("DropdownReady", "d75fe9e40ca7d39decfe6946bd110f51");
        return () => {
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbasicinfoffb0a((pre:any)=>({...pre,cr_currency:""}))
    else
      setInitialCount(1)
  },[forex_currency10f51?.refresh])
  

  if (forex_currency10f51?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {forex_currency10f51?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={basicinfoffb0a?.forex_currency10f51 ? [basicinfoffb0a?.forex_currency10f51] : (basicinfoffb0a?.cr_currency ? dropdownValue : [])}
        validationState={validate?.inboundScanProcessUi_v1?.cr_currency ? "invalid" : undefined}
        />
    </div>
  );
};
