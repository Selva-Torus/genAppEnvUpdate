...
const TextInputcr_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.cr_name"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1|abf150d5ae6f90069b1723bc4bdffb0a|c6a3a0892adf4a31ebafd887ee5517b4"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,cr_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_name: +e.target.value }));
    }
    else{
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c6a3a0892adf4a31ebafd887ee5517b4"
      );
      ...
      //     controlId: "c6a3a0892adf4a31ebafd887ee5517b4",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c6a3a0892adf4a31ebafd887ee5517b4") {
        handleChange({target:{value:basicinfoffb0aRef?.current?.cr_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c6a3a0892adf4a31ebafd887ee5517b4") {
        handleBlur({target:{value:basicinfoffb0aRef?.current?.cr_name||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfoffb0a((pre:any)=>({...pre,cr_name:dfd_scansaveprocessdfd_v1Props.data[0]?.cr_name}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (cr_name517b4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basicinfoffb0a?.cr_name||""}
         disabled= {cr_name517b4?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Cr Name"
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.cr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
