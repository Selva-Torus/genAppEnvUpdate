...
const TextInputbase_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1|abf150d5ae6f90069b1723bc4bdffb0a|9156fa4b7669e373206b9cfed6b2df6d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,cr_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_amount: +e.target.value }));
    }
    else{
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9156fa4b7669e373206b9cfed6b2df6d"
      );
      ...
      //     controlId: "9156fa4b7669e373206b9cfed6b2df6d",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9156fa4b7669e373206b9cfed6b2df6d") {
        handleChange({target:{value:basicinfoffb0aRef?.current?.cr_amount||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9156fa4b7669e373206b9cfed6b2df6d") {
        handleBlur({target:{value:basicinfoffb0aRef?.current?.cr_amount||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfoffb0a((pre:any)=>({...pre,cr_amount:dfd_scansaveprocessdfd_v1Props.data[0]?.cr_amount}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (base_amount2df6d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `23 / 35`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basicinfoffb0a?.cr_amount||""}
         disabled= {base_amount2df6d?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.cr_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
