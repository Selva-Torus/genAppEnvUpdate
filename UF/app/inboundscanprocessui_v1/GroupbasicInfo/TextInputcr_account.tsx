...
const TextInputcr_account = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.cr_account"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1|abf150d5ae6f90069b1723bc4bdffb0a|dba3c3598752ad38bbd3dd292a4ddb15"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,cr_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_account: +e.target.value }));
    }
    else{
    setbasicinfoffb0a((prev: any) => ({ ...prev, cr_account: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "dba3c3598752ad38bbd3dd292a4ddb15"
      );
      ...
      //     controlId: "dba3c3598752ad38bbd3dd292a4ddb15",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "dba3c3598752ad38bbd3dd292a4ddb15") {
        handleChange({target:{value:basicinfoffb0aRef?.current?.cr_account||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "dba3c3598752ad38bbd3dd292a4ddb15") {
        handleBlur({target:{value:basicinfoffb0aRef?.current?.cr_account||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfoffb0a((pre:any)=>({...pre,cr_account:dfd_scansaveprocessdfd_v1Props.data[0]?.cr_account}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (cr_accountddb15?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basicinfoffb0a?.cr_account||""}
         disabled= {cr_accountddb15?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.cr_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
