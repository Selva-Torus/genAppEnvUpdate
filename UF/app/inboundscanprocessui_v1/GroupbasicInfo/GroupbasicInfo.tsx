...
const Groupbasicinfo = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "abf150d5ae6f90069b1723bc4bdffb0a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbasicinfoffb0aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("basic_info")){
        setbasic_info219cf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(basic_info219cf?.isDisabled==null)
      {
        setbasic_info219cf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account")){
        setcr_accountddb15((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_accountddb15?.isDisabled==null)
      {
        setcr_accountddb15((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name")){
        setcr_name517b4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_name517b4?.isDisabled==null)
      {
        setcr_name517b4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_code")){
        setcr_bank_code9af27((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_bank_code9af27?.isDisabled==null)
      {
        setcr_bank_code9af27((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("forex_currency")){
        setforex_currency10f51((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(forex_currency10f51?.isDisabled==null)
      {
        setforex_currency10f51((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("forex_amount")){
        setforex_amount2d477((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(forex_amount2d477?.isDisabled==null)
      {
        setforex_amount2d477((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("base_amount")){
        setbase_amount2df6d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(base_amount2df6d?.isDisabled==null)
      {
        setbase_amount2df6d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_groupb9855((prev:any) => ({ ...prev, ...basicinfoffb0a }));
      // copyFormData for group
      setcommoninfod7eda((prev:any) => ({ ...prev, ...basicinfoffb0a }));
  }
  ...
  const basicinfoffb0aRef = useRef<any>(null);
  const handleClearSearch = () => {
    basicinfoffb0aRef.current?.setSearchParams();
    basicinfoffb0aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(basicinfoffb0a) && Object.keys(basicinfoffb0a)?.length>0)
      {
        setbasicinfoffb0a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [basicinfoffb0aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-3 pr-4 pl-4 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"basicinfo"}))
        }}
    >
          {allowedControls.includes("basic_info") ?<Textbasic_info   /* 219cf */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_account") ?<TextInputcr_account   /* ddb15 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_name") ?<TextInputcr_name   /* 517b4 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_bank_code") ?<Dropdowncr_bank_code   /* 9af27 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("forex_currency") ?<Dropdownforex_currency   /* 10f51 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("forex_amount") ?<TextInputforex_amount   /* 2d477 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("base_amount") ?<TextInputbase_amount   /* 2df6d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
