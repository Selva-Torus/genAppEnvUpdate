...
const TextInputremittance_info = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,remittance_info:undefined}}));
    if(dynamicStateandType.type=="number"){
    setadditionalinfo4baba((prev: any) => ({ ...prev, remittance_info: +e.target.value }));
    }
    else{
    setadditionalinfo4baba((prev: any) => ({ ...prev, remittance_info: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "94834857091fe8714e3f9159cc20bded"
      );
      ...
      //     controlId: "94834857091fe8714e3f9159cc20bded",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "94834857091fe8714e3f9159cc20bded") {
        handleChange({target:{value:additionalinfo4babaRef?.current?.remittance_info||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "94834857091fe8714e3f9159cc20bded") {
        handleBlur({target:{value:additionalinfo4babaRef?.current?.remittance_info||""}});
      }
    };
  ...
  if (remittance_info0bded?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={additionalinfo4baba?.remittance_info||""}
         disabled= {remittance_info0bded?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.remittance_info ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
