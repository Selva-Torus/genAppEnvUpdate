...
const TextInputvgphstm_uuid = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'vgphstm_uuid',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,vgphstm_uuid:undefined}}));
    if(dynamicStateandType.type=="number"){
    setadditionalinfo4baba((prev: any) => ({ ...prev, vgphstm_uuid: +e.target.value }));
    }
    else{
    setadditionalinfo4baba((prev: any) => ({ ...prev, vgphstm_uuid: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        controlData,
        "7e9ab6f094ce4f1565494f005e44baba",
        "86a56c669d1458aac7ce6538457f9485"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1",
      //     componentId: "7e9ab6f094ce4f1565494f005e44baba",
      //     controlId: "86a56c669d1458aac7ce6538457f9485",
      //     isTable: false,
      //     from:"TextInputvgphstm_uuid",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'vgphstm_uuid', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'vgphstm_uuid',type:'text'};
      //   type={
      //     name:'vgphstm_uuid',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vgphstm_uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vgphstm_uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.vgphstm_uuid.type
      //   }
      ...
      //   type={
      //     name:'vgphstm_uuid',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vgphstm_uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vgphstm_uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.vgphstm_uuid.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  const additionalinfo4babaRef = useRef<any>(additionalinfo4baba);
  useEffect(() => { additionalinfo4babaRef.current = additionalinfo4baba; }, [additionalinfo4baba]);
  useEffect(()=>{
      handleMapperValue();
      if(validateRefetch.init!=0)
        handleValidate();
    const handlerChange = (id:any) => {
      if (id === "86a56c669d1458aac7ce6538457f9485") {
        handleChange({target:{value:additionalinfo4babaRef?.current?.vgphstm_uuid||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "86a56c669d1458aac7ce6538457f9485") {
        handleBlur({target:{value:additionalinfo4babaRef?.current?.vgphstm_uuid||""}});
      }
    };
  ...
  if (vgphstm_uuidf9485?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 19`,gridRow: `10 / 20`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
        {isRequredData && <span style={{ color: 'red' }}>*</span>}
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className=""
        label={keyset("vgphstm uuid")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={additionalinfo4baba?.vgphstm_uuid||""}
         disabled= {vgphstm_uuidf9485?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.vgphstm_uuid ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputvgphstm_uuid
