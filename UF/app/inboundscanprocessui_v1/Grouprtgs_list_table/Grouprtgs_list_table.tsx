...
const Grouprtgs_list_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2dd50c83e10eedb8e9f91cb7b56ad2c7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_tablead2c7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tran_id")){
        settran_id6705e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tran_id6705e?.isDisabled==null)
      {
        settran_id6705e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_acnt_no")){
        setdr_acnt_no28ad2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_acnt_no28ad2?.isDisabled==null)
      {
        setdr_acnt_no28ad2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_acnt_no")){
        setcr_acnt_no58585((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_acnt_no58585?.isDisabled==null)
      {
        setcr_acnt_no58585((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("amnt")){
        setamnt95ed1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(amnt95ed1?.isDisabled==null)
      {
        setamnt95ed1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_code")){
        setcr_bank_code01850((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_bank_code01850?.isDisabled==null)
      {
        setcr_bank_code01850((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("created_by")){
        setcreated_byb7915((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(created_byb7915?.isDisabled==null)
      {
        setcreated_byb7915((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("created_date")){
        setcreated_date6b8a8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(created_date6b8a8?.isDisabled==null)
      {
        setcreated_date6b8a8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const rtgs_list_tablead2c7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_tablead2c7Ref.current?.setSearchParams();
    rtgs_list_tablead2c7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_tablead2c7) && Object.keys(rtgs_list_tablead2c7)?.length>0)
      {
        setrtgs_list_tablead2c7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_tablead2c7Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-1 pr-1 pl-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_list_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_tablead2c7Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
