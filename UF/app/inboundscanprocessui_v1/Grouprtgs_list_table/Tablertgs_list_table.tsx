...
let defaultColumns:any = [
  {
    "id": "tran_id",
    "nodeid": "f01403921bded19d581e6fccb1a6705e",
    "name": "Tran ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "dr_acnt_no",
    "nodeid": "f349f9df259dcf043846a584ee228ad2",
    "name": "Dr Account No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_acnt_no",
    "nodeid": "b41d2e61eb57b06093cfe1d95c458585",
    "name": "Cr Account No",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "amnt",
    "nodeid": "6aa5d31303e5f86789c8e2d54d295ed1",
    "name": "Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_bank_code",
    "nodeid": "74bf57133a02f85615433d3ddf801850",
    "name": "Cr Bank Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "created_by",
    "nodeid": "342e00c6ee8b150cdef8d1cc290b7915",
    "name": "Created By",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "created_date",
    "nodeid": "3553a44d515468fd5042662abc36b8a8",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {tran_id6705e, settran_id6705e}= useContext(TotalContext) as TotalContextProps  
  const {dr_acnt_no28ad2, setdr_acnt_no28ad2}= useContext(TotalContext) as TotalContextProps  
  const {cr_acnt_no58585, setcr_acnt_no58585}= useContext(TotalContext) as TotalContextProps  
  const {amnt95ed1, setamnt95ed1}= useContext(TotalContext) as TotalContextProps  
  const {cr_bank_code01850, setcr_bank_code01850}= useContext(TotalContext) as TotalContextProps  
  const {created_byb7915, setcreated_byb7915}= useContext(TotalContext) as TotalContextProps  
  const {created_date6b8a8, setcreated_date6b8a8}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "2dd50c83e10eedb8e9f91cb7b56ad2c7",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_tablead2c7 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_tablead2c7 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_tablead2c7 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_tablead2c7Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      rtgs_list_tablead2c7.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setrtgs_list_tablead2c7Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setrtgs_list_tablead2c7Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['tran_id'] = tran_id6705e,
        codeStates['settran_id'] = settran_id6705e,
        codeStates['dr_acnt_no'] = dr_acnt_no28ad2,
        codeStates['setdr_acnt_no'] = setdr_acnt_no28ad2,
        codeStates['cr_acnt_no'] = cr_acnt_no58585,
        codeStates['setcr_acnt_no'] = setcr_acnt_no58585,
        codeStates['amnt'] = amnt95ed1,
        codeStates['setamnt'] = setamnt95ed1,
        codeStates['cr_bank_code'] = cr_bank_code01850,
        codeStates['setcr_bank_code'] = setcr_bank_code01850,
        codeStates['created_by'] = created_byb7915,
        codeStates['setcreated_by'] = setcreated_byb7915,
        codeStates['created_date'] = created_date6b8a8,
        codeStates['setcreated_date'] = setcreated_date6b8a8,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_tablead2c7Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_tablead2c7Props?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_tablead2c7Props?.presetValues || Object.keys(rtgs_list_tablead2c7Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1:UO', 
                nodeId: '2dd50c83e10eedb8e9f91cb7b56ad2c7',
                elementId: '2dd50c83e10eedb8e9f91cb7b56ad2c7'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_tablead2c7(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_tablead2c7))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_tablead2c7Props.filterInitalLoad) return;

    const filterControllers = rtgs_list_tablead2c7Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_tablead2c7']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_tablead2c7']).filter((k: string) => jsonEntry['rtgs_list_tablead2c7'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_tablead2c7Props.filterProps);
      return;
    }
    ...
    setrtgs_list_tablead2c7Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_tablead2c7Props.filterProps);

  }, [
    rtgs_list_tablead2c7Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_tablead2c7Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_tablead2c7Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_tablead2c7Props.searchFilter) || {};
...
}, [rtgs_list_tablead2c7Props.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_tablead2c7Props?.clearData === true) {
      setrtgs_list_tablead2c7([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_tablead2c7Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_tablead2c7Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_tablead2c7Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_tablead2c7Props?.selectedIds[rtgs_list_tablead2c7Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_tablead2c7) && rtgs_list_tablead2c7.length>0)
    {
      let data = rtgs_list_tablead2c7.find((data:any)=>(data?.vgphstm_uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_uuid",
  ...
  useEffect(() => {
    if (rtgs_list_tablead2c7Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_tablead2c7Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_tablead2c7Props?.selectedIds)
    }
  }, [rtgs_list_tablead2c7Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_tablead2c7?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_tablead2c7?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_tablead2c7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_tablead2c7?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_tablead2c7?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_tablead2c7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_tablead2c7?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_tablead2c7?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"2dd50c83e10eedb8e9f91cb7b56ad2c7"}  onClick={(e:any) => {
          ...
          >
            <Table
              className="!rounded-lg p-1 pr-1 pl-1"
              data={Array.isArray(allDataObject) && translatedColumns?.length ? allDataObject : []}
              columns={translatedColumns}
              primaryKey="vgphstm_uuid"
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_tablead2c7Props?.selectedIds}  
