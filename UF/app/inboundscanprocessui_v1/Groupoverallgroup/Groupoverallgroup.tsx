...
const Groupoverallgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "fd57aaf3e40fda009745ed7b87b1218f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverallgroup1218fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("controlgroup")){
        setcontrolgroupfbb48((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(controlgroupfbb48?.isDisabled==null)
      {
        setcontrolgroupfbb48((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_groupb9855((prev:any) => ({ ...prev, ...overallgroup1218f }));
  }
  ...
  const overallgroup1218fRef = useRef<any>(null);
  const handleClearSearch = () => {
    overallgroup1218fRef.current?.setSearchParams();
    overallgroup1218fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overallgroup1218f) && Object.keys(overallgroup1218f)?.length>0)
      {
        setoverallgroup1218f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overallgroup1218fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"overallgroup"}))
        }}
    >
