...
const Groupvalidtn_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1b20c899cb66e878aac5a6dce0d3a9a1");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalidtn_list3a9a1Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_validtn_table")){
        setrtgs_list_validtn_table10755Props((pre:any)=>({...pre,...rtgs_list_validtn_table10755,isDisabled:true}));

    }else
    {
      if(rtgs_list_validtn_table10755?.isDisabled==null)
      {
        setrtgs_list_validtn_table10755Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(rtgs_list_tab_grp43579?.rtgs_list_tab_grp === "validtn_list"){
    handleOnChange()
    }
  },[rtgs_list_tab_grp43579?.rtgs_list_tab_grp])
  ...
  const validtn_list3a9a1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    validtn_list3a9a1Ref.current?.setSearchParams();
    validtn_list3a9a1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validtn_list3a9a1) && Object.keys(validtn_list3a9a1)?.length>0)
      {
        setvalidtn_list3a9a1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validtn_list3a9a1Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"validtn_list"}))
        }}
    >
