...
const Groupvaldnlisttable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "350585c031252e28732f2f119b74db84");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvaldnlisttable4db84Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vldcode")){
        setvldcoded6381((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vldcoded6381?.isDisabled==null)
      {
        setvldcoded6381((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vldreason")){
        setvldreasonfca81((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vldreasonfca81?.isDisabled==null)
      {
        setvldreasonfca81((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const valdnlisttable4db84Ref = useRef<any>(null);
  const handleClearSearch = () => {
    valdnlisttable4db84Ref.current?.setSearchParams();
    valdnlisttable4db84Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(valdnlisttable4db84) && Object.keys(valdnlisttable4db84)?.length>0)
      {
        setvaldnlisttable4db84({})
      }
    }else 
      prevRefreshRef.current= true
  }, [valdnlisttable4db84Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"valdnlisttable"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<TablevaldnListTable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={valdnlisttable4db84Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
