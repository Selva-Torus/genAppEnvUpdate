...
let defaultColumns:any = [
  {
    "id": "vldCode",
    "nodeid": "dba8815a93accffe031320b6dd8d6381",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "vldReason",
    "nodeid": "baccd9a04071f21265556ea3b39fca81",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const TablevaldnListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {vldcoded6381, setvldcoded6381}= useContext(TotalContext) as TotalContextProps  
  const {vldreasonfca81, setvldreasonfca81}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "350585c031252e28732f2f119b74db84",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, valdnlisttable4db84 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, valdnlisttable4db84 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, valdnlisttable4db84 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setvaldnlisttable4db84Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      valdnlisttable4db84.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setvaldnlisttable4db84Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setvaldnlisttable4db84Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['vldcode'] = vldcoded6381,
        codeStates['setvldcode'] = setvldcoded6381,
        codeStates['vldreason'] = vldreasonfca81,
        codeStates['setvldreason'] = setvldreasonfca81,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setvaldnlisttable4db84Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:valdnlisttable4db84Props?.presetValues?.vgphstm_uuid}
    ...
    if(!valdnlisttable4db84Props?.presetValues || Object.keys(valdnlisttable4db84Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '350585c031252e28732f2f119b74db84',
                elementId: '350585c031252e28732f2f119b74db84'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setvaldnlisttable4db84(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(valdnlisttable4db84))
    {
      GetTableDetails()
    }
  }, [valdnlisttable4db84])
  ...
  useEffect(() => {
    ...
    if (valdnlisttable4db84Props.filterInitalLoad) return;

    const filterControllers = valdnlisttable4db84Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['valdnlisttable4db84']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['valdnlisttable4db84']).filter((k: string) => jsonEntry['valdnlisttable4db84'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(valdnlisttable4db84Props.filterProps);
      return;
    }
    ...
    setvaldnlisttable4db84Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(valdnlisttable4db84Props.filterProps);

  }, [
    valdnlisttable4db84Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    valdnlisttable4db84Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(valdnlisttable4db84Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(valdnlisttable4db84Props.searchFilter) || {};
...
}, [valdnlisttable4db84Props.searchFilter])
  ...
  useEffect(() => {
    if (valdnlisttable4db84Props?.clearData === true) {
      setvaldnlisttable4db84([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setvaldnlisttable4db84Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [valdnlisttable4db84Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [valdnlisttable4db84Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = valdnlisttable4db84Props?.selectedIds[valdnlisttable4db84Props?.selectedIds?.length-1]
    if(Array.isArray(valdnlisttable4db84) && valdnlisttable4db84.length>0)
    {
      let data = valdnlisttable4db84.find((data:any)=>(data?.vgphtelm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
  ...
  useEffect(() => {
    if (valdnlisttable4db84Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (valdnlisttable4db84Props?.selectedIds?.length == 0){
      handleOnRowClick({},valdnlisttable4db84Props?.selectedIds)
    }
  }, [valdnlisttable4db84Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(valdnlisttable4db84?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&valdnlisttable4db84?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](valdnlisttable4db84?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: valdnlisttable4db84?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&valdnlisttable4db84?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](valdnlisttable4db84?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: valdnlisttable4db84?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (valdnlisttable4db84?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"350585c031252e28732f2f119b74db84"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={valdnlisttable4db84Props?.selectedIds}  
