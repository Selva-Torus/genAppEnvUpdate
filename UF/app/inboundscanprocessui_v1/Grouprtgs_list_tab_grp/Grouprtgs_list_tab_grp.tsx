...
const Grouprtgs_list_tab_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[], setTableData ,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData={}, controlData={}}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "6953d8e18e8150d339ea1cc4aec43579"
      );
  ...
  for(let i=0;i<tabOptions?.length;i++){
    ...
    {
      setrtgs_list_tab_grp43579((pre:any)=>({...pre,rtgs_list_tab_grp:tabOptions[i]?.id}));
      break;
    }
  }   
  /////////////
        setvalidtn_list3a9a1({...validtn_list3a9a1,isDisabled:orchestrationData?.data?.readableControls.includes("validtn_list")});
        setcmnt_list18a3b({...cmnt_list18a3b,isDisabled:orchestrationData?.data?.readableControls.includes("cmnt_list")});
  //////////////
    if (code != '') {
      let codeStates: any = {};
        codeStates['selected']  = "validtn_list",
  ...
  const handleOnChange=async(id?:string)=>{
    ...
    setrtgs_list_tab_grp43579((pre:any)=>({...pre,rtgs_list_tab_grp:id}));

  }
  const rtgs_list_tab_grp43579Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_tab_grp43579Ref.current?.setSearchParams();
    rtgs_list_tab_grp43579Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_tab_grp43579) && Object.keys(rtgs_list_tab_grp43579)?.length>0)
      {
        setrtgs_list_tab_grp43579({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_tab_grp43579Props?.refresh])
...
let tabHeaderItems : any =[
  ...
  return (
    ...
    <Tabs
      ...
      defaultActiveId={rtgs_list_tab_grp43579?.rtgs_list_tab_grp || "validtn_list"}
      activeTab={rtgs_list_tab_grp43579?.rtgs_list_tab_grp || "validtn_list"}
