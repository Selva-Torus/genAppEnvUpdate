...
const Groupcmnt_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e4c3a1adc9705084fe34102ed7d18a3b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcmnt_list18a3bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_cmnts_list")){
        setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre,...rtgs_list_cmnts_list85130,isDisabled:true}));

    }else
    {
      if(rtgs_list_cmnts_list85130?.isDisabled==null)
      {
        setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(rtgs_list_tab_grp43579?.rtgs_list_tab_grp === "cmnt_list"){
    handleOnChange()
    }
  ...
  const cmnt_list18a3bRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmnt_list18a3bRef.current?.setSearchParams();
    cmnt_list18a3bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmnt_list18a3b) && Object.keys(cmnt_list18a3b)?.length>0)
      {
        setcmnt_list18a3b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmnt_list18a3bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"cmnt_list"}))
        }}
    >
