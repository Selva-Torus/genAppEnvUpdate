...
const Grouplist_tab_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[], setTableData ,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData={}, controlData={}}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "2c2e152064ae82195c30cb6e72d6782e"
      );
  ...
  for(let i=0;i<tabOptions?.length;i++){
    ...
    {
      setlist_tab_group6782e((pre:any)=>({...pre,list_tab_group:tabOptions[i]?.id}));
      break;
    }
  }   
  /////////////
        setvalidation_listcc09d({...validation_listcc09d,isDisabled:orchestrationData?.data?.readableControls.includes("validation_list")});
        setcomment_listb158b({...comment_listb158b,isDisabled:orchestrationData?.data?.readableControls.includes("comment_list")});
  //////////////
    if (code != '') {
      let codeStates: any = {};
        codeStates['selected']  = "validation_list",
  ...
  const handleOnChange=async(id?:string)=>{
    ...
    setlist_tab_group6782e((pre:any)=>({...pre,list_tab_group:id}));

  }
  const list_tab_group6782eRef = useRef<any>(null);
  const handleClearSearch = () => {
    list_tab_group6782eRef.current?.setSearchParams();
    list_tab_group6782eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(list_tab_group6782e) && Object.keys(list_tab_group6782e)?.length>0)
      {
        setlist_tab_group6782e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [list_tab_group6782eProps?.refresh])
...
let tabHeaderItems : any =[
  ...
  return (
    ...
    <Tabs
      ...
      defaultActiveId={list_tab_group6782e?.list_tab_group || "validation_list"}
      activeTab={list_tab_group6782e?.list_tab_group || "validation_list"}
