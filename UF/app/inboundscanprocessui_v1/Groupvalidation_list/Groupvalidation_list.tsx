...
const Groupvalidation_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "87e2f73507ee87c18d388ce813fcc09d");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalidation_listcc09dProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valdnlisttable")){
        setvaldnlisttable4db84Props((pre:any)=>({...pre,...valdnlisttable4db84,isDisabled:true}));

    }else
    {
      if(valdnlisttable4db84?.isDisabled==null)
      {
        setvaldnlisttable4db84Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(list_tab_group6782e?.list_tab_group === "validation_list"){
    handleOnChange()
    }
  ...
  const validation_listcc09dRef = useRef<any>(null);
  const handleClearSearch = () => {
    validation_listcc09dRef.current?.setSearchParams();
    validation_listcc09dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validation_listcc09d) && Object.keys(validation_listcc09d)?.length>0)
      {
        setvalidation_listcc09d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validation_listcc09dProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"validation_list"}))
        }}
    >
