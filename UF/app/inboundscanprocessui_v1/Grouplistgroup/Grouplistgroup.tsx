...
const Grouplistgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6e93f89fdeb9f4e1efa447c56c697a7c");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setlistgroup97a7cProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("list_tab_group")){
        setlist_tab_group6782e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(list_tab_group6782e?.isDisabled==null)
      {
        setlist_tab_group6782e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const listgroup97a7cRef = useRef<any>(null);
  const handleClearSearch = () => {
    listgroup97a7cRef.current?.setSearchParams();
    listgroup97a7cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(listgroup97a7c) && Object.keys(listgroup97a7c)?.length>0)
      {
        setlistgroup97a7c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [listgroup97a7cProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"listgroup"}))
        }}
    >
