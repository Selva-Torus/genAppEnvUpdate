...
const Groupcomment_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1b8d062cc71ffa58dac0a018123b158b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcomment_listb158bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmntlisttable")){
        setcmntlisttable96834Props((pre:any)=>({...pre,...cmntlisttable96834,isDisabled:true}));

    }else
    {
      if(cmntlisttable96834?.isDisabled==null)
      {
        setcmntlisttable96834Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(list_tab_group6782e?.list_tab_group === "comment_list"){
    handleOnChange()
    }
  },[list_tab_group6782e?.list_tab_group])
  ...
  const comment_listb158bRef = useRef<any>(null);
  const handleClearSearch = () => {
    comment_listb158bRef.current?.setSearchParams();
    comment_listb158bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(comment_listb158b) && Object.keys(comment_listb158b)?.length>0)
      {
        setcomment_listb158b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [comment_listb158bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"comment_list"}))
        }}
    >
