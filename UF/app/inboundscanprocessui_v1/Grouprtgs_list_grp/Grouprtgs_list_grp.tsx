...
import Grouprtgs_list_tble_group  from "../Grouprtgs_list_tble_group/Grouprtgs_list_tble_group";
...
const Grouprtgs_list_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "83a42671bbe999f13d1d19b9af082cfc");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_grp82cfcProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_tble_group")){
        setrtgs_list_tble_groupe1ac5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_tble_groupe1ac5?.isDisabled==null)
      {
        setrtgs_list_tble_groupe1ac5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("group")){
        setgroup1b1aa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(group1b1aa?.isDisabled==null)
      {
        setgroup1b1aa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const rtgs_list_grp82cfcRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_grp82cfcRef.current?.setSearchParams();
    rtgs_list_grp82cfcRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_grp82cfc) && Object.keys(rtgs_list_grp82cfc)?.length>0)
      {
        setrtgs_list_grp82cfc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_grp82cfcProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_grp"}))
        }}
    >
        {allowedComponent.includes("rtgs_list_tble_group")  &&<Grouprtgs_list_tble_group  
