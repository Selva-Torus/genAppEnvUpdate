...
const Grouprtgs_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6a68f6dc67743c9aadc85138fa0f12c6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_listf12c6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_grp")){
        setrtgs_list_grp82cfc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_grp82cfc?.isDisabled==null)
      {
        setrtgs_list_grp82cfc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(control_tab_group161ff?.control_tab_group === "rtgs_list"){
    handleOnChange()
    }
  },[control_tab_group161ff?.control_tab_group])
  ...
  const rtgs_listf12c6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_listf12c6Ref.current?.setSearchParams();
    rtgs_listf12c6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_listf12c6) && Object.keys(rtgs_listf12c6)?.length>0)
      {
        setrtgs_listf12c6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_listf12c6Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list"}))
        }}
    >
