...
const Grouprtgs_list_validtn_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {vld_code_rtgs_lst274ca, setvld_code_rtgs_lst274ca}= useContext(TotalContext) as TotalContextProps;
  const {vld_reason_rtgs_listff18d, setvld_reason_rtgs_listff18d}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d5e9a4b8e350e8aad037498875a10755");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_validtn_table10755Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vld_code_rtgs_lst")){
        setvld_code_rtgs_lst274ca((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vld_code_rtgs_lst274ca?.isDisabled==null)
      {
        setvld_code_rtgs_lst274ca((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("vld_reason_rtgs_list")){
        setvld_reason_rtgs_listff18d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(vld_reason_rtgs_listff18d?.isDisabled==null)
      {
        setvld_reason_rtgs_listff18d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const handleCustomCode=async () => {
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['vld_code_rtgs_lst'] = vld_code_rtgs_lst274ca,
        codeStates['setvld_code_rtgs_lst'] = setvld_code_rtgs_lst274ca,
        codeStates['vld_reason_rtgs_list'] = vld_reason_rtgs_listff18d,
        codeStates['setvld_reason_rtgs_list'] = setvld_reason_rtgs_listff18d,
  ...
  const rtgs_list_validtn_table10755Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_validtn_table10755Ref.current?.setSearchParams();
    rtgs_list_validtn_table10755Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_validtn_table10755) && Object.keys(rtgs_list_validtn_table10755)?.length>0)
      {
        setrtgs_list_validtn_table10755({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_validtn_table10755Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_validtn_table"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_list_validtn_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_validtn_table10755Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
