...
let defaultColumns:any = [
  {
    ...
    "nodeid": "cad759e28c3f2650d35980662f2274ca",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "56dce118c734b0989f3f19d4494ff18d",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "d5e9a4b8e350e8aad037498875a10755",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_validtn_table10755 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_validtn_table10755 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_validtn_table10755 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_validtn_table10755Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      rtgs_list_validtn_table10755.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setrtgs_list_validtn_table10755Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setrtgs_list_validtn_table10755Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_validtn_table10755Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_validtn_table10755Props?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_validtn_table10755Props?.presetValues || Object.keys(rtgs_list_validtn_table10755Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'd5e9a4b8e350e8aad037498875a10755',
                elementId: 'd5e9a4b8e350e8aad037498875a10755'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_validtn_table10755(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_validtn_table10755))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_validtn_table10755Props.filterInitalLoad) return;

    const filterControllers = rtgs_list_validtn_table10755Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_validtn_table10755']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_validtn_table10755']).filter((k: string) => jsonEntry['rtgs_list_validtn_table10755'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_validtn_table10755Props.filterProps);
      return;
    }
    ...
    setrtgs_list_validtn_table10755Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_validtn_table10755Props.filterProps);

  }, [
    rtgs_list_validtn_table10755Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_validtn_table10755Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_validtn_table10755Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_validtn_table10755Props.searchFilter) || {};
...
}, [rtgs_list_validtn_table10755Props.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table10755Props?.clearData === true) {
      setrtgs_list_validtn_table10755([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_validtn_table10755Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_validtn_table10755Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_validtn_table10755Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_validtn_table10755Props?.selectedIds[rtgs_list_validtn_table10755Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_validtn_table10755) && rtgs_list_validtn_table10755.length>0)
    {
      let data = rtgs_list_validtn_table10755.find((data:any)=>(data?.vgphtelm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table10755Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_validtn_table10755Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_validtn_table10755Props?.selectedIds)
    }
  }, [rtgs_list_validtn_table10755Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_validtn_table10755?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_validtn_table10755?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_validtn_table10755?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table10755?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_validtn_table10755?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_validtn_table10755?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table10755?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_validtn_table10755?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"d5e9a4b8e350e8aad037498875a10755"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_validtn_table10755Props?.selectedIds}  
