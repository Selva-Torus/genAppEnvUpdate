...
const Groupcontrolgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "5a4e89d4eb0db96f18eb6619ebafbb48");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcontrolgroupfbb48Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("control_tab_group")){
        setcontrol_tab_group161ff((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(control_tab_group161ff?.isDisabled==null)
      {
        setcontrol_tab_group161ff((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const controlgroupfbb48Ref = useRef<any>(null);
  const handleClearSearch = () => {
    controlgroupfbb48Ref.current?.setSearchParams();
    controlgroupfbb48Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(controlgroupfbb48) && Object.keys(controlgroupfbb48)?.length>0)
      {
        setcontrolgroupfbb48({})
      }
    }else 
      prevRefreshRef.current= true
  }, [controlgroupfbb48Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"controlgroup"}))
        }}
    >
