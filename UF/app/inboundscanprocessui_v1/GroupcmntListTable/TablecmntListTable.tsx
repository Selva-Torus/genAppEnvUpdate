...
let defaultColumns:any = [
  {
    "id": "cmnts",
    "nodeid": "947c0a4e9477951734ad4086f76a418a",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const TablecmntListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {cmntsa418a, setcmntsa418a}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "ac53fbfc1a67cb0563533d6ce2596834",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, cmntlisttable96834 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, cmntlisttable96834 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, cmntlisttable96834 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setcmntlisttable96834Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      cmntlisttable96834.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setcmntlisttable96834Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setcmntlisttable96834Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['cmnts'] = cmntsa418a,
        codeStates['setcmnts'] = setcmntsa418a,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setcmntlisttable96834Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:cmntlisttable96834Props?.presetValues?.vgphstm_uuid}
    ...
    if(!cmntlisttable96834Props?.presetValues || Object.keys(cmntlisttable96834Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'ac53fbfc1a67cb0563533d6ce2596834',
                elementId: 'ac53fbfc1a67cb0563533d6ce2596834'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setcmntlisttable96834(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(cmntlisttable96834))
    {
      GetTableDetails()
    }
  }, [cmntlisttable96834])
  ...
  useEffect(() => {
    ...
    if (cmntlisttable96834Props.filterInitalLoad) return;

    const filterControllers = cmntlisttable96834Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['cmntlisttable96834']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['cmntlisttable96834']).filter((k: string) => jsonEntry['cmntlisttable96834'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(cmntlisttable96834Props.filterProps);
      return;
    }
    ...
    setcmntlisttable96834Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(cmntlisttable96834Props.filterProps);

  }, [
    cmntlisttable96834Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    cmntlisttable96834Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(cmntlisttable96834Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(cmntlisttable96834Props.searchFilter) || {};
...
}, [cmntlisttable96834Props.searchFilter])
  ...
  useEffect(() => {
    if (cmntlisttable96834Props?.clearData === true) {
      setcmntlisttable96834([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setcmntlisttable96834Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [cmntlisttable96834Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [cmntlisttable96834Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = cmntlisttable96834Props?.selectedIds[cmntlisttable96834Props?.selectedIds?.length-1]
    if(Array.isArray(cmntlisttable96834) && cmntlisttable96834.length>0)
    {
      let data = cmntlisttable96834.find((data:any)=>(data?.vgphtlm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtlm_id",
  ...
  useEffect(() => {
    if (cmntlisttable96834Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (cmntlisttable96834Props?.selectedIds?.length == 0){
      handleOnRowClick({},cmntlisttable96834Props?.selectedIds)
    }
  }, [cmntlisttable96834Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(cmntlisttable96834?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&cmntlisttable96834?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](cmntlisttable96834?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: cmntlisttable96834?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&cmntlisttable96834?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](cmntlisttable96834?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: cmntlisttable96834?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (cmntlisttable96834?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"ac53fbfc1a67cb0563533d6ce2596834"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={cmntlisttable96834Props?.selectedIds}  
