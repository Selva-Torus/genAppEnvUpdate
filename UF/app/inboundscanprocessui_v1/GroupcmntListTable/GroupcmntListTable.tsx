...
const Groupcmntlisttable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ac53fbfc1a67cb0563533d6ce2596834");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcmntlisttable96834Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts")){
        setcmntsa418a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cmntsa418a?.isDisabled==null)
      {
        setcmntsa418a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const cmntlisttable96834Ref = useRef<any>(null);
  const handleClearSearch = () => {
    cmntlisttable96834Ref.current?.setSearchParams();
    cmntlisttable96834Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmntlisttable96834) && Object.keys(cmntlisttable96834)?.length>0)
      {
        setcmntlisttable96834({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmntlisttable96834Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"cmntlisttable"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<TablecmntListTable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={cmntlisttable96834Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
