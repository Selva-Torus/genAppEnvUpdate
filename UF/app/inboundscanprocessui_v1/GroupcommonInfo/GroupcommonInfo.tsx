...
const Groupcommoninfo = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "229399c32b1d4a11f4f06811a49d7eda");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcommoninfod7edaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("common_info")){
        setcommon_infoe66a9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(common_infoe66a9?.isDisabled==null)
      {
        setcommon_infoe66a9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account")){
        setdr_account953ea((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account953ea?.isDisabled==null)
      {
        setdr_account953ea((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name")){
        setdr_named06e2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_named06e2?.isDisabled==null)
      {
        setdr_named06e2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("base_currency")){
        setbase_currency57d7d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(base_currency57d7d?.isDisabled==null)
      {
        setbase_currency57d7d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_groupb9855((prev:any) => ({ ...prev, ...commoninfod7eda }));
  }
  ...
  const commoninfod7edaRef = useRef<any>(null);
  const handleClearSearch = () => {
    commoninfod7edaRef.current?.setSearchParams();
    commoninfod7edaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(commoninfod7eda) && Object.keys(commoninfod7eda)?.length>0)
      {
        setcommoninfod7eda({})
      }
    }else 
      prevRefreshRef.current= true
  }, [commoninfod7edaProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg p-3 pr-4 pl-4 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"commoninfo"}))
        }}
    >
          {allowedControls.includes("common_info") ?<Textcommon_info   /* e66a9 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dr_account") ?<TextInputdr_account   /* 953ea */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dr_name") ?<TextInputdr_name   /* d06e2 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("base_currency") ?<TextInputbase_currency   /* 57d7d */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
