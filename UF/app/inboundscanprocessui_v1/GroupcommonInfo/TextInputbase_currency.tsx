...
const TextInputbase_currency = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.dr_currency"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1|229399c32b1d4a11f4f06811a49d7eda|288a1082ca02bfc45fc5a499edf57d7d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,inboundScanProcessUi_v1:{...pre?.inboundScanProcessUi_v1,dr_currency:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfod7eda((prev: any) => ({ ...prev, dr_currency: +e.target.value }));
    }
    else{
    setcommoninfod7eda((prev: any) => ({ ...prev, dr_currency: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "288a1082ca02bfc45fc5a499edf57d7d"
      );
      ...
      //     controlId: "288a1082ca02bfc45fc5a499edf57d7d",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "288a1082ca02bfc45fc5a499edf57d7d") {
        handleChange({target:{value:commoninfod7edaRef?.current?.dr_currency||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "288a1082ca02bfc45fc5a499edf57d7d") {
        handleBlur({target:{value:commoninfod7edaRef?.current?.dr_currency||""}});
      }
    };
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setcommoninfod7eda((pre:any)=>({...pre,dr_currency:dfd_scansaveprocessdfd_v1Props.data[0]?.dr_currency}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (base_currency57d7d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `9 / 21`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={commoninfod7eda?.dr_currency||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.inboundScanProcessUi_v1?.dr_currency ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
