...
import Buttonforce_pass  from "./Buttonforce_pass";
import Buttonip_approve  from "./Buttonip_approve";
...
const Groupbutton_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "14e844df4cccf4759585851079bb9855");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbutton_groupb9855Props((pre:any)=>({...pre,isHaveRule:true}))
      actionRuleHandle(orchestrationData?.data?.rule.nodes,{...decodedTokenObj,session:decodedTokenObj,
});
  }
    
  /////////////
    if(orchestrationData?.data?.readableControls.includes("hold")){
        sethold12b6e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(hold12b6e?.isDisabled==null)
      {
        sethold12b6e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("force_pass")){
        setforce_pass93cf0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(force_pass93cf0?.isDisabled==null)
      {
        setforce_pass93cf0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("ip_approve")){
        setip_approve2a0bf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(ip_approve2a0bf?.isDisabled==null)
      {
        setip_approve2a0bf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("return")){
        setreturn0f9cc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(return0f9cc?.isDisabled==null)
      {
        setreturn0f9cc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cancel")){
        setcancel568c6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cancel568c6?.isDisabled==null)
      {
        setcancel568c6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const button_groupb9855Ref = useRef<any>(null);
  const handleClearSearch = () => {
    button_groupb9855Ref.current?.setSearchParams();
    button_groupb9855Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setbutton_groupb9855Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setbutton_groupb9855Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(button_groupb9855) && Object.keys(button_groupb9855)?.length>0)
      {
        setbutton_groupb9855({})
      }
    }else 
      prevRefreshRef.current= true
  }, [button_groupb9855Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !mr-8 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"button_group"}))
        }}
    >
        {        ((ruleData?.length>0 && "hold" in ButtonGoRuleData)?ButtonGoRuleData["hold"]:true) && 
          allowedControls.includes("hold")  ?            <Buttonhold tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
        {        ((ruleData?.length>0 && "force_pass" in ButtonGoRuleData)?ButtonGoRuleData["force_pass"]:true) && 
          allowedControls.includes("force_pass")  ?            <Buttonforce_pass tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
        {        ((ruleData?.length>0 && "ip_approve" in ButtonGoRuleData)?ButtonGoRuleData["ip_approve"]:true) && 
          allowedControls.includes("ip_approve")  ?            <Buttonip_approve tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
        {        ((ruleData?.length>0 && "return" in ButtonGoRuleData)?ButtonGoRuleData["return"]:true) && 
          allowedControls.includes("return")  ?            <Buttonreturn tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
