 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "66b7db538775088dcdfd056f837568c6"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in button_groupb9855Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_groupb9855Props?.dynamicActionRule?.cancel,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in inboundscanprocessui_v1?.button_group && inboundscanprocessui_v1?.button_group["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(inboundscanprocessui_v1?.button_group["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(inboundscanprocessui_v1?.button_group?.cancel?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "66b7db538775088dcdfd056f837568c6") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "cancel568c6");
    return () => {
  ...
  useEffect(()=>{
    if (!cancel568c6?.trigger) return;
      if(cancel568c6?.trigger){
      setcancel568c6((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setbutton_groupb9855((prev: any) => ({ ...prev, cancel: true }));
        //onClick

    // showArtifact
    let filterProps2: any =  [];
      let filterData2 = await getFilterProps(filterProps2,{...overallgroup1218f,...controlgroupfbb48,...control_tab_group161ff,...rtgs_info5957a,...allcontrolsb8c72,...commoninfod7eda,...basicinfoffb0a,...additionalinfo4baba,...listgroup97a7c,...list_tab_group6782e,...validation_listcc09d,...comment_listb158b,...rtgs_listf12c6,...rtgs_list_grp82cfc,...rtgs_list_tble_groupe1ac5,...group1b1aa,...rtgs_list_tab_grp43579,...validtn_list3a9a1,...cmnt_list18a3b,...button_groupb9855});
    settransactionproduct_v1Props([...filterData2 ]);
    ...
    //riseListen
    // for group
    const mergedGetFormData4 = {...button_groupb9855Ref.current, ...control_tab_group161ff};
    setbutton_groupb9855(mergedGetFormData4);
    button_groupb9855Ref.current = mergedGetFormData4;

    const mergedGetFormDataProps4 = {...button_groupb9855PropsRef.current, ...control_tab_group161ffProps};
    setbutton_groupb9855Props(mergedGetFormDataProps4);
    button_groupb9855PropsRef.current = mergedGetFormDataProps4;
    // clearHandler riseListen
    // for group
    Object.keys(overallgroup1218f).map((keys:any)=>{         
      overallgroup1218f[keys]="";
    })
    setoverallgroup1218f({...overallgroup1218f});
    setValidate({});
    setValidateRefetch({
      value:false,
      init:0
    ...
    setValidateRefetch({
      ...
      setIsProcessing(false);
        setbutton_groupb9855((prev: any) => ({ ...prev, cancel: false }));
  ...
  }, [inboundscanprocessui_v1?.button_group?.cancel,button_groupb9855Props?.dynamicActionRule?.cancel,control_tab_group161ff])

 if (cancel568c6?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {cancel568c6?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
