...
let defaultColumns:any = [
  {
    "id": "cmnts_rtgs_list",
    "nodeid": "5b72b6ce89f0bb33fafda114db7d0091",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_cmnts_list = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const tableName = "vgph_tran_log_main"
 ...
 /////////////
  ...
  const {cmnts_rtgs_listd0091, setcmnts_rtgs_listd0091}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "aa8324302c4cf6d0af7179b8c0a85130",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list85130 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list85130 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, rtgs_list_cmnts_list85130 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.vgphtlm_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      rtgs_list_cmnts_list85130.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.vgphtlm_id){
          selectedData?.push(item)
          postIds.push(item.vgphtlm_id)
      ...
      //////////
      //////////
        setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
    else if(needLockingAndRule.lockMode==='Multi'){
      // its for ui level selected list show for multi select
      rtgs_list_cmnts_list85130.filter((item:any,id:number)=>{
        if (ids.includes(item.vgphtlm_id)){
          selectedData?.push(item)
          postIds.push(item.vgphtlm_id) 
          processIds.push(item?.trs_process_id)
        } 
      ...
      setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['cmnts_rtgs_list'] = cmnts_rtgs_listd0091,
        codeStates['setcmnts_rtgs_list'] = setcmnts_rtgs_listd0091,
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphstm_uuid"]:rtgs_list_cmnts_list85130Props?.presetValues?.vgphstm_uuid}
    ...
    if(!rtgs_list_cmnts_list85130Props?.presetValues || Object.keys(rtgs_list_cmnts_list85130Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'aa8324302c4cf6d0af7179b8c0a85130',
                elementId: 'aa8324302c4cf6d0af7179b8c0a85130'
              },
            }
          }else
          {
      ...
      if (api_pagination?.data?.records.length > 0) {
        ...
        result.vgphtlm_id = emp?.vgphtlm_id;
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrtgs_list_cmnts_list85130(api_pagination?.data?.records||[])
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,vgphtlm_id:filtertedData[i]?.vgphtlm_id}
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    if(allData.length!=0)
    {
      ...
      let row:any = {}
      allData?.map((data:any,i:any)=>{
        if(data?.vgphtlm_id==item?.vgphtlm_id)
        {
          index=i
          row=data
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_list_cmnts_list85130))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (rtgs_list_cmnts_list85130Props.filterInitalLoad) return;

    const filterControllers = rtgs_list_cmnts_list85130Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['rtgs_list_cmnts_list85130']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['rtgs_list_cmnts_list85130']).filter((k: string) => jsonEntry['rtgs_list_cmnts_list85130'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(rtgs_list_cmnts_list85130Props.filterProps);
      return;
    }
    ...
    setrtgs_list_cmnts_list85130Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(rtgs_list_cmnts_list85130Props.filterProps);

  }, [
    rtgs_list_cmnts_list85130Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    rtgs_list_cmnts_list85130Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(rtgs_list_cmnts_list85130Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(rtgs_list_cmnts_list85130Props.searchFilter) || {};
...
}, [rtgs_list_cmnts_list85130Props.searchFilter])
  ...
  useEffect(() => {
    if (rtgs_list_cmnts_list85130Props?.clearData === true) {
      setrtgs_list_cmnts_list85130([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrtgs_list_cmnts_list85130Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_cmnts_list85130Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_cmnts_list85130Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_cmnts_list85130Props?.selectedIds[rtgs_list_cmnts_list85130Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_cmnts_list85130) && rtgs_list_cmnts_list85130.length>0)
    {
      let data = rtgs_list_cmnts_list85130.find((data:any)=>(data?.vgphtlm_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtlm_id",
        value: data["vgphtlm_id"],
        parentData: data
      })
    }
  }
  useEffect(() => {
    if (rtgs_list_cmnts_list85130Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_cmnts_list85130Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_cmnts_list85130Props?.selectedIds)
    }
  }, [rtgs_list_cmnts_list85130Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
      ...
      allData?.map((item:any,i:any)=>{
        if(item?.vgphtlm_id==data?.vgphtlm_id)
        {
          index=i
          row=item
        }
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_cmnts_list85130?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_cmnts_list85130?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_cmnts_list85130?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list85130?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_cmnts_list85130?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_cmnts_list85130?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list85130?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_cmnts_list85130?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"aa8324302c4cf6d0af7179b8c0a85130"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={rtgs_list_cmnts_list85130Props?.selectedIds}  
