...
const Grouprtgs_list_cmnts_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "aa8324302c4cf6d0af7179b8c0a85130");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_cmnts_list85130Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts_rtgs_list")){
        setcmnts_rtgs_listd0091((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cmnts_rtgs_listd0091?.isDisabled==null)
      {
        setcmnts_rtgs_listd0091((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const rtgs_list_cmnts_list85130Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_cmnts_list85130Ref.current?.setSearchParams();
    rtgs_list_cmnts_list85130Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_cmnts_list85130) && Object.keys(rtgs_list_cmnts_list85130)?.length>0)
      {
        setrtgs_list_cmnts_list85130({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_cmnts_list85130Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !pl-1 pr-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_list_cmnts_list"}))
        }}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablertgs_list_cmnts_list headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_cmnts_list85130Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
