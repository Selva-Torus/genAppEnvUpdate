...
const Grouprtgs_info = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "32cd4adf409f4d81f12d3d29e715957a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_info5957aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("allcontrols")){
        setallcontrolsb8c72((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(allcontrolsb8c72?.isDisabled==null)
      {
        setallcontrolsb8c72((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(control_tab_group161ff?.control_tab_group === "rtgs_info"){
    handleOnChange()
    }
  ...
  const rtgs_info5957aRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_info5957aRef.current?.setSearchParams();
    rtgs_info5957aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_info5957a) && Object.keys(rtgs_info5957a)?.length>0)
      {
        setrtgs_info5957a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_info5957aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"rtgs_info"}))
        }}
    >
