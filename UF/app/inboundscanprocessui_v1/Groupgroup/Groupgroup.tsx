...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "18967abf1bde5146a754c9200bc1b1aa");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroup1b1aaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_tab_grp")){
        setrtgs_list_tab_grp43579((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(rtgs_list_tab_grp43579?.isDisabled==null)
      {
        setrtgs_list_tab_grp43579((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const group1b1aaRef = useRef<any>(null);
  const handleClearSearch = () => {
    group1b1aaRef.current?.setSearchParams();
    group1b1aaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group1b1aa) && Object.keys(group1b1aa)?.length>0)
      {
        setgroup1b1aa({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group1b1aaProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-lg ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
