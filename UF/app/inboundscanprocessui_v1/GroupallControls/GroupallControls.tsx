...
const Groupallcontrols = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "89b7107bfa3beade12d66a26193b8c72");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setallcontrolsb8c72Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("commoninfo")){
        setcommoninfod7eda((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(commoninfod7eda?.isDisabled==null)
      {
        setcommoninfod7eda((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("basicinfo")){
        setbasicinfoffb0a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(basicinfoffb0a?.isDisabled==null)
      {
        setbasicinfoffb0a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("additionalinfo")){
        setadditionalinfo4baba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(additionalinfo4baba?.isDisabled==null)
      {
        setadditionalinfo4baba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("listgroup")){
        setlistgroup97a7c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(listgroup97a7c?.isDisabled==null)
      {
        setlistgroup97a7c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const allcontrolsb8c72Ref = useRef<any>(null);
  const handleClearSearch = () => {
    allcontrolsb8c72Ref.current?.setSearchParams();
    allcontrolsb8c72Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(allcontrolsb8c72) && Object.keys(allcontrolsb8c72)?.length>0)
      {
        setallcontrolsb8c72({})
      }
    }else 
      prevRefreshRef.current= true
  }, [allcontrolsb8c72Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setinboundscanprocessui_v1((pre:any)=>({...pre,_selectedGroup_:"allcontrols"}))
        }}
    >
