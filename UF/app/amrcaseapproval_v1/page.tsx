...
export default function PageAmrcaseapprovalV1({ onReady }: { onReady?: () => void } = {}) {
    ...
    async function addcase_v1DFD(pagination:any): Promise<void>{
        ...
        if(amrcaseapproval_v1Props.length > 0){
          for(let i=0;i< amrcaseapproval_v1Props.length;i++){
            if(amrcaseapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:addCase:AFVK:v1"){
  ...
  useEffect(()=>{
    ...
    async function doctypenamecombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(amrcaseapproval_v1Props.length > 0){
          for(let i=0;i< amrcaseapproval_v1Props.length;i++){
            if(amrcaseapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:docTypeNameCombo:AFVK:v1"){
              // delete amrcaseapproval_v1Props[i].DFDkey;
              let temp=structuredClone(amrcaseapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function amrchecklistcombo_v1DFD(pagination:any): Promise<void>{
        ...
        if(amrcaseapproval_v1Props.length > 0){
          for(let i=0;i< amrcaseapproval_v1Props.length;i++){
            if(amrcaseapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:amrChecklistCombo:AFVK:v1"){
              // delete amrcaseapproval_v1Props[i].DFDkey;
              let temp=structuredClone(amrcaseapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function venuespecialrules_v1DFD(pagination:any): Promise<void>{
        ...
        if(amrcaseapproval_v1Props.length > 0){
          for(let i=0;i< amrcaseapproval_v1Props.length;i++){
            if(amrcaseapproval_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:LAP:AFGK:LAP:AFK:venueSpecialRules:AFVK:v1"){
              // delete amrcaseapproval_v1Props[i].DFDkey;
              let temp=structuredClone(amrcaseapproval_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setamrcaseapproval_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    setamrcaseapproval_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])

  useEffect(()=>{
    if(amrcaseapproval_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(amrcaseapproval_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
