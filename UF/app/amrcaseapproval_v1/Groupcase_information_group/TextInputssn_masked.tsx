...
const TextInputssn_masked = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|9b270458f4e722fe38d9cd7c50335ed3|faa244d50bf7a9bdb449951536a36fce"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,ssn_masked:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group35ed3((prev: any) => ({ ...prev, ssn_masked: +e.target.value }));
    }
    else{
    setcase_information_group35ed3((prev: any) => ({ ...prev, ssn_masked: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "faa244d50bf7a9bdb449951536a36fce"
      );
      ...
      //     controlId: "faa244d50bf7a9bdb449951536a36fce",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "faa244d50bf7a9bdb449951536a36fce") {
        handleChange({target:{value:case_information_group35ed3Ref?.current?.ssn_masked||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "faa244d50bf7a9bdb449951536a36fce") {
        handleBlur({target:{value:case_information_group35ed3Ref?.current?.ssn_masked||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group35ed3((pre:any)=>({...pre,ssn_masked:dfd_addcase_v1Props.data[0]?.ssn_masked}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (ssn_masked36fce?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group35ed3?.ssn_masked||""}
         disabled= {ssn_masked36fce?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="SSN"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.ssn_masked ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
