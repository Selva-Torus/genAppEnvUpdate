...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,charge_off_date:undefined}}));
  if (!date) {
    setcase_information_group35ed3((prev: any) => ({ ...prev, charge_off_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group35ed3((prev: any) => ({ ...prev, charge_off_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "4ff37554ffabf75e5d6af8c9daaf5bba"
      );
...
if (charge_off_datef5bba?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `52 / 64`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group35ed3?.charge_off_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {charge_off_datef5bba?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Charge Off Date"
      dateValidation=""
      validationState={validate?.AMRCaseApproval_v1?.charge_off_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
