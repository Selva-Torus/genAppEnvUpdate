...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,dob:undefined}}));
  if (!date) {
    setcase_information_group35ed3((prev: any) => ({ ...prev, dob: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group35ed3((prev: any) => ({ ...prev, dob: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "0d04d534b3dbeec68ba1493fc4719a93"
      );
...
return (
  ...
  style={{gridColumn: `13 / 25`,gridRow: `25 / 37`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group35ed3?.dob}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {dob19a93?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="DOB"
      dateValidation=""
      validationState={validate?.AMRCaseApproval_v1?.dob ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
