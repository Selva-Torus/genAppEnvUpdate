...
const TextInputcase_display_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|9b270458f4e722fe38d9cd7c50335ed3|a0958d2ff2396e1fd9531a1b8643ba0a"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,case_display_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group35ed3((prev: any) => ({ ...prev, case_display_id: +e.target.value }));
    }
    else{
    setcase_information_group35ed3((prev: any) => ({ ...prev, case_display_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a0958d2ff2396e1fd9531a1b8643ba0a"
      );
      ...
      //     controlId: "a0958d2ff2396e1fd9531a1b8643ba0a",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a0958d2ff2396e1fd9531a1b8643ba0a") {
        handleChange({target:{value:case_information_group35ed3Ref?.current?.case_display_id||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a0958d2ff2396e1fd9531a1b8643ba0a") {
        handleBlur({target:{value:case_information_group35ed3Ref?.current?.case_display_id||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group35ed3((pre:any)=>({...pre,case_display_id:dfd_addcase_v1Props.data[0]?.case_display_id}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (case_display_id3ba0a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group35ed3?.case_display_id||""}
         disabled= {case_display_id3ba0a?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.case_display_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
