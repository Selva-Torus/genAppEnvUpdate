...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|9b270458f4e722fe38d9cd7c50335ed3|9827e44dca376633123b89acf8aa603a"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,debtor_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcase_information_group35ed3((prev: any) => ({ ...prev, debtor_name: +e.target.value }));
    }
    else{
    setcase_information_group35ed3((prev: any) => ({ ...prev, debtor_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9827e44dca376633123b89acf8aa603a"
      );
      ...
      //     controlId: "9827e44dca376633123b89acf8aa603a",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9827e44dca376633123b89acf8aa603a") {
        handleChange({target:{value:case_information_group35ed3Ref?.current?.debtor_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9827e44dca376633123b89acf8aa603a") {
        handleBlur({target:{value:case_information_group35ed3Ref?.current?.debtor_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setcase_information_group35ed3((pre:any)=>({...pre,debtor_name:dfd_addcase_v1Props.data[0]?.debtor_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (debtor_namea603a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={case_information_group35ed3?.debtor_name||""}
         disabled= {debtor_namea603a?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Debtor"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.debtor_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
