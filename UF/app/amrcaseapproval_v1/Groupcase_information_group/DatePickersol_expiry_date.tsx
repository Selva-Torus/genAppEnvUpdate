...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,sol_expiry_date:undefined}}));
  if (!date) {
    setcase_information_group35ed3((prev: any) => ({ ...prev, sol_expiry_date: null }));
    return;
  }
  const now = new Date();
  ...
  setcase_information_group35ed3((prev: any) => ({ ...prev, sol_expiry_date: isoDate }))
  }catch (err: any) {
...
const handleBlur=async () => {
    ...
    const orchestrationData : any = getControlOrchestrationData(
        ...
        "3685ca9ad8af5d0bc06141b24d93775f"
      );
...
if (sol_expiry_date3775f?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `1 / 13`,gridRow: `98 / 110`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className=""
      //label={keyset("")}
      value={case_information_group35ed3?.sol_expiry_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ false }
      readOnly={true}
      disabled= {sol_expiry_date3775f?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="SOL Expires"
      dateValidation=""
      validationState={validate?.AMRCaseApproval_v1?.sol_expiry_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
