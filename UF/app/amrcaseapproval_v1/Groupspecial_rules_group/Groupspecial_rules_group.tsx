...
const Groupspecial_rules_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {special_rules_textdd5e9, setspecial_rules_textdd5e9}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9385e35d90348ac4d93446d14a5c1585");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setspecial_rules_groupc1585Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("special_rules_text")){
        setspecial_rules_textdd5e9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_textdd5e9?.isDisabled==null)
      {
        setspecial_rules_textdd5e9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules")){
        setspecial_rules1fc30((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules1fc30?.isDisabled==null)
      {
        setspecial_rules1fc30((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['special_rules_text'] = special_rules_textdd5e9,
        codeStates['setspecial_rules_text'] = setspecial_rules_textdd5e9,
  ...
  const special_rules_groupc1585Ref = useRef<any>(null);
  const handleClearSearch = () => {
    special_rules_groupc1585Ref.current?.setSearchParams();
    special_rules_groupc1585Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(special_rules_groupc1585) && Object.keys(special_rules_groupc1585)?.length>0)
      {
        setspecial_rules_groupc1585({})
      }
    }else 
      prevRefreshRef.current= true
  }, [special_rules_groupc1585Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"special_rules_group"}))
        }}
    >
        {allowedComponent.includes("special_rules")  &&<Groupspecial_rules  
          ...
          {allowedControls.includes("special_rules_text") ?<Textspecial_rules_text   /* dd5e9 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
