...
const Grouptotal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9b0226dffcefd16acc4bd36b7ace6175");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settotal_groupe6175Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_balance_text")){
        settotal_balance_texta06df((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balance_texta06df?.isDisabled==null)
      {
        settotal_balance_texta06df((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_balance")){
        settotal_balance92fd1((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_balance92fd1?.isDisabled==null)
      {
        settotal_balance92fd1((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['total_balance_text'] = total_balance_texta06df,
        codeStates['settotal_balance_text'] = settotal_balance_texta06df,
        codeStates['total_balance'] = total_balance92fd1,
        codeStates['settotal_balance'] = settotal_balance92fd1,
  ...
  const total_groupe6175Ref = useRef<any>(null);
  const handleClearSearch = () => {
    total_groupe6175Ref.current?.setSearchParams();
    total_groupe6175Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(total_groupe6175) && Object.keys(total_groupe6175)?.length>0)
      {
        settotal_groupe6175({})
      }
    }else 
      prevRefreshRef.current= true
  }, [total_groupe6175Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"total_group"}))
        }}
    >
          {allowedControls.includes("total_balance_text") ?<Texttotal_balance_text   /* a06df */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("total_balance") ?<Texttotal_balance   /* 92fd1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
