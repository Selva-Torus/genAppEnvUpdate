...
const Texttotal_balance = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {total_balance_texta06df, settotal_balance_texta06df}= useContext(TotalContext) as TotalContextProps;
  const {total_balance92fd1, settotal_balance92fd1}= useContext(TotalContext) as TotalContextProps;
  ...
  const {total_balance92fd1Props, settotal_balance92fd1Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        settotal_groupe6175((pre: any) => ({
          ...pre,
          total_balance: total_balance92fd1Props?.filteredData?.length > 0
            ? total_balance92fd1Props?.filteredData[0]?.total_balance
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !total_groupe6175.total_balance){
        settotal_groupe6175((pre:any)=>({...pre,total_balance:dfd_addcase_v1Props[0]?.total_balance}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[total_balance92fd1?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !total_groupe6175.total_balance){
    settotal_groupe6175((pre:any)=>({...pre,total_balance:dfd_addcase_v1Props[0]?.total_balance}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!total_balance92fd1Props?.filterProps) return;
    handleMapperValue(total_balance92fd1Props?.filterProps,total_balance92fd1Props?.filterFlag);
  },[total_balance92fd1Props?.filterProps])

  if (total_balance92fd1?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.total_balance : (total_groupe6175?.total_balance || ""))}
</Text>
  </div>
  )
}
