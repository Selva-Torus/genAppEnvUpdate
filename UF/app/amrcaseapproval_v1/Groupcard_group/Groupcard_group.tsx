...
const Groupcard_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "8577bf096548dea2319b4f9d6a4e78fa");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcard_groupe78faProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_group")){
        setprincipal_group9ae9f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_group9ae9f?.isDisabled==null)
      {
        setprincipal_group9ae9f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("intrest_group")){
        setintrest_group8df75((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_group8df75?.isDisabled==null)
      {
        setintrest_group8df75((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees_group")){
        setfees_groupac23b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_groupac23b?.isDisabled==null)
      {
        setfees_groupac23b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("total_group")){
        settotal_groupe6175((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(total_groupe6175?.isDisabled==null)
      {
        settotal_groupe6175((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const card_groupe78faRef = useRef<any>(null);
  const handleClearSearch = () => {
    card_groupe78faRef.current?.setSearchParams();
    card_groupe78faRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card_groupe78fa) && Object.keys(card_groupe78fa)?.length>0)
      {
        setcard_groupe78fa({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card_groupe78faProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"card_group"}))
        }}
    >
