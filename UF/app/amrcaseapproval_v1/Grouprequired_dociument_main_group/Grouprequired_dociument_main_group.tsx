...
const Grouprequired_dociument_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {reeq_doc_text01111, setreeq_doc_text01111}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "a434d4b5bf7ed57338a6850b2b6999a8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrequired_dociument_main_group999a8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reeq_doc_text")){
        setreeq_doc_text01111((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reeq_doc_text01111?.isDisabled==null)
      {
        setreeq_doc_text01111((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("doc_table")){
        setdoc_table45b8d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(doc_table45b8d?.isDisabled==null)
      {
        setdoc_table45b8d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['reeq_doc_text'] = reeq_doc_text01111,
        codeStates['setreeq_doc_text'] = setreeq_doc_text01111,
  ...
  const required_dociument_main_group999a8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    required_dociument_main_group999a8Ref.current?.setSearchParams();
    required_dociument_main_group999a8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(required_dociument_main_group999a8) && Object.keys(required_dociument_main_group999a8)?.length>0)
      {
        setrequired_dociument_main_group999a8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [required_dociument_main_group999a8Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"required_dociument_main_group"}))
        }}
    >
        {allowedComponent.includes("doc_table")  &&<Groupdoc_table  
          ...
          {allowedControls.includes("reeq_doc_text") ?<Textreeq_doc_text   /* 01111 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
