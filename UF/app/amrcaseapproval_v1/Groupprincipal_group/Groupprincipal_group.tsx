...
const Groupprincipal_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {principal_textb6dc5, setprincipal_textb6dc5}= useContext(TotalContext) as TotalContextProps;
  const {principalae986, setprincipalae986}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "033ced1190dd01f35c5cb534c759ae9f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setprincipal_group9ae9fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("principal_text")){
        setprincipal_textb6dc5((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principal_textb6dc5?.isDisabled==null)
      {
        setprincipal_textb6dc5((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("principal")){
        setprincipalae986((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(principalae986?.isDisabled==null)
      {
        setprincipalae986((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['principal_text'] = principal_textb6dc5,
        codeStates['setprincipal_text'] = setprincipal_textb6dc5,
        codeStates['principal'] = principalae986,
        codeStates['setprincipal'] = setprincipalae986,
  ...
  const principal_group9ae9fRef = useRef<any>(null);
  const handleClearSearch = () => {
    principal_group9ae9fRef.current?.setSearchParams();
    principal_group9ae9fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(principal_group9ae9f) && Object.keys(principal_group9ae9f)?.length>0)
      {
        setprincipal_group9ae9f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [principal_group9ae9fProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"principal_group"}))
        }}
    >
          {allowedControls.includes("principal_text") ?<Textprincipal_text   /* b6dc5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("principal") ?<Textprincipal   /* ae986 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
