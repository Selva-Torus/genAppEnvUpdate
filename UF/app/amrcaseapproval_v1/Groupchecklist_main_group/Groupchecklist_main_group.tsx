...
const Groupchecklist_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {valid_checklist_text6a0da, setvalid_checklist_text6a0da}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "9eba77b8dbbb22ec37f00f13f68da0ff");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setchecklist_main_groupda0ffProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valid_checklist_text")){
        setvalid_checklist_text6a0da((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(valid_checklist_text6a0da?.isDisabled==null)
      {
        setvalid_checklist_text6a0da((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_table")){
        setchecklist_table0e25b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_table0e25b?.isDisabled==null)
      {
        setchecklist_table0e25b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['valid_checklist_text'] = valid_checklist_text6a0da,
        codeStates['setvalid_checklist_text'] = setvalid_checklist_text6a0da,
  ...
  const checklist_main_groupda0ffRef = useRef<any>(null);
  const handleClearSearch = () => {
    checklist_main_groupda0ffRef.current?.setSearchParams();
    checklist_main_groupda0ffRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(checklist_main_groupda0ff) && Object.keys(checklist_main_groupda0ff)?.length>0)
      {
        setchecklist_main_groupda0ff({})
      }
    }else 
      prevRefreshRef.current= true
  }, [checklist_main_groupda0ffProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"checklist_main_group"}))
        }}
    >
        {allowedComponent.includes("checklist_table")  &&<Groupchecklist_table  
          ...
          {allowedControls.includes("valid_checklist_text") ?<Textvalid_checklist_text   /* 6a0da */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
