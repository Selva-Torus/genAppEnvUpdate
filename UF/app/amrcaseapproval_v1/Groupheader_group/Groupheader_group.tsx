...
const Groupheader_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {craete_header_text26dc6, setcraete_header_text26dc6}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "7a298065d6c77511b58852715c7bae8a");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setheader_groupbae8aProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("craete_header_text")){
        setcraete_header_text26dc6((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(craete_header_text26dc6?.isDisabled==null)
      {
        setcraete_header_text26dc6((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['craete_header_text'] = craete_header_text26dc6,
        codeStates['setcraete_header_text'] = setcraete_header_text26dc6,
  ...
  const header_groupbae8aRef = useRef<any>(null);
  const handleClearSearch = () => {
    header_groupbae8aRef.current?.setSearchParams();
    header_groupbae8aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(header_groupbae8a) && Object.keys(header_groupbae8a)?.length>0)
      {
        setheader_groupbae8a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [header_groupbae8aProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"header_group"}))
        }}
    >
          {allowedControls.includes("craete_header_text") ?<Textcraete_header_text   /* 26dc6 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
