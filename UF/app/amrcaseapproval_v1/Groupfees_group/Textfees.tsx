...
const Textfees = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {feesd3a22Props, setfeesd3a22Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setfees_groupac23b((pre: any) => ({
          ...pre,
          fees: feesd3a22Props?.filteredData?.length > 0
            ? feesd3a22Props?.filteredData[0]?.fees
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !fees_groupac23b.fees){
        setfees_groupac23b((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !fees_groupac23b.fees){
    setfees_groupac23b((pre:any)=>({...pre,fees:dfd_addcase_v1Props[0]?.fees}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!feesd3a22Props?.filterProps) return;
    handleMapperValue(feesd3a22Props?.filterProps,feesd3a22Props?.filterFlag);
  },[feesd3a22Props?.filterProps])

  if (feesd3a22?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.fees : (fees_groupac23b?.fees || ""))}
</Text>
  </div>
  )
}
