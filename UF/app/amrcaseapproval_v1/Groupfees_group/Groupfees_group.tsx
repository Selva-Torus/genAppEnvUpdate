...
import Textfees_text  from "./Textfees_text";
import Textfees  from "./Textfees";
...
const Groupfees_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {fees_textdf3db, setfees_textdf3db}= useContext(TotalContext) as TotalContextProps;
  const {feesd3a22, setfeesd3a22}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "81400eec819bd4fa82088eef7c3ac23b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfees_groupac23bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("fees_text")){
        setfees_textdf3db((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(fees_textdf3db?.isDisabled==null)
      {
        setfees_textdf3db((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("fees")){
        setfeesd3a22((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(feesd3a22?.isDisabled==null)
      {
        setfeesd3a22((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['fees_text'] = fees_textdf3db,
        codeStates['setfees_text'] = setfees_textdf3db,
        codeStates['fees'] = feesd3a22,
        codeStates['setfees'] = setfeesd3a22,
  ...
  const fees_groupac23bRef = useRef<any>(null);
  const handleClearSearch = () => {
    fees_groupac23bRef.current?.setSearchParams();
    fees_groupac23bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(fees_groupac23b) && Object.keys(fees_groupac23b)?.length>0)
      {
        setfees_groupac23b({})
      }
    }else 
      prevRefreshRef.current= true
  }, [fees_groupac23bProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"fees_group"}))
        }}
    >
          {allowedControls.includes("fees_text") ?<Textfees_text   /* df3db */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("fees") ?<Textfees   /* d3a22 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupfees_group
