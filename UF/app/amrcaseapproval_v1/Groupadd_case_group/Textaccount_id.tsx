...
const Textaccount_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {account_id8944aProps, setaccount_id8944aProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      if ("hasLogicCenter" in dfd_addcase_v1Props && !dfd_addcase_v1Props.hasLogicCenter) {
        ...
        setadd_case_group77747((pre: any) => ({
          ...pre,
      ...
      if(filterFlag){
        setadd_case_group77747((pre: any) => ({
          ...pre,
          account_id: account_id8944aProps?.filteredData?.length > 0
            ? account_id8944aProps?.filteredData[0]?.account_id
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !add_case_group77747.account_id){
        setadd_case_group77747((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[account_id8944a?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !add_case_group77747.account_id){
    setadd_case_group77747((pre:any)=>({...pre,account_id:dfd_addcase_v1Props[0]?.account_id}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!account_id8944aProps?.filterProps) return;
    handleMapperValue(account_id8944aProps?.filterProps,account_id8944aProps?.filterFlag);
  },[account_id8944aProps?.filterProps])

  if (account_id8944a?.isHidden) {
    return <></>
  }
