...
const Groupadd_case_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {account_id8944a, setaccount_id8944a}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "357a72acbbc2118c81dfde7d64e77747");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadd_case_group77747Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("header_group")){
        setheader_groupbae8a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(header_groupbae8a?.isDisabled==null)
      {
        setheader_groupbae8a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("required_dociument_main_group")){
        setrequired_dociument_main_group999a8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(required_dociument_main_group999a8?.isDisabled==null)
      {
        setrequired_dociument_main_group999a8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("case_information_group")){
        setcase_information_group35ed3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(case_information_group35ed3?.isDisabled==null)
      {
        setcase_information_group35ed3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("venue_details_group")){
        setvenue_details_group6904e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(venue_details_group6904e?.isDisabled==null)
      {
        setvenue_details_group6904e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("checklist_main_group")){
        setchecklist_main_groupda0ff((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(checklist_main_groupda0ff?.isDisabled==null)
      {
        setchecklist_main_groupda0ff((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("special_rules_group")){
        setspecial_rules_groupc1585((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(special_rules_groupc1585?.isDisabled==null)
      {
        setspecial_rules_groupc1585((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("account_id")){
        setaccount_id8944a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(account_id8944a?.isDisabled==null)
      {
        setaccount_id8944a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dynamicactions")){
        setdynamicactions37e34((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(dynamicactions37e34?.isDisabled==null)
      {
        setdynamicactions37e34((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['account_id'] = account_id8944a,
        codeStates['setaccount_id'] = setaccount_id8944a,
  ...
  const add_case_group77747Ref = useRef<any>(null);
  const handleClearSearch = () => {
    add_case_group77747Ref.current?.setSearchParams();
    add_case_group77747Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(add_case_group77747) && Object.keys(add_case_group77747)?.length>0)
      {
        setadd_case_group77747({})
      }
    }else 
      prevRefreshRef.current= true
  }, [add_case_group77747Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !p-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"add_case_group"}))
        }}
    >
        ...
        {allowedComponent.includes("dynamicactions")  &&<Groupdynamicactions  
          ...
          groupData={groupData}
          controlData={controlData}        />}
          {allowedControls.includes("account_id") ?<Textaccount_id   /* 8944a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
