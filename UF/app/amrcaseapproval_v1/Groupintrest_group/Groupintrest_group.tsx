...
import Textintrest_text  from "./Textintrest_text";
import Textinterest  from "./Textinterest";
...
const Groupintrest_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {intrest_textfab09, setintrest_textfab09}= useContext(TotalContext) as TotalContextProps;
  const {interestf94e4, setinterestf94e4}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "faa07290345d0e57ccaa86b93da8df75");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setintrest_group8df75Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("intrest_text")){
        setintrest_textfab09((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(intrest_textfab09?.isDisabled==null)
      {
        setintrest_textfab09((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("interest")){
        setinterestf94e4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(interestf94e4?.isDisabled==null)
      {
        setinterestf94e4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['intrest_text'] = intrest_textfab09,
        codeStates['setintrest_text'] = setintrest_textfab09,
        codeStates['interest'] = interestf94e4,
        codeStates['setinterest'] = setinterestf94e4,
  ...
  const intrest_group8df75Ref = useRef<any>(null);
  const handleClearSearch = () => {
    intrest_group8df75Ref.current?.setSearchParams();
    intrest_group8df75Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(intrest_group8df75) && Object.keys(intrest_group8df75)?.length>0)
      {
        setintrest_group8df75({})
      }
    }else 
      prevRefreshRef.current= true
  }, [intrest_group8df75Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"intrest_group"}))
        }}
    >
          {allowedControls.includes("intrest_text") ?<Textintrest_text   /* fab09 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
          {allowedControls.includes("interest") ?<Textinterest   /* f94e4 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupintrest_group
