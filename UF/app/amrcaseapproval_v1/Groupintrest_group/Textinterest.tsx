...
const Textinterest = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {interestf94e4Props, setinterestf94e4Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperValue=async(filterProps?:any,filterFlag?:boolean)=>{
    try{
      ...
      if(filterFlag){
        setintrest_group8df75((pre: any) => ({
          ...pre,
          interest: interestf94e4Props?.filteredData?.length > 0
            ? interestf94e4Props?.filteredData[0]?.interest
            : "0"
        }))
      }else if(Array.isArray(dfd_addcase_v1Props) && dfd_addcase_v1Props && !intrest_group8df75.interest){
        setintrest_group8df75((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[interestf94e4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_addcase_v1Props) && !intrest_group8df75.interest){
    setintrest_group8df75((pre:any)=>({...pre,interest:dfd_addcase_v1Props[0]?.interest}));
  }
  },[dfd_addcase_v1Props])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!interestf94e4Props?.filterProps) return;
    handleMapperValue(interestf94e4Props?.filterProps,interestf94e4Props?.filterFlag);
  },[interestf94e4Props?.filterProps])

  if (interestf94e4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.interest : (intrest_group8df75?.interest || ""))}
</Text>
  </div>
  )
}
