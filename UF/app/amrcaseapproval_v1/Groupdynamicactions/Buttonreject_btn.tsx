...
const Buttonreject_btn = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "fe510eb7cdb4407c98ca3c9e6cb27005"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("reject_btn" in dynamicactions37e34Props?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(dynamicactions37e34Props?.dynamicActionRule?.reject_btn,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("reject_btn" in amrcaseapproval_v1?.dynamicactions && amrcaseapproval_v1?.dynamicactions["reject_btn"]?.itsHaveArtifact== true)
      {
        setShowFlag(amrcaseapproval_v1?.dynamicactions["reject_btn"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(amrcaseapproval_v1?.dynamicactions?.reject_btn?.order,parentRowSpan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "reject_btn27005") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "fe510eb7cdb4407c98ca3c9e6cb27005") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "reject_btn27005");
    return () => {
  ...
  useEffect(()=>{
    if (!reject_btn27005?.trigger) return;
      if(reject_btn27005?.trigger){
      setreject_btn27005((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[reject_btn27005?.trigger])

  useEffect(()=>{
    if(reject_btn27005?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[reject_btn27005?.refresh])
  ...
  async function handleSave7005_1_1_1(){
  ...
  "id": "fe510eb7cdb4407c98ca3c9e6cb27005",
  "type": "button",
  "name": "reject_btn",
  "label": "reject_btn",
  "sequence": 1,
  "children": [
    {
      "id": "fe510eb7cdb4407c98ca3c9e6cb27005.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "fe510eb7cdb4407c98ca3c9e6cb27005.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "fe510eb7cdb4407c98ca3c9e6cb27005.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "fe510eb7cdb4407c98ca3c9e6cb27005.1.1.1.1.1",
                  ...
                  "children": [
                    {
                      "id": "0c63bfb020994c3893dc9644e65ee8e0.1.1.1.1.1.1",
                      "value": "",
                      "type": "screen",
                    ...
                    {
                      "id": "fe510eb7cdb4407c98ca3c9e6cb27005.1.1.1.1.1.2",
                      "eventContext": "riseListen",
                      "value": "",
                      "type": "handlerNode",
                      "name": "refreshElement",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "AMR Case Has Been Rejected",
                    "enabled": true
                  },
                  {
                    "name": "type",
  ...
  // saveHandler
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              ...
              controlId:"fe510eb7cdb4407c98ca3c9e6cb27005"
            };
            if (encryptionFlagCont) {
    ...
    //infoMsg
    toast('AMR Case Has Been Rejected', 'success',true);
    // showArtifact
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setdynamicactions37e34((prev: any) => ({ ...prev, reject_btn: true }));
        //onClick

    //eventEmitter
    await handleSave7005_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setdynamicactions37e34((prev: any) => ({ ...prev, reject_btn: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
  ...
  }, [amrcaseapproval_v1?.dynamicactions?.reject_btn,dynamicactions37e34Props?.dynamicActionRule?.reject_btn,])

 if (reject_btn27005?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      >
        {showFlag && <Button 
          ref={buttonRef}
          className="!text-white !rounded-md !bg-[#CE362B]"
          onClick={handleClick}
          view='normal'
          disabled= {reject_btn27005?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
...
export default Buttonreject_btn
