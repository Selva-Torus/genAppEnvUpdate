...
import Buttonreject_btn  from "./Buttonreject_btn";
import Buttonpass_sign_btn  from "./Buttonpass_sign_btn";
...
const Groupdynamicactions = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cc5f66ec250c4e54943633dceb637e34");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdynamicactions37e34Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cancel_btn")){
        setcancel_btnc64a4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(cancel_btnc64a4?.isDisabled==null)
      {
        setcancel_btnc64a4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("reject_btn")){
        setreject_btn27005((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(reject_btn27005?.isDisabled==null)
      {
        setreject_btn27005((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("pass_sign_btn")){
        setpass_sign_btn916fa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
      if(pass_sign_btn916fa?.isDisabled==null)
      {
        setpass_sign_btn916fa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const dynamicactions37e34Ref = useRef<any>(null);
  const handleClearSearch = () => {
    dynamicactions37e34Ref.current?.setSearchParams();
    dynamicactions37e34Ref.current?.handleSearch({});
  };
    ...
    if(ruleData?.length > 0){
      ...
      if(Object.keys(buttonOrder)?.length)
      {
        ...
        setdynamicactions37e34Props((pre:any)=>({...pre,dynamicActionRule:buttonOrder||{}}))
      }else{
        setButtonGoRuleData({})
        setdynamicactions37e34Props((pre:any)=>({...pre,dynamicActionRule:{}}))
      }
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(dynamicactions37e34) && Object.keys(dynamicactions37e34)?.length>0)
      {
        setdynamicactions37e34({})
      }
    }else 
      prevRefreshRef.current= true
  }, [dynamicactions37e34Props?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setamrcaseapproval_v1((pre:any)=>({...pre,_selectedGroup_:"dynamicactions"}))
        }}
    >
        ...
        {        ((ruleData?.length>0 && "reject_btn" in ButtonGoRuleData)?ButtonGoRuleData["reject_btn"]:true) && 
          allowedControls.includes("reject_btn")  ?            <Buttonreject_btn tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
        {        ((ruleData?.length>0 && "pass_sign_btn" in ButtonGoRuleData)?ButtonGoRuleData["pass_sign_btn"]:true) && 
          allowedControls.includes("pass_sign_btn")  ?            <Buttonpass_sign_btn tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} controlData={controlData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}
