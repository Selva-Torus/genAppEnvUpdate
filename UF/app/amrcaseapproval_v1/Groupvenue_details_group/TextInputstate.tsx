...
const TextInputstate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|e023880da4ed7136adc17f2872110d95"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,state:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, state: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, state: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "e023880da4ed7136adc17f2872110d95"
      );
      ...
      //     controlId: "e023880da4ed7136adc17f2872110d95",
      //     isTable: false,
      //     from:"TextInputstate",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "e023880da4ed7136adc17f2872110d95") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.state||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "e023880da4ed7136adc17f2872110d95") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.state||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,state:dfd_addcase_v1Props.data[0]?.state}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (state10d95?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.state||""}
         disabled= {state10d95?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="State"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.state ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
