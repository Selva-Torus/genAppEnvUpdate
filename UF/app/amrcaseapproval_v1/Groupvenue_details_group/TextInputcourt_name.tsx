...
const TextInputcourt_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|656bd61b4dfc7a27729420150f870da0"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,court_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, court_name: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, court_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "656bd61b4dfc7a27729420150f870da0"
      );
      ...
      //     controlId: "656bd61b4dfc7a27729420150f870da0",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "656bd61b4dfc7a27729420150f870da0") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.court_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "656bd61b4dfc7a27729420150f870da0") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.court_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,court_name:dfd_addcase_v1Props.data[0]?.court_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (court_name70da0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.court_name||""}
         disabled= {court_name70da0?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Court"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.court_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
