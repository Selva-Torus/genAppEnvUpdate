...
const TextInputcountry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|33d6ed1f957dfbdcfa1f90cf120c8824"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,county:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, county: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, county: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "33d6ed1f957dfbdcfa1f90cf120c8824"
      );
      ...
      //     controlId: "33d6ed1f957dfbdcfa1f90cf120c8824",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "33d6ed1f957dfbdcfa1f90cf120c8824") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.county||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "33d6ed1f957dfbdcfa1f90cf120c8824") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.county||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,county:dfd_addcase_v1Props.data[0]?.county}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (countryc8824?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.county||""}
         disabled= {countryc8824?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Country"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.county ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
