...
const TextInputjudge_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|63880362b0cfe230a1b03654e5b833b7"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,judge_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, judge_name: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, judge_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "63880362b0cfe230a1b03654e5b833b7"
      );
      ...
      //     controlId: "63880362b0cfe230a1b03654e5b833b7",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "63880362b0cfe230a1b03654e5b833b7") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.judge_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "63880362b0cfe230a1b03654e5b833b7") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.judge_name||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,judge_name:dfd_addcase_v1Props.data[0]?.judge_name}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (judge_name833b7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.judge_name||""}
         disabled= {judge_name833b7?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Judge"
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.judge_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
