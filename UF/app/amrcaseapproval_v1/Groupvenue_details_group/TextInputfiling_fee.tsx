...
const TextInputfiling_fee = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|5dd0438c2ce51c2a155b0b8a94a9d0c4"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,filing_fee:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, filing_fee: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, filing_fee: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5dd0438c2ce51c2a155b0b8a94a9d0c4"
      );
      ...
      //     controlId: "5dd0438c2ce51c2a155b0b8a94a9d0c4",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "5dd0438c2ce51c2a155b0b8a94a9d0c4") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.filing_fee||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "5dd0438c2ce51c2a155b0b8a94a9d0c4") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.filing_fee||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,filing_fee:dfd_addcase_v1Props.data[0]?.filing_fee}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (filing_fee9d0c4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.filing_fee||""}
         disabled= {filing_fee9d0c4?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='$ 0.00'      
        readOnly={true}
        view='normal'
      ...
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.filing_fee ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
