...
const TextInputefiling_system = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:LAP:AFGK:LAP:AFK:AMRCaseApproval:AFVK:v1|aff28ef16afdc1c856480a953726904e|d5f68fa0574e908d8b7f7aa7b5d056da"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,AMRCaseApproval_v1:{...pre?.AMRCaseApproval_v1,efiling_system:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvenue_details_group6904e((prev: any) => ({ ...prev, efiling_system: +e.target.value }));
    }
    else{
    setvenue_details_group6904e((prev: any) => ({ ...prev, efiling_system: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "d5f68fa0574e908d8b7f7aa7b5d056da"
      );
      ...
      //     controlId: "d5f68fa0574e908d8b7f7aa7b5d056da",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "d5f68fa0574e908d8b7f7aa7b5d056da") {
        handleChange({target:{value:venue_details_group6904eRef?.current?.efiling_system||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "d5f68fa0574e908d8b7f7aa7b5d056da") {
        handleBlur({target:{value:venue_details_group6904eRef?.current?.efiling_system||""}});
      }
    };
  ...
  if(dfd_addcase_v1Props?.setSearchFilters && dfd_addcase_v1Props?.data)
  {
    if(Array.isArray(dfd_addcase_v1Props.data) && dfd_addcase_v1Props.data.length > 0){
      setvenue_details_group6904e((pre:any)=>({...pre,efiling_system:dfd_addcase_v1Props.data[0]?.efiling_system}));
    }
  }
  },[dfd_addcase_v1Props?.setSearchFilters])
  if (efiling_system056da?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={venue_details_group6904e?.efiling_system||""}
         disabled= {efiling_system056da?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.AMRCaseApproval_v1?.efiling_system ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
