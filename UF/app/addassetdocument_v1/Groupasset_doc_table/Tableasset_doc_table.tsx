...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "eb1807c2fdec4b3ab35ecdc7671185f0",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "4d921974240f410c9ae4495639a6421d",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "1ba8b6fc16db474194df518278041b3b",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "a8eba8a2cc8e4ae6a0feb7195b32eb99",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "00b2eaaafadf48c69d16b354484fae15",
    "name": "Created By",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      if (ids.length == 0) {
        let keys:any
        setasset_doc_table49f40Props((pre:any)=>({...pre, selectedIds:[]}))
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      asset_doc_table49f40.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
      setasset_doc_table49f40Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      setasset_doc_table49f40Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setasset_doc_table49f40Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["asset_id"]:asset_doc_table49f40Props?.presetValues?.asset_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!asset_doc_table49f40Props?.presetValues || Object.keys(asset_doc_table49f40Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addAssetDocument:AFVK:v1:UO', 
                nodeId: '858e60aa8d764228bc44c6400a849f40',
                elementId: '858e60aa8d764228bc44c6400a849f40'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setasset_doc_table49f40(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in addassetdocument_v1?.asset_doc_table && addassetdocument_v1?.asset_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(addassetdocument_v1?.asset_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(addassetdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,asset_doc_table:filteredData},{asset_doc_table:addassetdocument_v1?.asset_doc_table})
          if(result?.asset_doc_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(asset_doc_table49f40))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    ...
    if (asset_doc_table49f40Props.filterInitalLoad) return;

    const filterControllers = asset_doc_table49f40Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['asset_doc_table49f40']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['asset_doc_table49f40']).filter((k: string) => jsonEntry['asset_doc_table49f40'][k] === false)
      : [];
    ...
    setasset_doc_table49f40Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(asset_doc_table49f40Props.filterProps);

  }, [
    asset_doc_table49f40Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    asset_doc_table49f40Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(asset_doc_table49f40Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(asset_doc_table49f40Props.searchFilter) || {};
...
}, [asset_doc_table49f40Props.searchFilter])
  ...
  useEffect(() => {
    if (asset_doc_table49f40Props?.clearData === true) {
      setasset_doc_table49f40([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setasset_doc_table49f40Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [asset_doc_table49f40Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  }, [asset_doc_table49f40Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = asset_doc_table49f40Props?.selectedIds[asset_doc_table49f40Props?.selectedIds?.length-1]
    if(Array.isArray(asset_doc_table49f40) && asset_doc_table49f40.length>0)
    {
      let data = asset_doc_table49f40[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "asset_id",
        value: data["asset_id"],
  ...
  useEffect(() => {
    if (asset_doc_table49f40Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (asset_doc_table49f40Props?.selectedIds?.length == 0){
      handleOnRowClick({},asset_doc_table49f40Props?.selectedIds)
    }
  }, [asset_doc_table49f40Props?.selectedIds])
    ...
    let filterProps2:any =  [
  {
    ...
    let filterData2 = await getFilterProps(filterProps2,asset_doc_table49f40);
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(asset_doc_table49f40?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&asset_doc_table49f40?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](asset_doc_table49f40?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_doc_table49f40?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&asset_doc_table49f40?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](asset_doc_table49f40?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: asset_doc_table49f40?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (asset_doc_table49f40?.isHidden) {
    return <></>
  }
  return(
      ...
      >
        <PageDocumentviewerpage2/>
          ...
          <div
            ...
            id={"858e60aa8d764228bc44c6400a849f40"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={asset_doc_table49f40Props?.selectedIds}  
...
export default Tableasset_doc_table
