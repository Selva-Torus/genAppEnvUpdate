...
export default function PageAddassetdocumentV1() {
  ...
  const [checkasset_doc_table,setCheckasset_doc_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetdoctable_v1DFD(pagination:any): Promise<void>{
        ...
        if(addassetdocument_v1Props.length > 0){
          for(let i=0;i< addassetdocument_v1Props.length;i++){
            if(addassetdocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetDocTable:AFVK:v1"){
              // delete addassetdocument_v1Props[i].DFDkey;
              let temp=structuredClone(addassetdocument_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setaddassetdocument_v1({...result,_artfactPFRule_:rule})
  }
  ...
  async function securityCheck(): Promise<void> {
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addAssetDocument:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
    ...
    await assetdoctable_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'asset_doc_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckasset_doc_table(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaddassetdocument_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(addassetdocument_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(addassetdocument_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
