...
let defaultColumns:any = [
  {
    "id": "assign_id",
    "nodeid": "b1c6ebfb3d644522b867d0cc689ac541",
    "name": "Assign ID",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "expected_return_date"
  },
  {
    "id": "condition_at_assign",
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  if (allData.length !== 0) {
   ...
   async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in artifactRuleState?.assignments_table && artifactRuleState?.assignments_table[controller]?.itsHaveArtifact== true)
      {
        if(artifactRuleState?.assignments_table[controller])
        {
          let result :any = await getAftfactLevelRule(artifactRuleState._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,assignments_table:filteredData},{assignments_table:artifactRuleState?.assignments_table})
          if(result?.assignments_table?.[controller]?.show==true)
            return true
          else
            return false
        }
  ...
  useEffect(() => {
    async function loadPopupData() {
      let bt_view:any = await handleSecurity("bt_view") || false
...
const Tableassignments_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setassignments_table75a5dProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setassignments_table75a5dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1:UO', 
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( assignments_table75a5dProps?.presetValues&&Object.keys(assignments_table75a5dProps?.presetValues).length > 0) {
          filtertedData = [assignments_table75a5dProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setassignments_table75a5d(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={assetassignments_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (assignments_table75a5dProps.filterInitalLoad) return;

    const filterControllers = assignments_table75a5dProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['assignments_table75a5d']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['assignments_table75a5d']).filter((k: string) => jsonEntry['assignments_table75a5d'][k] === false)
      : [];
    ...
    setassignments_table75a5dProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(assignments_table75a5dProps.filterProps);

  }, [
    assignments_table75a5dProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    assignments_table75a5dProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(assignments_table75a5dProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(assignments_table75a5dProps.searchFilter) || {};
...
}, [assignments_table75a5dProps.searchFilter])
  ...
  useEffect(() => {
    if (assignments_table75a5dProps?.clearData === true) {
      setassignments_table75a5d([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setassignments_table75a5dProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [assignments_table75a5dProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [assignments_table75a5dProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = assignments_table75a5dProps?.selectedIds[assignments_table75a5dProps?.selectedIds?.length-1]
    if(Array.isArray(assignments_table75a5d) && assignments_table75a5d.length>0)
    {
      let data = assignments_table75a5d[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "assign_id",
  ...
  useEffect(() => {
    if (assignments_table75a5dProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (assignments_table75a5dProps?.selectedIds?.length == 0){
      handleOnRowClick({},assignments_table75a5dProps?.selectedIds)
    }
  }, [assignments_table75a5dProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(assignments_table75a5d?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&assignments_table75a5d?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](assignments_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: assignments_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&assignments_table75a5d?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](assignments_table75a5d?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: assignments_table75a5d?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (assignments_table75a5d?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          ...
          >
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={assignments_table75a5dProps?.selectedIds}  
...
export default Tableassignments_table
