...
export default function PageLicensedeleteV1() {
  ...
  const {licensedelete_v1Props, setlicensedelete_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetsoftwarelicenses_v1DFD(pagination:any): Promise<void>{
        ...
        if(licensedelete_v1Props.length > 0){
          for(let i=0;i< licensedelete_v1Props.length;i++){
            if(licensedelete_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1"){
              // delete licensedelete_v1Props[i].DFDkey;
              let temp=structuredClone(licensedelete_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setlicensedelete_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:licenseDelete:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setlicensedelete_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(licensedelete_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(licensedelete_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
