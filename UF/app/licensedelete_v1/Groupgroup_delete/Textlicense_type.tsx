...
const Textlicense_type = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetsoftwarelicenses_v1Props && !dfd_assetsoftwarelicenses_v1Props.hasLogicCenter) {
        ...
        setgroup_deletedf5b8((pre: any) => {
          return { ...pre, license_type: api_paginationData.data.records[0]?.license_type }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && dfd_assetsoftwarelicenses_v1Props && !group_deletedf5b8.license_type){
        setgroup_deletedf5b8((pre:any)=>({...pre,license_type:dfd_assetsoftwarelicenses_v1Props[0]?.license_type}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[license_typecec9e?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && !group_deletedf5b8.license_type){
    setgroup_deletedf5b8((pre:any)=>({...pre,license_type:dfd_assetsoftwarelicenses_v1Props[0]?.license_type}));
  }
  },[dfd_assetsoftwarelicenses_v1Props])

  if (license_typecec9e?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `16 / 22`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.license_type : (group_deletedf5b8?.license_type || ""))}
</Text>
  </div>
  )
}

export default Textlicense_type
