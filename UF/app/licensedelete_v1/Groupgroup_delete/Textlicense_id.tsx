...
const Textlicense_id = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && dfd_assetsoftwarelicenses_v1Props && !group_deletedf5b8.license_id){
        setgroup_deletedf5b8((pre:any)=>({...pre,license_id:dfd_assetsoftwarelicenses_v1Props[0]?.license_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[license_id027b5?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && !group_deletedf5b8.license_id){
    setgroup_deletedf5b8((pre:any)=>({...pre,license_id:dfd_assetsoftwarelicenses_v1Props[0]?.license_id}));
  }
  },[dfd_assetsoftwarelicenses_v1Props])

  if (license_id027b5?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 3`,gridRow: `50 / 51`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.license_id : (group_deletedf5b8?.license_id || ""))}
</Text>
  </div>
  )
}
