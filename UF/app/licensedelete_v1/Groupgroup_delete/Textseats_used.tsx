...
const Textseats_used = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetsoftwarelicenses_v1Props && !dfd_assetsoftwarelicenses_v1Props.hasLogicCenter) {
        ...
        setgroup_deletedf5b8((pre: any) => {
          return { ...pre, seats_used: api_paginationData.data.records[0]?.seats_used }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && dfd_assetsoftwarelicenses_v1Props && !group_deletedf5b8.seats_used){
        setgroup_deletedf5b8((pre:any)=>({...pre,seats_used:dfd_assetsoftwarelicenses_v1Props[0]?.seats_used}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[seats_used8c8d5?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && !group_deletedf5b8.seats_used){
    setgroup_deletedf5b8((pre:any)=>({...pre,seats_used:dfd_assetsoftwarelicenses_v1Props[0]?.seats_used}));
  }
  },[dfd_assetsoftwarelicenses_v1Props])

  if (seats_used8c8d5?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `37 / 43`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.seats_used : (group_deletedf5b8?.seats_used || ""))}
</Text>
  </div>
  )
}

export default Textseats_used
