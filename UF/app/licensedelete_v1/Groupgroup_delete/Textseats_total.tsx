...
const Textseats_total = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetsoftwarelicenses_v1Props && !dfd_assetsoftwarelicenses_v1Props.hasLogicCenter) {
        ...
        setgroup_deletedf5b8((pre: any) => {
          return { ...pre, seats_total: api_paginationData.data.records[0]?.seats_total }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && dfd_assetsoftwarelicenses_v1Props && !group_deletedf5b8.seats_total){
        setgroup_deletedf5b8((pre:any)=>({...pre,seats_total:dfd_assetsoftwarelicenses_v1Props[0]?.seats_total}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[seats_totalf37ee?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && !group_deletedf5b8.seats_total){
    setgroup_deletedf5b8((pre:any)=>({...pre,seats_total:dfd_assetsoftwarelicenses_v1Props[0]?.seats_total}));
  }
  },[dfd_assetsoftwarelicenses_v1Props])

  if (seats_totalf37ee?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `30 / 36`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.seats_total : (group_deletedf5b8?.seats_total || ""))}
</Text>
  </div>
  )
}

export default Textseats_total
