 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "a025997676377d4c9cdbdfa3125f3727"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonf3727") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "a025997676377d4c9cdbdfa3125f3727") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonf3727");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonf3727?.trigger) return;
      if(ok_buttonf3727?.trigger){
      setok_buttonf3727((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonf3727?.trigger])

  useEffect(()=>{
    if(ok_buttonf3727?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonf3727?.refresh])
  ...
  async function handleSave3727_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_deletedf5b8Props = group_deletedf5b8PropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.licenseDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_deletedf5b8Ref.current);
      ...
      let lockedKeysLength:number;
        if(group_deletedf5b8?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "a025997676377d4c9cdbdfa3125f3727",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "a025997676377d4c9cdbdfa3125f3727.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "a025997676377d4c9cdbdfa3125f3727.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "a025997676377d4c9cdbdfa3125f3727.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
              ...
              "children": [
                {
                  "id": "a025997676377d4c9cdbdfa3125f3727.1.1.1.1.1",
                  ...
                  "children": [
                    {
                      "id": "356fd7ddf8ed4df7909d896283975a5d.1.1.1.1.1.1",
                      "value": "",
                      "type": "screen",
                      "name": "assetSoftwareLicenses.v1|software_licenses_table",
                      "label": "assetSoftwareLicenses.v1|software_licenses_table",
                      "key": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|software_licenses_table",
                      "elementType": "group",
                      "groupType": "table",
                ...
                "params": [
                  {
                    "name": "message",
                    "_type": "text",
                    "value": "Software License Deleted successfully",
                    "enabled": true
                  },
                  {
                    "name": "type",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:licenseDelete:AFVK:v1";
      sourceId+= "|"+"70cd602d5b1d604a33a77213e65df5b8";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:licenseDelete:AFVK:v1"+"|"+"70cd602d5b1d604a33a77213e65df5b8"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_deletedf5b8?.upId? [group_deletedf5b8?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_deletedf5b8?.upId? [group_deletedf5b8?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_deletedf5b8?.upId? [group_deletedf5b8?.upId ] : group_deletedf5b8?.trs_process_id? [group_deletedf5b8?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_deletedf5b8Ref.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"70cd602d5b1d604a33a77213e65df5b8",
              controlId:"a025997676377d4c9cdbdfa3125f3727"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_deletedf5b8Props.ssKey !== '' && group_deletedf5b8Props.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_deletedf5b8Props.ssKey;          
    }
    ...
    //infoMsg
    toast('Software License Deleted successfully', 'success',true);
    // refreshElement
    //riseListen
    // for group
    setsoftware_licenses_table75a5dProps((pre:any)=>({...pre,refresh:!pre?.refresh}));
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_deletedf5b8((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSave3727_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_deletedf5b8((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_buttonf3727?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
