...
const Textauto_renewal = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetsoftwarelicenses_v1Props && !dfd_assetsoftwarelicenses_v1Props.hasLogicCenter) {
        ...
        setgroup_deletedf5b8((pre: any) => {
          return { ...pre, auto_renewal: api_paginationData.data.records[0]?.auto_renewal }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && dfd_assetsoftwarelicenses_v1Props && !group_deletedf5b8.auto_renewal){
        setgroup_deletedf5b8((pre:any)=>({...pre,auto_renewal:dfd_assetsoftwarelicenses_v1Props[0]?.auto_renewal}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[auto_renewal8e280?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetsoftwarelicenses_v1Props) && !group_deletedf5b8.auto_renewal){
    setgroup_deletedf5b8((pre:any)=>({...pre,auto_renewal:dfd_assetsoftwarelicenses_v1Props[0]?.auto_renewal}));
  }
  },[dfd_assetsoftwarelicenses_v1Props])

  if (auto_renewal8e280?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `23 / 29`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.auto_renewal : (group_deletedf5b8?.auto_renewal || ""))}
</Text>
  </div>
  )
}

export default Textauto_renewal
