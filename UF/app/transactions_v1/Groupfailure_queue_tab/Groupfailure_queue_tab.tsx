 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("failure_queue_table")){
        setfailure_queue_tablea476fProps((pre:any)=>({...pre,...failure_queue_tablea476f,isDisabled:true}));

    }else
    {
      if(failure_queue_tablea476f?.isDisabled==null)
      {
        setfailure_queue_tablea476fProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("failure_queue_journey_group")){
        setfailure_queue_journey_group36aba((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(failure_queue_journey_group36aba?.isDisabled==null)
      {
        setfailure_queue_journey_group36aba((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tran_tab_group08b64?.tran_tab_group === "failure_queue_tab"){
    handleOnChange()
    }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"failure_queue_tab"}))
        }}
    >
