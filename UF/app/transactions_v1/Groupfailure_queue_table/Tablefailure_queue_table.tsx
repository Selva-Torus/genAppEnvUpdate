...
let defaultColumns:any = [
  ...
  },
  {
    ...
    "nodeid": "5fd0c91ebd1e4aa78c766be9694b7b55",
    "name": "UUID",
    "meta": {
      "sort": true
    },
...
const Tablefailure_queue_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, failure_queue_tablea476f })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, failure_queue_tablea476f }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, failure_queue_tablea476f } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      failure_queue_tablea476f.filter((item:any,id:number)=>{
      ...
      setfailure_queue_tablea476fProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (failure_queue_tablea476fProps.filterInitalLoad) return;

    const filterControllers = failure_queue_tablea476fProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['failure_queue_tablea476f']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['failure_queue_tablea476f']).filter((k: string) => jsonEntry['failure_queue_tablea476f'][k] === false)
      : [];
    ...
    setfailure_queue_tablea476fProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(failure_queue_tablea476fProps.filterProps);

  }, [
    failure_queue_tablea476fProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(failure_queue_tablea476fProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(failure_queue_tablea476fProps.searchFilter) || {};
...
}, [failure_queue_tablea476fProps.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = failure_queue_tablea476f.find((data:any)=>(data?.vgphstm_uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_uuid",
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setfailure_queue_tablea476fProps((pre:any)=>({...pre, selectedIds:[row?.vgphstm_uuid]}))  
    onButtonSecurityHandle(data)
    ...
    const result = allData.find((item:any) => item["vgphstm_uuid"] === data[pkCol?.id || "vgphstm_uuid"]);
          ...
          setfailure_queue_journey_group36aba({...failure_queue_journey_group36aba,presetValues:allData[index]})
