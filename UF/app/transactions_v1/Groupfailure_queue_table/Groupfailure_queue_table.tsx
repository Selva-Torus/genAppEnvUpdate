...
const Groupfailure_queue_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  "Operational Manager": {
    "allowedControls": [
      "value_date_failure_queue",
      "dr_account_failure_queue",
      "dr_name_failure_queue",
      "dr_currency_failure_queue",
      "dr_amount_failure_queue",
      "cr_account_failure_queue",
      "cr_name_failure_queue",
      "cr_currency_failure_queue",
 ...
 /////////////
  ...
  const {value_date_failure_queue12297, setvalue_date_failure_queue12297}= useContext(TotalContext) as TotalContextProps;
  const {dr_account_failure_queue42953, setdr_account_failure_queue42953}= useContext(TotalContext) as TotalContextProps;
  const {dr_name_failure_queue03c86, setdr_name_failure_queue03c86}= useContext(TotalContext) as TotalContextProps;
  const {dr_currency_failure_queuef9d2d, setdr_currency_failure_queuef9d2d}= useContext(TotalContext) as TotalContextProps;
  ...
  const {cr_name_failure_queue57c4d, setcr_name_failure_queue57c4d}= useContext(TotalContext) as TotalContextProps;
  const {cr_currency_failure_queue09d7a, setcr_currency_failure_queue09d7a}= useContext(TotalContext) as TotalContextProps;
  const {cr_amount_failure_queue0aef8, setcr_amount_failure_queue0aef8}= useContext(TotalContext) as TotalContextProps;
  const {uuid_failure_queueb7b55, setuuid_failure_queueb7b55}= useContext(TotalContext) as TotalContextProps;
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("value_date_failure_queue")){
        setvalue_date_failure_queue12297((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(value_date_failure_queue12297?.isDisabled==null)
      {
        setvalue_date_failure_queue12297((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_failure_queue")){
        setdr_account_failure_queue42953((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_failure_queue42953?.isDisabled==null)
      {
        setdr_account_failure_queue42953((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name_failure_queue")){
        setdr_name_failure_queue03c86((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_name_failure_queue03c86?.isDisabled==null)
      {
        setdr_name_failure_queue03c86((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_currency_failure_queue")){
        setdr_currency_failure_queuef9d2d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_currency_failure_queuef9d2d?.isDisabled==null)
      {
        setdr_currency_failure_queuef9d2d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_amount_failure_queue")){
        setdr_amount_failure_queue95d4e((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_amount_failure_queue95d4e?.isDisabled==null)
      {
        setdr_amount_failure_queue95d4e((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account_failure_queue")){
        setcr_account_failure_queuea7246((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_account_failure_queuea7246?.isDisabled==null)
      {
        setcr_account_failure_queuea7246((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name_failure_queue")){
        setcr_name_failure_queue57c4d((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_name_failure_queue57c4d?.isDisabled==null)
      {
        setcr_name_failure_queue57c4d((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_currency_failure_queue")){
        setcr_currency_failure_queue09d7a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_currency_failure_queue09d7a?.isDisabled==null)
      {
        setcr_currency_failure_queue09d7a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_amount_failure_queue")){
        setcr_amount_failure_queue0aef8((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_failure_queue0aef8?.isDisabled==null)
      {
        setcr_amount_failure_queue0aef8((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_failure_queue")){
        setuuid_failure_queueb7b55((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_failure_queueb7b55?.isDisabled==null)
      {
        setuuid_failure_queueb7b55((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const handleCustomCode=async () => {
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['value_date_failure_queue'] = value_date_failure_queue12297,
        codeStates['setvalue_date_failure_queue'] = setvalue_date_failure_queue12297,
        codeStates['dr_account_failure_queue'] = dr_account_failure_queue42953,
        codeStates['setdr_account_failure_queue'] = setdr_account_failure_queue42953,
        codeStates['dr_name_failure_queue'] = dr_name_failure_queue03c86,
        codeStates['setdr_name_failure_queue'] = setdr_name_failure_queue03c86,
        codeStates['dr_currency_failure_queue'] = dr_currency_failure_queuef9d2d,
        codeStates['setdr_currency_failure_queue'] = setdr_currency_failure_queuef9d2d,
        ...
        codeStates['cr_name_failure_queue'] = cr_name_failure_queue57c4d,
        codeStates['setcr_name_failure_queue'] = setcr_name_failure_queue57c4d,
        codeStates['cr_currency_failure_queue'] = cr_currency_failure_queue09d7a,
        codeStates['setcr_currency_failure_queue'] = setcr_currency_failure_queue09d7a,
        codeStates['cr_amount_failure_queue'] = cr_amount_failure_queue0aef8,
        codeStates['setcr_amount_failure_queue'] = setcr_amount_failure_queue0aef8,
        codeStates['uuid_failure_queue'] = uuid_failure_queueb7b55,
        codeStates['setuuid_failure_queue'] = setuuid_failure_queueb7b55,
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"failure_queue_table"}))
        }}
    >
