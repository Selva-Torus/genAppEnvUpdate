 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("success_queue_journey")){
        setsuccess_queue_journey68ac9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(success_queue_journey68ac9?.isDisabled==null)
      {
        setsuccess_queue_journey68ac9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !ml-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"success_queue_journey_group"}))
        }}
    >
