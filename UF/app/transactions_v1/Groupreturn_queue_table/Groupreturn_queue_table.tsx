...
const Groupreturn_queue_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("value_date_return_queue")){
        setvalue_date_return_queuee5e11((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(value_date_return_queuee5e11?.isDisabled==null)
      {
        setvalue_date_return_queuee5e11((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_return_queue")){
        setdr_account_return_queuebdabb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_return_queuebdabb?.isDisabled==null)
      {
        setdr_account_return_queuebdabb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name_return_queue")){
        setdr_name_return_queue958c9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_name_return_queue958c9?.isDisabled==null)
      {
        setdr_name_return_queue958c9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_currency_return_queue")){
        setdr_currency_return_queuee94b2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_currency_return_queuee94b2?.isDisabled==null)
      {
        setdr_currency_return_queuee94b2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_amount_return_queue")){
        setdr_amount_return_queue2f324((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_amount_return_queue2f324?.isDisabled==null)
      {
        setdr_amount_return_queue2f324((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account_return_queue")){
        setcr_account_return_queue21a57((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_account_return_queue21a57?.isDisabled==null)
      {
        setcr_account_return_queue21a57((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name_return_queue")){
        setcr_name_return_queue13fec((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_name_return_queue13fec?.isDisabled==null)
      {
        setcr_name_return_queue13fec((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_currency_return_queue")){
        setcr_currency_return_queuef37f7((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_currency_return_queuef37f7?.isDisabled==null)
      {
        setcr_currency_return_queuef37f7((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_amount_return_queue")){
        setcr_amount_return_queue95903((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_return_queue95903?.isDisabled==null)
      {
        setcr_amount_return_queue95903((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_return_queue")){
        setuuid_return_queue9fa04((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_return_queue9fa04?.isDisabled==null)
      {
        setuuid_return_queue9fa04((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"return_queue_table"}))
        }}
    >
