 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("success_queue_table")){
        setsuccess_queue_table63aaeProps((pre:any)=>({...pre,...success_queue_table63aae,isDisabled:true}));

    }else
    {
      if(success_queue_table63aae?.isDisabled==null)
      {
        setsuccess_queue_table63aaeProps((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("success_queue_journey_group")){
        setsuccess_queue_journey_group755eb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(success_queue_journey_group755eb?.isDisabled==null)
      {
        setsuccess_queue_journey_group755eb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tran_tab_group08b64?.tran_tab_group === "success_queue_tab"){
    handleOnChange()
    }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"success_queue_tab"}))
        }}
    >
