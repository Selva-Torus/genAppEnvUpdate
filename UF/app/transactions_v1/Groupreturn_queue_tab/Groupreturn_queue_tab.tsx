 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("return_queue_table")){
        setreturn_queue_table267f0Props((pre:any)=>({...pre,...return_queue_table267f0,isDisabled:true}));

    }else
    {
      if(return_queue_table267f0?.isDisabled==null)
      {
        setreturn_queue_table267f0Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("return_queue_journey_group")){
        setreturn_queue_journey_group92c55((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(return_queue_journey_group92c55?.isDisabled==null)
      {
        setreturn_queue_journey_group92c55((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tran_tab_group08b64?.tran_tab_group === "return_queue_tab"){
    handleOnChange()
    }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"return_queue_tab"}))
        }}
    >
