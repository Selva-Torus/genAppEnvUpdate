...
const Groupsuccess_queue_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  "Operational Manager": {
    "allowedControls": [
      "value_date_success_queue",
      "dr_account_success_queue",
      "dr_name_success_queue",
      "dr_currency_operational_pending",
      "dr_amount_success_queue",
      "cr_account_success_queue",
      "cr_name_success_queue",
      "cr_currency_success_queue",
 ...
 /////////////
  ...
  const {value_date_success_queue7c209, setvalue_date_success_queue7c209}= useContext(TotalContext) as TotalContextProps;
  const {dr_account_success_queueeddaf, setdr_account_success_queueeddaf}= useContext(TotalContext) as TotalContextProps;
  const {dr_name_success_queuec805b, setdr_name_success_queuec805b}= useContext(TotalContext) as TotalContextProps;
  const {dr_currency_operational_pending10a49, setdr_currency_operational_pending10a49}= useContext(TotalContext) as TotalContextProps;
  ...
  const {cr_name_success_queueb80d4, setcr_name_success_queueb80d4}= useContext(TotalContext) as TotalContextProps;
  const {cr_currency_success_queue2f950, setcr_currency_success_queue2f950}= useContext(TotalContext) as TotalContextProps;
  const {cr_amount_success_queue019a2, setcr_amount_success_queue019a2}= useContext(TotalContext) as TotalContextProps;
  const {uuid_success_queued0e34, setuuid_success_queued0e34}= useContext(TotalContext) as TotalContextProps;
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("value_date_success_queue")){
        setvalue_date_success_queue7c209((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(value_date_success_queue7c209?.isDisabled==null)
      {
        setvalue_date_success_queue7c209((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_success_queue")){
        setdr_account_success_queueeddaf((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_success_queueeddaf?.isDisabled==null)
      {
        setdr_account_success_queueeddaf((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name_success_queue")){
        setdr_name_success_queuec805b((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_name_success_queuec805b?.isDisabled==null)
      {
        setdr_name_success_queuec805b((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_currency_operational_pending")){
        setdr_currency_operational_pending10a49((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_currency_operational_pending10a49?.isDisabled==null)
      {
        setdr_currency_operational_pending10a49((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_amount_success_queue")){
        setdr_amount_success_queueda254((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_amount_success_queueda254?.isDisabled==null)
      {
        setdr_amount_success_queueda254((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account_success_queue")){
        setcr_account_success_queue60480((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_account_success_queue60480?.isDisabled==null)
      {
        setcr_account_success_queue60480((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name_success_queue")){
        setcr_name_success_queueb80d4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_name_success_queueb80d4?.isDisabled==null)
      {
        setcr_name_success_queueb80d4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_currency_success_queue")){
        setcr_currency_success_queue2f950((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_currency_success_queue2f950?.isDisabled==null)
      {
        setcr_currency_success_queue2f950((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_amount_success_queue")){
        setcr_amount_success_queue019a2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_success_queue019a2?.isDisabled==null)
      {
        setcr_amount_success_queue019a2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_success_queue")){
        setuuid_success_queued0e34((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_success_queued0e34?.isDisabled==null)
      {
        setuuid_success_queued0e34((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const handleCustomCode=async () => {
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['value_date_success_queue'] = value_date_success_queue7c209,
        codeStates['setvalue_date_success_queue'] = setvalue_date_success_queue7c209,
        codeStates['dr_account_success_queue'] = dr_account_success_queueeddaf,
        codeStates['setdr_account_success_queue'] = setdr_account_success_queueeddaf,
        codeStates['dr_name_success_queue'] = dr_name_success_queuec805b,
        codeStates['setdr_name_success_queue'] = setdr_name_success_queuec805b,
        codeStates['dr_currency_operational_pending'] = dr_currency_operational_pending10a49,
        codeStates['setdr_currency_operational_pending'] = setdr_currency_operational_pending10a49,
        ...
        codeStates['cr_name_success_queue'] = cr_name_success_queueb80d4,
        codeStates['setcr_name_success_queue'] = setcr_name_success_queueb80d4,
        codeStates['cr_currency_success_queue'] = cr_currency_success_queue2f950,
        codeStates['setcr_currency_success_queue'] = setcr_currency_success_queue2f950,
        codeStates['cr_amount_success_queue'] = cr_amount_success_queue019a2,
        codeStates['setcr_amount_success_queue'] = setcr_amount_success_queue019a2,
        codeStates['uuid_success_queue'] = uuid_success_queued0e34,
        codeStates['setuuid_success_queue'] = setuuid_success_queued0e34,
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"success_queue_table"}))
        }}
    >
