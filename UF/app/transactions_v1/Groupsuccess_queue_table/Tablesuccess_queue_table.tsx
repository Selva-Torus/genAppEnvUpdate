...
let defaultColumns:any = [
  ...
  },
  {
    ...
    "nodeid": "ac2038a77dde4116b1287e73995d0e34",
    "name": "UUID",
    "meta": {
      "sort": true
    },
...
const Tablesuccess_queue_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, success_queue_table63aae })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, success_queue_table63aae }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, success_queue_table63aae } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      success_queue_table63aae.filter((item:any,id:number)=>{
      ...
      setsuccess_queue_table63aaeProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (success_queue_table63aaeProps.filterInitalLoad) return;

    const filterControllers = success_queue_table63aaeProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['success_queue_table63aae']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['success_queue_table63aae']).filter((k: string) => jsonEntry['success_queue_table63aae'][k] === false)
      : [];
    ...
    setsuccess_queue_table63aaeProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(success_queue_table63aaeProps.filterProps);

  }, [
    success_queue_table63aaeProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(success_queue_table63aaeProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(success_queue_table63aaeProps.searchFilter) || {};
...
}, [success_queue_table63aaeProps.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = success_queue_table63aae.find((data:any)=>(data?.vgphstm_uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_uuid",
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    setsuccess_queue_table63aaeProps((pre:any)=>({...pre, selectedIds:[row?.vgphstm_uuid]}))  
    onButtonSecurityHandle(data)
    ...
    const result = allData.find((item:any) => item["vgphstm_uuid"] === data[pkCol?.id || "vgphstm_uuid"]);
          ...
          setsuccess_queue_journey_group755eb({...success_queue_journey_group755eb,presetValues:allData[index]})
