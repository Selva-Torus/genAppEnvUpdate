 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("view_all_journey")){
        setview_all_journeyd3ae9((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(view_all_journeyd3ae9?.isDisabled==null)
      {
        setview_all_journeyd3ae9((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !ml-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"view_all_journey_group"}))
        }}
    >
