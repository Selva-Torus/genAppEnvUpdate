 ...
 /////////////
  ...
  useEffect(()=>{
    ...
    const triggerElementHandler = async (id:any) => {
      if (id === "22571c749d1a41599018e5b102214cf0") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "search14cf0");
    return () => {
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    let filterProps2:any =  [];
      let filterData2 = await getFilterProps(filterProps2,{...tran_main_group1dc7f,...view_all_journey_group67ce4,...failure_queue_journey_group36aba,...success_queue_journey_group755eb,...return_queue_journey_group92c55,...operational_pending_journey_group63667,...technical_pending_journey_groupe4f03,...tran_tab_group08b64});
 ...
 if (search14cf0?.isHidden) {
  ...
  return (
    <div
      ...
      >
        {showProfileAsModalOpen2 && hiddenModalForTrigger && (
          <div style={{ display: 'none' }}>
            <PageTransactionsearchpage2 onReady={handleAssetPageReady}/>
          </div>
        )}
      <Modal 
      ...
      >
        {!hiddenModalForTrigger && <PageTransactionsearchpage2  onReady={handleAssetPageReady}/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
