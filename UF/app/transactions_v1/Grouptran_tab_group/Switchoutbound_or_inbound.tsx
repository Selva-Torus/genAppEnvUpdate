 ...
 /////////////
  ...
  const handleMapperValue=async()=>{
    ...
    const orchestrationData = getControlOrchestrationData(  
      ...
      await delay(1000)
      handleChange(tran_tab_group08b64?.outbound_or_inbound||false,orchestrationData?.data?.rule?.nodes)
    }catch(err)
    {
      console.log(err)
    }
      ...
      let filterProps8:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterPropsEvaluateDecisionTable10:any = evaluateDecisionTable(
        presentRule,
        {outbound_or_inbound: checked},
        {...decodedTokenObj,...memoryVariables}
      )
      let filterProps10:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterPropsEvaluateDecisionTable12:any = evaluateDecisionTable(
        presentRule,
        {outbound_or_inbound: checked},
        {...decodedTokenObj,...memoryVariables}
      )
      let filterProps12:any =  [
  {
    ...
    // for group
    Object.keys(operational_pending_journey_group63667).map((keys:any)=>{
     operational_pending_journey_group63667[keys]="";
    })
    setoperational_pending_journey_group63667({...operational_pending_journey_group63667});
    // clearHandler riseListen
    // for group
    Object.keys(view_all_journey_group67ce4).map((keys:any)=>{
     view_all_journey_group67ce4[keys]="";
    })
    setview_all_journey_group67ce4({...view_all_journey_group67ce4});
    // clearHandler riseListen
    // for group
    Object.keys(failure_queue_journey_group36aba).map((keys:any)=>{
     failure_queue_journey_group36aba[keys]="";
    })
    setfailure_queue_journey_group36aba({...failure_queue_journey_group36aba});
    // clearHandler riseListen
    // for group
    Object.keys(success_queue_journey_group755eb).map((keys:any)=>{
     success_queue_journey_group755eb[keys]="";
    })
    setsuccess_queue_journey_group755eb({...success_queue_journey_group755eb});
    // clearHandler riseListen
    // for group
    Object.keys(return_queue_journey_group92c55).map((keys:any)=>{
     return_queue_journey_group92c55[keys]="";
    })
    setreturn_queue_journey_group92c55({...return_queue_journey_group92c55});
    // clearHandler riseListen
    // for group
    Object.keys(technical_pending_journey_groupe4f03).map((keys:any)=>{
     technical_pending_journey_groupe4f03[keys]="";
    })
    settechnical_pending_journey_groupe4f03({...technical_pending_journey_groupe4f03});
    }catch (err: any) {
