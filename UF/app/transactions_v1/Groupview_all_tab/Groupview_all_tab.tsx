 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("view_all_table")){
        setview_all_tablec9e87Props((pre:any)=>({...pre,...view_all_tablec9e87,isDisabled:true}));

    }else
    {
      if(view_all_tablec9e87?.isDisabled==null)
      {
        setview_all_tablec9e87Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("view_all_journey_group")){
        setview_all_journey_group67ce4((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(view_all_journey_group67ce4?.isDisabled==null)
      {
        setview_all_journey_group67ce4((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tran_tab_group08b64?.tran_tab_group === "view_all_tab"){
    handleOnChange()
    }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"view_all_tab"}))
        }}
    >
