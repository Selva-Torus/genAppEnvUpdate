...
const statusMap: Record<string, { icon: any; color: string }> = {
  ...
  let filterData4 = await getFilterProps(filterProps4,{...tran_main_group1dc7f,...tran_tab_group08b64,...view_all_tab4a963,...view_all_journey_group67ce4,...failure_queue_tab69f01,...success_queue_tabef582,...success_queue_journey_group755eb,...return_queue_tab5611e,...return_queue_journey_group92c55,...operational_pending_tab67331,...operational_pending_journey_group63667,...technical_pending_tab0b23f,...technical_pending_journey_groupe4f03,...failure_queue_journey_group36aba});
  ...
  const fetchPagination = async (key: string, page: number, count: number) => {
    ...
    const api_paginationBody: api_paginationDto = {
      ...
      page: page || 1,
      count: count,
      searchFilter: {"vgphstm_uuid":failure_queue_journey_group36aba?.vgphstm_uuid},
    }
    if (encryptionFlagCont) {
  ...
  useEffect(() => {
    if (!false && !failure_queue_journey_group36aba?.vgphstm_uuid) {
      setSteps([]);
      setHasMore(false);
      return;
    }
