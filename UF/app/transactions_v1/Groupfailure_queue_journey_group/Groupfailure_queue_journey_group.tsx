 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("failure_queue_journey")){
        setfailure_queue_journeyc8638((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(failure_queue_journeyc8638?.isDisabled==null)
      {
        setfailure_queue_journeyc8638((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !ml-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"failure_queue_journey_group"}))
        }}
    >
