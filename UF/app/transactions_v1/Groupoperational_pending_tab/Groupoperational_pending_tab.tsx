 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("operational_pending_table")){
        setoperational_pending_table0a253Props((pre:any)=>({...pre,...operational_pending_table0a253,isDisabled:true}));

    }else
    {
      if(operational_pending_table0a253?.isDisabled==null)
      {
        setoperational_pending_table0a253Props((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("operational_pending_journey_group")){
        setoperational_pending_journey_group63667((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(operational_pending_journey_group63667?.isDisabled==null)
      {
        setoperational_pending_journey_group63667((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(()=>{
    if(tran_tab_group08b64?.tran_tab_group === "operational_pending_tab"){
    handleOnChange()
    }
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"operational_pending_tab"}))
        }}
    >
