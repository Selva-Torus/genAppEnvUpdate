...
const Grouptran_main_group = dynamic(() => import("./Grouptran_main_group/Grouptran_main_group"), { ssr: false });

export default function PageTransactionsV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionProduct:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    settransactionproduct_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
