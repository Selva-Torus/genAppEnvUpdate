...
let defaultColumns:any = [
  {
    "id": "value_date_technical_pending",
  ...
  {
    "id": "dr_currency_technical_pending",
  ...
  {
    "id": "cr_currency_technical_pending",
  ...
  },
  {
    ...
    "nodeid": "e6313d8dd93044faac2d0f4df9d73334",
    "name": "UUID",
    "meta": {
      "sort": true
    },
...
const Tabletechnical_pending_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, technical_pending_table84f30 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, technical_pending_table84f30 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, technical_pending_table84f30 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      technical_pending_table84f30.filter((item:any,id:number)=>{
      ...
      settechnical_pending_table84f30Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (technical_pending_table84f30Props.filterInitalLoad) return;

    const filterControllers = technical_pending_table84f30Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['technical_pending_table84f30']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['technical_pending_table84f30']).filter((k: string) => jsonEntry['technical_pending_table84f30'][k] === false)
      : [];
    ...
    settechnical_pending_table84f30Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(technical_pending_table84f30Props.filterProps);

  }, [
    technical_pending_table84f30Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  const payload = JSON.stringify(technical_pending_table84f30Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(technical_pending_table84f30Props.searchFilter) || {};
...
}, [technical_pending_table84f30Props.searchFilter])
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = technical_pending_table84f30.find((data:any)=>(data?.vgphstm_uuid==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstm_uuid",
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    settechnical_pending_table84f30Props((pre:any)=>({...pre, selectedIds:[row?.vgphstm_uuid]}))  
    onButtonSecurityHandle(data)
    ...
    const result = allData.find((item:any) => item["vgphstm_uuid"] === data[pkCol?.id || "vgphstm_uuid"]);
          ...
          settechnical_pending_journey_groupe4f03({...technical_pending_journey_groupe4f03,presetValues:allData[index]})
