...
const Grouptechnical_pending_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
  ...
  "Operational Manager": {
    "allowedControls": [
      "value_date_technical_pending",
      "dr_account_technical_pending",
      "dr_name_technical_pending",
      "dr_currency_technical_pending",
      "dr_amount_technical_pending",
      "cr_account_technical_pending",
      "cr_name_technical_pending",
      "cr_currency_technical_pending",
 ...
 /////////////
  ...
  const {value_date_technical_pending11fe0, setvalue_date_technical_pending11fe0}= useContext(TotalContext) as TotalContextProps;
  const {dr_account_technical_pendinge182f, setdr_account_technical_pendinge182f}= useContext(TotalContext) as TotalContextProps;
  const {dr_name_technical_pendingbc6bb, setdr_name_technical_pendingbc6bb}= useContext(TotalContext) as TotalContextProps;
  const {dr_currency_technical_pendingbc856, setdr_currency_technical_pendingbc856}= useContext(TotalContext) as TotalContextProps;
  ...
  const {cr_name_technical_pending1bc34, setcr_name_technical_pending1bc34}= useContext(TotalContext) as TotalContextProps;
  const {cr_currency_technical_pending78349, setcr_currency_technical_pending78349}= useContext(TotalContext) as TotalContextProps;
  const {cr_amount_technical_pending738a2, setcr_amount_technical_pending738a2}= useContext(TotalContext) as TotalContextProps;
  const {uuid_failure_queue73334, setuuid_failure_queue73334}= useContext(TotalContext) as TotalContextProps;
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("value_date_technical_pending")){
        setvalue_date_technical_pending11fe0((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(value_date_technical_pending11fe0?.isDisabled==null)
      {
        setvalue_date_technical_pending11fe0((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_account_technical_pending")){
        setdr_account_technical_pendinge182f((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_account_technical_pendinge182f?.isDisabled==null)
      {
        setdr_account_technical_pendinge182f((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_name_technical_pending")){
        setdr_name_technical_pendingbc6bb((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_name_technical_pendingbc6bb?.isDisabled==null)
      {
        setdr_name_technical_pendingbc6bb((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_currency_technical_pending")){
        setdr_currency_technical_pendingbc856((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_currency_technical_pendingbc856?.isDisabled==null)
      {
        setdr_currency_technical_pendingbc856((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dr_amount_technical_pending")){
        setdr_amount_technical_pending5e6cc((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(dr_amount_technical_pending5e6cc?.isDisabled==null)
      {
        setdr_amount_technical_pending5e6cc((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account_technical_pending")){
        setcr_account_technical_pending3c4aa((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_account_technical_pending3c4aa?.isDisabled==null)
      {
        setcr_account_technical_pending3c4aa((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name_technical_pending")){
        setcr_name_technical_pending1bc34((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_name_technical_pending1bc34?.isDisabled==null)
      {
        setcr_name_technical_pending1bc34((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_currency_technical_pending")){
        setcr_currency_technical_pending78349((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_currency_technical_pending78349?.isDisabled==null)
      {
        setcr_currency_technical_pending78349((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("cr_amount_technical_pending")){
        setcr_amount_technical_pending738a2((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(cr_amount_technical_pending738a2?.isDisabled==null)
      {
        setcr_amount_technical_pending738a2((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("uuid_failure_queue")){
        setuuid_failure_queue73334((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(uuid_failure_queue73334?.isDisabled==null)
      {
        setuuid_failure_queue73334((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
  }
  ...
  const handleCustomCode=async () => {
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['value_date_technical_pending'] = value_date_technical_pending11fe0,
        codeStates['setvalue_date_technical_pending'] = setvalue_date_technical_pending11fe0,
        codeStates['dr_account_technical_pending'] = dr_account_technical_pendinge182f,
        codeStates['setdr_account_technical_pending'] = setdr_account_technical_pendinge182f,
        codeStates['dr_name_technical_pending'] = dr_name_technical_pendingbc6bb,
        codeStates['setdr_name_technical_pending'] = setdr_name_technical_pendingbc6bb,
        codeStates['dr_currency_technical_pending'] = dr_currency_technical_pendingbc856,
        codeStates['setdr_currency_technical_pending'] = setdr_currency_technical_pendingbc856,
        ...
        codeStates['cr_name_technical_pending'] = cr_name_technical_pending1bc34,
        codeStates['setcr_name_technical_pending'] = setcr_name_technical_pending1bc34,
        codeStates['cr_currency_technical_pending'] = cr_currency_technical_pending78349,
        codeStates['setcr_currency_technical_pending'] = setcr_currency_technical_pending78349,
        codeStates['cr_amount_technical_pending'] = cr_amount_technical_pending738a2,
        codeStates['setcr_amount_technical_pending'] = setcr_amount_technical_pending738a2,
        codeStates['uuid_failure_queue'] = uuid_failure_queue73334,
        codeStates['setuuid_failure_queue'] = setuuid_failure_queue73334,
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"technical_pending_table"}))
        }}
    >
