 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("technical_pending_journey")){
        settechnical_pending_journey6601c((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(technical_pending_journey6601c?.isDisabled==null)
      {
        settechnical_pending_journey6601c((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2  !ml-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"technical_pending_journey_group"}))
        }}
    >
