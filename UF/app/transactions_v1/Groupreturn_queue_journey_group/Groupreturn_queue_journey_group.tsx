 ...
 /////////////
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("return_queue_journey")){
        setreturn_queue_journeycc9d3((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(return_queue_journeycc9d3?.isDisabled==null)
      {
        setreturn_queue_journeycc9d3((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !ml-3 ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settransactionproduct_v1((pre:any)=>({...pre,_selectedGroup_:"return_queue_journey_group"}))
        }}
    >
