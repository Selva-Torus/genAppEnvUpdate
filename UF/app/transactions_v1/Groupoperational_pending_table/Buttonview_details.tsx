...
const Buttonview_details = ({ mainData,lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData,onSelectLock,rowIndex,currentSelectedIds,skipUnlockRef,tableName}: { mainData:any,lockedData:any,setLockedData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any,onSelectLock?:any,rowIndex?:number,currentSelectedIds?:string[],skipUnlockRef?:React.MutableRefObject<boolean>,tableName?:string}) => {
 ...
 /////////////
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      ...
      codeStates['inboundscanprocessui_v1'] = inboundscanprocessui_v1Props,
      codeStates['setinboundscanprocessui_v1'] = setinboundscanprocessui_v1Props,
  ...
  const handleClick=async()=>{
    ...
    // showArtifact
      if(validatedCondition(`direction === 'OUTBOUND'`,selectedData)) 
      {
    ...
    //bindTran
    // For group or table
    let bindData4 = filterByKeys(mainData,doclisttable56e97Props?.controls);
    setdoclisttable56e97(bindData4||{})
    setdoclisttable56e97Props({...doclisttable56e97Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData6 = filterByKeys(mainData,valdnlisttable17ec7Props?.controls);
    setvaldnlisttable17ec7(bindData6||{})
    setvaldnlisttable17ec7Props({...valdnlisttable17ec7Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData8 = filterByKeys(mainData,commoninfof4607Props?.controls);
    setcommoninfof4607(bindData8||{})
    setcommoninfof4607Props({...commoninfof4607Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData10 = filterByKeys(mainData,basicinfo3d198Props?.controls);
    setbasicinfo3d198(bindData10||{})
    setbasicinfo3d198Props({...basicinfo3d198Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData12 = filterByKeys(mainData,additionalinfod2894Props?.controls);
    setadditionalinfod2894(bindData12||{})
    setadditionalinfod2894Props({...additionalinfod2894Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData14 = filterByKeys(mainData,overallgroup01c61Props?.controls);
    setoverallgroup01c61(bindData14||{})
    ...
    //bindTran
    // For group or table
    let bindData20 = filterByKeys(mainData,rtgs_list_table2926aProps?.controls);
    setrtgs_list_table2926a(bindData20||{})
    ...
    // showArtifact
      if(validatedCondition(`direction === 'INBOUND' || direction === 'OUTBOUND' && trs_status ==='IR_VALIDATION_PASSED'`,selectedData)) 
      {
    let filterProps28: any =  [];
    let filterData28 = await getFilterProps(filterProps28,mainData);
    setinboundscanprocessui_v1Props([...filterData28 ]);
    if (skipUnlockRef) skipUnlockRef.current = true
    routes.push(getRouteScreenDetails('CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:inboundScanProcessUi:AFVK:v1', 'inboundscanprocessui_v1'));
      }
    //bindTran
    // For group or table
    let bindData30 = filterByKeys(mainData,overallgroup1218fProps?.controls);
    setoverallgroup1218f(bindData30||{})
    setoverallgroup1218fProps({...overallgroup1218fProps,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData32 = filterByKeys(mainData,commoninfod7edaProps?.controls);
    setcommoninfod7eda(bindData32||{})
    setcommoninfod7edaProps({...commoninfod7edaProps,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData34 = filterByKeys(mainData,basicinfoffb0aProps?.controls);
    setbasicinfoffb0a(bindData34||{})
    setbasicinfoffb0aProps({...basicinfoffb0aProps,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData36 = filterByKeys(mainData,additionalinfo4babaProps?.controls);
    setadditionalinfo4baba(bindData36||{})
    setadditionalinfo4babaProps({...additionalinfo4babaProps,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData38 = filterByKeys(mainData,valdnlisttable4db84Props?.controls);
    setvaldnlisttable4db84(bindData38||{})
    setvaldnlisttable4db84Props({...valdnlisttable4db84Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    let bindData40 = filterByKeys(mainData,button_groupb9855Props?.controls);
    setbutton_groupb9855(bindData40||{})
    setbutton_groupb9855Props({...button_groupb9855Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    setgroupdd3f6(mainData||{})
    setgroupdd3f6Props({...groupdd3f6Props,presetValues:{...(mainData||{})}})
    //bindTran
    // For group or table
    setcontrol_tab_group161ff(mainData||{})
    setcontrol_tab_group161ffProps({...control_tab_group161ffProps,presetValues:{...(mainData||{})}})
