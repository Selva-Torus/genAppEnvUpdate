...
import Textoverage_text  from "./Textoverage_text";
import BarChartssingle_barchart  from "./BarChartssingle_barchart";
...
const Groupoverage_charges_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "270ac2130b364de49c3f235476544542");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverage_charges_group44542Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("overage_text")){
        setoverage_textf2644((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(overage_textf2644?.isDisabled==null)
      {
        setoverage_textf2644((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("single_barchart")){
        setsingle_barchart0a200((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(single_barchart0a200?.isDisabled==null)
      {
        setsingle_barchart0a200((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['overage_text'] = overage_textf2644,
        codeStates['setoverage_text'] = setoverage_textf2644,
        codeStates['single_barchart'] = single_barchart0a200,
        codeStates['setsingle_barchart'] = setsingle_barchart0a200,
  ...
  const overage_charges_group44542Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overage_charges_group44542Ref.current?.setSearchParams();
    overage_charges_group44542Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(overage_charges_group44542) &&
        Object.keys(overage_charges_group44542)?.length > 0
      ) {
        setoverage_charges_group44542({})
      }
    } else prevRefreshRef.current = true
  }, [overage_charges_group44542Props?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmonetizationdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"overage_charges_group"}))
        }}
    >
          {allowedControls.includes("overage_text") ?<Textoverage_text   /* f2644 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("single_barchart") ?<BarChartssingle_barchart /* 0a200 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupoverage_charges_group
