...
import Textapi_text  from "./Textapi_text";
...
const Grouppiechart_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c4bcbe5aec8b4821a8adb5a787ace72b");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setpiechart_groupce72bProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_text")){
        setapi_text0685a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(api_text0685a?.isDisabled==null)
      {
        setapi_text0685a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("piechart")){
        setpiechart04991((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(piechart04991?.isDisabled==null)
      {
        setpiechart04991((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['api_text'] = api_text0685a,
        codeStates['setapi_text'] = setapi_text0685a,
        codeStates['piechart'] = piechart04991,
        codeStates['setpiechart'] = setpiechart04991,
  ...
  const piechart_groupce72bRef = useRef<any>(null);
  const handleClearSearch = () => {
    piechart_groupce72bRef.current?.setSearchParams();
    piechart_groupce72bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(piechart_groupce72b) &&
        Object.keys(piechart_groupce72b)?.length > 0
      ) {
        setpiechart_groupce72b({})
      }
    } else prevRefreshRef.current = true
  }, [piechart_groupce72bProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmonetizationdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"piechart_group"}))
        }}
    >
          {allowedControls.includes("api_text") ?<Textapi_text   /* 0685a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("piechart") ?<PieChartspiechart /* 04991 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouppiechart_group
