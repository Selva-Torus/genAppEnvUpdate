...
const Textapi_text = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const {api_text0685aProps, setapi_text0685aProps} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  useEffect(()=>{
    handleMapperValue()
  },[api_text0685a?.refresh])

  if (api_text0685a?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset("Revenue by API Type")}
</Text>
  </div>
  )
}

export default Textapi_text
