...
const Cardavg_revenue_tpp_card = ({checkToAdd,setCheckToAdd,encryptionFlagCompData,setIsProcessing,controlData}:any) => {
  ...
  const {avg_revenue_tpp_card6eb26Props, setavg_revenue_tpp_card6eb26Props} = useContext(TotalContext) as TotalContextProps;
  //////////////
  ...
  const handleMapperDetails=async(filterProps?:any,filterFlag?:boolean):Promise<void>=>{
    ...
    const orchestrationData : any = getControlOrchestrationData(
        controlData,
        "b878467bae374a6fbf54c8f1c8656d8e",
        "c121e16d00294c06bfa47b8e9566eb26"
      );
    ...
    try{
      if ("hasLogicCenter" in dfd_tob_mzdsh_totalcards_dfd_v1Props && !dfd_tob_mzdsh_totalcards_dfd_v1Props.hasLogicCenter) {
        ...
        setavg_revenue_tpp_card_group56d8e((pre: any) => ({
          ...pre,
          avg_revenue_per_tpp: api_paginationData.data.records?.length > 0
            ? api_paginationData.data.records[0]?.avg_revenue_per_tpp
            : "0"
        }))
      }
      else{
        if(filterFlag){
          setavg_revenue_tpp_card_group56d8e((pre: any) => ({
            ...pre,
            avg_revenue_per_tpp: avg_revenue_tpp_card6eb26Props?.filteredData?.length > 0
              ? avg_revenue_tpp_card6eb26Props?.filteredData[0]?.avg_revenue_per_tpp
              : "0"
          }))
        }else if(Array.isArray(dfd_tob_mzdsh_totalcards_dfd_v1Props) && dfd_tob_mzdsh_totalcards_dfd_v1Props && !avg_revenue_tpp_card_group56d8e.avg_revenue_per_tpp){
          setavg_revenue_tpp_card_group56d8e((pre:any)=>({...pre,avg_revenue_per_tpp:dfd_tob_mzdsh_totalcards_dfd_v1Props[0]?.avg_revenue_per_tpp}))
        }
      }
    }catch(err){
      console.log(err)
  ...
  },[avg_revenue_tpp_card6eb26?.refresh])
  ...
  // setSearchFilters
  useEffect(() => {
    if (!avg_revenue_tpp_card6eb26Props?.filterProps) return;
    handleMapperDetails(avg_revenue_tpp_card6eb26Props?.filterProps,avg_revenue_tpp_card6eb26Props?.filterFlag);
  },[avg_revenue_tpp_card6eb26Props?.filterProps])
  ...
  if (avg_revenue_tpp_card6eb26?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `5 / 25`,gridRow: `1 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className="p-1 !text-xl !text-white font-bold"   
      theme="normal"
      view="clear"
      label={keyset("Avg Revenue / TPP")}
      disabled= {avg_revenue_tpp_card6eb26?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {avg_revenue_tpp_card_group56d8e?.avg_revenue_per_tpp?avg_revenue_tpp_card_group56d8e?.avg_revenue_per_tpp:"0"}
      </Card>
    </div>
  )
}

export default Cardavg_revenue_tpp_card
