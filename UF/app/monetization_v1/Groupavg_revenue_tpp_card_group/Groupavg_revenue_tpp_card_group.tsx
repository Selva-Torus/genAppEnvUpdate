...
import Cardavg_revenue_tpp_card  from "./Cardavg_revenue_tpp_card";
import Icontpp_icon  from "./Icontpp_icon";
...
const Groupavg_revenue_tpp_card_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {avg_revenue_tpp_card6eb26, setavg_revenue_tpp_card6eb26}= useContext(TotalContext) as TotalContextProps;
  const {tpp_icon1dafd, settpp_icon1dafd}= useContext(TotalContext) as TotalContextProps;
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "b878467bae374a6fbf54c8f1c8656d8e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setavg_revenue_tpp_card_group56d8eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("avg_revenue_tpp_card")){
        setavg_revenue_tpp_card6eb26((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(avg_revenue_tpp_card6eb26?.isDisabled==null)
      {
        setavg_revenue_tpp_card6eb26((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("tpp_icon")){
        settpp_icon1dafd((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(tpp_icon1dafd?.isDisabled==null)
      {
        settpp_icon1dafd((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['avg_revenue_tpp_card'] = avg_revenue_tpp_card6eb26,
        codeStates['setavg_revenue_tpp_card'] = setavg_revenue_tpp_card6eb26,
        codeStates['tpp_icon'] = tpp_icon1dafd,
        codeStates['settpp_icon'] = settpp_icon1dafd,
  ...
  const avg_revenue_tpp_card_group56d8eRef = useRef<any>(null);
  const handleClearSearch = () => {
    avg_revenue_tpp_card_group56d8eRef.current?.setSearchParams();
    avg_revenue_tpp_card_group56d8eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(avg_revenue_tpp_card_group56d8e) &&
        Object.keys(avg_revenue_tpp_card_group56d8e)?.length > 0
      ) {
        setavg_revenue_tpp_card_group56d8e({})
      }
    } else prevRefreshRef.current = true
  }, [avg_revenue_tpp_card_group56d8eProps?.refresh])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://tdps3api.toruslowcode.com/torus/9.1/CT009/resources/images/blue.jpg')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmonetizationdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"avg_revenue_tpp_card_group"}))
        }}
    >
        {allowedControls.includes("avg_revenue_tpp_card") ?<Cardavg_revenue_tpp_card  /* 6eb26 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("tpp_icon")?<Icontpp_icon /* 1dafd */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}

export default Groupavg_revenue_tpp_card_group
