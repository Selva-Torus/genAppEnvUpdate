...
import Cardmonthly_revenue_card  from "./Cardmonthly_revenue_card";
import Iconmonthly_icon  from "./Iconmonthly_icon";
...
const Groupmonthly_revenue_card_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "386a0be2ea694e1db14213aad673bf72");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setmonthly_revenue_card_group3bf72Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("monthly_revenue_card")){
        setmonthly_revenue_card0ab73((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(monthly_revenue_card0ab73?.isDisabled==null)
      {
        setmonthly_revenue_card0ab73((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("monthly_icon")){
        setmonthly_icone201a((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(monthly_icone201a?.isDisabled==null)
      {
        setmonthly_icone201a((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const monthly_revenue_card_group3bf72Ref = useRef<any>(null);
  const handleClearSearch = () => {
    monthly_revenue_card_group3bf72Ref.current?.setSearchParams();
    monthly_revenue_card_group3bf72Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(monthly_revenue_card_group3bf72) &&
        Object.keys(monthly_revenue_card_group3bf72)?.length > 0
      ) {
        setmonthly_revenue_card_group3bf72({})
      }
    } else prevRefreshRef.current = true
  }, [monthly_revenue_card_group3bf72Props?.refresh])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://tdps3api.toruslowcode.com/torus/9.1/CT009/resources/images/monet.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmonetizationdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"monthly_revenue_card_group"}))
        }}
    >
        {allowedControls.includes("monthly_revenue_card") ?<Cardmonthly_revenue_card  /* 0ab73 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("monthly_icon")?<Iconmonthly_icon /* e201a */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}

export default Groupmonthly_revenue_card_group
