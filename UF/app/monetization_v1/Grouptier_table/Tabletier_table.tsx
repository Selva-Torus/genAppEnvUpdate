...
let defaultColumns:any = [
  {
    "id": "pricing_id_column",
    "nodeid": "d08506050a874a3b911aca9dee62bf94",
    "name": "Pricing ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "monthly_fee_column",
    "nodeid": "2505f42936484ce1a30fe2bed2b42a75",
    "name": "Monthly Fee",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "overage_rate_column",
    "nodeid": "fa9ed45c63a140a5800c5ecbf7a50e70",
    "name": "Overage Rate",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "overage_price_per_call"
  },
  {
    "id": "tpps_column",
    "nodeid": "94adca3ff8bb405ba5ec4a8f81d84964",
    "name": "TPPs On their Tier",
    "meta": {
      "sort": true
    },
    "className": "",
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "882d36d872e54124b044d8dfed717c1c",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, tier_table17c1c })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, tier_table17c1c }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, tier_table17c1c } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settier_table17c1cProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      tier_table17c1c.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settier_table17c1cProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settier_table17c1cProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settier_table17c1cProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false,sortingDetails:any={}){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '882d36d872e54124b044d8dfed717c1c',
                elementId: '882d36d872e54124b044d8dfed717c1c'
              },
              sortingDetails
            }
          }else
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( tier_table17c1cProps?.presetValues&&Object.keys(tier_table17c1cProps?.presetValues).length > 0) {
          filtertedData = [tier_table17c1cProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settier_table17c1c(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (tier_table17c1cProps.filterInitalLoad) return;

    const filterControllers = tier_table17c1cProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['tier_table17c1c']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['tier_table17c1c']).filter((k: string) => jsonEntry['tier_table17c1c'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(tier_table17c1cProps.filterProps);
      return;
    }
    ...
    settier_table17c1cProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(tier_table17c1cProps.filterProps);

  }, [
    tier_table17c1cProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    tier_table17c1cProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(tier_table17c1cProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(tier_table17c1cProps.searchFilter) || {};
...
}, [tier_table17c1cProps.searchFilter])
  ...
  useEffect(() => {
    if (tier_table17c1cProps?.clearData === true) {
      settier_table17c1c([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settier_table17c1cProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [tier_table17c1cProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = tier_table17c1cProps?.selectedIds[tier_table17c1cProps?.selectedIds?.length-1]
    if(Array.isArray(tier_table17c1c) && tier_table17c1c.length>0)
    {
      let data = tier_table17c1c.find((data:any)=>(data?.test==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "test",
        value: data["test"],
  ...
  useEffect(() => {
    if (tier_table17c1cProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (tier_table17c1cProps?.selectedIds?.length == 0){
      handleOnRowClick({},tier_table17c1cProps?.selectedIds)
    }
  }, [tier_table17c1cProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(tier_table17c1c?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&tier_table17c1c?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](tier_table17c1c?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tier_table17c1c?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&tier_table17c1c?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](tier_table17c1c?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: tier_table17c1c?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"882d36d872e54124b044d8dfed717c1c"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={tier_table17c1cProps?.selectedIds}  
...
export default Tabletier_table
