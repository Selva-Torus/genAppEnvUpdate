...
import Cardinvoice_raised_card  from "./Cardinvoice_raised_card";
import Iconinvoice_icon  from "./Iconinvoice_icon";
...
const Groupinvoice_raised_card_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d4353774fbf8421aa2e2af1740923315");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setinvoice_raised_card_group23315Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("invoice_raised_card")){
        setinvoice_raised_card87b51((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(invoice_raised_card87b51?.isDisabled==null)
      {
        setinvoice_raised_card87b51((pre:any)=>({...pre,isDisabled:false}));
      }
    }
    if(orchestrationData?.data?.readableControls.includes("invoice_icon")){
        setinvoice_icon43350((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(invoice_icon43350?.isDisabled==null)
      {
        setinvoice_icon43350((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  const invoice_raised_card_group23315Ref = useRef<any>(null);
  const handleClearSearch = () => {
    invoice_raised_card_group23315Ref.current?.setSearchParams();
    invoice_raised_card_group23315Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(invoice_raised_card_group23315) &&
        Object.keys(invoice_raised_card_group23315)?.length > 0
      ) {
        setinvoice_raised_card_group23315({})
      }
    } else prevRefreshRef.current = true
  }, [invoice_raised_card_group23315Props?.refresh])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://tdps3api.toruslowcode.com/torus/9.1/CT009/resources/images/neeew1.jpg')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-xl &lt;div class=&#34;col-span-1 row-span-22&#34;&gt;      &lt;!-- Static container (NO transform here) --&gt;   &lt;div class=&#34;h-full w-full&#34;&gt;      &lt;!-- Animated card --&gt;     &lt;div class=&#34;h-full w-full p-0.5 bg-white rounded-xl shadow-md                  transition-transform duration-300 ease-out                  hover:-translate-y-2 hover:shadow-2xl                  active:scale-95                  cursor-pointer&#34;&gt;              &lt;h3 class=&#34;text-lg font-semibold&#34;&gt;Card Title&lt;/h3&gt;       &lt;p class=&#34;text-gray-600&#34;&gt;Content here&lt;/p&gt;      &lt;/div&gt;    &lt;/div&gt; &lt;/div&gt; ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setmonetizationdashboard_v1((pre:any)=>({...pre,_selectedGroup_:"invoice_raised_card_group"}))
        }}
    >
        {allowedControls.includes("invoice_raised_card") ?<Cardinvoice_raised_card  /* 87b51 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
        {allowedControls.includes("invoice_icon")?<Iconinvoice_icon /* 43350 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}  />: <div></div>}
    </div>
 )
}

export default Groupinvoice_raised_card_group
