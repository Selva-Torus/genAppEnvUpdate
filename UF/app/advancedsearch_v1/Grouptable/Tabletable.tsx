...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "150eacbb64df42d4a5eb45d62c038670",
    "name": "country",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "state",
    "nodeid": "16ded0c2820d48ad976d1785abbf29da",
    "name": "state",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {advancedsearch_v1, setadvancedsearch_v1} = useContext(TotalContext) as TotalContextProps;
  const {advancedsearch_v1Props, setadvancedsearch_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const {country38670, setcountry38670}= useContext(TotalContext) as TotalContextProps  
  const {statef29da, setstatef29da}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "83a889c5cf1a4aa0baa4e6e8d337d435",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['country'] = country38670,
        codeStates['setcountry'] = setcountry38670,
        codeStates['state'] = statef29da,
        codeStates['setstate'] = setstatef29da,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    settable7d435Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable7d435Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(advancedsearch_v1Props.length > 0){
      for(let i=0;i< advancedsearch_v1Props.length;i++){
        if(advancedsearch_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TGW01:AFGK:TGW004:AFK:country_code_dfd:AFVK:v1"){
          // delete advancedsearch_v1Props[i].DFDkey;
          let temp=structuredClone(advancedsearch_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:advancedsearch:AFVK:v1:UO', 
                nodeId: '83a889c5cf1a4aa0baa4e6e8d337d435',
                elementId: '83a889c5cf1a4aa0baa4e6e8d337d435'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:advancedsearch:AFVK:v1',
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table7d435Props?.presetValues&&Object.keys(table7d435Props?.presetValues).length > 0) {
          filtertedData = [table7d435Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable7d435(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table7d435Props.filterInitalLoad) return;

    const filterControllers = table7d435Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table7d435']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table7d435']).filter((k: string) => jsonEntry['table7d435'][k] === false)
      : [];
    ...
    settable7d435Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table7d435Props.filterProps);

  }, [
    table7d435Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table7d435Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table7d435Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table7d435Props.searchFilter) || {};
  ...
  useEffect(() => {
    if (table7d435Props?.clearData === true) {
      settable7d435([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      settable7d435Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table7d435Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [table7d435Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = table7d435Props?.selectedIds[table7d435Props?.selectedIds?.length-1]
    if(Array.isArray(table7d435) && table7d435.length>0)
    {
      let data = table7d435[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "test",
        value: data["test"],
  ...
  useEffect(() => {
    if (table7d435Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (table7d435Props?.selectedIds?.length == 0){
      handleOnRowClick({},table7d435Props?.selectedIds)
    }
  }, [table7d435Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table7d435?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table7d435?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table7d435?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table7d435?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table7d435?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table7d435?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table7d435?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"83a889c5cf1a4aa0baa4e6e8d337d435"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={table7d435Props?.selectedIds}  
