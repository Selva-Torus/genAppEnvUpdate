...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TGW01:AFGK:TGW004:AFK:advancedsearch:AFVK:v1',
    [user],
    'GroupGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "62fca87896cc475fa1bcb5e25e9f6bcb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setgroupf6bcbProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("advancesearch")){
        setadvancesearch6c997({...advancesearch6c997,isDisabled:true});

    }else
    {
      if(advancesearch6c997?.isDisabled==null)
      {
        setadvancesearch6c997({...advancesearch6c997,isDisabled:false});
      }
    }
    if(orchestrationData?.data?.readableControls.includes("dateandtime")){
        setdateandtimeb3eda({...dateandtimeb3eda,isDisabled:true});

    }else
    {
      if(dateandtimeb3eda?.isDisabled==null)
      {
        setdateandtimeb3eda({...dateandtimeb3eda,isDisabled:false});
      }
    }
  //////////////
    if (code != '') {
  ...
  const groupf6bcbRef = useRef<any>(null);
  const handleClearSearch = () => {
    groupf6bcbRef.current?.setSearchParams();
    groupf6bcbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupf6bcb) && Object.keys(groupf6bcb)?.length>0)
      {
        setgroupf6bcb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupf6bcbProps?.refresh,token])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          setadvancedsearch_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
        {allowedControls.includes("advancesearch")?<AdvancedSearchadvancesearch /* 6c997 */ encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dateandtime") ?<DateAndTimedateandtime   /* b3eda */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
