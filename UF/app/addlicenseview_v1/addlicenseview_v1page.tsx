...
export default function PageAddlicenseviewV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetsoftwarelicenses_v1DFD(pagination:any): Promise<void>{
        ...
        if(addlicenseview_v1Props.length > 0){
          for(let i=0;i< addlicenseview_v1Props.length;i++){
            if(addlicenseview_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1"){
              // delete addlicenseview_v1Props[i].DFDkey;
              let temp=structuredClone(addlicenseview_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setaddlicenseview_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setaddlicenseview_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(addlicenseview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(addlicenseview_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
