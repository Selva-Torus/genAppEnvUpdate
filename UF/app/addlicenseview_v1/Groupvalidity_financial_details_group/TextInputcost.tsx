...
const TextInputcost = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.cost"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|7bc0f89fb2071723321071b41d9b8a9f|a0e388129d691de69111b0f3e042568f"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,cost:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, cost: +e.target.value }));
    }
    else{
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, cost: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "a0e388129d691de69111b0f3e042568f"
      );
      ...
      //     controlId: "a0e388129d691de69111b0f3e042568f",
      //     isTable: false,
      //     from:"TextInputcost",
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "a0e388129d691de69111b0f3e042568f") {
        handleChange({target:{value:validity_financial_details_groupb8a9fRef?.current?.cost||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "a0e388129d691de69111b0f3e042568f") {
        handleBlur({target:{value:validity_financial_details_groupb8a9fRef?.current?.cost||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setvalidity_financial_details_groupb8a9f((pre:any)=>({...pre,cost:dfd_assetsoftwarelicenses_v1Props.data[0]?.cost}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (cost2568f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={validity_financial_details_groupb8a9f?.cost||""}
         disabled= {cost2568f?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerText="Cost"
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.cost ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
