...
const TextInputsupport_expiry = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.support_expiry"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|7bc0f89fb2071723321071b41d9b8a9f|f696651a31ad4ea18d146cadcfabfd9e"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'support_expiry',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,support_expiry:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, support_expiry: +e.target.value }));
    }
    else{
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, support_expiry: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "f696651a31ad4ea18d146cadcfabfd9e"
      );
      ...
      //     controlId: "f696651a31ad4ea18d146cadcfabfd9e",
      //     isTable: false,
      //     from:"TextInputsupport_expiry",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'support_expiry', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'support_expiry',type:'text'};
      //   type={
      //     name:'support_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.support_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.support_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.support_expiry.type
      //   }
      ...
      //   type={
      //     name:'support_expiry',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.support_expiry.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.support_expiry.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.support_expiry.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "f696651a31ad4ea18d146cadcfabfd9e") {
        handleChange({target:{value:validity_financial_details_groupb8a9fRef?.current?.support_expiry||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "f696651a31ad4ea18d146cadcfabfd9e") {
        handleBlur({target:{value:validity_financial_details_groupb8a9fRef?.current?.support_expiry||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setvalidity_financial_details_groupb8a9f((pre:any)=>({...pre,support_expiry:dfd_assetsoftwarelicenses_v1Props.data[0]?.support_expiry}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (support_expirybfd9e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={validity_financial_details_groupb8a9f?.support_expiry||""}
         disabled= {support_expirybfd9e?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.support_expiry ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputsupport_expiry
