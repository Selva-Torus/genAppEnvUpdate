...
const TextInputexpiry_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.expiry_date"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|7bc0f89fb2071723321071b41d9b8a9f|6a85756a234a449ab6804ce8fc174df0"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'expiry_date',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,expiry_date:undefined}}));
    if(dynamicStateandType.type=="number"){
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, expiry_date: +e.target.value }));
    }
    else{
    setvalidity_financial_details_groupb8a9f((prev: any) => ({ ...prev, expiry_date: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6a85756a234a449ab6804ce8fc174df0"
      );
      ...
      //     controlId: "6a85756a234a449ab6804ce8fc174df0",
      //     isTable: false,
      //     from:"TextInputexpiry_date",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'expiry_date', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'expiry_date',type:'text'};
      //   type={
      //     name:'expiry_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expiry_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expiry_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.expiry_date.type
      //   }
      ...
      //   type={
      //     name:'expiry_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expiry_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expiry_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.expiry_date.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6a85756a234a449ab6804ce8fc174df0") {
        handleChange({target:{value:validity_financial_details_groupb8a9fRef?.current?.expiry_date||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6a85756a234a449ab6804ce8fc174df0") {
        handleBlur({target:{value:validity_financial_details_groupb8a9fRef?.current?.expiry_date||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setvalidity_financial_details_groupb8a9f((pre:any)=>({...pre,expiry_date:dfd_assetsoftwarelicenses_v1Props.data[0]?.expiry_date}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (expiry_date74df0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={validity_financial_details_groupb8a9f?.expiry_date||""}
         disabled= {expiry_date74df0?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.expiry_date ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputexpiry_date
