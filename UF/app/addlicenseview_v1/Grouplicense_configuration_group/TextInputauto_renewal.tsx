...
const TextInputauto_renewal = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.auto_renewal"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|8705ff89e003dcf5ffb5c6c5dc7a329d|6e98b2100ed34ac4859632ca3293bee1"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'auto_renewal',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,auto_renewal:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_configuration_groupa329d((prev: any) => ({ ...prev, auto_renewal: +e.target.value }));
    }
    else{
    setlicense_configuration_groupa329d((prev: any) => ({ ...prev, auto_renewal: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6e98b2100ed34ac4859632ca3293bee1"
      );
      ...
      //     controlId: "6e98b2100ed34ac4859632ca3293bee1",
      //     isTable: false,
      //     from:"TextInputauto_renewal",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'auto_renewal', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'auto_renewal',type:'text'};
      //   type={
      //     name:'auto_renewal',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.auto_renewal.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.auto_renewal.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.auto_renewal.type
      //   }
      ...
      //   type={
      //     name:'auto_renewal',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.auto_renewal.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.auto_renewal.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.auto_renewal.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6e98b2100ed34ac4859632ca3293bee1") {
        handleChange({target:{value:license_configuration_groupa329dRef?.current?.auto_renewal||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6e98b2100ed34ac4859632ca3293bee1") {
        handleBlur({target:{value:license_configuration_groupa329dRef?.current?.auto_renewal||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_configuration_groupa329d((pre:any)=>({...pre,auto_renewal:dfd_assetsoftwarelicenses_v1Props.data[0]?.auto_renewal}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (auto_renewal3bee1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_configuration_groupa329d?.auto_renewal||""}
         disabled= {auto_renewal3bee1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.auto_renewal ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputauto_renewal
