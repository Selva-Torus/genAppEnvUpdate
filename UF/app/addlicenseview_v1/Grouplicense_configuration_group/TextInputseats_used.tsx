...
const TextInputseats_used = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|8705ff89e003dcf5ffb5c6c5dc7a329d|cecc70d893eb39d9deb1a1465cd3bba9"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,seats_used:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_configuration_groupa329d((prev: any) => ({ ...prev, seats_used: +e.target.value }));
    }
    else{
    setlicense_configuration_groupa329d((prev: any) => ({ ...prev, seats_used: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "cecc70d893eb39d9deb1a1465cd3bba9"
      );
      ...
      //     controlId: "cecc70d893eb39d9deb1a1465cd3bba9",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "cecc70d893eb39d9deb1a1465cd3bba9") {
        handleChange({target:{value:license_configuration_groupa329dRef?.current?.seats_used||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "cecc70d893eb39d9deb1a1465cd3bba9") {
        handleBlur({target:{value:license_configuration_groupa329dRef?.current?.seats_used||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_configuration_groupa329d((pre:any)=>({...pre,seats_used:dfd_assetsoftwarelicenses_v1Props.data[0]?.seats_used}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (seats_used3bba9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_configuration_groupa329d?.seats_used||""}
         disabled= {seats_used3bba9?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.seats_used ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
