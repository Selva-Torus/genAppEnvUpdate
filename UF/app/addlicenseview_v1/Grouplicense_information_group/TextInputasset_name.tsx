...
const TextInputasset_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.asset_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|33d0de4ebc31e801cefbbc9fa134e03c|c9db3159bd41477886a1bac954f1ae9b"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,asset_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_information_group4e03c((prev: any) => ({ ...prev, asset_name: +e.target.value }));
    }
    else{
    setlicense_information_group4e03c((prev: any) => ({ ...prev, asset_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c9db3159bd41477886a1bac954f1ae9b"
      );
      ...
      //     controlId: "c9db3159bd41477886a1bac954f1ae9b",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "c9db3159bd41477886a1bac954f1ae9b") {
        handleChange({target:{value:license_information_group4e03cRef?.current?.asset_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "c9db3159bd41477886a1bac954f1ae9b") {
        handleBlur({target:{value:license_information_group4e03cRef?.current?.asset_name||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_information_group4e03c((pre:any)=>({...pre,asset_name:dfd_assetsoftwarelicenses_v1Props.data[0]?.asset_name}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (asset_name1ae9b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_information_group4e03c?.asset_name||""}
         disabled= {asset_name1ae9b?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.asset_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
