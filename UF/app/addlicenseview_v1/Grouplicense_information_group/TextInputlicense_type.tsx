...
const TextInputlicense_type = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.license_type"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|33d0de4ebc31e801cefbbc9fa134e03c|9e9711cf92745b7e76a98f016e0c8c15"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'license_type',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,license_type:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_information_group4e03c((prev: any) => ({ ...prev, license_type: +e.target.value }));
    }
    else{
    setlicense_information_group4e03c((prev: any) => ({ ...prev, license_type: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9e9711cf92745b7e76a98f016e0c8c15"
      );
      ...
      //     controlId: "9e9711cf92745b7e76a98f016e0c8c15",
      //     isTable: false,
      //     from:"TextInputlicense_type",
      ...
      //   type={
      //     name:'license_type',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.license_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.license_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.license_type.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "9e9711cf92745b7e76a98f016e0c8c15") {
        handleChange({target:{value:license_information_group4e03cRef?.current?.license_type||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "9e9711cf92745b7e76a98f016e0c8c15") {
        handleBlur({target:{value:license_information_group4e03cRef?.current?.license_type||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_information_group4e03c((pre:any)=>({...pre,license_type:dfd_assetsoftwarelicenses_v1Props.data[0]?.license_type}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (license_typec8c15?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_information_group4e03c?.license_type||""}
         disabled= {license_typec8c15?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.license_type ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputlicense_type
