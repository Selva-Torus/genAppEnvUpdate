...
const TextInputproduct_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetSoftwareLicenses:AFVK:v1|a1f54b62a6e841a3810307b90725a55a|properties.product_name"
      ],
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|33d0de4ebc31e801cefbbc9fa134e03c|395c11430a3c89dd5214ecaab7711c98"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'product_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,product_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_information_group4e03c((prev: any) => ({ ...prev, product_name: +e.target.value }));
    }
    else{
    setlicense_information_group4e03c((prev: any) => ({ ...prev, product_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "395c11430a3c89dd5214ecaab7711c98"
      );
      ...
      //     controlId: "395c11430a3c89dd5214ecaab7711c98",
      //     isTable: false,
      //     from:"TextInputproduct_name",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'product_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'product_name',type:'text'};
      //   type={
      //     name:'product_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.product_name.type
      //   }
      ...
      //   type={
      //     name:'product_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.product_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "395c11430a3c89dd5214ecaab7711c98") {
        handleChange({target:{value:license_information_group4e03cRef?.current?.product_name||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "395c11430a3c89dd5214ecaab7711c98") {
        handleBlur({target:{value:license_information_group4e03cRef?.current?.product_name||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_information_group4e03c((pre:any)=>({...pre,product_name:dfd_assetsoftwarelicenses_v1Props.data[0]?.product_name}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (product_name11c98?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_information_group4e03c?.product_name||""}
         disabled= {product_name11c98?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.product_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputproduct_name
