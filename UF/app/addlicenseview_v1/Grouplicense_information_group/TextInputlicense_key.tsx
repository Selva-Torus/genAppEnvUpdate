...
const TextInputlicense_key = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:addLicenseView:AFVK:v1|33d0de4ebc31e801cefbbc9fa134e03c|6ea4df4b2e744ebc8561ec1fec1ab6d1"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,addLicenseView_v1:{...pre?.addLicenseView_v1,license_key:undefined}}));
    if(dynamicStateandType.type=="number"){
    setlicense_information_group4e03c((prev: any) => ({ ...prev, license_key: +e.target.value }));
    }
    else{
    setlicense_information_group4e03c((prev: any) => ({ ...prev, license_key: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "6ea4df4b2e744ebc8561ec1fec1ab6d1"
      );
      ...
      //     controlId: "6ea4df4b2e744ebc8561ec1fec1ab6d1",
      //     isTable: false,
  ...
  useEffect(()=>{
    ...
    const handlerChange = (id:any) => {
      if (id === "6ea4df4b2e744ebc8561ec1fec1ab6d1") {
        handleChange({target:{value:license_information_group4e03cRef?.current?.license_key||""}});
      }
    };
    const handlerBlur = (id:any) => {
      if (id === "6ea4df4b2e744ebc8561ec1fec1ab6d1") {
        handleBlur({target:{value:license_information_group4e03cRef?.current?.license_key||""}});
      }
    };
  ...
  if(dfd_assetsoftwarelicenses_v1Props?.setSearchFilters && dfd_assetsoftwarelicenses_v1Props?.data)
  {
    if(Array.isArray(dfd_assetsoftwarelicenses_v1Props.data) && dfd_assetsoftwarelicenses_v1Props.data.length > 0){
      setlicense_information_group4e03c((pre:any)=>({...pre,license_key:dfd_assetsoftwarelicenses_v1Props.data[0]?.license_key}));
    }
  }
  },[dfd_assetsoftwarelicenses_v1Props?.setSearchFilters])
  if (license_keyab6d1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={license_information_group4e03c?.license_key||""}
         disabled= {license_keyab6d1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.addLicenseView_v1?.license_key ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
