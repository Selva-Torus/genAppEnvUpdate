...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "5be6bd9d106949d28fcffdcc8a30de1c",
    "name": "country",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "state",
    "nodeid": "b50fef887ce14624bffc2005f7e64484",
    "name": "state",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "city",
    "nodeid": "8fc62e2e1f0b4f41971102d4ab66c663",
    "name": "city",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "0076e480888e46f2a4a89dadcac03366",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, advancesearchtable03366 })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, advancesearchtable03366 }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, advancesearchtable03366 } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setadvancesearchtable03366Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      advancesearchtable03366.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        setadvancesearchtable03366Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setadvancesearchtable03366Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setadvancesearchtable03366Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:ad_search_table:AFVK:v1:UO', 
                nodeId: '0076e480888e46f2a4a89dadcac03366',
                elementId: '0076e480888e46f2a4a89dadcac03366'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( advancesearchtable03366Props?.presetValues&&Object.keys(advancesearchtable03366Props?.presetValues).length > 0) {
          filtertedData = [advancesearchtable03366Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setadvancesearchtable03366(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (advancesearchtable03366Props.filterInitalLoad) return;

    const filterControllers = advancesearchtable03366Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['advancesearchtable03366']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['advancesearchtable03366']).filter((k: string) => jsonEntry['advancesearchtable03366'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(advancesearchtable03366Props.filterProps);
      return;
    }
    ...
    setadvancesearchtable03366Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(advancesearchtable03366Props.filterProps);

  }, [
    advancesearchtable03366Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    advancesearchtable03366Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(advancesearchtable03366Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(advancesearchtable03366Props.searchFilter) || {};
...
}, [advancesearchtable03366Props.searchFilter])
  ...
  useEffect(() => {
    if (advancesearchtable03366Props?.clearData === true) {
      setadvancesearchtable03366([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setadvancesearchtable03366Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [advancesearchtable03366Props?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = advancesearchtable03366Props?.selectedIds[advancesearchtable03366Props?.selectedIds?.length-1]
    if(Array.isArray(advancesearchtable03366) && advancesearchtable03366.length>0)
    {
      let data = advancesearchtable03366.find((data:any)=>(data?.my_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "my_id",
        value: data["my_id"],
  ...
  useEffect(() => {
    if (advancesearchtable03366Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (advancesearchtable03366Props?.selectedIds?.length == 0){
      handleOnRowClick({},advancesearchtable03366Props?.selectedIds)
    }
  }, [advancesearchtable03366Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(advancesearchtable03366?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&advancesearchtable03366?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](advancesearchtable03366?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: advancesearchtable03366?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&advancesearchtable03366?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](advancesearchtable03366?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: advancesearchtable03366?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (advancesearchtable03366?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"0076e480888e46f2a4a89dadcac03366"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={advancesearchtable03366Props?.selectedIds}  
...
export default Tableadvancesearchtable
