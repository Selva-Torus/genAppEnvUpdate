...
let defaultColumns:any = [
  {
    "id": "state_two",
    "nodeid": "9cc9281c0e8f4d7bb2b31044d207430b",
    "name": "State",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "8b24119968b045fcabecd19f3fdb639c",
      );
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, table_oneb639c })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, table_oneb639c }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, table_oneb639c } = latestLockStateRef.current
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settable_oneb639cProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      table_oneb639c.filter((item:any,id:number)=>{
      ...
      //////////
      //////////
        settable_oneb639cProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      settable_oneb639cProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable_oneb639cProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(set_memory_table_v1Props.length > 0){
      for(let i=0;i< set_memory_table_v1Props.length;i++){
        if(set_memory_table_v1Props[i].DFDkey == ""){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '8b24119968b045fcabecd19f3fdb639c',
                elementId: '8b24119968b045fcabecd19f3fdb639c'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table_oneb639cProps?.presetValues&&Object.keys(table_oneb639cProps?.presetValues).length > 0) {
          filtertedData = [table_oneb639cProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable_oneb639c(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table_oneb639cProps.filterInitalLoad) return;

    const filterControllers = table_oneb639cProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table_oneb639c']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table_oneb639c']).filter((k: string) => jsonEntry['table_oneb639c'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(table_oneb639cProps.filterProps);
      return;
    }
    ...
    settable_oneb639cProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table_oneb639cProps.filterProps);

  }, [
    table_oneb639cProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table_oneb639cProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table_oneb639cProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table_oneb639cProps.searchFilter) || {};
...
}, [table_oneb639cProps.searchFilter])
  ...
  useEffect(() => {
    if (table_oneb639cProps?.clearData === true) {
      settable_oneb639c([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settable_oneb639cProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table_oneb639cProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = table_oneb639cProps?.selectedIds[table_oneb639cProps?.selectedIds?.length-1]
    if(Array.isArray(table_oneb639c) && table_oneb639c.length>0)
    {
      let data = table_oneb639c.find((data:any)=>(data?.my_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "my_id",
        value: data["my_id"],
  ...
  useEffect(() => {
    if (table_oneb639cProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (table_oneb639cProps?.selectedIds?.length == 0){
      handleOnRowClick({},table_oneb639cProps?.selectedIds)
    }
  }, [table_oneb639cProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table_oneb639c?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table_oneb639c?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table_oneb639c?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table_oneb639c?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table_oneb639c?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table_oneb639c?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table_oneb639c?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"8b24119968b045fcabecd19f3fdb639c"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              disable={disable}
              selectedIds={table_oneb639cProps?.selectedIds}  
