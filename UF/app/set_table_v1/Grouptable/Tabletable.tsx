...
let defaultColumns:any = [
  {
    "id": "country",
    "nodeid": "ad736850dc794dc8b93f3b9e6e572935",
    "name": "country",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "state",
    "nodeid": "c2ced97e63864c7abdf85d95e0dbf0ec",
    "name": "state",
    "meta": {
      "sort": true
    },
...
const Tabletable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
  ...
  const {set_memory_table_v1Props, setset_memory_table_v1Props} = useContext(TotalContext) as TotalContextProps;
 ...
 /////////////
  ...
  const latestLockStateRef = useRef({ needLockingAndRule, lockedData, allData, table2f83e })
  useEffect(() => {
    latestLockStateRef.current = { needLockingAndRule, lockedData, allData, table2f83e }
  })
  useEffect(() => {
    ...
    const { needLockingAndRule, lockedData, allData, table2f83e } = latestLockStateRef.current
    /// settable2f83eProps
    let postIds: any = []
    let processIds: any = []
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        settable2f83eProps((pre:any)=>({...pre, selectedIds:[]}))
        ...
        allData?.map((data:any,i:any)=>{
          if(data?.my_id==ids[ids.length - 1])
          {
            index=i
            row=data
          }
      ...
      table2f83e.filter((item:any,id:number)=>{
        if (ids.at(-1)==item.my_id){
      ...
      settable2f83eProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable2f83eProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(set_memory_table_v1Props.length > 0){
      for(let i=0;i< set_memory_table_v1Props.length;i++){
        if(set_memory_table_v1Props[i].DFDkey == "CK:CT001:FNGK:AF:FNK:DF-DFD:CATK:TAM:AFGK:TA:AFK:set_where:AFVK:v1"){
          // delete set_memory_table_v1Props[i].DFDkey;
          let temp=structuredClone(set_memory_table_v1Props[i])
          delete temp?.DFDkey
    ...
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
            searchParams={...searchParams,
                "country":memoryVariables?.country,  
            }
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT001:FNGK:AF:FNK:UF-UFW:CATK:TAM:AFGK:TA:AFK:set_memory_table:AFVK:v1:UO', 
                nodeId: '29eefd9927fc4ddb8b5fa1b207b2f83e',
                elementId: '29eefd9927fc4ddb8b5fa1b207b2f83e'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( table2f83eProps?.presetValues&&Object.keys(table2f83eProps?.presetValues).length > 0) {
          filtertedData = [table2f83eProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          settable2f83e(api_pagination?.data?.records||[])
        }
        ...
        for (let i = 0; i < filtertedData.length; i++) {     
          ...
          Object.keys(JSONType).map((key: any) => {
              JSONType={...JSONType,my_id:filtertedData[i]?.my_id}
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    ...
    return <RowActionComponent
      ...
      artifactRuleState={set_memory_table_v1}
      groupData = {groupData}
      controlData = {controlData}
      onSelectLock={setLockMode}
      currentSelectedIds={table2f83eProps?.selectedIds}
      skipUnlockRef={skipUnlockRef}
      tableName={tableName}
    />
  }, [allData, setRefetch, encryptionFlagCompData, table2f83eProps?.selectedIds]);
////////////////////////
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (table2f83eProps.filterInitalLoad) return;

    const filterControllers = table2f83eProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['table2f83e']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['table2f83e']).filter((k: string) => jsonEntry['table2f83e'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(table2f83eProps.filterProps);
      return;
    }
    ...
    settable2f83eProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(table2f83eProps.filterProps);

  }, [
    table2f83eProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    table2f83eProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(table2f83eProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(table2f83eProps.searchFilter) || {};
  ...
  useEffect(() => {
    if (table2f83eProps?.clearData === true) {
      settable2f83e([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      settable2f83eProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [table2f83eProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
  ...
  const handlePrimaryTable = () => {
    let findData = table2f83eProps?.selectedIds[table2f83eProps?.selectedIds?.length-1]
    if(Array.isArray(table2f83e) && table2f83e.length>0)
    {
      let data = table2f83e.find((data:any)=>(data?.my_id==findData))||{}
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "my_id",
        value: data["my_id"],
  ...
  useEffect(() => {
    if (table2f83eProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (table2f83eProps?.selectedIds?.length == 0){
      handleOnRowClick({},table2f83eProps?.selectedIds)
    }
  }, [table2f83eProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(table2f83e?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&table2f83e?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](table2f83e?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table2f83e?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&table2f83e?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](table2f83e?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: table2f83e?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"29eefd9927fc4ddb8b5fa1b207b2f83e"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableActions={true}
              disable={disable}
              selectedIds={table2f83eProps?.selectedIds}  
