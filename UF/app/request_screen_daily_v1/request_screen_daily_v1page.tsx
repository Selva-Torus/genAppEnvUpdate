...
import Grouprequest_daily_group  from "./Grouprequest_daily_group/Grouprequest_daily_group";


export default function PageRequestScreenDailyV1() {
  ...
  const {request_screen_daily_v1Props, setrequest_screen_daily_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [checkrequest_daily_group,setCheckrequest_daily_group,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function claims_v1(pagination:any): Promise<void>{
        ...
        if(request_screen_daily_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< request_screen_daily_v1Props.length;i++){
            if(request_screen_daily_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:claims:AFVK:v1"){
              delete request_screen_daily_v1Props[i].DFDkey;
              filterData.push(request_screen_daily_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function code_description_v1(pagination:any): Promise<void>{
        ...
        if(request_screen_daily_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< request_screen_daily_v1Props.length;i++){
            if(request_screen_daily_v1Props[i].DFDkey == "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:code_description:AFVK:v1"){
              delete request_screen_daily_v1Props[i].DFDkey;
              filterData.push(request_screen_daily_v1Props[i])
            }           
          }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:Request_Screen_Daily:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  return (
    ...
    {securityData[accessProfile]?.allowedGroups?.includes("request_daily_group") && initialLoad &&<Grouprequest_daily_group 
