...
const Dropdownclaim_category = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
    getDropdownData()
  },[claim_categoryc7c5e?.refresh])

  const handlechange = async(value: any) => {
    if(value.length>0){
      setrequest_daily_group44e40((prev: any) => ({ ...prev, claim_category: getMapperDetailsBindValues[value],cdcategory: value }))
        setIsRequredData(false)
    }else{
      let temp:any = request_daily_group44e40
      delete temp.claim_category
      delete temp.cdcategory
      setrequest_daily_group44e40(temp)
      getDropdownData()
  ...
  useEffect(() => {
    ...
    // setrequest_daily_group44e40((pre:any)=>({...pre,claim_category:dfd_code_description_v1Props[0]?.claim_category}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setrequest_daily_group44e40((prev: any) => ({ ...prev, claim_category: getMapperDetailsBindValues[value]}))
      setIsRequredData(false) 
    }else {
      setrequest_daily_group44e40((prev: any) => ({ ...prev, claim_category: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,claim_category:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
      setrequest_daily_group44e40((pre:any)=>({...pre,claim_category:""}))
    else
      setInitialCount(1)
  },[claim_categoryc7c5e?.refresh])

  if (claim_categoryc7c5e?.isHidden) {
    return <></>
  }
  return (
    <div 
      ...
      <Dropdown   
        className=""    
        disabled= {claim_categoryc7c5e?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        placeholder={keyset("Select Category")} 
        filterable={true} 
        hasClear={true} 
        onChange={handlechange} 
        value={request_daily_group44e40?.cdcategory ? [request_daily_group44e40?.cdcategory] : []}
      />
