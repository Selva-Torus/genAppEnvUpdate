...
const Textfull_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_leavemanagedoctable_v1Props) && dfd_leavemanagedoctable_v1Props && !table_groupf34e5.full_name){
        settable_groupf34e5((pre:any)=>({...pre,full_name:dfd_leavemanagedoctable_v1Props[0]?.full_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[full_name74b7d?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_leavemanagedoctable_v1Props) && !table_groupf34e5.full_name){
    settable_groupf34e5((pre:any)=>({...pre,full_name:dfd_leavemanagedoctable_v1Props[0]?.full_name}));
  }
  },[dfd_leavemanagedoctable_v1Props])

  if (full_name74b7d?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.full_name : (table_groupf34e5?.full_name || ""))}
</Text>
  </div>
  )
}
