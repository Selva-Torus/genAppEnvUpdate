...
let defaultColumns:any = [
  {
    "id": "attachment_id",
    "nodeid": "e69ec06f58c51cb3f38b983e9c74aa49",
    "name": "Attachment ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_group",
    "nodeid": "852017d41d26c07ffe3b8a75f6a8a432",
    "name": "Document Group",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "doc_name",
    "nodeid": "09a6f6f18cc1daae0238c6a759195718",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_date",
    "nodeid": "6a8e8b78634a029fe699f76acf919c75",
    "name": "Created Date",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "trs_created_by",
    "nodeid": "395a9cf4a8cbcbb4cfac7bd483aeb1ff",
    "name": "Created By",
    "meta": {
      "sort": true
    },
...
const Tablerequest_doc_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const lockRecord = async (id: number) => {
    try {
      const res = await AxiosService.post(
        '/UF/lock',
        { tableName: 'hrm_employee_access_requests', key:"access_req_id",value:id},
        {
          headers: {
  ...
  const unlockRecord = async (id: number) => {
    ...
    /// setrequest_doc_tabled1189Props
    let postIds: any = []
    let processIds: any = []
    let selectedData:any=[];
    const currentTableIdSet = new Set(request_doc_tabled1189.map((item: any) => item.access_req_id));
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds:[]}))
      ...
      try {
        ...
        if (previousLockedId === row.access_req_id) {
          return
        }

        await lockRecord(row.access_req_id)

        if (previousLockedId && previousLockedId !== row.access_req_id) {
          await unlockRecord(previousLockedId)
        }

      request_doc_tabled1189.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.access_req_id)
      ...
      //////////
      //////////
        setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
      } catch (err: any) {
        toast(
          err?.message ||
        ...
        if (previousLockedId) {
          ...
          if (previousIndex !== -1) {
            setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds: [previousIndex.toString()]}))
          }
        } else {
          setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds: []}))
        }
      ...
      setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
      } catch (err: any) {
        ...
        const previousSelectedIds = request_doc_tabled1189
          .map((item: any, index: number) =>
            previousLockedIds.includes(item.access_req_id) ? index.toString() : null
          )
          .filter(Boolean)

        setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds: previousSelectedIds}))
        return
      }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrequest_doc_tabled1189Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(leavemanagementdocument_v1Props.length > 0){
      for(let i=0;i< leavemanagementdocument_v1Props.length;i++){
        if(leavemanagementdocument_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:HRM:AFK:leaveManageDocTable:AFVK:v1"){
          // delete leavemanagementdocument_v1Props[i].DFDkey;
          let temp=structuredClone(leavemanagementdocument_v1Props[i])
          delete temp?.DFDkey
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["employee_id"]:request_doc_tabled1189Props?.presetValues?.employee_id}
    ...
    if(!request_doc_tabled1189Props?.presetValues || Object.keys(request_doc_tabled1189Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leaveManagementDocument:AFVK:v1:UO', 
                nodeId: '537d96f852345700a710b349837d1189',
                elementId: '537d96f852345700a710b349837d1189'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:leaveManagementDocument:AFVK:v1',
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setrequest_doc_tabled1189(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in leavemanagementdocument_v1?.request_doc_table && leavemanagementdocument_v1?.request_doc_table[controller]?.itsHaveArtifact== true)
      {
        if(leavemanagementdocument_v1?.request_doc_table[controller])
        {
          let result :any = await getAftfactLevelRule(leavemanagementdocument_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,request_doc_table:filteredData},{request_doc_table:leavemanagementdocument_v1?.request_doc_table})
          if(result?.request_doc_table?.[controller]?.show==true)
            return true
          else
            return false
        }
    ...
    if(await handleSecurity("bt_delete") &&nodeName?.toLowerCase()=="buttonbt_delete"){
      let tempLockedData:any = 
      {
        ...
        data:filteredData,
        primaryKeys:[filteredData?.access_req_id],
        lockMode: needLockingAndRule,
        ttl: needLockingAndRule?.ttl
      }
      return (
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(request_doc_tabled1189))
    {
      GetTableDetails()
    }
  }, [request_doc_tabled1189])
  ...
  useEffect(() => {
    ...
    if (request_doc_tabled1189Props.filterInitalLoad) return;

    const filterControllers = request_doc_tabled1189Props.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['request_doc_tabled1189']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['request_doc_tabled1189']).filter((k: string) => jsonEntry['request_doc_tabled1189'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(request_doc_tabled1189Props.filterProps);
      return;
    }
    ...
    setrequest_doc_tabled1189Props((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(request_doc_tabled1189Props.filterProps);

  }, [
    request_doc_tabled1189Props.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    request_doc_tabled1189Props.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(request_doc_tabled1189Props.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(request_doc_tabled1189Props.searchFilter) || {};
...
}, [request_doc_tabled1189Props.searchFilter])
  ...
  useEffect(() => {
    if (request_doc_tabled1189Props?.clearData === true) {
      if (lastLockedDataRef.current?.primaryKeys?.length) {
        lastLockedDataRef.current.primaryKeys.forEach((id: number) => unlockRecord(id));
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setrequest_doc_tabled1189Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [request_doc_tabled1189Props?.clearData])
  useEffect(() => {
  ...
  }, [request_doc_tabled1189Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = request_doc_tabled1189Props?.selectedIds[request_doc_tabled1189Props?.selectedIds?.length-1]
    if(Array.isArray(request_doc_tabled1189) && request_doc_tabled1189.length>0)
    {
      let data = request_doc_tabled1189[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "access_req_id",
        value: data["access_req_id"],
        parentData: data
      })
    }
  }
  useEffect(() => {
    if (request_doc_tabled1189Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (request_doc_tabled1189Props?.selectedIds?.length == 0){
      handleOnRowClick({},request_doc_tabled1189Props?.selectedIds)
    }
  }, [request_doc_tabled1189Props?.selectedIds])
  ...
  const handleOnRowClick=async(data?:any,ids?:any)=>{
    ...
    let copyFormhandlerData :any = {}
          //bindtran
    const pkCol = translatedColumns.find((col:any) => col.dfdName === "access_req_id"); // Find the column whose dfdName matches
    const result = allData.find((item:any) => item["access_req_id"] === data[pkCol?.id || "access_req_id"]);
    ...
    let filterProps4:any =  [
  {
    ...
    "nodeBasedData": [
      ...
      let filterData4 = await getFilterProps(filterProps4,{...doc_attached_groupe2bd6,...table_groupf34e5,...request_doc_tabled1189});
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(request_doc_tabled1189?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&request_doc_tabled1189?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](request_doc_tabled1189?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: request_doc_tabled1189?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&request_doc_tabled1189?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](request_doc_tabled1189?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: request_doc_tabled1189?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (request_doc_tabled1189?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
      <Modal 
        ...
        title="View Leave Supporting Documents"
        variant="header-1"
        showOverlay = {true}
        position = {"center"}
      ...
      >
        <PageHrmdocumentviewerpage4/>
          ...
          <div
            ...
            id={"537d96f852345700a710b349837d1189"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={request_doc_tabled1189Props?.selectedIds}  
...
export default Tablerequest_doc_table
