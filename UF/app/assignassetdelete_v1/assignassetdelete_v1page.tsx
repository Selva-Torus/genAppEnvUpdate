...
export default function PageAssignassetdeleteV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function assetassignments_v1DFD(pagination:any): Promise<void>{
        ...
        if(assignassetdelete_v1Props.length > 0){
          for(let i=0;i< assignassetdelete_v1Props.length;i++){
            if(assignassetdelete_v1Props[i].DFDkey == "CK:CT006:FNGK:AF:FNK:DF-DFD:CATK:ECP:AFGK:AMS:AFK:assetAssignments:AFVK:v1"){
              // delete assignassetdelete_v1Props[i].DFDkey;
              let temp=structuredClone(assignassetdelete_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setassignassetdelete_v1({...result,_artfactPFRule_:rule})
  }
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetDelete:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setassignassetdelete_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(assignassetdelete_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
        session:decodedTokenObj,
      }
      handleArtfactRule(assignassetdelete_v1?._artfactPFRule_,data,allRuleData)
    }
  },[])
  return (
    <>
