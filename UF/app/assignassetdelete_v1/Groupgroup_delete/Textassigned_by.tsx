...
const Textassigned_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetassignments_v1Props && !dfd_assetassignments_v1Props.hasLogicCenter) {
        ...
        setgroup_delete0df4b((pre: any) => {
          return { ...pre, assigned_by: api_paginationData.data.records[0]?.assigned_by }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetassignments_v1Props) && dfd_assetassignments_v1Props && !group_delete0df4b.assigned_by){
        setgroup_delete0df4b((pre:any)=>({...pre,assigned_by:dfd_assetassignments_v1Props[0]?.assigned_by}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[assigned_bycb5ab?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetassignments_v1Props) && !group_delete0df4b.assigned_by){
    setgroup_delete0df4b((pre:any)=>({...pre,assigned_by:dfd_assetassignments_v1Props[0]?.assigned_by}));
  }
  },[dfd_assetassignments_v1Props])

  if (assigned_bycb5ab?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.assigned_by : (group_delete0df4b?.assigned_by || ""))}
</Text>
  </div>
  )
}

export default Textassigned_by
