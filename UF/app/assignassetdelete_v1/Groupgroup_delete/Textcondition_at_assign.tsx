...
const Textcondition_at_assign = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetassignments_v1Props && !dfd_assetassignments_v1Props.hasLogicCenter) {
        ...
        setgroup_delete0df4b((pre: any) => {
          return { ...pre, condition_at_assign: api_paginationData.data.records[0]?.condition_at_assign }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetassignments_v1Props) && dfd_assetassignments_v1Props && !group_delete0df4b.condition_at_assign){
        setgroup_delete0df4b((pre:any)=>({...pre,condition_at_assign:dfd_assetassignments_v1Props[0]?.condition_at_assign}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[condition_at_assignc35c4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetassignments_v1Props) && !group_delete0df4b.condition_at_assign){
    setgroup_delete0df4b((pre:any)=>({...pre,condition_at_assign:dfd_assetassignments_v1Props[0]?.condition_at_assign}));
  }
  },[dfd_assetassignments_v1Props])

  if (condition_at_assignc35c4?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.condition_at_assign : (group_delete0df4b?.condition_at_assign || ""))}
</Text>
  </div>
  )
}

export default Textcondition_at_assign
