...
const Textexpected_return_date = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_assetassignments_v1Props && !dfd_assetassignments_v1Props.hasLogicCenter) {
        ...
        setgroup_delete0df4b((pre: any) => {
          return { ...pre, expected_return_date: api_paginationData.data.records[0]?.expected_return_date }
        })
        }
      }
      else{
      if(Array.isArray(dfd_assetassignments_v1Props) && dfd_assetassignments_v1Props && !group_delete0df4b.expected_return_date){
        setgroup_delete0df4b((pre:any)=>({...pre,expected_return_date:dfd_assetassignments_v1Props[0]?.expected_return_date}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[expected_return_date11169?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetassignments_v1Props) && !group_delete0df4b.expected_return_date){
    setgroup_delete0df4b((pre:any)=>({...pre,expected_return_date:dfd_assetassignments_v1Props[0]?.expected_return_date}));
  }
  },[dfd_assetassignments_v1Props])

  if (expected_return_date11169?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 24`,gridRow: `31 / 36`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.expected_return_date : (group_delete0df4b?.expected_return_date || ""))}
</Text>
  </div>
  )
}

export default Textexpected_return_date
