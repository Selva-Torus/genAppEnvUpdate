...
const Textasset_name = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_assetassignments_v1Props) && dfd_assetassignments_v1Props && !group_delete0df4b.asset_name){
        setgroup_delete0df4b((pre:any)=>({...pre,asset_name:dfd_assetassignments_v1Props[0]?.asset_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[asset_named51ee?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_assetassignments_v1Props) && !group_delete0df4b.asset_name){
    setgroup_delete0df4b((pre:any)=>({...pre,asset_name:dfd_assetassignments_v1Props[0]?.asset_name}));
  }
  },[dfd_assetassignments_v1Props])

  if (asset_named51ee?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.asset_name : (group_delete0df4b?.asset_name || ""))}
</Text>
  </div>
  )
}
