 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "370ea4b01c495c84b69e089a018fa294"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "ok_buttonfa294") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "370ea4b01c495c84b69e089a018fa294") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "ok_buttonfa294");
    return () => {
  ...
  useEffect(()=>{
    if (!ok_buttonfa294?.trigger) return;
      if(ok_buttonfa294?.trigger){
      setok_buttonfa294((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[ok_buttonfa294?.trigger])

  useEffect(()=>{
    if(ok_buttonfa294?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[ok_buttonfa294?.refresh])
  ...
  async function handleSavea294_1_1_1(){

    setValidateRefetch((pre: any) => ({ ...pre, value: !pre.value, init: pre.init + 1 }));
    // await delay(1000); // RTGS New Payment: isProcessing turns false before API completion.

    const group_delete0df4bProps = group_delete0df4bPropsRef.current;
  ...
  const hasInvalidField = Object.values(currentValidate?.assignAssetDelete_v1 || {}).some(val => {
  ...
  return val === 'invalid';
    ...
    try{
      let mainData:any=structuredClone(group_delete0df4bRef.current);
      ...
      let lockedKeysLength:number;
        if(group_delete0df4b?.upId === "" && (!lockedData?.data || Object.keys(lockedData?.data)?.length == 0)){
          throw 'Please give proper data';
        }
      let eventProperty :any = {
  "id": "370ea4b01c495c84b69e089a018fa294",
  "type": "button",
  "name": "ok_button",
  "label": "ok_button",
  "sequence": 1,
  "children": [
    {
      "id": "370ea4b01c495c84b69e089a018fa294.1.1",
      "type": "eventNode",
      "name": "onClick",
      "label": "onClick",
      "sequence": "1.1",
      "children": [
        {
          "id": "370ea4b01c495c84b69e089a018fa294.1.1.1",
          "eventContext": "rise",
          "value": "",
          "type": "handlerNode",
          "name": "eventEmitter",
          ...
          "children": [
            {
              "id": "370ea4b01c495c84b69e089a018fa294.1.1.1.1",
              "eventContext": "rise",
              "value": "",
              "type": "handlerNode",
              "name": "infoMsg",
            ...
            {
              "id": "370ea4b01c495c84b69e089a018fa294.1.1.1.2",
      ...
      let sourceId : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetDelete:AFVK:v1";
      sourceId+= "|"+"9e8addb372fc9347531f4bc53780df4b";
      let pathIds = SourceIdFilter(eventProperty,"1.1.1");
      let sourceIdNewPath : string = "CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:AMS:AFK:assignAssetDelete:AFVK:v1"+"|"+"9e8addb372fc9347531f4bc53780df4b"+"|"+eventProperty.id;
      ...
      te_eventEmitterBody = {
        ...
        upId : group_delete0df4b?.upId? [group_delete0df4b?.upId ] : lockedData.processIds,
      ...
      if(Array.isArray(tableData) && hasValidupId){
        ...
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete0df4b?.upId? [group_delete0df4b?.upId ] : upId,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId
      }}else{
        te_eventEmitterBody = {
        ...uf_initiatePf.data.nodeProperty,
        data:{"trs_event_process_status":uf_getPFDetails.status},
        upId : group_delete0df4b?.upId? [group_delete0df4b?.upId ] : group_delete0df4b?.trs_process_id? [group_delete0df4b?.trs_process_id ] : lockedData.processIds,
        event : uf_initiatePf.data.eventProperty?.source?.status,
        sourceId : uf_initiatePf.data.eventProperty?.sourceId,
        controlName: "ok_button"
      }
  ...
  // saveHandler
    ...
    let reworkedObject:any = nullFilter({ ...group_delete0df4bRef.current });
    let reworkKeys:any[]=[];
        ...
        if(Array.isArray(reworkedObject))
        {
          ...
          for( const dataList of formData )
          {
            ...
            const uf_ifoBody:uf_ifoDto={
              formData:dataList,
              key:uf_getPFDetails.key,
              groupId:"9e8addb372fc9347531f4bc53780df4b",
              controlId:"370ea4b01c495c84b69e089a018fa294"
            };
            if (encryptionFlagCont) {
    ...
    //eventEmitter
    if (group_delete0df4bProps.ssKey !== '' && group_delete0df4bProps.ssKey !== undefined) {
    te_eventEmitterBody["ssKey"] = group_delete0df4bProps.ssKey;          
    }
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        setgroup_delete0df4b((prev: any) => ({ ...prev, ok_button: true }));
        //onClick

    //eventEmitter
    await handleSavea294_1_1_1();
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        setgroup_delete0df4b((prev: any) => ({ ...prev, ok_button: false }));
      if(typeof err == 'string')
        toast(err, 'danger');
      else
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {ok_buttonfa294?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
