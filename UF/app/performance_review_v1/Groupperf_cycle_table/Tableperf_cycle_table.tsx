...
let defaultColumns:any = [
  {
    "id": "review_id",
    "nodeid": "c08c835f85cb53ba755cf71914214901",
    "name": "Review ID",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "review_number",
    "nodeid": "b54c6c114307a379635ae50ce3ce1dc8",
    "name": "Review Number",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "full_name",
    "nodeid": "dc87f6f4311a4e198090768faf1be804",
    "name": "Employee",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cycle_name",
    "nodeid": "0fff2356676e41eb823de82dedd718ff",
    "name": "Cycle Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "review_type",
    "nodeid": "a6b35ce12daa66ae54719c734411bdf9",
    "name": "Review Type",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "review_status",
    "nodeid": "77d06eb1f538a17ec6576c29e592b312",
    "name": "Review Status",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "final_rating",
    "nodeid": "ba3b0289689464acc2afea305e481ca1",
    "name": "Final Rating",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj,artifactRuleState,groupData,controlData}: any) => {
  ...
  useEffect(() => {
    async function loadPopupData() {
      ...
      let attachments:any = await handleSecurity("attachments") || false
      const content = (
        ...
        {
        attachments&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonattachments'))&&(<Buttonattachments mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />)}
        </div>
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      ttl: needLockingAndRule.ttl
        ...
        let keys:any
        setperf_cycle_table1d11fProps((pre:any)=>({...pre, selectedIds:[]}))
      ...
      //////////
      //////////
        setperf_cycle_table1d11fProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))
    }
      ...
      setperf_cycle_table1d11fProps((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  }
    }
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setperf_cycle_table1d11fProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT006:FNGK:AF:FNK:UF-UFW:CATK:ECP:AFGK:HRM:AFK:performanceReview:AFVK:v1:UO', 
                nodeId: '7ad86c050521a42a5cd27edbda51d11f',
                elementId: '7ad86c050521a42a5cd27edbda51d11f'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( perf_cycle_table1d11fProps?.presetValues&&Object.keys(perf_cycle_table1d11fProps?.presetValues).length > 0) {
          filtertedData = [perf_cycle_table1d11fProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setperf_cycle_table1d11f(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
 ...
 const RowAction = React.useCallback(({item,index}: any) => {
    return <RowActionComponent
      ...
      artifactRuleState={performancereview_v1}
      groupData = {groupData}
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    ...
    if (perf_cycle_table1d11fProps.filterInitalLoad) return;

    const filterControllers = perf_cycle_table1d11fProps.filterControllers ?? {};
    const jsonEntry = (filterData as any[]).find((e: any) => e['perf_cycle_table1d11f']);
    const requiredKeys: string[] = jsonEntry
      ? Object.keys(jsonEntry['perf_cycle_table1d11f']).filter((k: string) => jsonEntry['perf_cycle_table1d11f'][k] === false)
      : [];

    if (requiredKeys.length === 0) {
      UpdatedDataHandle(perf_cycle_table1d11fProps.filterProps);
      return;
    }
    ...
    setperf_cycle_table1d11fProps((prev: any) => ({ ...prev, filterInitalLoad: true }));
    UpdatedDataHandle(perf_cycle_table1d11fProps.filterProps);

  }, [
    perf_cycle_table1d11fProps.filterControllers,
    DFkeyAndRule
  ])
  ...
  useEffect(() => {
    ...
    perf_cycle_table1d11fProps.filterProps
  ])

  useEffect(() => {
  console.log("search filter changed")
  const payload = JSON.stringify(perf_cycle_table1d11fProps.searchFilter ?? {});
  ...
  prevSearchFilterRef.current = payload;
  const searchParams = nullFilter(perf_cycle_table1d11fProps.searchFilter) || {};
...
}, [perf_cycle_table1d11fProps.searchFilter])
  ...
  useEffect(() => {
    if (perf_cycle_table1d11fProps?.clearData === true) {
      setperf_cycle_table1d11f([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setLockedData((pre:any) => ({...pre, data:[]}));
      setperf_cycle_table1d11fProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [perf_cycle_table1d11fProps?.clearData])
  useEffect(() => {
    if (!refreshInitRef.current) {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [perf_cycle_table1d11fProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = perf_cycle_table1d11fProps?.selectedIds[perf_cycle_table1d11fProps?.selectedIds?.length-1]
    if(Array.isArray(perf_cycle_table1d11f) && perf_cycle_table1d11f.length>0)
    {
      let data = perf_cycle_table1d11f[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (perf_cycle_table1d11fProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (perf_cycle_table1d11fProps?.selectedIds?.length == 0){
      handleOnRowClick({},perf_cycle_table1d11fProps?.selectedIds)
    }
  }, [perf_cycle_table1d11fProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(perf_cycle_table1d11f?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&perf_cycle_table1d11f?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](perf_cycle_table1d11f?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: perf_cycle_table1d11f?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&perf_cycle_table1d11f?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](perf_cycle_table1d11f?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: perf_cycle_table1d11f?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (perf_cycle_table1d11f?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"7ad86c050521a42a5cd27edbda51d11f"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              tableActions={true}
              tableSorting={true}
              selectedIds={perf_cycle_table1d11fProps?.selectedIds}  
