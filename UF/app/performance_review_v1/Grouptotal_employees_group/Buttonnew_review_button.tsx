...
const Buttonnew_review_button = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      ...
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "924046c87b086da4f7f0c75179d070c7"
      );
  ...
  useEffect(()=>{
    ...
    const handler = async (id:any) => {
      if (id === "new_review_button070c7") {
        handleClick(false);
      }
    };
    const triggerElementHandler = async (id:any) => {
      if (id === "924046c87b086da4f7f0c75179d070c7") {
        handleClick(false);
      }
    };
    eventBus.on("triggerButton", handler);
    eventBus.on("triggerElement|onClick", triggerElementHandler);
    eventBus.emit("buttonReady", "new_review_button070c7");
    return () => {
  ...
  useEffect(()=>{
    if (!new_review_button070c7?.trigger) return;
      if(new_review_button070c7?.trigger){
      setnew_review_button070c7((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[new_review_button070c7?.trigger])

  useEffect(()=>{
    setShowProfileAsModalOpen2(false)
    if(new_review_button070c7?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[new_review_button070c7?.refresh])
  ...
  const handleClick=async(showModal: boolean = true)=>{
    ...
    try{  
      setIsProcessing(true);
        settotal_employees_group5fd1a((prev: any) => ({ ...prev, new_review_button: true }));
        //onClick
    ...
    let filterProps2:any =  [
  {
    ...
    //enableElement
    setsave02eeb((prev: any) => ({ ...prev, isDisabled: false }));
    //disableElement
    setbutton_update1b289((prev: any) => ({ ...prev, isDisabled: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
        settotal_employees_group5fd1a((prev: any) => ({ ...prev, new_review_button: false }));
 ...
 if (new_review_button070c7?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      >
        {!hiddenModalForTrigger && <PageNewperformancereviewpage2/>}
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='normal'
          disabled= {new_review_button070c7?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdAdd"
...
export default Buttonnew_review_button
