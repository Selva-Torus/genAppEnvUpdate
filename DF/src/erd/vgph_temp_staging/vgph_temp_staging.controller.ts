...
export class vgph_temp_stagingController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_temp_stagingService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphtsid') vgphtsid:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtsid"] = req.headers.vgphtsid || "";
      detokenizeData["prcts_uuid"] = req.headers.prcts_uuid || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_stagingService.findOne(+vgphtsid,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_temp_stagingService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_temp_stagingEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_temp_staging'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_temp_stagingDto: Createvgph_temp_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtsid"] = req.headers.vgphtsid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_temp_stagingService.createMaster(createvgph_temp_stagingDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_temp_stagingService.create(createvgph_temp_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_temp_staging', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphtsid',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphtsid') vgphtsid:number,
    @Body() updatevgph_temp_stagingDto: Updatevgph_temp_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtsid"] = req.headers.vgphtsid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_temp_stagingService.updateMaster(+vgphtsid,updatevgph_temp_stagingDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_temp_stagingService.update(+vgphtsid,updatevgph_temp_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }
...
@Param('vgphtsid') vgphtsid:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_temp_stagingService.deleteMaster(+vgphtsid,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_temp_stagingService.remove(+vgphtsid,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_stagingService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_stagingService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_temp_stagingService.getLockById(key, value, token);
    return plainToInstance(vgph_temp_stagingEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_temp_stagingService.releaseLockById(key, value, token);
    return result;
  }
}