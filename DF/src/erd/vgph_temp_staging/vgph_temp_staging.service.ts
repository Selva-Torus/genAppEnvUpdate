 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
  ...
  async create(createvgph_temp_stagingDto: Prisma.vgph_temp_stagingCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_temp_stagingDto, 'vgph_temp_staging', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphtsid:number, updatevgph_temp_stagingDto: Prisma.vgph_temp_stagingUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_temp_stagingDto,'vgph_temp_staging','update'), token);
