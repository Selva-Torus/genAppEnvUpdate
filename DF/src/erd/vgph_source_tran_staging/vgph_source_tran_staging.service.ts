...
export class vgph_source_tran_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphsts_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.vgph_source_tran_staging.findMany({ 
      where: {vgphsts_id,trs_tenant_id,trs_app_code,trs_product_code, ...tenantFilter },
  ...
  async create(createvgph_source_tran_stagingDto: Createvgph_source_tran_stagingDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_source_tran_stagingDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            tran_date :(v.union([v.pipe(v.string(), v.isoDate()), v.pipe(v.string(), v.isoTimestamp())])) , 
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_tran_stagingDto, 'vgph_source_tran_staging', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_source_tran_stagingDto: Createvgph_source_tran_stagingDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_source_tran_staging:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_source_tran_staging');
        }
  ...
  async update(uuid: string, updatevgph_source_tran_stagingDto: Updatevgph_source_tran_stagingDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_tran_staging.findFirst({
          where: {uuid, trs_tenant_id: authContext.tenant },
          select: {uuid:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_tran_stagingDto,'vgph_source_tran_staging','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_source_tran_stagingDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_source_tran_staging', {
          primaryKey: {uuid},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
uuid: string,
    updatevgph_source_tran_stagingDto: Updatevgph_source_tran_stagingDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_source_tran_staging.findFirst({
          where: {uuid },
  ...
  async remove(vgphsts_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphsts_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_source_tran_staging columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('vgph_source_tran_staging'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'vgph_source_tran_staging',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_source_tran_staging'));
      }
      return decryptedRes;
    } catch (error: any) {
