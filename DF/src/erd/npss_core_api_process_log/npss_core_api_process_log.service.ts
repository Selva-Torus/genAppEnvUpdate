...
import { Createnpss_core_api_process_logDto } from './dto/Createnpss_core_api_process_log.dto';
import { Updatenpss_core_api_process_logDto } from './dto/Updatenpss_core_api_process_log.dto';
...
import { npss_core_api_process_logEntity } from './entity/npss_core_api_process_log.entity';
...
@Injectable()
export class npss_core_api_process_logService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npsscapl_id }: {npsscapl_id : bigint} = queryValue;
      const { process_name }: {process_name : string} = queryValue;
      const { processing_system }: {processing_system : string} = queryValue;
      const { response_code }: {response_code : string} = queryValue;
      const { process_ref_no }: {process_ref_no : string} = queryValue;
      const { request_data_json }: {request_data_json : string} = queryValue;
      const { response_data_json }: {response_data_json : string} = queryValue;
      const { currency }: {currency : string} = queryValue;
      const { accountholderbic }: {accountholderbic : string} = queryValue;
      const { accountnumber }: {accountnumber : string} = queryValue;
      const { datasource }: {datasource : string} = queryValue;
      const { senderbic }: {senderbic : string} = queryValue;
      const { fromdate }: {fromdate :  Date} = queryValue;
      const { todate }: {todate :  Date} = queryValue;
      const { reftype }: {reftype : string} = queryValue;
      const { ref }: {ref : string} = queryValue;
      const { datetime }: {datetime :  Date} = queryValue;
      const { cyclenumber }: {cyclenumber : string} = queryValue;
      const { pl_status }: {pl_status : string} = queryValue;
      const { fromtime }: {fromtime : string} = queryValue;
      const { totime }: {totime : string} = queryValue;
      ...
      if(npsscapl_id){ 
        query.npsscapl_id = { [queryCondition['npsscapl_id']]: npsscapl_id };
      }
      if(process_name){ 
      ...
      if(response_code){ 
        query.response_code = { [queryCondition['response_code']]: response_code };
      }
      if(process_ref_no){ 
        query.process_ref_no = { [queryCondition['process_ref_no']]: process_ref_no };
      }
      if(request_data_json){ 
        query.request_data_json = { [queryCondition['request_data_json']]: request_data_json };
      }
      if(response_data_json){ 
        query.response_data_json = { [queryCondition['response_data_json']]: response_data_json };
      }
      if(currency){ 
        query.currency = { [queryCondition['currency']]: currency };
      }
      if(accountholderbic){ 
        query.accountholderbic = { [queryCondition['accountholderbic']]: accountholderbic };
      }
      if(accountnumber){ 
        query.accountnumber = { [queryCondition['accountnumber']]: accountnumber };
      }
      if(datasource){ 
        query.datasource = { [queryCondition['datasource']]: datasource };
      }
      if(senderbic){ 
        query.senderbic = { [queryCondition['senderbic']]: senderbic };
      }
      if(fromdate){ 
        query.fromdate = { [queryCondition['fromdate']]: fromdate };
      }
      if(todate){ 
        query.todate = { [queryCondition['todate']]: todate };
      }
      if(reftype){ 
        query.reftype = { [queryCondition['reftype']]: reftype };
      }
      if(ref){ 
        query.ref = { [queryCondition['ref']]: ref };
      }
      if(datetime){ 
        query.datetime = { [queryCondition['datetime']]: datetime };
      }
      if(cyclenumber){ 
        query.cyclenumber = { [queryCondition['cyclenumber']]: cyclenumber };
      }
      if(pl_status){ 
        query.pl_status = { [queryCondition['pl_status']]: pl_status };
      }
      if(fromtime){ 
        query.fromtime = { [queryCondition['fromtime']]: fromtime };
      }
      if(totime){ 
        query.totime = { [queryCondition['totime']]: totime };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_core_api_process_log.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_core_api_process_log');
      ...
      this.prismaService.npss_core_api_process_log.count({
        where: query,
      }));
  ...
  async findOne(npsscapl_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_core_api_process_log.findMany({ 
      where: {npsscapl_id, ...tenantFilter },
      select: {npsscapl_id:true,process_name:true,processing_system:true,response_code:true,process_ref_no:true,request_data_json:true,response_data_json:true,currency:true,accountholderbic:true,accountnumber:true,datasource:true,senderbic:true,fromdate:true,todate:true,reftype:true,ref:true,datetime:true,cyclenumber:true,pl_status:true,fromtime:true,totime:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_core_api_process_log');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_core_api_process_log');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_core_api_process_logDto: Createnpss_core_api_process_logDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_core_api_process_logDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            process_name :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            processing_system :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            response_code :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            process_ref_no :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            request_data_json :  v.optional(v.pipe(v.string(),v.maxLength(80000 ))), 
            response_data_json :  v.optional(v.pipe(v.string(),v.maxLength(80000 ))), 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            accountholderbic :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            accountnumber :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            datasource :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            senderbic :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            fromdate :  v.optional((v.any())), 
            todate :  v.optional((v.any())), 
            reftype :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            ref :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            datetime :  v.optional((v.any())), 
            cyclenumber :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            pl_status :  v.optional(v.pipe(v.string(),v.maxLength(256 ))), 
            fromtime :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            totime :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_core_api_process_logDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_core_api_process_logDto, 'npss_core_api_process_log', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_core_api_process_log.create({
          data: encryptedData,
          select:{npsscapl_id:true,process_name:true,processing_system:true,response_code:true,process_ref_no:true,request_data_json:true,response_data_json:true,currency:true,accountholderbic:true,accountnumber:true,datasource:true,senderbic:true,fromdate:true,todate:true,reftype:true,ref:true,datetime:true,cyclenumber:true,pl_status:true,fromtime:true,totime:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_core_api_process_logDto: Createnpss_core_api_process_logDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_core_api_process_log:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_core_api_process_log');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_core_api_process_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_core_api_process_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_core_api_process_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npsscapl_id',
        ...
        return {
          success: true,
          message: 'npss_core_api_process_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npsscapl_id:number, updatenpss_core_api_process_logDto: Updatenpss_core_api_process_logDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_core_api_process_log.findFirst({
          where: {npsscapl_id, trs_tenant_id: authContext.tenant },
          select: {npsscapl_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_core_api_process_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_core_api_process_logDto,'npss_core_api_process_log','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_core_api_process_logDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_core_api_process_log.updateMany({
        where: {npsscapl_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_core_api_process_log', {
          primaryKey: {npsscapl_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npsscapl_id:number,
    updatenpss_core_api_process_logDto: Updatenpss_core_api_process_logDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npsscapl_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_core_api_process_log.findFirst({
          where: {npsscapl_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_core_api_process_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_core_api_process_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_core_api_process_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npsscapl_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_core_api_process_log.deleteMany({
      where: {npsscapl_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_core_api_process_log');
    let decryptedRes: any = [];
...
npsscapl_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npsscapl_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_core_api_process_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_core_api_process_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_core_api_process_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_core_api_process_log columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('npss_core_api_process_log'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'npss_core_api_process_log',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_core_api_process_log'));
      }
      return decryptedRes;
    } catch (error: any) {
