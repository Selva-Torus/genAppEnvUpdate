...
//import { CreateNpssCoreApiProcessLogDto } from '../prisma/dto/create-npssCoreApiProcessLog.dto';
//import { UpdateNpssCoreApiProcessLogDto } from '../prisma/dto/update-npssCoreApiProcessLog.dto';
...
@Controller('npss_core_api_process_log')
@ApiTags('ERD API')
export class npss_core_api_process_logController {
  constructor(
    private readonly npss_core_api_process_logService: npss_core_api_process_logService,
  ...
  @ApiOkResponse({ type: npss_core_api_process_logEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_core_api_process_log table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_core_api_process_logService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_core_api_process_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_core_api_process_log table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_core_api_process_logService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npsscapl_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npsscapl_id',type:Number})
  @ApiOkResponse({ type: npss_core_api_process_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_core_api_process_log table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npsscapl_id') npsscapl_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapl_id"] = req.headers.npsscapl_id || "";
      ...
      detokenizeData["accountholderbic"] = req.headers.accountholderbic || "";
      detokenizeData["accountnumber"] = req.headers.accountnumber || "";
      detokenizeData["datasource"] = req.headers.datasource || "";
      detokenizeData["senderbic"] = req.headers.senderbic || "";
      detokenizeData["fromdate"] = req.headers.fromdate || "";
      detokenizeData["todate"] = req.headers.todate || "";
      detokenizeData["reftype"] = req.headers.reftype || "";
      detokenizeData["ref"] = req.headers.ref || "";
      detokenizeData["datetime"] = req.headers.datetime || "";
      detokenizeData["cyclenumber"] = req.headers.cyclenumber || "";
      detokenizeData["pl_status"] = req.headers.pl_status || "";
      detokenizeData["fromtime"] = req.headers.fromtime || "";
      detokenizeData["totime"] = req.headers.totime || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_core_api_process_logService.findOne(+npsscapl_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_core_api_process_log table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_core_api_process_logService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_core_api_process_logEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_core_api_process_log'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_core_api_process_logDto })
  @ApiCreatedResponse({ type: npss_core_api_process_logEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_core_api_process_log table',
  })
  ...
  async create(
    ...
    @Body() createnpss_core_api_process_logDto: Createnpss_core_api_process_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapl_id"] = req.headers.npsscapl_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_core_api_process_logService.createMaster(createnpss_core_api_process_logDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_core_api_process_logService.create(createnpss_core_api_process_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_core_api_process_log', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npsscapl_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_core_api_process_logDto })
  @ApiOkResponse({ type: npss_core_api_process_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_core_api_process_log table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npsscapl_id') npsscapl_id:number,
    @Body() updatenpss_core_api_process_logDto: Updatenpss_core_api_process_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapl_id"] = req.headers.npsscapl_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_core_api_process_logService.updateMaster(+npsscapl_id,updatenpss_core_api_process_logDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_core_api_process_logService.update(+npsscapl_id,updatenpss_core_api_process_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_core_api_process_log table',
  })
...
@Param('npsscapl_id') npsscapl_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_core_api_process_logService.deleteMaster(+npsscapl_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_core_api_process_logService.remove(+npsscapl_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_core_api_process_log table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_core_api_process_logService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_core_api_process_log table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_core_api_process_logService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_core_api_process_logService.getLockById(key, value, token);
    return plainToInstance(npss_core_api_process_logEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_core_api_process_logService.releaseLockById(key, value, token);
    return result;
  }
}