...
import { Createnpss_merchant_shopsDto } from './dto/Createnpss_merchant_shops.dto';
import { Updatenpss_merchant_shopsDto } from './dto/Updatenpss_merchant_shops.dto';
...
import { npss_merchant_shopsEntity } from './entity/npss_merchant_shops.entity';
...
@Injectable()
export class npss_merchant_shopsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npssms_id }: {npssms_id : bigint} = queryValue;
      const { npssmp_id }: {npssmp_id : number} = queryValue;
      const { product_code }: {product_code : string} = queryValue;
      const { bankuserid }: {bankuserid : string} = queryValue;
      const { shop_name }: {shop_name : string} = queryValue;
      const { shop_mid }: {shop_mid : string} = queryValue;
      const { shop_id }: {shop_id : string} = queryValue;
      const { shop_type }: {shop_type : string} = queryValue;
      const { shop_mcc }: {shop_mcc : string} = queryValue;
      const { shop_iban }: {shop_iban : string} = queryValue;
      const { building }: {building : string} = queryValue;
      const { street }: {street : string} = queryValue;
      const { city }: {city : string} = queryValue;
      const { postal_code }: {postal_code : string} = queryValue;
      const { county }: {county : string} = queryValue;
      const { country }: {country : string} = queryValue;
      const { exhf_id }: {exhf_id : number} = queryValue;
      const { shop_account_id }: {shop_account_id : string} = queryValue;
      ...
      if(npssms_id){ 
        query.npssms_id = { [queryCondition['npssms_id']]: npssms_id };
      }
      if(npssmp_id){ 
      ...
      if(shop_name){ 
        query.shop_name = { [queryCondition['shop_name']]: shop_name };
      }
      if(shop_mid){ 
        query.shop_mid = { [queryCondition['shop_mid']]: shop_mid };
      }
      if(shop_id){ 
        query.shop_id = { [queryCondition['shop_id']]: shop_id };
      }
      if(shop_type){ 
        query.shop_type = { [queryCondition['shop_type']]: shop_type };
      }
      if(shop_mcc){ 
        query.shop_mcc = { [queryCondition['shop_mcc']]: shop_mcc };
      }
      if(shop_iban){ 
        query.shop_iban = { [queryCondition['shop_iban']]: shop_iban };
      }
      if(building){ 
        query.building = { [queryCondition['building']]: building };
      }
      if(street){ 
        query.street = { [queryCondition['street']]: street };
      }
      if(city){ 
      ...
      if(county){ 
        query.county = { [queryCondition['county']]: county };
      }
      if(country){ 
      ...
      if(shop_account_id){ 
        query.shop_account_id = { [queryCondition['shop_account_id']]: shop_account_id };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_merchant_shops.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_merchant_shops');
      ...
      this.prismaService.npss_merchant_shops.count({
        where: query,
      }));
  ...
  async findOne(npssms_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_merchant_shops.findMany({ 
      where: {npssms_id, ...tenantFilter },
      select: {npssms_id:true,npssmp_id:true,product_code:true,bankuserid:true,shop_name:true,shop_mid:true,shop_id:true,shop_type:true,shop_mcc:true,shop_iban:true,building:true,street:true,city:true,postal_code:true,county:true,country:true,exhf_id:true,shop_account_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_shops');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_merchant_shops');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_merchant_shopsDto: Createnpss_merchant_shopsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_merchant_shopsDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            shop_name :v.pipe(v.string(),v.maxLength(100 )) , 
            shop_mid :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
            shop_id :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            shop_type :v.pipe(v.string(),v.maxLength(30 )) , 
            shop_mcc :  v.optional(v.pipe(v.string(),v.maxLength(4 ))), 
            shop_iban :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            building :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            street :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            city :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            postal_code :  v.optional(v.pipe(v.string(),v.maxLength(10 ))), 
            county :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            country :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            exhf_id :  v.optional(v.number()), 
            shop_account_id :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_merchant_shopsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_merchant_shopsDto, 'npss_merchant_shops', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_shops.create({
          data: encryptedData,
          select:{npssms_id:true,npssmp_id:true,product_code:true,bankuserid:true,shop_name:true,shop_mid:true,shop_id:true,shop_type:true,shop_mcc:true,shop_iban:true,building:true,street:true,city:true,postal_code:true,county:true,country:true,exhf_id:true,shop_account_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_merchant_shopsDto: Createnpss_merchant_shopsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_merchant_shops:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_merchant_shops');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_shops creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_shops creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_merchant_shops',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npssms_id',
        ...
        return {
          success: true,
          message: 'npss_merchant_shops creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npssms_id:number, updatenpss_merchant_shopsDto: Updatenpss_merchant_shopsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_shops.findFirst({
          where: {npssms_id, trs_tenant_id: authContext.tenant },
          select: {npssms_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          npssmp_id :  v.optional(v.number()), 
          product_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          bankuserid :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          shop_name :  v.optional(v.pipe(v.string(),v.maxLength(100 ))), 
          shop_mid :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
          shop_id :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          shop_type :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
          shop_mcc :  v.optional(v.pipe(v.string(),v.maxLength(4 ))), 
          shop_iban :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
          building :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          street :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_merchant_shopsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_merchant_shopsDto,'npss_merchant_shops','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_merchant_shopsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_shops.updateMany({
        where: {npssms_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_merchant_shops', {
          primaryKey: {npssms_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npssms_id:number,
    updatenpss_merchant_shopsDto: Updatenpss_merchant_shopsDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npssms_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_merchant_shops.findFirst({
          where: {npssms_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_shops update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_shops update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_shops update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npssms_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_merchant_shops.deleteMany({
      where: {npssms_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_shops');
    let decryptedRes: any = [];
...
npssms_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npssms_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_shops deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_shops deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_shops deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_merchant_shops columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_merchant_shops'));
      }
      return decryptedRes;
    } catch (error: any) {
