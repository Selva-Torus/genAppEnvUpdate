...
import { npss_merchant_shopsService } from './npss_merchant_shops.service';
...
//import { CreateNpssMerchantShopsDto } from '../prisma/dto/create-npssMerchantShops.dto';
//import { UpdateNpssMerchantShopsDto } from '../prisma/dto/update-npssMerchantShops.dto';
...
@Controller('npss_merchant_shops')
@ApiTags('ERD API')
export class npss_merchant_shopsController {
  constructor(
    private readonly npss_merchant_shopsService: npss_merchant_shopsService,
  ...
  @ApiOkResponse({ type: npss_merchant_shopsEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_merchant_shops table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_merchant_shopsService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_merchant_shopsEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_merchant_shops table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_merchant_shopsService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npssms_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npssms_id',type:Number})
  @ApiOkResponse({ type: npss_merchant_shopsEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_merchant_shops table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npssms_id') npssms_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssms_id"] = req.headers.npssms_id || "";
      ...
      detokenizeData["shop_name"] = req.headers.shop_name || "";
      detokenizeData["shop_mid"] = req.headers.shop_mid || "";
      detokenizeData["shop_id"] = req.headers.shop_id || "";
      detokenizeData["shop_type"] = req.headers.shop_type || "";
      detokenizeData["shop_mcc"] = req.headers.shop_mcc || "";
      detokenizeData["shop_iban"] = req.headers.shop_iban || "";
      detokenizeData["building"] = req.headers.building || "";
      detokenizeData["street"] = req.headers.street || "";
      detokenizeData["city"] = req.headers.city || "";
      detokenizeData["postal_code"] = req.headers.postal_code || "";
      detokenizeData["county"] = req.headers.county || "";
      detokenizeData["country"] = req.headers.country || "";
      detokenizeData["exhf_id"] = req.headers.exhf_id || "";
      detokenizeData["shop_account_id"] = req.headers.shop_account_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_shopsService.findOne(+npssms_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_merchant_shops table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_merchant_shopsService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_merchant_shopsEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_shops'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_merchant_shopsDto })
  @ApiCreatedResponse({ type: npss_merchant_shopsEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_merchant_shops table',
  })
  ...
  async create(
    ...
    @Body() createnpss_merchant_shopsDto: Createnpss_merchant_shopsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssms_id"] = req.headers.npssms_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_shopsService.createMaster(createnpss_merchant_shopsDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_merchant_shopsService.create(createnpss_merchant_shopsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_shops', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npssms_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_merchant_shopsDto })
  @ApiOkResponse({ type: npss_merchant_shopsEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_merchant_shops table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npssms_id') npssms_id:number,
    @Body() updatenpss_merchant_shopsDto: Updatenpss_merchant_shopsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssms_id"] = req.headers.npssms_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_shopsService.updateMaster(+npssms_id,updatenpss_merchant_shopsDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_shopsService.update(+npssms_id,updatenpss_merchant_shopsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_merchant_shops table',
  })
...
@Param('npssms_id') npssms_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_merchant_shopsService.deleteMaster(+npssms_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_shopsService.remove(+npssms_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_merchant_shops table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_shopsService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_merchant_shops table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_shopsService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_merchant_shopsService.getLockById(key, value, token);
    return plainToInstance(npss_merchant_shopsEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_merchant_shopsService.releaseLockById(key, value, token);
    return result;
  }
}