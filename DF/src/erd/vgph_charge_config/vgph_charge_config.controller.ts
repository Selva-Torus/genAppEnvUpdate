...
export class vgph_charge_configController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_charge_configService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphcc_id') vgphcc_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcc_id"] = req.headers.vgphcc_id || "";
      detokenizeData["channel_name"] = req.headers.channel_name || "";
      detokenizeData["cust_segment"] = req.headers.cust_segment || "";
      detokenizeData["customer_type"] = req.headers.customer_type || "";
      detokenizeData["cif_number"] = req.headers.cif_number || "";
      detokenizeData["customer_account_no"] = req.headers.customer_account_no || "";
      detokenizeData["message_code"] = req.headers.message_code || "";
      detokenizeData["charge_code"] = req.headers.charge_code || "";
      detokenizeData["charge_borne_by"] = req.headers.charge_borne_by || "";
      detokenizeData["charge_gl_ac_no"] = req.headers.charge_gl_ac_no || "";
      detokenizeData["vat_gl_ac_no"] = req.headers.vat_gl_ac_no || "";
      detokenizeData["start_active_date"] = req.headers.start_active_date || "";
      detokenizeData["end_active_date"] = req.headers.end_active_date || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_charge_configService.findOne(+vgphcc_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphcc_id') vgphcc_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcc_id"] = req.headers.vgphcc_id || "";
      detokenizeData["channel_name"] = req.headers.channel_name || "";
    ...
    const result = await this.vgph_charge_configService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphcc_id);
    return plainToInstance(vgph_charge_configEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_charge_config'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_charge_configDto: Createvgph_charge_configDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcc_id"] = req.headers.vgphcc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_charge_configService.createMaster(createvgph_charge_configDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_charge_configService.create(createvgph_charge_configDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_charge_configEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_charge_config', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphcc_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphcc_id') vgphcc_id:number,
    @Body() updatevgph_charge_configDto: Updatevgph_charge_configDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcc_id"] = req.headers.vgphcc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_charge_configService.updateMaster(+vgphcc_id,updatevgph_charge_configDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_charge_configService.update(+vgphcc_id,updatevgph_charge_configDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_charge_configEntity, result);
  }
...
@Param('vgphcc_id') vgphcc_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_charge_configService.deleteMaster(+vgphcc_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_charge_configService.remove(+vgphcc_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_charge_configEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_charge_configService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_charge_configEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_charge_configService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_charge_configEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_charge_configService.getLockById(key, value, token);
    return plainToInstance(vgph_charge_configEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_charge_configService.releaseLockById(key, value, token);
    return result;
  }
}