...
export class vgph_charge_configService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphcc_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
...
,vgphcc_id?:number) {
  ...
  async create(createvgph_charge_configDto: Createvgph_charge_configDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_charge_configDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            start_active_date :(v.union([v.pipe(v.string(), v.isoDate()), v.pipe(v.string(), v.isoTimestamp())])) , 
            end_active_date :(v.union([v.pipe(v.string(), v.isoDate()), v.pipe(v.string(), v.isoTimestamp())])) , 
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_charge_configDto, 'vgph_charge_config', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_charge_configDto: Createvgph_charge_configDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_charge_config:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_charge_config');
        }
  ...
  async update(vgphcc_id:number, updatevgph_charge_configDto: Updatevgph_charge_configDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_charge_config.findFirst({
          where: {vgphcc_id, trs_tenant_id: authContext.tenant },
          select: {vgphcc_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_charge_configDto,'vgph_charge_config','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_charge_configDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_charge_config', {
          primaryKey: {vgphcc_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphcc_id:number,
    updatevgph_charge_configDto: Updatevgph_charge_configDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_charge_config.findFirst({
          where: {vgphcc_id },
  ...
  async remove(vgphcc_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphcc_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_charge_config columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_charge_config'));
      }
      return decryptedRes;
    } catch (error: any) {
