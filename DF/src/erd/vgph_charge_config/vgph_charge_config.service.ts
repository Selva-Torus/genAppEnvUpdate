...
,vgphcc_id?:number) {
  ...
  async create(createvgph_charge_configDto: Prisma.vgph_charge_configCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_charge_configDto, 'vgph_charge_config', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphcc_id:number, updatevgph_charge_configDto: Prisma.vgph_charge_configUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_charge_configDto,'vgph_charge_config','update'), token);
