...
@Injectable()
export class hrm_performance_reviewsService {
  ...
  async commonDecimalDatahandle(data:any){
    const plainData = { ...data,
       self_rating:data?.self_rating != null ? Number(data?.self_rating?.toString()) : null,
       manager_rating:data?.manager_rating != null ? Number(data?.manager_rating?.toString()) : null,
       final_rating:data?.final_rating != null ? Number(data?.final_rating?.toString()) : null,
      }
    return plainData
  }
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            ...
            trs_token_id:"string",
            trs_version:"string",
      employee_id :"number",      cycle_id :"number",    }
    return data;
  }
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any) {
    try {
      ...
      const { review_number }: {review_number : string} = queryValue;
      const { review_type }: {review_type : string} = queryValue;
      const { review_status }: {review_status : string} = queryValue;
      const { self_rating }: {self_rating : Date} = queryValue;
      const { manager_rating }: {manager_rating : Date} = queryValue;
      const { final_rating }: {final_rating : Date} = queryValue;
      const { improvements }: {improvements : string} = queryValue;
      const { completed_at }: {completed_at :  Date} = queryValue;
      ...
      if(review_number){ 
        query.review_number = { [queryCondition['review_number']]: review_number };
      }
      if(review_type){ 
        query.review_type = { [queryCondition['review_type']]: review_type };
      }
      if(review_status){ 
        query.review_status = { [queryCondition['review_status']]: review_status };
      }
      if(self_rating){ 
        query.self_rating = { [queryCondition['self_rating']]: self_rating };
      }
      if(manager_rating){ 
        query.manager_rating = { [queryCondition['manager_rating']]: manager_rating };
      }
      if(final_rating){ 
        query.final_rating = { [queryCondition['final_rating']]: final_rating };
      }
      if(improvements){ 
        query.improvements = { [queryCondition['improvements']]: improvements };
      }
      if(completed_at){ 
        query.completed_at = { [queryCondition['completed_at']]: completed_at };
      }
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.hrm_performance_reviews.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'hrm_performance_reviews');
      ...
      this.prismaService.hrm_performance_reviews.count({
        where: query,
      }));
  ...
  async create(createhrm_performance_reviewsDto: Prisma.hrm_performance_reviewsCreateInput,token:string,detokenize:string,detokenizeData?: any) {
    try{

      const dataSchema:any =  v.object({
            review_number :v.string() , 
            review_type :v.string() , 
            review_status :v.string() , 
            self_rating :  v.optional(v.number()), 
            manager_rating :  v.optional(v.number()), 
            final_rating :  v.optional(v.number()), 
            improvements :  v.optional(v.string()), 
            completed_at :  v.optional((v.any())), 
      ...
      let validate : any = v.safeParse(dataSchema,createhrm_performance_reviewsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createhrm_performance_reviewsDto, 'hrm_performance_reviews', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.hrm_performance_reviews.create({
          data: encryptedData,
          select:{review_id:true,review_number:true,review_type:true,review_status:true,self_rating:true,manager_rating:true,final_rating:true,improvements:true,completed_at:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,employee_id :true,cycle_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createhrm_performance_reviewsDto: Prisma.hrm_performance_reviewsCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_reviews creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_reviews creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'hrm_performance_reviews',
        ...
        return {
          success: true,
          message: 'hrm_performance_reviews creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(review_id:number, updatehrm_performance_reviewsDto: Prisma.hrm_performance_reviewsUpdateInput,token:string, detokenize:string,detokenizeData?: any) {   
    try{

      const dataSchema:any =  v.object({
          review_number :  v.optional(v.string()), 
          review_type :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatehrm_performance_reviewsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatehrm_performance_reviewsDto,'hrm_performance_reviews','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatehrm_performance_reviewsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.hrm_performance_reviews.updateMany({
        where: {review_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'hrm_performance_reviews', {
...
/**
  ...
  async updateMaster(
review_id:number,
    updatehrm_performance_reviewsDto: Prisma.hrm_performance_reviewsUpdateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_performance_reviews update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_reviews update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_performance_reviews update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(review_id:number,token : string, detokenize: string,detokenizeData?: any) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.hrm_performance_reviews.deleteMany({
      where: {review_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'hrm_performance_reviews');
    let decryptedRes: any = [];
...
review_id:number,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_performance_reviews deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_reviews deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_performance_reviews deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string, detokenize: string, detokenizeData?: any) {
    try{
      ...
      this.prismaService.hrm_performance_reviews.findFirst({ 
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      const locked = await this.ufservice.acquireLock({
        tableName: 'hrm_performance_reviews',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'hrm_performance_reviews'));
      }
      return decryptedRes;
    } catch (error: any) {
