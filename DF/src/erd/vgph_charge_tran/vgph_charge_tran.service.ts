...
,vgphct_id?:number) {
  ...
  async create(createvgph_charge_tranDto: Prisma.vgph_charge_tranCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_charge_tranDto, 'vgph_charge_tran', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphct_id:number, updatevgph_charge_tranDto: Prisma.vgph_charge_tranUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_charge_tranDto,'vgph_charge_tran','update'), token);
