...
,vgphcs_id?:number) {
  ...
  async create(createvgph_charge_setupDto: Prisma.vgph_charge_setupCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_charge_setupDto, 'vgph_charge_setup', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphcs_id:number, updatevgph_charge_setupDto: Prisma.vgph_charge_setupUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_charge_setupDto,'vgph_charge_setup','update'), token);
