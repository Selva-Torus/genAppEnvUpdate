 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
  ...
  async create(createvgph_temp_mainDto: Prisma.vgph_temp_mainCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_temp_mainDto, 'vgph_temp_main', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphtm_id:number, updatevgph_temp_mainDto: Prisma.vgph_temp_mainUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_temp_mainDto,'vgph_temp_main','update'), token);
