...
export class vgph_temp_mainController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_temp_mainService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphtm_id') vgphtm_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtm_id"] = req.headers.vgphtm_id || "";
      detokenizeData["prctm_uuid"] = req.headers.prctm_uuid || "";
      detokenizeData["data_json"] = req.headers.data_json || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_mainService.findOne(+vgphtm_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_temp_mainService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_temp_mainEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_temp_main'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_temp_mainDto: Createvgph_temp_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtm_id"] = req.headers.vgphtm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_temp_mainService.createMaster(createvgph_temp_mainDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_temp_mainService.create(createvgph_temp_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_mainEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_temp_main', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphtm_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphtm_id') vgphtm_id:number,
    @Body() updatevgph_temp_mainDto: Updatevgph_temp_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtm_id"] = req.headers.vgphtm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_temp_mainService.updateMaster(+vgphtm_id,updatevgph_temp_mainDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_temp_mainService.update(+vgphtm_id,updatevgph_temp_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_mainEntity, result);
  }
...
@Param('vgphtm_id') vgphtm_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_temp_mainService.deleteMaster(+vgphtm_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_temp_mainService.remove(+vgphtm_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_mainEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_mainService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_mainEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_temp_mainService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_temp_mainEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_temp_mainService.getLockById(key, value, token);
    return plainToInstance(vgph_temp_mainEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_temp_mainService.releaseLockById(key, value, token);
    return result;
  }
}