...
@Injectable()
export class npss_customer_accountsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npssca_id }: {npssca_id : bigint} = queryValue;
      const { npsscp_id }: {npsscp_id : number} = queryValue;
      ...
      const { instant_payment }: {instant_payment : string} = queryValue;
      ...
      if(npssca_id){ 
        query.npssca_id = { [queryCondition['npssca_id']]: npssca_id };
      }
      if(npsscp_id){ 
      ...
      if(instant_payment){ 
        query.instant_payment = { [queryCondition['instant_payment']]: instant_payment };
      }
      if(default_account){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_customer_accounts.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_customer_accounts');
      ...
      this.prismaService.npss_customer_accounts.count({
        where: query,
      }));
  ...
  async findOne(npssca_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_customer_accountsDto: Createnpss_customer_accountsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_customer_accountsDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            instant_payment :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_customer_accountsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_customer_accountsDto, 'npss_customer_accounts', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_accounts.create({
          data: encryptedData,
          select:{npssca_id:true,npsscp_id:true,product_code:true,bankuserid:true,iban:true,currency:true,instant_payment:true,default_account:true,exhf_id:true,account_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_customer_accountsDto: Createnpss_customer_accountsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_customer_accounts:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_customer_accounts');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_accounts creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_accounts creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_customer_accounts',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npssca_id',
        ...
        return {
          success: true,
          message: 'npss_customer_accounts creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npssca_id:number, updatenpss_customer_accountsDto: Updatenpss_customer_accountsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_accounts.findFirst({
          where: {npssca_id, trs_tenant_id: authContext.tenant },
          select: {npssca_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_customer_accountsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_customer_accountsDto,'npss_customer_accounts','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_customer_accountsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_accounts.updateMany({
        where: {npssca_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_customer_accounts', {
          primaryKey: {npssca_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npssca_id:number,
    updatenpss_customer_accountsDto: Updatenpss_customer_accountsDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npssca_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_customer_accounts.findFirst({
          where: {npssca_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_accounts update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_accounts update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_accounts update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npssca_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_customer_accounts.deleteMany({
      where: {npssca_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_customer_accounts');
    let decryptedRes: any = [];
...
npssca_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npssca_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_accounts deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_accounts deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_accounts deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_customer_accounts columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_customer_accounts'));
      }
      return decryptedRes;
    } catch (error: any) {
