...
export class vgph_corporate_relationship_exceptionService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphcre_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
...
,vgphcre_id?:number) {
  ...
  async create(createvgph_corporate_relationship_exceptionDto: Createvgph_corporate_relationship_exceptionDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_corporate_relationship_exceptionDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_corporate_relationship_exceptionDto, 'vgph_corporate_relationship_exception', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_corporate_relationship_exceptionDto: Createvgph_corporate_relationship_exceptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_corporate_relationship_exception:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_corporate_relationship_exception');
        }
  ...
  async update(vgphcre_id:number, updatevgph_corporate_relationship_exceptionDto: Updatevgph_corporate_relationship_exceptionDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_corporate_relationship_exception.findFirst({
          where: {vgphcre_id, trs_tenant_id: authContext.tenant },
          select: {vgphcre_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_corporate_relationship_exceptionDto,'vgph_corporate_relationship_exception','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_corporate_relationship_exceptionDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_corporate_relationship_exception', {
          primaryKey: {vgphcre_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphcre_id:number,
    updatevgph_corporate_relationship_exceptionDto: Updatevgph_corporate_relationship_exceptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_corporate_relationship_exception.findFirst({
          where: {vgphcre_id },
  ...
  async remove(vgphcre_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphcre_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_corporate_relationship_exception columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('vgph_corporate_relationship_exception'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'vgph_corporate_relationship_exception',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_corporate_relationship_exception'));
      }
      return decryptedRes;
    } catch (error: any) {
