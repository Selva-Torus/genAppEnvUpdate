...
export class claims_detailService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.claims_detail.findMany({
          select:columns,
           where: query,
      ...
      this.prismaService.claims_detail.count({
        where: query,
      }));
  ...
  async findOne(claim_detail_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.claims_detail.findMany({ 
      where: {claim_detail_id, ...tenantFilter },
      select: {claim_detail_id:true,expense_category:true,expense_description:true,expense_date:true,expense_amount:true,name:true,attachment:true,claim_category:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,claim_id :true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'claims_detail');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'claims_detail');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createclaims_detailDto: Createclaims_detailDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createclaims_detailDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createclaims_detailDto, 'claims_detail', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.claims_detail.create({
          data: encryptedData,
          select:{claim_detail_id:true,expense_category:true,expense_description:true,expense_date:true,expense_amount:true,name:true,attachment:true,claim_category:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,claim_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createclaims_detailDto: Createclaims_detailDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'claims_detail:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for claims_detail');
        }
  ...
  async update(claim_detail_id:number, updateclaims_detailDto: Updateclaims_detailDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.claims_detail.findFirst({
          where: {claim_detail_id, trs_tenant_id: authContext.tenant },
          select: {claim_detail_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateclaims_detailDto,'claims_detail','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updateclaims_detailDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.claims_detail.updateMany({
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'claims_detail', {
...
/**
  ...
  async updateMaster(
claim_detail_id:number,
    updateclaims_detailDto: Updateclaims_detailDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.claims_detail.findFirst({
          where: {claim_detail_id },
  ...
  async remove(claim_detail_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.claims_detail.deleteMany({
      where: {claim_detail_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'claims_detail');
    let decryptedRes: any = [];
...
claim_detail_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real claims_detail columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'claims_detail'));
      }
      return decryptedRes;
    } catch (error: any) {
