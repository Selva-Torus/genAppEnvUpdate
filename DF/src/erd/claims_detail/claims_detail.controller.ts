...
export class claims_detailController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.claims_detailService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('claim_detail_id') claim_detail_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["claim_detail_id"] = req.headers.claim_detail_id || "";
      detokenizeData["expense_category"] = req.headers.expense_category || "";
      ...
      detokenizeData["attachment"] = req.headers.attachment || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.claims_detailService.findOne(+claim_detail_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.claims_detailService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(claims_detailEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('claims_detail'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createclaims_detailDto: Createclaims_detailDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["claim_detail_id"] = req.headers.claim_detail_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.claims_detailService.createMaster(createclaims_detailDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.claims_detailService.create(createclaims_detailDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(claims_detailEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('claims_detail', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'claim_detail_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('claim_detail_id') claim_detail_id:number,
    @Body() updateclaims_detailDto: Updateclaims_detailDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["claim_detail_id"] = req.headers.claim_detail_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.claims_detailService.updateMaster(+claim_detail_id,updateclaims_detailDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.claims_detailService.update(+claim_detail_id,updateclaims_detailDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(claims_detailEntity, result);
  }
...
@Param('claim_detail_id') claim_detail_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.claims_detailService.deleteMaster(+claim_detail_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.claims_detailService.remove(+claim_detail_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(claims_detailEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.claims_detailService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(claims_detailEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.claims_detailService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(claims_detailEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.claims_detailService.getLockById(key, value, token);
    return plainToInstance(claims_detailEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.claims_detailService.releaseLockById(key, value, token);
    return result;
  }
}