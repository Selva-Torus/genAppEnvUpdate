...
export class vgph_destination_tran_stagingController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_destination_tran_stagingService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphdts_id') vgphdts_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphdts_id"] = req.headers.vgphdts_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_destination_tran_stagingService.findOne(+vgphdts_id,trs_tenant_id,trs_app_code,trs_product_code,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_destination_tran_stagingService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_destination_tran_staging'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_destination_tran_stagingDto: Createvgph_destination_tran_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphdts_id"] = req.headers.vgphdts_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_destination_tran_stagingService.createMaster(createvgph_destination_tran_stagingDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_destination_tran_stagingService.create(createvgph_destination_tran_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_destination_tran_staging', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphdts_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphdts_id') vgphdts_id:number,
    @Body() updatevgph_destination_tran_stagingDto: Updatevgph_destination_tran_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphdts_id"] = req.headers.vgphdts_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_destination_tran_stagingService.updateMaster(+vgphdts_id,updatevgph_destination_tran_stagingDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_destination_tran_stagingService.update(+vgphdts_id,updatevgph_destination_tran_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }
...
@Param('vgphdts_id') vgphdts_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_destination_tran_stagingService.deleteMaster(+vgphdts_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_destination_tran_stagingService.remove(+vgphdts_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_destination_tran_stagingService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_destination_tran_stagingService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_destination_tran_stagingService.getLockById(key, value, token);
    return plainToInstance(vgph_destination_tran_stagingEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_destination_tran_stagingService.releaseLockById(key, value, token);
    return result;
  }
}