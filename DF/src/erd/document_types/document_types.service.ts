...
import { Createdocument_typesDto } from './dto/Createdocument_types.dto';
import { Updatedocument_typesDto } from './dto/Updatedocument_types.dto';
...
import { document_typesEntity } from './entity/document_types.entity';
...
@Injectable()
export class document_typesService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { doc_type_id }: {doc_type_id : bigint} = queryValue;
      const { doc_type_name }: {doc_type_name : string} = queryValue;
      ...
      if(doc_type_id){ 
        query.doc_type_id = { [queryCondition['doc_type_id']]: doc_type_id };
      }
      if(doc_type_name){ 
        query.doc_type_name = { [queryCondition['doc_type_name']]: doc_type_name };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.document_types.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'document_types');
      ...
      this.prismaService.document_types.count({
        where: query,
      }));
  ...
  async findOne(doc_type_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.document_types.findMany({ 
      where: {doc_type_id, ...tenantFilter },
      select: {doc_type_id:true,doc_type_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            account_documents:{
              select:{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'document_types');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'document_types');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createdocument_typesDto: Createdocument_typesDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createdocument_typesDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            doc_type_name :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createdocument_typesDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createdocument_typesDto, 'document_types', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.document_types.create({
          data: encryptedData,
          select:{doc_type_id:true,doc_type_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,account_documents:true,}          
        })
      );
   ...
   */
  async createMaster(
    createdocument_typesDto: Createdocument_typesDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'document_types:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for document_types');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'document_types creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'document_types creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            ...
            p_record_id      := NULL,
            p_record_id_column := 'doc_type_id',
        ...
        return {
          success: true,
          message: 'document_types creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(doc_type_id:number, updatedocument_typesDto: Updatedocument_typesDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.document_types.findFirst({
          where: {doc_type_id, trs_tenant_id: authContext.tenant },
          select: {doc_type_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatedocument_typesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatedocument_typesDto,'document_types','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatedocument_typesDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.document_types.updateMany({
        where: {doc_type_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'document_types', {
...
/**
  ...
  async updateMaster(
doc_type_id:number,
    updatedocument_typesDto: Updatedocument_typesDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =doc_type_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.document_types.findUnique({
          where: { doc_type_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'document_types update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'document_types update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'document_types update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(doc_type_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.document_types.deleteMany({
      where: {doc_type_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'document_types');
    let decryptedRes: any = [];
...
doc_type_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =doc_type_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.document_types.findUnique({
          where: { doc_type_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'document_types deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'document_types deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'document_types deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real document_types columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('document_types'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'document_types'));
      }
      return decryptedRes;
    } catch (error: any) {
