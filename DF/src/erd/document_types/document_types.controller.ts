...
//import { CreateDocumentTypesDto } from '../prisma/dto/create-documentTypes.dto';
//import { UpdateDocumentTypesDto } from '../prisma/dto/update-documentTypes.dto';
...
export class document_typesController {
  constructor(
    private readonly document_typesService: document_typesService,
  ...
  @ApiOkResponse({ type: document_typesEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the document_types table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.document_typesService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: document_typesEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the document_types table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.document_typesService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':doc_type_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'doc_type_id',type:Number})
  @ApiOkResponse({ type: document_typesEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the document_types table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('doc_type_id') doc_type_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["doc_type_id"] = req.headers.doc_type_id || "";
      detokenizeData["doc_type_name"] = req.headers.doc_type_name || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.document_typesService.findOne(+doc_type_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the document_types table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('document_types'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createdocument_typesDto })
  @ApiCreatedResponse({ type: document_typesEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the document_types table',
  })
  ...
  async create(
    ...
    @Body() createdocument_typesDto: Createdocument_typesDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["doc_type_id"] = req.headers.doc_type_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.document_typesService.createMaster(createdocument_typesDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.document_typesService.create(createdocument_typesDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(document_typesEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('document_types', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'doc_type_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatedocument_typesDto })
  @ApiOkResponse({ type: document_typesEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the document_types table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('doc_type_id') doc_type_id:number,
    @Body() updatedocument_typesDto: Updatedocument_typesDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["doc_type_id"] = req.headers.doc_type_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.document_typesService.updateMaster(+doc_type_id,updatedocument_typesDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.document_typesService.update(+doc_type_id,updatedocument_typesDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(document_typesEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the document_types table',
  })
...
@Param('doc_type_id') doc_type_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.document_typesService.deleteMaster(+doc_type_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.document_typesService.remove(+doc_type_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(document_typesEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the document_types table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the document_types table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.document_typesService.getLockById(key, value, token);
    return plainToInstance(document_typesEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.document_typesService.releaseLockById(key, value, token);
    return result;
  }
}