...
,vgphe_id?:number) {
  ...
  async create(createvgph_employeeDto: Prisma.vgph_employeeCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_employeeDto, 'vgph_employee', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphe_id:number, updatevgph_employeeDto: Prisma.vgph_employeeUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_employeeDto,'vgph_employee','update'), token);
