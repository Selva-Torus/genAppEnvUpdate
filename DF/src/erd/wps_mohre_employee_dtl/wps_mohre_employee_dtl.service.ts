...
@Injectable()
export class wps_mohre_employee_dtlService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { wpsmed_id }: {wpsmed_id : bigint} = queryValue;
      ...
      const { mohre_reg_status }: {mohre_reg_status : string} = queryValue;
      const { employee_card_number }: {employee_card_number : string} = queryValue;
      const { employee_unified_number }: {employee_unified_number : string} = queryValue;
      const { employee_eid }: {employee_eid : string} = queryValue;
      const { employee_is_active }: {employee_is_active : string} = queryValue;
      ...
      const { employee_name_ar }: {employee_name_ar : string} = queryValue;
      const { total_salary }: {total_salary : Date} = queryValue;
      const { status_date }: {status_date : any } = queryValue;
      ...
      if(wpsmed_id){ 
        query.wpsmed_id = { [queryCondition['wpsmed_id']]: wpsmed_id };
      }
      if(wpsmac_id){ 
      ...
      if(mohre_reg_status){ 
        query.mohre_reg_status = { [queryCondition['mohre_reg_status']]: mohre_reg_status };
      }
      if(employee_card_number){ 
        query.employee_card_number = { [queryCondition['employee_card_number']]: employee_card_number };
      }
      ...
      if(employee_is_active){ 
        query.employee_is_active = { [queryCondition['employee_is_active']]: employee_is_active };
      }
      ...
      if(employee_name_ar){ 
        query.employee_name_ar = { [queryCondition['employee_name_ar']]: employee_name_ar };
      }
      if(total_salary){ 
        query.total_salary = { [queryCondition['total_salary']]: total_salary };
      }
      if(status_date){ 
        query.status_date = { [queryCondition['status_date']]: status_date };
      }
      if(employee_iban){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.wps_mohre_employee_dtl.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'wps_mohre_employee_dtl');
      ...
      this.prismaService.wps_mohre_employee_dtl.count({
        where: query,
      }));
  ...
  async findOne(wpsmed_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.wps_mohre_employee_dtl.findMany({ 
      where: {wpsmed_id, ...tenantFilter },
      select: {wpsmed_id:true,wpsmac_id:true,employer_mol_number:true,employee_mol_number:true,employer_reg_id:true,employer_eid:true,mohre_reg_status:true,employee_card_number:true,employee_unified_number:true,employee_eid:true,employee_is_active:true,employee_passport_number:true,employee_name:true,employee_name_ar:true,total_salary:true,status_date:true,employee_iban:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'wps_mohre_employee_dtl');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'wps_mohre_employee_dtl');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createwps_mohre_employee_dtlDto: Createwps_mohre_employee_dtlDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createwps_mohre_employee_dtlDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            mohre_reg_status :  v.optional(v.pipe(v.string(),v.maxLength(1024 ))), 
            employee_card_number :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            employee_unified_number :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            employee_eid :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            employee_is_active :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            employee_passport_number :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            employee_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            employee_name_ar :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            total_salary :  v.optional(v.pipe(v.number(),v.maxValue(9999999999999999999 ))), 
            status_date :  v.optional(v.pipe(
                  v.string(),
                  v.regex(
      ...
      let validate : any = v.safeParse(dataSchema,createwps_mohre_employee_dtlDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createwps_mohre_employee_dtlDto, 'wps_mohre_employee_dtl', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_employee_dtl.create({
          data: encryptedData,
          select:{wpsmed_id:true,wpsmac_id:true,employer_mol_number:true,employee_mol_number:true,employer_reg_id:true,employer_eid:true,mohre_reg_status:true,employee_card_number:true,employee_unified_number:true,employee_eid:true,employee_is_active:true,employee_passport_number:true,employee_name:true,employee_name_ar:true,total_salary:true,status_date:true,employee_iban:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createwps_mohre_employee_dtlDto: Createwps_mohre_employee_dtlDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'wps_mohre_employee_dtl:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for wps_mohre_employee_dtl');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_employee_dtl creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_employee_dtl creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'wps_mohre_employee_dtl',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'wpsmed_id',
        ...
        return {
          success: true,
          message: 'wps_mohre_employee_dtl creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(wpsmed_id:number, updatewps_mohre_employee_dtlDto: Updatewps_mohre_employee_dtlDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_employee_dtl.findFirst({
          where: {wpsmed_id, trs_tenant_id: authContext.tenant },
          select: {wpsmed_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatewps_mohre_employee_dtlDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatewps_mohre_employee_dtlDto,'wps_mohre_employee_dtl','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatewps_mohre_employee_dtlDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_employee_dtl.updateMany({
        where: {wpsmed_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'wps_mohre_employee_dtl', {
          primaryKey: {wpsmed_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
wpsmed_id:number,
    updatewps_mohre_employee_dtlDto: Updatewps_mohre_employee_dtlDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =wpsmed_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.wps_mohre_employee_dtl.findFirst({
          where: {wpsmed_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'wps_mohre_employee_dtl update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_employee_dtl update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'wps_mohre_employee_dtl update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(wpsmed_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.wps_mohre_employee_dtl.deleteMany({
      where: {wpsmed_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'wps_mohre_employee_dtl');
    let decryptedRes: any = [];
...
wpsmed_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =wpsmed_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'wps_mohre_employee_dtl deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_employee_dtl deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'wps_mohre_employee_dtl deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real wps_mohre_employee_dtl columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'wps_mohre_employee_dtl'));
      }
      return decryptedRes;
    } catch (error: any) {
