...
@Injectable()
export class ams_asset_disposalService {
  ...
  async commonDecimalDatahandle(data:any){
    const plainData = { ...data,
       disposal_value:data?.disposal_value != null ? Number(data?.disposal_value?.toString()) : null,
       disposal_cost:data?.disposal_cost != null ? Number(data?.disposal_cost?.toString()) : null,
       resale_amount:data?.resale_amount != null ? Number(data?.resale_amount?.toString()) : null,
      }
    return plainData
  }
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { disposal_id }: {disposal_id : bigint} = queryValue;
      const { disposal_method }: {disposal_method : string} = queryValue;
      const { reason }: {reason : string} = queryValue;
      const { approval_reference }: {approval_reference : string} = queryValue;
      const { witness_name }: {witness_name : string} = queryValue;
      const { data_wiped }: {data_wiped : Date} = queryValue;
      const { data_wipe_method }: {data_wipe_method : string} = queryValue;
      const { certificate_url }: {certificate_url : string} = queryValue;
      const { disposal_date }: {disposal_date : any } = queryValue;
      const { disposal_value }: {disposal_value : Date} = queryValue;
      const { disposal_cost }: {disposal_cost : Date} = queryValue;
      const { resale_amount }: {resale_amount : Date} = queryValue;
      ...
      if(disposal_id){ 
        query.disposal_id = { [queryCondition['disposal_id']]: disposal_id };
      }
      if(disposal_method){ 
      ...
      if(approval_reference){ 
        query.approval_reference = { [queryCondition['approval_reference']]: approval_reference };
      }
      if(witness_name){ 
        query.witness_name = { [queryCondition['witness_name']]: witness_name };
      }
      if(data_wiped){ 
        query.data_wiped = { [queryCondition['data_wiped']]: data_wiped };
      }
      if(data_wipe_method){ 
        query.data_wipe_method = { [queryCondition['data_wipe_method']]: data_wipe_method };
      }
      if(certificate_url){ 
        query.certificate_url = { [queryCondition['certificate_url']]: certificate_url };
      }
      if(disposal_date){ 
        query.disposal_date = { [queryCondition['disposal_date']]: disposal_date };
      }
      if(disposal_value){ 
        query.disposal_value = { [queryCondition['disposal_value']]: disposal_value };
      }
      if(disposal_cost){ 
        query.disposal_cost = { [queryCondition['disposal_cost']]: disposal_cost };
      }
      if(resale_amount){ 
        query.resale_amount = { [queryCondition['resale_amount']]: resale_amount };
      }
      if(vendor_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.ams_asset_disposal.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'ams_asset_disposal');
      ...
      this.prismaService.ams_asset_disposal.count({
        where: query,
      }));
  ...
  async create(createams_asset_disposalDto: Prisma.ams_asset_disposalCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            disposal_id :v.number() , 
            disposal_method :v.string() , 
            reason :  v.optional(v.string()), 
            approval_reference :  v.optional(v.string()), 
            witness_name :  v.optional(v.string()), 
            data_wiped :  v.optional(v.boolean()), 
            data_wipe_method :  v.optional(v.string()), 
            ...
            disposal_value :  v.optional(v.number()), 
            disposal_cost :  v.optional(v.number()), 
            resale_amount :  v.optional(v.number()), 
        ...
        let validate : any = v.safeParse(dataSchema,createams_asset_disposalDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createams_asset_disposalDto, 'ams_asset_disposal', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.ams_asset_disposal.create({
          data: encryptedData,
          select:{disposal_id:true,disposal_method:true,reason:true,approval_reference:true,witness_name:true,data_wiped:true,data_wipe_method:true,certificate_url:true,disposal_date:true,disposal_value:true,disposal_cost:true,resale_amount:true,vendor_id:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,asset_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createams_asset_disposalDto: Prisma.ams_asset_disposalCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_disposal creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_disposal creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'ams_asset_disposal',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'disposal_id',
        ...
        return {
          success: true,
          message: 'ams_asset_disposal creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(disposal_id:number, updateams_asset_disposalDto: Prisma.ams_asset_disposalUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateams_asset_disposalDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateams_asset_disposalDto,'ams_asset_disposal','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_disposal.updateMany({
      where: {disposal_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
disposal_id:number,
    updateams_asset_disposalDto: Prisma.ams_asset_disposalUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =disposal_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_asset_disposal update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_disposal update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_asset_disposal update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(disposal_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_disposal.deleteMany({
      where: {disposal_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'ams_asset_disposal');
    let decryptedRes: any = [];
...
disposal_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =disposal_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_asset_disposal deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_disposal deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_asset_disposal deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_disposal.findFirst({ 
