...
@Injectable()
export class npss_camt_account_reportService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { ncar_id }: {ncar_id : bigint} = queryValue;
      const { header_msg_id }: {header_msg_id : string} = queryValue;
      const { report_id }: {report_id : string} = queryValue;
      const { rpt_created_datetime }: {rpt_created_datetime : string} = queryValue;
      const { iban }: {iban : string} = queryValue;
      const { account_currency }: {account_currency : string} = queryValue;
      const { account_name }: {account_name : string} = queryValue;
      const { bic }: {bic : string} = queryValue;
      const { balance_type_code }: {balance_type_code : string} = queryValue;
      const { balance_amount }: {balance_amount : Date} = queryValue;
      const { cr_dr_indicator }: {cr_dr_indicator : string} = queryValue;
      const { rpt_bal_datetime }: {rpt_bal_datetime : string} = queryValue;
      ...
      if(ncar_id){ 
        query.ncar_id = { [queryCondition['ncar_id']]: ncar_id };
      }
      if(header_msg_id){ 
        query.header_msg_id = { [queryCondition['header_msg_id']]: header_msg_id };
      }
      if(report_id){ 
        query.report_id = { [queryCondition['report_id']]: report_id };
      }
      if(rpt_created_datetime){ 
        query.rpt_created_datetime = { [queryCondition['rpt_created_datetime']]: rpt_created_datetime };
      }
      if(iban){ 
        query.iban = { [queryCondition['iban']]: iban };
      }
      if(account_currency){ 
        query.account_currency = { [queryCondition['account_currency']]: account_currency };
      }
      if(account_name){ 
        query.account_name = { [queryCondition['account_name']]: account_name };
      }
      if(bic){ 
        query.bic = { [queryCondition['bic']]: bic };
      }
      if(balance_type_code){ 
        query.balance_type_code = { [queryCondition['balance_type_code']]: balance_type_code };
      }
      if(balance_amount){ 
      ...
      if(rpt_bal_datetime){ 
        query.rpt_bal_datetime = { [queryCondition['rpt_bal_datetime']]: rpt_bal_datetime };
      }
      if(message_data){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_camt_account_report.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_camt_account_report');
      ...
      this.prismaService.npss_camt_account_report.count({
        where: query,
      }));
  ...
  async findOne(ncar_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_camt_account_reportDto: Createnpss_camt_account_reportDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_camt_account_reportDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            header_msg_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            report_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            rpt_created_datetime :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            iban :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            account_currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            account_name :  v.optional(v.pipe(v.string(),v.maxLength(70 ))), 
            bic :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            balance_type_code :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            balance_amount :  v.optional(v.pipe(v.number(),v.maxValue(999999999999999999 ))), 
            cr_dr_indicator :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            rpt_bal_datetime :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_camt_account_reportDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_camt_account_reportDto, 'npss_camt_account_report', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_account_report.create({
          data: encryptedData,
          select:{ncar_id:true,header_msg_id:true,report_id:true,rpt_created_datetime:true,iban:true,account_currency:true,account_name:true,bic:true,balance_type_code:true,balance_amount:true,cr_dr_indicator:true,rpt_bal_datetime:true,message_data:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_camt_account_reportDto: Createnpss_camt_account_reportDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_camt_account_report:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_camt_account_report');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_account_report creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_account_report creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_camt_account_report',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'ncar_id',
        ...
        return {
          success: true,
          message: 'npss_camt_account_report creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(ncar_id:number, updatenpss_camt_account_reportDto: Updatenpss_camt_account_reportDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_account_report.findFirst({
          where: {ncar_id, trs_tenant_id: authContext.tenant },
          select: {ncar_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_camt_account_reportDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_camt_account_reportDto,'npss_camt_account_report','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_camt_account_reportDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_account_report.updateMany({
        where: {ncar_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_camt_account_report', {
          primaryKey: {ncar_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
ncar_id:number,
    updatenpss_camt_account_reportDto: Updatenpss_camt_account_reportDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_camt_account_report.findFirst({
          where: {ncar_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_camt_account_report update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_account_report update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_camt_account_report update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(ncar_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_camt_account_report.deleteMany({
      where: {ncar_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_camt_account_report');
    let decryptedRes: any = [];
...
ncar_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_camt_account_report deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_account_report deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_camt_account_report deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_camt_account_report columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('npss_camt_account_report'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'npss_camt_account_report',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_camt_account_report'));
      }
      return decryptedRes;
    } catch (error: any) {
