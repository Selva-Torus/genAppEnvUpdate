...
export class vgph_source_stagingController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_source_stagingService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphss_id') vgphss_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_stagingService.findOne(+vgphss_id,trs_tenant_id,trs_app_code,trs_product_code,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_source_stagingService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_source_stagingEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_source_staging'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_source_stagingDto: Createvgph_source_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphss_id"] = req.headers.vgphss_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_stagingService.createMaster(createvgph_source_stagingDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_source_stagingService.create(createvgph_source_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_stagingEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_source_staging', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
...
@Param('uuid') uuid: string,
    @Body() updatevgph_source_stagingDto: Updatevgph_source_stagingDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphss_id"] = req.headers.vgphss_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_stagingService.updateMaster(uuid,updatevgph_source_stagingDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_stagingService.update(uuid,updatevgph_source_stagingDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_stagingEntity, result);
  }
...
@Param('vgphss_id') vgphss_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_source_stagingService.deleteMaster(+vgphss_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_stagingService.remove(+vgphss_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_stagingEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_stagingService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_stagingEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_stagingService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_stagingEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_source_stagingService.getLockById(key, value, token);
    return plainToInstance(vgph_source_stagingEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_source_stagingService.releaseLockById(key, value, token);
    return result;
  }
}