 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
  ...
  async create(createvgph_source_stagingDto: Prisma.vgph_source_stagingCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_stagingDto, 'vgph_source_staging', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphss_id:number,uuid: string, updatevgph_source_stagingDto: Prisma.vgph_source_stagingUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_stagingDto,'vgph_source_staging','update'), token);
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_staging.updateMany({
      where: {vgphss_id,uuid},
      data: encryptedData
    }));
...
vgphss_id:number,uuid: string,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphss_id||uuid;
