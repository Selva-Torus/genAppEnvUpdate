...
//import { CreateAttorneysDto } from '../prisma/dto/create-attorneys.dto';
//import { UpdateAttorneysDto } from '../prisma/dto/update-attorneys.dto';
...
export class attorneysController {
  ...
  @ApiOkResponse({ type: attorneysEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the attorneys table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.attorneysService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: attorneysEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the attorneys table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.attorneysService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':attorney_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'attorney_id',type:Number})
  @ApiOkResponse({ type: attorneysEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the attorneys table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('attorney_id') attorney_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["attorney_id"] = req.headers.attorney_id || "";
      detokenizeData["full_name"] = req.headers.full_name || "";
      detokenizeData["title"] = req.headers.title || "";
      detokenizeData["email"] = req.headers.email || "";
      detokenizeData["persona_role"] = req.headers.persona_role || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.attorneysService.findOne(+attorney_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the attorneys table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('attorneys'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: CreateattorneysDto })
  @ApiCreatedResponse({ type: attorneysEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the attorneys table',
  })
  ...
  async create(
    ...
    @Body() createattorneysDto: CreateattorneysDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["attorney_id"] = req.headers.attorney_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.attorneysService.createMaster(createattorneysDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.attorneysService.create(createattorneysDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(attorneysEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('attorneys', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'attorney_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: UpdateattorneysDto })
  @ApiOkResponse({ type: attorneysEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the attorneys table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('attorney_id') attorney_id:number,
    @Body() updateattorneysDto: UpdateattorneysDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["attorney_id"] = req.headers.attorney_id || "";
    ...
    const result = await this.attorneysService.update(+attorney_id,updateattorneysDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(attorneysEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the attorneys table',
  })
...
@Param('attorney_id') attorney_id:number,
    ...
    const result = await this.attorneysService.remove(+attorney_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(attorneysEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the attorneys table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the attorneys table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.attorneysService.getLockById(key, value, token);
    return plainToInstance(attorneysEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.attorneysService.releaseLockById(key, value, token);
    return result;
  }
}