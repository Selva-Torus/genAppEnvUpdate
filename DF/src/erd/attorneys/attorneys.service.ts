...
import { CreateattorneysDto } from './dto/Createattorneys.dto';
import { UpdateattorneysDto } from './dto/Updateattorneys.dto';
...
import { attorneysEntity } from './entity/attorneys.entity';
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { attorney_id }: {attorney_id : bigint} = queryValue;
      const { full_name }: {full_name : string} = queryValue;
      const { title }: {title : string} = queryValue;
      const { email }: {email : string} = queryValue;
      const { persona_role }: {persona_role : string} = queryValue;
      ...
      if(attorney_id){ 
        query.attorney_id = { [queryCondition['attorney_id']]: attorney_id };
      }
      if(full_name){ 
        query.full_name = { [queryCondition['full_name']]: full_name };
      }
      if(title){ 
        query.title = { [queryCondition['title']]: title };
      }
      if(email){ 
        query.email = { [queryCondition['email']]: email };
      }
      if(persona_role){ 
        query.persona_role = { [queryCondition['persona_role']]: persona_role };
      }
      if(is_active){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.attorneys.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'attorneys');
      ...
      this.prismaService.attorneys.count({
        where: query,
      }));
  ...
  async findOne(attorney_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      select: {attorney_id:true,full_name:true,title:true,email:true,persona_role:true,is_active:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            accounts:{
            ...
            amr_review_sessions:{
              select:{
              review_id:true,              start_timestamp:true,              end_timestamp:true,              review_duration_sec:true,              decision:true,              rejection_comment:true,              signature_timestamp:true,              trs_created_date:true,              trs_created_by:true,              trs_modified_date:true,              trs_modified_by:true,              trs_process_id:true,              trs_access_profile:true,              trs_org_grp_code:true,              trs_org_code:true,              trs_role_grp_code:true,              trs_role_code:true,              trs_ps_grp_code:true,              trs_ps_code:true,              trs_sub_org_grp_code:true,              trs_sub_org_code:true,              trs_locked_by:true,              trs_locked_time:true,              trs_tenant_id:true,              trs_app_code:true,              trs_product_code:true,              trs_event_process_status:true,              trs_event_status:true,              trs_token_id:true,              trs_version:true            ,
              }
            },
            activity_log:{
              select:{
              activity_id:true,              activity_type:true,              description:true,              activity_timestamp:true,              trs_created_date:true,              trs_created_by:true,              trs_modified_date:true,              trs_modified_by:true,              trs_process_id:true,              trs_access_profile:true,              trs_org_grp_code:true,              trs_org_code:true,              trs_role_grp_code:true,              trs_role_code:true,              trs_ps_grp_code:true,              trs_ps_code:true,              trs_sub_org_grp_code:true,              trs_sub_org_code:true,              trs_locked_by:true,              trs_locked_time:true,              trs_tenant_id:true,              trs_app_code:true,              trs_product_code:true,              trs_event_process_status:true,              trs_event_status:true,              trs_token_id:true,              trs_version:true            ,
              }
            },
        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'attorneys');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'attorneys');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createattorneysDto: CreateattorneysDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createattorneysDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            full_name :  v.optional(v.string()), 
            title :  v.optional(v.string()), 
            email :  v.optional(v.string()), 
            persona_role :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createattorneysDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createattorneysDto, 'attorneys', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.attorneys.create({
          data: encryptedData,
          select:{attorney_id:true,full_name:true,title:true,email:true,persona_role:true,is_active:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,accounts:true,amr_review_sessions:true,activity_log:true,}          
        })
      );
   ...
   */
  async createMaster(
    createattorneysDto: CreateattorneysDto,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'attorneys creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'attorneys creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'attorneys',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'attorney_id',
        ...
        return {
          success: true,
          message: 'attorneys creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(attorney_id:number, updateattorneysDto: UpdateattorneysDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.attorneys.findFirst({
          where: {attorney_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updateattorneysDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateattorneysDto,'attorneys','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'attorneys', {
          primaryKey: {attorney_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
attorney_id:number,
    updateattorneysDto: UpdateattorneysDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =attorney_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.attorneys.findUnique({
          where: { attorney_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'attorneys update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'attorneys update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'attorneys update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(attorney_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.attorneys.deleteMany({
      where: {attorney_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'attorneys');
    let decryptedRes: any = [];
...
attorney_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =attorney_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.attorneys.findUnique({
          where: { attorney_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'attorneys deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'attorneys deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'attorneys deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real attorneys columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('attorneys'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'attorneys'));
      }
      return decryptedRes;
    } catch (error: any) {
