...
@Injectable()
export class wps_mohre_api_callsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { wpsm_id }: {wpsm_id : bigint} = queryValue;
      ...
      const { employee_mol_numberadditional_info }: {employee_mol_numberadditional_info : string} = queryValue;
      const { additional_info }: {additional_info : string} = queryValue;
      const { employer_reg_id }: {employer_reg_id : string} = queryValue;
      const { employer_eid }: {employer_eid : string} = queryValue;
      const { service_req }: {service_req : any } = queryValue;
      const { service_resp }: {service_resp : any } = queryValue;
      const { service_req_encr }: {service_req_encr : string} = queryValue;
      const { service_resp_encr }: {service_resp_encr : string} = queryValue;
      const { client_req }: {client_req : any } = queryValue;
      const { client_resp }: {client_resp : any } = queryValue;
      const { employer_unified_number }: {employer_unified_number : string} = queryValue;
      ...
      if(wpsm_id){ 
        query.wpsm_id = { [queryCondition['wpsm_id']]: wpsm_id };
      }
      if(process_name){ 
      ...
      if(employee_mol_numberadditional_info){ 
        query.employee_mol_numberadditional_info = { [queryCondition['employee_mol_numberadditional_info']]: employee_mol_numberadditional_info };
      }
      if(additional_info){ 
      ...
      if(service_req){ 
        query.service_req = { [queryCondition['service_req']]: service_req };
      }
      if(service_resp){ 
        query.service_resp = { [queryCondition['service_resp']]: service_resp };
      }
      if(service_req_encr){ 
        query.service_req_encr = { [queryCondition['service_req_encr']]: service_req_encr };
      }
      if(service_resp_encr){ 
        query.service_resp_encr = { [queryCondition['service_resp_encr']]: service_resp_encr };
      }
      if(client_req){ 
        query.client_req = { [queryCondition['client_req']]: client_req };
      }
      if(client_resp){ 
        query.client_resp = { [queryCondition['client_resp']]: client_resp };
      }
      if(employer_unified_number){ 
        query.employer_unified_number = { [queryCondition['employer_unified_number']]: employer_unified_number };
      }
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.wps_mohre_api_calls.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'wps_mohre_api_calls');
      ...
      this.prismaService.wps_mohre_api_calls.count({
        where: query,
      }));
  ...
  async findOne(wpsm_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.wps_mohre_api_calls.findMany({ 
      where: {wpsm_id, ...tenantFilter },
      select: {wpsm_id:true,process_name:true,processing_system:true,response_code:true,employer_mol_number:true,employee_mol_numberadditional_info:true,additional_info:true,employer_reg_id:true,employer_eid:true,service_req:true,service_resp:true,service_req_encr:true,service_resp_encr:true,client_req:true,client_resp:true,employer_unified_number:true,employee_unified_number:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'wps_mohre_api_calls');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'wps_mohre_api_calls');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createwps_mohre_api_callsDto: Createwps_mohre_api_callsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createwps_mohre_api_callsDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            employee_mol_numberadditional_info :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            additional_info :  v.optional(v.pipe(v.string(),v.maxLength(1024 ))), 
            employer_reg_id :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            employer_eid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            service_req :  v.optional(v.any() ), 
            service_resp :  v.optional(v.any() ), 
            service_req_encr :  v.optional(v.pipe(v.string(),v.maxLength(80000 ))), 
            service_resp_encr :  v.optional(v.pipe(v.string(),v.maxLength(80000 ))), 
            client_req :  v.optional(v.any() ), 
            client_resp :  v.optional(v.any() ), 
            employer_unified_number :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createwps_mohre_api_callsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createwps_mohre_api_callsDto, 'wps_mohre_api_calls', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_api_calls.create({
          data: encryptedData,
          select:{wpsm_id:true,process_name:true,processing_system:true,response_code:true,employer_mol_number:true,employee_mol_numberadditional_info:true,additional_info:true,employer_reg_id:true,employer_eid:true,service_req:true,service_resp:true,service_req_encr:true,service_resp_encr:true,client_req:true,client_resp:true,employer_unified_number:true,employee_unified_number:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createwps_mohre_api_callsDto: Createwps_mohre_api_callsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'wps_mohre_api_calls:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for wps_mohre_api_calls');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_api_calls creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_api_calls creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'wps_mohre_api_calls',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'wpsm_id',
        ...
        return {
          success: true,
          message: 'wps_mohre_api_calls creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(wpsm_id:number, updatewps_mohre_api_callsDto: Updatewps_mohre_api_callsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_api_calls.findFirst({
          where: {wpsm_id, trs_tenant_id: authContext.tenant },
          select: {wpsm_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatewps_mohre_api_callsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatewps_mohre_api_callsDto,'wps_mohre_api_calls','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatewps_mohre_api_callsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_api_calls.updateMany({
        where: {wpsm_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'wps_mohre_api_calls', {
          primaryKey: {wpsm_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
wpsm_id:number,
    updatewps_mohre_api_callsDto: Updatewps_mohre_api_callsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.wps_mohre_api_calls.findFirst({
          where: {wpsm_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'wps_mohre_api_calls update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_api_calls update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'wps_mohre_api_calls update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(wpsm_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.wps_mohre_api_calls.deleteMany({
      where: {wpsm_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'wps_mohre_api_calls');
    let decryptedRes: any = [];
...
wpsm_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'wps_mohre_api_calls deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'wps_mohre_api_calls deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'wps_mohre_api_calls deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real wps_mohre_api_calls columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'wps_mohre_api_calls'));
      }
      return decryptedRes;
    } catch (error: any) {
