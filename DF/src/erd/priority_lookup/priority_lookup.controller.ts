...
import { priority_lookupService } from './priority_lookup.service';
...
//import { CreatePriorityLookupDto } from '../prisma/dto/create-priorityLookup.dto';
//import { UpdatePriorityLookupDto } from '../prisma/dto/update-priorityLookup.dto';
...
@Controller('priority_lookup')
@ApiTags('ERD API')
export class priority_lookupController {
  constructor(
    private readonly priority_lookupService: priority_lookupService,
  ...
  @ApiOkResponse({ type: priority_lookupEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the priority_lookup table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.priority_lookupService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: priority_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the priority_lookup table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.priority_lookupService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':priority_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'priority_id',type:Number})
  @ApiOkResponse({ type: priority_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the priority_lookup table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('priority_id') priority_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["priority_id"] = req.headers.priority_id || "";
      detokenizeData["priority_name"] = req.headers.priority_name || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.priority_lookupService.findOne(+priority_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the priority_lookup table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('priority_lookup'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createpriority_lookupDto })
  @ApiCreatedResponse({ type: priority_lookupEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the priority_lookup table',
  })
  ...
  async create(
    ...
    @Body() createpriority_lookupDto: Createpriority_lookupDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["priority_id"] = req.headers.priority_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.priority_lookupService.createMaster(createpriority_lookupDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.priority_lookupService.create(createpriority_lookupDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(priority_lookupEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('priority_lookup', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'priority_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatepriority_lookupDto })
  @ApiOkResponse({ type: priority_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the priority_lookup table',
  })
...
@Param('priority_id') priority_id:number,
    @Body() updatepriority_lookupDto: Updatepriority_lookupDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["priority_id"] = req.headers.priority_id || "";
    ...
    const result = await this.priority_lookupService.update(+priority_id,updatepriority_lookupDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(priority_lookupEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the priority_lookup table',
  })
...
@Param('priority_id') priority_id:number,
    ...
    const result = await this.priority_lookupService.remove(+priority_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(priority_lookupEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the priority_lookup table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the priority_lookup table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.priority_lookupService.getLockById(key, value, token);
    return plainToInstance(priority_lookupEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.priority_lookupService.releaseLockById(key, value, token);
    return result;
  }
}