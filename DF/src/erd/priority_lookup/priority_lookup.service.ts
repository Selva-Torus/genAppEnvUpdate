...
import { Createpriority_lookupDto } from './dto/Createpriority_lookup.dto';
import { Updatepriority_lookupDto } from './dto/Updatepriority_lookup.dto';
...
import { priority_lookupEntity } from './entity/priority_lookup.entity';
...
@Injectable()
export class priority_lookupService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { priority_id }: {priority_id : bigint} = queryValue;
      const { priority_name }: {priority_name : string} = queryValue;
      ...
      if(priority_id){ 
        query.priority_id = { [queryCondition['priority_id']]: priority_id };
      }
      if(priority_name){ 
        query.priority_name = { [queryCondition['priority_name']]: priority_name };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.priority_lookup.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'priority_lookup');
      ...
      this.prismaService.priority_lookup.count({
        where: query,
      }));
  ...
  async findOne(priority_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createpriority_lookupDto: Createpriority_lookupDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createpriority_lookupDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            priority_name :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createpriority_lookupDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createpriority_lookupDto, 'priority_lookup', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.priority_lookup.create({
          data: encryptedData,
          select:{priority_id:true,priority_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,accounts:true,}          
        })
      );
   ...
   */
  async createMaster(
    createpriority_lookupDto: Createpriority_lookupDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'priority_lookup:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for priority_lookup');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'priority_lookup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'priority_lookup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'priority_lookup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'priority_id',
        ...
        return {
          success: true,
          message: 'priority_lookup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(priority_id:number, updatepriority_lookupDto: Updatepriority_lookupDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.priority_lookup.findFirst({
          where: {priority_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updatepriority_lookupDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatepriority_lookupDto,'priority_lookup','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatepriority_lookupDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.priority_lookup.updateMany({
        where: {priority_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'priority_lookup', {
...
/**
  ...
  async updateMaster(
priority_id:number,
    updatepriority_lookupDto: Updatepriority_lookupDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =priority_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.priority_lookup.findUnique({
          where: { priority_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'priority_lookup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'priority_lookup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'priority_lookup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(priority_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.priority_lookup.deleteMany({
      where: {priority_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'priority_lookup');
    let decryptedRes: any = [];
...
priority_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =priority_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.priority_lookup.findUnique({
          where: { priority_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'priority_lookup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'priority_lookup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'priority_lookup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real priority_lookup columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'priority_lookup'));
      }
      return decryptedRes;
    } catch (error: any) {
