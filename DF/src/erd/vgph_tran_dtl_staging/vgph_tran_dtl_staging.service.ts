...
export class vgph_tran_dtl_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphtds_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.vgph_tran_dtl_staging.findMany({ 
      where: {vgphtds_id,trs_tenant_id,trs_app_code,trs_product_code, ...tenantFilter },
  ...
  async create(createvgph_tran_dtl_stagingDto: Createvgph_tran_dtl_stagingDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_tran_dtl_stagingDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_tran_dtl_stagingDto, 'vgph_tran_dtl_staging', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_tran_dtl_stagingDto: Createvgph_tran_dtl_stagingDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_tran_dtl_staging:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_tran_dtl_staging');
        }
  ...
  async update(vgphtds_id:number, updatevgph_tran_dtl_stagingDto: Updatevgph_tran_dtl_stagingDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_tran_dtl_staging.findFirst({
          where: {vgphtds_id, trs_tenant_id: authContext.tenant },
          select: {vgphtds_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_tran_dtl_stagingDto,'vgph_tran_dtl_staging','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_tran_dtl_stagingDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_tran_dtl_staging', {
          primaryKey: {vgphtds_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphtds_id:number,
    updatevgph_tran_dtl_stagingDto: Updatevgph_tran_dtl_stagingDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_tran_dtl_staging.findFirst({
          where: {vgphtds_id },
  ...
  async remove(vgphtds_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphtds_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_tran_dtl_staging columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_tran_dtl_staging'));
      }
      return decryptedRes;
    } catch (error: any) {
