...
,vgphc_id?:number) {
  ...
  async create(createvgph_corporateDto: Prisma.vgph_corporateCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_corporateDto, 'vgph_corporate', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphc_id:number, updatevgph_corporateDto: Prisma.vgph_corporateUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_corporateDto,'vgph_corporate','update'), token);
