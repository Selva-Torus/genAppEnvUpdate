...
export class vgph_correspondent_exceptionService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphce_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createvgph_correspondent_exceptionDto: Createvgph_correspondent_exceptionDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_correspondent_exceptionDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_correspondent_exceptionDto, 'vgph_correspondent_exception', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_correspondent_exceptionDto: Createvgph_correspondent_exceptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_correspondent_exception:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_correspondent_exception');
        }
  ...
  async update(vgphce_id:number, updatevgph_correspondent_exceptionDto: Updatevgph_correspondent_exceptionDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_correspondent_exception.findFirst({
          where: {vgphce_id, trs_tenant_id: authContext.tenant },
          select: {vgphce_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_correspondent_exceptionDto,'vgph_correspondent_exception','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_correspondent_exceptionDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_correspondent_exception', {
          primaryKey: {vgphce_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphce_id:number,
    updatevgph_correspondent_exceptionDto: Updatevgph_correspondent_exceptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_correspondent_exception.findFirst({
          where: {vgphce_id },
  ...
  async remove(vgphce_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphce_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_correspondent_exception columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('vgph_correspondent_exception'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'vgph_correspondent_exception',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_correspondent_exception'));
      }
      return decryptedRes;
    } catch (error: any) {
