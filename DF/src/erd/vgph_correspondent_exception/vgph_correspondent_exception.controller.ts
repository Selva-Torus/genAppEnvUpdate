...
export class vgph_correspondent_exceptionController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_correspondent_exceptionService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphce_id') vgphce_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphce_id"] = req.headers.vgphce_id || "";
      detokenizeData["customer_name"] = req.headers.customer_name || "";
      detokenizeData["customer_cif"] = req.headers.customer_cif || "";
      detokenizeData["currency"] = req.headers.currency || "";
      detokenizeData["swift_bic"] = req.headers.swift_bic || "";
      detokenizeData["nostro_account"] = req.headers.nostro_account || "";
      detokenizeData["comment"] = req.headers.comment || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_exceptionService.findOne(+vgphce_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_correspondent_exceptionService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_exception'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_correspondent_exceptionDto: Createvgph_correspondent_exceptionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphce_id"] = req.headers.vgphce_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_exceptionService.createMaster(createvgph_correspondent_exceptionDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_correspondent_exceptionService.create(createvgph_correspondent_exceptionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_exception', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphce_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphce_id') vgphce_id:number,
    @Body() updatevgph_correspondent_exceptionDto: Updatevgph_correspondent_exceptionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphce_id"] = req.headers.vgphce_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_exceptionService.updateMaster(+vgphce_id,updatevgph_correspondent_exceptionDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_exceptionService.update(+vgphce_id,updatevgph_correspondent_exceptionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }
...
@Param('vgphce_id') vgphce_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_correspondent_exceptionService.deleteMaster(+vgphce_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_exceptionService.remove(+vgphce_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_exceptionService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_exceptionService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_correspondent_exceptionService.getLockById(key, value, token);
    return plainToInstance(vgph_correspondent_exceptionEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_correspondent_exceptionService.releaseLockById(key, value, token);
    return result;
  }
}