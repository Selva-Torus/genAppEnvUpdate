...
export class vgph_correspondent_nostroController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_correspondent_nostroService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphcn_id') vgphcn_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcn_id"] = req.headers.vgphcn_id || "";
      detokenizeData["currency"] = req.headers.currency || "";
      detokenizeData["correspondent_bank_name"] = req.headers.correspondent_bank_name || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_nostroService.findOne(+vgphcn_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_correspondent_nostroService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_nostro'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_correspondent_nostroDto: Createvgph_correspondent_nostroDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcn_id"] = req.headers.vgphcn_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_nostroService.createMaster(createvgph_correspondent_nostroDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_correspondent_nostroService.create(createvgph_correspondent_nostroDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_nostro', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphcn_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphcn_id') vgphcn_id:number,
    @Body() updatevgph_correspondent_nostroDto: Updatevgph_correspondent_nostroDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcn_id"] = req.headers.vgphcn_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_nostroService.updateMaster(+vgphcn_id,updatevgph_correspondent_nostroDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_nostroService.update(+vgphcn_id,updatevgph_correspondent_nostroDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }
...
@Param('vgphcn_id') vgphcn_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_correspondent_nostroService.deleteMaster(+vgphcn_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_nostroService.remove(+vgphcn_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_nostroService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_nostroService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_correspondent_nostroService.getLockById(key, value, token);
    return plainToInstance(vgph_correspondent_nostroEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_correspondent_nostroService.releaseLockById(key, value, token);
    return result;
  }
}