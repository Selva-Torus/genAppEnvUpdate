...
import { Createnpss_customer_proxyDto } from './dto/Createnpss_customer_proxy.dto';
import { Updatenpss_customer_proxyDto } from './dto/Updatenpss_customer_proxy.dto';
...
import { npss_customer_proxyEntity } from './entity/npss_customer_proxy.entity';
...
@Injectable()
export class npss_customer_proxyService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npsscp_id }: {npsscp_id : bigint} = queryValue;
      ...
      const { optout }: {optout : string} = queryValue;
      ...
      if(npsscp_id){ 
        query.npsscp_id = { [queryCondition['npsscp_id']]: npsscp_id };
      }
      if(product_code){ 
      ...
      if(optout){ 
        query.optout = { [queryCondition['optout']]: optout };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_customer_proxy.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_customer_proxy');
      ...
      this.prismaService.npss_customer_proxy.count({
        where: query,
      }));
  ...
  async findOne(npsscp_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_customer_proxy.findMany({ 
      where: {npsscp_id, ...tenantFilter },
      select: {npsscp_id:true,product_code:true,bankuserid:true,group_code:true,bank_code:true,customer_name:true,surname:true,mobile:true,proxy_type:true,proxy_value:true,email:true,document_id:true,exhf_id:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,operation_date:true,operation_type:true,optout:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_customer_proxy');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_customer_proxy');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_customer_proxyDto: Createnpss_customer_proxyDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_customer_proxyDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            customer_name :v.pipe(v.string(),v.maxLength(50 )) , 
            surname :v.pipe(v.string(),v.maxLength(50 )) , 
            ...
            optout :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_customer_proxyDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_customer_proxyDto, 'npss_customer_proxy', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_proxy.create({
          data: encryptedData,
          select:{npsscp_id:true,product_code:true,bankuserid:true,group_code:true,bank_code:true,customer_name:true,surname:true,mobile:true,proxy_type:true,proxy_value:true,email:true,document_id:true,exhf_id:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,operation_date:true,operation_type:true,optout:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_customer_proxyDto: Createnpss_customer_proxyDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_customer_proxy:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_customer_proxy');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_proxy creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_proxy creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_customer_proxy',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npsscp_id',
        ...
        return {
          success: true,
          message: 'npss_customer_proxy creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npsscp_id:number, updatenpss_customer_proxyDto: Updatenpss_customer_proxyDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_proxy.findFirst({
          where: {npsscp_id, trs_tenant_id: authContext.tenant },
          select: {npsscp_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          ...
          group_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
          bank_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
          customer_name :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_customer_proxyDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_customer_proxyDto,'npss_customer_proxy','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_customer_proxyDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_proxy.updateMany({
        where: {npsscp_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_customer_proxy', {
          primaryKey: {npsscp_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npsscp_id:number,
    updatenpss_customer_proxyDto: Updatenpss_customer_proxyDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npsscp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_customer_proxy.findFirst({
          where: {npsscp_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_proxy update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_proxy update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_proxy update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npsscp_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_customer_proxy.deleteMany({
      where: {npsscp_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_customer_proxy');
    let decryptedRes: any = [];
...
npsscp_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npsscp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_proxy deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_proxy deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_proxy deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_customer_proxy columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_customer_proxy'));
      }
      return decryptedRes;
    } catch (error: any) {
