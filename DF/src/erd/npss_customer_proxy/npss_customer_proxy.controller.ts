...
import { npss_customer_proxyService } from './npss_customer_proxy.service';
...
//import { CreateNpssCustomerProxyDto } from '../prisma/dto/create-npssCustomerProxy.dto';
//import { UpdateNpssCustomerProxyDto } from '../prisma/dto/update-npssCustomerProxy.dto';
...
@Controller('npss_customer_proxy')
@ApiTags('ERD API')
export class npss_customer_proxyController {
  constructor(
    private readonly npss_customer_proxyService: npss_customer_proxyService,
  ...
  @ApiOkResponse({ type: npss_customer_proxyEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_customer_proxy table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_customer_proxyService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_customer_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_customer_proxy table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_customer_proxyService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npsscp_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npsscp_id',type:Number})
  @ApiOkResponse({ type: npss_customer_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_customer_proxy table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npsscp_id') npsscp_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      ...
      detokenizeData["customer_name"] = req.headers.customer_name || "";
      ...
      detokenizeData["optout"] = req.headers.optout || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_proxyService.findOne(+npsscp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_customer_proxy table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_customer_proxyService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_customer_proxyEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_customer_proxy'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_customer_proxyDto })
  @ApiCreatedResponse({ type: npss_customer_proxyEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_customer_proxy table',
  })
  ...
  async create(
    ...
    @Body() createnpss_customer_proxyDto: Createnpss_customer_proxyDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscp_id"] = req.headers.npsscp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_customer_proxyService.createMaster(createnpss_customer_proxyDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_customer_proxyService.create(createnpss_customer_proxyDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_customer_proxy', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npsscp_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_customer_proxyDto })
  @ApiOkResponse({ type: npss_customer_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_customer_proxy table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npsscp_id') npsscp_id:number,
    @Body() updatenpss_customer_proxyDto: Updatenpss_customer_proxyDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscp_id"] = req.headers.npsscp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_customer_proxyService.updateMaster(+npsscp_id,updatenpss_customer_proxyDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_customer_proxyService.update(+npsscp_id,updatenpss_customer_proxyDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_customer_proxy table',
  })
...
@Param('npsscp_id') npsscp_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_customer_proxyService.deleteMaster(+npsscp_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_customer_proxyService.remove(+npsscp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_customer_proxy table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_proxyService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_customer_proxy table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_proxyService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_customer_proxyService.getLockById(key, value, token);
    return plainToInstance(npss_customer_proxyEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_customer_proxyService.releaseLockById(key, value, token);
    return result;
  }
}