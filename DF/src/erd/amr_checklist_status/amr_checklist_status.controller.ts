...
import { amr_checklist_statusService } from './amr_checklist_status.service';
...
//import { CreateAmrChecklistStatusDto } from '../prisma/dto/create-amrChecklistStatus.dto';
//import { UpdateAmrChecklistStatusDto } from '../prisma/dto/update-amrChecklistStatus.dto';
...
@Controller('amr_checklist_status')
@ApiTags('ERD API')
export class amr_checklist_statusController {
  constructor(
    private readonly amr_checklist_statusService: amr_checklist_statusService,
  ...
  @ApiOkResponse({ type: amr_checklist_statusEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the amr_checklist_status table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.amr_checklist_statusService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: amr_checklist_statusEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the amr_checklist_status table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.amr_checklist_statusService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':checklist_status_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'checklist_status_id',type:Number})
  @ApiOkResponse({ type: amr_checklist_statusEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the amr_checklist_status table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('checklist_status_id') checklist_status_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_status_id"] = req.headers.checklist_status_id || "";
      detokenizeData["is_complete"] = req.headers.is_complete || "";
      detokenizeData["verified_by"] = req.headers.verified_by || "";
      detokenizeData["verified_timestamp"] = req.headers.verified_timestamp || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.amr_checklist_statusService.findOne(+checklist_status_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the amr_checklist_status table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('amr_checklist_status'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createamr_checklist_statusDto })
  @ApiCreatedResponse({ type: amr_checklist_statusEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the amr_checklist_status table',
  })
  ...
  async create(
    ...
    @Body() createamr_checklist_statusDto: Createamr_checklist_statusDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_status_id"] = req.headers.checklist_status_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.amr_checklist_statusService.createMaster(createamr_checklist_statusDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.amr_checklist_statusService.create(createamr_checklist_statusDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(amr_checklist_statusEntity, result);
  }
 
  @Patch(':checklist_status_id')
  ...
  @UsePipes(new PrismaModelValidationPipe('amr_checklist_status', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'checklist_status_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updateamr_checklist_statusDto })
  @ApiOkResponse({ type: amr_checklist_statusEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the amr_checklist_status table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('checklist_status_id') checklist_status_id:number,
    @Body() updateamr_checklist_statusDto: Updateamr_checklist_statusDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_status_id"] = req.headers.checklist_status_id || "";
    ...
    const result = await this.amr_checklist_statusService.update(+checklist_status_id,updateamr_checklist_statusDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(amr_checklist_statusEntity, result);
  }
 
  @Delete(':checklist_status_id')
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'checklist_status_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the amr_checklist_status table',
  })
...
@Param('checklist_status_id') checklist_status_id:number,
    ...
    const result = await this.amr_checklist_statusService.remove(+checklist_status_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(amr_checklist_statusEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the amr_checklist_status table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the amr_checklist_status table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.amr_checklist_statusService.getLockById(key, value, token);
    return plainToInstance(amr_checklist_statusEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.amr_checklist_statusService.releaseLockById(key, value, token);
    return result;
  }
}