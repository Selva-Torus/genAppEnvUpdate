...
@Injectable()
export class amr_checklist_statusService {
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            ...
            trs_token_id:"string",
            trs_version:"string",
      checklist_item_id :"number",      account_id :"number",    }
    return data;
  }
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
    try {
      ...
      let query: any = {}; 
      const { checklist_status_id }: {checklist_status_id : bigint} = queryValue;
      const { is_complete }: {is_complete : Date} = queryValue;
      const { verified_by }: {verified_by : string} = queryValue;
      const { verified_timestamp }: {verified_timestamp :  Date} = queryValue;
      ...
      if(checklist_status_id){ 
        query.checklist_status_id = { [queryCondition['checklist_status_id']]: checklist_status_id };
      }
      if(is_complete){ 
        query.is_complete = { [queryCondition['is_complete']]: is_complete };
      }
      if(verified_by){ 
        query.verified_by = { [queryCondition['verified_by']]: verified_by };
      }
      if(verified_timestamp){ 
        query.verified_timestamp = { [queryCondition['verified_timestamp']]: verified_timestamp };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'amr_checklist_status');
      ...
      this.prismaService.amr_checklist_status.count({
        where: query,
      }));
  ...
  async findOne(checklist_status_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      select: {checklist_status_id:true,is_complete:true,verified_by:true,verified_timestamp:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,checklist_item_id :true,account_id :true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'amr_checklist_status');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'amr_checklist_status');
      decryptedRes.push(decryptedData);
    }
  ...
  async findAll(token : string,authContext?: any,detokenize?: string,detokenizeData?: any
    ...
    try{
      ...
      take: MAX_PAGE_SIZE,
      select: {checklist_status_id:true,is_complete:true,verified_by:true,verified_timestamp:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,          checklist_item_id :true,    
          account_id :true,    
      }
      }));
      let decryptedRes: any = [];
  ...
  async create(createamr_checklist_statusDto: Prisma.amr_checklist_statusCreateInput,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            verified_timestamp :  v.optional((v.any())), 
      ...
      let validate : any = v.safeParse(dataSchema,createamr_checklist_statusDto);
      if (!validate.success) {
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.amr_checklist_status.create({
          data: encryptedData,
          select:{checklist_status_id:true,is_complete:true,verified_by:true,verified_timestamp:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,checklist_item_id :true,account_id :true,}          
        })
      );
  ...
  async createMaster(
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'amr_checklist_status:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for amr_checklist_status');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'amr_checklist_status creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'amr_checklist_status creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'amr_checklist_status',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'checklist_status_id',
        ...
        return {
          success: true,
          message: 'amr_checklist_status creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(checklist_status_id:number, updateamr_checklist_statusDto: Prisma.amr_checklist_statusUpdateInput,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateamr_checklist_statusDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
    ...
    const currentVersion = (updateamr_checklist_statusDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.amr_checklist_status.updateMany({
        where: {checklist_status_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'amr_checklist_status', {
...
checklist_status_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (!(await this.ufservice.hasCapability(token, 'amr_checklist_status:AUTHORIZE'))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for amr_checklist_status');
        }
        ...
        if (success) {
          return {
            success: true,
            message: 'amr_checklist_status update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'amr_checklist_status update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'amr_checklist_status update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(checklist_status_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.amr_checklist_status.deleteMany({
      where: {checklist_status_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'amr_checklist_status');
    let decryptedRes: any = [];
...
checklist_status_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'amr_checklist_status deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'amr_checklist_status deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'amr_checklist_status deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real amr_checklist_status columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'amr_checklist_status'));
      }
      return decryptedRes;
    } catch (error: any) {
