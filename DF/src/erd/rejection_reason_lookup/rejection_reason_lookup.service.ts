...
@Injectable()
export class rejection_reason_lookupService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { reason_id }: {reason_id : bigint} = queryValue;
      const { reason_text }: {reason_text : string} = queryValue;
      ...
      if(reason_id){ 
        query.reason_id = { [queryCondition['reason_id']]: reason_id };
      }
      if(reason_text){ 
        query.reason_text = { [queryCondition['reason_text']]: reason_text };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.rejection_reason_lookup.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'rejection_reason_lookup');
      ...
      this.prismaService.rejection_reason_lookup.count({
        where: query,
      }));
  ...
  async findOne(reason_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'rejection_reason_lookup');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'rejection_reason_lookup');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createrejection_reason_lookupDto: Createrejection_reason_lookupDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      const dataSchema:any =  v.object({
            reason_text :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createrejection_reason_lookupDto);
      if (!validate.success) {
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.rejection_reason_lookup.create({
          data: encryptedData,
          select:{reason_id:true,reason_text:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,amr_review_sessions:true,}          
        })
      );
   ...
   */
  async createMaster(
    createrejection_reason_lookupDto: Createrejection_reason_lookupDto,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'rejection_reason_lookup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'rejection_reason_lookup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'rejection_reason_lookup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'reason_id',
        ...
        return {
          success: true,
          message: 'rejection_reason_lookup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(reason_id:number, updaterejection_reason_lookupDto: Updaterejection_reason_lookupDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.rejection_reason_lookup.findFirst({
          where: {reason_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updaterejection_reason_lookupDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
    ...
    const currentVersion = (updaterejection_reason_lookupDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.rejection_reason_lookup.updateMany({
        where: {reason_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'rejection_reason_lookup', {
...
/**
  ...
  async updateMaster(
reason_id:number,
    updaterejection_reason_lookupDto: Updaterejection_reason_lookupDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.rejection_reason_lookup.findUnique({
          where: { reason_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'rejection_reason_lookup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'rejection_reason_lookup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'rejection_reason_lookup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(reason_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.rejection_reason_lookup.deleteMany({
      where: {reason_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'rejection_reason_lookup');
    let decryptedRes: any = [];
...
reason_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.rejection_reason_lookup.findUnique({
          where: { reason_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'rejection_reason_lookup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'rejection_reason_lookup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'rejection_reason_lookup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real rejection_reason_lookup columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('rejection_reason_lookup'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'rejection_reason_lookup',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'rejection_reason_lookup'));
      }
      return decryptedRes;
    } catch (error: any) {
