...
//import { CreateCreditorsDto } from '../prisma/dto/create-creditors.dto';
//import { UpdateCreditorsDto } from '../prisma/dto/update-creditors.dto';
...
export class creditorsController {
  ...
  @ApiOkResponse({ type: creditorsEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the creditors table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.creditorsService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: creditorsEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the creditors table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.creditorsService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':creditor_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'creditor_id',type:Number})
  @ApiOkResponse({ type: creditorsEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the creditors table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('creditor_id') creditor_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["creditor_id"] = req.headers.creditor_id || "";
      detokenizeData["creditor_name"] = req.headers.creditor_name || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.creditorsService.findOne(+creditor_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the creditors table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('creditors'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: CreatecreditorsDto })
  @ApiCreatedResponse({ type: creditorsEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the creditors table',
  })
  ...
  async create(
    ...
    @Body() createcreditorsDto: CreatecreditorsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["creditor_id"] = req.headers.creditor_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.creditorsService.createMaster(createcreditorsDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.creditorsService.create(createcreditorsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(creditorsEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('creditors', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'creditor_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: UpdatecreditorsDto })
  @ApiOkResponse({ type: creditorsEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the creditors table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('creditor_id') creditor_id:number,
    @Body() updatecreditorsDto: UpdatecreditorsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["creditor_id"] = req.headers.creditor_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.creditorsService.updateMaster(+creditor_id,updatecreditorsDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.creditorsService.update(+creditor_id,updatecreditorsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(creditorsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the creditors table',
  })
...
@Param('creditor_id') creditor_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.creditorsService.deleteMaster(+creditor_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.creditorsService.remove(+creditor_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(creditorsEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the creditors table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the creditors table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.creditorsService.getLockById(key, value, token);
    return plainToInstance(creditorsEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.creditorsService.releaseLockById(key, value, token);
    return result;
  }
}