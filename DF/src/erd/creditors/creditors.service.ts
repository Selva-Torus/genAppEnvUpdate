...
import { CreatecreditorsDto } from './dto/Createcreditors.dto';
import { UpdatecreditorsDto } from './dto/Updatecreditors.dto';
...
import { creditorsEntity } from './entity/creditors.entity';
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { creditor_id }: {creditor_id : bigint} = queryValue;
      const { creditor_name }: {creditor_name : string} = queryValue;
      ...
      if(creditor_id){ 
        query.creditor_id = { [queryCondition['creditor_id']]: creditor_id };
      }
      if(creditor_name){ 
        query.creditor_name = { [queryCondition['creditor_name']]: creditor_name };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.creditors.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'creditors');
      ...
      this.prismaService.creditors.count({
        where: query,
      }));
  ...
  async findOne(creditor_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'creditors');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'creditors');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createcreditorsDto: CreatecreditorsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createcreditorsDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            creditor_name :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createcreditorsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createcreditorsDto, 'creditors', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.creditors.create({
          data: encryptedData,
   ...
   */
  async createMaster(
    createcreditorsDto: CreatecreditorsDto,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'creditors creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'creditors creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'creditors',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'creditor_id',
        ...
        return {
          success: true,
          message: 'creditors creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(creditor_id:number, updatecreditorsDto: UpdatecreditorsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.creditors.findFirst({
          where: {creditor_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updatecreditorsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatecreditorsDto,'creditors','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'creditors', {
          primaryKey: {creditor_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
creditor_id:number,
    updatecreditorsDto: UpdatecreditorsDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =creditor_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.creditors.findUnique({
          where: { creditor_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'creditors update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'creditors update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'creditors update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(creditor_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.creditors.deleteMany({
      where: {creditor_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'creditors');
    let decryptedRes: any = [];
...
creditor_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =creditor_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.creditors.findUnique({
          where: { creditor_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'creditors deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'creditors deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'creditors deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real creditors columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('creditors'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'creditors'));
      }
      return decryptedRes;
    } catch (error: any) {
