...
export class vgph_participant_bankController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_participant_bankService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphpb_id') vgphpb_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpb_id"] = req.headers.vgphpb_id || "";
      ...
      detokenizeData["clearing_sort_code"] = req.headers.clearing_sort_code || "";
      detokenizeData["bic_code"] = req.headers.bic_code || "";
      detokenizeData["iban_code"] = req.headers.iban_code || "";
      ...
      detokenizeData["contact_no"] = req.headers.contact_no || "";
      detokenizeData["is_small_bank"] = req.headers.is_small_bank || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_participant_bankService.findOne(+vgphpb_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_participant_bankService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_participant_bankEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_participant_bank'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_participant_bankDto: Createvgph_participant_bankDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpb_id"] = req.headers.vgphpb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_participant_bankService.createMaster(createvgph_participant_bankDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_participant_bankService.create(createvgph_participant_bankDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_participant_bankEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_participant_bank', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphpb_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphpb_id') vgphpb_id:number,
    @Body() updatevgph_participant_bankDto: Updatevgph_participant_bankDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpb_id"] = req.headers.vgphpb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_participant_bankService.updateMaster(+vgphpb_id,updatevgph_participant_bankDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_participant_bankService.update(+vgphpb_id,updatevgph_participant_bankDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_participant_bankEntity, result);
  }
...
@Param('vgphpb_id') vgphpb_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_participant_bankService.deleteMaster(+vgphpb_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_participant_bankService.remove(+vgphpb_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_participant_bankEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_participant_bankService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_participant_bankEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_participant_bankService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_participant_bankEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_participant_bankService.getLockById(key, value, token);
    return plainToInstance(vgph_participant_bankEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_participant_bankService.releaseLockById(key, value, token);
    return result;
  }
}