...
@Injectable()
export class dynamic_tableService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { rtp_identifier_id }: {rtp_identifier_id : number} = queryValue;
      const { rtp_identifier }: {rtp_identifier : any } = queryValue;
      ...
      if(rtp_identifier_id){ 
        query.rtp_identifier_id = { [queryCondition['rtp_identifier_id']]: rtp_identifier_id };
      }
      if(rtp_identifier){ 
        query.rtp_identifier = { [queryCondition['rtp_identifier']]: rtp_identifier };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.dynamic_table.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'dynamic_table');
      ...
      this.prismaService.dynamic_table.count({
        where: query,
      }));
  ...
  async findOne(rtp_identifier_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createdynamic_tableDto: Createdynamic_tableDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createdynamic_tableDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            rtp_identifier :  v.optional(v.any() ), 
      ...
      let validate : any = v.safeParse(dataSchema,createdynamic_tableDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createdynamic_tableDto, 'dynamic_table', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.dynamic_table.create({
          data: encryptedData,
          select:{rtp_identifier_id:true,rtp_identifier:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createdynamic_tableDto: Createdynamic_tableDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'dynamic_table:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for dynamic_table');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'dynamic_table creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'dynamic_table creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        return {
          success: true,
          message: 'dynamic_table creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(rtp_identifier_id:number, updatedynamic_tableDto: Updatedynamic_tableDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.dynamic_table.findFirst({
          where: {rtp_identifier_id, trs_tenant_id: authContext.tenant },
          select: {rtp_identifier_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatedynamic_tableDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatedynamic_tableDto,'dynamic_table','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatedynamic_tableDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.dynamic_table.updateMany({
        where: {rtp_identifier_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'dynamic_table', {
          primaryKey: {rtp_identifier_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
rtp_identifier_id:number,
    updatedynamic_tableDto: Updatedynamic_tableDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =rtp_identifier_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.dynamic_table.findFirst({
          where: {rtp_identifier_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'dynamic_table update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'dynamic_table update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'dynamic_table update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(rtp_identifier_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.dynamic_table.deleteMany({
      where: {rtp_identifier_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'dynamic_table');
    let decryptedRes: any = [];
...
rtp_identifier_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =rtp_identifier_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'dynamic_table deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'dynamic_table deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'dynamic_table deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real dynamic_table columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('dynamic_table'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'dynamic_table'));
      }
      return decryptedRes;
    } catch (error: any) {
