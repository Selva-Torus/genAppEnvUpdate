 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
  ...
  async create(createvgph_nf_tran_mainDto: Prisma.vgph_nf_tran_mainCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_nf_tran_mainDto, 'vgph_nf_tran_main', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(uuid: string, updatevgph_nf_tran_mainDto: Prisma.vgph_nf_tran_mainUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_nf_tran_mainDto,'vgph_nf_tran_main','update'), token);
