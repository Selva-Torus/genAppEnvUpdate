...
import { Createnpss_merchant_proxyDto } from './dto/Createnpss_merchant_proxy.dto';
import { Updatenpss_merchant_proxyDto } from './dto/Updatenpss_merchant_proxy.dto';
...
import { npss_merchant_proxyEntity } from './entity/npss_merchant_proxy.entity';
...
@Injectable()
export class npss_merchant_proxyService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npssmp_id }: {npssmp_id : bigint} = queryValue;
      const { product_code }: {product_code : string} = queryValue;
      const { bankuserid }: {bankuserid : string} = queryValue;
      const { group_code }: {group_code : string} = queryValue;
      const { bank_code }: {bank_code : string} = queryValue;
      const { merchant_tag }: {merchant_tag : string} = queryValue;
      const { merchant_name }: {merchant_name : string} = queryValue;
      const { surname }: {surname : string} = queryValue;
      const { denomination }: {denomination : string} = queryValue;
      const { vat_number }: {vat_number : string} = queryValue;
      const { mcc }: {mcc : string} = queryValue;
      const { mobile }: {mobile : string} = queryValue;
      const { proxy_type }: {proxy_type : string} = queryValue;
      const { proxy_value }: {proxy_value : string} = queryValue;
      const { email }: {email : string} = queryValue;
      const { document_id }: {document_id : string} = queryValue;
      const { channel_name }: {channel_name : string} = queryValue;
      const { logo }: {logo : string} = queryValue;
      const { exhf_id }: {exhf_id : number} = queryValue;
      const { channel_id }: {channel_id : string} = queryValue;
      const { channel_refno }: {channel_refno : string} = queryValue;
      const { channel_userid }: {channel_userid : string} = queryValue;
      const { channel_product }: {channel_product : string} = queryValue;
      const { channel_sub_product }: {channel_sub_product : string} = queryValue;
      const { channel_tran_code }: {channel_tran_code : string} = queryValue;
      const { operation_date }: {operation_date : string} = queryValue;
      const { operation_type }: {operation_type : string} = queryValue;
      ...
      if(npssmp_id){ 
        query.npssmp_id = { [queryCondition['npssmp_id']]: npssmp_id };
      }
      if(product_code){ 
        query.product_code = { [queryCondition['product_code']]: product_code };
      }
      if(bankuserid){ 
        query.bankuserid = { [queryCondition['bankuserid']]: bankuserid };
      }
      if(group_code){ 
        query.group_code = { [queryCondition['group_code']]: group_code };
      }
      if(bank_code){ 
        query.bank_code = { [queryCondition['bank_code']]: bank_code };
      }
      if(merchant_tag){ 
        query.merchant_tag = { [queryCondition['merchant_tag']]: merchant_tag };
      }
      if(merchant_name){ 
        query.merchant_name = { [queryCondition['merchant_name']]: merchant_name };
      }
      if(surname){ 
        query.surname = { [queryCondition['surname']]: surname };
      }
      if(denomination){ 
        query.denomination = { [queryCondition['denomination']]: denomination };
      }
      if(vat_number){ 
        query.vat_number = { [queryCondition['vat_number']]: vat_number };
      }
      if(mcc){ 
        query.mcc = { [queryCondition['mcc']]: mcc };
      }
      if(mobile){ 
        query.mobile = { [queryCondition['mobile']]: mobile };
      }
      if(proxy_type){ 
        query.proxy_type = { [queryCondition['proxy_type']]: proxy_type };
      }
      if(proxy_value){ 
        query.proxy_value = { [queryCondition['proxy_value']]: proxy_value };
      }
      if(email){ 
        query.email = { [queryCondition['email']]: email };
      }
      if(document_id){ 
        query.document_id = { [queryCondition['document_id']]: document_id };
      }
      if(channel_name){ 
        query.channel_name = { [queryCondition['channel_name']]: channel_name };
      }
      if(logo){ 
        query.logo = { [queryCondition['logo']]: logo };
      }
      if(exhf_id){ 
        query.exhf_id = { [queryCondition['exhf_id']]: exhf_id };
      }
      if(channel_id){ 
        query.channel_id = { [queryCondition['channel_id']]: channel_id };
      }
      if(channel_refno){ 
        query.channel_refno = { [queryCondition['channel_refno']]: channel_refno };
      }
      if(channel_userid){ 
        query.channel_userid = { [queryCondition['channel_userid']]: channel_userid };
      }
      if(channel_product){ 
        query.channel_product = { [queryCondition['channel_product']]: channel_product };
      }
      if(channel_sub_product){ 
        query.channel_sub_product = { [queryCondition['channel_sub_product']]: channel_sub_product };
      }
      if(channel_tran_code){ 
        query.channel_tran_code = { [queryCondition['channel_tran_code']]: channel_tran_code };
      }
      if(operation_date){ 
        query.operation_date = { [queryCondition['operation_date']]: operation_date };
      }
      if(operation_type){ 
        query.operation_type = { [queryCondition['operation_type']]: operation_type };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_merchant_proxy.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_merchant_proxy');
      ...
      this.prismaService.npss_merchant_proxy.count({
        where: query,
      }));
  ...
  async findOne(npssmp_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_merchant_proxy.findMany({ 
      where: {npssmp_id, ...tenantFilter },
      select: {npssmp_id:true,product_code:true,bankuserid:true,group_code:true,bank_code:true,merchant_tag:true,merchant_name:true,surname:true,denomination:true,vat_number:true,mcc:true,mobile:true,proxy_type:true,proxy_value:true,email:true,document_id:true,channel_name:true,logo:true,exhf_id:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,operation_date:true,operation_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_proxy');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_merchant_proxy');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_merchant_proxyDto: Createnpss_merchant_proxyDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_merchant_proxyDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            product_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            bankuserid :v.pipe(v.string(),v.maxLength(16 )) , 
            group_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
            bank_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
            merchant_tag :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
            merchant_name :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            surname :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            denomination :v.pipe(v.string(),v.maxLength(100 )) , 
            vat_number :v.pipe(v.string(),v.maxLength(20 )) , 
            mcc :v.pipe(v.string(),v.maxLength(4 )) , 
            mobile :v.pipe(v.string(),v.maxLength(30 )) , 
            proxy_type :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            proxy_value :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            email :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            document_id :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
            channel_name :  v.optional(v.pipe(v.string(),v.maxLength(100 ))), 
            logo :  v.optional(v.pipe(v.string(),v.maxLength(4000 ))), 
            exhf_id :  v.optional(v.number()), 
            channel_id :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            channel_refno :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            channel_userid :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            channel_product :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            channel_sub_product :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            channel_tran_code :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            operation_date :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            operation_type :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_merchant_proxyDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_merchant_proxyDto, 'npss_merchant_proxy', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_proxy.create({
          data: encryptedData,
          select:{npssmp_id:true,product_code:true,bankuserid:true,group_code:true,bank_code:true,merchant_tag:true,merchant_name:true,surname:true,denomination:true,vat_number:true,mcc:true,mobile:true,proxy_type:true,proxy_value:true,email:true,document_id:true,channel_name:true,logo:true,exhf_id:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,operation_date:true,operation_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_merchant_proxyDto: Createnpss_merchant_proxyDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_merchant_proxy:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_merchant_proxy');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_proxy creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_proxy creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_merchant_proxy',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npssmp_id',
        ...
        return {
          success: true,
          message: 'npss_merchant_proxy creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npssmp_id:number, updatenpss_merchant_proxyDto: Updatenpss_merchant_proxyDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_proxy.findFirst({
          where: {npssmp_id, trs_tenant_id: authContext.tenant },
          select: {npssmp_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          product_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          bankuserid :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          group_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
          bank_code :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
          merchant_tag :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
          merchant_name :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          surname :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          denomination :  v.optional(v.pipe(v.string(),v.maxLength(100 ))), 
          vat_number :  v.optional(v.pipe(v.string(),v.maxLength(20 ))), 
          mcc :  v.optional(v.pipe(v.string(),v.maxLength(4 ))), 
          mobile :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
          proxy_type :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          proxy_value :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          email :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
          document_id :  v.optional(v.pipe(v.string(),v.maxLength(50 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_merchant_proxyDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_merchant_proxyDto,'npss_merchant_proxy','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_merchant_proxyDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_proxy.updateMany({
        where: {npssmp_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_merchant_proxy', {
          primaryKey: {npssmp_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npssmp_id:number,
    updatenpss_merchant_proxyDto: Updatenpss_merchant_proxyDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npssmp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_merchant_proxy.findFirst({
          where: {npssmp_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_proxy update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_proxy update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_proxy update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npssmp_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_merchant_proxy.deleteMany({
      where: {npssmp_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_proxy');
    let decryptedRes: any = [];
...
npssmp_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npssmp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_proxy deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_proxy deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_proxy deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_merchant_proxy columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_merchant_proxy'));
      }
      return decryptedRes;
    } catch (error: any) {
