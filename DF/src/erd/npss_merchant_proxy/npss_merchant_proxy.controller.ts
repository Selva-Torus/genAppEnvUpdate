...
import { npss_merchant_proxyService } from './npss_merchant_proxy.service';
...
//import { CreateNpssMerchantProxyDto } from '../prisma/dto/create-npssMerchantProxy.dto';
//import { UpdateNpssMerchantProxyDto } from '../prisma/dto/update-npssMerchantProxy.dto';
...
@Controller('npss_merchant_proxy')
@ApiTags('ERD API')
export class npss_merchant_proxyController {
  constructor(
    private readonly npss_merchant_proxyService: npss_merchant_proxyService,
  ...
  @ApiOkResponse({ type: npss_merchant_proxyEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_merchant_proxy table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_merchant_proxyService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_merchant_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_merchant_proxy table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_merchant_proxyService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npssmp_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npssmp_id',type:Number})
  @ApiOkResponse({ type: npss_merchant_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_merchant_proxy table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npssmp_id') npssmp_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      ...
      detokenizeData["group_code"] = req.headers.group_code || "";
      detokenizeData["bank_code"] = req.headers.bank_code || "";
      detokenizeData["merchant_tag"] = req.headers.merchant_tag || "";
      detokenizeData["merchant_name"] = req.headers.merchant_name || "";
      detokenizeData["surname"] = req.headers.surname || "";
      detokenizeData["denomination"] = req.headers.denomination || "";
      detokenizeData["vat_number"] = req.headers.vat_number || "";
      detokenizeData["mcc"] = req.headers.mcc || "";
      detokenizeData["mobile"] = req.headers.mobile || "";
      detokenizeData["proxy_type"] = req.headers.proxy_type || "";
      detokenizeData["proxy_value"] = req.headers.proxy_value || "";
      detokenizeData["email"] = req.headers.email || "";
      detokenizeData["document_id"] = req.headers.document_id || "";
      detokenizeData["channel_name"] = req.headers.channel_name || "";
      detokenizeData["logo"] = req.headers.logo || "";
      ...
      detokenizeData["operation_date"] = req.headers.operation_date || "";
      detokenizeData["operation_type"] = req.headers.operation_type || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_proxyService.findOne(+npssmp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_merchant_proxy table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_merchant_proxyService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_merchant_proxyEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_proxy'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_merchant_proxyDto })
  @ApiCreatedResponse({ type: npss_merchant_proxyEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_merchant_proxy table',
  })
  ...
  async create(
    ...
    @Body() createnpss_merchant_proxyDto: Createnpss_merchant_proxyDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssmp_id"] = req.headers.npssmp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_proxyService.createMaster(createnpss_merchant_proxyDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_merchant_proxyService.create(createnpss_merchant_proxyDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_proxy', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npssmp_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_merchant_proxyDto })
  @ApiOkResponse({ type: npss_merchant_proxyEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_merchant_proxy table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npssmp_id') npssmp_id:number,
    @Body() updatenpss_merchant_proxyDto: Updatenpss_merchant_proxyDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssmp_id"] = req.headers.npssmp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_proxyService.updateMaster(+npssmp_id,updatenpss_merchant_proxyDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_proxyService.update(+npssmp_id,updatenpss_merchant_proxyDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_merchant_proxy table',
  })
...
@Param('npssmp_id') npssmp_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_merchant_proxyService.deleteMaster(+npssmp_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_proxyService.remove(+npssmp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_merchant_proxy table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_proxyService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_merchant_proxy table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_proxyService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_merchant_proxyService.getLockById(key, value, token);
    return plainToInstance(npss_merchant_proxyEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_merchant_proxyService.releaseLockById(key, value, token);
    return result;
  }
}