...
import { checklist_itemsService } from './checklist_items.service';
...
//import { CreateChecklistItemsDto } from '../prisma/dto/create-checklistItems.dto';
//import { UpdateChecklistItemsDto } from '../prisma/dto/update-checklistItems.dto';
...
@ApiTags('ERD API')
export class checklist_itemsController {
  constructor(
    private readonly checklist_itemsService: checklist_itemsService,
  ...
  @ApiOkResponse({ type: checklist_itemsEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the checklist_items table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.checklist_itemsService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: checklist_itemsEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the checklist_items table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.checklist_itemsService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':checklist_item_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'checklist_item_id',type:Number})
  @ApiOkResponse({ type: checklist_itemsEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the checklist_items table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('checklist_item_id') checklist_item_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_item_id"] = req.headers.checklist_item_id || "";
      detokenizeData["item_name"] = req.headers.item_name || "";
      detokenizeData["category_label"] = req.headers.category_label || "";
      detokenizeData["default_verification_type"] = req.headers.default_verification_type || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.checklist_itemsService.findOne(+checklist_item_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the checklist_items table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('checklist_items'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createchecklist_itemsDto })
  @ApiCreatedResponse({ type: checklist_itemsEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the checklist_items table',
  })
  ...
  async create(
    ...
    @Body() createchecklist_itemsDto: Createchecklist_itemsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_item_id"] = req.headers.checklist_item_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.checklist_itemsService.createMaster(createchecklist_itemsDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.checklist_itemsService.create(createchecklist_itemsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(checklist_itemsEntity, result);
  }
 
  @Patch(':checklist_item_id')
  ...
  @UsePipes(new PrismaModelValidationPipe('checklist_items', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'checklist_item_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatechecklist_itemsDto })
  @ApiOkResponse({ type: checklist_itemsEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the checklist_items table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('checklist_item_id') checklist_item_id:number,
    @Body() updatechecklist_itemsDto: Updatechecklist_itemsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["checklist_item_id"] = req.headers.checklist_item_id || "";
    ...
    const result = await this.checklist_itemsService.update(+checklist_item_id,updatechecklist_itemsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(checklist_itemsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the checklist_items table',
  })
...
@Param('checklist_item_id') checklist_item_id:number,
    ...
    const result = await this.checklist_itemsService.remove(+checklist_item_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(checklist_itemsEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the checklist_items table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the checklist_items table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.checklist_itemsService.getLockById(key, value, token);
    return plainToInstance(checklist_itemsEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.checklist_itemsService.releaseLockById(key, value, token);
    return result;
  }
}