...
import { Createchecklist_itemsDto } from './dto/Createchecklist_items.dto';
import { Updatechecklist_itemsDto } from './dto/Updatechecklist_items.dto';
...
@Injectable()
export class checklist_itemsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { checklist_item_id }: {checklist_item_id : bigint} = queryValue;
      const { item_name }: {item_name : string} = queryValue;
      const { category_label }: {category_label : string} = queryValue;
      const { default_verification_type }: {default_verification_type : string} = queryValue;
      ...
      if(checklist_item_id){ 
        query.checklist_item_id = { [queryCondition['checklist_item_id']]: checklist_item_id };
      }
      if(item_name){ 
        query.item_name = { [queryCondition['item_name']]: item_name };
      }
      if(category_label){ 
        query.category_label = { [queryCondition['category_label']]: category_label };
      }
      if(default_verification_type){ 
        query.default_verification_type = { [queryCondition['default_verification_type']]: default_verification_type };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.checklist_items.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'checklist_items');
      ...
      this.prismaService.checklist_items.count({
        where: query,
      }));
  ...
  async findOne(checklist_item_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.checklist_items.findMany({ 
      where: {checklist_item_id, ...tenantFilter },
      select: {checklist_item_id:true,item_name:true,category_label:true,default_verification_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            amr_checklist_status:{
              select:{
              checklist_status_id:true,              is_complete:true,              verified_by:true,              verified_timestamp:true,              trs_created_date:true,              trs_created_by:true,              trs_modified_date:true,              trs_modified_by:true,              trs_process_id:true,              trs_access_profile:true,              trs_org_grp_code:true,              trs_org_code:true,              trs_role_grp_code:true,              trs_role_code:true,              trs_ps_grp_code:true,              trs_ps_code:true,              trs_sub_org_grp_code:true,              trs_sub_org_code:true,              trs_locked_by:true,              trs_locked_time:true,              trs_tenant_id:true,              trs_app_code:true,              trs_product_code:true,              trs_event_process_status:true,              trs_event_status:true,              trs_token_id:true,              trs_version:true            ,
              }
            },
        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'checklist_items');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'checklist_items');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createchecklist_itemsDto: Createchecklist_itemsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            category_label :  v.optional(v.string()), 
            default_verification_type :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createchecklist_itemsDto);
      if (!validate.success) {
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.checklist_items.create({
          data: encryptedData,
          select:{checklist_item_id:true,item_name:true,category_label:true,default_verification_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,amr_checklist_status:true,}          
        })
      );
   ...
   */
  async createMaster(
    createchecklist_itemsDto: Createchecklist_itemsDto,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'checklist_items creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'checklist_items creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'checklist_items',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'checklist_item_id',
        ...
        return {
          success: true,
          message: 'checklist_items creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(checklist_item_id:number, updatechecklist_itemsDto: Updatechecklist_itemsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.checklist_items.findFirst({
          where: {checklist_item_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updatechecklist_itemsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
    ...
    const currentVersion = (updatechecklist_itemsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.checklist_items.updateMany({
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'checklist_items', {
...
checklist_item_id:number,
    updatechecklist_itemsDto: Updatechecklist_itemsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.checklist_items.findUnique({
          where: { checklist_item_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'checklist_items update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'checklist_items update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'checklist_items update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(checklist_item_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.checklist_items.deleteMany({
      where: {checklist_item_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'checklist_items');
    let decryptedRes: any = [];
...
checklist_item_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.checklist_items.findUnique({
          where: { checklist_item_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'checklist_items deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'checklist_items deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'checklist_items deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real checklist_items columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('checklist_items'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'checklist_items'));
      }
      return decryptedRes;
    } catch (error: any) {
