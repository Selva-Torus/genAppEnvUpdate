...
import { test_erdService } from './test_erd.service';
...
//import { CreateTestErdDto } from '../prisma/dto/create-testErd.dto';
//import { UpdateTestErdDto } from '../prisma/dto/update-testErd.dto';
...
@Controller('test_erd')
@ApiTags('ERD API')
export class test_erdController {
  constructor(
    private readonly test_erdService: test_erdService,
  ...
  @ApiOkResponse({ type: test_erdEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the test_erd table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.test_erdService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: test_erdEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the test_erd table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.test_erdService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read only one records from the test_erd table',
  })
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('id') id:number,@Req() req: any) {
    ...
    let detokenizeData = {};
      ...
      detokenizeData["emp_id"] = req.headers.emp_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.test_erdService.findOne(+id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the test_erd table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.test_erdService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(test_erdEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('test_erd'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createtest_erdDto })
  @ApiCreatedResponse({ type: test_erdEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the test_erd table',
  })
  ...
  async create(
    ...
    @Body() createtest_erdDto: Createtest_erdDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["id"] = req.headers.id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.test_erdService.createMaster(createtest_erdDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.test_erdService.create(createtest_erdDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('test_erd', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatetest_erdDto })
  @ApiOkResponse({ type: test_erdEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the test_erd table',
  })
  ...
  async update(
    ...
    @Headers() authHeader: string,
@Param('id') id:number,
    @Body() updatetest_erdDto: Updatetest_erdDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["id"] = req.headers.id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.test_erdService.updateMaster(+id,updatetest_erdDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.test_erdService.update(+id,updatetest_erdDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the test_erd table',
  })
...
@Param('id') id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.test_erdService.deleteMaster(+id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.test_erdService.remove(+id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the test_erd table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.test_erdService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the test_erd table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.test_erdService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.test_erdService.getLockById(key, value, token);
    return plainToInstance(test_erdEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.test_erdService.releaseLockById(key, value, token);
    return result;
  }
}