...
import { Createtest_erdDto } from './dto/Createtest_erd.dto';
import { Updatetest_erdDto } from './dto/Updatetest_erd.dto';
...
import { test_erdEntity } from './entity/test_erd.entity';
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { id }: {id : bigint} = queryValue;
      const { emp_id }: {emp_id : number} = queryValue;
      ...
      if(emp_id){ 
        query.emp_id = { [queryCondition['emp_id']]: emp_id };
      }
      if(name){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.test_erd.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'test_erd');
      ...
      this.prismaService.test_erd.count({
        where: query,
      }));
  ...
  async create(createtest_erdDto: Createtest_erdDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createtest_erdDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            emp_id :  v.optional(v.number()), 
      ...
      let validate : any = v.safeParse(dataSchema,createtest_erdDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createtest_erdDto, 'test_erd', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.test_erd.create({
          data: encryptedData,
          select:{id:true,emp_id:true,name:true,email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createtest_erdDto: Createtest_erdDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'test_erd:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for test_erd');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'test_erd creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'test_erd creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        return {
          success: true,
          message: 'test_erd creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(id:number, updatetest_erdDto: Updatetest_erdDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.test_erd.findFirst({
      ...
      let validate : any = v.safeParse(dataSchema,updatetest_erdDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatetest_erdDto,'test_erd','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'test_erd', {
          primaryKey: {id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
id:number,
    updatetest_erdDto: Updatetest_erdDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.test_erd.findFirst({
          where: {id },
        ...
        if (success) {
          return {
            success: true,
            message: 'test_erd update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'test_erd update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'test_erd update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.test_erd.deleteMany({
      where: {id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'test_erd');
    let decryptedRes: any = [];
...
id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'test_erd deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'test_erd deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'test_erd deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real test_erd columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('test_erd'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'test_erd'));
      }
      return decryptedRes;
    } catch (error: any) {
