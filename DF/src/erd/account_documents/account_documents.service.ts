...
@Injectable()
export class account_documentsService {
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            doc_instance_id:"bigint",
            doc_status:"string",
            doc_reference_url:"string[]",
            ...
            trs_token_id:"string",
            trs_version:"string",
      doc_type_id :"number",      account_id :"number",    }
    return data;
  }
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
    try {
      ...
      let query: any = {}; 
      const { doc_instance_id }: {doc_instance_id : bigint} = queryValue;
      const { doc_status }: {doc_status : string} = queryValue;
      const { doc_reference_url }: {doc_reference_url : Date} = queryValue;
      ...
      if(doc_instance_id){ 
        query.doc_instance_id = { [queryCondition['doc_instance_id']]: doc_instance_id };
      }
      if(doc_status){ 
        query.doc_status = { [queryCondition['doc_status']]: doc_status };
      }
      if(doc_reference_url){ 
        query.doc_reference_url = { [queryCondition['doc_reference_url']]: doc_reference_url };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.account_documents.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'account_documents');
      ...
      this.prismaService.account_documents.count({
        where: query,
      }));
  ...
  async findOne(doc_instance_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      this.prismaService.account_documents.findMany({ 
      where: {doc_instance_id, ...tenantFilter },
      select: {doc_instance_id:true,doc_status:true,doc_reference_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,doc_type_id :true,account_id :true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'account_documents');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'account_documents');
      decryptedRes.push(decryptedData);
    }
  ...
  async findAll(token : string,authContext?: any,detokenize?: string,detokenizeData?: any
    ...
    try{
      ...
      take: MAX_PAGE_SIZE,
      select: {doc_instance_id:true,doc_status:true,doc_reference_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,          doc_type_id :true,    
          account_id :true,    
      }
      }));
      let decryptedRes: any = [];
  ...
  async create(createaccount_documentsDto: Prisma.account_documentsCreateInput,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      if (authContext?.tenant) {
        (createaccount_documentsDto as any).trs_tenant_id = authContext.tenant;
      }

      const dataSchema:any =  v.object({
            doc_status :  v.optional(v.string()), 
            doc_reference_url :  v.optional(v.array(v.string())), 
      ...
      let validate : any = v.safeParse(dataSchema,createaccount_documentsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createaccount_documentsDto, 'account_documents', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.account_documents.create({
          data: encryptedData,
          select:{doc_instance_id:true,doc_status:true,doc_reference_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,doc_type_id :true,account_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createaccount_documentsDto: Prisma.account_documentsCreateInput,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'account_documents:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for account_documents');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'account_documents creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'account_documents creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'account_documents',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'doc_instance_id',
        ...
        return {
          success: true,
          message: 'account_documents creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(doc_instance_id:number, updateaccount_documentsDto: Prisma.account_documentsUpdateInput,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    try{
      ...
      if (authContext?.tenant) {
        const owned = await this.prismaService.withConnection(() =>
          this.prismaService.account_documents.findFirst({
            where: {doc_instance_id, trs_tenant_id: authContext.tenant },
            select: {doc_instance_id:true, },
          }),
        );
        if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updateaccount_documentsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateaccount_documentsDto,'account_documents','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updateaccount_documentsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.account_documents.updateMany({
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'account_documents', {
...
/**
   ...
   */
  async updateMaster(
doc_instance_id:number,
    updateaccount_documentsDto: Prisma.account_documentsUpdateInput,
    ...
    try {
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =doc_instance_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (!(await this.ufservice.hasCapability(token, 'account_documents:AUTHORIZE'))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for account_documents');
        }
        ...
        if (authContext?.tenant) {
          const targetRecord = await this.prismaService.account_documents.findUnique({
            where: { doc_instance_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'account_documents update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'account_documents update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'account_documents update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(doc_instance_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    const tenantFilter = authContext?.tenant ? { trs_tenant_id: authContext.tenant } : {};
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.account_documents.deleteMany({
      where: {doc_instance_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'account_documents');
    let decryptedRes: any = [];
...
doc_instance_id:number,
    ...
    try {
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =doc_instance_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (authContext?.tenant) {
          const targetRecord = await this.prismaService.account_documents.findUnique({
            where: { doc_instance_id: deleteMaster_id },
            select: { trs_tenant_id: true },
          });
          if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
            throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'account_documents deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'account_documents deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'account_documents deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real account_documents columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('account_documents'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'account_documents'));
      }
      return decryptedRes;
    } catch (error: any) {
