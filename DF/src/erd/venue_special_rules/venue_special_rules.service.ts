...
@Injectable()
export class venue_special_rulesService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
    try {
      ...
      let query: any = {}; 
      const { rule_id }: {rule_id : bigint} = queryValue;
      const { rule_text }: {rule_text : string} = queryValue;
      const { rule_type }: {rule_type : string} = queryValue;
      ...
      if(rule_id){ 
        query.rule_id = { [queryCondition['rule_id']]: rule_id };
      }
      if(rule_text){ 
        query.rule_text = { [queryCondition['rule_text']]: rule_text };
      }
      if(rule_type){ 
        query.rule_type = { [queryCondition['rule_type']]: rule_type };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.venue_special_rules.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'venue_special_rules');
      ...
      this.prismaService.venue_special_rules.count({
        where: query,
      }));
  ...
  async findOne(rule_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      this.prismaService.venue_special_rules.findMany({ 
      where: {rule_id, ...tenantFilter },
      select: {rule_id:true,rule_text:true,rule_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,venue_id :true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'venue_special_rules');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'venue_special_rules');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createvenue_special_rulesDto: Prisma.venue_special_rulesCreateInput,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    try{
      ...
      if (authContext?.tenant) {
        (createvenue_special_rulesDto as any).trs_tenant_id = authContext.tenant;
      }

      const dataSchema:any =  v.object({
            rule_text :  v.optional(v.string()), 
            rule_type :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createvenue_special_rulesDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvenue_special_rulesDto, 'venue_special_rules', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.venue_special_rules.create({
          data: encryptedData,
          select:{rule_id:true,rule_text:true,rule_type:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,venue_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createvenue_special_rulesDto: Prisma.venue_special_rulesCreateInput,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'venue_special_rules creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'venue_special_rules creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'venue_special_rules',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'rule_id',
        ...
        return {
          success: true,
          message: 'venue_special_rules creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(rule_id:number, updatevenue_special_rulesDto: Prisma.venue_special_rulesUpdateInput,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    try{
      ...
      if (authContext?.tenant) {
        const owned = await this.prismaService.withConnection(() =>
          this.prismaService.venue_special_rules.findFirst({
            where: {rule_id, trs_tenant_id: authContext.tenant },
            select: {rule_id:true, },
          }),
        );
        if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatevenue_special_rulesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevenue_special_rulesDto,'venue_special_rules','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevenue_special_rulesDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.venue_special_rules.updateMany({
        where: {rule_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'venue_special_rules', {
          primaryKey: {rule_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
rule_id:number,
    updatevenue_special_rulesDto: Prisma.venue_special_rulesUpdateInput,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (authContext?.tenant) {
          const targetRecord = await this.prismaService.venue_special_rules.findUnique({
            where: { rule_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'venue_special_rules update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'venue_special_rules update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'venue_special_rules update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(rule_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    const tenantFilter = authContext?.tenant ? { trs_tenant_id: authContext.tenant } : {};
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.venue_special_rules.deleteMany({
      where: {rule_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'venue_special_rules');
    let decryptedRes: any = [];
...
rule_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'venue_special_rules deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'venue_special_rules deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'venue_special_rules deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real venue_special_rules columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'venue_special_rules'));
      }
      return decryptedRes;
    } catch (error: any) {
