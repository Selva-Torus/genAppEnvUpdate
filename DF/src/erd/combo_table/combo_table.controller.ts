...
//import { CreateComboTableDto } from '../prisma/dto/create-comboTable.dto';
//import { UpdateComboTableDto } from '../prisma/dto/update-comboTable.dto';
...
@Controller('combo_table')
@ApiTags('ERD API')
export class combo_tableController {
  constructor(
    private readonly combo_tableService: combo_tableService,
  ...
  @ApiOkResponse({ type: combo_tableEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the combo_table table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.combo_tableService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: combo_tableEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the combo_table table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.combo_tableService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('my_id') my_id:number,@Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.combo_tableService.findOne(+my_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the combo_table table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.combo_tableService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(combo_tableEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('combo_table'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createcombo_tableDto })
  @ApiCreatedResponse({ type: combo_tableEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the combo_table table',
  })
  ...
  async create(
    ...
    @Body() createcombo_tableDto: Createcombo_tableDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["my_id"] = req.headers.my_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.combo_tableService.createMaster(createcombo_tableDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.combo_tableService.create(createcombo_tableDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('combo_table', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'my_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatecombo_tableDto })
  @ApiOkResponse({ type: combo_tableEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the combo_table table',
  })
...
@Param('my_id') my_id:number,
    @Body() updatecombo_tableDto: Updatecombo_tableDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["my_id"] = req.headers.my_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.combo_tableService.updateMaster(+my_id,updatecombo_tableDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.combo_tableService.update(+my_id,updatecombo_tableDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the combo_table table',
  })
...
@Param('my_id') my_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.combo_tableService.deleteMaster(+my_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.combo_tableService.remove(+my_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the combo_table table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.combo_tableService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the combo_table table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.combo_tableService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.combo_tableService.getLockById(key, value, token);
    return plainToInstance(combo_tableEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.combo_tableService.releaseLockById(key, value, token);
    return result;
  }
}