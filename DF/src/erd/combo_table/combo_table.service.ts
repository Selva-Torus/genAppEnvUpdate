...
import { Createcombo_tableDto } from './dto/Createcombo_table.dto';
import { Updatecombo_tableDto } from './dto/Updatecombo_table.dto';
...
import { combo_tableEntity } from './entity/combo_table.entity';
...
@Injectable()
export class combo_tableService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { my_id }: {my_id : number} = queryValue;
      ...
      if(my_id){ 
        query.my_id = { [queryCondition['my_id']]: my_id };
      }
      if(country){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.combo_table.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'combo_table');
      ...
      this.prismaService.combo_table.count({
        where: query,
      }));
  ...
  async findOne(my_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createcombo_tableDto: Createcombo_tableDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createcombo_tableDto as any).trs_tenant_id = authContext.tenant;
      ...
      let validate : any = v.safeParse(dataSchema,createcombo_tableDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createcombo_tableDto, 'combo_table', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.combo_table.create({
          data: encryptedData,
          select:{my_id:true,country:true,state:true,city:true,country_code:true,state_code:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createcombo_tableDto: Createcombo_tableDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'combo_table:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for combo_table');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'combo_table creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'combo_table creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'combo_table',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'my_id',
        ...
        return {
          success: true,
          message: 'combo_table creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(my_id:number, updatecombo_tableDto: Updatecombo_tableDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.combo_table.findFirst({
          where: {my_id, trs_tenant_id: authContext.tenant },
          select: {my_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatecombo_tableDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatecombo_tableDto,'combo_table','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatecombo_tableDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.combo_table.updateMany({
        where: {my_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'combo_table', {
          primaryKey: {my_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
my_id:number,
    updatecombo_tableDto: Updatecombo_tableDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.combo_table.findFirst({
          where: {my_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'combo_table update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'combo_table update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'combo_table update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(my_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.combo_table.deleteMany({
      where: {my_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'combo_table');
    let decryptedRes: any = [];
...
my_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'combo_table deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'combo_table deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'combo_table deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real combo_table columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('combo_table'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'combo_table'));
      }
      return decryptedRes;
    } catch (error: any) {
