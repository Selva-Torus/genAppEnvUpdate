...
export class vgph_process_exception_mainController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_process_exception_mainService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphpem_id') vgphpem_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpem_id"] = req.headers.vgphpem_id || "";
      ...
      detokenizeData["process_code"] = req.headers.process_code || "";
      detokenizeData["exception_cateogry"] = req.headers.exception_cateogry || "";
      detokenizeData["exception_data"] = req.headers.exception_data || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_process_exception_mainService.findOne(+vgphpem_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_process_exception_mainService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_process_exception_main'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_process_exception_mainDto: Createvgph_process_exception_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpem_id"] = req.headers.vgphpem_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_process_exception_mainService.createMaster(createvgph_process_exception_mainDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_process_exception_mainService.create(createvgph_process_exception_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_process_exception_main', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphpem_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphpem_id') vgphpem_id:number,
    @Body() updatevgph_process_exception_mainDto: Updatevgph_process_exception_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphpem_id"] = req.headers.vgphpem_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_process_exception_mainService.updateMaster(+vgphpem_id,updatevgph_process_exception_mainDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_process_exception_mainService.update(+vgphpem_id,updatevgph_process_exception_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }
...
@Param('vgphpem_id') vgphpem_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_process_exception_mainService.deleteMaster(+vgphpem_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_process_exception_mainService.remove(+vgphpem_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_process_exception_mainService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_process_exception_mainService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_process_exception_mainService.getLockById(key, value, token);
    return plainToInstance(vgph_process_exception_mainEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_process_exception_mainService.releaseLockById(key, value, token);
    return result;
  }
}