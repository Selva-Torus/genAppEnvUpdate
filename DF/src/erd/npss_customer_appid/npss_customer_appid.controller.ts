...
import { npss_customer_appidService } from './npss_customer_appid.service';
...
//import { CreateNpssCustomerAppidDto } from '../prisma/dto/create-npssCustomerAppid.dto';
//import { UpdateNpssCustomerAppidDto } from '../prisma/dto/update-npssCustomerAppid.dto';
...
@Controller('npss_customer_appid')
@ApiTags('ERD API')
export class npss_customer_appidController {
  constructor(
    private readonly npss_customer_appidService: npss_customer_appidService,
  ...
  @ApiOkResponse({ type: npss_customer_appidEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_customer_appid table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_customer_appidService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_customer_appidEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_customer_appid table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_customer_appidService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npsscapp_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npsscapp_id',type:Number})
  @ApiOkResponse({ type: npss_customer_appidEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_customer_appid table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npsscapp_id') npsscapp_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapp_id"] = req.headers.npsscapp_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_appidService.findOne(+npsscapp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_customer_appid table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_customer_appidService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_customer_appidEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_customer_appid'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_customer_appidDto })
  @ApiCreatedResponse({ type: npss_customer_appidEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_customer_appid table',
  })
  ...
  async create(
    ...
    @Body() createnpss_customer_appidDto: Createnpss_customer_appidDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapp_id"] = req.headers.npsscapp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_customer_appidService.createMaster(createnpss_customer_appidDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_customer_appidService.create(createnpss_customer_appidDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_customer_appid', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npsscapp_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_customer_appidDto })
  @ApiOkResponse({ type: npss_customer_appidEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_customer_appid table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npsscapp_id') npsscapp_id:number,
    @Body() updatenpss_customer_appidDto: Updatenpss_customer_appidDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npsscapp_id"] = req.headers.npsscapp_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_customer_appidService.updateMaster(+npsscapp_id,updatenpss_customer_appidDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_customer_appidService.update(+npsscapp_id,updatenpss_customer_appidDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_customer_appid table',
  })
...
@Param('npsscapp_id') npsscapp_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_customer_appidService.deleteMaster(+npsscapp_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_customer_appidService.remove(+npsscapp_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_customer_appid table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_appidService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_customer_appid table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_customer_appidService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_customer_appidService.getLockById(key, value, token);
    return plainToInstance(npss_customer_appidEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_customer_appidService.releaseLockById(key, value, token);
    return result;
  }
}