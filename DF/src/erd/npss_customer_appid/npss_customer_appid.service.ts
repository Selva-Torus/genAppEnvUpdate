...
import { Createnpss_customer_appidDto } from './dto/Createnpss_customer_appid.dto';
import { Updatenpss_customer_appidDto } from './dto/Updatenpss_customer_appid.dto';
...
import { npss_customer_appidEntity } from './entity/npss_customer_appid.entity';
...
@Injectable()
export class npss_customer_appidService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npsscapp_id }: {npsscapp_id : bigint} = queryValue;
      const { npsscp_id }: {npsscp_id : number} = queryValue;
      const { mobile }: {mobile : string} = queryValue;
      const { appid }: {appid : string} = queryValue;
      const { source_type }: {source_type : string} = queryValue;
      const { appid_status }: {appid_status : string} = queryValue;
      ...
      if(npsscapp_id){ 
        query.npsscapp_id = { [queryCondition['npsscapp_id']]: npsscapp_id };
      }
      if(npsscp_id){ 
      ...
      if(appid){ 
        query.appid = { [queryCondition['appid']]: appid };
      }
      if(source_type){ 
        query.source_type = { [queryCondition['source_type']]: source_type };
      }
      if(appid_status){ 
        query.appid_status = { [queryCondition['appid_status']]: appid_status };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_customer_appid.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_customer_appid');
      ...
      this.prismaService.npss_customer_appid.count({
        where: query,
      }));
  ...
  async findOne(npsscapp_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_customer_appidDto: Createnpss_customer_appidDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_customer_appidDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            npsscp_id :  v.optional(v.number()), 
            mobile :v.pipe(v.string(),v.maxLength(30 )) , 
            appid :v.pipe(v.string(),v.maxLength(64 )) , 
            source_type :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            appid_status :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_customer_appidDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_customer_appidDto, 'npss_customer_appid', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_appid.create({
          data: encryptedData,
          select:{npsscapp_id:true,npsscp_id:true,mobile:true,appid:true,source_type:true,appid_status:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_customer_appidDto: Createnpss_customer_appidDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_customer_appid:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_customer_appid');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_appid creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_appid creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_customer_appid',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npsscapp_id',
        ...
        return {
          success: true,
          message: 'npss_customer_appid creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npsscapp_id:number, updatenpss_customer_appidDto: Updatenpss_customer_appidDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_appid.findFirst({
          where: {npsscapp_id, trs_tenant_id: authContext.tenant },
          select: {npsscapp_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          ...
          appid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          source_type :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          appid_status :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_customer_appidDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_customer_appidDto,'npss_customer_appid','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_customer_appidDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_customer_appid.updateMany({
        where: {npsscapp_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_customer_appid', {
          primaryKey: {npsscapp_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npsscapp_id:number,
    updatenpss_customer_appidDto: Updatenpss_customer_appidDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npsscapp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_customer_appid.findFirst({
          where: {npsscapp_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_appid update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_appid update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_appid update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npsscapp_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_customer_appid.deleteMany({
      where: {npsscapp_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_customer_appid');
    let decryptedRes: any = [];
...
npsscapp_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npsscapp_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_customer_appid deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_customer_appid deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_customer_appid deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_customer_appid columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_customer_appid'));
      }
      return decryptedRes;
    } catch (error: any) {
