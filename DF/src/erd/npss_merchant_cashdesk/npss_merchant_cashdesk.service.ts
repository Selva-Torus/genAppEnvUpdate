...
import { Createnpss_merchant_cashdeskDto } from './dto/Createnpss_merchant_cashdesk.dto';
import { Updatenpss_merchant_cashdeskDto } from './dto/Updatenpss_merchant_cashdesk.dto';
...
import { npss_merchant_cashdeskEntity } from './entity/npss_merchant_cashdesk.entity';
...
@Injectable()
export class npss_merchant_cashdeskService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npssmc_id }: {npssmc_id : bigint} = queryValue;
      const { npssms_id }: {npssms_id : number} = queryValue;
      const { product_code }: {product_code : string} = queryValue;
      const { bankuserid }: {bankuserid : string} = queryValue;
      const { cashdesk_overlay_id }: {cashdesk_overlay_id : string} = queryValue;
      const { cashdesk_id }: {cashdesk_id : string} = queryValue;
      ...
      if(npssmc_id){ 
        query.npssmc_id = { [queryCondition['npssmc_id']]: npssmc_id };
      }
      if(npssms_id){ 
      ...
      if(cashdesk_overlay_id){ 
        query.cashdesk_overlay_id = { [queryCondition['cashdesk_overlay_id']]: cashdesk_overlay_id };
      }
      if(cashdesk_id){ 
        query.cashdesk_id = { [queryCondition['cashdesk_id']]: cashdesk_id };
      }
      if(exhf_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_merchant_cashdesk.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_merchant_cashdesk');
      ...
      this.prismaService.npss_merchant_cashdesk.count({
        where: query,
      }));
  ...
  async findOne(npssmc_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_merchant_cashdeskDto: Createnpss_merchant_cashdeskDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_merchant_cashdeskDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            cashdesk_overlay_id :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
            cashdesk_id :v.pipe(v.string(),v.maxLength(16 )) , 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_merchant_cashdeskDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_merchant_cashdeskDto, 'npss_merchant_cashdesk', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_cashdesk.create({
          data: encryptedData,
          select:{npssmc_id:true,npssms_id:true,product_code:true,bankuserid:true,cashdesk_overlay_id:true,cashdesk_id:true,exhf_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_merchant_cashdeskDto: Createnpss_merchant_cashdeskDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_merchant_cashdesk:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_merchant_cashdesk');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_cashdesk creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_cashdesk creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_merchant_cashdesk',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npssmc_id',
        ...
        return {
          success: true,
          message: 'npss_merchant_cashdesk creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npssmc_id:number, updatenpss_merchant_cashdeskDto: Updatenpss_merchant_cashdeskDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_cashdesk.findFirst({
          where: {npssmc_id, trs_tenant_id: authContext.tenant },
          select: {npssmc_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          npssms_id :  v.optional(v.number()), 
          product_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          bankuserid :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          cashdesk_overlay_id :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
          cashdesk_id :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          exhf_id :  v.optional(v.number()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_merchant_cashdeskDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_merchant_cashdeskDto,'npss_merchant_cashdesk','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_merchant_cashdeskDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_cashdesk.updateMany({
        where: {npssmc_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_merchant_cashdesk', {
          primaryKey: {npssmc_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npssmc_id:number,
    updatenpss_merchant_cashdeskDto: Updatenpss_merchant_cashdeskDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npssmc_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_merchant_cashdesk.findFirst({
          where: {npssmc_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_cashdesk update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_cashdesk update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_cashdesk update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npssmc_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_merchant_cashdesk.deleteMany({
      where: {npssmc_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_cashdesk');
    let decryptedRes: any = [];
...
npssmc_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npssmc_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_cashdesk deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_cashdesk deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_cashdesk deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_merchant_cashdesk columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_merchant_cashdesk'));
      }
      return decryptedRes;
    } catch (error: any) {
