...
import { npss_merchant_cashdeskService } from './npss_merchant_cashdesk.service';
...
//import { CreateNpssMerchantCashdeskDto } from '../prisma/dto/create-npssMerchantCashdesk.dto';
//import { UpdateNpssMerchantCashdeskDto } from '../prisma/dto/update-npssMerchantCashdesk.dto';
...
@Controller('npss_merchant_cashdesk')
@ApiTags('ERD API')
export class npss_merchant_cashdeskController {
  constructor(
    private readonly npss_merchant_cashdeskService: npss_merchant_cashdeskService,
  ...
  @ApiOkResponse({ type: npss_merchant_cashdeskEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_merchant_cashdesk table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_merchant_cashdeskService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_merchant_cashdeskEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_merchant_cashdesk table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_merchant_cashdeskService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npssmc_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npssmc_id',type:Number})
  @ApiOkResponse({ type: npss_merchant_cashdeskEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_merchant_cashdesk table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npssmc_id') npssmc_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssmc_id"] = req.headers.npssmc_id || "";
      ...
      detokenizeData["cashdesk_overlay_id"] = req.headers.cashdesk_overlay_id || "";
      detokenizeData["cashdesk_id"] = req.headers.cashdesk_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_cashdeskService.findOne(+npssmc_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_merchant_cashdesk table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_merchant_cashdeskService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_cashdesk'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_merchant_cashdeskDto })
  @ApiCreatedResponse({ type: npss_merchant_cashdeskEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_merchant_cashdesk table',
  })
  ...
  async create(
    ...
    @Body() createnpss_merchant_cashdeskDto: Createnpss_merchant_cashdeskDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssmc_id"] = req.headers.npssmc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_cashdeskService.createMaster(createnpss_merchant_cashdeskDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_merchant_cashdeskService.create(createnpss_merchant_cashdeskDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_cashdesk', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npssmc_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_merchant_cashdeskDto })
  @ApiOkResponse({ type: npss_merchant_cashdeskEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_merchant_cashdesk table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npssmc_id') npssmc_id:number,
    @Body() updatenpss_merchant_cashdeskDto: Updatenpss_merchant_cashdeskDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssmc_id"] = req.headers.npssmc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_cashdeskService.updateMaster(+npssmc_id,updatenpss_merchant_cashdeskDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_cashdeskService.update(+npssmc_id,updatenpss_merchant_cashdeskDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_merchant_cashdesk table',
  })
...
@Param('npssmc_id') npssmc_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_merchant_cashdeskService.deleteMaster(+npssmc_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_cashdeskService.remove(+npssmc_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_merchant_cashdesk table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_cashdeskService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_merchant_cashdesk table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_cashdeskService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_merchant_cashdeskService.getLockById(key, value, token);
    return plainToInstance(npss_merchant_cashdeskEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_merchant_cashdeskService.releaseLockById(key, value, token);
    return result;
  }
}