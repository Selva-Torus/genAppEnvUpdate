...
,vgphse_id?:number) {
  ...
  async create(createvgph_source_employeeDto: Prisma.vgph_source_employeeCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_employeeDto, 'vgph_source_employee', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphse_id:number, updatevgph_source_employeeDto: Prisma.vgph_source_employeeUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_employeeDto,'vgph_source_employee','update'), token);
