...
import { employee_detailsService } from './employee_details.service';
...
//import { CreateEmployeeDetailsDto } from '../prisma/dto/create-employeeDetails.dto';
//import { UpdateEmployeeDetailsDto } from '../prisma/dto/update-employeeDetails.dto';
...
@ApiTags('ERD API')
export class employee_detailsController {
  constructor(
    private readonly employee_detailsService: employee_detailsService,
  ...
  @ApiOkResponse({ type: employee_detailsEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the employee_details table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.employee_detailsService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: employee_detailsEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the employee_details table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.employee_detailsService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('id') id:number,@Req() req: any) {
    ...
    let detokenizeData = {};
      ...
      detokenizeData["url"] = req.headers.url || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.employee_detailsService.findOne(+id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the employee_details table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.employee_detailsService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(employee_detailsEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('employee_details'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createemployee_detailsDto })
  @ApiCreatedResponse({ type: employee_detailsEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the employee_details table',
  })
  ...
  async create(
    ...
    @Body() createemployee_detailsDto: Createemployee_detailsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["id"] = req.headers.id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.employee_detailsService.createMaster(createemployee_detailsDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.employee_detailsService.create(createemployee_detailsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('employee_details', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updateemployee_detailsDto })
  @ApiOkResponse({ type: employee_detailsEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the employee_details table',
  })
  ...
  async update(
    ...
    @Headers() authHeader: string,
@Param('id') id:number,
    @Body() updateemployee_detailsDto: Updateemployee_detailsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["id"] = req.headers.id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.employee_detailsService.updateMaster(+id,updateemployee_detailsDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.employee_detailsService.update(+id,updateemployee_detailsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the employee_details table',
  })
...
@Param('id') id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.employee_detailsService.deleteMaster(+id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.employee_detailsService.remove(+id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the employee_details table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.employee_detailsService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the employee_details table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.employee_detailsService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.employee_detailsService.getLockById(key, value, token);
    return plainToInstance(employee_detailsEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.employee_detailsService.releaseLockById(key, value, token);
    return result;
  }
}