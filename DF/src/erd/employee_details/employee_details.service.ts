...
import { Createemployee_detailsDto } from './dto/Createemployee_details.dto';
import { Updateemployee_detailsDto } from './dto/Updateemployee_details.dto';
...
import { employee_detailsEntity } from './entity/employee_details.entity';
...
@Injectable()
export class employee_detailsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      const { address }: {address : any } = queryValue;
      const { category_total_amount }: {category_total_amount : number} = queryValue;
      const { claim_category }: {claim_category : Date} = queryValue;
      const { url }: {url : Date} = queryValue;
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.employee_details.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'employee_details');
      ...
      this.prismaService.employee_details.count({
        where: query,
      }));
  ...
  async findOne(id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createemployee_detailsDto: Createemployee_detailsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createemployee_detailsDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            claim_category :  v.optional(v.array(v.string())), 
      ...
      let validate : any = v.safeParse(dataSchema,createemployee_detailsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createemployee_detailsDto, 'employee_details', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.employee_details.create({
          data: encryptedData,
          select:{id:true,name:true,email:true,address:true,category_total_amount:true,claim_category:true,url:true,expense_date:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createemployee_detailsDto: Createemployee_detailsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'employee_details:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for employee_details');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_details creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_details creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        return {
          success: true,
          message: 'employee_details creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(id:number, updateemployee_detailsDto: Updateemployee_detailsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.employee_details.findFirst({
          where: {id, trs_tenant_id: authContext.tenant },
          select: {id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updateemployee_detailsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateemployee_detailsDto,'employee_details','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updateemployee_detailsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.employee_details.updateMany({
        where: {id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'employee_details', {
          primaryKey: {id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
id:number,
    updateemployee_detailsDto: Updateemployee_detailsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.employee_details.findFirst({
          where: {id },
        ...
        if (success) {
          return {
            success: true,
            message: 'employee_details update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_details update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'employee_details update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.employee_details.deleteMany({
      where: {id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'employee_details');
    let decryptedRes: any = [];
...
id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'employee_details deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_details deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'employee_details deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real employee_details columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('employee_details'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'employee_details'));
      }
      return decryptedRes;
    } catch (error: any) {
