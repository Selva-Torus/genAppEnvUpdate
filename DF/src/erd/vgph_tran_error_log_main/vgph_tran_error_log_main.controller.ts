...
export class vgph_tran_error_log_mainController {
  ...
  async findOne(@Headers() authHeader: string,@Param('vgphtelm_id') vgphtelm_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_error_log_mainService.findOne(+vgphtelm_id,trs_tenant_id,trs_app_code,trs_product_code,token);
  ...
  async findAll(@Headers() authHeader: string,@Req() req: any,@Query('vgphtelm_id') vgphtelm_id?:string,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_tran_error_log_mainService.findAll(token,+vgphtelm_id);
    return plainToInstance(vgph_tran_error_log_mainEntity, result);
  } 
  ...
  @Patch(':vgphtelm_id')
  @UsePipes(new PrismaModelValidationPipe('vgph_tran_error_log_main', true))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphtelm_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphtelm_id') vgphtelm_id:number,
    ...
    const result = await this.vgph_tran_error_log_mainService.update(+vgphtelm_id,updatevgph_tran_error_log_mainDto,token);
    return plainToInstance(vgph_tran_error_log_mainEntity, result);
  }
...
@Param('vgphtelm_id') vgphtelm_id:number,
    ...
    const result = await this.vgph_tran_error_log_mainService.remove(+vgphtelm_id,token);
    return plainToInstance(vgph_tran_error_log_mainEntity, result);
  }  
  ...
  async findFirst(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_error_log_mainService.findFirst(token);
    return plainToInstance(vgph_tran_error_log_mainEntity, result);
  }
  ...
  async findLast(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_error_log_mainService.findLast(token);
    return plainToInstance(vgph_tran_error_log_mainEntity, result);
  }
}