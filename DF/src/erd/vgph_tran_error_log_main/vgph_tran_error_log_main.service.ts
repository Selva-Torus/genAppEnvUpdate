...
export class vgph_tran_error_log_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphtelm_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.vgph_tran_error_log_main.findMany({ 
      where: {vgphtelm_id,trs_tenant_id,trs_app_code,trs_product_code, ...tenantFilter },
...
,vgphtelm_id?:number) {
  ...
  async create(createvgph_tran_error_log_mainDto: Createvgph_tran_error_log_mainDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_tran_error_log_mainDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_tran_error_log_mainDto, 'vgph_tran_error_log_main', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_tran_error_log_mainDto: Createvgph_tran_error_log_mainDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_tran_error_log_main:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_tran_error_log_main');
        }
  ...
  async update(vgphtelm_id:number, updatevgph_tran_error_log_mainDto: Updatevgph_tran_error_log_mainDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_tran_error_log_main.findFirst({
          where: {vgphtelm_id, trs_tenant_id: authContext.tenant },
          select: {vgphtelm_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_tran_error_log_mainDto,'vgph_tran_error_log_main','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_tran_error_log_mainDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_tran_error_log_main', {
...
/**
  ...
  async updateMaster(
vgphtelm_id:number,
    updatevgph_tran_error_log_mainDto: Updatevgph_tran_error_log_mainDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_tran_error_log_main.findFirst({
          where: {vgphtelm_id },
  ...
  async remove(vgphtelm_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    const toDelete = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_tran_error_log_main.findMany({
      where: {vgphtelm_id ,...tenantFilter},
      select: {vgphtelm_id:true,direction:true,process_type:true,tran_category:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,vgphsts_uuid:true,vgphntm_uuid:true,vgphstm_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}
    })); 
    await this.prismaService.withConnection(() =>
...
vgphtelm_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_tran_error_log_main columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('vgph_tran_error_log_main'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'vgph_tran_error_log_main',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_tran_error_log_main'));
      }
      return decryptedRes;
    } catch (error: any) {
