...
export class vgph_destination_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(vgphdm_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,uuid: string,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.vgph_destination_main.findMany({ 
      where: {vgphdm_id,trs_tenant_id,trs_app_code,trs_product_code,uuid, ...tenantFilter },
...
,vgphdm_id?:number) {
  ...
  async create(createvgph_destination_mainDto: Createvgph_destination_mainDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_destination_mainDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_destination_mainDto, 'vgph_destination_main', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_destination_mainDto: Createvgph_destination_mainDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_destination_main:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_destination_main');
        }
  ...
  async update(uuid: string, updatevgph_destination_mainDto: Updatevgph_destination_mainDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_destination_mainDto,'vgph_destination_main','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_destination_mainDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_destination_main', {
          primaryKey: {uuid},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
uuid: string,
    updatevgph_destination_mainDto: Updatevgph_destination_mainDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_destination_main.findFirst({
          where: {uuid },
  ...
  async remove(vgphdm_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphdm_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_destination_main columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_destination_main'));
      }
      return decryptedRes;
    } catch (error: any) {
