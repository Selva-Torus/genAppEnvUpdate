...
export class codedescriptionController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.codedescriptionService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('ccdid') ccdid:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["ccdid"] = req.headers.ccdid || "";
      detokenizeData["cdcategory"] = req.headers.cdcategory || "";
      detokenizeData["clientcode"] = req.headers.clientcode || "";
      detokenizeData["code"] = req.headers.code || "";
      detokenizeData["description"] = req.headers.description || "";
      detokenizeData["sortorder"] = req.headers.sortorder || "";
      detokenizeData["parentcode"] = req.headers.parentcode || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.codedescriptionService.findOne(+ccdid,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.codedescriptionService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(codedescriptionEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('codedescription'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createcodedescriptionDto: CreatecodedescriptionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["ccdid"] = req.headers.ccdid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.codedescriptionService.createMaster(createcodedescriptionDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.codedescriptionService.create(createcodedescriptionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(codedescriptionEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('codedescription', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'ccdid',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('ccdid') ccdid:number,
    @Body() updatecodedescriptionDto: UpdatecodedescriptionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["ccdid"] = req.headers.ccdid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.codedescriptionService.updateMaster(+ccdid,updatecodedescriptionDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.codedescriptionService.update(+ccdid,updatecodedescriptionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(codedescriptionEntity, result);
  }
...
@Param('ccdid') ccdid:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.codedescriptionService.deleteMaster(+ccdid,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.codedescriptionService.remove(+ccdid,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(codedescriptionEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.codedescriptionService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(codedescriptionEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.codedescriptionService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(codedescriptionEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.codedescriptionService.getLockById(key, value, token);
    return plainToInstance(codedescriptionEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.codedescriptionService.releaseLockById(key, value, token);
    return result;
  }
}