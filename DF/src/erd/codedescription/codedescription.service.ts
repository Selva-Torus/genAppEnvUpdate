...
export class codedescriptionService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.codedescription.findMany({
          select:columns,
           where: query,
      ...
      this.prismaService.codedescription.count({
        where: query,
      }));
  ...
  async findOne(ccdid:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'codedescription');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'codedescription');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createcodedescriptionDto: CreatecodedescriptionDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createcodedescriptionDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createcodedescriptionDto, 'codedescription', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.codedescription.create({
          data: encryptedData,
          select:{ccdid:true,cdcategory:true,clientcode:true,code:true,description:true,sortorder:true,parentcode:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createcodedescriptionDto: CreatecodedescriptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'codedescription:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for codedescription');
        }
  ...
  async update(ccdid:number, updatecodedescriptionDto: UpdatecodedescriptionDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.codedescription.findFirst({
          where: {ccdid, trs_tenant_id: authContext.tenant },
          select: {ccdid:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatecodedescriptionDto,'codedescription','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatecodedescriptionDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.codedescription.updateMany({
        where: {ccdid},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'codedescription', {
          primaryKey: {ccdid},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
ccdid:number,
    updatecodedescriptionDto: UpdatecodedescriptionDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.codedescription.findFirst({
          where: {ccdid },
  ...
  async remove(ccdid:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.codedescription.deleteMany({
      where: {ccdid ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'codedescription');
    let decryptedRes: any = [];
...
ccdid:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real codedescription columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'codedescription'));
      }
      return decryptedRes;
    } catch (error: any) {
