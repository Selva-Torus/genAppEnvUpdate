...
export class vgph_clearing_sessionController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_clearing_sessionService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgph_csid') vgph_csid:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_csid"] = req.headers.vgph_csid || "";
      ...
      detokenizeData["processing_day"] = req.headers.processing_day || "";
      detokenizeData["session_info"] = req.headers.session_info || "";
      detokenizeData["start_date"] = req.headers.start_date || "";
      detokenizeData["end_date"] = req.headers.end_date || "";
      detokenizeData["rule_code"] = req.headers.rule_code || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_sessionService.findOne(+vgph_csid,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_clearing_sessionService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_clearing_session'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_clearing_sessionDto: Createvgph_clearing_sessionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_csid"] = req.headers.vgph_csid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_clearing_sessionService.createMaster(createvgph_clearing_sessionDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_clearing_sessionService.create(createvgph_clearing_sessionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_clearing_session', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgph_csid',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgph_csid') vgph_csid:number,
    @Body() updatevgph_clearing_sessionDto: Updatevgph_clearing_sessionDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_csid"] = req.headers.vgph_csid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_clearing_sessionService.updateMaster(+vgph_csid,updatevgph_clearing_sessionDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_clearing_sessionService.update(+vgph_csid,updatevgph_clearing_sessionDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }
...
@Param('vgph_csid') vgph_csid:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_clearing_sessionService.deleteMaster(+vgph_csid,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_clearing_sessionService.remove(+vgph_csid,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_sessionService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_sessionService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_clearing_sessionService.getLockById(key, value, token);
    return plainToInstance(vgph_clearing_sessionEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_clearing_sessionService.releaseLockById(key, value, token);
    return result;
  }
}