...
export class vgph_source_tran_mainController {
  ...
  async findOne(@Headers() authHeader: string,@Param('vgphstm_id') vgphstm_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_tran_mainService.findOne(+vgphstm_id,trs_tenant_id,trs_app_code,trs_product_code,token);
  ...
  async findAll(@Headers() authHeader: string,@Req() req: any,@Query('vgphstm_id') vgphstm_id?:string,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_source_tran_mainService.findAll(token,+vgphstm_id);
    return plainToInstance(vgph_source_tran_mainEntity, result);
  } 
  ...
  @Patch(':uuid')
  @UsePipes(new PrismaModelValidationPipe('vgph_source_tran_main', true))
  @ApiBearerAuth('JWT-auth')
...
@Param('uuid') uuid: string,
    ...
    const result = await this.vgph_source_tran_mainService.update(uuid,updatevgph_source_tran_mainDto,token);
    return plainToInstance(vgph_source_tran_mainEntity, result);
  }
...
@Param('vgphstm_id') vgphstm_id:number,
    ...
    const result = await this.vgph_source_tran_mainService.remove(+vgphstm_id,token);
    return plainToInstance(vgph_source_tran_mainEntity, result);
  }  
  ...
  async findFirst(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_tran_mainService.findFirst(token);
    return plainToInstance(vgph_source_tran_mainEntity, result);
  }
  ...
  async findLast(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_tran_mainService.findLast(token);
    return plainToInstance(vgph_source_tran_mainEntity, result);
  }
}