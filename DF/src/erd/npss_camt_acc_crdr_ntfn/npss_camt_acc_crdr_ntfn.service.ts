...
@Injectable()
export class npss_camt_acc_crdr_ntfnService {
  ...
  async commonDecimalDatahandle(data:any){
    const plainData = { ...data,
       limit_amount:data?.limit_amount != null ? Number(data?.limit_amount?.toString()) : null,
      }
    return plainData
  }
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { ncacn_id }: {ncacn_id : bigint} = queryValue;
      const { header_msg_id }: {header_msg_id : string} = queryValue;
      const { notification_id }: {notification_id : string} = queryValue;
      const { ntfn_created_datetime }: {ntfn_created_datetime : string} = queryValue;
      const { iban }: {iban : string} = queryValue;
      const { currency }: {currency : string} = queryValue;
      const { limit_amount }: {limit_amount : Date} = queryValue;
      const { cr_dr_indicator }: {cr_dr_indicator : string} = queryValue;
      const { ntfn_status }: {ntfn_status : string} = queryValue;
      const { booking_datetime }: {booking_datetime : string} = queryValue;
      const { end_to_end_id }: {end_to_end_id : string} = queryValue;
      const { message_data }: {message_data : string} = queryValue;
      const { ntfn_account_id }: {ntfn_account_id : string} = queryValue;
      const { account_type_code }: {account_type_code : string} = queryValue;
      const { bank_tran_code }: {bank_tran_code : string} = queryValue;
      const { bank_domain_code }: {bank_domain_code : string} = queryValue;
      const { bank_tran_prop_code }: {bank_tran_prop_code : string} = queryValue;
      const { dbtr_acct_id }: {dbtr_acct_id : string} = queryValue;
      const { cdtr_acct_id }: {cdtr_acct_id : string} = queryValue;
      ...
      if(ncacn_id){ 
        query.ncacn_id = { [queryCondition['ncacn_id']]: ncacn_id };
      }
      if(header_msg_id){ 
        query.header_msg_id = { [queryCondition['header_msg_id']]: header_msg_id };
      }
      if(notification_id){ 
        query.notification_id = { [queryCondition['notification_id']]: notification_id };
      }
      if(ntfn_created_datetime){ 
        query.ntfn_created_datetime = { [queryCondition['ntfn_created_datetime']]: ntfn_created_datetime };
      }
      if(iban){ 
        query.iban = { [queryCondition['iban']]: iban };
      }
      if(currency){ 
        query.currency = { [queryCondition['currency']]: currency };
      }
      if(limit_amount){ 
        query.limit_amount = { [queryCondition['limit_amount']]: limit_amount };
      }
      if(cr_dr_indicator){ 
        query.cr_dr_indicator = { [queryCondition['cr_dr_indicator']]: cr_dr_indicator };
      }
      if(ntfn_status){ 
        query.ntfn_status = { [queryCondition['ntfn_status']]: ntfn_status };
      }
      if(booking_datetime){ 
        query.booking_datetime = { [queryCondition['booking_datetime']]: booking_datetime };
      }
      if(end_to_end_id){ 
        query.end_to_end_id = { [queryCondition['end_to_end_id']]: end_to_end_id };
      }
      if(message_data){ 
        query.message_data = { [queryCondition['message_data']]: message_data };
      }
      if(ntfn_account_id){ 
        query.ntfn_account_id = { [queryCondition['ntfn_account_id']]: ntfn_account_id };
      }
      if(account_type_code){ 
        query.account_type_code = { [queryCondition['account_type_code']]: account_type_code };
      }
      if(bank_tran_code){ 
        query.bank_tran_code = { [queryCondition['bank_tran_code']]: bank_tran_code };
      }
      if(bank_domain_code){ 
        query.bank_domain_code = { [queryCondition['bank_domain_code']]: bank_domain_code };
      }
      if(bank_tran_prop_code){ 
        query.bank_tran_prop_code = { [queryCondition['bank_tran_prop_code']]: bank_tran_prop_code };
      }
      if(dbtr_acct_id){ 
        query.dbtr_acct_id = { [queryCondition['dbtr_acct_id']]: dbtr_acct_id };
      }
      if(cdtr_acct_id){ 
        query.cdtr_acct_id = { [queryCondition['cdtr_acct_id']]: cdtr_acct_id };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_camt_acc_crdr_ntfn.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_camt_acc_crdr_ntfn');
      ...
      this.prismaService.npss_camt_acc_crdr_ntfn.count({
        where: query,
      }));
  ...
  async findOne(ncacn_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_camt_acc_crdr_ntfn.findMany({ 
      where: {ncacn_id, ...tenantFilter },
      select: {ncacn_id:true,header_msg_id:true,notification_id:true,ntfn_created_datetime:true,iban:true,currency:true,limit_amount:true,cr_dr_indicator:true,ntfn_status:true,booking_datetime:true,end_to_end_id:true,message_data:true,ntfn_account_id:true,account_type_code:true,bank_tran_code:true,bank_domain_code:true,bank_tran_prop_code:true,dbtr_acct_id:true,cdtr_acct_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_camt_acc_crdr_ntfn');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_camt_acc_crdr_ntfn');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_camt_acc_crdr_ntfnDto: Createnpss_camt_acc_crdr_ntfnDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_camt_acc_crdr_ntfnDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            header_msg_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            notification_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            ntfn_created_datetime :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            iban :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            limit_amount :  v.optional(v.pipe(v.number(),v.maxValue(999999999999999999 ))), 
            cr_dr_indicator :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            ntfn_status :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            booking_datetime :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            end_to_end_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            message_data :  v.optional(v.pipe(v.string(),v.maxLength(80000 ))), 
            ntfn_account_id :  v.optional(v.pipe(v.string(),v.maxLength(34 ))), 
            account_type_code :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            bank_tran_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            bank_domain_code :  v.optional(v.pipe(v.string(),v.maxLength(4 ))), 
            bank_tran_prop_code :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            dbtr_acct_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
            cdtr_acct_id :  v.optional(v.pipe(v.string(),v.maxLength(35 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_camt_acc_crdr_ntfnDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_camt_acc_crdr_ntfnDto, 'npss_camt_acc_crdr_ntfn', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_acc_crdr_ntfn.create({
          data: encryptedData,
          select:{ncacn_id:true,header_msg_id:true,notification_id:true,ntfn_created_datetime:true,iban:true,currency:true,limit_amount:true,cr_dr_indicator:true,ntfn_status:true,booking_datetime:true,end_to_end_id:true,message_data:true,ntfn_account_id:true,account_type_code:true,bank_tran_code:true,bank_domain_code:true,bank_tran_prop_code:true,dbtr_acct_id:true,cdtr_acct_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_camt_acc_crdr_ntfnDto: Createnpss_camt_acc_crdr_ntfnDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_camt_acc_crdr_ntfn:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_camt_acc_crdr_ntfn');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_acc_crdr_ntfn creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_acc_crdr_ntfn creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_camt_acc_crdr_ntfn',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'ncacn_id',
        ...
        return {
          success: true,
          message: 'npss_camt_acc_crdr_ntfn creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(ncacn_id:number, updatenpss_camt_acc_crdr_ntfnDto: Updatenpss_camt_acc_crdr_ntfnDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_acc_crdr_ntfn.findFirst({
          where: {ncacn_id, trs_tenant_id: authContext.tenant },
          select: {ncacn_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_camt_acc_crdr_ntfnDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_camt_acc_crdr_ntfnDto,'npss_camt_acc_crdr_ntfn','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_camt_acc_crdr_ntfnDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_camt_acc_crdr_ntfn.updateMany({
        where: {ncacn_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_camt_acc_crdr_ntfn', {
          primaryKey: {ncacn_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
ncacn_id:number,
    updatenpss_camt_acc_crdr_ntfnDto: Updatenpss_camt_acc_crdr_ntfnDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =ncacn_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_camt_acc_crdr_ntfn.findFirst({
          where: {ncacn_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_camt_acc_crdr_ntfn update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_acc_crdr_ntfn update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_camt_acc_crdr_ntfn update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(ncacn_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_camt_acc_crdr_ntfn.deleteMany({
      where: {ncacn_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_camt_acc_crdr_ntfn');
    let decryptedRes: any = [];
...
ncacn_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =ncacn_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_camt_acc_crdr_ntfn deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_camt_acc_crdr_ntfn deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_camt_acc_crdr_ntfn deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_camt_acc_crdr_ntfn columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('npss_camt_acc_crdr_ntfn'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'npss_camt_acc_crdr_ntfn',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_camt_acc_crdr_ntfn'));
      }
      return decryptedRes;
    } catch (error: any) {
