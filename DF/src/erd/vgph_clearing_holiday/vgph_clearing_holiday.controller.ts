...
export class vgph_clearing_holidayController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_clearing_holidayService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgph_chid') vgph_chid:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_chid"] = req.headers.vgph_chid || "";
      detokenizeData["currency"] = req.headers.currency || "";
      detokenizeData["holiday_code"] = req.headers.holiday_code || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_holidayService.findOne(+vgph_chid,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_clearing_holidayService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_clearing_holiday'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_clearing_holidayDto: Createvgph_clearing_holidayDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_chid"] = req.headers.vgph_chid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_clearing_holidayService.createMaster(createvgph_clearing_holidayDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_clearing_holidayService.create(createvgph_clearing_holidayDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_clearing_holiday', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgph_chid',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgph_chid') vgph_chid:number,
    @Body() updatevgph_clearing_holidayDto: Updatevgph_clearing_holidayDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_chid"] = req.headers.vgph_chid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_clearing_holidayService.updateMaster(+vgph_chid,updatevgph_clearing_holidayDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_clearing_holidayService.update(+vgph_chid,updatevgph_clearing_holidayDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }
...
@Param('vgph_chid') vgph_chid:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_clearing_holidayService.deleteMaster(+vgph_chid,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_clearing_holidayService.remove(+vgph_chid,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_holidayService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_clearing_holidayService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_clearing_holidayService.getLockById(key, value, token);
    return plainToInstance(vgph_clearing_holidayEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_clearing_holidayService.releaseLockById(key, value, token);
    return result;
  }
}