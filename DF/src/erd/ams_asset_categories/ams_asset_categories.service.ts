...
@Injectable()
export class ams_asset_categoriesService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { acat_id }: {acat_id : bigint} = queryValue;
      const { parent_acat_id }: {parent_acat_id : bigint} = queryValue;
      const { category_code }: {category_code : string} = queryValue;
      const { asset_prefix }: {asset_prefix : string} = queryValue;
      const { category_name }: {category_name : string} = queryValue;
      const { depreciation_method }: {depreciation_method : string} = queryValue;
      const { useful_life_years }: {useful_life_years : string} = queryValue;
      const { approval_required }: {approval_required : Date} = queryValue;
      const { maintenance_required }: {maintenance_required : Date} = queryValue;
      ...
      if(acat_id){ 
        query.acat_id = { [queryCondition['acat_id']]: acat_id };
      }
      if(parent_acat_id){ 
        query.parent_acat_id = { [queryCondition['parent_acat_id']]: parent_acat_id };
      }
      if(category_code){ 
        query.category_code = { [queryCondition['category_code']]: category_code };
      }
      if(asset_prefix){ 
        query.asset_prefix = { [queryCondition['asset_prefix']]: asset_prefix };
      }
      if(category_name){ 
        query.category_name = { [queryCondition['category_name']]: category_name };
      }
      if(depreciation_method){ 
        query.depreciation_method = { [queryCondition['depreciation_method']]: depreciation_method };
      }
      if(useful_life_years){ 
        query.useful_life_years = { [queryCondition['useful_life_years']]: useful_life_years };
      }
      if(approval_required){ 
        query.approval_required = { [queryCondition['approval_required']]: approval_required };
      }
      if(maintenance_required){ 
        query.maintenance_required = { [queryCondition['maintenance_required']]: maintenance_required };
      }
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.ams_asset_categories.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'ams_asset_categories');
      ...
      this.prismaService.ams_asset_categories.count({
        where: query,
      }));
  ...
  async findOne(acat_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_categories.findMany({ 
      where: {acat_id},
      select: {acat_id:true,parent_acat_id:true,category_code:true,asset_prefix:true,category_name:true,depreciation_method:true,useful_life_years:true,approval_required:true,maintenance_required:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            ams_assets:{
              select:{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'ams_asset_categories');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'ams_asset_categories');
      decryptedRes.push(decryptedData);
    }
    return decryptedRes;
  } catch (error:any) {
  ...
  async create(createams_asset_categoriesDto: Prisma.ams_asset_categoriesCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            acat_id :v.number() , 
            parent_acat_id :  v.optional(v.number()), 
            category_code :v.pipe(v.string(),v.maxLength(30 )) , 
            asset_prefix :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            category_name :v.pipe(v.string(),v.maxLength(30 )) , 
            depreciation_method :  v.optional(v.string()), 
            useful_life_years :  v.optional(v.string()), 
            approval_required :  v.optional(v.boolean()), 
            maintenance_required :  v.optional(v.boolean()), 
        ...
        let validate : any = v.safeParse(dataSchema,createams_asset_categoriesDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createams_asset_categoriesDto, 'ams_asset_categories', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.ams_asset_categories.create({
          data: encryptedData,
          select:{acat_id:true,parent_acat_id:true,category_code:true,asset_prefix:true,category_name:true,depreciation_method:true,useful_life_years:true,approval_required:true,maintenance_required:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,ams_assets:true,}          
        })
      );
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'ams_asset_categories');
  } catch (error:any) {
   ...
   */
  async createMaster(
    createams_asset_categoriesDto: Prisma.ams_asset_categoriesCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_categories creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_categories creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'ams_asset_categories',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'acat_id',
        ...
        return {
          success: true,
          message: 'ams_asset_categories creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(acat_id:number, updateams_asset_categoriesDto: Prisma.ams_asset_categoriesUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          parent_acat_id :  v.optional(v.number()), 
          category_code :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
          asset_prefix :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          category_name :  v.optional(v.pipe(v.string(),v.maxLength(30 ))), 
          depreciation_method :  v.optional(v.string()), 
          useful_life_years :  v.optional(v.string()), 
          approval_required :  v.optional(v.boolean()), 
          maintenance_required :  v.optional(v.boolean()), 
      ...
      let validate : any = v.safeParse(dataSchema,updateams_asset_categoriesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateams_asset_categoriesDto,'ams_asset_categories','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_categories.updateMany({
      where: {acat_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
acat_id:number,
    updateams_asset_categoriesDto: Prisma.ams_asset_categoriesUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =acat_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_asset_categories update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_categories update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_asset_categories update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(acat_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_categories.deleteMany({
      where: {acat_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'ams_asset_categories');
    let decryptedRes: any = [];
...
acat_id:number,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_asset_categories deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_asset_categories deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_asset_categories deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.ams_asset_categories.findFirst({ 
