...
export class userService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any) {
    try {
      ...
      const { onlydate }: {onlydate : any } = queryValue;
      const { dateandtime }: {dateandtime :  Date} = queryValue;
      ...
      if(onlydate){ 
        query.onlydate = { [queryCondition['onlydate']]: onlydate };
      }
      if(dateandtime){ 
        query.dateandtime = { [queryCondition['dateandtime']]: dateandtime };
      }
      if(trs_created_date){ 
