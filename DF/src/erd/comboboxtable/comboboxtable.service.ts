...
export class comboboxtableService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      const { otherdetails }: {otherdetails : any } = queryValue;
      ...
      if(otherdetails){ 
        query.otherdetails = { [queryCondition['otherdetails']]: otherdetails };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'comboboxtable');
      ...
      this.prismaService.comboboxtable.count({
        where: query,
      }));
   ...
   */
  async createMaster(
    createcomboboxtableDto: Prisma.comboboxtableCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'comboboxtable creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'comboboxtable creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        return {
          success: true,
          message: 'comboboxtable creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
...
/**
  ...
  async updateMaster(
my_id:number,
    updatecomboboxtableDto: Prisma.comboboxtableUpdateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'comboboxtable update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'comboboxtable update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'comboboxtable update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(my_id:number,token : string) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.comboboxtable.deleteMany({
      where: {my_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'comboboxtable');
    let decryptedRes: any = [];
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'comboboxtable deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'comboboxtable deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'comboboxtable deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
