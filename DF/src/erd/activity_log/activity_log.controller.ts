...
import { activity_logService } from './activity_log.service';
...
//import { CreateActivityLogDto } from '../prisma/dto/create-activityLog.dto';
//import { UpdateActivityLogDto } from '../prisma/dto/update-activityLog.dto';
...
@ApiTags('ERD API')
export class activity_logController {
  constructor(
    private readonly activity_logService: activity_logService,
  ...
  @ApiOkResponse({ type: activity_logEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the activity_log table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.activity_logService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: activity_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the activity_log table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.activity_logService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':activity_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'activity_id',type:Number})
  @ApiOkResponse({ type: activity_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the activity_log table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('activity_id') activity_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["activity_id"] = req.headers.activity_id || "";
      detokenizeData["activity_type"] = req.headers.activity_type || "";
      detokenizeData["description"] = req.headers.description || "";
      detokenizeData["activity_timestamp"] = req.headers.activity_timestamp || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.activity_logService.findOne(+activity_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the activity_log table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.activity_logService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(activity_logEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('activity_log'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createactivity_logDto })
  @ApiCreatedResponse({ type: activity_logEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the activity_log table',
  })
  ...
  async create(
    ...
    @Body() createactivity_logDto: Createactivity_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["activity_id"] = req.headers.activity_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.activity_logService.createMaster(createactivity_logDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.activity_logService.create(createactivity_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('activity_log', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'activity_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updateactivity_logDto })
  @ApiOkResponse({ type: activity_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the activity_log table',
  })
...
@Param('activity_id') activity_id:number,
    @Body() updateactivity_logDto: Updateactivity_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["activity_id"] = req.headers.activity_id || "";
    ...
    const result = await this.activity_logService.update(+activity_id,updateactivity_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the activity_log table',
  })
...
@Param('activity_id') activity_id:number,
    ...
    const result = await this.activity_logService.remove(+activity_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the activity_log table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.activity_logService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the activity_log table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.activity_logService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.activity_logService.getLockById(key, value, token);
    return plainToInstance(activity_logEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.activity_logService.releaseLockById(key, value, token);
    return result;
  }
}