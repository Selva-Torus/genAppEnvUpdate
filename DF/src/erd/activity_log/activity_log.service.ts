...
import { Createactivity_logDto } from './dto/Createactivity_log.dto';
import { Updateactivity_logDto } from './dto/Updateactivity_log.dto';
...
import { activity_logEntity } from './entity/activity_log.entity';
...
@Injectable()
export class activity_logService {
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            ...
            trs_token_id:"string",
            trs_version:"string",
      attorney_id :"number",      account_id :"number",    }
    return data;
  }
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { activity_id }: {activity_id : bigint} = queryValue;
      const { activity_type }: {activity_type : string} = queryValue;
      const { description }: {description : string} = queryValue;
      const { activity_timestamp }: {activity_timestamp :  Date} = queryValue;
      ...
      if(activity_id){ 
        query.activity_id = { [queryCondition['activity_id']]: activity_id };
      }
      if(activity_type){ 
        query.activity_type = { [queryCondition['activity_type']]: activity_type };
      }
      if(description){ 
        query.description = { [queryCondition['description']]: description };
      }
      if(activity_timestamp){ 
        query.activity_timestamp = { [queryCondition['activity_timestamp']]: activity_timestamp };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.activity_log.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'activity_log');
      ...
      this.prismaService.activity_log.count({
        where: query,
      }));
  ...
  async findOne(activity_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.activity_log.findMany({ 
      where: {activity_id, ...tenantFilter },
      select: {activity_id:true,activity_type:true,description:true,activity_timestamp:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,attorney_id :true,account_id :true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'activity_log');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'activity_log');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createactivity_logDto: Createactivity_logDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createactivity_logDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            activity_type :  v.optional(v.string()), 
            description :  v.optional(v.string()), 
            activity_timestamp :  v.optional((v.any())), 
      ...
      let validate : any = v.safeParse(dataSchema,createactivity_logDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createactivity_logDto, 'activity_log', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.activity_log.create({
          data: encryptedData,
          select:{activity_id:true,activity_type:true,description:true,activity_timestamp:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,attorney_id :true,account_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createactivity_logDto: Createactivity_logDto,
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'activity_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'activity_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'activity_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'activity_id',
        ...
        return {
          success: true,
          message: 'activity_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(activity_id:number, updateactivity_logDto: Updateactivity_logDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.activity_log.findFirst({
          where: {activity_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updateactivity_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateactivity_logDto,'activity_log','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updateactivity_logDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.activity_log.updateMany({
        where: {activity_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'activity_log', {
...
/**
  ...
  async updateMaster(
activity_id:number,
    updateactivity_logDto: Updateactivity_logDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =activity_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.activity_log.findUnique({
          where: { activity_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'activity_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'activity_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'activity_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(activity_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.activity_log.deleteMany({
      where: {activity_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'activity_log');
    let decryptedRes: any = [];
...
activity_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =activity_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.activity_log.findUnique({
          where: { activity_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'activity_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'activity_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'activity_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real activity_log columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('activity_log'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'activity_log'));
      }
      return decryptedRes;
    } catch (error: any) {
