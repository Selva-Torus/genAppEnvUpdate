...
@Injectable()
export class hrm_leave_policiesService {
  ...
  async commonDecimalDatahandle(data:any){
    const plainData = { ...data,
       days_per_year:data?.days_per_year != null ? Number(data?.days_per_year?.toString()) : null,
       carry_forward_days:data?.carry_forward_days != null ? Number(data?.carry_forward_days?.toString()) : null,
      }
    return plainData
  }
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            ...
            policy_rules_json:"json",
            accrual_rules_json:"json",
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any) {
    try {
      ...
      let query: any = {}; 
      const { policy_id }: {policy_id : bigint} = queryValue;
      const { policy_code }: {policy_code : string} = queryValue;
      const { policy_name }: {policy_name : string} = queryValue;
      const { leave_type }: {leave_type : string} = queryValue;
      const { days_per_year }: {days_per_year : Date} = queryValue;
      const { carry_forward_days }: {carry_forward_days : Date} = queryValue;
      const { applicable_to }: {applicable_to : string} = queryValue;
      const { gender_applicability }: {gender_applicability : string} = queryValue;
      const { accrual_frequency }: {accrual_frequency : string} = queryValue;
      const { max_consecutive_days }: {max_consecutive_days : number} = queryValue;
      const { approval_required }: {approval_required : Date} = queryValue;
      const { encashment_allowed }: {encashment_allowed : Date} = queryValue;
      const { carry_forward_expiry }: {carry_forward_expiry : number} = queryValue;
      const { applicable_locations }: {applicable_locations : any } = queryValue;
      const { policy_rules_json }: {policy_rules_json : any } = queryValue;
      const { accrual_rules_json }: {accrual_rules_json : any } = queryValue;
      ...
      if(policy_id){ 
        query.policy_id = { [queryCondition['policy_id']]: policy_id };
      }
      if(policy_code){ 
        query.policy_code = { [queryCondition['policy_code']]: policy_code };
      }
      if(policy_name){ 
        query.policy_name = { [queryCondition['policy_name']]: policy_name };
      }
      if(leave_type){ 
        query.leave_type = { [queryCondition['leave_type']]: leave_type };
      }
      if(days_per_year){ 
        query.days_per_year = { [queryCondition['days_per_year']]: days_per_year };
      }
      if(carry_forward_days){ 
        query.carry_forward_days = { [queryCondition['carry_forward_days']]: carry_forward_days };
      }
      if(applicable_to){ 
        query.applicable_to = { [queryCondition['applicable_to']]: applicable_to };
      }
      if(gender_applicability){ 
        query.gender_applicability = { [queryCondition['gender_applicability']]: gender_applicability };
      }
      if(accrual_frequency){ 
        query.accrual_frequency = { [queryCondition['accrual_frequency']]: accrual_frequency };
      }
      if(max_consecutive_days){ 
        query.max_consecutive_days = { [queryCondition['max_consecutive_days']]: max_consecutive_days };
      }
      if(approval_required){ 
        query.approval_required = { [queryCondition['approval_required']]: approval_required };
      }
      if(encashment_allowed){ 
        query.encashment_allowed = { [queryCondition['encashment_allowed']]: encashment_allowed };
      }
      if(carry_forward_expiry){ 
        query.carry_forward_expiry = { [queryCondition['carry_forward_expiry']]: carry_forward_expiry };
      }
      if(applicable_locations){ 
        query.applicable_locations = { [queryCondition['applicable_locations']]: applicable_locations };
      }
      if(policy_rules_json){ 
        query.policy_rules_json = { [queryCondition['policy_rules_json']]: policy_rules_json };
      }
      if(accrual_rules_json){ 
        query.accrual_rules_json = { [queryCondition['accrual_rules_json']]: accrual_rules_json };
      }
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.hrm_leave_policies.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'hrm_leave_policies');
      ...
      this.prismaService.hrm_leave_policies.count({
        where: query,
      }));
  ...
  async findOne(policy_id:number,token : string,detokenize?: string,detokenizeData?: any) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.hrm_leave_policies.findMany({ 
      where: {policy_id},
      select: {policy_id:true,policy_code:true,policy_name:true,leave_type:true,days_per_year:true,carry_forward_days:true,applicable_to:true,gender_applicability:true,accrual_frequency:true,max_consecutive_days:true,approval_required:true,encashment_allowed:true,carry_forward_expiry:true,applicable_locations:true,policy_rules_json:true,accrual_rules_json:true,additional_details:true,secure_data:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            hrm_leave_requests:{
              select:{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'hrm_leave_policies');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'hrm_leave_policies');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createhrm_leave_policiesDto: Prisma.hrm_leave_policiesCreateInput,token:string,detokenize:string,detokenizeData?: any) {
    try{

      const dataSchema:any =  v.object({
            policy_code :v.string() , 
            policy_name :v.string() , 
            leave_type :v.string() , 
            days_per_year :v.number() , 
            carry_forward_days :v.number() , 
            applicable_to :v.string() , 
            gender_applicability :  v.optional(v.string()), 
            accrual_frequency :  v.optional(v.string()), 
            max_consecutive_days :  v.optional(v.number()), 
            approval_required :v.boolean()  , 
            encashment_allowed :v.boolean()  , 
            carry_forward_expiry :  v.optional(v.number()), 
            applicable_locations :  v.optional(v.any() ), 
            policy_rules_json :  v.optional(v.any() ), 
            accrual_rules_json :  v.optional(v.any() ), 
      ...
      let validate : any = v.safeParse(dataSchema,createhrm_leave_policiesDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createhrm_leave_policiesDto, 'hrm_leave_policies', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.hrm_leave_policies.create({
          data: encryptedData,
          select:{policy_id:true,policy_code:true,policy_name:true,leave_type:true,days_per_year:true,carry_forward_days:true,applicable_to:true,gender_applicability:true,accrual_frequency:true,max_consecutive_days:true,approval_required:true,encashment_allowed:true,carry_forward_expiry:true,applicable_locations:true,policy_rules_json:true,accrual_rules_json:true,additional_details:true,secure_data:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,hrm_leave_requests:true,}          
        })
      );
   ...
   */
  async createMaster(
    createhrm_leave_policiesDto: Prisma.hrm_leave_policiesCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_leave_policies creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_leave_policies creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'hrm_leave_policies',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'policy_id',
        ...
        return {
          success: true,
          message: 'hrm_leave_policies creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(policy_id:number, updatehrm_leave_policiesDto: Prisma.hrm_leave_policiesUpdateInput,token:string, detokenize:string,detokenizeData?: any) {   
    try{

      const dataSchema:any =  v.object({
          policy_code :  v.optional(v.string()), 
          policy_name :  v.optional(v.string()), 
          leave_type :  v.optional(v.string()), 
          days_per_year :  v.optional(v.number()), 
          carry_forward_days :  v.optional(v.number()), 
          applicable_to :  v.optional(v.string()), 
          gender_applicability :  v.optional(v.string()), 
          accrual_frequency :  v.optional(v.string()), 
          max_consecutive_days :  v.optional(v.number()), 
          approval_required :  v.optional(v.boolean()), 
          encashment_allowed :  v.optional(v.boolean()), 
          carry_forward_expiry :  v.optional(v.number()), 
          applicable_locations :  v.optional(v.any()), 
          policy_rules_json :  v.optional(v.any()), 
          accrual_rules_json :  v.optional(v.any()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatehrm_leave_policiesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatehrm_leave_policiesDto,'hrm_leave_policies','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatehrm_leave_policiesDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.hrm_leave_policies.updateMany({
        where: {policy_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'hrm_leave_policies', {
...
/**
  ...
  async updateMaster(
policy_id:number,
    updatehrm_leave_policiesDto: Prisma.hrm_leave_policiesUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =policy_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_leave_policies update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_leave_policies update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_leave_policies update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(policy_id:number,token : string, detokenize: string,detokenizeData?: any) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.hrm_leave_policies.deleteMany({
      where: {policy_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'hrm_leave_policies');
    let decryptedRes: any = [];
...
policy_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =policy_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_leave_policies deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_leave_policies deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_leave_policies deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string, detokenize: string, detokenizeData?: any) {
    try{
      ...
      this.prismaService.hrm_leave_policies.findFirst({ 
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'hrm_leave_policies'));
      }
      return decryptedRes;
    } catch (error: any) {
