...
//import { CreateVenuesDto } from '../prisma/dto/create-venues.dto';
//import { UpdateVenuesDto } from '../prisma/dto/update-venues.dto';
...
export class venuesController {
  ...
  @ApiOkResponse({ type: venuesEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the venues table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.venuesService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: venuesEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the venues table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.venuesService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':venue_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'venue_id',type:Number})
  @ApiOkResponse({ type: venuesEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the venues table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('venue_id') venue_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["venue_id"] = req.headers.venue_id || "";
      detokenizeData["state"] = req.headers.state || "";
      detokenizeData["county"] = req.headers.county || "";
      detokenizeData["court_name"] = req.headers.court_name || "";
      detokenizeData["judge_name"] = req.headers.judge_name || "";
      detokenizeData["filing_fee"] = req.headers.filing_fee || "";
      detokenizeData["service_method"] = req.headers.service_method || "";
      detokenizeData["efiling_required"] = req.headers.efiling_required || "";
      detokenizeData["efiling_system"] = req.headers.efiling_system || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.venuesService.findOne(+venue_id,token,detokenize,detokenizeData,req.authContext);
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the venues table',
  })
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('venues'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: CreatevenuesDto })
  @ApiCreatedResponse({ type: venuesEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the venues table',
  })
  ...
  async create(
    ...
    @Body() createvenuesDto: CreatevenuesDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["venue_id"] = req.headers.venue_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.venuesService.createMaster(createvenuesDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.venuesService.create(createvenuesDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(venuesEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('venues', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'venue_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: UpdatevenuesDto })
  @ApiOkResponse({ type: venuesEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the venues table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('venue_id') venue_id:number,
    @Body() updatevenuesDto: UpdatevenuesDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["venue_id"] = req.headers.venue_id || "";
    ...
    const result = await this.venuesService.update(+venue_id,updatevenuesDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(venuesEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the venues table',
  })
...
@Param('venue_id') venue_id:number,
    ...
    const result = await this.venuesService.remove(+venue_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(venuesEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the venues table',
  })
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the venues table',
  })
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.venuesService.getLockById(key, value, token);
    return plainToInstance(venuesEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.venuesService.releaseLockById(key, value, token);
    return result;
  }
}