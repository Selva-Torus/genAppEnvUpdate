...
export class venuesService {
  ...
  async commonDecimalDatahandle(data:any){
    const plainData = { ...data,
       filing_fee:data?.filing_fee != null ? Number(data?.filing_fee?.toString()) : null,
      }
    return plainData
  }
 ...
 private formatInTimezone(date: Date, ianaTimezone: string): string {
  ...
  async findSchema (token) {
    const data = {
            ...
            efiling_required:"boolean",
            efiling_system:"string",
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { venue_id }: {venue_id : bigint} = queryValue;
      const { state }: {state : string} = queryValue;
      const { county }: {county : string} = queryValue;
      const { court_name }: {court_name : string} = queryValue;
      const { judge_name }: {judge_name : string} = queryValue;
      const { filing_fee }: {filing_fee : Date} = queryValue;
      const { service_method }: {service_method : string} = queryValue;
      const { efiling_required }: {efiling_required : Date} = queryValue;
      const { efiling_system }: {efiling_system : string} = queryValue;
      ...
      if(venue_id){ 
        query.venue_id = { [queryCondition['venue_id']]: venue_id };
      }
      if(state){ 
      ...
      if(court_name){ 
        query.court_name = { [queryCondition['court_name']]: court_name };
      }
      if(judge_name){ 
        query.judge_name = { [queryCondition['judge_name']]: judge_name };
      }
      if(filing_fee){ 
        query.filing_fee = { [queryCondition['filing_fee']]: filing_fee };
      }
      if(service_method){ 
        query.service_method = { [queryCondition['service_method']]: service_method };
      }
      if(efiling_required){ 
        query.efiling_required = { [queryCondition['efiling_required']]: efiling_required };
      }
      if(efiling_system){ 
        query.efiling_system = { [queryCondition['efiling_system']]: efiling_system };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'venues');
      ...
      this.prismaService.venues.count({
        where: query,
      }));
  ...
  async findOne(venue_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'venues');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'venues');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createvenuesDto: CreatevenuesDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    ...
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            court_name :  v.optional(v.string()), 
            judge_name :  v.optional(v.string()), 
            filing_fee :  v.optional(v.number()), 
            service_method :  v.optional(v.string()), 
            efiling_required :  v.optional(v.boolean()), 
            efiling_system :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,createvenuesDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvenuesDto, 'venues', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.venues.create({
          data: encryptedData,
          select:{venue_id:true,state:true,county:true,court_name:true,judge_name:true,filing_fee:true,service_method:true,efiling_required:true,efiling_system:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,venue_special_rules:true,accounts:true,}          
        })
      );
  ...
  async createMaster(
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'venues creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'venues creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        return {
          success: true,
          message: 'venues creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(venue_id:number, updatevenuesDto: UpdatevenuesDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    ...
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevenuesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevenuesDto,'venues','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'venues', {
          primaryKey: {venue_id},
          currentVersion,
          data: encryptedData,
        }),
...
venue_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'venues update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'venues update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'venues update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(venue_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.venues.deleteMany({
      where: {venue_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'venues');
    let decryptedRes: any = [];
...
venue_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'venues deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'venues deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'venues deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real venues columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('venues'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'venues'));
      }
      return decryptedRes;
    } catch (error: any) {
