...
export class vgph_source_corporateController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_source_corporateService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphsc_id') vgphsc_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsc_id"] = req.headers.vgphsc_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_corporateService.findOne(+vgphsc_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphsc_id') vgphsc_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsc_id"] = req.headers.vgphsc_id || "";
      detokenizeData["uuid"] = req.headers.uuid || "";
    ...
    const result = await this.vgph_source_corporateService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphsc_id);
    return plainToInstance(vgph_source_corporateEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_source_corporate'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_source_corporateDto: Createvgph_source_corporateDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsc_id"] = req.headers.vgphsc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_corporateService.createMaster(createvgph_source_corporateDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_source_corporateService.create(createvgph_source_corporateDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_corporateEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_source_corporate', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphsc_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphsc_id') vgphsc_id:number,
    @Body() updatevgph_source_corporateDto: Updatevgph_source_corporateDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsc_id"] = req.headers.vgphsc_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_corporateService.updateMaster(+vgphsc_id,updatevgph_source_corporateDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_corporateService.update(+vgphsc_id,updatevgph_source_corporateDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_corporateEntity, result);
  }
...
@Param('vgphsc_id') vgphsc_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_source_corporateService.deleteMaster(+vgphsc_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_corporateService.remove(+vgphsc_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_corporateEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_corporateService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_corporateEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_corporateService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_corporateEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_source_corporateService.getLockById(key, value, token);
    return plainToInstance(vgph_source_corporateEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_source_corporateService.releaseLockById(key, value, token);
    return result;
  }
}