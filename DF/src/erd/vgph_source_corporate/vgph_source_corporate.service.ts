...
,vgphsc_id?:number) {
  ...
  async create(createvgph_source_corporateDto: Prisma.vgph_source_corporateCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_corporateDto, 'vgph_source_corporate', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphsc_id:number, updatevgph_source_corporateDto: Prisma.vgph_source_corporateUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_corporateDto,'vgph_source_corporate','update'), token);
