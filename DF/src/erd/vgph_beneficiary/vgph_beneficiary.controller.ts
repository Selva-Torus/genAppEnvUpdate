...
export class vgph_beneficiaryController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_beneficiaryService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphb_id') vgphb_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphb_id"] = req.headers.vgphb_id || "";
      detokenizeData["beneficiary_category"] = req.headers.beneficiary_category || "";
      detokenizeData["beneficiary_id"] = req.headers.beneficiary_id || "";
      detokenizeData["beneficiary_common"] = req.headers.beneficiary_common || "";
      detokenizeData["beneficiary_address"] = req.headers.beneficiary_address || "";
      detokenizeData["beneficiary_bank"] = req.headers.beneficiary_bank || "";
      detokenizeData["beneficiary_identity"] = req.headers.beneficiary_identity || "";
      detokenizeData["beneficiary_compliance"] = req.headers.beneficiary_compliance || "";
      detokenizeData["beneficiary_preference"] = req.headers.beneficiary_preference || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_beneficiaryService.findOne(+vgphb_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_beneficiaryService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_beneficiaryEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_beneficiary'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_beneficiaryDto: Createvgph_beneficiaryDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphb_id"] = req.headers.vgphb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_beneficiaryService.createMaster(createvgph_beneficiaryDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_beneficiaryService.create(createvgph_beneficiaryDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_beneficiary', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphb_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphb_id') vgphb_id:number,
    @Body() updatevgph_beneficiaryDto: Updatevgph_beneficiaryDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphb_id"] = req.headers.vgphb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_beneficiaryService.updateMaster(+vgphb_id,updatevgph_beneficiaryDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_beneficiaryService.update(+vgphb_id,updatevgph_beneficiaryDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }
...
@Param('vgphb_id') vgphb_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_beneficiaryService.deleteMaster(+vgphb_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_beneficiaryService.remove(+vgphb_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_beneficiaryService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_beneficiaryService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_beneficiaryService.getLockById(key, value, token);
    return plainToInstance(vgph_beneficiaryEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_beneficiaryService.releaseLockById(key, value, token);
    return result;
  }
}