 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      const { rct_subscription }: {rct_subscription : string} = queryValue;
      const { bct_subscription }: {bct_subscription : string} = queryValue;
      ...
      if(rct_subscription){ 
        query.rct_subscription = { [queryCondition['rct_subscription']]: rct_subscription };
      }
      if(bct_subscription){ 
        query.bct_subscription = { [queryCondition['bct_subscription']]: bct_subscription };
      }
      if(trs_created_date){ 
  ...
  async findOne(vgphbpbs_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createvgph_participant_bank_statusDto: Createvgph_participant_bank_statusDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_participant_bank_statusDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            from_date_time :(v.union([v.pipe(v.string(), v.isoDate()), v.pipe(v.string(), v.isoTimestamp())])) , 
            to_date_time :(v.union([v.pipe(v.string(), v.isoDate()), v.pipe(v.string(), v.isoTimestamp())])) , 
            ...
            rct_subscription :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            bct_subscription :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_participant_bank_statusDto, 'vgph_participant_bank_status', 'create'), token);
   ...
   */
  async createMaster(
    createvgph_participant_bank_statusDto: Createvgph_participant_bank_statusDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_participant_bank_status:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_participant_bank_status');
        }
  ...
  async update(vgphbpbs_id:number, updatevgph_participant_bank_statusDto: Updatevgph_participant_bank_statusDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_participant_bank_status.findFirst({
          where: {vgphbpbs_id, trs_tenant_id: authContext.tenant },
          select: {vgphbpbs_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_participant_bank_statusDto,'vgph_participant_bank_status','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_participant_bank_statusDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_participant_bank_status', {
          primaryKey: {vgphbpbs_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphbpbs_id:number,
    updatevgph_participant_bank_statusDto: Updatevgph_participant_bank_statusDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_participant_bank_status.findFirst({
          where: {vgphbpbs_id },
  ...
  async remove(vgphbpbs_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
vgphbpbs_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_participant_bank_status columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('vgph_participant_bank_status'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'vgph_participant_bank_status',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_participant_bank_status'));
      }
      return decryptedRes;
    } catch (error: any) {
