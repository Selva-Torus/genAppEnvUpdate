...
import { npss_merchant_accountsService } from './npss_merchant_accounts.service';
...
//import { CreateNpssMerchantAccountsDto } from '../prisma/dto/create-npssMerchantAccounts.dto';
//import { UpdateNpssMerchantAccountsDto } from '../prisma/dto/update-npssMerchantAccounts.dto';
...
@Controller('npss_merchant_accounts')
@ApiTags('ERD API')
export class npss_merchant_accountsController {
  constructor(
    private readonly npss_merchant_accountsService: npss_merchant_accountsService,
  ...
  @ApiOkResponse({ type: npss_merchant_accountsEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the npss_merchant_accounts table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.npss_merchant_accountsService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: npss_merchant_accountsEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the npss_merchant_accounts table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.npss_merchant_accountsService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }

  @Get(':npssma_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'npssma_id',type:Number})
  @ApiOkResponse({ type: npss_merchant_accountsEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the npss_merchant_accounts table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('npssma_id') npssma_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssma_id"] = req.headers.npssma_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_accountsService.findOne(+npssma_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the npss_merchant_accounts table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.npss_merchant_accountsService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(npss_merchant_accountsEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_accounts'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createnpss_merchant_accountsDto })
  @ApiCreatedResponse({ type: npss_merchant_accountsEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the npss_merchant_accounts table',
  })
  ...
  async create(
    ...
    @Body() createnpss_merchant_accountsDto: Createnpss_merchant_accountsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssma_id"] = req.headers.npssma_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_accountsService.createMaster(createnpss_merchant_accountsDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.npss_merchant_accountsService.create(createnpss_merchant_accountsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('npss_merchant_accounts', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'npssma_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatenpss_merchant_accountsDto })
  @ApiOkResponse({ type: npss_merchant_accountsEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the npss_merchant_accounts table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('npssma_id') npssma_id:number,
    @Body() updatenpss_merchant_accountsDto: Updatenpss_merchant_accountsDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["npssma_id"] = req.headers.npssma_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.npss_merchant_accountsService.updateMaster(+npssma_id,updatenpss_merchant_accountsDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_accountsService.update(+npssma_id,updatenpss_merchant_accountsDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the npss_merchant_accounts table',
  })
...
@Param('npssma_id') npssma_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.npss_merchant_accountsService.deleteMaster(+npssma_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.npss_merchant_accountsService.remove(+npssma_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the npss_merchant_accounts table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_accountsService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the npss_merchant_accounts table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.npss_merchant_accountsService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.npss_merchant_accountsService.getLockById(key, value, token);
    return plainToInstance(npss_merchant_accountsEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.npss_merchant_accountsService.releaseLockById(key, value, token);
    return result;
  }
}