...
import { Createnpss_merchant_accountsDto } from './dto/Createnpss_merchant_accounts.dto';
import { Updatenpss_merchant_accountsDto } from './dto/Updatenpss_merchant_accounts.dto';
...
import { npss_merchant_accountsEntity } from './entity/npss_merchant_accounts.entity';
...
@Injectable()
export class npss_merchant_accountsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npssma_id }: {npssma_id : bigint} = queryValue;
      ...
      const { default_account }: {default_account : string} = queryValue;
      const { exhf_id }: {exhf_id : number} = queryValue;
      const { account_id }: {account_id : string} = queryValue;
      ...
      if(npssma_id){ 
        query.npssma_id = { [queryCondition['npssma_id']]: npssma_id };
      }
      if(npssmp_id){ 
      ...
      if(default_account){ 
        query.default_account = { [queryCondition['default_account']]: default_account };
      }
      if(exhf_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_merchant_accounts.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_merchant_accounts');
      ...
      this.prismaService.npss_merchant_accounts.count({
        where: query,
      }));
  ...
  async findOne(npssma_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_merchant_accountsDto: Createnpss_merchant_accountsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_merchant_accountsDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            iban :v.pipe(v.string(),v.maxLength(35 )) , 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            default_account :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
            exhf_id :  v.optional(v.number()), 
            account_id :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_merchant_accountsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_merchant_accountsDto, 'npss_merchant_accounts', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_accounts.create({
          data: encryptedData,
          select:{npssma_id:true,npssmp_id:true,product_code:true,bankuserid:true,iban:true,currency:true,default_account:true,exhf_id:true,account_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_merchant_accountsDto: Createnpss_merchant_accountsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_merchant_accounts:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_merchant_accounts');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_accounts creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_accounts creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_merchant_accounts',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npssma_id',
        ...
        return {
          success: true,
          message: 'npss_merchant_accounts creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npssma_id:number, updatenpss_merchant_accountsDto: Updatenpss_merchant_accountsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_accounts.findFirst({
          where: {npssma_id, trs_tenant_id: authContext.tenant },
          select: {npssma_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_merchant_accountsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_merchant_accountsDto,'npss_merchant_accounts','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_merchant_accountsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_merchant_accounts.updateMany({
        where: {npssma_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_merchant_accounts', {
          primaryKey: {npssma_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npssma_id:number,
    updatenpss_merchant_accountsDto: Updatenpss_merchant_accountsDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npssma_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_merchant_accounts.findFirst({
          where: {npssma_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_accounts update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_accounts update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_accounts update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npssma_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_merchant_accounts.deleteMany({
      where: {npssma_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_merchant_accounts');
    let decryptedRes: any = [];
...
npssma_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npssma_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_merchant_accounts deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_merchant_accounts deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_merchant_accounts deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_merchant_accounts columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_merchant_accounts'));
      }
      return decryptedRes;
    } catch (error: any) {
