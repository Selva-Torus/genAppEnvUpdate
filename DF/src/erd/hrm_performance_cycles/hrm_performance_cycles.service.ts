...
@Injectable()
export class hrm_performance_cyclesService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any) {
    try {
      ...
      let query: any = {}; 
      const { cycle_id }: {cycle_id : bigint} = queryValue;
      const { cycle_code }: {cycle_code : string} = queryValue;
      const { cycle_name }: {cycle_name : string} = queryValue;
      const { cycle_type }: {cycle_type : string} = queryValue;
      const { start_date }: {start_date : any } = queryValue;
      const { end_date }: {end_date : any } = queryValue;
      const { review_frequency }: {review_frequency : string} = queryValue;
      const { self_review_required }: {self_review_required : Date} = queryValue;
      const { manager_review_required }: {manager_review_required : Date} = queryValue;
      ...
      if(cycle_id){ 
        query.cycle_id = { [queryCondition['cycle_id']]: cycle_id };
      }
      if(cycle_code){ 
        query.cycle_code = { [queryCondition['cycle_code']]: cycle_code };
      }
      if(cycle_name){ 
        query.cycle_name = { [queryCondition['cycle_name']]: cycle_name };
      }
      if(cycle_type){ 
        query.cycle_type = { [queryCondition['cycle_type']]: cycle_type };
      }
      if(start_date){ 
      ...
      if(review_frequency){ 
        query.review_frequency = { [queryCondition['review_frequency']]: review_frequency };
      }
      if(self_review_required){ 
        query.self_review_required = { [queryCondition['self_review_required']]: self_review_required };
      }
      if(manager_review_required){ 
        query.manager_review_required = { [queryCondition['manager_review_required']]: manager_review_required };
      }
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.hrm_performance_cycles.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'hrm_performance_cycles');
      ...
      this.prismaService.hrm_performance_cycles.count({
        where: query,
      }));
  ...
  async findOne(cycle_id:number,token : string,detokenize?: string,detokenizeData?: any) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.hrm_performance_cycles.findMany({ 
      where: {cycle_id},
      select: {cycle_id:true,cycle_code:true,cycle_name:true,cycle_type:true,start_date:true,end_date:true,review_frequency:true,self_review_required:true,manager_review_required:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,            hrm_performance_reviews:{
              select:{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'hrm_performance_cycles');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'hrm_performance_cycles');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createhrm_performance_cyclesDto: Prisma.hrm_performance_cyclesCreateInput,token:string,detokenize:string,detokenizeData?: any) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            review_frequency :  v.optional(v.string()), 
            self_review_required :v.boolean()  , 
            manager_review_required :v.boolean()  , 
      ...
      let validate : any = v.safeParse(dataSchema,createhrm_performance_cyclesDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createhrm_performance_cyclesDto, 'hrm_performance_cycles', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.hrm_performance_cycles.create({
          data: encryptedData,
          select:{cycle_id:true,cycle_code:true,cycle_name:true,cycle_type:true,start_date:true,end_date:true,review_frequency:true,self_review_required:true,manager_review_required:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,hrm_performance_reviews:true,}          
        })
      );
   ...
   */
  async createMaster(
    createhrm_performance_cyclesDto: Prisma.hrm_performance_cyclesCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_cycles creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_cycles creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'hrm_performance_cycles',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'cycle_id',
        ...
        return {
          success: true,
          message: 'hrm_performance_cycles creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(cycle_id:number, updatehrm_performance_cyclesDto: Prisma.hrm_performance_cyclesUpdateInput,token:string, detokenize:string,detokenizeData?: any) {   
    try{

      const dataSchema:any =  v.object({
          cycle_code :  v.optional(v.string()), 
          cycle_name :  v.optional(v.string()), 
          cycle_type :  v.optional(v.string()), 
          ...
          review_frequency :  v.optional(v.string()), 
          self_review_required :  v.optional(v.boolean()), 
          manager_review_required :  v.optional(v.boolean()), 
          additional_details :  v.optional(v.any()), 
          doc_url :  v.optional(v.array(v.string())), 
      ...
      let validate : any = v.safeParse(dataSchema,updatehrm_performance_cyclesDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatehrm_performance_cyclesDto,'hrm_performance_cycles','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatehrm_performance_cyclesDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.hrm_performance_cycles.updateMany({
        where: {cycle_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'hrm_performance_cycles', {
          primaryKey: {cycle_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
cycle_id:number,
    updatehrm_performance_cyclesDto: Prisma.hrm_performance_cyclesUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =cycle_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_performance_cycles update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_cycles update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_performance_cycles update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(cycle_id:number,token : string, detokenize: string,detokenizeData?: any) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.hrm_performance_cycles.deleteMany({
      where: {cycle_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'hrm_performance_cycles');
    let decryptedRes: any = [];
...
cycle_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =cycle_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'hrm_performance_cycles deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'hrm_performance_cycles deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'hrm_performance_cycles deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string, detokenize: string, detokenizeData?: any) {
    try{
      ...
      this.prismaService.hrm_performance_cycles.findFirst({ 
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      const locked = await this.ufservice.acquireLock({
        tableName: 'hrm_performance_cycles',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'hrm_performance_cycles'));
      }
      return decryptedRes;
    } catch (error: any) {
