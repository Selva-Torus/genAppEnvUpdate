...
,vgphsm_id?:number) {
  ...
  async create(createvgph_source_mainDto: Prisma.vgph_source_mainCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_mainDto, 'vgph_source_main', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(uuid: string, updatevgph_source_mainDto: Prisma.vgph_source_mainUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_mainDto,'vgph_source_main','update'), token);
