...
export class vgph_source_mainController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_source_mainService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphsm_id') vgphsm_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      ...
      detokenizeData["total_count"] = req.headers.total_count || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_mainService.findOne(+vgphsm_id,trs_tenant_id,trs_app_code,trs_product_code,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphsm_id') vgphsm_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsm_id"] = req.headers.vgphsm_id || "";
      detokenizeData["direction"] = req.headers.direction || "";
    ...
    const result = await this.vgph_source_mainService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphsm_id);
    return plainToInstance(vgph_source_mainEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_source_main'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_source_mainDto: Createvgph_source_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsm_id"] = req.headers.vgphsm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_mainService.createMaster(createvgph_source_mainDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_source_mainService.create(createvgph_source_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_mainEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_source_main', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
...
@Param('uuid') uuid: string,
    @Body() updatevgph_source_mainDto: Updatevgph_source_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphsm_id"] = req.headers.vgphsm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_source_mainService.updateMaster(uuid,updatevgph_source_mainDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_mainService.update(uuid,updatevgph_source_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_mainEntity, result);
  }
...
@Param('vgphsm_id') vgphsm_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_source_mainService.deleteMaster(+vgphsm_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_source_mainService.remove(+vgphsm_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_mainEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_mainService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_mainEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_source_mainService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_source_mainEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_source_mainService.getLockById(key, value, token);
    return plainToInstance(vgph_source_mainEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_source_mainService.releaseLockById(key, value, token);
    return result;
  }
}