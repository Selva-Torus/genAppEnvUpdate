...
//import { CreateUsersDto } from '../prisma/dto/create-users.dto';
//import { UpdateUsersDto } from '../prisma/dto/update-users.dto';
...
export class usersController {
  ...
  @ApiOperation({
    ...
    description: 'Retrive the datatype of the users table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.usersService.findSchema(token);
  }
  ...
  @ApiOperation({
    ...
    description: 'Filter all the records from the users table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.usersService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read only one records from the users table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('user_id') user_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["user_id"] = req.headers.user_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.usersService.findOne(+user_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the users table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.usersService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(usersEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('users'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiOperation({
    ...
    description: 'Create the record for the users table',
  })
  ...
  async create(
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.usersService.createMaster(createusersDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.usersService.create(createusersDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('users', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'user_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiOperation({
    ...
    description: 'Update the record for the users table',
  })
...
@Param('user_id') user_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.usersService.updateMaster(+user_id,updateusersDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.usersService.update(+user_id,updateusersDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the users table',
  })
...
@Param('user_id') user_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.usersService.deleteMaster(+user_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.usersService.remove(+user_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the users table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.usersService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the users table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.usersService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(usersEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.usersService.getLockById(key, value, token);
    return plainToInstance(usersEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.usersService.releaseLockById(key, value, token);
    return result;
  }
}