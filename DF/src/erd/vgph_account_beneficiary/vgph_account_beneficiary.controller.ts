...
export class vgph_account_beneficiaryController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_account_beneficiaryService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphab_id') vgphab_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphab_id"] = req.headers.vgphab_id || "";
      detokenizeData["account"] = req.headers.account || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_account_beneficiaryService.findOne(+vgphab_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_account_beneficiaryService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_account_beneficiary'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_account_beneficiaryDto: Createvgph_account_beneficiaryDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphab_id"] = req.headers.vgphab_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_account_beneficiaryService.createMaster(createvgph_account_beneficiaryDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_account_beneficiaryService.create(createvgph_account_beneficiaryDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_account_beneficiary', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphab_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphab_id') vgphab_id:number,
    @Body() updatevgph_account_beneficiaryDto: Updatevgph_account_beneficiaryDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphab_id"] = req.headers.vgphab_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_account_beneficiaryService.updateMaster(+vgphab_id,updatevgph_account_beneficiaryDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_account_beneficiaryService.update(+vgphab_id,updatevgph_account_beneficiaryDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }
...
@Param('vgphab_id') vgphab_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_account_beneficiaryService.deleteMaster(+vgphab_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_account_beneficiaryService.remove(+vgphab_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_account_beneficiaryService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_account_beneficiaryService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_account_beneficiaryService.getLockById(key, value, token);
    return plainToInstance(vgph_account_beneficiaryEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_account_beneficiaryService.releaseLockById(key, value, token);
    return result;
  }
}