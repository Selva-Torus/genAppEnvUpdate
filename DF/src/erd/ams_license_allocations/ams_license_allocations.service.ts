...
@Injectable()
export class ams_license_allocationsService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { alloc_id }: {alloc_id : bigint} = queryValue;
      const { device_asset_id }: {device_asset_id : bigint} = queryValue;
      const { allocation_type }: {allocation_type :  Date} = queryValue;
      const { allocation_status }: {allocation_status :  Date} = queryValue;
      const { device_details }: {device_details : any } = queryValue;
      const { allocation_metadata }: {allocation_metadata : any } = queryValue;
      const { allocated_at }: {allocated_at :  Date} = queryValue;
      ...
      if(alloc_id){ 
        query.alloc_id = { [queryCondition['alloc_id']]: alloc_id };
      }
      if(device_asset_id){ 
        query.device_asset_id = { [queryCondition['device_asset_id']]: device_asset_id };
      }
      if(allocation_type){ 
        query.allocation_type = { [queryCondition['allocation_type']]: allocation_type };
      }
      if(allocation_status){ 
        query.allocation_status = { [queryCondition['allocation_status']]: allocation_status };
      }
      if(device_details){ 
        query.device_details = { [queryCondition['device_details']]: device_details };
      }
      if(allocation_metadata){ 
        query.allocation_metadata = { [queryCondition['allocation_metadata']]: allocation_metadata };
      }
      if(allocated_at){ 
        query.allocated_at = { [queryCondition['allocated_at']]: allocated_at };
      }
      if(revoked_at){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.ams_license_allocations.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'ams_license_allocations');
      ...
      this.prismaService.ams_license_allocations.count({
        where: query,
      }));
  ...
  async create(createams_license_allocationsDto: Prisma.ams_license_allocationsCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            alloc_id :v.number() , 
            device_asset_id :  v.optional(v.number()), 
            allocation_type :  v.optional((v.any())), 
            allocation_status :  v.optional((v.any())), 
            device_details :  v.optional(v.any() ), 
            allocation_metadata :  v.optional(v.any() ), 
            allocated_at :(v.pipe(v.string(),v.isoDate()))  , 
        ...
        let validate : any = v.safeParse(dataSchema,createams_license_allocationsDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createams_license_allocationsDto, 'ams_license_allocations', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.ams_license_allocations.create({
          data: encryptedData,
          select:{alloc_id:true,device_asset_id:true,allocation_type:true,allocation_status:true,device_details:true,allocation_metadata:true,allocated_at:true,revoked_at:true,additional_details:true,doc_url:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,license_id :true,}          
        })
      );
   ...
   */
  async createMaster(
    createams_license_allocationsDto: Prisma.ams_license_allocationsCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_license_allocations creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_license_allocations creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'ams_license_allocations',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'alloc_id',
        ...
        return {
          success: true,
          message: 'ams_license_allocations creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(alloc_id:number, updateams_license_allocationsDto: Prisma.ams_license_allocationsUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateams_license_allocationsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateams_license_allocationsDto,'ams_license_allocations','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.ams_license_allocations.updateMany({
      where: {alloc_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
alloc_id:number,
    updateams_license_allocationsDto: Prisma.ams_license_allocationsUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =alloc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_license_allocations update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_license_allocations update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_license_allocations update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(alloc_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.ams_license_allocations.deleteMany({
      where: {alloc_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'ams_license_allocations');
    let decryptedRes: any = [];
...
alloc_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =alloc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'ams_license_allocations deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'ams_license_allocations deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'ams_license_allocations deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.ams_license_allocations.findFirst({ 
