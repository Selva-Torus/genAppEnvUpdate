...
import { Createstatus_lookupDto } from './dto/Createstatus_lookup.dto';
import { Updatestatus_lookupDto } from './dto/Updatestatus_lookup.dto';
...
import { status_lookupEntity } from './entity/status_lookup.entity';
...
@Injectable()
export class status_lookupService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { status_id }: {status_id : bigint} = queryValue;
      const { status_name }: {status_name : string} = queryValue;
      ...
      if(status_id){ 
        query.status_id = { [queryCondition['status_id']]: status_id };
      }
      if(status_name){ 
        query.status_name = { [queryCondition['status_name']]: status_name };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.status_lookup.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'status_lookup');
      ...
      this.prismaService.status_lookup.count({
        where: query,
      }));
  ...
  async findOne(status_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
    ...
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'status_lookup');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'status_lookup');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createstatus_lookupDto: Createstatus_lookupDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createstatus_lookupDto as any).trs_tenant_id = authContext.tenant;
      ...
      let validate : any = v.safeParse(dataSchema,createstatus_lookupDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createstatus_lookupDto, 'status_lookup', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.status_lookup.create({
          data: encryptedData,
   ...
   */
  async createMaster(
    createstatus_lookupDto: Createstatus_lookupDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'status_lookup:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for status_lookup');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'status_lookup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'status_lookup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'status_lookup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'status_id',
        ...
        return {
          success: true,
          message: 'status_lookup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(status_id:number, updatestatus_lookupDto: Updatestatus_lookupDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.status_lookup.findFirst({
          where: {status_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updatestatus_lookupDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatestatus_lookupDto,'status_lookup','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatestatus_lookupDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.status_lookup.updateMany({
        where: {status_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'status_lookup', {
...
/**
  ...
  async updateMaster(
status_id:number,
    updatestatus_lookupDto: Updatestatus_lookupDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =status_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.status_lookup.findUnique({
          where: { status_id: updateMaster_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'status_lookup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'status_lookup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'status_lookup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(status_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.status_lookup.deleteMany({
      where: {status_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'status_lookup');
    let decryptedRes: any = [];
...
status_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =status_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.status_lookup.findUnique({
          where: { status_id: deleteMaster_id },
          select: { trs_tenant_id: true },
        });
        if (targetRecord && targetRecord.trs_tenant_id !== authContext.tenant) {
          throw new ForbiddenException('Record belongs to a different tenant');
        ...
        if (success) {
          return {
            success: true,
            message: 'status_lookup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'status_lookup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'status_lookup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real status_lookup columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('status_lookup'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'status_lookup'));
      }
      return decryptedRes;
    } catch (error: any) {
