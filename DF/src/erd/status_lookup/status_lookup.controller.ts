...
import { status_lookupService } from './status_lookup.service';
...
//import { CreateStatusLookupDto } from '../prisma/dto/create-statusLookup.dto';
//import { UpdateStatusLookupDto } from '../prisma/dto/update-statusLookup.dto';
...
@Controller('status_lookup')
@ApiTags('ERD API')
export class status_lookupController {
  constructor(
    private readonly status_lookupService: status_lookupService,
  ...
  @ApiOkResponse({ type: status_lookupEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the status_lookup table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.status_lookupService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: status_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the status_lookup table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.status_lookupService.findAllmethod(query, +limit,selectColumns,token,req.authContext);
  }

  @Get(':status_id')
  @ApiBearerAuth('JWT-auth')
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiParam({name: 'status_id',type:Number})
  @ApiOkResponse({ type: status_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the status_lookup table',
  })
  
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('status_id') status_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["status_id"] = req.headers.status_id || "";
      detokenizeData["status_name"] = req.headers.status_name || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.status_lookupService.findOne(+status_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the status_lookup table',
  })
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.status_lookupService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(status_lookupEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('status_lookup'))
  @ApiBearerAuth('JWT-auth')
  ...
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Createstatus_lookupDto })
  @ApiCreatedResponse({ type: status_lookupEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the status_lookup table',
  })
  ...
  async create(
    ...
    @Body() createstatus_lookupDto: Createstatus_lookupDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["status_id"] = req.headers.status_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.status_lookupService.createMaster(createstatus_lookupDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.status_lookupService.create(createstatus_lookupDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('status_lookup', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'status_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
  ...
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiHeader({ name: 'xDetokenize', required: false })
  @ApiBody({ type: Updatestatus_lookupDto })
  @ApiOkResponse({ type: status_lookupEntity, isArray: true })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the status_lookup table',
  })
...
@Param('status_id') status_id:number,
    @Body() updatestatus_lookupDto: Updatestatus_lookupDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["status_id"] = req.headers.status_id || "";
    ...
    const result = await this.status_lookupService.update(+status_id,updatestatus_lookupDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the status_lookup table',
  })
...
@Param('status_id') status_id:number,
    ...
    const result = await this.status_lookupService.remove(+status_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the status_lookup table',
  })
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.status_lookupService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the status_lookup table',
  })
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.status_lookupService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.status_lookupService.getLockById(key, value, token);
    return plainToInstance(status_lookupEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.status_lookupService.releaseLockById(key, value, token);
    return result;
  }
}