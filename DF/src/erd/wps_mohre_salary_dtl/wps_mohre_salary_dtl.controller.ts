...
export class wps_mohre_salary_dtlController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.wps_mohre_salary_dtlService.findAllmethod(query, +limit,selectColumns,token);
  }
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('wps_mohre_salary_dtl'))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createwps_mohre_salary_dtlDto: Prisma.wps_mohre_salary_dtlCreateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.wps_mohre_salary_dtlService.createMaster(createwps_mohre_salary_dtlDto, makerInfo, token);
      return result;
    }

    const result = this.wps_mohre_salary_dtlService.create(createwps_mohre_salary_dtlDto,token);
    return plainToInstance(wps_mohre_salary_dtlEntity, result);
  }
...
@Param('wpsmsd_id') wpsmsd_id:number,
    @Body() updatewps_mohre_salary_dtlDto: Prisma.wps_mohre_salary_dtlUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.wps_mohre_salary_dtlService.updateMaster(+wpsmsd_id,updatewps_mohre_salary_dtlDto,makerInfo,token);
      return result;
    }
...
@Param('wpsmsd_id') wpsmsd_id:number,
    ...
    if (mcRole && mcUsername) {
      const makerInfo = { role: mcRole, username: mcUsername, remarks: mcRemarks,approvalStatus: mcApprovalStatus };
      const result = await this.wps_mohre_salary_dtlService.deleteMaster(+wpsmsd_id,makerInfo,token);
      return result;
    }
