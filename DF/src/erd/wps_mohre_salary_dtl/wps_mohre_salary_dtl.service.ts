...
export class wps_mohre_salary_dtlService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  ...
  async findOne(wpsmsd_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createwps_mohre_salary_dtlDto: Createwps_mohre_salary_dtlDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createwps_mohre_salary_dtlDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createwps_mohre_salary_dtlDto, 'wps_mohre_salary_dtl', 'create'), token);
   ...
   */
  async createMaster(
    createwps_mohre_salary_dtlDto: Createwps_mohre_salary_dtlDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'wps_mohre_salary_dtl:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for wps_mohre_salary_dtl');
        }
  ...
  async update(wpsmsd_id:number, updatewps_mohre_salary_dtlDto: Updatewps_mohre_salary_dtlDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.wps_mohre_salary_dtl.findFirst({
          where: {wpsmsd_id, trs_tenant_id: authContext.tenant },
          select: {wpsmsd_id:true, },
        }),
      );
      if (!owned) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatewps_mohre_salary_dtlDto,'wps_mohre_salary_dtl','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatewps_mohre_salary_dtlDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'wps_mohre_salary_dtl', {
          primaryKey: {wpsmsd_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
wpsmsd_id:number,
    updatewps_mohre_salary_dtlDto: Updatewps_mohre_salary_dtlDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.wps_mohre_salary_dtl.findFirst({
          where: {wpsmsd_id },
  ...
  async remove(wpsmsd_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
...
wpsmsd_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real wps_mohre_salary_dtl columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'wps_mohre_salary_dtl'));
      }
      return decryptedRes;
    } catch (error: any) {
