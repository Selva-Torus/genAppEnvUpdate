...
export class accountsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  ...
  async create(createaccountsDto: CreateaccountsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async update(account_id:number, updateaccountsDto: UpdateaccountsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
...
/**
  ...
  async updateMaster(
account_id:number,
    updateaccountsDto: UpdateaccountsDto,
