...
,vgphsi_id?:number) {
  ...
  async create(createvgph_siDto: Prisma.vgph_siCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_siDto, 'vgph_si', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphsi_id:number, updatevgph_siDto: Prisma.vgph_siUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_siDto,'vgph_si','update'), token);
