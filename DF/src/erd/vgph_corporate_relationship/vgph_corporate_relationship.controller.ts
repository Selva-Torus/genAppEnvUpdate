...
export class vgph_corporate_relationshipController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_corporate_relationshipService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphcr_id') vgphcr_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcr_id"] = req.headers.vgphcr_id || "";
      detokenizeData["parent_company"] = req.headers.parent_company || "";
      detokenizeData["company"] = req.headers.company || "";
      detokenizeData["branch"] = req.headers.branch || "";
      detokenizeData["parent_company_process_flag"] = req.headers.parent_company_process_flag || "";
      detokenizeData["company_process_flag"] = req.headers.company_process_flag || "";
      detokenizeData["branch_process_flag"] = req.headers.branch_process_flag || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_corporate_relationshipService.findOne(+vgphcr_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphcr_id') vgphcr_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcr_id"] = req.headers.vgphcr_id || "";
      detokenizeData["parent_company"] = req.headers.parent_company || "";
    ...
    const result = await this.vgph_corporate_relationshipService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphcr_id);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_corporate_relationship'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_corporate_relationshipDto: Createvgph_corporate_relationshipDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcr_id"] = req.headers.vgphcr_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_corporate_relationshipService.createMaster(createvgph_corporate_relationshipDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_corporate_relationshipService.create(createvgph_corporate_relationshipDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_corporate_relationship', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphcr_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphcr_id') vgphcr_id:number,
    @Body() updatevgph_corporate_relationshipDto: Updatevgph_corporate_relationshipDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcr_id"] = req.headers.vgphcr_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_corporate_relationshipService.updateMaster(+vgphcr_id,updatevgph_corporate_relationshipDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_corporate_relationshipService.update(+vgphcr_id,updatevgph_corporate_relationshipDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }
...
@Param('vgphcr_id') vgphcr_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_corporate_relationshipService.deleteMaster(+vgphcr_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_corporate_relationshipService.remove(+vgphcr_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_corporate_relationshipService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_corporate_relationshipService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_corporate_relationshipService.getLockById(key, value, token);
    return plainToInstance(vgph_corporate_relationshipEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_corporate_relationshipService.releaseLockById(key, value, token);
    return result;
  }
}