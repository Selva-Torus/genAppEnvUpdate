 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { common_id }: {common_id : number} = queryValue;
      const { from }: {from : string} = queryValue;
      const { to }: {to : string} = queryValue;
      const { username }: {username : string} = queryValue;
      const { subject }: {subject : string} = queryValue;
      const { body }: {body : string} = queryValue;
      const { to_email }: {to_email : Date} = queryValue;
      const { cc_email }: {cc_email : Date} = queryValue;
      const { bcc_email }: {bcc_email : Date} = queryValue;
      ...
      if(common_id){ 
        query.common_id = { [queryCondition['common_id']]: common_id };
      }
      if(from){ 
        query.from = { [queryCondition['from']]: from };
      }
      if(to){ 
        query.to = { [queryCondition['to']]: to };
      }
      if(username){ 
        query.username = { [queryCondition['username']]: username };
      }
      if(subject){ 
        query.subject = { [queryCondition['subject']]: subject };
      }
      if(body){ 
        query.body = { [queryCondition['body']]: body };
      }
      if(to_email){ 
        query.to_email = { [queryCondition['to_email']]: to_email };
      }
      if(cc_email){ 
        query.cc_email = { [queryCondition['cc_email']]: cc_email };
      }
      if(bcc_email){ 
        query.bcc_email = { [queryCondition['bcc_email']]: bcc_email };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.communication.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'communication');
      ...
      this.prismaService.communication.count({
        where: query,
      }));
  ...
  async findOne(common_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createcommunicationDto: CreatecommunicationDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createcommunicationDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            to_email :  v.optional(v.array(v.string())), 
            cc_email :  v.optional(v.array(v.string())), 
            bcc_email :  v.optional(v.array(v.string())), 
      ...
      let validate : any = v.safeParse(dataSchema,createcommunicationDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createcommunicationDto, 'communication', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.communication.create({
          data: encryptedData,
          select:{common_id:true,from:true,to:true,username:true,subject:true,body:true,to_email:true,cc_email:true,bcc_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
  ...
  async createMaster(
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'communication:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for communication');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'communication creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'communication creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        return {
          success: true,
          message: 'communication creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(common_id:number, updatecommunicationDto: UpdatecommunicationDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.communication.findFirst({
          where: {common_id, trs_tenant_id: authContext.tenant },
          select: {common_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatecommunicationDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatecommunicationDto,'communication','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatecommunicationDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.communication.updateMany({
        where: {common_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'communication', {
...
common_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =common_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.communication.findFirst({
          where: {common_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'communication update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'communication update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'communication update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(common_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.communication.deleteMany({
      where: {common_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'communication');
    let decryptedRes: any = [];
...
common_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =common_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'communication deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'communication deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'communication deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real communication columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('communication'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'communication'));
      }
      return decryptedRes;
    } catch (error: any) {
