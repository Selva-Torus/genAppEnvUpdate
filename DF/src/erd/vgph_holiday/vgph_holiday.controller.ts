...
export class vgph_holidayController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_holidayService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgph_hid') vgph_hid:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_hid"] = req.headers.vgph_hid || "";
      detokenizeData["code"] = req.headers.code || "";
      detokenizeData["holiday_date"] = req.headers.holiday_date || "";
      detokenizeData["description"] = req.headers.description || "";
      detokenizeData["holiday_flag"] = req.headers.holiday_flag || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_holidayService.findOne(+vgph_hid,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_holidayService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_holidayEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_holiday'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_holidayDto: Createvgph_holidayDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_hid"] = req.headers.vgph_hid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_holidayService.createMaster(createvgph_holidayDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_holidayService.create(createvgph_holidayDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_holidayEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_holiday', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgph_hid',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgph_hid') vgph_hid:number,
    @Body() updatevgph_holidayDto: Updatevgph_holidayDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgph_hid"] = req.headers.vgph_hid || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_holidayService.updateMaster(+vgph_hid,updatevgph_holidayDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_holidayService.update(+vgph_hid,updatevgph_holidayDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_holidayEntity, result);
  }
...
@Param('vgph_hid') vgph_hid:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_holidayService.deleteMaster(+vgph_hid,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_holidayService.remove(+vgph_hid,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_holidayEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_holidayService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_holidayEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_holidayService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_holidayEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_holidayService.getLockById(key, value, token);
    return plainToInstance(vgph_holidayEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_holidayService.releaseLockById(key, value, token);
    return result;
  }
}