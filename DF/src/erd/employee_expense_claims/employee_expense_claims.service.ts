...
import { Createemployee_expense_claimsDto } from './dto/Createemployee_expense_claims.dto';
import { Updateemployee_expense_claimsDto } from './dto/Updateemployee_expense_claims.dto';
...
import { employee_expense_claimsEntity } from './entity/employee_expense_claims.entity';
...
@Injectable()
export class employee_expense_claimsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { detail_id }: {detail_id : number} = queryValue;
      const { employee_detail }: {employee_detail : any } = queryValue;
      ...
      if(detail_id){ 
        query.detail_id = { [queryCondition['detail_id']]: detail_id };
      }
      if(employee_detail){ 
        query.employee_detail = { [queryCondition['employee_detail']]: employee_detail };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.employee_expense_claims.findMany({
          select:columns,
           where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'employee_expense_claims');
      ...
      this.prismaService.employee_expense_claims.count({
        where: query,
      }));
      ...
      await this.commonService.errorLog(
        ...
        "TG020",
        error,
        "CK:CT001:FNGK:AF:FNK:API-ERD:CATK:TAM:AFGK:TA:AFK:Dynamic_Json_ERD:AFVK:v1",
        token
      );
  ...
  async findOne(detail_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createemployee_expense_claimsDto: Createemployee_expense_claimsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createemployee_expense_claimsDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            employee_detail :  v.optional(v.any() ), 
      ...
      let validate : any = v.safeParse(dataSchema,createemployee_expense_claimsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createemployee_expense_claimsDto, 'employee_expense_claims', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.employee_expense_claims.create({
          data: encryptedData,
          select:{detail_id:true,employee_detail:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createemployee_expense_claimsDto: Createemployee_expense_claimsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'employee_expense_claims:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for employee_expense_claims');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_expense_claims creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_expense_claims creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'employee_expense_claims',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'detail_id',
        ...
        return {
          success: true,
          message: 'employee_expense_claims creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(detail_id:number, updateemployee_expense_claimsDto: Updateemployee_expense_claimsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.employee_expense_claims.findFirst({
          where: {detail_id, trs_tenant_id: authContext.tenant },
      ...
      let validate : any = v.safeParse(dataSchema,updateemployee_expense_claimsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateemployee_expense_claimsDto,'employee_expense_claims','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updateemployee_expense_claimsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.employee_expense_claims.updateMany({
        where: {detail_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'employee_expense_claims', {
...
/**
  ...
  async updateMaster(
detail_id:number,
    updateemployee_expense_claimsDto: Updateemployee_expense_claimsDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =detail_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.employee_expense_claims.findFirst({
          where: {detail_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'employee_expense_claims update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_expense_claims update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'employee_expense_claims update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(detail_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.employee_expense_claims.deleteMany({
      where: {detail_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'employee_expense_claims');
    let decryptedRes: any = [];
...
detail_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =detail_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'employee_expense_claims deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'employee_expense_claims deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'employee_expense_claims deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real employee_expense_claims columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('employee_expense_claims'));
      if (typeof key !== 'string' || !allowedKeys.includes(key)) {
        throw new BadRequestException(`Invalid lock key: ${key}`);
      }

      const locked = await this.ufservice.acquireLock({
        tableName: 'employee_expense_claims',
        key: key,
        value: value,
        userId: loginId,
      } as LockRecordDto);
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'employee_expense_claims'));
      }
      return decryptedRes;
    } catch (error: any) {
