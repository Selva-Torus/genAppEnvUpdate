...
@Injectable()
export class vgph_cob_error_logService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { vgphcel_id }: {vgphcel_id : bigint} = queryValue;
      const { direction }: {direction : string} = queryValue;
      ...
      const { vgphe_uuid }: {vgphe_uuid : string} = queryValue;
      const { vgphc_uuid }: {vgphc_uuid : string} = queryValue;
      ...
      if(vgphcel_id){ 
        query.vgphcel_id = { [queryCondition['vgphcel_id']]: vgphcel_id };
      }
      if(direction){ 
      ...
      if(vgphe_uuid){ 
        query.vgphe_uuid = { [queryCondition['vgphe_uuid']]: vgphe_uuid };
      }
      if(vgphc_uuid){ 
        query.vgphc_uuid = { [queryCondition['vgphc_uuid']]: vgphc_uuid };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_cob_error_log.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_cob_error_log');
      ...
      this.prismaService.vgph_cob_error_log.count({
        where: query,
      }));
  ...
  async findOne(vgphcel_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
...
,vgphcel_id?:number) {
    ...
    try{
      ...
      if (vgphcel_id) {
        whereClause.vgphcel_id = vgphcel_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_cob_error_log.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_cob_error_logDto: Createvgph_cob_error_logDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createvgph_cob_error_logDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            direction :v.pipe(v.string(),v.maxLength(32 )) , 
            process_type :v.pipe(v.string(),v.maxLength(16 )) , 
            ...
            vgphe_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            vgphc_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createvgph_cob_error_logDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_cob_error_logDto, 'vgph_cob_error_log', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_cob_error_log.create({
          data: encryptedData,
          select:{vgphcel_id:true,direction:true,process_type:true,error_code:true,error_desc:true,error_data:true,vgphe_uuid:true,vgphc_uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_cob_error_logDto: Createvgph_cob_error_logDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'vgph_cob_error_log:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for vgph_cob_error_log');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_cob_error_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_cob_error_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'vgph_cob_error_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcel_id',
        ...
        return {
          success: true,
          message: 'vgph_cob_error_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcel_id:number, updatevgph_cob_error_logDto: Updatevgph_cob_error_logDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_cob_error_log.findFirst({
          where: {vgphcel_id, trs_tenant_id: authContext.tenant },
          select: {vgphcel_id:true, },
        }),
      );
      if (!owned) {
      ...
      const dataSchema:any =  v.object({
          direction :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          process_type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          error_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          error_desc :  v.optional(v.pipe(v.string(),v.maxLength(1024 ))), 
          error_data :  v.optional(v.any()), 
          vgphe_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_cob_error_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_cob_error_logDto,'vgph_cob_error_log','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatevgph_cob_error_logDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.vgph_cob_error_log.updateMany({
        where: {vgphcel_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'vgph_cob_error_log', {
          primaryKey: {vgphcel_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
vgphcel_id:number,
    updatevgph_cob_error_logDto: Updatevgph_cob_error_logDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcel_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.vgph_cob_error_log.findFirst({
          where: {vgphcel_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_cob_error_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_cob_error_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_cob_error_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcel_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_cob_error_log.deleteMany({
      where: {vgphcel_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_cob_error_log');
    let decryptedRes: any = [];
...
vgphcel_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcel_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_cob_error_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_cob_error_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_cob_error_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real vgph_cob_error_log columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'vgph_cob_error_log'));
      }
      return decryptedRes;
    } catch (error: any) {
