...
export class vgph_cob_error_logController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_cob_error_logService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphcel_id') vgphcel_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcel_id"] = req.headers.vgphcel_id || "";
      ...
      detokenizeData["vgphe_uuid"] = req.headers.vgphe_uuid || "";
      detokenizeData["vgphc_uuid"] = req.headers.vgphc_uuid || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_cob_error_logService.findOne(+vgphcel_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphcel_id') vgphcel_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcel_id"] = req.headers.vgphcel_id || "";
      detokenizeData["direction"] = req.headers.direction || "";
    ...
    const result = await this.vgph_cob_error_logService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphcel_id);
    return plainToInstance(vgph_cob_error_logEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_cob_error_log'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_cob_error_logDto: Createvgph_cob_error_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcel_id"] = req.headers.vgphcel_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_cob_error_logService.createMaster(createvgph_cob_error_logDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_cob_error_logService.create(createvgph_cob_error_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_cob_error_log', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphcel_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphcel_id') vgphcel_id:number,
    @Body() updatevgph_cob_error_logDto: Updatevgph_cob_error_logDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcel_id"] = req.headers.vgphcel_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_cob_error_logService.updateMaster(+vgphcel_id,updatevgph_cob_error_logDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_cob_error_logService.update(+vgphcel_id,updatevgph_cob_error_logDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }
...
@Param('vgphcel_id') vgphcel_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_cob_error_logService.deleteMaster(+vgphcel_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_cob_error_logService.remove(+vgphcel_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_cob_error_logService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_cob_error_logService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_cob_error_logService.getLockById(key, value, token);
    return plainToInstance(vgph_cob_error_logEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_cob_error_logService.releaseLockById(key, value, token);
    return result;
  }
}