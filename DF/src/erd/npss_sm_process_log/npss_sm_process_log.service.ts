...
@Injectable()
export class npss_sm_process_logService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { npsssmpl_id }: {npsssmpl_id : bigint} = queryValue;
      ...
      const { result_code }: {result_code : string} = queryValue;
      const { result }: {result : string} = queryValue;
      const { result_message }: {result_message : string} = queryValue;
      const { result_x_request_id }: {result_x_request_id : string} = queryValue;
      ...
      const { reject_reason_code }: {reject_reason_code : string} = queryValue;
      const { reject_reason }: {reject_reason : string} = queryValue;
      ...
      if(npsssmpl_id){ 
        query.npsssmpl_id = { [queryCondition['npsssmpl_id']]: npsssmpl_id };
      }
      if(bankuserid){ 
      ...
      if(result_code){ 
        query.result_code = { [queryCondition['result_code']]: result_code };
      }
      if(result){ 
        query.result = { [queryCondition['result']]: result };
      }
      if(result_message){ 
        query.result_message = { [queryCondition['result_message']]: result_message };
      }
      if(result_x_request_id){ 
        query.result_x_request_id = { [queryCondition['result_x_request_id']]: result_x_request_id };
      }
      if(process_ref_no){ 
      ...
      if(reject_reason_code){ 
        query.reject_reason_code = { [queryCondition['reject_reason_code']]: reject_reason_code };
      }
      if(reject_reason){ 
        query.reject_reason = { [queryCondition['reject_reason']]: reject_reason };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_sm_process_log.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_sm_process_log');
      ...
      this.prismaService.npss_sm_process_log.count({
        where: query,
      }));
  ...
  async findOne(npsssmpl_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      ...
      this.prismaService.npss_sm_process_log.findMany({ 
      where: {npsssmpl_id, ...tenantFilter },
      select: {npsssmpl_id:true,bankuserid:true,process_name:true,processing_system:true,result_code:true,result:true,result_message:true,result_x_request_id:true,process_ref_no:true,request_data_json:true,response_data_json:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,reject_reason_code:true,reject_reason:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_sm_process_log');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'npss_sm_process_log');
      decryptedRes.push(decryptedData);
    }
  ...
  async create(createnpss_sm_process_logDto: Createnpss_sm_process_logDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_sm_process_logDto as any).trs_tenant_id = authContext.tenant;
      ...
      const dataSchema:any =  v.object({
            ...
            result_code :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            result :  v.optional(v.pipe(v.string(),v.maxLength(5 ))), 
            result_message :  v.optional(v.pipe(v.string(),v.maxLength(100 ))), 
            result_x_request_id :  v.optional(v.pipe(v.string(),v.maxLength(100 ))), 
            ...
            reject_reason_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            reject_reason :  v.optional(v.pipe(v.string(),v.maxLength(256 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_sm_process_logDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_sm_process_logDto, 'npss_sm_process_log', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_sm_process_log.create({
          data: encryptedData,
          select:{npsssmpl_id:true,bankuserid:true,process_name:true,processing_system:true,result_code:true,result:true,result_message:true,result_x_request_id:true,process_ref_no:true,request_data_json:true,response_data_json:true,channel_id:true,channel_refno:true,channel_userid:true,channel_product:true,channel_sub_product:true,channel_tran_code:true,reject_reason_code:true,reject_reason:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_sm_process_logDto: Createnpss_sm_process_logDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_sm_process_log:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_sm_process_log');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_sm_process_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_sm_process_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_sm_process_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'npsssmpl_id',
        ...
        return {
          success: true,
          message: 'npss_sm_process_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(npsssmpl_id:number, updatenpss_sm_process_logDto: Updatenpss_sm_process_logDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_sm_process_log.findFirst({
          where: {npsssmpl_id, trs_tenant_id: authContext.tenant },
          select: {npsssmpl_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_sm_process_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_sm_process_logDto,'npss_sm_process_log','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_sm_process_logDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_sm_process_log.updateMany({
        where: {npsssmpl_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_sm_process_log', {
          primaryKey: {npsssmpl_id},
          currentVersion,
          data: encryptedData,
        }),
      );
...
/**
  ...
  async updateMaster(
npsssmpl_id:number,
    updatenpss_sm_process_logDto: Updatenpss_sm_process_logDto,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const updateMaster_id =npsssmpl_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_sm_process_log.findFirst({
          where: {npsssmpl_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_sm_process_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_sm_process_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_sm_process_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(npsssmpl_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_sm_process_log.deleteMany({
      where: {npsssmpl_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_sm_process_log');
    let decryptedRes: any = [];
...
npsssmpl_id:number,
    ...
    try {
      ...
      const workflowRole = userInfo.role?.toUpperCase();
      const deleteMaster_id =npsssmpl_id;
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_sm_process_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_sm_process_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_sm_process_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_sm_process_log columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_sm_process_log'));
      }
      return decryptedRes;
    } catch (error: any) {
