...
export class claimsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.claims.findMany({
          select:columns,
           where: query,
      ...
      this.prismaService.claims.count({
        where: query,
      }));
  ...
  async findOne(claim_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createclaimsDto: CreateclaimsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createclaimsDto as any).trs_tenant_id = authContext.tenant;
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createclaimsDto, 'claims', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.claims.create({
          data: encryptedData,
          select:{claim_id:true,employee_id:true,employee_name:true,department_name:true,employee_grade:true,receipt_image:true,total_amount:true,from_date:true,to_date:true,email:true,claim_category:true,comments:true,expense_date:true,is_comment_enabled:true,expense_name:true,manager_comments:true,expense_description:true,expense_amount:true,category_total_amount:true,claim_expense_type:true,name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,claims_detail:true,}          
        })
      );
   ...
   */
  async createMaster(
    createclaimsDto: CreateclaimsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'claims:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for claims');
        }
  ...
  async update(claim_id:number, updateclaimsDto: UpdateclaimsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.claims.findFirst({
          where: {claim_id, trs_tenant_id: authContext.tenant },
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updateclaimsDto,'claims','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'claims', {
          primaryKey: {claim_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
claim_id:number,
    updateclaimsDto: UpdateclaimsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.claims.findFirst({
          where: {claim_id },
  ...
  async remove(claim_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.claims.deleteMany({
      where: {claim_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'claims');
    let decryptedRes: any = [];
...
claim_id:number,
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real claims columns (via Prisma DMMF) —
      ...
      const allowedKeys = Object.keys(getModelFieldTypes('claims'));
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'claims'));
      }
      return decryptedRes;
    } catch (error: any) {
