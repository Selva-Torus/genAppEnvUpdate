...
export class vgph_tran_dtl_mainController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_tran_dtl_mainService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphtdm_id') vgphtdm_id:number,@Param('trs_tenant_id') trs_tenant_id: string,@Param('trs_app_code') trs_app_code: string,@Param('trs_product_code') trs_product_code: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtdm_id"] = req.headers.vgphtdm_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_dtl_mainService.findOne(+vgphtdm_id,trs_tenant_id,trs_app_code,trs_product_code,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query('vgphtdm_id') vgphtdm_id?:string,@Query() query?: Record<string, any>) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtdm_id"] = req.headers.vgphtdm_id || "";
      detokenizeData["direction"] = req.headers.direction || "";
    ...
    const result = await this.vgph_tran_dtl_mainService.findAll(token,req.authContext,detokenize,detokenizeData,+vgphtdm_id);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_tran_dtl_main'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_tran_dtl_mainDto: Createvgph_tran_dtl_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtdm_id"] = req.headers.vgphtdm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_tran_dtl_mainService.createMaster(createvgph_tran_dtl_mainDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_tran_dtl_mainService.create(createvgph_tran_dtl_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_tran_dtl_main', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphtdm_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphtdm_id') vgphtdm_id:number,
    @Body() updatevgph_tran_dtl_mainDto: Updatevgph_tran_dtl_mainDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtdm_id"] = req.headers.vgphtdm_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_tran_dtl_mainService.updateMaster(+vgphtdm_id,updatevgph_tran_dtl_mainDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_tran_dtl_mainService.update(+vgphtdm_id,updatevgph_tran_dtl_mainDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }
...
@Param('vgphtdm_id') vgphtdm_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_tran_dtl_mainService.deleteMaster(+vgphtdm_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_tran_dtl_mainService.remove(+vgphtdm_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_dtl_mainService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_tran_dtl_mainService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_tran_dtl_mainService.getLockById(key, value, token);
    return plainToInstance(vgph_tran_dtl_mainEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_tran_dtl_mainService.releaseLockById(key, value, token);
    return result;
  }
}