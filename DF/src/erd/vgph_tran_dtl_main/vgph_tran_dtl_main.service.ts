...
,vgphtdm_id?:number) {
  ...
  async create(createvgph_tran_dtl_mainDto: Prisma.vgph_tran_dtl_mainCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_tran_dtl_mainDto, 'vgph_tran_dtl_main', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphtdm_id:number, updatevgph_tran_dtl_mainDto: Prisma.vgph_tran_dtl_mainUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_tran_dtl_mainDto,'vgph_tran_dtl_main','update'), token);
