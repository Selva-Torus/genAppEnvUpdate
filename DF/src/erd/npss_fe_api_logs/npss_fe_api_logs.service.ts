...
@Injectable()
export class npss_fe_api_logsService {
 ...
 async findAllmethod(queryDto: any, limit:number, selectColumns:any, token:any, authContext?: any,sortingcolumns:any={}) {
  if (!authContext?.tenant) {
    ...
    try {
      ...
      let query: any = {}; 
      const { nal_id }: {nal_id : bigint} = queryValue;
      const { api_name }: {api_name : string} = queryValue;
      const { x_req_id }: {x_req_id : string} = queryValue;
      const { ref_no }: {ref_no : string} = queryValue;
      const { process_date }: {process_date : string} = queryValue;
      ...
      const { cdtr_iban }: {cdtr_iban : string} = queryValue;
      ...
      if(nal_id){ 
        query.nal_id = { [queryCondition['nal_id']]: nal_id };
      }
      if(api_name){ 
        query.api_name = { [queryCondition['api_name']]: api_name };
      }
      if(x_req_id){ 
        query.x_req_id = { [queryCondition['x_req_id']]: x_req_id };
      }
      if(ref_no){ 
        query.ref_no = { [queryCondition['ref_no']]: ref_no };
      }
      if(process_date){ 
        query.process_date = { [queryCondition['process_date']]: process_date };
      }
      ...
      if(cdtr_iban){ 
        query.cdtr_iban = { [queryCondition['cdtr_iban']]: cdtr_iban };
      }
      if(response_code){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.npss_fe_api_logs.findMany({
          select:columns,
          where: query,
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'npss_fe_api_logs');
      ...
      this.prismaService.npss_fe_api_logs.count({
        where: query,
      }));
  ...
  async findOne(nal_id:number,token : string,detokenize?: string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
  ...
  async create(createnpss_fe_api_logsDto: Createnpss_fe_api_logsDto,token:string,detokenize:string,detokenizeData?: any,authContext?: any) {
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      (createnpss_fe_api_logsDto as any).trs_tenant_id = authContext.tenant;

      const dataSchema:any =  v.object({
            api_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            x_req_id :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            ref_no :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            process_date :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            ...
            channel_id :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            cdtr_iban :  v.optional(v.pipe(v.string(),v.maxLength(23 ))), 
            response_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            additional_info :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,createnpss_fe_api_logsDto);
      if (!validate.success) {
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createnpss_fe_api_logsDto, 'npss_fe_api_logs', 'create'), token);
      ...
      let res:any = await this.prismaService.withConnection(() =>
        this.prismaService.npss_fe_api_logs.create({
          data: encryptedData,
          select:{nal_id:true,api_name:true,x_req_id:true,ref_no:true,process_date:true,request_data_json:true,response_data_json:true,processing_system:true,channel_id:true,cdtr_iban:true,response_code:true,additional_info:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createnpss_fe_api_logsDto: Createnpss_fe_api_logsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        // confirm the match.
        if (!(await this.ufservice.hasCapability(token, 'npss_fe_api_logs:AUTHORIZE', authContext?.tenant))) {
          throw new ForbiddenException('Caller is not entitled to authorize changes for npss_fe_api_logs');
        }
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_fe_api_logs creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_fe_api_logs creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(workflowRole === 'SUBMITTER')
      {
        ...
        this.cdcPrismaService.$queryRaw<any[]>`
          SELECT tam.request_change(
            p_table_name     := 'npss_fe_api_logs',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'nal_id',
        ...
        return {
          success: true,
          message: 'npss_fe_api_logs creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(nal_id:number, updatenpss_fe_api_logsDto: Updatenpss_fe_api_logsDto,token:string, detokenize:string,detokenizeData?: any,authContext?: any) {   
    if (!authContext?.tenant) {
      throw new ForbiddenException('Tenant claim is missing from the token');
    }
    try{
      const owned = await this.prismaService.withConnection(() =>
        this.prismaService.npss_fe_api_logs.findFirst({
          where: {nal_id, trs_tenant_id: authContext.tenant },
          select: {nal_id:true, },
        }),
      );
      if (!owned) {
      ...
      let validate : any = v.safeParse(dataSchema,updatenpss_fe_api_logsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      let encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatenpss_fe_api_logsDto,'npss_fe_api_logs','update'), token);
      if (this.tokenizationRules?.rules?.fields?.length > 0) {
        let tokenizedData = { ...this.tokenizationRules, payload: encryptedData };
        tokenizedData =     await axios.post(
        `${process.env.TOKENIZATION_BASE_URL}/dynamic-tokenization`,
    ...
    const currentVersion = (updatenpss_fe_api_logsDto as any)?.trs_version;
    if (currentVersion === undefined || currentVersion === null || currentVersion === '') {
      await this.prismaService.withConnection(() =>
        this.prismaService.npss_fe_api_logs.updateMany({
        where: {nal_id},
        data: encryptedData
      }));
    }else{
      ...
      await this.prismaService.withConnection(() =>
        this.optimisticLockService.updateWithVersionCheck(this.prismaService, 'npss_fe_api_logs', {
          primaryKey: {nal_id},
          currentVersion,
          data: encryptedData,
        }),
...
/**
  ...
  async updateMaster(
nal_id:number,
    updatenpss_fe_api_logsDto: Updatenpss_fe_api_logsDto,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        const targetRecord = await this.prismaService.npss_fe_api_logs.findFirst({
          where: {nal_id },
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_fe_api_logs update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_fe_api_logs update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_fe_api_logs update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(nal_id:number,token : string, detokenize: string,detokenizeData?: any,authContext?: any) {
    try{
    // Fail closed when the JWT has no tenant claim — without a verified
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.npss_fe_api_logs.deleteMany({
      where: {nal_id ,...tenantFilter}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'npss_fe_api_logs');
    let decryptedRes: any = [];
...
nal_id:number,
    ...
    try {
      ...
      if (workflowRole === 'AUTHORIZER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'npss_fe_api_logs deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'npss_fe_api_logs deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'npss_fe_api_logs deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async getLockById(key: string, value: any, token: string) {
    try {
      ...
      // Allow-list `key` against real npss_fe_api_logs columns (via Prisma DMMF) —
      ...
      for (const record of res) {
        ...
        decryptedRes.push(await this.decryptData(plain, 'npss_fe_api_logs'));
      }
      return decryptedRes;
    } catch (error: any) {
