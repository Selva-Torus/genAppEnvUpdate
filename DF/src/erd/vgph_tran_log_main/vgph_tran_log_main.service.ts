...
,vgphtlm_id?:number) {
  ...
  async create(createvgph_tran_log_mainDto: Prisma.vgph_tran_log_mainCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_tran_log_mainDto, 'vgph_tran_log_main', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphtlm_id:number, updatevgph_tran_log_mainDto: Prisma.vgph_tran_log_mainUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_tran_log_mainDto,'vgph_tran_log_main','update'), token);
