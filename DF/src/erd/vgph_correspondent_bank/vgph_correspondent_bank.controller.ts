...
export class vgph_correspondent_bankController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_correspondent_bankService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphcb_id') vgphcb_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcb_id"] = req.headers.vgphcb_id || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_bankService.findOne(+vgphcb_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_correspondent_bankService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_bank'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_correspondent_bankDto: Createvgph_correspondent_bankDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcb_id"] = req.headers.vgphcb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_bankService.createMaster(createvgph_correspondent_bankDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_correspondent_bankService.create(createvgph_correspondent_bankDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_correspondent_bank', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphcb_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphcb_id') vgphcb_id:number,
    @Body() updatevgph_correspondent_bankDto: Updatevgph_correspondent_bankDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphcb_id"] = req.headers.vgphcb_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_correspondent_bankService.updateMaster(+vgphcb_id,updatevgph_correspondent_bankDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_bankService.update(+vgphcb_id,updatevgph_correspondent_bankDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }
...
@Param('vgphcb_id') vgphcb_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_correspondent_bankService.deleteMaster(+vgphcb_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_correspondent_bankService.remove(+vgphcb_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_bankService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_correspondent_bankService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_correspondent_bankService.getLockById(key, value, token);
    return plainToInstance(vgph_correspondent_bankEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_correspondent_bankService.releaseLockById(key, value, token);
    return result;
  }
}