...
export class vgph_trn_secure_tokenController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_trn_secure_tokenService.findAllmethod(query, +limit,selectColumns,token,req.authContext,sortingcolumns);
  }
  ...
  async findOne(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param('vgphtst_id') vgphtst_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtst_id"] = req.headers.vgphtst_id || "";
      detokenizeData["token_id"] = req.headers.token_id || "";
      detokenizeData["token_data"] = req.headers.token_data || "";
      detokenizeData["encryption_key_id"] = req.headers.encryption_key_id || "";
      detokenizeData["setup_code"] = req.headers.setup_code || "";
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_trn_secure_tokenService.findOne(+vgphtst_id,token,detokenize,detokenizeData,req.authContext);
  ...
  async findAll(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Req() req: any,@Query() query?: Record<string, any>) {
    ...
    const result = await this.vgph_trn_secure_tokenService.findAll(token,req.authContext,detokenize,detokenizeData,);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  } 

  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_trn_secure_token'), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_trn_secure_tokenDto: Createvgph_trn_secure_tokenDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtst_id"] = req.headers.vgphtst_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_trn_secure_tokenService.createMaster(createvgph_trn_secure_tokenDto, makerInfo, token, req.authContext);
      return result;
    }

    const result = this.vgph_trn_secure_tokenService.create(createvgph_trn_secure_tokenDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }
  ...
  @UsePipes(new PrismaModelValidationPipe('vgph_trn_secure_token', true), new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'vgphtst_id',type:Number})
  @ApiHeader({ name: 'xCdcaRole', required: false })
  @ApiHeader({ name: 'xCdcaUsername', required: false })
...
@Param('vgphtst_id') vgphtst_id:number,
    @Body() updatevgph_trn_secure_tokenDto: Updatevgph_trn_secure_tokenDto,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    let detokenizeData = {};
      detokenizeData["vgphtst_id"] = req.headers.vgphtst_id || "";
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_trn_secure_tokenService.updateMaster(+vgphtst_id,updatevgph_trn_secure_tokenDto,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_trn_secure_tokenService.update(+vgphtst_id,updatevgph_trn_secure_tokenDto,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }
...
@Param('vgphtst_id') vgphtst_id:number,
    ...
    if (mcRole && mcUsername) {
      ...
      const makerInfo = { role: mcRole, username: mcIdentity, remarks: mcRemarks,approvalStatus: mcApprovalStatus,detokenize: detokenize };
      const result = await this.vgph_trn_secure_tokenService.deleteMaster(+vgphtst_id,makerInfo,token,req.authContext);
      return result;
    }

    const result = await this.vgph_trn_secure_tokenService.remove(+vgphtst_id,token,detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }  
  ...
  async findFirst(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_trn_secure_tokenService.findFirst(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }
  ...
  async findLast(@Headers('xDetokenize') detokenize: string,@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    ...
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = await this.vgph_trn_secure_tokenService.findLast(token, detokenize,detokenizeData,req.authContext);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }
  ...
  async getLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    ...
    const result = await this.vgph_trn_secure_tokenService.getLockById(key, value, token);
    return plainToInstance(vgph_trn_secure_tokenEntity, result);
  }
  ...
  async releaseLockById(@Body('key') key: string, @Body('value') value: any, @Req() req: any) {
    const token: string = req.headers.authorization.split(' ')[1];
    const result = await this.vgph_trn_secure_tokenService.releaseLockById(key, value, token);
    return result;
  }
}