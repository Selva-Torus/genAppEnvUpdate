...
,vgphssi_id?:number) {
  ...
  async create(createvgph_source_siDto: Prisma.vgph_source_siCreateInput,token:string) {
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(createvgph_source_siDto, 'vgph_source_si', 'create'), token);
      const res = await this.prismaService.withConnection(() =>
  ...
  async update(vgphssi_id:number, updatevgph_source_siDto: Prisma.vgph_source_siUpdateInput,token:string) {   
    try{
      ...
      const encryptedData = await this.normalizeDatesToUTC(await this.encryptData(updatevgph_source_siDto,'vgph_source_si','update'), token);
